...
from shared.types.execution_types import StronglyTypedUserExecutionContext, ContextValidationError, IsolationViolationError, upgrade_legacy_context, downgrade_to_legacy_context
from shared.types.core_types import UserID, ThreadID, RunID, RequestID, WebSocketID, ensure_user_id, ensure_thread_id, ensure_run_id, ensure_request_id
...
class TestStronglyTypedUserExecutionContextCreation:
    ...
    def test_create_valid_context_with_all_fields(self):
        ...
        context = StronglyTypedUserExecutionContext(user_id=user_id, thread_id=thread_id, run_id=run_id, request_id=request_id, websocket_client_id=websocket_id, agent_context={'test_key': 'test_value'}, audit_metadata={'audit_key': 'audit_value'})
    ...
    def test_create_minimal_context_with_required_fields_only(self):
        ...
        run_id = RunID(f'run_{uuid.uuid4().hex[:12]}')
        request_id = RequestID(f'req_{uuid.uuid4().hex[:12]}')
        context = StronglyTypedUserExecutionContext(user_id=user_id, thread_id=thread_id, run_id=run_id, request_id=request_id)
        assert context.user_id == user_id
        assert context.thread_id == thread_id
        assert context.run_id == run_id
        assert context.request_id == request_id
...
class TestStronglyTypedUserExecutionContextValidation:
    ...
    def test_reject_empty_user_id(self):
        ...
        with pytest.raises(ContextValidationError, match='Invalid user_id'):
            StronglyTypedUserExecutionContext(user_id=UserID(''), thread_id=thread_id, run_id=run_id, request_id=request_id)
        with pytest.raises(ContextValidationError, match='Invalid user_id'):
            StronglyTypedUserExecutionContext(user_id=UserID('   '), thread_id=thread_id, run_id=run_id, request_id=request_id)
    ...
    def test_reject_placeholder_values(self):
        ...
        run_id = RunID(f'run_{uuid.uuid4().hex[:12]}')
        request_id = RequestID(f'req_{uuid.uuid4().hex[:12]}')
        forbidden_user_ids = ['test_user', 'mock_user', 'placeholder', 'default', 'example']
        for forbidden_id in forbidden_user_ids:
            with pytest.raises(ContextValidationError, match='Forbidden placeholder value'):
                StronglyTypedUserExecutionContext(user_id=UserID(forbidden_id), thread_id=thread_id, run_id=run_id, request_id=request_id)
        user_id = UserID(f'user_{uuid.uuid4().hex[:12]}')
        with pytest.raises(ContextValidationError, match='Forbidden placeholder value'):
            StronglyTypedUserExecutionContext(user_id=user_id, thread_id=ThreadID('test_thread'), run_id=run_id, request_id=request_id)
    ...
    def test_reject_invalid_operation_depth(self):
        ...
        with pytest.raises(ContextValidationError, match='Invalid operation_depth'):
            StronglyTypedUserExecutionContext(user_id=user_id, thread_id=thread_id, run_id=run_id, request_id=request_id, operation_depth=-1)
        with pytest.raises(ContextValidationError, match='Invalid operation_depth'):
            StronglyTypedUserExecutionContext(user_id=user_id, thread_id=thread_id, run_id=run_id, request_id=request_id, operation_depth=15)
    ...
    def test_validate_context_data_types(self):
        ...
        with pytest.raises(ContextValidationError, match='agent_context must be a dictionary'):
            StronglyTypedUserExecutionContext(user_id=user_id, thread_id=thread_id, run_id=run_id, request_id=request_id, agent_context='not_a_dict')
        with pytest.raises(ContextValidationError, match='audit_metadata must be a dictionary'):
            StronglyTypedUserExecutionContext(user_id=user_id, thread_id=thread_id, run_id=run_id, request_id=request_id, audit_metadata=['not', 'a', 'dict'])
...
class TestStronglyTypedUserExecutionContextChildCreation:
    ...
    def test_create_child_context_with_auto_generated_id(self):
        ...
        parent_context = StronglyTypedUserExecutionContext(user_id=parent_user_id, thread_id=parent_thread_id, run_id=parent_run_id, request_id=parent_request_id, agent_context={'parent_key': 'parent_value'}, audit_metadata={'audit_parent': 'audit_value'})
        ...
        assert child_context.request_id != parent_request_id
    ...
    def test_create_child_context_with_custom_request_id(self):
        ...
        parent_run_id = RunID(f'run_{uuid.uuid4().hex[:12]}')
        parent_request_id = RequestID(f'req_{uuid.uuid4().hex[:12]}')
        parent_context = StronglyTypedUserExecutionContext(user_id=parent_user_id, thread_id=parent_thread_id, run_id=parent_run_id, request_id=parent_request_id)
    ...
    def test_multi_level_child_context_hierarchy(self):
        ...
        root_context = StronglyTypedUserExecutionContext(user_id=root_user_id, thread_id=root_thread_id, run_id=root_run_id, request_id=root_request_id)
...
class TestStronglyTypedUserExecutionContextCompatibility:
    ...
    def test_metadata_property_backward_compatibility(self):
        ...
        run_id = RunID(f'run_{uuid.uuid4().hex[:12]}')
        request_id = RequestID(f'req_{uuid.uuid4().hex[:12]}')
        context = StronglyTypedUserExecutionContext(user_id=user_id, thread_id=thread_id, run_id=run_id, request_id=request_id, agent_context={'agent_key': 'agent_value'}, audit_metadata={'audit_key': 'audit_value'})
        expected_metadata = {'agent_key': 'agent_value', 'audit_key': 'audit_value'}
    ...
    def test_websocket_connection_id_alias(self):
        ...
        request_id = RequestID(f'req_{uuid.uuid4().hex[:12]}')
        websocket_id = WebSocketID(f'ws_{uuid.uuid4().hex[:12]}')
        context = StronglyTypedUserExecutionContext(user_id=user_id, thread_id=thread_id, run_id=run_id, request_id=request_id, websocket_client_id=websocket_id)
    ...
    def test_downgrade_to_legacy_format(self):
        ...
        request_id = RequestID(f'req_{uuid.uuid4().hex[:12]}')
        websocket_id = WebSocketID(f'ws_{uuid.uuid4().hex[:12]}')
        typed_context = StronglyTypedUserExecutionContext(user_id=user_id, thread_id=thread_id, run_id=run_id, request_id=request_id, websocket_client_id=websocket_id, agent_context={'typed_key': 'typed_value'}, audit_metadata={'typed_audit': 'audit_value'})
    ...
    def test_round_trip_conversion(self):
        ...
        original_dict = {'user_id': f'user_{uuid.uuid4().hex[:12]}', 'thread_id': f'thread_{uuid.uuid4().hex[:12]}', 'run_id': f'run_{uuid.uuid4().hex[:12]}', 'request_id': f'req_{uuid.uuid4().hex[:12]}', 'agent_context': {'test_key': 'test_value'}, 'audit_metadata': {'audit_key': 'audit_value'}}
