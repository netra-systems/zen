...
from netra_backend.app.schemas.shared_types import ErrorContext, ErrorContextManager, get_enriched_error_context
...
class TestNetraExceptions:
    ...
    def test_service_error(self):
        """Test ServiceError."""
        exc = ServiceError("Payment failed", details={"service": "payment"})
        
        assert exc.error_details.code == ErrorCode.SERVICE_UNAVAILABLE.value
        assert exc.error_details.details.get("service") == "payment"
...
class TestApiErrorHandler:
    ...
    def test_handle_pydantic_validation_error(self):
        ...
        # Mock justification: External Pydantic library component - creating real ValidationError requires complex schema setup
        exc = Mock()
...
class TestApiErrorHandlerFunctions:
    ...
    async def test_netra_exception_handler(self):
        ...
        # Mock justification: FastAPI framework component - Request object requires HTTP server context unavailable in unit tests
