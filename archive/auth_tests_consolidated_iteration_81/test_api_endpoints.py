...
class TestMissingAPIEndpoints:
    ...
    def test_oauth_providers_endpoint_missing_fails(self, client):
        ...
        if isinstance(providers_data, list):
            ...
            # Handle provider objects with "name" field
            provider_list = providers_data.get("providers", [])
            if provider_list and isinstance(provider_list[0], dict):
                provider_names = [p.get("name", "").lower() for p in provider_list]
            else:
                provider_names = [p.lower() for p in provider_list if isinstance(p, str)]
