...
class TestDatabaseInitializationIdempotency:
    ...
    async def test_init_database_script_idempotency(self):
        ...
        """
        # Reset the auth_db state before starting to avoid loop conflicts
        try:
            if hasattr(auth_db, '_initialized'):
                auth_db._initialized = False
            if hasattr(auth_db, 'engine') and auth_db.engine:
    ...
    async def test_create_all_metadata_idempotency(self):
        ...
        async with auth_db.get_session() as session:
            # Use PostgreSQL-compatible query to check for tables
            result = await session.execute(
                text("SELECT tablename FROM pg_tables WHERE schemaname = 'public' AND tablename LIKE 'auth_%'")
            )
