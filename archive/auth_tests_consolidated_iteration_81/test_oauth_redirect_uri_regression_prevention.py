...
class TestOAuthRedirectURIRegressionPrevention:
    ...
    def test_oauth_redirect_uri_always_uses_auth_service_url(self):
        ...
        print(f"[OK] OAuth redirect URI validation passed: {correct_oauth_redirect}")
        ...
        with patch.object(AuthConfig, 'get_environment', return_value=environment):
            with patch.object(AuthConfig, 'get_auth_service_url') as mock_auth_url:
                with patch.object(AuthConfig, 'get_frontend_url') as mock_frontend_url:
                    ...
                    print(f"[OK] {environment} OAuth redirect URI: {oauth_redirect_uri}")
    ...
    def test_oauth_initiation_endpoint_redirect_uri_parameter(self):
        ...
        if response.status_code == 302:
            ...
            if "redirect_uri" in query_params:
                ...
                print(f"[OK] OAuth initiation redirect_uri: {redirect_uri}")
            else:
    ...
    def test_auth_routes_code_compliance_check(self):
        ...
        if "redirect_uri" in source_code and "_determine_urls()" in source_code:
            ...
            print("[OK] Auth routes code compliance check passed")
        else:
            print("[WARN] OAuth implementation not found in auth_routes.py")
    ...
    def test_staging_oauth_redirect_uri_configuration_validation(self):
        ...
        print(f"[OK] Staging OAuth redirect URI validated: {oauth_redirect_uri}")
    ...
    def test_oauth_callback_endpoint_exists_and_responds(self):
        ...
        for endpoint in callback_endpoints:
            ...
            # Additional check: endpoint responds properly
            # 401 (Unauthorized) is expected for missing session cookies (security working)
            # 400 (Bad Request) is expected for malformed requests (validation working)
            # 405 (Method Not Allowed) is acceptable - endpoint exists but wrong method
            # Also accept 200, 302 for working endpoints
            expected_statuses = [200, 302, 400, 401, 405, 422]
            assert response.status_code in expected_statuses, (
                f"OAuth callback endpoint {endpoint} returned {response.status_code}.\n"
                f"Expected one of {expected_statuses} (working/security responses).\n"
                f"404 would mean endpoint missing, 500+ indicates server issues.\n"
                f"Response: {response.text[:200]}"
            )
            
            print(f"[OK] OAuth callback endpoint exists: {endpoint}")
        ...
        if response.status_code == 302:
            ...
            if "state" in query_params:
                ...
                try:
                    ...
                    print(f"[OK] OAuth state parameter format valid")
                    ...
                    print(f"[OK] OAuth state parameter present (custom format)")
            else:
    ...
    def test_multi_provider_oauth_redirect_uri_consistency(self, oauth_provider):
        ...
        if response.status_code == 302:
            ...
            if "redirect_uri" in query_params:
                ...
                print(f"[OK] {oauth_provider} OAuth redirect URI: {redirect_uri}")
            else:
...
- 'No token received' errors increasing (MEDIUM)
    ...
    def test_oauth_deployment_validation_checklist(self):
        ...
        deployment_checklist = {
            "pre_deployment": [
                "[OK] OAuth redirect URIs point to auth service URLs",
                "[OK] OAuth redirect URIs match Google Console configuration",
                "[OK] OAuth callback endpoints exist and respond",
                "[OK] OAuth state parameter validation working",
                "[OK] OAuth token exchange functionality tested",
                "[OK] OAuth flow tested end-to-end in staging",
                "[OK] No frontend URLs in OAuth configuration"
            ],
            "deployment_validation": [
                "[OK] OAuth initiation endpoints return 302 redirects",
                "[OK] OAuth callback endpoints return non-404 responses",
                "[OK] Health checks include OAuth configuration validation", 
                "[OK] OAuth authentication monitoring enabled",
                "[OK] OAuth error logging configured"
            ],
            "post_deployment": [
                "[OK] OAuth authentication success rate > 95%",
                "[OK] No 'No token received' errors in logs",
                "[OK] OAuth callback completion rate > 95%",
                "[OK] OAuth provider response times < 3s",
                "[OK] No redirect_uri_mismatch errors"
            ]
        }
