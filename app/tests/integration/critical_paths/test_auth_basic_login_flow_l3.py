...
class TestAuthBasicLoginFlow(L3IntegrationTest):
    """Test basic authentication login flow from multiple angles."""
    
    async def test_successful_login_with_valid_credentials(self):
    ...
    async def test_login_failure_with_incorrect_password(self):
    ...
    async def test_login_with_case_insensitive_email(self):
    ...
    async def test_login_with_expired_account(self):
        ...
        async with aiohttp.ClientSession() as session:
            ...
            async with session.post(
                f"{self.backend_url}/api/v1/admin/users/{user_id}/deactivate",
    ...
    async def test_login_with_special_characters_in_password(self):
    ...
    async def test_login_token_expiry_configuration(self):
