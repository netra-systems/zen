...
class PermissionService:
    ...
    def update_user_role(db: Session, user: User, check_developer: bool = True) -> User:
        ...
        Returns:
            Updated user object
        """
        if check_developer and PermissionService.detect_developer_status(user):
            if user.role != "developer" and user.role not in ["admin", "super_admin"]:
                # Only elevate if not already admin or higher
                user.role = "developer"
    ...
    def get_user_permissions(user: User) -> Set[str]:
        ...
        if "*" in role_perms:
            # Return a comprehensive set of all permissions
            all_perms = set()
    ...
    def set_user_role(db: Session, user: User, role: str) -> User:
        ...
        # Update is_developer flag if role changes
