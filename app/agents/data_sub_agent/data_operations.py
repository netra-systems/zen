...
class DataOperations:
    ...
    async def get_cached_schema(self, table_name: str):
        return await self.data_fetching.get_cached_schema(table_name)
    
    async def fetch_clickhouse_data(self, query: str, cache_key=None):
        return await self.data_fetching.fetch_clickhouse_data(query, cache_key)
    
    async def check_data_availability(self, user_id: int, start_time, end_time):
        return await self.data_fetching.check_data_availability(user_id, start_time, end_time)
    
    async def get_available_metrics(self, user_id: int):
        return await self.data_fetching.get_available_metrics(user_id)
    
    async def get_workload_list(self, user_id: int, limit: int = 100):
        return await self.data_fetching.get_workload_list(user_id, limit)
    
    async def validate_query_parameters(self, user_id: int, workload_id, metrics):
    ...
    async def analyze_performance_metrics(self, user_id: int, workload_id, time_range, query_builder, analysis_engine):
        return await self.analysis_ops.analyze_performance_metrics(
            user_id, workload_id, time_range, query_builder, analysis_engine
        )
    
    async def detect_anomalies(self, user_id: int, metric_name: str, time_range, query_builder, z_score_threshold: float = 2.0):
        return await self.analysis_ops.detect_anomalies(
            user_id, metric_name, time_range, query_builder, z_score_threshold
        )
    
    async def analyze_correlations(self, user_id: int, metrics, time_range, query_builder):
        return await self.analysis_ops.analyze_correlations(
            user_id, metrics, time_range, query_builder
        )
    
    async def analyze_usage_patterns(self, user_id: int, query_builder, days_back: int = 30):
        return await self.analysis_ops.analyze_usage_patterns(
            user_id, query_builder, days_back
        )
    
    async def generate_insights(self, performance_data, usage_data):
