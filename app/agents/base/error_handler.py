...
Business Value: Orchestrates error recovery and fallback strategies.
"""
...
class ExecutionErrorHandler:
    ...
    async def _try_cached_fallback(self, context: ExecutionContext) -> Optional[ExecutionResult]:
        ...
        return self._create_cached_result(cached_data) if cached_data else None
    ...
    def _create_error_result(self, error: Exception, context: ExecutionContext,
        ...
        error_metadata = self._build_error_metadata(classification)
        return self._build_failed_execution_result(error, error_metadata)
    ...
    def _build_error_metadata(self, classification: ErrorClassification) -> Dict[str, Any]:
        """Build metadata for error result."""
        return {
