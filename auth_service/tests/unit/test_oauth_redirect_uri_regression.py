...
class OAuthRedirectURIConfigurationTests:
    ...
    def test_staging_redirect_uri_uses_proper_domain(self):
        ...
        assert redirect_uri == 'https://auth.staging.netrasystems.ai/auth/oauth/callback', f'Staging redirect URI must use auth.staging.netrasystems.ai, got: {redirect_uri}'
        assert 'run.app' not in redirect_uri, 'Redirect URI must NOT use Cloud Run app URL'

    def test_production_redirect_uri_uses_proper_domain(self):
        """Test production OAuth redirect URI uses auth.netrasystems.ai."""
        provider = GoogleOAuthProvider()
        redirect_uri = provider.get_redirect_uri()
        assert redirect_uri == 'https://auth.netrasystems.ai/auth/oauth/callback', f'Production redirect URI must use auth.netrasystems.ai, got: {redirect_uri}'
        assert 'run.app' not in redirect_uri, 'Redirect URI must NOT use Cloud Run app URL'
    ...
    def test_development_redirect_uri_uses_localhost(self):
        ...
        provider = GoogleOAuthProvider()
        redirect_uri = provider.get_redirect_uri()
        assert 'localhost' in redirect_uri or '127.0.0.1' in redirect_uri, f'Development redirect URI must use localhost, got: {redirect_uri}'
        assert 'netrasystems.ai' not in redirect_uri, 'Development must not use production domains'
    ...
    def test_redirect_uri_uses_auth_environment_ssot(self):
        ...
        assert actual_uri == expected_uri, f'Redirect URI must use AuthEnvironment SSOT. Expected: {expected_uri}, Got: {actual_uri}'
...
class OAuthConfigurationValidationTests:
    ...
    def test_staging_configuration_validation(self):
        ...
        is_valid, error = provider.validate_configuration()
        if not is_valid:
            assert 'redirect URI' not in error.lower() or 'staging' in error.lower(), f'Staging validation should not fail on redirect URI: {error}'
...
class OAuthAuthorizationURLTests:
    ...
    def test_authorization_url_uses_correct_redirect_uri(self):
        ...
        assert expected_redirect in auth_url or 'auth.staging.netrasystems.ai' in auth_url, f'Authorization URL must include proper redirect URI: {auth_url}'
        assert 'run.app' not in auth_url, 'Authorization URL must not include Cloud Run app URL'
