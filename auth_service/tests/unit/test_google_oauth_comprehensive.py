...
class GoogleOAuthProviderInitializationTests(SSotBaseTestCase):
    ...
    def test_provider_credentials_initialization(self):
        ...
        with patch('auth_service.auth_core.secret_loader.AuthSecretLoader.get_google_client_id') as mock_client_id, patch('auth_service.auth_core.secret_loader.AuthSecretLoader.get_google_client_secret') as mock_client_secret:
...
class GoogleOAuthProviderConfigurationTests(SSotBaseTestCase):
    ...
    def test_validate_configuration_with_invalid_client_id(self):
        ...
        with patch('auth_service.auth_core.secret_loader.AuthSecretLoader.get_google_client_id') as mock_client_id, patch('auth_service.auth_core.secret_loader.AuthSecretLoader.get_google_client_secret') as mock_client_secret:
            invalid_client_ids = [None, '', 'short-id', 'invalid-format', '123456789-abcdefghijklmnop.invalid.com']
            mock_client_secret.return_value = self.client_secret
...
class GoogleOAuthProviderCodeExchangeTests(SSotBaseTestCase):
    ...
    def test_exchange_code_for_user_info_real_api_success(self):
        ...
        with patch('auth_service.auth_core.secret_loader.AuthSecretLoader.get_google_client_id') as mock_client_id, patch('auth_service.auth_core.secret_loader.AuthSecretLoader.get_google_client_secret') as mock_client_secret, patch('requests.post') as mock_post, patch('requests.get') as mock_get:
            mock_client_id.return_value = self.client_id
            mock_client_secret.return_value = self.client_secret
            token_response = Mock()
            token_response.raise_for_status = Mock()
            token_response.json.return_value = {'access_token': 'ya29.test-access-token', 'token_type': 'Bearer', 'expires_in': 3600}
            mock_post.return_value = token_response
            user_response = Mock()
            user_response.raise_for_status = Mock()
            user_response.json.return_value = {'email': 'user@example.com', 'name': 'Real User', 'id': '123456789', 'verified_email': True, 'picture': 'https://example.com/photo.jpg'}
    ...
    def test_exchange_code_for_user_info_token_request_failure(self):
        ...
        with patch('auth_service.auth_core.secret_loader.AuthSecretLoader.get_google_client_id') as mock_client_id, patch('auth_service.auth_core.secret_loader.AuthSecretLoader.get_google_client_secret') as mock_client_secret, patch('requests.post') as mock_post:
            mock_client_id.return_value = self.client_id
            mock_client_secret.return_value = self.client_secret
...
class GoogleOAuthProviderSelfCheckTests(SSotBaseTestCase):
    ...
    def test_self_check_includes_redirect_uri(self):
        ...
        with patch('auth_service.auth_core.secret_loader.AuthSecretLoader.get_google_client_id') as mock_client_id, patch('auth_service.auth_core.secret_loader.AuthSecretLoader.get_google_client_secret') as mock_client_secret:
            ...
            redirect_checks = [check for check in check_result['checks_passed'] + check_result['checks_failed'] if 'redirect_uri' in check]
    ...
    def test_self_check_client_id_privacy(self):
        ...
        with patch('auth_service.auth_core.secret_loader.AuthSecretLoader.get_google_client_id') as mock_client_id, patch('auth_service.auth_core.secret_loader.AuthSecretLoader.get_google_client_secret') as mock_client_secret:
            ...
            if 'client_id_prefix' in check_result:
                ...
                assert len(client_id_prefix) <= 30
...
class GoogleOAuthProviderSecurityTests(SSotBaseTestCase):
    ...
    def test_redirect_uri_validation_by_environment(self):
        ...
        test_cases = [('staging', 'http://localhost:8081/auth/callback', False), ('staging', 'https://staging.example.com/auth/callback', True), ('production', 'http://localhost:8081/auth/callback', False), ('production', 'https://staging.example.com/auth/callback', False), ('production', 'https://app.example.com/auth/callback', True)]
    ...
    def test_input_sanitization(self):
        ...
        with patch('auth_service.auth_core.secret_loader.AuthSecretLoader.get_google_client_id') as mock_client_id, patch('auth_service.auth_core.secret_loader.AuthSecretLoader.get_google_client_secret') as mock_client_secret:
            ...
            mock_client_secret.return_value = self.client_secret
            provider = GoogleOAuthProvider()
            malicious_inputs = ["'; DROP TABLE users; --", "<script>alert('xss')</script>", '../../../etc/passwd', "javascript:alert('xss')", '\x00\x01\x02']
