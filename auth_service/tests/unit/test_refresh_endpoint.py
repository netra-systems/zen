...
class TestRefreshEndpointUnit:
    ...
    def test_refresh_with_valid_token_in_body(self, test_client, mock_auth_service):
        ...
        response = test_client.post('/auth/refresh', json={'refresh_token': 'valid.refresh.token'})
    ...
    def test_refresh_with_camelcase_field(self, test_client, mock_auth_service):
        ...
        mock_auth_service.refresh_tokens.return_value = ('new.access.token', 'new.refresh.token')
        response = test_client.post('/auth/refresh', json={'refreshToken': 'valid.refresh.token'})
        assert response.status_code == 200
    ...
    def test_refresh_with_token_field(self, test_client, mock_auth_service):
        ...
        mock_auth_service.refresh_tokens.return_value = ('new.access.token', 'new.refresh.token')
        response = test_client.post('/auth/refresh', json={'token': 'valid.refresh.token'})
        assert response.status_code == 200
        assert 'access_token' in response.json()
        mock_auth_service.refresh_tokens.assert_called_once_with('valid.refresh.token')
    ...
    def test_refresh_with_invalid_json(self, test_client):
        ...
        response = test_client.post('/auth/refresh', content='not valid json', headers={'Content-Type': 'application/json'})
        assert response.status_code == 422
        error = response.json()
    ...
    def test_refresh_with_invalid_token(self, test_client, mock_auth_service):
        ...
        response = test_client.post('/auth/refresh', json={'refresh_token': 'invalid.token'})
    ...
    def test_refresh_with_service_error(self, test_client, mock_auth_service):
        ...
        response = test_client.post('/auth/refresh', json={'refresh_token': 'valid.token'})
...
class TestJWTHandlerRefreshTokens:
    ...
    def test_create_refresh_token(self, jwt_handler):
        ...
        assert len(token) > 0
        payload = jwt.decode(token, jwt_handler.secret, algorithms=[jwt_handler.algorithm], options={'verify_aud': False})
    ...
    def test_validate_refresh_token_wrong_type(self, jwt_handler):
        ...
        access_token = jwt_handler.create_access_token(user_id='test-user', email='test@example.com')
    ...
    def test_validate_expired_refresh_token(self, jwt_handler):
        ...
        payload = {'sub': 'test-user', 'type': 'refresh', 'exp': past_time.timestamp(), 'iat': past_time.timestamp()}
