...
class TestSessionSecurity:
    ...
    async def test_concurrent_session_limit_prevents_account_sharing(self, session_manager):
        ...
        assert len(active_sessions) == max_concurrent_sessions, f"Expected {max_concurrent_sessions} sessions, got {len(active_sessions)}"
    ...
    async def test_session_invalidation_cascade_prevents_orphaned_sessions(self, session_manager):
        ...
        assert len(remaining_sessions) == 0, f"Orphaned sessions found: {len(remaining_sessions)}"
