...
class TestRefreshLoopPrevention:
    ...
    async def test_tokens_always_unique_on_refresh(self, auth_service):
        ...
        for i in range(10):
            ...
            assert access_token not in all_tokens, f"Duplicate access token at refresh {i+1}!"
            assert new_refresh not in all_tokens, f"Duplicate refresh token at refresh {i+1}!"
            assert new_refresh != current_refresh, f"Same refresh token returned at refresh {i+1}!"
        ...
        assert len(all_tokens) == 21, f"Expected 21 unique tokens, got {len(all_tokens)}"
    ...
    async def test_rapid_refresh_attempts_handled(self, auth_service):
        ...
        for i in range(1, 5):
            assert results[i] is None, f"Attempt {i+1} with same token should fail"
    ...
    pass
        ...
        for token in malformed_tokens:
            ...
            assert response.status_code in [401, 422], f"Token '{token}' should be rejected"
    ...
    return await auth_service.refresh_tokens(initial_refresh)
        ...
        assert len(successful) <= 1, f"Only one concurrent refresh should succeed, got {len(successful)}"
        assert len(failed) >= 9, f"Most concurrent attempts should fail, only {len(failed)} failed"
    ...
    pass
        ...
        for i in range(20):
            token = jwt_handler.create_access_token(f"user-{i}", f"user{i}@test.com")
    ...
    pass
        ...
        for cycle in range(5):
            result = await auth_service.refresh_tokens(current_refresh)
            assert result is not None, f"Refresh cycle {cycle+1} failed"

            access_token, new_refresh = result

            # Verify user data in access token
            access_payload = auth_service.jwt_handler.validate_token(access_token, "access")
            assert access_payload["sub"] == user_id, f"User ID changed in cycle {cycle+1}"
            assert access_payload["email"] == email, f"Email changed in cycle {cycle+1}"
