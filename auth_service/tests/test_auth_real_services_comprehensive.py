...
try:
    from test_framework.environment_isolation import get_test_env_manager, isolated_test_env, test_env_with_llm
except ImportError:
...
class TestRealDatabaseConnections:
    ...
    async def test_create_auth_user_real_db(self, real_repository_factory):
        ...
        try:
            ...
            user = await user_repo.create_local_user(email=email, password_hash=password_hash, full_name='Test User')
    ...
    async def test_auth_session_persistence(self, real_repository_factory):
        ...
        try:
            client_info = {'ip': '192.168.1.1', 'user_agent': 'TestAgent/1.0', 'device_id': 'test_device'}
            auth_session = await session_repo.create_session(user_id=user_id, refresh_token='test_refresh_token_12345', client_info=client_info)
...
class TestRealRedisConnections:
    ...
    async def test_redis_session_storage(self, real_redis_manager):
        ...
        session_data = {'user_id': str(uuid.uuid4()), 'email': 'test@example.com', 'created_at': datetime.now(timezone.utc).isoformat()}
...
class TestRealJWTValidation:
    ...
    def test_jwt_token_creation_and_validation(self, real_jwt_handler):
        ...
        access_token = real_jwt_handler.create_access_token(user_id=user_id, email=email, permissions=permissions)
    ...
    def test_jwt_token_expiration(self, real_jwt_handler):
        ...
        real_jwt_handler.access_expiry = -1
        try:
...
class TestRealAuthServiceFlows:
    ...
    async def test_user_registration_real_flow(self, real_auth_service, real_repository_factory):
        ...
        try:
            result = await real_auth_service.register_user(email=email, password=password, full_name=full_name)
    ...
    async def test_user_login_real_flow(self, real_auth_service, real_repository_factory):
        ...
        real_auth_service.db_session = session
        try:
            await real_auth_service.register_user(email=email, password=password, full_name='Test User')
    ...
    async def test_token_refresh_real_flow(self, real_auth_service, real_repository_factory):
        ...
        real_auth_service.db_session = session
        try:
            register_result = await real_auth_service.register_user(email=email, password=password, full_name='Test User')
