...
class TestRefreshTokenFix:
    ...
    async def test_refresh_prevents_infinite_loop(self, auth_service):
        ...
        for i in range(3):
            result = await auth_service.refresh_tokens(refresh_token)
            assert result is not None, f"Refresh operation {i+1} should succeed"
            
            access_token, new_refresh_token = result
            
            # Verify tokens are unique
            assert access_token not in generated_tokens, f"Access token {i+1} should be unique"
            assert new_refresh_token not in generated_tokens, f"Refresh token {i+1} should be unique"
