    ...
    def _validate_database_configuration(self) -> None:
        ...
        try:
            ...
            if not database_url:
                ...
                self.validation_report["critical_issues"].append("Database: DATABASE_URL not configured")
                self.validation_report["validations"]["database"] = validation_result
                return
            
            # Validate URL format
            try:
                parsed = urlparse(database_url)
                validation_result["details"]["scheme"] = parsed.scheme
                validation_result["details"]["hostname"] = parsed.hostname
                validation_result["details"]["port"] = parsed.port
                validation_result["details"]["database"] = parsed.path.lstrip("/")
                
                # Check scheme
                if not database_url.startswith(("postgresql://", "postgres://")):
                    validation_result["status"] = "failed"
                    validation_result["issues"].append(f"Invalid database scheme: {parsed.scheme}")
                    self.validation_report["critical_issues"].append(f"Database: Invalid scheme {parsed.scheme}")
                
            except Exception as e:
                validation_result["status"] = "failed"
                validation_result["issues"].append(f"Invalid DATABASE_URL format: {e}")
                self.validation_report["critical_issues"].append("Database: Invalid URL format")
            ...
            if parsed.username and parsed.password:
                ...
                for pattern in placeholder_patterns:
                    if re.search(pattern, parsed.password or "", re.IGNORECASE):
                        validation_result["status"] = "failed"
                        validation_result["issues"].append("Database password contains placeholder text")
                        self.validation_report["critical_issues"].append("Database: Placeholder password detected")
                        break
                
                # Check for weak credentials
                if parsed.password and len(parsed.password) < 8:
                    validation_result["warnings"].append("Database password is shorter than 8 characters")
                    self.validation_report["warnings"].append("Database: Password shorter than 8 characters")
                
                if parsed.username in ["postgres", "root", "admin"] and AuthConfig.get_environment() in ["staging", "production"]:
                    validation_result["warnings"].append(f"Using common username '{parsed.username}' in {AuthConfig.get_environment()}")
                    self.validation_report["warnings"].append(f"Database: Common username in {AuthConfig.get_environment()}")
                
            else:
                validation_result["status"] = "failed"
                validation_result["issues"].append("Database credentials missing in URL")
                self.validation_report["critical_issues"].append("Database: Missing credentials")
            
            # Test URL conversion for asyncpg
            try:
                async_url = AuthDatabaseManager.get_auth_database_url_async()
                if not async_url.startswith("postgresql+asyncpg://"):
                    validation_result["warnings"].append("Database URL conversion may not use asyncpg driver")
                    self.validation_report["warnings"].append("Database: URL conversion issue")
                validation_result["details"]["async_url_format"] = "valid"
            except Exception as e:
                validation_result["status"] = "failed"
                validation_result["issues"].append(f"Database URL conversion failed: {e}")
                self.validation_report["critical_issues"].append("Database: URL conversion failed")
            
            # Validate SSL configuration
            ssl_validation = self._validate_database_ssl_config(database_url)
            validation_result["details"]["ssl_config"] = ssl_validation
            if not ssl_validation["valid"]:
                validation_result["status"] = "failed" if ssl_validation.get("critical") else validation_result["status"]
                validation_result["issues"].extend(ssl_validation.get("issues", []))
                if ssl_validation.get("critical"):
                    self.validation_report["critical_issues"].append(f"Database SSL: {ssl_validation.get('error', 'SSL configuration issue')}")
                
        except Exception as e:
            validation_result["status"] = "failed"
            validation_result["issues"].append(f"Database validation failed: {e}")
            self.validation_report["critical_issues"].append("Database: Validation process failed")
