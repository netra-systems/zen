...
# JWT Manager wrapper for test compatibility
class JWTManagerWrapper:
    """Wrapper to provide expected JWT manager interface for tests"""
    
    def __init__(self, jwt_handler):
        self._jwt_handler = jwt_handler
    
    def decode_token(self, token: str):
        """Decode and validate a token - returns payload or raises exception"""
        payload = self._jwt_handler.validate_token(token, "refresh")
        if not payload:
            raise Exception("Invalid token")
        return payload
    
    def generate_tokens(self, user_id: str, email: str, permissions=None):
        """Generate both access and refresh tokens"""
        if permissions is None:
            permissions = []
        
        access_token = self._jwt_handler.create_access_token(user_id, email, permissions)
        refresh_token = self._jwt_handler.create_refresh_token(user_id)
        
        return {
            "access_token": access_token,
            "refresh_token": refresh_token,
            "expires_in": self._jwt_handler.access_expiry * 60  # Convert minutes to seconds
        }
    
    async def is_token_blacklisted(self, token: str) -> bool:
        """Check if token is blacklisted (async version for test compatibility)"""
        return self._jwt_handler.is_token_blacklisted(token)

# Provide jwt_manager attribute for backwards compatibility and test mocking
# This allows tests to patch 'auth_service.auth_core.routes.auth_routes.jwt_manager'
jwt_manager = JWTManagerWrapper(auth_service.jwt_handler)
...
# Test compatibility functions - these provide the interface expected by tests
...
# Database manager compatibility wrapper for tests
class DBManagerWrapper:
    """Wrapper to provide database manager interface expected by tests"""
    
    def __init__(self, auth_service):
        self._auth_service = auth_service
    
    async def get_user_by_id(self, user_id: str) -> dict:
        """Get user by ID - for test compatibility"""
        # Use the auth service to validate and get user info
