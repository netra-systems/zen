...
import { Message, ToolCall, ToolOutput, StateUpdate } from '../types/chat';
...
const getToolName = (toolCalls?: ToolCall[]): string | null => {
  if (!toolCalls || toolCalls.length === 0) return null;
  const toolNames = toolCalls.map(tc => tc.name).filter(name => name !== 'update_state');
...
export function MessageCard({ message, user }: MessageCardProps) {
  ...
  const toolName = message.type === 'artifact' ? getToolName(message.tool_calls) : null;
  ...
  return (
    <Card
      className={`flex items-start gap-4 p-4 ${message.role === 'user' ? 'justify-end' : ''}`}>
      {message.role === 'agent' && (
        <Avatar>
          <AvatarImage src="/agent-avatar.png" />
          <AvatarFallback>A</AvatarFallback>
      ...
      <div
        className={`rounded-lg p-3 max-w-[75%] ${message.role === 'user' ? 'bg-primary text-primary-foreground' : 'bg-muted'}`}>
          <>
        ...
        {message.role === 'agent' && (
          <div>
            <button onClick={toggleThinking} className="text-blue-500 hover:underline mt-2">
              {showThinking ? 'Hide Raw Message' : 'Show Raw Message'}
            </button>
            {showThinking && (
              <div className="mt-2">
