...
async def test_start_agent(client, agent_service_mock):
    ...
    response = client.post("/api/v3/agent/start_agent/client123", json={"settings": {"debug_mode": True}, "request": {"id": "req_123", "user_id": "user123", "query": "test message", "workloads": []}})
...
async def test_start_agent_with_different_messages(client, agent_service_mock):
    ...
    app.dependency_overrides[ActiveUserDep] = lambda: User(email="dev@example.com", hashed_password="test")
    # First message
    response1 = client.post("/api/v3/agent/start_agent/client123", json={"settings": {"debug_mode": True}, "request": {"id": "req_123", "user_id": "user123", "query": "first message", "workloads": []}})
    assert response1.status_code == 200
    assert response1.json() == {"run_id": "test_run"}

    # Second message
    response2 = client.post("/api/v3/agent/start_agent/client123", json={"settings": {"debug_mode": True}, "request": {"id": "req_123", "user_id": "user123", "query": "second message", "workloads": []}})
...
async def test_start_agent_with_empty_message(client, agent_service_mock):
    ...
    app.dependency_overrides[ActiveUserDep] = lambda: User(email="dev@example.com", hashed_password="test")
    response = client.post("/api/v3/agent/start_agent/client123", json={"settings": {"debug_mode": True}, "request": {"id": "req_123", "user_id": "user123", "query": "", "workloads": []}})
...
async def test_websocket_connection(client):
    with client.websocket_connect("/api/v3/agent/test_run?token=test_token") as websocket:
...
async def test_start_agent_invalid_request(client):
    app.dependency_overrides[ActiveUserDep] = lambda: User(email="dev@example.com", hashed_password="test")
    response = client.post("/api/v3/agent/start_agent/client123", json={"invalid": "request"})
