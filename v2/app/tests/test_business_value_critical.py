...
class TestBusinessValueCritical:
    ...
    @pytest.fixture
    def setup_test_infrastructure(self, mock_workload_data):
        ...
        # LLM Manager with realistic responses
        llm_manager = Mock(spec=LLMManager)
        
        # Different responses for different agent types
        async def mock_ask_llm(prompt, llm_config_name=None, *args, **kwargs):
            # Debug: Check what agent is calling
            print(f"DEBUG: llm_config_name={llm_config_name}, prompt_length={len(prompt)}")
            
            # Use llm_config_name to determine which agent is calling
            if llm_config_name == 'triage' or "triage" in prompt.lower():
                return json.dumps({
            ...
            elif llm_config_name == 'actions_to_meet_goals' or "action planning specialist" in prompt.lower():
                # ActionsToMeetGoalsSubAgent specific response
                return json.dumps({
            ...
            elif llm_config_name == 'data' or ("data" in prompt.lower() and "action" not in prompt.lower()):
                return json.dumps({
            ...
            elif llm_config_name == 'optimizations_core' or "optimiz" in prompt.lower():
                return json.dumps({
                    "recommendations": [
                        {
        ...
        # Tool Dispatcher with realistic tool responses
        ...
        tool_dispatcher.dispatch_tool = AsyncMock(side_effect=mock_dispatch_tool)
        
        # Create Supervisor with proper state management
        with patch('app.services.state_persistence_service.state_persistence_service'):
            ...
            # Ensure all sub-agents have their websocket manager properly set
            ...
            if hasattr(supervisor, '_impl') and supervisor._impl and hasattr(supervisor._impl, 'sub_agents'):
                for agent in supervisor._impl.sub_agents:
        ...
        return {
            ...
            "workload_data": mock_workload_data
        }
