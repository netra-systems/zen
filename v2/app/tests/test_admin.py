...
def test_set_log_table_success(superuser_client):
    ...
    response = superuser_client.post(f"/api/v3/settings/log_table?log_table={table_name}")
    assert response.status_code == 200
    assert settings.clickhouse_logging.default_table == table_name

def test_set_log_table_not_available(superuser_client):
    response = superuser_client.post("/api/v3/settings/log_table?log_table=non_existent_table")
    assert response.status_code == 400

def test_add_log_table_success(superuser_client):
    new_table = "new_test_table"
    response = superuser_client.post(f"/api/v3/settings/log_tables?log_table={new_table}")
    assert response.status_code == 200
...
def test_add_log_table_already_exists(superuser_client):
    ...
    response = superuser_client.post(f"/api/v3/settings/log_tables?log_table={existing_table}")
    assert response.status_code == 400
...
def test_set_time_period_success(superuser_client):
    ...
    response = superuser_client.post(f"/api/v3/settings/time_period?days={days}")
    assert response.status_code == 200
    assert settings.clickhouse_logging.default_time_period_days == days

def test_set_time_period_not_available(superuser_client):
    response = superuser_client.post("/api/v3/settings/time_period?days=9999")
    assert response.status_code == 400
...
def test_set_default_log_table_for_context_success(superuser_client):
    ...
    table_name = settings.clickhouse_logging.available_tables[0]
    response = superuser_client.post(f"/api/v3/settings/default_log_table?context={context}&log_table={table_name}")
    assert response.status_code == 200
...
def test_set_default_log_table_for_context_table_not_available(superuser_client):
    response = superuser_client.post("/api/v3/settings/default_log_table?context=test&log_table=not_a_table")
    assert response.status_code == 400
...
def test_remove_default_log_table_for_context_success(superuser_client):
    ...
    response = superuser_client.delete(f"/api/v3/settings/default_log_table?context={context}")
    assert response.status_code == 200
...
def test_remove_default_log_table_for_context_not_found(superuser_client):
    response = superuser_client.delete("/api/v3/settings/default_log_table?context=non_existent_context")
    assert response.status_code == 400