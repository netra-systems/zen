...
class NetraOptimizerAgentSupervisor:
    ...
    def _get_agent_definition(self, llm_manager: LLMManager, tools: list) -> SubAgent:
        ...
        return SubAgent(
            name="netra_optimizer_agent",
            description="An agent for optimizing LLM usage. Your goal is to analyze the user's request "
    ...
    async def run_agent(self, run_id: str):
        ...
        async for event in self.graph.astream_events(state, {"recursion_limit": 20}):
            ...
            if event["event"] == "on_tool_end":
                ...
                if isinstance(tool_result, ToolResult) and tool_result.status == ToolStatus.ERROR:
                    state["status"] = "failed"
                    state["error_message"] = tool_result.message
                    return
