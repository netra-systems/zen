...
class BoundaryEnforcer:
    ...
    def generate_pr_comment(self) -> str:
        ...
        # Generate comment
        comment = f"""# {status_emoji} Boundary Enforcement Report
...
"""
        ...
        for boundary, count in report.violations_by_boundary.items():
            boundary_emoji = "🔴" if count > 0 else "✅"
            comment += f"- **{boundary.replace('_', ' ').title()}:** {boundary_emoji} {count} violations\n"
        ...
        if report.violations:
            ...
            if critical_violations:
                ...
                for i, violation in enumerate(critical_violations, 1):
                    comment += f"{i}. **{violation.file_path}** - {violation.description}\n"
                    if violation.auto_split_suggestion:
                        comment += f"   💡 *{violation.auto_split_suggestion}*\n"
