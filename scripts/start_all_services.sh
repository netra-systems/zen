#!/bin/bash
# Start all Netra infrastructure services (6 containers)
# PostgreSQL, Redis, ClickHouse for both test and dev environments
...
if [ $? -eq 0 ]; then
    ...
    echo
    echo "[ERROR] Failed to start services. Check Docker is running."
    exit 1
fi