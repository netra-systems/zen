...
class CodeReviewer:
    ...
    def run(self):
        ...
        # Step 1: Smoke Tests (fail-fast for critical issues)
        ...
        # Step 3: Spec-Code Alignment (skip in quick mode)
        if self.mode != "quick":
            self.check_spec_code_alignment()
        
        # Step 4: AI Coding Issues
        self.detect_ai_coding_issues()
        
        # Step 5: Performance Analysis (full mode only)
        if self.mode == "full":
