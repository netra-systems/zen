#!/usr/bin/env python3
"""
Issue #885 Phase 1: Focused WebSocket Import Fix for execution_engine.py

This script specifically targets the critical production file that needs import consolidation.
It focuses on high-impact, low-risk changes to improve SSOT compliance.

Business Value Justification:
- Segment: Platform/Infrastructure
- Business Goal: Achieve SSOT compliance in critical agent execution path
- Value Impact: Fix key import violation in supervisor execution engine
...
def create_backup(repo_path: Path) -> Path:
    """Create backup of execution engine file."""
    timestamp = subprocess.check_output(['powershell', 'Get-Date -Format "yyyyMMdd_HHmmss"'],
                                      text=True).strip()
    backup_dir = repo_path / "backups" / f"execution_engine_import_fix_{timestamp}"
    backup_dir.mkdir(parents=True, exist_ok=True)

    print(f"Creating backup at: {backup_dir}")

    # Backup the specific file
    source_file = repo_path / "netra_backend/app/agents/supervisor/execution_engine.py"
    if source_file.exists():
        dest_file = backup_dir / "execution_engine.py"
...
def apply_import_fix(repo_path: Path) -> bool:
    """Apply the specific import fix to execution_engine.py."""
    target_file = repo_path / "netra_backend/app/agents/supervisor/execution_engine.py"
    ...
    try:
        ...
        if old_import in content:
            ...
            print(f"SUCCESS: Replaced import in execution_engine.py")
...
def rollback_changes(backup_dir: Path, repo_path: Path) -> bool:
    ...
    try:
        backup_file = backup_dir / "execution_engine.py"
        target_file = repo_path / "netra_backend/app/agents/supervisor/execution_engine.py"
...
def main():
    ...
    if len(sys.argv) != 2:
        print("Usage: python issue_885_focused_execution_engine_fix.py <repo_path>")
        sys.exit(1)

    repo_path = Path(sys.argv[1])

    print("Issue #885 Phase 1: Focused WebSocket Import Fix")
    print("=" * 50)
    ...
    try:
        ...
        # Success!
        print("\n" + "=" * 50)
        print("SUCCESS: Phase 1 Import Fix completed!")
        print("Fixed: netra_backend/app/agents/supervisor/execution_engine.py")
        print(f"Backup: {backup_dir}")
        print("\nNext steps:")
        print("1. Run SSOT compliance tests to measure improvement")
        print("2. Commit this atomic change")
        print("3. Continue with additional import consolidations")
