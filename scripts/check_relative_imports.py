...
def main():
    ...
    for file_arg in sys.argv[1:]:
        ...
        if violations:
            ...
            print(f"\n❌ RELATIVE IMPORTS DETECTED in {file_path}:")
            print("=" * 60)
    ...
    if total_violations > 0:
        print("\n" + "=" * 80)
        print(f"❌ COMMIT BLOCKED: Found {total_violations} relative import(s) in {len(files_with_violations)} file(s)")
        print("=" * 80)
        print("\n📋 IMPORT RULES (from CLAUDE.md):")
        print("  • ALL Python files MUST use absolute imports")
        print("  • NEVER use relative imports (. or ..)")
        print("  • This rule overrides any existing patterns")
        print("\n✅ CORRECT EXAMPLE:")
