#!/usr/bin/env python3
"""
Script to apply Issue #712 patches to UnifiedWebSocketManager.
This script modifies the WebSocket manager to include SSOT validation and user isolation enforcement.
"""
...
def apply_patches():
    """Apply all Issue #712 patches to UnifiedWebSocketManager."""
    ...
    # Patch 1: Modify constructor signature
    print("Applying constructor signature patch...")
    content = re.sub(
        r'def __init__\(self, mode: WebSocketManagerMode = WebSocketManagerMode\.UNIFIED, user_context: Optional\[Any\] = None, config: Optional\[Dict\[str, Any\]\] = None\):',
    ...
    # Patch 2: Modify constructor docstring
    print("Applying constructor docstring patch...")
    old_docstring = '''        """Initialize UnifiedWebSocketManager - ALL MODES CONSOLIDATED TO UNIFIED.
    ...
    new_docstring = '''        """Initialize UnifiedWebSocketManager - ALL MODES CONSOLIDATED TO UNIFIED.
    ...
    # Patch 3: Replace constructor body implementation
    print("Applying constructor body patch...")
    old_body_start = '        # DEPRECATED: Mode is ignored - all instances use unified behavior\n        self.mode = WebSocketManagerMode.UNIFIED  # Force unified mode\n        self.user_context = user_context\n        self.config = config or {}'

    new_body_start = '''        # ISSUE #712 FIX: SSOT Validation Integration
    ...
    # Patch 4: Add user isolation validation methods
    print("Adding user isolation validation methods...")

    # Find the end of the __init__ method to insert new methods
    init_end_pattern = r'(        # Add compatibility for legacy tests expecting connection_manager.*?\n        self\.active_connections = \{\}  # Compatibility mapping\n)'
...
'''
    ...
    # Insert the new methods after the __init__ method
    content = re.sub(init_end_pattern, r'\1' + user_isolation_methods, content, flags=re.DOTALL)

    print("Writing patched file...")
    with open(file_path, 'w', encoding='utf-8') as f:
        f.write(content)

    print("✅ Issue #712 patches applied successfully!")
    ...
    if '_ssot_authorization_token' in patched_content:
        print("✅ Constructor signature patch applied")
    else:
        print("❌ Constructor signature patch failed")

    if 'SSOT validation integration' in patched_content:
        print("✅ Constructor body patch applied")
    else:
        print("❌ Constructor body patch failed")

    if '_validate_user_isolation' in patched_content:
        print("✅ User isolation methods patch applied")
    else:
        print("❌ User isolation methods patch failed")
