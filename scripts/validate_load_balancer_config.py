...
class LoadBalancerConfigValidator:
    ...
    def validate_requirement_1(self) -> bool:
        ...
        print("🔍 Validating Requirement 1: Backend Protocol HTTPS...")
        
        if not self.load_balancer_tf.exists():
            self.validation_results.append("❌ load-balancer.tf file not found")
            return False
        ...
        if all(protocol == "HTTPS" for protocol in backend_services):
            self.validation_results.append("✅ All backend services use HTTPS protocol")
            return True
        else:
            self.validation_results.append(f"❌ Backend services protocols: {backend_services}")
            return False
    
    def validate_requirement_2(self) -> bool:
        """Requirement 2: WebSocket Support - 3600s timeout and session affinity."""
        print("🔍 Validating Requirement 2: WebSocket Support...")
        ...
        if any("var.backend_timeout_sec" in timeout for timeout in timeout_configs):
            self.validation_results.append("✅ Timeout configured using variables")
        elif all("3600" in timeout for timeout in timeout_configs):
            self.validation_results.append("✅ Timeout set to 3600 seconds")
        else:
            self.validation_results.append(f"❌ Timeout configurations: {timeout_configs}")
            all_checks_pass = False
        
        if all(affinity == "GENERATED_COOKIE" for affinity in session_affinity_configs):
            self.validation_results.append("✅ Session affinity configured for WebSocket")
        else:
            self.validation_results.append(f"❌ Session affinity configurations: {session_affinity_configs}")
            all_checks_pass = False
            
        if cookie_ttl_configs:
            self.validation_results.append("✅ Cookie TTL configured")
        else:
            self.validation_results.append("❌ Cookie TTL not configured")
            all_checks_pass = False
    ...
    def validate_requirement_3(self) -> bool:
        ...
        print("🔍 Validating Requirement 3: Protocol Headers...")
        ...
        if len(x_forwarded_proto_headers) >= 3 and len(custom_headers) >= 3:
            self.validation_results.append("✅ X-Forwarded-Proto headers configured on all backend services")
            return True
        else:
            self.validation_results.append(f"❌ X-Forwarded-Proto headers found: {len(x_forwarded_proto_headers)}, expected: 3")
            return False
    
    def validate_requirement_4(self) -> bool:
        """Requirement 4: Health Checks use HTTPS protocol with port 443."""
        print("🔍 Validating Requirement 4: HTTPS Health Checks...")
        ...
        if https_health_checks and not http_health_checks and port_443_configs:
            self.validation_results.append("✅ Health checks use HTTPS on port 443")
            return True
        else:
            self.validation_results.append(f"❌ HTTPS health checks: {len(https_health_checks)}, HTTP health checks: {len(http_health_checks)}, Port 443: {len(port_443_configs)}")
            return False
    
    def validate_requirement_5(self) -> bool:
        """Requirement 5: CORS - HTTPS-only origins in staging/production."""
        print("🔍 Validating Requirement 5: CORS Configuration...")
        ...
        if cors_origins:
            ...
            if "https://" in origins_text and "http://" not in origins_text.replace("https://", ""):
                self.validation_results.append("✅ CORS configured with HTTPS-only origins")
                cors_valid = True
            else:
                self.validation_results.append(f"❌ CORS origins may include non-HTTPS: {origins_text[:100]}")
                cors_valid = False
        else:
            self.validation_results.append("❌ CORS configuration not found")
            cors_valid = False
        
        if websocket_path_rules:
            self.validation_results.append("✅ WebSocket path matchers configured")
            websocket_valid = True
        else:
            self.validation_results.append("❌ WebSocket path matchers not found")
    ...
    def validate_requirement_6(self) -> bool:
        ...
        print("🔍 Validating Requirement 6: Cloud Run Configuration...")
        
        if not self.deploy_script.exists():
            self.validation_results.append("❌ deploy_to_gcp.py script not found")
            return False
        ...
        if len(force_https_configs) >= 3:  # Should be in all 3 services
            self.validation_results.append("✅ FORCE_HTTPS=true configured for all services")
        else:
            self.validation_results.append(f"❌ FORCE_HTTPS configurations found: {len(force_https_configs)}, expected: 3")
            all_checks_pass = False
        
        if ingress_all_configs:
            self.validation_results.append("✅ Cloud Run ingress set to 'all'")
        else:
            self.validation_results.append("❌ Cloud Run ingress 'all' configuration not found")
            all_checks_pass = False
            
        if gen2_configs:
            self.validation_results.append("✅ Generation 2 execution environment configured")
        else:
            self.validation_results.append("⚠️ Generation 2 execution environment not explicitly configured")
        
        return all_checks_pass
    
    def validate_variables(self) -> bool:
        """Validate that required variables are defined."""
        print("🔍 Validating Variables Configuration...")
        
        if not self.variables_tf.exists():
            self.validation_results.append("❌ variables.tf file not found")
            return False
        ...
        if len(found_vars) == len(required_vars):
            self.validation_results.append("✅ All required variables defined")
            return True
        else:
            missing = set(required_vars) - set(found_vars)
            self.validation_results.append(f"❌ Missing variables: {missing}")
            return False
    
    def run_all_validations(self) -> bool:
        """Run all validation checks."""
        print("🚀 GCP Load Balancer Configuration Validator")
        print("=" * 60)
