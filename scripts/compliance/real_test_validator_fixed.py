...
class RealTestValidator:
    ...
    def _check_integration_test_mocking(self, file_path: Path, tree: ast.AST, lines: List[str]):
        ...
        # Count mock usage with regex patterns (fixed escaping)
        mock_count = 0
        mock_patterns = [
            'Mock\\(\\)', 'AsyncMock\\(\\)', 'MagicMock\\(\\)',
            'patch\\(', 'mock_\\w+\\s*=', '\\.return_value\\s*='
        ]
    ...
    def generate_report(self) -> str:
        ...
        for violation_type, violations in by_type.items():
            ...
            for violation in violations[:10]:  # Show first 10 per type
                severity_label = {"critical": "[CRITICAL]", "major": "[MAJOR]", "minor": "[MINOR]"}[violation.severity]
                report.append(f"{severity_label} **{violation.file_path}:{violation.line_number}** - {violation.description}")
