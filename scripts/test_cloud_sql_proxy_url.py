...
def test_cloud_sql_url_construction():
    ...
    print(f"[U+2713] Cloud SQL detected: {is_cloud_sql}")
    assert is_cloud_sql, "Failed to detect Cloud SQL configuration"
    
    # Get async URL
    async_url = builder.cloud_sql.async_url
    print(f"[U+2713] Async URL: {DatabaseURLBuilder.mask_url_for_logging(async_url)}")
    ...
    print(f"[U+2713] Sync URL: {DatabaseURLBuilder.mask_url_for_logging(sync_url)}")
    ...
    print(f"[U+2713] Staging environment URL: {DatabaseURLBuilder.mask_url_for_logging(staging_url)}")
    ...
    print(f"[U+2713] Staging sync URL: {DatabaseURLBuilder.mask_url_for_logging(staging_sync_url)}")
    assert staging_sync_url == sync_url, "Environment sync URL doesn't match expected sync URL"
    
    print("\n" + "=" * 50)
    print(" PASS:  All Cloud SQL proxy URL tests passed!")
    ...
    print(f"[U+2713] Cloud SQL detected (should be False): {is_cloud_sql_tcp}")
    assert not is_cloud_sql_tcp, "Incorrectly detected TCP as Cloud SQL"
    
    tcp_url = tcp_builder.tcp.async_url
    print(f"[U+2713] TCP URL: {DatabaseURLBuilder.mask_url_for_logging(tcp_url)}")
...
if __name__ == "__main__":
    try:
        ...
        if test_passed and compliance_passed:
            print("\n" + "=" * 50)
            print(" CELEBRATION:  All Cloud SQL proxy tests passed!")
