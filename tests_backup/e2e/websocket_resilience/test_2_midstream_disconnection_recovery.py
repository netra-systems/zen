"""
WebSocket Test 2: Mid-Stream Disconnection and Recovery

Tests that validate network drops while the agent is actively streaming a response 
preserves agent state and the response resumes or re-delivers upon reconnection.

Business Value: Prevents $75K+ MRR churn from mid-stream failures, ensures 99.95% 
response delivery guarantee for Enterprise customers.
"""
...
async def mock_websocket():
    ...
    ws = MockStreamingWebSocket("ws://mock-server/ws", "test_token_123")
    await ws.connect()
    yield ws
    await ws.disconnect()
...
def response_configs():
    """Standard response configurations for testing."""
    return {
        "small_text": {
        ...
        "multipart_response": {
            ...
            "components": [
                {"type": "text", "size": 2048, "id": "summary"},
                {"type": "data", "size": 10000, "id": "analysis", "schema": {"metric": "string", "value": "number"}},
                {"type": "text", "size": 1024, "id": "recommendations"}
            ]
        }
    }
