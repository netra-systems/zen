...
class TestSupervisorWebSocketIntegration:
    ...
    @pytest.fixture
    async def supervisor_agent(self, mock_db_session, mock_llm_manager, mock_websocket_manager, mock_tool_dispatcher):
        """Create supervisor agent with mocked dependencies."""
        with patch('app.agents.supervisor_consolidated.SupervisorInitializationHelpers'), \
             patch('app.agents.supervisor_consolidated.AgentRegistry'), \
             patch('app.agents.supervisor_consolidated.ExecutionEngine'), \
             patch('app.agents.supervisor_consolidated.PipelineExecutor'), \
             patch('app.agents.supervisor_consolidated.StateManager'):
    ...
    async def test_supervisor_receives_websocket_message(self, supervisor_agent):
        ...
        with patch.object(supervisor_agent, 'run', return_value=expected_response) as mock_run:
            # Call supervisor run method (simulating WebSocket message processing)
    ...
    @pytest.mark.asyncio
    async def test_supervisor_sends_websocket_response(self, supervisor_agent, mock_websocket_manager):
        ...
        mock_websocket_manager.send_message.assert_called_once_with(user_id, response_data)
    ...
    async def test_agent_service_handles_websocket_message(self, mock_db_session):
        ...
        with patch.object(agent_service, 'message_handler') as mock_handler:
            ...
            await agent_service.handle_websocket_message(user_id, message_data, mock_db_session)
            
            # Verify message was routed to handler
            mock_handler.handle_user_message.assert_called_once()
    
    @pytest.mark.asyncio 
    async def test_websocket_error_handling(self, supervisor_agent, mock_websocket_manager):
        """Test error handling in WebSocket communication."""
        user_id = "user-error-test"
        
        # Mock websocket manager to raise an exception
        mock_websocket_manager.send_message.side_effect = Exception("WebSocket connection lost")
    ...
    async def test_agent_service_websocket_message_validation(self):
        ...
        with patch.object(agent_service, 'message_handler') as mock_handler:
            ...
            await agent_service.handle_websocket_message(user_id, valid_message, db_session)
            mock_handler.handle_user_message.assert_called_once()
        ...
        with patch('app.ws_manager.manager.send_error') as mock_send_error:
            with patch.object(agent_service, '_parse_message', return_value=invalid_message):
                await agent_service.handle_websocket_message(user_id, invalid_message, db_session)
