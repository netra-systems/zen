...
class TestWebSocketPerformance:
    ...
    async def test_connection_pool_scaling(self, factory):
        ...
        for num_users in [5, 10, 20, 30]:
            ...
            for i in range(num_users):
                await pool.broadcast_to_user(f"user-{i}", {"type": "test"})
    ...
    pass
        ...
        for emitter in emitters:
            for j in range(events_per_user):
                await emitter.emit({"type": f"event_{j}", "data": {"index": j}})
        ...
        # Cleanup
        for i in range(num_users):
            await factory.remove_user_emitter(f"user-{i}")
    ...
    async def test_event_latency(self, factory):
        ...
        for i in range(num_samples):
            event = {"type": f"event_{i}", "data": {"index": i}}
        ...
        if latencies:
            ...
            print(f"Latency - Avg: {avg_latency:.2f}ms, Median: {median_latency:.2f}ms, P95: {p95_latency:.2f}ms")
    ...
    pass
        ...
        for queue_size in queue_sizes:
            ...
            for i in range(queue_size):
                emitter.queue_event({"type": f"event_{i}", "data": {"index": i}})
    ...
    async def test_concurrent_broadcasts(self, factory):
        ...
        for i in range(num_users):
            ...
            users.append((f"user-{i}", emitter, ws))
        ...
        async def broadcast_to_user(user_id, emitter, count):
            for i in range(count):
                await emitter.emit({"type": f"broadcast_{i}", "data": {"user": user_id}})
