...
class DockerComposeConfigValidationTests:
    ...
    def test_staging_backend_has_service_id_configured(self):
        ...
        assert 'SERVICE_ID' in backend_env, 'SERVICE_ID must be added to docker-compose.staging.yml backend environment. This is required to fix Issue #115 - System User Authentication Failure. Add: SERVICE_ID: netra-backend'
        assert backend_env['SERVICE_ID'] == 'netra-backend', f"SERVICE_ID should be 'netra-backend' (hardcoded stable value), got: '{backend_env['SERVICE_ID']}'. This ensures consistent service identification across all environments."
    ...
    def test_staging_backend_has_service_secret_configured(self):
        ...
        with open(staging_compose_path, 'r') as f:
            compose_config = yaml.safe_load(f)
        backend_env = compose_config['services']['backend']['environment']
        assert 'SERVICE_SECRET' in backend_env, 'SERVICE_SECRET must be added to docker-compose.staging.yml backend environment. This is required to fix Issue #115 - System User Authentication Failure. Add: SERVICE_SECRET: <secure_secret_32_chars_minimum>'
        service_secret = backend_env['SERVICE_SECRET']
        assert isinstance(service_secret, str) and len(service_secret) >= 32, f"SERVICE_SECRET should be at least 32 characters for security, got: {(len(service_secret) if isinstance(service_secret, str) else 'non-string')}. Use a secure secret like: staging_service_secret_secure_32_chars_minimum_2024"
    ...
    def test_service_secret_different_from_jwt_secret(self):
        ...
        assert service_secret != jwt_secret, f'SERVICE_SECRET should be different from JWT_SECRET_KEY for security isolation. Service authentication and JWT authentication should use separate secrets. Both currently have the same value.'
    ...
    def test_auth_service_has_matching_service_secret(self):
        ...
        assert auth_service_secret is not None, 'Auth service should have SERVICE_SECRET configured to validate backend requests. Add SERVICE_SECRET to auth service environment with the same value as backend.'
        assert backend_service_secret == auth_service_secret, f"Backend and auth services should have matching SERVICE_SECRET for cross-service auth. Backend: '{backend_service_secret}', Auth: '{auth_service_secret}'"
    ...
    def test_all_required_auth_variables_present(self):
        ...
        with open(staging_compose_path, 'r') as f:
            compose_config = yaml.safe_load(f)
        backend_env = compose_config['services']['backend']['environment']
        required_auth_vars = {'SERVICE_ID': 'netra-backend', 'SERVICE_SECRET': lambda x: isinstance(x, str) and len(x) >= 32, 'JWT_SECRET_KEY': lambda x: isinstance(x, str) and len(x) >= 32}
        missing_vars = []
        invalid_vars = []
...
class ServiceAuthConfigurationRegressionTests:
    ...
    def test_no_dynamic_service_id_generation(self):
        ...
        if service_id_line:
            ...
            for pattern in dynamic_patterns:
                assert pattern not in line_content, f"SERVICE_ID configuration at line {line_num} contains dynamic pattern '{pattern}': {line_content}. SERVICE_ID must be hardcoded as 'netra-backend' to prevent authentication failures. Dynamic generation causes service identification mismatches."
    ...
    def test_service_secret_not_empty_or_placeholder(self):
        ...
        if 'SERVICE_SECRET' not in backend_env:
            pytest.skip('SERVICE_SECRET not configured yet - run after Issue #115 fix')
        service_secret = backend_env['SERVICE_SECRET']
        invalid_values = ['', 'changeme', 'placeholder', 'todo', 'fixme', 'temp', 'test', 'secret', 'password', 'key', 'replace', 'update']
        service_secret_lower = service_secret.lower()
        for invalid_value in invalid_values:
            assert invalid_value not in service_secret_lower, f"SERVICE_SECRET appears to contain placeholder value '{invalid_value}': {service_secret}. Use a proper secure secret for staging environment. Example: staging_service_secret_secure_32_chars_minimum_2024"
        assert not service_secret.isdigit(), f'SERVICE_SECRET should not be only digits: {service_secret}. Use a complex alphanumeric secret for security.'
        assert not service_secret.isalpha(), f'SERVICE_SECRET should not be only letters: {service_secret}. Use a complex alphanumeric secret for security.'
