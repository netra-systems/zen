...
class TestWebSocketSupervisorIsolation(SSotBaseTestCase):
    ...
    async def _create_real_websocket_context(self, auth_helper: E2EWebSocketAuthHelper, user_id: str, thread_id: str, run_id: Optional[str]=None) -> Tuple[WebSocketContext, websockets.ServerConnection]:
        """
        ...
        context = WebSocketContext.create_for_user(websocket=websocket, user_id=user_id, thread_id=thread_id, run_id=run_id)
    ...
    @pytest.mark.asyncio
    async def test_basic_websocket_context_isolation(self, auth_helper: E2EWebSocketAuthHelper, database_manager: DatabaseTestManager):
        """
        ...
        try:
            for i in range(3):
                ...
                context, websocket = await self._create_real_websocket_context(auth_helper, user_id, thread_id)
    ...
    @pytest.mark.asyncio
    async def test_concurrent_supervisor_creation_isolation(self, auth_helper: E2EWebSocketAuthHelper, database_manager: DatabaseTestManager):
        """
        ...
        try:
            ...
            async with database_manager.get_async_session() as db_session:
                ...
                for context, _ in contexts_and_connections:
                    task = asyncio.create_task(get_websocket_scoped_supervisor(context, db_session))
                    tasks.append(task)
    ...
    @pytest.mark.asyncio
    async def test_websocket_context_lifecycle_isolation(self, auth_helper: E2EWebSocketAuthHelper):
        """
    ...
    @pytest.mark.asyncio
    async def test_concurrent_message_handling_isolation(self, auth_helper: E2EWebSocketAuthHelper, database_manager: DatabaseTestManager):
        """
        ...
        try:
            ...
            for i, (context, websocket) in enumerate(contexts_and_connections):
                message = {'type': 'user_message', 'content': f'Test message from user {i}', 'user_id': context.user_id, 'thread_id': context.thread_id, 'timestamp': time.time(), 'message_id': str(uuid.uuid4())}
    ...
    @pytest.mark.asyncio
    async def test_error_isolation_between_users(self, auth_helper: E2EWebSocketAuthHelper, database_manager: DatabaseTestManager):
        """
    ...
    @pytest.mark.asyncio
    async def test_performance_under_concurrent_load(self, auth_helper: E2EWebSocketAuthHelper, database_manager: DatabaseTestManager):
        """
        ...
        try:
            ...
            for i in range(num_connections):
                ...
                task = asyncio.create_task(self._create_real_websocket_context(auth_helper, user_id, thread_id))
                connection_tasks.append(task)
            connections_results = await asyncio.gather(*connection_tasks, return_exceptions=True)
    ...
    @pytest.mark.asyncio
    async def test_websocket_disconnection_isolation(self, auth_helper: E2EWebSocketAuthHelper):
        """
    ...
    @pytest.mark.asyncio
    async def test_websocket_context_factory_isolation(self, auth_helper: E2EWebSocketAuthHelper):
        """
        ...
        try:
            for i in range(3):
                ...
                context, websocket = await self._create_real_websocket_context(auth_helper, user_id, thread_id, run_id)
                contexts_and_connections.append((context, websocket))
            connection_ids = set()
            isolation_keys = set()
    ...
    @pytest.mark.asyncio
    async def test_cross_user_data_isolation_validation(self, auth_helper: E2EWebSocketAuthHelper):
        """
        ...
        contexts_and_connections = []
        try:
            sensitive_data = [('user_alice_sensitive', 'thread_confidential_001', 'run_secret_alpha'), ('user_bob_private', 'thread_personal_002', 'run_private_beta'), ('user_charlie_secure', 'thread_classified_003', 'run_secure_gamma')]
