...
class TestParameterValidationErrorDetection:
    ...
    def test_why_3_supervisor_create_parameter_type_validation(self):
        ...
        """
        invalid_llm_types = [None, 'invalid_string_llm', 123, [], {}, MagicMock()]
        for invalid_llm in invalid_llm_types:
            with pytest.raises((TypeError, ValueError, AttributeError)) as exc_info:
                SupervisorAgent.create(llm_manager=invalid_llm, websocket_bridge=self.mock_websocket_bridge)
            error_msg = str(exc_info.value).lower()
            assert any((term in error_msg for term in ['llm', 'manager', 'type', 'invalid'])), f'Error message should indicate LLM manager type issue: {exc_info.value}'
        invalid_bridge_types = [None, 'invalid_string_bridge', 456, [], {}, MagicMock()]
        for invalid_bridge in invalid_bridge_types:
            with pytest.raises((TypeError, ValueError, AttributeError)) as exc_info:
                SupervisorAgent.create(llm_manager=self.mock_llm_manager, websocket_bridge=invalid_bridge)
            error_msg = str(exc_info.value).lower()
            assert any((term in error_msg for term in ['websocket', 'bridge', 'type', 'invalid'])), f'Error message should indicate WebSocket bridge type issue: {exc_info.value}'
    ...
    def test_why_3_user_execution_context_parameter_validation(self):
        ...
        for missing_param in required_params:
            base_params = {'user_id': 'test_user', 'thread_id': 'test_thread', 'run_id': 'test_run', 'websocket_client_id': 'test_ws_client', 'db_session': None}
            ...
            for invalid_value in invalid_values:
                ...
                error_msg = str(exc_info.value).lower()
                assert missing_param.lower() in error_msg, f'Error message should mention {missing_param}: {exc_info.value}'
    ...
    def test_why_3_websocket_parameter_name_validation(self):
        ...
        with pytest.raises(TypeError) as exc_info:
            UserExecutionContext(user_id='test_user', thread_id='test_thread', run_id='test_run', websocket_connection_id='deprecated_param_value', db_session=None)
        error_msg = str(exc_info.value)
        assert 'websocket_connection_id' in error_msg, 'Error should mention deprecated parameter name'
        assert 'unexpected keyword' in error_msg.lower(), 'Error should indicate unexpected keyword argument'
        context = UserExecutionContext(user_id='test_user', thread_id='test_thread', run_id='test_run', websocket_client_id='correct_param_value', db_session=None)
    ...
    def test_why_3_factory_parameter_validation_integration(self):
        ...
        for param_name in required_create_params:
            assert param_name in create_signature.parameters, f'SupervisorAgent.create() missing required parameter: {param_name}'
    ...
    def test_why_3_error_message_clarity_validation(self):
        ...
        """
        try:
            SupervisorAgent.create(invalid_param='should_fail', websocket_bridge=self.mock_websocket_bridge)
            pytest.fail('Should have raised TypeError for invalid parameter')
        except TypeError as e:
            error_msg = str(e).lower()
            assert 'unexpected keyword' in error_msg or 'invalid_param' in error_msg, f'Error message should clearly indicate parameter issue: {e}'
        try:
            UserExecutionContext(user_id='test_user', thread_id='test_thread', run_id='test_run', invalid_websocket_param='should_fail', db_session=None)
            pytest.fail('Should have raised TypeError for invalid websocket parameter')
        except TypeError as e:
            error_msg = str(e).lower()
            assert 'unexpected keyword' in error_msg or 'invalid_websocket_param' in error_msg, f'Error message should clearly indicate websocket parameter issue: {e}'
        try:
            UserExecutionContext(user_id='test_user', run_id='test_run', db_session=None)
            pytest.fail('Should have raised error for missing required parameter')
        except (TypeError, InvalidContextError) as e:
            error_msg = str(e).lower()
            assert 'thread_id' in error_msg or 'required' in error_msg, f'Error message should indicate missing required parameter: {e}'
    ...
    def test_why_3_validation_prevents_silent_failures(self):
        ...
        """
        validation_scenarios = [{'function': SupervisorAgent.create, 'params': {'llm_manager': None, 'websocket_bridge': self.mock_websocket_bridge}, 'expected_error': (TypeError, ValueError, AttributeError), 'description': 'SupervisorAgent.create() with None llm_manager'}, {'function': SupervisorAgent.create, 'params': {'llm_manager': self.mock_llm_manager, 'websocket_bridge': None}, 'expected_error': (TypeError, ValueError, AttributeError), 'description': 'SupervisorAgent.create() with None websocket_bridge'}, {'function': UserExecutionContext, 'params': {'user_id': None, 'thread_id': 'test', 'run_id': 'test', 'db_session': None}, 'expected_error': (InvalidContextError, TypeError, ValueError), 'description': 'UserExecutionContext with None user_id'}, {'function': UserExecutionContext, 'params': {'user_id': '', 'thread_id': 'test', 'run_id': 'test', 'db_session': None}, 'expected_error': (InvalidContextError, TypeError, ValueError), 'description': 'UserExecutionContext with empty user_id'}]
    ...
    def test_why_3_comprehensive_validation_integration(self):
        ...
        try:
            ...
            supervisor_validation_works = True
        validation_results.append({'check': 'supervisor_create_validation_active', 'result': supervisor_validation_works, 'critical': True})
        ...
        try:
            ...
            context_validation_works = True
        validation_results.append({'check': 'user_context_validation_active', 'result': context_validation_works, 'critical': True})
        param_name_validation_works = True
        try:
            UserExecutionContext(user_id='test', thread_id='test', run_id='test', websocket_connection_id='deprecated', db_session=None)
            param_name_validation_works = False
        except TypeError:
            param_name_validation_works = True
        validation_results.append({'check': 'parameter_name_validation_active', 'result': param_name_validation_works, 'critical': True})
        ...
        try:
            ...
            type_validation_works = True
        validation_results.append({'check': 'type_validation_active', 'result': type_validation_works, 'critical': True})
