...
class TestWebSocketSupervisorInterfaceContracts:
    ...
    def test_why_2_websocket_factory_supervisor_create_parameter_mapping(self):
        ...
        assert expected_create_params.issubset(create_params), f'SupervisorAgent.create() missing parameters: {expected_create_params - create_params}'
    ...
    def test_why_2_user_execution_context_parameter_standardization(self):
        ...
        assert 'websocket_client_id' in context_params, 'REGRESSION: UserExecutionContext missing websocket_client_id parameter'
        assert 'websocket_connection_id' not in context_params, 'REGRESSION: UserExecutionContext has deprecated websocket_connection_id parameter'
        websocket_param = context_signature.parameters['websocket_client_id']
        assert websocket_param.default is None, 'websocket_client_id should default to None for optional usage'
    ...
    def test_why_2_websocket_supervisor_factory_source_code_consistency(self):
        ...
        if os.path.exists(factory_file):
            ...
            if 'UserExecutionContext(' in source_code:
                assert 'websocket_client_id=' in source_code, 'REGRESSION: WebSocket factory not using websocket_client_id parameter'
                assert 'websocket_connection_id=' not in source_code, 'REGRESSION: WebSocket factory using deprecated websocket_connection_id parameter'
    ...
    def test_why_2_parameter_flow_validation_end_to_end(self):
        ...
        """
        websocket_connection_data = {'connection_id': 'ws_conn_12345', 'user_id': 'test_user_67890', 'thread_id': 'test_thread_abcde'}
        context = UserExecutionContext(user_id=websocket_connection_data['user_id'], thread_id=websocket_connection_data['thread_id'], run_id='test_run_xyz123', websocket_client_id=websocket_connection_data['connection_id'], db_session=None)
    ...
    def test_why_2_interface_contract_regression_prevention(self):
        ...
        validation_results.append({'check': 'supervisor_create_interface_stable', 'result': expected_create_params == actual_create_params, 'expected': expected_create_params, 'actual': actual_create_params})
        context_sig = inspect.signature(UserExecutionContext.__init__)
        context_params = set(context_sig.parameters.keys())
        validation_results.append({'check': 'user_context_has_websocket_client_id', 'result': 'websocket_client_id' in context_params, 'missing': 'websocket_client_id' not in context_params})
        validation_results.append({'check': 'user_context_no_deprecated_websocket_connection_id', 'result': 'websocket_connection_id' not in context_params, 'has_deprecated': 'websocket_connection_id' in context_params})
        factory_sig = inspect.signature(get_websocket_scoped_supervisor)
        factory_params = set(factory_sig.parameters.keys())
        validation_results.append({'check': 'websocket_factory_has_context_param', 'result': 'context' in factory_params, 'missing': 'context' not in factory_params})
