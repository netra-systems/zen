...
class TestUserContextSecurityVulnerabilities:
    ...
    pass
        ...
        for admin_pattern in admin_patterns:
            ...
            context = UserExecutionContext(
                user_id=f"user_{admin_pattern}_123",  # Legitimate user with admin in name
                thread_id="thread_123",
                run_id="run_456"
            )
    ...
    pass
        ...
        for i in range(100):
            context = UserExecutionContext(
                user_id=f"user_{i:03d}_concurrent",
                thread_id=f"thread_{i:03d}",
                run_id=f"run_{i:03d}",
                metadata={"user_index": i, "secret": f"secret_{i}"}
            )
            contexts.append(context)
        
        # Verify each context maintains its own data
        for i, context in enumerate(contexts):
            assert context.user_id == f"user_{i:03d}_concurrent"
            assert context.metadata["user_index"] == i
            assert context.metadata["secret"] == f"secret_{i}"
    ...
    pass
        ...
        for i in range(10):
            context = UserExecutionContext(
                user_id=f"user_mem_{i}",
                thread_id=f"thread_mem_{i}",
                run_id=f"run_mem_{i}"
            )
...
class TestAgentSecurityWithContext:
 ...
 def real_tool_dispatcher():
    ...
    async def test_supervisor_agent_context_validation(self, mock_llm_manager, 
