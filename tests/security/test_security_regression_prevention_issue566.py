...
class SecurityRegressionPreventionIssue566Tests(SSotAsyncTestCase):
    ...
    def test_startup_module_never_contains_vulnerable_pattern(self):
        ...
        self.assertNotIn('create_llm_manager()', source_code, 'SECURITY REGRESSION DETECTED: startup_module.py contains vulnerable create_llm_manager() pattern again. This was fixed in Issue #566 and must not be reintroduced as it compromises user cache isolation.')
        self.assertIn('llm_manager_factory', source_code, 'SECURITY REGRESSION DETECTED: startup_module.py missing secure factory pattern. The fix for Issue #566 requires llm_manager_factory to ensure user-isolated LLM manager creation.')
    ...
    def test_smd_never_contains_vulnerable_pattern(self):
        ...
        self.assertNotIn('create_llm_manager()', source_code, 'SECURITY REGRESSION DETECTED: smd.py contains vulnerable create_llm_manager() pattern again. This was fixed in Issue #566 and must not be reintroduced as it compromises user cache isolation.')
        self.assertIn('llm_manager_factory', source_code, 'SECURITY REGRESSION DETECTED: smd.py missing secure factory pattern. The fix for Issue #566 requires llm_manager_factory to ensure user-isolated LLM manager creation.')
    ...
    def test_no_other_modules_introduce_vulnerable_pattern(self):
        ...
        """
        modules_to_check = ['netra_backend.app.startup_module', 'netra_backend.app.smd']
                        ...
                        if attr.__module__ == module_name:
                            try:
                                ...
                                if 'create_llm_manager()' in source:
                                    self.fail(f'SECURITY REGRESSION DETECTED: {module_name}.{attr_name} contains vulnerable create_llm_manager() pattern. This was fixed in Issue #566 and compromises cache isolation.')
    ...
    def test_security_fix_documentation_exists(self):
        ...
        self.assertIn('SECURITY FIX Issue #566', startup_source, 'SECURITY DOCUMENTATION MISSING: startup_module.py missing Issue #566 security fix documentation. This helps prevent future regressions.')
        self.assertIn('SECURITY FIX Issue #566', smd_source, 'SECURITY DOCUMENTATION MISSING: smd.py missing Issue #566 security fix documentation. This helps prevent future regressions.')
    ...
    def test_factory_pattern_compliance(self):
        ...
        self.assertTrue(hasattr(test_app.state, 'llm_manager_factory'), 'SECURITY REGRESSION: startup_module.py missing llm_manager_factory. This is required for Issue #566 security fix.')
        self.assertTrue(callable(test_app.state.llm_manager_factory), 'SECURITY REGRESSION: llm_manager_factory is not callable. This breaks the Issue #566 security fix.')
...
class SecurityAwarenessDocumentationTests(SSotAsyncTestCase):
    ...
    def test_security_test_file_exists(self):
        ...
        self.assertTrue(os.path.exists(security_test_file), 'SECURITY REGRESSION: Security test file for Issue #566 has been deleted. This file is critical for preventing security regressions.')
    ...
    def test_security_directory_structure(self):
        ...
        self.assertTrue(os.path.exists(security_dir), 'SECURITY REGRESSION: Security tests directory has been deleted. This directory contains critical security vulnerability tests.')
