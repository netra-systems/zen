...
class AgentLifecycleEventValidator:
    ...
    def record_event(self, event_data: Dict[str, Any]) -> None:
        ...
        logger.debug(f"Recorded event: {event_type} at {timing.received_at}")
    ...
    def _validate_payload_structure(self, event_type: str, payload: Dict[str, Any],
        ...
        if not isinstance(payload, dict):
            errors.append(f"{event_type}: Payload must be a dictionary")
            return False
        
        if not payload:
            errors.append(f"{event_type}: Payload cannot be empty")
            return False
    ...
    def _validate_required_fields(self, event_type: str, payload: Dict[str, Any],
        ...
        if missing_fields:
            errors.append(f"{event_type}: Missing required fields: {missing_fields}")
            return False
    ...
    def _validate_timing_constraints(self, timing: EventTiming, errors: List[str]) -> bool:
        ...
        if not (min_time <= elapsed_time <= max_time):
            errors.append(
                f"{timing.event_type}: Event timing {elapsed_time}ms outside "
                f"{timing.ui_layer.value} layer range ({min_time}-{max_time}ms)"
            )
            return False
    ...
    def _validate_event_order(self, timing: EventTiming, errors: List[str]) -> bool:
        ...
        for i in range(expected_index):
            ...
            if not any(e.event_type == expected_event for e in self.received_events[:timing.order_index]):
                errors.append(
                    f"{timing.event_type}: Event received before required predecessor {expected_event}"
                )
                return False
...
class TestAgentLifecycleWebSocketEvents:
    ...
    async def test_complete_agent_lifecycle_event_flow(self, agent_lifecycle_test_core):
        ...
        assert validation_results["all_critical_events_received"], \
            f"Missing critical events: {validation_results['missing_critical_events']}"
        
        # Assert validation success rate
        assert validation_results["validation_success_rate"] >= 0.8, \
            f"Event validation success rate too low: {validation_results['validation_success_rate']}"
        ...
        for event_type in critical_events:
            assert event_type in event_types, f"Critical event {event_type} not received"
    ...
    async def test_agent_started_event_payload_validation(self, agent_lifecycle_test_core):
        ...
        for field in required_fields:
            assert field in payload, f"Missing required field in agent_started: {field}"
    ...
    async def test_ui_layer_timing_validation(self, agent_lifecycle_test_core):
        ...
        for detail in validation_results["validation_details"]:
            if not detail["valid"] and any("timing" in error for error in detail["errors"]):
                timing_failures.append({
                    ...
                    "errors": [e for e in detail["errors"] if "timing" in e]
                })
        
        # Allow some timing variance but not complete failures
        timing_failure_rate = len(timing_failures) / max(len(events), 1)
        assert timing_failure_rate < 0.3, f"Too many timing failures: {timing_failures}"
        ...
        if fast_events and slow_events:
            first_fast = min(fast_events, key=lambda x: x.received_at)
            first_slow = min(slow_events, key=lambda x: x.received_at)
    ...
    async def test_event_payload_field_consistency(self, agent_lifecycle_test_core):
        ...
        for event in events:
            ...
            if "timestamp" in payload:
                ...
                assert isinstance(timestamp, (int, float)), \
                    f"Timestamp should be numeric, got {type(timestamp)} in {event['type']}"
                
                # Timestamp should be reasonable (not too far in past/future)
                current_time = time.time()
                assert abs(timestamp - current_time) < 300, \
                    f"Timestamp seems unreasonable in {event['type']}: {timestamp}"
        ...
        assert len(agent_names) <= 5, f"Too many different agent names: {agent_names}"
    ...
    async def test_missing_events_detection(self, agent_lifecycle_test_core):
        ...
        agent_request = {
            ...
            "user_id": session["user_data"].id,
            "message": "Hello",
            "thread_id": f"missing-test-{int(time.time())}"
        }
        ...
        assert len(missing_minimum) == 0, \
            f"Missing critical minimum events: {missing_minimum}"
    ...
    async def test_frontend_event_processing_simulation(self, agent_lifecycle_test_core):
        ...
        agent_request = {
            ...
            "thread_id": f"frontend-test-{int(time.time())}"
        }
        ...
        for event in events:
            try:
                ...
                if event_type == "agent_started":
                    ...
                    processed_events.append(f"UI: Agent started - {payload.get('agent_name', 'Unknown')}")
                    
                elif event_type == "agent_thinking":
                    # Frontend would show thinking indicator
                    assert "thought" in payload
                    processed_events.append(f"UI: Thinking - {payload.get('thought', '')[:50]}")
                    
                elif event_type == "partial_result":
                    # Frontend would accumulate partial content
                    assert "content" in payload
                    processed_events.append(f"UI: Partial result - {len(payload.get('content', ''))} chars")
                    
                elif event_type == "tool_executing":
                    # Frontend would show tool execution indicator
                    assert "tool_name" in payload
                    processed_events.append(f"UI: Tool executing - {payload.get('tool_name')}")
                ...
                processing_errors.append(f"Error processing {event_type}: {e}")
        ...
        assert processing_success_rate >= 0.7, \
            f"Frontend processing success rate too low: {processing_success_rate}"
        
        if processing_errors:
            # Log errors but don't fail test if most events processed successfully
            logger.warning(f"Frontend processing errors: {processing_errors}")
            assert len(processing_errors) <= len(events) * 0.3, \
                f"Too many frontend processing errors: {processing_errors}"
