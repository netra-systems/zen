...
class OAuthStagingTester:
    ...
    def __init__(self):
        self.auth_service_url = os.getenv('AUTH_SERVICE_URL', 'https://auth.staging.netrasystems.ai')
        self.frontend_url = os.getenv('FRONTEND_URL', 'https://app.staging.netrasystems.ai')
    ...
    async def test_auth_config_endpoint(self) -> bool:
        ...
        async with httpx.AsyncClient() as client:
            try:
                ...
                required_fields = ['google_client_id', 'endpoints', 'authorized_redirect_uris']
                ...
                if expected_callback not in data.get('authorized_redirect_uris', []):
                    print(f"❌ Expected callback URL not in redirect URIs: {expected_callback}")
        ...
        async with httpx.AsyncClient(follow_redirects=False) as client:
            try:
                ...
                if response.status_code != 302:
                    print(f"❌ OAuth initiation didn't redirect: {response.status_code}")
                    return False
                ...
                if f"{self.auth_service_url}/auth/callback" not in location:
                    print(f"❌ Incorrect redirect_uri in OAuth URL")
                    return False
                
                print("✅ OAuth initiation works correctly")
                print(f"   Redirects to Google OAuth")
                print(f"   Callback URL: {self.auth_service_url}/auth/callback")
                return True
    ...
    async def test_frontend_chat_page(self) -> bool:
        ...
        async with httpx.AsyncClient() as client:
            try:
                ...
                if response.status_code != 200:
                    print(f"❌ Chat page not accessible: {response.status_code}")
                    return False
        ...
        async with httpx.AsyncClient() as client:
            try:
                ...
                if response.status_code == 401:
                    print("✅ Token validation endpoint works (correctly rejects invalid token)")
                    return True
                ...
                print(f"❌ Unexpected token validation response: {response.status_code}")
                return False
