...
def _should_load_mocks(request) -> bool:
    ...
    if hasattr(request, 'fixturenames'):
        mock_fixtures = [name for name in request.fixturenames if name.startswith('mock_')]
        if mock_fixtures:
            return True
...
@pytest.fixture(scope="session", autouse=False)  # Not autouse to avoid loading for all tests
...
def auto_fixture_loader(request):
    ...
    if needs_loading:
        ...
        try:
            ...
            import warnings
            warnings.warn(f"Fixture loading issue: {e}", UserWarning)
...
def pytest_sessionfinish(session, exitstatus):
    ...
    print(f"
=== Memory Usage Report ===")
    print(f"Loaded fixture modules: {', '.join(_loaded_modules) if _loaded_modules else 'base only'}")
