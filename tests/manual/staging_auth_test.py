...
class StagingAuthTester:
    ...
    async def test_health_endpoints(self):
        ...
        try:
            # Test ready endpoint
            response = await self.client.get(f"{AUTH_SERVICE_URL}/health/ready")
            if response.status_code == 200:
                data = response.json()
                self.log_result(
                    "Health Check - Ready",
                    True,
                    f"Service ready: {data.get('status')} (DB: {data.get('database_status')})",
                    data
                )
            else:
                self.log_result(
            ...
            self.log_result("Health Check - Ready", False, f"Exception: {e}")

    async def test_oauth_flow_initiation(self):
        """Test OAuth flow initiation"""
        try:
            response = await self.client.get(
                f"{AUTH_SERVICE_URL}/auth/google?return_url=https://staging.netra.ai/dashboard"
            )
            ...
            if response.status_code == 302:
                ...
                if "accounts.google.com" in location:
                    ...
                    if not missing_params:
                        ...
                        if "auth.staging.netrasystems.ai" in redirect_uri:
                            ...
                            self.log_result(
                                "OAuth Flow Initiation",
                                False,
                                f"Incorrect redirect URI: {redirect_uri}",
                    ...
                    self.log_result(
                        "OAuth Flow Initiation",
                        False,
                        f"Redirect not to Google: {location}"
                    )
            else:
                self.log_result(
                    "OAuth Flow Initiation",
            ...
            self.log_result("OAuth Flow Initiation", False, f"Exception: {e}")
    ...
    async def test_token_validation(self):
        ...
        try:
            ...
            self.log_result("Token Validation - Invalid Token", False, f"Exception: {e}")
        ...
        try:
            ...
            self.log_result("Token Validation - Malformed Request", False, f"Exception: {e}")
            ...
            try:
                ...
                if response.status_code in [200, 204]:
                    ...
                    if origin in allowed_origins or "*" in allowed_origins:
                        self.log_result(
                            f"CORS Configuration - {origin}",
                            True,
                            f"CORS properly configured for {origin}",
                        ...
                        self.log_result(
                            f"CORS Configuration - {origin}",
                            False,
                            f"CORS not configured for {origin}, allowed: {allowed_origins}"
                        )
                else:
                    self.log_result(
                        f"CORS Configuration - {origin}",
                ...
                self.log_result(f"CORS Configuration - {origin}", False, f"Exception: {e}")
        ...
        try:
            ...
            for header, expected_value in security_headers.items():
                ...
                if actual_value:
                    ...
                    if expected_value.lower() not in actual_value.lower():
                        missing_headers.append(f"{header}={expected_value}")
                else:
            ...
            if not missing_headers:
                ...
                self.log_result(
                    "Security Headers",
                    False,
                    f"Missing or incorrect headers: {missing_headers}",
                    present_headers
                )
        except Exception as e:
            self.log_result("Security Headers", False, f"Exception: {e}")
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if environment != "staging":
                    issues.append(f"Environment should be 'staging', got '{environment}'")
                if service != "auth-service":
                    issues.append(f"Service should be 'auth-service', got '{service}'")
                if not version:
                ...
                if not issues:
                    self.log_result(
                        "Service Metadata", 
                        True,
                        f"Service metadata correct: {service} v{version} in {environment}",
                        data
                    )
                else:
                    self.log_result(
                ...
                self.log_result(
                    "Service Metadata",
                    False,
                    f"Could not fetch metadata: {response.status_code}"
                )
        except Exception as e:
            self.log_result("Service Metadata", False, f"Exception: {e}")
