...
class FrontendIntegrationTester:
    ...
    async def test_frontend_auth_config_endpoint(self):
        ...
        try:
            ...
            response = await self.client.get(f"{AUTH_SERVICE_URL}/auth/config")
            ...
            self.log_result("Frontend Auth Config", False, f"Exception: {e}")
    ...
    async def test_oauth_flow_complete_simulation(self):
        ...
        try:
            ...
            oauth_response = await self.client.get(
                ...
                params={"return_url": f"{FRONTEND_URL}/dashboard"},
                ...
                if "accounts.google.com" in location:
                    ...
                    if callback_response.status_code in [400, 401, 422]:
                        self.log_result(
                            ...
                            True,
                            f"Callback endpoint handles invalid code appropriately (status: {callback_response.status_code})",
                    ...
                    self.log_result(
                        "OAuth Initiation",
                        False,
                        f"OAuth redirect not to Google: {location}"
                    )
            else:
                self.log_result(
                    "OAuth Initiation",
                    False,
                    f"OAuth initiation failed: {oauth_response.status_code}"
                )
        except Exception as e:
            self.log_result("OAuth Flow Simulation", False, f"Exception: {e}")
    ...
    async def test_websocket_auth_compatibility(self):
        ...
        try:
            ...
            ws_auth_response = await self.client.get(f"{AUTH_SERVICE_URL}/ws/auth")
            ...
            if ws_auth_response.status_code == 401:
                ...
                alt_response = await self.client.get(f"{AUTH_SERVICE_URL}/auth/ws")
                ...
                self.log_result(
                    "WebSocket Auth Compatibility",
                    False,
                    f"Unexpected WebSocket auth response: {ws_auth_response.status_code}",
                    {"status_code": ws_auth_response.status_code}
                )
        except Exception as e:
            self.log_result("WebSocket Auth Compatibility", False, f"Exception: {e}")
        ...
        try:
            ...
            for i in range(5):
                response = await self.client.get(
                    f"{AUTH_SERVICE_URL}/auth/google",
                    params={"return_url": f"{FRONTEND_URL}/test"}
                )
            ...
            self.log_result("Rate Limiting", False, f"Exception: {e}")
        ...
        try:
            ...
            if error_response.status_code in [400, 422]:
                self.log_result(
                    ...
                    True,
                    f"Correctly handled malformed OAuth request (status: {error_response.status_code})",
                ...
                if "accounts.google.com" in location:
                    ...
                    self.log_result(
                        "Error Handling - Malformed OAuth",
                        False,
                        f"Unexpected redirect for malformed request: {location}",
                        {"location": location}
                    )
            else:
                self.log_result(
                    "Error Handling - Malformed OAuth",
                    False,
                    f"Unexpected error handling: {error_response.status_code}",
                    {"status_code": error_response.status_code}
                )
        except Exception as e:
    ...
    async def run_all_tests(self):
        ...
        print(f"Integration Test Summary: {passed}/{total} tests passed ({success_rate:.1f}%)")
