...
class TestConcurrentUserIsolation:
    ...
    async def test_concurrent_users_dont_share_state(self):
        ...
        async def execute_for_user(user: UserContext):
            ...
            context = AgentExecutionContext(agent_name='test_agent', thread_id=user.thread_id, user_id=user.user_id, run_id=user.run_id, metadata={'user_specific': user.user_id})
            state = DeepAgentState(thread_id=user.thread_id, user_message=f'Message from {user.user_id}')
            ...
            for run_id, active_context in engine.active_runs.items():
                if run_id != user.run_id:
                    assert user.user_id not in str(active_context.metadata), f'User {user.user_id} data leaked into run {run_id}'
                    assert user.thread_id != active_context.thread_id, f"User {user.user_id} thread_id found in other user's context"
    ...
    async def test_execution_semaphore_fairness(self):
        ...
        async def slow_agent_execution(user_id: str, delay: float):
            ...
            context = AgentExecutionContext(agent_name='slow_agent', thread_id=f'thread_{user_id}', user_id=user_id, run_id=f'run_{user_id}', metadata={'delay': delay})
            ...
            with patch.object(engine.agent_core, 'execute_agent', new_callable=AsyncMock) as mock_exec:
                mock_exec.return_value = AgentExecutionResult(success=True, agent_name='slow_agent', context=context, result='Done')
    ...
    async def test_websocket_events_go_to_correct_user(self):
        ...
        def create_user_websocket(user_id: str):
            ...
            async def send_event(event_type: str, data: dict):
                ws.events_received.append({'type': event_type, 'data': data, 'timestamp': datetime.now(timezone.utc)})
        ...
        async def track_notify(run_id: str, *args, **kwargs):
            ...
            events_by_run[run_id].append({'method': 'notify_agent_started', 'args': args, 'kwargs': kwargs, 'timestamp': datetime.now(timezone.utc)})
        ...
        async def execute_for_user(user_id: str):
            context = AgentExecutionContext(agent_name='test_agent', thread_id=f'thread_{user_id}', user_id=user_id, run_id=f'run_{user_id}_{uuid.uuid4()}', metadata={'user': user_id})
            state = DeepAgentState(thread_id=f'thread_{user_id}')
            with patch.object(engine.agent_core, 'execute_agent', new_callable=AsyncMock) as mock_exec:
                mock_exec.return_value = AgentExecutionResult(success=True, agent_name='test_agent', context=context, result=f'Result for {user_id}')
                result = await engine.execute_agent(context, state)
                ...
                for event in events_by_run[run_id]:
                    ...
                    for other_user in user_websockets:
                        if other_user != user_id:
                            assert other_user not in event_str, f"User {other_user} data found in {user_id}'s events"
            ...
            async for session in mock_get_db():
                ...
                for other_session in sessions_created:
                    if other_session.id != session.id and other_session.in_transaction:
                        assert other_session.id != session.id, f'Request {request_id} sharing session with another request'
                session.in_transaction = False
                return session.id
...
class TestGlobalStateExecutionPath:
    ...
    async def test_execution_engine_global_run_tracking(self):
        ...
        async def user_execution(user_id: str):
            ...
            context = AgentExecutionContext(agent_name='test_agent', thread_id=f'thread_{user_id}', user_id=user_id, run_id=run_id, metadata={'user': user_id})
            for active_run_id, active_context in engine.active_runs.items():
                if active_context.user_id != user_id:
                    conflicts.append({'user': user_id, 'found_active_run_from': active_context.user_id, 'run_id': active_run_id})
    ...
    async def test_tool_dispatcher_shared_executor(self):
        ...
        llm_manager = Mock(spec=LLMManager)
        websocket_bridge = Mock(spec=AgentWebSocketBridge)
        dispatcher = ToolDispatcher(llm_client=llm_manager, websocket_bridge=websocket_bridge)
        executions = []
        ...
        async def user_tool_execution(user_id: str):
            ...
            async def track_execute(name, *args, **kwargs):
                executions.append({'user': user_id, 'tool': name, 'timestamp': time.time()})
                recent_executions = [e for e in executions if time.time() - e['timestamp'] < 0.01]
...
class TestThreadUserContextMixing:
    ...
    async def test_context_ids_pollution(self):
        ...
        async def check_context_isolation(user_id: str, thread_num: int):
            ...
            context = AgentExecutionContext(agent_name='test_agent', thread_id=thread_id, user_id=user_id, run_id=run_id, metadata={'original_user': user_id, 'original_thread': thread_id})
            for active_run_id, active_context in engine.active_runs.items():
                if active_context.thread_id == thread_id and active_context.user_id != user_id:
                    context_issues.append({'issue': 'thread_id_collision', 'our_user': user_id, 'other_user': active_context.user_id, 'thread_id': thread_id})
            engine.active_runs[run_id] = context
            ...
            if run_id in engine.active_runs:
                ...
                if current_context.metadata.get('original_user') != user_id:
                    context_issues.append({'issue': 'context_modification', 'expected_user': user_id, 'found_user': current_context.metadata.get('original_user')})
...
class TestRaceConditions:
    ...
    async def test_websocket_bridge_setting_race_condition(self):
        ...
        async def set_bridge_for_request(request_id: int):
            ...
            registry.set_websocket_bridge(bridge)
            bridge_changes.append({'request': request_id, 'old_bridge_id': getattr(old_bridge, 'id', None) if old_bridge else None, 'new_bridge_id': request_id})
            await asyncio.sleep(random.uniform(0.001, 0.01))
            current_bridge = registry.websocket_bridge
...
class TestScalabilityLimits:
    ...
    async def test_system_cannot_handle_target_concurrent_users(self):
        ...
        async def simulate_user(user_id: str):
            ...
            context = AgentExecutionContext(agent_name='test_agent', thread_id=f'thread_{user_id}', user_id=user_id, run_id=f'run_{user_id}', metadata={})
            state = DeepAgentState(thread_id=f'thread_{user_id}')
            with patch.object(engine.agent_core, 'execute_agent', new_callable=AsyncMock) as mock_exec:

                async def slow_execution(*args, **kwargs):
                    await asyncio.sleep(1.0)
                    return AgentExecutionResult(success=True, agent_name='test_agent', context=context, result='Done')
        ...
        assert avg_response_time > 2.0, f'System cannot meet <2s response time target. Avg: {avg_response_time:.2f}s'
        assert total_time > 1.5, f'Concurrency limitations cause serialization. Total time: {total_time:.2f}s'
