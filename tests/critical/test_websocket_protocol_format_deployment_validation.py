...
class WebSocketProtocolFormatDeploymentValidationTests(SSotAsyncTestCase):
    ...
    async def test_websocket_protocol_format_deployment_validation(self):
        ...
        await self._test_protocol_format_scenario(scenario_name='correct_protocol_format', protocol=expected_protocol, token=test_token, should_succeed=True)
        await self._test_protocol_format_scenario(scenario_name='missing_jwt_prefix', protocol=['jwt-auth', test_token], token=test_token, should_succeed=False)
        await self._test_protocol_format_scenario(scenario_name='wrong_auth_type', protocol=['bearer-auth', f'jwt.{test_token}'], token=test_token, should_succeed=False)
        await self._test_deployment_cache_mismatch_scenario(cached_protocol=['jwt-auth', f'jwt.{test_token}'], deployed_protocol=['jwt-auth', test_token], token=test_token)
    ...
    async def _test_protocol_format_scenario(self, scenario_name: str, protocol: List[str], token: str, should_succeed: bool):
        """Test a specific protocol format scenario."""
        result = ProtocolValidationResult(test_type=scenario_name, protocol_sent=protocol, protocol_expected=['jwt-auth', f'jwt.{token}'])
        try:
            logger.info(f' CYCLE:  Testing {scenario_name} with protocol: {protocol}')
            websocket_url = await self._get_websocket_url()
            connection_result = await self._attempt_websocket_connection(url=websocket_url, subprotocols=protocol, token=token)
    ...
    async def _test_deployment_cache_mismatch_scenario(self, cached_protocol: List[str], deployed_protocol: List[str], token: str):
        """Test deployment cache mismatch scenario that causes protocol inconsistency."""
        result = ProtocolValidationResult(test_type='deployment_cache_mismatch', protocol_sent=deployed_protocol, protocol_expected=cached_protocol, deployment_source='simulated_deployment_mismatch')
        try:
            ...
            websocket_url = await self._get_websocket_url()
            frontend_result = await self._attempt_websocket_connection(url=websocket_url, subprotocols=cached_protocol, token=token, simulation_context='frontend_cached')
            backend_validation = await self._simulate_backend_protocol_validation(received_protocol=cached_protocol, expected_protocol=deployed_protocol)
    ...
    async def _attempt_websocket_connection(self, url: str, subprotocols: List[str], token: str, simulation_context: str='test') -> Dict[str, Any]:
        ...
        try:
            ...
            async with websockets.connect(url, subprotocols=subprotocols, ssl=ssl_context, timeout=10.0, ping_interval=None, ping_timeout=None) as websocket:
                ...
                test_message = {'type': 'protocol_validation_test', 'timestamp': time.time(), 'context': simulation_context}
            ...
            return {'success': False, 'error_code': getattr(e, 'status_code', 1011), 'error_message': f'{type(e).__name__}: {str(e)}', 'response_time_ms': response_time}

    async def _simulate_backend_protocol_validation(self, received_protocol: List[str], expected_protocol: List[str]) -> Dict[str, Any]:
        ...
        if len(received_protocol) != 2:
            ...
            jwt_token = received_protocol[1][4:]
            if len(jwt_token) < 10:
                validation_result['valid'] = False
...
pytestmark = [pytest.mark.critical, pytest.mark.websocket_protocol, pytest.mark.deployment_validation, pytest.mark.asyncio]
