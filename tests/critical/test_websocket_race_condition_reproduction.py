...
from netra_backend.app.websocket_core.connection_state_machine import ApplicationConnectionState, ApplicationConnectionStateMachine, StateTransitionError
...
class WebSocketRaceConditionReproduction:
    ...
    async def test_multiple_connection_state_machines_race(self):
        ...
        for i in range(num_concurrent_connections):
            ...
            task = asyncio.create_task(self._create_competing_state_machine(user_id, connection_id, i))
            tasks.append(task)
        ...
        for i, result in enumerate(results):
            if isinstance(result, Exception):
                failed_machines.append({'connection_id': f'ws_race_{i}', 'error': str(result), 'error_type': type(result).__name__})
        ...
        assert len(failed_machines) > 0, f"RACE CONDITION NOT DETECTED: All {len(successful_machines)} state machines succeeded, but race conditions should cause failures. This indicates the race condition either doesn't exist or isn't being triggered properly."
        race_error_types = [fm['error_type'] for fm in failed_machines]
        expected_errors = ['StateTransitionError', 'RuntimeError', 'ValueError']
        assert any((error_type in expected_errors for error_type in race_error_types)), f'Expected race condition errors {expected_errors}, got {race_error_types}'
    ...
    async def test_invalid_state_transition_connecting_to_services_ready(self):
        ...
        with pytest.raises(StateTransitionError) as exc_info:
            transition_result = state_machine.transition_to(ApplicationConnectionState.SERVICES_READY, reason='Attempting invalid state skip (this should fail)', metadata={'test_type': 'invalid_transition_reproduction'})
            pytest.fail(f'RACE CONDITION NOT DETECTED: Invalid transition CONNECTING  ->  SERVICES_READY succeeded when it should have failed. Result: {transition_result}')
        error_message = str(exc_info.value).lower()
        assert any((keyword in error_message for keyword in ['invalid transition', 'invalid state', 'transition not allowed'])), f'Expected invalid transition error, got: {error_message}'
    ...
    async def test_phase1_phase4_initialization_overlap_race(self):
        ...
        phase1_task = asyncio.create_task(self._simulate_phase1_transport_accept(websocket_mock))
        phase4_task = asyncio.create_task(self._simulate_phase4_application_ready(websocket_mock))
        try:
            results = await asyncio.wait_for(asyncio.gather(phase1_task, phase4_task, return_exceptions=True), timeout=5.0)
        ...
        assert len(failures) > 0, f'RACE CONDITION NOT DETECTED: Both phases completed successfully. Phase 1: {phase1_result}, Phase 4: {phase4_result}. Expected at least one failure due to overlap race condition.'
    ...
    async def test_message_processing_before_websocket_accept_complete(self):
        ...
        test_message = {'type': 'user_message', 'text': 'Test message that triggers race condition', 'thread_id': 'test-thread-123'}
        with pytest.raises(RuntimeError) as exc_info:
            await self._attempt_message_processing_before_accept(websocket_mock, test_message)
        error_message = str(exc_info.value).lower()
        assert 'accept' in error_message, f"Expected 'accept' error message, got: {error_message}"
    ...
    async def test_concurrent_user_execution_context_creation_race(self):
        ...
        for i in range(num_contexts):
            task = asyncio.create_task(self._create_user_execution_context(user_id, f'conn_{i}'))
            context_creation_tasks.append(task)
        context_results = await asyncio.gather(*context_creation_tasks, return_exceptions=True)
        ...
        if len(successful_contexts) > 1:
            ...
            assert len(unique_context_ids) < len(context_ids), f'RACE CONDITION NOT DETECTED: {len(successful_contexts)} contexts created with unique IDs {unique_context_ids}. Expected context conflicts or coordination failures.'
        assert len(failed_contexts) > 0 or len(successful_contexts) <= 1, f'RACE CONDITION NOT DETECTED: Multiple contexts ({len(successful_contexts)}) created successfully without coordination failures.'
    ...
    async def _create_competing_state_machine(self, user_id: str, connection_id: str, index: int) -> RaceConditionResult:
        ...
        try:
            ...
            return RaceConditionResult(connection_id=connection_id, success=True, timing_ms=elapsed_ms, state_transitions=transitions)
        except Exception as e:
            elapsed_ms = (time.time() - start_time) * 1000
            return RaceConditionResult(connection_id=connection_id, success=False, error_type=type(e).__name__, error_message=str(e), timing_ms=elapsed_ms)
    ...
    async def _simulate_phase1_transport_accept(self, websocket_mock) -> Dict[str, Any]:
        ...
        try:
            ...
            return {'phase': 'phase1_transport', 'success': True, 'state': 'CONNECTED'}
        except Exception as e:
            return {'phase': 'phase1_transport', 'success': False, 'error': str(e)}
    ...
    async def _simulate_phase4_application_ready(self, websocket_mock) -> Dict[str, Any]:
        ...
        try:
            ...
            return {'phase': 'phase4_application', 'success': True, 'state': 'PROCESSING_READY'}
        except Exception as e:
            return {'phase': 'phase4_application', 'success': False, 'error': str(e)}
    ...
    async def _create_user_execution_context(self, user_id: str, connection_id: str):
        ...
        try:
            ...
            raise
pytestmark = [pytest.mark.race_condition, pytest.mark.critical, pytest.mark.xfail(reason='Tests designed to FAIL with current implementation')]
