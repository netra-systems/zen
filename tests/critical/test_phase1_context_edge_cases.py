...
class TestMemoryLeakEdgeCases:
    ...
    def test_context_retention_memory_leak(self):
        ...
        assert collected_contexts >= len(weak_refs) * 0.8, f"Only {collected_contexts}/{len(weak_refs)} contexts collected"
    ...
    def test_metadata_memory_explosion(self):
        ...
        for i in range(10):
            context = context.create_child_context(
                operation_name=f"expand_{i}",
                additional_metadata={f"step_{i}": list(range(50))}
            )
        
        # Verify metadata size is reasonable
        metadata_size = len(str(context.metadata))
        assert metadata_size < 100000, f"Metadata size grew to {metadata_size} characters"
        ...
        def create_and_serialize_context(thread_id):
            try:
                ...
                child = context.create_child_context(
                    operation_name=f"child_op_{thread_id}"
                )
...
class TestWebSocketEventEdgeCases:
    ...
    def test_websocket_connection_switching(self):
        ...
        connection_ids = [f"ws_conn_{i}" for i in range(20)]
    ...
    def test_maximum_concurrent_contexts(self):
        ...
        try:
            for i in range(max_contexts):
                context = UserExecutionContext(
                    user_id=f"max_user_{i:04d}",
                    thread_id=f"max_thread_{i:04d}",
                    run_id=f"max_run_{i:04d}",
                    metadata={"index": i}
                )
                ...
                if i % 100 == 0:  # Check every 100th context
                    assert context.user_id == f"max_user_{i:04d}"
                    context_dict = context.to_dict()
                    assert isinstance(context_dict, dict)
