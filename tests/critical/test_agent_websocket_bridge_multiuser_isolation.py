...
class WebSocketEventCollector:
    ...
    def __init__(self, user_id: str):
        ...
        self.leaked_events: List[Dict] = []

    async def send_json(self, data: Dict):
        """Mock WebSocket send that collects events."""
        self.events.append(data)
        if 'user_id' in data and data['user_id'] != self.user_id:
            self.leaked_events.append({'intended_user': data['user_id'], 'actual_user': self.user_id, 'event': data})
...
class AgentWebSocketBridgeMultiUserIsolationTests:
    ...
    async def test_singleton_causes_cross_user_leakage(self):
        ...
        with patch('netra_backend.app.websocket_core.get_websocket_manager') as mock_ws_mgr:
            ...
            await bridge.notify_agent_started(run1_id, 'DataAgent', metadata={'user_id': user1_id, 'connection_id': f'conn_{user1_id}'})
            run2_id = f'run_{user2_id}_{uuid.uuid4()}'
            await bridge.notify_agent_started(run2_id, 'OptimizationAgent', metadata={'user_id': user2_id, 'connection_id': f'conn_{user2_id}'})
            assert user1_collector.has_leaked_events(), 'SECURITY VIOLATION: User 1 received events intended for User 2'
            assert user2_collector.has_leaked_events(), 'SECURITY VIOLATION: User 2 received events intended for User 1'
    ...
    async def test_factory_pattern_prevents_cross_user_leakage(self):
        ...
        """
        user1_context = UserExecutionContext(user_id='user_001', thread_id=f'thread_{uuid.uuid4()}', run_id=f'run_{uuid.uuid4()}', websocket_client_id=f'conn_user_001')
        user2_context = UserExecutionContext(user_id='user_002', thread_id=f'thread_{uuid.uuid4()}', run_id=f'run_{uuid.uuid4()}', websocket_client_id=f'conn_user_002')
        ...
        with patch('netra_backend.app.websocket_core.get_websocket_manager') as mock_ws_mgr:
            ...
            mock_manager = MagicMock()
            mock_ws_mgr.return_value = mock_manager
            connection_map = {'conn_user_001': user1_collector, 'conn_user_002': user2_collector}
            ...
            assert not user1_collector.has_leaked_events(), "User 1 should not receive User 2's events with factory pattern"
            assert not user2_collector.has_leaked_events(), "User 2 should not receive User 1's events with factory pattern"
    ...
    async def test_concurrent_user_operations_maintain_isolation(self):
        ...
        for i in range(num_users):
            ...
            context = UserExecutionContext(user_id=user_id, thread_id=f'thread_{uuid.uuid4()}', run_id=f'run_{uuid.uuid4()}', websocket_client_id=f'conn_{user_id}')
            collector = WebSocketEventCollector(user_id)
            collectors[user_id] = collector
            users.append({'id': user_id, 'context': context, 'collector': collector})
        bridge = AgentWebSocketBridge()
        with patch('netra_backend.app.websocket_core.get_websocket_manager') as mock_ws_mgr:
            websocket = WebSocketConnection()
            mock_manager = MagicMock()
            ...
            async def user_operations(user_id: str, emitter):
                ...
                for op in range(num_operations_per_user):
                    await emitter.emit_agent_thinking(f'Thinking about operation {op} for {user_id}')
                    await asyncio.sleep(0.001)
                    if op % 10 == 0:
                        await emitter.emit_tool_executing(f'Tool_{op}', {'user': user_id, 'operation': op})
            tasks = [user_operations(user['id'], emitters[user['id']]) for user in users]
            ...
            for user_id, collector in collectors.items():
                if collector.has_leaked_events():
                    ...
                    for leak in collector.leaked_events[:5]:
                        print(f"  - Event for {leak['intended_user']} sent to {leak['actual_user']}")
            assert not leakage_found, f'Cross-user event leakage detected in concurrent operations'
            for user_id, collector in collectors.items():
    ...
    async def test_background_task_maintains_user_context(self):
        ...
        """
        user_context = UserExecutionContext(user_id='background_user', thread_id=f'thread_{uuid.uuid4()}', run_id=f'run_{uuid.uuid4()}', websocket_client_id='conn_background')
        ...
        with patch('netra_backend.app.websocket_core.get_websocket_manager') as mock_ws_mgr:
            ...
            async def background_processing(emitter, context):
                ...
                await emitter.emit_agent_completed('BackgroundAgent', {'message': 'Background task completed'})
            task = asyncio.create_task(background_processing(emitter, user_context))
    ...
    async def test_error_handling_in_isolated_emitters(self):
        ...
        """
        user1_context = UserExecutionContext(user_id='error_user', thread_id=f'thread_{uuid.uuid4()}', run_id=f'run_{uuid.uuid4()}', websocket_client_id='conn_error')
        user2_context = UserExecutionContext(user_id='normal_user', thread_id=f'thread_{uuid.uuid4()}', run_id=f'run_{uuid.uuid4()}', websocket_client_id='conn_normal')
    ...
    async def test_emitter_cleanup_on_context_exit(self):
        ...
        """
        user_context = UserExecutionContext(user_id='cleanup_user', thread_id=f'thread_{uuid.uuid4()}', run_id=f'run_{uuid.uuid4()}', websocket_client_id='conn_cleanup')
...
class MigrationFromSingletonToFactoryTests:
    ...
    async def test_identify_singleton_usage_points(self):
        ...
        """
        singleton_usage_points = ['netra_backend.app.services.agent_service_core', 'netra_backend.app.services.message_handlers', 'netra_backend.app.services.message_processing', 'netra_backend.app.agents.base_agent', 'netra_backend.app.agents.supervisor.workflow_orchestrator', 'netra_backend.app.agents.supervisor.pipeline_executor']
        migration_needed = []
        for module_path in singleton_usage_points:
            migration_needed.append({'module': module_path, 'uses_singleton': True, 'migration_priority': 'CRITICAL'})
        assert len(migration_needed) > 0, 'Migration points should be identified'
        migration_report = {'total_modules': len(singleton_usage_points), 'needs_migration': len(migration_needed), 'modules': migration_needed}
