...
class TestHealthRouteConfigurationChaos:
    ...
    async def test_environment_specific_health_behavior_inconsistencies(
...
')
        ...
        assert len(chaos_detector.env_inconsistencies) == 0, \
            f"Environment health behavior inconsistencies: {chaos_detector.env_inconsistencies}"
    ...
    async def test_configuration_drift_between_environments(self, chaos_detector):
                ...
                if config_path.is_file():
                    ...
                    for env_config in env_configs:
                        if env_config.is_file():
                            config_contents[f"{config_path.name}/{env_config.name}"] = env_config.read_text()
        ...
        env_files = [name for name in config_contents.keys() if any(env in name.lower() for env in ['dev', 'stag', 'prod'])]
            ...
            for config in all_configs:
                ...
                if missing_in:
                    config_drift.append({
                        ...
                        'present_in': [env for env, configs in health_configs_per_env.items() if config in configs]
                    })
        ...
        assert len(config_drift) == 0, f"Configuration drift detected: {config_drift}"
    ...
    pass
        ...
        assert len(timeout_conflicts) == 0, f"Health check timeout conflicts: {timeout_conflicts}"
...
')
            ...
            postgres_checks = [item for item in health_check_order if item['database'] == 'postgres']
            clickhouse_checks = [item for item in health_check_order if item['database'] == 'clickhouse']
            
            if postgres_checks and clickhouse_checks:
                first_postgres = min(postgres_checks, key=lambda x: x['line'])['line']
                first_clickhouse = min(clickhouse_checks, key=lambda x: x['line'])['line']
        ...
        assert len(priority_misconfigs) == 0, f"Service priority misconfigurations: {priority_misconfigs}"
    ...
    pass
        ...
        if cb_health_file.exists():
            ...
            cb_endpoints = []
            import re
            cb_endpoint_matches = re.findall(r'@router\.get\(["\']([^"\']*)["\']', content)
        ...
        assert len(circuit_breaker_issues) == 0, f"Circuit breaker health check issues: {circuit_breaker_issues}"
    ...
    async def test_environment_variable_conflicts_in_health_checks(self, chaos_detector):
                            ...
                            try:
                                ...
                                if isinstance(data, dict):
                                    ...
                                    if conflict_name == 'database_url_conflicts':
                                        ...
                                        found_hosts = [host for host in db_hosts if host in response_str]
                                        ...
                                        found_timeouts = [t for t in timeout_values if t in response_str]
                    ...
                    with patch.dict(os.environ, test_env, clear=False):
                        try:
                            ...
                            precedence_conflicts.append({
                                'env_var': env_var,
                                'error': str(e),
                                'issue': f'Environment variable {env_var} causes configuration conflicts'
                            })
        ...
        assert len(env_var_conflicts) == 0, f"Environment variable conflicts: {env_var_conflicts}"
...
class TestHealthRouteConfigurationConsistency:
    ...
    async def test_database_connection_pool_conflicts_in_health(self):
                ...
                if len(files_with_pattern) > 1:
                    ...
                    if len(set(all_values)) > 1:
                        pool_conflicts.append({
                            'type': f'{pattern}_inconsistency',
                            'files': files_with_pattern,
                            'values': list(set(all_values)),
                            'issue': f'Different services use different {pattern} values'
                        })
        ...
        assert len(pool_conflicts) == 0, f"Database connection pool conflicts: {pool_conflicts}"
    ...
    pass
        ...
        health_route_files = [fp for fp in logger_names.keys() if 'health' in fp]
        ...
        if len(health_route_files) > 1:
            ...
            module_name_files = [fp for fp, patterns in logger_name_patterns.items() if patterns['uses_module_name']]
            hardcoded_files = [fp for fp, patterns in logger_name_patterns.items() if patterns['uses_hardcoded']]
        ...
        assert len(logging_inconsistencies) == 0, f"Logging configuration inconsistencies: {logging_inconsistencies}"
