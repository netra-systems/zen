...
class NotificationCapture:
    ...
    def get_failed_deliveries(self) -> List[NotificationEvent]:
        ...
        return [event for event in self.events if event.delivery_status == "error"]
...
class TestWebSocketBridgeInitializationFailures:
    ...
    return False
        ...
        for user_id in users:
            ...
            failed_events = [e for e in user_events if e.delivery_status == "error"]
            if failed_events:
    ...
    async def test_bridge_becomes_none_during_tool_execution(self, notification_capture):
        ...
        await working_bridge.send_tool_event("tool_started", {
            "tool_name": "test_tool",
            "user_id": user_id
        })
        notification_capture.record_event(
        ...
        try:
            if context.websocket_bridge:
                await context.websocket_bridge.send_tool_event("tool_progress", {
                    "tool_name": "test_tool", 
                    "progress": 50,
                    "user_id": user_id
                })
        ...
        try:
            if context.websocket_bridge:
                await context.websocket_bridge.send_tool_event("tool_completed", {
                    "tool_name": "test_tool",
                    "result": "success",
                    "user_id": user_id
                })
        ...
        user_events = notification_capture.get_events_for_user(user_id)
        delivered_events = [e for e in user_events if e.delivery_status == "delivered"]
        failed_events = [e for e in user_events if e.delivery_status == "error"]
...
class TestCrossUserIsolationViolations:
    ...
    async def test_notification_sent_to_wrong_user(self, notification_capture, mock_websocket_manager):
        ...
        await shared_context.websocket_bridge.send_agent_event("tool_started", {
            "tool_name": "test_tool",
    ...
    pass
        ...
        assert len(notification_capture.cross_user_violations) >= 6  # 3 users × 2 wrong recipients each
    ...
    pass
            for op_num in range(num_operations):
                ...
                shared_notification_context["current_thread"] = f"thread_{user_id}_{op_num}"
...
class TestNotificationDeliveryFailures:
    ...
    return True
        ...
        connection_errors = [e for e in failed_events if "connection lost" in e.error_message.lower()]
    ...
    pass
        ...
        user_events = notification_capture.get_events_for_user(user_id)
        queued_events = [e for e in user_events if e.delivery_status == "queued"]
        lost_events = [e for e in user_events if "overflow" in e.error_message]
    ...
    return True
        ...
        try:
            ...
            notification_capture.record_event(
                ...
                delivery_status="error",
                error_message=f"Notification timeout after {end_time - start_time:.1f}s - user left hanging"
            )
        
        # Verify timeout was recorded
        failed_events = notification_capture.get_failed_deliveries()
        timeout_events = [e for e in failed_events if "timeout" in e.error_message.lower()]
        ...
        for i, user_id in enumerate(users):
            for msg_num in range(10):
                sensitive_message = {
                    "type": "tool_result",
                    "user_id": user_id,
                    "sensitive_token": f"secret_{user_id}_{msg_num}",
                    "private_data": f"confidential_data_for_{user_id}",
                    "tool_output": f"result_for_{user_id}"
                }
...
class TestErrorHandlingAndRecovery:
        ...
        for notification in critical_notifications:
            try:
                ...
                notification_capture.record_event(
                    ...
                    payload=notification,
                    delivery_status="error",
                    error_message=f"No error handling: {str(e)}"
                )
        
        # Verify some notifications failed with no recovery
        failed_events = notification_capture.get_failed_deliveries()
        no_recovery_events = [e for e in failed_events if "No error handling" in e.error_message]
    ...
    return True
        ...
        duplicate_events = [e for e in delivered_events if e.payload.get("duplicate") is True]
...
class TestPerformanceAndLoadScenarios:
    ...
    pass
            ...
            if actual_delay > max_acceptable_delay:
                notification_capture.record_event(
                    ...
                    delivery_status="error",
                    error_message=f"Notification delayed {actual_delay:.2f}s (max: {max_acceptable_delay}s)"
                )
        ...
        delayed_events = [e for e in notification_capture.events if e.delivery_status == "delayed"]
        error_events = notification_capture.get_failed_deliveries()
        delay_errors = [e for e in error_events if "delayed" in e.error_message]
        ...
        assert delayed_percentage > 0.5, f"Expected >50% delay rate, got {delayed_percentage:.1%}"
    ...
    async def test_memory_leak_in_notification_tracking(self, notification_capture):
        ...
        for user_num in range(num_users):
            ...
            for notification_num in range(notifications_per_user):
                notification = {
                    "id": f"{user_id}_{notification_num}",
                    "timestamp": time.time(),
                    "type": "tool_progress",
                    "payload": {
            ...
            persistent_notification_cache[f"user_{user_id}_notifications"] = user_notifications
        ...
        if total_cached_items >= expected_items:
            notification_capture.record_event(
                ...
                delivery_status="error",
                error_message=f"Memory leak: {total_cached_items} items cached indefinitely"
            )
        
        # Verify memory leak was detected
        memory_errors = [e for e in notification_capture.events if e.event_type == "memory_leak_detected"]
        assert len(memory_errors) > 0, "Expected memory leak to be detected"
        
        # Check that cache size is excessive
        assert total_cached_items > 5000, f"Expected large cache (>5000), got {total_cached_items}"
