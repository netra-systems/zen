...
from tests.mission_critical.websocket_real_test_base import RealWebSocketTestBase, RealWebSocketTestConfig
...
class JWTAuthenticationTestSuite:
    ...
    def test_context(self):
        ...
        context = create_test_context(require_docker=True, require_real_services=True, environment='test')
        yield context
        context.cleanup()
    ...
    async def expired_jwt_token(self):
        ...
        payload = {'sub': 'test_user_123', 'email': 'test@example.com', 'permissions': ['user'], 'exp': int(time.time()) - 3600, 'iat': int(time.time()) - 7200}
...
class AuthServiceCommunicationBreakdownsTests(JWTAuthenticationTestSuite):
    ...
    async def test_auth_service_http_error_responses(self, auth_client_instance):
        ...
        error_scenarios = [(401, 'Unauthorized'), (403, 'Forbidden'), (404, 'Not Found'), (500, 'Internal Server Error'), (502, 'Bad Gateway'), (503, 'Service Unavailable')]
...
class JWTSecretMismatchesTests(JWTAuthenticationTestSuite):
    ...
    async def test_jwt_secret_mismatch_detection(self, auth_client_instance):
        ...
        correct_payload = {'sub': 'test_user_123', 'email': 'test@example.com', 'permissions': ['user'], 'exp': int(time.time()) + 3600, 'iat': int(time.time())}
    ...
    def test_service_secret_vs_jwt_secret_isolation(self, auth_client_instance):
        ...
        if service_secret and jwt_secret:
            assert service_secret != jwt_secret, 'CRITICAL SECURITY ISSUE: SERVICE_SECRET and JWT_SECRET_KEY are identical! This creates signature collision vulnerabilities. Use different secrets for different purposes.'
        assert service_secret or jwt_secret, 'CRITICAL CONFIGURATION ERROR: Neither SERVICE_SECRET nor JWT_SECRET_KEY is configured'
    ...
    async def test_cross_service_signature_validation(self):
        ...
        service_signature = hmac.new(service_secret.encode(), payload_json.encode(), hashlib.sha256).hexdigest()
...
class TokenExpirationHandlingTests(JWTAuthenticationTestSuite):
    ...
    async def test_token_expiration_detection(self, auth_client_instance):
        ...
        test_scenarios = [('expired_1_second_ago', int(time.time()) - 1), ('expired_1_minute_ago', int(time.time()) - 60), ('expired_1_hour_ago', int(time.time()) - 3600), ('expired_1_day_ago', int(time.time()) - 86400)]
        secret = 'test_jwt_secret_key_12345'
        for scenario_name, exp_time in test_scenarios:
            payload = {'sub': f'test_user_{scenario_name}', 'email': 'test@example.com', 'exp': exp_time, 'iat': int(time.time()) - 3600}
    ...
    async def test_token_refresh_flow_with_expired_access_token(self, auth_client_instance):
        ...
        with patch.object(auth_client_instance, '_execute_refresh_request') as mock_refresh:
            mock_refresh.return_value = {'access_token': 'new_access_token_12345', 'refresh_token': 'new_refresh_token_12345', 'token_type': 'Bearer', 'expires_in': 3600}
    ...
    async def test_concurrent_token_refresh_race_condition(self, auth_client_instance):
        ...
        async def mock_refresh_with_delay(request_data):
            ...
            return {'access_token': f'refreshed_token_{refresh_call_count}', 'refresh_token': 'new_refresh_token', 'expires_in': 3600}
        with patch.object(auth_client_instance, '_execute_refresh_request', side_effect=mock_refresh_with_delay):
            ...
            tasks = [auth_client_instance.refresh_token(refresh_token) for _ in range(5)]
...
class ServiceToServiceAuthenticationTests(JWTAuthenticationTestSuite):
    ...
    def test_service_secret_configuration_validation(self, auth_client_instance):
        ...
        assert auth_client_instance.service_secret is not None, 'CRITICAL CONFIGURATION ERROR: SERVICE_SECRET is not configured. Inter-service authentication will fail.'
        assert len(auth_client_instance.service_secret) >= 32, 'SECURITY WEAKNESS: SERVICE_SECRET is too short. Should be at least 32 characters for security.'
        assert auth_client_instance.service_id is not None, 'CONFIGURATION ERROR: SERVICE_ID is not configured'
    ...
    async def test_missing_service_credentials_handling(self):
        ...
        with patch('netra_backend.app.clients.auth_client_core.get_env') as mock_env:
            ...
            with patch('netra_backend.app.core.configuration.get_configuration') as mock_config:
                ...
                mock_config_obj.service_secret = None
...
class WebSocketAuthenticationMiddlewareTests(JWTAuthenticationTestSuite):
    ...
    async def test_websocket_auth_middleware_concurrent_connections(self, websocket_authenticator):
        ...
        with patch.object(websocket_authenticator, 'authenticate') as mock_auth:
            ...
            auth_tasks = [websocket_authenticator.authenticate_websocket(conn) for conn in connections]
    ...
    async def test_websocket_auth_with_malicious_headers(self, websocket_authenticator):
        ...
        malicious_scenarios = [{'authorization': 'Bearer valid_token\r\nX-Malicious: injected'}, {'authorization': 'Bearer valid_token\nSet-Cookie: malicious=1'}, {'authorization': f"Bearer {'x' * 10000}"}, {'authorization': "Bearer <script>alert('xss')</script>"}, {'authorization': 'Bearer %3Cscript%3Ealert%28%27xss%27%29%3C%2Fscript%3E'}, {'authorization': 'Bearer token\x00malicious'}, {'authorization': f'Bearer token{chr(31)}'}, {'authorization': ['Bearer token1', 'Bearer token2']}]
    ...
    async def test_websocket_auth_timing_attacks(self, websocket_authenticator):
        ...
        test_tokens = ['very_short', 'medium_length_token_abc123', f"very_long_token_{'x' * 1000}", 'malformed_token_without_proper_structure', 'jwt.eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJzdWIiOiIxMjM0NTY3ODkwIiwibmFtZSI6IkpvaG4gRG9lIiwiaWF0IjoxNTE2MjM5MDIyfQ.SflKxwRJSMeKKF2QT4fwpMeJf36POk6yJV_adQssw5c']
        timing_results = []
...
class TokenBlacklistFunctionalityTests(JWTAuthenticationTestSuite):
    ...
    async def test_token_blacklist_atomic_checking(self, auth_client_instance):
        ...
        async def mock_cached_token(token):
            ...
            return {'valid': True, 'user_id': 'test_user', 'email': 'test@example.com'}
    ...
    async def test_token_blacklist_cache_invalidation(self, auth_client_instance):
        ...
        auth_client_instance.token_cache.cache_token(test_token, {'valid': True, 'user_id': 'test_user', 'email': 'test@example.com'})
    ...
    async def test_concurrent_token_blacklist_operations(self, auth_client_instance):
        ...
        for i, token in enumerate(test_tokens):
            blacklist_responses[token] = i % 2 == 0
        ...
        with patch.object(auth_client_instance, '_is_token_blacklisted_atomic', side_effect=mock_blacklist_check):
            tasks = [auth_client_instance._is_token_blacklisted_atomic(token) for token in test_tokens]
            results = await asyncio.gather(*tasks)
...
class MultiUserAuthenticationIsolationTests(JWTAuthenticationTestSuite):
    ...
    async def test_multi_user_websocket_authentication_isolation(self, websocket_authenticator):
        ...
        user_scenarios = [('user_1', 'user1@example.com', ['user']), ('user_2', 'user2@example.com', ['user', 'premium']), ('admin_1', 'admin1@example.com', ['user', 'admin']), ('guest_1', 'guest1@example.com', ['guest'])]
        auth_tasks = []
        for user_id, email, permissions in user_scenarios:
            websocket = RealWebSocketConnection()
            websocket.headers = {'authorization': f'Bearer token_for_{user_id}', 'user-agent': f'client_for_{user_id}'}
            ...
            async def mock_user_auth(token, expected_user=user_id, expected_email=email, expected_perms=permissions):
                if f'token_for_{expected_user}' in token:
                    return {'user_id': expected_user, 'email': expected_email, 'permissions': expected_perms, 'authenticated': True}
                return None
            ...
            def auth_side_effect(token):
                for user_id, email, permissions in user_scenarios:
                    if f'token_for_{user_id}' in token:
                        return {'user_id': user_id, 'email': email, 'permissions': permissions, 'authenticated': True}
                return None
            mock_auth.side_effect = auth_side_effect
            concurrent_auths = [websocket_authenticator.authenticate_websocket(websocket) for websocket, user_id, email, permissions in auth_tasks]
    ...
    async def test_user_session_isolation_and_token_validation(self, auth_client_instance):
        ...
        user_tokens = {'user_1': 'token_abc_123_user1', 'user_2': 'token_def_456_user2', 'admin_1': 'token_ghi_789_admin1'}

        def mock_validation_response(token):
            if 'user1' in token:
                return {'valid': True, 'user_id': 'user_1', 'email': 'user1@example.com', 'permissions': ['user']}
            elif 'user2' in token:
                return {'valid': True, 'user_id': 'user_2', 'email': 'user2@example.com', 'permissions': ['user', 'premium']}
            elif 'admin1' in token:
                return {'valid': True, 'user_id': 'admin_1', 'email': 'admin1@example.com', 'permissions': ['user', 'admin']}
            return None
        with patch.object(auth_client_instance, '_validate_token_remote', side_effect=mock_validation_response):
            validation_tasks = [auth_client_instance.validate_token_jwt(token) for token in user_tokens.values()]
    ...
    async def test_user_authentication_cross_contamination_prevention(self, auth_client_instance):
        ...
        contamination_attempts = [('user1_token', 'user_2_data_request'), ('admin_token', 'different_user_request'), ('guest_token', 'admin_privilege_request')]
        for token, data_request in contamination_attempts:
            with patch.object(auth_client_instance, '_validate_token_remote') as mock_validate:
                if 'user1' in token:
                    mock_validate.return_value = {'valid': True, 'user_id': 'user_1', 'email': 'user1@example.com', 'permissions': ['user']}
                elif 'admin' in token:
                    mock_validate.return_value = {'valid': True, 'user_id': 'admin_user', 'email': 'admin@example.com', 'permissions': ['admin', 'user']}
                elif 'guest' in token:
                    mock_validate.return_value = {'valid': True, 'user_id': 'guest_user', 'email': 'guest@example.com', 'permissions': ['guest']}
...
class EdgeCasesAndRaceConditionsTests(JWTAuthenticationTestSuite):
    ...
    async def test_authentication_during_service_restart(self, auth_client_instance):
        ...
        restart_sequence = [httpx.ConnectError('Connection refused'), httpx.HTTPStatusError('Service Unavailable', request=MagicMock(), response=MagicMock(status_code=503)), {'valid': True, 'user_id': 'test_user', 'email': 'test@example.com', 'permissions': ['user']}]
    ...
    async def test_memory_pressure_during_authentication(self, auth_client_instance):
        ...
        async def memory_pressure_auth(token):
            large_objects.extend([b'x' * 1024 * 1024 for _ in range(10)])
            try:
                return {'valid': True, 'user_id': 'memory_test_user', 'email': 'test@example.com', 'permissions': ['user']}
            finally:
                large_objects.clear()
    ...
    async def test_authentication_with_network_instability(self, auth_client_instance):
        ...
        network_responses = [httpx.ConnectError('Network unreachable'), httpx.TimeoutException('Request timeout'), {'valid': True, 'user_id': 'network_user', 'email': 'test@example.com', 'permissions': ['user']}, httpx.ConnectError('Connection reset'), {'valid': True, 'user_id': 'network_user', 'email': 'test@example.com', 'permissions': ['user']}]
        call_count = 0
    ...
    async def test_authentication_state_consistency_under_load(self, auth_client_instance):
        ...
        def load_test_auth_response(token):
            ...
            return {'valid': True, 'user_id': f'load_user_{token_num}', 'email': f'user{token_num}@example.com', 'permissions': ['user']}
        with patch.object(auth_client_instance, '_validate_token_remote', side_effect=load_test_auth_response):
            ...
            for i in range(0, len(load_test_tokens), batch_size):
                ...
                batch_tasks = [auth_client_instance.validate_token_jwt(token) for token in batch]
