...
class RealAuthTestClient:
    ...
    async def authenticate_user(self, user_id: str, auth_method: str='test') -> str:
        ...
        try:
            auth_result = await self.auth_service.authenticate_user(user_id=user_id, auth_method=auth_method, metadata={'test': True})
...
class AuthenticationIntegrationAgentWorkflowsTests(SSotAsyncTestCase):
    ...
    async def setup_real_auth_infrastructure(self):
        ...
        self.websocket_bridge = AgentWebSocketBridge(websocket_manager=self.websocket_manager, auth_service=self.auth_service)
    ...
    async def test_jwt_authentication_agent_execution_basic(self):
        ...
        user_context = UserExecutionContext(user_id=self.test_user_id, thread_id=self.conversation_id, auth_token=jwt_token, metadata={'authenticated': True})
        ...
        result = await execution_engine.execute_authenticated_request('Analyze supply chain data with authenticated access', auth_token=jwt_token)
    ...
    async def test_jwt_token_expiration_during_long_execution(self):
        ...
        user_context = UserExecutionContext(user_id=self.test_user_id, thread_id=self.conversation_id, auth_token=short_lived_token, metadata={'expiration_test': True})
        execution_engine = UserExecutionEngine(user_context=user_context, websocket_manager=self.websocket_manager, auth_service=self.auth_service)
        start_time = datetime.now()
        result = await execution_engine.execute_authenticated_request('Perform long analysis that might outlast token expiration', auth_token=short_lived_token, timeout_seconds=10.0)
    ...
    async def test_jwt_token_validation_performance_under_load(self):
        ...
        execution_engines = []
        for i in range(5):
            user_context = UserExecutionContext(user_id=self.test_user_id, thread_id=f'{self.conversation_id}_{i}', auth_token=jwt_token, metadata={'load_test': True, 'instance': i})
            engine = UserExecutionEngine(user_context=user_context, websocket_manager=self.websocket_manager, auth_service=self.auth_service)
        ...
        async def concurrent_authenticated_execution(engine_index: int):
            ...
            result = await engine.execute_authenticated_request(f'Load test execution {engine_index}: analyze data with auth validation', auth_token=jwt_token)
            execution_time = time.time() - start_time
            return (engine_index, result, execution_time)
        concurrent_results = await asyncio.gather(*[concurrent_authenticated_execution(i) for i in range(len(execution_engines))])
    ...
    async def test_jwt_token_refresh_during_agent_workflow(self):
        ...
        user_context = UserExecutionContext(user_id=self.test_user_id, thread_id=self.conversation_id, auth_token=initial_token, metadata={'refresh_test': True})
        execution_engine = UserExecutionEngine(user_context=user_context, websocket_manager=self.websocket_manager, auth_service=self.auth_service)
        initial_result = await execution_engine.execute_authenticated_request('Start multi-step analysis requiring token refresh', auth_token=initial_token)
        ...
        continued_result = await execution_engine.execute_authenticated_request('Continue analysis with refreshed authentication', auth_token=refreshed_token)
    ...
    async def test_oauth_flow_completion_before_agent_execution(self):
        ...
        user_context = UserExecutionContext(user_id=oauth_result.get('user_id', self.test_user_id), thread_id=self.conversation_id, auth_token=oauth_token, auth_method='oauth', metadata={'oauth_provider': 'google'})
        execution_engine = UserExecutionEngine(user_context=user_context, websocket_manager=self.websocket_manager, auth_service=self.auth_service)
    ...
    async def test_oauth_token_scope_validation_during_execution(self):
        ...
        execution_engine = UserExecutionEngine(user_context=user_context, websocket_manager=self.websocket_manager, auth_service=self.auth_service)
        allowed_result = await execution_engine.execute_authenticated_request('Analyze supply chain data (within OAuth scope)', auth_token=oauth_token, required_scopes=['analyze_supply_chain'])
        restricted_result = await execution_engine.execute_authenticated_request('Delete user data (outside OAuth scope)', auth_token=oauth_token, required_scopes=['delete_data'])
    ...
    async def test_session_persistence_across_agent_executions(self):
        ...
        await self.setup_real_auth_infrastructure()
        jwt_token = await self.auth_client.authenticate_user(self.test_user_id)
        session_data = {'user_preferences': {'output_format': 'detailed', 'language': 'english'}, 'ongoing_analysis': {'type': 'supply_chain', 'stage': 'data_gathering'}, 'security_level': 'standard'}
        session_id = await self.session_manager.create_session(user_id=self.test_user_id, session_data=session_data, auth_token=jwt_token)
        user_context = UserExecutionContext(user_id=self.test_user_id, thread_id=self.conversation_id, auth_token=jwt_token, session_id=session_id, metadata={'session_test': True})
        execution_engine = UserExecutionEngine(user_context=user_context, websocket_manager=self.websocket_manager, auth_service=self.auth_service)
        result_1 = await execution_engine.execute_authenticated_request('Start supply chain analysis in this session', auth_token=jwt_token)
        result_2 = await execution_engine.execute_authenticated_request('Continue analysis from previous step in same session', auth_token=jwt_token)
        result_3 = await execution_engine.execute_authenticated_request('Finalize analysis using session preferences', auth_token=jwt_token)
    ...
    async def test_session_security_isolation_between_users(self):
        ...
        session_1_data = {'sensitive_info': 'confidential_user_1_data', 'client_secrets': {'api_key': 'secret_key_user_1'}, 'business_data': {'revenue': 1000000, 'clients': ['client_a', 'client_b']}}
        session_2_data = {'sensitive_info': 'confidential_user_2_data', 'client_secrets': {'api_key': 'secret_key_user_2'}, 'business_data': {'revenue': 2000000, 'clients': ['client_x', 'client_y']}}
        session_1_id = await self.session_manager.create_session(user_id=user_1_id, session_data=session_1_data, auth_token=token_1)
        session_2_id = await self.session_manager.create_session(user_id=user_2_id, session_data=session_2_data, auth_token=token_2)
        context_1 = UserExecutionContext(user_id=user_1_id, thread_id=f'conv_1_{self.test_id}', auth_token=token_1, session_id=session_1_id)
        engine_1 = UserExecutionEngine(user_context=context_1, websocket_manager=self.websocket_manager, auth_service=self.auth_service)
        result_1 = await engine_1.execute_authenticated_request('Analyze my confidential business data', auth_token=token_1)
        context_2 = UserExecutionContext(user_id=user_2_id, thread_id=f'conv_2_{self.test_id}', auth_token=token_2, session_id=session_2_id)
        engine_2 = UserExecutionEngine(user_context=context_2, websocket_manager=self.websocket_manager, auth_service=self.auth_service)
        result_2 = await engine_2.execute_authenticated_request('Analyze my confidential business data', auth_token=token_2)
    ...
    async def test_multi_user_concurrent_authentication_isolation(self):
        ...
        async def concurrent_authenticated_workflow(user_index: int):
            ...
            user_context = UserExecutionContext(user_id=user_id, thread_id=f'concurrent_conv_{user_index}_{self.test_id}', auth_token=token, metadata={'concurrent_test': True, 'user_index': user_index})
            execution_engine = UserExecutionEngine(user_context=user_context, websocket_manager=self.websocket_manager, auth_service=self.auth_service)
            results = []
            for op_index in range(3):
                result = await execution_engine.execute_authenticated_request(f'User {user_index} operation {op_index}: secure analysis', auth_token=token)
                results.append(result)
                await asyncio.sleep(0.1)
            return (user_index, user_id, results)
        concurrent_results = await asyncio.gather(*[concurrent_authenticated_workflow(i) for i in range(num_users)])
        successful_users = 0
