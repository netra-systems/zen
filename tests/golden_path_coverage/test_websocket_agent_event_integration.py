...
class RealWebSocketTestClient:
    ...
    async def connect(self, uri: str):
        ...
        try:
            headers = {'Authorization': f'Bearer {self.auth_token}', 'X-User-ID': self.user_id}
...
class WebSocketAgentEventIntegrationTests(SSotAsyncTestCase):
    ...
    async def test_all_five_critical_events_delivered(self):
        ...
        user_message = {'type': 'user_message', 'content': 'Help me analyze supply chain efficiency', 'conversation_id': self.conversation_id}
    ...
    async def test_event_timing_and_ordering(self):
        ...
        await self.setup_real_websocket_infrastructure()
        client = await self.create_websocket_client()
        user_message = {'type': 'user_message', 'content': 'Perform multi-step analysis with tools', 'conversation_id': self.conversation_id}
        ...
        if len(agent_events) > 1:
            ...
            if 'agent_started' in event_types and 'agent_completed' in event_types:
                ...
                completed_index = event_types.rindex('agent_completed')
    ...
    async def test_websocket_event_content_validation(self):
        ...
        await self.setup_real_websocket_infrastructure()
        client = await self.create_websocket_client()
        user_message = {'type': 'user_message', 'content': 'Analyze data and provide recommendations', 'conversation_id': self.conversation_id, 'metadata': {'test_case': 'content_validation'}}
        await client.send_message(user_message)
    ...
    async def test_websocket_event_delivery_during_long_execution(self):
        ...
        await self.setup_real_websocket_infrastructure()
        client = await self.create_websocket_client()
        user_message = {'type': 'user_message', 'content': 'Perform comprehensive supply chain analysis with multiple data sources and optimization recommendations', 'conversation_id': self.conversation_id}
        start_time = datetime.now()
        await client.send_message(user_message)
        ...
        agent_events = [event for event in events_during_execution if event.get('type', '').startswith('agent_') or event.get('type', '').startswith('tool_')]
        ...
        if len(agent_events) > 2:
            ...
            time_deltas = [(event_times[i + 1] - event_times[i]).total_seconds() for i in range(len(event_times) - 1)]
    ...
    async def test_websocket_event_delivery_with_tool_failures(self):
        ...
        await self.setup_real_websocket_infrastructure()
        client = await self.create_websocket_client()
        user_message = {'type': 'user_message', 'content': 'Search for data at invalid URL: http://definitely-does-not-exist-12345.invalid', 'conversation_id': self.conversation_id}
        await client.send_message(user_message)
        await asyncio.sleep(5)
        messages = client.get_received_messages()
        event_types = [msg.get('type') for msg in messages]
        ...
        completion_events = [event_type for event_type in event_types if event_type in ['agent_completed', 'agent_error']]
    ...
    async def test_multi_user_event_isolation_concurrent_sessions(self):
        ...
        messages = [(client_1, {'type': 'user_message', 'content': 'User 1: Analyze supply chain', 'conversation_id': f'conv_1_{self.test_id}'}), (client_2, {'type': 'user_message', 'content': 'User 2: Review financial data', 'conversation_id': f'conv_2_{self.test_id}'}), (client_3, {'type': 'user_message', 'content': 'User 3: Generate report', 'conversation_id': f'conv_3_{self.test_id}'})]
    ...
    async def test_multi_user_websocket_connection_independence(self):
        ...
        user_2_message = {'type': 'user_message', 'content': 'User 2 message after User 1 disconnected', 'conversation_id': f'conv_2_{self.test_id}'}
    ...
    async def test_websocket_connection_stability_under_load(self):
        ...
        rapid_messages = []
        for i in range(10):
            rapid_messages.append({'type': 'user_message', 'content': f'Rapid message {i + 1}: Quick analysis request', 'conversation_id': self.conversation_id, 'sequence': i + 1})
        start_time = datetime.now()
        ...
        test_message = {'type': 'user_message', 'content': 'Test message after load test', 'conversation_id': self.conversation_id}
    ...
    async def test_websocket_reconnection_and_event_continuity(self):
        ...
        await self.setup_real_websocket_infrastructure()
        client = await self.create_websocket_client()
        long_operation_message = {'type': 'user_message', 'content': 'Start comprehensive analysis that takes time', 'conversation_id': self.conversation_id}
        await client.send_message(long_operation_message)
        await asyncio.sleep(2)
        await client.disconnect()
        await asyncio.sleep(1)
        reconnected_client = await self.create_websocket_client(self.user_id)
        continuation_message = {'type': 'user_message', 'content': 'Continue previous analysis after reconnection', 'conversation_id': self.conversation_id}
    ...
    async def test_websocket_heartbeat_and_health_monitoring(self):
        ...
        while (datetime.now() - monitoring_start).total_seconds() < monitoring_duration:
            await asyncio.sleep(1)
            health_status = {'is_connected': client.is_connected, 'timestamp': datetime.now(), 'received_message_count': len(client.get_received_messages())}
            health_checks.append(health_status)
        health_test_message = {'type': 'user_message', 'content': 'Health check message', 'conversation_id': self.conversation_id}
