...
class TestIntegratedRaceConditionPrevention:
    ...
    pass
            ...
            try:
                ...
                for i in range(operation_count):
                    # Cache user token
                    token = f"token_{user_id}_{i}_{time.time()}"
                    await auth_cache.set_user_scoped(user_id, f"token_{i}", token)
                    
                    # Retrieve token
                    cached_token = await auth_cache.get_user_scoped(user_id, f"token_{i}")
    ...
    return session_results
        ...
        for i, result in enumerate(results):
            user_id = users[i]
            assert len(result['errors']) == 0, f"Errors in {user_id}: {result['errors']}"
