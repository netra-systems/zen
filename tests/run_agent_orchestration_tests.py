...
def run_performance_tests(concurrent_load: int = 10, detailed_reports: bool = False):
    ...
    print(f"Testing with concurrent load: {concurrent_load}")
    print()
...
def run_all_agent_orchestration_tests(concurrent_load: int = 10, detailed_reports: bool = False):
    ...
    print(f"Performance Tests: {'PASSED' if perf_result == 0 else 'FAILED'}")
    print(f"Error Recovery Tests: {'PASSED' if recovery_result == 0 else 'FAILED'}")
