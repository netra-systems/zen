...
class StressTestMetrics:
    ...
    def get_summary(self) -> Dict[str, Any]:
        ...
        return {'duration_seconds': duration, 'connections': {'created': self.connections_created, 'closed': self.connections_closed, 'failed': self.connections_failed, 'max_concurrent': self.max_concurrent_connections, 'success_rate': (self.connections_created - self.connections_failed) / max(1, self.connections_created)}, 'messages': {'sent': self.messages_sent, 'failed': self.messages_failed, 'throughput_per_second': self.messages_sent / max(1, duration), 'success_rate': self.messages_sent / max(1, self.messages_sent + self.messages_failed)}, 'performance': {'avg_latency_ms': sum(self.latency_samples) / max(1, len(self.latency_samples)) * 1000 if self.latency_samples else 0, 'p95_latency_ms': sorted(self.latency_samples)[int(len(self.latency_samples) * 0.95)] * 1000 if self.latency_samples else 0, 'p99_latency_ms': sorted(self.latency_samples)[int(len(self.latency_samples) * 0.99)] * 1000 if self.latency_samples else 0, 'avg_connection_duration_s': sum(self.connection_durations) / max(1, len(self.connection_durations)) if self.connection_durations else 0}, 'memory': {'initial_mb': self.memory_samples[0] if self.memory_samples else 0, 'peak_mb': max(self.memory_samples) if self.memory_samples else 0, 'final_mb': self.memory_samples[-1] if self.memory_samples else 0, 'growth_mb': self.memory_samples[-1] - self.memory_samples[0] if self.memory_samples else 0}, 'errors': dict(self.error_counts)}
...
class SimulatedWebSocket:
    ...
    def __init__(self, connection_id: str, network_profile: str='stable'):
        ...
        self.profiles = {'stable': {'latency': 0.001, 'packet_loss': 0.0, 'jitter': 0.0}, 'unstable': {'latency': 0.05, 'packet_loss': 0.05, 'jitter': 0.02}, 'poor': {'latency': 0.2, 'packet_loss': 0.1, 'jitter': 0.1}, 'terrible': {'latency': 0.5, 'packet_loss': 0.2, 'jitter': 0.3}}
...
class TestWebSocketConnectionStability:
    ...
    async def setup_stress_environment(self):
        ...
        self.heartbeat_manager = WebSocketHeartbeatManager(HeartbeatConfig(heartbeat_interval_seconds=10, heartbeat_timeout_seconds=30, max_missed_heartbeats=3))
    ...
    async def create_connection(self, user_id: str, thread_id: str, network_profile: str='stable') -> Tuple[str, SimulatedWebSocket]:
        ...
        self.ws_manager.connections[conn_id] = {'connection_id': conn_id, 'user_id': user_id, 'websocket': mock_ws, 'thread_id': thread_id, 'connected_at': datetime.now(timezone.utc), 'last_activity': datetime.now(timezone.utc), 'message_count': 0, 'is_healthy': True}
    ...
    async def test_high_concurrent_connections(self):
        ...
        for batch_start in range(0, target_connections, batch_size):
            ...
            for i in range(batch_start, min(batch_start + batch_size, target_connections)):
                ...
                thread_id = f'stress_thread_{i % 10}'
        ...
        for thread_id in range(10):
            context = AgentExecutionContext(run_id=f'stress_run_{thread_id}', thread_id=f'stress_thread_{thread_id}', user_id=f'stress_user_{thread_id}', agent_name='stress_test', retry_count=0, max_retries=1)
    ...
    async def test_network_instability_resilience(self):
            ...
            for conn_id, mock_ws in connections:
                for _ in range(10):
                    try:
                        ...
                        self.metrics.record_message_failed()
                        failures += 1
            profile_results[profile] = {'success_rate': successes / max(1, successes + failures), 'total_sent': successes, 'total_failed': failures}
        ...
        assert profile_results['unstable']['success_rate'] > 0.9, 'Unstable network should have >90% success'
        ...
        while time.time() - start_time < test_duration_seconds:
            ...
            for thread_num in range(5):
                ...
                async def send_periodic(tid):
                    try:
                        await self.ws_manager.send_to_thread(tid, {'type': 'heartbeat', 'timestamp': time.time(), 'thread': tid})
    ...
    async def test_message_buffer_overflow(self):
        ...
        for i in range(messages_per_connection):
            for thread_num in range(10):
                context = AgentExecutionContext(run_id=f'flood_{i}_{thread_num}', thread_id=f'buffer_thread_{thread_num}', user_id=f'buffer_user_{thread_num}', agent_name='flood_test', retry_count=0, max_retries=1)
