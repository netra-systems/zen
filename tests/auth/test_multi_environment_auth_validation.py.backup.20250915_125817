...
class EnvironmentAuthTester:
    ...
    def _get_environment_config(self) -> Dict[str, str]:
        ...
        configs = {'development': {'auth_service_url': 'http://localhost:8081', 'backend_url': 'http://localhost:8000', 'test_user_email': 'dev-user@test.local'}, 'staging': {'auth_service_url': 'https://auth.staging.netrasystems.ai', 'backend_url': 'https://api.staging.netrasystems.ai', 'test_user_email': 'staging-e2e-user-001@test.local'}, 'production': {'auth_service_url': 'https://auth.netrasystems.ai', 'backend_url': 'https://api.netrasystems.ai', 'test_user_email': None}}
    ...
    async def test_service_health(self) -> Dict[str, Any]:
        ...
        try:
            async with httpx.AsyncClient(timeout=10.0) as client:
                ...
                results['auth_service'] = {'accessible': True, 'healthy': response.status_code == 200, 'status_code': response.status_code, 'data': response.json() if response.status_code == 200 else None}
        except Exception as e:
            results['auth_service'] = {'accessible': False, 'error': str(e)}
        try:
            async with httpx.AsyncClient(timeout=10.0) as client:
                response = await client.get(f"{self.config['backend_url']}/health")
                results['backend_service'] = {'accessible': True, 'healthy': response.status_code == 200, 'status_code': response.status_code, 'data': response.json() if response.status_code == 200 else None}
        except Exception as e:
            results['backend_service'] = {'accessible': False, 'error': str(e)}
        return results
    ...
    def test_jwt_secret_configuration(self) -> Dict[str, Any]:
        ...
        try:
            ...
            available_sources = [source for source in expected_sources if self.env.get(source)]
            return {'unified_secret_available': True, 'unified_secret_length': len(unified_secret), 'environment_specific_available': has_env_specific, 'generic_available': has_generic, 'legacy_available': has_legacy, 'expected_sources': expected_sources, 'available_sources': available_sources, 'configuration_valid': len(available_sources) > 0, 'debug_info': debug_info}
        except Exception as e:
            return {'unified_secret_available': False, 'error': str(e)}

    async def test_token_generation_validation_cycle(self) -> Dict[str, Any]:
        """Test complete token generation and validation cycle."""
        if self.environment == 'production':
            return {'skipped': True, 'reason': 'Token generation testing not allowed in production'}
        try:
            test_email = self.config['test_user_email']
            async with httpx.AsyncClient(timeout=30.0) as client:
                response = await client.post(f"{self.config['auth_service_url']}/auth/dev-login", json={'email': test_email, 'provider': 'test'})
                if response.status_code != 200:
                    return {'success': False, 'step': 'token_generation', 'error': f'Status {response.status_code}: {response.text}'}
                token_data = response.json()
                test_token = token_data.get('access_token')
                if not test_token:
                    return {'success': False, 'step': 'token_generation', 'error': 'No access token in response'}
            async with httpx.AsyncClient(timeout=30.0) as client:
                response = await client.post(f"{self.config['auth_service_url']}/auth/validate", json={'token': test_token, 'token_type': 'access'})
            ...
            backend_validation_success = backend_validation_result is not None and backend_validation_result.get('valid', False)
            return {'success': auth_validation_success and backend_validation_success, 'token_generated': True, 'auth_service_validation': {'success': auth_validation_success, 'data': auth_validation_data}, 'backend_service_validation': {'success': backend_validation_success, 'data': backend_validation_result}, 'cross_service_consistency': auth_validation_success and backend_validation_success}
        except Exception as e:
    ...
    async def run_comprehensive_validation(self) -> Dict[str, Any]:
        ...
        validation_results = {'environment': self.environment, 'timestamp': datetime.now(timezone.utc).isoformat(), 'tests': {}}
        ...
        services_healthy = all([service_health.get('auth_service', {}).get('healthy', False), service_health.get('backend_service', {}).get('healthy', False)])
        ...
        validation_results['overall_assessment'] = {'success': overall_success, 'services_healthy': services_healthy, 'jwt_properly_configured': jwt_properly_configured, 'token_validation_works': token_cycle.get('cross_service_consistency', None), 'issues': []}
...
class TestMultiEnvironmentAuthValidation:
    ...
    async def test_environment_service_health(self, auth_tester):
        ...
        assert health_results.get('auth_service', {}).get('accessible', False), f'Auth service not accessible in {auth_tester.environment}'
        assert health_results.get('backend_service', {}).get('accessible', False), f'Backend service not accessible in {auth_tester.environment}'
        assert health_results.get('auth_service', {}).get('healthy', False), f'Auth service not healthy in {auth_tester.environment}'
        assert health_results.get('backend_service', {}).get('healthy', False), f'Backend service not healthy in {auth_tester.environment}'
    ...
    def test_environment_jwt_configuration(self, auth_tester):
        ...
        assert jwt_config.get('unified_secret_available', False), f'Unified JWT secret not available in {auth_tester.environment}'
        assert jwt_config.get('unified_secret_length', 0) >= 32, f"JWT secret too short in {auth_tester.environment}: {jwt_config.get('unified_secret_length', 0)} chars"
        assert jwt_config.get('configuration_valid', False), f"JWT configuration invalid in {auth_tester.environment}: expected {jwt_config.get('expected_sources', [])}, got {jwt_config.get('available_sources', [])}"
    ...
    async def test_environment_token_validation_consistency(self, auth_tester):
        ...
        assert token_cycle.get('token_generated', False), f"Token generation failed in {auth_tester.environment}: {token_cycle.get('error', 'Unknown error')}"
        auth_validation = token_cycle.get('auth_service_validation', {})
        assert auth_validation.get('success', False), f"Auth service failed to validate its own token in {auth_tester.environment}: {auth_validation.get('data', 'No data')}"
        backend_validation = token_cycle.get('backend_service_validation', {})
        assert backend_validation.get('success', False), f"Backend service failed to validate auth service token in {auth_tester.environment}: {backend_validation.get('data', 'No data')}"
        assert token_cycle.get('cross_service_consistency', False), f'Cross-service token validation inconsistent in {auth_tester.environment} - This indicates JWT secret mismatch!'
    ...
    async def test_comprehensive_environment_validation(self, auth_tester):
        ...
        if not overall_assessment.get('success', False):
            ...
            pytest.fail(f'Comprehensive validation failed in {auth_tester.environment}. Issues: {issues}')
...
class TestStagingSpecificValidation:
    ...
    async def test_staging_jwt_secret_regression_prevention(self, staging_tester):
        ...
        assert 'JWT_SECRET_STAGING' in available_sources or 'JWT_SECRET_KEY' in available_sources, f'Neither JWT_SECRET_STAGING nor JWT_SECRET_KEY available. Found: {available_sources}'
        if not token_cycle.get('skipped', False):
            ...
            if not cross_service_works:
                auth_validation = token_cycle.get('auth_service_validation', {})
                backend_validation = token_cycle.get('backend_service_validation', {})
                failure_details = {'auth_service_validation': auth_validation, 'backend_service_validation': backend_validation, 'jwt_configuration': jwt_config, 'environment_debug': staging_tester.jwt_manager.get_debug_info()}
                pytest.fail(f"STAGING JWT SECRET CONSISTENCY FAILURE DETECTED!\nThis is the same issue identified in the Five Whys analysis.\nAuth service validates: {auth_validation.get('success', False)}\nBackend service validates: {backend_validation.get('success', False)}\nFailure details: {json.dumps(failure_details, indent=2)}")
    ...
    async def test_staging_authentication_success_rate(self, staging_tester):
        ...
        assert success_rate >= 95, f'Staging authentication success rate too low: {success_rate}% (required: 95%+). This indicates the JWT secret consistency issue is still present.'
