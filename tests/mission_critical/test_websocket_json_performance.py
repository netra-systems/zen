...
class TestWebSocketJSONPerformance:
    ...
    def large_agent_state(self):
        ...
        large_actions = [
            {
                "id": i,
                "action": f"Action {i}: Implement optimization strategy {i}",
                ...
                "metadata": {
                    ...
                    "risk_level": i % 5,
                    "resource_requirements": [f"resource_{j}" for j in range(i % 10)]
                }
            }
            for i in range(500)
        ]
        ...
        action_plan = ActionPlanResult(
            ...
            required_approvals=[f"Approver {i}" for i in range(50)],
        ...
        large_messages = [
            {
                ...
                "content": f"Message {i}: " + "Content " * 100,  # Each message ~700 chars
        ...
        return DeepAgentState(
            ...
            quality_metrics={f"metric_{i}": i * 0.01 for i in range(100)},
            context_tracking={f"context_{i}": f"value_{i}" for i in range(200)}
        )
    ...
    async def test_async_serialization_performance_complex(self, websocket_manager, large_agent_state):
        ...
        assert duration < 5.0, f"Async serialization took {duration:.2f}s, should be < 5s"
    ...
    async def test_sync_vs_async_serialization_performance(self, websocket_manager, large_agent_state):
        ...
        assert sync_duration < 10.0, f"Sync serialization took {sync_duration:.2f}s"
        assert async_duration < 10.0, f"Async serialization took {async_duration:.2f}s"
    ...
    async def test_concurrent_async_serialization_performance(self, websocket_manager, large_agent_state):
        ...
        duration = end_time - start_time
        assert duration < 15.0, f"Concurrent serialization took {duration:.2f}s, should be < 15s"
        ...
        for i, result in enumerate(results):
            json_str = json.dumps(result)
            deserialized = json.loads(json_str)
            assert deserialized["run_id"] == f"run-concurrent-{i}"
    ...
    async def test_serialization_timeout_handling(self, websocket_manager):
        ...
        with patch.object(websocket_manager, '_serialize_message_safely') as mock_sync:
            ...
            duration = end_time - start_time
            assert 4.5 <= duration <= 6.0, f"Timeout took {duration:.2f}s, expected ~5s"
    ...
    async def test_memory_usage_during_serialization(self, websocket_manager, large_agent_state):
        ...
        for i in range(5):
            ...
            state = large_agent_state.copy_with_updates(run_id=f"memory-test-{i}")
        ...
        assert memory_growth_mb < 100, f"Memory grew by {memory_growth_mb:.2f}MB, should be < 100MB"
    ...
    def test_json_dumps_performance_comparison(self, websocket_manager, large_agent_state):
        ...
        assert standard_duration < 5.0, f"Standard JSON serialization took {standard_duration:.2f}s"
        assert compact_duration < 5.0, f"Compact JSON serialization took {compact_duration:.2f}s"
        assert unicode_duration < 5.0, f"Unicode JSON serialization took {unicode_duration:.2f}s"
    ...
    async def test_serialization_thread_pool_performance(self, websocket_manager, large_agent_state):
        ...
        tasks = [
            websocket_manager._serialize_message_safely_async(
                large_agent_state.copy_with_updates(run_id=f"pool-test-{i}")
            )
            for i in range(5)
        ]
        ...
        assert duration < 20.0, f"Thread pool serialization took {duration:.2f}s"
    ...
    def test_serialization_error_performance(self, websocket_manager):
        ...
        assert duration < 5.0, f"Error handling took {duration:.2f}s"
    ...
    async def test_websocket_send_performance_with_serialization(self, websocket_manager, large_agent_state):
        ...
        assert duration < 10.0, f"WebSocket send with serialization took {duration:.2f}s"
    ...
    def test_deep_recursion_performance(self, websocket_manager):
        ...
        assert duration < 5.0, f"Deep structure serialization took {duration:.2f}s"
    ...
    async def test_high_frequency_serialization(self, websocket_manager):
        ...
        assert duration < 10.0, f"High frequency serialization took {duration:.2f}s"
