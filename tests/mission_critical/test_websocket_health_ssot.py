...
class WebSocketHealthSSotTests(SSotAsyncTestCase):
    ...
    def setup_method(self, method):
        ...
        self.test_user_id = fhealth_test_{uuid.uuid4().hex[:8]}"
        self.test_user_id = fhealth_test_{uuid.uuid4().hex[:8]}"
        self.test_thread_id = f"health_thread_{uuid.uuid4().hex[:8]}"
        self.test_run_id = fhealth_run_{uuid.uuid4().hex[:8]}
...
"
        ...
        try:
            ...
            try:
                ...
                timestamp_parsed = datetime.fromisoformat(timestamp_str.replace('Z', '+0:0'))
                ...
                pytest.fail(fHealth check timestamp invalid: {e}")"
            ...
            if get_websocket_manager_factory" in str(e):"
                ...
                pytest.fail(fCRITICAL: Health check endpoint system failure - {e})"
                pytest.fail(fCRITICAL: Health check endpoint system failure - {e})""
        ...
        try:
            ...
            if error in config_result:
                ...
                if get_websocket_manager_factory in error_msg:
                    ...
                    pytest.fail(fCRITICAL: Configuration endpoint system error - {error_msg})
            ...
            pytest.fail(fCRITICAL: Configuration endpoint not accessible - {e})
            ...
            if get_websocket_manager_factory in str(e):
                ...
                pytest.fail(fCRITICAL: Configuration endpoint system failure - {e})
        ...
        try:
            ...
            if error in stats_result:
                ...
                if get_websocket_manager_factory in error_msg:"
                    ...
                    pytest.fail(fCRITICAL: Statistics endpoint system error - {error_msg})
                ...
                if "performance_metrics in stats_result:"
                    ...
                    for metric_name, metric_value in perf_metrics.items():
                        assert isinstance(metric_value, (int, float, dict)), fInvalid metric type for {metric_name}: {type(metric_value)}""
                ...
                assert len(found_sections) > 0, fStatistics missing expected sections: {expected_sections}"
                assert len(found_sections) > 0, fStatistics missing expected sections: {expected_sections}""
            ...
            pytest.fail(fCRITICAL: Statistics endpoint not accessible - {e})
            ...
            if get_websocket_manager_factory in str(e):
                ...
                pytest.fail(fCRITICAL: Statistics endpoint system failure - {e})
    ...
    async def test_health_endpoints_production_readiness(self):
        ...
        try:
            ...
            for endpoint in successful_endpoints:
                logger.info(f    CHECK {endpoint['endpoint']}: {endpoint['status']})"
                logger.info(f    CHECK {endpoint['endpoint']}: {endpoint['status']})""

                
            for endpoint in failed_endpoints:
                logger.warning(f"    X {endpoint['endpoint']}: {endpoint.get('error', 'unknown error')}))"
            ...
            if health_check_working:
                ...
                if system_failures > 0:
                    ...
                    pytest.fail(fCRITICAL: {system_failures} system failures in health endpoints)
            ...
            pytest.fail(fCRITICAL: Production readiness validation failed - {e})
