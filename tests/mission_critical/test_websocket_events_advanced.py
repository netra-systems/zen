...
class EventOrderValidator:
    ...
    def validate_complete_flow(self, request_id: str) -> Tuple[bool, List[str]]:
        ...
        errors.append(fFlow didnt start with agent_started: {events[0][1]})

            # Must end with completion
        last_event = events[-1][1]
        if last_event not in ["agent_completed", final_report]:
        errors.append(fFlow didnt end with completion: {last_event})
...
await notifier.send_agent_thinking(conn_id, req_id, 
await asyncio.sleep(random.uniform(0.01, 0.03))
...
logger.info(fThroughput test results:)
logger.info(formatted_string")"
logger.info(
logger.info(formatted_string")"
logger.info(
