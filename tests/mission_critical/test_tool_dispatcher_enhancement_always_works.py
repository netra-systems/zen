...
def create_test_user_context(user_id: str = None) -> UserExecutionContext:
    ...
    return UserExecutionContext(
        user_id=user_id or ftest_user_{uuid.uuid4().hex[:8]},
...
class MockTool(BaseTool):
    ...
    def _run(self, query: str) -> str:
        return fMock result for: {query}"

    async def _arun(self, query: str) -> str:
        return fMock async result for: {query}
...
""
    ...
    "
        ...
        async with tool_dispatcher_scope(self.test_user_context) as dispatcher:
            ...
            dispatcher.register_tool(test_scoped_tool", lambda x: fscoped: {x}, Test scoped tool)
...
"
        ...
        dispatcher.register_tool(event_test_tool, lambda x: fresult: {x}", "Event test tool)
    ...
    ""
        ...
        for metric in required_metrics:
            assert metric in metrics, fFactory must provide {metric} metric"
