    ...
    async def connect(self, websocket_url: str, max_retries: int = 3) -> bool:
        ...
        for attempt in range(max_retries):
            ...
            try:
                ...
                print(f"ChaosClient {self.connection_id} connected in {duration_ms:.1f}ms (attempt {attempt + 1})")
                return True
...
class TestWebSocketBridgeChaosStandalone:
    ...
    async def test_random_connection_drops_medium_chaos(self):
        ...
        clients = []
        for i in range(5):
            client = ChaosWebSocketClient(f"chaos_user_{i}", conditions)
        ...
        print(f"Medium chaos test completed: {successful_clients}/{len(clients)} clients succeeded, "
    ...
    pass
        ...
        for client in clients:
            ...
            for i in range(5):
                message = {
                    "type": "user_message",
                    "content": f"High chaos message {i+1}",
                    "timestamp": time.time()
                }
        ...
        print(f"High chaos test completed: {functioning_clients}/{len(clients)} clients functioning, "
    ...
    async def test_network_latency_injection(self):
        ...
        clients = []
        for i in range(3):
            client = ChaosWebSocketClient(f"latency_user_{i}", conditions)
            success = await client.connect("ws://mock-server/ws")
            if success:
                clients.append(client)
        ...
        if response_times:
            ...
            print(f"Latency test results: avg response {avg_response_time:.2f}s, "
    ...
    pass
        ...
        print(f"Rapid cycles: {successful_reconnections}/15 successful ({success_rate:.2%}), "
              f"fast reconnections: {fast_reconnections} ({fast_rate:.2%})")
    ...
    async def test_comprehensive_chaos_resilience(self):
        ...
        clients = []
        for i in range(5):
            client = ChaosWebSocketClient(f"comprehensive_user_{i}", conditions)
            success = await client.connect("ws://mock-server/ws")
            if success:
                clients.append(client)
        ...
        for client in clients[:3]:
            ...
            for i in range(8):
                message = {
                    "type": "user_message",
                    "content": f"Comprehensive test message {i+1}",
                    "timestamp": time.time()
                }
        ...
        print(f"Comprehensive chaos results: "
              f"{functioning_clients}/{len(clients)} clients functioning ({client_function_rate:.2%}), "
              f"{total_messages} total messages, "
              f"{total_chaos_events} chaos events")
...
Running {test_method.__name__}")
            ...
            results.append(f"FAIL: {test_method.__name__} - ERROR: {e}")
