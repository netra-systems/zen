...
class TestWebSocketSSOTValidation:
    ...
    async def test_user_isolation_25_concurrent_sessions(self):
        ...
        logger.info("🧪 Testing user isolation with 25+ concurrent sessions")
        ...
        for user_idx in range(users_count):
            ...
            for conn_idx in range(connections_per_user):
                ...
                websocket = MockWebSocket(user_id, f"conn_{user_idx}_{conn_idx}")
        ...
        logger.info(f"⏱️ Created {total_connections} connections in {connection_time:.2f}s")
        ...
        for user_id, connections in user_connections.items():
            ...
            assert len(user_conns) == connections_per_user, \
                f"User {user_id} should have {connections_per_user} connections, got {len(user_conns)}"
            
            # Verify connections belong only to this user
            for conn_id in user_conns:
                conn_info = manager.connections.get(conn_id)
                assert conn_info is not None, f"Connection {conn_id} not found"
                assert conn_info['user_id'] == user_id, \
                    f"Connection {conn_id} belongs to {conn_info['user_id']}, expected {user_id}"
        ...
        for user_id, expected_message in test_messages:
            ...
            for conn_id in user_conns:
                ...
                assert len(websocket.sent_messages) >= 1, \
                    f"User {user_id} did not receive message"
                
                # Check message content
                received_message = websocket.sent_messages[-1]['data']
                assert received_message['user_id'] == user_id, \
                    f"Message routing error: {received_message['user_id']} != {user_id}"
        
        # Verify no cross-user contamination
        other_users = list(user_connections.keys())[5:]  # Users who shouldn't receive messages
        for user_id in other_users:
            ...
            for conn_id in user_conns:
                ...
                assert len(websocket.sent_messages) == 0, \
                    f"User {user_id} received messages not intended for them"
        
        logger.info("✅ User isolation with 25+ concurrent sessions PASSED")
    ...
    async def test_message_routing_accuracy(self):
        ...
        for user_id, threads in users_threads.items():
            ...
            for thread_id in threads:
                websocket = MockWebSocket(user_id, f"conn_{user_id}_{thread_id}")
        ...
        await manager.send_to_thread('thread_1_a', {
            'type': 'thread_message',
        ...
        for user_id in ['user_2', 'user_3']:
            for thread_id, websocket in websockets[user_id].items():
                assert len(websocket.sent_messages) == 0, \
                    f"Thread {thread_id} of {user_id} incorrectly received message"
        
        logger.info("✅ Message routing accuracy PASSED")
    ...
    pass
        ...
        for i in range(manager.MAX_CONNECTIONS_PER_USER):
            websocket = MockWebSocket(user_id, f"conn_{i}")
            conn_id = await manager.connect_user(
                user_id=user_id,
                websocket=websocket,
        ...
        logger.info("✅ Connection limits enforcement PASSED")

    @pytest.mark.asyncio
    async def test_performance_response_time(self):
        """Test response time < 2 seconds for 10 concurrent users."""
        logger.info("🧪 Testing performance response time < 2 seconds")
        ...
        for i in range(concurrent_users):
            ...
            websocket = MockWebSocket(user_id, f"perf_conn_{i}")
        ...
        for user_info in user_data:
            message = {
                ...
                'content': f"Performance test message for {user_info['user_id']}",
                'timestamp': datetime.now(timezone.utc).isoformat()
            }
        ...
        assert total_time < 2.0, f"Response time {total_time:.2f}s exceeds 2 second requirement"
        
        # Verify all messages were delivered
        for user_info in user_data:
            websocket = user_info['websocket']
            assert len(websocket.sent_messages) == 1, \
                f"User {user_info['user_id']} did not receive message"
        
        logger.info(f"✅ Performance response time: {total_time:.3f}s < 2.0s PASSED")
    ...
    pass
        ...
        assert total_events >= 5, f"Expected 5 events, but found {total_events}"
        
        logger.info("✅ WebSocket event flow validation PASSED")

    @pytest.mark.asyncio
    async def test_cleanup_stale_connections(self):
        """Test cleanup of stale connections."""
        logger.info("🧪 Testing cleanup of stale connections")
        ...
        logger.info("✅ Cleanup of stale connections PASSED")
    ...
    pass
        ...
        for cycle in range(cycles):
            ...
            for i in range(connections_per_cycle):
                ...
                websocket = MockWebSocket(user_id, f"memory_conn_{cycle}_{i}")
                
                conn_id = await manager.connect_user(user_id, websocket, f"memory_thread_{cycle}_{i}")
    ...
    async def test_heartbeat_performance(self):
        ...
        users = []
        for i in range(5):
            user_id = f"heartbeat_user_{i}"
            websocket = MockWebSocket(user_id, f"heartbeat_conn_{i}")
            
            conn_id = await manager.connect_user(user_id, websocket, f"heartbeat_thread_{i}")
        ...
        assert ping_time < 1.0, f"Ping time {ping_time:.2f}s too slow"
        ...
        logger.info(f"Successful pings: {successful_pings}/{len(results)}")
        ...
        logger.info("✅ Heartbeat performance PASSED")
    ...
    pass
        ...
        logger.info("✅ Thread-based message isolation PASSED")

    @pytest.mark.asyncio
    async def test_factory_based_isolation_pattern(self):
        """Test factory-based isolation pattern validation."""
        logger.info("🧪 Testing factory-based isolation pattern")
        ...
        logger.info("✅ Factory-based isolation pattern PASSED")
    ...
    pass
        ...
        for scenario in test_scenarios:
            ...
            for i, thread_id in enumerate(threads):
                websocket = MockWebSocket(user_id, f"conn_{user_id}_{i}")
        ...
        for scenario in test_scenarios:
            ...
            message = {
                ...
                'user_id': user_id,
                'content': f'Integration test message for {user_id}'
            }
        ...
        for conn in all_connections[:3]:  # First 3 connections
            ...
            message = {
                ...
                'thread_id': thread_id,
                'content': f'Thread integration test for {thread_id}'
            }
        ...
        # Verify performance
        assert total_time < 3.0, f"Integration test took {total_time:.2f}s, should be < 3.0s"
        ...
        for user_id in [s["user_id"] for s in test_scenarios]:
            assert user_message_counts.get(user_id, 0) > 0, \
                f"User {user_id} did not receive any messages"
        
        logger.info(f"✅ Comprehensive integration test completed in {total_time:.3f}s")

    def test_websocket_manager_singleton_isolation(self):
        """Test WebSocket manager singleton doesn't cause isolation issues."""
        logger.info("🧪 Testing WebSocket manager singleton isolation")
        ...
        logger.info("✅ WebSocket manager singleton isolation PASSED")
