...
class TestFixtureScopeRegression:
    ...
    @pytest.mark.asyncio
    @pytest.mark.integration
    async def test_multiple_function_scoped_fixtures_interaction(
        self,
        real_services,
        real_postgres,
        real_redis,
    ...
    @pytest.mark.asyncio
    @pytest.mark.integration
    async def test_websocket_and_http_fixture_compatibility(
        self,
        real_services,
        real_websocket_client,
        real_http_client
    ...
    @pytest.mark.asyncio
    @pytest.mark.integration
    async def test_connection_fixture_scope_compatibility(
        self,
        real_services,
        websocket_connection
    ):
    ...
    @pytest.mark.asyncio  
    @pytest.mark.integration
    async def test_api_client_fixture_compatibility(
        self,
        real_services,
        api_client
    ):
    ...
    @pytest.mark.asyncio
    @pytest.mark.integration
    async def test_data_fixture_scope_compatibility(
        self,
        real_services,
        test_user,
        test_organization,
    ...
    @pytest.mark.asyncio
    @pytest.mark.integration
    async def test_conversation_fixture_deep_dependency_chain(
        self,
        real_services,
        test_user,
        test_organization,  
    ...
    async def test_fixture_cleanup_isolation(self, real_services):
        ...
        test_key = f"cleanup_test_{time.time()}"
    ...
    async def test_concurrent_fixture_usage(self, real_services):
        ...
        async def concurrent_operation(operation_id: int):
            ...
            db_result = await real_services.postgres.fetchval(
                "SELECT $1::text as result", f"concurrent_{operation_id}"
            )
        ...
        for i, result in enumerate(results):
            ...
            assert result["db_result"] == f"concurrent_{i}"
            assert result["redis_result"] == f"value_{i}"
    ...
    async def test_fixture_scope_performance_impact(self, real_services):
        ...
        for i in range(10):
            await real_services.postgres.fetchval("SELECT $1::text as result", f"perf_test_{i}")
    ...
    async def test_test_context_fixture_compatibility(self):
        ...
        for i, context in enumerate(contexts):
            # Test event capture
            test_event = {
                "type": f"context_fixture_test_{i}",
                "user_id": context.user_context.user_id,
                "data": f"context_data_{i}"
            }
            context.event_capture.capture_event(test_event)
            
            # Verify isolation
            assert len(context.event_capture.events) == 1
            assert f"context_fixture_test_{i}" in context.event_capture.event_types
