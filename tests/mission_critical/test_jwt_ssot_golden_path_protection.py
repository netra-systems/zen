...
GOLDEN PATH: Protect $"500K" plus ARR functionality during SSOT consolidation
...
class JWTSSOTGoldenPathProtectionTests(SSotAsyncTestCase):
    ...
    async def test_login_to_ai_chat_flow_works_with_ssot_jwt(self):
        ...
        CRITICAL: This is the core $"500K" plus ARR user flow.
...
"
        ...
        with patch('netra_backend.app.clients.auth_client_core.auth_client') as mock_auth_client:
            ...
            self.assertLess(total_execution_time, 30.0,
                          fGolden Path too slow: {total_execution_time:."2f"}s > "30s"")"
    ...
    ""
        ...
        with patch('netra_backend.app.clients.auth_client_core.auth_client') as mock_auth_client:
            ...
            self.assertTrue(all(uid == self.golden_path_user["user_id] for uid in user_ids),"
                          fInconsistent user IDs across services: {user_ids})
    ...
    """
        ...
        with patch('netra_backend.app.clients.auth_client_core.auth_client') as mock_auth_client:
            ...
            self.assertLess(avg_validation_time, 0.5,
                          fAverage JWT validation too slow: {avg_validation_time:."3f"}s)""

            self.assertLess(max_validation_time, 1.0,
                          fMax JWT validation too slow: {max_validation_time:."3f"}s)
    ...
    async def test_websocket_events_delivery_with_ssot_auth(self):
        ...
        with patch('netra_backend.app.clients.auth_client_core.auth_client') as mock_auth_client:
            ...
            self.assertEqual(len(events_delivered), len(critical_websocket_events),
                           fMissing WebSocket events: expected {len(critical_websocket_events)}, got {len(events_delivered)})
            ...
            for required_event in critical_websocket_events:
                self.assertIn(required_event, delivered_event_types,
                            fCritical WebSocket event not delivered: {required_event})
        ...
        with patch('netra_backend.app.clients.auth_client_core.auth_client') as mock_auth_client:
            ...
            for i, token in enumerate(existing_tokens):
                try:
                    ...
                    self.assertTrue(result.valid, fToken format {i} failed validation)
                    self.record_metric(flegacy_token_format_{i}_valid, True)
        ...
        with patch('netra_backend.app.clients.auth_client_core.auth_client') as mock_auth_client:
            ...
            for token in invalid_tokens:
                try:
                    ...
                    self.assertFalse(result.valid, fInvalid token was accepted: {token})
