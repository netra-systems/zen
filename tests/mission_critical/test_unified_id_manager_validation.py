        ...
        for module_path in production_files:
            try:
                ...
                if "takes 1 positional argument but 2 were given" in str(e):
                    pytest.fail(f"{module_path} still has 2-argument bug: {e}")
...
class TestConcurrentUsage:
    ...
    return [
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=10) as executor:
            futures = [
                executor.submit(generate_ids, f"thread_{i}", 100)
                for i in range(10)
            ]
    ...
    return UnifiedIDManager.generate_run_id(thread_id)
        ...
        # Generate IDs concurrently
        tasks = [
            generate_id_async(f"async_thread_{i}")
            for i in range(100)
        ]
...
class TestStartupValidation:
    ...
    def test_critical_methods_exist(self):
        ...
        for method_name, expected_args in critical_methods:
            assert hasattr(UnifiedIDManager, method_name), \
                f"Critical method {method_name} missing"
            ...
            assert param_count <= expected_args, \
                f"{method_name} has wrong number of required parameters"
    ...
    pass
        ...
        # Test normalization
        normalized = UnifiedIDManager.normalize_thread_id(f"thread_{test_thread}")
