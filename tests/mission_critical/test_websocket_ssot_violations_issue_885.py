...
"
        ...
        for import_path in import_attempts:
            ...
            try:
                ...
                if hasattr(module, class_name):
                    ...
                    self.violations_found.append(fMultiple implementation found: {import_path})"
        ...
        self.assertGreater(
            ...
            fSSOT VIOLATION DETECTED: Found {len(manager_implementations)} WebSocket manager implementations. 
            fShould be exactly 1 after SSOT consolidation. 
            fImplementations: {[impl['path'] for impl in manager_implementations]}""
        )

        logger.error(fSSOT VIOLATION CONFIRMED: {len(manager_implementations)} manager implementations found)
...
""
        ...
        self.assertGreater(
            ...
            fFACTORY PATTERN SSOT VIOLATION: Found {len(factory_patterns)} factory patterns. 
            fShould be exactly 1 canonical pattern. 
            fFactories: {[pattern['path'] for pattern in factory_patterns]}""
        )

        logger.error(fFACTORY SSOT VIOLATION CONFIRMED: {len(factory_patterns)} factory patterns found)
...
""
                        ...
                        for check_module in modules_to_test:
                            if check_module != module_name and check_module in content:
                                circular_imports_detected.append({
                                    ...
                                    'evidence': fFound '{check_module}' in {module_name}
                                }
                                self.violations_found.append(f"Circular import: {module_name} -> {check_module})

            except (ImportError, FileNotFoundError) as e:
                logger.debug(fModule test failed: {module_name} - {e}")

        # ASSERTION THAT SHOULD FAIL: Circular imports exist
        self.assertGreater(
            len(circular_imports_detected), 0,
            fCIRCULAR IMPORT VIOLATIONS DETECTED: Found {len(circular_imports_detected)} potential circular imports. 
            fShould be 0 after SSOT consolidation. "
            f"Violations: {[ci['from_module'] + ' -> ' + ci['imports_module'] for ci in circular_imports_detected]}
        )

        logger.error(fCIRCULAR IMPORT VIOLATIONS CONFIRMED: {len(circular_imports_detected)} detected)
    ...
    ""
        ...
        self.assertGreater(
            ...
            fbut found {len(deprecation_warnings)} warnings. 
            fLegacy import paths should be properly deprecated during SSOT migration.
        )

        logger.error(fDEPRECATION WARNING VIOLATION: {len(deprecation_warnings)} warnings found")"
    ...
    async def test_user_isolation_violations_exist(self):
        ...
        try:
            ...
            isolation_violations.append(fCannot import canonical factory: {e})

        # ASSERTION THAT SHOULD FAIL: User isolation violations exist
        self.assertGreater(
            len(isolation_violations), 0,
            fUSER ISOLATION VIOLATIONS DETECTED: Found {len(isolation_violations)} violations. 
            fShould be 0 after proper user isolation implementation. ""
            fViolations: {isolation_violations}
        )

        logger.error(fUSER ISOLATION VIOLATIONS CONFIRMED: {len(isolation_violations)} violations)
    ...
    ""
            ...
            try:
                ...
                if hasattr(module, class_name):
                    ...
                    if hasattr(component_class, '_instance'):
                        ...
                        self.violations_found.append(fSingleton violation: {component_path})

                    if hasattr(component_class, 'get_instance'):
                        singleton_violations.append(fSingleton get_instance method: {component_path})
                        self.violations_found.append(fSingleton violation: {component_path}")"
                    ...
                    try:
                        ...
                        if instance1 is instance2:
                            singleton_violations.append(fSame instance returned: {component_path})
                            self.violations_found.append(fSingleton violation: {component_path})
                    except Exception:
        ...
        self.assertGreater(
            ...
            fShould be 0 after factory pattern implementation. "
            fViolations: {singleton_violations}
        )

        logger.error(fSINGLETON VIOLATIONS CONFIRMED: {len(singleton_violations)} violations)"
    ...
    def tearDown(self):
        ...
        if self.violations_found:
            ...
            logger.error(=*80)
            logger.error(fTOTAL VIOLATIONS DETECTED: {len(self.violations_found)})
            logger.error(="*80)"
