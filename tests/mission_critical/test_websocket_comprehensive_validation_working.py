...
Required WebSocket Events (MANDATORY):
    1.0agent_started - User must see agent began processing 
2.0agent_thinking - Real-time reasoning visibility
3.0tool_executing - Tool usage transparency  
4.0tool_completed - Tool results display
5.0agent_completed - User must know when response is ready
...
user_id, thread_id, agent_thinking,""
{"agent_name: agent_name, run_id: run_id, reasoning: Processing request.0.00.0}"
if not success:
                return False
...
await asyncio.sleep(0.1)
        ...
        # Validate sequence
results = self.test_harness.get_comprehensive_results()
assert results[validation_passed"], fSequence validation failed: {results['validation_failures']}"
