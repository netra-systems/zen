...
class TestReportingAgentSSOTViolations:
 ...
 def real_context():
    ...
    pass
        ...
        assert "await websocket.send_json" not in source_code, \
        ...
        for method in infrastructure_methods:
            if method in base_methods and method in agent_methods:
                ...
                if agent_impl and base_impl and agent_impl != base_impl:
                    pytest.fail(f"VIOLATION: Method {method} duplicates BaseAgent functionality")
...
class TestReportingAgentComplexScenarios:
    ...
    pass
        ...
        for i in range(10):
            ...
            ctx.db_session = Magic            ctx.metadata = {
                ...
                "triage_result": f"triage_{i}",
                "user_request": f"request_{i}"
            }
            contexts.append(ctx)
    ...
    return await agent.execute(ctx)
        ...
        for i, result in enumerate(results):
            ...
            assert f"Report for {expected_user}" in str(result), \
                f"VIOLATION: Context isolation broken - wrong user data in result {i}"
    ...
    pass
        ...
        assert object_growth < 1000, \
            f"VIOLATION: Memory leak detected - {object_growth} objects retained"
    ...
    pass
            ...
            for i in range(50):
                with patch.object(agent, '_execute_reporting_llm_with_observability') as mock_llm:
                    mock_llm.return_value = f'{{"report": "Report {i}"}}'
    ...
    return results
        ...
        assert len(errors) == 0, f"VIOLATION: Race conditions detected - {len(errors)} errors"
