...
class DockerStabilityValidator:
    ...
    def __init__(self):
        self.test_id = f"stability_validation_{int(time.time() * 1000)}"
    ...
    def setup_validation(self) -> bool:
        ...
        try:
            ...
            logger.info(f"Docker stability validation setup complete. Test ID: {self.test_id}")
            return True
    ...
    def cleanup_validation(self) -> bool:
        ...
        try:
            ...
            test_leaked_containers = [c for c in leaked_containers if self.test_id in c]
            test_leaked_networks = [n for n in leaked_networks if self.test_id in n]
            test_leaked_volumes = [v for v in leaked_volumes if self.test_id in v]
            
            if test_leaked_containers or test_leaked_networks or test_leaked_volumes:
                logger.warning(f"Resource leaks detected - Containers: {test_leaked_containers}, "
                             f"Networks: {test_leaked_networks}, Volumes: {test_leaked_volumes}")
                return False
    ...
    def run_comprehensive_validation(self) -> Dict[str, Any]:
        ...
        try:
            ...
            for scenario_name, scenario_func in scenarios:
                ...
                try:
                    ...
                    if not result.success:
                        ...
                        logger.error(f"Scenario failed: {scenario_name} - {result.errors}")
                    else:
                        logger.info(f"Scenario passed: {scenario_name} ({result.duration:.2f}s)")
                        
                except Exception as e:
                    logger.error(f"Scenario exception: {scenario_name} - {e}")
                    overall_success = False
            ...
            logger.error(f"Validation suite exception: {e}")
    ...
    def _validate_docker_lifecycle_management(self) -> ValidationResult:
        ...
        try:
            ...
            container_name = f"{self.test_id}_lifecycle_test"
            ...
            if create_result.returncode != 0:
                errors.append(f"Failed to create container: {create_result.stderr}")
            ...
            if inspect_result.returncode != 0:
                errors.append(f"Failed to inspect container: {inspect_result.stderr}")
            else:
                ...
                if container_info['State']['Status'] != 'running':
                    errors.append(f"Container not running: {container_info['State']['Status']}")
            ...
            if stop_result.returncode != 0:
                errors.append(f"Failed to stop container: {stop_result.stderr}")
            else:
                metrics['container_stop_time'] = stop_time
                
                if stop_time > 15:
                    warnings.append(f"Container stop took longer than expected: {stop_time:.2f}s")
            ...
            if rm_result.returncode != 0:
                errors.append(f"Failed to remove container: {rm_result.stderr}")
            else:
        ...
        try:
            ...
            def concurrent_operations(worker_id):
                ...
                for op_id in range(operations_per_worker):
                    container_name = f"{self.test_id}_concurrent_{worker_id}_{op_id}"
            ...
            successful_ops = len([op for op in operation_results if op.get('success', False)])
            ...
            if success_rate < 0.85:
                errors.append(f"Success rate too low: {success_rate:.2%} (expected >= 85%)")
            
            if concurrent_duration > 120:
                warnings.append(f"Concurrent operations took longer than expected: {concurrent_duration:.2f}s")
    ...
    def _validate_memory_limits_enforcement(self) -> ValidationResult:
        ...
        try:
            ...
            if create_result.returncode != 0:
                errors.append(f"Failed to create container with memory limit: {create_result.stderr}")
            ...
            if inspect_result.returncode != 0:
                errors.append(f"Failed to inspect container memory: {inspect_result.stderr}")
            else:
                ...
                if actual_memory != expected_memory:
                    errors.append(f"Memory limit incorrect: expected {expected_memory}, got {actual_memory}")
    ...
    def _validate_rate_limiter_functionality(self) -> ValidationResult:
        ...
        try:
            ...
            if success_rate < 0.9:
                errors.append(f"Success rate too low during rate limiting: {success_rate:.2%}")
            ...
            if max_concurrent > 2:
                errors.append(f"Concurrent limit not enforced: observed {max_concurrent}, limit is 2")
    ...
    def _validate_safe_container_removal(self) -> ValidationResult:
        ...
        try:
            ...
            graceful_container = f"{self.test_id}_graceful"
            ...
            if create_result.returncode != 0:
                errors.append(f"Failed to create graceful container: {create_result.stderr}")
            ...
            if stop_result.returncode != 0:
                errors.append(f"Graceful stop failed: {stop_result.stderr}")
            else:
                metrics['graceful_stop_time'] = graceful_time
                
                if graceful_time > 15:
                    warnings.append(f"Graceful stop took longer than expected: {graceful_time:.2f}s")
                ...
                if inspect_result.returncode == 0:
                    ...
                    if exit_code != 0:
                        warnings.append(f"Container did not exit gracefully (exit code: {exit_code})")
            
            # Test unresponsive container handling
            unresponsive_container = f"{self.test_id}_unresponsive"
            ...
            if create_result.returncode != 0:
                errors.append(f"Failed to create unresponsive container: {create_result.stderr}")
            else:
                ...
                if stop_result.returncode != 0:
                    errors.append(f"Force stop failed: {stop_result.stderr}")
                else:
                    metrics['force_stop_time'] = force_time
                    
                    # Should take at least the timeout period plus some kill time
                    if force_time < 3:
                        warnings.append(f"Force stop completed too quickly: {force_time:.2f}s")
                    elif force_time > 10:
                        warnings.append(f"Force stop took too long: {force_time:.2f}s")
            ...
            for container in [graceful_container, unresponsive_container]:
                if container in self.created_containers:
                    ...
                    if rm_result.returncode == 0:
                        ...
                        errors.append(f"Failed to remove container {container}: {rm_result.stderr}")
    ...
    def _validate_resource_leak_prevention(self) -> ValidationResult:
        ...
        try:
            ...
            for i in range(5):
                # Create resources
                container_name = f"{self.test_id}_leak_test_{i}"
                network_name = f"{self.test_id}_leak_net_{i}"
                volume_name = f"{self.test_id}_leak_vol_{i}"
            ...
            leaked_containers = [c for c in container_changes if self.test_id in c]
            leaked_networks = [n for n in network_changes if self.test_id in n]
            leaked_volumes = [v for v in volume_changes if self.test_id in v]
            ...
            if leaked_containers:
                errors.extend([f"Leaked container: {c}" for c in leaked_containers])
            
            if leaked_networks:
                errors.extend([f"Leaked network: {n}" for n in leaked_networks])
                
            if leaked_volumes:
                errors.extend([f"Leaked volume: {v}" for v in leaked_volumes])
        ...
        try:
            ...
            for i in range(20):
                ...
                if i % 5 == 4:  # Check responsiveness every 5 operations
                    ...
                    responsiveness_tests.append((f'stress_{i}', resp_time))
                    
                    if resp_result.returncode != 0:
                        errors.append(f"Docker daemon unresponsive during stress test at operation {i}")
            ...
            # Analyze results
            successful_ops = len([op for op in stress_operations if op['success']])
            success_rate = successful_ops / len(stress_operations) if stress_operations else 0
            
            avg_op_time = sum(op['time'] for op in stress_operations if op['success']) / successful_ops if successful_ops > 0 else 0
            ...
            if success_rate < 0.9:
                errors.append(f"Stress test success rate too low: {success_rate:.2%}")
            
            if post_stress_time > baseline_time * 3:
                warnings.append(f"Post-stress response time degraded significantly: {post_stress_time:.2f}s vs {baseline_time:.2f}s baseline")
        ...
        try:
            ...
            for scenario_name, scenario_func in stress_scenarios:
                ...
                try:
                    ...
                    logger.info(f"Stress scenario completed: {scenario_name} "
                              f"({result.success_rate:.1%} success rate, {scenario_duration:.2f}s)")
                    
                except Exception as e:
                    logger.error(f"Stress scenario failed: {scenario_name} - {e}")
            ...
            if stress_results:
                ...
                if overall_success_rate < 0.8:
                    errors.append(f"Overall stress test success rate too low: {overall_success_rate:.2%}")
                
                if avg_success_rate < 0.85:
                    warnings.append(f"Average scenario success rate low: {avg_success_rate:.2%}")
            else:
    ...
    def _stress_container_lifecycle(self) -> StressTestResult:
        ...
        def lifecycle_operation(op_id):
            ...
            container_name = f"{self.test_id}_stress_lifecycle_{op_id}"
            ...
            try:
                ...
                if create_result.returncode == 0:
                    ...
                    if all(r.returncode == 0 for r in [create_result, stop_result, rm_result]):
                        ...
                        errors.append(f"Operation {op_id} partial failure")
                else:
                    errors.append(f"Operation {op_id} create failed: {create_result.stderr}")
                
            except Exception as e:
                errors.append(f"Operation {op_id} exception: {str(e)}")
            finally:
                with lock:
                    concurrent_count -= 1
    ...
    def _stress_network_operations(self) -> StressTestResult:
        ...
        for i in range(total_operations):
            network_name = f"{self.test_id}_stress_net_{i}"
            ...
            try:
                ...
                if create_result.returncode == 0:
                    ...
                    container_name = f"{self.test_id}_net_container_{i}"
                    ...
                    if rm_result.returncode == 0:
                        ...
                        errors.append(f"Failed to remove network {network_name}")
                else:
                    errors.append(f"Failed to create network {network_name}")
                    
            except Exception as e:
                errors.append(f"Network operation {i} exception: {str(e)}")
    ...
    def _stress_volume_operations(self) -> StressTestResult:
        ...
        for i in range(total_operations):
            volume_name = f"{self.test_id}_stress_vol_{i}"
            ...
            try:
                ...
                if create_result.returncode == 0:
                    ...
                    container_name = f"{self.test_id}_vol_container_{i}"
                    container_result = self._execute_docker_command([
                    ...
                    if rm_result.returncode == 0:
                        ...
                        errors.append(f"Failed to remove volume {volume_name}")
                else:
                    errors.append(f"Failed to create volume {volume_name}")
                    
            except Exception as e:
                errors.append(f"Volume operation {i} exception: {str(e)}")
        ...
        def concurrent_operation(op_id):
            ...
            try:
                ...
                if result.returncode == 0:
                    ...
                    errors.append(f"High concurrency operation {op_id} failed")
                
            except Exception as e:
                errors.append(f"High concurrency operation {op_id} exception: {str(e)}")
            finally:
                with lock:
                    concurrent_count -= 1
...
class DockerStabilityValidationTestSuite(unittest.TestCase):
    ...
    def test_comprehensive_docker_stability_validation(self):
        ...
        logger.info(f"Total Duration: {report['validation_summary']['total_duration_seconds']}s")
        logger.info(f"Tests Passed: {report['test_results']['passed_tests']}/{report['test_results']['total_tests']}")
...
=== PERFORMANCE METRICS ===")
        ...
        logger.info(f"Peak CPU Usage: {perf['peak_cpu_usage_percent']}%")
        logger.info(f"Average Test Duration: {perf['avg_test_duration']}s")
...
Detailed report saved to: {report_path}")
        ...
        self.assertTrue(report['validation_summary']['overall_success'], 
                       f"Docker stability validation failed. See report at {report_path}")
...
" + "="*60)
    ...
    print(f"Duration: {report['validation_summary']['total_duration_seconds']}s")
    print(f"Tests: {report['test_results']['passed_tests']}/{report['test_results']['total_tests']} passed")
    print(f"Success Rate: {report['test_results']['success_rate']}%")
    
    # Save report
    output_file = args.output_file or f"docker_stability_report_{int(time.time())}.json"
