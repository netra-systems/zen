...
""
    ...
    ""
        ...
        if has_setup_method and not calls_super_setup:
            ...
            details = fClass {class_name} inherits from test base class but has no setup_method()
            remediation_steps = [
...
class WebSocketHandlerSetupInheritanceTests(SSotBaseTestCase):
    ...
    def test_websocket_handler_classes_discovered(self):
        ...
        for expected_class in expected_classes:
            assert expected_class in discovered_names, (
                fExpected test class '{expected_class}' not found. 
    ...
    def _format_violation_report(self, violations: List[SetupMethodInheritanceViolation) -> str:
        ...
        report_lines.extend([
            f\n{'='*80},
            fTOTAL VIOLATIONS: {len(violations)},
            fACTION REQUIRED: Fix all violations before proceeding,
            f"{'='*80}\n"
        ]
