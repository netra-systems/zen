...
class DatabaseManagerSSOTConsolidationTests(SSotBaseTestCase):
    ...
    """
        ...
        logger.info(fFound DatabaseManager classes in: {database_manager_locations})
...
""
        ...
        logger.info(fFound DatabaseManager import patterns: {len(import_patterns)})
    ...
    """
        ...
        if len(database_manager_locations) != 1:
            ...
            logger.info(fCanonical DatabaseManager location: {canonical_location}")"
        ...
        if len(unique_sources) > 1:
            consolidation_success = False
            consolidation_issues.append(
                fMultiple import sources: {list(unique_sources)}
            )
                ...
                try:
                    ...
                    for node in ast.walk(tree):
                        ...
                        if import_info:
                            ...
                            logger.debug(fFound DatabaseManager import: {import_info})
                            
                except (SyntaxError, UnicodeDecodeError) as e:
                    logger.warning(fCould not parse {py_file}: {e}")"
                except Exception as e:
                    logger.warning(fError scanning {py_file}: {e})
