...
class FirstMessageEventValidator:
    ...
    def _validate_sequence(self) -> Tuple[bool, List[str]]:
        ...
        for required in self.REQUIRED_EVENT_SEQUENCE:
            if required not in received_types:
                errors.append(fMissing required event: {required})
        ...
        if not errors:
            ...
            for i in range(1, len(sequence_indices)):
                if sequence_indices[i] < sequence_indices[i-1]:
                    errors.append(fEvent sequence violation at position {i})
    ...
    def _validate_event_timing(self) -> Tuple[bool, List[str]]:
        ...
        for timestamp, event_type, _ in self.event_timeline:
            if event_type == "message_received and timestamp * 1000 > self.SLO_MESSAGE_RECEIVED_MS:"
                errors.append(fmessage_received took {timestamp*1000:.0f}ms (SLO: {self.SLO_MESSAGE_RECEIVED_MS}ms))
            elif event_type == agent_started and timestamp * 1000 > self.SLO_AGENT_STARTED_MS:"
                errors.append(fagent_started took {timestamp*1000:.0f}ms (SLO: {self.SLO_AGENT_STARTED_MS}ms)")
        ...
        if len(self.thinking_intervals) > 1:
            for i in range(1, len(self.thinking_intervals)):
                ...
                if interval > self.SLO_THINKING_INTERVAL_S:
                    errors.append(fThinking interval {interval:.1f}s exceeds {self.SLO_THINKING_INTERVAL_S}s)
    ...
    def _validate_message_completeness(self) -> Tuple[bool, List[str]]:
        ...
        if agent_response is None:
            ...
            errors.append(fResponse too short for substantive value: {len(str(agent_response))} chars)
    ...
    def _validate_user_isolation(self) -> Tuple[bool, List[str]]:
        ...
        if len(user_ids) > 1:
            errors.append(fMultiple user IDs detected: {user_ids})
        ...
        if len(thread_ids) > 1:
            errors.append(fMultiple thread IDs detected: {thread_ids})
    ...
    def generate_report(self) -> str:
        ...
        report = [
            ...
            fStatus: {' PASS:  VALUE DELIVERED' if is_valid else ' FAIL:  NO VALUE - BROKEN'},
            ,"
            fTotal Events: {len(self.events)}",
            fResponse Time: {self.event_timeline[-1][0] if self.event_timeline else 0:.2f}s,
            fThinking Updates: {len(self.thinking_intervals)},"
            ",
            Event Sequence:
        ]
...
class FirstMessageExperienceTests:
    ...
    def setup(self):
        ...
        logger.info(fTesting against backend: {self.backend_url}")
        logger.info(fWebSocket URL: {self.ws_url})
        logger.info(fEnvironment: {'staging' if self.is_staging else 'development'})"
    ...
    async def _create_websocket_connection(self, user_id: str, max_retries: int = 3) -> Optional[Any]:
        ...
        logger.info(fGenerated JWT token for user {user_id} (staging: {self.is_staging}")"
        
        for attempt in range(max_retries):
            try:
                logger.info(fWebSocket connection attempt {attempt + 1}/{max_retries} to {self.ws_url})
                ...
                if self.is_staging:
                    ...
                    logger.info(fConnected to staging WebSocket: {self.ws_url})
                    ...
                    try:
                        ...
                        if auth_data.get("type) == auth_success:
                            ...
                            logger.warning(fUnexpected auth response: {auth_data}")"
                    ...
                    logger.info(fConnected to local WebSocket: {self.ws_url})"
                ...
                logger.error(fWebSocket connection attempt {attempt + 1} failed: {e})
                if attempt < max_retries - 1:
                    wait_time = 2 ** attempt  # Exponential backoff
                    logger.info(fRetrying in {wait_time} seconds...)
                    await asyncio.sleep(wait_time)
                else:
                    logger.error(f"All {max_retries} connection attempts failed)
                    raise ConnectionError(fFailed to connect to WebSocket after {max_retries} attempts: {e}")
    ...
    async def test_happy_path_first_message(self):
        ...
        user_id = ffirst_user_{uuid.uuid4().hex[:8]}
        ...
        try:
                    ...
                    while True:
                        ...
                        if message:
                            try:
                                ...
                                logger.info(fReceived event: {event.get('type', 'unknown')}")"
                            except json.JSONDecodeError as e:
                                logger.warning(fFailed to parse WebSocket message: {message[:100]}... Error: {e})
                except (ConnectionClosedError, websockets.ConnectionClosed):
                    logger.info(WebSocket connection closed by server)
                except asyncio.TimeoutError:
                    logger.info("WebSocket receive timeout)"
                except Exception as e:
                    logger.error(fError in event capture: {e})
            ...
            logger.error(fTest failed with exception: {e}")"
            raise
        finally:
            ...
            if ws:
                try:
                    ...
                    logger.warning(fError closing WebSocket: {e})
                
        # Validate results
        logger.info(fCaptured {len(events_received)} events)
        if events_received:
        ...
        if self.is_staging:
            ...
            assert len(events_received) >= 1, fNo events received from staging environment"
            if not is_valid:
                logger.warning(fStaging validation issues (may be expected): {failures})
        else:
            # Full validation for local environment
            assert is_valid, fFirst message experience failed:\n + \n.join(failures)
            assert len(events_received) >= 5, fToo few events received: {len(events_received)}""
    ...
    async def test_concurrent_first_messages(self):
        ...
        async def send_first_message(user_num: int) -> Tuple[bool, FirstMessageEventValidator]:
            ...
            validator = FirstMessageEventValidator()
            user_id = fconcurrent_user_{user_num}_{uuid.uuid4().hex[:8]}
            thread_id = fthread_{user_num}_{uuid.uuid4().hex[:8]}"
        ...
        for i, result in enumerate(results):
            if isinstance(result, Exception):
                logger.error(fUser {i} failed with exception: {result})
            else:
                ...
                if is_valid:
                    ...
                    logger.error(fUser {i}:  FAIL:  Failed")
                    
        # More lenient success rate for staging environment
        min_success_rate = 0.6 if self.is_staging else 0.8
        assert successful >= num_users * min_success_rate, fOnly {successful}/{num_users} users succeeded (required: {min_success_rate * 100}%)
    ...
    async def test_first_message_with_service_degradation(self):
        ...
        assert thinking_count >= 2, fNot enough thinking updates during degradation: {thinking_count}"
    ...
    async def test_rapid_first_messages(self):
        ...
        try:
            ...
            user_id = frapid_user_{uuid.uuid4().hex[:8]}
                ...
                while True:
                    try:
                        ...
                        if msg:
                            ...
                            logger.info(fEvent: {event.get('type')})
                    except:
                        break
            ...
            for i, msg_text in enumerate(messages):
                ...
                logger.info(fSent message {i+1}: {msg_text})
            ...
            while time.time() - start < 60:
                ...
                if current_completed > completed_count:
                    ...
                    logger.info(fCompleted responses: {completed_count}/{len(messages)})
        ...
        assert completed_count >= 2, fOnly {completed_count}/3 messages got responses
        
        # Verify no message was lost
        message_received_count = sum(1 for e in all_events if e.get(type") == "message_received)
        assert message_received_count >= 2, fOnly {message_received_count}/3 messages acknowledged
    ...
    async def test_malformed_first_message(self):
        ...
        for case_name, message_content in test_cases:
            logger.info(fTesting case: {case_name})
            ...
            try:
                ...
                assert has_error or has_response, fCase {case_name}: No response to malformed message"
...
class FirstMessageIntegrationTests:
    ...
    async def test_user_context_isolation(self):
        ...
        for i, ctx in enumerate(contexts):
            assert ctx.user_id == fisolated_user_{i}"
            assert ctx.thread_id == fthread_{i}
            assert ctx.message == fUser {i} first message"
...
class FirstMessagePerformanceTests:
    ...
    def setup(self):
        ...
        logger.info(fPerformance tests using backend: {self.backend_url})
    ...
    async def test_load_50_concurrent_users(self):
        ...
        async def send_user_message(user_num: int) -> Tuple[bool, float]:
            ...
            try:
                ...
                logger.error(fUser {user_num} failed: {e})
                return False, time.time() - start_time
                
        # Launch all users concurrently
        logger.info(fLaunching {num_users} concurrent users...)"
...
- P99 Response: {p99_response:.2f}s
        ...
        assert p99_response <= 60, fP99 response time {p99_response}s exceeds 60s SLO
