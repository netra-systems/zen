...
async def get_test_websocket_url() -> str:
    ...
    return f"{ws_scheme}://{backend_host}:{backend_port}/ws"

async def get_test_jwt_token(user_id: str = "chaos_test_user") -> str:
    """Get test JWT token."""
    return create_test_jwt_token(user_id=user_id, email=f"{user_id}@chaos.test")
...
async def test_user_context(user_id: str = "chaos_test_user"):
    ...
    return {
        ...
        "token": await get_test_jwt_token(user_id)
    }
...
class ChaosWebSocketClient:
    ...
    async def connect(self, websocket_url: str, jwt_token: str, max_retries: int = 3) -> bool:
        ...
        for attempt in range(max_retries):
            ...
            try:
                ...
                logger.info(f"ChaosClient {self.connection_id} connected in {duration_ms:.1f}ms (attempt {attempt + 1})")
                return True
    ...
    def _record_chaos_event(self, event_type: str, details: Dict[str, Any] = None, severity: str = "medium"):
        ...
        logger.debug(f"Chaos event {event_type} for {self.connection_id}: {details}")
    ...
    def get_metrics(self) -> Dict[str, Any]:
        ...
        reconnection_durations = [r.duration_ms for r in self.reconnection_attempts if r.success]
...
class ChaosTestOrchestrator:
    ...
    async def create_chaotic_environment(self, num_clients: int, conditions: NetworkConditions) -> List[ChaosWebSocketClient]:
        ...
        logger.info(f"Created chaotic environment: {len(successful_clients)}/{num_clients} clients connected")
    ...
    async def simulate_chat_workflow(self, client: ChaosWebSocketClient, num_messages: int = 5) -> List[Dict]:
        ...
        for i in range(num_messages):
            ...
            user_message = {
                "type": "user_message",
                "content": f"Test message {i+1} from {client.user_id}",
                "thread_id": f"thread_{client.user_id}",
                "timestamp": time.time()
            }
    ...
    async def validate_eventual_consistency(self, clients: List[ChaosWebSocketClient], test_duration: int = 30) -> Dict[str, Any]:
        ...
        while time.time() < end_time:
            ...
            consistency_message = {
                ...
                "content": f"Consistency test message {message_id}",
                "timestamp": time.time()
            }
...
class TestWebSocketBridgeChaos:
    ...
    async def test_random_connection_drops_medium_chaos(self, chaos_orchestrator):
        ...
        for i, (client, result) in enumerate(zip(clients, chat_results)):
            ...
            # Log client metrics
            logger.info(f"Client {i} metrics: {metrics['messages_sent']} sent, "
                       f"{metrics['messages_received']} received, "
                       f"{metrics['chaos_events']} chaos events, "
                       f"reconnection rate: {metrics['reconnection_success_rate']:.2f}")
            
            # Assertions for medium chaos resilience
            assert metrics["messages_sent"] > 0, f"Client {i} should send some messages despite chaos"
            assert metrics["reconnection_success_rate"] >= 0.5, f"Client {i} should maintain >= 50% reconnection success"
        ...
        logger.info(f"Medium chaos test completed: {successful_clients}/{len(clients)} clients succeeded, "
                   f"{total_messages_sent} total messages sent")
    ...
    async def test_high_chaos_extreme_conditions(self, chaos_orchestrator):
        ...
        for i, (client, result) in enumerate(zip(clients, chat_results)):
            ...
            logger.info(f"High chaos client {i}: {metrics['messages_sent']} sent, "
                       f"{metrics['chaos_events']} chaos events, "
                       f"drop events: {metrics['chaos_events_by_type'].get('message_dropped', 0)}")
            
            # Under extreme chaos, we accept lower thresholds but require basic functionality
            if metrics["messages_sent"] > 0:
                assert metrics["reconnection_success_rate"] >= 0.3, f"Client {i} should maintain >= 30% reconnection success under extreme chaos"
        ...
        logger.info(f"High chaos test completed: {functioning_clients}/{len(clients)} clients functioning, "
                   f"{total_chaos_events} total chaos events")
    ...
    async def test_network_latency_injection(self, chaos_orchestrator):
        ...
        for client in clients:
            metrics = client.get_metrics()
            logger.info(f"Latency test client: {metrics['messages_sent']} sent, "
                       f"latency range: {metrics['network_conditions']['latency_range_ms']}ms")
        ...
        if response_times:
            ...
            logger.info(f"Latency test results: avg response {avg_response_time:.2f}s, "
                       f"max response {max_response_time:.2f}s")
        ...
        logger.info(f"Network latency test completed in {test_duration:.2f}s")
    ...
    async def test_message_corruption_resilience(self, chaos_orchestrator):
        ...
        for client in clients:
            for msg_template in message_types:
                for i in range(3):
                    ...
                    message["test_id"] = f"{client.user_id}_{i}"
        ...
        for client in clients:
            ...
            logger.info(f"Corruption test client: {metrics['messages_sent']} sent, "
                       f"{corruption_events} corrupted messages, "
                       f"corruption rate: {metrics['network_conditions']['corruption_rate']}")
        ...
        logger.info(f"Corruption resilience: {corruption_stats['handled']}/{corruption_stats['sent']} messages handled "
                   f"({handled_rate:.2%} success rate)")
    ...
    async def test_rapid_connect_disconnect_cycles(self, chaos_orchestrator):
        ...
        logger.info(f"Rapid cycles: {cycle_results['successful_reconnections']}/{cycle_results['total_cycles']} "
                   f"successful ({cycle_results['success_rate']:.2%}), "
                   f"fast reconnections: {cycle_results['fast_reconnections']} "
                   f"({cycle_results['fast_reconnection_rate']:.2%})")
        ...
        fast_cycles = [c for c in cycle_results["cycle_details"] if c.get("within_3_second_requirement")]
        if fast_cycles:
            avg_fast_time = statistics.mean([c["duration_seconds"] for c in fast_cycles])
            logger.info(f"Average fast reconnection time: {avg_fast_time:.2f}s")
    ...
    async def test_automatic_reconnection_within_3_seconds(self, chaos_orchestrator):
        ...
        for client in clients:
            ...
            logger.info(f"Reconnection test {client.connection_id}: "
                       f"{'SUCCESS' if success else 'FAILED'} in {reconnection_duration:.2f}s")
        
        # Validate 3-second requirement
        successful_reconnections = [t for t in reconnection_tests if t["success"]]
        fast_reconnections = [t for t in successful_reconnections if t["within_requirement"]]
        ...
        logger.info(f"3-second requirement test: {len(successful_reconnections)}/{len(reconnection_tests)} "
                   f"successful reconnections ({success_rate:.2%}), "
                   f"{len(fast_reconnections)} within 3s requirement ({fast_rate:.2%})")
        ...
        if fast_reconnections:
            ...
            logger.info(f"Fast reconnection stats: avg {avg_fast_time:.2f}s, max {max_fast_time:.2f}s")
    ...
    async def test_system_recovery_eventual_consistency(self, chaos_orchestrator):
        ...
        logger.info(f"Eventual consistency results: "
                   f"{consistency_results['messages_delivered']}/{consistency_results['messages_sent']} "
                   f"messages delivered, avg delivery rate: {consistency_results['average_delivery_rate']:.2%}, "
                   f"consistency achieved: {consistency_results['eventual_consistency_achieved']}")
        ...
        successful_recoveries = [t for t in recovery_tests if t["recovery_success"]]
        recovery_rate = len(successful_recoveries) / max(1, len(recovery_tests))
        
        logger.info(f"System recovery validation: {len(successful_recoveries)}/{len(recovery_tests)} "
                   f"successful recoveries ({recovery_rate:.2%})")
    ...
    async def test_comprehensive_chaos_resilience(self, chaos_orchestrator):
        ...
        for i, ((scenario_type, client_id, _), result) in enumerate(zip(test_tasks, scenario_results)):
            if isinstance(result, Exception):
                logger.error(f"Scenario {scenario_type} for {client_id} failed: {result}")
            else:
                successful_scenarios += 1
                logger.info(f"Scenario {scenario_type} for {client_id} completed successfully")
        ...
        for client in clients:
            ...
            logger.info(f"Comprehensive chaos client {client.connection_id}: "
                       f"{metrics['messages_sent']} messages, {metrics['chaos_events']} chaos events, "
                       f"reconnection rate: {metrics['reconnection_success_rate']:.2%}")
        ...
        logger.info(f"Comprehensive chaos results: "
                   f"{successful_scenarios}/{total_scenarios} scenarios successful ({scenario_success_rate:.2%}), "
                   f"{overall_metrics['functioning_clients']}/{len(clients)} clients functioning ({client_function_rate:.2%}), "
                   f"{overall_metrics['total_messages']} total messages, "
                   f"{overall_metrics['total_chaos_events']} chaos events")
    ...
    try:
        ...
        for test_method in test_methods:
            try:
                ...
                results.append(f"✅ {test_method.__name__} - PASSED")
            except Exception as e:
                results.append(f"❌ {test_method.__name__} - FAILED: {e}")
