...
"
        ...
        try:
            ...
            self.logger.info(fAuth redis manager not found (good): {e})
        ...
        self.logger.error(fRedis Instances Found: {len(redis_instances)})
...
"
    ...
    async def test_ssot_redis_singleton_validation(self):
        ...
        self.logger.info(fUnique instance IDs: {len(unique_ids)})
        self.logger.info(fInstance type: {type(rm1).__name__}")"

        # This test validates the SSOT solution
        self.assertEqual(
            len(unique_ids),
            1,
            fSSOT singleton pattern failed: {len(unique_ids)} unique instances found. 
            fExpected: 1 singleton instance. IDs: {instance_ids}
        )
    ...
    async def asyncTearDown(self):
        ...
        try:
            ...
            self.logger.warning(fFailed to clean up test keys: {e})
