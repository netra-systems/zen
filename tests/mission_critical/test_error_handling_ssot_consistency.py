...
class ErrorHandlingSSOTValidator:
        ...
        for search_path in search_paths:
            ...
            for file_path in search_path.rglob("*.py"):
                ...
                try:
                    ...
                    self.warnings_found.append(f"Could not parse {file_path}: {e}")
        ...
        for search_path in search_paths:
            ...
            for file_path in search_path.rglob("*.py"):
                ...
                try:
                    ...
                    self.warnings_found.append(f"Could not parse {file_path} for imports: {e}")
    ...
    def validate_no_duplicate_definitions(self, error_classes: Dict[str, List[ErrorClassDefinition]]) -> bool:
        ...
        for class_name, definitions in error_classes.items():
            canonical_defs = [d for d in definitions if d.is_canonical]
            non_canonical_defs = [d for d in definitions if not d.is_canonical]
            
            if len(canonical_defs) == 0:
                self.errors_found.append(f"No canonical definition found for {class_name}")
                all_valid = False
            elif len(canonical_defs) > 1:
                self.errors_found.append(f"Multiple canonical definitions found for {class_name}")
                all_valid = False
            ...
            for definition in non_canonical_defs:
                ...
                self.errors_found.append(
                    f"Duplicate {class_name} definition found at {definition.file_path}:{definition.line_number} "
                    f"(should use canonical from {self.canonical_websocket_error_path if class_name == 'WebSocketError' else self.canonical_error_context_path})"
                )
                all_valid = False
        ...
        for class_name, import_list in imports.items():
            ...
            for import_stmt in import_list:
                ...
                if import_stmt.module != canonical_module:
                    self.errors_found.append(
                        f"Incorrect import of {class_name} at {import_stmt.file_path}:{import_stmt.line_number} "
                        f"(importing from '{import_stmt.module}' instead of '{canonical_module}')"
                    )
                    all_valid = False
    ...
    def check_agent_communication_file(self) -> bool:
        ...
        if not agent_comm_path.exists():
            self.warnings_found.append(f"agent_communication.py not found at expected path: {agent_comm_path}")
            return True
        ...
        if re.search(r'^class WebSocketError\([^)]*\):', content, re.MULTILINE):
            self.errors_found.append(
                f"Local WebSocketError definition found in {agent_comm_path} - "
                f"should import from {self.canonical_websocket_error_path}"
            )
            return False
        ...
        if re.search(r'^class ErrorContext\([^)]*\):', content, re.MULTILINE):
            self.errors_found.append(
                f"Local ErrorContext definition found in {agent_comm_path} - "
                f"should import from {self.canonical_error_context_path}"
            )
            return False
        ...
        for file_path in agent_files:
            ...
            if "except" in content:
                ...
                if "WebSocketError" in content and "from netra_backend.app.core.exceptions_websocket import" not in content:
                    self.errors_found.append(
                        f"{file_path} uses WebSocketError but doesn't import from canonical source"
                    )
                    all_valid = False
                ...
                if "ErrorContext" in content and "from netra_backend.app.schemas.shared_types import" not in content:
                    ...
                    if re.search(r'\bErrorContext\b', content):
                        self.errors_found.append(
                            f"{file_path} uses ErrorContext but doesn't import from canonical source"
                        )
                        all_valid = False
...
class TestErrorHandlingSSOTConsistency:
    ...
    pass
        ...
        assert canonical_count == 1, f"Expected 1 canonical WebSocketError, found {canonical_count}"
        
        # The only non-test definition should be canonical
        non_canonical = [d for d in non_test_websocket_errors if not d.is_canonical]
        assert len(non_canonical) == 0, (
            f"Found {len(non_canonical)} non-canonical WebSocketError definitions: "
            f"{[f'{d.file_path}:{d.line_number}' for d in non_canonical]}"
        )
        ...
        assert canonical_count >= 1, f"Expected at least 1 canonical ErrorContext, found {canonical_count}"
    ...
    def test_agent_communication_uses_canonical_imports(self, validator):
        ...
        assert result, f"agent_communication.py has local error definitions: {validator.errors_found}"
    ...
    def test_error_handling_consistency(self, validator):
        ...
        assert result, f"Inconsistent error handling found: {validator.errors_found}"
    ...
    def test_error_context_has_required_fields(self):
        ...
        for field in required_fields:
            assert field in CanonicalErrorContext.model_fields, (
                f"ErrorContext missing required field: {field}"
            )
...
=== WARNINGS ===")
        ...
        assert all_valid, (
            f"SSOT validation failed with {len(errors)} errors and {len(warnings)} warnings"
        )
...
if __name__ == "__main__":
    ...
    validator = ErrorHandlingSSOTValidator()
    all_valid, errors, warnings = validator.run_full_validation()
    
    print("=" * 80)
    print("ERROR HANDLING SSOT VALIDATION REPORT")
    print("=" * 80)
