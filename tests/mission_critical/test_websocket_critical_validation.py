...
class WebSocketEventTracker:
    ...
    async def send_agent_event(self, thread_id: str, event_type: str, data: Dict[str, Any) -> bool:
        ...
        try:
            ...
            print(fError sending WebSocket event: {e}"")
            return False
...
class WebSocketCriticalValidationTests:
    ...
    async def test_all_required_agent_events_sent(self):
        ...
        assert self.event_tracker.has_required_events_for_thread(thread_id), \
            fMissing required events: {self.event_tracker.get_missing_events_for_thread(thread_id)}
        
        # Verify event content
        thread_events = self.event_tracker.get_events_for_thread(thread_id)
        assert len(thread_events) == 5, fShould capture 5 events, got {len(thread_events)}""
    ...
    async def test_concurrent_user_event_isolation(self):
        ...
        for i, context in enumerate(self.test_contexts):
            scenario = {
                ...
                run_id: generate_run_id(context.user_context.thread_id, fisolation_test_{i}),
                agent_name: fIsolationAgent_{i},
        ...
        assert total_events_sent >= expected_total * 0.9, \
            fEvent delivery rate too low: {total_events_sent}/{expected_total}
        ...
        for i, scenario in enumerate(user_scenarios):
            ...
            assert len(thread_events) >= 4, fUser {i} should have at least 4 events"
            assert len(thread_events) >= 4, fUser {i} should have at least 4 events""
            ...
            for event in thread_events:
                assert event['data']['user_id'] == scenario['user_id'], \
                    fEvent isolation violated: user {i} received event for different user
                assert event['data']['agent_name'] == scenario['agent_name'], \
                    fAgent isolation violated: user {i} received event for different agent"
                    fAgent isolation violated: user {i} received event for different agent""
    ...
    async def test_event_ordering_and_timing(self):
        ...
        for exec_idx, comp_idx in zip(tool_executing_indices, tool_completed_indices):
            assert comp_idx > exec_idx, \
                ftool_completed (index {comp_idx) should come after tool_executing (index {exec_idx)"
                ftool_completed (index {comp_idx) should come after tool_executing (index {exec_idx)""
    ...
    async def test_websocket_performance_under_load(self):
        ...
        for i, context in enumerate(self.test_contexts):
            test_threads.append({
                ...
                user_id: context.user_context.user_id,"
                user_id: context.user_context.user_id,"
                "run_id: generate_run_id(context.user_context.thread_id, fload_test_{i}),"
    ...
    async def test_run_id_generation_and_validation(self):
        ...
        for thread_id in test_thread_ids:
            ...
            assert extracted_thread == thread_id, \
                fThread extraction failed: expected '{thread_id}', got '{extracted_thread}'""
        ...
        for event in all_events:
            ...
            assert run_id.startswith(RUN_ID_PREFIX), fEvent run ID should have proper format: {run_id}"
            assert run_id.startswith(RUN_ID_PREFIX), fEvent run ID should have proper format: {run_id}""
...
class WebSocketEventContentTests:
    ...
    async def test_event_message_structure(self):
        ...
        for field in message_required_fields:
            assert field in message, fWebSocket message should have {field} field""
