...
class AdvancedWebSocketCapture:
    ...
    async def capture_event(self, event_type: str, run_id: str, agent_name: str, *args, **kwargs):
        ...
        if self.failure_rate > 0 and random.random() < self.failure_rate:
            raise ConnectionError(f"Simulated WebSocket failure for {event_type}")
...
class TestProperWebSocketHandling(unittest.TestCase):
    ...
    pass
            ...
            for i in range(3):
                agent = DataSubAgent()
                agent.set_websocket_bridge(mock_bridge, f"order_test_{i}")
                agents.append((agent, f"order_test_{i}"))
            
            # Execute agents sequentially and verify ordering
            for agent, run_id in agents:
                await agent.emit_thinking(f"Thinking for {run_id}")
...
class TestConcurrentAgentExecution(unittest.TestCase):
    ...
    def test_concurrent_agent_websocket_notifications(self):
        ...
        async def run_test():
            ...
            async def execute_agent(agent, run_id):
                for step in range(5):
                    await agent.emit_thinking(f"Step {step} for {run_id}")
    ...
    pass
                ...
                for i in range(50):  # 50 events per agent
                    await agent.emit_thinking(f"Event {i}")
                    event_count += 1
    ...
    def test_performance_degradation_monitoring(self):
        ...
        async def run_test():
            ...
            print(f"Degradation detected: {degradation_ratio:.2f}x slower")
...
class TestErrorHandlingAndRecovery(unittest.TestCase):
    ...
    pass
            ...
            for target_latency in latency_scenarios:
                ...
                # Create agent
                agent = DataSubAgent()
                agent.set_websocket_bridge(mock_bridge, f"latency_test_{target_latency}")
                ...
                print(f"Target: {target_latency}ms, Actual: {actual_latency:.2f}ms")
...
class TestPerformanceAndMemory(unittest.TestCase):
    ...
    def test_memory_efficiency(self):
        ...
        async def run_test():
            ...
            for iteration in range(200):
                ...
                agent = DataSubAgent()
                bridge = AgentWebSocketBridge()
                agent.set_websocket_bridge(bridge, f"mem_test_{iteration}")
            ...
            if memory_per_iteration:
                ...
                self.assertLess(avg_memory_per_iteration, 50 * 1024,
                              f"Memory usage too high: {avg_memory_per_iteration / 1024:.2f}KB per iteration")
    ...
    pass
            ...
            for i in range(50):
                agent = DataSubAgent() if i % 2 == 0 else ValidationSubAgent()
                agent.set_websocket_bridge(mock_bridge, f"throughput_test_{i}")
                agents.append(agent)
            ...
            while time.perf_counter() < end_time:
                ...
                if event_count % 3 == 0:
                    await agent.emit_thinking(f"Thought {event_count}")
                elif event_count % 3 == 1:
                    await agent.emit_tool_executing(f"tool_{event_count}", {})
                else:
                    await agent.emit_progress(f"Progress {event_count}", is_complete=False)
            ...
            print(f"Sustained throughput: {throughput:.2f} events/second over {duration:.2f} seconds")
            ...
            for event_type, metrics in perf_summary.items():
                self.assertLess(metrics['avg_ms'], 5.0,
                              f"{event_type} average latency too high: {metrics['avg_ms']:.2f}ms")
    ...
    def test_performance_requirements(self):
        ...
        async def run_test():
            ...
            for i in range(requirements['max_concurrent_agents']):
                agent = DataSubAgent()
                agent.set_websocket_bridge(mock_bridge, f"req_test_{i}")
                agents.append(agent)
            ...
            while time.perf_counter() - start_time < 10:
                ...
                await agent.emit_thinking(f"Event {event_count}")
...
Performance Test Results:")
            ...
            print(f"  Memory/Agent: {avg_memory:.2f}KB (required: <{requirements['max_memory_kb_per_agent']}KB)")
