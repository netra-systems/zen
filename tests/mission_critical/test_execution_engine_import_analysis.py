...
class ExecutionEngineImportAnalysisTests(SSotBaseTestCase):
    ...
    def test_detect_illegal_execution_engine_imports(self):
        ...
        # Log findings
        logger.info(f"Found {len(illegal_imports)} illegal ExecutionEngine imports:")
        ...
        self.import_violations.extend([
            f"Illegal import ({violation_type}): {file_path}:{line_num} - {import_statement.strip()}"
            for file_path, line_num, import_statement, violation_type in illegal_imports
        ])
        ...
        self.assertGreater(
            ...
            f"Found {len(illegal_imports)} illegal imports requiring cleanup."
        )
    ...
    def test_detect_missing_canonical_imports(self):
        ...
        # Log findings
        logger.info(f"Found {len(missing_imports)} files missing canonical imports:")
        ...
        self.missing_canonical_imports.extend([
            f"Missing canonical import: {file_path} - Should import {missing_import} ({reason})"
            for file_path, missing_import, reason in missing_imports
        ])
        ...
        self.assertGreater(
            ...
            f"Found {len(missing_imports)} files requiring canonical imports."
        )
    ...
    def test_analyze_import_consistency_across_services(self):
        ...
        # Log findings
        logger.info(f"Found {len(inconsistencies)} import consistency issues:")
        ...
        self.assertGreater(
            ...
            f"Found {len(inconsistencies)} inconsistencies requiring standardization."
        )
    ...
    def test_validate_import_path_resolution(self):
        ...
        # Log findings
        logger.info(f"Found {len(resolution_failures)} import resolution failures:")
        ...
        self.import_violations.extend([
            f"Broken import: {file_path} - {import_statement} ({error})"
        ...
        if broken_imports:
            self.assertGreater(
                ...
                f"Found {len(broken_imports)} broken import paths requiring fixes."
            )
        else:
    ...
    def test_comprehensive_import_analysis_report(self):
        ...
        logger.info(f"IMPORT ANALYSIS SUMMARY:")
        ...
        self.assertGreater(
            ...
            f"Detected {import_analysis_summary['total_violations']} import violations requiring resolution. "
            f"Priority: {import_analysis_summary['cleanup_priority']}"
        )
                        ...
                        if isinstance(node, ast.Import):
                            for alias in node.names:
                                ...
                                if violation_type:
                                    illegal_imports.append((
                                        ...
                                        f"import {alias.name}", violation_type
                                    ))
                            ...
                            if node.module:
                                ...
                                if violation_type:
                                    ...
                                    illegal_imports.append((
                                        str(py_file), node.lineno,
                                        f"from {node.module} import {', '.join(names)}", violation_type
                                    ))
                                        ...
                                        if 'execution_engine' in alias.name.lower():
                                            try:
                                                ...
                                                resolution_failures.append((
                                                    str(py_file), f"import {alias.name}", str(e)
                                                ))
