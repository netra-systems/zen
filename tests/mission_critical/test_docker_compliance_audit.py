...
class DockerComplianceInfrastructureTests:
    ...
    def test_compliance_audit_performance_scalability(self) -> ComplianceMetrics:
        ...
        self.logger.info("📊 Testing compliance audit performance scalability")
        ...
        self.logger.info(f"✅ Compliance audit performance:")
        self.logger.info(f"   Files scanned: {metrics.total_files_scanned}")
        self.logger.info(f"   Audit duration: {audit_duration:.2f}ms")
        self.logger.info(f"   Scanning rate: {files_per_second:.2f} files/sec")
        self.logger.info(f"   Memory usage: {memory_usage:.2f}MB")
        self.logger.info(f"   Memory per file: {memory_per_file:.4f}MB/file")
        self.logger.info(f"   Violations found: {metrics.violations_detected}")
        self.logger.info(f"   Compliance score: {metrics.compliance_score:.1f}%")
        
        # Performance assertions
        assert files_per_second > 50, f"Audit scanning rate too slow: {files_per_second:.2f} files/sec"
        assert memory_per_file < 0.5, f"Memory usage per file too high: {memory_per_file:.4f}MB/file"
        assert audit_duration < 30000, f"Total audit duration too long: {audit_duration:.2f}ms"
        
        return metrics
    
    def test_compliance_violation_detection_accuracy(self) -> Dict[str, Any]:
        """Test accuracy of compliance violation detection."""
        self.logger.info("🔍 Testing compliance violation detection accuracy")
        ...
        self.logger.info(f"✅ Violation detection accuracy:")
        self.logger.info(f"   Tests run: {total_tests}")
        self.logger.info(f"   Accurate detections: {accurate_tests}/{total_tests} ({accuracy_rate:.1f}%)")
        self.logger.info(f"   False positive rate: {false_positive_rate:.2f}")
        self.logger.info(f"   False negative rate: {false_negative_rate:.2f}")
        
        # Accuracy assertions
        assert accuracy_rate >= 90, f"Detection accuracy too low: {accuracy_rate:.1f}%"
        assert false_positive_rate <= 0.1, f"False positive rate too high: {false_positive_rate:.2f}"
        assert false_negative_rate <= 0.05, f"False negative rate too high: {false_negative_rate:.2f}"
    ...
    def test_automated_remediation_effectiveness(self) -> Dict[str, Any]:
        ...
        self.logger.info("🔧 Testing automated remediation effectiveness")
        ...
        for violation in test_violations:
            test_file = self.project_root / f"temp_violation_{violation['name']}.py"
            try:
                # Create violation
                with open(test_file, 'w') as f:
        ...
        self.logger.info(f"✅ Automated remediation effectiveness:")
        self.logger.info(f"   Remediations tested: {total_remediations}")
        self.logger.info(f"   Successful generations: {successful_remediations}/{total_remediations} ({effectiveness_rate:.1f}%)")
        self.logger.info(f"   Average quality score: {avg_quality:.2f}/10")
        
        # Effectiveness assertions
        assert effectiveness_rate >= 80, f"Remediation effectiveness too low: {effectiveness_rate:.1f}%"
        assert avg_quality >= 6.0, f"Remediation quality too low: {avg_quality:.2f}/10"
    ...
    def test_continuous_monitoring_performance(self) -> Dict[str, Any]:
        ...
        self.logger.info("📊 Testing continuous compliance monitoring")
        ...
        def compliance_monitoring_cycle(cycle_id: int) -> Dict[str, Any]:
            ...
            try:
                ...
                return {
                    ...
                    'files_monitored': len([f for f in critical_files if f.exists()]),
                    'success': True
                }
            ...
            for future in futures:
                try:
                    ...
                    monitoring_results.append({
                        'success': False,
                        'error': f"Monitoring cycle timeout: {str(e)}",
                        'monitoring_time_ms': 30000
                    })
        
        # Analyze monitoring performance
        successful_cycles = [r for r in monitoring_results if r.get('success', False)]
        ...
        self.logger.info(f"✅ Continuous monitoring performance:")
        self.logger.info(f"   Monitoring cycles: {len(monitoring_results)}")
        self.logger.info(f"   Success rate: {success_rate:.1f}%")
        self.logger.info(f"   Average cycle time: {avg_monitoring_time:.2f}ms")
        self.logger.info(f"   Maximum cycle time: {max_monitoring_time:.2f}ms")
        self.logger.info(f"   Average memory delta: {avg_memory_delta:.4f}MB")
        self.logger.info(f"   Total violations detected: {total_violations}")
        self.logger.info(f"   Failed cycles: {failed_cycles}")
        
        # Performance assertions
        assert success_rate >= 95, f"Monitoring success rate too low: {success_rate:.1f}%"
        assert avg_monitoring_time < 1000, f"Average monitoring time too high: {avg_monitoring_time:.2f}ms"
        assert max_monitoring_time < 5000, f"Maximum monitoring time too high: {max_monitoring_time:.2f}ms"
        assert failed_cycles == 0, f"Failed monitoring cycles detected: {failed_cycles}"
    ...
    def test_risk_assessment_and_impact_analysis(self) -> Dict[str, Any]:
        ...
        self.logger.info("⚠️ Testing risk assessment and impact analysis")
        ...
        self.logger.info(f"✅ Risk assessment and impact analysis:")
        self.logger.info(f"   Total violations: {total_violations}")
        self.logger.info(f"   Average risk score: {avg_risk_score:.2f}/10")
        self.logger.info(f"   Overall risk level: {overall_risk_level}")
        self.logger.info(f"   Estimated remediation time: {estimated_remediation_hours:.1f} hours")
        
        # Log violation breakdown
        for violation_type, count in violation_counts.items():
            self.logger.info(f"     {violation_type}: {count} violations")
        
        # Risk assertions - adjust based on acceptable risk levels
        assert overall_risk_level in ['LOW', 'MEDIUM'], f"Overall risk level too high: {overall_risk_level}"
        assert avg_risk_score < 7.0, f"Average risk score too high: {avg_risk_score:.2f}/10"
        assert estimated_remediation_hours < 40, f"Remediation time estimate too high: {estimated_remediation_hours:.1f} hours"
    ...
    def test_cross_platform_compliance_validation(self) -> Dict[str, Any]:
        ...
        self.logger.info("🌐 Testing cross-platform compliance validation")
        ...
        self.logger.info(f"✅ Cross-platform compliance validation:")
        self.logger.info(f"   Platform: {current_platform}")
        self.logger.info(f"   Total files checked: {total_files_checked}")
        self.logger.info(f"   Total violations: {total_violations}")
        self.logger.info(f"   Cross-platform compliance: {cross_platform_compliance:.1f}%")
        
        # Log platform-specific results
        for pattern, result in platform_results.items():
            if 'error' not in result:
                self.logger.info(f"     {pattern}: {result['files_found']} files, {result['violations_found']} violations")
        
        # Cross-platform assertions
        assert cross_platform_compliance >= 95, f"Cross-platform compliance too low: {cross_platform_compliance:.1f}%"
        
        # Ensure at least one Docker Compose file exists
        existing_compose_files = [f for f, data in compose_compatibility.items() if data.get('exists', False)]
