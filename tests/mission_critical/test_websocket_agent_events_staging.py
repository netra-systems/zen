...
class StagingWebSocketEventValidator:
    ...
    def _validate_paired_events(self) -> bool:
        ...
        if tool_starts != tool_ends:
            self.errors.append(fUnpaired tool events: {tool_starts} starts, {tool_ends} completions")
            return False
    ...
    def _validate_staging_timing(self) -> bool:
        ...
        for timestamp, event_type, _ in self.event_timeline:
            if timestamp > 15:  # 15 second timeout for staging services per CLAUDE.md
                self.errors.append(fEvent {event_type} arrived too late: {timestamp:.2f}s > 15s)
                return False
    ...
    "
        ...
        try:
            ...
            async def capture_staging_events():
                ...
                while event_count < max_events:
                    try:
                        ...
                        logger.warning(fEvent capture error: {e})
                        break
            ...
            if not success:
                ...
                pytest.fail(fCRITICAL WEBSOCKET EVENT FAILURES - Golden Path broken:\n + 
                           \n.join(failures))"
        ...
        try:
            ...
            for i in range(3):
                ...
                try:
                    ...
                    assert response is not None, fNo response for message {i+1}
...
"
        ...
        assert validation_result[valid], fToken validation failed: {validation_result.get('error')}
...
"
        ...
        try:
            ...
            assert response_time < 15.0, fResponse time {response_time:.2f}s exceeds 15s staging limit
