        ...
        for i in range(connection_attempts):
            ...
            try:
                ...
                if state_registry in str(e):
                    name_errors += 1
                    failures += 1
                    logger.warning(fConnection {i+1}: NameError - {e}")"
                else:
                    failures += 1
                    logger.warning(fConnection {i+1}: Other NameError - {e})
            except Exception as e:
                failures += 1
                logger.warning(fConnection {i+1}: Other error - {e})
...
"
    ...
    """"
        ...
        if not os.path.exists(websocket_py_path):
            pytest.fail(fWebSocket route file not found: {websocket_py_path})
