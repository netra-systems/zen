...
class TestFactoryPatternViolations(SSotAsyncTestCase):
    ...
    def test_multiple_factory_implementations_exist(self):
        ...
        for module_path, factory_name in factory_patterns:
            ...
            try:
                ...
                logger.debug(f"Factory pattern test - expected failure: {module_path}.{factory_name} - {e}")
        ...
        self.assertGreater(
            ...
            f"Should be exactly 1 canonical factory pattern after SSOT consolidation. "
            f"Factories found: {[impl['path'] for impl in factory_implementations]}"
        )

        logger.error(f"FACTORY PATTERN VIOLATIONS: {len(factory_implementations)} factory implementations found")
    ...
    def test_factory_returns_singleton_instances(self):
        ...
        try:
            ...
            rapid_instances = []
            for i in range(5):
                context = {"user_id": f"rapid_user_{i}", "session_id": f"rapid_session_{i}"}
            ...
            violation = f"Cannot test factory pattern - import failed: {e}"
            singleton_violations.append(violation)
            self.factory_violations.append(violation)
        ...
        self.assertGreater(
            ...
            f"FACTORY SINGLETON VIOLATION: Found {len(singleton_violations)} singleton violations. "
    ...
    def test_factory_lacks_user_context_isolation(self):
        ...
        try:
            ...
            violation = f"Context isolation test failed: {e}"
            context_isolation_violations.append(violation)
            self.factory_violations.append(violation)
        ...
        self.assertGreater(
            ...
            f"Factory should properly isolate and validate user contexts. "
            f"Violations: {context_isolation_violations}"
        )

        logger.error(f"CONTEXT ISOLATION VIOLATIONS: {len(context_isolation_violations)} violations detected")
    ...
    def test_factory_configuration_sharing_violation(self):
        ...
        self.assertGreater(
            ...
            f"CONFIGURATION SHARING VIOLATION: Found {len(config_sharing_violations)} configuration sharing violations. "
            f"Factory instances should have isolated configurations. "
            f"Violations: {config_sharing_violations}"
        )
    ...
    def test_factory_lacks_cleanup_mechanisms(self):
        ...
        self.assertGreater(
            ...
            f"CLEANUP MECHANISM VIOLATION: Found {len(cleanup_violations)} cleanup mechanism violations. "
            f"Factory should provide proper resource cleanup mechanisms. "
            f"Violations: {cleanup_violations}"
        )
    ...
    def tearDown(self):
        ...
        if self.factory_violations:
            ...
            logger.error("="*80)
            logger.error(f"TOTAL FACTORY VIOLATIONS: {len(self.factory_violations)}")
            logger.error("="*80)
