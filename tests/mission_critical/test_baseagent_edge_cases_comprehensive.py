...
class EdgeCaseSimulator:
    ...
    async def simulate_llm_edge_cases(self, llm_manager: LLMManager, case_type: str):
        ...
        cases = {
            ...
            "unicode_chaos": {"content": "🔥💀🚨🎯⚡🌟💥🎉🔮🌈" * 100, "tokens": {"input": 10, "output": 300, "total": 310}},
        ...
        if case_type not in cases:
            raise ValueError(f"Unknown LLM edge case: {case_type}")
...
class ConcurrentExecutionTester:
        ...
        async def concurrent_state_modifier(agent_index: int, operation_count: int):
            ...
            for op_index in range(operation_count):
                try:
                    ...
                    shared_state.context[f"agent_{agent_index}_op_{op_index}"] = time.time()
    ...
    return results
        ...
        counter_values = [op.get("counter_value", 0) for op in all_operations if op.get("success")]
        ...
        for op in all_operations:
            if op.get("success"):
                op_id = f"agent_{op['agent_index']}_op_{op['operation']}"
...
async def test_agents():
    ...
    agents = []
    for i in range(5):
        agent = TestableBaseAgent(f"test_agent_{i}")
        agents.append(agent)
...
async def test_baseagent_handles_llm_edge_cases(test_agents, edge_case_simulator):
    ...
    for case_type in edge_cases:
        state = DeepAgentState()
        state.run_id = f"edge_case_{case_type}_{uuid.uuid4().hex[:8]}"
    ...
    assert len(successful_cases) >= len(edge_cases) * 0.8, \
        f"Too many edge cases failed: {len(successful_cases)}/{len(edge_cases)} succeeded"
    
    # Should not have any silent failures
    gracefully_handled = [r for r in successful_cases if r.get("handled_gracefully")]
    ...
    extreme_results = [r for r in results if r["case_type"] in extreme_cases and r["success"]]
    assert len(extreme_results) >= 2, \
        f"Too many extreme cases failed: {len(extreme_results)}/{len(extreme_cases)}"
    
    logger.info(f"LLM edge cases test: {len(successful_cases)}/{len(edge_cases)} cases handled, "
                f"{len(gracefully_handled)} gracefully handled")
...
async def test_baseagent_state_corruption_resilience(test_agents, edge_case_simulator):
    ...
    for corruption_type in corruption_types:
        state = DeepAgentState()
        state.run_id = f"corruption_test_{uuid.uuid4().hex[:8]}"
    ...
    survived_corruptions = [r for r in resilience_results if r["survived"]]
    ...
    assert len(basic_survivors) >= 3, \
        f"Too many basic corruptions caused failures: {len(basic_survivors)}/4"
    
    # Complex corruptions can fail but should not cause crashes
    complex_corruptions = ["circular_reference", "corrupted_context"]
    complex_results = [r for r in resilience_results if r["corruption_type"] in complex_corruptions]
    ...
    assert total_handled >= len(corruption_types) * 0.8, \
        f"Too many unexpected corruption behaviors: {total_handled}/{len(corruption_types)}"
    
    logger.info(f"State corruption test: {len(survived_corruptions)} survived, "
                f"{len(expected_failures)} expected failures, "
                f"{total_handled}/{len(corruption_types)} handled properly")
...
async def test_concurrent_baseagent_execution_race_conditions(
    ...
    assert success_rate > 0.8, \
        f"Too many concurrent operations failed: {success_rate:.2f} success rate"
    
    # State should be consistent
    expected_counter = result["expected_counter"]
    actual_counter = result["actual_counter"]
    assert actual_counter == expected_counter, \
        f"State inconsistency detected: expected {expected_counter}, got {actual_counter}"
    ...
    if result["resource_usage"]:
        ...
        assert peak_memory < 1024, f"Memory usage too high during concurrency: {peak_memory:.1f}MB"
        assert avg_threads < 100, f"Too many threads during concurrency: {avg_threads:.1f}"
    
    logger.info(f"Concurrent execution test: {result['successful_operations']}/{result['total_operations_attempted']} "
                f"operations succeeded, {len(race_conditions)} race conditions, "
                f"final counter: {actual_counter}")
...
async def test_baseagent_memory_leak_prevention(test_agents, memory_leak_detector):
    ...
    for batch in range(0, iterations, batch_size):
        ...
        for i in range(batch_size):
            agent = TestableBaseAgent(f"leak_test_agent_{batch}_{i}")
        ...
        for agent in batch_agents:
            for op in range(3):
                state = DeepAgentState()
                state.run_id = f"leak_test_{batch}_{agent.name}_{op}"
                
                # Configure mock response
                agent.llm_manager.chat_completion.return_value = {
                    "content": f"Response for {agent.name} op {op}",
    ...
    assert not leak_analysis["leak_detected"], \
        f"Memory leak detected: {leak_analysis['leak_severity']} - " \
        f"{leak_analysis['memory_increase_mb']:.1f}MB increase over {iterations} iterations"
    
    assert leak_analysis["memory_increase_mb"] < 50, \
        f"Memory usage increased too much: {leak_analysis['memory_increase_mb']:.1f}MB"
    
    # Object cleanup should be effective
    assert leak_analysis["cleanup_success_rate"] > 0.9, \
        f"Poor object cleanup: {leak_analysis['cleanup_success_rate']:.2f} success rate"
    ...
    assert memory_per_iteration < 100, \
        f"Too much memory per iteration: {memory_per_iteration:.1f}KB"
    
    logger.info(f"Memory leak test: {leak_analysis['memory_increase_mb']:.1f}MB increase over "
                f"{iterations} iterations, {leak_analysis['cleanup_success_rate']:.2f} cleanup rate, "
...
async def test_baseagent_resource_exhaustion_handling(test_agents, edge_case_simulator):
    ...
    for resource_type in resource_types:
        ...
        try:
            ...
            for i in range(10):
                state = DeepAgentState()
                state.run_id = f"exhaustion_{resource_type}_{i}"
                
                # Configure mock response
                agent.llm_manager.chat_completion.return_value = {
                    "content": f"Response under {resource_type} stress",
                    "tokens": {"input": 10, "output": 20, "total": 30}
                }
    ...
    successful_tests = [r for r in exhaustion_results if "test_error" not in r]
    assert len(successful_tests) >= len(resource_types) * 0.8, \
        f"Too many resource exhaustion tests failed: {len(successful_tests)}/{len(resource_types)}"
    ...
    for result in successful_tests:
        ...
        assert success_rate > 0.3, \
            f"{resource_type}: Success rate too low under stress: {success_rate:.2f}"
        
        # Execution time should not be extremely high
        if result["avg_execution_time"] > 0:
            assert result["avg_execution_time"] < 10.0, \
                f"{resource_type}: Execution time too high: {result['avg_execution_time']:.2f}s"
    
    logger.info(f"Resource exhaustion test: {len(successful_tests)}/{len(resource_types)} "
...
async def test_baseagent_error_handling_and_recovery(test_agents):
    ...
    for scenario in error_scenarios:
        ...
        state = DeepAgentState()
        state.run_id = f"error_test_{scenario['type']}"
    ...
    properly_handled = [r for r in recovery_results if r["error_handled"]]
    assert len(properly_handled) >= len(error_scenarios) * 0.9, \
        f"Too many errors not handled properly: {len(properly_handled)}/{len(error_scenarios)}"
    
    # State should be preserved during errors
    state_preserved = [r for r in recovery_results if r["state_preserved"]]
    assert len(state_preserved) >= len(error_scenarios) * 0.8, \
        f"State not preserved during errors: {len(state_preserved)}/{len(error_scenarios)}"
    ...
    assert instrumentation["total_errors"] >= len(error_scenarios), \
        f"Errors not properly tracked: {instrumentation['total_errors']} < {len(error_scenarios)}"
