...
class TestRedisMultiUserIsolation(SSotAsyncTestCase):
    ...
    async def test_concurrent_user_operations_no_conflicts(self):
        ...
        async def user1_operations():
            for i in range(10):
                await self.user1_manager.set(f"test:user:1:key_{i}, fuser1_value_{i}")
                await asyncio.sleep(0.01)  # Small delay to allow interleaving
        
        async def user2_operations():
            for i in range(10):
                await self.user2_manager.set(f"test:user:2:key_{i}, fuser2_value_{i}")
                await asyncio.sleep(0.01)  # Small delay to allow interleaving
        ...
        for i in range(10):
            ...
            self.assertEqual(user1_value, f"user1_value_{i}, fUser 1 key {i} should have correct value")
            self.assertEqual(user2_value, f"user2_value_{i}, fUser 2 key {i} should have correct value")
