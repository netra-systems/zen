...
class TestWebSocketSSotSimpleViolations(SSotBaseTestCase):
    ...
    def test_multiple_websocket_implementations_exist(self):
        ...
        for module_path, class_name in test_imports:
            try:
                ...
                if hasattr(module, class_name):
                    ...
                    self.violations_found.append(f"Found implementation: {module_path}.{class_name}")
        ...
        print(f"SSOT VIOLATION CONFIRMED: {len(implementations)} implementations detected")
    ...
    def test_factory_patterns_exist(self):
        ...
        self.assertGreater(
            len(factories), 1,
            f"FACTORY PATTERN VIOLATION: Found {len(factories)} factory patterns. "
            f"Should be exactly 1. Factories: {factories}"
        )
