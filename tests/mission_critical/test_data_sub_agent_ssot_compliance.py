    ...
    pass
        with patch.dict(os.environ, {"TEST_VAR": "should_not_access"}):
            ...
            for pattern in forbidden_patterns:
                assert pattern not in source, f"Found forbidden environment access pattern: {pattern}"
            ...
            with patch.object(DataAnalysisCore, '__init__', return_value=None):
                with patch.object(DataAnalysisCore, 'analyze_performance', new_callable=AsyncMock) as mock_analyze:
                    mock_analyze.return_value = {
                        "data_points": 100,
                        "summary": f"Analysis for {user_id}",
    ...
    return result
        ...
        for i, result in enumerate(results):
            if not isinstance(result, Exception):
                assert result["user_id"] == users[i], f"User isolation failed for user {users[i]}"
                assert result["run_id"].startswith(f"run_{users[i]}_")
            ...
            for i, line in enumerate(lines):
                if 'json.loads' in line or 'json.dumps' in line:
                    ...
                    if 'logger' not in line and 'log' not in line.lower():
                        pytest.fail(f"Found direct JSON operation at line {i}: {line.strip()}")
    ...
    async def test_websocket_event_emission(self, data_agent, user_context):
        ...
        with patch.object(DataAnalysisCore, '__init__', return_value=None):
            with patch.object(DataAnalysisCore, 'analyze_performance', new_callable=AsyncMock) as mock_analyze:
                ...
                thinking_indices = [i for i, (name, _) in enumerate(calls) if 'thinking' in name]
                progress_indices = [i for i, (name, _) in enumerate(calls) if 'progress' in name]
                ...
                for attr_name in dir(agent):
                    if not attr_name.startswith('_'):
                        ...
                        if isinstance(attr_value, str):
                            assert "user1" not in attr_value, f"User data stored in {attr_name}"
                            assert "thread1" not in attr_value, f"Thread data stored in {attr_name}"
                            assert "run1" not in attr_value, f"Run data stored in {attr_name}"
...
')
            for line in lines:
                if pattern in line and not line.strip().startswith('#'):
                    ...
                    if 'try:' in source[max(0, source.index(line)-100):source.index(line)+100]:
                        pytest.fail(f"Found custom retry pattern: {pattern}")
    ...
    pass
        ...
        for pattern in forbidden_patterns:
            assert pattern not in source, f"Found direct config access: {pattern}"
...
')
        ...
        try:
            ...
            if BaseAgent:
                ...
                for method in inherited_methods:
                    ...
                    assert hasattr(data_agent, method), f"Missing method: {method}"
                    agent_method = getattr(data_agent, method, None)
                    assert callable(agent_method), f"Method {method} is not callable"
            else:
            ...
            logger.warning(f"Could not import BaseAgent due to circular dependency: {e}")
            ...
            for method in required_methods:
                assert hasattr(data_agent, method), f"DataSubAgent missing required method: {method}"
    ...
    async def test_memory_leak_prevention(self, mock_llm_manager, mock_tool_dispatcher):
        ...
        initial_values = {attr: getattr(agent, attr) for attr in initial_attrs if not callable(getattr(agent, attr))}
        ...
        assert len(new_attrs) == 0, f"New attributes added during execution: {new_attrs}"
            ...
            if not callable(getattr(agent, attr)):
                ...
                if isinstance(current_value, (list, dict)):
                    if isinstance(initial_value, (list, dict)):
                        ...
                        assert len(str(current_value)) <= len(str(initial_value)) * 2, f"Possible memory leak in {attr}"
...
class TestDataSubAgentStressTests:
        ...
        async def execute_for_user(user_id: int):
            ...
            with patch.object(DataAnalysisCore, '__init__', return_value=None):
                with patch.object(DataAnalysisCore, 'analyze_performance', new_callable=AsyncMock) as mock:
                    mock.return_value = {"data_points": user_id, "user_id": f"stress_user_{user_id}"}
    ...
    return result
        ...
        assert len(successful) >= 95, f"Too many failures: {100 - len(successful)}"
    ...
    # TODO: Initialize real service
        ...
        for i in range(10):
            context = UserExecutionContext(
                ...
                db_session=MockResource(f"session_{i}"),
                metadata={"analysis_type": "performance"}
            )
            ...
            with patch.object(DataAnalysisCore, '__init__', return_value=None):
                with patch.object(DataAnalysisCore, 'analyze_performance', new_callable=AsyncMock) as mock:
                    ...
                    with patch('netra_backend.app.agents.data_sub_agent.data_sub_agent.DatabaseSessionManager') as mock_manager:
                        ...
                        await agent.execute(context, stream_updates=False)
                        assert cleanup_called, f"Cleanup not called for user {i}"
    ...
    pass
        ...
        for pattern in required_patterns:
            assert pattern in module_source, f"Required pattern missing: {pattern}"
        
        # Check forbidden patterns
        for pattern in forbidden_patterns:
            assert pattern not in module_source, f"Forbidden pattern found: {pattern}"
        
        print("✅ All SSOT compliance tests passed!")
...
class TestDataSubAgentEdgeCases:
    ...
    async def test_extremely_large_result_handling(self):
        ...
        large_result = {
            ...
            "metrics": {f"metric_{i}": i for i in range(10000)}
        }
