    ...
    def test_no_duplicate_websocket_methods(self):
        ...
        for filepath in self.agent_files:
            ...
            for method in results.get('websocket_methods', []):
                ...
                location = f"{filepath.name}:{method['line']}"
        ...
        assert len(duplicates) == 0, (
            f"Found {len(duplicates)} duplicate WebSocket methods:
" +
            "
".join([f"  - {d['method']}: {d['count']} occurrences in {', '.join(d['locations'])}" 
                      for d in duplicates])
        )
    ...
    def test_no_mixed_websocket_patterns(self):
        ...
        assert len(mixed_files) == 0, (
            f"Found {len(mixed_files)} files with mixed WebSocket patterns:
" +
            "
".join([f"  - {f['file']}: {f['manager_refs']} manager refs, {f['bridge_refs']} bridge refs"
    ...
    pass
        ...
        assert len(manager_usage) == 0, (
            f"Found {len(manager_usage)} files using WebSocket manager directly:
" +
            "
".join([f"  - {f['file']}: lines {f['lines']}" for f in manager_usage])
        )
    ...
    def test_consistent_event_emission_pattern(self):
        ...
        assert len(inconsistent_patterns) == 0, (
            f"Found {len(inconsistent_patterns)} files with inconsistent emission patterns:
" +
            "
".join([f"  - {f['file']}: {f['non_standard_methods']}" 
    ...
    def test_bridge_methods_are_consistent(self):
        ...
        emit_methods = [m for m in dir(bridge_adapter) if m.startswith('emit_')]
        ...
        for method_name in emit_methods:
            ...
            assert 'self' in params, f"{method_name} missing self parameter"
    ...
    pass
        ...
        print(f"
🎯 WebSocket SSOT Compliance Score: {compliance_score:.1f}%")
        print(f"   Compliant files: {compliant_files}/{total_files}")
        
        if violations:
            print(f"   ❌ Non-compliant files: {', '.join(violations[:5])}")
            if len(violations) > 5:
                print(f"      ... and {len(violations) - 5} more")
        
        # Fail if compliance is below threshold
        assert compliance_score >= 80, (
            f"WebSocket SSOT compliance ({compliance_score:.1f}%) is below 80% threshold. "
            f"Found {len(violations)} non-compliant files."
        )
...
class TestWebSocketBridgeIntegration:
    ...
    async def test_concurrent_agents_using_bridge(self):
        ...
        for i, agent in enumerate(agents):
            agent.set_websocket_bridge(bridge, f"run-{i}")
        
        # Concurrent emissions
        tasks = [
            agent.emit_thinking(f"Thinking from {agent.__class__.__name__}")
            for agent in agents
        ]
...
class TestWebSocketMigrationPath:
    ...
    def test_identify_files_needing_migration(self):
        ...
        if files_to_migrate:
            print("
📋 Files Needing Migration to Bridge Pattern:")
            for f in files_to_migrate:
                print(f"  - {f['file']} ({f['current_pattern']}, {f['manager_refs']} refs)")
    ...
    pass
        ...
        for method in required_methods:
            assert hasattr(adapter, method), f"Bridge adapter missing required method: {method}"
            
            # Verify it's callable
            method_obj = getattr(adapter, method)
            assert callable(method_obj), f"{method} is not callable"
