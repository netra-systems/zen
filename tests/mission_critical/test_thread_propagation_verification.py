    ...
    "
        ...
        try:
            ...
            if websocket_manager and thread_id:
                ...
                # UserExecutionContext should already be imported at module level
            ...
            logger.error(fThreadPropagationTestMessageHandler error: {e})
            return {
                'status': 'error',
...
class ThreadPropagationVerificationTests(SSotAsyncTestCase):
    ...
    def setup_method(self, method):
        ...
        # Initialize services if available - defer WebSocket manager to async methods
        if REAL_SERVICES_AVAILABLE:
            try:
                # ISSUE #556 FIX: WebSocketManager requires async factory pattern
                # Will initialize in async test methods using get_websocket_manager()
        ...
        logger.info(fThread propagation test setup - User: {self.user_id[:8]}, Thread: {self.thread_id[:8]})
    ...
    async def teardown_method(self, method):
        ...
        try:
            ...
            logger.warning(fWebsocket cleanup warning: {e})
    ...
    async def test_websocket_to_message_handler_propagation_FAIL_FIRST(self):
        ...
        # ISSUE #556 FIX: Initialize WebSocketManager using async factory pattern
        if REAL_SERVICES_AVAILABLE and get_websocket_manager:
            try:
                # Create UserExecutionContext for proper WebSocket manager initialization
                user_context = UserExecutionContext(
                    user_id=self.user_id,
                ...
                logger.info(fWebSocket manager initialized with user context: {self.user_id[:8]})
    ...
    async def test_message_handler_to_agent_registry_propagation_FAIL_FIRST(self):
        ...
        try:
            ...
            logger.error(fMessage handler error indicates thread propagation issue: {e})
            raise
...
""
        ...
        try:
            # ISSUE #556 FIX: Initialize WebSocket manager for isolation testing
            if REAL_SERVICES_AVAILABLE and get_websocket_manager:
                # Initialize WebSocket manager with test user context
                ...
                for ctx, conn in connections:
                    try:
                        ...
                        logger.error(fConnection failed for user {ctx['user_id']}: {e})
            ...
            logger.warning(fExpected failure in thread isolation: {e})
            raise
        except Exception as e:
