...
class TestInfrastructureFixesComprehensive:
    ...
    pass
        ...
        for i, context in enumerate(contexts):
            ...
            test_event = {
                "type": f"isolated_test_{i}",
                "user_id": context.user_context.user_id,
                "data": f"context_{i}_data"
            }
            context.event_capture.capture_event(test_event)
            
            # Verify isolation - each context should only have its own events
            assert len(context.event_capture.events) == 1
            assert f"isolated_test_{i}" in context.event_capture.event_types
    ...
    pass
        ...
        primary_user_data = {
            'email': f"{primary_context.user_context.user_id}@test.com",
            'name': f'Primary User {primary_context.user_context.user_id[:8]}',
            'test_data': 'primary_test_data'
        }
        
        secondary_user_data = {
            'email': f"{secondary_context.user_context.user_id}@test.com", 
            'name': f'Secondary User {secondary_context.user_context.user_id[:8]}',
        ...
        primary_key = f"user:{primary_context.user_context.user_id}:data"
        secondary_key = f"user:{secondary_context.user_context.user_id}:data"
    ...
    async def test_per_request_isolation_validation(self, real_services):
        ...
        for i, context in enumerate(contexts):
            request_id = f"request_{i}_{uuid.uuid4().hex[:8]}"
        ...
        for i, context in enumerate(contexts):
            ...
            for event in events:
                ...
                if "request_id" in event:
                    assert f"request_{i}_" in event["request_id"]
                if "tool_name" in event:
                    assert event["tool_name"] == f"isolated_tool_{i}"
                if "result" in event:
                    assert event["result"] == f"isolated_result_{i}"
