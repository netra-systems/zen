...
class SsotComplianceViolation:
    ...
    def __repr__(self):
        return f"SsotComplianceViolation(type={self.violation_type}, location={self.location}, severity={self.severity})"
        ...
        for file_path in self.production_files:
            try:
                ...
                violations.append(SsotComplianceViolation(
                    ...
                    description=f"Unable to parse production file: {e}",
                    severity="critical"
                ))
            ...
            if len(locations) > 1:
                ...
                if len(production_locations) > 1:
                    violations.append(SsotComplianceViolation(
                        ...
                        description=f"Class '{class_name}' defined in multiple production locations: {production_locations}",
                        severity="high"
                    ))
            ...
            if len(definitions) > 1:
                ...
                if len(prod_definitions) > 1:
                    ...
                    violations.append(SsotComplianceViolation(
                        ...
                        description=f"Type '{type_name}' defined in {len(prod_definitions)} production locations",
                        severity=severity
                    ))
        ...
        for class_name, locations in test_base_classes.items():
            if len(locations) > 1:
                violations.append(SsotComplianceViolation(
                    ...
                    location="; ".join(locations),
                    description=f"Test base class '{class_name}' implemented in {len(locations)} files (violates SSOT)",
                    severity="critical"
                ))
        ...
        for mock_name, locations in mock_implementations.items():
            if len(locations) > 1:
                violations.append(SsotComplianceViolation(
                    ...
                    location="; ".join(locations),
                    description=f"Mock class '{mock_name}' implemented in {len(locations)} files (violates SSOT)",
                    severity="high"
                ))
        ...
        for pattern, implementations in auth_implementations.items():
            if len(implementations) > 1:
                violations.append(SsotComplianceViolation(
                    ...
                    description=f"Multiple implementations of auth pattern '{pattern}' violate SSOT principle",
                    severity="critical"
                ))
...
class TestSsotComplianceValidationSuite(SSotBaseTestCase):
    ...
    def test_unit_production_ssot_compliance_gap_detection(self):
        ...
        self.assertGreater(
            ...
            f"Expected 20+ production SSOT violations but found {len(violations)}. "
    ...
    def test_unit_duplicate_type_definitions_detection(self):
        ...
        self.assertGreater(
            ...
            f"Expected 30+ duplicate type definition violations but found {len(violations)}. "
            f"The analysis indicated 89 duplicates - test may need adjustment."
        )
        ...
        for pattern in critical_patterns:
            ...
            self.assertGreater(
                ...
                f"Expected violations for critical SSOT pattern '{pattern}' but found none"
            )
    ...
    def test_integration_test_infrastructure_fragmentation(self):
        ...
        if violations:
            ...
            print(f"Duplicate base classes: {len(duplicate_base_classes)}")
            print(f"Duplicate mock implementations: {len(duplicate_mocks)}")
            print(f"Direct pytest usage: {len(direct_pytest)}")
            print()
        ...
        self.assertGreater(
            ...
            f"Expected 50+ test infrastructure violations but found {len(violations)}. "
    ...
    def test_integration_auth_service_ssot_violations(self):
        ...
        if len(violations) > 0:
            ...
            self.assertGreater(
                ...
                f"Found {len(violations)} auth violations but none were critical - this suggests minor issues only"
            )
    ...
    def test_comprehensive_ssot_compliance_summary(self):
        ...
        print(f"Estimated SSOT Compliance: {compliance_percentage:.1f}%")
        print(f"Business Risk Level: {'CRITICAL' if compliance_percentage < 70 else 'HIGH' if compliance_percentage < 85 else 'MEDIUM'}")
        print()
        ...
        self.assertGreater(
            ...
            f"Expected 75+ total SSOT violations across all categories but found {total_violations}. "
            f"This indicates either the violations have been fixed or the analysis needs adjustment."
        )
        ...
        self.assertGreaterEqual(
            ...
            f"Expected violations in at least 3 categories but found violations in {categories_with_violations} categories"
        )
