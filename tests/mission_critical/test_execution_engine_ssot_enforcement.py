...
class TestExecutionEngineSSotEnforcement(SSotBaseTestCase):
        ...
        try:
            ...
            for method in required_methods:
                self.assertTrue(
                    ...
                    f"UserExecutionEngine missing required method: {method}"
                )
            
        except ImportError as e:
            self.fail(f"CRITICAL: Canonical UserExecutionEngine cannot be imported: {e}")
    ...
    def test_no_legacy_adapter_imports_in_production_code(self):
        ...
        self.assertEqual(
            len(violations), 0,
            f"SSOT VIOLATION: Legacy ExecutionEngine imports found in production code:\n" + 
            "\n".join(violations)
        )
    ...
    def test_single_execution_engine_class_definition(self):
        ...
        self.assertLessEqual(
            ...
            f"SSOT VIOLATION: Multiple ExecutionEngine implementations found (should only be UserExecutionEngine):\n" +
            "\n".join(violations)
        )
        ...
        for legacy_file, description in self.legacy_files.items():
            ...
            if full_path.exists():
                ...
                if not has_deprecation_warning:
                    ...
                    self.addWarning(
                        f"Legacy file {legacy_file} ({description}) exists but lacks deprecation warnings. "
                        f"Should be marked for removal after SSOT consolidation."
                    )
                ...
                for other_file, other_imports in import_graph.items():
                    if file != other_file and any(file.replace('/', '.').replace('.py', '') in imp for imp in other_imports):
                        if any(other_file.replace('/', '.').replace('.py', '') in imp for imp in imports):
                            self.fail(f"CIRCULAR DEPENDENCY detected between {file} and {other_file}")
    ...
    def test_ssot_compliance_execution_engine_imports(self):
        ...
        self.assertLessEqual(
            ...
            f"SSOT VIOLATION: Non-canonical ExecutionEngine imports found:\n" + 
