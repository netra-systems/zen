...
class TestSsotEnvironmentValidationCritical(SSotAsyncTestCase):
    ...
    def test_central_validator_ssot_compliance(self):
        ...
        try:
            ...
            with self.temp_env_vars(ENVIRONMENT="staging"):
                ...
                if env1 != env2:
                    validation_errors.append(f"Environment detection inconsistent: {env1} != {env2}")
            ...
            if missing_envs:
                validation_errors.append(f"Configuration rules missing for environments: {missing_envs}")
            ...
            for var in critical_vars:
                if var not in rules_by_var:
                    validation_errors.append(f"Critical variable {var} missing from validation rules")
                elif rules_by_var[var].requirement not in [ConfigRequirement.REQUIRED, ConfigRequirement.REQUIRED_SECURE]:
                    validation_errors.append(f"Critical variable {var} not marked as required/secure")
            ...
            for var in postgres_vars:
                if var not in rules_by_var:
                    ...
                    validation_errors.append(f"Database variable {var} must be REQUIRED, REQUIRED_SECURE, or OPTIONAL for deployment pattern flexibility")
            ...
            with self.temp_env_vars(ENVIRONMENT="production"):
                ...
                try:
                    ...
                    if "required" not in error_msg.lower():
                        validation_errors.append(f"Central validator error message unclear: {error_msg}")
            ...
            if validation_errors:
                ...
                self.fail(f"Central Configuration Validator SSOT compliance failed:\n" + 
                         "\n".join(f"  - {error}" for error in validation_errors))
            
            self.logger.info(f"Central validator SSOT compliance validated in {validation_time:.1f}ms")
            
        except Exception as e:
            self.record_metric("ssot_validation_critical_error", 1)
            self.fail(f"Critical error in central validator SSOT compliance test: {e}")
    ...
    def test_environment_isolation_ssot_consistency(self):
        ...
        try:
            ...
            self.logger.info(f"Environment isolation SSOT consistency validated in {validation_time:.1f}ms")
            
        except Exception as e:
            self.record_metric("environment_isolation_critical_error", 1)
            self.fail(f"Critical error in environment isolation SSOT test: {e}")
        finally:
            # Cleanup
    ...
    def test_startup_validator_ssot_integration(self):
        ...
        try:
            ...
            if not config_validation_result[0]:  # success, message format
                validation_errors.append(f"Startup validator configuration check failed: {config_validation_result[1]}")
            ...
            with self.temp_env_vars(ENVIRONMENT="staging"):
                ...
                with self.temp_env_vars(JWT_SECRET_STAGING=""):
                    try:
                        ...
                        validation_errors.append(f"Unexpected startup validation error: {e}")
            
            # Test 3: Verify import integrity validation
            import_result = startup_validator._validate_imports()
            if not import_result[0]:
                validation_errors.append(f"Startup validator import check failed: {import_result[1]}")
            ...
            if not signature_result[0]:
                validation_errors.append(f"Startup validator signature check failed: {signature_result[1]}")
            ...
            self.logger.info(f"Startup validator SSOT integration validated in {validation_time:.1f}ms")
            
        except Exception as e:
            self.record_metric("startup_validator_critical_error", 1)
            self.fail(f"Critical error in startup validator SSOT integration test: {e}")
    ...
    def test_backend_configuration_validator_ssot_compliance(self):
        ...
        try:
            ...
            try:
                ...
                validation_errors.append(f"Backend ConfigurationValidator instantiation failed: {e}")
            ...
            if backend_env not in ["development", "testing", "staging", "production"]:
                validation_errors.append(f"Backend validator returned invalid environment: {backend_env}")
            ...
            if validation_errors:
                ...
                self.fail(f"Backend configuration validator SSOT compliance failed:\n" + 
                         "\n".join(f"  - {error}" for error in validation_errors))
            
            self.logger.info(f"Backend configuration validator SSOT compliance validated in {validation_time:.1f}ms")
            
        except Exception as e:
            self.record_metric("backend_validator_critical_error", 1)
            self.fail(f"Critical error in backend configuration validator SSOT test: {e}")
    ...
    def test_cross_environment_validation_consistency(self):
        ...
        try:
                    ...
                    try:
                        ...
                        if detected_env != expected_env:
                            validation_errors.append(
                                f"Environment detection inconsistent for {env_name}: "
                        ...
                        validation_errors.append(f"Environment validation failed for {env_name}: {e}")
            ...
            if validation_errors:
                ...
                self.fail(f"Cross-environment validation consistency failed:\n" + 
                         "\n".join(f"  - {error}" for error in validation_errors))
            ...
            self.fail(f"Critical error in cross-environment validation test: {e}")
    ...
    def _test_environment_consistency(self) -> List[str]:
        ...
        try:
            ...
            if mapped_central_env != backend_env:
                errors.append(f"Environment detection inconsistent: central={mapped_central_env}, backend={backend_env}")
                
        except Exception as e:
            errors.append(f"Environment consistency test failed: {e}")
    ...
    def _test_ssot_env_patterns(self) -> List[str]:
        ...
        try:
            ...
            if not isinstance(env, IsolatedEnvironment):
                errors.append(f"Environment not using IsolatedEnvironment: {type(env)}")
            ...
            errors.append(f"SSOT environment pattern test failed: {e}")
    ...
    def _test_strict_environment_validation(self, validator: CentralConfigurationValidator, env_name: str, errors: List[str]):
        ...
        try:
            ...
            original_jwt_secret = self.get_env_var(f"JWT_SECRET_{env_name.upper()}")
            
            # Clear critical secret
            self.delete_env_var(f"JWT_SECRET_{env_name.upper()}")
            
            try:
                validator.validate_all_requirements()
                errors.append(f"Strict environment {env_name} allowed missing JWT secret")
            except ValueError:
                # Expected behavior
                pass
            except Exception as e:
                errors.append(f"Unexpected error in strict validation for {env_name}: {e}")
            finally:
                # Restore if it existed
                if original_jwt_secret:
                    self.set_env_var(f"JWT_SECRET_{env_name.upper()}", original_jwt_secret)
                ...
                with self.temp_env_vars(JWT_SECRET_KEY="dev-secret-32-characters-long-for-testing"):
                    try:
                        ...
                        if "JWT_SECRET_KEY" in str(e):
                            errors.append(f"Development environment improperly rejected valid JWT_SECRET_KEY: {e}")
                    except Exception as e:
                        errors.append(f"Unexpected error in permissive validation for {env_name}: {e}")
                        
        except Exception as e:
            errors.append(f"Permissive environment validation test failed for {env_name}: {e}")
    ...
    def teardown_method(self, method):
        ...
        try:
            ...
            self.logger.info(
                f"SSOT validation test completed: {method.__name__ if method else 'unknown'} "
                f"Duration: {test_duration:.3f}s, "
                f"Critical validations: {metrics.get('critical_validations_performed', 0)}, "
                f"Violations detected: {metrics.get('ssot_violations_detected', 0)}, "
                f"Revenue protection checks: {metrics.get('revenue_protection_checks', 0)}"
            )
            
        except Exception as e:
            self.logger.warning(f"Error recording test metrics: {e}")
        finally:
