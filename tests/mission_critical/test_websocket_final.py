...
def test_1_websocket_notifier_required_methods():
    ...
    for method in required_methods:
        if not hasattr(notifier, method):
            raise AssertionError(f"CRITICAL: Missing method {method}")
        if not callable(getattr(notifier, method)):
            raise AssertionError(f"CRITICAL: Method {method} not callable")
...
def test_3_execution_engine_websocket_notifier():
    ...
    if not isinstance(engine.websocket_notifier, WebSocketNotifier):
        raise AssertionError(f"CRITICAL: Wrong notifier type: {type(engine.websocket_notifier)}")
...
async def test_4_unified_tool_execution_sends_events():
    ...
    if len(sent_events) < 2:
        raise AssertionError(f"CRITICAL: Expected ≥2 events, got {len(sent_events)}")
    
    # Check for critical event types
    event_types = [event.get('type') for event in sent_events]
    
    if 'tool_executing' not in event_types:
        raise AssertionError(f"CRITICAL: Missing tool_executing event. Got: {event_types}")
    
    if 'tool_completed' not in event_types:
        raise AssertionError(f"CRITICAL: Missing tool_completed event. Got: {event_types}")
    
    print(f"PASS: Enhanced tool execution sent {len(sent_events)} events: {event_types}")
...
async def test_5_all_required_websocket_events():
    ...
    if missing_events:
        raise AssertionError(f"CRITICAL: Missing required events: {missing_events}")
    
    if len(sent_events) < 5:
        raise AssertionError(f"CRITICAL: Expected ≥5 events, got {len(sent_events)}")
    
    print(f"PASS: All {len(sent_events)} required events sent: {sorted(event_types)}")
