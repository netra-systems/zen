...
" + "="*80)
            ...
            print(f"Time since death: {report['time_since_death']:.2f}s" if report['time_since_death'] else "N/A")
            print(f"Empty responses sent: {report['empty_responses']}")
            print(f"Health checks that missed death: {len(report['health_check_failures'])}")
            print("="*80)
...
" + "="*80)
        ...
        print(f"WebSocket messages after death: {message_types}")
...
" + "="*80)
        ...
        print(f"Checks performed: {health_service.checks_performed}")
...
" + "="*80)
        ...
        print("="*80)
        print(f"Total missing: {len(failed_requirements)}/{len(required_checks)}")
        ...
        assert len(failed_requirements) == 0, \
            f"Missing {len(failed_requirements)} critical death detection mechanisms!"
