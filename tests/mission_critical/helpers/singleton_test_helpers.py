...
class IsolationTestResult:
    ...
    def get_failure_summary(self) -> str:
        ...
        if self.is_isolated:
            return "✅ Perfect isolation achieved"
        
        issues = []
        if self.data_leakage_incidents:
            issues.append(f"{len(self.data_leakage_incidents)} data leakage incidents")
        if self.shared_state_violations:
            issues.append(f"{len(self.shared_state_violations)} shared state violations")
        if self.cross_user_contamination:
            issues.append(f"{len(self.cross_user_contamination)} cross-user contamination")
        if self.race_conditions_detected:
            issues.append(f"{self.race_conditions_detected} race conditions")
        
        singleton_issues = [
            f"{component}({count}/{self.users_tested})" 
            for component, count in self.singleton_components_detected.items() 
            if count < self.users_tested
        ]
        if singleton_issues:
            issues.append(f"Singleton components: {', '.join(singleton_issues)}")
        
        return f"❌ Isolation failed: {'; '.join(issues)}"
...
class SingletonDetector:
    ...
    def detect_singleton_instances(
        ...
        for component_name, instances in components.items():
            instance_ids = [id(instance) for instance in instances if instance is not None]
    ...
    def validate_factory_uniqueness(
        ...
        for i in range(iterations):
            try:
                ...
                issues.append(f"Factory call {i+1} failed: {e}")
        ...
        if len(unique_ids) != len(instances):
            ...
            issues.append(
                f"Factory returned shared instances: {len(instances)} calls produced "
                f"only {len(unique_ids)} unique instances ({duplicate_count} duplicates)"
            )
            ...
            duplicated_ids = [iid for iid, count in id_counts.items() if count > 1]
            issues.append(f"Duplicated instance IDs: {duplicated_ids}")
...
class ConcurrentUserSimulator:
    ...
    def create_mock_users(
        ...
        for i in range(count):
            ...
            user_id = f"{user_id_prefix}_{i}_{uuid.uuid4().hex[:8]}"
            ...
            session_data = {
                ...
                'user_state': f'state_{user_id}_{time.time()}'
            }
            
            private_data = {
                'email': f'{user_id}@example.com',
                'api_keys': [f'key_{i}_{uuid.uuid4().hex}' for _ in range(3)],
                'private_documents': [f'doc_{i}_{j}' for j in range(5)]
            }
            
            user_secrets = {}
            if include_secrets:
                user_secrets = {
                    'auth_secret': f'secret_{user_id}_{uuid.uuid4().hex}',
                    'encryption_key': f'key_{user_id}_{uuid.uuid4().hex}',
                    'private_token': f'token_{user_id}_{uuid.uuid4().hex}'
                }
