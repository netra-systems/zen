...
class TestWebSocketEventsRouting(SSotBaseTestCase):
    ...
    def test_user_specific_event_delivery(self):
        ...
        async def test_event_delivery():
            try:
                ...
                users = []
                for i in range(3):
                    user_id = UnifiedIdGenerator.generate_base_id(f"event_user_{i}", True, 8)
                    thread_id, run_id, _ = UnifiedIdGenerator.generate_user_context_ids(user_id, f"event_test_{i}")
                ...
                for i, user in enumerate(users):
                    ...
                    await emitter.notify_agent_started(f"agent_{i}", {
                        "user_specific_data": f"data_for_user_{i}",
                        "agent_id": f"agent_{i}"
                    })
                    
                    await emitter.notify_agent_thinking(f"agent_{i}", f"User {i} is thinking", i+1)
                    
                    await emitter.notify_tool_executing(f"tool_{i}", {
                        "tool_name": f"user_tool_{i}",
                        "parameters": {"user": i}
                    })
                    
                    await emitter.notify_tool_completed(f"tool_{i}", {
                        "result": f"result_for_user_{i}",
                        "success": True
                    })
                    
                    await emitter.notify_agent_completed(f"agent_{i}", {
                        "final_result": f"completed_for_user_{i}",
                        "user_id": user_id
                    })
                ...
                for i, user in enumerate(users):
                    ...
                    if len(events) == 0:
                        ...
                        self.event_delivery_issues.append(f"User {user_id} received no events")
                        continue
                    ...
                    for event in events:
                        ...
                        if isinstance(event_data, dict):
                            ...
                            if (f"data_for_user_{i}" in str(event_data) or
                                f"result_for_user_{i}" in str(event_data) or
                                f"completed_for_user_{i}" in str(event_data)):
                                user_specific_found = True
                            
                            # Check for contamination from other users
                            for j in range(3):
                                if j != i:
                                    if (f"data_for_user_{j}" in str(event_data) or
                                        f"result_for_user_{j}" in str(event_data)):
                                        cross_user_contamination = True
                                        self.cross_user_leaks.append(
                                            f"User {user_id} received data for user {j}: {event_data}"
                                        )
                    
                    if not user_specific_found:
                        self.routing_violations.append(f"User {user_id} did not receive user-specific events")
                    
                    logger.info(f"User {i} ({user_id}): Received {len(events)} events, user-specific: {user_specific_found}")
                ...
                self.routing_violations.append(f"Event delivery test failed: {e}")
    ...
    def test_concurrent_websocket_events(self):
        ...
        async def concurrent_event_sender(user_index: int, event_count: int = 20):
            ...
            try:
                ...
                user_id = UnifiedIdGenerator.generate_base_id(f"concurrent_event_user_{user_index}", True, 8)
                ...
                for i in range(event_count):
                    ...
                    if i % 5 == 0:
                        task = emitter.notify_agent_started(f"agent_{i}", {"event_index": i})
                    elif i % 5 == 1:
                        task = emitter.notify_agent_thinking(f"agent_{i}", f"Thinking {i}", i)
                    elif i % 5 == 2:
                        task = emitter.notify_tool_executing(f"tool_{i}", {"tool_index": i})
                    elif i % 5 == 3:
                        task = emitter.notify_tool_completed(f"tool_{i}", {"result_index": i})
                    else:
                        task = emitter.notify_agent_completed(f"agent_{i}", {"completion_index": i})
                ...
                logger.error(f"Concurrent event sender failed for user {user_index}: {e}")
                return {
                    'user_index': user_index,
                    'error': str(e),
                    'success': False
        ...
        async def test_concurrent_events():
            ...
            for result in results:
                if isinstance(result, dict) and result.get('success', False):
                    ...
                    logger.info(f"User {result['user_index']}: Sent {result['events_sent']}/{result['expected_events']} events")
                else:
                    self.performance_issues.append(f"Concurrent event test failed: {result}")
            ...
            self.assertEqual(total_events_sent, expected_total_events, f"All {expected_total_events} events should be sent")
            self.assertTrue(performance_ok, f"Event throughput should be adequate (got {events_per_second:.1f} events/sec)")
        ...
        async def test_data_integrity():
            try:
                ...
                for i, test_data in enumerate(test_data_sets):
                    await websocket_emitter.notify_agent_started(f"integrity_agent_{i}", test_data)
                    await websocket_emitter.notify_tool_executing(f"integrity_tool_{i}", {
                        "original_data": test_data,
                        "test_index": i
                    })
                    await websocket_emitter.notify_agent_completed(f"integrity_agent_{i}", {
                        "result": test_data,
                ...
                for i, event in enumerate(captured_events):
                    ...
                    if not isinstance(event_data, dict):
                        ...
                        self.event_delivery_issues.append(f"Event {i}: Data not dictionary - {type(event_data)}")
                        continue
                    ...
                    if "integrity_check" in str(event_data):
                        ...
                        if "result" not in event_data:
                            data_corruption_found = True
                            self.event_delivery_issues.append(f"Event {i}: Missing result in completion event")
                    
                    if "unicode" in str(event_data):
                        # Check unicode preservation
                        if "🚀" not in str(event_data) or "測試" not in str(event_data):
                            data_corruption_found = True
                            self.event_delivery_issues.append(f"Event {i}: Unicode characters corrupted")
                    ...
                    if not user_context_found and "user_context" in str(event_data):
                        data_corruption_found = True
                        self.event_delivery_issues.append(f"Event {i}: User context corrupted")
                ...
                self.event_delivery_issues.append(f"Data integrity test failed: {e}")
            ...
            try:
                ...
                for i, (event, expected_type) in enumerate(zip(ordered_events, expected_sequence)):
                    if expected_type not in event['event_type']:
                        ordering_violations.append(f"Position {i}: Expected {expected_type}, got {event['event_type']}")
                ...
                # Validate ordering
                self.assertEqual(len(ordering_violations), 0, f"Event ordering violations: {ordering_violations}")
                
                logger.info(f"✅ PASS: Event ordering maintained - {len(ordered_events)} events in correct sequence")
                return True
                
            except Exception as e:
                self.routing_violations.append(f"Event ordering test failed: {e}")
    ...
    def test_comprehensive_websocket_routing_report(self):
        ...
        self.assertEqual(
            ...
            f"Found {routing_summary['total_issues']} issues: Security Risk: {routing_summary['security_risk']}, "
            f"Performance Risk: {routing_summary['performance_risk']}"
        )
