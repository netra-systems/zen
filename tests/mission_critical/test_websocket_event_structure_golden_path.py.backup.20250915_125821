...
class TestWebSocketEventStructureGoldenPath(RealWebSocketTestBase):
                ...
                if event_type == "agent_started":
                    ...
                    for field in required_top_level_fields:
                        if field not in transmitted_event:
                            structure_validation_failures.append(
                                f"GOLDEN PATH FAILURE: {event_type} missing top-level {field}. "
                    ...
                    for field in required_top_level_fields:
                        if field not in transmitted_event:
                            structure_validation_failures.append(
                                ...
                                f"Tool transparency broken. Event: {transmitted_event}"
                            )
                    ...
                    if transmitted_event.get("tool_name") != event["tool_name"]:
                        structure_validation_failures.append(
                            ...
                            f"Expected: {event['tool_name']}, Got: {transmitted_event.get('tool_name')}"
                        )
                    ...
                    if "results" in transmitted_event and isinstance(transmitted_event["results"], dict):
                        ...
                        if transmitted_results != original_results:
                            structure_validation_failures.append(
                                f"GOLDEN PATH FAILURE: {event_type} results structure corrupted. "
                    ...
                    for field in required_top_level_fields:
                        if field not in transmitted_event:
                            structure_validation_failures.append(
                                f"GOLDEN PATH CRITICAL FAILURE: {event_type} missing top-level {field}. "
                                f"Completion status broken. Event: {transmitted_event}"
                            )
    ...
    async def test_websocket_transmission_end_to_end_structure(self):
            ...
            if event_type == "tool_executing":
                if "tool_name" not in transmitted_event:
                    transmission_failures.append(
                        f"tool_executing transmission missing tool_name: {transmitted_event}"
                    )
                ...
                if "tool_name" not in transmitted_event:
                    transmission_failures.append(
                        f"tool_completed transmission missing tool_name: {transmitted_event}"
                    )
                if "results" not in transmitted_event:
                    transmission_failures.append(
                        f"tool_completed transmission missing results: {transmitted_event}"
                    )
                if "execution_time" not in transmitted_event:
                    transmission_failures.append(
                        f"tool_completed transmission missing execution_time: {transmitted_event}"
                    )
                ...
                if "results" in transmitted_event:
                    ...
                    if transmitted_results != original_results:
                        transmission_failures.append(
                            f"tool_completed results structure corrupted during transmission"
                        )
        ...
        if transmission_failures:
            ...
            pytest.fail(
                f"MISSION CRITICAL: WebSocket transmission structure failures (Issue #1021):\n\n"
                f"{failure_summary}\n\n"
                f"IMPACT: Users cannot access business data from WebSocket events.\n"
    ...
    async def test_frontend_event_consumption_compatibility(self):
        ...
        if frontend_compatibility_failures:
            ...
            pytest.fail(
                f"MISSION CRITICAL: Frontend compatibility failures (Issue #1021):\n\n"
                f"{compatibility_summary}\n\n"
                f"IMPACT: Frontend JavaScript cannot access WebSocket event business data.\n"
                f"USER IMPACT: No real-time agent progress, tool transparency, or completion status.\n"
                f"REVENUE IMPACT: Chat functionality (90% of platform value) broken for users."
            )