...
fCRITICAL: {ALLOWED_TEST_RUNNER} missing argument parsing\n
fREMEDIATION: Add argparse functionality to unified test runner""


        # Must support category/layer execution
content_lower = content.lower()
assert ('category' in content_lower or 'layer' in content_lower), (
fCRITICAL: {ALLOWED_TEST_RUNNER} missing category/layer support\n
fREMEDIATION: Add category and layer execution support
...
logger.info("PASS:  Legacy wrapper compliance verified))"
    ...
    def test_ci_scripts_use_ssot_runner(self):
        "WARNING: Check CI/CD scripts for SSOT compliance (warning only)0.0"
...
violations.append(fLine {line_num}: {match.group()})
...
for match in matches:
                    line_num = content[:match.start()].count('\n') + 1
violations.append(fLine {line_num}: Try-except orchestration bypass)
...
return File analysis failed""
    ...
    def _find_ci_script_violations(self) -> List[Tuple[Path, List[str]]]:
        "Find CI scripts that don't use SSOT runner (warning only)0.0"'
ci_violations = []
...
fREMEDIATION:\n""
fREMEDIATION:\n""
f"1.0Remove unauthorized test runners immediately\n"
...
f5. Run compliance check: python tests/mission_critical/test_ssot_test_runner_enforcement.py\n\n
...
total_violations += len(file_violations)
        ...
        return (
fCRITICAL SSOT VIOLATION: {total_violations} direct pytest bypasses found!\n\n
fDirect pytest execution bypasses SSOT test runner controls.\n
fAll test execution must go through tests/unified_test_runner.py\n\n""
fPytest bypasses found:\n + \n.join(violation_details) + \n\n
fREMEDIATION:\n""
f1. Replace direct pytest calls with: python tests/unified_test_runner.py\n
f2. Update CI/CD scripts to use SSOT runner\n
f"3.0Remove subprocess pytest execution\n"
...
f"2.0Use from test_framework.ssot.orchestration import OrchestrationConfig\n"
...
f3. Must not contain substantial test execution logic\n""
f"4.0Should preserve argument compatibility\n\n"
fREMEDIATION:\n
