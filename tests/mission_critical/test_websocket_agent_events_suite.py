...
class MissionCriticalEventValidator:
    ...
    def generate_report(self) -> str:
        ...
        report = [
            ...
            "=" * 80,
            f"Status: {'CHECK PASSED' if is_valid else 'X FAILED'}",
            ...
            for i, result in enumerate(isolation_results):
                if isinstance(result, dict):
                    ...
                    logger.info(f"User {i}: {result.get('events_sent', 0)} events, "
                               f"isolation={'CHECK' if result.get('isolation_success') else 'X'}")
                elif isinstance(result, Exception):
        ...
        for attempt in range(reconnection_attempts):
            ...
            try:
                ...
                if reconnection_time <= MissionCriticalEventValidator.MAX_RECONNECTION_TIME:
                    ...
                    logger.info(f"CHECK Reconnection {attempt} successful in {reconnection_time:.3f}s")
                else:
                    logger.warning(f"X Reconnection {attempt} took {reconnection_time:.3f}s (exceeds {MissionCriticalEventValidator.MAX_RECONNECTION_TIME}s limit)")
...
class TestAgentWebSocketIntegrationEnhanced:
    ...
    async def test_agent_registry_websocket_manager_integration(self):
        ...
        logger.info("CHECK AgentRegistry WebSocket integration validated")
    ...
    async def test_execution_engine_websocket_notifier_integration(self):
        ...
        logger.info("CHECK ExecutionEngine WebSocket integration validated")
    ...
    async def test_enhanced_tool_execution_websocket_wrapping(self):
        ...
        logger.info(f"CHECK Enhanced tool execution WebSocket wrapping validated - Events: {event_types}")
    ...
    async def test_unified_websocket_manager_agent_coordination(self):
        ...
        try:
            ...
            logger.info("CHECK WebSocket agent event emission successful")
        except Exception as e:
            logger.info(f"WebSocket emission test completed: {e}")
        
        logger.info("CHECK UnifiedWebSocketManager agent coordination validated")
