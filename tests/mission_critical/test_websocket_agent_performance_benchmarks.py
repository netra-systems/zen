...
class AgentPerformanceMonitor:
    ...
    def record_resource_usage(self) -> None:
        ...
        try:
            ...
            logger.warning(fResource monitoring error: {e})
...
class RealAgentPerformanceTester:
    ...
    async def execute_performance_agent_workflow(
        ...
        user_context = UserExecutionContext.create_for_request(
            user_id=user_id,
            request_id=fperf_req_{uuid.uuid4().hex[:8]},
            thread_id=fperf_thread_{uuid.uuid4().hex[:8]}""
        )
        ...
        try:
            ...
            logger.info(fPerformance workflow completed: {e}")"
    ...
    async def _execute_fast_workflow(self, context: AgentExecutionContext, workflow_type: str):
        ...
        await context.websocket_notifier.send_event(tool_executing, {
            "tool_name: ffast_{workflow_type}_tool"
        }
    ...
    async def _execute_intensive_workflow(self, context: AgentExecutionContext, workflow_type: str):
        ...
        await context.websocket_notifier.send_event("tool_executing, {"
            tool_name: fintensive_{workflow_type}_analyzer,
            complexity: high
        }
...
""
        ...
        try:
            async def latency_test_user(user_index: int) -> Dict[str, Any]:
                ...
                user_id = flatency_user_{user_index:02d}
        ...
        assert sla_compliance[meets_95_percent_sla], \"
            ...
            fViolations: {sla_compliance['total_violations']}/{sla_compliance['total_events']}
        ...
        for event_type in required_events:
            if event_type in event_performance:
                ...
                assert event_stats[p95_ms] <= 200.0, \
                    fEvent {event_type} P95 latency {event_stats['p95_ms']:.1f}ms > 200ms SLA"
...
""
        ...
        for user_count in user_scaling:
            logger.info(fTesting {user_count} concurrent users...)"
            ...
            async def scaling_user(user_index: int) -> Dict[str, Any]:
                ...
                user_id = fscale_{user_count}_user_{user_index:02d}""
            ...
            assert performance_report[sla_compliance"][meets_95_percent_sla], \
                fPerformance degraded at {user_count} users:  \
        ...
        assert min_throughput >= max_throughput * 0.5, \
            fThroughput degraded significantly: {min_throughput:.1f} vs {max_throughput:.1f} users/s""
...
""
        ...
        async def high_throughput_agent(agent_index: int) -> Dict[str, Any]:
            ...
            agent_id = fthroughput_agent_{agent_index:02d}
            execution_results = []
        ...
        assert throughput_per_second >= 50.0, \
            fInsufficient throughput: {throughput_per_second:.1f} executions/s < 50 required
        ...
        if resource_performance:
            assert resource_performance[cpu_max"] <= 90.0, \
                fCPU usage too high: {resource_performance['cpu_max']:.1f}% > 90%
...
"
        ...
        async def loaded_websocket_connection(connection_index: int) -> Dict[str, Any]:
            ...
            user_context = UserExecutionContext.create_for_request(
                ...
                request_id=fload_req_{uuid.uuid4().hex[:8]}"
            )
        ...
        assert performance_report["sla_compliance][meets_95_percent_sla], \
            fWebSocket load SLA violation: {performance_report['sla_compliance']['compliance_rate']:.1%} < 95%
        
        assert len(successful_connections) >= connection_count * 0.95, \
            fConnection success rate too low: {len(successful_connections)}/{connection_count}"
        ...
        assert overall_avg_latency <= 150.0, \
            fAverage latency too high under load: {overall_avg_latency:.1f}ms > 150ms
