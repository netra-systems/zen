...
class TestMessageRouterImportPathMigration(SSotAsyncTestCase):
    ...
    async def test_legacy_import_path_compatibility(self):
        ...
        try:
            ...
            self.migration_tracking["successful_imports"].append({
                "path": "netra_backend.app.core.message_router",
                "type": "legacy_proxy",
            ...
            self.migration_tracking["failed_imports"].append({
                "path": "netra_backend.app.core.message_router",
                "error": str(e),
                "time": time.time() - start_time
            })
    ...
    async def test_canonical_import_path_stability(self):
        ...
        try:
            ...
            # Test instance creation and singleton
    ...
    async def test_services_import_path_compatibility(self):
        ...
        try:
            ...
            # Should not be a proxy (should be canonical)
            self.assertFalse(hasattr(router, '_canonical_router'))
            
            self.migration_tracking["successful_imports"].append({
                "path": "netra_backend.app.services.message_router",
    ...
    async def test_import_path_migration_gradual_transition(self):
        ...
        migration_steps = [
            {
                "name": "legacy_proxy",
                "import": "from netra_backend.app.core.message_router import MessageRouter",
                "expected_type": "proxy"
            },
            {
                "name": "services_canonical",
                "import": "from netra_backend.app.services.message_router import MessageRouter",
                "expected_type": "canonical"
            },
            {
                "name": "direct_canonical",
                "import": "from netra_backend.app.websocket_core.handlers import MessageRouter",
                "expected_type": "canonical"
            }
        ]
            ...
            try:
                ...
                if step["expected_type"] == "proxy":
                    self.assertTrue(hasattr(router, '_canonical_router'),
                                   f"Step {step['name']} should be proxy but isn't")
                else:
                    self.assertFalse(hasattr(router, '_canonical_router'),
                                    f"Step {step['name']} should be canonical but is proxy")
    ...
    async def test_import_path_functional_equivalence(self):
        ...
        for name, router in routers:
            ...
            for method in required_methods:
                self.assertTrue(hasattr(router, method),
                               f"Router from {name} missing {method}")
            
            # All should handle basic operations
    ...
    async def test_migration_impact_on_websocket_integration(self):
        ...
        # Test WebSocket manager with different router import paths
        ws_manager = WebSocketManager(user_context=self.user_context)
        
        # WebSocket manager should work regardless of import path used internally
        self.assertIsNotNone(ws_manager)
        
        # Test that manager can get router instances
        # (Implementation may vary, but basic functionality should work)
        try:
            # Basic WebSocket manager functionality
            self.assertTrue(hasattr(ws_manager, 'user_context'))
            self.assertEqual(ws_manager.user_context.user_id, self.test_user_id)
            
            self.migration_tracking["successful_imports"].append({
                "integration": "websocket_manager",
                "status": "compatible",
                "time": time.time()
            })
            
        except Exception as e:
            self.migration_tracking["compatibility_issues"].append({
                "integration": "websocket_manager",
                "error": str(e),
                "time": time.time()
            })
    ...
    async def test_import_path_performance_impact(self):
        ...
        import_paths = [
            ("core", "from netra_backend.app.core.message_router import MessageRouter"),
            ("handlers", "from netra_backend.app.websocket_core.handlers import MessageRouter"),
            ("services", "from netra_backend.app.services.message_router import MessageRouter")
        ]
        ...
        for path_name, import_statement in import_paths:
            ...
            try:
                ...
                self.migration_tracking["performance_metrics"][path_name] = {
                    "import_time": import_time,
                    "instance_time": instance_time,
                    "method_time": method_time,
                    "total_time": import_time + instance_time + method_time,
        ...
        if len(self.migration_tracking["performance_metrics"]) > 1:
            times = {name: metrics.get("total_time", float('inf')) 
                    for name, metrics in self.migration_tracking["performance_metrics"].items()
            ...
            if times:
                ...
                # Performance difference should be minimal (< 50ms)
                self.assertLess(slowest - fastest, 0.05,
                               "Import path performance difference too large")
    ...
    async def test_circular_import_prevention_during_migration(self):
        ...
        # Track import order to detect potential cycles
        import_order = []
        ...
        try:
            # Temporarily replace import to track
            ...
            # Create instances to trigger any lazy imports
            core_router = CoreRouter()
            handlers_router = HandlersRouter()
            services_router = ServicesRouter()
        ...
        self.assertEqual(len(import_order), len(unique_imports),
                        f"Potential circular imports detected: {import_order}")
    ...
    async def test_migration_rollback_capability(self):
        ...
        # Test that legacy import paths still work (rollback capability)
        try:
            ...
            # Should be proxy (indicating rollback capability exists)
            router = LegacyRouter()
            self.assertTrue(hasattr(router, '_canonical_router'))
            
            self.migration_tracking["successful_imports"].append({
                "capability": "rollback_legacy",
                "status": "available",
                "time": time.time()
            })
    ...
    def test_migration_tracking_summary(self):
        ...
        migration_summary = {
            ...
            "compatibility_issues": issue_count,
            "success_rate": (successful_count / (successful_count + failed_count)) * 100 if (successful_count + failed_count) > 0 else 0,
            "performance_metrics": self.migration_tracking["performance_metrics"],
            "details": {
                "successful": self.migration_tracking["successful_imports"],
                "failed": self.migration_tracking["failed_imports"],
                "issues": self.migration_tracking["compatibility_issues"]
            }
        }
        ...
        self.assertGreaterEqual(migration_summary["success_rate"], 90.0,
                               "Migration success rate should be at least 90%")
        self.assertLessEqual(failed_count, 1,
                            "Should have 1 or fewer failed imports")
        self.assertLessEqual(issue_count, 2,
                            "Should have 2 or fewer compatibility issues")
