...
""
        ...
        with patch('netra_backend.app.services.unified_authentication_service.get_unified_auth_service') as mock_get_service:
            ...
            websocket_subprotocol = self.create_mock_websocket({
                sec-websocket-protocol: fjwt.{jwt_token}
            }
            ...
            self.assertTrue(
                ...
                fWebSocket header authentication should succeed like HTTP. Error: {websocket_result.error_message}
            )
...
"""Empty docstring."""
        ...
        if critical_missing:
            inconsistencies.append(fCritical env vars missing: {critical_missing}")"
...
""
        ...
        if issues:
            raise AssertionError(fAuth service method issues: {issues}")"
        ...
        if method_comparison[http_auth_method] and method_comparison[websocket_auth_method]:
            ...
            try:
                ...
                issues.append(fMethod parity test failed: {e})
        
        if issues:
            raise AssertionError(fMethod parity issues: {issues}")"
