...
')
        ...
        for i, line in enumerate(lines, 1):
            ...
            if 'get_env()' in line and 'database' in line.lower():
                patterns['isolated_env_usage'].append(f"{file_path}:{i} - {line}")
            
            # Database URL construction
            if 'postgresql://' in line or 'postgres://' in line:
                patterns['database_url_construction'].append(f"{file_path}:{i} - {line}")
            ...
            if 'config.' in line and any(db_term in line.lower() for db_term in ['db', 'database', 'postgres']):
                if 'get_env()' not in content and 'IsolatedEnvironment' not in content:
                    patterns['config_inconsistencies'].append(f"{file_path}:{i} - {line}")
...
class DatabaseConfigurationTester:
    ...
    async def test_postgres_url_construction(self, config_vars: Dict[str, str]) -> Dict[str, Any]:
        ...
        try:
            ...
            results['issues'].append(f"Direct construction failed: {e}")
        ...
        try:
            ...
            isolated_url = f"postgresql://{user}:{password}@{host}:{port}/{db}"
            ...
            results['issues'].append(f"IsolatedEnvironment construction failed: {e}")
        ...
        try:
            ...
            results['issues'].append(f"Config class construction failed: {e}")
...
Verifying original environment unchanged:")
        for key, original_value in original_postgres_vars.items():
            ...
            if original_value is None:
                assert current_value is None or key not in os.environ, f"Original None value changed for {key}"
            else:
