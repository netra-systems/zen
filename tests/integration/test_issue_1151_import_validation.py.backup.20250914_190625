...
class TestFirstMessageExperienceImports:
    ...
    def test_first_message_experience_imports_resolve(self):
        ...
        # Get the path to the previously failing test file
        ...
        # This should not raise ImportError - the fix should resolve the issue
        try:
            spec.loader.exec_module(module)
        except ImportError as e:
            pytest.fail(f"ImportError still occurring after fix: {e}")
        except Exception as e:
            # Other exceptions are acceptable (e.g., missing Docker services)
            # We're only testing import resolution, not execution
    ...
    def test_specific_failing_import_statement(self):
        ...
        # This tests the exact import pattern from lines 51-55 that was failing
        try:
            from tests.mission_critical.websocket_real_test_base import (
                is_docker_available,        # <-- This was the missing function
                RealWebSocketTestConfig,
                send_test_agent_request
            )

            # Verify all imports are available and correct types
    ...
    def test_websocket_real_test_base_module_completeness(self):
        ...
        # All functions/classes that mission-critical tests expect to import
        expected_exports = {
            'is_docker_available': 'function',           # NEW - Issue #1151 fix
            'require_docker_services': 'function',       # Existing
            'require_docker_services_smart': 'function', # Existing
            'RealWebSocketTestConfig': 'class',          # Existing
            'send_test_agent_request': 'function',       # Existing
            'RealWebSocketTestBase': 'class',            # Main test base class
            'get_websocket_test_base': 'function',       # Factory function
        }
    ...
    def test_import_no_side_effects(self):
        ...
        # Import should not modify sys.modules unexpectedly
        assert 'tests.mission_critical.websocket_real_test_base' in sys.modules

        # Function should be callable without side effects
...
class TestMissionCriticalTestCollectability:
    ...
    def test_mission_critical_tests_discoverable(self):
        ...
        try:
            ...
            # Run pytest collection on mission-critical tests
            result = subprocess.run(
                [sys.executable, "-m", "pytest", "tests/mission_critical/", "--collect-only", "-q"],
                capture_output=True,
                text=True,
                timeout=30
            )

            # Collection should succeed (exit code 0 or 5 for no tests)
            assert result.returncode in [0, 5], f"Test collection failed with code {result.returncode}: {result.stderr}"

            # Should not contain ImportError messages
    ...
    def test_first_message_experience_test_collectible(self):
        ...
        try:
            ...
            # Run pytest collection specifically on the previously failing file
            result = subprocess.run(
            ...
            # Should not contain the specific ImportError from Issue #1151
            assert "cannot import name 'is_docker_available'" not in result.stderr
            assert "ImportError" not in result.stderr, f"ImportError still present: {result.stderr}"

            # Should show test collection progress
    ...
    def test_websocket_tests_collection_no_regression(self):
        ...
        try:
            ...
            # Run pytest collection on WebSocket-related tests
            result = subprocess.run(
                [sys.executable, "-m", "pytest",
                 "tests/mission_critical/", "-k", "websocket",
                 "--collect-only", "-v"],
                capture_output=True,
                text=True,
                timeout=30
...
class TestBackwardsCompatibility:
    ...
    def test_existing_docker_functions_still_work(self):
        ...
        # Functions should have proper docstrings
    ...
    def test_real_websocket_test_config_unchanged(self):
        ...
        # Should have expected attributes (from original implementation)
        expected_attributes = [
            'backend_url', 'websocket_url', 'connection_timeout',
            'event_timeout', 'max_retries', 'docker_startup_timeout'
        ]
    ...
    def test_send_test_agent_request_unchanged(self):
        ...
        # Function should be callable and async
    ...
    def test_unified_docker_manager_integration_preserved(self):
        ...
        # Results should be consistent (same Docker detection logic)
