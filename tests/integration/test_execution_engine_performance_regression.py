...
class PerformanceMetrics:
    ...
    def to_dict(self) -> Dict[str, float]:
        ...
        return {'execution_time_ms': self.execution_time_ms, 'memory_used_mb': self.memory_used_mb, 'cpu_percent': self.cpu_percent, 'websocket_event_latency_ms': self.websocket_event_latency_ms, 'concurrent_users_supported': self.concurrent_users_supported, 'resource_cleanup_time_ms': self.resource_cleanup_time_ms, 'peak_memory_mb': self.peak_memory_mb, 'events_per_second': self.events_per_second}
...
class ExecutionEnginePerformanceRegressionTests(SSotAsyncTestCase):
    ...
    async def asyncSetUp(self):
        ...
        self.performance_targets = {'max_execution_time_ms': 2000, 'max_memory_per_user_mb': 50, 'max_cpu_percent': 80, 'max_websocket_latency_ms': 100, 'min_concurrent_users': 5, 'max_cleanup_time_ms': 500}
        self.test_users = [{'user_id': f'perf_user_{i:03d}', 'username': f'user{i}@performance.test', 'session_id': f'perf_session_{i}_{uuid.uuid4().hex[:8]}'} for i in range(10)]
    ...
    async def _warmup_system(self) -> None:
        ...
        try:
            ...
            context = UserExecutionContext(user_id='warmup_user', session_id='warmup_session', request_id='warmup_request')
    ...
    async def test_single_user_execution_performance(self):
        ...
        try:
            ...
            if 'UserExecutionContext' in globals():
                context = UserExecutionContext(user_id=user_data['user_id'], session_id=user_data['session_id'], request_id=f'perf_test_{int(time.time())}')
                engine = UserExecutionEngine(user_context=context)
            else:
            ...
            self.assertLess(metrics.execution_time_ms, self.performance_targets['max_execution_time_ms'], f"Execution time {metrics.execution_time_ms:.1f}ms exceeds target {self.performance_targets['max_execution_time_ms']}ms")
            self.assertLess(metrics.memory_used_mb, self.performance_targets['max_memory_per_user_mb'], f"Memory usage {metrics.memory_used_mb:.1f}MB exceeds target {self.performance_targets['max_memory_per_user_mb']}MB")
            self.assertLess(abs(metrics.cpu_percent), self.performance_targets['max_cpu_percent'], f"CPU usage {metrics.cpu_percent:.1f}% exceeds target {self.performance_targets['max_cpu_percent']}%")
            if metrics.websocket_event_latency_ms > 0:
                self.assertLess(metrics.websocket_event_latency_ms, self.performance_targets['max_websocket_latency_ms'], f"WebSocket latency {metrics.websocket_event_latency_ms:.1f}ms exceeds target {self.performance_targets['max_websocket_latency_ms']}ms")
            self.assertGreater(metrics.total_events_sent, 0, 'Should send WebSocket events during execution')
    ...
    async def test_concurrent_users_performance_scaling(self):
        ...
        for user_count in concurrent_user_counts:
            ...
            try:
                ...
                async def execute_user(user_data):
                    ...
                    try:
                        if 'UserExecutionContext' in globals():
                            context = UserExecutionContext(user_id=user_data['user_id'], session_id=user_data['session_id'], request_id=f'concurrent_test_{int(time.time())}')
                            engine = UserExecutionEngine(user_context=context)
                        else:
                            engine = UserExecutionEngine()
                        ...
                        user_results[user_data['user_id']] = {'success': True, 'execution_time_ms': execution_time}
                    except Exception as e:
                        user_results[user_data['user_id']] = {'success': False, 'error': str(e)}
                ...
                performance_results[user_count] = {'metrics': metrics, 'avg_execution_time_ms': avg_execution_time, 'max_execution_time_ms': max_execution_time, 'min_execution_time_ms': min_execution_time, 'successful_users': len([r for r in user_results.values() if r.get('success')]), 'failed_users': len([r for r in user_results.values() if not r.get('success')]), 'user_results': user_results}
                failed_users = [uid for uid, result in user_results.items() if not result.get('success')]
                self.assertEqual(len(failed_users), 0, f'Failed users with {user_count} concurrent: {failed_users}')
                self.assertLess(avg_execution_time, self.performance_targets['max_execution_time_ms'] * 1.5, f'Average execution time {avg_execution_time:.1f}ms too high for {user_count} users')
                self.logger.info(f'Concurrent {user_count} users test PASSED: Avg: {avg_execution_time:.1f}ms, Max: {max_execution_time:.1f}ms')
            except Exception as e:
        ...
        self.assertGreaterEqual(max_supported_users, self.performance_targets['min_concurrent_users'], f"Only supports {max_supported_users} users, target is {self.performance_targets['min_concurrent_users']}")
    ...
    async def test_memory_usage_efficiency(self):
        ...
        for cycle in range(5):
            ...
            for user_data in self.test_users[:3]:
                try:
                    if 'UserExecutionContext' in globals():
                        context = UserExecutionContext(user_id=user_data['user_id'], session_id=f"{user_data['session_id']}_cycle_{cycle}", request_id=f'memory_test_cycle_{cycle}')
                        engine = UserExecutionEngine(user_context=context)
                    else:
                        engine = UserExecutionEngine()
                    if not hasattr(engine, 'websocket_emitter'):
            ...
            memory_measurements.append({'cycle': cycle, 'initial_memory_mb': initial_memory, 'final_memory_mb': final_memory, 'memory_delta_mb': memory_delta})
            self.logger.info(f'Memory cycle {cycle}: {initial_memory:.1f}MB  ->  {final_memory:.1f}MB ([U+0394]{memory_delta:+.1f}MB)')
        ...
        self.assertLess(total_memory_growth, 100, f'Total memory growth {total_memory_growth:.1f}MB suggests memory leaks')
        self.assertLess(avg_memory_per_cycle, 20, f'Average memory growth per cycle {avg_memory_per_cycle:.1f}MB too high')
        final_measurement = memory_measurements[-1]
        memory_per_user = final_measurement['memory_delta_mb'] / 3
        self.assertLess(memory_per_user, self.performance_targets['max_memory_per_user_mb'], f"Memory per user {memory_per_user:.1f}MB exceeds target {self.performance_targets['max_memory_per_user_mb']}MB")
        self.logger.info(f'Memory efficiency test PASSED. Total growth: {total_memory_growth:.1f}MB, Avg/cycle: {avg_memory_per_cycle:.1f}MB')
    ...
    async def test_resource_cleanup_performance(self):
        ...
        for i in range(5):
            ...
            try:
                if 'UserExecutionContext' in globals():
                    context = UserExecutionContext(user_id=user_data['user_id'], session_id=user_data['session_id'], request_id=f'cleanup_test_{i}')
                    engine = UserExecutionEngine(user_context=context)
                else:
                    engine = UserExecutionEngine()
                if not hasattr(engine, 'websocket_emitter'):
                ...
                cleanup_times.append({'iteration': i, 'cleanup_time_ms': cleanup_time, 'cleanup_method_available': cleanup_successful})
                self.assertLess(cleanup_time, self.performance_targets['max_cleanup_time_ms'], f"Cleanup time {cleanup_time:.1f}ms exceeds target {self.performance_targets['max_cleanup_time_ms']}ms")
            except Exception as e:
                self.logger.error(f'Cleanup test iteration {i} failed: {e}')
                cleanup_times.append({'iteration': i, 'cleanup_time_ms': 0, 'cleanup_method_available': False, 'error': str(e)})
        valid_cleanup_times = [ct['cleanup_time_ms'] for ct in cleanup_times if ct.get('cleanup_method_available', False)]
        if valid_cleanup_times:
            avg_cleanup_time = statistics.mean(valid_cleanup_times)
            max_cleanup_time = max(valid_cleanup_times)
            self.assertLess(avg_cleanup_time, self.performance_targets['max_cleanup_time_ms'], f'Average cleanup time {avg_cleanup_time:.1f}ms exceeds target')
            self.logger.info(f'Resource cleanup test PASSED. Avg: {avg_cleanup_time:.1f}ms, Max: {max_cleanup_time:.1f}ms')
        else:
    ...
    async def _simulate_realistic_agent_execution(self, engine, user_data):
        ...
        try:
            if hasattr(engine, 'websocket_emitter') and engine.websocket_emitter:
                engine.websocket_emitter.emit('agent_started', {'agent_type': 'performance_test_agent', 'user_id': user_data['user_id'], 'timestamp': time.time()})
            await asyncio.sleep(0.05)
            thinking_steps = ['Analyzing user request for performance test...', 'Processing optimization parameters...', 'Generating response data...']
            for thought in thinking_steps:
                if hasattr(engine, 'websocket_emitter') and engine.websocket_emitter:
                    engine.websocket_emitter.emit('agent_thinking', {'thought': thought, 'user_id': user_data['user_id'], 'timestamp': time.time()})
                await asyncio.sleep(0.02)
            if hasattr(engine, 'websocket_emitter') and engine.websocket_emitter:
                engine.websocket_emitter.emit('tool_executing', {'tool_name': 'performance_analyzer', 'user_id': user_data['user_id'], 'timestamp': time.time()})
                await asyncio.sleep(0.03)
                engine.websocket_emitter.emit('tool_completed', {'tool_name': 'performance_analyzer', 'result': 'Performance analysis completed', 'user_id': user_data['user_id'], 'timestamp': time.time()})
            await asyncio.sleep(0.02)
            if hasattr(engine, 'websocket_emitter') and engine.websocket_emitter:
                engine.websocket_emitter.emit('agent_completed', {'result': 'Performance test execution completed', 'user_id': user_data['user_id'], 'timestamp': time.time()})
        except Exception as e:
    ...
    def _analyze_scaling_performance(self, performance_results: Dict) -> Dict[str, Any]:
        ...
        return {'user_counts': user_counts, 'execution_times_ms': execution_times, 'memory_usage_mb': memory_usage, 'time_scaling_factor': time_scaling_factor, 'memory_scaling_factor': memory_scaling_factor, 'user_scaling_factor': user_scaling_factor, 'time_efficiency': user_scaling_factor / time_scaling_factor if time_scaling_factor > 0 else float('inf'), 'memory_efficiency': user_scaling_factor / memory_scaling_factor if memory_scaling_factor > 0 else float('inf')}
