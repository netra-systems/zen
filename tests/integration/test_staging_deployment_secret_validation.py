...
class TestStagingDeploymentSecretValidation:
                        ...
                        "spec": {
                            "template": {
                                "spec": {
                                    "containers": [{
                                        "env": [
                                            {"name": "SECRET_KEY", "valueFrom": {"secretKeyRef": {"name": "secret-key-staging", "key": "latest"}}},
                                            {"name": "JWT_SECRET_KEY", "valueFrom": {"secretKeyRef": {"name": "jwt-secret-key-staging", "key": "latest"}}},
                                        ]
                                    }]
                                }
                            }
    ...
    def test_complete_secret_flow_auth_service(self):
        """Test complete secret configuration flow for auth service.
        
        This integration test validates:
        1. Secret definition in SecretConfig
        2. GSM mapping resolution  
    ...
    pass
        ...
        assert len(missing_mappings) == 0, (
            f"Auth service has secrets without GSM mappings: {missing_mappings}"
        )
        ...
        for critical_secret in critical_secrets:
            expected_mapping = f"{critical_secret}={SecretConfig.get_gsm_mapping(critical_secret)}:latest"
            assert expected_mapping in deployment_string, (
                f"Critical secret {critical_secret} missing from deployment string"
            )
    ...
    def test_complete_secret_flow_backend_service(self):
        ...
        assert len(missing_mappings) == 0, (
            f"Backend service has secrets without GSM mappings: {missing_mappings}"
        )
    ...
    pass
        ...
        for secret in common_secrets:
            ...
            assert auth_mapping == backend_mapping, (
                f"Inconsistent GSM mapping for {secret}: "
        ...
        for service_name in ["auth", "backend"]:
            ...
            for mapping in mappings:
                assert ":latest" in mapping, (
                    f"Staging mapping should use :latest version: {mapping}"
                )
            ...
            critical_secrets = SecretConfig.CRITICAL_SECRETS.get(service_name, [])
            for critical_secret in critical_secrets:
                secret_prefix = f"{critical_secret}="
                assert secret_prefix in staging_secrets, (
                    f"Staging deployment missing critical secret: {critical_secret}"
                )
            ...
            for category in expected_categories:
                ...
                for secret in category_secrets:
                    ...
                    assert gsm_mapping is not None, (
                        f"{service_name} secret {secret} in {category} lacks GSM mapping"
                    )
            ...
            for service_name in ["auth", "backend"]:
                ...
                if env == "staging":
                    ...
                    assert ":latest" in secrets_string, (
                        f"{env} should use :latest versions"
                    )
                ...
                assert len(secrets_string) > 0, (
                    f"{service_name} in {env} should have non-empty configuration"
                )
    ...
    pass
        ...
        for service_name in ["auth", "backend"]:
            ...
            assert "SECRET_KEY" in all_secrets, (
                f"Step 1 failed: SECRET_KEY not defined for {service_name}"
            )
        ...
        for service_name in ["auth", "backend"]:
            ...
            assert "SECRET_KEY" in critical_secrets, (
                f"Step 2 failed: SECRET_KEY not critical for {service_name}"
            )
        ...
        assert gsm_mapping == "secret-key-staging", (
            f"Step 3 failed: SECRET_KEY incorrect GSM mapping: {gsm_mapping}"
        )
        ...
        for service_name in ["auth", "backend"]:
            ...
            assert expected_mapping in deployment_string, (
                f"Step 4 failed: SECRET_KEY missing from {service_name} deployment string"
            )
        ...
        for oauth_secret in auth_oauth_secrets + backend_oauth_secrets:
            ...
            if categories:  # If secret exists in auth service
                assert "oauth" in categories, (
                    f"OAuth secret {oauth_secret} should be in oauth category"
                )
                assert "authentication" not in categories or oauth_secret.startswith("OAUTH"), (
                    f"OAuth secret {oauth_secret} should not be in authentication unless prefixed with OAUTH"
                )
        ...
        for service_name in ["auth", "backend"]:
            ...
            for redis_secret in redis_secrets:
                assert redis_secret in all_secrets, (
                    f"{redis_secret} missing from {service_name} service"
                )
                
                # Check GSM mapping exists
                gsm_mapping = SecretConfig.get_gsm_mapping(redis_secret)
                assert gsm_mapping is not None, (
                    f"{redis_secret} lacks GSM mapping"
                )
                
                # Check appears in deployment string
                deployment_string = SecretConfig.generate_secrets_string(service_name, "staging")
                expected_mapping = f"{redis_secret}={gsm_mapping}:latest"
                assert expected_mapping in deployment_string, (
                    f"{redis_secret} missing from {service_name} deployment string"
                )