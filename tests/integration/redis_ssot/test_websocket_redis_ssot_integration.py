...
class WebSocketRedisSSotIntegrationTest(SSotAsyncTestCase):
    ...
    def setUp(self):
        ...
        super().setUp()
        self.context = SsotTestContext(test_id=f'websocket_redis_ssot_{int(time.time())}', test_name='WebSocket-Redis SSOT Integration Test', test_category=CategoryType.MISSION_CRITICAL, metadata={'github_issue': '#190', 'business_impact': 'MISSION_CRITICAL', 'golden_path_protection': True, 'deployment_blocker': True, 'arr_at_risk': '$500K+', 'prevents_errors': ['1011_websocket_handshake_failure']})
        self.metrics.start_timing()
        self.critical_websocket_events = ['agent_started', 'agent_thinking', 'tool_executing', 'tool_completed', 'agent_completed']
        self.required_redis_operations = ['agent_state_persistence', 'websocket_session_tracking', 'user_context_storage', 'execution_state_caching', 'event_delivery_confirmation']
    ...
    async def test_redis_ssot_supports_websocket_operations(self):
        ...
        assert connection_stable, f' ALERT:  UNSTABLE CONNECTION: SSOT Redis manager connection unstable. This will cause 1011 WebSocket errors. Connection must be reliable.'
        missing_operations = set(self.required_redis_operations) - set(websocket_operations.keys())
        assert len(missing_operations) == 0, f' ALERT:  MISSING WEBSOCKET SUPPORT: SSOT Redis missing operations: {missing_operations}. Required for WebSocket/Agent integration.'
    ...
    async def test_agent_state_persistence_through_ssot(self):
        ...
        agent_state = {'user_id': f'test_user_{uuid.uuid4().hex[:8]}', 'session_id': f'session_{uuid.uuid4().hex[:8]}', 'agent_type': 'supervisor', 'execution_state': 'processing', 'current_step': 3, 'steps_completed': ['authentication', 'data_retrieval', 'analysis'], 'context': {'query': 'Optimize my AI infrastructure', 'data_sources': ['metrics', 'logs'], 'preferences': {'detail_level': 'high'}}, 'websocket_connection_id': f'ws_{uuid.uuid4().hex[:8]}', 'created_at': datetime.now(timezone.utc).isoformat(), 'updated_at': datetime.now(timezone.utc).isoformat()}
        ...
        assert len(failed_ops) == 0, f' ALERT:  AGENT STATE PERSISTENCE FAILED: Operations failed: {failed_ops}. This will cause agent execution failures and WebSocket errors.'
        state_integrity = persistence_results.get('state_integrity', {})
        assert state_integrity.get('data_consistent', False), f' ALERT:  STATE CORRUPTION: Agent state corrupted during persistence. This will cause unpredictable agent behavior.'
    ...
    async def test_websocket_event_delivery_with_ssot_redis(self):
        ...
        websocket_context = {'connection_id': f'ws_{uuid.uuid4().hex[:8]}', 'user_id': f'user_{uuid.uuid4().hex[:8]}', 'session_id': f'session_{uuid.uuid4().hex[:8]}', 'connected_at': datetime.now(timezone.utc).isoformat()}
        event_delivery_results = {}
        for event_type in self.critical_websocket_events:
            try:
                event_result = await self._test_websocket_event_delivery(event_type, websocket_context)
                event_delivery_results[event_type] = event_result
            except Exception as e:
                event_delivery_results[event_type] = {'success': False, 'error': str(e), 'delivery_time': None}
        ...
        assert len(failed_events) == 0, f' ALERT:  WEBSOCKET EVENT DELIVERY FAILED: Events failed: {failed_events}. This breaks real-time chat functionality for users.'
        slow_events = [event for event, result in event_delivery_results.items() if result.get('delivery_time', 0) > 1.0]
        assert len(slow_events) <= 1, f' ALERT:  SLOW EVENT DELIVERY: Events too slow: {slow_events}. Real-time UX requires <1s delivery for most events.'
    ...
    async def test_connection_stability_during_handshake(self):
        ...
        handshake_scenarios = ['normal_handshake', 'concurrent_handshakes', 'rapid_reconnection', 'high_load_handshake']
        handshake_results = {}
        for scenario in handshake_scenarios:
            try:
                ...
                handshake_results[scenario] = {'success': False, 'error': str(e), 'redis_stable': False, 'handshake_time': None}
        ...
        failed_critical = [scenario for scenario in critical_scenarios if not handshake_results[scenario].get('success', False)]
        assert len(failed_critical) == 0, f' ALERT:  HANDSHAKE FAILURES: Critical scenarios failed: {failed_critical}. This will cause 1011 WebSocket errors blocking user chat.'
        unstable_redis = [scenario for scenario, result in handshake_results.items() if not result.get('redis_stable', False)]
        assert len(unstable_redis) <= 1, f' ALERT:  REDIS INSTABILITY: Redis unstable during: {unstable_redis}. Instability causes connection race conditions and 1011 errors.'
    ...
    async def test_user_isolation_with_ssot_redis(self):
        ...
        user_contexts = [{'user_id': f'user_{i}_{uuid.uuid4().hex[:6]}', 'session_id': f'session_{i}_{uuid.uuid4().hex[:6]}', 'agent_state': f'processing_query_{i}', 'websocket_id': f'ws_{i}_{uuid.uuid4().hex[:6]}'} for i in range(3)]
        isolation_tasks = [self._test_user_isolation_operations(ctx) for ctx in user_contexts]
        ...
        for i, result in enumerate(isolation_results):
            if isinstance(result, Exception):
                failed_isolations.append({'user_context': user_contexts[i], 'error': str(result)})
            elif result.get('isolation_maintained', False):
                successful_isolations.append(result)
            else:
                failed_isolations.append({'user_context': user_contexts[i], 'result': result})
        ...
        assert len(failed_isolations) == 0, f' ALERT:  USER ISOLATION FAILURE: {len(failed_isolations)} users experienced isolation failures. This causes data leakage between users. Failures: {failed_isolations}'
        contamination_detected = self._check_cross_user_contamination(successful_isolations)
        assert not contamination_detected.get('has_contamination', False), f' ALERT:  DATA CONTAMINATION: Cross-user data leakage detected: {contamination_detected}. SSOT Redis manager must maintain user isolation.'
    ...
    async def test_memory_leak_prevention_with_ssot(self):
        ...
        leak_test_operations = ['repeated_connections', 'connection_pool_overflow', 'abandoned_connections', 'concurrent_session_creation']
        memory_metrics = {}
        for operation in leak_test_operations:
            try:
                ...
                memory_metrics[operation] = {'success': False, 'error': str(e), 'memory_stable': False}
                logger.error(f'Memory leak test {operation} failed: {e}')
        self.metrics.record_custom('memory_leak_tests', memory_metrics)
        unstable_operations = [op for op, metrics in memory_metrics.items() if not metrics.get('memory_stable', False)]
        assert len(unstable_operations) <= 1, f' ALERT:  MEMORY LEAKS DETECTED: Operations with unstable memory: {unstable_operations}. SSOT Redis must prevent memory leaks from connection pooling.'
    ...
    async def _simulate_session_tracking_op(self, redis_manager) -> Dict[str, Any]:
        ...
        try:
            ...
            if hasattr(redis_manager, 'expire'):
                await redis_manager.expire(session_key, 3600)
            if hasattr(redis_manager, 'delete'):
    ...
    async def _test_agent_state_operations(self, agent_state: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if 'retrieve_state' in results and results['retrieve_state']['success']:
                ...
                integrity_ok = retrieved.get('user_id') == agent_state['user_id'] and retrieved.get('session_id') == agent_state['session_id'] and (retrieved.get('agent_type') == agent_state['agent_type'])
    ...
    async def _test_websocket_event_delivery(self, event_type: str, websocket_context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            event_data = {'event_type': event_type, 'connection_id': websocket_context['connection_id'], 'user_id': websocket_context['user_id'], 'timestamp': datetime.now(timezone.utc).isoformat(), 'data': self._generate_event_data(event_type)}
            from netra_backend.app.redis_manager import RedisManager
            redis_manager = RedisManager()
            ...
            if hasattr(redis_manager, 'delete'):
                await redis_manager.delete(event_key)
            return {'success': True, 'delivery_time': delivery_time, 'event_type': event_type}
        except Exception as e:
            return {'success': False, 'error': str(e), 'delivery_time': None, 'event_type': event_type}

    def _generate_event_data(self, event_type: str) -> Dict[str, Any]:
        """Generate realistic event data for testing."""
        event_data_templates = {'agent_started': {'agent_type': 'supervisor', 'query': 'Optimize my infrastructure', 'estimated_duration': '30-60 seconds'}, 'agent_thinking': {'current_step': 'Analyzing requirements', 'progress': 0.25, 'reasoning': 'Processing user query to determine optimization approach'}, 'tool_executing': {'tool_name': 'data_analyzer', 'tool_description': 'Analyzing system metrics', 'input_params': {'source': 'metrics'}}, 'tool_completed': {'tool_name': 'data_analyzer', 'result_summary': 'Analysis complete', 'execution_time': 2.3}, 'agent_completed': {'result': 'Infrastructure optimization recommendations generated', 'recommendations_count': 5, 'total_time': 45.2}}
    ...
    async def _simulate_handshake_scenario(self, scenario: str) -> Dict[str, Any]:
        ...
        try:
            ...
            return {'success': False, 'error': str(e), 'redis_stable': False, 'handshake_time': time.time() - start_time}
    ...
    async def _simulate_normal_handshake(self, redis_manager) -> Dict[str, Any]:
        ...
        try:
            ...
            handshake_data = {'user_id': f'user_{uuid.uuid4().hex[:6]}', 'handshake_time': time.time(), 'status': 'connecting'}
            if hasattr(redis_manager, 'set'):
            ...
            if hasattr(redis_manager, 'delete'):
                await redis_manager.delete(session_key)
            return {'success': True, 'redis_stable': True, 'handshake_time': time.time() - start_time}
        except Exception as e:
            return {'success': False, 'error': str(e), 'redis_stable': False, 'handshake_time': time.time() - start_time}
    ...
    async def _simulate_concurrent_handshakes(self, redis_manager) -> Dict[str, Any]:
        ...
        start_time = time.time()
        try:
            handshake_tasks = [self._simulate_single_concurrent_handshake(redis_manager, i) for i in range(5)]
            results = await asyncio.gather(*handshake_tasks, return_exceptions=True)
            successful = sum((1 for r in results if isinstance(r, dict) and r.get('success', False)))
            return {'success': successful >= 4, 'redis_stable': successful >= 4, 'handshake_time': time.time() - start_time, 'concurrent_success_rate': successful / len(handshake_tasks)}
        except Exception as e:
            return {'success': False, 'error': str(e), 'redis_stable': False, 'handshake_time': time.time() - start_time}
    ...
    async def _test_user_isolation_operations(self, user_context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            for data_type in ['session', 'state', 'context']:
                ...
                data = {'user_id': user_id, 'data_type': data_type, 'timestamp': time.time(), 'test_data': f'data_for_{user_id}_{data_type}'}
                if hasattr(redis_manager, 'set'):
            ...
            return {'user_id': user_id, 'isolation_maintained': isolation_maintained, 'keys_tested': len(user_keys), 'data_retrieved': len(retrieved_data)}
        except Exception as e:
            return {'user_id': user_context['user_id'], 'isolation_maintained': False, 'error': str(e)}
    ...
    def _check_cross_user_contamination(self, isolation_results: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        contamination_info = {'has_contamination': False, 'contamination_details': []}
        failed_isolations = [result for result in isolation_results if not result.get('isolation_maintained', False)]
    ...
    async def _test_memory_leak_operation(self, operation: str) -> Dict[str, Any]:
        ...
        try:
            ...
            return {'success': False, 'error': str(e), 'memory_stable': False}
    ...
    async def _test_repeated_connections(self, redis_manager) -> Dict[str, Any]:
        ...
        try:
            ...
            return {'success': True, 'memory_stable': True, 'operation': 'repeated_connections'}
        except Exception as e:
            return {'success': False, 'error': str(e), 'memory_stable': False, 'operation': 'repeated_connections'}
    ...
    async def _test_connection_pool_overflow(self, redis_manager) -> Dict[str, Any]:
        ...
        try:
            ...
            return {'success': True, 'memory_stable': True, 'operation': 'connection_pool_overflow'}
        except Exception as e:
            return {'success': False, 'error': str(e), 'memory_stable': False, 'operation': 'connection_pool_overflow'}
    ...
    async def _test_abandoned_connections(self, redis_manager) -> Dict[str, Any]:
        ...
        try:
            ...
            for key in abandoned_keys:
                if hasattr(redis_manager, 'delete'):
                    await redis_manager.delete(key)
            return {'success': True, 'memory_stable': True, 'operation': 'abandoned_connections'}
        except Exception as e:
            return {'success': False, 'error': str(e), 'memory_stable': False, 'operation': 'abandoned_connections'}
    ...
    async def _test_concurrent_session_creation(self, redis_manager) -> Dict[str, Any]:
        ...
        try:
            ...
            successful_sessions = sum((1 for result in session_results if isinstance(result, dict) and result.get('success', False)))
            return {'success': successful_sessions >= 8, 'memory_stable': True, 'operation': 'concurrent_session_creation', 'success_rate': successful_sessions / len(session_tasks)}
        except Exception as e:
            return {'success': False, 'error': str(e), 'memory_stable': False, 'operation': 'concurrent_session_creation'}
    ...
    async def _create_test_session(self, redis_manager, session_id: int) -> Dict[str, Any]:
        ...
        try:
            ...
            session_data = {'session_id': session_id, 'user_id': f'test_user_{session_id}', 'created_at': time.time(), 'status': 'active'}
            if hasattr(redis_manager, 'set'):
                await redis_manager.set(session_key, json.dumps(session_data))
            await asyncio.sleep(0.01)
