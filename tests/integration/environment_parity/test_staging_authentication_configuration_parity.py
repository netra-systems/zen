...
class TestStagingAuthenticationConfigurationParity:
    ...
    def test_service_id_present_in_staging_docker_compose(self, isolated_env):
        ...
        assert 'SERVICE_ID' in compose_content, 'SERVICE_ID is missing from staging docker-compose.yml. This configuration gap causes authentication failures in staging environment.'
        ...
        for line in lines:
            if 'backend:' in line:
                ...
                if line.strip() and (not line.startswith(' ')) and (not line.startswith('-')):
                    break
        assert service_id_found, 'SERVICE_ID not found in backend service environment section of staging docker-compose.yml. This is the root cause of system user authentication failures.'
    ...
    def test_hardcoded_system_user_in_dependencies(self):
        ...
        assert not hardcoded_system_user_found, f"Hardcoded 'system' user_id found in dependencies files: {problematic_files}. This causes 403 authentication errors because 'system' is not a valid authenticated user_id. System operations should use proper service authentication or dynamic user context."
    ...
    async def test_system_user_authentication_with_real_services(self, isolated_env):
        ...
        try:
            ...
            try:
                ...
                pytest.fail(f"System user authentication failed: {auth_error}. This demonstrates the authentication configuration gap that's blocking the Golden Path.")
            if session_created:
            ...
            pytest.fail(f"System user authentication failed: {e}. This demonstrates the authentication configuration gap that's blocking the Golden Path.")
    ...
    def test_service_to_service_authentication_configuration(self, isolated_env):
        ...
        required_service_auth_vars = ['SERVICE_ID', 'SERVICE_SECRET', 'JWT_SECRET_KEY']
        missing_vars = []
        ...
        assert not missing_vars, f'Missing service authentication environment variables: {missing_vars}. These are required for proper service-to-service authentication. Without these, internal operations fail with 403 errors.'
    ...
    def test_staging_authentication_performance_baseline(self, isolated_env):
        ...
        baseline_data = {'average_auth_time': avg_auth_time, 'max_auth_time': max_auth_time, 'min_auth_time': min_auth_time, 'total_attempts': len(auth_times), 'timestamp': time.time()}
    ...
    def test_docker_compose_environment_variable_consistency(self):
        ...
        project_root = Path.cwd()
        compose_files = ['docker-compose.yml', 'docker-compose.staging.yml', 'docker-compose.alpine.yml', 'docker-compose.alpine-test.yml']
        ...
        assert not missing_vars_by_file, f'Critical authentication variables missing from compose files: {missing_vars_by_file}. Environment variable consistency is required for proper authentication across environments.'
