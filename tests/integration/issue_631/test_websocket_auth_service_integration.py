...
class WebSocketAuthServiceIntegrationTests(SSotAsyncTestCase):
    ...
    def test_backend_auth_service_communication(self):
        ...
        try:
            ...
            self.assertTrue(response.status_code in [200, 404], f'Auth service communication failed - root cause of Issue #631. Status: {response.status_code}, URL: {health_url}')
        except requests.exceptions.ConnectionError as e:
            self.fail(f'Backend cannot communicate with auth service at {self.test_auth_url}. This is the root cause of Issue #631 HTTP 403 errors. Error: {e}')
        except Exception as e:
    ...
    async def test_jwt_token_validation_integration(self):
        ...
        try:
            ...
            self.assertIsInstance(is_valid, bool, 'JWT validation should return boolean result')
            user_data = await websocket_auth.extract_user_from_token(test_token)
            self.assertIsNotNone(user_data, 'Should be able to extract user data from valid JWT token')
    ...
    async def test_websocket_middleware_auth_flow(self):
        ...
        connection_headers = {'Authorization': f'Bearer {test_token}', 'Origin': 'http://localhost:3000'}
        try:
            websocket_auth = UnifiedWebSocketAuth()
            mock_websocket = MagicMock()
            mock_websocket.request_headers = connection_headers
            auth_result = await websocket_auth.authenticate_websocket_connection(mock_websocket)
            self.assertIsNotNone(auth_result, 'WebSocket middleware should return authentication result')
            if isinstance(auth_result, dict):
                self.assertIn('authenticated', auth_result, "Authentication result should include 'authenticated' flag")
                self.assertIn('user_id', auth_result, "Authentication result should include 'user_id'")
        ...
        try:
            ...
            with patch.object(websocket_auth, 'validate_jwt_token', return_value=False):
                with self.assertLogs(level='WARNING') as log_context:
                    result = websocket_auth.handle_authentication_failure(invalid_token, 'Invalid JWT token')
                    self.assertTrue(any(('403' in record.message for record in log_context.records)), 'Should log 403 authentication failure')
                    self.assertTrue(any(('authentication' in record.message.lower() for record in log_context.records)), 'Should log authentication failure details')
        except (AttributeError, ImportError) as e:
    ...
    async def test_auth_service_unavailable_handling(self):
        ...
        try:
            ...
            with self.assertLogs(level='ERROR') as log_context:
                ...
                self.assertTrue(any(('auth service' in record.message.lower() for record in log_context.records)), 'Should log auth service unavailability')
                self.assertIsNotNone(result, 'Should return fallback result when auth service unavailable')
        except (AttributeError, ImportError) as e:
    ...
    def test_auth_client_connection_pool_configuration(self):
        ...
        try:
            auth_client = AuthClientCore()
            self.assertTrue(hasattr(auth_client, 'session') or hasattr(auth_client, '_session') or hasattr(auth_client, 'connection_pool'), 'Auth client should have connection pool configured')
            if hasattr(auth_client, 'session'):
                self.assertIsNotNone(auth_client.session, 'Auth client session should be configured for connection pooling')
        except Exception as e:
    ...
    async def test_websocket_auth_timeout_handling(self):
        ...
        try:
            ...
            with patch('requests.post') as mock_post:
                ...
                result = await websocket_auth.validate_jwt_token_with_timeout('test-token', timeout=2.0)
                end_time = time.time()
                self.assertLess(end_time - start_time, 5.0, 'Authentication should timeout within reasonable time')
                self.assertIsNotNone(result, 'Should return timeout result when auth service times out')
        except (AttributeError, ImportError) as e:
