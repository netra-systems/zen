...
class TestWebSocketEventsSSotDelivery(SSotAsyncTestCase, unittest.TestCase):
    ...
    async def setup_method(self, method):
        ...
        self.test_user_context = {
            'user_id': 'websocket_events_test_user',
            'thread_id': 'websocket_events_test_thread',
            'session_id': 'websocket_events_test_session'
        }
    ...
    async def test_all_five_websocket_events_sent_via_ssot_manager(self):
        ...
        # Mock a WebSocket connection in the manager
        mock_connection = self.create_mock_websocket_connection(
            self.test_user_context['user_id']
        )
        
        # Add connection to manager (if manager supports it)
        ...
        # Simulate sending all 5 required events
        test_events = [
            {
                'type': 'agent_started',
                'data': {'agent_id': 'test_agent', 'message': 'Agent started processing'}
            },
            {
                'type': 'agent_thinking',
                'data': {'progress': 0.25, 'thought': 'Analyzing request...'}
            },
            {
                'type': 'tool_executing', 
                'data': {'tool': 'cost_analyzer', 'status': 'running'}
            },
            {
                'type': 'tool_completed',
                'data': {'tool': 'cost_analyzer', 'result': {'savings': 1000}}
            },
            {
                'type': 'agent_completed',
                'data': {'result': 'Analysis complete', 'status': 'success'}
            }
        ]
        
        # Send events through SSOT manager
        for event in test_events:
            try:
                if hasattr(manager, 'send_event'):
                    ...
                    # Fallback: direct connection send if manager doesn't have event methods
        ...
        for required_event in self.REQUIRED_WEBSOCKET_EVENTS:
            # BEFORE REMEDIATION: This assertion might FAIL (events not sent)
            # AFTER REMEDIATION: This assertion should PASS (all events sent)
            self.assertIn(
                required_event,
                event_types_sent,
                f"Required WebSocket event '{required_event}' should be sent via SSOT manager"
            )
    ...
    async def test_websocket_event_delivery_to_correct_users_only(self):
        ...
        # Send targeted event to specific user
        ...
        test_event = {
            'type': 'agent_started',
            'data': {
                'message': f'Agent started for {target_user_id}',
                'user_specific_data': f'secret_data_for_{target_user_id}'
            }
        }
        
        # Send event via target user's SSOT manager
        try:
            if hasattr(target_manager, 'send_event'):
                await target_manager.send_event(
                ...
                # Direct connection send as fallback
        ...
        # Verify event was delivered to correct user only
        events_by_user = {}
        ...
        # BEFORE REMEDIATION: Event might be sent to wrong users
        # AFTER REMEDIATION: Event should only go to target user
        self.assertIn(
            target_user_id,
            events_by_user,
            f"Target user {target_user_id} should receive the event"
        )
        
        # Verify other users didn't receive the event
        other_user_id = 'event_user_2'
        if other_user_id in events_by_user:
            # If other user received events, they shouldn't contain target user's data
            other_user_events = events_by_user[other_user_id]
            for event in other_user_events:
                event_data_str = str(event.get('event_data', {}))
                self.assertNotIn(
                    f'secret_data_for_{target_user_id}',
                    event_data_str,
                    f"Other user {other_user_id} should not receive target user's data"
                )
    ...
    async def test_websocket_event_ordering_consistency_with_ssot(self):
        ...
        # Send events in expected order for agent workflow
        workflow_events = [
            ('agent_started', {'step': 1}),
            ('agent_thinking', {'step': 2}), 
            ('tool_executing', {'step': 3}),
            ('tool_completed', {'step': 4}),
            ('agent_completed', {'step': 5})
        ]
        
        # Send events with small delays to ensure ordering
        ...
        received_events = [
            ...
            if event['user_id'] == self.test_user_context['user_id']
        ]
        
        self.assertEqual(
            len(received_events),
            len(workflow_events),
            "All workflow events should be received"
        )
        ...
        # BEFORE REMEDIATION: This assertion might FAIL (wrong order)
        # AFTER REMEDIATION: This assertion should PASS (correct order)
        self.assertEqual(
            actual_order,
            expected_order,
            f"Events should be received in correct order. Expected: {expected_order}, Got: {actual_order}"
        )
    ...
    async def test_websocket_event_error_handling_with_ssot_manager(self):
        ...
        # Create a failing mock connection
        ...
        # Add failing connection to manager
        if hasattr(manager, 'add_connection'):
        ...
        # Try to send event through failing connection
        test_event = {
            'type': 'agent_started',
            'data': {'message': 'This should fail gracefully'}
        }
        ...
        try:
            ...
            # BEFORE REMEDIATION: Error might not be handled gracefully
            # AFTER REMEDIATION: Error should be handled with proper logging/recovery
            self.logger.info(f"Error handled gracefully: {error_message}")
            
        # Create a working connection to verify manager recovery
        working_connection = self.create_mock_websocket_connection(
            self.test_user_context['user_id']
        )
        
        # Replace failing connection with working one
        ...
        # Try sending event again with working connection
        ...
        try:
            if hasattr(manager, 'send_event'):
                ...
                await working_connection.send_json({
                    'type': 'agent_started',
                    'data': {'message': 'Recovery test'}
                })
        ...
        # BEFORE REMEDIATION: Recovery might fail
        # AFTER REMEDIATION: Manager should recover from connection failures
        self.assertTrue(
            recovery_successful,
            "SSOT WebSocket manager should recover from connection failures"
        )
    ...
    async def test_agent_workflow_integration_with_ssot_websockets(self):
        ...
        # Simulate complete agent workflow with all required events
        agent_workflow = [
            {
                'type': 'agent_started',
                'data': {
                    'agent_id': 'cost_optimizer',
                    'user_query': 'Analyze my AWS costs',
            ...
            },
            {
                ...
                'data': {
                    ...
                    'thought': 'Breaking down cost analysis requirements...'
                }
            },
            {
                'type': 'tool_executing',
            ...
            },
            {
                ...
                'data': {
                    ...
                    'result': {
                        ...
                        'recommendations': ['resize_instances', 'use_spot_instances']
                    },
                    'status': 'success'
                }
            },
            {
                ...
                'data': {
                    'result': {
                        'summary': 'Found $1,200 in monthly savings opportunities',
                        'confidence': 0.95,
                        'next_actions': ['implement_recommendations']
                    },
                    'workflow_id': 'workflow_123',
                    'status': 'success'
                }
        ...
        # Verify complete workflow was delivered
        received_events = [
            event for event in self.event_history 
            if event['user_id'] == self.test_user_context['user_id']
        ]
        
        # All 5 required events should be present
        received_event_types = [event['event_type'] for event in received_events]
        
        for required_event in self.REQUIRED_WEBSOCKET_EVENTS:
            # BEFORE REMEDIATION: This assertion might FAIL (missing events)
            # AFTER REMEDIATION: This assertion should PASS (all events delivered)
            self.assertIn(
                required_event,
                received_event_types,
                f"Agent workflow should deliver all required events including '{required_event}'"
            )
            
        # Verify workflow integrity (specific data is preserved)
        workflow_start_event = next(
            (e for e in received_events if e['event_type'] == 'agent_started'), 
            None
        )
        workflow_end_event = next(
            (e for e in received_events if e['event_type'] == 'agent_completed'),
            None
        )
        ...
        if start_workflow_id and end_workflow_id:
            self.assertEqual(
                start_workflow_id,
                end_workflow_id,
                "Workflow ID should be consistent from start to end"
            )
