...
class TestWebSocketRedisAuthRaceConditions:
    ...
    async def setup_real_services(self):
        ...
        await self.docker_manager.start_services_smart(services=services_needed, wait_healthy=True)
    ...
    async def _verify_service_health(self):
        ...
        env = get_env()
        try:
            redis_client = redis.Redis.from_url('redis://localhost:6381')
    ...
    async def test_websocket_redis_auth_race_conditions(self):
        ...
        for i in range(5):
            context = await create_authenticated_user_context(user_id=f'race-test-user-{i}-{uuid.uuid4().hex[:8]}', environment='test', websocket_enabled=True)
        ...
        async def test_concurrent_operations(manager_index: int):
            ...
            try:
                operations = []
                for op in range(3):
                    operations.append(self._simulate_websocket_operation(manager, context, f'op_{op}'))
    ...
    async def _simulate_websocket_operation(self, manager: WebSocketManager, context: StronglyTypedUserExecutionContext, operation_id: str) -> bool:
        ...
        try:
            ...
            operation_data = {'user_id': str(context.user_id), 'operation_id': operation_id, 'timestamp': time.time()}
...
class TestMultiUserWebSocketIsolation:
    ...
    async def test_multi_user_websocket_isolation_concurrent(self):
        ...
        """
        concurrent_user_count = 12
        user_contexts = []
        for i in range(concurrent_user_count):
            context = await create_authenticated_user_context(user_id=f'isolation-user-{i}-{uuid.uuid4().hex[:6]}', user_email=f'isolation_test_{i}@example.com', environment='test', websocket_enabled=True)
            user_contexts.append(context)
        ...
        async def create_user_websocket_connection(user_index: int):
            ...
            try:
                test_context = await self.websocket_test_base.create_test_context(user_id=str(context.user_id), jwt_token=context.agent_context.get('jwt_token'))
                await test_context.setup_websocket_connection(endpoint='/ws/test', auth_required=True)
                test_message = {'type': 'isolation_test', 'user_id': str(context.user_id), 'user_email': context.agent_context.get('user_email'), 'test_data': f'user_{user_index}_data_{uuid.uuid4().hex[:8]}', 'timestamp': time.time()}
                await test_context.send_message(test_message)
                try:
                    response = await test_context.receive_message(timeout=5.0)
                    return {'success': True, 'user_index': user_index, 'user_id': str(context.user_id), 'test_context': test_context, 'sent_message': test_message, 'received_response': response}
                except asyncio.TimeoutError:
                    return {'success': True, 'user_index': user_index, 'user_id': str(context.user_id), 'test_context': test_context, 'sent_message': test_message, 'received_response': None}
            except Exception as e:
                return {'success': False, 'user_index': user_index, 'user_id': str(context.user_id), 'error': str(e)}
        connection_tasks = []
        for i in range(concurrent_user_count):
        ...
        async def test_message_isolation(connection_data):
            ...
            isolation_message = {'type': 'isolation_verification', 'user_id': user_id, 'unique_token': f'{user_id}_token_{uuid.uuid4().hex[:12]}', 'timestamp': time.time()}
            await test_context.send_message(isolation_message)
            return {'user_id': user_id, 'unique_token': isolation_message['unique_token'], 'message_sent': True}
        for connection in successful_connections:
...
class TestAgentEventDeliveryLifecycle:
    ...
    async def test_agent_event_delivery_lifecycle(self):
        ...
        user_context = await create_authenticated_user_context(user_id=f'agent-event-user-{uuid.uuid4().hex[:8]}', environment='test', websocket_enabled=True)
        ...
        await test_context.setup_websocket_connection(endpoint='/ws/test', auth_required=True)
        agent_request = {'type': 'agent_request', 'agent_name': 'test_agent', 'task': 'Perform a test task that triggers all agent events', 'user_id': str(user_context.user_id), 'thread_id': str(user_context.thread_id), 'request_id': str(user_context.request_id), 'timestamp': datetime.now(timezone.utc).isoformat()}
        await test_context.send_message(agent_request)
        event_timeout = 60.0
        validation_result = await self.websocket_test_base.validate_agent_events(test_context=test_context, required_events=required_events, timeout=event_timeout)
