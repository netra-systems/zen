...
from netra_backend.app.core.configuration.base import UnifiedConfigManager, config_manager, get_unified_config, get_config, reload_unified_config, validate_unified_config, get_environment, is_production, is_development, is_testing
...
class TestUnifiedConfigManagerIntegration(SSotBaseTestCase):
    ...
    def test_development_environment_configuration_loading(self):
        ...
        with self.temp_env_vars(ENVIRONMENT='development'):
            ...
            self.assertLess(load_time, 15.0, 'Development config loading too slow')
    ...
    def test_staging_environment_configuration_validation(self):
        ...
        """
        staging_env_vars = {'ENVIRONMENT': 'staging', 'POSTGRES_HOST': 'staging-postgres.internal', 'POSTGRES_USER': 'staging_user', 'POSTGRES_PASSWORD': 'staging_secure_password_32_characters', 'POSTGRES_DB': 'netra_staging', 'JWT_SECRET_KEY': 'staging_jwt_secret_key_32_characters_minimum_length', 'SECRET_KEY': 'staging_secret_key_32_characters_minimum_required_length', 'SERVICE_SECRET': 'staging_service_secret_32_characters_minimum_required', 'REDIS_HOST': 'staging-redis.internal', 'CLICKHOUSE_HOST': 'staging-clickhouse.internal', 'CLICKHOUSE_PASSWORD': 'staging_clickhouse_password'}
    ...
    def test_production_environment_configuration_security(self):
        ...
        """
        production_env_vars = {'ENVIRONMENT': 'production', 'POSTGRES_HOST': 'prod-postgres.internal', 'POSTGRES_USER': 'prod_user', 'POSTGRES_PASSWORD': 'prod_highly_secure_password_64_characters_absolutely_minimum', 'POSTGRES_DB': 'netra_production', 'JWT_SECRET_KEY': 'prod_jwt_secret_highly_secure_64_characters_absolutely_minimum_req', 'SECRET_KEY': 'prod_secret_key_highly_secure_64_characters_absolutely_minimum_req', 'SERVICE_SECRET': 'prod_service_secret_highly_secure_64_characters_absolutely_min', 'REDIS_HOST': 'prod-redis.internal', 'CLICKHOUSE_HOST': 'prod-clickhouse.internal', 'CLICKHOUSE_PASSWORD': 'prod_clickhouse_highly_secure_password'}
    ...
    def test_database_connection_configuration_validation(self):
        ...
        for env_name, expected_config_class in test_environments:
            with self.temp_env_vars(ENVIRONMENT=env_name, POSTGRES_HOST=f'{env_name}-postgres.internal', POSTGRES_USER=f'{env_name}_user', POSTGRES_PASSWORD=f'{env_name}_secure_password_32_characters', POSTGRES_DB=f'netra_{env_name}', JWT_SECRET_KEY=f'{env_name}_jwt_secret_32_characters_min', SECRET_KEY=f'{env_name}_secret_key_32_characters_min', SERVICE_SECRET=f'{env_name}_service_secret_32_chars_min'):
    ...
    def test_websocket_service_configuration_management(self):
        ...
        """
        with self.temp_env_vars(ENVIRONMENT='development', WEBSOCKET_SSOT_CONSOLIDATION='true'):
            config = self.config_manager.get_config()
    ...
    def test_auth_service_integration_configuration(self):
        ...
        for env_name in environments:
            with self.temp_env_vars(ENVIRONMENT=env_name, AUTH_SERVICE_URL=f'http://{env_name}-auth.internal:8081', JWT_SECRET_KEY=f'{env_name}_jwt_secret_32_characters_minimum', SECRET_KEY=f'{env_name}_secret_key_32_characters_minimum', SERVICE_SECRET=f'{env_name}_service_secret_32_chars_min', SERVICE_ID='netra-backend'):
                self.config_manager.reload_config(force=True)
                config = self.config_manager.get_config()
    ...
    def test_redis_clickhouse_service_configuration(self):
        ...
        """
        with self.temp_env_vars(ENVIRONMENT='staging', REDIS_URL='redis://staging-redis.internal:6379', REDIS_MODE='shared', CLICKHOUSE_HOST='staging-clickhouse.internal', CLICKHOUSE_PORT='8123', CLICKHOUSE_PASSWORD='staging_clickhouse_secure_password', CLICKHOUSE_MODE='shared'):
            config = self.config_manager.get_config()
    ...
    def test_configuration_loading_and_caching_behavior(self):
        ...
        with self.temp_env_vars(ENVIRONMENT='development'):
            ...
            self.assertLess(first_load_time, 15.0, 'Initial load too slow')
            self.assertLess(second_load_time, 5.0, 'Cached load too slow')
    ...
    def test_cache_invalidation_and_refresh_mechanisms(self):
        ...
        with self.temp_env_vars(ENVIRONMENT='staging', JWT_SECRET_KEY='staging_jwt_32_chars_minimum', SECRET_KEY='staging_secret_32_chars_minimum', SERVICE_SECRET='staging_service_32_chars_min'):
            self.config_manager._environment = None
    ...
    def test_configuration_loading_latency_optimization(self):
        ...
        latency_results = {}
        for env_name in environments:
            env_vars = {'ENVIRONMENT': env_name, 'JWT_SECRET_KEY': f'{env_name}_jwt_secret_32_characters_minimum', 'SECRET_KEY': f'{env_name}_secret_key_32_characters_minimum', 'SERVICE_SECRET': f'{env_name}_service_secret_32_chars_min'}
    ...
    def test_isolated_environment_integration_validation(self):
        ...
        with self.temp_env_vars(ENVIRONMENT='development', TEST_CONFIG_VAR='test_value_from_environment', GEMINI_API_KEY='test_gemini_api_key_from_env'):
            config = self.config_manager.get_config()
    ...
    def test_environment_variable_precedence_override_logic(self):
        ...
        """
        base_env_vars = {'ENVIRONMENT': 'staging', 'JWT_SECRET_KEY': 'base_jwt_secret_32_characters_minimum', 'SECRET_KEY': 'base_secret_key_32_characters_minimum', 'SERVICE_SECRET': 'base_service_secret_32_chars_min'}
        with self.temp_env_vars(**base_env_vars):
            ...
            override_env_vars = {**base_env_vars, 'JWT_SECRET_KEY': 'override_jwt_secret_32_characters_minimum', 'SERVICE_SECRET': 'override_service_secret_32_chars_min'}
    ...
    def test_secure_environment_variable_handling(self):
        ...
        """
        secure_env_vars = {'ENVIRONMENT': 'production', 'JWT_SECRET_KEY': 'prod_jwt_highly_secure_64_characters_absolutely_minimum_required', 'SECRET_KEY': 'prod_secret_highly_secure_64_characters_absolutely_minimum_required', 'SERVICE_SECRET': 'prod_service_highly_secure_64_characters_absolutely_minimum_req', 'POSTGRES_PASSWORD': 'prod_postgres_highly_secure_password_64_chars_minimum', 'REDIS_PASSWORD': 'prod_redis_highly_secure_password_64_chars_minimum'}
    ...
    def test_environment_isolation_between_test_runs(self):
        ...
        """
        with self.temp_env_vars(ENVIRONMENT='development', TEST_RUN_ID='first_test_run', CUSTOM_CONFIG_VALUE='first_value'):
            config1 = self.config_manager.get_config()
            self.assertEqual(config1.environment, 'development')
        ...
        with self.temp_env_vars(ENVIRONMENT='staging', TEST_RUN_ID='second_test_run', CUSTOM_CONFIG_VALUE='second_value', JWT_SECRET_KEY='staging_jwt_32_chars_minimum', SECRET_KEY='staging_secret_32_chars_minimum', SERVICE_SECRET='staging_service_32_chars_min'):
            self.config_manager.reload_config(force=True)
            config2 = self.config_manager.get_config()
    ...
    def test_invalid_configuration_detection_handling(self):
        ...
        with self.temp_env_vars(ENVIRONMENT='invalid_environment'):
            config = self.config_manager.get_config()
            self.assertIsInstance(config, DevelopmentConfig)
        with self.temp_env_vars(ENVIRONMENT='production', JWT_SECRET_KEY='short', SECRET_KEY='prod_secret_key_32_characters_minimum_required', SERVICE_SECRET='prod_service_secret_32_characters_minimum_req'):
    ...
    def test_missing_configuration_parameter_validation(self):
        ...
        with self.temp_env_vars(ENVIRONMENT='testing', SECRET_KEY='test_secret_key_32_characters_minimum_required', JWT_SECRET_KEY='test_jwt_secret_32_characters_minimum_required'):
            config = self.config_manager.get_config()
    ...
    def test_configuration_schema_validation(self):
        ...
        """
        with self.temp_env_vars(ENVIRONMENT='development', SECRET_KEY='dev_secret_key_32_characters_minimum_required', JWT_SECRET_KEY='dev_jwt_secret_32_characters_minimum_required'):
            config = self.config_manager.get_config()
    ...
    def test_error_reporting_and_recovery_mechanisms(self):
        ...
        with self.temp_env_vars(ENVIRONMENT='production', SECRET_KEY='short', SERVICE_SECRET='also_short'):
            try:
                config = self.config_manager.get_config()
                validation_result = self.config_manager.validate_config_integrity()
                ...
                error_message = str(e)
                self.assertTrue('32 characters' in error_message or 'length' in error_message.lower(), f'Error message not helpful: {error_message}')
        with self.temp_env_vars(ENVIRONMENT='development'):
    ...
    def test_sensitive_configuration_parameter_protection(self):
        ...
        """
        sensitive_config = {'ENVIRONMENT': 'production', 'JWT_SECRET_KEY': 'prod_jwt_highly_secure_64_characters_absolutely_minimum_required', 'SECRET_KEY': 'prod_secret_highly_secure_64_characters_absolutely_minimum_required', 'SERVICE_SECRET': 'prod_service_highly_secure_64_characters_absolutely_minimum_req', 'POSTGRES_PASSWORD': 'prod_postgres_highly_secure_password', 'REDIS_PASSWORD': 'prod_redis_highly_secure_password', 'GEMINI_API_KEY': 'prod_gemini_api_key_highly_secure'}
        with self.temp_env_vars(**sensitive_config):
            ...
            sensitive_values = ['prod_jwt_highly_secure', 'prod_service_highly_secure', 'prod_postgres_highly_secure', 'prod_gemini_api_key_highly_secure']
            for sensitive_value in sensitive_values:
                if sensitive_value in config_str:
                    masked_version = sensitive_value[:3] + '***'
                    self.assertTrue(masked_version in config_str or '***' in config_str, f'Sensitive value {sensitive_value} not properly masked')
    ...
    def test_configuration_access_control_validation(self):
        ...
        for env_name in environments_with_restrictions:
            with self.temp_env_vars(ENVIRONMENT=env_name, JWT_SECRET_KEY=f'{env_name}_jwt_secure_64_characters_absolutely_minimum_required', SECRET_KEY=f'{env_name}_secret_secure_64_characters_absolutely_minimum_required', SERVICE_SECRET=f'{env_name}_service_secure_64_characters_absolutely_minimum_req'):
                config = self.config_manager.get_config()
                self.assertGreaterEqual(len(config.jwt_secret_key), 32)
                self.assertGreaterEqual(len(config.service_secret), 32)
                if env_name == 'production':
    ...
    def test_audit_trail_for_configuration_changes(self):
        ...
        for env_name in environments:
            with self.temp_env_vars(ENVIRONMENT=env_name, JWT_SECRET_KEY=f'{env_name}_jwt_32_characters_minimum', SECRET_KEY=f'{env_name}_secret_32_characters_minimum', SERVICE_SECRET=f'{env_name}_service_32_chars_min'):
                config_before = self.config_manager._config_cache
                self.config_manager.reload_config(force=True)
                config_after = self.config_manager.get_config()
                audit_events.append({'environment': env_name, 'config_changed': config_before != config_after, 'timestamp': time.time()})
    ...
    def test_configuration_foundation_supports_all_business_services(self):
        ...
        """
        business_service_environments = {'development': 'Development environment supporting daily engineering ($50K+ monthly engineering costs)', 'staging': 'Staging environment supporting deployment validation ($100K+ ARR protection)', 'production': 'Production environment supporting all $2M+ business revenue operations'}
        service_validation_results = {}
        for env_name, business_impact in business_service_environments.items():
            env_config = {'ENVIRONMENT': env_name, 'JWT_SECRET_KEY': f'{env_name}_jwt_highly_secure_64_characters_absolutely_minimum_required', 'SECRET_KEY': f'{env_name}_secret_highly_secure_64_characters_absolutely_minimum_required', 'SERVICE_SECRET': f'{env_name}_service_highly_secure_64_characters_absolutely_minimum_req', 'POSTGRES_HOST': f'{env_name}-postgres.internal', 'POSTGRES_USER': f'{env_name}_user', 'POSTGRES_PASSWORD': f'{env_name}_secure_database_password_64_characters', 'POSTGRES_DB': f'netra_{env_name}', 'REDIS_HOST': f'{env_name}-redis.internal', 'CLICKHOUSE_HOST': f'{env_name}-clickhouse.internal', 'CLICKHOUSE_PASSWORD': f'{env_name}_clickhouse_secure_password', 'GEMINI_API_KEY': f'{env_name}_gemini_api_key_for_llm_operations'}
            with self.temp_env_vars(**env_config):
                self.config_manager.reload_config(force=True)
                with self.measure_config_load_time():
                    config = self.config_manager.get_config()
                config_load_time = self.get_metric('config_load_time_seconds')
                validation_results = {'config_loaded': config is not None, 'correct_environment': config.environment == env_name, 'database_configured': config.database_url is not None and ('postgresql' in config.database_url or 'sqlite' in config.database_url), 'auth_configured': config.jwt_secret_key is not None and len(config.jwt_secret_key) >= 32, 'service_auth_configured': config.service_secret is not None and len(config.service_secret) >= 32, 'websocket_configured': config.ws_config is not None and config.ws_config.connection_timeout > 0, 'redis_configured': config.redis.host is not None, 'clickhouse_configured': config.clickhouse_native.host is not None, 'llm_configured': config.gemini_api_key is not None, 'performance_acceptable': config_load_time < 15.0, 'validation_passes': self.config_manager.validate_config_integrity()}
                ...
                service_validation_results[env_name] = {'business_impact': business_impact, 'validation_results': validation_results, 'config_load_time': config_load_time, 'all_services_supported': all_validations_pass}
        all_environments_support_business = all((result['all_services_supported'] for result in service_validation_results.values()))
        self.assertTrue(all_environments_support_business, f'Configuration foundation does not support all business services: {service_validation_results}')
    ...
    def test_enterprise_compliance_configuration_requirements(self):
        ...
        """
        enterprise_config = {'ENVIRONMENT': 'production', 'JWT_SECRET_KEY': 'enterprise_jwt_highly_secure_256_bit_key_absolutely_minimum_required_for_compliance', 'SECRET_KEY': 'enterprise_secret_highly_secure_256_bit_key_absolutely_minimum_required_for_compliance', 'SERVICE_SECRET': 'enterprise_service_highly_secure_256_bit_key_absolutely_minimum_required_compliance', 'FERNET_KEY': 'enterprise_fernet_highly_secure_key_for_data_encryption_compliance', 'POSTGRES_HOST': 'enterprise-postgres-primary.internal', 'POSTGRES_USER': 'enterprise_admin_user', 'POSTGRES_PASSWORD': 'enterprise_postgres_highly_secure_password_256_bit_compliance_required', 'POSTGRES_DB': 'netra_enterprise_production', 'REDIS_HOST': 'enterprise-redis-cluster.internal', 'CLICKHOUSE_HOST': 'enterprise-clickhouse-cluster.internal', 'CLICKHOUSE_PASSWORD': 'enterprise_clickhouse_highly_secure_password_compliance', 'OTEL_ENABLED': 'true', 'OTEL_SERVICE_NAME': 'netra-backend-enterprise'}
        with self.temp_env_vars(**enterprise_config):
            config = self.config_manager.get_config()
            security_compliance = {'jwt_secret_length': len(config.jwt_secret_key) >= 64, 'service_secret_length': len(config.service_secret) >= 64, 'secret_key_length': len(config.secret_key) >= 64, 'secrets_are_different': len(set([config.jwt_secret_key, config.service_secret, config.secret_key])) == 3, 'no_debug_mode': not config.debug, 'secure_log_level': config.log_level in ['INFO', 'WARNING', 'ERROR']}
            reliability_compliance = {'database_configured': config.database_url is not None, 'redis_mandatory': config.redis.host is not None, 'clickhouse_mandatory': config.clickhouse_native.host is not None, 'websocket_timeouts_configured': all([config.ws_config.connection_timeout > 0, config.ws_config.recv_timeout > 0, config.ws_config.heartbeat_timeout > 0]), 'database_pool_configured': config.db_pool_size >= 20}
            observability_compliance = {'telemetry_enabled': config.otel_enabled == 'true', 'service_named': config.otel_service_name is not None, 'no_console_export': config.otel_console_exporter == 'false', 'logging_configured': config.log_level != 'DEBUG'}
            all_compliance_checks = {**security_compliance, **reliability_compliance, **observability_compliance}
            compliance_failures = [check_name for check_name, passed in all_compliance_checks.items() if not passed]
            self.assertEqual(len(compliance_failures), 0, f'Enterprise compliance failures: {compliance_failures}')
            with self.measure_config_load_time():
