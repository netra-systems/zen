...
from netra_backend.app.middleware.uvicorn_protocol_enhancement import UvicornWebSocketExclusionMiddleware, create_uvicorn_websocket_exclusion_middleware
...
class Issue449MiddlewareCompatibilityValidationTests(SSotBaseTestCase):
    """
    ...
    def test_enhanced_middleware_handles_cors_compatibility(self):
        ...
        http_response = client.get('/test')
        self.assertEqual(http_response.status_code, 200)
        self.assertIn('access-control-allow-origin', [h.lower() for h in http_response.headers.keys()], 'HTTP response should have CORS headers')
        try:
            with client.websocket_connect('/ws') as websocket:
                data = websocket.receive_text()
                self.assertEqual(data, 'Enhanced WebSocket working')
    ...
    def test_enhanced_middleware_stack_ordering_validation(self):
        ...
        """
        for order_index, middleware_order in enumerate([[self.enhanced_middleware_class, CompatibleSessionMiddleware, CompatibleCORSMiddleware], [CompatibleSessionMiddleware, self.enhanced_middleware_class, CompatibleCORSMiddleware]]):
    ...
    def test_enhanced_middleware_websocket_upgrade_detection(self):
        ...
        self.test_client = client
        try:
            response = client.get('/ws', headers={'Connection': 'Upgrade', 'Upgrade': 'websocket', 'Sec-WebSocket-Version': '13', 'Sec-WebSocket-Key': 'dGhlIHNhbXBsZSBub25jZQ=='})
            self.assertEqual(response.status_code, 426, 'WebSocket upgrade detection should return 426 Upgrade Required')
            response_data = response.json()
            self.assertEqual(response_data['error'], 'websocket_upgrade_required', 'Should identify WebSocket upgrade requirement')
            self.assertEqual(response_data['middleware'], 'uvicorn_websocket_exclusion', 'Should identify enhanced middleware handling')
        except Exception as e:
...
@pytest.mark.integration
class Issue449EnhancedMiddlewareFactoryTests(SSotBaseTestCase):
    """
    ...
    def test_enhanced_middleware_factory_creation(self):
        ...
        self.assertEqual(middleware_class, UvicornWebSocketExclusionMiddleware, 'Factory should return UvicornWebSocketExclusionMiddleware class')
        mock_app = AsyncMock()
        middleware_instance = middleware_class(mock_app)
        self.assertIsNotNone(middleware_instance.protocol_validator, 'Middleware should have protocol validator')
        self.assertIsInstance(middleware_instance.middleware_conflicts, list, 'Middleware should track conflicts')
        self.assertIsInstance(middleware_instance.protocol_recoveries, list, 'Middleware should track recoveries')
