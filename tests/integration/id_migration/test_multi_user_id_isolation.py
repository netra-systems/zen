...
class MultiUserIdIsolationTests(SSotAsyncTestCase):
    ...
    async def test_user_context_id_isolation(self):
        ...
        for i, user_a in enumerate(user_ids):
            for j, user_b in enumerate(user_ids[i + 1:], i + 1):
                ...
                isolation_issues = await self._check_user_context_isolation(user_a, context_a, user_b, context_b)
        ...
        assert len(isolation_violations) == 0, f'Found {len(isolation_violations)} user isolation violations across {len(user_contexts)} users. Isolation success rate: {isolation_success_rate:.2f}%. User contexts must maintain complete isolation. Sample violations: {isolation_violations[:3]}'
    ...
    async def test_websocket_connection_id_isolation(self):
        ...
        if duplicate_count > 0:
            connection_isolation_violations.append({'type': 'duplicate_connection_ids', 'count': duplicate_count, 'total_connections': len(all_connection_ids)})
        for user_id, connections in user_connections.items():
            for connection_id in connections:
                if not await self._connection_contains_user_context(connection_id, user_id):
                    connection_isolation_violations.append({'type': 'missing_user_context', 'user_id': user_id, 'connection_id': connection_id})
        ...
        assert len(connection_isolation_violations) == 0, f'Found {len(connection_isolation_violations)} WebSocket isolation violations. Total connections: {len(all_connection_ids)}, Unique connections: {len(connection_set)}, Duplicates: {duplicate_count}. WebSocket connections must maintain strict user isolation.'
    ...
    async def test_concurrent_user_id_generation_isolation(self):
        ...
        async def generate_user_ids(user_index: int) -> Dict[str, List[str]]:
            ...
            user_ids = {'user_id': user_id, 'session_ids': [], 'execution_ids': [], 'request_ids': [], 'websocket_ids': []}
            try:
                for _ in range(ids_per_user):
                    user_ids['session_ids'].append(await self._generate_real_session_id(user_id))
                    user_ids['execution_ids'].append(await self._generate_real_execution_id(user_id))
                    user_ids['request_ids'].append(await self._generate_real_request_id(user_id))
                    user_ids['websocket_ids'].append(await self._generate_real_websocket_connection_id(user_id))
            except Exception as e:
                self.record_metric(f'concurrent_generation_error_{user_index}', str(e))
            return user_ids
        concurrent_tasks = [generate_user_ids(i) for i in range(concurrent_users)]
        user_id_collections = await asyncio.gather(*concurrent_tasks, return_exceptions=True)
        valid_collections = [collection for collection in user_id_collections if not isinstance(collection, Exception)]
        all_generated_ids = []
        user_id_mappings = {}
        ...
        if collision_count > 0:
            concurrency_violations.append({'type': 'concurrent_id_collision', 'collision_count': collision_count, 'total_ids': len(all_generated_ids)})
        for user_a, collection_a in user_id_mappings.items():
            for user_b, collection_b in user_id_mappings.items():
                if user_a >= user_b:
                    continue
                contamination = await self._detect_concurrent_cross_contamination(user_a, collection_a, user_b, collection_b)
        ...
        assert len(concurrency_violations) == 0, f'Found {len(concurrency_violations)} concurrent isolation violations. Concurrent users: {len(valid_collections)}, Total IDs generated: {len(all_generated_ids)}, Collisions: {collision_count}. Concurrent ID generation must maintain user isolation boundaries.'
    ...
    async def test_database_id_consistency_across_users(self):
        ...
        for user_id in test_users:
            try:
                ...
                user_database_ids[user_id] = {'session_id': db_session_id, 'execution_id': db_execution_id}
            except Exception as e:
                self.record_metric(f'database_operation_error_{user_id}', str(e))
        for user_id, db_ids in user_database_ids.items():
            try:
                retrieved_session = await self._retrieve_database_session(user_id, db_ids['session_id'])
                retrieved_execution = await self._retrieve_database_execution(user_id, db_ids['execution_id'])
                if not retrieved_session:
                    db_consistency_violations.append({'type': 'session_retrieval_failed', 'user_id': user_id, 'session_id': db_ids['session_id']})
                if not retrieved_execution:
                    db_consistency_violations.append({'type': 'execution_retrieval_failed', 'user_id': user_id, 'execution_id': db_ids['execution_id']})
            except Exception as e:
                db_consistency_violations.append({'type': 'database_retrieval_error', 'user_id': user_id, 'error': str(e)})
        for user_a in test_users:
            for user_b in test_users:
                ...
                if user_a in user_database_ids and user_b in user_database_ids:
                    ...
                    try:
                        leaked_data = await self._retrieve_database_session(user_a, user_b_session_id)
                        if leaked_data:
                            db_consistency_violations.append({'type': 'cross_user_data_leak', 'accessing_user': user_a, 'target_user': user_b, 'leaked_session_id': user_b_session_id})
                    except Exception:
                        pass
        ...
        assert len(db_consistency_violations) == 0, f'Found {len(db_consistency_violations)} database consistency violations. Users tested: {len(test_users)}, Database operations: {len(user_database_ids) * 2}. Database operations must maintain strict user data isolation.'
    ...
    async def _create_real_user_execution_context(self, user_id: str) -> Dict[str, Any]:
        ...
        try:
            user_context = UserExecutionContext.create_for_user(user_id=UserID(user_id), request_data={'integration_test': True})
            return {'user_id': user_id, 'thread_id': user_context.thread_id, 'run_id': user_context.run_id, 'execution_id': user_context.execution_id, 'context_object': user_context}
        except Exception as e:
            self.record_metric(f'user_context_creation_failed_{user_id}', str(e))
            return {'user_id': user_id, 'thread_id': f'fallback_thread_{user_id}', 'run_id': f'fallback_run_{user_id}', 'execution_id': f'fallback_execution_{user_id}', 'context_object': None}

    async def _check_user_context_isolation(self, user_a: str, context_a: Dict[str, Any], user_b: str, context_b: Dict[str, Any]) -> List[Dict[str, Any]]:
        """Check isolation between two user contexts."""
        violations = []
        if context_a['thread_id'] == context_b['thread_id']:
            violations.append({'type': 'thread_id_collision', 'user_a': user_a, 'user_b': user_b, 'shared_id': context_a['thread_id']})
        if context_a['run_id'] == context_b['run_id']:
            violations.append({'type': 'run_id_collision', 'user_a': user_a, 'user_b': user_b, 'shared_id': context_a['run_id']})
        if context_a['execution_id'] == context_b['execution_id']:
            violations.append({'type': 'execution_id_collision', 'user_a': user_a, 'user_b': user_b, 'shared_id': context_a['execution_id']})
        if await self._ids_have_security_overlap(self._extract_all_ids(context_a), self._extract_all_ids(context_b)):
            violations.append({'type': 'security_pattern_overlap', 'user_a': user_a, 'user_b': user_b, 'description': "User ID patterns detected in other user's context"})
        return violations
            ...
            for j, user_b in enumerate(users[i + 1:], i + 1):
                ...
                for conn_a in connections_a:
                    for conn_b in connections_b:
                        ...
                        if similarity > 0.8:
                            contamination.append({'type': 'connection_pattern_similarity', 'user_a': user_a, 'user_b': user_b, 'connection_a': conn_a, 'connection_b': conn_b, 'similarity': similarity})
    ...
    async def _detect_concurrent_cross_contamination(self, user_a: str, collection_a: Dict[str, Any], user_b: str, collection_b: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        for id_type in id_types:
            ...
            if common_ids:
                contamination.append({'type': f'concurrent_{id_type}_collision', 'user_a': user_a, 'user_b': user_b, 'common_ids': list(common_ids)})
        return contamination
