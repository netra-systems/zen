...
from netra_backend.app.services.user_execution_context import UserExecutionContext, UserContextFactory, InvalidContextError, ContextIsolationError
from netra_backend.app.services.unified_authentication_service import UnifiedAuthenticationService, AuthResult
from netra_backend.app.clients.auth_client_core import AuthServiceClient, AuthServiceError, AuthServiceConnectionError, AuthServiceValidationError
...
class CrossServiceSessionContinuityRegressionTests(SSotBaseTestCase):
    """
            ...
            if self.auth_db and self.test_sessions_created:
                async with self.auth_db.get_session() as session:
                    for session_id in self.test_sessions_created:
                        await session.execute('DELETE FROM auth_sessions WHERE id = :session_id', {'session_id': session_id})
                    await session.commit()
    ...
    async def test_backend_to_auth_service_context_preservation(self):
        ...
        try:
            ...
            original_context = UserExecutionContext.from_request(user_id=user_id, thread_id=thread_id, run_id=run_id, agent_context={'operation_name': 'cross_service_test'}, audit_metadata={'test_type': 'backend_to_auth'})
            ...
            auth_result = await self.auth_service.validate_token(token=jwt_token, execution_context=original_context)
            ...
            self.assertEqual(child_context.thread_id, thread_id, 'Child context must preserve original thread_id')
            self.assertEqual(child_context.run_id, run_id, 'Child context must preserve original run_id')
            self.assertEqual(child_context.user_id, user_id, 'Child context must preserve original user_id')
            self.assertEqual(child_context.parent_request_id, original_request_id, 'Child context must reference original parent request_id')
            self.assertEqual(child_context.operation_depth, 1, 'Child context should have incremented operation depth')
    ...
    async def test_auth_service_response_context_consistency(self):
        ...
        try:
            ...
            thread_id = UnifiedIdGenerator.generate_thread_id()
            run_id = UnifiedIdGenerator.generate_run_id(thread_id)
            context_with_audit = UserExecutionContext.from_request(user_id=user_id, thread_id=thread_id, run_id=run_id, agent_context={'operation_name': 'auth_response_test', 'service_boundary': 'backend_to_auth'}, audit_metadata={'test_type': 'auth_response_consistency', 'client_ip': '127.0.0.1', 'user_agent': 'test-agent/1.0', 'original_request_time': datetime.now(timezone.utc).isoformat()})
            jwt_token = await self.auth_helper.create_test_jwt(user_id=user_id)
            consistency_start = time.time()
            results = []
            for i in range(3):
                result = await self.auth_service.validate_token(token=jwt_token, execution_context=context_with_audit)
            ...
            self.assertEqual(audit_data['test_type'], 'auth_response_consistency', 'Audit metadata should be preserved')
            self.assertIn('original_request_time', audit_data, 'Original audit metadata should be preserved')
    ...
    async def test_cross_service_session_reuse_prevention(self):
        ...
        try:
            ...
            async with self.backend_db.get_session() as db_session:
                context_with_session = UserExecutionContext.from_request(user_id=user_id, thread_id=thread_id, run_id=run_id, db_session=db_session, agent_context={'operation_name': 'session_reuse_test'}).with_db_session(db_session)
                session_reuse_start = time.time()
                jwt_token = await self.auth_helper.create_test_jwt(user_id=user_id)
                session_operations = []
                for i in range(5):
                    operation_start = time.time()
                    auth_result = await self.auth_service.validate_token(token=jwt_token, execution_context=context_with_session)
                    operation_time = time.time() - operation_start
                    session_operations.append({'operation_id': i, 'duration_ms': round(operation_time * 1000, 2), 'user_id': auth_result.user_id, 'context_request_id': context_with_session.request_id})
                ...
                self.assertLessEqual(efficiency_ratio, 1.5, 'Session reuse should maintain or improve performance')
                request_ids = set((op['context_request_id'] for op in session_operations))
                self.assertEqual(len(request_ids), 1, 'All operations should use the same execution context')
    ...
    async def test_websocket_cross_service_auth_continuity(self):
        ...
        try:
            ...
            websocket_context = UserExecutionContext.from_request(user_id=user_id, thread_id=thread_id, run_id=run_id, websocket_client_id=websocket_client_id, agent_context={'operation_name': 'websocket_auth_test', 'connection_type': 'websocket', 'client_id': websocket_client_id}, audit_metadata={'test_type': 'websocket_cross_service_auth', 'connection_source': 'test_client', 'auth_flow': 'jwt_websocket'})
            jwt_token = await self.auth_helper.create_test_jwt(user_id=user_id, additional_claims={'websocket_client_id': websocket_client_id})
            websocket_auth_start = time.time()
            auth_result = await self.auth_service.validate_websocket_token(token=jwt_token, websocket_client_id=websocket_client_id, execution_context=websocket_context)
            ...
            self.assertEqual(websocket_context.websocket_client_id, websocket_client_id, 'WebSocket client ID preserved')
            self.assertEqual(websocket_context.thread_id, thread_id, 'Thread ID preserved in WebSocket context')
            self.assertEqual(websocket_context.run_id, run_id, 'Run ID preserved in WebSocket context')
            message_context = websocket_context.create_child_context('websocket_message_handling', additional_agent_context={'message_type': 'test_message', 'authenticated': True})
            self.assertEqual(message_context.websocket_client_id, websocket_client_id, 'Child context preserves WebSocket client ID')
            self.assertEqual(message_context.user_id, user_id, 'Child context preserves user ID')
            self.assertEqual(message_context.thread_id, thread_id, 'Child context preserves thread ID')
            self.assertEqual(message_context.parent_request_id, websocket_context.request_id, 'Child context references correct parent')
            audit_trail = message_context.get_audit_trail()
            self.assertIn('websocket_client_id', audit_trail.get('audit_metadata', {}), 'Audit trail should include WebSocket client ID')
            self.assertEqual(audit_trail['user_id'], user_id, 'Audit trail preserves user ID')
    ...
    async def test_multi_service_conversation_flow_continuity(self):
        ...
        try:
            ...
            thread_id = UnifiedIdGenerator.generate_thread_id()
            run_id = UnifiedIdGenerator.generate_run_id(thread_id)
            conversation_context = UserExecutionContext.from_request(user_id=user_id, thread_id=thread_id, run_id=run_id, agent_context={'operation_name': 'multi_service_conversation', 'conversation_phase': 'initialization'}, audit_metadata={'test_type': 'multi_service_flow', 'conversation_id': str(uuid.uuid4()), 'flow_stage': 'start'})
            conversation_start = time.time()
            conversation_steps = []
            jwt_token = await self.auth_helper.create_test_jwt(user_id=user_id)
            step1_start = time.time()
            auth_result = await self.auth_service.validate_token(token=jwt_token, execution_context=conversation_context)
            step1_time = time.time() - step1_start
            self.assertTrue(auth_result.success, 'Step 1: Auth validation should succeed')
            conversation_steps.append({'step': 1, 'operation': 'auth_validation', 'duration_ms': round(step1_time * 1000, 2), 'context_preserved': auth_result.user_id == user_id})
            step2_start = time.time()
            agent_context = conversation_context.create_child_context('agent_processing', additional_agent_context={'agent_type': 'data_analysis', 'authenticated': True, 'conversation_phase': 'processing'}, additional_audit_metadata={'flow_stage': 'agent_processing', 'auth_validated': True})
            step2_time = time.time() - step2_start
            conversation_steps.append({'step': 2, 'operation': 'child_context_creation', 'duration_ms': round(step2_time * 1000, 2), 'context_preserved': agent_context.thread_id == thread_id})
            step3_start = time.time()
            secondary_auth = await self.auth_service.validate_token(token=jwt_token, execution_context=agent_context)
            step3_time = time.time() - step3_start
            self.assertTrue(secondary_auth.success, 'Step 3: Secondary auth should succeed')
            conversation_steps.append({'step': 3, 'operation': 'secondary_auth_validation', 'duration_ms': round(step3_time * 1000, 2), 'context_preserved': secondary_auth.user_id == user_id})
            step4_start = time.time()
            response_context = agent_context.create_child_context('response_generation', additional_agent_context={'conversation_phase': 'response', 'response_type': 'data_analysis_result'}, additional_audit_metadata={'flow_stage': 'response_generation', 'processing_completed': True})
            step4_time = time.time() - step4_start
            conversation_steps.append({'step': 4, 'operation': 'response_context_creation', 'duration_ms': round(step4_time * 1000, 2), 'context_preserved': response_context.thread_id == thread_id})
            ...
            for i, context in enumerate(contexts_to_validate):
                self.assertEqual(context.user_id, user_id, f'Context {i}: User ID preserved throughout flow')
                self.assertEqual(context.thread_id, thread_id, f'Context {i}: Thread ID preserved throughout flow')
                self.assertEqual(context.run_id, run_id, f'Context {i}: Run ID preserved throughout flow')
                context.verify_isolation()
            self.assertEqual(agent_context.parent_request_id, conversation_context.request_id, 'Agent context should reference initial context as parent')
            self.assertEqual(response_context.parent_request_id, agent_context.request_id, 'Response context should reference agent context as parent')
    ...
    async def test_service_boundary_context_consistency_validation(self):
        ...
        try:
            ...
            thread_id = UnifiedIdGenerator.generate_thread_id()
            run_id = UnifiedIdGenerator.generate_run_id(thread_id)
            boundary_tests = [{'boundary': 'backend_to_auth', 'context_type': 'api_request', 'expected_fields': ['user_id', 'thread_id', 'run_id', 'request_id']}, {'boundary': 'auth_to_backend', 'context_type': 'auth_response', 'expected_fields': ['user_id', 'thread_id', 'run_id', 'request_id']}, {'boundary': 'websocket_to_auth', 'context_type': 'websocket_auth', 'expected_fields': ['user_id', 'thread_id', 'run_id', 'websocket_client_id']}, {'boundary': 'database_cross_service', 'context_type': 'db_transaction', 'expected_fields': ['user_id', 'thread_id', 'run_id', 'db_session']}]
            boundary_validation_results = []
            validation_start = time.time()
            for boundary_test in boundary_tests:
                test_start = time.time()
                if boundary_test['context_type'] == 'websocket_auth':
                    test_context = UserExecutionContext.from_request(user_id=user_id, thread_id=thread_id, run_id=run_id, websocket_client_id=f'ws_{uuid.uuid4().hex[:8]}', agent_context={'boundary_test': boundary_test['boundary'], 'context_type': boundary_test['context_type']})
                elif boundary_test['context_type'] == 'db_transaction':
                    async with self.backend_db.get_session() as db_session:
                        test_context = UserExecutionContext.from_request(user_id=user_id, thread_id=thread_id, run_id=run_id, db_session=db_session, agent_context={'boundary_test': boundary_test['boundary'], 'context_type': boundary_test['context_type']})
                else:
                    test_context = UserExecutionContext.from_request(user_id=user_id, thread_id=thread_id, run_id=run_id, agent_context={'boundary_test': boundary_test['boundary'], 'context_type': boundary_test['context_type']})
                validation_errors = []
                ...
                try:
                    ...
                    serialization_valid = all((key in context_dict for key in ['user_id', 'thread_id', 'run_id']))
                except Exception as e:
                    validation_errors.append(f'Serialization error: {e}')
                    serialization_valid = False
                test_time = time.time() - test_start
                boundary_result = {'boundary': boundary_test['boundary'], 'context_type': boundary_test['context_type'], 'validation_time_ms': round(test_time * 1000, 2), 'validation_errors': validation_errors, 'isolation_valid': isolation_valid, 'serialization_valid': serialization_valid, 'overall_valid': len(validation_errors) == 0 and isolation_valid and serialization_valid}
                boundary_validation_results.append(boundary_result)
                self.assertTrue(boundary_result['overall_valid'], f"Boundary validation failed for {boundary_test['boundary']}: {validation_errors}")
            ...
            self.record_metric('boundary_validation_success_rate', sum((1 for r in boundary_validation_results if r['overall_valid'])) / len(boundary_validation_results))
...
async def simulate_cross_service_api_call(source_service: str, target_service: str, context: UserExecutionContext, operation: str) -> Dict[str, Any]:
    """
    ...
    return {'success': True, 'source_service': source_service, 'target_service': target_service, 'operation': operation, 'context_preserved': True, 'user_id': context.user_id, 'thread_id': context.thread_id, 'run_id': context.run_id, 'call_duration_ms': round(call_time * 1000, 2)}

def validate_cross_service_context_integrity(original_context: UserExecutionContext, returned_context: UserExecutionContext) -> List[str]:
    """
