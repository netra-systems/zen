...
class TestRedisWebSocketIntegration(SSotAsyncTestCase):
    ...
    async def asyncSetUp(self):
        ...
        logger.info(f"Test setup complete - Run ID: {self.test_run_id}, User ID: {self.test_user_id}")
    ...
    async def asyncTearDown(self):
        ...
        try:
            ...
            if redis_manager.is_connected:
                # Clean up test keys
                test_patterns = [
                    f"websocket_state:{self.test_run_id}*",
                    f"agent_state:{self.test_run_id}*", 
            ...
            logger.debug(f"Integration cleanup error (non-critical): {e}")
    ...
    async def test_websocket_state_persistence_in_redis(self):
        ...
        state_key = f"websocket_state:{self.test_run_id}"
    ...
    async def test_websocket_event_sequence_redis_tracking(self):
        ...
        events_key = f"websocket_events:{self.test_run_id}"
    ...
    async def test_concurrent_websocket_redis_operations(self):
        ...
        async def simulate_websocket_session(session_id: str, user_id: str):
            session_run_id = f"run_{session_id}"
        ...
        sessions = [
            ("session_1", f"user_1_{uuid.uuid4().hex[:4]}"),
            ("session_2", f"user_2_{uuid.uuid4().hex[:4]}"),
        ...
        for session_id, user_id in sessions:
            session_data_json = await redis_manager.get(f"session:{session_id}")
            self.assertIsNotNone(session_data_json, f"Session {session_id} should be stored")
            ...
            self.assertEqual(events_count, 3, f"Session {session_id} should have 3 events")
    ...
    async def test_redis_circuit_breaker_websocket_resilience(self):
        ...
        if redis_manager.is_connected and can_execute:
            ...
            logger.info(f"Circuit breaker status after reset: {status}")
...
class TestRedisWebSocketErrorRecovery(SSotAsyncTestCase):
    ...
    async def asyncSetUp(self):
        ...
        self.test_run_id = f"recovery_test_{uuid.uuid4().hex[:8]}"
        self.test_user_id = f"recovery_user_{uuid.uuid4().hex[:8]}"
    ...
    async def asyncTearDown(self):
        ...
        try:
            ...
            logger.debug(f"Recovery cleanup error (non-critical): {e}")
    ...
    async def test_websocket_state_recovery_after_redis_reconnect(self):
        ...
        state_key = f"recovery:websocket:{self.test_run_id}"
        ...
        for event in events_to_test:
            ...
            try:
                ...
                if stored:
                    ...
                    logger.info(f"Event stored in Redis: {event['type']}")
                else:
                    logger.info(f"Event storage failed gracefully: {event['type']}")
            except Exception as e:
                logger.info(f"Event storage exception handled gracefully: {e}")
