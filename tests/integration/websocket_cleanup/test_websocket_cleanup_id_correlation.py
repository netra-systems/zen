...
class TestWebSocketCleanupCorrelation(SSotAsyncTestCase):
    ...
    async def test_cleanup_correlation_with_mismatched_ids(self):
        ...
        user_context = self._create_mock_user_context(user_id=legacy_user_id, thread_id=legacy_thread_id, run_id=legacy_run_id, websocket_client_id=connection_id)
        ...
        self.assertFalse(cleanup_result.success, 'Cleanup should fail with mismatched ID formats')
        self.assertGreater(cleanup_result.leaked_resources, 0, 'Resource leaks should be detected')
        self.assertTrue(cleanup_result.correlation_failed, 'Correlation failure should be detected')
    ...
    async def test_cleanup_correlation_with_ssot_ids(self):
        ...
        user_context = self._create_mock_user_context(user_id=ssot_user_id, thread_id=ssot_thread_id, run_id=ssot_run_id, websocket_client_id=connection_id)
        await self._register_websocket_resources(mock_websocket, user_context)
        cleanup_result = await self._attempt_websocket_cleanup(user_context)
        self.assertTrue(cleanup_result.success, 'Cleanup should succeed with SSOT ID formats')
        self.assertEqual(cleanup_result.leaked_resources, 0, 'No resource leaks should occur')
        self.assertFalse(cleanup_result.correlation_failed, 'Correlation should work correctly')
        self.assertGreater(cleanup_result.cleaned_resources, 0, 'Resources should be successfully cleaned')
    ...
    async def test_websocket_connection_id_audit_trail(self):
        ...
        user_context = self._create_mock_user_context(user_id=user_id, thread_id=thread_id, run_id=run_id, websocket_client_id=connection_id)
        ...
        await audit_trail.track_websocket_events(connection_id, run_id, ['agent_started', 'agent_thinking', 'tool_executing', 'tool_completed', 'agent_completed'])
        await audit_trail.track_cleanup_attempt(user_context)
        trail_validation = audit_trail.validate_consistency()
        self.assertTrue(trail_validation.is_complete, 'Audit trail should be complete')
        self.assertTrue(trail_validation.ids_consistent, 'IDs should be consistent throughout lifecycle')
        self.assertEqual(trail_validation.missing_phases, [], 'No lifecycle phases should be missing')
        self.assertTrue(trail_validation.thread_run_correlation, 'Thread ID and run ID should correlate')
        self.assertTrue(trail_validation.user_connection_correlation, 'User ID and connection ID should correlate')
    ...
    async def test_legacy_to_ssot_migration_compatibility(self):
        ...
        ssot_thread_id = self.id_manager.generate_id(IDType.THREAD, prefix='demo')
        ssot_run_id = UnifiedIDManager.generate_run_id(ssot_thread_id)
        user_context = self._create_mock_user_context(user_id=legacy_user_id, thread_id=ssot_thread_id, run_id=ssot_run_id, websocket_client_id=f'ws-{uuid.uuid4()}')
        migration_result = await self._test_migration_compatibility(user_context)
        self.assertTrue(migration_result.handles_mixed_patterns, 'System should handle mixed ID patterns')
        self.assertLessEqual(migration_result.resource_leaks, 1, 'Resource leaks should be minimal during migration')

    def _create_mock_user_context(self, user_id: str, thread_id: str, run_id: str, websocket_client_id: str) -> UserExecutionContext:
        ...
        mock_context._validate_thread_run_id_consistency = MagicMock(return_value=self._test_correlation(thread_id, run_id))
        return mock_context
    ...
    async def _register_websocket_resources(self, websocket: 'MockWebSocket', user_context: UserExecutionContext) -> None:
    ...
    def _test_id_correlation(self, user_context: UserExecutionContext) -> bool:
        ...
        try:
            ...
            if user_context.run_id.startswith(('websocket_factory_', 'context_', 'agent_')):
                return user_context.run_id in user_context.thread_id and user_context.thread_id.startswith('thread_')
...
class WebSocketAuditTrail:
    ...
    async def track_context_creation(self, user_context: UserExecutionContext) -> None:
        ...
        self._record_id_usage('context_creation', [user_context.user_id, user_context.thread_id, user_context.run_id, user_context.websocket_client_id])
    ...
    async def track_agent_execution(self, user_context: UserExecutionContext, agent_name: str) -> None:
        ...
        self._record_id_usage('agent_execution', [user_context.thread_id, user_context.run_id])
    ...
    async def track_cleanup_attempt(self, user_context: UserExecutionContext) -> None:
        ...
        self._record_id_usage('cleanup_attempt', [user_context.user_id, user_context.websocket_client_id])

    def validate_consistency(self) -> 'TrailValidation':
        """Validate audit trail consistency."""
        validation = TrailValidation()
        expected_phases = ['connection_establishment', 'context_creation', 'agent_execution', 'websocket_events', 'cleanup_attempt']
    ...
    def _validate_user_connection_correlation(self) -> bool:
        ...
        return bool(user_ids_conn and user_ids_context and any((uid in user_ids_context for uid in user_ids_conn)))
