...
class TestMultiUserMessageIsolation(SSotAsyncTestCase):
    ...
    def setUp(self):
        ...
        super().setUp()
        self.test_users = [
            "user_isolation_alice",
            "user_isolation_bob",
            "user_isolation_charlie",
            "user_isolation_diana"
        ]
    ...
    def _prepare_isolation_test_scenarios(self):
        ...
        self.isolation_test_scenarios = [
            {
                'name': 'basic_user_message_isolation',
                'user_count': 2,
                'messages_per_user': 5,
                'message_type': 'user_message',
                'expected_isolation_rate': 1.0  # 100% - critical security requirement
            },
            {
                'name': 'agent_response_targeting',
                'user_count': 3,
                'messages_per_user': 8,
                'message_type': 'agent_response',
                'expected_isolation_rate': 1.0  # 100% - critical for Golden Path
            },
            {
                ...
                'expected_isolation_rate': 1.0  # 100% - business critical
            },
            {
                'name': 'high_frequency_user_events',
                'user_count': 2,
                'messages_per_user': 20,
                'message_type': 'system_message',
                'expected_isolation_rate': 0.98  # 98% - allow minimal cross-talk for system messages
            },
            {
                'name': 'user_context_sensitive_data',
                'user_count': 3,
                'messages_per_user': 6,
                'message_type': 'agent_response',
                'expected_isolation_rate': 1.0,  # 100% - privacy critical
    ...
    async def test_multi_user_message_isolation_maintenance(self):
        ...
        # Log comprehensive isolation analysis
        ...
        # GOLDEN PATH PROTECTION: Must maintain near-perfect isolation
        min_required_isolation_rate = 0.95  # 95% minimum for Golden Path protection

        # SECURITY REQUIREMENT: Sensitive data scenarios must have 100% isolation
        ...
        if overall_isolation_rate >= min_required_isolation_rate and not sensitive_violations:
            ...
            self.fail(
                f"GOLDEN PATH VIOLATION: Multi-user message isolation compromised. "
                f"Failed scenarios: {failed_scenarios}. "
                f"Issues: {' | '.join(error_details)}. "
                f"This indicates MessageRouter SSOT changes are breaking user isolation, "
                f"creating security vulnerabilities and compliance risks for $500K+ ARR platform."
            )
    ...
    async def _test_isolation_scenario(self, scenario: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            for i in range(user_count):
                ...
                user_contexts[user_id] = await self._create_isolated_user_context(
                    user_id, contains_sensitive_data
                )

            # Send messages for each user and track delivery
            message_delivery_tracking = await self._execute_multi_user_message_test(
                user_contexts, message_type, messages_per_user
            )

            # Analyze isolation effectiveness
            isolation_analysis = self._analyze_message_isolation(
                message_delivery_tracking, user_contexts.keys()
            )
            ...
            return {
                ...
                'expected_isolation_rate': expected_isolation_rate,
                'total_messages': isolation_analysis['total_messages'],
                'properly_isolated_messages': isolation_analysis['properly_isolated_messages'],
                'cross_user_leaks': isolation_analysis['cross_user_leaks'],
    ...
    async def _create_isolated_user_context(self, user_id: str, contains_sensitive_data: bool) -> Dict[str, Any]:
        ...
        # Track all messages sent to this user's WebSocket
        async def track_websocket_message(message):
            # Add user context to track message targeting
    ...
    async def _execute_multi_user_message_test(self, user_contexts: Dict[str, Dict[str, Any]],
                                             message_type: str, messages_per_user: int) -> Dict[str, List[Dict]]:
        """Execute multi-user message test and track delivery."""
        message_tracking = {}

        # Send messages to each user and track where they end up
        for sender_user_id, sender_context in user_contexts.items():
            user_messages = []

            for i in range(messages_per_user):
                # Create message with user-specific content
                message_content = self._create_user_specific_message(
                    sender_user_id, sender_context, message_type, i
                )

                # Simulate message routing to ALL users (to test isolation)
                for target_user_id, target_context in user_contexts.items():
                    delivery_result = await self._simulate_message_delivery(
                        message_content, sender_user_id, target_user_id, target_context
                    )
    ...
    def _create_user_specific_message(self, user_id: str, user_context: Dict[str, Any],
                                    message_type: str, message_index: int) -> Dict[str, Any]:
        """Create message with user-specific content for isolation testing."""
        base_message = {
            'type': message_type,
            'user_id': user_id,
            'thread_id': user_context['thread_id'],
            'message_id': f"{user_id}_msg_{message_index}",
            'timestamp': time.time(),
            'payload': {
                'content': f'Message {message_index} for {user_id}',
                'sequence_number': message_index
            }
        }

        # Add sensitive data if this is a sensitive scenario
    ...
    async def _simulate_message_delivery(self, message: Dict[str, Any], sender_user_id: str,
                                       target_user_id: str, target_context: Dict[str, Any]) -> Dict[str, Any]:
        """Simulate message delivery to a target user."""
        try:
            target_websocket = target_context['websocket']

            # This simulates the MessageRouter deciding whether to deliver the message
            # In correct isolation, messages should only be delivered to the intended user
            should_deliver = (sender_user_id == target_user_id)  # Proper isolation logic

            if should_deliver:
                # Deliver message to correct user
                await target_websocket.send_json(message)
                delivery_status = 'delivered'
            else:
                # Message correctly NOT delivered to other users (proper isolation)
            ...
            return {
                ...
                'contains_sensitive_data': 'user_secret' in message.get('payload', {}),
                'timestamp': time.time()
            }
    ...
    def _analyze_message_isolation(self, message_tracking: Dict[str, List[Dict]],
                                 user_ids: Set[str]) -> Dict[str, Any]:
        ...
        for sender_user_id, messages in message_tracking.items():
            for delivery_result in messages:
                ...
                if sender == target and status == 'delivered':
                    # Correct: Message delivered to intended user
                    properly_isolated_messages += 1
                elif sender != target and status == 'properly_isolated':
                    # Correct: Message NOT delivered to other users (proper isolation)
                    properly_isolated_messages += 1
                elif sender != target and status == 'delivered':
                    # VIOLATION: Message leaked to wrong user
                    cross_user_leaks.append({
                        'sender': sender,
                        'leaked_to': target,
                        'message_id': delivery_result['message_id'],
                        'contains_sensitive_data': delivery_result.get('contains_sensitive_data', False)
                    })
                elif sender == target and status in ['properly_isolated', 'error']:
                    # PROBLEM: Message not delivered to intended user
                    cross_user_leaks.append({
                        'sender': sender,
                        'failed_delivery_to': target,
                        'status': status,
                        'message_id': delivery_result['message_id']
                    })
        ...
        return {
            ...
            'properly_isolated_messages': properly_isolated_messages,
            'isolation_rate': isolation_rate,
    ...
    async def test_user_data_contamination_prevention(self):
        ...
        contamination_scenarios = [
            {
                ...
                'user_pairs': [('alice', 'bob'), ('charlie', 'diana')],
                'data_types': ['session_id', 'user_preferences', 'conversation_history'],
                'expected_isolation_rate': 1.0  # 100% - critical for privacy
            },
            {
                'name': 'agent_context_isolation',
                'user_pairs': [('alice', 'bob')],
                'data_types': ['agent_state', 'tool_results', 'execution_context'],
                'expected_isolation_rate': 1.0  # 100% - critical for accuracy
            },
            {
                'name': 'concurrent_session_contamination',
                'user_pairs': [('alice', 'bob'), ('charlie', 'diana')],
                'data_types': ['concurrent_requests', 'response_data'],
                'expected_isolation_rate': 0.98  # 98% - allow minimal system data overlap
            }
        ]
        ...
        if overall_success:
            ...
            self.fail(
                f"GOLDEN PATH VIOLATION: User data contamination prevention compromised in scenarios: {failed_scenarios}. "
                f"This indicates MessageRouter SSOT changes are allowing data leakage between users, "
                f"creating privacy violations and accuracy issues for Golden Path user experience."
            )
        ...
        try:
            ...
            for user_a, user_b in user_pairs:
                for data_type in data_types:
                    test_result = await self._test_user_pair_data_isolation(
                        user_a, user_b, data_type
                    )
            ...
            return {
                ...
                'contamination_details': [t for t in contamination_tests if not t['isolated']]
            }
    ...
    async def _test_user_pair_data_isolation(self, user_a: str, user_b: str, data_type: str) -> Dict[str, Any]:
        ...
        try:
            ...
            # Simulate data operations for both users
            message_a = {
                'type': 'user_data_operation',
                'user_id': user_a,
                'data_type': data_type,
                'data_content': user_a_data,
                'operation': 'store'
            }
            ...
            message_b = {
                ...
                'user_id': user_b,
                'data_type': data_type,
                'data_content': user_b_data,
                'operation': 'store'
            }

            # Process both messages (simulate concurrent operations)
            await asyncio.gather(
                self._simulate_message_delivery(message_a, user_a, user_a, context_a),
                self._simulate_message_delivery(message_b, user_b, user_b, context_b)
            )

            # Check for data contamination
            # In proper isolation, user_a should not see user_b's data and vice versa
            contamination_detected = self._check_for_data_contamination(
                context_a, context_b, user_a_data, user_b_data
            )
            ...
            return {
                ...
                'user_b': user_b,
                'data_type': data_type,
                'isolated': False,  # Assume contamination if we can't test properly
                'error': str(e)
            }

    def _check_for_data_contamination(self, context_a: Dict, context_b: Dict,
                                    user_a_data: str, user_b_data: str) -> bool:
        """Check if data contamination occurred between user contexts."""
        try:
            # Check if user_a received user_b's data (contamination)
            ...
            # Check if user_b received user_a's data (contamination)
            ...
            return True  # Assume contamination if we can't verify isolation
