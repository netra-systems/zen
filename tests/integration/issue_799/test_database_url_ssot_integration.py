...
class TestDatabaseURLSSOTIntegration(BaseIntegrationTest):
    ...
    async def test_appconfig_database_url_ssot_integration_development(self, real_services_fixture):
        ...
        try:
            ...
            # Clear pre-set database_url to force SSOT path
            config.database_url = None
            
            # Test SSOT database URL construction
            database_url = config.get_database_url()
            
            # Verify result is valid PostgreSQL URL
            self.assertIsNotNone(database_url)
            self.assertIn("postgresql://", database_url)
            
            # Verify components are present (should be from SSOT construction)
    ...
    async def test_staging_config_database_url_ssot_integration(self, real_services_fixture):
        ...
        try:
            ...
            config.database_url = None  # Force SSOT path
            ...
            # Verify staging-specific components
    ...
    async def test_database_url_builder_direct_integration(self, real_services_fixture):
        ...
        try:
            # Set up test environment for DatabaseURLBuilder
            test_env_vars = {
                'ENVIRONMENT': 'development',
            ...
            # Test sync URL construction (what AppConfig uses)
            ...
            self.assertIn("5433", sync_url)  # Custom port
            self.assertIn("test_db", sync_url)
            
            # Test async URL construction as well
            async_url = builder.get_url_for_environment(sync=False)
            if async_url:  # May be None in some environments
    ...
    async def test_appconfig_fallback_behavior_integration(self, real_services_fixture):
        ...
        try:
            # Set up environment for fallback testing
            test_env_vars = {
                'ENVIRONMENT': 'development',
            ...
            # Mock DatabaseURLBuilder to fail (simulating import/runtime error)
            with patch('shared.database_url_builder.DatabaseURLBuilder') as mock_builder:
                ...
                # This should trigger fallback method
                database_url = config.get_database_url()
                
                # Verify fallback produces valid URL
                self.assertIsNotNone(database_url)
                self.assertIn("postgresql://", database_url)
                
                # Verify fallback used our test environment vars
    ...
    async def test_multiple_environment_url_construction_consistency(self, real_services_fixture):
        ...
        """
        environments = [
            ('development', 'dev-host', 'dev_user', 'dev_db'),
            ('staging', 'staging-host', 'staging_user', 'staging_db'),  
            ('production', 'prod-host', 'prod_user', 'prod_db')
        ]
        ...
        for env_name, host, user, db_name in environments:
            with self.subTest(environment=env_name):
                ...
                try:
                    ...
                    # Verify environment-specific components
                    self.assertIsNotNone(database_url)
                    self.assertIn("postgresql://", database_url)
        ...
        for config_class in config_classes:
            with self.subTest(config_class=config_class.__name__):
                ...
                try:
                    ...
                    test_env_vars = {
                        'ENVIRONMENT': 'development',
                        'POSTGRES_HOST': 'ssot-test-host',
                        'POSTGRES_USER': 'ssot_user',
                        'POSTGRES_PASSWORD': 'ssot_pass',
                        'POSTGRES_DB': 'ssot_db', 
                        'POSTGRES_PORT': '5432'
                    }
                    ...
                    # Verify SSOT construction worked
                    self.assertIsNotNone(database_url)
                    self.assertIn("postgresql://", database_url)
