...
from netra_backend.app.websocket_core.event_validator import AgentEventValidator, CriticalAgentEventType, WebSocketEventMessage, validate_agent_events, assert_critical_events_received
...
class ChatWorkflowAgentEventsIntegrationTests(SSotBaseTestCase):
    ...
    async def _create_authenticated_agent_context(self, user_email: Optional[str]=None, agent_name: str='test_chat_agent') -> Tuple[StronglyTypedUserExecutionContext, str]:
        """
        ...
        user_context = await create_authenticated_user_context(user_email=user_email, environment='test', permissions=['read', 'write', 'agent_execution', 'websocket'], websocket_enabled=True)
    ...
    def _create_mock_agent_events(self, user_context: StronglyTypedUserExecutionContext, agent_name: str) -> List[Dict[str, Any]]:
        """Create mock agent events for testing."""
        base_timestamp = datetime.now(timezone.utc)
        events = [{'type': 'agent_started', 'agent_name': agent_name, 'user_id': str(user_context.user_id), 'thread_id': str(user_context.thread_id), 'timestamp': base_timestamp.isoformat(), 'message': f'Agent {agent_name} started processing chat request', 'event_id': f'start_{uuid.uuid4().hex[:8]}'}, {'type': 'agent_thinking', 'agent_name': agent_name, 'user_id': str(user_context.user_id), 'thread_id': str(user_context.thread_id), 'timestamp': base_timestamp.isoformat(), 'message': f'Agent {agent_name} analyzing request and planning approach', 'thinking_stage': 'analysis', 'event_id': f'think_{uuid.uuid4().hex[:8]}'}, {'type': 'tool_executing', 'agent_name': agent_name, 'user_id': str(user_context.user_id), 'thread_id': str(user_context.thread_id), 'timestamp': base_timestamp.isoformat(), 'message': f'Agent {agent_name} executing data analysis tool', 'tool_name': 'data_analysis', 'event_id': f'tool_exec_{uuid.uuid4().hex[:8]}'}, {'type': 'tool_completed', 'agent_name': agent_name, 'user_id': str(user_context.user_id), 'thread_id': str(user_context.thread_id), 'timestamp': base_timestamp.isoformat(), 'message': f'Agent {agent_name} completed data analysis', 'tool_name': 'data_analysis', 'tool_result': 'Analysis complete: 3 insights generated', 'event_id': f'tool_comp_{uuid.uuid4().hex[:8]}'}, {'type': 'agent_completed', 'agent_name': agent_name, 'user_id': str(user_context.user_id), 'thread_id': str(user_context.thread_id), 'timestamp': base_timestamp.isoformat(), 'message': f'Agent {agent_name} completed request processing', 'response': 'Generated comprehensive analysis with actionable insights', 'event_id': f'complete_{uuid.uuid4().hex[:8]}'}]
        return events
    ...
    async def test_agent_event_generation_integration(self):
        ...
        user_context, agent_name = await self._create_authenticated_agent_context(user_email='event_generation_test@example.com')
        self.event_validator = AgentEventValidator(user_context=user_context, strict_mode=True, timeout_seconds=30.0)
    ...
    async def test_websocket_event_delivery_integration(self):
        ...
        user_context, agent_name = await self._create_authenticated_agent_context(user_email='websocket_delivery_test@example.com')
        ...
        for event_data in mock_events:
            try:
                delivery_result = await self.websocket_bridge.route_agent_event(event=event_data, user_id=str(user_context.user_id), websocket_client_id=str(user_context.websocket_client_id))
    ...
    async def test_agent_lifecycle_events_sequence(self):
        ...
        user_context, agent_name = await self._create_authenticated_agent_context(user_email='lifecycle_sequence_test@example.com')
        self.event_validator = AgentEventValidator(user_context=user_context, strict_mode=True, timeout_seconds=30.0)
        lifecycle_events = [{'type': 'agent_started', 'agent_name': agent_name, 'user_id': str(user_context.user_id), 'timestamp': datetime.now(timezone.utc).isoformat(), 'sequence_id': 1}, {'type': 'agent_thinking', 'agent_name': agent_name, 'user_id': str(user_context.user_id), 'timestamp': datetime.now(timezone.utc).isoformat(), 'sequence_id': 2}, {'type': 'tool_executing', 'agent_name': agent_name, 'user_id': str(user_context.user_id), 'timestamp': datetime.now(timezone.utc).isoformat(), 'tool_name': 'analysis_tool', 'sequence_id': 3}, {'type': 'tool_completed', 'agent_name': agent_name, 'user_id': str(user_context.user_id), 'timestamp': datetime.now(timezone.utc).isoformat(), 'tool_name': 'analysis_tool', 'sequence_id': 4}, {'type': 'agent_completed', 'agent_name': agent_name, 'user_id': str(user_context.user_id), 'timestamp': datetime.now(timezone.utc).isoformat(), 'sequence_id': 5}]
    ...
    async def test_business_value_event_content(self):
        ...
        user_context, agent_name = await self._create_authenticated_agent_context(user_email='business_value_test@example.com')
        business_events = [{'type': 'agent_started', 'agent_name': agent_name, 'user_id': str(user_context.user_id), 'timestamp': datetime.now(timezone.utc).isoformat(), 'message': 'Starting comprehensive data analysis to identify key insights', 'business_context': 'Analyzing user data to improve decision making'}, {'type': 'agent_thinking', 'agent_name': agent_name, 'user_id': str(user_context.user_id), 'timestamp': datetime.now(timezone.utc).isoformat(), 'message': 'Analyzing patterns and correlations in dataset', 'thinking_process': 'Identifying trends that could impact business metrics', 'progress_indicator': '40% analysis complete'}, {'type': 'tool_executing', 'agent_name': agent_name, 'user_id': str(user_context.user_id), 'timestamp': datetime.now(timezone.utc).isoformat(), 'message': 'Running statistical analysis on customer behavior data', 'tool_name': 'statistical_analyzer', 'expected_output': 'Customer segmentation and behavioral insights'}, {'type': 'tool_completed', 'agent_name': agent_name, 'user_id': str(user_context.user_id), 'timestamp': datetime.now(timezone.utc).isoformat(), 'message': 'Statistical analysis complete - 5 key insights discovered', 'tool_name': 'statistical_analyzer', 'insights_count': 5, 'business_impact': 'High potential for improving customer retention'}, {'type': 'agent_completed', 'agent_name': agent_name, 'user_id': str(user_context.user_id), 'timestamp': datetime.now(timezone.utc).isoformat(), 'message': 'Analysis complete: Generated actionable recommendations', 'deliverables': ['Customer segmentation report', 'Behavioral trend analysis', '5 actionable recommendations'], 'business_value': 'Identified $50K potential annual savings through customer retention improvements'}]
        for event_data in business_events:
            ...
            self.captured_events.append(event)
        total_business_content_length = sum((len(event_data.get('message', '')) for event_data in business_events))
    ...
    async def test_concurrent_agent_events_isolation(self):
        ...
        user1_context, agent1_name = await self._create_authenticated_agent_context(user_email='concurrent_user1@example.com', agent_name='user1_agent')
        user2_context, agent2_name = await self._create_authenticated_agent_context(user_email='concurrent_user2@example.com', agent_name='user2_agent')
        ...
        async def process_user_events(events: List[Dict], user_id: str):
            ...
            return processed
        user1_processed, user2_processed = await asyncio.gather(process_user_events(user1_events, str(user1_context.user_id)), process_user_events(user2_events, str(user2_context.user_id)))
