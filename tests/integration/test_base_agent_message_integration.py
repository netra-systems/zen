...
# Core agent and service imports
    ...
    async def execute(self, user_message: str, context: Dict[str, Any] = None) -> Dict[str, Any]:
        ...
        if self.llm_manager:
            try:
                ...
                self.execution_metadata["llm_usage"] = {
...
class TestBaseAgentMessageIntegration(BaseIntegrationTest):
    ...
    async def setup_test_environment(self, real_services_fixture):
        ...
        # Set up LLM manager for real testing
        try:
            self.llm_manager = LLMManager()
            await self.llm_manager.initialize()
        except Exception as e:
            # Use fallback for testing without LLM
    ...
    async def test_agent_execute_with_user_context_isolation(self, real_services_fixture):
        ...
        for i in range(3):
            ...
            # Create isolated user context using test helpers
        ...
        # Execute agents concurrently with different messages
        execution_tasks = []
        for i, (agent, user_context) in enumerate(zip(agents, user_contexts)):
            task = asyncio.create_task(
                agent.execute(
                    user_message=f"User {i} optimization request: analyze costs for user {user_context.user_id}",
                    context={"user_specific_data": f"data_for_user_{i}"}
                )
            )
        ...
        for i, result in enumerate(results):
            ...
            # Verify user-specific content in response
        ...
        # Verify each agent maintained separate processing results
    ...
    async def test_agent_metadata_storage_integration(self, real_services_fixture):
        ...
        user_context = await create_test_user_context(
            ...
            request_id=f"metadata_test_{uuid.uuid4().hex[:8]}",
            thread_id=self.test_thread_id,
            real_services=real_services_fixture
        )
        
        # Create agent with metadata tracking
        agent = TestAgent(
            llm_manager=self.llm_manager,
    ...
    async def test_agent_llm_usage_tracking_integration(self, real_services_fixture):
        ...
        user_context = await create_test_user_context(
            user_id=self.test_user_id,
            request_id=f"llm_tracking_{uuid.uuid4().hex[:8]}",
            thread_id=self.test_thread_id,
            real_services=real_services_fixture
        )
        
        # Create agent with LLM tracking enabled
        agent = TestAgent(
            llm_manager=self.llm_manager,
            name="LLMTrackingAgent",
            user_context=user_context
        )
        
        # Execute multiple messages to test cumulative tracking
        test_messages = [
            "What are the best practices for cloud cost optimization?",
            "How can I automate my deployment pipeline?",
            "Analyze my infrastructure for security vulnerabilities"
        ]
        ...
        # Verify each execution has unique metadata
    ...
    async def test_agent_websocket_event_emission_integration(self, real_services_fixture):
        ...
        """
        # Create user context with WebSocket support
        user_context = await create_test_user_context(
            user_id=self.test_user_id,
            request_id=f"websocket_test_{uuid.uuid4().hex[:8]}",
            thread_id=self.test_thread_id,
            real_services=real_services_fixture
        )
        
        # Set up WebSocket client for event monitoring
        async with self.ws_utility.connected_client(user_id=self.test_user_id) as client:
            ...
            # Create and set WebSocket bridge
            ws_bridge = AgentWebSocketBridge(
                user_context=user_context,
                websocket_manager=None  # Will be set by execution context
            )
            agent.set_websocket_bridge(ws_bridge)
            
            # Execute agent and monitor WebSocket events
            start_time = time.time()
            execution_task = asyncio.create_task(
                agent.execute(
                    user_message="Provide detailed cloud optimization recommendations",
                    context={"enable_websocket_events": True}
                )
            )
            ...
            # Monitor events while execution is running
            ...
            if received_events:
                ...
                # WebSocket events may not be available in all test environments
    ...
    async def test_agent_session_isolation_validation(self, real_services_fixture):
        ...
        for i in range(num_sessions):
            ...
            user_context = await create_test_user_context(
                user_id=user_id,
                request_id=f"session_req_{i}_{uuid.uuid4().hex[:8]}",
                thread_id=f"session_thread_{i}_{uuid.uuid4().hex[:8]}",
                real_services=real_services_fixture,
        ...
        # Execute concurrent agent sessions with session-specific data
        concurrent_tasks = []
        for i, session in enumerate(session_data):
            task = asyncio.create_task(
                self._execute_session_isolated_agent(
                    session["agent"],
                    session["user_context"],
                    f"Session {i} request: optimize costs for session {session['session_id']}"
                )
            )
        ...
        for i, result in enumerate(session_results):
            ...
            # Verify session-specific data integrity
        ...
        # Validate session isolation at agent level
        for i, session in enumerate(session_data):
            agent = session["agent"]
            assert len(agent.message_processing_results) == 1, f"Agent {i} session state corrupted"
            
            # Verify agent maintains correct session context
    ...
    async def _execute_session_isolated_agent(self, agent: TestAgent, user_context: UserExecutionContext, message: str) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "session_id": getattr(user_context, 'session_id', 'unknown'),
...
if __name__ == "__main__":
    ...
    python -m pytest tests/integration/test_base_agent_message_integration.py -v --real-services
    
    Run specific test:
    python -m pytest tests/integration/test_base_agent_message_integration.py::TestBaseAgentMessageIntegration::test_agent_execute_with_user_context_isolation -v --real-services
    """
