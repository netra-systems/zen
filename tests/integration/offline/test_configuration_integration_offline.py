...
class TestConfigurationIntegrationOffline(SSotBaseTestCase):
    ...
    async def test_configuration_loading_precedence_integration(self):
        ...
        override_count, override_errors = self.get_env().load_from_file(override_config_path, override_existing=False)
        assert override_count > 0, f'Failed to load override config: {override_errors}'
        assert self.get_env_var('DATABASE_NAME') == 'base_db'
        assert self.get_env_var('SECRET_KEY') == 'base_secret_key'
        assert self.get_env_var('NEW_CONFIG_VALUE') == 'override_value'
        override_count2, override_errors2 = self.get_env().load_from_file(override_config_path, override_existing=True)
        assert self.get_env_var('DATABASE_NAME') == 'override_db'
        assert self.get_env_var('SECRET_KEY') == 'override_secret_key'
        assert self.get_env_var('AUTH_SERVICE_URL') == 'http://override-auth:8081'
    ...
    async def test_environment_detection_integration(self):
        ...
        """
        environment_scenarios = [{'name': 'development_explicit', 'config': {'ENVIRONMENT': 'development', 'DEBUG': 'true', 'DATABASE_URL': 'postgresql://dev:dev@localhost:5432/dev_db', 'AUTH_SERVICE_URL': 'http://localhost:8081'}, 'expected_env': 'development', 'expected_debug': True}, {'name': 'test_normalized', 'config': {'ENVIRONMENT': 'testing', 'DEBUG': 'true', 'DATABASE_URL': 'postgresql://test:test@localhost:5434/test_db', 'AUTH_SERVICE_URL': 'http://test-auth:8081'}, 'expected_env': 'test', 'expected_debug': True}, {'name': 'staging_secure', 'config': {'ENVIRONMENT': 'staging', 'DEBUG': 'false', 'DATABASE_URL': 'postgresql://staging_user:secure_pass@staging-db:5432/staging_db', 'AUTH_SERVICE_URL': 'https://auth.staging.example.com'}, 'expected_env': 'staging', 'expected_debug': False}, {'name': 'production_locked', 'config': {'ENVIRONMENT': 'production', 'DEBUG': 'false', 'DATABASE_URL': 'postgresql://prod_user:ultra_secure@prod-db:5432/prod_db', 'AUTH_SERVICE_URL': 'https://auth.example.com'}, 'expected_env': 'production', 'expected_debug': False}]
        for scenario in environment_scenarios:
            if scenario['name'] != 'development_explicit':
            ...
            assert detected_env == scenario['expected_env'], f"Environment detection failed for {scenario['name']}: expected '{scenario['expected_env']}', got '{detected_env}'"
            ...
            assert actual_debug == expected_debug, f"Debug setting mismatch for {scenario['name']}: expected {expected_debug}, got {actual_debug}"
    ...
    async def test_configuration_validation_integration(self):
        ...
        """
        minimal_valid_config = {'DATABASE_URL': 'postgresql://user:pass@localhost:5432/db', 'JWT_SECRET_KEY': 'valid_jwt_secret_at_least_32_chars', 'SECRET_KEY': 'valid_secret_key_at_least_32_chars'}
        ...
        assert validation_result.is_valid, f'Minimal config validation failed: {validation_result.errors}'
        assert len(validation_result.errors) == 0
        missing_var_scenarios = [{'missing_var': 'DATABASE_URL', 'expected_error_pattern': 'DATABASE_URL'}, {'missing_var': 'JWT_SECRET_KEY', 'expected_error_pattern': 'JWT_SECRET_KEY'}, {'missing_var': 'SECRET_KEY', 'expected_error_pattern': 'SECRET_KEY'}]
        for scenario in missing_var_scenarios:
            ...
            validation_result = self.get_env().validate_all()
            assert not validation_result.is_valid, f"Validation should fail when {scenario['missing_var']} is missing"
            assert len(validation_result.errors) > 0
            error_found = any((scenario['expected_error_pattern'] in error for error in validation_result.errors))
            assert error_found, f"Expected error pattern '{scenario['expected_error_pattern']}' not found. Errors: {validation_result.errors}"
            if original_value:
                self.set_env_var(scenario['missing_var'], original_value)
        invalid_scenarios = [{'name': 'short_jwt_secret', 'config': {'JWT_SECRET_KEY': 'short'}, 'expected_error_pattern': 'JWT_SECRET_KEY'}, {'name': 'short_secret_key', 'config': {'SECRET_KEY': 'short'}, 'expected_error_pattern': 'SECRET_KEY'}, {'name': 'invalid_database_url', 'config': {'DATABASE_URL': 'not_a_url'}, 'expected_error_pattern': 'DATABASE_URL'}]
        ...
        self.record_metric('validation_scenarios_tested', 1 + len(missing_var_scenarios) + len(invalid_scenarios))
    ...
    async def test_cross_service_configuration_consistency(self):
        ...
        """
        multi_service_config = {'ENVIRONMENT': 'test', 'LOG_LEVEL': 'INFO', 'SECRET_KEY': 'shared_secret_key_for_all_services', 'DATABASE_URL': 'postgresql://test_user:test_pass@localhost:5434/test_db', 'DATABASE_POOL_SIZE': '10', 'AUTH_SERVICE_URL': 'http://localhost:8081', 'JWT_SECRET_KEY': 'jwt_secret_key_shared_across_services', 'JWT_EXPIRY_HOURS': '24', 'BACKEND_HOST': 'localhost', 'BACKEND_PORT': '8000', 'CORS_ORIGINS': 'http://localhost:3000,http://localhost:3001', 'REDIS_URL': 'redis://localhost:6381', 'REDIS_TIMEOUT': '5', 'ANALYTICS_ENABLED': 'true', 'METRICS_COLLECTION_INTERVAL': '60'}
        ...
        for key, expected_value in multi_service_config.items():
            ...
            assert actual_value == expected_value, f"Configuration inconsistency: {key} expected '{expected_value}', got '{actual_value}'"
        ...
        assert self.get_env().is_test()
        validation_result = self.get_env().validate_all()
        assert validation_result.is_valid, f'Multi-service configuration validation failed: {validation_result.errors}'
        ...
        for key, expected_value in multi_service_config.items():
            ...
            assert subprocess_env[key] == expected_value, f"Subprocess config inconsistency: {key} expected '{expected_value}', got '{subprocess_env[key]}'"
    ...
    async def test_configuration_error_handling_integration(self):
        ...
        for i in range(100):
            ...
            var_value = f"large_config_value_{i}_{'x' * 100}"
        ...
        validation_result = self.get_env().validate_all()
        assert validation_result.is_valid, f'Validation should pass after recovery: {validation_result.errors}'
    ...
    async def test_configuration_performance_integration(self):
        ...
        assert len(all_vars) >= len(bulk_config_vars), f'get_all() should return at least {len(bulk_config_vars)} vars, got {len(all_vars)}'
        ...
        assert read_ops_per_sec >= 500, f'Read performance too slow: {read_ops_per_sec:.0f} ops/sec (expected >= 500)'
