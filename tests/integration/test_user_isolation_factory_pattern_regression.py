...
class UserIsolationFactoryPatternRegressionTests(SSotBaseTestCase):
    ...
    def setup_method(self, method=None):
        ...
        self.test_users = ['usr_factory_isolation_user001_12345abcdef890', 'usr_factory_isolation_user002_67890fedcba543', 'usr_factory_isolation_user003_abcdef1234567890', 'usr_factory_isolation_user004_fedcba0987654321', 'usr_factory_isolation_user005_543210987654abcd']
        self.conversation_threads = {'enterprise_support': 'thd_factory_enterprise_support_001234', 'data_analysis': 'thd_factory_data_analysis_567890', 'report_generation': 'thd_factory_report_generation_abcdef', 'user_onboarding': 'thd_factory_user_onboarding_fedcba', 'system_integration': 'thd_factory_system_integration_123abc'}
    ...
    def test_factory_pattern_creates_properly_isolated_user_contexts(self):
        ...
        for i, user_id in enumerate(self.test_users[:3]):
            ...
            context = UserContextFactory.create_context(user_id=user_id, thread_id=thread_id, run_id=None, websocket_client_id=f'ws_{user_id[-8:]}')
    ...
    def test_multiple_users_get_completely_separate_context_instances(self):
        ...
        def create_user_context_safely(user_id: str, thread_id: str) -> Dict[str, Any]:
            ...
            try:
                start_time = time.time()
                context = UserContextFactory.create_context(user_id=user_id, thread_id=thread_id, run_id=None)
                creation_time = time.time() - start_time
                return {'success': True, 'context': context, 'creation_time': creation_time, 'thread_id': threading.current_thread().ident, 'user_id': user_id}
            except Exception as e:
        ...
        assert success_count == len(self.test_users), f'Factory concurrent operations failed: {success_count}/{len(self.test_users)} succeeded'
    ...
    def test_factory_methods_use_get_user_execution_context_correctly(self):
        ...
        session_test_results['first_message'] = {'context': context1, 'run_id': original_run_id, 'creation_time': first_creation_time}
        start_time = time.time()
        ...
        assert context2.run_id == original_run_id, f'CRITICAL FACTORY REGRESSION: Session not reused! Expected run_id={original_run_id}, got run_id={context2.run_id}. This indicates factory is using create instead of get pattern!'
        self.metrics.session_reuse_count += 1
        session_test_results['second_message'] = {'context': context2, 'run_id': context2.run_id, 'creation_time': second_creation_time, 'session_reused': context2.run_id == original_run_id}
        start_time = time.time()
        context3 = get_user_execution_context(user_id=user_id, thread_id=thread_id)
        third_creation_time = time.time() - start_time
        assert context3.run_id == original_run_id, 'Factory pattern failed to maintain session continuity across multiple messages'
        self.metrics.session_reuse_count += 1
        avg_reuse_time = (second_creation_time + third_creation_time) / 2
        if avg_reuse_time > first_creation_time * 2:
            logger.warning(f'Session reuse performance concern: reuse={avg_reuse_time:.4f}s, initial={first_creation_time:.4f}s')
        anti_pattern_context = create_user_execution_context(user_id=user_id, thread_id=thread_id)
        assert anti_pattern_context.run_id != original_run_id, 'create_user_execution_context should ALWAYS create new context, not reuse'
    ...
    def test_factory_operations_no_memory_leaks_excessive_context_creation(self):
        ...
        max_allowed_growth = 200
        if memory_growth > max_allowed_growth:
            self.metrics.memory_leak_detected = True
            pytest.fail(f'MEMORY LEAK DETECTED: {memory_growth:.1f}MB growth ({memory_per_context:.3f}MB per context) exceeds threshold {max_allowed_growth}MB')
        self._validate_no_excessive_context_creation()
        avg_creation_time = sum(self.metrics.context_creation_times) / len(self.metrics.context_creation_times) if self.metrics.context_creation_times else 0
        max_allowed_time = 0.05
        if avg_creation_time > max_allowed_time:
            logger.warning(f'Factory performance concern: {avg_creation_time:.4f}s average creation time')
        ...
        logger.info(f'Factory memory leak test completed: {contexts_created} contexts, {memory_growth:.1f}MB growth, {avg_creation_time:.4f}s avg time')
    ...
    def test_factory_pattern_maintains_performance_standards_under_load(self):
        ...
        def execute_user_operations(user_index: int) -> Dict[str, Any]:
            ...
            for op_index in range(operations_per_user):
                ...
                try:
                    ...
                    operation_results.append({'success': True, 'duration': op_duration, 'context_id': context.request_id, 'thread_id': thread_id})
                except Exception as e:
                    operation_results.append({'success': False, 'error': str(e), 'duration': time.time() - op_start})
            user_total_time = time.time() - user_start_time
            success_count = sum((1 for r in operation_results if r['success']))
            return {'user_index': user_index, 'user_id': user_id, 'total_time': user_total_time, 'operations_succeeded': success_count, 'operations_failed': operations_per_user - success_count, 'operation_results': operation_results}
        with ThreadPoolExecutor(max_workers=concurrent_users) as executor:
            futures = [executor.submit(execute_user_operations, i) for i in range(concurrent_users)]
            for future in as_completed(futures):
                result = future.result()
        ...
        logger.info(f'Factory load test completed: {total_successes}/{total_operations} operations succeeded, {throughput:.2f} ops/sec, {load_test_duration:.2f}s duration')
    ...
    def test_factory_error_handling_preserves_user_isolation_boundaries(self):
        ...
        isolation_validation_results = {}
        try:
            invalid_context = UserContextFactory.create_context(user_id='', thread_id=self.conversation_threads['enterprise_support'], run_id=None)
            error_scenarios.append({'scenario': 'invalid_user_id', 'success': False, 'error': 'Expected validation error did not occur'})
        except Exception as e:
            error_scenarios.append({'scenario': 'invalid_user_id', 'success': True, 'error': str(e)})
            self.metrics.factory_error_count += 1
        try:
            invalid_thread_context = UserContextFactory.create_context(user_id=self.test_users[0], thread_id='', run_id=None)
            error_scenarios.append({'scenario': 'invalid_thread_id', 'success': False, 'error': 'Expected validation error did not occur'})
        except Exception as e:
            error_scenarios.append({'scenario': 'invalid_thread_id', 'success': True, 'error': str(e)})
            self.metrics.factory_error_count += 1
        valid_contexts = []
        for i, user_id in enumerate(self.test_users[:3]):
            try:
                context = UserContextFactory.create_context(user_id=user_id, thread_id=list(self.conversation_threads.values())[i], run_id=None)
                valid_contexts.append(context)
                self.created_contexts.append(context)
                isolation_result = self._validate_context_isolation(context, user_id)
                isolation_validation_results[user_id] = isolation_result
                error_scenarios.append({'scenario': f'valid_context_after_errors_{i + 1}', 'success': True, 'error': None})
            except Exception as e:
                error_scenarios.append({'scenario': f'valid_context_after_errors_{i + 1}', 'success': False, 'error': str(e)})
                self.metrics.factory_error_count += 1
        self._validate_error_handling_isolation_preservation(valid_contexts)
        self._validate_factory_recovery_after_errors()
        for context in valid_contexts:
            assert context.verify_isolation(), f'Context isolation compromised after error conditions for user {context.user_id}'
        ...
        logger.info(f'Factory error handling test completed: {successful_error_handling}/{total_scenarios} scenarios handled correctly, {self.metrics.factory_error_count} errors processed')
    ...
    def _validate_context_isolation(self, context: UserExecutionContext, expected_user_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            return {'is_isolated': isolation_verified, 'user_id_correct': context.user_id == expected_user_id, 'has_required_ids': all([context.thread_id, context.run_id, context.request_id])}
        except Exception as e:
            logger.error(f'Context isolation validation failed: {e}')
            return {'is_isolated': False, 'error': str(e)}
        ...
        for i, context1 in enumerate(self.created_contexts[:-1]):
            for j, context2 in enumerate(self.created_contexts[i + 1:], i + 1):
                if id(context1.agent_context) == id(context2.agent_context):
                    pytest.fail(f'ISOLATION VIOLATION: Contexts {i} and {j} share agent_context reference')
                if id(context1.audit_metadata) == id(context2.audit_metadata):
                    pytest.fail(f'ISOLATION VIOLATION: Contexts {i} and {j} share audit_metadata reference')
    ...
    def _validate_unique_context_identifiers(self) -> None:
        ...
        if len(unique_request_ids) != len(request_ids):
            pytest.fail(f'ISOLATION VIOLATION: Non-unique request IDs detected. Expected {len(request_ids)} unique, got {len(unique_request_ids)}')
    ...
    def _validate_complete_user_separation(self, concurrent_results: Dict[str, Any]) -> None:
        ...
        assert len(user_ids) == len(request_ids), 'User separation validation failed - contexts not properly isolated'
    ...
    def _validate_no_excessive_context_creation(self) -> None:
        ...
        if self.metrics.session_creation_count > self.metrics.session_reuse_count * 2:
            logger.warning(f'Possible excessive context creation: {self.metrics.session_creation_count} created, {self.metrics.session_reuse_count} reused')
    ...
    def _validate_load_test_performance_standards(self, results: Dict[str, Any], duration: float) -> None:
        ...
        if throughput < min_throughput:
            pytest.fail(f'PERFORMANCE REGRESSION: Throughput {throughput:.2f} ops/sec below minimum {min_throughput} ops/sec')
    ...
    def _validate_no_performance_degradation_under_load(self) -> None:
        ...
        if len(self.metrics.concurrent_operation_times) > 50:
            ...
            if degradation_ratio > 2.0:
                logger.warning(f'Performance degradation detected: {degradation_ratio:.2f}x slower ({early_avg:.4f}s -> {late_avg:.4f}s)')
    ...
    def _validate_factory_recovery_after_errors(self) -> None:
        ...
        try:
            recovery_context = UserContextFactory.create_context(user_id=self.test_users[0], thread_id='thd_recovery_test_after_errors', run_id=None)
