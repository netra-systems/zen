    ...
    def validate_required_events(self) -> bool:
        ...
        for event_type, config in self.REQUIRED_EVENTS.items():
            if config['critical'] and event_type not in received_types:
                self.validation_errors.append(fMissing critical event: {event_type} - {config['description']})
...
async def test_websocket_concurrent_messages():
    ...
    assert result['success'], fConcurrent messages failed: {result['successful']}/{result['total']}
...
async def test_websocket_deduplication():
    ...
    assert result['success'], fDeduplication failed: {result['agent_started_count']} agent_started events
