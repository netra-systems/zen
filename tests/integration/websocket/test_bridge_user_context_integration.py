...
class WebSocketBridgeUserContextIntegrationTests(SSotAsyncTestCase):
    ...
    async def asyncSetUp(self):
        ...
        self.user_context_1 = UserExecutionContext(user_id=self.user_id_1, thread_id=self.thread_id, run_id=self.run_id, request_id=f'req_{uuid.uuid4()}', websocket_client_id=f'ws_client_1_{uuid.uuid4()}', agent_context={'test_scenario': 'bridge_integration', 'user_role': 'primary_user'}, audit_metadata={'test_case': 'websocket_bridge_integration', 'created_at': '2025-01-09T12:00:00Z'})
        self.user_context_2 = UserExecutionContext(user_id=self.user_id_2, thread_id=ThreadID(str(uuid.uuid4())), run_id=RunID(str(uuid.uuid4())), request_id=f'req_{uuid.uuid4()}', websocket_client_id=f'ws_client_2_{uuid.uuid4()}', agent_context={'test_scenario': 'bridge_integration', 'user_role': 'secondary_user'}, audit_metadata={'test_case': 'websocket_bridge_integration', 'created_at': '2025-01-09T12:00:00Z'})
    ...
    async def test_websocket_bridge_creates_with_user_context(self):
        ...
        assert str(bridge_user_id) == str(self.user_context_1.user_id), f'Bridge should be associated with correct user ID. Expected: {self.user_context_1.user_id}, Got: {bridge_user_id}'
    ...
    async def test_websocket_bridge_user_isolation(self):
        ...
        with patch('netra_backend.app.websocket_core.websocket_manager.get_websocket_manager') as mock_ws_manager:
            ...
            mock_manager.get_connection_by_client_id.side_effect = lambda client_id: {self.user_context_1.websocket_client_id: mock_connection_1, self.user_context_2.websocket_client_id: mock_connection_2}.get(client_id)
    ...
    async def test_websocket_bridge_agent_event_coordination(self):
        ...
        with patch('netra_backend.app.websocket_core.websocket_manager.get_websocket_manager') as mock_ws_manager:
            ...
            mock_ws_manager.return_value = mock_manager
            agent_events = [('agent_started', {'agent_type': 'supervisor', 'user_id': str(self.user_context_1.user_id), 'thread_id': str(self.user_context_1.thread_id), 'timestamp': '2025-01-09T12:00:00Z'}), ('agent_thinking', {'reasoning': 'Analyzing user request for optimal response', 'step': 1, 'total_steps': 3}), ('tool_executing', {'tool_name': 'data_analyzer', 'tool_input': {'query': 'test query'}, 'execution_id': str(uuid.uuid4())}), ('tool_completed', {'tool_name': 'data_analyzer', 'result': {'status': 'success', 'data': 'analysis complete'}, 'duration': 2.5}), ('agent_completed', {'result': 'Agent execution completed successfully', 'total_duration': 5.2, 'status': 'success'})]
            ...
            for required_event in required_events:
                assert any((required_event in event_type for event_type in event_types)), f'Missing required event: {required_event}. Events sent: {event_types}'
            for event in sent_events:
                if 'data' in event and isinstance(event['data'], dict):
                    event_user_id = event['data'].get('user_id')
                    if event_user_id:
                        assert event_user_id == str(self.user_context_1.user_id), f'Event should include correct user ID. Expected: {self.user_context_1.user_id}, Got: {event_user_id}'
    ...
    async def test_websocket_manager_factory_integration(self):
        ...
        try:
            ...
            if hasattr(websocket_manager, 'user_context'):
                ...
                assert str(manager_user_id) == str(self.user_context_1.user_id), f'Manager should be associated with correct user'
        except ImportError:
    ...
    async def test_websocket_bridge_error_handling_with_user_context(self):
        ...
        with patch('netra_backend.app.websocket_core.websocket_manager.get_websocket_manager') as mock_ws_manager:
            ...
            try:
                if hasattr(websocket_bridge, 'send_event'):
                    await websocket_bridge.send_event('agent_started', {'user_id': str(self.user_context_1.user_id), 'test': 'error_handling'})
                elif hasattr(websocket_bridge, 'emit_event'):
                    await websocket_bridge.emit_event('agent_started', {'user_id': str(self.user_context_1.user_id), 'test': 'error_handling'})
            ...
            if hasattr(websocket_bridge, 'user_context'):
                ...
                assert str(context_user_id) == str(self.user_context_1.user_id), 'User context should remain intact despite WebSocket errors'
            bridge_2 = create_agent_websocket_bridge(self.user_context_2)
            if hasattr(bridge_2, 'user_context'):
                context_2_user_id = getattr(bridge_2.user_context, 'user_id', None)
                assert str(context_2_user_id) == str(self.user_context_2.user_id), "Second user's bridge should be unaffected by first user's errors"
    ...
    async def test_websocket_bridge_concurrent_user_handling(self):
        ...
        with patch('netra_backend.app.websocket_core.websocket_manager.get_websocket_manager') as mock_ws_manager:
            ...
            async def send_user_events(user_index, bridge, user_context):
                ...
                events = [('agent_started', {'user_index': user_index, 'message': f'User {user_index} started'}), ('agent_thinking', {'user_index': user_index, 'message': f'User {user_index} thinking'}), ('agent_completed', {'user_index': user_index, 'message': f'User {user_index} completed'})]
                    ...
                    for message in messages:
                        try:
                            ...
                            if message_user_index is not None:
                                assert message_user_index == user_index, f'User {user_index} received message intended for user {message_user_index}. This indicates user isolation is broken.'
...
pytestmark = [pytest.mark.integration, pytest.mark.websocket_bridge, pytest.mark.user_context, pytest.mark.user_isolation, pytest.mark.issue_409, pytest.mark.issue_426_cluster, pytest.mark.no_docker_required, pytest.mark.real_services_not_required]
