...
class TestAgentDeathRecoveryIntegration:
    ...
    @pytest.mark.asyncio
    async def test_agent_death_detected_with_websocket_notification(
...
" + "="*80)
        ...
        print(f"✅ Progress notifications sent: {len(progress_notifications)}")
...
💀 SIMULATING AGENT DEATH...")
        ...
        for i in range(max_wait_seconds):
            ...
            if death_notifications or failure_notifications:
                death_detected = True
                print(f"💀 Death detected after {i+1} seconds")
                break
            ...
            if status and status.execution_record.state in [ExecutionState.FAILED, ExecutionState.TIMEOUT]:
                death_detected = True
                print(f"💀 Death detected via execution status after {i+1} seconds")
                break
        ...
        print("✅ INTEGRATION TEST PASSED: Agent death detected with proper notifications")
        print("="*80)
...
" + "="*80)
        ...
        for i, scenario in enumerate(scenarios):
            ...
            context = AgentExecutionContext(
                run_id=f"multi-death-{scenario['name']}",
            ...
            print(f"✅ Started {scenario['name']}: {exec_id}")
...
💀 Killing agents with different scenarios...")
            ...
            for j, exec_id in enumerate(execution_ids):
                if not deaths_detected[j]:
                    ...
                    if status and status.execution_record.state in [
                        ...
                        print(f"💀 Death {j+1} detected after {i+1} seconds: {scenarios[j]['name']}")
        ...
        for j, detected in enumerate(deaths_detected):
            assert detected, f"Agent {j} death ({scenarios[j]['name']}) was not detected"
        ...
        assert total_notifications >= 3, f"Expected at least 3 death notifications, got {total_notifications}"
        
        print(f"✅ All agent deaths detected and notified: {total_notifications} notifications")
        print("✅ MULTI-AGENT DEATH TEST PASSED")
        print("="*80)
...
" + "="*80)
    ...
    pass
        ...
        # Agent works briefly
        await simulator.do_work_phases([
            {'stage': 'initial', 'percentage': 25, 'message': 'Starting work...', 'duration': 0.5}
        ])
        
        print(f"✅ Agent started working: {execution_id}")
        
        # Simulate death
        await simulator.die_silently()
        print("💀 Agent died silently")
...
🔄 Attempting recovery...")
        ...
        await recovery_simulator.do_work_phases([
...
" + "="*80)
        ...
        print(f"   Active executions: {initial_health['active_executions']}")
        print(f"   Dead agents: {initial_health['dead_agents']}")
...
💀 Health after agent deaths:")
        ...
        print(f"   Timed out agents: {death_health['timed_out_agents']}")
...
" + "="*80)
        ...
        print(f"   Total executions: {initial_metrics['tracker_metrics']['total_executions_started']}")
        print(f"   Successful: {initial_metrics['tracker_metrics']['successful_executions']}")
        print(f"   Failed: {initial_metrics['tracker_metrics']['failed_executions']}")
        ...
        for scenario in scenarios:
            ...
            context = AgentExecutionContext(
                run_id=f"metrics-{scenario['name']}",
                agent_name=f"metrics-agent-{scenario['name']}",
                thread_id=f"metrics-thread-{scenario['name']}",
        ...
        print(f"✅ Started {len(scenarios)} executions")
...
" + "="*80)
        ...
        print(f"✅ All {num_agents} agents started")
...
💀 Killing {kill_count} agents simultaneously...")
        ...
        print(f"💀 {kill_count} agents killed simultaneously")
        
        # Let remaining agents complete
        for i in range(kill_count, num_agents):
            await simulators[i].complete_successfully({"concurrent_test": f"agent_{i}_success"})
...
🔍 Waiting for death detection...")
        ...
        for wait_second in range(max_wait_seconds):
            ...
            if current_deaths > deaths_detected:
                ...
                print(f"💀 Deaths detected: {deaths_detected}/{kill_count} (after {wait_second+1}s)")
