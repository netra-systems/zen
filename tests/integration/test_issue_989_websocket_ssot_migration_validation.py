...
class TestIssue989WebSocketSSOTMigrationValidation(SSotBaseTestCase):
    ...
    def setup_method(self, method):
        ...
        # Critical paths for migration validation
        ...
        # Search paths for comprehensive import analysis
        self.search_paths = [
        ...
        # SSOT target patterns (what should remain post-migration)
        self.ssot_target_patterns = [
            'get_websocket_manager',           # SSOT function from websocket_manager.py
            'WebSocketManager',                # Direct class usage
            'from netra_backend.app.websocket_core.websocket_manager import'
        ]

        # Deprecated patterns (should be eliminated post-migration)
        self.deprecated_patterns = [
            'get_websocket_manager_factory',   # PRIMARY VIOLATION
            'WebSocketManagerFactory',         # Factory class pattern
            'from netra_backend.app.websocket_core.websocket_manager_factory import',
            'canonical_imports.*get_websocket_manager_factory'
        ]
    ...
    def test_validate_canonical_imports_deprecated_export_removal(self):
        ...
        validation_result = MigrationValidationResult(
            validation_name='canonical_imports_export_removal',
            pre_migration_violations=1,  # Known issue: get_websocket_manager_factory exported
        ...
        if self.canonical_imports_path.exists():
            try:
                ...
                # Parse AST to extract __all__ exports
                ...
                for deprecated_pattern in self.deprecated_patterns:
                    if deprecated_pattern in current_exports:
                        ...
                        validation_result.remaining_issues.append(
                            f"Still exports deprecated: {deprecated_pattern}"
                        )
        ...
        # ASSERTION: Migration successful if no deprecated exports remain
        assert validation_result.migration_success, (
            f"SSOT MIGRATION INCOMPLETE: Found {validation_result.post_migration_violations} deprecated exports "
            f"still present in canonical_imports.py. "
            f"Remaining issues: {validation_result.remaining_issues}. "
            f"Migration success requires removal of ALL deprecated exports."
        )
    ...
    def test_validate_codebase_import_pattern_consolidation(self):
        ...
        import_analysis = ImportPatternAnalysis(
            total_files_analyzed=0,
            ssot_compliant_files=0,
            deprecated_pattern_files=0,
            mixed_pattern_files=0
        )

        validation_result = MigrationValidationResult(
            validation_name='codebase_import_consolidation',
            pre_migration_violations=0,  # Will be calculated from analysis
            post_migration_violations=0,
            migration_success=False
        )
        ...
        for search_path in self.search_paths:
            if search_path.exists():
                for py_file in search_path.rglob("*.py"):
                    # Skip the deprecated factory file itself
                    ...
                    try:
                        ...
                        # Only analyze files with WebSocket imports
                        ...
                        if has_deprecated_imports and has_ssot_imports:
                            # Mixed patterns - migration incomplete
                            mixed_pattern_files.append({
                                'file': file_rel_path,
                                'deprecated_patterns': deprecated_matches,
                                'ssot_patterns': ssot_matches
                            })
                            import_analysis.mixed_pattern_files += 1
                            logger.warning(f"⚠️ MIXED PATTERNS: {file_rel_path}")

                        elif has_deprecated_imports:
                            # Only deprecated patterns - needs migration
                            deprecated_import_files.append({
                                'file': file_rel_path,
                                'deprecated_patterns': deprecated_matches
                            })
                            ...
                            # Only SSOT patterns - migration complete for this file
        ...
        if import_analysis.total_files_analyzed > 0:
            validation_result.compliance_percentage = (
                import_analysis.ssot_compliant_files / import_analysis.total_files_analyzed
            ) * 100
        ...
        # Record issues for remediation
        for file_info in deprecated_import_files[:5]:  # Limit to first 5 for readability
            validation_result.remaining_issues.append(
                f"Deprecated imports in {file_info['file']}: {file_info['deprecated_patterns']}"
            )

        for file_info in mixed_pattern_files[:5]:
            validation_result.remaining_issues.append(
                f"Mixed patterns in {file_info['file']}"
            )

        self.migration_results.append(validation_result)

        # ASSERTION: Migration successful if no deprecated imports remain
        assert validation_result.migration_success, (
            f"SSOT MIGRATION INCOMPLETE: Found {validation_result.post_migration_violations} files "
            f"with deprecated import patterns. "
            f"Deprecated pattern files: {import_analysis.deprecated_pattern_files}, "
            f"Mixed pattern files: {import_analysis.mixed_pattern_files}. "
            f"SSOT compliance: {validation_result.compliance_percentage:.1f}%. "
            f"Migration requires 100% SSOT compliance."
        )
    ...
    def test_validate_single_websocket_initialization_pattern_enforcement(self):
        ...
        validation_result = MigrationValidationResult(
            validation_name='single_initialization_pattern',
            pre_migration_violations=2,  # Multiple patterns before migration
            post_migration_violations=0,
            migration_success=False
        )

        # Track different initialization patterns found
        initialization_patterns = defaultdict(list)
        pattern_violations = []

        import re

        # Patterns to detect different initialization approaches
        init_patterns = {
            'ssot_get_websocket_manager': r'get_websocket_manager\(',
            'direct_websocket_manager': r'WebSocketManager\(',
            'deprecated_factory_func': r'get_websocket_manager_factory\(',
            'deprecated_factory_class': r'WebSocketManagerFactory\(',
            'compatibility_create': r'create_websocket_manager\('
        }

        for search_path in self.search_paths:
            if search_path.exists():
                for py_file in search_path.rglob("*.py"):
                    # Skip factory and manager files themselves
                    ...
                    try:
                        ...
                        # Only analyze files with WebSocket initialization
                        ...
                        # Check for each initialization pattern
                        ...
                        # If file uses multiple patterns, it's a violation
        ...
        # Check for deprecated patterns still in use
        ...
        for violation in pattern_violations[:5]:
            validation_result.remaining_issues.append(
                f"Multiple patterns in {violation['file']}: {violation['patterns']}"
            )

        if deprecated_patterns_in_use:
            validation_result.remaining_issues.append(
                f"Deprecated patterns still used in {len(deprecated_patterns_in_use)} files"
            )

        self.migration_results.append(validation_result)

        # ASSERTION: Migration successful if single pattern enforced
        assert validation_result.migration_success, (
            f"SINGLE PATTERN ENFORCEMENT FAILED: Found {validation_result.post_migration_violations} "
            f"pattern violations. "
            f"Multiple pattern files: {len(pattern_violations)}, "
            f"Deprecated pattern files: {len(deprecated_patterns_in_use)}. "
            f"SSOT requires single initialization pattern only."
        )
    ...
    def test_validate_websocket_import_path_consistency_post_migration(self):
        ...
        validation_result = MigrationValidationResult(
            validation_name='import_path_consistency',
            pre_migration_violations=0,  # Will calculate from analysis
            post_migration_violations=0,
            migration_success=False
        )
        ...
        canonical_import_paths = {
            'websocket_manager': 'from netra_backend.app.websocket_core.websocket_manager import',
            'websocket_protocols': 'from netra_backend.app.websocket_core.protocols import',
            'websocket_unified': 'from netra_backend.app.websocket_core.unified_manager import'
        }

        # Define deprecated import paths (should not exist post-migration)
        deprecated_import_paths = {
            'factory_imports': 'from netra_backend.app.websocket_core.websocket_manager_factory import',
            'canonical_factory': 'from netra_backend.app.websocket_core.canonical_imports import.*get_websocket_manager_factory'
        }
                            ...
                            if 'websocket' in line.lower() and ('import' in line or 'from' in line):
                                ...
                                for path_name, path_pattern in canonical_import_paths.items():
                                    if re.search(path_pattern, line, re.IGNORECASE):
                                        ...
                                        import_paths[path_name].append({
                                            'file': file_rel_path,
                                            'line_num': line_num,
                                            'line': line.strip()
                                        })

                                # Check for deprecated paths (violations)
                                for path_name, path_pattern in deprecated_import_paths.items():
                                    if re.search(path_pattern, line, re.IGNORECASE):
                                        validation_result.post_migration_violations += 1
                                        validation_result.remaining_issues.append(
                                            f"Deprecated import in {file_rel_path}:{line_num}: {line.strip()}"
                                        )

                        # Check for inconsistent import patterns within same file
                        unique_paths = set(file_import_paths)
                        if len(unique_paths) > 1:
                            inconsistent_files.append({
                                'file': file_rel_path,
                                'import_paths': list(unique_paths)
                            })
        ...
        canonical_usages = sum(
            len(import_paths[path]) for path in canonical_import_paths.keys()
        ...
        for file_info in inconsistent_files[:5]:
            validation_result.remaining_issues.append(
                f"Inconsistent imports in {file_info['file']}: {file_info['import_paths']}"
            )

        self.migration_results.append(validation_result)

        # ASSERTION: Migration successful if import paths are consistent
        assert validation_result.migration_success, (
            f"IMPORT PATH CONSISTENCY FAILED: Found {validation_result.post_migration_violations} "
            f"import path violations. "
            f"Inconsistent files: {len(inconsistent_files)}. "
            f"Import path compliance: {validation_result.compliance_percentage:.1f}%. "
            f"SSOT migration requires consistent import paths."
        )
        ...
        if self.migration_results:
            ...
            for result in self.migration_results:
                ...
                if result.remaining_issues:
                    ...
                    for issue in result.remaining_issues[:3]:  # Show first 3 issues
...
if __name__ == "__main__":
    # Run this test directly to validate SSOT migration completion
    logger.info("📋 Running Issue #989 WebSocket SSOT Migration Validation Tests...")
