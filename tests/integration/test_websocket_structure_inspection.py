...
class TestWebSocketStructureInspection:
    ...
    async def test_compare_validator_expectations_vs_reality(self, bridge_with_capture):
        ...
        print(f"tool_executing requires: {validator_tool_executing_schema}")
        ...
        print(f"Event data fields: {list(event_data.keys()) if isinstance(event_data, dict) else 'not_dict'}")
        print(f"Has tool_name: {'tool_name' in event_data}")
        print(f"Has type: {'type' in event_data}")
        ...
        print(f"\n=== VALIDATION TEST ===")
        print(f"Simulated event for validator: {json.dumps(test_event_for_validator, indent=2)}")

        result = validator.validate_event(test_event_for_validator, "test-user-123")

        print(f"Validator result: {result.is_valid}")
