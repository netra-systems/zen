...
class TestAgentWebSocketEventSequenceIntegration(SSotAsyncTestCase):
    ...
    async def test_complete_event_sequence_simple_message(self):
        ...
        with patch.object(UnifiedWebSocketEmitter, 'emit_agent_event') as mock_emit:
            ...
            self.assertTrue(validation['has_all_events'], f"Missing critical events. Got: {validation['sequence_received']}")
            self.assertTrue(validation['correct_order'], f"Incorrect event order. Expected: [agent_started, agent_thinking, tool_executing, tool_completed, agent_completed], Got: {validation['sequence_received']}")
            self.assertLessEqual(validation['total_duration_ms'], 5000, f"Event sequence too slow: {validation['total_duration_ms']}ms")
            self.assertGreaterEqual(validation['event_count'], 5, f"Insufficient events captured: {validation['event_count']}")
