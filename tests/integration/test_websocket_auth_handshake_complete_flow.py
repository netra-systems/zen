...
class WebSocketAuthHandshakeTester:
    ...
    async def perform_oauth_flow(self, username: str, password: str) -> Dict[str, Any]:
        ...
        return {
            access_token: ftest_jwt_token_{username},
...
@pytest.mark.asyncio
class WebSocketAuthHandshakeCompleteFlowTests(BaseIntegrationTest):
...
# COMMENTED OUT: Mock-dependent test -         
        # Validate results
# COMMENTED OUT: Mock-dependent test -         assert result.success, "fConnection failed: {result.error_message}"
