...
class TestSSotClassDuplicationDetection(SSotBaseTestCase):
    ...
    def setup_codebase_scanner(self):
        ...
        self.scan_paths = [self.project_root / 'netra_backend' / 'app', self.project_root / 'shared', self.project_root / 'auth_service', self.project_root / 'frontend']
        self.critical_patterns = ['SessionMetrics', 'UserContext', 'WebSocketManager', 'AuthValidator', 'ConfigManager', 'DatabaseManager']
    ...
    def test_scan_for_duplicate_class_names(self):
        ...
        duplicates = {name: classes for name, classes in self.discovered_classes.items() if len(classes) > 1}
        if duplicates:
            ...
            for class_name, class_defs in duplicates.items():
                ...
                violation = SSotViolation(violation_type='duplicate_class_name', class_name=class_name, occurrences=class_defs, severity=severity, description=f"Class '{class_name}' defined in {len(class_defs)} different locations")
        ...
        if critical_violations:
            ...
            pytest.fail(f'CRITICAL SSOT VIOLATIONS: {len(critical_violations)} duplicate critical classes found: {critical_names}\nThese duplicates cause runtime errors and violate SSOT architectural principles.\nEach concept should have ONE canonical implementation per service.')
    ...
    def test_session_metrics_specific_ssot_violation(self):
        ...
        for class_name, fields in field_comparison.items():
            ...
            if missing_fields or extra_fields:
                field_differences[class_name] = {'missing': missing_fields, 'extra': extra_fields}
        ...
        pytest.fail(f'SSOT VIOLATION CONFIRMED: {len(session_metrics_classes)} SessionMetrics classes with different interfaces.\nField differences found: {len(field_differences)} classes have interface mismatches.\nThis proves the exact bug that causes AttributeError in error handling code.\nSolution: Consolidate into single SessionMetrics class with consistent interface.')
    ...
    def test_critical_class_pattern_violations(self):
        ...
        if critical_violations_found:
            pytest.fail(f'CRITICAL SSOT VIOLATIONS: {len(critical_violations_found)} critical class patterns have duplicates: {critical_violations_found}\nThese violations can cause system instability and runtime errors.\nImmediate action required to consolidate duplicate implementations.')
    ...
    def test_import_path_consistency(self):
        ...
        for class_name, class_defs in self.discovered_classes.items():
            if len(class_defs) > 1:
                ...
                if len(top_level_modules) > 1:
                    problematic_imports.append({'class_name': class_name, 'modules': top_level_modules, 'definitions': class_defs})
        ...
        if problematic_imports:
            pytest.fail(f'IMPORT PATH CONFLICTS: {len(problematic_imports)} classes have conflicting import paths.\nThis creates ambiguity in import statements and can cause wrong classes to be imported.\nResolve by renaming classes or consolidating implementations.')
    ...
    def _analyze_class_node(self, node: ast.ClassDef, file_path: Path) -> ClassDefinition:
        ...
        return ClassDefinition(name=node.name, module_path=module_path, file_path=str(file_path), line_number=node.lineno, fields=fields, methods=methods, base_classes=base_classes, docstring=docstring)
    ...
    def test_generate_ssot_violation_report(self):
        ...
        report_lines = ['# SSOT Class Duplication Violation Report', f"Generated: {(pytest.current_date if hasattr(pytest, 'current_date') else 'Unknown')}", '', f'## Summary', f'- Total violations: {len(self.ssot_violations)}', f"- Critical: {len(by_severity['critical'])}", f"- Warning: {len(by_severity['warning'])}", f"- Info: {len(by_severity['info'])}", '']
        for severity in ['critical', 'warning', 'info']:
            violations = by_severity[severity]
            if violations:
        ...
        if critical_violations:
            pytest.fail(f'CRITICAL SSOT VIOLATIONS DETECTED: {len(critical_violations)} violations require immediate attention.\nSee detailed report above for resolution guidance.')
