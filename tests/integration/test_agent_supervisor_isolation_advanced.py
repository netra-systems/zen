...
class SupervisorIsolationTracker:
    ...
    def record_supervisor_instance(self, user_id: str, supervisor_id: str, metadata: Dict[str, Any]):
        ...
        with self._lock:
            ...
            self.supervisor_instances[user_id][supervisor_id] = {'metadata': metadata.copy(), 'timestamp': datetime.now().isoformat(), 'state_isolated': True}
    ...
    def record_execution_context(self, user_id: str, context_id: str, context_data: Dict[str, Any]):
        ...
        with self._lock:
            ...
            self.execution_contexts[user_id][context_id] = {'data': context_data.copy(), 'timestamp': datetime.now().isoformat()}
    ...
    def record_state_leakage(self, from_user: str, to_user: str, leakage_type: str, details: Dict[str, Any]):
        ...
        with self._lock:
            leakage = {'from_user': from_user, 'to_user': to_user, 'leakage_type': leakage_type, 'details': details.copy(), 'timestamp': datetime.now().isoformat(), 'severity': 'critical'}
            self.state_leakages.append(leakage)

    def record_concurrent_execution(self, user_id: str, execution_id: str, start_time: datetime, end_time: datetime, result: Any):
        """Record concurrent execution results"""
        with self._lock:
            execution = {'execution_id': execution_id, 'user_id': user_id, 'start_time': start_time.isoformat(), 'end_time': end_time.isoformat(), 'duration_ms': int((end_time - start_time).total_seconds() * 1000), 'success': result is not None, 'result_type': type(result).__name__ if result else None}
    ...
    def get_isolation_analysis(self) -> Dict[str, Any]:
        ...
        with self._lock:
            return {'users_tested': list(self.supervisor_instances.keys()), 'total_supervisors': sum((len(supervisors) for supervisors in self.supervisor_instances.values())), 'total_contexts': sum((len(contexts) for contexts in self.execution_contexts.values())), 'state_leakages': len(self.state_leakages), 'concurrent_executions': sum((len(execs) for execs in self.concurrent_executions.values())), 'leakage_details': self.state_leakages.copy(), 'isolation_successful': len(self.state_leakages) == 0}
...
class TestAgentSupervisorIsolation(DockerTestBase):
    ...
    def setup_test_environment(self):
        ...
        self.test_users = [{'user_id': f'isolation_user_{i}_{uuid.uuid4().hex[:8]}', 'thread_id': f'thread_{i}_{uuid.uuid4().hex[:8]}', 'session_id': f'session_{i}_{uuid.uuid4().hex[:8]}'} for i in range(5)]
    ...
    async def _create_isolated_supervisor(self, user_data: Dict[str, Any]) -> Tuple[Any, str]:
        ...
        async for db_session in get_request_scoped_db_session():
            try:
                supervisor = await create_supervisor_core(user_id=user_id, thread_id=thread_id, run_id=str(uuid.uuid4()), db_session=db_session, websocket_connection_id=f'conn_{user_id}')
                supervisor_id = f'supervisor_{user_id}_{int(time.time() * 1000)}'
                self.isolation_tracker.record_supervisor_instance(user_id, supervisor_id, {'supervisor_type': type(supervisor).__name__, 'thread_id': thread_id, 'session_id': session_id, 'has_isolated_session': db_session is not None})
    ...
    async def _create_user_execution_context(self, user_data: Dict[str, Any]) -> UserExecutionContext:
        ...
        async for db_session in get_request_scoped_db_session():
            try:
                context = UserExecutionContext(user_id=user_id, thread_id=thread_id, run_id=run_id, db_session=db_session, metadata={'test_type': 'supervisor_isolation', 'session_id': user_data['session_id']})
                context_id = f'context_{user_id}_{int(time.time() * 1000)}'
                self.isolation_tracker.record_execution_context(user_id, context_id, {'context_type': type(context).__name__, 'run_id': run_id, 'has_session': context.db_session is not None})
                return context
            except Exception as e:
                logger.error(f'Failed to create execution context for {user_id}: {e}')
                raise

    async def _execute_agent_with_supervisor(self, user_data: Dict[str, Any], query: str) -> Dict[str, Any]:
        ...
        try:
            ...
            self.isolation_tracker.record_concurrent_execution(user_id, execution_id, start_time, end_time, result)
            await self._validate_result_isolation(result, user_id, unique_query)
            return {'user_id': user_id, 'execution_id': execution_id, 'supervisor_id': supervisor_id, 'result': result, 'duration_ms': int((end_time - start_time).total_seconds() * 1000)}
        except Exception as e:
            end_time = datetime.now()
            self.isolation_tracker.record_concurrent_execution(user_id, execution_id, start_time, end_time, None)
    ...
    async def _validate_context_isolation(self, context: UserExecutionContext, expected_user_id: str):
        ...
        if context.user_id != expected_user_id:
            self.isolation_tracker.record_state_leakage(context.user_id, expected_user_id, 'user_id_mismatch', {'expected': expected_user_id, 'actual': context.user_id})
        if hasattr(context.db_session, '_netra_user_tag'):
            ...
            if session_user != expected_user_id:
                self.isolation_tracker.record_state_leakage(session_user, expected_user_id, 'db_session_cross_user', {'expected_user': expected_user_id, 'session_user': session_user})
    ...
    async def _validate_result_isolation(self, result: Any, user_id: str, expected_query: str):
        ...
        for test_user in self.test_users:
            ...
            if other_user_id != user_id and other_user_id[:8] in result_str:
                self.isolation_tracker.record_state_leakage(other_user_id, user_id, 'result_contamination', {'contaminated_result': result_str[:200], 'foreign_user_id': other_user_id})
    ...
    async def test_concurrent_multi_user_isolation(self):
        ...
        assert len(results) >= expected_total_results * 0.8, f'Expected at least {int(expected_total_results * 0.8)} successful results, got {len(results)}'
        ...
        analysis = self.isolation_tracker.get_isolation_analysis()
        assert analysis['state_leakages'] == 0, f"State leakages detected: {analysis['leakage_details']}"
    ...
    async def test_factory_pattern_memory_isolation(self):
        ...
        for i in range(len(supervisors)):
            for j in range(i + 1, len(supervisors)):
                assert supervisors[i] is not supervisors[j], f'Supervisors {i} and {j} should be different instances'
                if hasattr(supervisors[i], 'user_id') and hasattr(supervisors[j], 'user_id'):
                    assert supervisors[i].user_id != supervisors[j].user_id, f'Supervisors {i} and {j} should have different user_ids'
        memory_addresses = [id(supervisor) for supervisor in supervisors]
        assert len(set(memory_addresses)) == len(supervisors), 'All supervisors should have unique memory addresses'
    ...
    async def test_database_session_isolation(self):
        ...
        assert len(set(session_ids)) == len(db_sessions), 'All database sessions should be unique instances'
        for i, context in enumerate(contexts):
            if hasattr(context.db_session, 'info') and 'user_id' in context.db_session.info:
                ...
                assert session_user == expected_user, f'Session user tag mismatch: expected {expected_user}, got {session_user}'
    ...
    async def test_websocket_routing_isolation(self):
        ...
        for user_data in self.test_users[:3]:
            async for db_session in get_request_scoped_db_session():
                context = UserExecutionContext(user_id=user_data['user_id'], thread_id=user_data['thread_id'], run_id=str(uuid.uuid4()), db_session=db_session, websocket_connection_id=f"ws_conn_{user_data['user_id']}", metadata={'websocket_test': True})
                websocket_contexts.append(context)
                break
        connection_ids = [context.websocket_connection_id for context in websocket_contexts]
        assert len(set(connection_ids)) == len(connection_ids), 'All WebSocket connection IDs should be unique'
        for i, context in enumerate(websocket_contexts):
            expected_user = self.test_users[i]['user_id']
            assert expected_user in context.websocket_connection_id, f'WebSocket connection ID should contain user ID for {expected_user}'
