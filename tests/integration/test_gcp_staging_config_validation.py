...
class TestGCPStagingConfigValidation:
    ...
    def test_critical_environment_variables_mapping(self):
        ...
        for var in critical_vars:
            assert var in critical_vars, f"Critical variable {var} not in mapping"
    ...
    @pytest.mark.skipif(not os.getenv("TEST_GCP_INTEGRATION"), reason="GCP integration tests disabled")
    ...
    def test_circuit_breaker_configuration_dependency(self):
        ...
        for _ in range(6):
            with pytest.raises(Exception, match="SERVICE_SECRET"):
                breaker_no_secret.call(lambda: "success")
        ...
        result = breaker_with_secret.call(lambda: "success")
    ...
    def test_deployment_validation_prevents_missing_config(self):
        ...
        def validate_deployment_config(config):
            ...
            if missing:
                return False, f"Missing critical configuration: {missing}"
