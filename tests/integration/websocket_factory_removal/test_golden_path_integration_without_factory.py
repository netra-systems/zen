...
class TestGoldenPathIntegrationWithoutFactory(SSotAsyncTestCase):
    ...
    async def test_golden_path_user_authentication_flow_without_factory(self):
        ...
        try:
            ...
            with patch('netra_backend.app.services.unified_authentication_service.AuthClient') as mock_auth_client:
                ...
                mock_auth_instance.validate_jwt_token.return_value = {'user_id': self.test_user_id, 'valid': True, 'email': f'{self.test_user_id}@test.com'}
    ...
    async def test_golden_path_agent_execution_integration_without_factory(self):
        ...
        try:
            ...
            execution_context = {'user_context': user_context, 'websocket_manager': websocket_manager, 'agent_data': mock_agent_execution_data}
    ...
    async def test_golden_path_websocket_events_delivery_without_factory(self):
        ...
        try:
            ...
            def mock_send_event(event_type: str, data: Dict[str, Any]):
                delivered_events.append({'event_type': event_type, 'user_id': data.get('user_id'), 'websocket_client_id': data.get('websocket_client_id'), 'timestamp': data.get('timestamp')})
                return True
            with patch.object(websocket_manager, 'send_event', side_effect=mock_send_event):
                for event_type in critical_events:
                    event_data = {'user_id': self.test_user_id, 'websocket_client_id': self.test_websocket_client_id, 'message': f'Test {event_type} event', 'timestamp': '2025-09-14T12:00:00Z'}
                    websocket_manager.send_event(event_type, event_data)
            self.assertEqual(len(delivered_events), len(critical_events), f'Not all events delivered. Expected: {len(critical_events)}, Got: {len(delivered_events)}')
    ...
    async def test_golden_path_end_to_end_flow_without_factory(self):
        ...
        try:
            ...
            mock_ai_response = {'response_text': 'Test AI response demonstrating Golden Path functionality', 'agent_type': 'supervisor_agent', 'processing_time': 2.5, 'user_id': self.test_user_id}
            flow_components = {'authenticated_user': authenticated_user_context, 'websocket_connection': websocket_connection, 'agent_request': agent_request_data, 'ai_response': mock_ai_response}
    ...
    async def test_golden_path_performance_without_factory_overhead(self):
        ...
        try:
            ...
            for i in range(20):
                user_context = UserExecutionContext(user_id=f'{self.test_user_id}_{i}', websocket_client_id=f'golden_path_perf_{i}')
                websocket_manager = WebSocketManager(user_context=user_context)
                golden_path_sessions.append({'user_context': user_context, 'websocket_manager': websocket_manager})
            end_time = time.perf_counter()
            performance_time = end_time - start_time
            self.assertEqual(len(golden_path_sessions), 20)
            self.assertLess(performance_time, 2.0, f'Golden Path performance too slow: {performance_time:.3f}s')
            user_ids = set()
