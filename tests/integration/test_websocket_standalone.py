...
class WebSocketStandaloneTest:
    ...
    async def test_websocket_connection_basic(
        ...
        try:
            ...
            error_msg = fUnexpected error: {type(e).__name__}: {str(e)}
            logger.error(f FAIL:  {test_name} - FAILED: {error_msg})
            return False, error_msg
    ...
    def test_critical_websocket_event_structure(self) -> Tuple[bool, str]:
        ...
        try:
            ...
            error_msg = fEvent structure test failed: {type(e).__name__}: {str(e)}
            logger.error(f FAIL:  {test_name} - FAILED: {error_msg}")"
            return False, error_msg
    ...
    async def test_agent_event_flow_sequence(self) -> Tuple[bool, str]:
        ...
        try:
            ...
            for field in context_fields:
                ...
                if len(values) > 1:
                    sequence_errors.append(fInconsistent {field} across events: {values})
            ...
            error_msg = fAgent event flow test failed: {type(e).__name__}: {str(e)}
            logger.error(f FAIL:  {test_name} - FAILED: {error_msg}")"
            return False, error_msg
