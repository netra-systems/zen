...
class MessageRouterSSOTComplianceTrackingTests(SSotAsyncTestCase):
    ...
    def setUp(self):
        ...
        self.compliance_metrics = {'proxy_usage_count': 0, 'canonical_usage_count': 0, 'services_usage_count': 0, 'ssot_violations': [], 'import_path_tracking': {}}
    ...
    async def test_import_path_consistency_across_services(self):
        ...
        import_paths = [('netra_backend.app.core.message_router', ProxyMessageRouter), ('netra_backend.app.websocket_core.handlers', CanonicalMessageRouter), ('netra_backend.app.services.message_router', ServicesMessageRouter)]
        for path, router_class in import_paths:
            self.compliance_metrics['import_path_tracking'][path] = {'class_name': router_class.__name__, 'module': router_class.__module__, 'is_proxy': hasattr(router_class(), '_canonical_router') if hasattr(router_class, '__call__') else False}
    ...
    async def test_proxy_deprecation_compliance_tracking(self):
        ...
        with warnings.catch_warnings(record=True) as warning_list:
            ...
            self.compliance_metrics['ssot_violations'].append({'type': 'deprecated_proxy_usage', 'warning_count': len(deprecation_warnings), 'timestamp': time.time()})
        logger.warning(f"Proxy usage tracked: {self.compliance_metrics['proxy_usage_count']} instances, {len(self.compliance_metrics['ssot_violations'])} violations")
    ...
    async def test_real_message_routing_ssot_compliance(self):
        ...
        test_message = {'type': 'test_message', 'payload': {'content': 'SSOT compliance test'}, 'timestamp': time.time(), 'user_id': self.test_user_id, 'thread_id': self.test_thread_id}
    ...
    async def test_handler_registration_ssot_compliance(self):
        ...
        try:
            ...
            self.compliance_metrics['ssot_violations'].append({'type': 'proxy_handler_registration_failure', 'error': str(e), 'timestamp': time.time()})
        logger.info(f"Handler registration compliance: canonical={self.compliance_metrics['canonical_usage_count']}, proxy={self.compliance_metrics['proxy_usage_count']}")
    ...
    async def test_websocket_event_routing_ssot_compliance(self):
        ...
        test_messages = [{'type': 'user_message', 'payload': {'content': 'test user message'}, 'timestamp': time.time()}, {'type': 'ping', 'payload': {}, 'timestamp': time.time()}, {'type': 'agent_request', 'payload': {'message': 'test agent request'}, 'timestamp': time.time()}]
        successful_routes = 0
        for message in test_messages:
            try:
                result = await canonical_router.route_message(self.test_user_id, mock_websocket, message)
                if result:
                    successful_routes += 1
            except Exception as e:
                self.compliance_metrics['ssot_violations'].append({'type': 'routing_failure', 'message_type': message['type'], 'error': str(e), 'timestamp': time.time()})
    ...
    async def test_service_integration_ssot_compliance(self):
        ...
        ws_manager = WebSocketManager(user_context=self.user_context)
        context = UserExecutionContext.from_request(user_id=self.test_user_id, thread_id=self.test_thread_id, run_id=self.test_run_id)
    ...
    def test_ssot_compliance_summary(self):
        ...
        total_usage = self.compliance_metrics['proxy_usage_count'] + self.compliance_metrics['canonical_usage_count'] + self.compliance_metrics['services_usage_count']
        if total_usage > 0:
            canonical_percentage = self.compliance_metrics['canonical_usage_count'] / total_usage * 100
        else:
            canonical_percentage = 0
        compliance_report = {'ssot_compliance_percentage': canonical_percentage, 'total_usage_count': total_usage, 'canonical_usage_count': self.compliance_metrics['canonical_usage_count'], 'proxy_usage_count': self.compliance_metrics['proxy_usage_count'], 'services_usage_count': self.compliance_metrics['services_usage_count'], 'violation_count': len(self.compliance_metrics['ssot_violations']), 'violations': self.compliance_metrics['ssot_violations'], 'import_path_tracking': self.compliance_metrics['import_path_tracking']}
        logger.info(f'SSOT Compliance Report: {json.dumps(compliance_report, indent=2)}')
        self.assertGreaterEqual(canonical_percentage, 70.0, 'SSOT compliance should be at least 70% canonical usage')
        self.assertLessEqual(len(self.compliance_metrics['ssot_violations']), 5, 'Should have 5 or fewer SSOT violations')
