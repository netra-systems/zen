...
class StartupClickHouseEnvironmentIntegrationTests:
    ...
    def mock_cloud_environment_detector(self):
        ...
        staging_context = EnvironmentContext(environment_type=EnvironmentType.STAGING, cloud_platform=CloudPlatform.CLOUD_RUN, service_name='netra-backend-staging', project_id='netra-staging', region='us-central1', confidence_score=0.9, detection_metadata={'method': 'cloud_run_metadata', 'service_name': 'netra-backend-staging'})
        mock_detector = MagicMock()
        mock_detector.detect_environment_context = AsyncMock(return_value=staging_context)
        with patch('netra_backend.app.core.environment_context.cloud_environment_detector.get_cloud_environment_detector', return_value=mock_detector):
            yield (mock_detector, staging_context)

    @pytest.mark.asyncio
    async def test_real_docker_availability_check_integration(self, staging_cloud_run_environment, real_docker_check):
        """
    ...
    @pytest.mark.asyncio
    async def test_cloud_environment_detector_integration_with_clickhouse_startup(self, staging_cloud_run_environment, mock_cloud_environment_detector):
        """
    ...
    async def test_environment_specific_docker_requirements(self):
        ...
        """
        logger = central_logger.get_logger(__name__)
        staging_env = {'ENVIRONMENT': 'staging', 'K_SERVICE': 'netra-backend-staging', 'CLICKHOUSE_REQUIRED': 'false'}
            ...
            with patch('netra_backend.app.startup_module.get_config', return_value=mock_config_staging):
                with patch('netra_backend.app.startup_module.get_env') as mock_get_env:
                    ...
                    with patch('netra_backend.app.startup_module._setup_clickhouse_tables') as mock_setup:
                        ...
                        assert result['required'] == False
                        assert result['status'] == 'skipped'
        production_env = {'ENVIRONMENT': 'production', 'CLICKHOUSE_REQUIRED': 'true'}
    ...
    @pytest.mark.asyncio
    async def test_docker_availability_detection_integration_with_real_subprocess(self, staging_cloud_run_environment):
        """
