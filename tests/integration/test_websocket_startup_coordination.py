...
class TestWebSocketStartupCoordination(SSotAsyncTestCase):
    ...
    async def test_websocket_readiness_validation_integration(self):
        ...
        successful_early_connections = [attempt for attempt in connection_attempts if attempt['success'] and attempt['websocket_ready'] == False]
        ...
        if successful_early_connections:
            self.assertEqual(len(successful_early_connections), 0, f'EXPECTED FAILURE: Service accepts connections before WebSocket ready. Found {len(successful_early_connections)} successful early connections. This causes 1011 errors during startup.')
        else:
    ...
    async def test_dependency_initialization_order(self):
        ...
        correct_order = ['configuration', 'database', 'redis', 'agent_registry', 'websocket_manager', 'http_server']
    ...
    async def test_health_check_websocket_integration(self):
        ...
        if problematic_health_checks:
            self.assertEqual(len(problematic_health_checks), 0, f'EXPECTED FAILURE: Health checks pass while WebSocket not ready. Found {len(problematic_health_checks)} problematic health checks. This causes 1011 errors when traffic routes to unready instances.')
        else:
    ...
    async def test_concurrent_startup_websocket_race_condition(self):
        ...
        monitor = StartupPhaseMonitor()
        concurrent_tasks = [self._simulate_service_startup('database', monitor, delay=1.0), self._simulate_service_startup('redis', monitor, delay=0.8), self._simulate_service_startup('agent_registry', monitor, delay=1.2), self._simulate_service_startup('websocket_manager', monitor, delay=0.9)]
        ...
        self.assertEqual(len(race_conditions), 0, f'Race conditions detected during concurrent startup: {race_conditions}. Failed services: {failed_services}')
    ...
    async def _simulate_websocket_startup(self, monitor: StartupPhaseMonitor) -> Dict[str, Any]:
        ...
        websocket_state = {'initialized': False, 'dependencies_ready': False, 'listening': False, 'ready_for_connections': False}
        await monitor.record_phase('init', 'websocket_manager', 'started')
        await asyncio.sleep(0.1)
    ...
    async def _simulate_early_connection_attempts(self, monitor: StartupPhaseMonitor, websocket_state: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        for phase_info in phases:
            ...
            await monitor.record_phase('connection_attempt', 'client', 'success' if connection_success else 'failed', attempt)
