...
class DeploymentSsotIntegrationTests(SSotBaseTestCase):
    ...
    def test_unified_test_runner_deployment_mode_execution(self):
        ...
        with patch('subprocess.run') as mock_subprocess:
            mock_subprocess.return_value = Mock(returncode=0, stdout='Dry-run deployment completed successfully', stderr='')
            cmd = ['python', str(self.unified_runner_path), '--execution-mode', 'deploy', '--project', self.test_project_id, '--dry-run']
            try:
                result = subprocess.run(cmd, capture_output=True, text=True, timeout=60, cwd=str(self.project_root))
                ...
                deployment_indicators = ['deploy', 'gcp', 'project', self.test_project_id]
                output_text = (result.stdout + result.stderr).lower()
                found_indicators = []
                for indicator in deployment_indicators:
                    if indicator in output_text:
                        found_indicators.append(indicator)
                assert len(found_indicators) >= 2, f'Deployment mode execution missing indicators. Found: {found_indicators}'
    ...
    def test_terraform_vs_scripts_configuration_consistency(self):
        ...
        critical_configs = ['project_id', 'region', 'vpc_connector', 'service_names', 'environment_variables']
        for config_key in critical_configs:
            ...
            if terraform_value != script_value:
                config_consistency_issues.append({'config_key': config_key, 'terraform_value': terraform_value, 'script_value': script_value, 'issue_type': 'value_mismatch'})
        ...
        if config_consistency_issues:
            ...
            critical_issues = [issue for issue in config_consistency_issues if issue['config_key'] in ['project_id', 'region']]
            if critical_issues:
                issue_details = '\n'.join([f"  - {issue['config_key']}: terraform='{issue['terraform_value']}' vs script='{issue['script_value']}'" for issue in critical_issues])
                pytest.fail(f'CRITICAL CONFIG MISMATCH: {len(critical_issues)} critical configuration inconsistencies:\n{issue_details}\n\nExpected: Terraform and script configurations should match\nFix: Align critical configuration values between Terraform and scripts')
    ...
    def test_multi_environment_deployment_consistency(self):
        ...
        for env in self.test_environments:
            try:
                ...
                required_keys = ['project_id', 'service_configs', 'environment_variables']
                missing_keys = []
                for key in required_keys:
                    if key not in env_config:
                        missing_keys.append(key)
                environment_results[env] = {'config_loaded': True, 'missing_keys': missing_keys, 'config_size': len(env_config), 'has_required_keys': len(missing_keys) == 0}
            except Exception as e:
                environment_results[env] = {'config_loaded': False, 'error': str(e), 'has_required_keys': False}
        successful_envs = [env for env, result in environment_results.items() if result.get('has_required_keys', False)]
        ...
        assert actual_consistency >= consistency_threshold, f'Environment consistency too low: {actual_consistency:.2%} < {consistency_threshold:.2%}. Failed environments: {[env for env in self.test_environments if env not in successful_envs]}'
    ...
    def test_ssot_compliance_during_deployment_flow(self):
        ...
        """
        with patch('test_framework.unified_docker_manager.UnifiedDockerManager') as mock_docker, patch('shared.isolated_environment.get_env') as mock_env, patch('subprocess.run') as mock_subprocess:
            mock_env.return_value = self.get_env()
            mock_docker.return_value = Mock()
            mock_subprocess.return_value = Mock(returncode=0)
            deployment_steps = ['environment_setup', 'configuration_validation', 'docker_image_build', 'gcp_deployment', 'health_check']
            ...
            for step in deployment_steps:
                try:
                    if step == 'environment_setup':
                        ...
                        test_config = {'project': self.test_project_id, 'environment': 'test'}
                    ...
                    ssot_compliance_violations.append({'step': step, 'error': str(e), 'violation_type': 'step_execution_failure'})
            ...
            if ssot_compliance_violations:
                violation_details = '\n'.join([f"  - {v['step']}: {v['error']}" for v in ssot_compliance_violations])
                pytest.fail(f'SSOT COMPLIANCE VIOLATION: {len(ssot_compliance_violations)} violations during deployment flow:\n{violation_details}\n\nExpected: All deployment steps should use SSOT patterns\nFix: Update violating steps to use SSOT infrastructure')
    ...
    def test_deployment_configuration_drift_detection(self):
        ...
        """
        baseline_config = {'project_id': self.test_project_id, 'region': 'us-central1', 'services': ['backend', 'auth', 'frontend'], 'environment_variables': {'ENVIRONMENT': 'staging', 'LOG_LEVEL': 'INFO'}}
        test_configs = [baseline_config.copy(), {**baseline_config, 'environment_variables': {**baseline_config['environment_variables'], 'LOG_LEVEL': 'DEBUG'}}, {**baseline_config, 'region': 'us-west1', 'services': ['backend', 'auth']}]
        drift_detections = []
        for i, test_config in enumerate(test_configs):
            drift_score = self._calculate_configuration_drift(baseline_config, test_config)
            drift_detections.append({'config_index': i, 'drift_score': drift_score, 'is_significant_drift': drift_score > 0.3})
        ...
        assert major_change_detected, 'Failed to detect significant configuration drift'
        minor_change_flagged = any((d['config_index'] == 1 for d in significant_drifts))
        assert not minor_change_flagged, 'Incorrectly flagged minor configuration change as significant drift'
    ...
    def _get_environment_deployment_config(self, environment: str) -> Dict[str, Any]:
        ...
        base_config = {'project_id': f'netra-{environment}', 'service_configs': {'backend': {'memory': '1Gi', 'cpu': '1'}, 'auth': {'memory': '512Mi', 'cpu': '0.5'}, 'frontend': {'memory': '256Mi', 'cpu': '0.25'}}, 'environment_variables': {'ENVIRONMENT': environment, 'LOG_LEVEL': 'INFO' if environment == 'production' else 'DEBUG'}}
        return base_config
...
class DeploymentSsotIntegrationEdgeCasesTests(SSotBaseTestCase):
    ...
    def test_deployment_rollback_integration(self):
        ...
        """
        with patch('subprocess.run') as mock_subprocess:
            mock_subprocess.return_value = Mock(returncode=0, stdout='Rollback completed successfully')
            cmd = ['python', str(Path(__file__).parent.parent.parent.parent / 'tests' / 'unified_test_runner.py'), '--execution-mode', 'deploy', '--project', 'netra-staging', '--rollback', '--dry-run']
            try:
    ...
    def test_deployment_service_specific_integration(self):
        ...
        for service in services_to_test:
            with patch('subprocess.run') as mock_subprocess:
                mock_subprocess.return_value = Mock(returncode=0)
                cmd = ['python', str(Path(__file__).parent.parent.parent.parent / 'tests' / 'unified_test_runner.py'), '--execution-mode', 'deploy', '--project', 'netra-staging', '--service', service, '--dry-run']
                try:
