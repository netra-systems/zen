...
class MockMessageProcessor:
    ...
    async def process_message(self, user_message: str, user_context: UserExecutionContext) -> Dict[str, Any]:
        ...
        try:
            result = await self.agent_registry.execute_agent(agent_type=agent_type, user_message=user_message, user_context=user_context)
            self.processing_steps.append('completion')
            return {'status': 'success', 'agent_type': agent_type, 'result': result, 'processing_steps': self.processing_steps.copy(), 'processed_at': datetime.now(timezone.utc).isoformat()}
        except Exception as e:
            self.processing_steps.append('error')
            return {'status': 'error', 'error': str(e), 'processing_steps': self.processing_steps.copy(), 'failed_at': datetime.now(timezone.utc).isoformat()}
...
class TestPipelineAgent(BaseAgent):
    ...
    async def execute(self, user_message: str, context: Dict[str, Any]=None) -> Dict[str, Any]:
        ...
        return {'agent_type': self.agent_type, 'status': 'success', 'message': user_message, 'response': f'Processed {user_message} using {self.agent_type}', 'execution_steps': self.execution_steps.copy(), 'context_data': self.context_data.copy(), 'processed_at': datetime.now(timezone.utc).isoformat()}
...
class TestMessageProcessingPipelineIntegration(BaseIntegrationTest):
    ...
    async def test_user_message_to_agent_execution_pipeline(self, real_services_fixture):
        ...
        """
        user_context = await create_test_user_context(user_id=self.test_user_id, request_id=f'pipeline_req_{uuid.uuid4().hex[:8]}', thread_id=self.test_thread_id, real_services=real_services_fixture)
        test_messages = [{'message': 'Help me optimize my cloud costs and reduce monthly billing', 'expected_agent': 'cost_analysis_agent', 'expected_keywords': ['cost', 'optimize']}, {'message': 'Scan my infrastructure for security vulnerabilities', 'expected_agent': 'security_agent', 'expected_keywords': ['security', 'vulnerabilities']}, {'message': 'Set up automated deployment pipeline for my application', 'expected_agent': 'deployment_agent', 'expected_keywords': ['deployment', 'pipeline']}, {'message': 'What are the general best practices for cloud management?', 'expected_agent': 'triage_agent', 'expected_keywords': ['general', 'practices']}]
        for i, test_case in enumerate(test_messages):
            start_time = time.time()
            result = await self.message_processor.process_message(user_message=test_case['message'], user_context=user_context)
            ...
            assert result['agent_type'] == test_case['expected_agent'], f"Test case {i} wrong agent selected: {result['agent_type']} vs {test_case['expected_agent']}"
    ...
    async def test_multi_step_message_processing_flow(self, real_services_fixture):
        ...
        """
        user_context = await create_test_user_context(user_id=self.test_user_id, request_id=f'multistep_req_{uuid.uuid4().hex[:8]}', thread_id=self.test_thread_id, real_services=real_services_fixture)
        complex_message = 'I need a comprehensive analysis of my infrastructure including cost optimization recommendations, security vulnerability assessment, and deployment pipeline improvements'
        result = await self.message_processor.process_message(user_message=complex_message, user_context=user_context)
    ...
    async def test_message_context_preservation_across_steps(self, real_services_fixture):
        ...
        """
        user_context = await create_test_user_context(user_id=self.test_user_id, request_id=f'context_req_{uuid.uuid4().hex[:8]}', thread_id=self.test_thread_id, real_services=real_services_fixture, metadata={'user_tier': 'enterprise', 'session_id': f'session_{uuid.uuid4().hex[:8]}', 'preference_settings': {'response_detail': 'high', 'include_metrics': True}, 'request_origin': 'web_chat', 'client_version': '2.1.0'})
        test_message = 'Analyze my infrastructure costs with detailed breakdown'
        result = await self.message_processor.process_message(user_message=test_message, user_context=user_context)
    ...
    async def test_message_error_recovery_integration(self, real_services_fixture):
        ...
        """
        user_context = await create_test_user_context(user_id=self.test_user_id, request_id=f'error_req_{uuid.uuid4().hex[:8]}', thread_id=self.test_thread_id, real_services=real_services_fixture)
        error_scenarios = [{'name': 'empty_message', 'message': '', 'expected_error_type': 'validation'}, {'name': 'whitespace_only', 'message': '   \n\t  ', 'expected_error_type': 'validation'}, {'name': 'very_long_message', 'message': 'x' * 10000, 'expected_error_type': 'processing'}]
        for scenario in error_scenarios:
            result = await self.message_processor.process_message(user_message=scenario['message'], user_context=user_context)
        ...
        recovery_result = await self.message_processor.process_message(user_message=recovery_message, user_context=user_context)
