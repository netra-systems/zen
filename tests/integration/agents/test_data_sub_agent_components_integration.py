...
class DataSubAgentComponentsIntegrationTests(BaseIntegrationTest):
    ...
    def setUp(self) -> None:
        ...
        self.data_request_scenario = {'user_request': "I need to optimize my AWS costs but I'm not sure what data I need to provide", 'context': {'optimization_goal': 'cost_reduction', 'current_data': 'minimal', 'user_experience': 'beginner'}}
        self.validation_scenario = {'data_payload': {'aws_bill': '$1,250/month', 'instance_types': ['t3.medium', 'm5.large'], 'usage_patterns': '24/7 for web app'}, 'expected_validation': ['cost_validation', 'instance_optimization', 'usage_pattern_analysis']}
    ...
    async def test_data_helper_agent_request_generation_workflow(self):
        ...
        data_helper_agent = DataHelperAgent(llm_manager=llm_manager, tool_dispatcher=tool_dispatcher)
        user_context = UserExecutionContext(user_id=self.user_id, thread_id=self.thread_id, run_id=self.run_id, agent_state=DeepAgentState(user_request=self.data_request_scenario['user_request'], user_prompt=self.data_request_scenario['user_request'], user_id=self.user_id, chat_thread_id=self.thread_id, run_id=self.run_id))
        start_time = time.time()
        with patch.object(llm_manager, 'generate_response') as mock_llm:
            mock_llm.return_value = {'data_requests': [{'category': 'aws_billing', 'required_fields': ['monthly_spend', 'service_breakdown', 'region_usage'], 'priority': 'high', 'rationale': 'Essential for cost optimization analysis'}, {'category': 'infrastructure', 'required_fields': ['instance_inventory', 'utilization_metrics', 'scaling_patterns'], 'priority': 'medium', 'rationale': 'Needed for rightsizing recommendations'}], 'estimated_savings_potential': '15-25%', 'confidence_level': 0.8}
        ...
        if hasattr(data_helper_agent, 'websocket_bridge'):
            self.websocket_utility.verify_agent_events_sent(['agent_started', 'agent_thinking', 'agent_completed'])
    ...
    async def test_validation_sub_agent_data_validation_workflow(self):
        ...
        llm_manager = self._create_mock_llm_manager()
        tool_dispatcher = self._create_mock_tool_dispatcher()
        validation_agent = ValidationSubAgent(llm_manager=llm_manager, tool_dispatcher=tool_dispatcher)
        user_context = UserExecutionContext(user_id=self.user_id, thread_id=self.thread_id, run_id=self.run_id, agent_state=DeepAgentState(user_request='Please validate my AWS optimization data', user_prompt='Please validate my AWS optimization data', user_id=self.user_id, chat_thread_id=self.thread_id, run_id=self.run_id, agent_input=self.validation_scenario))
        start_time = time.time()
        with patch.object(llm_manager, 'generate_response') as mock_llm:
            mock_llm.return_value = {'validation_results': [{'field': 'aws_bill', 'status': 'valid', 'confidence': 0.95, 'insights': ['Monthly spend within expected range for workload']}, {'field': 'instance_types', 'status': 'optimization_opportunity', 'confidence': 0.85, 'insights': ['t3.medium may be oversized for typical web workloads']}, {'field': 'usage_patterns', 'status': 'needs_clarification', 'confidence': 0.6, 'insights': ['24/7 usage suggests potential for reserved instances']}], 'overall_data_quality': 'good', 'recommendations': ['Consider reserved instances', 'Evaluate t3.small instances']}
    ...
    async def test_tool_discovery_sub_agent_workflow(self):
        ...
        llm_manager = self._create_mock_llm_manager()
        tool_dispatcher = self._create_mock_tool_dispatcher()
        tool_discovery_agent = ToolDiscoverySubAgent(llm_manager=llm_manager, tool_dispatcher=tool_dispatcher)
        user_context = UserExecutionContext(user_id=self.user_id, thread_id=self.thread_id, run_id=self.run_id, agent_state=DeepAgentState(user_request='Find tools to help with AWS cost analysis and optimization', user_prompt='Find tools to help with AWS cost analysis and optimization', user_id=self.user_id, chat_thread_id=self.thread_id, run_id=self.run_id))
        start_time = time.time()
        with patch.object(llm_manager, 'generate_response') as mock_llm:
            mock_llm.return_value = {'discovered_tools': [{'tool_name': 'aws_cost_explorer', 'capabilities': ['cost_analysis', 'usage_patterns', 'recommendations'], 'confidence': 0.9, 'use_case': 'Primary tool for AWS cost analysis'}, {'tool_name': 'rightsizing_analyzer', 'capabilities': ['instance_optimization', 'utilization_analysis'], 'confidence': 0.85, 'use_case': 'Optimize instance sizing recommendations'}], 'execution_plan': ['cost_analysis', 'usage_analysis', 'optimization_recommendations'], 'estimated_execution_time': '3-5 minutes'}
    ...
    async def test_multi_agent_data_coordination_workflow(self):
        ...
        user_context = UserExecutionContext(user_id=self.user_id, thread_id=self.thread_id, run_id=self.run_id, agent_state=DeepAgentState(user_request='Complete AWS optimization analysis with data coordination', user_prompt='Complete AWS optimization analysis with data coordination', user_id=self.user_id, chat_thread_id=self.thread_id, run_id=self.run_id))
        workflow_results = {}
        start_time = time.time()
        with patch.object(llm_manager, 'generate_response') as mock_llm:
    ...
    async def test_data_sub_agent_error_handling_recovery(self):
        ...
        tool_dispatcher = self._create_mock_tool_dispatcher()
        data_helper = DataHelperAgent(llm_manager, tool_dispatcher)
        user_context = UserExecutionContext(user_id=self.user_id, thread_id=self.thread_id, run_id=self.run_id, agent_state=DeepAgentState(user_request='Invalid data optimization request', user_prompt='Invalid data optimization request', user_id=self.user_id, chat_thread_id=self.thread_id, run_id=self.run_id))
        with patch.object(llm_manager, 'generate_response') as mock_llm:
