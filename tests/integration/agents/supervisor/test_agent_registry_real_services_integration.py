...
class RealWebSocketManagerIntegrationTests(SSotAsyncTestCase):
    ...
    async def test_real_websocket_manager_connection_and_event_delivery(self):
        ...
        try:
            ...
            self.assertIsNotNone(self.registry.websocket_manager, 'Real WebSocket manager must be set on registry')
            self.assertIs(self.registry.websocket_manager, websocket_manager, 'Registry must reference the same WebSocket manager instance')
            ...
            self.assertIsNotNone(user_session._websocket_bridge, 'User session must have WebSocket bridge for real connections')
            if hasattr(websocket_manager, 'get_connection_count'):
                connection_count = await websocket_manager.get_connection_count()
                self.assertGreaterEqual(connection_count, 0, 'WebSocket manager must track connections')
    ...
    async def test_websocket_event_propagation_to_multiple_users(self):
        ...
        try:
            ...
            for user_id, session, context in user_sessions:
                self.assertIsNotNone(session._websocket_bridge, f'User {user_id} must have WebSocket bridge')
                bridge = session._websocket_bridge
                self.assertTrue(hasattr(bridge, 'notify_agent_started'), f'User {user_id} bridge must support agent notifications')
        except Exception as e:
    ...
    async def test_websocket_manager_adapter_with_real_manager(self):
        ...
        try:
            ...
            required_methods = ['notify_agent_started', 'notify_agent_thinking', 'notify_tool_executing', 'notify_tool_completed', 'notify_agent_completed', 'notify_agent_error']
            for method_name in required_methods:
                self.assertTrue(hasattr(adapter, method_name), f'Adapter must provide {method_name} method')
                method = getattr(adapter, method_name)
                self.assertTrue(callable(method), f'Adapter {method_name} must be callable')
            self.assertIs(adapter._websocket_manager, websocket_manager, 'Adapter must reference real WebSocket manager')
        except Exception as e:
...
class RealToolDispatcherIntegrationTests(SSotAsyncTestCase):
    ...
    async def asyncSetUp(self):
        ...
        self.llm_manager = LLMManager()
        self.registry = AgentRegistry(llm_manager=self.llm_manager)
        self.test_user_context = UserExecutionContext(user_id=f'tool_user_{uuid.uuid4().hex[:8]}', request_id=f'req_{uuid.uuid4().hex[:8]}', thread_id=f'tool_thread_{uuid.uuid4().hex[:8]}', run_id=f'run_{uuid.uuid4().hex[:8]}')
    ...
    async def test_unified_tool_dispatcher_creation_with_real_services(self):
        ...
        try:
            ...
            self.assertIsInstance(dispatcher, UnifiedToolDispatcher, 'Must create UnifiedToolDispatcher instance')
            if hasattr(dispatcher, 'user_context'):
                self.assertEqual(dispatcher.user_context.user_id, self.test_user_context.user_id, 'Dispatcher must be associated with correct user')
    ...
    async def test_tool_dispatcher_websocket_integration(self):
        ...
        try:
            ...
            dispatcher = await self.registry.create_tool_dispatcher_for_user(user_context=self.test_user_context, websocket_bridge=websocket_bridge, enable_admin_tools=False)
            self.assertIsNotNone(dispatcher, 'Dispatcher with WebSocket must be created')
            if hasattr(dispatcher, '_websocket_bridge'):
                self.assertIsNotNone(dispatcher._websocket_bridge, 'Dispatcher must have WebSocket bridge for notifications')
        except Exception as e:
    ...
    async def test_user_isolated_tool_dispatchers_with_real_services(self):
        ...
        try:
            user_contexts = []
            for i in range(3):
                context = UserExecutionContext(user_id=f'isolated_tool_user_{i}_{uuid.uuid4().hex[:8]}', request_id=f'req_{uuid.uuid4().hex[:8]}', thread_id=f'thread_tool_{i}_{uuid.uuid4().hex[:8]}', run_id=f'run_{uuid.uuid4().hex[:8]}')
                user_contexts.append(context)
            dispatchers = []
            for context in user_contexts:
                dispatcher = await self.registry.create_tool_dispatcher_for_user(user_context=context, websocket_bridge=None, enable_admin_tools=False)
            ...
            for i, (context, dispatcher) in enumerate(dispatchers):
                ...
                for j, (other_context, other_dispatcher) in enumerate(dispatchers):
                    if i != j:
                        self.assertNotEqual(dispatcher, other_dispatcher, f'Dispatchers {i} and {j} must be isolated instances')
        except Exception as e:
    ...
    async def test_tool_execution_with_real_backend_services(self):
        ...
        try:
            ...
            if hasattr(dispatcher, 'execute_tool'):
                self.assertTrue(callable(dispatcher.execute_tool), 'Tool dispatcher must support tool execution')
...
class RealDatabaseIntegrationTests(SSotAsyncTestCase):
    ...
    async def test_agent_registry_with_real_database_sessions(self):
        ...
        try:
            async with self.db_manager.get_session() as db_session:
                user_context = UserExecutionContext(user_id=f'db_user_{uuid.uuid4().hex[:8]}', request_id=f'req_{uuid.uuid4().hex[:8]}', thread_id=f'thread_db_{uuid.uuid4().hex[:8]}', run_id=f'run_{uuid.uuid4().hex[:8]}', db_session=db_session)
                ...
                self.assertEqual(user_session.user_id, user_context.user_id, 'Session must be associated with correct user')
        except Exception as e:
    ...
    async def test_agent_state_persistence_with_real_database(self):
        ...
        try:
            async with self.db_manager.get_session() as db_session:
                user_context = UserExecutionContext(user_id=f'persist_user_{uuid.uuid4().hex[:8]}', request_id=f'req_{uuid.uuid4().hex[:8]}', thread_id=f'thread_persist_{uuid.uuid4().hex[:8]}', run_id=f'run_{uuid.uuid4().hex[:8]}', db_session=db_session)
                user_session = await self.registry.get_user_session(user_context.user_id)
                for i in range(3):
                    agent = BaseAgent(llm_manager=self.llm_manager, context=user_context, name=f'test_agent_{i}')
                    await user_session.register_agent(f'test_agent_{i}', agent)
                metrics = user_session.get_metrics()
                self.assertEqual(metrics['agent_count'], 3, 'Session must track all registered agents')
                await user_session.cleanup_all_agents()
                final_metrics = user_session.get_metrics()
                self.assertEqual(final_metrics['agent_count'], 0, 'Cleanup must properly clear agent state')
        except Exception as e:
    ...
    async def test_concurrent_database_operations_through_registry(self):
        ...
        try:
            ...
            async def create_user_with_database(user_index: int):
                ...
                async with self.db_manager.get_session() as db_session:
                    user_context = UserExecutionContext(user_id=f'concurrent_db_user_{user_index}_{uuid.uuid4().hex[:8]}', request_id=f'req_{uuid.uuid4().hex[:8]}', thread_id=f'thread_concurrent_{user_index}_{uuid.uuid4().hex[:8]}', run_id=f'run_{uuid.uuid4().hex[:8]}', db_session=db_session)
                    user_session = await self.registry.get_user_session(user_context.user_id)
                    agent = BaseAgent(llm_manager=self.llm_manager, context=user_context, name=f'concurrent_agent_{user_index}')
                    await user_session.register_agent('concurrent_agent', agent)
                    return (user_context.user_id, user_session.get_metrics())
            results = await asyncio.gather(*[create_user_with_database(i) for i in range(concurrent_users)], return_exceptions=True)
            successful_results = [r for r in results if not isinstance(r, Exception)]
            self.assertEqual(len(successful_results), concurrent_users, 'All concurrent database operations must succeed')
            for user_id, metrics in successful_results:
                self.assertEqual(metrics['agent_count'], 1, f'User {user_id} must have correct agent count')
        except Exception as e:
...
class RealAgentExecutionFlowTests(SSotAsyncTestCase):
    ...
    async def test_end_to_end_agent_creation_and_execution_with_real_services(self):
        ...
        """
        try:
            user_context = UserExecutionContext(user_id=f'e2e_user_{uuid.uuid4().hex[:8]}', request_id=f'req_{uuid.uuid4().hex[:8]}', thread_id=f'thread_e2e_{uuid.uuid4().hex[:8]}', run_id=f'run_{uuid.uuid4().hex[:8]}')
            try:
                websocket_manager = WebSocketManager()
                await websocket_manager.initialize()
                await self.registry.set_websocket_manager_async(websocket_manager)
            ...
            if available_agents:
                ...
                try:
                    agent = await self.registry.create_agent_for_user(user_id=user_context.user_id, agent_type=agent_type, user_context=user_context, websocket_manager=getattr(self.registry, 'websocket_manager', None))
                    self.assertIsNotNone(agent, f'Agent {agent_type} must be created')
                    retrieved_agent = await self.registry.get_user_agent(user_context.user_id, agent_type)
                    self.assertIs(agent, retrieved_agent, 'Created agent must be retrievable from registry')
                    cleanup_success = await self.registry.remove_user_agent(user_context.user_id, agent_type)
                    self.assertTrue(cleanup_success, 'Agent cleanup must succeed')
                except KeyError as e:
                    self.assertIn('No factory registered', str(e))
            else:
                self.assertEqual(len(available_agents), 0, 'Registry should report empty state when no agents registered')
        except Exception as e:
    ...
    async def test_multi_agent_coordination_with_real_services(self):
        ...
        try:
            user_contexts = []
            for i in range(3):
                context = UserExecutionContext(user_id=f'coord_user_{i}_{uuid.uuid4().hex[:8]}', request_id=f'req_{uuid.uuid4().hex[:8]}', thread_id=f'thread_coord_{i}_{uuid.uuid4().hex[:8]}', run_id=f'run_{uuid.uuid4().hex[:8]}')
                user_contexts.append(context)
            user_sessions = []
            for context in user_contexts:
            ...
            self.assertEqual(len(set(session_ids)), len(user_sessions), 'All user sessions must be unique instances')
            monitoring_report = await self.registry.monitor_all_users()
            self.assertEqual(monitoring_report['total_users'], len(user_contexts), 'Monitoring must track all active users')
            self.assertIn('users', monitoring_report, 'Monitoring must provide per-user details')
        except Exception as e:
    ...
    async def test_agent_error_recovery_with_real_services(self):
        ...
        """
        try:
            user_context = UserExecutionContext(user_id=f'error_recovery_user_{uuid.uuid4().hex[:8]}', request_id=f'req_{uuid.uuid4().hex[:8]}', thread_id=f'thread_error_{uuid.uuid4().hex[:8]}', run_id=f'run_{uuid.uuid4().hex[:8]}')
            user_session = await self.registry.get_user_session(user_context.user_id)
            failing_agent = BaseAgent(llm_manager=self.llm_manager, context=user_context, name='failing_agent')
            ...
            self.assertEqual(session_metrics['agent_count'], 1, 'Registry must track agents even if they fail')
            reset_report = await self.registry.reset_user_agents(user_context.user_id)
            self.assertEqual(reset_report['status'], 'reset_complete', 'Error recovery through reset must succeed')
            self.assertEqual(reset_report['agents_reset'], 1, 'Reset must report correct number of agents reset')
        except Exception as e:
    ...
    async def test_registry_performance_under_concurrent_real_service_load(self):
        ...
        try:
            ...
            async def user_operation_with_real_services(user_index: int):
                ...
                user_context = UserExecutionContext(user_id=user_id, request_id=f'req_{uuid.uuid4().hex[:8]}', thread_id=f'thread_perf_{user_index}_{uuid.uuid4().hex[:8]}', run_id=f'run_{uuid.uuid4().hex[:8]}')
                session = await self.registry.get_user_session(user_id)
                try:
                    dispatcher = await self.registry.create_tool_dispatcher_for_user(user_context=user_context, websocket_bridge=None, enable_admin_tools=False)
                except Exception:
                    dispatcher = None
                for op_index in range(operations_per_user):
                    agent = BaseAgent(llm_manager=self.llm_manager, context=user_context, name=f'perf_agent_{op_index}')
                    await session.register_agent(f'perf_agent_{op_index}', agent)
                return (user_id, operations_per_user)
            start_time = time.time()
            results = await asyncio.gather(*[user_operation_with_real_services(i) for i in range(concurrent_users)], return_exceptions=True)
            execution_time = time.time() - start_time
            successful_results = [r for r in results if not isinstance(r, Exception)]
            success_rate = len(successful_results) / concurrent_users
            self.assertGreaterEqual(success_rate, 0.8, f'At least 80% of operations must succeed with real services, got {success_rate:.1%}')
            self.assertLess(execution_time, 30.0, f'Real service integration must complete within 30s, took {execution_time:.2f}s')
            self.assertGreaterEqual(len(self.registry._user_sessions), len(successful_results), 'Registry must track successful user sessions')
        except Exception as e:
        ...
        try:
            ...
            for cycle in range(cycles):
                ...
                for i in range(users_per_cycle):
                    ...
                    user_context = UserExecutionContext(user_id=user_id, request_id=f'req_{uuid.uuid4().hex[:8]}', thread_id=f'thread_memory_{cycle}_{i}_{uuid.uuid4().hex[:8]}', run_id=f'run_{uuid.uuid4().hex[:8]}')
                    user_session = await self.registry.get_user_session(user_id)
                    agent = BaseAgent(llm_manager=self.llm_manager, context=user_context, name=f'memory_agent_{i}')
                    await user_session.register_agent('memory_agent', agent)
                    user_ids.append(user_id)
                self.assertEqual(len(self.registry._user_sessions), users_per_cycle, f'Cycle {cycle}: Must create all user sessions')
                ...
                self.assertEqual(len(self.registry._user_sessions), 0, f'Cycle {cycle}: Must clear all sessions')
        except Exception as e:
