...
class IntegrationTestMetrics:
    ...
    def calculate_overall_score(self) -> float:
        ...
        weights = {'supervisor_interaction_score': 0.25, 'tool_dispatcher_integration_score': 0.2, 'state_management_score': 0.15, 'websocket_propagation_score': 0.2, 'error_recovery_score': 0.1, 'performance_score': 0.1}
...
class IntegrationWebSocketCapture:
    ...
    async def send_to_thread(self, thread_id: str, message: Dict[str, Any]) -> bool:
        ...
        async with self._lock:
            event_data = {'thread_id': thread_id, 'message': message, 'event_type': message.get('type', 'unknown'), 'timestamp': time.time(), 'integration_context': {'source': 'actions_agent_integration', 'sequence': len(self.captured_events)}}
    ...
    def get_integration_analysis(self, thread_id: str) -> Dict[str, Any]:
        ...
        if not events:
            return {'total_events': 0, 'integration_score': 0.0, 'missing_patterns': ['all events missing']}
        event_types = [e['event_type'] for e in events]
        event_timeline = [(e['timestamp'], e['event_type']) for e in events]
        required_patterns = ['agent_started', 'agent_thinking', 'tool_executing', 'tool_completed', 'agent_completed']
        ...
        return {'total_events': len(events), 'event_types': event_types, 'pattern_coverage': pattern_coverage, 'coverage_score': coverage_score, 'timing_score': timing_score, 'integration_score': integration_score, 'missing_patterns': missing_patterns, 'duration_seconds': total_duration if len(event_timeline) >= 2 else 0}
...
class RealisticDataGenerator:
    ...
    def create_optimization_result(scenario: str='default') -> OptimizationsResult:
        ...
        scenarios = {'cost_optimization': OptimizationsResult(optimization_type='cost', recommendations=['Reduce compute instance sizes during off-peak hours', 'Implement auto-scaling policies for dynamic workloads', 'Migrate underutilized resources to spot instances', 'Optimize storage tiers based on access patterns'], confidence_score=0.85, estimated_savings={'monthly': 2500, 'annual': 30000}, implementation_complexity='medium'), 'performance_optimization': OptimizationsResult(optimization_type='performance', recommendations=['Implement caching layer for frequently accessed data', 'Optimize database queries with proper indexing', 'Enable CDN for static content delivery', 'Configure load balancing across multiple regions'], confidence_score=0.78, estimated_improvement={'response_time': '40%', 'throughput': '60%'}, implementation_complexity='high'), 'security_optimization': OptimizationsResult(optimization_type='security', recommendations=['Enable multi-factor authentication for all admin accounts', 'Implement network segmentation for sensitive services', 'Update security group rules to follow least privilege', 'Enable encrypted storage for all data at rest'], confidence_score=0.92, risk_reduction={'high_risk': 75, 'medium_risk': 60}, implementation_complexity='low'), 'default': OptimizationsResult(optimization_type='general', recommendations=['Review current setup', 'Implement monitoring'], confidence_score=0.6)}
...
class ActionsAgentSupervisorIntegrationTests:
    ...
    async def test_agent_registry_integration(self):
        ...
        assert hasattr(tool_dispatcher.executor, 'websocket_notifier') or hasattr(tool_dispatcher.executor, 'websocket_bridge'), 'Tool dispatcher not properly enhanced with WebSocket support'
        state = DeepAgentState(user_request='Test agent registry integration', optimizations_result=RealisticDataGenerator.create_optimization_result('cost_optimization'), data_result=RealisticDataGenerator.create_data_analysis_result('cost_analysis'))
        ...
        assert state.action_plan_result is not None, 'Agent should produce action plan through registry integration'
        integration_analysis = self.websocket_capture.get_integration_analysis(thread_id)
        assert integration_analysis['total_events'] > 0, f'No WebSocket events captured during registry integration'
        assert integration_analysis['integration_score'] >= 0.6, f"Integration score too low: {integration_analysis['integration_score']:.2f}. Missing: {integration_analysis['missing_patterns']}"
        assert execution_time < 45.0, f'Registry integration too slow: {execution_time:.2f}s'
        logger.info(f" PASS:  Registry integration: {integration_analysis['integration_score']:.2f} score, {execution_time:.2f}s duration")
    ...
    async def test_execution_engine_coordination(self):
        ...
        state = DeepAgentState(user_request='Comprehensive cost optimization analysis', optimizations_result=RealisticDataGenerator.create_optimization_result('cost_optimization'), data_result=RealisticDataGenerator.create_data_analysis_result('cost_analysis'))
        ...
        integration_analysis = self.websocket_capture.get_integration_analysis(thread_id)
        assert integration_analysis['total_events'] >= 2, f"Insufficient WebSocket events from execution engine: {integration_analysis['total_events']}"
    ...
    async def test_websocket_notifier_propagation(self):
        ...
        agent_registry.set_websocket_manager(self.websocket_capture)
        actions_agent = agent_registry.get_agent('actions')
        state = DeepAgentState(user_request='Multi-faceted optimization with real-time feedback', optimizations_result=RealisticDataGenerator.create_optimization_result('performance_optimization'), data_result=RealisticDataGenerator.create_data_analysis_result('performance_analysis'))
        ...
        for required_event in required_events:
            assert required_event in captured_types, f'Missing critical event: {required_event}. Captured: {captured_types}'
...
class ActionsAgentToolDispatcherIntegrationTests:
    ...
    async def test_tool_dispatcher_enhancement_integration(self):
        ...
        assert enhanced_executor != original_executor, 'Tool dispatcher should be enhanced with WebSocket support'
        assert isinstance(enhanced_executor, UnifiedToolExecutionEngine), f'Enhanced executor should be UnifiedToolExecutionEngine, got {type(enhanced_executor)}'
        assert hasattr(enhanced_executor, 'websocket_notifier') or hasattr(enhanced_executor, 'websocket_bridge'), 'Enhanced executor missing WebSocket integration'
        actions_agent = agent_registry.get_agent('actions')
        state = DeepAgentState(user_request='Test tool dispatcher integration with action planning', optimizations_result=RealisticDataGenerator.create_optimization_result('security_optimization'), data_result=RealisticDataGenerator.create_data_analysis_result('cost_analysis'))
        ...
        if tool_events:
            assert 'tool_executing' in event_types, 'Tool execution should generate tool_executing events'
            tool_executing_count = event_types.count('tool_executing')
            tool_completed_count = event_types.count('tool_completed')
            assert tool_executing_count <= tool_completed_count + 1, f'Unpaired tool events: {tool_executing_count} executing, {tool_completed_count} completed'
    ...
    async def test_state_management_comprehensive(self):
        ...
        initial_state = DeepAgentState(user_request='Comprehensive system optimization with multiple data sources', optimizations_result=RealisticDataGenerator.create_optimization_result('cost_optimization'), data_result=RealisticDataGenerator.create_data_analysis_result('cost_analysis'))
        ...
        assert initial_state.user_request == initial_request, 'Original user request should be preserved'
        assert initial_state.optimizations_result.optimization_type == initial_opt_type, 'Original optimization type should be preserved'
        assert initial_state.data_result.query == initial_data_query, 'Original data query should be preserved'
        assert initial_state.action_plan_result is not None, 'State should be enhanced with action plan result'
        action_plan = initial_state.action_plan_result
        assert isinstance(action_plan, ActionPlanResult), 'Action plan result should be correct type'
        if hasattr(action_plan, 'steps') and action_plan.steps:
            assert len(action_plan.steps) > 0, 'Action plan should contain actionable steps'
        if hasattr(action_plan, 'metadata'):
            metadata = action_plan.metadata
            assert 'generated_from' in metadata or 'user_request' in metadata, 'Action plan metadata should reference input sources'
    ...
    async def test_action_plan_generation_with_real_data(self):
        ...
        tool_dispatcher = ToolDispatcher()
        actions_agent = ActionsToMeetGoalsSubAgent(self.llm_manager, tool_dispatcher)
        scenarios = [{'name': 'Cost Optimization Scenario', 'optimization': RealisticDataGenerator.create_optimization_result('cost_optimization'), 'data': RealisticDataGenerator.create_data_analysis_result('cost_analysis'), 'request': 'Reduce our cloud costs while maintaining performance'}, {'name': 'Performance Optimization Scenario', 'optimization': RealisticDataGenerator.create_optimization_result('performance_optimization'), 'data': RealisticDataGenerator.create_data_analysis_result('performance_analysis'), 'request': 'Improve system performance and response times'}]
        for i, scenario in enumerate(scenarios):
            state = DeepAgentState(user_request=scenario['request'], optimizations_result=scenario['optimization'], data_result=scenario['data'])
            thread_id = f'scenario-{i}-{uuid.uuid4()}'
            start_time = time.time()
            await actions_agent.execute(state, f'run-{thread_id}', stream_updates=True)
            execution_time = time.time() - start_time
            assert state.action_plan_result is not None, f"Scenario '{scenario['name']}' should produce action plan"
            action_plan = state.action_plan_result
            if hasattr(action_plan, 'recommendations') and action_plan.recommendations:
                assert len(action_plan.recommendations) > 0, f"Scenario '{scenario['name']}' should have recommendations"
...
class ActionsAgentErrorRecoveryIntegrationTests:
    ...
    async def test_graceful_degradation_integration(self):
        ...
        agent_registry.set_websocket_manager(self.websocket_capture)
        actions_agent = agent_registry.get_agent('actions')
        degradation_scenarios = [{'name': 'Missing Optimizations', 'state': DeepAgentState(user_request='Plan actions with limited optimization data', data_result=RealisticDataGenerator.create_data_analysis_result('cost_analysis'))}, {'name': 'Missing Data Analysis', 'state': DeepAgentState(user_request='Plan actions with limited analysis data', optimizations_result=RealisticDataGenerator.create_optimization_result('cost_optimization'))}, {'name': 'Minimal Input', 'state': DeepAgentState(user_request='Create action plan with minimal data')}]
        for i, scenario in enumerate(degradation_scenarios):
            ...
            assert state.optimizations_result is not None, f"Scenario '{scenario['name']}' should apply optimization defaults"
            assert state.data_result is not None, f"Scenario '{scenario['name']}' should apply data defaults"
            assert state.action_plan_result is not None, f"Scenario '{scenario['name']}' should generate action plan with defaults"
            integration_analysis = self.websocket_capture.get_integration_analysis(thread_id)
            assert integration_analysis['total_events'] > 0, f"Scenario '{scenario['name']}' should send WebSocket events even with degraded input"
    ...
    async def test_circuit_breaker_integration_behavior(self):
        ...
        tool_dispatcher = ToolDispatcher()
        actions_agent = ActionsToMeetGoalsSubAgent(self.llm_manager, tool_dispatcher)
        assert hasattr(actions_agent, 'reliability_manager'), 'Agent must have reliability manager for circuit breaker'
        reliability_manager = actions_agent.reliability_manager
        assert hasattr(reliability_manager, 'circuit_breaker_config'), 'Reliability manager must have circuit breaker configuration'
        ...
        normal_state = DeepAgentState(user_request='Test circuit breaker normal operation', optimizations_result=RealisticDataGenerator.create_optimization_result('cost_optimization'), data_result=RealisticDataGenerator.create_data_analysis_result('cost_analysis'))
        ...
        assert 'failure_count' in final_cb_status or 'failures' in final_cb_status, 'Circuit breaker should track failures'
        if normal_execution_success:
            assert normal_state.action_plan_result is not None, 'Normal execution should produce results when circuit breaker allows'
    ...
    async def test_concurrent_error_recovery(self):
        ...
        tool_dispatcher = ToolDispatcher()
        agents = [ActionsToMeetGoalsSubAgent(self.llm_manager, tool_dispatcher) for _ in range(3)]
        error_scenarios = [DeepAgentState(user_request=''), DeepAgentState(user_request='Test with no data'), DeepAgentState(user_request='Minimal test', optimizations_result=OptimizationsResult(optimization_type='minimal', recommendations=[], confidence_score=0.1))]
        ...
        async def run_error_scenario(agent, state, scenario_id):
            ...
            try:
                ...
                execution_time = time.time() - start_time
                return {'success': True, 'scenario_id': scenario_id, 'execution_time': execution_time, 'action_plan_generated': state.action_plan_result is not None}
            except Exception as e:
                return {'success': False, 'scenario_id': scenario_id, 'error': str(e)}
        start_time = time.time()
        tasks = [run_error_scenario(agents[i], error_scenarios[i], i) for i in range(3)]
        ...
        assert recovery_rate >= 0.67, f'Concurrent error recovery rate too low: {recovery_rate:.1%} (need  >= 67%)'
...
class ActionsAgentPerformanceIntegrationTests:
    ...
    async def test_realistic_load_performance(self):
        ...
        for i in range(5):
            ...
            state = DeepAgentState(user_request=f'Realistic load test scenario {i}: {scenario_type}', optimizations_result=RealisticDataGenerator.create_optimization_result(scenario_type), data_result=RealisticDataGenerator.create_data_analysis_result(scenario_type.replace('_optimization', '_analysis')))
        ...
        async def execute_load_scenario(state, scenario_id):
            ...
            try:
                ...
                execution_time = time.time() - start_time
                integration_analysis = self.websocket_capture.get_integration_analysis(thread_id)
                return {'success': True, 'scenario_id': scenario_id, 'execution_time': execution_time, 'websocket_events': integration_analysis['total_events'], 'action_plan_quality': len(state.action_plan_result.recommendations) if state.action_plan_result and hasattr(state.action_plan_result, 'recommendations') else 0}
            except Exception as e:
                return {'success': False, 'scenario_id': scenario_id, 'error': str(e)}
        ...
        assert success_rate >= 0.8, f'Performance under load failed: {success_rate:.1%} success rate (need  >= 80%)'
        assert total_execution_time < 100.0, f'Realistic load performance too slow: {total_execution_time:.2f}s (need <100s)'
        execution_times = [r['execution_time'] for r in successful_executions]
        if execution_times:
            avg_execution_time = sum(execution_times) / len(execution_times)
            max_execution_time = max(execution_times)
            assert avg_execution_time < 35.0, f'Average execution time too high: {avg_execution_time:.2f}s'
            assert max_execution_time < 60.0, f'Max execution time too high: {max_execution_time:.2f}s'
        ...
        assert events_per_second > 0.5, f'WebSocket event rate too low: {events_per_second:.2f} events/s'
...
class ActionsAgentIntegrationComprehensiveTests:
    ...
    async def test_complete_integration_compliance_suite(self):
        ...
        try:
            ...
            state = DeepAgentState(user_request='Integration compliance test', optimizations_result=RealisticDataGenerator.create_optimization_result('cost_optimization'), data_result=RealisticDataGenerator.create_data_analysis_result('cost_analysis'))
            ...
            error_state = DeepAgentState(user_request='')
            try:
            ...
            metrics.performance_score = 0.75
