...
class TestUserContextExtractionSSOT(SSotAsyncTestCase):
    ...
    async def _get_auth_token_and_user_context(self):
        ...
        login_payload = {'email': self.staging_test_email, 'password': self.staging_test_password, 'include_full_context': True}
        try:
            async with self.http_session.post(f'{self.staging_auth_service_url}/auth/login', json=login_payload, timeout=30) as response:
                if response.status == 200:
                    ...
                    logger.info(f"Reference user context: user_id={self.auth_service_user_context.get('user_id')}, email={self.auth_service_user_context.get('email')}, tier={self.auth_service_user_context.get('tier')}")
    ...
    async def _test_user_context_across_endpoints(self):
        ...
        test_endpoints = [{'path': '/api/v1/user/profile', 'context_fields': ['user_id', 'email', 'tier'], 'description': 'User profile endpoint'}, {'path': '/api/v1/user/preferences', 'context_fields': ['user_id'], 'description': 'User preferences endpoint'}, {'path': '/api/v1/threads', 'context_fields': ['user_id'], 'description': 'User threads endpoint'}, {'path': '/api/v1/messages/recent', 'context_fields': ['user_id'], 'description': 'Recent messages endpoint'}, {'path': '/api/v1/user/context', 'context_fields': ['user_id', 'email', 'tier', 'permissions'], 'description': 'User context endpoint'}]
        self.endpoint_contexts = []
        for endpoint_config in test_endpoints:
            try:
                async with self.http_session.get(f"{self.staging_backend_url}{endpoint_config['path']}", headers=headers, timeout=30) as response:
                    if response.status == 200:
                        ...
                        if context_data:
                            self.endpoint_contexts.append({'endpoint': endpoint_config['path'], 'description': endpoint_config['description'], 'context': context_data, 'status': 'success'})
    ...
    async def _validate_user_context_consistency(self):
        ...
        user_ids = [ctx['context'].get('user_id') for ctx in self.endpoint_contexts if 'user_id' in ctx['context']]
        if user_ids:
            unique_user_ids = set(user_ids)
            assert len(unique_user_ids) == 1, f'User ID inconsistent across endpoints: {unique_user_ids}'
            reference_user_id = self.auth_service_user_context.get('user_id')
            if reference_user_id:
                backend_user_id = user_ids[0]
                assert backend_user_id == reference_user_id, f"Backend user ID ({backend_user_id}) doesn't match auth service ({reference_user_id})"
            logger.info(f'User ID consistent across {len(user_ids)} endpoints: {user_ids[0]}')
        emails = [ctx['context'].get('email') for ctx in self.endpoint_contexts if 'email' in ctx['context']]
        if emails:
            unique_emails = set(emails)
            assert len(unique_emails) == 1, f'Email inconsistent across endpoints: {unique_emails}'
            reference_email = self.auth_service_user_context.get('email')
            if reference_email:
                assert emails[0] == reference_email, f"Backend email doesn't match auth service reference"
            logger.info(f'Email consistent across {len(emails)} endpoints')
        tiers = [ctx['context'].get('tier') for ctx in self.endpoint_contexts if 'tier' in ctx['context']]
        if tiers:
            unique_tiers = set(tiers)
            assert len(unique_tiers) == 1, f'Tier inconsistent across endpoints: {unique_tiers}'
        ...
        try:
            async with self.http_session.get(f'{self.staging_backend_url}/api/v1/user/context', headers=headers, timeout=30) as response:
                if response.status == 200:
                    ...
                    async with self.http_session.get(f'{self.staging_backend_url}/api/v1/user/profile', headers=headers, timeout=30) as profile_response:
    ...
    async def _update_user_context_via_auth_service(self) -> bool:
        ...
        headers = {'Authorization': f'Bearer {self.auth_token}'}
        update_payload = {'preferences': {'theme': 'dark', 'notifications': True, 'test_update_timestamp': time.time()}}
        try:
            async with self.http_session.put(f'{self.staging_auth_service_url}/auth/user/preferences', headers=headers, json=update_payload, timeout=30) as response:
                if response.status == 200:
    ...
    async def _get_permissions_from_auth_service(self) -> Set[str]:
        ...
        headers = {'Authorization': f'Bearer {self.auth_token}'}
        try:
            async with self.http_session.get(f'{self.staging_auth_service_url}/auth/user/permissions', headers=headers, timeout=30) as response:
                if response.status == 200:
    ...
    async def _get_permissions_from_backend(self) -> Set[str]:
        ...
        permission_endpoints = ['/api/v1/user/permissions', '/api/v1/user/context', '/api/v1/user/profile']
    ...
    async def _validate_permission_consistency(self, auth_perms: Set[str], backend_perms: Set[str]):
        ...
        if auth_perms and backend_perms:
            ...
            assert len(common_perms) > 0 or len(auth_perms) == 0, 'Backend permissions should align with auth service'
    ...
    async def _end_auth_service_session(self) -> bool:
        ...
        headers = {'Authorization': f'Bearer {self.auth_token}'}
        try:
            async with self.http_session.post(f'{self.staging_auth_service_url}/auth/logout', headers=headers, timeout=30) as response:
                if response.status == 200:
    ...
    async def _verify_backend_context_invalidated(self):
        ...
        try:
            async with self.http_session.get(f'{self.staging_backend_url}/api/v1/user/context', headers=headers, timeout=30) as response:
                assert response.status == 401, 'Backend should reject invalidated session'
