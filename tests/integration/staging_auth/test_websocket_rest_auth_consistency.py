...
class TestWebSocketRESTAuthConsistency(SSotAsyncTestCase):
    ...
    async def asyncSetUpClass(cls):
        ...
        cls.staging_websocket_url = os.getenv(

            "STAGING_WEBSOCKET_URL",

            "wss://websocket-staging.netra-apex.com"
    ...
    async def test_websocket_rest_same_auth_token_validation(self):
        ...
        # Step 2: Validate token works with REST API
        ...
        # Step 3: Validate same token works with WebSocket
        ...
        # Step 4: Compare user data consistency
    ...
    async def _validate_cross_protocol_consistency(self, rest_data: Dict, websocket_data: Dict):
        ...
        if "user_id" in rest_data and "user_id" in websocket_data:
            ...
            assert rest_data["user_id"] == websocket_data["user_id"], \

                "User ID consistent between REST and WebSocket"



        # Compare email if available

        if "email" in rest_data and "email" in websocket_data:

            assert rest_data["email"] == websocket_data["email"], \

                "Email consistent between REST and WebSocket"



        # Compare tier if available

        if "tier" in rest_data and "tier" in websocket_data:

            assert rest_data["tier"] == websocket_data["tier"], \

                "Tier consistent between REST and WebSocket"
    ...
    async def _get_websocket_permissions(self) -> Dict[str, Any]:
        ...
        # Request permissions via WebSocket

        permissions_request = {

            "type": "get_permissions",

            "request_id": "permission-check-123"
        ...
        # Wait for permissions response

        try:

            permissions_msg = await asyncio.wait_for(
    ...
    async def _validate_permission_consistency(self, rest_perms: Dict, ws_perms: Dict):
        ...
        # Compare permission lists if available
        ...
        if rest_permission_list and ws_permission_list:
            ...
            assert rest_sorted == ws_sorted, \

                f"Permission mismatch: REST={rest_sorted}, WebSocket={ws_sorted}"
    ...
    async def _test_rest_session(self) -> Dict[str, Any]:
        ...
        try:
            ...
            async with self.http_session.get(
                ...
                f"{self.staging_backend_url}/api/v1/session/info",
    ...
    async def _test_websocket_session(self) -> Dict[str, Any]:
        ...
        # Request session info via WebSocket

        session_request = {

            "type": "get_session_info",

            "request_id": "session-check-456"
        ...
        try:
            ...
            session_msg = await asyncio.wait_for(
    ...
    async def _validate_session_consistency(self, rest_session: Dict, ws_session: Dict):
        ...
        # Compare session IDs if available
        ...
        if rest_session_id and ws_session_id:
            ...
            assert rest_session_id == ws_session_id, \

                f"Session ID mismatch: REST={rest_session_id}, WebSocket={ws_session_id}"
        ...
        # Compare session expiration if available
        ...
        if rest_expires and ws_expires:
            ...
            assert rest_expires == ws_expires, \

                "Session expiration mismatch between protocols"
    ...
    async def test_auth_token_refresh_consistency(self):
        ...
        # Verify refreshed token works with REST
        ...
        # Verify refreshed token works with WebSocket
    ...
    async def _refresh_token_via_rest(self):
        ...
        try:
            ...
            async with self.http_session.post(
                ...
                f"{self.staging_auth_service_url}/auth/refresh",
    ...
    async def _test_refreshed_token_websocket(self):
        ...
        # Establish new connection with refreshed token
        ...
        assert connection_data["type"] == "connection_established", \
            ...
            "WebSocket accepts refreshed token"
...
if __name__ == "__main__":
    ...
    # Run with: python -m pytest tests/integration/staging_auth/test_websocket_rest_auth_consistency.py -v
