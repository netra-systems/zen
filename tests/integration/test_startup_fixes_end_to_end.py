...
class TestStartupFixesEndToEnd:
    ...
    async def test_complete_startup_fixes_flow_success(self):
        ...
        with patch.multiple(
            ...
            assert result['total_fixes'] == 5, f"Expected 5 fixes, got {result['total_fixes']}"
            assert len(result['successful_fixes']) == 5, f"Expected 5 successful fixes, got {len(result['successful_fixes'])}"
            assert len(result['failed_fixes']) == 0, f"Expected 0 failed fixes, got {result['failed_fixes']}"
    ...
    return await startup_fixes_validator.validate_all_fixes_applied(level, timeout)
                ...
                return ValidationResult(
                    ...
                    critical_failures=[],
                    warnings=[f"Still waiting for {4 - completion_call_count['count']} fixes"],
                    details={},
                    duration=0.1
                )
    ...
    async def test_performance_characteristics(self):
        ...
        with patch.multiple(
            ...
            assert duration < 10.0, f"Startup fixes took too long: {duration:.2f}s"
            
            # Should complete within expected time (under 5 seconds for fast scenario)
            assert duration < 5.0, f"Startup fixes slower than expected: {duration:.2f}s"
