...
class WebSocketMessageRoutingIntegrationTests(SSotAsyncTestCase):
    ...
    def _create_test_user_context(self, user_suffix: str = None) -> UserExecutionContext:
        ...
        suffix = user_suffix or f"routing_{uuid.uuid4().hex[:8]}"
    ...
    async def _simulate_event_routing(self, event: WebSocketEventMessage, target_connection_id: str) -> Dict[str, Any]:
        ...
        if target_connection_id not in self.mock_connections:
            raise ValueError(f"Connection {target_connection_id} not found")
    ...
    def _create_test_agent_event(self, event_type: str, user_context: UserExecutionContext, **kwargs) -> WebSocketEventMessage:
        ...
        event_data = {
            ...
            "data": {
                "message": f"Test {event_type} event for routing validation",
    ...
    async def test_basic_event_routing_pipeline(self):
        ...
        for i, event in enumerate(test_events):
            ...
            assert routed_event["routing_success"], f"Event {i+1} routing failed"
            assert routed_event["event"]["type"] == event.event_type, f"Event type mismatch for event {i+1}"
            assert routed_event["event"]["user_id"] == user_context.user_id, f"User ID mismatch for event {i+1}"
        ...
        assert len(connection_events) == len(test_events), f"Connection missing events: expected {len(test_events)}, got {len(connection_events)}"
        
        # Validate routing performance
        avg_routing_time = sum(routing_times) / len(routing_times)
        max_routing_time = max(routing_times)
        
        assert avg_routing_time < 0.1, f"Average routing time too slow: {avg_routing_time * 1000:.1f}ms"
        ...
        assert routed_event_types == expected_order, f"Event ordering mismatch: expected {expected_order}, got {routed_event_types}"
    ...
    async def test_multi_user_routing_isolation(self):
        ...
        for i, user_context in enumerate(users):
            events = [
                self._create_test_agent_event("agent_started", user_context, data={"user_marker": f"user_{i+1}"}),
                self._create_test_agent_event("agent_thinking", user_context, data={"user_marker": f"user_{i+1}"}),
                self._create_test_agent_event("agent_completed", user_context, data={"user_marker": f"user_{i+1}"})
            ]
        ...
        for user_context in users:
            ...
            assert len(connection_events) == expected_count, f"User {user_id} event count mismatch: expected {expected_count}, got {len(connection_events)}"
            
            # Check user isolation
            for routed_event in connection_events:
                event_data = routed_event["event"]
                assert event_data["user_id"] == user_id, f"User isolation violation: event user {event_data['user_id']} != connection user {user_id}"
                assert event_data["thread_id"] == user_context.thread_id, f"Thread isolation violation for user {user_id}"
                
                # Check user markers
                user_marker = event_data["data"].get("user_marker", "")
                assert user_marker in f"user_{users.index(user_context) + 1}", f"User marker contamination for {user_id}: {user_marker}"
            ...
            for other_user in users:
                if other_user.user_id != user_id:
                    ...
                    for routed_event in connection["received_events"]:
                        ...
                        assert event_user == user_id, f"Cross-contamination: user {user_id} received event from user {event_user}"
        
        # Validate concurrent performance
        events_per_second = len(all_events) / total_routing_time
        assert events_per_second > 100, f"Concurrent routing too slow: {events_per_second:.1f} events/sec"
    ...
    async def test_event_ordering_guarantees(self):
        ...
        for i, routed_event in enumerate(received_events_sorted):
            ...
            assert actual_sequence_id == expected_sequence_id, f"Sequence order violation: position {i+1} has sequence_id {actual_sequence_id}, expected {expected_sequence_id}"
        ...
        assert initialization_phase == "initialization", f"First phase must be initialization, got {initialization_phase}"
        assert completion_phase == "completion", f"Last phase must be completion, got {completion_phase}"
        ...
        for tool_name, pair in tool_pairs.items():
            ...
            assert executing_pos is not None, f"Tool {tool_name} missing executing event"
            assert completed_pos is not None, f"Tool {tool_name} missing completed event"
            assert executing_pos < completed_pos, f"Tool {tool_name} completed event ({completed_pos}) before executing event ({executing_pos})"
    ...
    async def test_connection_health_and_failover(self):
        ...
        async def route_with_failover(event, primary_conn_id, backup_conn_id):
            ...
            try:
                ...
                raise Exception(f"Failover routing failed: {e}")
        ...
        assert total_routed_events == total_expected_events, f"Event count mismatch: expected {total_expected_events}, got {total_routed_events}"
    ...
    async def test_routing_performance_under_load(self):
        ...
        assert avg_routing_time < 0.1, f"Average routing time too slow: {avg_routing_time * 1000:.1f}ms > 100ms"
        assert events_per_second > 50, f"Throughput too low: {events_per_second:.1f} events/sec < 50 events/sec"
        ...
        for user_context in load_users:
            ...
            for routed_event in user_routed_events:
                event_user = routed_event["event"]["user_id"]
                assert event_user == user_id, f"User isolation failure under load: {event_user} != {user_id}"
