...
class TestUserContextFactoryIsolation(SSotAsyncTestCase):
    ...
    def setup_method(self, method):
        ...
        try:
            ...
            self.context_manager.create_context = AsyncMock(return_value=UserExecutionContext.from_request_supervisor(user_id='mock_user', thread_id='mock_thread', run_id='mock_run'))
    ...
    async def test_user_context_factory_shared_state_violation(self):
        ...
        factory1 = ExecutionEngineFactory(websocket_bridge=create_agent_websocket_bridge())
        factory2 = ExecutionEngineFactory(websocket_bridge=create_agent_websocket_bridge())
        self.factory_instances.extend([factory1, factory2])
        user1_context = UserExecutionContext.from_request_supervisor(user_id='isolation_user_001', thread_id='isolation_thread_001', run_id='isolation_run_001')
        user2_context = UserExecutionContext.from_request_supervisor(user_id='isolation_user_002', thread_id='isolation_thread_002', run_id='isolation_run_002')
    ...
    async def test_memory_leaks_in_user_context_creation(self):
        ...
        try:
            for i in range(contexts_to_create):
                context = UserExecutionContext.from_request_supervisor(user_id=f'memory_test_user_{i:03d}', thread_id=f'memory_test_thread_{i:03d}', run_id=f'memory_test_run_{i:03d}')
            ...
            leak_threshold = contexts_to_create * 0.1
    ...
    async def test_race_conditions_concurrent_factory_usage(self):
        ...
        factory = ExecutionEngineFactory(websocket_bridge=create_agent_websocket_bridge())
        ...
        async def worker_create_contexts(worker_id: int) -> Dict[str, Any]:
            ...
            worker_results = {'worker_id': worker_id, 'contexts_created': 0, 'errors': [], 'duplicate_ids': [], 'timing_issues': []}
            for request_id in range(requests_per_worker):
                try:
                    start_time = time.time()
                    context = UserExecutionContext.from_request_supervisor(user_id=f'race_user_{worker_id:02d}_{request_id:03d}', thread_id=f'race_thread_{worker_id:02d}_{request_id:03d}', run_id=f'race_run_{worker_id:02d}_{request_id:03d}')
                    ...
                    if processing_time > 0.1:
                        worker_results['timing_issues'].append({'request_id': request_id, 'processing_time': processing_time})
                    ...
                    worker_results['errors'].append({'request_id': request_id, 'error': str(e)})
    ...
    async def test_context_validation_bypasses_under_load(self):
        ...
        async def create_context_with_validation_check(context_data: Dict[str, Any]) -> Dict[str, Any]:
            ...
            result = {'context_data': context_data, 'creation_success': False, 'validation_bypassed': False, 'error': None}
            try:
                if context_data.get('is_invalid'):
                    context = UserExecutionContext.from_request_supervisor(user_id=context_data.get('user_id', ''), thread_id=context_data.get('thread_id', ''), run_id=context_data.get('run_id', ''))
                else:
                    context = UserExecutionContext.from_request_supervisor(user_id=context_data['user_id'], thread_id=context_data['thread_id'], run_id=context_data['run_id'])
        ...
        for i in range(invalid_contexts_to_create):
            context_requests.append({'user_id': '', 'thread_id': f'invalid_thread_{i:03d}', 'run_id': f'invalid_run_{i:03d}', 'is_invalid': True})
        for i in range(valid_contexts_to_create):
            context_requests.append({'user_id': f'valid_user_{i:03d}', 'thread_id': f'valid_thread_{i:03d}', 'run_id': f'valid_run_{i:03d}', 'is_invalid': False})
        batch_tasks = []
