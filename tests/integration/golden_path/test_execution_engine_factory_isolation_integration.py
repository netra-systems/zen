...
class ExecutionEngineFactoryIsolationIntegrationTests(SSotAsyncTestCase):
    ...
    async def async_setup_method(self, method):
        ...
        self.execution_factory = ExecutionEngineFactory(websocket_bridge=self.websocket_bridge)
        for i in range(self.num_test_users):
            context = UserExecutionContext(user_id=str(uuid.uuid4()), thread_id=str(uuid.uuid4()), run_id=str(uuid.uuid4()), agent_context={'user_index': i, 'tier': 'enterprise' if i < 3 else 'free', 'test_data': f'user_{i}_test_data'})
    ...
    async def test_execution_engine_factory_user_isolation_creation(self):
        ...
        """
        engines = []
        for i, context in enumerate(self.test_user_contexts[:5]):
        ...
        for i, engine in enumerate(engines):
            ...
            state_value = {'user_index': i, 'sensitive_data': f'secret_data_for_user_{i}', 'execution_count': 0, 'user_preferences': {'theme': f'theme_{i}', 'language': 'en'}}
    ...
    async def test_concurrent_engine_creation_and_execution(self):
        ...
        async def create_and_execute_engine(user_index: int) -> Dict[str, Any]:
            context = UserExecutionContext(user_id=str(uuid.uuid4()), thread_id=str(uuid.uuid4()), run_id=str(uuid.uuid4()), agent_context={'user_index': user_index, 'concurrent_test': True})
            ...
            return {'user_index': user_index, 'engine_id': id(engine), 'user_id': context.user_id, 'creation_time': creation_time, 'operations': operation_results, 'all_operations_successful': all((op['success'] for op in operation_results))}
        concurrent_start = time.time()
        concurrent_tasks = [create_and_execute_engine(i) for i in range(num_concurrent_users)]
        ...
        assert user_success_rate >= 0.9, f'User success rate too low: {user_success_rate:.2%}'
    ...
    async def test_engine_memory_isolation_and_cleanup(self):
        ...
        for i, engine in enumerate(created_engines[:5]):
        ...
        for i, engine in enumerate(remaining_engines[:3]):
            engine_state = engine.get_agent_state('large_state')
    ...
    async def test_factory_performance_under_load(self):
        ...
        async def create_engine_batch(batch_index: int) -> List[Dict[str, Any]]:
            ...
            for engine_index in range(engines_per_batch):
                context = UserExecutionContext(user_id=str(uuid.uuid4()), thread_id=str(uuid.uuid4()), run_id=str(uuid.uuid4()), agent_context={'batch': batch_index, 'engine': engine_index})
                creation_start = time.time()
                engine = await self.execution_factory.create_for_user(context)
                creation_time = time.time() - creation_start
                self.created_engines.append(engine)
        ...
        batch_tasks = [create_engine_batch(i) for i in range(num_concurrent_batches)]
    ...
    async def test_factory_error_handling_and_recovery(self):
        ...
        failing_factory = ExecutionEngineFactory(websocket_bridge=failing_bridge)
        ...
        async def resilient_engine_creation(user_index: int, max_retries=3) -> Dict[str, Any]:
            ...
            context = UserExecutionContext(user_id=str(uuid.uuid4()), thread_id=str(uuid.uuid4()), run_id=str(uuid.uuid4()), agent_context={'user_index': user_index})
            for attempt in range(max_retries):
                try:
                    ...
                    self.created_engines.append(engine)
                    return {'user_index': user_index, 'success': True, 'attempts': attempt + 1, 'engine_id': id(engine), 'recovery': attempt > 0}
                except Exception as e:
                    if attempt == max_retries - 1:
                        return {'user_index': user_index, 'success': False, 'attempts': attempt + 1, 'error': str(e), 'recovery': False}
                    await asyncio.sleep(0.01 * 2 ** attempt)
            return {'user_index': user_index, 'success': False, 'attempts': max_retries, 'error': 'Max retries exceeded', 'recovery': False}
        num_test_users = 15
        error_handling_tasks = [resilient_engine_creation(i) for i in range(num_test_users)]
    ...
    def _get_memory_usage(self) -> int:
        ...
        try:
            ...
            return len(self.created_engines) * 1024
        except Exception:
            return 1024 * 1024
