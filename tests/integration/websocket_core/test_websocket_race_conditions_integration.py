...
class TestWebSocketRaceConditionsIntegration(SSotAsyncTestCase):
    ...
    async def asyncSetUp(self):
        ...
        # Create WebSocket managers for different users
    ...
    async def test_integration_multi_user_concurrent_disconnect_race(self):
        ...
        for user_key, manager in self.websocket_managers.items():
            ...
            mock_connection = WebSocketConnection(
                ...
                connection_id=ConnectionID(f"integration_conn_{user_key}")
            )
        ...
        # Prepare agent events for all users
        golden_path_events = [
            {"type": "agent_started", "agent": "supervisor", "user_count": len(connections)},
            {"type": "agent_thinking", "content": "Processing multi-user request..."},
            {"type": "tool_executing", "tool": "multi_user_analyzer"},
            {"type": "tool_completed", "tool": "multi_user_analyzer", "status": "success"},
            {"type": "agent_completed", "agent": "supervisor", "result": "all_users_processed"}
        ]
            ...
            for event_idx, event in enumerate(golden_path_events):
                ...
                for user_key, conn_data in connections.items():
                    task = asyncio.create_task(
                        self._send_event_with_error_tracking(
                            conn_data['manager'],
                            conn_data['connection_id'],
                            {**event, "event_idx": event_idx, "target_user": user_key}
                        )
                    )
        ...
        async def simulate_concurrent_user_disconnects():
            ...
            await asyncio.sleep(0.05)  # Let some events start

            # Disconnect users concurrently at different times
            disconnect_tasks = []
            for i, (user_key, conn_data) in enumerate(connections.items()):
                delay = i * 0.01  # Stagger disconnects slightly
                task = asyncio.create_task(
                    self._simulate_user_disconnect(user_key, conn_data, delay)
                )
        ...
        # Run event sending and disconnects concurrently
        ...
        # This test expects race conditions during concurrent disconnects
        # DESIGNED TO FAIL initially to reproduce the bug
        assert len(send_errors) > 0, (
            f"Expected race condition errors during concurrent disconnects. "
            f"Successful sends: {successful_sends}/{total_expected_sends}, "
            f"Send errors: {len(send_errors)}, Disconnect errors: {len(disconnect_errors)}"
        )

        logger.error(f"🔥 MULTI-USER RACE CONDITIONS REPRODUCED: "
                    f"{len(send_errors)} send errors, {len(disconnect_errors)} disconnect errors")
    ...
    async def test_integration_websocket_manager_state_corruption_race(self):
        ...
        # Create multiple connections for the same user (e.g., multiple browser tabs)
        for i in range(5):
            mock_websocket = AsyncMock()
            mock_connection = WebSocketConnection(
                websocket=mock_websocket,
                user_id=self.user_contexts['user_0'].user_id,
                connection_id=ConnectionID(f"state_test_conn_{i}")
            )
        ...
        # Track state corruption indicators
        state_errors = []
        operation_results = []
        ...
        async def perform_concurrent_operations():
            ...
            for i in range(20):  # 20 concurrent operations
                ...
                if operation_type == 0:  # Send message
                    task = asyncio.create_task(
                        self._send_message_with_state_check(
                            manager, conn_data, f"state_test_msg_{i}"
                        )
                    )
                elif operation_type == 1:  # Add/remove connection
                    task = asyncio.create_task(
                        self._toggle_connection(manager, conn_data, i)
                    )
                elif operation_type == 2:  # Query connections
                    task = asyncio.create_task(
                        self._query_manager_state(manager, f"query_{i}")
                    )
                else:  # Bulk operation
                    task = asyncio.create_task(
                        self._bulk_send_operation(manager, connections_data, i)
                    )
        ...
        # Check for inconsistencies in state snapshots
        state_inconsistencies = self._analyze_state_snapshots(manager_state_snapshots)

        # This test expects state corruption during concurrent operations
        # DESIGNED TO FAIL initially to reproduce the bug
        assert (
            len(state_errors) > 0 or
            len(state_inconsistencies) > 0
        ), (
            f"Expected state corruption indicators but found none. "
            f"State errors: {len(state_errors)}, "
            f"State inconsistencies: {len(state_inconsistencies)}, "
            f"Successful operations: {len(operation_results)}"
        )

        logger.error(f"🔥 STATE CORRUPTION RACE CONDITIONS REPRODUCED: "
                    f"{len(state_errors)} errors, {len(state_inconsistencies)} inconsistencies")
    ...
    async def test_integration_golden_path_agent_event_race_conditions(self):
        ...
        # Use multiple users for Golden Path testing
        ...
        for user_key in ['user_0', 'user_1']:
            ...
            mock_connection = WebSocketConnection(
                websocket=mock_websocket,
                user_id=self.user_contexts[user_key].user_id,
                connection_id=ConnectionID(f"golden_path_{user_key}")
            )

            connection_id = f"golden_path_{user_key}_conn"
            golden_path_connections[user_key] = {
                'connection_id': connection_id,
                'connection': mock_connection,
                'manager': manager
            }
        ...
        # Golden Path critical event sequence
        critical_agent_sequence = [
            {"type": "agent_started", "agent": "supervisor", "critical": True},
            {"type": "agent_thinking", "content": "Analyzing business requirements...", "critical": True},
            {"type": "tool_executing", "tool": "business_analyzer", "critical": True},
            {"type": "tool_completed", "tool": "business_analyzer", "result": {"revenue_impact": 50000}, "critical": True},
            {"type": "agent_thinking", "content": "Generating optimization recommendations...", "critical": True},
            {"type": "tool_executing", "tool": "optimization_engine", "critical": True},
            {"type": "tool_completed", "tool": "optimization_engine", "result": {"optimizations": 15}, "critical": True},
            {"type": "agent_completed", "agent": "supervisor", "result": "mission_critical_complete", "critical": True}
        ]

        # Track Golden Path delivery results
        ...
        async def deliver_golden_path_events():
            ...
            for event_idx, event in enumerate(critical_agent_sequence):
                # Send to all Golden Path users
                send_tasks = []
                for user_key, conn_data in golden_path_connections.items():
                    task = asyncio.create_task(
                        self._send_critical_event_with_tracking(
                            conn_data['manager'],
                            conn_data['connection_id'],
                            {**event, "event_sequence": event_idx, "user": user_key}
                        )
                    )
                    send_tasks.append((user_key, task))

                # Wait for critical event delivery
                for user_key, task in send_tasks:
                    try:
                        result = await task
                        if not result:
                            critical_event_failures.append(
                                f"User {user_key}, Event {event_idx} ({event['type']}): Delivery failed"
                            )
                    except Exception as e:
                        critical_event_failures.append(
                            f"User {user_key}, Event {event_idx} ({event['type']}): {e}"
                        )

                await asyncio.sleep(0.03)  # Golden Path event timing

        async def simulate_golden_path_disruptions():
            """Simulate disruptions during Golden Path execution"""
            await asyncio.sleep(0.08)  # Let Golden Path start

            # Simulate user disconnecting during critical business operation
            ...
            # Make the user's connection fail during critical events
            conn_data['connection'].websocket.send_json = AsyncMock(
                side_effect=ConnectionError("Golden Path user disconnected during critical operation")
            )

            logger.warning(f"Simulating {user_to_disconnect} disconnect during Golden Path")

        # Execute Golden Path with disruptions
        ...
        # Analyze Golden Path race condition impact
        critical_events_affected = len(critical_event_failures)
        business_impact_events = [
            failure for failure in critical_event_failures
            if any(critical_type in failure for critical_type in
                   ['agent_started', 'agent_completed', 'business_analyzer', 'optimization_engine'])
        ]

        # This test expects Golden Path race conditions to affect business-critical events
        # DESIGNED TO FAIL initially to reproduce the bug
        assert critical_events_affected > 0, (
            f"Expected Golden Path race conditions but found none. "
            f"Critical event failures: {critical_events_affected}, "
            f"Business impact events: {len(business_impact_events)}"
        )

        # Verify business-critical events were specifically affected
        assert len(business_impact_events) > 0, (
            f"Expected business-critical events to be affected. "
            f"All failures: {critical_event_failures}"
        )

        logger.error(f"🔥 GOLDEN PATH RACE CONDITIONS REPRODUCED: "
                    f"{critical_events_affected} critical failures, "
                    f"{len(business_impact_events)} business-impact events affected")

        # Clean up Golden Path connections
        for user_key, conn_data in golden_path_connections.items():
            try:
                await conn_data['manager'].remove_connection(conn_data['connection_id'])
            except Exception as cleanup_error:
    ...
    async def _simulate_user_disconnect(self, user_key, conn_data, delay):
        ...
        conn_data['connection'].websocket.send_json = AsyncMock(
            side_effect=ConnectionError(f"User {user_key} network disconnected")
        )
    ...
    async def _query_manager_state(self, manager, query_id):
        ...
        try:
            # Get current connections count and state
            state_info = {
                'query_id': query_id,
                'timestamp': time.time(),
                'has_get_connection': hasattr(manager, 'get_connection'),
    ...
    async def _take_manager_state_snapshot(self, manager, snapshot_id):
        ...
        return {
            ...
            'timestamp': time.time(),
            'manager_type': type(manager).__name__,
            'has_connections': hasattr(manager, '_connections') if hasattr(manager, '_connections') else False
        }
    ...
    def _analyze_state_snapshots(self, snapshots):
        ...
        # Check for manager type changes (should be stable)
    ...
    async def _send_critical_event_with_tracking(self, manager, connection_id, event):
        ...
        try:
            ...
            logger.debug(f"Critical event {event['type']} to {connection_id}: "
                        f"{'SUCCESS' if result else 'FAILED'} in {end_time - start_time:.3f}s")
    ...
    async def asyncTearDown(self):
        ...
        # Clean up all WebSocket managers
        for user_key, manager in self.websocket_managers.items():
            try:
