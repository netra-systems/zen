...
class MockAgent(BaseAgent):
    ...
    async def execute(self, state: DeepAgentState, run_id: str) -> DeepAgentState:
        ...
        execution_id = f"exec_{self._execution_count}_{int(time.time() * 1000)}"
        ...
        if hasattr(self, '_websocket_adapter') and self._websocket_adapter:
            try:
                ...
                logger.warning(f"WebSocket emission failed in mock agent: {e}")
        ...
        result_state = DeepAgentState(
            user_message=f"Processed by {self.name} for user {getattr(self, 'user_id', 'unknown')}",
            thread_id=getattr(state, 'thread_id', 'unknown'),
            additional_context={
...
class TestAgentClassRegistryIsolation:
    ...
    async def test_concurrent_registry_reads(self):
        ...
        for i in range(10):
            ...
            registry.register(agent_name, MockAgent, f"Test agent {i}")
        ...
        async def concurrent_reader(reader_id: int) -> Dict[str, Any]:
            ...
            for _ in range(100):
                ...
                agent_name = f"agent_{reader_id % 10}"
        ...
        assert total_reads == 20 * 100  # 20 readers × 100 reads each
        
        for results in all_results:
            for result in results:
                assert result['class_found'], f"Agent class not found: {result}"
                assert result['info_found'], f"Agent info not found: {result}"
                assert result['class_correct'], f"Incorrect agent class: {result}"
...
class TestAgentInstanceFactoryIsolation:
    ...
    @pytest.mark.asyncio
    async def test_user_execution_context_creation_and_cleanup(self, 
                                                              mock_agent_registry, 
    ...
    @pytest.mark.asyncio
    async def test_agent_instance_creation_isolation(self,
                                                    mock_agent_registry,
                                                    mock_websocket_bridge, 
                                                    test_database_sessions):
    ...
    async def test_websocket_event_isolation(self,
        ...
        for i, (agent, context) in enumerate(zip(agents, contexts)):
            state = DeepAgentState(
                user_message=f"Message from user {i:03d}",
                thread_id=f"thread_{i:03d}"
            )
            tasks.append(agent.execute(state, f"run_{i:03d}"))
        ...
        for i in range(3):
            ...
            for event in user_events:
                assert event['thread_id'] == thread_id, f"Event thread mismatch: {event}"
    ...
    @pytest.mark.asyncio
    async def test_concurrent_user_execution_stress(self,
                                                   mock_agent_registry,
                                                   mock_websocket_bridge,
                                                   test_database_sessions):
        ...
        async def execute_user_workflow(user_index: int) -> ConcurrentUserTestResult:
            ...
            thread_id = f"stress_thread_{user_index:03d}"
            ...
            try:
                ...
                for exec_index in range(executions_per_user):
                    run_id = f"stress_run_{user_index:03d}_{exec_index:03d}"
                    ...
                    async with factory.user_execution_scope(
                        ...
                        state = DeepAgentState(
                            user_message=f"Stress test message {exec_index} from {user_id}",
                            thread_id=thread_id
                        )
                        ...
                        if user_id not in result.user_message:
                            success = False
                            error_message = f"User ID not found in result: {result.user_message}"
                        
                        if result.additional_context.get("agent_user_id") != user_id:
                            success = False
                            error_message = f"Agent user ID mismatch: expected {user_id}, got {result.additional_context.get('agent_user_id')}"
                ...
                for event in mock_websocket_bridge.events_sent:
                    if event['run_id'].startswith(f"stress_run_{user_index:03d}"):
                        websocket_events.append(event)
                
            except Exception as e:
                success = False
                error_message = str(e)
                logger.error(f"Error in user {user_index} workflow: {e}")
        ...
        logger.info(f"Starting stress test with {num_concurrent_users} concurrent users")
        start_time = time.time()
        ...
        # Analyze results
        successful_results = [r for r in results if isinstance(r, ConcurrentUserTestResult) and r.success]
        failed_results = [r for r in results if isinstance(r, Exception) or (isinstance(r, ConcurrentUserTestResult) and not r.success)]
        
        logger.info(f"Successful executions: {len(successful_results)}/{num_concurrent_users}")
        logger.info(f"Failed executions: {len(failed_results)}")
        
        # Verify success criteria
        assert len(successful_results) >= (num_concurrent_users * 0.8), f"Less than 80% success rate: {len(successful_results)}/{num_concurrent_users}"
        ...
        for event in all_websocket_events:
            ...
            if len(user_match) >= 3:
                user_key = f"{user_match[0]}_{user_match[1]}_{user_match[2]}"  # stress_user_XXX
        ...
        for result in successful_results:
            ...
            assert user_key in events_by_user, f"No WebSocket events found for user {user_key}"
            user_events = events_by_user[user_key]
            assert len(user_events) >= executions_per_user, f"Insufficient events for user {user_key}: {len(user_events)}"
        
        logger.info("✅ Concurrent user stress test passed with proper isolation")
...
class TestUserExecutionContextValidation:
    ...
    async def test_context_isolation_across_requests(self):
        ...
        for i, context in enumerate(contexts):
            expected_user_id = f"user_{i:03d}"
            expected_thread_id = f"thread_{i:03d}"
            expected_run_id = f"run_{i:03d}"
            expected_request_id = f"req_{i:03d}"
...
class TestEndToEndIntegration:
    ...
    @pytest.mark.asyncio
    async def test_complete_request_flow_with_isolation(self,
                                                       mock_agent_registry,
                                                       mock_websocket_bridge,
                                                       test_database_sessions):
        ...
        for expected_user, _ in test_users:
            assert expected_user in events_by_user, f"No WebSocket events for {expected_user}"
            user_events = events_by_user[expected_user]
            assert len(user_events) > 0, f"No events found for {expected_user}"
            ...
            for event in user_events:
                ...
                assert event["thread_id"] == expected_thread_id, f"Thread ID mismatch in event: {event}"
        
        logger.info("✅ Complete end-to-end integration test passed with proper isolation")
    ...
    async def test_memory_leak_prevention(self,
        ...
        for i in range(50):  # Large number to detect leaks
            user_id = f"leak_test_user_{i:03d}"
            thread_id = f"leak_test_thread_{i:03d}"
            run_id = f"leak_test_run_{i:03d}"
            ...
            async with instance_factory.user_execution_scope(
                ...
                state = DeepAgentState(
                    user_message=f"Leak test message {i}",
                    thread_id=thread_id
                )
        ...
        logger.info(f"Memory usage: before={memory_before/1024/1024:.1f}MB, after={memory_after/1024/1024:.1f}MB, increase={memory_increase_mb:.1f}MB")
        ...
        logger.info(f"Objects still alive: contexts={alive_contexts}/50, agents={alive_agents}/50, sessions={alive_sessions}/50, emitters={alive_emitters}/50")
        
        # Memory increase should be reasonable (less than 50MB for 50 iterations)
        assert memory_increase_mb < 50, f"Excessive memory increase: {memory_increase_mb:.1f}MB"
        ...
        assert alive_agents <= 10, f"Too many agents still alive: {alive_agents}"
        assert alive_emitters <= 10, f"Too many emitters still alive: {alive_emitters}"
...
class TestErrorScenariosAndEdgeCases:
    ...
    @pytest.mark.asyncio
    async def test_invalid_agent_creation_handling(self,
                                                  mock_agent_registry,
                                                  mock_websocket_bridge,
                                                  test_database_sessions):
    ...
    @pytest.mark.asyncio
    async def test_database_session_failure_handling(self,
                                                    mock_agent_registry,
                                                    mock_websocket_bridge):
