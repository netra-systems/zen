        ...
        try:
            ...
            try:
                ...
                if init_duration > 3.0:
                    self.fail(f"WebSocket bridge initialization took {init_duration:.2f}s - timeout issue detected")
                ...
                if "timeout" in str(e).lower() or "connection" in str(e).lower():
                    self.fail(f"WebSocket bridge initialization failed with timeout-related error: {e}")
                raise
                ...
                with self.subTest(scenario=scenario):
                    ...
                    try:
                        ...
                        if not mock_websocket.send_json.called:
                            self.fail(f"Message routing failed for {message_type} - no response sent")
                        ...
                        if call_args:
                            ...
                            if "error" in response and "interface" in str(response.get("error", "")).lower():
                                self.fail(f"Interface mismatch detected in message routing: {response['error']}")

                    except Exception as e:
                        # Interface mismatch should cause routing failures
                        if "interface" in str(e).lower() or "bridge" in str(e).lower():
                            self.fail(f"Message routing failed due to interface mismatch: {e}")
                        else:
                            raise
        ...
        try:
            ...
            for event_type, notify_method in critical_events:
                try:
                    ...
                    self.fail(f"Failed to deliver {event_type} event: {e}")
            ...
            if missing_events:
                self.fail(f"Critical WebSocket events not delivered: {missing_events}")

            # Verify event delivery timing
            if total_duration > 2.0:
                self.fail(f"WebSocket event delivery took {total_duration:.2f}s - performance issue detected")

            # Verify event ordering
            expected_order = [event_type for event_type, _ in critical_events]
            if delivered_events != expected_order:
                self.fail(f"WebSocket events delivered in wrong order. Expected: {expected_order}, Got: {delivered_events}")
