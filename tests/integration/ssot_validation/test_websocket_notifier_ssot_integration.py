...
class WebSocketNotifierSSOTIntegrationPatternsTests(SSotAsyncTestCase, SSotDatabaseTestMixin):
    ...
    def setUp(self):
        ...
        self.mock_websocket_manager = SSotMockFactory.create_mock('WebSocketManager', spec=['send_to_user', 'get_connection', 'is_connected'])
    ...
    async def test_websocket_event_delivery_through_ssot_bridge(self):
        ...
        try:
            ...
            with patch.object(bridge, '_websocket_manager', self.mock_websocket_manager):
                ...
                event_data = {'type': 'agent_started', 'run_id': 'test_run_123', 'agent_type': 'supervisor'}
...
class WebSocketNotifierSSOTServiceCoordinationTests(SSotAsyncTestCase):
    ...
    async def test_websocket_bridge_coordinates_with_execution_engine(self):
        ...
        try:
            ...
            bridge = AgentWebSocketBridge()
            mock_engine = SSotMockFactory.create_mock('ExecutionEngine', spec=['set_websocket_notifier', 'get_run_id'])
    ...
    async def test_websocket_events_reach_correct_agent_context(self):
        ...
        try:
            ...
            with patch.object(bridge, '_websocket_manager', mock_manager):
                event_data = {'type': 'agent_started', 'run_id': run_id, 'agent_type': 'supervisor', 'context': {'user_id': user_id}}
                result = await bridge.send_agent_event(user_id, event_data)
                self.assertTrue(result)
                mock_manager.send_to_user.assert_called_once_with(user_id, event_data)
        except ImportError as e:
            ...
            if 'websocket' in module_name.lower() and hasattr(module, '__dict__'):
                for attr_name, attr_value in module.__dict__.items():
                    if attr_name.isupper() and 'WEBSOCKET' in attr_name:
                        if not attr_name.startswith('WEBSOCKET_EVENT_TYPE'):
