...
class TestWebSocketManagerProtocolCompliance:
    ...
    def test_isolated_manager_protocol_compliance(self):
        ...
        assert validation_result['compliant'], f"IsolatedWebSocketManager MUST be protocol compliant. Missing methods: {validation_result['missing_methods']}, Invalid signatures: {validation_result['invalid_signatures']}. Compliance: {validation_result['summary']['compliance_percentage']}%"
    ...
    async def test_five_whys_critical_methods_functionality(self):
        ...
        assert found_connection_id == self.test_connection_id, f'get_connection_id_by_websocket should return {self.test_connection_id}, got {found_connection_id}'
    ...
    async def test_protocol_method_signatures(self):
        ...
        for method_name, args in async_methods:
            ...
            assert asyncio.iscoroutinefunction(method), f'{method_name} should be an async method'
            try:
        ...
        sync_methods = [('get_connection', [self.test_connection_id]), ('get_user_connections', [self.test_user_id]), ('is_connection_active', [self.test_user_id]), ('get_connection_id_by_websocket', [self.mock_websocket]), ('update_connection_thread', [self.test_connection_id, self.test_thread_id]), ('get_connection_health', [self.test_user_id])]
        for method_name, args in sync_methods:
            method = getattr(manager, method_name)
            assert not asyncio.iscoroutinefunction(method), f'{method_name} should be a sync method'
            try:
                result = method(*args)
    ...
    def test_require_protocol_compliance_enforcement(self):
        ...
        with pytest.raises(RuntimeError) as exc_info:
            WebSocketManagerProtocolValidator.require_protocol_compliance(non_compliant, 'Test Context')
