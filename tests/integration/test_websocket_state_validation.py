...
""
        ...
        for scenario in test_scenarios:
            logger.info(fTesting scenario: {scenario['description']})
        ...
        for case in edge_cases:
            ...
            try:
                ...
                if case['name'] == 'no_attributes_at_all':
                    ...
                    raise AssertionError(fEdge case '{case['name']}' raised unexpected exception: {e}. Function should handle all WebSocket state scenarios gracefully.)
    ...
    ""
        ...
        for scenario in cloud_scenarios:
            logger.info(fTesting cloud scenario: {scenario['name']}")"
            with patch.dict('os.environ', {'ENVIRONMENT': scenario['environment']):
                with patch('netra_backend.app.websocket_core.utils.get_env') as mock_get_env:
                    mock_env = Mock()
                    mock_env.get.return_value = scenario['environment']
                    ...
                    if scenario['expected'] and scenario['environment'] in ['staging', 'production']:
                        assert result == scenario['expected'], fCRITICAL: Cloud fallback detection failed for {scenario['name']}. Expected {scenario['expected']}, got {result}. This fix is needed to prevent ConnectionHandler silent failures in {scenario['environment']} environment.""
                    else:
