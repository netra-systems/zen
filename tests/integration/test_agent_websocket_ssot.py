...
class TestWebSocketSSOT:
    ...
    def mock_bridge(self):
        ...
        bridge.notify_progress_update = AsyncMock()
    ...
    async def test_no_duplicate_websocket_methods(self):
        ...
        comm_methods = [m for m in dir(AgentCommunicationMixin) if m in methods_to_check]
        ...
        duplicates = {k: v for k, v in all_occurrences.items() if v["count"] > 1}
        assert not duplicates, f"SSOT Violation! Duplicate methods found: {duplicates}"
    ...
    async def test_all_websocket_events_sent_correctly(self, mock_bridge, execution_context):
        ...
        mock_bridge.notify_agent_started.side_effect = lambda *a, **k: track_event("agent_started", *a, **k)
        mock_bridge.notify_agent_thinking.side_effect = lambda *a, **k: track_event("agent_thinking", *a, **k)
        mock_bridge.notify_tool_executing.side_effect = lambda *a, **k: track_event("tool_executing", *a, **k)
        mock_bridge.notify_tool_completed.side_effect = lambda *a, **k: track_event("tool_completed", *a, **k)
        mock_bridge.notify_agent_completed.side_effect = lambda *a, **k: track_event("agent_completed", *a, **k)
        ...
        with patch('netra_backend.app.services.agent_websocket_bridge.get_agent_websocket_bridge',
            ...
            for event in required_events:
                assert event in events_received, f"Missing critical event: {event}"
    ...
    async def test_dead_code_removed(self):
        ...
        assert not hasattr(DataSubAgent, dead_method), \
            f"Dead code found: DataSubAgent.{dead_method} should be removed"
        
        assert not hasattr(ValidationSubAgent, dead_method), \
            f"Dead code found: ValidationSubAgent.{dead_method} should be removed"
    ...
    async def test_websocket_bridge_consistency(self, mock_bridge):
        ...
        assert len(patterns_found) == 1, \
            f"Inconsistent WebSocket patterns found: {patterns_found}"
    ...
    async def test_multiple_inheritance_resolved(self):
        ...
        base_classes = [c for c in mro if c.__name__ not in ['object', 'DataSubAgent']]
        
        # Should have clean single inheritance or well-defined MRO
        execution_interfaces = [c for c in base_classes if 'ExecutionInterface' in c.__name__]
        
        assert len(execution_interfaces) <= 1, \
            f"Multiple ExecutionInterface inheritance found: {execution_interfaces}"
    ...
    async def test_import_organization(self):
        ...
        if os.path.exists(file_path):
            ...
            assert not late_imports, \
                f"Found imports at end of file (violates PEP 8): {late_imports}"
...
class TestWebSocketCriticalPath:
    ...
    async def test_chat_value_delivery_path(self, mock_bridge):
        ...
        mock_bridge.notify_agent_started = AsyncMock(side_effect=lambda *a, **k: track_chat_event("started", *a, **k))
        mock_bridge.notify_agent_thinking = AsyncMock(side_effect=lambda *a, **k: track_chat_event("thinking", *a, **k))
        mock_bridge.notify_agent_completed = AsyncMock(side_effect=lambda *a, **k: track_chat_event("completed", *a, **k))
        ...
        with patch('netra_backend.app.services.agent_websocket_bridge.get_agent_websocket_bridge',
            ...
            lifecycle.logger = Mock()
            
            await lifecycle.send_agent_thinking("chat_123", "Analyzing your request...")
            await lifecycle.send_final_report("chat_123", {"response": "Here's your answer"}, 250.0)
