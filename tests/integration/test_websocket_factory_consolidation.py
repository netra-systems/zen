    ...
    async def _create_test_user_contexts(self) -> List[Any]:
        ...
        for i in range(3):
            try:
                ...
                logger.info(fCreated test user context {i}: {user_id})
                
            except Exception as e:
                logger.error(fFailed to create user context {i}: {e}")"
                # Fallback minimal context
                context = type('MinimalTestContext', (), {
                    'user_id': ftest_factory_user_{i},
                    'is_test': True,
                    'factory_test_index': i
                }()
                user_contexts.append(context)
...
"
        ...
        try:
            ...
            for i, user_context in enumerate(self.test_user_contexts):
                ...
                logger.info(fCreated manager {manager_id} for user {user_context.user_id} (manager thinks it's for: {manager_user_id})
    ...
    async def _test_concurrent_user_isolation(self, user_managers: Dict[str, Any] -> List[str]:
            ...
            try:
                ...
                if hasattr(manager, 'user_context'):
                    ...
                    if manager_user_id != user_id:
                        return {
                            'violation': fManager for {user_id} has wrong user context: {manager_user_id},
                            'user_id': user_id,
        ...
        for user_id, manager in user_managers.items():
            for op_id in range(3):  # Multiple operations per user
                task = user_operation(user_id, manager, fop_{op_id})
                tasks.append(task)
            ...
            with concurrent.futures.ThreadPoolExecutor(max_workers=5) as executor:
                ...
                for future in concurrent.futures.as_completed(futures):
                    ...
                    if result.get('success'):
                        ...
                        thread_errors.append(fThread {thread_id}: {result.get('error', 'Unknown error')})
            
            # Analyze results for race conditions
            manager_types = set(r['manager_type'] for r in thread_results.values())
            if len(manager_types) > 1:
                race_condition_violations.append(fDifferent manager types created concurrently: {manager_types})
    ...
    "
        ...
        try:
                ...
                for j, (manager2, key2, value2, user_id2) in enumerate(created_managers_memory):
                    if i != j:  # Different managers
                        ...
                        if hasattr(manager1, key2) and getattr(manager1, key2, None) == value2:
                            memory_violations.append(fManager for {user_id1} has direct attribute contamination from {user_id2}: {key2}={value2})
            ...
            self.assertEqual(len(memory_violations), 0,
                           fMEMORY ISOLATION VIOLATIONS: Found {len(memory_violations)} memory contamination issues. 
                           fViolations: {memory_violations}")"
                           
        except Exception as e:
            self.fail(fFailed to test memory isolation: {e})
    ...
    async def asyncTearDown(self):
        ...
        for manager in self.created_managers:
            try:
                ...
                logger.warning(fFailed to cleanup manager {manager}: {e})
