...
class TestRequestScopedSessionFactory:
    ...
    async def test_session_isolation_validation(self, factory: RequestScopedSessionFactory):
        ...
        async with factory.get_request_scoped_session(user_id) as session:
            ...
            # Should validate successfully for correct user
    ...
    async def test_pool_metrics_tracking(self, factory: RequestScopedSessionFactory):
        ...
        async def create_session(i):
            ...
            async with factory.get_request_scoped_session(f"user_{i}") as session:
                ...
                await asyncio.sleep(0.1)  # Brief session duration
...
class TestGlobalSessionFactory:
    ...
    async def test_shutdown_session_factory(self):
        ...
        # Get factory to ensure it's created
        ...
        # Note: Can't easily test this without accessing private globals
...
class TestConcurrentLoadTesting:
    ...
    async def test_concurrent_session_load(self, factory: RequestScopedSessionFactory):
        ...
        num_concurrent = 50  # Test with 50 concurrent sessions
                ...
                async with factory.get_request_scoped_session(user_id, request_id) as session:
                    ...
                    for query_index in range(queries_per_session):
                        ...
                        result = await session.execute(
                            ...
                            text("SELECT :session_id as s_id, :query_id as q_id"),

                            {'session_id': session_index, 'query_id': query_index}
        ...
        assert pool_metrics.active_sessions == 0  # All cleaned up
    ...
    async def test_connection_pool_exhaustion_protection(self, factory: RequestScopedSessionFactory):
        ...
        # Try to create more sessions than the pool can handle

        num_sessions = 100  # This should exceed normal pool limits
        ...
        async def long_running_session(session_index: int):
            ...
            try:
                ...
                # Pool exhaustion is expected for some sessions
        ...
        # Some sessions should succeed, system shouldn't crash
        ...
        # Pool should recover after load
    ...
    async def test_session_leak_detection(self, factory: RequestScopedSessionFactory):
        ...
        # Simulate a leaked session by not properly cleaning up

        user_id = "leak_test_user"

        

        # Create a session but don't use proper context manager

        # (This is testing the leak detection, not recommending this pattern)

        session_id = f"{user_id}_leak_test_{uuid.uuid4().hex[:8]}"

        

        # Manually register a "leaked" session

        from netra_backend.app.database.request_scoped_session_factory import SessionMetrics, SessionState

        leak_metrics = SessionMetrics(
        ...
        # Manually add to factory (simulating a leak)
        ...
        # Leaked session should be detected and cleaned up

        # Note: This test might not trigger cleanup if session is too new

        # The actual leak detection has time thresholds
    ...
    async def test_extreme_concurrent_load(self, factory: RequestScopedSessionFactory):
        ...
        assert success_rate >= 0.90, f"Success rate too low for extreme load: {success_rate:.2%}"
