...
class TestWebSocketContextImportRegression:
    ...
    def test_websocket_context_is_available(self):
        ...
        """
        assert WEBSOCKET_CONTEXT_AVAILABLE, f"WebSocketContext should be importable from websocket_core. Error: {(WEBSOCKET_CONTEXT_ERROR if not WEBSOCKET_CONTEXT_AVAILABLE else 'None')}"
    ...
    def test_websocket_request_context_alias_available_EXPECTED_TO_FAIL(self):
        ...
        """
        assert WEBSOCKET_REQUEST_CONTEXT_AVAILABLE, f' ALERT:  REGRESSION DETECTED: WebSocketRequestContext alias cannot be imported from websocket_core package. Error: {WEBSOCKET_REQUEST_CONTEXT_ERROR}. This breaks backward compatibility and agent-WebSocket integration.'
        if WEBSOCKET_REQUEST_CONTEXT_AVAILABLE:
            assert WebSocketRequestContext is WebSocketContext, 'WebSocketRequestContext should be an alias for WebSocketContext'
    ...
    def test_alias_exists_in_context_module(self):
        ...
        """
        assert DIRECT_IMPORT_AVAILABLE, f'WebSocketRequestContext alias should exist in context module. Error: {DIRECT_IMPORT_ERROR}'
        if WEBSOCKET_CONTEXT_AVAILABLE and DIRECT_IMPORT_AVAILABLE:
            assert DirectWebSocketRequestContext is WebSocketContext, 'WebSocketRequestContext should be an alias for WebSocketContext'
    ...
    def test_backward_compatibility_broken_EXPECTED_TO_FAIL(self):
        ...
        assert import_successful, f" ALERT:  BACKWARD COMPATIBILITY BROKEN: The import statement '{import_statement}' fails, breaking existing code that depends on WebSocketRequestContext alias. This violates SSOT principles and breaks agent-WebSocket integration."
    ...
    def test_websocket_core_exports_completeness(self):
        ...
        try:
            assert 'WebSocketRequestContext' in __all__, 'WebSocketRequestContext alias should be in __all__ for backward compatibility'
        ...
        if missing_exports:
            pytest.fail(f' ALERT:  EXPORT REGRESSION: websocket_core __all__ is missing: {missing_exports}. This breaks backward compatibility and SSOT principles.')
    ...
    def test_module_structure_integrity(self):
        ...
        assert context_module.WebSocketRequestContext is context_module.WebSocketContext, 'WebSocketRequestContext should be an alias for WebSocketContext in context module'
    ...
    def test_import_paths_documentation(self):
        ...
        """
        import_results = {'from netra_backend.app.websocket_core import WebSocketContext': WEBSOCKET_CONTEXT_AVAILABLE, 'from netra_backend.app.websocket_core import WebSocketRequestContext': WEBSOCKET_REQUEST_CONTEXT_AVAILABLE, 'from netra_backend.app.websocket_core.context import WebSocketContext': WEBSOCKET_CONTEXT_AVAILABLE, 'from netra_backend.app.websocket_core.context import WebSocketRequestContext': DIRECT_IMPORT_AVAILABLE}
