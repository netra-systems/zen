...
class AgentRegistryWebSocketIntegrationFailuresTests(SSotAsyncTestCase):
    ...
    async def async_setup_method(self, method=None):
        ...
        self.test_users = [{'user_id': 'websocket-user-1', 'session_id': 'ws-session-1'}, {'user_id': 'websocket-user-2', 'session_id': 'ws-session-2'}]
    ...
    def _setup_websocket_event_capture(self):
        ...
        async def capture_websocket_event(event_type: str, event_data: dict, user_id: str=None):
            ...
            captured_event = {'timestamp': time.time(), 'event_type': event_type, 'event_data': event_data, 'user_id': user_id, 'source': 'captured'}
    ...
    async def test_websocket_bridge_incompatibility_with_basic_registry(self):
        ...
        for registry_name, registry in registries.items():
            registry_websocket_compatibility[registry_name] = {'bridge_methods_available': [], 'bridge_methods_missing': [], 'integration_success': False, 'integration_errors': [], 'websocket_events_supported': [], 'websocket_events_missing': []}
            websocket_bridge_methods = ['set_websocket_bridge', 'get_websocket_bridge', 'notify_websocket_event', '_websocket_bridge', 'websocket_bridge']
            for method_name in websocket_bridge_methods:
                if hasattr(registry, method_name):
                    method = getattr(registry, method_name)
                    registry_websocket_compatibility[registry_name]['bridge_methods_available'].append({'method_name': method_name, 'is_callable': callable(method), 'is_async': asyncio.iscoroutinefunction(method) if callable(method) else False})
            ...
            try:
                if hasattr(registry, 'set_websocket_bridge'):
                    ...
                    registry_websocket_compatibility[registry_name]['integration_errors'].append('No WebSocket bridge or manager setup method available')
            except Exception as e:
                registry_websocket_compatibility[registry_name]['integration_errors'].append(f'WebSocket bridge setup failed: {e}')
            ...
            for event_type in self.critical_websocket_events:
                ...
                event_methods_to_test = ['_notify_agent_event', 'emit_agent_event', 'notify_websocket_event', 'send_websocket_event']
                for method_name in event_methods_to_test:
                    if hasattr(registry, method_name):
                        try:
                            method = getattr(registry, method_name)
                            test_event_data = {'event_type': event_type, 'test_data': f'test_{event_type}_{registry_name}', 'timestamp': time.time()}
        ...
        if basic_compat['bridge_methods_missing'] and (not advanced_compat['bridge_methods_missing']):
            websocket_bridge_integration_failures.append({'failure_type': 'missing_bridge_methods', 'registry': 'basic', 'missing_methods': basic_compat['bridge_methods_missing'], 'description': f"Basic registry missing WebSocket bridge methods: {basic_compat['bridge_methods_missing']}"})
        if not basic_compat['integration_success'] and advanced_compat['integration_success']:
            websocket_bridge_integration_failures.append({'failure_type': 'integration_failure', 'registry': 'basic', 'integration_errors': basic_compat['integration_errors'], 'description': f"Basic registry WebSocket bridge integration failed: {basic_compat['integration_errors']}"})
        if basic_compat['websocket_events_missing'] and (not advanced_compat['websocket_events_missing']):
            websocket_bridge_integration_failures.append({'failure_type': 'missing_event_support', 'registry': 'basic', 'missing_events': basic_compat['websocket_events_missing'], 'description': f"Basic registry missing WebSocket event support: {basic_compat['websocket_events_missing']}"})
        for user_info in self.test_users:
            ...
            for registry_name, registry in registries.items():
                if registry_websocket_compatibility[registry_name]['integration_success']:
                    for event_type in self.critical_websocket_events:
                        try:
                            test_event_data = {'user_id': user_id, 'event_type': event_type, 'test_payload': f'test_payload_{event_type}_{registry_name}_{user_id}'}
                            event_sent = False
                            for method_name in ['_notify_agent_event', 'emit_agent_event']:
                                if hasattr(registry, method_name):
                                    ...
                                    break
                            if not event_sent:
                                websocket_bridge_integration_failures.append({'failure_type': 'event_send_failure', 'registry': registry_name, 'user_id': user_id, 'event_type': event_type, 'description': f'Could not send {event_type} event for {user_id} via {registry_name} registry'})
                        except Exception as e:
                            websocket_bridge_integration_failures.append({'failure_type': 'event_send_error', 'registry': registry_name, 'user_id': user_id, 'event_type': event_type, 'error': str(e), 'description': f'Error sending {event_type} event for {user_id} via {registry_name} registry: {e}'})
        failure_reasons = []
        ...
        if basic_event_support < advanced_event_support:
            failure_reasons.append(f"Basic registry supports {basic_event_support} WebSocket events vs advanced registry's {advanced_event_support} events - incompatible capabilities")
        if failure_reasons:
            pytest.fail(f"CRITICAL WEBSOCKET BRIDGE INCOMPATIBILITY: Basic registry incompatible with WebSocket bridge. This prevents real-time agent progress updates blocking Golden Path user experience. Integration failures: {'; '.join(failure_reasons)}. IMPACT: Users don't see agent progress when basic registry is used.")
    ...
    async def test_missing_websocket_events_blocking_golden_path(self):
        ...
        registries = {'basic': self.basic_registry, 'advanced': self.advanced_registry}
        for registry_name, registry in registries.items():
            registry_event_analysis[registry_name] = {'events_sent_successfully': [], 'events_failed_to_send': [], 'events_received_count': 0, 'critical_events_missing': [], 'event_delivery_rate': 0.0, 'event_sequence_correct': True, 'event_timing_issues': []}
            ...
            for user_info in self.test_users:
                ...
                agent_execution_sequence = [{'event': 'agent_started', 'data': {'agent_id': 'test-agent', 'user_id': user_id, 'status': 'started'}, 'expected_order': 1}, {'event': 'agent_thinking', 'data': {'agent_id': 'test-agent', 'user_id': user_id, 'message': 'Analyzing request'}, 'expected_order': 2}, {'event': 'tool_executing', 'data': {'agent_id': 'test-agent', 'user_id': user_id, 'tool': 'data_analyzer'}, 'expected_order': 3}, {'event': 'tool_completed', 'data': {'agent_id': 'test-agent', 'user_id': user_id, 'tool': 'data_analyzer', 'result': 'success'}, 'expected_order': 4}, {'event': 'agent_completed', 'data': {'agent_id': 'test-agent', 'user_id': user_id, 'status': 'completed', 'result': 'Task completed'}, 'expected_order': 5}]
                    ...
                    try:
                        ...
                        for method_name in ['_notify_agent_event', 'emit_agent_event', 'notify_websocket_event']:
                            if hasattr(registry, method_name):
                                ...
                                registry_event_analysis[registry_name]['events_sent_successfully'].append({'event_type': event_type, 'user_id': user_id, 'method_used': method_name, 'timestamp': time.time(), 'expected_order': event_info['expected_order']})
                        ...
                        if not event_sent_successfully:
                            registry_event_analysis[registry_name]['events_failed_to_send'].append({'event_type': event_type, 'user_id': user_id, 'reason': 'No suitable event sending method found', 'expected_order': event_info['expected_order']})
                    except Exception as e:
                        registry_event_analysis[registry_name]['events_failed_to_send'].append({'event_type': event_type, 'user_id': user_id, 'reason': f'Event sending failed: {e}', 'error': str(e), 'expected_order': event_info['expected_order']})
            ...
            for user_id, user_events in sent_events_by_user.items():
                ...
                for i, event in enumerate(user_events):
                    ...
                    if actual_order != expected_order:
                        ...
                        registry_event_analysis[registry_name]['event_timing_issues'].append({'user_id': user_id, 'event_type': event['event_type'], 'expected_order': expected_order, 'actual_order': actual_order, 'issue': f"Event {event['event_type']} was #{actual_order} but should be #{expected_order}"})
        ...
        if basic_analysis['critical_events_missing']:
            websocket_event_completeness_failures.append({'failure_type': 'missing_critical_events', 'registry': 'basic', 'missing_events': basic_analysis['critical_events_missing'], 'description': f"Basic registry missing critical WebSocket events: {basic_analysis['critical_events_missing']}"})
        if advanced_analysis['critical_events_missing']:
            websocket_event_completeness_failures.append({'failure_type': 'missing_critical_events', 'registry': 'advanced', 'missing_events': advanced_analysis['critical_events_missing'], 'description': f"Advanced registry missing critical WebSocket events: {advanced_analysis['critical_events_missing']}"})
        for registry_name, analysis in registry_event_analysis.items():
            if analysis['event_delivery_rate'] < 100.0:
                websocket_event_completeness_failures.append({'failure_type': 'poor_event_delivery_rate', 'registry': registry_name, 'delivery_rate': analysis['event_delivery_rate'], 'events_failed': len(analysis['events_failed_to_send']), 'description': f"{registry_name} registry event delivery rate only {analysis['event_delivery_rate']:.1f}%"})
        for registry_name, analysis in registry_event_analysis.items():
            if not analysis['event_sequence_correct']:
                websocket_event_completeness_failures.append({'failure_type': 'event_sequence_incorrect', 'registry': registry_name, 'timing_issues': analysis['event_timing_issues'], 'description': f"{registry_name} registry event sequence incorrect: {len(analysis['event_timing_issues'])} timing issues"})
        basic_delivery_rate = basic_analysis['event_delivery_rate']
        advanced_delivery_rate = advanced_analysis['event_delivery_rate']
        if abs(basic_delivery_rate - advanced_delivery_rate) > 20.0:
            websocket_event_completeness_failures.append({'failure_type': 'asymmetric_event_capabilities', 'basic_delivery_rate': basic_delivery_rate, 'advanced_delivery_rate': advanced_delivery_rate, 'difference': abs(basic_delivery_rate - advanced_delivery_rate), 'description': f'Event delivery rates differ significantly: basic {basic_delivery_rate:.1f}% vs advanced {advanced_delivery_rate:.1f}%'})
        failure_reasons = []
        if websocket_event_completeness_failures:
            for failure in websocket_event_completeness_failures:
                failure_reasons.append(f"{failure['failure_type']}: {failure['description']}")
        if failure_reasons:
            pytest.fail(f"CRITICAL MISSING WEBSOCKET EVENTS: WebSocket events missing or incomplete blocking Golden Path. This prevents users from seeing real-time agent progress causing poor user experience. Event delivery failures: {'; '.join(failure_reasons)}. IMPACT: Users cannot track agent execution progress and assume system is broken.")
    ...
    async def test_chat_functionality_degradation_from_registry_conflicts(self):
        ...
        chat_test_scenarios = [{'name': 'basic_registry_only', 'description': 'All components use basic registry', 'registry_config': {'agent_registry': self.basic_registry, 'websocket_registry': self.basic_registry, 'execution_registry': self.basic_registry}}, {'name': 'advanced_registry_only', 'description': 'All components use advanced registry', 'registry_config': {'agent_registry': self.advanced_registry, 'websocket_registry': self.advanced_registry, 'execution_registry': self.advanced_registry}}, {'name': 'mixed_registry_usage', 'description': 'Components use different registries (conflict scenario)', 'registry_config': {'agent_registry': self.basic_registry, 'websocket_registry': self.advanced_registry, 'execution_registry': self.basic_registry}}]
        for scenario in chat_test_scenarios:
            scenario_name = scenario['name']
            registry_config = scenario['registry_config']
            chat_experience_analysis[scenario_name] = {'scenario_description': scenario['description'], 'chat_initiation_success': False, 'agent_discovery_success': False, 'websocket_connection_success': False, 'real_time_updates_working': False, 'chat_completion_success': False, 'user_experience_score': 0, 'critical_failures': [], 'degradation_indicators': []}
            ...
            try:
                ...
                if hasattr(agent_registry, 'list_available_agents'):
                    ...
                    if available_agents is not None:
                        ...
                        chat_experience_analysis[scenario_name]['critical_failures'].append('Agent discovery returned None')
                else:
                    chat_experience_analysis[scenario_name]['critical_failures'].append('Agent registry missing list_available_agents method')
                ...
                if hasattr(websocket_registry, 'set_websocket_manager'):
                    ...
                    chat_experience_analysis[scenario_name]['critical_failures'].append('WebSocket registry missing set_websocket_manager method')
                    ...
                    try:
                        if hasattr(execution_registry, 'create_user_session'):
                            ...
                            if user_session:
                                ...
                                chat_experience_analysis[scenario_name]['degradation_indicators'].append(f'User session creation returned None for {user_id}')
                        else:
                            chat_experience_analysis[scenario_name]['degradation_indicators'].append('Execution registry missing create_user_session method')
                    except Exception as e:
                        chat_experience_analysis[scenario_name]['degradation_indicators'].append(f'User session creation failed for {user_id}: {e}')
                ...
                if websocket_setup_success:
                    ...
                    for user_info in self.test_users:
                        ...
                        for event_type in self.critical_websocket_events:
                            ...
                            event_data = {'user_id': user_id, 'event': event_type, 'test_data': f'chat_test_{scenario_name}_{event_type}'}
                            try:
                                event_sent = False
                                for method_name in ['_notify_agent_event', 'emit_agent_event']:
                                    if hasattr(websocket_registry, method_name):
                                        ...
                                        break
                                if not event_sent:
                                    chat_experience_analysis[scenario_name]['degradation_indicators'].append(f'Could not send {event_type} event for {user_id} - no suitable method')
                            except Exception as e:
                                chat_experience_analysis[scenario_name]['degradation_indicators'].append(f'Failed to send {event_type} event for {user_id}: {e}')
                    if total_events_attempted > 0:
                        real_time_success_rate = events_sent_successfully / total_events_attempted * 100
                        if real_time_success_rate >= 80:
                            chat_experience_analysis[scenario_name]['real_time_updates_working'] = True
                        else:
                            chat_experience_analysis[scenario_name]['degradation_indicators'].append(f'Real-time update success rate only {real_time_success_rate:.1f}%')
                chat_completion_steps = [chat_experience_analysis[scenario_name]['agent_discovery_success'], chat_experience_analysis[scenario_name]['websocket_connection_success'], chat_experience_analysis[scenario_name]['chat_initiation_success'], chat_experience_analysis[scenario_name]['real_time_updates_working']]
                ...
                if completed_steps == total_steps:
                    ...
                    if completion_rate < 75:
                        chat_experience_analysis[scenario_name]['critical_failures'].append(f'Chat completion rate only {completion_rate:.1f}% - poor user experience')
            except Exception as e:
                chat_experience_analysis[scenario_name]['critical_failures'].append(f'Scenario execution failed: {e}')
        ...
        if score_variance > 30:
            chat_functionality_degradation.append({'degradation_type': 'inconsistent_chat_experience', 'best_scenario': max(scenario_scores, key=scenario_scores.get), 'worst_scenario': min(scenario_scores, key=scenario_scores.get), 'best_score': best_score, 'worst_score': worst_score, 'variance': score_variance, 'description': f'Chat experience varies by {score_variance:.1f}% between scenarios'})
        ...
        if mixed_scenario_analysis:
            ...
            if pure_scores and mixed_score < min(pure_scores):
                chat_functionality_degradation.append({'degradation_type': 'mixed_registry_degradation', 'mixed_score': mixed_score, 'pure_scenario_min': min(pure_scores), 'degradation_amount': min(pure_scores) - mixed_score, 'critical_failures': mixed_scenario_analysis['critical_failures'], 'description': f'Mixed registry usage causes {min(pure_scores) - mixed_score:.1f}% degradation'})
        for scenario_name, analysis in chat_experience_analysis.items():
            if analysis['critical_failures']:
                chat_functionality_degradation.append({'degradation_type': 'critical_failures', 'scenario': scenario_name, 'failures': analysis['critical_failures'], 'description': f"{scenario_name} has critical failures: {'; '.join(analysis['critical_failures'])}"})
        failure_reasons = []
        ...
        for scenario_name, analysis in chat_experience_analysis.items():
            if analysis['user_experience_score'] < 75:
        ...
        if failure_reasons:
            pytest.fail(f"CRITICAL CHAT FUNCTIONALITY DEGRADATION: Registry conflicts cause unpredictable chat experience. This creates unreliable Golden Path where chat sometimes works, sometimes fails completely. Degradation patterns: {'; '.join(failure_reasons)}. IMPACT: Users experience inconsistent chat functionality depending on deployment configuration.")
    ...
    async def test_real_time_user_experience_failures(self):
        ...
        registries_to_test = {'basic': self.basic_registry, 'advanced': self.advanced_registry}
        for registry_name, registry in registries_to_test.items():
            user_experience_metrics[registry_name] = {'response_times': [], 'event_delivery_delays': [], 'progress_visibility_score': 0, 'user_feedback_responsiveness': 0, 'real_time_experience_score': 0, 'experience_failures': [], 'performance_issues': []}
            websocket_setup_success = False
            try:
                if hasattr(registry, 'set_websocket_manager'):
                ...
                user_experience_metrics[registry_name]['experience_failures'].append(f'WebSocket setup failed: {e}')
            if not websocket_setup_success:
            ...
            for user_info in self.test_users:
                ...
                try:
                    ...
                    user_experience_metrics[registry_name]['experience_failures'].append(f'User session creation failed for {user_id}: {e}')
                if not user_session_success:
                    continue
                agent_interaction_sequence = [{'event': 'user_message_received', 'expected_delay_ms': 0}, {'event': 'agent_started', 'expected_delay_ms': 100}, {'event': 'agent_thinking', 'expected_delay_ms': 200}, {'event': 'tool_executing', 'expected_delay_ms': 500}, {'event': 'tool_completed', 'expected_delay_ms': 1000}, {'event': 'agent_completed', 'expected_delay_ms': 1200}]
                interaction_start_time = time.time()
                previous_event_time = interaction_start_time
                for event_info in agent_interaction_sequence:
                    event_type = event_info['event']
                    expected_delay = event_info['expected_delay_ms'] / 1000.0
                    ...
                    try:
                        event_data = {'user_id': user_id, 'session_id': session_id, 'event_type': event_type, 'timestamp': event_send_start, 'sequence_number': len(agent_interaction_sequence)}
                        event_sent = False
                        for method_name in ['_notify_agent_event', 'emit_agent_event']:
                            if hasattr(registry, method_name):
                                method = getattr(registry, method_name)
                        ...
                        event_processing_time = (event_send_end - event_send_start) * 1000
                        ...
                        delivery_delay = abs(actual_event_time - expected_delay) * 1000
                        user_experience_metrics[registry_name]['event_delivery_delays'].append(delivery_delay)
                        if not event_sent:
                            user_experience_metrics[registry_name]['experience_failures'].append(f'Failed to send {event_type} event for {user_id} - no method available')
                        if event_processing_time > 100:
                            user_experience_metrics[registry_name]['performance_issues'].append({'issue_type': 'slow_event_processing', 'event_type': event_type, 'user_id': user_id, 'processing_time_ms': event_processing_time, 'description': f'{event_type} event took {event_processing_time:.1f}ms to process'})
                        if delivery_delay > 500:
                            user_experience_metrics[registry_name]['performance_issues'].append({'issue_type': 'event_timing_delay', 'event_type': event_type, 'user_id': user_id, 'delay_ms': delivery_delay, 'expected_delay': expected_delay * 1000, 'actual_delay': actual_event_time * 1000, 'description': f'{event_type} event delayed by {delivery_delay:.1f}ms'})
                        previous_event_time = event_send_end
                    except Exception as e:
                        user_experience_metrics[registry_name]['experience_failures'].append(f'Event {event_type} failed for {user_id}: {e}')
            ...
            overall_score = progress_visibility * progress_weight + user_experience_metrics[registry_name]['user_feedback_responsiveness'] * responsiveness_weight
        ...
        for registry_name, metrics in user_experience_metrics.items():
            if metrics['progress_visibility_score'] < 90:
                real_time_experience_failures.append({'failure_type': 'poor_progress_visibility', 'registry': registry_name, 'visibility_score': metrics['progress_visibility_score'], 'description': f"{registry_name} registry progress visibility only {metrics['progress_visibility_score']:.1f}%"})
            if metrics['user_feedback_responsiveness'] < 80:
                real_time_experience_failures.append({'failure_type': 'poor_responsiveness', 'registry': registry_name, 'responsiveness_score': metrics['user_feedback_responsiveness'], 'description': f"{registry_name} registry responsiveness only {metrics['user_feedback_responsiveness']:.1f}%"})
            if metrics['performance_issues']:
                performance_issue_count = len(metrics['performance_issues'])
                real_time_experience_failures.append({'failure_type': 'performance_issues', 'registry': registry_name, 'issue_count': performance_issue_count, 'issues': metrics['performance_issues'], 'description': f'{registry_name} registry has {performance_issue_count} performance issues'})
            if metrics['experience_failures']:
                failure_count = len(metrics['experience_failures'])
                real_time_experience_failures.append({'failure_type': 'experience_failures', 'registry': registry_name, 'failure_count': failure_count, 'failures': metrics['experience_failures'], 'description': f'{registry_name} registry has {failure_count} experience failures'})
        ...
        if abs(basic_score - advanced_score) > 20:
            real_time_experience_failures.append({'failure_type': 'inconsistent_real_time_experience', 'basic_score': basic_score, 'advanced_score': advanced_score, 'difference': abs(basic_score - advanced_score), 'description': f'Real-time experience differs by {abs(basic_score - advanced_score):.1f}% between registries'})
        unacceptable_registries = []
        for registry_name, metrics in user_experience_metrics.items():
            if metrics['real_time_experience_score'] < 75:
                unacceptable_registries.append(f"{registry_name}: {metrics['real_time_experience_score']:.1f}%")
        if unacceptable_registries:
            real_time_experience_failures.append({'failure_type': 'unacceptable_user_experience', 'registries': unacceptable_registries, 'description': f"Unacceptable real-time experience: {'; '.join(unacceptable_registries)}"})
        failure_reasons = []
        if real_time_experience_failures:
            for failure in real_time_experience_failures:
                failure_reasons.append(f"{failure['failure_type']}: {failure['description']}")
        if failure_reasons:
            pytest.fail(f"CRITICAL REAL-TIME USER EXPERIENCE FAILURES: Registry conflicts cause poor real-time experience. This makes Golden Path feel broken and unresponsive causing user abandonment. Experience failures: {'; '.join(failure_reasons)}. IMPACT: Users perceive system as slow, broken, or unresponsive during agent interactions.")
