...
@pytest.mark.unit
class CORSConfigurationBuilderTests:
    ...
    def test_environment_detection(self):
        ...
        env_var_names = ['ENVIRONMENT', 'ENV', 'NETRA_ENVIRONMENT', 'NETRA_ENV', 'NODE_ENV', 'AUTH_ENV', 'K_SERVICE', 'GCP_PROJECT_ID']
        test_cases = [({'ENVIRONMENT': 'production'}, 'production'), ({'ENV': 'staging'}, 'staging'), ({'NODE_ENV': 'development'}, 'development'), ({'NETRA_ENV': 'prod'}, 'production'), ({'AUTH_ENV': 'stg'}, 'staging'), ({}, 'development')]
...
class OriginsBuilderTests:
    ...
    def test_custom_cors_origins(self):
        ...
        env_vars = {'ENVIRONMENT': 'production', 'CORS_ORIGINS': 'https://custom1.com,https://custom2.com'}
        cors = CORSConfigurationBuilder(env_vars)
        origins = cors.origins.allowed
    ...
    def test_is_allowed_localhost_in_dev(self):
        ...
        assert cors.origins.is_allowed('http://172.17.0.1:3000') is True
    ...
    def test_validate_origin_format(self):
        ...
        invalid_cases = [('', 'Origin cannot be empty'), ('example.com', 'Origin must include scheme'), ('ftp://example.com', 'Invalid scheme'), ('https://', 'Origin must include hostname'), ('https://example.com/path', 'Origin should not include path')]
...
class SecurityBuilderTests:
    ...
    def test_validate_content_type_allowed(self):
        ...
        cors = CORSConfigurationBuilder()
        allowed_types = ['application/json', 'application/json; charset=utf-8', 'text/plain', 'multipart/form-data; boundary=----WebKitFormBoundary', '']
    ...
    def test_validate_content_type_suspicious(self):
        ...
        cors = CORSConfigurationBuilder()
        suspicious_types = ['text/x-shellscript', 'application/x-msdownload', 'text/vbscript', 'application/vnd.ms-excel']
    ...
    def test_log_security_event(self):
        ...
        cors.security.log_security_event('test_event', 'https://test.com', '/api/test', request_id='req-123', additional_info={'key': 'value'})
    ...
    def test_security_events_bounded(self):
        ...
        cors.security.clear_security_events()
        for i in range(1100):
            cors.security.log_security_event(f'test_event_{i}', f'https://test{i}.com', f'/api/test{i}')
...
class HealthBuilderTests:
    ...
    def test_validate_config(self):
        ...
        cors = CORSConfigurationBuilder()
        valid_config = {'allow_origins': ['https://example.com'], 'allow_credentials': True, 'allow_methods': ['GET', 'POST'], 'allow_headers': ['Content-Type'], 'expose_headers': ['X-Request-ID'], 'max_age': 3600}
        ...
        invalid_config = valid_config.copy()
        invalid_config['allow_credentials'] = 'true'
        assert cors.health.validate_config(invalid_config) is False
...
@pytest.mark.unit
class StaticAssetsBuilderTests:
...
class ValidationTests:
    ...
    def test_validate_production_with_wildcard(self):
        ...
        env_vars = {'ENVIRONMENT': 'production', 'CORS_ORIGINS': '*'}
        cors = CORSConfigurationBuilder(env_vars)
        valid, error = cors.validate()
        assert valid is False
...
class BackwardCompatibilityTests:
    ...
    def test_is_origin_allowed(self):
        ...
        allowed = is_origin_allowed('https://netrasystems.ai', [], environment='production')
