...
@pytest.mark.unit
class UserIsolationValidationTests(SSotAsyncTestCase):
    """Test user isolation in RequestScopedToolDispatcher SSOT implementation."""

    async def asyncSetUp(self):
        """Set up test fixtures."""
        await super().asyncSetUp()
        self.users = {'alice': UserExecutionContext(user_id='alice_user_id', thread_id='alice_thread_id', run_id=f'alice_run_{uuid.uuid4()}'), 'bob': UserExecutionContext(user_id='bob_user_id', thread_id='bob_thread_id', run_id=f'bob_run_{uuid.uuid4()}'), 'charlie': UserExecutionContext(user_id='charlie_user_id', thread_id='charlie_thread_id', run_id=f'charlie_run_{uuid.uuid4()}')}
    ...
    async def test_tool_registry_isolation_between_users(self):
        ...
        try:
            ...
            for user_name, user_context in self.users.items():
                dispatchers[user_name] = RequestScopedToolDispatcher(user_context=user_context, websocket_emitter=None)
            ...
            self.assertTrue(dispatchers['alice'].has_tool('alice_secret_tool'), 'Alice should have access to her own tool')
            self.assertFalse(dispatchers['alice'].has_tool('bob_confidential_tool'), "USER ISOLATION VIOLATION: Alice should NOT have access to Bob's tool")
            self.assertFalse(dispatchers['alice'].has_tool('charlie_private_tool'), "USER ISOLATION VIOLATION: Alice should NOT have access to Charlie's tool")
            self.assertTrue(dispatchers['bob'].has_tool('bob_confidential_tool'), 'Bob should have access to his own tool')
            self.assertFalse(dispatchers['bob'].has_tool('alice_secret_tool'), "USER ISOLATION VIOLATION: Bob should NOT have access to Alice's tool")
            self.assertFalse(dispatchers['bob'].has_tool('charlie_private_tool'), "USER ISOLATION VIOLATION: Bob should NOT have access to Charlie's tool")
            self.assertTrue(dispatchers['charlie'].has_tool('charlie_private_tool'), 'Charlie should have access to his own tool')
            self.assertFalse(dispatchers['charlie'].has_tool('alice_secret_tool'), "USER ISOLATION VIOLATION: Charlie should NOT have access to Alice's tool")
            self.assertFalse(dispatchers['charlie'].has_tool('bob_confidential_tool'), "USER ISOLATION VIOLATION: Charlie should NOT have access to Bob's tool")
            ...
            self.assertEqual(len(alice_tools), 1, f'Alice should have exactly 1 tool, got: {alice_tools}')
            self.assertEqual(len(bob_tools), 1, f'Bob should have exactly 1 tool, got: {bob_tools}')
            self.assertEqual(len(charlie_tools), 1, f'Charlie should have exactly 1 tool, got: {charlie_tools}')
    ...
    async def test_execution_context_isolation(self):
        ...
        try:
            ...
            dispatchers = {}
            for user_name, user_context in self.users.items():
                dispatchers[user_name] = RequestScopedToolDispatcher(user_context=user_context)
                ...
                dispatchers[user_name].register_tool('shared_tool', create_user_tool(user_name), f"{user_name}'s implementation")
            ...
            tasks = [execute_for_user(user_name, dispatcher) for user_name, dispatcher in dispatchers.items()]
            ...
            if 'alice' in execution_results:
                ...
                self.assertIn('ALICE_RESULT', alice_content.upper(), f'EXECUTION ISOLATION VIOLATION: Alice got wrong result: {alice_content}')
            if 'bob' in execution_results:
                ...
                self.assertIn('BOB_RESULT', bob_content.upper(), f'EXECUTION ISOLATION VIOLATION: Bob got wrong result: {bob_content}')
            if 'charlie' in execution_results:
                ...
                self.assertIn('CHARLIE_RESULT', charlie_content.upper(), f'EXECUTION ISOLATION VIOLATION: Charlie got wrong result: {charlie_content}')
            for user_name, result in execution_results.items():
                ...
                for other_user in other_users:
                    self.assertNotIn(f'{other_user.upper()}_RESULT', result_content, f"CROSS-USER DATA LEAKAGE: {user_name}'s result contains {other_user}'s data: {result_content}")
        except Exception as e:
        ...
        try:
            ...
            for user_name, user_context in self.users.items():
                ...
                def create_test_tool(user_identifier):
                    ...
                    return test_tool
                dispatchers[user_name].register_tool('test_tool', create_test_tool(user_name))
            await dispatchers['alice'].dispatch('test_tool', query='alice_test_1')
            await dispatchers['alice'].dispatch('test_tool', query='alice_test_2')
            await dispatchers['bob'].dispatch('test_tool', query='bob_test_1')
            ...
            await dispatchers['charlie'].dispatch('test_tool', query='charlie_test_3')
            ...
            self.assertNotEqual(alice_metrics['dispatcher_id'], bob_metrics['dispatcher_id'], 'METRICS ISOLATION VIOLATION: Alice and Bob have same dispatcher_id')
            self.assertNotEqual(bob_metrics['dispatcher_id'], charlie_metrics['dispatcher_id'], 'METRICS ISOLATION VIOLATION: Bob and Charlie have same dispatcher_id')
            self.assertNotEqual(alice_metrics['dispatcher_id'], charlie_metrics['dispatcher_id'], 'METRICS ISOLATION VIOLATION: Alice and Charlie have same dispatcher_id')
            self.assertEqual(alice_metrics['user_id'], 'alice_user_id', f"METRICS USER_ID VIOLATION: Alice metrics have wrong user_id: {alice_metrics['user_id']}")
            self.assertEqual(bob_metrics['user_id'], 'bob_user_id', f"METRICS USER_ID VIOLATION: Bob metrics have wrong user_id: {bob_metrics['user_id']}")
            self.assertEqual(charlie_metrics['user_id'], 'charlie_user_id', f"METRICS USER_ID VIOLATION: Charlie metrics have wrong user_id: {charlie_metrics['user_id']}")
            self.assertNotEqual(alice_metrics['run_id'], bob_metrics['run_id'], 'METRICS RUN_ID VIOLATION: Alice and Bob have same run_id')
            self.assertNotEqual(bob_metrics['run_id'], charlie_metrics['run_id'], 'METRICS RUN_ID VIOLATION: Bob and Charlie have same run_id')
            self.assertEqual(alice_metrics['tools_executed'], 2, f"METRICS COUNT VIOLATION: Alice should have 2 tool executions, got: {alice_metrics['tools_executed']}")
            self.assertEqual(bob_metrics['tools_executed'], 1, f"METRICS COUNT VIOLATION: Bob should have 1 tool execution, got: {bob_metrics['tools_executed']}")
            self.assertEqual(charlie_metrics['tools_executed'], 3, f"METRICS COUNT VIOLATION: Charlie should have 3 tool executions, got: {charlie_metrics['tools_executed']}")
        except Exception as e:
    ...
    async def test_memory_isolation_and_cleanup(self):
        ...
        try:
            from netra_backend.app.agents.request_scoped_tool_dispatcher import RequestScopedToolDispatcher
            for i in range(10):
                user_context = UserExecutionContext(user_id=f'memory_test_user_{i}', thread_id=f'memory_test_thread_{i}', run_id=f'memory_test_run_{uuid.uuid4()}')
                dispatcher = RequestScopedToolDispatcher(user_context=user_context)
                dispatcher_refs.append(weakref.ref(dispatcher))
                dispatcher.register_tool(f'test_tool_{i}', lambda x: f'Result {i}: {x}', f'Test tool {i}')
            ...
            self.assertEqual(live_dispatchers, 0, f'MEMORY LEAK VIOLATION: {live_dispatchers} dispatchers still alive after cleanup. All dispatchers should be garbage collected after cleanup.')
            final_objects = len(gc.get_objects())
            object_growth = final_objects - initial_objects
            max_allowed_growth = 1000
            self.assertLess(object_growth, max_allowed_growth, f'MEMORY GROWTH VIOLATION: Created {object_growth} new objects, expected less than {max_allowed_growth}. Possible memory leak.')
        except Exception as e:
    ...
    async def test_concurrent_user_execution_isolation(self):
        ...
        try:
            ...
            async def create_and_execute_user(user_id: int):
                ...
                try:
                    user_context = UserExecutionContext(user_id=f'concurrent_user_{user_id}', thread_id=f'concurrent_thread_{user_id}', run_id=f'concurrent_run_{user_id}_{uuid.uuid4()}')
                    dispatcher = RequestScopedToolDispatcher(user_context=user_context)
                    ...
                    for exec_id in range(num_executions_per_user):
                        result = await dispatcher.dispatch(f'user_{user_id}_tool', query=f'exec_{exec_id}')
                        user_results.append(result)
                    await dispatcher.cleanup()
                    return (user_id, user_results, None)
                except Exception as e:
                    return (user_id, None, str(e))
            tasks = [create_and_execute_user(user_id) for user_id in range(num_concurrent_users)]
            ...
            self.assertEqual(len(errors), 0, f'CONCURRENCY ERROR VIOLATION: Errors occurred during concurrent execution: {errors[:5]}')
            self.assertEqual(len(results), num_concurrent_users, f'CONCURRENCY COMPLETION VIOLATION: Only {len(results)}/{num_concurrent_users} users completed successfully')
            for user_id, user_results in results.items():
                self.assertEqual(len(user_results), num_executions_per_user, f'CONCURRENCY RESULT COUNT VIOLATION: User {user_id} has {len(user_results)}/{num_executions_per_user} results')
                for result in user_results:
                    result_content = str(result).upper()
                    expected_user_marker = f'USER_{user_id}_RESULT'
                    self.assertIn(expected_user_marker.upper(), result_content, f"CONCURRENCY ISOLATION VIOLATION: User {user_id} result doesn't contain expected marker: {result_content}")
                    for other_user_id in range(num_concurrent_users):
                        if other_user_id != user_id:
                            other_user_marker = f'USER_{other_user_id}_RESULT'
                            self.assertNotIn(other_user_marker.upper(), result_content, f'CONCURRENCY CROSS-USER VIOLATION: User {user_id} result contains User {other_user_id} data: {result_content}')
        except Exception as e:
            self.fail(f'Concurrent user execution isolation test failed: {e}')

@pytest.mark.unit
class ResourceCleanupIsolationTests(SSotAsyncTestCase):
    """Test resource cleanup and disposal isolation."""

    async def asyncSetUp(self):
        """Set up test fixtures."""
        await super().asyncSetUp()
        self.user_context = UserExecutionContext(user_id='cleanup_test_user', thread_id='cleanup_test_thread', run_id=f'cleanup_test_run_{uuid.uuid4()}')
    ...
    async def test_dispatcher_cleanup_prevents_further_usage(self):
        ...
        try:
            ...
            await dispatcher.cleanup()
            self.assertFalse(dispatcher.is_active(), 'CLEANUP VIOLATION: Dispatcher should be inactive after cleanup')
