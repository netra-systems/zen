...
class TestCloudEnvironmentDetector:
    ...
    async def test_cloud_run_staging_detection_success(self, detector, clear_cache):
        ...
        mock_responses = {'instance/attributes/goog-cloudrun-service-name': 'netra-backend-staging', 'project/project-id': 'netra-staging-test', 'instance/region': 'projects/123456/regions/us-central1', 'instance/attributes/goog-revision': 'netra-backend-staging-00001-abc'}
    ...
    async def test_cloud_run_production_detection_success(self, detector, clear_cache):
        ...
        mock_responses = {'instance/attributes/goog-cloudrun-service-name': 'netra-backend', 'project/project-id': 'netra-production', 'instance/region': 'projects/123456/regions/us-central1', 'instance/attributes/goog-revision': 'netra-backend-00001-xyz'}
    ...
    async def test_gcp_project_staging_detection(self, detector, clear_cache):
        ...
        with patch.dict(os.environ, {'GOOGLE_CLOUD_PROJECT': 'netra-staging-test', 'K_REVISION': 'netra-backend-staging-00001'}, clear=True):
            with patch.object(detector, '_fetch_metadata', return_value=None):
                context = await detector.detect_environment_context()
                assert context.environment_type == EnvironmentType.STAGING
                assert context.project_id == 'netra-staging-test'
    ...
    async def test_app_engine_detection(self, detector, clear_cache):
        ...
        with patch.dict(os.environ, {'GAE_APPLICATION': 'netra-staging-app', 'GAE_VERSION': 'staging-v1', 'GAE_SERVICE': 'default'}, clear=True):
            with patch.object(detector, '_fetch_metadata', return_value=None):
                context = await detector.detect_environment_context()
                assert context.environment_type == EnvironmentType.STAGING
    ...
    async def test_detection_failure_low_confidence(self, detector, clear_cache):
        ...
        env_vars_to_clear = ['ENVIRONMENT', 'K_SERVICE', 'GOOGLE_CLOUD_PROJECT', 'GAE_APPLICATION', 'GAE_VERSION', 'GAE_SERVICE']
    ...
    def test_service_name_classification(self, detector):
        ...
        test_cases = [('netra-backend-staging', EnvironmentType.STAGING), ('netra-backend', EnvironmentType.PRODUCTION), ('api-prod-v1', EnvironmentType.PRODUCTION), ('service-dev-test', EnvironmentType.DEVELOPMENT), ('unknown-service', EnvironmentType.UNKNOWN)]
    ...
    def test_environment_type_parsing(self, detector):
        ...
        test_cases = [('staging', EnvironmentType.STAGING), ('PRODUCTION', EnvironmentType.PRODUCTION), ('dev', EnvironmentType.DEVELOPMENT), ('test', EnvironmentType.TESTING), ('unknown', EnvironmentType.UNKNOWN)]
...
class TestEnvironmentDetectionIntegration:
    ...
    async def test_real_cloud_run_metadata_structure(self):
        ...
        detector.clear_cache()
        realistic_responses = {'instance/attributes/goog-cloudrun-service-name': 'netra-backend-staging', 'project/project-id': 'netra-staging-12345', 'instance/region': 'projects/123456789/regions/us-central1', 'instance/attributes/goog-revision': 'netra-backend-staging-00001-rev'}
    ...
    async def test_golden_path_critical_scenario(self):
        ...
        detector = CloudEnvironmentDetector()
        detector.clear_cache()
        with patch.dict(os.environ, {'K_SERVICE': 'netra-backend-staging', 'GOOGLE_CLOUD_PROJECT': 'netra-staging', 'K_REVISION': 'netra-backend-staging-00001-abc'}, clear=False):
            realistic_metadata = {'instance/attributes/goog-cloudrun-service-name': 'netra-backend-staging', 'project/project-id': 'netra-staging', 'instance/region': 'projects/123456/regions/us-central1'}
...
class TestEnvironmentDetectionFailureCases:
    ...
    async def test_partial_metadata_availability(self):
        ...
        detector = CloudEnvironmentDetector()
        detector.clear_cache()
        partial_responses = {'instance/attributes/goog-cloudrun-service-name': 'netra-backend-staging'}
    ...
    async def test_conflicting_environment_signals(self):
        ...
        detector = CloudEnvironmentDetector()
        detector.clear_cache()
        with patch.dict(os.environ, {'ENVIRONMENT': 'production', 'K_SERVICE': 'netra-backend-staging'}, clear=False):
            with patch.object(detector, '_fetch_metadata', return_value=None):
                context = await detector.detect_environment_context()
                assert context.environment_type == EnvironmentType.PRODUCTION
                assert context.confidence_score >= 0.7
