...
class WebSocketEvent:
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {'type': self.event_type.value, 'user_id': str(self.user_id), 'thread_id': str(self.thread_id), 'run_id': str(self.run_id), 'timestamp': self.timestamp, 'agent_name': self.agent_name, 'tool_name': self.tool_name, 'data': self.data or {}, 'event_id': self.event_id}

    @classmethod
    def from_dict(cls, event_dict: Dict[str, Any]) -> 'WebSocketEvent':
        """Create event from dictionary."""
        return cls(event_type=WebSocketEventType(event_dict['type']), user_id=UserID(event_dict['user_id']), thread_id=ThreadID(event_dict['thread_id']), run_id=RunID(event_dict['run_id']), timestamp=event_dict.get('timestamp', time.time()), agent_name=event_dict.get('agent_name'), tool_name=event_dict.get('tool_name'), data=event_dict.get('data', {}), event_id=event_dict.get('event_id'))
...
class WebSocketEventValidator:
    ...
    def _get_required_events(self) -> Set[WebSocketEventType]:
        ...
        return {WebSocketEventType.AGENT_STARTED, WebSocketEventType.AGENT_THINKING, WebSocketEventType.TOOL_EXECUTING, WebSocketEventType.TOOL_COMPLETED, WebSocketEventType.AGENT_COMPLETED}
    ...
    def _get_event_order_rules(self) -> Dict[WebSocketEventType, List[WebSocketEventType]]:
        ...
        return {WebSocketEventType.AGENT_STARTED: [], WebSocketEventType.AGENT_THINKING: [WebSocketEventType.AGENT_STARTED], WebSocketEventType.TOOL_EXECUTING: [WebSocketEventType.AGENT_STARTED, WebSocketEventType.AGENT_THINKING], WebSocketEventType.TOOL_COMPLETED: [WebSocketEventType.AGENT_STARTED, WebSocketEventType.AGENT_THINKING, WebSocketEventType.TOOL_EXECUTING], WebSocketEventType.AGENT_COMPLETED: [WebSocketEventType.AGENT_STARTED, WebSocketEventType.AGENT_THINKING]}

    def _get_event_content_rules(self) -> Dict[WebSocketEventType, Dict[str, Any]]:
        """Get content validation rules for each event type."""
        return {WebSocketEventType.AGENT_STARTED: {'required_fields': ['agent_name'], 'optional_fields': ['user_message', 'context'], 'data_type': dict}, WebSocketEventType.AGENT_THINKING: {'required_fields': ['agent_name'], 'optional_fields': ['reasoning', 'next_steps'], 'data_type': dict}, WebSocketEventType.TOOL_EXECUTING: {'required_fields': ['agent_name', 'tool_name'], 'optional_fields': ['tool_params', 'expected_duration'], 'data_type': dict}, WebSocketEventType.TOOL_COMPLETED: {'required_fields': ['agent_name', 'tool_name'], 'optional_fields': ['tool_result', 'execution_time'], 'data_type': dict}, WebSocketEventType.AGENT_COMPLETED: {'required_fields': ['agent_name'], 'optional_fields': ['final_response', 'execution_summary'], 'data_type': dict}}
    ...
    def validate_event_sequence(self, events: List[WebSocketEvent]) -> Dict[str, Any]:
        ...
        """
        validation_result = {'valid': True, 'errors': [], 'warnings': [], 'missing_events': [], 'order_violations': [], 'content_errors': [], 'event_count': len(events), 'coverage_percentage': 0.0}
        if not events:
        ...
        if missing_events:
            ...
            validation_result['errors'].extend([f'Missing critical event: {event.value}' for event in missing_events])
            validation_result['valid'] = False
        validation_result['coverage_percentage'] = len(event_types_received & self.required_events) / len(self.required_events) * 100
        ...
        if content_errors:
            ...
            validation_result['valid'] = False
        if len(events) > 20:
            validation_result['warnings'].append(f'High event count ({len(events)}) may indicate performance issues')
        return validation_result
        ...
        for agent_name, agent_event_list in agent_events.items():
            ...
            for i, event in enumerate(agent_event_list):
                ...
                if missing_prerequisites:
                    violations.append(f'Agent {agent_name}: {event.event_type.value} missing prerequisites: {[p.value for p in missing_prerequisites]}')
        return violations
    ...
    def _validate_event_content(self, event: WebSocketEvent, rules: Dict[str, Any]) -> List[str]:
        ...
        if not isinstance(event.data, expected_type):
            errors.append(f'Event {event.event_type.value}: data should be {expected_type.__name__}, got {type(event.data).__name__}')
            return errors
    ...
    def validate_single_event(self, event: WebSocketEvent) -> Dict[str, Any]:
        ...
        validation_result = {'valid': True, 'errors': [], 'warnings': [], 'event_type': event.event_type.value, 'is_critical': event.event_type in self.required_events}
        if event.event_type in self.event_content_rules:
            rules = self.event_content_rules[event.event_type]
        ...
        elif event.timestamp < current_time - 3600:
            validation_result['warnings'].append('Event timestamp is very old')
        return validation_result

    def get_missing_events_for_completion(self, received_events: List[WebSocketEvent]) -> List[WebSocketEventType]:
...
class WebSocketEventValidatorTests(SSotAsyncTestCase):
    ...
    def _create_test_event(self, event_type: WebSocketEventType, agent_name: str='test_agent', tool_name: str=None, data: Dict[str, Any]=None, timestamp: float=None) -> WebSocketEvent:
        """Helper to create test events."""
        return WebSocketEvent(event_type=event_type, user_id=self.user_id, thread_id=self.thread_id, run_id=self.run_id, agent_name=agent_name, tool_name=tool_name, data=data or {}, timestamp=timestamp or time.time())

    @pytest.mark.unit
    def test_complete_golden_path_event_sequence_validation(self):
        """Test validation of complete Golden Path event sequence."""
        base_time = time.time()
        events = [self._create_test_event(WebSocketEventType.AGENT_STARTED, agent_name='data_agent', data={'message': 'Starting data analysis'}, timestamp=base_time), self._create_test_event(WebSocketEventType.AGENT_THINKING, agent_name='data_agent', data={'reasoning': 'Analyzing cost data'}, timestamp=base_time + 1), self._create_test_event(WebSocketEventType.TOOL_EXECUTING, agent_name='data_agent', tool_name='cost_analyzer', data={'tool_params': {'period': '30d'}}, timestamp=base_time + 2), self._create_test_event(WebSocketEventType.TOOL_COMPLETED, agent_name='data_agent', tool_name='cost_analyzer', data={'tool_result': {'total_cost': 1000}}, timestamp=base_time + 3), self._create_test_event(WebSocketEventType.AGENT_COMPLETED, agent_name='data_agent', data={'final_response': 'Analysis complete'}, timestamp=base_time + 4)]
    ...
    def test_missing_critical_events_detection(self):
        ...
        incomplete_events = [self._create_test_event(WebSocketEventType.AGENT_STARTED, agent_name='data_agent'), self._create_test_event(WebSocketEventType.AGENT_THINKING, agent_name='data_agent'), self._create_test_event(WebSocketEventType.AGENT_COMPLETED, agent_name='data_agent')]
        ...
        assert WebSocketEventType.TOOL_EXECUTING in result['missing_events'], 'Should detect missing tool_executing'
        assert WebSocketEventType.TOOL_COMPLETED in result['missing_events'], 'Should detect missing tool_completed'
    ...
    def test_event_order_violation_detection(self):
        ...
        base_time = time.time()
        wrong_order_events = [self._create_test_event(WebSocketEventType.TOOL_EXECUTING, agent_name='data_agent', tool_name='analyzer', timestamp=base_time), self._create_test_event(WebSocketEventType.AGENT_STARTED, agent_name='data_agent', timestamp=base_time + 1), self._create_test_event(WebSocketEventType.AGENT_THINKING, agent_name='data_agent', timestamp=base_time + 2), self._create_test_event(WebSocketEventType.TOOL_COMPLETED, agent_name='data_agent', tool_name='analyzer', timestamp=base_time + 3), self._create_test_event(WebSocketEventType.AGENT_COMPLETED, agent_name='data_agent', timestamp=base_time + 4)]
    ...
    def test_event_content_validation(self):
        ...
        invalid_event = self._create_test_event(WebSocketEventType.TOOL_EXECUTING, agent_name=None, tool_name=None, data={})
        ...
        assert any(('agent_name' in error for error in validation['errors'])), 'Should detect missing agent_name'
        assert any(('tool_name' in error for error in validation['errors'])), 'Should detect missing tool_name'
        valid_event = self._create_test_event(WebSocketEventType.TOOL_EXECUTING, agent_name='data_agent', tool_name='cost_analyzer', data={'tool_params': {'period': '30d'}})
    ...
    def test_event_quality_scoring(self):
        ...
        perfect_events = [self._create_test_event(WebSocketEventType.AGENT_STARTED, agent_name='agent1'), self._create_test_event(WebSocketEventType.AGENT_THINKING, agent_name='agent1'), self._create_test_event(WebSocketEventType.TOOL_EXECUTING, agent_name='agent1', tool_name='tool1'), self._create_test_event(WebSocketEventType.TOOL_COMPLETED, agent_name='agent1', tool_name='tool1'), self._create_test_event(WebSocketEventType.AGENT_COMPLETED, agent_name='agent1')]
        ...
        imperfect_events = [self._create_test_event(WebSocketEventType.AGENT_STARTED, agent_name='agent1'), self._create_test_event(WebSocketEventType.AGENT_COMPLETED, agent_name='agent1')]
    ...
    def test_multi_agent_event_sequence_validation(self):
        ...
        base_time = time.time()
        multi_agent_events = [self._create_test_event(WebSocketEventType.AGENT_STARTED, agent_name='data_agent', timestamp=base_time), self._create_test_event(WebSocketEventType.AGENT_THINKING, agent_name='data_agent', timestamp=base_time + 1), self._create_test_event(WebSocketEventType.TOOL_EXECUTING, agent_name='data_agent', tool_name='data_collector', timestamp=base_time + 2), self._create_test_event(WebSocketEventType.TOOL_COMPLETED, agent_name='data_agent', tool_name='data_collector', timestamp=base_time + 3), self._create_test_event(WebSocketEventType.AGENT_COMPLETED, agent_name='data_agent', timestamp=base_time + 4), self._create_test_event(WebSocketEventType.AGENT_STARTED, agent_name='optimization_agent', timestamp=base_time + 5), self._create_test_event(WebSocketEventType.AGENT_THINKING, agent_name='optimization_agent', timestamp=base_time + 6), self._create_test_event(WebSocketEventType.TOOL_EXECUTING, agent_name='optimization_agent', tool_name='optimizer', timestamp=base_time + 7), self._create_test_event(WebSocketEventType.TOOL_COMPLETED, agent_name='optimization_agent', tool_name='optimizer', timestamp=base_time + 8), self._create_test_event(WebSocketEventType.AGENT_COMPLETED, agent_name='optimization_agent', timestamp=base_time + 9)]
    ...
    def test_get_missing_events_for_completion(self):
        ...
        partial_events = [self._create_test_event(WebSocketEventType.AGENT_STARTED, agent_name='agent1'), self._create_test_event(WebSocketEventType.AGENT_THINKING, agent_name='agent1'), self._create_test_event(WebSocketEventType.AGENT_COMPLETED, agent_name='agent1')]
        ...
        complete_events = [self._create_test_event(WebSocketEventType.AGENT_STARTED, agent_name='agent1'), self._create_test_event(WebSocketEventType.AGENT_THINKING, agent_name='agent1'), self._create_test_event(WebSocketEventType.TOOL_EXECUTING, agent_name='agent1', tool_name='tool1'), self._create_test_event(WebSocketEventType.TOOL_COMPLETED, agent_name='agent1', tool_name='tool1'), self._create_test_event(WebSocketEventType.AGENT_COMPLETED, agent_name='agent1')]
    ...
    def test_event_serialization_and_deserialization(self):
        ...
        original_event = self._create_test_event(WebSocketEventType.TOOL_EXECUTING, agent_name='test_agent', tool_name='test_tool', data={'param1': 'value1', 'param2': 42})
    ...
    def test_timestamp_validation(self):
        ...
        current_time = time.time()
        future_event = self._create_test_event(WebSocketEventType.AGENT_STARTED, timestamp=current_time + 120)
        ...
        assert any(('future' in warning.lower() for warning in future_result['warnings'])), 'Should specifically warn about future timestamp'
        old_event = self._create_test_event(WebSocketEventType.AGENT_STARTED, timestamp=current_time - 7200)
        ...
        assert any(('old' in warning.lower() for warning in old_result['warnings'])), 'Should specifically warn about old timestamp'
        normal_event = self._create_test_event(WebSocketEventType.AGENT_STARTED, timestamp=current_time)
        normal_result = self.event_validator.validate_single_event(normal_event)
        assert len([w for w in normal_result['warnings'] if 'timestamp' in w.lower()]) == 0, 'Should not warn about normal timestamp'
