...
class AgentExecutionOrderValidator:
    ...
    def _get_required_execution_order(self) -> List[AgentType]:
        ...
        return [AgentType.DATA, AgentType.OPTIMIZATION, AgentType.REPORT]

    def _build_dependency_map(self) -> Dict[AgentType, List[AgentType]]:
        """Build dependency map for agent execution order."""
        return {AgentType.DATA: [], AgentType.OPTIMIZATION: [AgentType.DATA], AgentType.REPORT: [AgentType.DATA, AgentType.OPTIMIZATION], AgentType.TRIAGE: []}
    ...
    def validate_execution_order(self, proposed_order: List[AgentType]) -> Dict[str, Any]:
        ...
        """
        validation_result = {'valid': True, 'errors': [], 'warnings': [], 'corrected_order': None, 'dependency_violations': [], 'missing_agents': [], 'extra_agents': []}
    ...
    def check_agent_dependencies_met(self, agent_type: AgentType, completed_agents: List[AgentType]) -> Dict[str, Any]:
        ...
        return {'dependencies_met': len(missing_deps) == 0, 'required_dependencies': dependencies, 'missing_dependencies': missing_deps, 'can_execute': len(missing_deps) == 0}
    ...
    def get_next_executable_agents(self, completed_agents: List[AgentType]) -> List[AgentType]:
        """Get list of agents that can execute next based on completed agents."""
        executable = []
        core_pipeline_complete = all((agent in completed_agents for agent in self.required_order))
...
class TestAgentExecutionOrderValidator(SSotAsyncTestCase):
    ...
    def test_correct_agent_execution_order_validation(self):
        ...
        assert len(result['dependency_violations']) == 0, f"Should have no dependency violations: {result['dependency_violations']}"
    ...
    def test_missing_required_agents_detection(self):
        ...
        assert AgentType.OPTIMIZATION in result['missing_agents'], f"Should detect missing optimization agent: {result['missing_agents']}"
    ...
    def test_extra_agents_handling(self):
        ...
        assert AgentType.TRIAGE in result['extra_agents'], f"Should identify triage as extra: {result['extra_agents']}"
    ...
    def test_agent_dependency_checking(self):
        ...
        data_deps = self.order_validator.check_agent_dependencies_met(AgentType.DATA, [])
        ...
        opt_deps_without_data = self.order_validator.check_agent_dependencies_met(AgentType.OPTIMIZATION, [])
        assert opt_deps_without_data['dependencies_met'] is False, 'Optimization agent should need data dependency'
        assert AgentType.DATA in opt_deps_without_data['missing_dependencies'], 'Should identify data as missing dependency'
        opt_deps_with_data = self.order_validator.check_agent_dependencies_met(AgentType.OPTIMIZATION, [AgentType.DATA])
        assert opt_deps_with_data['dependencies_met'] is True, 'Optimization agent should be satisfied with data'
        assert opt_deps_with_data['can_execute'] is True, 'Optimization agent should be executable with data'
        report_deps_partial = self.order_validator.check_agent_dependencies_met(AgentType.REPORT, [AgentType.DATA])
        assert report_deps_partial['dependencies_met'] is False, 'Report agent needs both data and optimization'
        assert AgentType.OPTIMIZATION in report_deps_partial['missing_dependencies'], 'Should identify optimization as missing'
        report_deps_complete = self.order_validator.check_agent_dependencies_met(AgentType.REPORT, [AgentType.DATA, AgentType.OPTIMIZATION])
        assert report_deps_complete['dependencies_met'] is True, 'Report agent should be satisfied with both dependencies'
    ...
    def test_corrected_order_generation(self):
        ...
        test_cases = [{'wrong_order': [AgentType.REPORT, AgentType.DATA, AgentType.OPTIMIZATION], 'description': 'Completely reversed order'}, {'wrong_order': [AgentType.OPTIMIZATION, AgentType.REPORT, AgentType.DATA], 'description': 'Random wrong order'}, {'wrong_order': [AgentType.DATA, AgentType.REPORT], 'description': 'Missing middle agent'}]
        for case in test_cases:
            ...
            assert corrected_result['valid'] is True, f"Corrected order should be valid for: {case['description']}"
    ...
    def test_next_executable_agents_calculation(self):
        ...
        assert AgentType.OPTIMIZATION in next_after_data, 'Optimization should be executable after data'
        assert AgentType.REPORT not in next_after_data, 'Report should still not be executable'
        next_after_opt = self.order_validator.get_next_executable_agents([AgentType.DATA, AgentType.OPTIMIZATION])
        assert AgentType.REPORT in next_after_opt, 'Report should be executable after data and optimization'
        next_after_all = self.order_validator.get_next_executable_agents([AgentType.DATA, AgentType.OPTIMIZATION, AgentType.REPORT])
    ...
    def test_execution_context_tracking(self):
        ...
        data_context = AgentExecutionContext(agent_type=AgentType.DATA, user_id=user_id, thread_id=thread_id, run_id=run_id)
        ...
        assert len(self.order_validator.execution_contexts) == 1, 'Should track execution context'
    ...
    def test_actual_execution_order_validation(self):
        ...
        run_id = RunID(UnifiedIDManager.generate_run_id(str(thread_id)))
        base_time = time.time()
        contexts = [AgentExecutionContext(agent_type=AgentType.DATA, user_id=user_id, thread_id=thread_id, run_id=run_id, start_time=base_time), AgentExecutionContext(agent_type=AgentType.OPTIMIZATION, user_id=user_id, thread_id=thread_id, run_id=run_id, start_time=base_time + 1.0), AgentExecutionContext(agent_type=AgentType.REPORT, user_id=user_id, thread_id=thread_id, run_id=run_id, start_time=base_time + 2.0)]
    ...
    def test_concurrent_agent_dependency_resolution(self):
        ...
        assert AgentType.OPTIMIZATION not in concurrent_executable, 'Optimization cannot run concurrently with data'
        after_data = self.order_validator.get_next_executable_agents([AgentType.DATA])
        assert AgentType.OPTIMIZATION in after_data, 'Optimization should be executable after data'
        triage_only = self.order_validator.get_next_executable_agents([AgentType.TRIAGE])
        assert AgentType.OPTIMIZATION not in triage_only, 'Optimization should not be executable with only triage complete'
