...
class TestDatabaseConfigSSotCompliance:
    ...
    def test_ssot_fallback_violation_detection(self):
        ...
        manual_url_pattern = 'f["\\\']postgresql://.*?["\\\']'
        ...
        if direct_string_construction or f_string_violation:
            pytest.fail(f'SSOT VIOLATION DETECTED: _fallback_manual_url_construction() contains manual URL construction. This violates SSOT principles. The fallback method must also use DatabaseURLBuilder. Violation found in source: {fallback_source}')
    ...
    def test_import_error_recovery_with_ssot_check(self):
        ...
        with patch('shared.database_url_builder.DatabaseURLBuilder', side_effect=ImportError('Module not found')):
            with patch('shared.isolated_environment.get_env') as mock_get_env:
                mock_env = MagicMock()
                mock_env.get.side_effect = lambda key, default: {'POSTGRES_HOST': 'localhost', 'POSTGRES_PORT': '5432', 'POSTGRES_USER': 'testuser', 'POSTGRES_PASSWORD': 'testpass', 'POSTGRES_DB': 'testdb'}.get(key, default)
                mock_get_env.return_value = mock_env
                result = self.config.get_database_url()
                assert result is not None
                assert 'postgresql://' in result
                fallback_source = inspect.getsource(self.config._fallback_manual_url_construction)
                assert 'f"postgresql://' not in fallback_source, 'Fallback method contains SSOT violation - manual URL construction detected'
    ...
    def test_comprehensive_ssot_violation_scan(self):
        ...
        """
        config_methods = [self.config.get_database_url, self.config._fallback_manual_url_construction]
        violations = []
        for method in config_methods:
            ...
            if '"postgresql://' in method_source and 'f"postgresql://' not in method_source:
                if re.search('"postgresql://[^"]*"', method_source):
                    violations.append(f'Method {method_name}: Direct string URL construction')
        if violations:
            violation_details = '\n'.join(violations)
            pytest.fail(f'SSOT VIOLATIONS DETECTED in database configuration:\n{violation_details}\n\nAll database URL construction must use DatabaseURLBuilder from shared.database_url_builder. Manual URL construction violates SSOT principles and can cause configuration drift.')
