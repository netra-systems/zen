...
class RedisConfigurationValidationTests:
    ...
    def test_staging_redis_url_not_localhost(self):
        ...
        with patch.dict(os.environ, {'ENVIRONMENT': 'staging', 'REDIS_HOST': 'redis-staging-memory-store.googleapis.com', 'REDIS_PORT': '6379', 'REDIS_URL': 'redis://redis-staging-memory-store.googleapis.com:6379/0'}):
            env = IsolatedEnvironment()
            backend_env = BackendEnvironment(env_manager=env)
            redis_url = backend_env.get_redis_url()
            print(f'Staging Redis URL: {redis_url}')
            assert 'localhost' not in redis_url.lower(), f"CRITICAL FAILURE: Staging Redis URL contains 'localhost': {redis_url}. This will cause GCP Memory Store connectivity failure!"
            expected_gcp_patterns = ['googleapis.com', 'memory-store', 'redis-staging']
            ...
            assert ':6379' in redis_url, f'Redis URL should use port 6379, got: {redis_url}'
    ...
    def test_production_redis_url_not_localhost(self):
        ...
        with patch.dict(os.environ, {'ENVIRONMENT': 'production', 'REDIS_HOST': 'redis-production-memory-store.googleapis.com', 'REDIS_PORT': '6379', 'REDIS_URL': 'redis://redis-production-memory-store.googleapis.com:6379/0'}):
            env = IsolatedEnvironment()
            backend_env = BackendEnvironment(env_manager=env)
            redis_url = backend_env.get_redis_url()
            print(f'Production Redis URL: {redis_url}')
            assert 'localhost' not in redis_url.lower(), f"CRITICAL FAILURE: Production Redis URL contains 'localhost': {redis_url}. This will cause GCP Memory Store connectivity failure!"
            assert redis_url.startswith('redis://'), f"Redis URL should start with 'redis://', got: {redis_url}"
    ...
    def test_test_environment_can_use_localhost(self):
        ...
        with patch.dict(os.environ, {'ENVIRONMENT': 'test', 'REDIS_HOST': 'localhost', 'REDIS_PORT': '6381', 'REDIS_URL': 'redis://localhost:6381/0'}):
            env = IsolatedEnvironment()
            backend_env = BackendEnvironment(env_manager=env)
            redis_url = backend_env.get_redis_url()
            print(f'Test Redis URL: {redis_url}')
            assert 'localhost' in redis_url, f'Test environment should be able to use localhost, got: {redis_url}'
            assert ':6381' in redis_url, f'Test environment should use port 6381, got: {redis_url}'
    ...
    def test_redis_port_configuration_by_environment(self):
        ...
        test_cases = [{'environment': 'test', 'expected_port': '6381', 'description': 'Test environment uses port 6381'}, {'environment': 'staging', 'expected_port': '6379', 'description': 'Staging environment uses standard port 6379'}, {'environment': 'production', 'expected_port': '6379', 'description': 'Production environment uses standard port 6379'}]
        for case in test_cases:
            print(f"  Testing {case['environment']} environment port configuration")
            env_vars = {'ENVIRONMENT': case['environment'], 'REDIS_PORT': case['expected_port']}
            ...
            with patch.dict(os.environ, env_vars):
                ...
                assert f":{case['expected_port']}" in redis_url, f"{case['description']} failed. Expected port {case['expected_port']} in URL: {redis_url}"
    ...
    def test_deprecated_redis_url_detection(self):
        ...
        with patch.dict(os.environ, {'ENVIRONMENT': 'staging', 'REDIS_URL': 'redis://localhost:6379/0'}):
            env = IsolatedEnvironment()
            redis_url_from_env = env.get('REDIS_URL')
            print(f'Deprecated REDIS_URL detected: {redis_url_from_env}')
            assert redis_url_from_env == 'redis://localhost:6379/0', f'Expected to detect deprecated pattern, got: {redis_url_from_env}'
    ...
    def test_redis_connection_config_validation(self):
        ...
        valid_config = RedisConnectionConfig(url='redis://redis-staging.googleapis.com:6379/0', host='redis-staging.googleapis.com', port=6379, db=0, timeout=5.0)
        print(f'Valid config: {valid_config}')
        assert valid_config.url.startswith('redis://'), f'Valid config URL should start with redis://, got: {valid_config.url}'
        assert valid_config.port == 6379, f'Valid config should use port 6379, got: {valid_config.port}'
        localhost_config = RedisConnectionConfig(url='redis://localhost:6379/0', host='localhost', port=6379, db=0, timeout=5.0)
        print(f'Localhost config: {localhost_config}')
        assert 'localhost' in localhost_config.host, f'Should detect localhost in config, got host: {localhost_config.host}'
        ...
        def is_staging_safe_config(config: RedisConnectionConfig, environment: str) -> bool:
            ...
            return True
        assert is_staging_safe_config(valid_config, 'staging'), f'Valid config should be staging-safe: {valid_config.host}'
        assert not is_staging_safe_config(localhost_config, 'staging'), f'Localhost config should not be staging-safe: {localhost_config.host}'
    ...
    def test_redis_configuration_error_handling(self):
        ...
        with patch.dict(os.environ, {}, clear=True):
            ...
            try:
                ...
                assert redis_url is not None, 'Should provide fallback Redis URL when configuration missing'
                assert isinstance(redis_url, str), f'Redis URL should be string, got: {type(redis_url)}'
            except Exception as e:
                print(f'Configuration error (expected): {e}')
                assert 'redis' in str(e).lower() or 'configuration' in str(e).lower(), f'Error message should mention Redis or configuration: {e}'
        with patch.dict(os.environ, {'REDIS_URL': 'invalid-url-format'}):
            env = IsolatedEnvironment()
            try:
                backend_env = BackendEnvironment(env_manager=env)
                redis_url = backend_env.get_redis_url()
                assert isinstance(redis_url, str), f'Invalid Redis URL should still return string, got: {type(redis_url)}'
        ...
        for environment in environments:
            ...
            with patch.dict(os.environ, env_vars, clear=True):
                ...
                try:
                    ...
                    if environment == 'test':
                        expected_port = '6381'
                        assert expected_port in redis_url, f'Test environment should default to port {expected_port}, got: {redis_url}'
                    ...
                    assert redis_url.startswith('redis://'), f'{environment} Redis URL should start with redis://, got: {redis_url}'
                    print(f'     PASS:  {environment} defaults validated')
                except Exception as e:
                    print(f'     FAIL:  {environment} default config error: {e}')
                    assert 'redis' in str(e).lower(), f'{environment} error should mention Redis: {e}'
        print(' PASS:  PASS: Environment-specific Redis defaults validated')
UNIT_TEST_METADATA = {'suite_name': 'Redis Configuration Validation Infrastructure', 'test_layer': 'Unit', 'focus': 'Configuration logic and environment-specific patterns', 'business_impact': 'Prevents Redis configuration issues causing GCP connectivity failures', 'test_strategy': {'mocking': 'Environment variables and configuration objects', 'real_services': 'No Redis connections tested (unit focus)', 'validation': 'Configuration patterns and environment detection'}, 'expected_behavior': {'staging_production': 'Must not use localhost Redis URLs', 'test_development': 'Can use localhost Redis URLs', 'configuration': 'Must provide clear error messages for invalid config', 'deprecated_patterns': 'Must identify deprecated REDIS_URL patterns'}, 'failure_prevention': {'gcp_connectivity': 'Prevents localhost in staging causing Memory Store connectivity failure', 'port_configuration': 'Ensures correct Redis ports by environment', 'configuration_validation': 'Provides clear error messages for config issues'}}
