...
class TestIssue585PickleModuleSerializationErrors:
    ...
    def test_pickle_module_object_error_reproduction(self):
        ...
        problematic_context = {'user_data': {'message': 'Test reporting request'}, 'agent_instance': MagicMock(), 'execution_engine': UserExecutionEngine, 'sys_module': sys, 'imported_module': asyncio}
        ...
        assert "can't pickle" in error_message or 'cannot pickle' in error_message or 'module' in error_message or ('not picklable' in error_message)
    ...
    def test_agent_context_contamination_serialization(self, test_context):
        ...
        contaminated_metadata = {'user_request': 'Generate optimization report', 'agent_class': UserExecutionEngine, 'execution_module': sys.modules[__name__], 'callback_functions': {'on_complete': lambda x: x, 'error_handler': print}}
        agent_context = AgentExecutionContext(user_id=test_context['user_id'], thread_id=test_context['thread_id'], run_id=test_context['run_id'], request_id='req-585-contaminated', agent_name='reporting_agent', metadata=contaminated_metadata)
    ...
    def test_clean_context_serialization_success(self, test_context):
        ...
        clean_context = {'user_request': 'Generate optimization report', 'user_id': test_context['user_id'], 'thread_id': test_context['thread_id'], 'agent_name': 'reporting_agent', 'input_data': {'query_type': 'optimization', 'time_range': '7days', 'metrics': ['cpu_usage', 'memory_usage']}}
        try:
    ...
    def test_user_execution_context_serialization(self, test_context):
        ...
        problematic_user_context = UserExecutionContext(user_id=test_context['user_id'], thread_id=test_context['thread_id'], run_id=test_context['run_id'], request_id='req-585-problematic', agent_context={'agent_instance': UserExecutionEngine, 'module_ref': sys})
        with pytest.raises((TypeError, AttributeError, pickle.PickleError)):
    ...
    def test_agent_pipeline_context_sanitization_need(self, test_context):
        ...
        pipeline_input = {'message': 'Generate performance report', 'user_id': test_context['user_id'], 'agent_type': 'reporting_agent'}
        execution_state = {'input_data': pipeline_input, 'user_context': test_context['user_context'], 'execution_engine_ref': UserExecutionEngine, 'current_module': sys.modules[__name__], 'agent_registry': MagicMock()}
        with pytest.raises((TypeError, AttributeError, pickle.PicklingError)):
            pickle.dumps(execution_state)
        sanitized_state = {'input_data': pipeline_input, 'user_context_data': {'user_id': test_context['user_context'].user_id, 'thread_id': test_context['user_context'].thread_id, 'run_id': test_context['user_context'].run_id, 'request_id': test_context['user_context'].request_id}}
        try:
