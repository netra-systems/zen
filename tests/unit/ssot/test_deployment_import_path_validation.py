...
class TestDeploymentImportPathValidation(SSotBaseTestCase):
    ...
    def setup_method(self, method=None):
        ...
        self.ssot_import_patterns = {'test_framework.ssot.base_test_case', 'test_framework.unified_docker_manager', 'shared.isolated_environment', 'shared.types.core_types', 'test_framework.ssot.orchestration'}
        self.import_anti_patterns = {'from scripts.deploy_to_gcp import', 'import deploy_to_gcp', 'from deploy_to_gcp import', 'from scripts import deploy_to_gcp', 'from ..scripts.deploy_to_gcp import', 'import scripts.deploy_to_gcp'}
    ...
    def test_unified_test_runner_uses_ssot_import_patterns(self):
        ...
        for import_info in runner_imports:
            ...
            for ssot_pattern in self.ssot_import_patterns:
                if ssot_pattern in import_module:
                    ssot_imports_found.append({'pattern': ssot_pattern, 'actual_import': import_module, 'line': import_info.get('line', 0)})
        ...
        assert len(ssot_imports_found) >= minimum_ssot_imports, f"UnifiedTestRunner has insufficient SSOT imports: {len(ssot_imports_found)} < {minimum_ssot_imports}. Found: {[imp['pattern'] for imp in ssot_imports_found]}"
    ...
    def test_deployment_code_has_no_import_anti_patterns(self):
        ...
        files_to_check = [self.unified_runner_path]
                ...
                for import_info in file_imports:
                    ...
                    for anti_pattern in self.import_anti_patterns:
                        if anti_pattern in import_statement:
                            anti_pattern_violations.append({'file': str(file_path.relative_to(self.project_root)), 'line': import_info.get('line', 0), 'anti_pattern': anti_pattern, 'actual_import': import_statement, 'severity': 'critical'})
                for anti_pattern in self.import_anti_patterns:
                    if anti_pattern in file_content:
                        lines = file_content.split('\n')
                        for line_num, line in enumerate(lines, 1):
                            if anti_pattern in line:
                                anti_pattern_violations.append({'file': str(file_path.relative_to(self.project_root)), 'line': line_num, 'anti_pattern': anti_pattern, 'actual_import': line.strip(), 'severity': 'critical'})
                                break
        ...
        if anti_pattern_violations:
            violation_details = '\n'.join([f"  - {v['file']}:{v['line']}: {v['anti_pattern']}" for v in anti_pattern_violations[:10]])
            pytest.fail(f"IMPORT ANTI-PATTERN VIOLATION: {len(anti_pattern_violations)} anti-patterns detected:\n{violation_details}\n{('... and more' if len(anti_pattern_violations) > 10 else '')}\n\nExpected: No import anti-patterns in deployment code\nFix: Replace anti-pattern imports with SSOT equivalents\nDEPLOYMENT BLOCKED until all anti-patterns resolved")
    ...
    def test_import_path_absolute_compliance(self):
            ...
            try:
                ...
                for import_info in file_imports:
                    if import_info.get('is_relative', False):
                        relative_import_violations.append({'file': str(file_path.relative_to(self.project_root)), 'line': import_info.get('line', 0), 'import_statement': import_info.get('raw_statement', ''), 'module': import_info.get('module', '')})
            except Exception as e:
        ...
        if relative_import_violations:
            violation_details = '\n'.join([f"  - {v['file']}:{v['line']}: {v['import_statement']}" for v in relative_import_violations])
            pytest.fail(f'RELATIVE IMPORT VIOLATION: {len(relative_import_violations)} relative imports found:\n{violation_details}\n\nExpected: All imports should be absolute (SSOT requirement)\nFix: Convert relative imports to absolute imports')
    ...
    def test_circular_dependency_prevention(self):
        ...
        deployment_modules = ['tests.unified_test_runner', 'test_framework.unified_docker_manager', 'shared.isolated_environment']
        ...
        if circular_dependencies:
            cycle_details = '\n'.join([f"  - {' -> '.join(cycle)}" for cycle in circular_dependencies])
            pytest.fail(f'CIRCULAR DEPENDENCY VIOLATION: {len(circular_dependencies)} circular dependencies detected:\n{cycle_details}\n\nExpected: No circular dependencies in deployment code\nFix: Refactor imports to eliminate circular dependencies')
    ...
    def test_ssot_import_registry_compliance(self):
        ...
        try:
            ...
            for import_info in runner_imports:
                ...
                for broken_pattern in broken_patterns:
                    if broken_pattern in import_module:
                        registry_violations.append({'file': 'unified_test_runner.py', 'line': import_info.get('line', 0), 'violation': 'uses_broken_pattern', 'pattern': broken_pattern, 'import': import_module})
            ...
            if registry_violations:
                violation_details = '\n'.join([f"  - {v['file']}:{v['line']}: {v['violation']} - {v['pattern']}" for v in registry_violations])
                pytest.fail(f'SSOT REGISTRY VIOLATION: {len(registry_violations)} registry compliance violations:\n{violation_details}\n\nExpected: All imports should follow SSOT Import Registry patterns\nFix: Use verified import patterns from SSOT_IMPORT_REGISTRY.md')
        except Exception as e:
            ...
            for node in ast.walk(tree):
                if isinstance(node, ast.Import):
                    for alias in node.names:
                        imports.append({'type': 'import', 'module': alias.name, 'name': alias.asname or alias.name, 'line': node.lineno, 'is_relative': False, 'raw_statement': f'import {alias.name}'})
                elif isinstance(node, ast.ImportFrom):
                    module = node.module or ''
                    level = node.level
                    for alias in node.names:
                        imports.append({'type': 'from_import', 'module': module, 'name': alias.name, 'alias': alias.asname, 'line': node.lineno, 'is_relative': level > 0, 'relative_level': level, 'raw_statement': f"from {'.' * level}{module} import {alias.name}"})
        except Exception as e:
    ...
    def _resolve_module_path(self, module_name: str) -> Optional[Path]:
        ...
        possible_paths = [self.project_root / Path(*module_parts).with_suffix('.py'), self.project_root / Path(*module_parts) / '__init__.py']
...
class TestDeploymentImportPathValidationEdgeCases(SSotBaseTestCase):
    ...
    def test_dynamic_import_detection(self):
        ...
        """
        dynamic_import_patterns = ['__import__', 'importlib.import_module', 'exec(', 'eval(']
        violations = []
        if self.unified_runner_path.exists():
            ...
            for pattern in dynamic_import_patterns:
                if pattern in content:
                    violations.append({'file': 'unified_test_runner.py', 'pattern': pattern, 'type': 'dynamic_import'})
        self.record_metric('dynamic_import_violations', len(violations))
        if len(violations) > 2:
            pytest.fail(f'Too many dynamic imports detected: {len(violations)}. Dynamic imports can bypass SSOT import validation.')
    ...
    def test_import_path_consistency_across_environments(self):
        ...
        """
        critical_imports = ['test_framework.ssot.base_test_case', 'shared.isolated_environment']
        ...
        if import_resolution_failures:
            pytest.fail(f'Critical import resolution failures: {import_resolution_failures}')

    def _resolve_module_path(self, module_name: str) -> Optional[Path]:
        """Resolve a module name to its file path."""
        module_parts = module_name.split('.')
        possible_paths = [Path(__file__).parent.parent.parent.parent / Path(*module_parts).with_suffix('.py'), Path(__file__).parent.parent.parent.parent / Path(*module_parts) / '__init__.py']
        for path in possible_paths:
            if path.exists():
                return path
        return None
