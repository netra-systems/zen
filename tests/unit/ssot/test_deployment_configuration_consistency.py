...
class TestDeploymentConfigurationConsistency(SSotBaseTestCase):
    ...
    def setup_method(self, method=None):
        ...
        self.config_sources = ['terraform', 'docker_compose', 'deployment_scripts', 'environment_files']
    ...
    def test_deployment_environment_configuration_consistency(self):
        ...
        if len(environment_configs) > 1:
            ...
            for key in config_keys:
                ...
                if len(set(values.values())) > 1:
                    consistency_violations.append({'config_key': key, 'values': values, 'violation_type': 'value_mismatch'})
        ...
        critical_violations = [v for v in consistency_violations if v['config_key'] in critical_keys]
        if critical_violations:
            violation_details = '\n'.join([f"  - {v['config_key']}: {v['values']}" for v in critical_violations])
            pytest.fail(f'CRITICAL CONFIG INCONSISTENCY: {len(critical_violations)} critical configuration mismatches:\n{violation_details}\n\nExpected: Critical configurations should be consistent across all sources\nFix: Align critical configuration values across deployment sources')
    ...
    def test_deployment_service_configuration_alignment(self):
        ...
        for service in expected_services:
            ...
            if len(service_definitions) < 2:
                alignment_issues.append({'service': service, 'issue': 'insufficient_definitions', 'sources': list(service_definitions.keys())})
            if len(service_definitions) > 1:
                ...
                for prop in critical_properties:
                    ...
                    if len(set(prop_values.values())) > 1:
                        alignment_issues.append({'service': service, 'issue': 'property_mismatch', 'property': prop, 'values': prop_values})
        ...
        critical_alignment_issues = [issue for issue in alignment_issues if issue['issue'] == 'insufficient_definitions']
        if critical_alignment_issues:
            issue_details = '\n'.join([f"  - {issue['service']}: {issue['issue']} (sources: {issue['sources']})" for issue in critical_alignment_issues])
            pytest.fail(f'SERVICE ALIGNMENT FAILURE: {len(critical_alignment_issues)} critical service alignment issues:\n{issue_details}\n\nExpected: All services should be defined consistently across sources\nFix: Ensure service definitions exist in all relevant deployment sources')
    ...
    def test_deployment_infrastructure_configuration_validation(self):
        ...
        validation_failures = []
        infrastructure_components = ['vpc_configuration', 'networking_setup', 'security_settings', 'resource_limits']
        for component in infrastructure_components:
            try:
                ...
                if not infrastructure_configs[component]['valid']:
                    validation_failures.append({'component': component, 'issues': infrastructure_configs[component]['issues']})
            except Exception as e:
                validation_failures.append({'component': component, 'issues': [f'Validation error: {e}']})
        ...
        if validation_failures:
            failure_details = '\n'.join([f"  - {failure['component']}: {', '.join(failure['issues'])}" for failure in validation_failures])
            pytest.fail(f'INFRASTRUCTURE CONFIG FAILURE: {len(validation_failures)} infrastructure configuration issues:\n{failure_details}\n\nExpected: All infrastructure components should be properly configured\nFix: Address infrastructure configuration issues')
    ...
    def test_deployment_configuration_drift_detection(self):
        ...
        """
        baseline_config = {'project_id': 'netra-staging', 'region': 'us-central1', 'services': {'backend': {'memory': '1Gi', 'cpu': '1'}, 'auth': {'memory': '512Mi', 'cpu': '0.5'}, 'frontend': {'memory': '256Mi', 'cpu': '0.25'}}, 'environment_variables': {'LOG_LEVEL': 'INFO', 'ENVIRONMENT': 'staging'}}
        drift_test_scenarios = [{'name': 'no_drift', 'config': baseline_config.copy(), 'expected_drift': False}, {'name': 'minor_drift', 'config': {**baseline_config, 'environment_variables': {**baseline_config['environment_variables'], 'LOG_LEVEL': 'DEBUG'}}, 'expected_drift': False}, {'name': 'major_drift', 'config': {**baseline_config, 'region': 'us-west1', 'services': {'backend': {'memory': '2Gi', 'cpu': '2'}}}, 'expected_drift': True}]
        ...
        for scenario in drift_test_scenarios:
            ...
            drift_detection_results.append({'scenario': scenario['name'], 'drift_score': drift_score, 'drift_detected': drift_detected, 'expected_drift': scenario['expected_drift'], 'correct_detection': drift_detected == scenario['expected_drift']})
        ...
        if detection_accuracy < minimum_accuracy:
            failed_scenarios = [result for result in drift_detection_results if not result['correct_detection']]
            failure_details = '\n'.join([f"  - {result['scenario']}: detected={result['drift_detected']}, expected={result['expected_drift']}" for result in failed_scenarios])
            pytest.fail(f'DRIFT DETECTION FAILURE: Detection accuracy too low: {detection_accuracy:.1%} < {minimum_accuracy:.1%}\nFailed scenarios:\n{failure_details}\n\nExpected: Configuration drift detection should be accurate\nFix: Improve drift detection algorithm')
    ...
    def _extract_docker_compose_service_configs(self) -> Dict[str, Dict[str, Any]]:
        ...
        for compose_file in self.docker_compose_files:
            try:
                ...
                if 'services' in compose_data:
                    for service_name, service_config in compose_data['services'].items():
                        services[service_name] = {'image': service_config.get('image', ''), 'ports': service_config.get('ports', []), 'memory': service_config.get('deploy', {}).get('resources', {}).get('limits', {}).get('memory', '')}
            except Exception as e:
    ...
    def _validate_vpc_configuration(self) -> Dict[str, Any]:
        ...
        return {'valid': True, 'issues': [], 'vpc_exists': True}

    def _validate_networking_setup(self) -> Dict[str, Any]:
        """Validate networking setup."""
        return {'valid': True, 'issues': [], 'networking_configured': True}

    def _validate_security_settings(self) -> Dict[str, Any]:
        """Validate security settings."""
        return {'valid': True, 'issues': [], 'security_configured': True}

    def _validate_resource_limits(self) -> Dict[str, Any]:
        """Validate resource limits."""
        return {'valid': True, 'issues': [], 'limits_configured': True}
