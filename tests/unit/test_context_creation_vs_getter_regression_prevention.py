...
class TestContextReuseRegressionPrevention(SSotBaseTestCase):
    ...
    def test_context_reuse_in_conversation(self):
        ...
        assert context2.run_id == original_run_id, f'REGRESSION DETECTED: Run ID changed from {original_run_id} to {context2.run_id}. This breaks conversation continuity!'
    ...
    def test_websocket_handler_pattern_simulation(self):
        ...
        assert context1.user_id == context2.user_id, 'User ID must be consistent'
        context_none = get_user_execution_context(user_id=user_id, thread_id=None)
...
class TestContextCreationVsGetterRegression(SSotBaseTestCase):
    ...
    def test_get_user_execution_context_uses_session_manager(self):
        ...
        with patch.object(UnifiedIdGenerator, 'get_or_create_user_session') as mock_session:
            mock_session.return_value = {'thread_id': thread_id, 'run_id': run_id, 'request_id': 'req_session_manager_validation_001'}
            result = get_user_execution_context(user_id=user_id, thread_id=thread_id, run_id=run_id)
            mock_session.assert_called_once_with(user_id=user_id, thread_id=thread_id, run_id=run_id)
    ...
    def test_create_user_execution_context_always_creates_new(self):
        ...
        context1 = create_user_execution_context(user_id=user_id, thread_id=thread_id)
        context2 = create_user_execution_context(user_id=user_id, thread_id=thread_id)
    ...
    def test_get_context_with_none_values_handled_correctly(self):
        ...
        context = get_user_execution_context(user_id=user_id, thread_id=None, run_id=None)
...
class TestAntiPatternDetection(SSotBaseTestCase):
    ...
    def test_detect_id_generation_before_get_call(self):
        ...
        context1 = get_user_execution_context(user_id=user_id, thread_id=generated_thread, run_id=generated_run)
        different_thread = UnifiedIdGenerator.generate_base_id('thread')
        different_run = UnifiedIdGenerator.generate_base_id('run')
        context2 = get_user_execution_context(user_id=user_id, thread_id=different_thread, run_id=different_run)
        assert context1.thread_id != context2.thread_id
        assert context1.run_id != context2.run_id
    ...
    def test_correct_pattern_with_existing_ids(self):
        ...
        context1 = get_user_execution_context(user_id=user_id, thread_id=existing_thread, run_id=existing_run)
        context2 = get_user_execution_context(user_id=user_id, thread_id=existing_thread, run_id=existing_run)
...
class TestWebSocketHandlerPatterns:
    ...
    def test_websocket_handler_correct_pattern(self):
        ...
        class MockWebSocketMessage:
            ...
            def __init__(self, user_id: str, thread_id: Optional[str]=None, run_id: Optional[str]=None):
                ...
                self.run_id = run_id
                self.payload = {'thread_id': thread_id, 'run_id': run_id}
        ...
        def correct_handler_pattern(message: MockWebSocketMessage) -> UserExecutionContext:
            ...
            return get_user_execution_context(user_id=message.user_id, thread_id=thread_id, run_id=run_id)
    ...
    def test_quality_handler_pattern_regression_prevention(self):
        ...
        def corrected_quality_handler(user_id: str, message_context: dict) -> UserExecutionContext:
            ...
            return get_user_execution_context(user_id=user_id, thread_id=message_context.get('thread_id'), run_id=message_context.get('run_id'))
        user_id = 'quality_user'
        message_context = {'thread_id': 'quality_thread_123', 'run_id': 'quality_run_456'}
...
class TestSessionManagerIntegration:
    ...
    def test_session_manager_called_correctly(self):
        ...
        run_id = 'session_run'
        with patch.object(UnifiedIdGenerator, 'get_or_create_user_session') as mock_session:
            mock_session.return_value = {'thread_id': thread_id, 'run_id': run_id, 'request_id': 'session_request'}
            get_user_execution_context(user_id=user_id, thread_id=thread_id, run_id=run_id)
            mock_session.assert_called_once_with(user_id=user_id, thread_id=thread_id, run_id=run_id)
    ...
    def test_session_manager_return_values_used(self):
        ...
        session_data = {'thread_id': 'session_managed_thread', 'run_id': 'session_managed_run', 'request_id': 'session_managed_request'}
        with patch.object(UnifiedIdGenerator, 'get_or_create_user_session') as mock_session:
...
class TestParameterizedContextBehavior:
    ...
    def test_context_parameter_combinations(self, thread_id, run_id, expected_behavior):
        ...
        context = get_user_execution_context(user_id=user_id, thread_id=thread_id, run_id=run_id)
        assert context.user_id == user_id
        assert context.thread_id is not None
        assert context.run_id is not None
        assert context.request_id is not None
...
class TestSpecificRegressionPattern(SSotBaseTestCase):
    ...
    def test_exact_requirements_pattern_context_reuse_in_conversation(self):
        ...
        assert context2.run_id == original_run_id, f'CRITICAL REGRESSION DETECTED: Context reuse failed - run_id changed from {original_run_id} to {context2.run_id}. This breaks conversation continuity!'
        assert context1.user_id == context2.user_id, 'User ID must be consistent'
        assert context1.thread_id == context2.thread_id, 'Thread ID must be consistent'
    ...
    def test_real_services_simulation_with_context_reuse(self):
        ...
        assert context2.run_id == original_run_id, 'INTEGRATION FAILURE: Context not reused across messages in same thread'
        assert context2.thread_id == original_thread_id, 'INTEGRATION FAILURE: Thread ID changed across messages'
    ...
    def test_comprehensive_regression_prevention_validation(self):
        ...
        """
        scenarios = [{'name': 'basic_conversation', 'user_id': 'usr_comprehensive_scenario_001234', 'thread_id': 'thd_comprehensive_scenario_001', 'expected_reuse': True}, {'name': 'different_users_same_thread', 'user_id': 'usr_comprehensive_scenario_002345', 'thread_id': 'thd_comprehensive_scenario_001', 'expected_reuse': False}, {'name': 'same_user_different_thread', 'user_id': 'usr_comprehensive_scenario_001234', 'thread_id': 'thd_comprehensive_scenario_002', 'expected_reuse': False}]
        context_store = {}
        for scenario in scenarios:
            ...
            if expected_reuse:
                assert context1.run_id == context2.run_id, f'Scenario {scenario_name}: Expected context reuse but got different run_ids'
