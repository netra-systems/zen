...
class TestMessageRoutingConsistency(SSotAsyncTestCase):
    ...
    def _prepare_test_messages(self):
        ...
        # Import message types from websocket_core (expected canonical location)
        try:
            from netra_backend.app.websocket_core.types import MessageType, WebSocketMessage, create_standard_message

            self.test_messages = [
                create_standard_message(MessageType.USER_MESSAGE, {"content": "Hello", "user_id": "test_user"}),
                create_standard_message(MessageType.PING, {"timestamp": time.time()}),
                create_standard_message(MessageType.AGENT_REQUEST, {"message": "test request", "user_id": "test_user"}),
                create_standard_message(MessageType.HEARTBEAT, {"user_id": "test_user"}),
            ]
            ...
            # Fallback: Create simple message structure
            self.test_messages = [
                {"type": "user_message", "payload": {"content": "Hello", "user_id": "test_user"}},
                {"type": "ping", "payload": {"timestamp": time.time()}},
                {"type": "agent_request", "payload": {"message": "test request", "user_id": "test_user"}},
                {"type": "heartbeat", "payload": {"user_id": "test_user"}},
            ]
    ...
    async def test_message_routing_behavior_consistency(self):
        ...
        # Test each message through each router implementation
        ...
        if len(paths) >= 2:
            ...
            for compare_path in paths[1:]:
                ...
                for i, (base_result, compare_result) in enumerate(zip(base_results, compare_results)):
                    violations = self._compare_routing_results(
                        base_path, base_result,
                        compare_path, compare_result,
                        f"message_{i}"
                    )
        ...
        if len(consistency_violations) == 0:
            ...
            # THIS IS EXPECTED TO FAIL initially - proves routing inconsistencies
            ...
            self.fail(
                f"SSOT VIOLATION: MessageRouter implementations must have consistent routing behavior. "
                f"Found {len(consistency_violations)} violations proving fragmented routing logic."
            )
    ...
    def _compare_routing_results(self, base_path: str, base_result: Dict,
                                compare_path: str, compare_result: Dict, message_id: str) -> List[Dict]:
        ...
        if base_has_error != compare_has_error:
            violations.append({
                'type': 'error_presence_mismatch',
                'message_id': message_id,
                'base_path': base_path,
                'base_error': base_result['error'],
                'compare_path': compare_path,
                'compare_error': compare_result['error']
            })

        # Compare execution time patterns (significant differences indicate different implementations)
        if (base_result['execution_time'] is not None and
            compare_result['execution_time'] is not None):

            time_diff = abs(base_result['execution_time'] - compare_result['execution_time'])

            # Flag significant time differences (>100ms) as potential implementation differences
            if time_diff > 0.1:  # 100ms threshold
                violations.append({
                    'type': 'execution_time_significant_difference',
                    'message_id': message_id,
                    'base_path': base_path,
                    'base_time': base_result['execution_time'],
                    'compare_path': compare_path,
                    'compare_time': compare_result['execution_time'],
    ...
    async def test_handler_availability_consistency(self):
        ...
        for path, router_info in loaded_routers.items():
            instance = router_info['instance']
            analysis = {
                'has_handlers_attr': hasattr(instance, 'handlers'),
                'has_add_handler': hasattr(instance, 'add_handler'),
                'has_route_message': hasattr(instance, 'route_message'),
                'handler_count': None,
                'handler_types': []
            }
        ...
        # Compare handler availability consistency
            ...
            for compare_path in paths[1:]:
                ...
                for key in ['has_handlers_attr', 'has_add_handler', 'has_route_message']:
                    if base_analysis[key] != compare_analysis[key]:
                        handler_inconsistencies.append({
                            'attribute': key,
                            'base_path': base_path,
                            'base_value': base_analysis[key],
                            'compare_path': compare_path,
                            'compare_value': compare_analysis[key]
                        })

                # Compare handler counts if both have them
                if (base_analysis['handler_count'] is not None and
                    compare_analysis['handler_count'] is not None):
                    if base_analysis['handler_count'] != compare_analysis['handler_count']:
                        handler_inconsistencies.append({
                            'attribute': 'handler_count',
                            'base_path': base_path,
                            'base_value': base_analysis['handler_count'],
                            'compare_path': compare_path,
                            'compare_value': compare_analysis['handler_count']
                        })
        ...
        if len(handler_inconsistencies) == 0:
            ...
            # THIS IS EXPECTED TO FAIL initially - proves handler inconsistencies
            violation_msg = f"SSOT VIOLATION: {len(handler_inconsistencies)} handler availability inconsistencies detected"
            self.logger.error(f"❌ {violation_msg}")
            self.fail(
                f"SSOT VIOLATION: MessageRouter implementations must have consistent handler availability. "
                f"Found {len(handler_inconsistencies)} inconsistencies proving fragmented handler management."
            )
