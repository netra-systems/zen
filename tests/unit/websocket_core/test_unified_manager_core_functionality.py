...
class UnifiedWebSocketManagerCoreTests(SSotAsyncTestCase):
    ...
    async def test_connection_registration(self):
        ...
        await self.manager.register_connection(connection_id=connection_id, user_id=user_id, websocket=connection)
    ...
    async def test_event_routing_to_specific_user(self):
        ...
        await self.manager.register_connection(conn1_id, user1_id, connection1)
        await self.manager.register_connection(conn2_id, user2_id, connection2)
        event_message = {'type': 'agent_started', 'data': {'agent_id': 'test_agent', 'message': 'Hello User 1'}}
...
class WebSocketManagerModesTests(SSotAsyncTestCase):
    ...
    async def test_mode_deprecation_warnings(self):
        ...
        modes_to_test = [WebSocketManagerMode.ISOLATED, WebSocketManagerMode.EMERGENCY, WebSocketManagerMode.DEGRADED]
...
class WebSocketManagerWithUserContextTests(SSotAsyncTestCase):
    ...
    async def test_user_context_integration(self):
        ...
        user_context = UserExecutionContext(user_id='test_user_001', thread_id='thread_001', run_id='run_001', websocket_client_id='ws_client_001')
        connection = TestWebSocketConnection('conn_001')
        await self.manager.register_connection(connection_id=user_context.websocket_client_id, user_id=user_context.user_id, websocket=connection)
        self.assertTrue(await self.manager.has_connection(user_context.websocket_client_id))
        event = {'type': 'agent_thinking', 'data': {'user_id': user_context.user_id, 'thread_id': user_context.thread_id, 'message': 'Processing your request...'}}
    ...
    async def test_multiple_user_contexts_isolation(self):
        ...
        connections = []
        for i in range(3):
            context = UserExecutionContext(user_id=f'user_{i:03d}', thread_id=f'thread_{i:03d}', run_id=f'run_{i:03d}', websocket_client_id=f'ws_client_{i:03d}')
            connection = TestWebSocketConnection(f'conn_{i:03d}')
            contexts.append(context)
            connections.append(connection)
            await self.manager.register_connection(connection_id=context.websocket_client_id, user_id=context.user_id, websocket=connection)
        for i, context in enumerate(contexts):
            event = {'type': 'user_specific_event', 'data': {'user_index': i, 'message': f'Hello user {i}'}}
