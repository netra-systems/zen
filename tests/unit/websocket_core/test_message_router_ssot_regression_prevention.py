...
class TestMessageRouterSSOTRegressionPrevention(SSotBaseTestCase):
    ...
    def setUp(self):
        ...
        self.known_implementations = ['netra_backend/app/websocket_core/handlers.py', 'netra_backend/app/agents/message_router.py', 'netra_backend/app/core/message_router.py']
        self.canonical_implementation = 'netra_backend/app/websocket_core/handlers.py'
        self.max_allowed_implementations = 1
        self.allowed_compatibility_patterns = ['from netra_backend.app.websocket_core.handlers import MessageRouter', "__all__ = ['MessageRouter']"]
    ...
    def _scan_for_message_router_implementations(self) -> Dict[str, List[Dict]]:
        ...
        search_paths = [self.project_root / 'netra_backend' / 'app', self.project_root / 'netra_backend' / 'tests']
        ...
        return {'implementations': implementations, 'compatibility_layers': compatibility_layers}
        ...
        for py_file in directory.rglob('*.py'):
            try:
                ...
                if file_analysis['has_class_definition']:
                    implementations.append({'file_path': str(py_file.relative_to(self.project_root)), 'class_name': 'MessageRouter', 'implementation_type': file_analysis['implementation_type'], 'method_count': file_analysis['method_count'], 'line_count': file_analysis['line_count']})
                if file_analysis['has_compatibility_import']:
                    compatibility_layers.append({'file_path': str(py_file.relative_to(self.project_root)), 'import_source': file_analysis['import_source'], 'compatibility_type': file_analysis['compatibility_type']})
    ...
    def _analyze_file_for_message_router(self, file_path: Path) -> Dict[str, Any]:
        ...
        analysis = {'has_class_definition': False, 'has_compatibility_import': False, 'implementation_type': None, 'method_count': 0, 'line_count': 0, 'import_source': None, 'compatibility_type': None}
        try:
            ...
            try:
                ...
                for node in ast.walk(tree):
                    if isinstance(node, ast.ClassDef) and node.name == 'MessageRouter':
                        ...
                        if node.module and 'websocket_core.handlers' in node.module and any((alias.name == 'MessageRouter' for alias in node.names or [])):
    ...
    def test_canonical_message_router_completeness(self):
        ...
        try:
            ...
            essential_methods = ['handlers', 'add_handler', 'remove_handler', 'route_message', 'get_stats']
            missing_methods = []
            for method_name in essential_methods:
                if not hasattr(router, method_name):
                    missing_methods.append(method_name)
            if missing_methods:
                self.fail(f'Canonical MessageRouter implementation incomplete. Missing methods: {missing_methods}. This indicates improper SSOT consolidation.')
            initial_handler_count = len(router.handlers)
            self.assertGreaterEqual(initial_handler_count, 5, 'Canonical MessageRouter should have built-in handlers')
        ...
        for file_info in import_analysis['files_with_imports']:
            for import_info in file_info['imports']:
                ...
                if import_path == canonical_import_path:
                    continue
                is_compatibility = any((compat_path in import_path for compat_path in ['agents.message_router', 'core.message_router']))
                if not is_compatibility:
                    non_canonical_imports.append({'file': file_info['file_path'], 'import_path': import_path, 'line': import_info.get('line_number', 'unknown')})
    ...
    def _scan_for_message_router_imports(self) -> Dict[str, List[Dict]]:
        ...
        files_with_imports = []
        search_paths = [self.project_root / 'netra_backend' / 'app', self.project_root / 'tests']
        for search_path in search_paths:
            if search_path.exists():
        ...
        for py_file in directory.rglob('*.py'):
            try:
                ...
                if import_info['imports']:
                    files_with_imports.append({'file_path': str(py_file.relative_to(self.project_root)), 'imports': import_info['imports']})
            except Exception as e:
            ...
            for node in ast.walk(tree):
                if isinstance(node, ast.ImportFrom):
                    if node.names and any((alias.name == 'MessageRouter' for alias in node.names)):
                        imports.append({'type': 'from_import', 'from_module': node.module or '', 'names': [alias.name for alias in node.names], 'line_number': node.lineno})
        except Exception as e:
    ...
    def test_future_implementation_detection(self):
        ...
        expected_compatibility_count = 2
    ...
    def _generate_implementation_fingerprint(self) -> Dict[str, Any]:
        ...
        return {'implementation_count': len(analysis['implementations']), 'compatibility_count': len(analysis['compatibility_layers']), 'total_files': len(analysis['implementations']) + len(analysis['compatibility_layers']), 'implementations': [impl['file_path'] for impl in analysis['implementations']], 'compatibility_layers': [compat['file_path'] for compat in analysis['compatibility_layers']]}
