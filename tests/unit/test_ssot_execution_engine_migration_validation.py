...
class SSotExecutionEngineMigrationValidationTests(SSotBaseTestCase):
    ...
    def _create_test_users(self) -> List[Dict[str, Any]]:
        ...
        return [{'user_id': 'user_001', 'name': 'Test User 1', 'email': 'user1@test.com', 'subscription_tier': 'enterprise'}, {'user_id': 'user_002', 'name': 'Test User 2', 'email': 'user2@test.com', 'subscription_tier': 'mid'}, {'user_id': 'user_003', 'name': 'Test User 3', 'email': 'user3@test.com', 'subscription_tier': 'early'}]

    def _create_test_contexts(self) -> List[UserExecutionContext]:
        """Create UserExecutionContext instances for testing."""
        contexts = []
        for user in self.test_users:
            context = UserExecutionContext(user_id=user['user_id'], thread_id=f"thread_{user['user_id']}", run_id=str(uuid.uuid4()))
    ...
    def test_deprecated_execution_engine_import_still_works(self):
        ...
        try:
            ...
            with warnings.catch_warnings(record=True) as w:
                ...
                mock_user_context = UserExecutionContext(user_id='test_user_001', thread_id='test_thread_001', run_id='test_run_001')
    ...
    async def test_user_execution_engine_ssot_import_validation(self):
        ...
        try:
            from netra_backend.app.agents.supervisor.user_execution_engine import UserExecutionEngine
            user_context = UserExecutionContext(user_id='test_user_001', thread_id='test_thread_001', run_id='test_run_001')
            ...
            self.assertFalse(engine.is_compatibility_mode())
        except ImportError as e:
    ...
    async def test_user_isolation_validation_different_contexts(self):
        ...
        """
        user_context_1 = UserExecutionContext(user_id='user_001', thread_id='thread_001', run_id='run_001')
        user_context_2 = UserExecutionContext(user_id='user_002', thread_id='thread_002', run_id='run_002')
        ...
        engine_1 = UserExecutionEngine(context=user_context_1, agent_factory=mock_agent_factory_1, websocket_emitter=mock_websocket_emitter_1)
        engine_2 = UserExecutionEngine(context=user_context_2, agent_factory=mock_agent_factory_2, websocket_emitter=mock_websocket_emitter_2)
    ...
    async def test_deprecated_execution_engine_delegates_to_user_execution_engine(self):
        ...
        with patch.object(UserExecutionEngine, 'create_from_legacy', new_callable=AsyncMock) as mock_create:
            ...
            mock_create.assert_called_once_with(registry=mock_registry, websocket_bridge=mock_websocket_bridge, user_context=user_context)
    ...
    async def test_concurrent_user_execution_no_contamination(self):
        ...
        for i, user in enumerate(self.test_users):
            context = UserExecutionContext(user_id=user['user_id'], thread_id=f'thread_{i:03d}', run_id=f'run_{i:03d}')
            user_contexts.append(context)
            mock_agent_factory = MagicMock()
            mock_websocket_emitter = AsyncMock()
            engine = UserExecutionEngine(context=context, agent_factory=mock_agent_factory, websocket_emitter=mock_websocket_emitter)
            engines.append(engine)

        async def simulate_user_execution(engine, user_context, user_data):
            """Simulate agent execution for a specific user."""
            engine._user_specific_data = {'user_id': user_context.user_id, 'sensitive_data': user_data, 'request_timestamp': asyncio.get_event_loop().time()}
            await asyncio.sleep(0.1)
            return {'user_id': user_context.user_id, 'processed_data': f'processed_{user_data}', 'engine_id': engine.engine_id}
        ...
        for i in range(len(engines)):
            for j in range(len(engines)):
                if i != j:
                    self.assertNotEqual(engines[i]._user_specific_data['sensitive_data'], engines[j]._user_specific_data['sensitive_data'])
    ...
    def test_golden_path_compatibility_during_migration(self):
        ...
        try:
            from netra_backend.app.agents.supervisor.user_execution_engine import UserExecutionEngine
            user_context = UserExecutionContext(user_id='golden_path_user', thread_id='golden_path_thread', run_id='golden_path_run')
            mock_agent_factory = MagicMock()
            mock_websocket_emitter = MagicMock()
    ...
    async def test_migration_performance_impact_validation(self):
        ...
        mock_websocket_emitter = MagicMock()
        for i in range(100):
            user_context = UserExecutionContext(user_id=f'perf_user_{i}', thread_id=f'perf_thread_{i}', run_id=f'perf_run_{i}')
            ssot_engine = UserExecutionEngine(context=user_context, agent_factory=mock_agent_factory, websocket_emitter=mock_websocket_emitter)
        ssot_time = time.perf_counter() - start_time
        performance_ratio = ssot_time / deprecated_time
        self.assertLess(performance_ratio, 2.0, f'UserExecutionEngine should not be more than 2x slower than deprecated ExecutionEngine. Ratio: {performance_ratio:.2f} (deprecated: {deprecated_time:.4f}s, SSOT: {ssot_time:.4f}s)')
    ...
    async def test_websocket_event_delivery_isolation_validation(self):
        ...
        engines = []
        for i, user in enumerate(self.test_users):
            context = UserExecutionContext(user_id=user['user_id'], thread_id=f'ws_thread_{i}', run_id=f'ws_run_{i}')
            user_contexts.append(context)
            mock_emitter = AsyncMock()
            websocket_emitters.append(mock_emitter)
            mock_agent_factory = MagicMock()
            engine = UserExecutionEngine(context=context, agent_factory=mock_agent_factory, websocket_emitter=mock_emitter)
            engines.append(engine)
        ...
        async def emit_user_events(engine, user_context, emitter):
            ...
            for event in events:
                await emitter.emit_event(event, {'user_id': user_context.user_id, 'message': f'{event}_for_{user_context.user_id}', 'timestamp': time.time()})
        tasks = []
