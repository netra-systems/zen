...
class TestParameterMismatchDetection:
    ...
    def test_parameter_name_mismatch_detection(self):
        ...
        constructor_contract = MethodContract(method_name='__init__', parameters=[ParameterInfo('user_id', is_required=True, position=0), ParameterInfo('thread_id', is_required=True, position=1), ParameterInfo('run_id', is_required=True, position=2), ParameterInfo('websocket_client_id', is_required=False, position=3)])
...
class TestUserExecutionContextValidation:
    ...
    def test_user_execution_context_creation_with_correct_parameter(self):
        ...
        from netra_backend.app.services.user_execution_context import UserExecutionContext
        context = UserExecutionContext(user_id='usr_12345678901234567890', thread_id='thrd_12345678901234567890', run_id='run_12345678901234567890', websocket_client_id='ws_client_12345678901234567890')
...
def test_integration_validation_script():
    ...
    if script_path.exists():
        try:
            result = subprocess.run([sys.executable, str(script_path), '--specific-tests'], capture_output=True, text=True, timeout=30)
