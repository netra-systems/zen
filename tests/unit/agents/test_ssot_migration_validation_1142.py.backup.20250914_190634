...
class TestSSOTMigrationValidation1142(SSotAsyncTestCase):
    ...
    async def test_create_agent_instance_factory_per_request_isolation(self):
        ...
        # SSOT VALIDATION: Each user should get completely isolated factory instances
        assert factory1 is not factory2, (
            f"SSOT ISOLATION SUCCESS: Users get separate factory instances. "
            f"Factory1: {id(factory1)}, Factory2: {id(factory2)}. "
            f"This proves per-request isolation is working correctly."
        )
        ...
        # Validate unique run identifiers
    ...
    async def test_get_agent_instance_factory_deprecation_warning(self):
        ...
        with warnings.catch_warnings(record=True) as captured_warnings:
            ...
            # Call the deprecated singleton function
            factory = get_agent_instance_factory()
            
            # SSOT VALIDATION: Should be an AgentInstanceFactory instance
            assert isinstance(factory, AgentInstanceFactory), (
                f"SSOT COMPLIANCE: get_agent_instance_factory() returns AgentInstanceFactory. "
                f"Got: {type(factory)}"
            )
    ...
    async def test_configure_agent_instance_factory_deprecation_warning(self):
        ...
        Expected: PASS - Deprecation warning is shown
        """
        # Call the deprecated configuration function
        factory = await configure_agent_instance_factory(
            websocket_bridge=self.mock_websocket_bridge,
            llm_manager=self.mock_llm_manager
        )
        
        # SSOT VALIDATION: Should return an AgentInstanceFactory
        assert isinstance(factory, AgentInstanceFactory), (
            f"SSOT COMPLIANCE: configure_agent_instance_factory() returns AgentInstanceFactory. "
            f"Got: {type(factory)}"
        )
    ...
    async def test_concurrent_factory_creation_no_contamination(self):
        ...
        async def create_user_factory(user_context: UserExecutionContext) -> AgentInstanceFactory:
            ...
            factory.configure(
                websocket_bridge=self.mock_websocket_bridge,
                llm_manager=self.mock_llm_manager,
                agent_class_registry=self.mock_agent_registry,
        ...
        # Run concurrent factory creation
        ...
        # Verify no shared factory instances
        for i, factory1 in enumerate(factories):
            for j, factory2 in enumerate(factories):
                if i != j:
                    assert factory1 is not factory2, (
                        f"CONCURRENT ISOLATION VIOLATION: Factory {i} and {j} are the same instance. "
                        f"This indicates singleton contamination in concurrent execution."
                    )
            ...
            for j, context2 in enumerate(contexts):
                if i != j:
                    assert context1.user_id != context2.user_id, (
                        f"USER CONTEXT CONTAMINATION: Context {i} and {j} have same user_id. "
                        f"This indicates cross-user contamination."
                    )
    ...
    async def test_factory_configuration_isolation(self):
        ...
        # Configure factory1 with user-specific components
        mock_llm1 = MagicMock()
        mock_llm1.user_specific_config = "healthcare_config"
        factory1.configure(
            websocket_bridge=self.mock_websocket_bridge,
            llm_manager=mock_llm1
        )
        
        # Configure factory2 with different components
        mock_llm2 = MagicMock()
        mock_llm2.user_specific_config = "fintech_config"
        factory2.configure(
            websocket_bridge=self.mock_websocket_bridge,
            llm_manager=mock_llm2
        )
        
        # SSOT VALIDATION: Each factory should maintain its own configuration
        assert factory1._llm_manager is not factory2._llm_manager, (
            f"CONFIGURATION ISOLATION VIOLATION: Factories share LLM manager instance. "
            f"This causes configuration contamination between users."
        )
        
        # Verify user-specific configurations are isolated
    ...
    async def test_no_global_singleton_state(self):
        ...
        # SSOT VALIDATION: No factories should share the same memory address
        factory_ids = [id(factory) for factory in factories]
        unique_ids = set(factory_ids)
        
        assert len(unique_ids) == len(factories), (
            f"SINGLETON STATE DETECTED: {len(factories)} factories created but only "
            f"{len(unique_ids)} unique instances. This indicates global singleton state. "
        ...
        assert len(set(user_ids)) == len(factories), (
            f"USER CONTEXT CONTAMINATION: User IDs not unique across factories. "
            f"User IDs: {user_ids}"
        )
    ...
    async def test_memory_isolation_validation(self):
        ...
        """
        # Create factories with complex state
        factory1 = create_agent_instance_factory(self.user1_context)
        factory2 = create_agent_instance_factory(self.user2_context)
        
        # Configure with shared infrastructure but user-specific state
        ...
        # Create user execution contexts with state
        context1 = await factory1.create_user_execution_context(
            user_id=self.user1_context.user_id,
            thread_id=self.user1_context.thread_id,
            run_id=self.user1_context.run_id
        )
        context2 = await factory2.create_user_execution_context(
            user_id=self.user2_context.user_id,
            thread_id=self.user2_context.thread_id,
            run_id=self.user2_context.run_id
        )
        
        # SSOT VALIDATION: User contexts should be completely separate memory objects
        assert context1 is not context2, (
            f"MEMORY ISOLATION VIOLATION: User contexts share same memory reference. "
            f"Context1: {id(context1)}, Context2: {id(context2)}. "
            f"This causes cross-user state contamination."
        )
