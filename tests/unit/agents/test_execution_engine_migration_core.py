...
class TestExecutionEngineMigrationCore:
    ...
    def test_agent_execution_context_migration(self):
        ...
        context = MockAgentExecutionContext(agent_name='test_agent', task='test_task', user_context=user_context)
    ...
    def test_agent_execution_result_migration(self):
        ...
        user_context = MockUserExecutionContext()
        success_result = MockAgentExecutionResult(success=True, user_context=user_context, data='test_result')
        assert success_result.success is True
        assert success_result.user_context == user_context
        assert success_result.data == 'test_result'
        assert success_result.error is None
        error_result = MockAgentExecutionResult(success=False, user_context=user_context, error='Test error')
    ...
    def test_websocket_event_context_preservation(self):
        ...
        user_context = MockUserExecutionContext(user_id='ws_user_123', request_id='ws_req_456', thread_id='ws_thread_789')
        mock_bridge = Mock()
        mock_bridge.notify_agent_started = Mock()
        mock_bridge.notify_agent_completed = Mock()
        mock_bridge.notify_agent_error = Mock()
        mock_bridge.notify_agent_started(run_id=user_context.request_id, agent_name='test_agent', context={'user_id': user_context.user_id, 'isolated': True})
        mock_bridge.notify_agent_completed(run_id=user_context.request_id, agent_name='test_agent', result={'status': 'completed', 'user_id': user_context.user_id})
    ...
    def test_migration_pattern_validation(self):
        ...
        result_obj = MockAgentExecutionResult(success=True, user_context=user_context, data='migration_success')
        assert result_obj.user_context == user_context
        assert result_obj.data == 'migration_success'
        assert hasattr(result_obj, 'user_context')
    ...
    async def test_async_execution_pattern_migration(self):
        ...
        async def mock_execution_engine_execute(agent_name, task, user_context):
            ...
            return MockAgentExecutionResult(success=True, user_context=user_context, data=result)
        result = await mock_execution_engine_execute('test_agent', 'test_task', user_context)
    ...
    def test_error_handling_migration(self):
        ...
        user_context = MockUserExecutionContext()
        error_result = MockAgentExecutionResult(success=False, user_context=user_context, error='Test error occurred')
        assert error_result.success is False
        assert error_result.user_context == user_context
    ...
    def test_factory_pattern_migration(self):
        ...
        factory_config = {'max_concurrent_per_user': 5, 'execution_timeout': 30.0, 'enable_user_isolation': True}
        ...
        def create_execution_engine(user_context, config):
            ...
            return {'user_context': user_context, 'config': config, 'isolated': True, 'semaphore_limit': config['max_concurrent_per_user']}
        user_context = MockUserExecutionContext()
