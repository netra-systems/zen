...
class TestExecutionEngineFactoryUserIsolation884(SSotAsyncTestCase):
    ...
    def setUp(self):
        ...
        # User isolation test configuration
    ...
    # === USER CONTEXT ISOLATION VALIDATION TESTS ===
    ...
    async def test_concurrent_user_context_isolation_contamination(self):
        ...
        # Create test user contexts for concurrent testing
        ...
        for i in range(num_concurrent_users):
            context = UserExecutionContext(
                ...
                run_id=f"isolation_test_run_{i}_{int(time.time() * 1000)}_{uuid.uuid4().hex[:8]}",
                session_id=f"isolation_test_session_{i}_{uuid.uuid4().hex[:8]}",
                request_id=f"isolation_test_request_{i}_{uuid.uuid4().hex[:8]}"
            )
        ...
        async def create_user_engine_with_isolation_check(user_context, user_index):
            ...
            try:
                ...
                # Validate engine context isolation
                engine_context = engine.get_user_context()

                # Check if engine context matches input context
                if engine_context.user_id != user_context.user_id:
                    user_isolation_violations.append({
                        'user_index': user_index,
                        'violation_type': 'user_id_mismatch',
                        'expected': user_context.user_id,
                        'actual': engine_context.user_id,
                        'description': 'Engine user_id does not match input context'
                    })
                ...
                if engine_context.run_id != user_context.run_id:
                    user_isolation_violations.append({
                        ...
                        'actual': engine_context.run_id,
                        'description': 'Engine run_id does not match input context'
                    })
                ...
                if engine_context.session_id != user_context.session_id:
                    user_isolation_violations.append({
                        ...
                        'expected': user_context.session_id,
                        'actual': engine_context.session_id,
                        'description': 'Engine session_id does not match input context'
                    })
                ...
                user_isolation_violations.append({
                    'user_index': user_index,
                    'violation_type': 'engine_creation_failure',
                    'error': str(e),
                    'description': 'Failed to create engine for user'
                })
                return None, user_context, user_index

        # Execute concurrent engine creation with isolation validation
        tasks = [
            create_user_engine_with_isolation_check(ctx, i)
            ...
            for j, (engine2, context2, user_index2) in enumerate(created_engines):
                if i != j:
                    ...
                    self.assertNotEqual(
                        ...
                        f"USER ISOLATION VIOLATION: Engine {user_index1} and Engine {user_index2} "
                    ...
                    # CRITICAL ISOLATION CHECK: Session IDs must be unique
                    self.assertNotEqual(
                        engine1_context.session_id, engine2_context.session_id,
                        f"USER ISOLATION VIOLATION: Engine {user_index1} and Engine {user_index2} "
                        f"have the same session_id ({engine1_context.session_id}). "
                        f"Session contamination detected."
                    )

        # ASSERTION THAT SHOULD FAIL: No user isolation violations should occur
        self.assertEqual(
            len(user_isolation_violations), 0,
            f"USER ISOLATION VIOLATIONS DETECTED: Found {len(user_isolation_violations)} "
            f"user isolation violations during concurrent engine creation. "
            f"Violations: {user_isolation_violations}"
        )
    ...
    async def test_execution_engine_shared_state_contamination(self):
        ...
        for i in range(num_users):
            context = UserExecutionContext(
                user_id=f"shared_state_user_{i}",
                run_id=f"shared_state_run_{i}_{int(time.time() * 1000)}",
                session_id=f"shared_state_session_{i}",
                request_id=f"shared_state_request_{i}"
            )
            self.created_contexts.append(context)
        ...
        # Test for shared state between engines
        for i, (engine1, context1, user_index1) in enumerate(user_engines):
            for j, (engine2, context2, user_index2) in enumerate(user_engines):
                if i != j:
                    ...
                    # Check common attributes that might be shared
                    attributes_to_check = [
                        '_agent_factory',
                        '_websocket_emitter',
                        '_execution_context',
                    ...
                    for attr_name in attributes_to_check:
                        if (hasattr(engine1, attr_name) and
                            hasattr(engine2, attr_name)):
                            ...
                            if attr1 is attr2 and attr1 is not None:
                                shared_references.append({
                                    'attribute': attr_name,
                                    'shared_object_type': type(attr1).__name__,
                                    'engine1_user': user_index1,
                                    'engine2_user': user_index2
                                })
                    ...
                    if shared_references:
                        shared_state_violations.append({
                            ...
                            'violation_type': 'shared_object_references'
                        })

                    # Check for shared user execution context references
                    engine1_context = engine1.get_user_context()
                    engine2_context = engine2.get_user_context()
                    ...
                    if engine1_context is engine2_context:
                        shared_state_violations.append({
                            'engine1_user': user_index1,
                            'engine2_user': user_index2,
                            'violation_type': 'shared_user_context_reference',
                            'description': 'Engines share the same UserExecutionContext object'
                        })

        # ASSERTION THAT SHOULD FAIL: No shared state should exist between engines
        self.assertEqual(
            len(shared_state_violations), 0,
            f"SHARED STATE CONTAMINATION DETECTED: Found {len(shared_state_violations)} "
            f"instances of shared state between user engines. "
            f"Violations: {shared_state_violations}. "
            f"Shared state causes cross-user data contamination."
        )
    ...
    async def test_high_concurrency_user_isolation_stress(self):
        ...
        # Create factory for stress testing
        factory = ExecutionEngineFactory(websocket_bridge=None)
        self.factory_instances.append(factory)

        # High concurrency stress test configuration
        num_concurrent_users = 15  # High load
        num_iterations = 3
        ...
        for iteration in range(num_iterations):
            ...
            for i in range(num_concurrent_users):
                context = UserExecutionContext(
                    user_id=f"stress_user_{iteration}_{i}",
                    run_id=f"stress_run_{iteration}_{i}_{int(time.time() * 1000)}_{uuid.uuid4().hex[:8]}",
                    session_id=f"stress_session_{iteration}_{i}_{uuid.uuid4().hex[:8]}",
                    request_id=f"stress_request_{iteration}_{i}_{uuid.uuid4().hex[:8]}"
                )
                user_contexts.append(context)
                self.created_contexts.append(context)
            ...
            async def create_and_validate_engine(context, user_index):
                ...
                try:
                    ...
                    if (engine_context.user_id != context.user_id or
                        engine_context.run_id != context.run_id or
                        engine_context.session_id != context.session_id):
                    ...
                    return {
                        'success': False,
                        'user_index': user_index,
                        'violation_type': 'creation_failure',
                        'error': str(e),
                        'creation_time': time.time() - start_time if 'start_time' in locals() else 0
                    }

            # Execute high concurrency engine creation
            tasks = [
                create_and_validate_engine(ctx, i)
                for i, ctx in enumerate(user_contexts)
            ]
            results = await asyncio.gather(*tasks, return_exceptions=True)
            ...
            for result in results:
                if isinstance(result, dict):
                    ...
                    failed_creations.append({
                        'violation_type': 'exception_during_creation',
                        'error': str(result)
                    })

            # Cross-validate isolation between successful engines
            for i, engine_result1 in enumerate(successful_engines):
                for j, engine_result2 in enumerate(successful_engines):
                    if i != j:
                        ...
                        if (context1.user_id == context2.user_id or
                            context1.run_id == context2.run_id or
                            context1.session_id == context2.session_id):

                            iteration_violations.append({
                                'violation_type': 'cross_engine_context_collision',
                                'engine1_user': engine_result1['user_index'],
                                'engine2_user': engine_result2['user_index'],
                                'colliding_user_id': context1.user_id == context2.user_id,
                                'colliding_run_id': context1.run_id == context2.run_id,
                                'colliding_session_id': context1.session_id == context2.session_id
                            })

            # Track violations for this iteration
        ...
        # ASSERTION THAT SHOULD FAIL: No concurrency isolation violations
        self.assertEqual(
            len(concurrency_violations), 0,
            f"CONCURRENCY ISOLATION VIOLATIONS: Found isolation violations in "
            f"{len(concurrency_violations)} out of {num_iterations} high-concurrency iterations. "
            f"Violations: {concurrency_violations}. "
            f"Factory cannot maintain user isolation under concurrent load."
        )
    ...
    async def test_user_memory_isolation_contamination(self):
        ...
        for i in range(num_users):
            context = UserExecutionContext(
                ...
                run_id=f"memory_test_run_{i}_{int(time.time() * 1000)}",
                session_id=f"memory_test_session_{i}",
                request_id=f"memory_test_request_{i}"
            )
            self.created_contexts.append(context)
        ...
        # Test memory isolation by modifying engine state
        for engine, context, user_index in user_engines:
            # Set unique memory state for each engine
            unique_data = {
                'user_specific_data': f"user_{user_index}_secret_data_{uuid.uuid4().hex}",
                'user_index': user_index,
                'timestamp': time.time()
            }

            # Try to set engine-specific memory state
        ...
        # Check for memory contamination between engines
        for i, (engine1, context1, user_index1) in enumerate(user_engines):
            for j, (engine2, context2, user_index2) in enumerate(user_engines):
                if i != j:
                    # Check if engines share memory state
                    if (hasattr(engine1, '_user_memory_state') and
                        hasattr(engine2, '_user_memory_state')):
                        ...
                        if state1 is state2:
                            memory_contamination_violations.append({
                                'engine1_user': user_index1,
                                'engine2_user': user_index2,
                                'violation_type': 'shared_memory_reference',
                                'description': 'Engines share the same memory state object'
                            })

                        # Check for data contamination
                        if isinstance(state1, dict) and isinstance(state2, dict):
                            # Check if user1's data appears in user2's memory
                            ...
                            if user1_data in str(state2) and user1_data != user2_data:
                                memory_contamination_violations.append({
                                    ...
                                    'contaminated_data': user1_data,
                                    'description': f'User {user_index1} data found in User {user_index2} memory'
                                })

        # ASSERTION THAT SHOULD FAIL: No memory contamination should exist
        self.assertEqual(
            len(memory_contamination_violations), 0,
            f"MEMORY CONTAMINATION DETECTED: Found {len(memory_contamination_violations)} "
            f"instances of memory contamination between user engines. "
            f"Violations: {memory_contamination_violations}. "
            f"Memory contamination violates user data isolation."
        )
...
if __name__ == '__main__':
    # Run the user isolation violation tests
    import pytest
