...
class TestIssue508FixValidation(SSotAsyncTestCase):
    ...
    def test_connection_context_creation_with_fix(self):
        ...
        mock_websocket.headers = {'host': 'localhost:8000', 'authorization': 'Bearer jwt_token', 'user-agent': 'TestAgent/1.0'}
        connection_context = {'connection_id': 'test-connection-123', 'websocket_url': str(mock_websocket.url), 'path': mock_websocket.url.path, 'query_params': dict(QueryParams(mock_websocket.url.query)) if mock_websocket.url.query else {}, 'mode_parameter': 'agent', 'user_agent': 'TestAgent/1.0', 'client_host': getattr(mock_websocket.client, 'host', 'unknown') if mock_websocket.client else 'no_client', 'headers_count': len(mock_websocket.headers) if mock_websocket.headers else 0, 'has_auth_header': 'authorization' in (mock_websocket.headers or {})}
    ...
    def test_fix_backward_compatibility(self):
        ...
        """
        test_urls = ['ws://localhost:8000/ws/chat', 'ws://localhost:8000/ws/agent', 'ws://localhost:8000/ws/health', 'ws://localhost:8000/ws/chat?token=abc123', 'ws://localhost:8000/ws/agent?user_id=456', 'ws://localhost:8000/ws/health?check=true', 'ws://localhost:8000/ws/chat?token=abc123&user_id=456', 'ws://localhost:8000/ws/agent?user_id=456&thread_id=789&mode=chat', 'ws://localhost:8000/ws/health?check=true&detailed=false&version=1', 'ws://localhost:8000/ws/chat?token=eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9.eyJzdWIiOiIxMjM0NTY3ODkwIiwibmFtZSI6IkpvaG4gRG9lIiwiaWF0IjoxNTE2MjM5MDIyfQ&user_id=prod_user_123&session_id=session_abc_def_789&mode=chat&version=v2']
