...
class TestIssue586ResolutionValidation(SSotBaseTestCase):
    ...
    def test_gcp_cloud_run_staging_environment_detection_resolution(self):
        ...
        """
        staging_env = {'K_SERVICE': 'netra-backend-staging', 'K_REVISION': 'netra-backend-staging-00001-abc', 'K_CONFIGURATION': 'netra-backend-staging', 'GCP_PROJECT_ID': 'netra-staging', 'ENVIRONMENT': 'staging', 'PORT': '8080', 'GOOGLE_CLOUD_PROJECT': 'netra-staging'}
        with patch.object(IsolatedEnvironment, 'get') as mock_get:
            ...
            assert manager._environment == TimeoutEnvironment.CLOUD_RUN_STAGING, f"Environment detection failed: detected '{manager._environment}' instead of 'TimeoutEnvironment.CLOUD_RUN_STAGING' with K_SERVICE='{staging_env['K_SERVICE']}' and GCP_PROJECT_ID='{staging_env['GCP_PROJECT_ID']}'"
            min_expected_timeout = 15.0
            assert config.websocket_recv_timeout >= min_expected_timeout, f'Staging timeout insufficient: got {config.websocket_recv_timeout}s, expected >= {min_expected_timeout}s for Cloud Run staging environment'
            ...
            assert cold_start_buffer > 0, f'Cold start buffer not applied: got {cold_start_buffer}s, expected > 0s for Cloud Run deployment'
            hierarchy_valid = config.websocket_recv_timeout > config.agent_execution_timeout
            assert hierarchy_valid, f'Timeout hierarchy broken: WebSocket recv ({config.websocket_recv_timeout}s) should be > Agent execution ({config.agent_execution_timeout}s)'
    ...
    def test_environment_precedence_conflict_resolution(self):
        ...
        """
        precedence_scenarios = [{'name': 'k_service_staging_vs_env_development', 'env_vars': {'K_SERVICE': 'netra-backend-staging', 'ENVIRONMENT': 'development', 'GCP_PROJECT_ID': 'netra-staging'}, 'expected_environment': TimeoutEnvironment.CLOUD_RUN_STAGING, 'min_timeout': 15.0}, {'name': 'gcp_project_production_vs_env_staging', 'env_vars': {'GCP_PROJECT_ID': 'netra-production', 'ENVIRONMENT': 'staging', 'GOOGLE_CLOUD_PROJECT': 'netra-production', 'K_SERVICE': 'netra-backend-production'}, 'expected_environment': TimeoutEnvironment.CLOUD_RUN_PRODUCTION, 'min_timeout': 25.0}]
        resolution_results = []
        for scenario in precedence_scenarios:
            with patch.object(IsolatedEnvironment, 'get') as mock_get:
                ...
                resolution_results.append({'scenario': scenario['name'], 'environment_correct': environment_correct, 'timeout_adequate': timeout_adequate, 'detected_environment': manager._environment, 'timeout': config.websocket_recv_timeout, 'expected_env': scenario['expected_environment'], 'min_timeout': scenario['min_timeout']})
                assert environment_correct, f"Precedence resolution failed for {scenario['name']}: detected {manager._environment}, expected {scenario['expected_environment']} with env_vars {scenario['env_vars']}"
                assert timeout_adequate, f"Timeout inadequate for {scenario['name']}: got {config.websocket_recv_timeout}s, expected >= {scenario['min_timeout']}s"
        all_resolved = all((r['environment_correct'] and r['timeout_adequate'] for r in resolution_results))
        assert all_resolved, f'Precedence resolution failed for some scenarios: {resolution_results}'
    ...
    def test_cloud_run_cold_start_buffer_application(self):
        ...
        """
        cold_start_scenarios = [{'name': 'staging_cloud_run', 'env_vars': {'K_SERVICE': 'netra-backend-staging', 'GCP_PROJECT_ID': 'netra-staging', 'ENVIRONMENT': 'staging'}, 'expected_min_buffer': 2.0, 'environment': 'staging'}, {'name': 'production_cloud_run', 'env_vars': {'K_SERVICE': 'netra-backend-production', 'GCP_PROJECT_ID': 'netra-production', 'ENVIRONMENT': 'production'}, 'expected_min_buffer': 3.0, 'environment': 'production'}]
        buffer_results = []
        for scenario in cold_start_scenarios:
            with patch.object(IsolatedEnvironment, 'get') as mock_get:
                ...
                manager = CloudNativeTimeoutManager()
                gcp_markers = manager._detect_gcp_environment_markers()
                cold_start_buffer = manager._calculate_cold_start_buffer(scenario['environment'], gcp_markers)
                buffer_adequate = cold_start_buffer >= scenario['expected_min_buffer']
                cloud_run_detected = gcp_markers['is_gcp_cloud_run']
                buffer_results.append({'scenario': scenario['name'], 'buffer_adequate': buffer_adequate, 'cloud_run_detected': cloud_run_detected, 'actual_buffer': cold_start_buffer, 'expected_min': scenario['expected_min_buffer']})
                assert cloud_run_detected, f"Cloud Run not detected for {scenario['name']} with K_SERVICE='{scenario['env_vars']['K_SERVICE']}'"
                assert buffer_adequate, f"Cold start buffer inadequate for {scenario['name']}: got {cold_start_buffer}s, expected >= {scenario['expected_min_buffer']}s"
        all_adequate = all((r['buffer_adequate'] for r in buffer_results))
        assert all_adequate, f'Cold start buffer calculation failed for some scenarios: {buffer_results}'
    ...
    def test_timeout_hierarchy_validation_comprehensive(self):
        ...
        """
        environments_to_test = [{'name': 'local_development', 'env_vars': {}, 'expected_environment': TimeoutEnvironment.LOCAL_DEVELOPMENT}, {'name': 'staging_cloud_run', 'env_vars': {'K_SERVICE': 'netra-backend-staging', 'GCP_PROJECT_ID': 'netra-staging', 'ENVIRONMENT': 'staging'}, 'expected_environment': TimeoutEnvironment.CLOUD_RUN_STAGING}, {'name': 'production_cloud_run', 'env_vars': {'K_SERVICE': 'netra-backend-production', 'GCP_PROJECT_ID': 'netra-production', 'ENVIRONMENT': 'production'}, 'expected_environment': TimeoutEnvironment.CLOUD_RUN_PRODUCTION}]
        hierarchy_results = []
        for env_test in environments_to_test:
            with patch.object(IsolatedEnvironment, 'get') as mock_get:
                ...
                hierarchy_results.append({'environment': env_test['name'], 'hierarchy_valid': hierarchy_valid, 'websocket_recv': config.websocket_recv_timeout, 'agent_execution': config.agent_execution_timeout, 'gap': config.websocket_recv_timeout - config.agent_execution_timeout})
                assert hierarchy_valid, f"Timeout hierarchy broken for {env_test['name']}: WebSocket recv ({config.websocket_recv_timeout}s) should be > Agent execution ({config.agent_execution_timeout}s)"
        all_valid = all((r['hierarchy_valid'] for r in hierarchy_results))
        assert all_valid, f'Timeout hierarchy validation failed: {hierarchy_results}'
    ...
    def test_environment_detection_info_comprehensive(self):
        ...
        with patch.object(IsolatedEnvironment, 'get') as mock_get:
            ...
            required_keys = ['detected_environment', 'environment_sources', 'gcp_markers', 'timeout_values', 'hierarchy_validation']
