...
class TestGCPEnvironmentDetection(SSotBaseTestCase):
    ...
    def test_cloud_run_staging_markers_detection(self):
        ...
        """
        gcp_staging_env = {'K_SERVICE': 'netra-backend-staging', 'K_REVISION': 'netra-backend-staging-00001-abc', 'K_CONFIGURATION': 'netra-backend-staging', 'GCP_PROJECT_ID': 'netra-staging', 'ENVIRONMENT': 'staging', 'PORT': '8080', 'GOOGLE_CLOUD_PROJECT': 'netra-staging'}
        with patch.object(IsolatedEnvironment, 'get') as mock_get:
            ...
            assert detected_env == 'staging', f"Environment detection failed: detected '{detected_env}' instead of 'staging' despite K_SERVICE='{gcp_staging_env['K_SERVICE']}' and GCP_PROJECT_ID='{gcp_staging_env['GCP_PROJECT_ID']}'"
            expected_staging_timeout = 5.0
            assert timeout_config['websocket_startup_timeout'] >= expected_staging_timeout, f"Timeout configuration insufficient for Cloud Run: got {timeout_config['websocket_startup_timeout']}s, expected >= {expected_staging_timeout}s for staging environment"
    ...
    def test_cloud_run_production_markers_detection(self):
        ...
        """
        gcp_production_env = {'K_SERVICE': 'netra-backend-production', 'K_REVISION': 'netra-backend-production-00001-xyz', 'K_CONFIGURATION': 'netra-backend-production', 'GCP_PROJECT_ID': 'netra-production', 'ENVIRONMENT': 'production', 'PORT': '8080', 'GOOGLE_CLOUD_PROJECT': 'netra-production'}
        with patch.object(IsolatedEnvironment, 'get') as mock_get:
            mock_get.side_effect = lambda key, default=None: gcp_production_env.get(key, default)
            detected_env = self._detect_environment()
            timeout_config = self._get_timeout_configuration(detected_env)
            assert detected_env == 'production', f"Production environment detection failed: detected '{detected_env}' with GCP_PROJECT_ID='{gcp_production_env['GCP_PROJECT_ID']}'"
            expected_production_timeout = 10.0
            assert timeout_config['websocket_startup_timeout'] >= expected_production_timeout, f"Production timeout configuration insufficient: got {timeout_config['websocket_startup_timeout']}s, expected >= {expected_production_timeout}s"
    ...
    def test_environment_timeout_hierarchy_validation(self):
        ...
        """
        timeout_expectations = {'staging': 5.0, 'development': 1.2, 'production': 10.0}
        failed_environments = []
        for env_name, expected_timeout in timeout_expectations.items():
            with patch.object(IsolatedEnvironment, 'get') as mock_get:
                if env_name == 'staging':
                    env_vars = {'K_SERVICE': f'netra-backend-{env_name}', 'GCP_PROJECT_ID': f'netra-{env_name}', 'ENVIRONMENT': env_name}
                ...
                if actual_timeout < expected_timeout:
                    failed_environments.append({'environment': env_name, 'expected': expected_timeout, 'actual': actual_timeout, 'detected_as': detected_env})
        assert len(failed_environments) == 0, f'Timeout hierarchy validation failed for {len(failed_environments)} environments: {failed_environments}. This indicates systematic timeout configuration issues that will cause WebSocket 1011 failures in Cloud Run deployments.'
    ...
    def test_cold_start_environment_detection(self):
        ...
        """
        cold_start_phases = [{}, {'PORT': '8080', 'GOOGLE_CLOUD_PROJECT': 'netra-staging'}, {'K_SERVICE': 'netra-backend-staging', 'GCP_PROJECT_ID': 'netra-staging', 'ENVIRONMENT': 'staging', 'PORT': '8080', 'GOOGLE_CLOUD_PROJECT': 'netra-staging'}]
        detection_results = []
        for phase_num, env_vars in enumerate(cold_start_phases, 1):
            with patch.object(IsolatedEnvironment, 'get') as mock_get:
                ...
                detected_env = self._detect_environment()
                timeout_config = self._get_timeout_configuration(detected_env)
                detection_results.append({'phase': phase_num, 'env_vars_available': len(env_vars), 'detected_environment': detected_env, 'timeout': timeout_config['websocket_startup_timeout']})
        ...
        consistent_detection = all((env == final_detection or env in ['development', 'test'] for env in early_detections))
        assert consistent_detection, f'Environment detection inconsistent during cold start simulation: {detection_results}. Early phases should gracefully fallback to staging defaults rather than causing WebSocket timeout failures.'
        min_timeout_adequate = all((r['timeout'] >= 3.0 for r in detection_results))
        assert min_timeout_adequate, f'Cold start timeout configuration inadequate: {detection_results}. All phases should use safe timeout defaults >= 3.0s to prevent WebSocket 1011 failures during Cloud Run initialization.'
    ...
    def test_environment_detection_failure_modes(self):
        ...
        with patch.object(IsolatedEnvironment, 'get') as mock_get:
            ...
            detected_env = self._detect_environment()
            timeout_config = self._get_timeout_configuration(detected_env)
            safe_fallback_envs = ['staging', 'production']
            assert detected_env in safe_fallback_envs, f"Environment detection fallback unsafe: detected '{detected_env}' when all markers missing. Should default to safe environment ({safe_fallback_envs}) to prevent timeout issues."
            min_safe_timeout = 5.0
            assert timeout_config['websocket_startup_timeout'] >= min_safe_timeout, f"Fallback timeout configuration inadequate: got {timeout_config['websocket_startup_timeout']}s, expected >= {min_safe_timeout}s when environment detection fails. This will cause WebSocket 1011 failures in Cloud Run."
    ...
    def _get_timeout_configuration(self, environment: str) -> Dict[str, float]:
        ...
        """
        timeout_configs = {'development': {'websocket_startup_timeout': 1.2, 'auth_service_timeout': 2.0, 'database_timeout': 3.0}, 'staging': {'websocket_startup_timeout': 5.0, 'auth_service_timeout': 5.0, 'database_timeout': 8.0}, 'production': {'websocket_startup_timeout': 10.0, 'auth_service_timeout': 8.0, 'database_timeout': 15.0}}
        return timeout_configs.get(environment, timeout_configs['development'])
