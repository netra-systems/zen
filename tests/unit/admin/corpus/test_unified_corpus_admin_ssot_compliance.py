...
class TestUnifiedCorpusAdminSsotCompliance(SSotBaseTestCase):
    ...
    def setup_method(self, method=None):
        ...
        self.expected_violation_line_ranges = [(150, 160), (275, 285)]
        self.expected_violation_variable = 'CORPUS_BASE_PATH'
        self.expected_violation_context = '/data/corpus'
        ...
        for line_num, line in enumerate(lines, 1):
            ...
            for pattern in self.violation_patterns:
                if pattern in line:
                    ...
                    has_corpus_path_context = 'corpus' in line.lower() or '/data' in line.lower() or 'base_path' in line.lower()
                    has_default_path = self.expected_violation_context in line
                    violations.append({'line_number': line_num, 'code_line': line.strip(), 'pattern': pattern, 'environment_variable': env_var, 'has_corpus_context': has_corpus_path_context, 'has_default_path': has_default_path, 'violation_type': 'DIRECT_ENVIRONMENT_ACCESS'})
        return violations
    ...
    def test_unified_corpus_admin_direct_environment_access_violation(self):
        ...
        corpus_violations = [v for v in violations if v['environment_variable'] == self.expected_violation_variable]
        target_violations = []
        for line_range in self.expected_violation_line_ranges:
            range_violations = [v for v in corpus_violations if line_range[0] <= v['line_number'] <= line_range[1]]
            target_violations.extend(range_violations)
        contextual_violations = [v for v in target_violations if v['has_corpus_context'] or v['has_default_path']]
        ...
        assert len(target_violations) > 0, f"EXPECTED SSOT VIOLATION NOT DETECTED: Expected to find os.getenv('CORPUS_BASE_PATH') violation in lines around {self.expected_violation_line_ranges} of UnifiedCorpusAdmin, but none found. All corpus violations: {corpus_violations}. All violations: {violations}. This test should FAIL until the violation is fixed with IsolatedEnvironment."
        ...
        assert len(contextual_violations) > 0, f"Expected corpus base path context not found. Target violations: {target_violations}. The admin class should be accessing CORPUS_BASE_PATH with default '/data/corpus'."
    ...
    def test_unified_corpus_admin_lacks_isolated_environment_compliance(self):
        ...
        assert not has_any_compliant_pattern, f'UNEXPECTED SSOT COMPLIANCE DETECTED: UnifiedCorpusAdmin appears to already use SSOT patterns. Compliance status: {compliance_status}. Expected no SSOT compliance in current state. If remediation is complete, update test expectations.'
        assert not compliance_status['isolated_environment_import'], f'IsolatedEnvironment import detected in UnifiedCorpusAdmin, but expected direct os.getenv usage. If remediation is complete, this test should be updated.'
    ...
    def test_unified_corpus_admin_corpus_path_methods(self):
        ...
        for method_name, method in methods_to_check:
            try:
                ...
                if violations:
                    ...
                    if any((keyword in method_source.lower() for keyword in ['corpus', 'base_path', '/data'])):
        ...
        assert total_violations > 0, f'NO VIOLATIONS FOUND IN METHODS: Expected to find SSOT violations in UnifiedCorpusAdmin methods but found none. Methods checked: {[name for name, _ in methods_to_check]}. This may indicate the test is not detecting violations properly or remediation is complete.'
        corpus_methods = []
        ...
        assert len(corpus_methods) > 0, f"NO CORPUS_BASE_PATH VARIABLE ACCESS FOUND: Expected to find methods accessing 'CORPUS_BASE_PATH' variable but found none. Method violations: {method_violations}"
        assert len(corpus_path_methods) > 0, f'NO CORPUS PATH METHODS FOUND: Expected UnifiedCorpusAdmin to have corpus path related methods but found none. Methods checked: {[name for name, _ in methods_to_check]}'
    ...
    def test_unified_corpus_admin_file_system_path_logic(self):
        ...
        admin_class = self.import_unified_corpus_admin()
        source_code = self.get_source_code(admin_class)
        filesystem_patterns = ['path', 'dir', 'file', 'corpus', 'base_path', '/data', 'mkdir', 'exists', 'pathlib', 'os.path']
        filesystem_lines = []
        lines = source_code.splitlines()
        for line_num, line in enumerate(lines, 1):
            ...
            if any((pattern in line_lower for pattern in filesystem_patterns)):
                if any((violation_pattern in line for violation_pattern in self.violation_patterns)):
                    filesystem_lines.append({'line_number': line_num, 'code': line.strip(), 'has_violation': True})
        violations = self.parse_source_for_violations(source_code)
        ...
        self.record_metric('filesystem_lines_with_violations', len([line for line in filesystem_lines if line['has_violation']]))
        self.record_metric('corpus_path_violations_in_filesystem_logic', len(corpus_violations))
        assert len(filesystem_lines) > 0, f"NO FILE SYSTEM LOGIC FOUND: Expected UnifiedCorpusAdmin to contain file system path management logic but found none. This may indicate the admin doesn't handle corpus file operations."
        filesystem_violations = [line for line in filesystem_lines if line['has_violation']]
        assert len(filesystem_violations) > 0, f'NO VIOLATIONS IN FILE SYSTEM LOGIC: Expected to find SSOT violations in corpus path management logic but found none. Filesystem lines: {filesystem_lines}. Corpus violations: {corpus_violations}'
        default_path_lines = [line for line in filesystem_lines if self.expected_violation_context in line['code']]
        assert len(default_path_lines) > 0, f"DEFAULT CORPUS PATH NOT FOUND: Expected to find default '/data/corpus' path configuration but found none. Filesystem lines: {filesystem_lines}"
    ...
    def test_unified_corpus_admin_admin_functionality_impact(self):
        ...
        admin_class = self.import_unified_corpus_admin()
        source_code = self.get_source_code(admin_class)
        admin_patterns = ['admin', 'manage', 'create', 'delete', 'update', 'list', 'upload', 'download', 'corpus', 'content', 'file', 'document']
        admin_pattern_count = sum((1 for pattern in admin_patterns if pattern.lower() in source_code.lower()))
        violations = self.parse_source_for_violations(source_code)
        corpus_violations = [v for v in violations if v['environment_variable'] == 'CORPUS_BASE_PATH']
        has_admin_functionality = 'admin' in source_code.lower() or 'manage' in source_code.lower() or 'corpus' in source_code.lower()
        ...
        has_content_functionality = admin_pattern_count > 3
        has_corpus_violations = len(corpus_violations) > 0
        if has_content_functionality and has_corpus_violations:
            assert True, f'ADMIN FUNCTIONALITY IMPACT CONFIRMED: UnifiedCorpusAdmin contains {admin_pattern_count} admin/content patterns and {len(corpus_violations)} CORPUS_BASE_PATH variable violations. This affects content management functionality supporting $500K+ ARR platform operations.'
        elif has_content_functionality and (not has_corpus_violations):
            pytest.fail(f'SSOT REMEDIATION APPEARS COMPLETE: UnifiedCorpusAdmin has admin functionality but no CORPUS_BASE_PATH variable violations detected. If remediation is complete, update test expectations to validate proper IsolatedEnvironment usage.')
        else:
            pytest.fail(f'UNEXPECTED STATE: UnifiedCorpusAdmin analysis shows admin patterns: {admin_pattern_count}, CORPUS_BASE_PATH violations: {len(corpus_violations)}. Verify test logic and file content.')
        assert has_admin_functionality, f'ADMIN FUNCTIONALITY NOT DETECTED: Expected UnifiedCorpusAdmin to contain admin-specific patterns but found limited functionality. Verify this is actually an admin class.'
    ...
    def test_unified_corpus_admin_ssot_remediation_readiness(self):
        ...
        source_code = self.get_source_code(admin_class)
        violations = self.parse_source_for_violations(source_code)
        remediation_guide = {'violations_to_fix': [], 'recommended_patterns': {'os.getenv(': 'get_env().get(', 'os.environ.get(': 'get_env().get(', 'os.environ[': 'get_env().get('}, 'required_imports': ['from shared.isolated_environment import get_env'], 'corpus_path_fixes': []}
        for violation in violations:
            if violation['environment_variable'] == 'CORPUS_BASE_PATH':
                ...
                if self.expected_violation_context in current_line:
                    ...
                    remediation_guide['corpus_path_fixes'].append({'line': violation['line_number'], 'current': current_line, 'recommended': recommended_fix, 'note': 'Corpus base path access using IsolatedEnvironment with default /data/corpus'})
            remediation_guide['violations_to_fix'].append({'line': violation['line_number'], 'current': violation['code_line'], 'pattern': violation['pattern'], 'variable': violation['environment_variable'], 'has_corpus_context': violation['has_corpus_context'], 'has_default_path': violation['has_default_path'], 'recommended_fix': violation['code_line'].replace(violation['pattern'], remediation_guide['recommended_patterns'].get(violation['pattern'], 'get_env().get('))})
        ...
        assert len(violations) > 0, f"SSOT REMEDIATION GUIDE: Found {len(violations)} violations in UnifiedCorpusAdmin that need fixing. Remediation guide: {remediation_guide}. After remediation, these violations should be replaced with IsolatedEnvironment patterns. Pay special attention to corpus base path configuration: {len(remediation_guide['corpus_path_fixes'])} found."
        corpus_violations = [v for v in violations if v['environment_variable'] == 'CORPUS_BASE_PATH']
        assert len(corpus_violations) > 0, f'NO CORPUS_BASE_PATH VIOLATIONS FOR REMEDIATION: Expected to find CORPUS_BASE_PATH violations for remediation planning but found none. All violations: {violations}'
