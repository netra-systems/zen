"""
Issue #1017 Security Vulnerability Tests: Serialization Security Comprehensive
...
class TestIssue1017SerializationSecurityVulnerabilities(SSotBaseTestCase):
    ...
    def test_deepagentstate_to_dict_sensitive_data_exposure(self):
        ...
        # Create DeepAgentState with sensitive data in various fields
        for secret_name, secret_value in self.sensitive_secrets.items():
            with self.subTest(secret_type=secret_name):
                ...
                agent_metadata.execution_stats = {
                ...
                if len(secret_value) > 8:
                    secret_prefix = secret_value[:4]
                    secret_suffix = secret_value[-4:]
        ...
        with self.temp_env_vars(**env_secrets):
            ...
            for env_name, env_value in env_secrets.items():
                with self.subTest(env_var=env_name):
                    ...
                    # Check for common sensitive patterns that might indicate env var exposure
                    sensitive_patterns = [
                        "postgresql://",
                        "sk-",
                        "whsec_",
                    ...
                    for pattern in sensitive_patterns:
                        if pattern in env_value:
                            # Count occurrences to detect potential leakage
                            pattern_count = serialized_json.count(pattern)
                            # Allow for 1-2 legitimate uses, but flag excessive occurrences
                            self.assertLessEqual(pattern_count, 2,
                                f"VULNERABILITY: Sensitive pattern '{pattern}' appears {pattern_count} times, indicating potential env var leakage")
    ...
    def test_cross_user_data_contamination_in_serialization(self):
        ...
        for i, state in enumerate(states_list):
            ...
            with self.subTest(user=current_user):
                ...
                # Check that user isolation is maintained in nested structures
                if "agent_input" in serialized_state:
                    agent_input_json = json.dumps(serialized_state["agent_input"])
                    self.assertNotIn(other_secret, agent_input_json,
                        f"VULNERABILITY: Other user's secret '{other_secret}' contaminated agent_input of user {current_user}")

                if "context_tracking" in serialized_state:
                    context_json = json.dumps(serialized_state["context_tracking"])
                    self.assertNotIn(other_secret, context_json,
                        f"VULNERABILITY: Other user's secret '{other_secret}' contaminated context_tracking of user {current_user}")
            ...
            with self.subTest(serialization_method=method_name):
                ...
                for category, secrets_dict in internal_secrets.items():
                    for secret_key, secret_value in secrets_dict.items():
                        ...
                        if any(pattern in secret_value for pattern in ["_key", "_token", "_secret", "_pass"]):
                            # Allow for partial exposure in field names but not values
                            if len(secret_value) > 10:
                                partial_secret = secret_value[5:-5]  # Middle portion
                                if len(partial_secret) > 4:
                                    self.assertNotIn(partial_secret, serialized_json,
                                        f"VULNERABILITY: Partial secret '{partial_secret}' from '{secret_key}' exposed via {method_name}")

    def test_recursive_serialization_depth_exposure(self):
        """
        Test for sensitive data exposure through recursive serialization depth.

        EXPECTS TO FAIL: This test should demonstrate that deeply nested sensitive
        data gets exposed through recursive serialization.
        """
        self.record_metric("vulnerability_subtype", "recursive_serialization")

        # Create deeply nested structure with sensitive data at various levels
        deep_secret = "deeply_nested_secret_that_should_not_be_exposed_123"

        def create_nested_structure(depth: int, secret: str) -> Dict[str, Any]:
            """Create nested structure with secret at specified depth."""
            if depth == 0:
                return {"secret_data": secret, "sensitive_info": secret}

            return {
                f"level_{depth}": create_nested_structure(depth - 1, secret),
                f"metadata_{depth}": {
                    "debug_info": secret,
                    "nested_config": create_nested_structure(depth - 1, secret)
                }
            }

        # Test various nesting depths
        for depth in [3, 5, 8, 10]:
            with self.subTest(nesting_depth=depth):

                nested_structure = create_nested_structure(depth, deep_secret)

                # Create state with deeply nested sensitive data
                deep_state = DeepAgentState(
                    user_request="Process nested configuration",
                    user_id=self.test_user_id,
                    chat_thread_id=self.test_thread_id,
                    run_id=f"{self.test_run_id}_depth_{depth}",
                    agent_input={
                        "nested_config": nested_structure,
                        "deep_metadata": nested_structure
                    },
                    context_tracking={
                        "deep_context": nested_structure,
                        "recursive_data": nested_structure
                    }
                )

                # Serialize with potential recursion
                try:
                    serialized_deep = deep_state.model_dump()
                    serialized_json = json.dumps(serialized_deep, indent=2)

                    # VULNERABILITY CHECK: Deep secrets should not be exposed regardless of nesting
                    # This assertion should FAIL to prove vulnerability exists
                    self.assertNotIn(deep_secret, serialized_json,
                        f"VULNERABILITY: Deep secret at nesting depth {depth} was exposed in serialization")

                    # Check that the structure was actually serialized (not just truncated)
                    self.assertIn(f"level_{depth}", serialized_json,
                        f"INTERNAL ERROR: Nesting structure at depth {depth} not found - test may be invalid")

                except (RecursionError, ValueError) as e:
                    # If serialization fails due to depth, that might be a security feature
                    # But we should verify it's an intentional protection, not an accidental limit
                    if depth > 8:  # Allow for reasonable depth limits
                        continue
                    else:
                        self.fail(f"Serialization failed unexpectedly at depth {depth}: {e}")
