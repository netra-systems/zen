...
class TestOAuthNamingBugReproduction:
    ...
    def test_bug_reproduction_backend_missing_oauth_category(self):
        ...
        assert 'oauth' in backend_secrets, 'BUG REPRODUCTION: Backend service missing oauth category. Tests expect OAuth configuration but backend has none defined. Available categories: ' + str(list(backend_secrets.keys()))
    ...
    def test_bug_reproduction_backend_missing_simplified_oauth_names(self):
        ...
        assert 'GOOGLE_CLIENT_ID' in backend_all_secrets, 'BUG REPRODUCTION: Backend missing GOOGLE_CLIENT_ID. Test expects simplified OAuth naming but backend has no OAuth secrets. Available secrets: ' + str(backend_all_secrets[:10]) + '...'
        assert 'GOOGLE_CLIENT_SECRET' in backend_all_secrets, 'BUG REPRODUCTION: Backend missing GOOGLE_CLIENT_SECRET. Test expects simplified OAuth naming but backend has no OAuth secrets.'
    ...
    def test_bug_reproduction_missing_gsm_mappings_for_simplified_names(self):
        ...
        assert client_id_mapping is not None, "BUG REPRODUCTION: GOOGLE_CLIENT_ID has no GSM mapping. Tests expect this to map to google-oauth-client-id-staging but mapping doesn't exist."
        assert client_secret_mapping is not None, "BUG REPRODUCTION: GOOGLE_CLIENT_SECRET has no GSM mapping. Tests expect this to map to google-oauth-client-secret-staging but mapping doesn't exist."
        if client_id_mapping:
            assert client_id_mapping == 'google-oauth-client-id-staging', f'GOOGLE_CLIENT_ID should map to google-oauth-client-id-staging, got: {client_id_mapping}'
        if client_secret_mapping:
            assert client_secret_mapping == 'google-oauth-client-secret-staging', f'GOOGLE_CLIENT_SECRET should map to google-oauth-client-secret-staging, got: {client_secret_mapping}'
    ...
    def test_bug_reproduction_oauth_architecture_inconsistency(self):
        ...
        backend_secrets = SecretConfig.get_service_secrets('backend')
        assert 'oauth' in auth_secrets, 'Auth service should have OAuth configuration'
        auth_oauth_secrets = auth_secrets['oauth']
        assert 'GOOGLE_OAUTH_CLIENT_ID_STAGING' in auth_oauth_secrets, 'Auth service should have environment-specific OAuth client ID'
        assert 'oauth' in backend_secrets, "BUG REPRODUCTION: Backend service missing OAuth category. Architecture inconsistency: Auth has OAuth but backend doesn't. This creates unclear OAuth handling responsibilities."
    ...
    def test_bug_reproduction_dual_naming_convention_expectations(self):
        ...
        backend_all_secrets = SecretConfig.get_all_service_secrets('backend')
        assert 'GOOGLE_OAUTH_CLIENT_ID_STAGING' in auth_all_secrets, 'Auth service should have environment-specific OAuth client ID'
        assert 'GOOGLE_OAUTH_CLIENT_SECRET_STAGING' in auth_all_secrets, 'Auth service should have environment-specific OAuth client secret'
        assert 'GOOGLE_CLIENT_ID' in backend_all_secrets, 'BUG REPRODUCTION: Backend service should have simplified OAuth client ID. Dual naming convention not implemented.'
        assert 'GOOGLE_CLIENT_SECRET' in backend_all_secrets, 'BUG REPRODUCTION: Backend service should have simplified OAuth client secret. Dual naming convention not implemented.'
        assert 'GOOGLE_CLIENT_ID' not in auth_all_secrets, 'Auth service should NOT use simplified OAuth names (correct as-is)'
        assert 'GOOGLE_OAUTH_CLIENT_ID_STAGING' not in backend_all_secrets, 'Backend service should NOT use environment-specific OAuth names (will FAIL due to missing OAuth entirely)'
    ...
    def test_bug_reproduction_secrets_string_generation_fails_for_oauth(self):
        ...
        assert 'GOOGLE_CLIENT_ID=' in backend_secrets_string, f'BUG REPRODUCTION: Backend deployment string missing OAuth client ID. Generated string: {backend_secrets_string[:200]}...'
        assert 'GOOGLE_CLIENT_SECRET=' in backend_secrets_string, 'BUG REPRODUCTION: Backend deployment string missing OAuth client secret. OAuth configuration completely absent from backend deployment.'
    ...
    def test_bug_reproduction_critical_secrets_validation_oauth_missing(self):
        ...
        oauth_in_critical = any(('OAUTH' in secret or 'CLIENT_ID' in secret or 'CLIENT_SECRET' in secret for secret in backend_critical))
