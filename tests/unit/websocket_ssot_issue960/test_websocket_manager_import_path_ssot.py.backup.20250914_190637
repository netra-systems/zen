...
class TestWebSocketManagerImportPathSSOT(SSotBaseTestCase):
    ...
    async def test_all_websocket_manager_imports_resolve_to_canonical(self):
        ...
        # Discover all possible import paths for WebSocket managers
        import_patterns = [
        ...
        # CRITICAL TEST: Should have minimal import paths (SSOT compliance)
        # This will FAIL with current fragmentation showing multiple paths
        if len(successful_imports) > 2:  # Allow primary + compatibility layer
            import_list = [f"{path}.{name}" for path, name, _ in successful_imports]
            raise AssertionError(
                f"SSOT VIOLATION: Found {len(successful_imports)} WebSocket manager import paths: {import_list}. "
                f"SSOT requires minimal import surface (1-2 canonical paths maximum)."
            )
    ...
    async def test_websocket_event_delivery_ssot_compliance(self):
        ...
        try:
            ...
            # Create managers from different import paths
            ...
            # Send events through different managers
            ...
            # CRITICAL TEST: Only one manager should handle events (SSOT)
            # This will FAIL if multiple managers can send events independently
            total_events = events_sent_manager1 + events_sent_manager2
            if total_events > 1:
                raise AssertionError(
                    f"SSOT VIOLATION: Multiple WebSocket managers can send events independently. "
                    f"Manager1: {events_sent_manager1}, Manager2: {events_sent_manager2}. "
                    f"This causes duplicate event delivery."
                )

        except Exception as e:
            logger.error(f"Event delivery SSOT test failed: {e}")
            # This failure indicates fragmentation issues
    ...
    def test_websocket_manager_function_signatures_consistency(self):
        ...
        try:
            # Collect function signatures from different imports
            import_functions = [
                ("netra_backend.app.websocket_core.websocket_manager", "get_websocket_manager"),
                ("netra_backend.app.websocket_core.websocket_manager_factory", "create_websocket_manager"),
            ]
            ...
            # CRITICAL TEST: All functions should have identical signatures (SSOT)
            # This will FAIL if signatures are inconsistent due to fragmentation
            if len(function_signatures) > 1:
                ...
                for func_path, signature in function_signatures.items():
                    if signature != first_signature:
                        raise AssertionError(
                            f"SSOT VIOLATION: Inconsistent function signatures detected. "
                            f"Expected: {first_signature}, Got: {signature} for {func_path}. "
                            f"This indicates multiple implementations exist."
                        )
    ...
    async def test_websocket_manager_initialization_consistency(self):
        ...
        try:
            ...
            # Initialize managers through different paths
            ...
            # CRITICAL TEST: Managers should have identical states (SSOT)
            # This will FAIL if initialization paths create different states
            if state_differences:
                raise AssertionError(
                    f"SSOT VIOLATION: WebSocket managers initialized differently: {state_differences}. "
                    f"This indicates fragmented initialization logic."
                )
    ...
    def test_websocket_manager_import_path_documentation(self):
        ...
        # Use the discovered paths from previous test
        ...
        for module_path, class_or_function_name, obj in self.discovered_import_paths:
            path_info = {
                'import_path': f"{module_path}.{class_or_function_name}",
                'object_type': type(obj).__name__,
                'is_class': inspect.isclass(obj),
                'is_function': inspect.isfunction(obj),
                'module_file': getattr(inspect.getmodule(obj), '__file__', 'Unknown')
            }
        ...
        # Always pass - this is documentation, not validation
