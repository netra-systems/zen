...
class TestIDFormatSecurity(SSotBaseTestCase):
    ...
    def test_structured_id_format_consistency(self):
            ...
            for id_type in id_types:
                ...
                for i, id_val in enumerate(test_ids):
                    if not self._validate_structured_id_format(id_val):
                        format_violations.append({'service': service, 'id_type': id_type.value, 'id_value': id_val, 'error': f'ID does not follow structured format: {id_val}'})
                    if i > 0:
                        prev_id = test_ids[i - 1]
                        if not self._ids_have_consistent_format(prev_id, id_val):
                            inconsistent_patterns.append({'service': service, 'id_type': id_type.value, 'id1': prev_id, 'id2': id_val, 'error': 'Inconsistent format between IDs of same type'})
        ...
        assert total_violations == 0, f'Found {total_violations} ID format violations. Format violations: {len(format_violations)}, Inconsistent patterns: {len(inconsistent_patterns)}. All IDs must follow structured format: {{type}}_{{counter}}_{{uuid8}}'
    ...
    def test_id_collision_resistance(self):
        ...
        assert exact_duplicates == 0, f'Found {exact_duplicates} exact ID collisions out of {collision_test_size} generated IDs. Collision rate: {collision_rate:.4f}%. ID generation must guarantee uniqueness even under concurrent load.'
        assert len(near_collisions) == 0, f'Found {len(near_collisions)} near-collision patterns that could cause confusion. Near-collisions: {near_collisions[:5]}. ID patterns must be sufficiently distinct to prevent user confusion.'
    ...
    def test_id_predictability_resistance(self):
        ...
        assert len(predictability_violations) == 0, f"Found {len(predictability_violations)} predictability violations. Sequential patterns: {len(sequential_patterns)}, Timestamp patterns: {len(timestamp_patterns)}, Entropy score: {entropy_analysis['entropy_score']:.2f}. IDs must be cryptographically unpredictable for security."
    ...
    def test_cross_user_id_isolation(self):
        ...
        assert len(isolation_violations) == 0, f'Found {len(isolation_violations)} user isolation violations across {user_count} test users. User ID isolation must be strictly maintained to prevent cross-user access. Violations: {isolation_violations[:3]}'
    ...
    def test_id_format_backward_compatibility(self):
        ...
        for uuid_id in existing_uuids:
            if not self._is_backward_compatible_format(uuid_id):
                compatibility_violations.append({'type': 'uuid_rejection', 'id': uuid_id, 'error': 'Legacy UUID format not accepted'})
        for structured_id in new_structured_ids:
            if not self._is_valid_new_format(structured_id):
                compatibility_violations.append({'type': 'structured_format_invalid', 'id': structured_id, 'error': 'New structured format validation failed'})
        ...
        self.record_metric('uuid_compatibility_issues', len([v for v in compatibility_violations if v['type'] == 'uuid_rejection']))
        self.record_metric('structured_format_issues', len([v for v in compatibility_violations if v['type'] == 'structured_format_invalid']))
        assert len(compatibility_violations) == 0, f'Found {len(compatibility_violations)} backward compatibility violations. ID format migration must maintain compatibility with existing UUID-based systems. Violations: {compatibility_violations[:3]}'
        ...
        for i, id1 in enumerate(ids):
            for j, id2 in enumerate(ids[i + 1:], i + 1):
                ...
                if similarity_score > 0.8:
                    near_collisions.append({'id1': id1, 'id2': id2, 'similarity_score': similarity_score, 'indices': [i, j]})
    ...
    def _detect_sequential_patterns(self, ids: List[str]) -> List[Dict[str, Any]]:
        ...
        for i in range(len(ids) - 2):
            if self._ids_are_sequential(ids[i], ids[i + 1]) and self._ids_are_sequential(ids[i + 1], ids[i + 2]):
                sequential_patterns.append({'type': 'three_sequential', 'ids': [ids[i], ids[i + 1], ids[i + 2]], 'start_index': i})
    ...
    def _detect_timestamp_patterns(self, ids: List[str]) -> List[Dict[str, Any]]:
        ...
        for id_val in ids:
            import re
            numbers = re.findall('\\d{8,}', id_val)
            for num_str in numbers:
                num = int(num_str)
                if abs(num - current_time) < 3600 or abs(num - current_time * 1000) < 3600000:
                    timestamp_patterns.append({'type': 'timestamp_based', 'id': id_val, 'suspicious_number': num_str, 'time_diff': abs(num - current_time)})
    ...
    def _analyze_id_entropy(self, ids: List[str]) -> Dict[str, Any]:
        ...
        return {'entropy_score': entropy, 'insufficient_entropy': insufficient_entropy, 'char_distribution': dict(char_counts.most_common(10)), 'total_chars_analyzed': total_chars}

    def _generate_user_context_ids(self, user_id: str) -> Dict[str, List[str]]:
        """Generate various IDs associated with a user context."""
        context_ids = {'user_sessions': [], 'websocket_connections': [], 'agent_executions': [], 'requests': []}
        for _ in range(5):
            context_ids['user_sessions'].append(self.unified_id_manager.generate_id(IDType.SESSION, context={'user_id': user_id}))
            context_ids['websocket_connections'].append(self.unified_id_manager.generate_id(IDType.WEBSOCKET, context={'user_id': user_id}))
            context_ids['agent_executions'].append(self.unified_id_manager.generate_id(IDType.EXECUTION, context={'user_id': user_id}))
            context_ids['requests'].append(self.unified_id_manager.generate_id(IDType.REQUEST, context={'user_id': user_id}))
        return context_ids

    def _check_user_id_isolation(self, user1_ids: Dict[str, List[str]], user2_ids: Dict[str, List[str]], user1_id: str, user2_id: str) -> List[Dict[str, Any]]:
        ...
        if duplicates:
            violations.append({'type': 'exact_duplicate', 'user1': user1_id, 'user2': user2_id, 'duplicate_ids': list(duplicates)})
        for id1 in all_user1_ids:
            for id2 in all_user2_ids:
                if self._calculate_id_similarity(id1, id2) > 0.9:
                    violations.append({'type': 'high_similarity', 'user1': user1_id, 'user2': user2_id, 'id1': id1, 'id2': id2, 'similarity': self._calculate_id_similarity(id1, id2)})
        return violations
    ...
    def _check_cross_format_validation(self, uuid_id: str, structured_id: str) -> List[Dict[str, Any]]:
        ...
        if not self.unified_id_manager.is_valid_id_format_compatible(uuid_id):
            issues.append({'type': 'uuid_format_rejected', 'id': uuid_id, 'error': 'UUID format should be accepted during migration'})
        if not self.unified_id_manager.is_valid_id_format_compatible(structured_id):
            issues.append({'type': 'structured_format_rejected', 'id': structured_id, 'error': 'Structured format should be accepted'})
        return issues
