...
class TestAuthServiceConfigurationUnit(SSotBaseTestCase):
    ...
    def test_auth_service_url_configuration_loaded(self):
        ...
        self.assertIsNotNone(actual_value, 'AUTH_SERVICE_URL configuration is missing - this is the root cause of Issue #631')
        self.assertEqual(actual_value, test_auth_url, f'AUTH_SERVICE_URL should match environment variable. Got: {actual_value}, Expected: {test_auth_url}')
    ...
    def test_auth_service_url_missing_handling(self):
        ...
        with self.assertRaises((KeyError, AttributeError, ValueError), msg='Missing AUTH_SERVICE_URL should raise appropriate exception'):
            config = get_config()
    ...
    def test_auth_client_initialization_with_valid_url(self):
        ...
        try:
            ...
            self.assertIsNotNone(auth_client, 'AuthClientCore should initialize successfully with valid AUTH_SERVICE_URL')
            self.assertTrue(hasattr(auth_client, 'base_url') or hasattr(auth_client, '_base_url'), 'AuthClientCore should have base_url configured from AUTH_SERVICE_URL')
    ...
    def test_auth_client_initialization_with_invalid_url(self):
        ...
        """
        invalid_urls = ['not-a-url', 'http://nonexistent-service:99999', '', 'ftp://invalid-protocol']
            ...
            with self.subTest(url=invalid_url):
                ...
                try:
                    ...
                    if hasattr(auth_client, 'validate_url'):
                        self.assertFalse(auth_client.validate_url(invalid_url), f'Invalid URL {invalid_url} should be rejected')
    ...
    def test_jwt_validation_configuration(self):
        ...
        try:
            ...
            self.assertIsNotNone(auth_config, 'Auth configuration should be loaded successfully')
            self.assertTrue(hasattr(auth_config, 'jwt_secret_key') or hasattr(auth_config, 'jwt_validation_enabled'), 'Auth configuration should include JWT validation settings')
    ...
    def test_websocket_auth_middleware_configuration(self):
        ...
        try:
            ...
            self.assertTrue(hasattr(websocket_auth, 'auth_service_url') or hasattr(websocket_auth, '_auth_client') or hasattr(websocket_auth, 'auth_config'), 'WebSocket auth middleware should have access to AUTH_SERVICE_URL configuration')
        except (ImportError, AttributeError) as e:
    ...
    def test_auth_service_health_check_configuration(self):
        ...
        try:
            ...
            if auth_url:
                ...
                self.assertTrue(health_check_url.startswith('http'), 'Auth service health check URL should be properly formed')
            else:
