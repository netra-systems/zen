...
class WebSocketFactorySSotEnforcementTests(SSotBaseTestCase, unittest.TestCase):
    ...
    def test_deprecated_websocket_manager_factory_class_not_accessible(self):
        ...
        self.assertTrue(any((keyword in error_msg for keyword in ['deprecated', 'removed', 'not found', 'ssot'])), f'Error message should indicate deprecation/removal: {error_msg}')
    ...
    def test_deprecated_factory_class_not_exported(self):
        ...
        try:
            ...
            if hasattr(factory_module, '__all__'):
                ...
                self.assertNotIn('WebSocketManagerFactory', all_exports, 'WebSocketManagerFactory should NOT be exported after SSOT remediation')
            self.assertFalse(hasattr(factory_module, 'WebSocketManagerFactory'), 'WebSocketManagerFactory class should not exist after SSOT remediation')
        except ImportError:
    ...
    def test_canonical_get_websocket_manager_is_accessible(self):
        ...
        try:
            ...
            self.assertIn('user_context', param_names, 'get_websocket_manager should accept user_context parameter for isolation')
        ...
        try:
            with warnings.catch_warnings(record=True) as warning_list:
                ...
                try:
                    ...
                    self.assertGreater(len(warning_list), 0, 'Deprecated factory usage should issue warnings')
                    warning_messages = [str(w.message) for w in warning_list]
                    self.assertTrue(any(('deprecated' in msg.lower() for msg in warning_messages)), f'Warning should mention deprecation: {warning_messages}')
    ...
    def test_factory_pattern_replacement_guidance(self):
        ...
        deprecated_patterns = ['WebSocketManagerFactory', 'create_websocket_manager', 'get_websocket_manager_factory']
        for pattern in deprecated_patterns:
            try:
                import netra_backend.app.websocket_core.websocket_manager_factory as factory_module
                if hasattr(factory_module, pattern):
                    warnings.warn(f"Deprecated pattern '{pattern}' still exists - should be removed in SSOT remediation", DeprecationWarning)
            except ImportError:
                continue
