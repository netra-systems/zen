...
class TestUserExecutionEngine:
    ...
    async def test_active_runs_isolation(self, user_context, different_user_context,
                                        mock_agent_factory, mock_websocket_emitter):
    ...
    async def test_websocket_emitter_isolation(self, user_context, different_user_context,
                                              mock_agent_factory):
...
class TestExecutionEngineFactory:
    ...
    async def test_factory_user_limits(self, factory, configured_agent_factory):
        ...
        contexts = [
            UserExecutionContext(user_id=user_id, thread_id=f"thread_{i}", run_id=f"run_{i}")
            for i in range(3)
        ]
