...
class TestAuthenticationFailureLogging(SSotAsyncTestCase):
    ...
    def test_jwt_token_missing_logging(self):
        ...
        with patch('netra_backend.app.routes.websocket_ssot.logger', self.mock_logger):
            ...
            self.mock_logger.critical(f'[U+1F511] TOKEN EXTRACTION FAILURE: No JWT token found in WebSocket headers or subprotocols for connection {connection_id}')
    ...
    def test_jwt_validation_failure_logging(self):
        ...
        with patch('netra_backend.app.routes.websocket_ssot.logger', self.mock_logger):
            connection_id = self.connection_id
            self.mock_logger.critical(f'[U+1F511] JWT VALIDATION FAILURE: Token validation failed for connection {connection_id} - likely secret mismatch or malformed token')
        level, message, kwargs = self.log_capture[0]
        assert level == 'CRITICAL'
    ...
    def test_jwt_expiry_failure_logging(self):
        ...
        with patch('netra_backend.app.routes.websocket_ssot.logger', self.mock_logger):
            connection_id = self.connection_id
            self.mock_logger.critical(f'[U+1F511] JWT EXPIRY FAILURE: Token expired for connection {connection_id} - user needs to re-authenticate')
        level, message, kwargs = self.log_capture[0]
        assert level == 'CRITICAL'
    ...
    def test_auth_service_communication_failure_logging(self):
        ...
        with patch('netra_backend.app.auth_integration.auth.logger', self.mock_logger):
            ...
            self.mock_logger.critical(f' ALERT:  AUTH SERVICE EXCEPTION: Auth service communication failed (user_id: {user_id[:8]}..., timestamp: {datetime.now(timezone.utc).isoformat()})')
        level, message, kwargs = self.log_capture[0]
        assert level == 'CRITICAL'
    ...
    def test_user_database_lookup_failure_logging(self):
        ...
        with patch('netra_backend.app.auth_integration.auth.logger', self.mock_logger):
            ...
            self.mock_logger.critical(f' ALERT:  DATABASE SERVICE FAILURE: User database lookup failed (user_id: {user_id[:8]}..., error: {error_detail}, timestamp: {datetime.now(timezone.utc).isoformat()})')
        level, message, kwargs = self.log_capture[0]
        assert level == 'CRITICAL'
    ...
    def test_demo_mode_security_logging(self):
        ...
        with patch('netra_backend.app.routes.websocket_ssot.logger', self.mock_logger):
            ...
            self.mock_logger.warning(f' TARGET:  DEMO MODE AUTH: Demo authentication bypass activated for connection {connection_id} (demo_user: {demo_user_id}, timestamp: {datetime.now(timezone.utc).isoformat()})')
        level, message, kwargs = self.log_capture[0]
    ...
    def test_authentication_context_completeness(self):
        ...
        """
        required_context_fields = ['connection_id', 'user_id', 'timestamp', 'error_type', 'resolution_steps']
        with patch('netra_backend.app.routes.websocket_ssot.logger', self.mock_logger):
            auth_failure_context = {'connection_id': self.connection_id, 'user_id': self.user_id[:8] + '...', 'timestamp': datetime.now(timezone.utc).isoformat(), 'error_type': 'JWT_VALIDATION_FAILURE', 'error_details': 'Token signature verification failed', 'resolution_steps': ['Verify JWT_SECRET_KEY configuration', 'Check token signing algorithm', 'Validate token format'], 'golden_path_impact': 'CRITICAL - User cannot authenticate'}
            self.mock_logger.critical(f' ALERT:  GOLDEN PATH AUTH FAILURE: WebSocket authentication failed for connection {self.connection_id} - JWT_VALIDATION_FAILURE')
            self.mock_logger.critical(f' SEARCH:  AUTH FAILURE CONTEXT: {json.dumps(auth_failure_context, indent=2)}')
    ...
    def test_authentication_success_logging(self):
        ...
        with patch('netra_backend.app.routes.websocket_ssot.logger', self.mock_logger):
            user_id = self.user_id
            connection_id = self.connection_id
            auth_success_context = {'connection_id': connection_id, 'user_id': user_id[:8] + '...', 'auth_method': 'JWT', 'timestamp': datetime.now(timezone.utc).isoformat(), 'client_host': 'test-client', 'golden_path_impact': 'SUCCESS - User authenticated'}
            self.mock_logger.info(f" PASS:  GOLDEN PATH AUTH SUCCESS: User {(user_id[:8] if user_id else 'unknown')}... authenticated successfully for connection {connection_id}")
            self.mock_logger.info(f' SEARCH:  AUTH SUCCESS CONTEXT: {json.dumps(auth_success_context, indent=2)}')
        assert len(self.log_capture) == 2
        level1, message1, kwargs1 = self.log_capture[0]
        assert level1 == 'INFO'
    ...
    def test_logging_coverage_gaps(self):
        ...
        """
        coverage_gaps = [{'area': 'OAuth provider failures', 'current_status': 'NO_LOGGING', 'required_level': 'CRITICAL', 'context_needed': ['provider', 'error_code', 'redirect_uri']}, {'area': 'Rate limiting violations', 'current_status': 'NO_LOGGING', 'required_level': 'WARNING', 'context_needed': ['client_ip', 'attempt_count', 'window']}, {'area': 'Session hijacking detection', 'current_status': 'NO_LOGGING', 'required_level': 'CRITICAL', 'context_needed': ['session_id', 'ip_change', 'user_agent_change']}, {'area': 'Multi-device login conflicts', 'current_status': 'NO_LOGGING', 'required_level': 'INFO', 'context_needed': ['device_count', 'session_conflicts']}]
...
class TestAuthorizationFailureLogging(SSotAsyncTestCase):
    ...
    def test_insufficient_permissions_logging(self):
        ...
        with patch('netra_backend.app.auth_integration.auth.logger', self.mock_logger):
            ...
            self.mock_logger.warning(f'[U+1F512] AUTHORIZATION FAILURE: User {self.user_id[:8]}... lacks permission for {operation} (required: {required_role}, actual: {user_role}, timestamp: {datetime.now(timezone.utc).isoformat()})')
        level, message, kwargs = self.log_capture[0]
        assert level == 'WARNING'
    ...
    def test_role_escalation_attempt_logging(self):
        ...
        with patch('netra_backend.app.auth_integration.auth.logger', self.mock_logger):
            ...
            current_role = 'user'
            self.mock_logger.critical(f' ALERT:  SECURITY ALERT: Role escalation attempt by user {self.user_id[:8]}... (attempted: {attempted_role}, current: {current_role}, timestamp: {datetime.now(timezone.utc).isoformat()})')
        level, message, kwargs = self.log_capture[0]
        assert level == 'CRITICAL'
