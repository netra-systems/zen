...
class WebSocketConnectionFailureLoggingTests(SSotAsyncTestCase):
    ...
    def test_websocket_1011_error_logging(self):
        ...
        with patch('netra_backend.app.routes.websocket_ssot.logger', self.mock_logger):
            ...
            failure_context = {'connection_id': connection_id, 'duration_seconds': connection_duration, 'error_type': '1011_INTERNAL_ERROR', 'error_details': error_detail, 'client_host': 'test-client', 'timestamp': datetime.now(timezone.utc).isoformat(), 'golden_path_impact': 'CRITICAL - Connection failed completely'}
            self.mock_logger.critical(f' ALERT:  GOLDEN PATH CONNECTION FAILURE: WebSocket mode MAIN failed for connection {connection_id} after {connection_duration:.3f}s')
            self.mock_logger.critical(f' SEARCH:  CONNECTION FAILURE CONTEXT: {json.dumps(failure_context, indent=2)}')
    ...
    def test_gcp_load_balancer_header_stripping_logging(self):
        ...
        with patch('netra_backend.app.routes.websocket_ssot.logger', self.mock_logger):
            connection_id = self.connection_id
            self.mock_logger.critical(f" ALERT:  INFRASTRUCTURE FAILURE: GCP Load Balancer header stripping detected for connection {connection_id} (missing_headers: ['Authorization'], resolution: 'Update terraform-gcp-staging/load-balancer.tf')")
    ...
    def test_websocket_mode_failure_logging(self):
        ...
        with patch('netra_backend.app.routes.websocket_ssot.logger', self.mock_logger):
            ...
            error_context = {'connection_id': connection_id, 'requested_mode': invalid_mode, 'supported_modes': ['MAIN', 'FACTORY', 'ISOLATED', 'LEGACY'], 'timestamp': datetime.now(timezone.utc).isoformat(), 'golden_path_impact': 'CRITICAL - Connection rejected due to invalid mode'}
            self.mock_logger.critical(f' ALERT:  GOLDEN PATH MODE FAILURE: Unsupported WebSocket mode {invalid_mode} for connection {connection_id}')
            self.mock_logger.critical(f' SEARCH:  MODE FAILURE CONTEXT: {json.dumps(error_context, indent=2)}')
        assert len(self.log_capture) == 2
        level1, message1, kwargs1 = self.log_capture[0]
        assert level1 == 'CRITICAL'
    ...
    def test_websocket_manager_creation_failure_logging(self):
        ...
        with patch('netra_backend.app.routes.websocket_ssot.logger', self.mock_logger):
            ...
            manager_failure_context = {'connection_id': connection_id, 'user_id': user_id[:8] + '...', 'factory_error': factory_error, 'fallback_available': True, 'timestamp': datetime.now(timezone.utc).isoformat(), 'golden_path_impact': 'HIGH - Manager creation failed, fallback required'}
            self.mock_logger.critical(f" ALERT:  GOLDEN PATH MANAGER FAILURE: Failed to create WebSocket manager for user {(user_id[:8] if user_id else 'unknown')}... connection {connection_id}")
            self.mock_logger.critical(f' SEARCH:  MANAGER FAILURE CONTEXT: {json.dumps(manager_failure_context, indent=2)}')
        assert len(self.log_capture) == 2
        level1, message1, kwargs1 = self.log_capture[0]
        assert level1 == 'CRITICAL'
    ...
    def test_websocket_event_delivery_failure_logging(self):
        ...
        with patch('netra_backend.app.routes.websocket_ssot.logger', self.mock_logger):
            ...
            event_failure_context = {'connection_id': connection_id, 'user_id': user_id[:8] + '...', 'failed_event': failed_event, 'error_details': error_detail, 'retry_attempted': True, 'timestamp': datetime.now(timezone.utc).isoformat(), 'golden_path_impact': 'CRITICAL - User experience degraded'}
            self.mock_logger.critical(f' ALERT:  WEBSOCKET EVENT FAILURE: Failed to deliver {failed_event} event to user {user_id[:8]}... connection {connection_id}')
    ...
    def test_websocket_connection_timeout_logging(self):
        ...
        with patch('netra_backend.app.routes.websocket_ssot.logger', self.mock_logger):
            ...
            self.mock_logger.warning(f"[U+23F0] WEBSOCKET TIMEOUT: Connection {connection_id} timed out after {timeout_duration}s (phase: 'handshake', resolution: 'Increase timeout or check client connectivity')")
        level, message, kwargs = self.log_capture[0]
        assert level == 'WARNING'
    ...
    def test_websocket_unexpected_disconnect_logging(self):
        ...
        with patch('netra_backend.app.routes.websocket_ssot.logger', self.mock_logger):
            ...
            disconnect_context = {'connection_id': connection_id, 'user_id': user_id[:8] + '...', 'disconnect_reason': disconnect_reason, 'connection_duration': connection_duration, 'active_operations': ['agent_execution'], 'timestamp': datetime.now(timezone.utc).isoformat(), 'golden_path_impact': 'MEDIUM - User session interrupted'}
            self.mock_logger.warning(f'[U+1F50C] WEBSOCKET DISCONNECT: Unexpected disconnect for user {user_id[:8]}... connection {connection_id} after {connection_duration}s')
            self.mock_logger.info(f' SEARCH:  DISCONNECT CONTEXT: {json.dumps(disconnect_context, indent=2)}')
        assert len(self.log_capture) == 2
        level1, message1, kwargs1 = self.log_capture[0]
    ...
    def test_websocket_message_size_limit_exceeded_logging(self):
        ...
        with patch('netra_backend.app.routes.websocket_ssot.logger', self.mock_logger):
            ...
            message_size = 10240
            size_limit = 8192
            self.mock_logger.warning(f"[U+1F4CF] MESSAGE SIZE VIOLATION: Message from connection {connection_id} exceeds limit (size: {message_size} bytes, limit: {size_limit} bytes, action: 'rejected')")
        level, message, kwargs = self.log_capture[0]
        assert level == 'WARNING'
    ...
    def test_websocket_connection_establishment_success_logging(self):
        ...
        with patch('netra_backend.app.routes.websocket_ssot.logger', self.mock_logger):
            connection_id = self.connection_id
            user_id = self.user_id
            establishment_context = {'connection_id': connection_id, 'user_id': user_id[:8] + '...', 'critical_events': ['agent_started', 'agent_thinking', 'tool_executing', 'tool_completed', 'agent_completed'], 'timestamp': datetime.now(timezone.utc).isoformat(), 'golden_path_impact': 'SUCCESS - Full functionality available'}
            self.mock_logger.info(f" PASS:  GOLDEN PATH ESTABLISHED: Connection {connection_id} ready for user {(user_id[:8] if user_id else 'unknown')}... with all 5 critical events")
            self.mock_logger.info(f' SEARCH:  ESTABLISHMENT CONTEXT: {json.dumps(establishment_context, indent=2)}')
        assert len(self.log_capture) == 2
        level1, message1, kwargs1 = self.log_capture[0]
        assert level1 == 'INFO'
...
class WebSocketPerformanceLoggingTests(SSotAsyncTestCase):
    ...
    def test_websocket_performance_degradation_logging(self):
        ...
        with patch('netra_backend.app.routes.websocket_ssot.logger', self.mock_logger):
            ...
            response_time = 5.2
            threshold = 2.0
            self.mock_logger.warning(f"[U+1F40C] WEBSOCKET PERFORMANCE: Slow response detected for connection {connection_id} (response_time: {response_time}s, threshold: {threshold}s, action: 'monitor')")
        level, message, kwargs = self.log_capture[0]
        assert level == 'WARNING'
    ...
    def test_websocket_connection_metrics_logging(self):
        ...
        with patch('netra_backend.app.routes.websocket_ssot.logger', self.mock_logger):
            ...
            metrics = {'active_connections': active_connections, 'total_connections_today': total_connections, 'avg_response_time': avg_response_time, 'timestamp': datetime.now(timezone.utc).isoformat()}
            self.mock_logger.info(f' CHART:  WEBSOCKET METRICS: {json.dumps(metrics, indent=2)}')
        level, message, kwargs = self.log_capture[0]
        assert level == 'INFO'
