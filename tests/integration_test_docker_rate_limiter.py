    ...
    with patch('subprocess.run') as mock_run:
        ...
        def execute_docker_command(cmd_id):
            ...
            result = rate_limiter.execute_docker_command([
                "docker", "inspect", f"container-{cmd_id}"
            ])
        ...
        print(f"   Rate limited operations: {stats['rate_limited_operations']}")
...
2. Simulating WITHOUT rate limiting (would be faster but dangerous):")
    ...
    print(f"   With rate limiting:    {duration_with_limits:.2f}s ({rate_limited} operations rate-limited)")
    print(f"   Without rate limiting: {duration_without_limits:.2f}s (potential for API storm)")
    print(f"   Rate limiting overhead: {duration_with_limits - duration_without_limits:.2f}s")
...
[LINK] Testing Docker manager integration...")
    ...
    try:
        ...
        print(f"[ERROR] Error testing UnifiedDockerManager integration: {e}")
        return False
