...
class TestWebSocketE2ECrossUserContamination(SSotBaseTestCase):
    ...
    async def _analyze_cross_user_contamination(self, expected_messages: Dict[str, Dict[str, Any]]):
        ...
        for incident in self.contamination_incidents:
            contamination_errors.append(f"REAL-TIME CONTAMINATION: {incident['type']} - {incident['receiving_user']} received foreign data")
                        ...
                        if other_username != username:
                            ...
                            for marker in context_markers:
                                if marker and marker in message_content:
                                    result_contamination_errors.append(f"AGENT RESULT CONTAMINATION: User {username} received agent result with {other_username}'s context marker '{marker}'")
                    ...
                    if other_username != username:
                        for thread_id, expected_message in other_threads.items():
                            ...
                            if other_context in received_content:
                                isolation_errors.append(f"THREAD ISOLATION VIOLATION: User {username} received message with {other_username}'s thread context '{other_context}'")
        if isolation_errors:
