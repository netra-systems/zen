...
class TestStagingDatabaseMisconfiguration:
    ...
    def test_staging_should_not_use_dev_database_name(self):
        ...
        assert os.path.exists(staging_env_path), f"Staging env file not found: {staging_env_path}"
        ...
        assert postgres_db != 'netra_dev', \
            f"CRITICAL: Staging is using dev database! POSTGRES_DB={postgres_db} " \
    ...
    def test_database_manager_schema_for_staging(self):
        ...
        with mock.patch('netra_backend.app.core.environment_constants.get_current_environment') as mock_env:
            ...
            assert expected_schema == "public", \
                f"Staging should use 'public' schema, not '{expected_schema}'"
