...
class FrontendInitTestConfig:
    ...
    RETRY_DELAY = 1.0
    MAX_AUTH_TIME = 2000
    MAX_WS_CONNECT_TIME = 1500
    MAX_FIRST_PAINT_TIME = 1000
    MAX_TIME_TO_INTERACTIVE = 3000
...
class FrontendInitializationSimulator:
    ...
    async def simulate_first_page_load(self) -> Dict[str, Any]:
        ...
        results = {'success': False, 'metrics': {}, 'errors': [], 'events': []}
        try:
            auth_start = time.time()
            auth_result = await self._initialize_authentication()
            self.metrics['auth_init_time'] = (time.time() - auth_start) * 1000
            if not auth_result['success']:
                self.errors.append({'phase': 'auth_init', 'error': auth_result.get('error', 'Unknown auth error')})
            ...
            if not ws_result['success']:
                self.errors.append({'phase': 'websocket_connect', 'error': ws_result.get('error', 'WebSocket connection failed')})
                results['errors'] = self.errors
                return results
            ...
            msg_start = time.time()
            msg_result = await self._send_first_message(ws_result['websocket'], auth_result['token'])
            ...
            self.metrics['time_to_interactive'] = self.metrics['auth_init_time'] + self.metrics['ws_connect_time'] + self.metrics['initial_render_time']
            results['success'] = auth_result['success'] and ws_result['success'] and render_result['success'] and msg_result['success'] and events_valid
            results['metrics'] = self.metrics
            results['events'] = self.events_received
            results['errors'] = self.errors
        except Exception as e:
            logger.error(f'Frontend initialization failed: {e}')
            self.errors.append({'phase': 'general', 'error': str(e)})
            results['errors'] = self.errors
        return results
    ...
    async def _initialize_authentication(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {'success': True, 'token': token, 'user': user.dict()}
        except Exception as e:
            ...
            while connection_attempts < max_attempts:
                try:
                    with self.client.websocket_connect(ws_url) as websocket:
                        websocket.send_json({'type': 'connection_init', 'payload': {'token': token, 'client_id': f'test_{uuid.uuid4().hex[:8]}'}})
                        response = websocket.receive_json(timeout=5)
                        if response.get('type') == 'connection_ack':
                            logger.info('WebSocket connection established')
                            return {'success': True, 'websocket': websocket, 'connection_id': response.get('payload', {}).get('connection_id')}
                        else:
    ...
    async def _simulate_initial_render(self) -> Dict[str, Any]:
        ...
        try:
            components = ['AuthProvider', 'WebSocketProvider', 'AppWithLayout', 'MainChat', 'MessageList', 'MessageInput']
            mount_times = {}
            ...
            return {'success': all_mounted, 'mount_times': mount_times, 'total_mount_time': sum(mount_times.values())}
        except Exception as e:
    ...
    async def _send_first_message(self, websocket, token: str) -> Dict[str, Any]:
        ...
        try:
            thread_response = self.client.post('/api/threads', headers={'Authorization': f'Bearer {token}'}, json={'name': 'Test Thread'})
            ...
            while time.time() - start_time < timeout:
                try:
                    ...
                    if required_events_seen == set(FrontendInitTestConfig.REQUIRED_EVENTS):
                        ...
                        return {'success': True, 'events_received': list(required_events_seen), 'total_events': len(self.events_received)}
                except Exception as e:
            ...
            if missing_events:
                ...
                return {'success': False, 'error': f'Missing events: {missing_events}', 'events_received': list(required_events_seen)}
    ...
    async def _validate_websocket_events(self) -> bool:
        ...
        if missing:
            ...
            self.errors.append({'phase': 'event_validation', 'error': f'Missing events: {missing}'})
            return False
...
class ConcurrentSessionStressTest:
    ...
    def _analyze_stress_results(self) -> Dict[str, Any]:
        ...
        for metric_name in ['auth_init_time', 'ws_connect_time', 'initial_render_time', 'first_message_time']:
            ...
            if values:
                metric_stats[metric_name] = {'min': min(values), 'max': max(values), 'avg': sum(values) / len(values), 'p95': sorted(values)[int(len(values) * 0.95)] if len(values) > 1 else values[0]}
        all_errors = []
        for result in self.results:
            if 'errors' in result:
                all_errors.extend(result['errors'])
        return {'total_sessions': self.num_sessions, 'successful': successful, 'failed': failed, 'success_rate': successful / self.num_sessions * 100, 'metric_stats': metric_stats, 'errors': all_errors}
...
async def test_frontend_initialization_happy_path():
    ...
    assert metrics['auth_init_time'] < FrontendInitTestConfig.MAX_AUTH_TIME, f"Auth init too slow: {metrics['auth_init_time']}ms"
    assert metrics['ws_connect_time'] < FrontendInitTestConfig.MAX_WS_CONNECT_TIME, f"WebSocket connect too slow: {metrics['ws_connect_time']}ms"
    assert metrics['time_to_interactive'] < FrontendInitTestConfig.MAX_TIME_TO_INTERACTIVE, f"Time to interactive too slow: {metrics['time_to_interactive']}ms"
...
async def test_frontend_init_with_auth_failure():
    ...
    assert any((e['phase'] == 'auth_init' for e in result['errors'])), 'Should have auth_init error'
...
async def test_concurrent_session_stress():
    ...
    assert results['success_rate'] >= 80, f"Too many failures: {results['failed']}/{results['total_sessions']}"
    for metric, stats in results['metric_stats'].items():
        if 'auth_init' in metric:
            assert stats['p95'] < FrontendInitTestConfig.MAX_AUTH_TIME * 1.5, f"{metric} p95 too high: {stats['p95']}ms"
        elif 'ws_connect' in metric:
            assert stats['p95'] < FrontendInitTestConfig.MAX_WS_CONNECT_TIME * 1.5, f"{metric} p95 too high: {stats['p95']}ms"
