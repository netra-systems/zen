...
class TestWebSocketConnectionHandlerCloud(SSotBaseTestCase):
    ...
    async def test_exception_logging_captures_full_details(self):
        ...
        for exception_name, exception_creator in exception_test_cases:
            ...
            assert log_str != expected_empty_pattern, f"CRITICAL BUG: Exception logging truncated for {exception_name}. Log message: '{log_str}'. Expected more details than just user ID and empty string. Original exception: {test_exception}"
            exception_type_name = type(test_exception).__name__
            assert exception_type_name in log_str, f"Exception logging should include exception type '{exception_type_name}' but log is: '{log_str}'"
