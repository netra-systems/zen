...
class AgentStatePersistenceE2ETests(BaseE2ETest):
    ...
    async def setup_state_persistence_environment(self):
        ...
        self.test_users = []
        for i in range(2):
            user_context = await create_authenticated_user_context(user_email=f'state_persistence_test_{i}_{int(time.time())}@staging.netra.ai', environment='staging', permissions=['read', 'write', 'execute_agents', 'persist_state', 'manage_sessions'])
