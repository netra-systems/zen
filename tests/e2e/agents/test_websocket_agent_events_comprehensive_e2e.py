...
@pytest.mark.e2e
class WebSocketAgentEventsE2ETests(BaseE2ETest):
    """
    ...
    async def test_complete_agent_event_sequence_validation(self):
        ...
        async def monitor_complete_event_sequence():
            ...
            try:
                ...
                async for message in websocket:
                    ...
                    event_record = {**event, 'received_at': receive_time, 'sequence_position': len(collected_events) + 1, 'time_since_start': receive_time - event_start_time}
                    collected_events.append(event_record)
                    event_sequence.append(event_type)
                    event_payloads[event_type] = event_data
                    self.logger.info(f'📡 Event {len(collected_events)}: {event_type} ({receive_time - event_start_time:.3f}s since start)')
        ...
        complete_request = {'type': 'execute_agent', 'agent_type': 'comprehensive_event_test', 'user_id': user_context.user_id, 'thread_id': user_context.thread_id, 'request_id': user_context.request_id, 'data': {'event_sequence_validation': True, 'require_all_events': True, 'tool_execution_required': True, 'thinking_updates_required': True, 'completion_validation': True, 'event_timing_test': True}}
    ...
    async def test_real_time_event_timing_validation(self):
        ...
        async def monitor_event_timing():
            ...
            try:
                async for message in websocket:
                    ...
                    event_timestamps.append({'type': event_type, 'timestamp': receive_time, 'time_since_start': receive_time - first_event_time})
        ...
        timing_request = {'type': 'execute_agent', 'agent_type': 'realtime_timing_test', 'user_id': user_context.user_id, 'thread_id': user_context.thread_id, 'request_id': user_context.request_id, 'data': {'realtime_events': True, 'event_timing_test': True, 'immediate_responses': True, 'timing_validation': True}}
            ...
            try:
                ...
                async def collect_user_events():
                    ...
                    async for message in websocket:
                        event = json.loads(message)
                        event_type = event.get('type')
                        user_events.append({**event, 'user_index': user_index, 'user_id': user_context.user_id, 'received_at': time.time()})
                        if event_type == 'agent_completed':
                            event_complete = True
                            break
                event_task = asyncio.create_task(collect_user_events())
                load_request = {'type': 'execute_agent', 'agent_type': 'concurrent_event_load_test', 'user_id': user_context.user_id, 'thread_id': user_context.thread_id, 'request_id': user_context.request_id, 'data': {'user_index': user_index, 'concurrent_load_test': True, 'event_validation': True, 'stress_test': True}}
                start_time = time.time()
                ...
                return {'user_index': user_index, 'user_id': user_context.user_id, 'success': True, 'event_complete': event_complete, 'events_received': len(user_events), 'missing_events': missing_events, 'execution_duration': execution_duration, 'all_required_events': len(missing_events) == 0}
            except Exception as e:
                return {'user_index': user_index, 'user_id': user_context.user_id, 'success': False, 'error': str(e)}
        tasks = [run_concurrent_event_test(user_context, i) for i, user_context in enumerate(self.test_users)]
    ...
    async def test_event_payload_accuracy_validation(self):
        ...
        user_specific_data = {'user_name': f'Test User {user_context.user_id[:8]}', 'user_preference': 'detailed_analysis', 'user_context': f'payload_test_session_{int(time.time())}', 'expected_outcomes': ['cost_analysis', 'optimization_recommendations', 'risk_assessment']}
        payload_request = {'type': 'execute_agent', 'agent_type': 'payload_accuracy_test', 'user_id': user_context.user_id, 'thread_id': user_context.thread_id, 'request_id': user_context.request_id, 'data': {'payload_validation': True, 'user_specific_data': user_specific_data, 'accuracy_test': True, 'detailed_payloads': True}}
    ...
    async def test_websocket_event_error_recovery(self):
        ...
        error_recovery_request = {'type': 'execute_agent', 'agent_type': 'error_recovery_event_test', 'user_id': user_context.user_id, 'thread_id': user_context.thread_id, 'request_id': user_context.request_id, 'data': {'error_recovery_test': True, 'simulate_recoverable_error': True, 'continue_after_recovery': True, 'event_continuity_test': True}}
    ...
    def _validate_event_payload_structure(self, event_type: str, event_data: Dict[str, Any], user_context) -> Dict[str, Any]:
        ...
        validation = {'valid': True, 'issues': [], 'completeness_score': 0.0}
        try:
            if not isinstance(event_data, dict):
                validation['valid'] = False
                validation['issues'].append(f'Event data is not a dictionary: {type(event_data)}')
                return validation
            required_fields = {'agent_started': ['status'], 'agent_thinking': ['reasoning', 'message'], 'tool_executing': ['tool_name', 'tool'], 'tool_completed': ['result', 'output'], 'agent_completed': ['status', 'result']}
            optional_fields = {'agent_started': ['agent_name', 'timestamp', 'user_id'], 'agent_thinking': ['step_number', 'progress'], 'tool_executing': ['tool_args', 'execution_id'], 'tool_completed': ['execution_time', 'success'], 'agent_completed': ['duration', 'summary', 'success']}
