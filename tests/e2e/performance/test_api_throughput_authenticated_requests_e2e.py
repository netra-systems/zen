...
class APIThroughputAuthenticatedRequestsE2ETests(BaseE2ETest):
    ...
    async def test_authenticated_api_endpoint_throughput(self):
        ...
        for i in range(concurrent_users):
            ...
            auth_result = await self.auth_helper.create_and_authenticate_user(email=f'api_user_{i}@example.com', tier=tier, test_prefix=self.test_prefix)
            if auth_result.success:
                authenticated_users.append({'user_id': auth_result.user_id, 'tier': tier, 'access_token': auth_result.access_token})
        assert len(authenticated_users) >= concurrent_users * 0.8
        api_endpoints = [{'path': '/api/health', 'method': 'GET', 'expected_status': 200}, {'path': '/api/user/profile', 'method': 'GET', 'expected_status': 200}, {'path': '/api/projects', 'method': 'GET', 'expected_status': 200}]
        for endpoint in api_endpoints:
            print(f"Testing endpoint: {endpoint['path']}")
            request_tasks = []
            for user in authenticated_users:
                for req_num in range(requests_per_user):
                    task = self._make_authenticated_request(user=user, endpoint=endpoint, request_id=f"{user['user_id']}_{req_num}")
            ...
            assert success_rate >= 0.8, f"API success rate too low for {endpoint['path']}: {success_rate:.2%}"
    ...
    async def _make_authenticated_request(self, user: Dict[str, Any], endpoint: Dict[str, Any], request_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            async with httpx.AsyncClient(timeout=30.0) as client:
                response = await client.request(method=endpoint['method'], url=f"{self.backend_url}{endpoint['path']}", headers={'Authorization': f"Bearer {user['access_token']}", 'X-Test-ID': self.test_prefix})
                end_time = time.time()
                response_time = end_time - start_time
                return {'success': response.status_code == endpoint['expected_status'], 'status_code': response.status_code, 'response_time': response_time, 'user_tier': user['tier'], 'request_id': request_id}
