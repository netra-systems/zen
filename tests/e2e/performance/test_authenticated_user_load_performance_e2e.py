...
class TestAuthenticatedUserLoadPerformanceE2E(BaseE2ETest):
    ...
    async def test_concurrent_user_authentication_load(self):
        ...
        for i in range(concurrent_users):
            ...
            user_scenario = {'user_id': f'{self.test_prefix}_user_{i:03d}', 'email': f'load_test_{i}@example.com', 'tier': user_tier, 'expected_permissions': self._get_tier_permissions(user_tier)}
            authentication_scenarios.append(user_scenario)
        auth_start_time = time.time()
        auth_tasks = [self._authenticate_user_e2e(scenario) for scenario in authentication_scenarios]
        auth_results = await asyncio.gather(*auth_tasks, return_exceptions=True)
        auth_end_time = time.time()
        total_auth_duration = auth_end_time - auth_start_time
        successful_auths = [result for result in auth_results if isinstance(result, dict) and result.get('authenticated', False)]
        failed_auths = [result for result in auth_results if isinstance(result, Exception) or (isinstance(result, dict) and (not result.get('authenticated', False)))]
        auth_success_rate = len(successful_auths) / len(auth_results)
        assert auth_success_rate >= 0.9, f'Authentication success rate too low: {auth_success_rate:.2%}'
        ...
        for tier in user_tiers:
            ...
            for auth in tier_auths:
                ...
                assert actual_permissions.issuperset(expected_permissions), f'Missing permissions for {tier} tier: {expected_permissions - actual_permissions}'
    ...
    async def _authenticate_user_e2e(self, user_scenario: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            auth_result = await self.auth_helper.create_and_authenticate_user(email=user_scenario['email'], tier=user_scenario['tier'], test_prefix=self.test_prefix)
            auth_end = time.time()
            auth_duration = auth_end - auth_start
            if auth_result.success:
                return {'authenticated': True, 'user_id': user_scenario['user_id'], 'user_tier': user_scenario['tier'], 'auth_duration': auth_duration, 'access_token': auth_result.access_token, 'permissions': auth_result.permissions, 'session_id': auth_result.session_id}
            else:
                return {'authenticated': False, 'user_id': user_scenario['user_id'], 'error': auth_result.error, 'auth_duration': auth_duration}
        except Exception as e:
            return {'authenticated': False, 'user_id': user_scenario['user_id'], 'error': str(e)}
    ...
    def _get_tier_permissions(self, tier: str) -> set:
        ...
        if tier == 'free':
            ...
            return base_permissions | {'create_projects', 'advanced_analytics', 'cost_optimization', 'team_management', 'api_access', 'priority_support'}
        else:
            return base_permissions
    ...
    async def test_multi_user_agent_execution_performance(self):
        ...
        for i in range(concurrent_users):
            ...
            auth_result = await self.auth_helper.create_and_authenticate_user(email=f'agent_test_{i}@example.com', tier=tier, test_prefix=self.test_prefix)
            if auth_result.success:
                authenticated_users.append({'user_id': f'agent_user_{i}', 'tier': tier, 'access_token': auth_result.access_token, 'session_id': auth_result.session_id})
        assert len(authenticated_users) >= concurrent_users * 0.8, f'Insufficient authenticated users: {len(authenticated_users)}/{concurrent_users}'
        agent_start_time = time.time()
        agent_tasks = []
        for user in authenticated_users:
            for agent_num in range(agents_per_user):
                task = self._execute_agent_request_e2e(user=user, agent_type='cost_optimizer' if agent_num == 0 else 'triage_agent', request_data={'message': f"Test optimization request from {user['user_id']}", 'complexity': 'medium', 'priority': 'normal'})
                agent_tasks.append(task)
        agent_results = await asyncio.gather(*agent_tasks, return_exceptions=True)
        agent_end_time = time.time()
        total_agent_duration = agent_end_time - agent_start_time
        successful_executions = [result for result in agent_results if isinstance(result, dict) and result.get('success', False)]
        failed_executions = [result for result in agent_results if isinstance(result, Exception) or (isinstance(result, dict) and (not result.get('success', False)))]
        agent_success_rate = len(successful_executions) / len(agent_results)
        assert agent_success_rate >= 0.8, f'Agent execution success rate too low: {agent_success_rate:.2%}'
        expected_total_duration = len(authenticated_users) * agents_per_user * 10 / 3
        assert total_agent_duration < expected_total_duration, f'Multi-user agent execution too slow: {total_agent_duration:.2f}s (expected < {expected_total_duration:.2f}s)'
    ...
    async def _execute_agent_request_e2e(self, user: Dict[str, Any], agent_type: str, request_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            async with httpx.AsyncClient(timeout=60.0) as client:
                response = await client.post(f'{self.backend_url}/api/agents/execute', headers={'Authorization': f"Bearer {user['access_token']}", 'Content-Type': 'application/json', 'X-Test-ID': self.test_prefix}, json={'agent_type': agent_type, 'message': request_data['message'], 'options': {'complexity': request_data['complexity'], 'priority': request_data['priority']}})
                execution_end = time.time()
                execution_time = execution_end - execution_start
                if response.status_code == 200:
                    response_data = response.json()
                    return {'success': True, 'user_id': user['user_id'], 'user_tier': user['tier'], 'agent_type': agent_type, 'execution_time': execution_time, 'response_data': response_data, 'websocket_events': response_data.get('events', [])}
                else:
                    return {'success': False, 'user_id': user['user_id'], 'error': f'HTTP {response.status_code}: {response.text}', 'execution_time': execution_time}
        except Exception as e:
    ...
    async def test_authenticated_websocket_concurrent_connections(self):
        ...
        for i in range(concurrent_connections):
            auth_result = await self.auth_helper.create_and_authenticate_user(email=f'ws_test_{i}@example.com', tier='mid', test_prefix=self.test_prefix)
            if auth_result.success:
                authenticated_users.append({'user_id': f'ws_user_{i}', 'access_token': auth_result.access_token})
        assert len(authenticated_users) >= concurrent_connections * 0.7, f'Insufficient authenticated users for WebSocket test: {len(authenticated_users)}/{concurrent_connections}'
        ws_start_time = time.time()
        ws_tasks = [self._test_authenticated_websocket_connection(user) for user in authenticated_users]
        ...
        successful_connections = [result for result in ws_results if isinstance(result, dict) and result.get('connected', False)]
        ws_success_rate = len(successful_connections) / len(ws_results)
        if len(successful_connections) > 0:
            assert ws_success_rate >= 0.7, f'WebSocket connection success rate too low: {ws_success_rate:.2%}'
    ...
    async def _test_authenticated_websocket_connection(self, user: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            async with websockets.connect(ws_url_with_auth, open_timeout=10) as websocket:
                ...
                messages_received = 0
                for i in range(3):
                    test_message = {'type': 'test_message', 'user_id': user['user_id'], 'message_id': i, 'content': f'Test message {i}'}
                ...
                return {'connected': True, 'user_id': user['user_id'], 'connection_time': connection_time, 'messages_exchanged': messages_sent + messages_received, 'messages_sent': messages_sent, 'messages_received': messages_received}
        except Exception as e:
            return {'connected': False, 'user_id': user['user_id'], 'error': str(e)}
