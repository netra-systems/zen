...
class DevLauncherRealSystem:
    ...
    async def start_all_services(self) -> None:
        ...
        try:
            ...
            logger.info(f"All services ready in {elapsed:.2f}s")
    ...
    async def _run_launcher(self) -> None:
        ...
        try:
            ...
            if result != 0:
                self._startup_errors.append(f"Launcher returned exit code {result}")
        except Exception as e:
            self._startup_success = False
            self._startup_errors.append(f"Launcher exception: {str(e)}")
    ...
    async def _wait_for_startup_completion(self, timeout: int = 60) -> None:
        ...
        while (time.time() - start_time) < timeout:
            if self._launcher_task and self._launcher_task.done():
                ...
                if not self._startup_success:
                    raise RuntimeError(f"Launcher startup failed: {self._startup_errors}")
                return
                
            # Check if services are becoming available
            if await self._check_basic_health():
        ...
        raise TimeoutError(f"Startup timeout after {timeout}s")
    ...
    async def stop_all_services(self) -> None:
        ...
        if self.launcher:
            try:
                ...
                logger.warning(f"Error during launcher shutdown: {e}")
            finally:
                self.launcher = None
...
class DevLauncherTestContext:
    ...
    async def test_cleanup_test_environment(self) -> None:
        ...
        logger.info(f"Test environment cleaned up after {elapsed:.2f}s")
