...
class MissionCriticalWebSocketValidator:
    ...
    def _validate_event_order(self) -> bool:
        ...
        if self.event_timeline[0][1] != 'agent_started':
            ...
            return False
        has_activity = any((event_type in ['agent_thinking', 'tool_executing', 'partial_result'] for _, event_type, _ in self.event_timeline))
        if not has_activity:
...
class InMemoryWebSocketConnection:
    ...
    async def send_json(self, message: dict, timeout: float=None):
        ...
        try:
            ...
            logger.info(fWebSocket send_json #{self.send_count} SUCCESS: {serializable_message.get('type', 'unknown')} (timeout={timeout}")"
            return None
...
async def test_websocket_core_event_flow():
    ...
    try:
        ...
        try:
            ...
            context = AgentExecutionContext(run_id='core-req-123', thread_id=connection_id, user_id=user_id, agent_name='test_agent', retry_count=0, max_retries=1)
...
async def test_websocket_manager_real_connection_handling():
    ...
    try:
        ...
        notifier = WebSocketNotifier.create_for_user(ws_manager)
        context = AgentExecutionContext(run_id='broadcast-test', thread_id='broadcast', user_id='broadcast-user', agent_name='broadcast_agent', retry_count=0, max_retries=1)
        await notifier.send_agent_started(context)
        await asyncio.sleep(0.1)
