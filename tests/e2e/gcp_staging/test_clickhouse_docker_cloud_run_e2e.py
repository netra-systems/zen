...
from netra_backend.app.core.environment_context.cloud_environment_detector import get_cloud_environment_detector, detect_current_environment
from netra_backend.app.logging_config import central_logger
from fastapi import FastAPI

@pytest.mark.e2e
class ClickHouseDockerCloudRunE2ETests:
    ...
    def get_real_docker_availability(self) -> Dict[str, Any]:
        ...
        docker_status = {'available': False, 'error': None, 'containers': [], 'command_exists': False}
        try:
            ...
            if docker_status['command_exists']:
                docker_ps_result = subprocess.run(['docker', 'ps', '--format', '{{.Names}}'], capture_output=True, text=True, timeout=10)
    ...
    @pytest.mark.asyncio
    @pytest.mark.skipif(not os.environ.get('RUN_GCP_E2E_TESTS', '').lower() == 'true', reason='GCP E2E tests require RUN_GCP_E2E_TESTS=true environment variable')
    async def test_real_gcp_cloud_run_docker_unavailability_e2e(self, gcp_staging_app):
        ...
        if is_cloud_run:
            ...
            try:
                ...
                assert environment_context.cloud_platform.value in ['cloud_run', 'unknown'], f'Expected Cloud Run platform, got: {environment_context.cloud_platform.value}'
    ...
    async def test_full_startup_sequence_with_docker_unavailable_e2e(self, gcp_staging_app):
        ...
        if not docker_status['available']:
            ...
            try:
                ...
                assert hasattr(gcp_staging_app.state, 'startup_complete'), 'Startup should set completion flag'
                ...
                pytest.fail(f'Complete application startup failed due to Docker unavailability. This indicates Issue #568 has broader impact than just ClickHouse: {startup_error}')
        else:
    ...
    async def test_simulated_cloud_run_environment_docker_unavailable(self):
        ...
        """
        logger = central_logger.get_logger(__name__)
        cloud_run_env = {'K_SERVICE': 'netra-backend-staging', 'GOOGLE_CLOUD_PROJECT': 'netra-staging', 'PORT': '8080', 'ENVIRONMENT': 'staging', 'CLICKHOUSE_REQUIRED': 'false'}
