...
class ServiceSecretStagingValidationTests(SSotAsyncTestCase):
    ...
    async def _check_service_health(self, service_url: str, service_name: str) -> Dict[str, Any]:
        ...
        try:
            async with httpx.AsyncClient(timeout=10.0) as client:
                ...
                return {'service': service_name, 'url': service_url, 'status': response.status_code, 'healthy': response.status_code == 200, 'response': response.text[:100] if response.text else 'No response'}
        except Exception as e:
            return {'service': service_name, 'url': service_url, 'healthy': False, 'error': str(e)[:100]}
    ...
    def test_staging_service_health_prerequisite(self):
        ...
        async def check_all_services():
            ...
            health_checks = await asyncio.gather(self._check_service_health(self.staging_backend_url, 'backend'), self._check_service_health(self.staging_auth_url, 'auth'), return_exceptions=True)
            unhealthy_services = []
            for health in health_checks:
                if isinstance(health, Exception):
                    unhealthy_services.append(f'Exception: {str(health)}')
                elif not health.get('healthy', False):
                    unhealthy_services.append(f"{health['service']}: {health.get('status', 'Unknown')} - {health.get('error', health.get('response', 'No details'))}")
            if unhealthy_services:
                self.fail(f'Staging services not healthy for Issue #1037 testing:\n' + '\n'.join((f'  - {service}' for service in unhealthy_services)))
        ...
        async def test_service_auth_consistency():
            ...
            try:
                async with httpx.AsyncClient(timeout=30.0) as client:
                    ...
                    auth_validation_response = await client.post(f'{self.staging_auth_url}/auth/service/validate', headers={'Content-Type': 'application/json', 'X-Service-ID': 'backend-service'}, json={'service_check': True})
                    if auth_validation_response.status_code == 403:
                        authentication_failures.append({'test': 'backend_auth_validation', 'status': 403, 'details': 'Backend cannot authenticate with auth service', 'response': auth_validation_response.text[:200]})
                    logger.info('Testing service token request...')
                    token_response = await client.post(f'{self.staging_auth_url}/auth/service/token', headers={'Content-Type': 'application/json', 'X-Service-ID': 'backend-service'}, json={'grant_type': 'service_credentials'})
                    if token_response.status_code in [401, 403]:
                        authentication_failures.append({'test': 'service_token_request', 'status': token_response.status_code, 'details': 'Service token request failed', 'response': token_response.text[:200]})
                    ...
                    if token_response.status_code == 200:
                        try:
                            ...
                            if access_token:
                                health_response = await client.get(f'{self.staging_backend_url}/health', headers={'Authorization': f'Bearer {access_token}'})
                                if health_response.status_code == 403:
                                    authentication_failures.append({'test': 'authenticated_health_check', 'status': 403, 'details': 'Backend rejected service token', 'response': health_response.text[:200]})
                        except Exception as e:
                ...
                try:
                    async with websockets.connect(ws_url, timeout=10) as websocket:
                        ...
                        if 'error' in response.lower() or '403' in response:
                            websocket_failures.append({'test': 'websocket_connection', 'issue': 'Authentication error in WebSocket response', 'response': response[:200]})
                except (ConnectionClosedError, InvalidStatusCode, asyncio.TimeoutError) as e:
                    if '403' in str(e) or 'unauthorized' in str(e).lower():
                        websocket_failures.append({'test': 'websocket_connection', 'issue': 'WebSocket connection failed with auth error', 'error': str(e)[:200]})
                    else:
                        logger.warning(f'WebSocket connection failed (non-auth reason): {str(e)}')
                if websocket_failures:
                    failure_details = '\n'.join([f"  {f['test']}: {f['issue']}\n    Details: {f.get('response', f.get('error', 'No details'))}" for f in websocket_failures])
                    raise AssertionError(f'Issue #1037 WebSocket regression CONFIRMED IN STAGING:\nWebSocket authentication failures detected:\n{failure_details}\n\nThis matches the 503 WebSocket errors described in Issue #1037.')
                else:
    ...
    def test_staging_end_to_end_user_authentication_flow(self):
        ...
        async def test_user_auth_flow():
            ...
            try:
                async with httpx.AsyncClient(timeout=30.0) as client:
                    registration_response = await client.post(f'{self.staging_auth_url}/auth/register', headers={'Content-Type': 'application/json'}, json={'email': f'test-{datetime.now().timestamp()}@example.com', 'password': 'TestPassword123!', 'name': 'Issue 1037 Test User'})
                    if registration_response.status_code == 403:
                        user_flow_failures.append({'step': 'user_registration', 'status': 403, 'issue': 'User registration blocked by authentication issue', 'response': registration_response.text[:200]})
                    login_response = await client.post(f'{self.staging_auth_url}/auth/login', headers={'Content-Type': 'application/json'}, json={'email': 'test@example.com', 'password': 'TestPassword123!'})
                    if login_response.status_code == 403:
                        user_flow_failures.append({'step': 'user_login', 'status': 403, 'issue': 'User login failed with authentication error', 'response': login_response.text[:200]})
                    if login_response.status_code == 200:
                        try:
                            login_data = login_response.json()
                            refresh_token = login_data.get('refresh_token')
                            if refresh_token:
                                refresh_response = await client.post(f'{self.staging_auth_url}/auth/refresh', headers={'Content-Type': 'application/json'}, json={'refresh_token': refresh_token})
                                if refresh_response.status_code == 403:
                                    user_flow_failures.append({'step': 'token_refresh', 'status': 403, 'issue': 'Token refresh failed with authentication error', 'response': refresh_response.text[:200]})
                        except Exception as e:
                            user_flow_failures.append({'step': 'token_parsing', 'issue': 'Failed to parse login response for token refresh', 'error': str(e)[:200]})
                if user_flow_failures:
                    flow_summary = '\n'.join([f"  {f['step']}: {f.get('status', 'Error')} - {f['issue']}\n    Response: {f.get('response', f.get('error', 'No details'))}" for f in user_flow_failures])
                    raise AssertionError(f'Issue #1037 USER FLOW REGRESSION CONFIRMED IN STAGING:\nEnd-to-end authentication flow broken:\n{flow_summary}\n\nThis confirms Issue #1037 affects real user experience. Business Impact: Complete user lockout in staging environment.')
                else:
        ...
        async def analyze_staging_config():
            ...
            try:
                async with httpx.AsyncClient(timeout=15.0) as client:
                    config_endpoints = ['/config/auth', '/health/config', '/debug/config']
                        ...
                        try:
                            for service_url in [self.staging_backend_url, self.staging_auth_url]:
                                ...
                                if config_response.status_code == 200:
                                    ...
                                    issue_indicators = [('JWT_SECRET_KEY missing', 'JWT_SECRET_KEY' not in config_data), ('SERVICE_SECRET missing', 'SERVICE_SECRET' not in config_data), ('Mixed auth patterns', 'JWT_SECRET' in config_data and 'SERVICE_SECRET' in config_data), ('Auth service errors', 'error' in config_data.lower() or 'fail' in config_data.lower())]
                                    for indicator_name, condition in issue_indicators:
                                        if condition:
                                            config_issues.append({'service': service_url, 'endpoint': endpoint, 'issue': indicator_name, 'config_sample': config_data[:100]})
                        except httpx.RequestError:
                            continue
                if config_issues:
                    config_summary = '\n'.join([f"  {issue['service']}{issue['endpoint']}: {issue['issue']}\n    Config: {issue['config_sample']}..." for issue in config_issues])
                    logger.warning(f'Staging configuration analysis found potential Issue #1037 indicators:\n{config_summary}\n\nThese patterns may contribute to authentication problems.')
                else:
