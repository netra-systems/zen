...
class GCPStagingWebSocketTester:
    ...
    async def test_websocket_connection(self, path: str='/ws') -> Dict[str, Any]:
        ...
        result = {'url': full_ws_url, 'success': False, 'error': None, 'response_time': None, 'connection_state': None, 'protocol_version': None, 'subprotocol': None}
        start_time = time.time()
        try:
            ssl_context = ssl.create_default_context()
            ssl_context.check_hostname = False
            ssl_context.verify_mode = ssl.CERT_NONE
            async with websockets.connect(full_ws_url, ssl=ssl_context, timeout=self.timeout, subprotocols=['chat', 'graphql-ws'], extra_headers={'User-Agent': 'Issue-449-Test-Client', 'Origin': 'https://app.netra.co'}) as websocket:
                result['success'] = True
            ...
            self.connection_failures.append({'error': 'Connection timeout', 'timeout': self.timeout, 'url': full_ws_url})
            result['error'] = 'Connection timeout'
        except Exception as e:
            self.connection_failures.append({'error': 'General connection failure', 'details': str(e), 'url': full_ws_url})
    ...
    def test_http_to_websocket_upgrade(self, path: str='/ws') -> Dict[str, Any]:
        ...
        result = {'url': full_http_url, 'upgrade_attempted': False, 'upgrade_success': False, 'response_headers': {}, 'status_code': None, 'error': None}
        try:
            headers = {'Connection': 'Upgrade', 'Upgrade': 'websocket', 'Sec-WebSocket-Version': '13', 'Sec-WebSocket-Key': 'dGhlIHNhbXBsZSBub25jZQ==', 'Sec-WebSocket-Protocol': 'chat, superchat', 'Origin': 'https://app.netra.co', 'User-Agent': 'Issue-449-Upgrade-Test'}
            response = requests.get(full_http_url, headers=headers, timeout=self.timeout, verify=False)
    ...
    def test_staging_service_availability(self) -> Dict[str, Any]:
        ...
        result = {'service_available': False, 'health_status': None, 'response_time': None, 'error': None}
        start_time = time.time()
        try:
            response = requests.get(health_url, timeout=self.timeout, verify=False)
...
class TestIssue449GCPStagingWebSocketProtocol(SSotBaseTestCase):
    ...
    async def test_gcp_staging_websocket_connection_failure(self):
        ...
        self.assertTrue(result['success'], f"WebSocket connection should succeed in staging: {result['error']}")
        ...
        if not result['success']:
            self.assertGreater(len(self.gcp_tester.protocol_errors) + len(self.gcp_tester.middleware_conflicts), 0, 'Should have captured protocol or middleware errors')
    ...
    async def test_gcp_staging_websocket_subprotocol_negotiation_failure(self):
        ...
        result = await self.gcp_tester.test_websocket_connection('/ws')
        if result['success']:
            self.assertIsNotNone(result['subprotocol'], 'WebSocket subprotocol should be negotiated in staging')
            self.assertIn(result['subprotocol'], ['chat', 'graphql-ws'], f"Negotiated subprotocol should be valid: {result['subprotocol']}")
        else:
            error_msg = result['error'].lower()
            self.assertTrue('protocol' in error_msg or 'upgrade' in error_msg, f"Should be protocol-related failure: {result['error']}")
    ...
    def test_gcp_staging_http_websocket_upgrade_failure(self):
        ...
        self.assertTrue(result['upgrade_success'], f"HTTP to WebSocket upgrade should succeed: {result['error']}")
        self.assertEqual(result['status_code'], 101, f"Should get 101 Switching Protocols, got {result['status_code']}")
        headers = result['response_headers']
        self.assertIn('upgrade', [k.lower() for k in headers.keys()], 'Response should have Upgrade header')
        self.assertIn('connection', [k.lower() for k in headers.keys()], 'Response should have Connection header')
    ...
    async def test_gcp_staging_multiple_websocket_connections_failure(self):
        ...
        """
        connection_tasks = [self.gcp_tester.test_websocket_connection(f'/ws?id={i}') for i in range(3)]
        results = await asyncio.gather(*connection_tasks, return_exceptions=True)
        successful_connections = [r for r in results if isinstance(r, dict) and r.get('success')]
        self.assertEqual(len(successful_connections), 3, f'All 3 WebSocket connections should succeed, got {len(successful_connections)}')
        ...
        if failed_connections:
            ...
            self.assertTrue('middleware' in combined_errors or 'conflict' in combined_errors, f'Failures should be middleware-related: {error_messages}')
    ...
    async def test_gcp_staging_websocket_with_authentication_failure(self):
        ...
        result = {'success': False, 'error': None, 'auth_processed': False}
        try:
            ssl_context = ssl.create_default_context()
            ssl_context.check_hostname = False
            ssl_context.verify_mode = ssl.CERT_NONE
            async with websockets.connect(auth_ws_url, ssl=ssl_context, timeout=self.gcp_tester.timeout, extra_headers={'Authorization': 'Bearer test-token', 'Origin': 'https://app.netra.co'}) as websocket:
                result['success'] = True
        ...
        self.assertTrue(result['success'], f"WebSocket with authentication should work: {result['error']}")
        self.assertTrue(result['auth_processed'], 'WebSocket authentication should be processed correctly')
    ...
    def test_gcp_staging_websocket_cors_middleware_conflict(self):
        ...
        if result['upgrade_attempted']:
            ...
            found_cors_headers = [header for header in cors_headers if any((h.lower() == header for h in headers.keys()))]
            self.assertEqual(len(found_cors_headers), 0, f'WebSocket upgrade should not have CORS headers: {found_cors_headers}')
    ...
    async def test_gcp_staging_websocket_timeout_under_load(self):
        ...
        for i in range(5):
            ...
            timeout_result = {'connection_id': i, 'success': result['success'], 'response_time': result['response_time'], 'error': result['error'], 'exceeded_timeout': result['response_time'] > 15.0}
        ...
        self.assertEqual(len(failed_connections), 0, f'All WebSocket connections should complete within timeout, failed: {len(failed_connections)}')
        ...
        if successful_times:
            ...
            self.assertLess(avg_response_time, 5.0, f'Average WebSocket response time should be < 5s, got {avg_response_time:.2f}s')
