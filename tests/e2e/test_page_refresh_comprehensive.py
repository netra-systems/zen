...
@compliance CLAUDE.md - Chat is King (90% of value delivery)
...
class PageRefreshTestSuite:
    ...
    async def test_basic_refresh_with_active_chat(self, page: Page) -> bool:
        """
...
🔍 Testing: {test_name}")
        ...
        try:
            ...
            if test_message not in str(messages_after):
                raise AssertionError(f"Message not preserved after refresh. Before: {messages_before}, After: {messages_after}")
            
            print(f"✅ {test_name}: Messages preserved across refresh")
    ...
    async def test_websocket_reconnection_on_refresh(self, page: Page) -> bool:
        """
...
🔍 Testing: {test_name}")
    ...
    pass
                ...
                ws.on('close', lambda: ws_connections[-1].update({'closed': True}))
            ...
            print(f"✅ {test_name}: WebSocket reconnected successfully")
            self.test_results['passed'] += 1
            return True
    ...
    async def test_rapid_refresh_resilience(self, page: Page) -> bool:
        """
...
🔍 Testing: {test_name}")
    ...
    pass
            ...
            for i in range(refresh_count):
                print(f"  Refresh {i + 1}/{refresh_count}")
            ...
            if critical_errors:
                raise AssertionError(f"Critical errors during rapid refresh: {critical_errors[:3]}")
            ...
            if ws_count > refresh_count * 2:  # Allow some overhead
                raise AssertionError(f"Excessive WebSocket connections detected: {ws_count}")
            
            print(f"✅ {test_name}: System stable after {refresh_count} rapid refreshes")
            self.test_results['passed'] += 1
            return True
    ...
    async def test_draft_message_persistence(self, page: Page) -> bool:
        """
...
🔍 Testing: {test_name}")
        ...
        try:
            ...
            if restored_text == draft_text:
                print(f"✅ {test_name}: Draft message preserved")
                self.test_results['passed'] += 1
                return True
            else:
                print(f"⚠️ {test_name}: Draft not preserved (non-critical)")
    ...
    async def test_token_refresh_during_page_reload(self, page: Page) -> bool:
        """
...
🔍 Testing: {test_name}")
        ...
        try:
            ...
            if not chat_visible:
                ...
                print(f"⚠️ {test_name}: Chat not visible after token expiry (may be expected)")
            
            print(f"✅ {test_name}: Token refresh handled correctly")
            self.test_results['passed'] += 1
            return True
    ...
    async def test_scroll_position_restoration(self, page: Page) -> bool:
        """
...
🔍 Testing: {test_name}")
        ...
        try:
            ...
            if message_input:
                for i in range(5):
                    await message_input.fill(f"Message {i + 1}")
                    await message_input.press("Enter")
            ...
            if abs(scroll_after - scroll_before) < 100:
                print(f"✅ {test_name}: Scroll position preserved")
                self.test_results['passed'] += 1
                return True
            else:
                print(f"⚠️ {test_name}: Scroll position not preserved (non-critical)")
    ...
    async def test_performance_metrics_after_refresh(self, page: Page) -> bool:
        """
...
🔍 Testing: {test_name}")
        ...
        try:
            ...
            if refresh_load_time > initial_load_time * 1.5:
                print(f"⚠️ Refresh slower than initial load: {refresh_load_time:.2f}s vs {initial_load_time:.2f}s")
            
            if metrics['domInteractive'] > 3000:
                print(f"⚠️ Slow time to interactive: {metrics['domInteractive']}ms")
            
            print(f"✅ {test_name}: Performance metrics collected")
            print(f"   Initial load: {initial_load_time:.2f}s")
            print(f"   Refresh load: {refresh_load_time:.2f}s")
    ...
    async with async_playwright() as p:
        ...
        try:
            ...
            assert len(results['critical_failures']) == 0, \
                f"Critical failures detected: {results['critical_failures']}"
            
            # Assert reasonable pass rate
            pass_rate = results['passed'] / results['total'] if results['total'] > 0 else 0
            assert pass_rate >= 0.8, f"Pass rate too low: {pass_rate:.2%}"
