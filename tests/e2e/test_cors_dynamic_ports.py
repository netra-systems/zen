...
def mock_service_discovery(config: DynamicServiceConfig):
    ...
    with patch('app.core.middleware_setup.ServiceDiscovery', return_value=mock_discovery):
...
class TestCORSDynamicFrontendPorts:
    ...
    def dynamic_frontend_configs(self):
        ...
        return [
            ...
            DynamicServiceConfig(frontend_port=5173, backend_port=8000, auth_port=8081),  # Vite dev server
            DynamicServiceConfig(frontend_port=3333, backend_port=8000, auth_port=8081),  # Next.js alternative
        ]
    ...
    async def test_cors_hardcoded_localhost_origins_limitation(self, dynamic_frontend_configs):
        ...
        """
        # Force a scenario where only hardcoded origins are used (no wildcard, no patterns)
    ...
    async def test_service_discovery_integration_gap(self, dynamic_frontend_configs):
        ...
        for config in dynamic_frontend_configs:
            with mock_service_discovery(config):
                ...
                assert frontend_info is not None, "Service discovery should have frontend info"
                assert frontend_info['port'] == config.frontend_port, "Service discovery should track dynamic port"
                
                # But CORS origins are still hardcoded
                cors_origins = get_cors_origins()
                
                # Check if the dynamic URL from service discovery is in CORS origins
        ...
        for config in dynamic_frontend_configs:
            with mock_service_discovery(config):
                ...
                middleware = CustomCORSMiddleware(
                    app=mock_app,
                    service_discovery=mock_service_discovery_instance
                )
                ...
                is_allowed = middleware._is_origin_allowed_with_discovery(config.frontend_url)
        ...
        async with httpx.AsyncClient() as client:
            ...
            try:
                ...
                assert cors_origin in [dynamic_origin, "*"], (
                    f"CORS headers missing or incorrect for dynamic frontend port {dynamic_port}. "
                ...
                assert cors_origin in [dynamic_origin, "*"], (
                    f"Actual request CORS headers missing for dynamic frontend port {dynamic_port}. "
                    f"Expected: {dynamic_origin} or *, Got: {cors_origin}"
                )
...
class TestCORSDynamicBackendPorts:
    ...
    def dynamic_backend_configs(self):
        ...
        return [
            ...
            DynamicServiceConfig(frontend_port=3001, backend_port=5000, auth_port=8081),  # Flask default
        ]
    ...
    async def test_dynamic_backend_port_service_discovery(self, dynamic_backend_configs):
        ...
        the CORS middleware doesn't properly integrate with this information.
        """
        for config in dynamic_backend_configs:
            with mock_service_discovery(config):
                ...
                assert backend_info is not None, "Service discovery should return backend info"
                assert backend_info['port'] == config.backend_port, (
                    f"Service discovery should track dynamic backend port {config.backend_port}"
                )
                ...
                is_allowed = middleware._is_origin_allowed_with_discovery(frontend_origin)
                
                # This might pass for standard frontend port, but the real issue is that
                # the backend running on dynamic port won't be properly configured for CORS
                assert is_allowed, f"Standard frontend should be allowed with dynamic backend port {config.backend_port}"
        ...
        for config in dynamic_backend_configs:
            ...
            with patch.dict(os.environ, {
                ...
                with mock_service_discovery(config):
                    ...
                    # The default origins should include the dynamic backend port for self-requests
                    backend_self_origin = f"http://localhost:{config.backend_port}"
                    ...
                    assert backend_self_origin in origins or "*" in origins, (
                        f"Backend running on dynamic port {config.backend_port} should include "
    ...
    async def test_cross_service_requests_dynamic_backend(self):
        ...
        with mock_service_discovery(config):
            ...
            async with httpx.AsyncClient() as client:
                ...
                try:
                    # This request would go to the dynamically allocated backend port
...
class TestCORSDynamicAuthServicePorts:
    ...
    async def test_dynamic_auth_port_cors_validation(self, dynamic_auth_configs):
        ...
        for config in dynamic_auth_configs:
            with mock_service_discovery(config):
                # Test if frontend can make requests to auth service on dynamic port
                async with httpx.AsyncClient() as client:
                    headers = {
                        "Origin": config.frontend_url,
                        "Content-Type": "application/json"
                    ...
                    try:
                        ...
                        assert cors_origin in [config.frontend_url, "*"], (
                            f"Auth service on dynamic port {config.auth_port} should accept "
                            f"CORS requests from frontend but rejected origin {config.frontend_url}. "
                            f"Got CORS origin: {cors_origin}"
                        )
                        
                    except httpx.ConnectError:
                        pytest.skip(f"Auth service not running on dynamic port {config.auth_port}")
                    except httpx.TimeoutException:
                        pytest.skip(f"Auth service timeout on dynamic port {config.auth_port}")
    ...
    async def test_auth_service_discovery_integration(self, dynamic_auth_configs):
        ...
        for config in dynamic_auth_configs:
            with mock_service_discovery(config):
                ...
                assert auth_info is not None, "Auth service discovery should return info"
                assert auth_info['port'] == config.auth_port, (
                    f"Service discovery should track auth port {config.auth_port}"
                )
                ...
                assert is_allowed, (
                    f"Auth service on dynamic port {config.auth_port} should allow "
            ...
            async with httpx.AsyncClient() as client:
                ...
                try:
                    ...
                    callback_response = await client.options(
                        f"{config.auth_url}/auth/callback",
                        headers=headers,
                        timeout=5.0
                    )
                    ...
                    cors_origin = callback_response.headers.get("Access-Control-Allow-Origin")
                    assert cors_origin in [config.frontend_url, "*"], (
                        f"OAuth callback should allow frontend origin {config.frontend_url} "
                        f"but got CORS origin: {cors_origin}"
                    )
                    
                except httpx.ConnectError:
                    pytest.skip("Auth service not running on dynamic port for OAuth test")
...
class TestCORSComprehensiveDynamicPortFailures:
    ...
    async def test_complete_dynamic_environment_failure(self):
        ...
        # Force restrictive CORS environment (no wildcards, no pattern matching)
        with patch.dict(os.environ, {"ENVIRONMENT": "testing", "CORS_ORIGINS": "http://localhost:3000,http://localhost:8000"}):
            with patch('app.core.middleware_setup._check_pattern_matches', return_value=False):
                with patch('app.core.middleware_setup._check_wildcard_match', return_value=False):
                    with mock_service_discovery(config):
                        ...
                        # Test CORS validation for each dynamic origin
                        dynamic_origins = [config.frontend_url, config.backend_url, config.auth_url]
                        
                        for origin in dynamic_origins:
                            is_allowed = is_origin_allowed(origin, cors_origins)
                            if not is_allowed:
                                failures.append(f"Dynamic origin {origin} rejected by CORS")
                        
                        # This assertion WILL FAIL because of multiple CORS failures
                        assert len(failures) == 0, (
                            f"Multiple CORS failures in restrictive dynamic port environment:\n" +
                            "\n".join(f"- {failure}" for failure in failures) +
                            f"\n\nConfiguration: Frontend:{config.frontend_port}, "
                            f"Backend:{config.backend_port}, Auth:{config.auth_port}\n"
        ...
        with patch.dict(os.environ, {"ENVIRONMENT": "staging", "CORS_ORIGINS": ""}):
            with patch('app.core.middleware_setup._check_wildcard_match', return_value=False):
                ...
                assert is_origin_explicitly_allowed, (
                    f"Dynamic frontend port {config.frontend_port} at {origin} "
...
class TestCORSRegexPatternLimitations:
    ...
    async def test_localhost_pattern_edge_cases(self):
        ...
        with patch.dict(os.environ, {"ENVIRONMENT": "development"}):
            ...
            for origin in valid_cases:
                assert _check_localhost_pattern(origin), f"Valid localhost pattern should match: {origin}"
            ...
            for origin, expected, description in edge_cases:
                result = _check_localhost_pattern(origin)
                # Some of these assertions may FAIL due to regex limitations
                assert result == expected, f"{description}: {origin} -> expected {expected}, got {result}"
    ...
    async def test_pattern_vs_static_list_inconsistency(self):
        ...
        with patch.dict(os.environ, {"ENVIRONMENT": "development"}):
            ...
            # Check which pattern-allowed origins are actually allowed by full CORS logic
            actually_allowed = []
            pattern_rejected = []
            
            for origin in pattern_allowed:
