...
from tests.e2e.jwt_token_helpers import JWTSecurityTester, JWTTestFixtures, JWTTestHelper
...
class CrossServiceJWTValidator:
    ...
    async def validate_auth_to_backend_flow(self, token: str) -> Dict[str, Any]:
        ...
        return {'auth_status': auth_result['status'], 'backend_status': backend_result['status'], 'auth_duration': auth_duration, 'backend_duration': backend_duration, 'total_duration': total_duration, 'performance_ok': total_duration < 2.0}
    ...
    async def validate_websocket_token_flow(self, token: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if connected:
                ...
                message_sent = await ws_manager.send_chat_message('Test cross-service auth', f'test-{int(time.time())}')
                message_duration = self.performance.get_duration(message_start)
                alive = await ws_manager.test_connection_alive()
                total_duration = self.performance.get_duration(start_time)
                return {'connected': connected, 'message_sent': message_sent, 'connection_alive': alive, 'connection_duration': connection_duration, 'message_duration': message_duration, 'total_duration': total_duration, 'performance_ok': total_duration < 2.0}
            else:
                return {'connected': False, 'connection_duration': connection_duration, 'performance_ok': connection_duration < 2.0, 'error': 'WebSocket connection failed'}
        finally:
    ...
    async def validate_token_refresh_propagation(self, user_id: str) -> Dict[str, Any]:
        ...
        if refresh_response and 'access_token' in refresh_response:
            ...
            total_duration = self.performance.get_duration(start_time)
            return {'expired_token_rejected': expired_auth_result['status'] == 401 and expired_backend_result['status'] == 401, 'refresh_successful': refresh_response is not None, 'new_token_valid': new_auth_result['status'] in [200, 500] and new_backend_result['status'] in [200, 500], 'refresh_duration': refresh_duration, 'total_duration': total_duration, 'performance_ok': refresh_duration < 2.0}
        else:
            return {'refresh_failed': True, 'refresh_duration': refresh_duration, 'performance_ok': refresh_duration < 2.0}
    ...
    async def validate_token_revocation_effects(self, user_id: str) -> Dict[str, Any]:
        ...
        tampered_token = await self.jwt_helper.create_tampered_token(self.jwt_helper.create_valid_payload())
        ...
        total_duration = self.performance.get_duration(start_time)
        return {'initial_token_valid': initial_auth['status'] in [200, 500] and initial_backend['status'] in [200, 500], 'revoked_token_rejected': revoked_auth['status'] == 401 and revoked_backend['status'] == 401, 'consistent_revocation': revoked_auth['status'] == revoked_backend['status'], 'total_duration': total_duration, 'performance_ok': total_duration < 2.0}
    ...
    async def validate_service_to_service_auth(self) -> Dict[str, Any]:
        ...
        backend_token = self.jwt_helper.create_token({'sub': backend_creds['service_id'], 'service': 'netra-backend', 'token_type': 'service', 'iat': int(datetime.now(timezone.utc).timestamp()), 'exp': int((datetime.now(timezone.utc) + timedelta(minutes=5)).timestamp()), 'iss': 'netra-auth-service'})
        worker_token = self.jwt_helper.create_token({'sub': worker_creds['service_id'], 'service': 'netra-worker', 'token_type': 'service', 'iat': int(datetime.now(timezone.utc).timestamp()), 'exp': int((datetime.now(timezone.utc) + timedelta(minutes=5)).timestamp()), 'iss': 'netra-auth-service'})
        ...
        total_duration = self.performance.get_duration(start_time)
        return {'backend_service_token_valid': backend_auth_result['status'] in [200, 500], 'worker_service_token_valid': worker_auth_result['status'] in [200, 500], 'tokens_different': backend_token != worker_token, 'correct_token_type': backend_payload.get('token_type') == 'service' and worker_payload.get('token_type') == 'service', 'correct_service_fields': 'service' in backend_payload and 'service' in worker_payload, 'total_duration': total_duration, 'performance_ok': total_duration < 2.0}
...
async def test_jwt_performance_requirements():
    ...
    if auth_result['status'] not in [500]:
