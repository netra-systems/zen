...
class RealWebSocketAuthTestCore:
    ...
    async def setup_real_auth_infrastructure(self, isolated_env) -> Dict[str, Any]:
        ...
        return {'auth_client': self.auth_client, 'backend_client': self.backend_client, 'auth_healthy': auth_health, 'backend_healthy': backend_health, 'env': isolated_env}
    ...
    async def create_real_authenticated_user(self, plan_tier: PlanTier=PlanTier.PRO) -> Dict[str, Any]:
        ...
        register_response = await self.auth_client.register(email=test_email, password=test_password, name=test_name)
        ...
        user_data = {'user_id': user_id, 'email': test_email, 'token': user_token, 'plan_tier': plan_tier, 'token_expires_at': token_validation.get('expires_at'), 'created_at': time.time()}
    ...
    async def _validate_real_jwt_token(self, token: str) -> Dict[str, Any]:
        ...
        try:
            ...
            return {'valid': validation_response.get('valid', False), 'user_id': validation_response.get('user_id'), 'expires_at': validation_response.get('expires_at')}
    ...
    async def test_real_agent_interaction(self, ws_client: WebSocketTestClient, test_message: str) -> Dict[str, Any]:
        ...
        while time.time() - timeout_start < 30.0:
            ...
            if event:
                ...
                if event.get('type') == 'agent_started':
                    ...
                    break
        return {'responses': responses, 'agent_started': agent_started, 'agent_completed': agent_completed, 'response_count': len(responses), 'total_time': time.time() - timeout_start}
...
class TestRealWebSocketAuthIntegration:
    ...
    async def test_real_jwt_token_validation_websocket(self, auth_test_core):
        ...
        invalid_connection = await invalid_ws_client.connect(token='invalid.jwt.token', timeout=5.0)
    ...
    async def test_real_websocket_session_persistence(self, auth_test_core):
        ...
        user_data = await auth_test_core.create_real_authenticated_user(PlanTier.PRO)
        ws_client = await auth_test_core.establish_real_websocket_connection(user_data)
        messages = ['First authenticated message for session test', 'Second authenticated message - session should persist', 'Third authenticated message - validate continued access']
        interaction_results = []
    ...
    async def test_real_websocket_auth_error_handling(self, auth_test_core):
        ...
        backend_host = auth_test_core.backend_client.base_url.replace('http://', '').replace('https://', '')
        ws_url = f'ws://{backend_host}/ws'
        invalid_scenarios = [('expired.jwt.token', 'Expired token test'), ('malformed-token', 'Malformed token test'), ('', 'Empty token test'), (None, 'No token test')]
        ...
        test_result = await auth_test_core.test_real_agent_interaction(valid_ws_client, 'Test message after auth error scenarios')
    ...
    async def test_real_websocket_auth_performance(self, auth_test_core):
        ...
        interaction_result = await auth_test_core.test_real_agent_interaction(ws_client, 'Performance test message for auth validation')
