...
class TestWebSocketGCPTimingScenarios(SSotBaseTestCase):
    ...
    async def test_authenticated_websocket_gcp_timeout_import_scope_failure(self):
        ...
        gcp_timing_constraints = [{'timeout': 2.0, 'scenario': 'rapid_timeout'}, {'timeout': 5.0, 'scenario': 'moderate_timeout'}, {'timeout': 10.0, 'scenario': 'near_gcp_limit'}, {'timeout': 15.0, 'scenario': 'gcp_limit_timeout'}]
        ...
        if import_scope_failures:
            ...
            raise AssertionError(f'GCP TIMEOUT IMPORT SCOPE BUGS REPRODUCED in {len(import_scope_failures)} scenarios:\n{failure_summary}')
        pytest.fail(f'GCP timing import scope bugs not reproduced. All {len(gcp_timeout_scenarios)} GCP timeout scenarios completed without import scope errors.')
    ...
    async def test_authenticated_concurrent_users_import_scope_race_conditions(self):
        ...
        async def create_authenticated_concurrent_connection(user: AuthenticatedUser, conn_index: int) -> Dict[str, Any]:
            ...
            try:
                ...
                websocket = await asyncio.wait_for(websockets.connect(self.websocket_url, additional_headers=auth_headers, subprotocols=auth_subprotocols, open_timeout=3.0, close_timeout=1.0), timeout=5.0)
                concurrent_messages = [{'type': 'ping', 'user_id': user.user_id, 'connection_index': conn_index, 'message_index': msg_idx, 'timestamp': time.time()} for msg_idx in range(3)]
        ...
        try:
            ...
            if race_condition_import_failures:
                ...
                raise AssertionError(f'CONCURRENT USER RACE CONDITION IMPORT SCOPE BUGS REPRODUCED in {len(race_condition_import_failures)} connections:\n{failure_summary}')
            pytest.fail(f'Concurrent user race condition import scope bugs not reproduced. All {len(authenticated_users)} authenticated users connected successfully without import scope race conditions.')
    ...
    async def test_authenticated_websocket_agent_execution_import_scope_failure(self):
        ...
        pytest.fail(f'Agent execution import scope bugs not reproduced. All {len(agent_execution_scenarios)} authenticated agent execution scenarios completed without import scope errors.')
    ...
    async def test_authenticated_websocket_error_recovery_import_scope_failure(self):
        ...
        for scenario in error_recovery_scenarios:
            ...
            try:
                ...
                if scenario['trigger'] == 'malformed_json':
                    ...
                    permission_request = {'type': 'admin_request', 'user_id': self.primary_user.user_id, 'data': {'action': 'system_admin_operation'}}
                    await websocket.send(json.dumps(permission_request))
                elif scenario['trigger'] == 'rapid_reconnection':
                    await websocket.close()
                    websocket = await websockets.connect(self.websocket_url, additional_headers=auth_headers, open_timeout=2.0)
                try:
        ...
        pytest.fail(f'Error recovery import scope bugs not reproduced. All {len(error_recovery_scenarios)} authenticated error recovery scenarios completed without import scope errors.')
