...
class MultiUserAgentIsolationE2ETests(BaseE2ETest):
    ...
    async def create_isolated_user_session(self, user_email: str, permissions: List[str]=None) -> Tuple[Any, Any, str]:
        ...
        """
        auth_user = await self.auth_helper.create_authenticated_user(email=user_email, permissions=permissions or ['read', 'write', 'agent_execute'])
    ...
    async def execute_agent_for_user(self, auth_user: Any, websocket_connection: Any, user_context: Any, message: str, expected_response_indicators: List[str]=None) -> List[Dict[str, Any]]:
        """
        ...
        """
        received_events = []
        chat_message = {'type': 'agent_request', 'agent': 'triage_agent', 'message': message, 'user_id': auth_user.user_id, 'thread_id': str(user_context.thread_id), 'request_id': str(user_context.request_id), 'timestamp': datetime.now(timezone.utc).isoformat()}
        ...
        if expected_response_indicators:
            ...
            if completion_events:
                ...
                for indicator in expected_response_indicators:
                    assert indicator.lower() in response_text.lower(), f"User {auth_user.email} missing expected response indicator '{indicator}' in: {response_text[:100]}..."
    ...
    async def test_concurrent_user_agent_isolation_basic(self, real_services_fixture):
        ...
        user1_task = asyncio.create_task(self.execute_agent_for_user(user1_auth, user1_ws, user1_context, user1_message, user1_indicators))
        user2_task = asyncio.create_task(self.execute_agent_for_user(user2_auth, user2_ws, user2_context, user2_message, user2_indicators))
        ...
        for event in user1_events:
            event_user_id = event.get('data', {}).get('user_id')
            if event_user_id:
                assert event_user_id == user1_auth.user_id, f'User 1 event contaminated with user ID: {event_user_id}'
        for event in user2_events:
            event_user_id = event.get('data', {}).get('user_id')
            if event_user_id:
                assert event_user_id == user2_auth.user_id, f'User 2 event contaminated with user ID: {event_user_id}'
        user1_response = ''
        user2_response = ''
    ...
    async def test_multi_user_thread_isolation(self, real_services_fixture):
        ...
        messages = ['Analyze my monthly AWS costs and suggest optimizations for Q1 2024.', 'Review my quarterly AWS spending and recommend cost reductions for Q2 2024.', 'Evaluate my annual AWS expenditure and provide optimization strategies for 2024.']
        expected_indicators = [['monthly', 'q1', '2024'], ['quarterly', 'q2', '2024'], ['annual', '2024']]
        tasks = []
        for i, (user, ws, context) in enumerate(zip(users, connections, contexts)):
            task = asyncio.create_task(self.execute_agent_for_user(user, ws, context, messages[i], expected_indicators[i]))
            tasks.append(task)
        ...
        for i, events in enumerate(all_user_events):
            ...
            for event in events:
                ...
                if event_user_id:
                    assert event_user_id == users[i].user_id, f'User {i + 1} received event with wrong user_id: {event_user_id}'
                if event_thread_id:
                    assert event_thread_id == str(contexts[i].thread_id), f'User {i + 1} received event with wrong thread_id: {event_thread_id}'
        ...
        for i, response in enumerate(responses):
            for indicator in expected_indicators[i]:
                assert indicator.lower() in response.lower(), f"User {i + 1} response missing expected indicator '{indicator}'"
    ...
    async def test_user_permission_isolation(self, real_services_fixture):
        ...
        admin_user, admin_ws, admin_context = await self.create_isolated_user_session('admin_isolation@example.com', permissions=['read', 'write', 'agent_execute', 'admin', 'sensitive_data'])
        basic_user, basic_ws, basic_context = await self.create_isolated_user_session('basic_isolation@example.com', permissions=['read', 'agent_execute'])
        ...
        admin_task = asyncio.create_task(self.execute_agent_for_user(admin_user, admin_ws, admin_context, message))
        basic_task = asyncio.create_task(self.execute_agent_for_user(basic_user, basic_ws, basic_context, message))
    ...
    async def test_concurrent_user_resource_cleanup(self, real_services_fixture):
        ...
        for batch in range(2):
            ...
            for i, (user, ws, context) in enumerate(zip(batch_users, batch_connections, batch_contexts)):
                ...
                task = asyncio.create_task(self.execute_agent_for_user(user, ws, context, message))
                tasks.append(task)
        ...
        expected_min_events = num_users * 2 * 5
        assert total_events >= expected_min_events, f'Insufficient total events: {total_events} < {expected_min_events}'
        user_ids_seen = set()
        ...
        expected_users = num_users * 2
        assert len(user_ids_seen) == expected_users, f'Expected {expected_users} unique users, found {len(user_ids_seen)}'
