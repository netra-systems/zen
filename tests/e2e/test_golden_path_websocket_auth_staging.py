...
class TestWebSocketAuthGoldenPathStaging(StagingTestBase):
    ...
    async def _verify_staging_services(cls):
        ...
        try:
            backend_health = await cls.staging_helpers.check_service_health(cls.staging_config.get_backend_base_url())
            assert backend_health, 'Staging backend is not healthy'
            auth_health = await cls.staging_helpers.check_service_health(cls.staging_auth_url)
    ...
    async def _create_golden_path_test_user(cls) -> Dict[str, Any]:
        ...
        try:
            ...
            access_token = await cls.auth_client.generate_test_access_token(user_id=test_user_data['user_id'], email=test_user_data['email'])
    ...
    async def test_complete_golden_path_user_flow_staging(self):
        ...
        try:
            ...
            auth_verification = await self.auth_client.verify_token(self.test_user['access_token'])
            ...
            golden_path_message = {'type': 'chat_message', 'data': {'message': 'Hello! This is a Golden Path test message for WebSocket auth protocol validation.', 'test_scenario': 'golden_path_staging', 'timestamp': int(time.time()), 'user_id': self.test_user['user_id']}}
            ...
            self.logger.info(f'GOLDEN PATH SUCCESS: Complete user flow works in staging. Total duration: {total_golden_path_duration:.1f}s. Steps: {golden_path_steps}')
    ...
    async def test_websocket_connection_gcp_cloud_run_environment(self):
        ...
        """
        gcp_test_scenarios = [{'name': 'correct_protocol_format', 'subprotocols': ['jwt-auth', f"jwt.{self.test_user['encoded_token']}"], 'expected_result': 'success', 'timeout': 20}, {'name': 'incorrect_protocol_bug_format', 'subprotocols': ['jwt', self.test_user['encoded_token']], 'expected_result': 'failure', 'timeout': 25}, {'name': 'missing_jwt_prefix', 'subprotocols': ['jwt-auth', self.test_user['encoded_token']], 'expected_result': 'failure', 'timeout': 20}, {'name': 'no_subprotocols', 'subprotocols': [], 'expected_result': 'failure', 'timeout': 15}]
        gcp_test_results = []
        for scenario in gcp_test_scenarios:
            ...
            try:
                ...
                if scenario['expected_result'] == 'success' and result['status'] != 'success':
                    self.logger.error(f"GCP ISSUE: {scenario['name']} expected to succeed but failed. Duration: {scenario_duration:.1f}s")
                elif scenario['expected_result'] == 'failure' and result['status'] == 'success':
                    self.logger.warning(f"GCP UNEXPECTED: {scenario['name']} expected to fail but succeeded. May indicate bug is fixed.")
            except Exception as e:
        ...
        if success_count > expected_successes:
            self.logger.warning(f'More successes than expected ({success_count} vs {expected_successes}). Protocol bug may be fixed or test needs adjustment.')
    ...
    async def test_concurrent_user_websocket_connections_staging(self):
        ...
        for i in range(concurrent_user_count):
            ...
            access_token = await self.auth_client.generate_test_access_token(user_id=user_data['user_id'], email=user_data['email'])
        ...
        self.logger.info(f'Concurrent staging test completed in {total_concurrent_duration:.1f}s: {len(successful_connections)} successful, {len(failed_connections)} failed, {len(error_connections)} errors')
    ...
    async def _attempt_staging_websocket_connection(self, subprotocols: List[str], timeout: int, connection_description: str) -> Optional[websockets.WebSocketClientProtocol]:
        ...
        """
        try:
            self.logger.info(f'Attempting staging connection: {connection_description}, protocols: {subprotocols}, timeout: {timeout}s')
            ssl_context = ssl.create_default_context()
            ssl_context.check_hostname = False
            ssl_context.verify_mode = ssl.CERT_NONE
            connection_start = time.time()
            connection = await asyncio.wait_for(websockets.connect(self.staging_backend_url, subprotocols=subprotocols, ssl=ssl_context if self.staging_backend_url.startswith('wss') else None, ping_interval=20, ping_timeout=10, close_timeout=10), timeout=timeout)
            connection_duration = time.time() - connection_start
            self.logger.info(f'Staging connection successful: {connection_description} in {connection_duration:.1f}s')
            return connection
        except asyncio.TimeoutError:
            connection_duration = time.time() - connection_start
            self.logger.error(f'Staging connection timeout: {connection_description} after {connection_duration:.1f}s with protocols {subprotocols}')
            return None
        except Exception as e:
            connection_duration = time.time() - connection_start
            self.logger.error(f'Staging connection error: {connection_description} after {connection_duration:.1f}s: {e}')
            return None
