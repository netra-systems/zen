...
from netra_backend.app.services.billing.billing_engine import BillingEngine, BillingPeriod, BillingStatus, Bill, BillingLineItem
...
from netra_backend.app.schemas.billing import UsageEvent, UsageEventType, Invoice, InvoiceStatus, BillingTier, UsageSummary
...
class AgentCompensationIntegrationCoreTests(SSotAsyncTestCase):
    ...
    def setup_method(self, method=None):
        ...
        self.users = {'free_user': {'user_id': f'user_free_{uuid.uuid4()}', 'tier': 'free', 'expected_limits': True}, 'starter_user': {'user_id': f'user_starter_{uuid.uuid4()}', 'tier': 'starter', 'expected_limits': False}, 'enterprise_user': {'user_id': f'user_enterprise_{uuid.uuid4()}', 'tier': 'enterprise', 'expected_limits': False}}
        self.agent_types = {'data_helper': {'base_cost_per_call': Decimal('0.005'), 'token_multiplier': Decimal('0.00001'), 'typical_tokens': 1500}, 'optimization': {'base_cost_per_call': Decimal('0.02'), 'token_multiplier': Decimal('0.00005'), 'typical_tokens': 5000}, 'reporting': {'base_cost_per_call': Decimal('0.01'), 'token_multiplier': Decimal('0.00002'), 'typical_tokens': 3000}}
    ...
    async def test_multi_user_agent_cost_tracking_and_isolation(self):
        ...
        for user_key, user_data in self.users.items():
            ...
            context = await create_isolated_execution_context(user_id=user_id, request_id=request_id, thread_id=thread_id, run_id=run_id, validate_user=False)
            for agent_type, cost_profile in self.agent_types.items():
                ...
                if tier == 'enterprise':
                    total_cost = total_cost * Decimal('0.8')
                elif tier == 'free' and user_data['expected_limits']:
                    if len(usage_events) > 2:
                        total_cost = Decimal('0')
                await self.cost_tracker.track_operation_cost(operation_id=operation_id, operation_type=f'agent_{agent_type}', model_name=f'agent_{agent_type}_model', tokens_used=tokens_used, cost=total_cost, metadata={'user_id': user_id, 'tier': tier, 'agent_type': agent_type, 'thread_id': context.thread_id, 'run_id': context.run_id, 'context_request_id': context.request_id})
                usage_event = UsageEvent(id=operation_id, user_id=user_id, event_type=UsageEventType.API_CALL, resource_name=f'agent_{agent_type}', quantity=Decimal('1'), unit='execution', cost_per_unit=total_cost, total_cost=total_cost, billing_period=f"{self.billing_period_start.strftime('%Y%m%d')}_{self.billing_period_end.strftime('%Y%m%d')}", metadata={'agent_type': agent_type, 'tokens_used': tokens_used, 'tier': tier, 'context_isolation_verified': True})
    ...
    async def test_agent_billing_generation_accuracy(self):
        ...
        for tier in ['starter', 'professional', 'enterprise']:
            ...
            llm_tokens = api_calls * 1500
            usage_data[f'{tier}_api_call'] = {'quantity': api_calls, 'metadata': {'tier': tier, 'usage_type': 'api_call'}}
            usage_data[f'{tier}_llm_tokens'] = {'quantity': llm_tokens, 'metadata': {'tier': tier, 'usage_type': 'llm_tokens'}}
            test_bill = await self.billing_engine.generate_bill(user_id=f'{test_user_id}_{tier}', period_start=self.billing_period_start, period_end=self.billing_period_end, usage_data={'api_call': usage_data[f'{tier}_api_call'], 'llm_tokens': usage_data[f'{tier}_llm_tokens']}, tier=tier)
            ...
            assert abs(test_bill.subtotal - expected_subtotal) <= Decimal('0.01'), f'Subtotal calculation error for {tier}: expected {expected_subtotal}, got {test_bill.subtotal}'
            assert abs(test_bill.tax_amount - expected_tax) <= Decimal('0.01'), f'Tax calculation error for {tier}: expected {expected_tax}, got {test_bill.tax_amount}'
            assert abs(test_bill.total_amount - expected_total) <= Decimal('0.01'), f'Total calculation error for {tier}: expected {expected_total}, got {test_bill.total_amount}'
            expected_totals[tier] = expected_total
        assert expected_totals['enterprise'] > expected_totals['professional'], 'Enterprise usage should cost more due to higher base fee'
        assert expected_totals['professional'] > expected_totals['starter'], 'Professional usage should cost more than starter'
        enterprise_bill_id = list(self.billing_engine.bills.keys())[-1]
    ...
    async def test_agent_compensation_financial_accuracy_end_to_end(self):
        ...
        """
        financial_test_users = [{'user_id': f'finance_user_1_{uuid.uuid4()}', 'tier': 'professional'}, {'user_id': f'finance_user_2_{uuid.uuid4()}', 'tier': 'enterprise'}]
        ...
        for user_data in financial_test_users:
            user_id = user_data['user_id']
            tier = user_data['tier']
            context = await create_isolated_execution_context(user_id=user_id, request_id=f'finance_test_{uuid.uuid4()}', validate_user=False)
            agent_executions = [{'agent_type': 'data_helper', 'executions': 50, 'avg_tokens': 1200}, {'agent_type': 'optimization', 'executions': 20, 'avg_tokens': 4500}, {'agent_type': 'reporting', 'executions': 10, 'avg_tokens': 2800}]
            ...
            for execution_config in agent_executions:
                ...
                for i in range(exec_count):
                    ...
                    await self.cost_tracker.track_operation_cost(operation_id=operation_id, operation_type=f'agent_{agent_type}', model_name=f'model_{agent_type}', tokens_used=avg_tokens, cost=execution_cost, metadata={'user_id': user_id, 'tier': tier, 'financial_test': True, 'context_id': context.request_id})
                    user_tracked_cost += execution_cost
                    user_usage_data['api_call']['quantity'] += 1
                    user_usage_data['llm_tokens']['quantity'] += avg_tokens
            user_bill = await self.billing_engine.generate_bill(user_id=user_id, period_start=self.billing_period_start, period_end=self.billing_period_end, usage_data=user_usage_data, tier=tier)
            user_financial_records.append({'user_id': user_id, 'tier': tier, 'tracked_cost': user_tracked_cost, 'billed_amount': user_bill.total_amount, 'api_calls': user_usage_data['api_call']['quantity'], 'tokens': user_usage_data['llm_tokens']['quantity'], 'bill_id': user_bill.bill_id})
        ...
        for record in user_financial_records:
            payment_result = await self.billing_engine.process_payment(bill_id=record['bill_id'], payment_amount=record['billed_amount'], payment_method='test_payment')
    ...
    async def test_agent_cost_tracking_data_persistence_validation(self):
        ...
        context = await create_isolated_execution_context(user_id=test_user_id, request_id=f'persistence_test_{uuid.uuid4()}', validate_user=False)
        daily_costs = []
        for day_offset in range(7):
            ...
            for agent_type, cost_profile in self.agent_types.items():
                ...
                cost = cost_profile['base_cost_per_call'] + cost_profile['typical_tokens'] * cost_profile['token_multiplier']
                await self.cost_tracker.track_operation_cost(operation_id=operation_id, operation_type=f'agent_{agent_type}', model_name=f'persistence_model_{agent_type}', tokens_used=cost_profile['typical_tokens'], cost=cost, metadata={'user_id': test_user_id, 'date': test_date.isoformat(), 'persistence_test': True})
                daily_costs.append({'date': test_date, 'agent_type': agent_type, 'cost': cost, 'operation_id': operation_id})
        for day_offset in range(7):
            ...
            expected_daily_cost = sum((d['cost'] for d in daily_costs if d['date'].date() == query_date.date()))
