...
class RealChatOutputTester:
    ...
    TEST_PROMPTS = [{'message': 'What is the weather like today in New York?', 'expected_behavior': 'Should use weather tool or explain inability', 'expected_keywords': ['weather', 'New York', 'temperature', 'forecast'], 'requires_tool': True}, {'message': 'Calculate the factorial of 10', 'expected_behavior': 'Should calculate factorial with clear steps', 'expected_keywords': ['factorial', '10', '3628800', 'multiply'], 'requires_thinking': True}, {'message': 'Write a Python function to reverse a string', 'expected_behavior': 'Should provide working Python code', 'expected_keywords': ['def', 'reverse', 'return', 'Python', 'string'], 'requires_code': True}, {'message': 'Explain quantum computing in simple terms', 'expected_behavior': 'Should provide clear explanation', 'expected_keywords': ['quantum', 'qubit', 'superposition', 'classical'], 'requires_explanation': True}]
    ...
    async def send_chat_and_validate(self, message: str, expected_keywords: List[str]=None, timeout: float=30.0) -> ChatOutputValidation:
        ...
        validation = ChatOutputValidation(thread_id=thread_id, message_sent=message)
        await self.ws_client.send_chat(text=message, thread_id=thread_id, optimistic_id=str(uuid.uuid4()))
    ...
    async def _process_event(self, event: Dict[str, Any], validation: ChatOutputValidation):
        ...
        if event_type == 'agent_started':
            ...
            tool_info = {'name': event.get('data', {}).get('tool_name', 'unknown'), 'args': event.get('data', {}).get('args', {}), 'timestamp': event_time}
            ...
            if validation.tools_executed:
                ...
                validation.tools_executed[-1]['duration'] = event_time - validation.tools_executed[-1]['timestamp']
    ...
    def _validate_output(self, validation: ChatOutputValidation, expected_keywords: List[str]=None):
        ...
        if validation.tools_executed:
            validation.has_logical_tool_usage = any((tool.get('result') is not None for tool in validation.tools_executed))
    ...
    def generate_validation_report(self) -> str:
        ...
        for i, val in enumerate(self.validations, 1):
            ...
            if val.thinking_steps:
                ...
                for step in val.thinking_steps[:3]:
...
class TestRealChatOutput:
    ...
    async def test_simple_question_full_flow(self, chat_tester):
        ...
        validation = await chat_tester.send_chat_and_validate(message='What is 2 + 2?', expected_keywords=['4', 'four', 'sum', 'equals'], timeout=20.0)
        ...
        assert '4' in validation.final_response or 'four' in validation.final_response.lower(), 'Response should contain the answer'
        assert len(validation.event_types_seen) >= 2, 'Should have multiple event types'
        assert 'agent_started' in validation.event_types_seen or 'agent_thinking' in validation.event_types_seen, 'Should have agent events'

    async def test_complex_reasoning_with_thinking(self, chat_tester):
        """Test complex reasoning shows thinking steps."""
        validation = await chat_tester.send_chat_and_validate(message='Explain step by step how to calculate the area of a circle with radius 5', expected_keywords=['area', 'circle', 'radius', 'pi', '25', '78.5'], timeout=30.0)
        assert len(validation.thinking_steps) > 0 or validation.has_coherent_response, 'Should have thinking steps or direct response'
        assert validation.final_response, 'Should have final response'
        ...
        assert 'pi' in response_lower or '3.14' in response_lower, 'Should mention pi in calculation'

    async def test_code_generation_request(self, chat_tester):
        """Test code generation produces valid code output."""
        validation = await chat_tester.send_chat_and_validate(message='Write a Python function to check if a number is prime', expected_keywords=['def', 'prime', 'return', 'True', 'False'], timeout=30.0)
    ...
    async def test_tool_usage_scenario(self, chat_tester):
        ...
        validation = await chat_tester.send_chat_and_validate(message="What's the current date and time?", expected_keywords=['date', 'time', 'current', 'now', 'today'], timeout=25.0)
        ...
        if validation.tools_executed:
            ...
            assert 'tool_executing' in validation.event_types_seen, 'Should have tool executing event if tools used'

    async def test_multi_turn_context(self, chat_tester):
        """Test multi-turn conversation maintains context."""
        thread_id = str(uuid.uuid4())
        validation1 = await chat_tester.send_chat_and_validate(message='My name is Alice', expected_keywords=['Alice', 'nice', 'meet', 'hello'], timeout=20.0)
        await asyncio.sleep(1.0)
        await chat_tester.ws_client.send_chat(text="What's my name?", thread_id=thread_id, optimistic_id=str(uuid.uuid4()))
        validation2 = ChatOutputValidation(thread_id=thread_id, message_sent="What's my name?")
        start_time = time.time()
    ...
    async def test_error_handling_graceful(self, chat_tester):
        ...
        validation = await chat_tester.send_chat_and_validate(message='Parse this invalid JSON: {not valid json}', expected_keywords=['JSON', 'invalid', 'error', 'parse', 'format'], timeout=20.0)
        assert validation.final_response or len(validation.events_received) > 0, 'Should handle error gracefully with response or events'
    ...
    async def test_event_ordering_validation(self, chat_tester):
        ...
        validation = await chat_tester.send_chat_and_validate(message='Calculate 10 factorial', expected_keywords=['factorial', '10', '3628800'], timeout=25.0)
        ...
        if 'tool_executing' in event_sequence:
            assert 'tool_completed' in event_sequence or 'tool_error' in event_sequence, 'Tool execution should have completion or error'

    async def test_performance_metrics(self, chat_tester):
        """Test and validate performance metrics."""
        validations = []
        for i in range(3):
            validation = await chat_tester.send_chat_and_validate(message=f'What is {i + 1} + {i + 2}?', expected_keywords=[str(2 * i + 3)], timeout=15.0)
    ...
    async def test_comprehensive_validation_report(self, chat_tester):
        ...
        for prompt_config in chat_tester.TEST_PROMPTS[:2]:
            validation = await chat_tester.send_chat_and_validate(message=prompt_config['message'], expected_keywords=prompt_config['expected_keywords'], timeout=30.0)
