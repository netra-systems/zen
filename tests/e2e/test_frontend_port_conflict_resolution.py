...
class TestFrontendPortConflictResolution:
    ...
    async def test_port_3000_conflict_detection_and_fallback(self, mock_config, mock_services_config, 
        ...
        try:
            ...
            assert allocated_port != 3000, f"Should have allocated a different port, got {allocated_port}"
            assert 3001 <= allocated_port <= 3010, f"Port {allocated_port} should be in fallback range 3001-3010"
            assert is_port_available(allocated_port), f"Allocated port {allocated_port} should be available"
            
            print(f"SUCCESS: Frontend properly fallback from port 3000 to port {allocated_port}")
        ...
        try:
            ...
            with patch('dev_launcher.config.resolve_path') as mock_resolve:
                ...
                with patch('dev_launcher.frontend_starter.create_subprocess') as mock_subprocess:
                    ...
                    assert allocated_port != 3000, f"Should not use port 3000, got {allocated_port}"
                    assert 3001 <= allocated_port <= 3010, f"Port {allocated_port} should be in fallback range"
                    
                    print(f"SUCCESS: Frontend started successfully on port {allocated_port}")
    ...
    def test_port_manager_allocation_with_conflict(self):
        ...
        try:
            ...
            assert allocated_port != 3000, f"Should allocate alternative port, got {allocated_port}"
            assert 3001 <= allocated_port <= 3010, f"Port {allocated_port} should be in valid range"
            
            # Step 4: Verify port is tracked by manager
            tracked_port = port_manager.get_allocated_port("frontend")
            assert tracked_port == allocated_port, f"Manager should track port {allocated_port}"
            ...
            assert len(conflicts) == 0, f"Should not have conflicts, got: {conflicts}"
            
            print(f"SUCCESS: PortManager allocated alternative port {allocated_port}")
    ...
    def test_find_available_port_fallback_mechanism(self):
        ...
        try:
            ...
            assert available_port not in occupied_ports, f"Should not use occupied ports {occupied_ports}"
            assert 3003 <= available_port <= 3010, f"Port {available_port} should be in valid range"
            assert is_port_available(available_port, '0.0.0.0'), f"Port {available_port} should be available"
            
            print(f"SUCCESS: find_available_port found alternative port {available_port}")
    ...
    def test_concurrent_port_allocation_race_condition_prevention(self):
        ...
        def allocate_port(service_name: str, preferred_port: int):
            ...
            try:
                ...
                if port:
                    ...
                    print(f"Service {service_name} allocated port {port}")
                else:
                    errors.append(f"Failed to allocate port for {service_name}")
            except Exception as e:
                errors.append(f"Exception in {service_name}: {e}")
        ...
        assert len(errors) == 0, f"No allocation errors should occur: {errors}"
        assert len(allocated_ports) == 5, f"All 5 services should get ports, got {len(allocated_ports)}"
        
        # Step 5: Verify all ports are unique
        ports_only = [port for _, port in allocated_ports]
        assert len(set(ports_only)) == 5, f"All ports should be unique: {ports_only}"
        
        # Step 6: Verify all ports are in valid range
        for service_name, port in allocated_ports:
            assert 3000 <= port <= 3020, f"Port {port} for {service_name} should be in range"
        
        print(f"SUCCESS: Concurrent allocation successful: {allocated_ports}")
    ...
    def test_windows_specific_port_race_condition_fix(self):
        ...
        for result in results:
            assert result['allocated_port'] is not None, f"Allocation should succeed in iteration {result['iteration']}"
            assert result['duration'] < 1.0, f"Allocation should be fast, took {result['duration']}s"
    ...
    def test_port_availability_check_interface_consistency(self):
        ...
        if mock_server.start():
            try:
                ...
                print(f"SUCCESS: Interface consistency maintained for port {test_port}")
...
class TestFrontendPortConflictRemediation:
    ...
    def test_enhanced_port_allocation_with_process_detection(self):
        ...
        try:
            ...
            assert process_info is not None, f"Should detect process using port 3008: {process_info}"
            
            # Step 5: Verify conflict detection
            conflicts = port_manager.get_port_conflicts()
            if allocated_port in conflicts:
                print(f"Detected conflict: {conflicts[allocated_port]}")
            
            print(f"SUCCESS: Enhanced port allocation with process detection - allocated port {allocated_port}")
    ...
    def test_port_cleanup_verification(self):
        ...
        for service in test_services:
            ...
            assert port is not None, f"Should allocate port for {service}"
            allocated_ports.add(port)
        
        # Step 2: Verify all ports are tracked
        all_allocated = port_manager.get_all_allocated_ports()
        for service in test_services:
            assert service in all_allocated, f"Service {service} should be tracked"
        
        # Step 3: Release all ports
        for service in test_services:
            released = port_manager.release_port(service)
            assert released, f"Should successfully release port for {service}"
        
        # Step 4: Verify cleanup
        ports_still_in_use = port_manager.verify_port_cleanup(allocated_ports, max_attempts=3, wait_time=0.5)
        assert len(ports_still_in_use) == 0, f"All ports should be cleaned up, still in use: {ports_still_in_use}"
        
        # Step 5: Verify tracking is cleared
        final_allocated = port_manager.get_all_allocated_ports()
        for service in test_services:
            assert service not in final_allocated, f"Service {service} should not be tracked after release"
        
        print(f"SUCCESS: Port cleanup verification passed for {len(allocated_ports)} ports")
...
class TestFrontendPortConflictIntegration:
    ...
    def test_real_frontend_port_conflict_scenario(self):
        ...
        assert mock_server.start(), f"Failed to occupy test port {test_port}"
        ...
        try:
            ...
            assert determined_port != test_port, f"Should fallback from occupied port {test_port}, got {determined_port}"
            assert determined_port > test_port, f"Fallback port {determined_port} should be higher than {test_port}"
            
            print(f"SUCCESS: Real frontend port conflict resolved - fallback from {test_port} to {determined_port}")
