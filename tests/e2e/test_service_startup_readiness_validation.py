...
class ServiceMetrics:
    ...
    def get_summary(self) -> Dict[str, Any]:
        ...
        return {'service_name': self.service_name, 'duration_seconds': self.duration, 'time_to_ready_seconds': self.time_to_ready, 'time_to_healthy_seconds': self.time_to_healthy, 'readiness_success_rate_percent': self.readiness_success_rate, 'health_success_rate_percent': self.health_success_rate, 'dependency_success_rate_percent': self.dependency_success_rate, 'average_readiness_time_ms': self.average_readiness_time * 1000, 'average_health_time_ms': self.average_health_time * 1000, 'state_transitions': len(self.state_transitions), 'total_checks': {'readiness': self.readiness_checks, 'health': self.health_checks, 'dependency': self.dependency_checks}}
...
class ServiceStartupValidator:
    ...
    async def validate_service_startup_sequence(self, service_name: str, startup_steps: List[Callable], readiness_check: Callable, health_check: Callable, dependencies: List[str]=None, startup_timeout: float=30.0) -> ServiceMetrics:
        """
        ...
        try:
            ...
            while not ready_achieved and readiness_attempts < 10:
                ...
                try:
                    readiness_result = await asyncio.wait_for(readiness_check(), timeout=readiness_timeout / 10)
            ...
            for attempt in range(health_attempts):
                check_start = time.time()
                try:
                    health_result = await asyncio.wait_for(health_check(), timeout=5.0)
                    check_time = time.time() - check_start
    ...
    async def _stress_test_service_checks(self, metrics: ServiceMetrics, readiness_check: Callable, health_check: Callable, iterations: int=10):
    ...
    async def validate_service_coordination(self, services: List[str], coordination_checks: List[Callable], coordination_timeout: float=15.0) -> Dict[str, Any]:
        """
        Validate coordination between multiple services.
        
        Tests service discovery, communication, and synchronization.
        """
        coordination_metrics = {'services': services, 'start_time': time.time(), 'coordination_checks': len(coordination_checks), 'successful_checks': 0, 'failed_checks': 0, 'timeout_checks': 0, 'coordination_times': []}
        try:
    ...
    def get_startup_report(self) -> Dict[str, Any]:
        ...
        report = {'summary': {'total_services': len(self.metrics), 'running_services': len(self.running_services), 'average_startup_time': 0.0, 'average_time_to_ready': 0.0, 'average_time_to_healthy': 0.0, 'overall_readiness_success_rate': 0.0, 'overall_health_success_rate': 0.0, 'overall_dependency_success_rate': 0.0}, 'service_details': {}, 'recommendations': []}
        if not self.metrics:
            return report
        startup_times = []
        ready_times = []
        ...
        if report['summary']['overall_readiness_success_rate'] < 90:
            report['recommendations'].append('Readiness check success rate is below 90%. Consider improving readiness probe reliability.')
        if report['summary']['overall_health_success_rate'] < 95:
            report['recommendations'].append('Health check success rate is below 95%. Review health check implementation.')
        if report['summary']['average_time_to_ready'] > 10:
            report['recommendations'].append('Average time to ready exceeds 10 seconds. Consider optimizing startup sequence.')
        if report['summary']['running_services'] < report['summary']['total_services']:
            report['recommendations'].append('Not all services successfully started. Review failed service logs.')
        return report
...
class TestServiceStartupReadinessValidation:
    ...
    async def test_auth_service_startup_validation(self, startup_validator):
        ...
        async def auth_readiness_check():
            ...
            await asyncio.sleep(0.03)
            return {'ready': True, 'status': 'ready', 'port': 8080, 'endpoints': ['/health', '/auth/login', '/auth/verify']}

        async def auth_health_check():
            """Auth service health check."""
            await asyncio.sleep(0.02)
            return {'healthy': True, 'status': 'healthy', 'uptime': 5, 'dependencies': {'postgres': 'connected', 'redis': 'connected'}}
        startup_steps = [load_configuration, initialize_database, initialize_redis]
        metrics = await startup_validator.validate_service_startup_sequence(service_name='auth_service', startup_steps=startup_steps, readiness_check=auth_readiness_check, health_check=auth_health_check, dependencies=['postgres', 'redis'], startup_timeout=20.0)
    ...
    async def test_backend_service_startup_validation(self, startup_validator):
        ...
        async def backend_readiness_check():
            ...
            return {'ready': True, 'status': 'ready', 'port': 8000, 'websocket_port': 8001, 'services': ['threads', 'agents', 'websocket']}

        async def backend_health_check():
            """Backend service health check."""
            await asyncio.sleep(0.03)
            return {'healthy': True, 'status': 'healthy', 'uptime': 8, 'llm_providers': ['openai', 'anthropic'], 'database_status': 'connected', 'websocket_connections': 0}
        startup_steps = [initialize_database, initialize_llm_clients, start_websocket_server]
        metrics = await startup_validator.validate_service_startup_sequence(service_name='backend_service', startup_steps=startup_steps, readiness_check=backend_readiness_check, health_check=backend_health_check, dependencies=['postgres', 'auth_service'], startup_timeout=30.0)
    ...
    async def test_service_coordination_validation(self, startup_validator):
        ...
        async def auth_backend_communication_check():
            ...
            await asyncio.sleep(0.1)
            return {'success': True, 'test': 'auth_backend_communication', 'auth_reachable': True, 'token_validation': 'successful'}

        async def service_discovery_check():
            """Test service discovery mechanism."""
            await asyncio.sleep(0.05)
            return {'success': True, 'test': 'service_discovery', 'discovered_services': ['auth_service', 'backend_service']}

        async def cross_service_health_check():
            """Test cross-service health validation."""
            await asyncio.sleep(0.08)
            return {'success': True, 'test': 'cross_service_health', 'all_services_healthy': True, 'service_count': 2}
        coordination_checks = [auth_backend_communication_check, service_discovery_check, cross_service_health_check]
        coordination_metrics = await startup_validator.validate_service_coordination(services=['auth_service', 'backend_service'], coordination_checks=coordination_checks, coordination_timeout=10.0)
    ...
    async def test_startup_failure_recovery(self, startup_validator):
        ...
        async def flaky_readiness_check():
            ...
            return {'ready': failure_count > 2, 'status': 'ready' if failure_count > 2 else 'initializing', 'attempt': failure_count}

        async def stable_health_check():
            """Health check that remains stable."""
            await asyncio.sleep(0.02)
            return {'healthy': True, 'status': 'healthy', 'recovery_complete': failure_count > 2}
        startup_steps = []
        for attempt in range(3):
            startup_steps.append(flaky_initialization)
            startup_steps.append(recovery_step)
        try:
            metrics = await startup_validator.validate_service_startup_sequence(service_name='recovery_test_service', startup_steps=[recovery_step], readiness_check=flaky_readiness_check, health_check=stable_health_check, startup_timeout=15.0)
