...
class TestCompleteAgentWorkflowsStaging(BaseE2ETest):
    ...
    async def test_complete_cost_optimization_workflow_authenticated(self):
        ...
        optimization_request = {'type': 'execute_agent', 'agent_type': 'cost_optimization', 'user_id': user_context.user_id, 'thread_id': user_context.thread_id, 'request_id': user_context.request_id, 'data': {'analysis_type': 'comprehensive', 'optimization_focus': ['compute_costs', 'storage_optimization', 'idle_resources'], 'target_savings': 0.25, 'priority': 'high'}, 'auth': {'user_id': user_context.user_id, 'permissions': user_context.agent_context.get('permissions', [])}}
    ...
    async def test_multi_user_concurrent_optimization_authenticated(self):
        ...
        async def run_user_optimization(user_context, user_index):
            ...
            try:
                ...
                user_request = {'type': 'execute_agent', 'agent_type': 'cost_optimization', 'user_id': user_context.user_id, 'thread_id': user_context.thread_id, 'request_id': user_context.request_id, 'data': {'analysis_type': 'targeted', 'user_priority': f'user_{user_index}', 'concurrent_execution': True}}
                await websocket.send(json.dumps(user_request))
                await asyncio.wait_for(event_task, timeout=45.0)
                await websocket.close()
                return {'user_id': user_context.user_id, 'user_index': user_index, 'events': user_events, 'results': user_results, 'success': True}
            except Exception as e:
                return {'user_id': user_context.user_id, 'user_index': user_index, 'error': str(e), 'success': False}
        tasks = [run_user_optimization(user_context, i) for i, user_context in enumerate(self.test_users)]
    ...
    async def test_agent_execution_with_tool_chains_authenticated(self):
        ...
        complex_request = {'type': 'execute_agent', 'agent_type': 'comprehensive_analysis', 'user_id': user_context.user_id, 'thread_id': user_context.thread_id, 'request_id': user_context.request_id, 'data': {'analysis_scope': 'enterprise', 'required_tools': ['cost_analyzer', 'performance_profiler', 'security_scanner', 'optimization_engine'], 'depth': 'detailed', 'generate_report': True}}
        ...
        async def collect_progress_events():
            async for message in websocket:
                ...
                if event.get('type') == 'agent_thinking':
                    thinking_events.append({**event, 'timestamp': event_time})
                elif event.get('type') in ['tool_executing', 'agent_progress']:
                    progress_events.append({**event, 'timestamp': event_time})
                elif event.get('type') == 'agent_completed':
                    break
        event_task = asyncio.create_task(collect_progress_events())
        long_request = {'type': 'execute_agent', 'agent_type': 'deep_analysis', 'user_id': user_context.user_id, 'thread_id': user_context.thread_id, 'request_id': user_context.request_id, 'data': {'analysis_depth': 'comprehensive', 'progress_updates': True, 'estimated_duration': 30, 'detailed_reasoning': True}}
    ...
    async def test_error_recovery_with_authentication(self):
        ...
        error_prone_request = {'type': 'execute_agent', 'agent_type': 'resilience_test', 'user_id': user_context.user_id, 'thread_id': user_context.thread_id, 'request_id': user_context.request_id, 'data': {'test_error_recovery': True, 'simulate_failures': ['network_timeout', 'invalid_response'], 'recovery_strategy': 'retry_with_fallback'}}
    ...
    async def test_business_value_metrics_validation(self):
        ...
        business_request = {'type': 'execute_agent', 'agent_type': 'business_impact_analyzer', 'user_id': user_context.user_id, 'thread_id': user_context.thread_id, 'request_id': user_context.request_id, 'data': {'focus': 'business_value', 'calculate_roi': True, 'quantify_savings': True, 'benchmark_performance': True}}
        await websocket.send(json.dumps(business_request))
        await asyncio.wait_for(event_task, timeout=60.0)
        required_metrics = ['estimated_cost_savings', 'roi_percentage', 'efficiency_improvement', 'time_to_value', 'risk_reduction_score']
