...
Connection: Real WebSocket connections to staging.netra.ai
...
class TestWebSocketSendAfterCloseStagingE2E(SSotAsyncTestCase):
    ...
    async def asyncSetUpClass(cls):
        ...
        cls.staging_config = {
            'websocket_url': os.getenv('STAGING_WEBSOCKET_URL', 'wss://staging.netra.ai/ws'),
            'auth_url': os.getenv('STAGING_AUTH_URL', 'https://staging.netra.ai/auth'),
            'api_url': os.getenv('STAGING_API_URL', 'https://staging.netra.ai/api'),
        }

        # Test user credentials for staging
        cls.test_users = [
            {
                'email': 'test.user.1@staging.netra.ai',
                'user_id': 'staging_test_user_1',
                'display_name': 'Staging Test User 1'
            },
            {
                'email': 'test.user.2@staging.netra.ai',
                'user_id': 'staging_test_user_2',
                'display_name': 'Staging Test User 2'
            },
            {
                'email': 'test.user.3@staging.netra.ai',
                'user_id': 'staging_test_user_3',
                'display_name': 'Staging Test User 3'
            }
        ]

        # Authentication tokens for staging tests
        cls.auth_tokens = {}
    ...
    async def test_staging_websocket_race_condition_user_disconnect_during_agent_flow(self):
        ...
        # Establish real WebSocket connections to staging
        websocket_connections = {}
        connection_errors = []
        agent_event_failures = []

        try:
            # Connect multiple users to staging WebSocket
            for user in self.test_users[:2]:  # Use 2 users for race condition testing
            ...
            # Start Golden Path agent flow simulation
            golden_path_messages = [
                {
                    "type": "start_agent_flow",
                    "agent": "supervisor",
                    "request": "Analyze business optimization opportunities",
                    "priority": "high"
                },
                {"type": "request_data", "data_type": "business_metrics"},
                {"type": "analyze_optimization", "scope": "revenue_growth"},
                {"type": "generate_recommendations", "count": 5}
            ]

            # Send Golden Path messages and simulate disconnect
            disconnect_during_message = 1  # Disconnect during 2nd message
            ...
            for msg_idx, message in enumerate(golden_path_messages):
                ...
                for user_id, conn_data in websocket_connections.items():
                    if msg_idx == disconnect_during_message and user_id == list(websocket_connections.keys())[0]:
                        # Simulate user disconnect during message sending
                        disconnect_task = asyncio.create_task(
                            self._simulate_staging_user_disconnect(user_id, conn_data)
                        )
                        send_tasks.append(disconnect_task)

                    # Send message
                    send_task = asyncio.create_task(
                        self._send_message_to_staging_websocket(
                            conn_data['websocket'],
                            {**message, "message_index": msg_idx, "user_id": user_id}
                        )
                    )
                ...
                await asyncio.sleep(0.5)  # Realistic delay between messages

            # Listen for responses and potential errors
            await self._listen_for_race_condition_errors(
                websocket_connections,
                agent_event_failures,
                timeout=5.0
            )
        ...
        # This test expects to find race conditions in staging environment
        # DESIGNED TO FAIL initially to reproduce the bug
        assert (
            len(connection_errors) > 0 or
            len(agent_event_failures) > 0
        ), (
            f"Expected race condition errors in staging environment. "
            f"Connection errors: {len(connection_errors)}, "
            f"Agent event failures: {len(agent_event_failures)}"
        )

        logger.error(f"🔥 STAGING RACE CONDITIONS REPRODUCED: "
                    f"{len(connection_errors)} connection errors, "
                    f"{len(agent_event_failures)} agent event failures")
    ...
    async def test_staging_concurrent_users_websocket_race_conditions(self):
        ...
        try:
            # Connect all test users simultaneously
            connection_tasks = []
            for user in self.test_users:
                task = asyncio.create_task(
                    self._establish_staging_connection(user)
                )
            ...
            for user_id, task in connection_tasks:
                try:
                    ...
                    if websocket:
                        concurrent_connections[user_id] = {
                            'websocket': websocket,
                            'user_id': user_id,
                            'connected_at': time.time()
                        }
            ...
            # Perform concurrent operations that can cause race conditions
            concurrent_operations = [
                {"operation": "start_chat_session", "priority": 1},
                {"operation": "request_agent_analysis", "complexity": "high"},
                {"operation": "upload_data", "size": "large"},
                {"operation": "start_optimization", "scope": "full"},
                {"operation": "generate_report", "format": "detailed"}
            ]

            # Execute all operations concurrently across all users
            operation_tasks = []
            for op_idx, operation in enumerate(concurrent_operations):
                for user_id, conn_data in concurrent_connections.items():
                    task = asyncio.create_task(
                        self._execute_staging_operation(
                            conn_data['websocket'],
                            user_id,
                            {**operation, "operation_id": f"{user_id}_op_{op_idx}"}
                        )
                    )
            ...
            # Monitor for race condition errors during concurrent operations
            await self._monitor_staging_race_conditions(
                concurrent_connections,
                concurrent_errors,
        ...
        # Analyze concurrent race condition results
        total_expected_operations = len(concurrent_operations) * len(concurrent_connections)

        # This test expects race conditions during concurrent operations
        # DESIGNED TO FAIL initially to reproduce the bug
        assert len(concurrent_errors) > 0, (
            f"Expected concurrent race condition errors in staging. "
            f"Successful operations: {successful_operations}/{total_expected_operations}, "
            f"Concurrent errors: {len(concurrent_errors)}"
        )

        logger.error(f"🔥 CONCURRENT STAGING RACE CONDITIONS REPRODUCED: "
                    f"{len(concurrent_errors)} errors from {successful_operations} operations")
    ...
    async def test_staging_network_instability_websocket_race_conditions(self):
        ...
        try:
            # Establish stable connection first
            ...
            async def continuous_operations():
                ...
                while operation_active:
                    try:
                        message = {
                            "type": "continuous_operation",
                            "operation_id": operation_count,
                            "timestamp": time.time(),
                            "data": {"continuous": True, "operation_count": operation_count}
                        }
            ...
            async def simulate_network_instability():
                ...
                await asyncio.sleep(1.0)  # Let operations stabilize first

                # Simulate various network issues
                instability_scenarios = [
                    ("connection_interrupt", 2.0),
                    ("slow_network", 1.5),
                    ("packet_loss", 1.0),
                ...
                for scenario_name, duration in instability_scenarios:
                    ...
                    if scenario_name == "connection_interrupt":
                        # Close connection during active operations
                        try:
                        ...
                        # (This is simulation - in real network issues, staging would handle this)
            ...
            # Run continuous operations and network instability simulation
        ...
        # Analyze network instability race condition results
        # This test expects network instability to cause race conditions
        # DESIGNED TO FAIL initially to reproduce the bug
        assert (
            len(instability_errors) > 0 or
            len(recovery_attempts) > 0 or
            len(operation_errors) > 0
        ), (
            f"Expected network instability race conditions in staging. "
            f"Instability errors: {len(instability_errors)}, "
            f"Recovery attempts: {len(recovery_attempts)}, "
            f"Operation errors: {len(operation_errors)}"
        )

        logger.error(f"🔥 NETWORK INSTABILITY RACE CONDITIONS REPRODUCED: "
                    f"{len(instability_errors)} instability errors, "
                    f"{len(recovery_attempts)} recovery attempts, "
                    f"{len(operation_errors)} operation errors")
        ...
        try:
            ...
            async with aiohttp.ClientSession() as session:
                async with session.get(
                    f"{self.staging_config['api_url']}/health",
    ...
    async def _authenticate_test_users(self):
        ...
        # For staging E2E tests, we'll use mock JWT tokens
        # In real implementation, these would come from staging auth service
        ...
        for user in self.test_users:
            ...
            test_token_payload = {
                ...
                'iat': int(time.time()),
                'aud': 'staging.netra.ai'
            }

            # Use test secret for staging (in real staging, this would be proper auth flow)
    ...
    async def _establish_staging_connection(self, user) -> Optional[websockets.WebSocketServerProtocol]:
        ...
        try:
            ...
            websocket = await websockets.connect(
                ...
                ping_interval=20,
                ping_timeout=10,
                extra_headers={'User-Agent': f'NetraApex-E2E-Test/{user_id}'}
            )
    ...
    async def _simulate_staging_user_disconnect(self, user_id, conn_data):
        ...
        try:
            await asyncio.sleep(0.2)  # Small delay before disconnect
        ...
        while (time.time() - start_time) < monitoring_duration:
            for user_id, conn_data in connections.items():
                try:
                    ...
                    # Send heartbeat to detect connection issues
                    heartbeat = {
                        "type": "heartbeat",
                        "user_id": user_id,
                        "timestamp": time.time()
