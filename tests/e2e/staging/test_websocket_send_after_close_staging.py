...
class TestWebSocketSendAfterCloseStagingE2E(SSotAsyncTestCase):
    ...
    async def asyncSetUpClass(cls):
        ...
        cls.staging_config = {'websocket_url': os.getenv('STAGING_WEBSOCKET_URL', 'wss://staging.netra.ai/ws'), 'auth_url': os.getenv('STAGING_AUTH_URL', 'https://staging.netra.ai/auth'), 'api_url': os.getenv('STAGING_API_URL', 'https://staging.netra.ai/api')}
        cls.test_users = [{'email': 'test.user.1@staging.netra.ai', 'user_id': 'staging_test_user_1', 'display_name': 'Staging Test User 1'}, {'email': 'test.user.2@staging.netra.ai', 'user_id': 'staging_test_user_2', 'display_name': 'Staging Test User 2'}, {'email': 'test.user.3@staging.netra.ai', 'user_id': 'staging_test_user_3', 'display_name': 'Staging Test User 3'}]
        cls.auth_tokens = {}
    ...
    async def _authenticate_test_users(self):
        ...
        for user in self.test_users:
            test_token_payload = {'sub': user['user_id'], 'email': user['email'], 'exp': int(time.time()) + 3600, 'iat': int(time.time()), 'aud': 'staging.netra.ai'}
