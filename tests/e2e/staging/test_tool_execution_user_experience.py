...
class ToolExecutionUserExperienceE2ETest:
    ...
    async def _authenticate_with_staging(self) -> str:
        ...
        try:
            auth_data = {'email': self.test_user_email, 'password': self.test_user_password}
        ...
        async def event_listener():
            try:
                async for message in websocket:
                    ...
                    received_events.append({'event': event_data, 'received_at': datetime.now(timezone.utc), 'timestamp': time.time()})
                    event_timeline.append(event_data.get('type', 'unknown'))
            except Exception as e:
                logger.warning(f'WebSocket listener error: {e}')
        listener_task = asyncio.create_task(event_listener())
        try:
            tool_request = {'tool_name': 'data_helper', 'parameters': {'query': 'Show me recent user activity', 'user_experience_test': True}, 'expect_events': True}
            execution_start = datetime.now(timezone.utc)
            mock_response = {'success': True, 'result': 'Tool executed successfully', 'tool_name': 'data_helper'}
    ...
    async def test_concurrent_user_tool_execution_event_isolation_staging(self):
        ...
        async def user_event_listener(user_index, websocket):
            ...
            try:
                async for message in websocket:
                    event_data = json.loads(message)
                    user_events[user_id].append({'event': event_data, 'user_index': user_index, 'received_at': datetime.now(timezone.utc)})
            except Exception as e:
        ...
        try:
            ...
            for i, (session, token) in enumerate(user_sessions[:3]):
                tool_request = {'tool_name': 'data_helper', 'parameters': {'query': f'User {i} specific query', 'user_isolation_test': True, 'user_index': i}}
                headers = {'Authorization': f'Bearer {token}'}
                task = session.post(f'{self.backend_url}/api/tools/execute', json=tool_request, headers=headers, timeout=30)
                ...
                for event_data in events:
                    ...
                    if 'user_index' in event.get('data', {}):
                        ...
                        with self.assertRaises(AssertionError, msg='Event cross-contamination detected'):
                            self.assertEqual(actual_user_index, expected_user_index, f'User {user_id} received event for user {actual_user_index}')
        finally:
    ...
    async def test_long_running_tool_execution_event_reliability_staging(self):
        ...
        try:
            long_tool_request = {'tool_name': 'data_analysis', 'parameters': {'operation': 'comprehensive_analysis', 'dataset_size': 'large', 'expected_duration_seconds': 15, 'reliability_test': True}}
            headers = {'Authorization': f'Bearer {self.auth_token}'}
            start_time = time.time()
            async with self.session.post(f'{self.backend_url}/api/tools/execute', json=long_tool_request, headers=headers, timeout=60) as response:
                while time.time() - start_time < 20:
                    await asyncio.sleep(1)
                    if not connection_health['connected']:
                        break
                self.assertTrue(connection_health['connected'], 'Should maintain WebSocket connection during long operations')
                if response.status == 200:
                    result = await response.json()
                    progress_events = [e for e in received_events if e['event'].get('type') == 'tool_progress']
                    with self.assertRaises(AssertionError, msg='No progress events for long operations'):
                        self.assertGreater(len(progress_events), 0, 'Should receive progress events for long operations')
                    with self.assertRaises(KeyError, msg='No reliability features'):
                        reliability_info = result['reliability_info']
                        self.assertIsNotNone(reliability_info['heartbeat_interval'], 'Should implement heartbeat for long operations')
                        self.assertIsNotNone(reliability_info['progress_checkpoints'], 'Should provide progress checkpoints')
        finally:
            listener_task.cancel()
            await websocket.close()
    ...
    async def test_staging_websocket_silent_failure_detection(self):
        ...
        if not websocket:
            pytest.skip('Cannot connect to staging WebSocket')
        health_metrics = {'events_expected': 0, 'events_received': 0, 'silent_failures': 0, 'connection_checks': 0}
        received_events = []
        ...
        try:
            tools_to_execute = [{'tool_name': 'data_helper', 'parameters': {'query': 'test 1'}}, {'tool_name': 'data_helper', 'parameters': {'query': 'test 2'}}, {'tool_name': 'data_helper', 'parameters': {'query': 'test 3'}}]
            headers = {'Authorization': f'Bearer {self.auth_token}'}
            for i, tool_request in enumerate(tools_to_execute):
                health_metrics['events_expected'] += 2
                async with self.session.post(f'{self.backend_url}/api/tools/execute', json=tool_request, headers=headers, timeout=15) as response:
                    await asyncio.sleep(1)
                    ...
                    if response.status == 200:
                        ...
                        with self.assertRaises(KeyError, msg='No failure detection metadata'):
                            ...
                            self.assertIsNotNone(failure_detection['health_check_passed'], 'Should perform health checks')
                            self.assertIsNotNone(failure_detection['event_delivery_confirmed'], 'Should confirm event delivery')
            await asyncio.sleep(2)
            event_delivery_rate = health_metrics['events_received'] / max(health_metrics['events_expected'], 1)
            self.assertGreater(event_delivery_rate, 0.8, f'Event delivery rate too low: {event_delivery_rate:.2%}')
            with self.assertRaises(AssertionError, msg='No failure detection system'):
                self.assertEqual(health_metrics['silent_failures'], 0, 'Should detect and handle silent failures')
        finally:
            listener_task.cancel()
            await websocket.close()
    ...
    async def test_staging_tool_execution_user_feedback_quality(self):
        ...
        if not websocket:
            pytest.skip('Cannot connect to staging WebSocket')
        user_experience_metrics = {'feedback_quality_score': 0, 'real_time_updates': 0, 'context_information': 0, 'progress_indicators': 0, 'error_clarity': 0}
        received_events = []
        ...
        try:
            complex_tool_request = {'tool_name': 'complex_data_analysis', 'parameters': {'analysis_type': 'comprehensive', 'require_user_feedback': True, 'ux_test': True}}
            headers = {'Authorization': f'Bearer {self.auth_token}'}
            async with self.session.post(f'{self.backend_url}/api/tools/execute', json=complex_tool_request, headers=headers, timeout=30) as response:
                await asyncio.sleep(3)
                if response.status == 200:
                    result = await response.json()
                    self.assertGreater(user_experience_metrics['real_time_updates'], 0, 'Should provide real-time updates to user')
                    with self.assertRaises(KeyError, msg='No user experience metadata'):
                        ux_quality = result['user_experience_quality']
                        self.assertGreaterEqual(ux_quality['feedback_score'], 8.0, 'Should achieve high user feedback quality score')
                        self.assertTrue(ux_quality['real_time_communication'], 'Should provide real-time communication')
                    with self.assertRaises(KeyError, msg='No contextual feedback'):
                        contextual_info = result['contextual_feedback']
                        self.assertIsNotNone(contextual_info['what_is_happening'], 'Should explain what is happening')
                        self.assertIsNotNone(contextual_info['why_it_matters'], 'Should explain why it matters to user')
                        self.assertIsNotNone(contextual_info['estimated_completion'], 'Should provide time estimates')
                else:
                    ...
                    with self.assertRaises(KeyError, msg='No user-friendly error handling'):
                        ...
                        self.assertIsNotNone(user_friendly_error['what_went_wrong'], 'Should explain what went wrong clearly')
                        self.assertIsNotNone(user_friendly_error['what_user_can_do'], 'Should suggest what user can do next')
        finally:
            listener_task.cancel()
            await websocket.close()
