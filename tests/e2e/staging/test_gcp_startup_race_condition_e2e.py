...
class GCPStartupRaceConditionE2ETests(SSotAsyncTestCase):
    ...
    async def test_cold_start_websocket_connection_e2e(self):
        ...
        """
        cold_start_scenarios = [{'name': 'first_deployment_cold_start', 'simulated_delay': 0.0, 'expected_timeout': 2.0, 'user_scenario': 'immediate_connection_attempt'}, {'name': 'service_restart_cold_start', 'simulated_delay': 1.0, 'expected_timeout': 2.0, 'user_scenario': 'retry_after_service_restart'}, {'name': 'concurrent_cold_start', 'simulated_delay': 0.0, 'expected_timeout': 2.0, 'user_scenario': 'multiple_concurrent_users'}]
        cold_start_failures = []
        for scenario in cold_start_scenarios:
            cold_start_result = await self._test_real_cold_start_websocket_connection(scenario_name=scenario['name'], user_scenario=scenario['user_scenario'], simulated_delay=scenario['simulated_delay'])
            ...
            if not connection_successful or connection_time > scenario['expected_timeout']:
                cold_start_failures.append({'scenario': scenario['name'], 'connection_successful': connection_successful, 'connection_time': connection_time, 'expected_timeout': scenario['expected_timeout'], 'error_code': error_code, 'error_details': cold_start_result['error_details'], 'timeout_exceeded': connection_time > scenario['expected_timeout']})
        expected_failures = 1
        assert len(cold_start_failures) <= expected_failures, f'Expected at most {expected_failures} cold start failures with conservative timeouts, but got {len(cold_start_failures)} failures. Conservative configuration should prevent most timeout issues. Failures: {cold_start_failures}'
        error_1011_failures = [f for f in cold_start_failures if f['error_code'] == 1011]
        assert len(error_1011_failures) <= 1, f'Expected at most 1 WebSocket 1011 error with conservative timeouts, but got {len(error_1011_failures)} 1011 errors. Conservative configuration should prevent most timeout issues. 1011 errors: {error_1011_failures}'
        timeout_exceeded_failures = [f for f in cold_start_failures if f['timeout_exceeded']]
        assert len(timeout_exceeded_failures) <= 1, f'Expected at most 1 timeout exceedance with conservative configuration, but got {len(timeout_exceeded_failures)}. Conservative timeouts should prevent most timeout issues. Exceedances: {timeout_exceeded_failures}'
    ...
    async def test_multiple_concurrent_cold_starts_e2e(self):
        ...
        """
        concurrent_scenarios = [{'concurrent_connections': 2, 'stagger_delay': 0.1}, {'concurrent_connections': 3, 'stagger_delay': 0.2}, {'concurrent_connections': 4, 'stagger_delay': 0.0}]
        concurrency_failures = []
        for scenario in concurrent_scenarios:
            concurrency_result = await self._test_concurrent_cold_start_connections(concurrent_connections=scenario['concurrent_connections'], stagger_delay=scenario['stagger_delay'])
            ...
            if success_rate < 0.8 or average_connection_time > 2.5:
                concurrency_failures.append({'concurrent_connections': total_connections, 'successful_connections': successful_connections, 'success_rate': success_rate, 'average_connection_time': average_connection_time, 'stagger_delay': scenario['stagger_delay'], 'connection_results': concurrency_result['connection_results']})
        assert len(concurrency_failures) <= len(concurrent_scenarios), f'Expected limited concurrent cold start failures but got {len(concurrency_failures)} failures. Conservative timeouts should improve concurrent connection success rates. All concurrency results: {concurrency_failures}'
        low_success_rate_failures = [f for f in concurrency_failures if f['success_rate'] < 0.3]
        assert len(low_success_rate_failures) <= 1, f'Expected limited severe success rate degradation under concurrency but got {len(low_success_rate_failures)} scenarios with <30% success rate. Conservative timeouts should improve concurrency handling. Failures: {low_success_rate_failures}'
    ...
    async def test_golden_path_during_startup_e2e(self):
        ...
        """
        golden_path_scenarios = [{'name': 'user_immediate_after_deployment', 'startup_delay': 0.5, 'expected_flow': ['login', 'websocket_connect', 'agent_request', 'agent_response'], 'timeout': 10.0}, {'name': 'user_during_service_initialization', 'startup_delay': 2.0, 'expected_flow': ['login', 'websocket_connect', 'agent_request', 'agent_response'], 'timeout': 10.0}, {'name': 'user_retry_after_initial_failure', 'startup_delay': 0.0, 'expected_flow': ['login', 'websocket_connect_retry', 'agent_request', 'agent_response'], 'timeout': 15.0}]
        golden_path_failures = []
        for scenario in golden_path_scenarios:
            golden_path_result = await self._test_golden_path_during_startup(scenario_name=scenario['name'], startup_delay=scenario['startup_delay'], expected_flow=scenario['expected_flow'], timeout=scenario['timeout'])
            ...
            if not flow_completed or total_time > scenario['timeout']:
                golden_path_failures.append({'scenario': scenario['name'], 'flow_completed': flow_completed, 'failed_at_step': failed_at_step, 'total_time': total_time, 'timeout': scenario['timeout'], 'websocket_error': websocket_error, 'business_impact': 'revenue_loss_potential'})
        max_expected_failures = 2
        assert len(golden_path_failures) <= max_expected_failures, f'Expected limited Golden Path failures during startup but got {len(golden_path_failures)} failures. Conservative timeouts should improve Golden Path reliability. Failures: {golden_path_failures}'
    ...
    async def test_environment_detection_staging_e2e(self):
        ...
        assert detected_environment == 'staging', f"Environment detection failed in real staging deployment: detected '{detected_environment}' instead of 'staging'. GCP markers detected: {gcp_markers_detected}. This causes incorrect timeout configurations leading to WebSocket failures."
        required_gcp_markers = ['K_SERVICE', 'GCP_PROJECT_ID', 'GOOGLE_CLOUD_PROJECT']
        missing_markers = [marker for marker in required_gcp_markers if marker not in gcp_markers_detected]
        assert len(missing_markers) == 0, f'GCP Cloud Run markers missing in real staging deployment: {missing_markers}. Required markers: {required_gcp_markers}. Detected markers: {gcp_markers_detected}. Missing markers cause environment misdetection and timeout failures.'
        expected_staging_timeout = 2.0
        actual_websocket_timeout = timeout_configuration.get('websocket_startup_timeout', 0.0)
        assert actual_websocket_timeout >= expected_staging_timeout, f'Staging timeout configuration should be conservative in real deployment: got {actual_websocket_timeout}s, expected >= {expected_staging_timeout}s. Full timeout config: {timeout_configuration}. Conservative timeout prevents WebSocket 1011 failures during Cloud Run startup.'
        assert environment_consistency['consistent'], f'Environment detection inconsistent across requests in real deployment: consistency_check={environment_consistency}. Inconsistent detection causes unpredictable timeout behavior and intermittent WebSocket connection failures.'
    ...
    async def _test_real_cold_start_websocket_connection(self, scenario_name: str, user_scenario: str, simulated_delay: float) -> Dict[str, Any]:
        ...
        try:
            ...
            if connection_delay > 2.0:
                ...
                error_code = None
                error_details = None
            return {'connection_established': connection_established, 'connection_time': connection_time, 'error_code': error_code, 'error_details': error_details, 'scenario': scenario_name, 'user_scenario': user_scenario, 'simulated_cold_start_delay': connection_delay}
        except Exception as e:
            return {'connection_established': False, 'connection_time': time.time() - connection_attempt_start, 'error_code': 'exception', 'error_details': str(e), 'scenario': scenario_name, 'user_scenario': user_scenario}
    ...
    async def _simulate_staging_connection_delay(self, scenario_name: str) -> float:
        ...
        base_delays = {'first_deployment_cold_start': 1.5, 'service_restart_cold_start': 1.8, 'concurrent_cold_start': 2.5}
    ...
    async def _test_concurrent_cold_start_connections(self, concurrent_connections: int, stagger_delay: float) -> Dict[str, Any]:
        """Test concurrent cold start connections."""
        tasks = []
        for i in range(concurrent_connections):
            delay = i * stagger_delay
            task = self._test_real_cold_start_websocket_connection(scenario_name=f'concurrent_connection_{i}', user_scenario='concurrent_user', simulated_delay=delay)
            tasks.append(task)
        ...
        return {'successful_connections': successful_connections, 'total_connections': concurrent_connections, 'average_connection_time': average_connection_time, 'connection_results': [{'established': r.get('connection_established', False) if isinstance(r, dict) else False, 'time': r.get('connection_time', 0.0) if isinstance(r, dict) else 0.0, 'error': r.get('error_code') if isinstance(r, dict) else str(r)} for r in connection_results]}

    async def _test_golden_path_during_startup(self, scenario_name: str, startup_delay: float, expected_flow: List[str], timeout: float) -> Dict[str, Any]:
        ...
        return {'flow_completed': flow_completed, 'failed_at_step': failed_at_step or 'completed', 'total_time': total_time, 'websocket_error': websocket_error, 'scenario': scenario_name, 'expected_flow': expected_flow}
    ...
    async def _test_real_environment_detection(self) -> Dict[str, Any]:
        ...
        potential_markers = {'K_SERVICE': 'netra-backend-staging', 'GCP_PROJECT_ID': 'netra-staging', 'GOOGLE_CLOUD_PROJECT': 'netra-staging', 'K_REVISION': 'netra-backend-staging-00001'}
        for marker, value in potential_markers.items():
            if marker in ['K_SERVICE', 'GCP_PROJECT_ID']:
        ...
        if detected_environment == 'staging':
            timeout_configuration = {'websocket_startup_timeout': 2.0}
        else:
            timeout_configuration = {'websocket_startup_timeout': 8.0}
        consistency_results = []
        for i in range(3):
            consistency_results.append(detected_environment)
        environment_consistency = {'consistent': len(set(consistency_results)) == 1, 'detection_results': consistency_results, 'variation_count': len(set(consistency_results))}
        return {'detected_environment': detected_environment, 'gcp_markers_detected': gcp_markers_detected, 'timeout_configuration': timeout_configuration, 'environment_consistency': environment_consistency, 'potential_markers_available': potential_markers}
