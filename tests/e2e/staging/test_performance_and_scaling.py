...
class PerformanceMetrics:
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        return {'total_requests': len(self.response_times), 'success_count': self.success_count, 'error_count': self.error_count, 'success_rate': self.success_count / (self.success_count + self.error_count) if self.success_count + self.error_count > 0 else 0, 'avg_response_time': statistics.mean(self.response_times), 'median_response_time': statistics.median(self.response_times), 'min_response_time': min(self.response_times), 'max_response_time': max(self.response_times), 'p95_response_time': self._percentile(self.response_times, 0.95), 'p99_response_time': self._percentile(self.response_times, 0.99), 'total_duration': duration, 'requests_per_second': len(self.response_times) / duration if duration > 0 else 0}
...
class TestPerformanceAndScaling(BaseE2ETest):
    ...
    async def test_concurrent_websocket_connection_scaling(self):
        ...
        async def establish_concurrent_connection(user_index: int, user_context):
            ...
            try:
                ...
                for msg_index in range(5):
                    test_message = {'type': 'performance_test', 'user_index': user_index, 'message_index': msg_index, 'timestamp': time.time()}
                    msg_start = time.time()
                ...
                return {'user_index': user_index, 'success': True, 'connection_time': connection_time, 'messages_sent': messages_sent, 'messages_received': messages_received, 'message_success_rate': messages_received / messages_sent if messages_sent > 0 else 0}
            except Exception as e:
                connection_time = time.time() - connection_start
                connection_metrics.add_response_time(connection_time, False)
                return {'user_index': user_index, 'success': False, 'connection_time': connection_time, 'error': str(e)}
        connection_tasks = [establish_concurrent_connection(i, user_context) for i, user_context in enumerate(self.test_users)]
        ...
        expected_memory_per_connection = 5
    ...
    async def test_concurrent_agent_execution_performance(self):
        ...
        async def execute_concurrent_agent(user_index: int, user_context):
            ...
            try:
                ...
                async def collect_agent_events():
                    ...
                    try:
                        async for message in websocket:
                            ...
                            agent_events.append({'type': event.get('type'), 'timestamp': time.time(), 'relative_time': time.time() - execution_start})
                ...
                agent_request = {'type': 'execute_agent', 'agent_type': 'performance_test_agent', 'user_id': user_context.user_id, 'thread_id': user_context.thread_id, 'request_id': user_context.request_id, 'data': {'performance_test': True, 'user_index': user_index, 'concurrent_execution': True, 'complexity': 'medium'}}
                ...
                try:
                    ...
                    agent_completed = False
                await websocket.close()
                return {'user_index': user_index, 'success': agent_completed, 'execution_time': execution_time, 'events_received': len(agent_events), 'agent_events': agent_events}
            except Exception as e:
                execution_time = time.time() - execution_start
                agent_metrics.add_response_time(execution_time, False)
                return {'user_index': user_index, 'success': False, 'execution_time': execution_time, 'error': str(e)}
        start_memory = self.process.memory_info()
        start_cpu_percent = self.process.cpu_percent()
        concurrent_agents = self.test_users[:8]
        agent_tasks = [execute_concurrent_agent(i, user_context) for i, user_context in enumerate(concurrent_agents)]
    ...
    async def test_api_endpoint_performance_under_load(self):
        ...
        async def make_concurrent_api_requests(user_index: int, user_context):
            ...
            auth_token = auth_helper.create_test_jwt_token(user_id=user_context.user_id, email=user_context.agent_context['user_email'])
            headers = auth_helper.get_auth_headers(auth_token)
            request_results = []
            api_endpoints = [('health', f'{self.staging_config.urls.backend_url}/health', 'GET'), ('auth_health', f'{self.staging_config.urls.auth_url}/auth/health', 'GET')]
                        ...
                        try:
                            if method == 'GET':
                                async with session.get(endpoint_url) as resp:
                                    ...
                                    request_results.append({'endpoint': endpoint_name, 'url': endpoint_url, 'method': method, 'status': resp.status, 'response_time': request_time, 'success': success, 'request_index': req_index})
                        except Exception as e:
                            request_time = time.time() - request_start
                            api_metrics.add_response_time(request_time, False)
                            request_results.append({'endpoint': endpoint_name, 'url': endpoint_url, 'method': method, 'error': str(e), 'response_time': request_time, 'success': False, 'request_index': req_index})
                        await asyncio.sleep(0.1)
            return {'user_index': user_index, 'requests': request_results, 'total_requests': len(request_results), 'successful_requests': sum((1 for r in request_results if r.get('success')))}
        api_load_users = self.test_users[:10]
        api_tasks = [make_concurrent_api_requests(i, user_context) for i, user_context in enumerate(api_load_users)]
    ...
    async def test_memory_and_resource_usage_under_load(self):
        ...
        """
        resource_metrics = {'memory_samples': [], 'cpu_samples': [], 'connection_counts': [], 'start_time': time.time()}
        ...
        async def sustained_load_operation(user_index: int, user_context):
            ...
            try:
                ws_auth_helper = E2EWebSocketAuthHelper(environment='staging')
                websocket = await ws_auth_helper.connect_authenticated_websocket(timeout=30.0)
                operation_duration = 30.0
                operation_start = time.time()
                while time.time() - operation_start < operation_duration:
                    message = {'type': 'resource_test', 'user_index': user_index, 'operation_count': operations_completed, 'timestamp': time.time()}
                    await websocket.send(json.dumps(message))
                    operations_completed += 1
                    await asyncio.sleep(2.0)
                await websocket.close()
                return {'user_index': user_index, 'operations_completed': operations_completed, 'success': True}
            except Exception as e:
                return {'user_index': user_index, 'operations_completed': operations_completed, 'error': str(e), 'success': False}
        load_users = self.test_users[:6]
        load_tasks = [sustained_load_operation(i, user_context) for i, user_context in enumerate(load_users)]
        ...
        async def monitor_resources():
            ...
            while time.time() - monitor_start < 35.0:
                try:
                    ...
                    resource_metrics['memory_samples'].append({'timestamp': time.time() - monitor_start, 'memory_mb': memory_info.rss / (1024 * 1024), 'memory_vms_mb': memory_info.vms / (1024 * 1024) if hasattr(memory_info, 'vms') else 0})
                    resource_metrics['cpu_samples'].append({'timestamp': time.time() - monitor_start, 'cpu_percent': cpu_percent})
                    sample_count += 1
                except Exception as e:
        ...
        max_acceptable_memory_increase = len(load_users) * 10
    ...
    async def test_error_rate_and_reliability_under_stress(self):
        ...
        """
        stress_metrics = {'total_operations': 0, 'successful_operations': 0, 'failed_operations': 0, 'error_types': defaultdict(int), 'response_times': [], 'start_time': time.time()}

        async def generate_stress_load(user_index: int, user_context):
            """Generate stress load with various operation types."""
            user_operations = {'total': 0, 'successful': 0, 'failed': 0, 'errors': []}
            try:
                ws_auth_helper = E2EWebSocketAuthHelper(environment='staging')
                operation_types = ['quick_query', 'data_request', 'status_check', 'ping_test', 'connection_test']
                for operation_type in operation_types:
                    for attempt in range(3):
                        ...
                        try:
                            ...
                            operation_message = {'type': operation_type, 'user_index': user_index, 'attempt': attempt, 'timestamp': time.time()}
                            ...
                            user_operations['errors'].append({'operation_type': operation_type, 'attempt': attempt, 'error': str(e)})
                            op_time = time.time() - op_start
                            stress_metrics['response_times'].append(op_time)
                        await asyncio.sleep(0.5)
                return {'user_index': user_index, 'operations': user_operations, 'success': True}
            except Exception as e:
                return {'user_index': user_index, 'operations': user_operations, 'error': str(e), 'success': False}
        stress_users = self.test_users[:5]
        stress_tasks = [generate_stress_load(i, user_context) for i, user_context in enumerate(stress_users)]
