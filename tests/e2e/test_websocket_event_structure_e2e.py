...
class TestWebSocketEventStructureE2E(SSotAsyncTestCase):
    ...
    async def asyncSetUp(self):
        ...
        # Skip if not staging environment
        ...
        # Set up auth harness for real staging authentication
        ...
        # Captured events for validation
    ...
    async def setup_staging_websocket_connection(self):
        ...
        if not self.auth_token:
            ...
            login_response = await self.harness.auth_client.post(
                f"{self.staging_api_base}/auth/login",
                json={
        ...
        self.websocket = await websockets.connect(
            f"{self.staging_ws_url}?token={self.auth_token}",
    ...
    async def start_agent_conversation(self, user_message: str) -> str:
        ...
        response = await self.harness.auth_client.post(
            f"{self.staging_api_base}/chat/start",
            headers={"Authorization": f"Bearer {self.auth_token}"},
    ...
    async def test_staging_agent_execution_event_structure(self):
        ...
        try:
            ...
            conversation_id = await self.start_agent_conversation(
                "Analyze the current market trends for tech stocks"
            )
            
            # Capture events from the agent execution
            events = await self.capture_websocket_events(duration_seconds=15)
            
            # Validate we received the expected event types
            event_types = [event.get("type") for event in events]
            expected_types = ["agent_started", "agent_thinking", "tool_executing", "tool_completed", "agent_completed"]
            
            # Should have at least agent_started and some progression events
    ...
    def validate_agent_started_structure(self, event: Dict[str, Any]):
        ...
        self.assertIn("agent_type", event,
                     "agent_type should be at top level of agent_started event")
        self.assertIn("run_id", event,
                     "run_id should be at top level of agent_started event")
        self.assertIn("user_id", event,
                     "user_id should be at top level of agent_started event")
        
        # Should not be buried in data wrapper
    ...
    def validate_agent_thinking_structure(self, event: Dict[str, Any]):
        ...
        # FAILING ASSERTIONS: Thinking fields should be at top level
        self.assertIn("thinking_content", event,
                     "thinking_content should be at top level of agent_thinking event")
    ...
    def validate_tool_executing_structure(self, event: Dict[str, Any]):
        ...
        # FAILING ASSERTIONS: Tool fields should be at top level
        self.assertIn("tool_name", event,
                     "tool_name should be at top level of tool_executing event")
        self.assertIn("tool_description", event,
                     "tool_description should be at top level of tool_executing event")
    ...
    def validate_tool_completed_structure(self, event: Dict[str, Any]):
        ...
        # FAILING ASSERTIONS: Completion fields should be at top level
        self.assertIn("tool_name", event,
                     "tool_name should be at top level of tool_completed event")
        self.assertIn("result", event,
                     "result should be at top level of tool_completed event")
        self.assertIn("status", event,
                     "status should be at top level of tool_completed event")
    ...
    def validate_agent_completed_structure(self, event: Dict[str, Any]):
        ...
        # FAILING ASSERTIONS: Response fields should be at top level
        self.assertIn("response", event,
                     "response should be at top level of agent_completed event")
        self.assertIn("run_id", event,
                     "run_id should be at top level of agent_completed event")
    ...
    async def test_frontend_event_parsing_compatibility(self):
        ...
        try:
            # Start simple agent conversation
            conversation_id = await self.start_agent_conversation(
            ...
            # Simulate frontend event parsing requirements
            for event in events:
                event_type = event.get("type")
                
                # Frontend expects direct access to business fields
                if event_type == "agent_started":
                    # Frontend needs agent_type for UI display
                    agent_type = event.get("agent_type")
                    self.assertIsNotNone(agent_type, 
                                       "Frontend requires direct access to agent_type")
                    
                    # Frontend needs run_id for tracking
                    run_id = event.get("run_id")
                    self.assertIsNotNone(run_id,
                                       "Frontend requires direct access to run_id")
                
                elif event_type == "agent_thinking":
                    # Frontend needs thinking content for real-time display
                    thinking_content = event.get("thinking_content")
                    self.assertIsNotNone(thinking_content,
                                       "Frontend requires direct access to thinking_content")
                
                elif event_type == "tool_executing":
                    # Frontend needs tool details for progress indication
                    tool_name = event.get("tool_name")
                    self.assertIsNotNone(tool_name,
                                       "Frontend requires direct access to tool_name")
                
                elif event_type == "agent_completed":
                    # Frontend needs response for final display
                    response = event.get("response")
                    self.assertIsNotNone(response,
                                       "Frontend requires direct access to response")
                
                # Verify no required data is buried in wrapper
                if "data" in event and isinstance(event["data"], dict):
                    for key in event["data"]:
                        self.fail(f"Business field '{key}' should not be buried in 'data' wrapper - frontend cannot access it")
    ...
    async def test_real_time_event_streaming_structure(self):
        ...
        try:
            # Start longer conversation to test streaming
            conversation_id = await self.start_agent_conversation(
                "Give me a detailed analysis of the current cryptocurrency market trends"
            )
            
            # Capture events as they stream
            streaming_events = []
            timeout = 20  # Longer timeout for detailed analysis
            ...
            while (datetime.now() - start_time).seconds < timeout:
                try:
                    ...
                    # Validate structure immediately as events stream
            ...
            # Validate overall streaming consistency
            self.assertGreater(len(streaming_events), 0, 
                             "Should have received streaming events")
            
            # All events should have consistent base structure
            for event in streaming_events:
                ...
                # Critical business fields should never be in data wrapper
                if "data" in event and isinstance(event["data"], dict):
                    business_fields = ["agent_type", "run_id", "thinking_content", 
                                     "tool_name", "response", "result"]
                    for field in business_fields:
                        if field in event["data"]:
                            self.fail(f"Business field '{field}' buried in data wrapper affects real-time UX")
    ...
    def validate_streaming_event_structure(self, event: Dict[str, Any]):
        ...
        if event_type in ["agent_started", "agent_thinking", "tool_executing", 
            ...
            # Should have timestamp at top level
            self.assertIn("timestamp", event, 
                         f"{event_type} should have timestamp at top level")
            
            # Should not have business data buried in wrapper
            if "data" in event:
                self.assertIsInstance(event["data"], (dict, type(None)),
                                    "If data exists, it should be dict or None")
                
                if isinstance(event["data"], dict) and len(event["data"]) > 0:
                    # Check if business fields are buried
                    business_fields = list(event["data"].keys())
                    if business_fields:
                        self.fail(f"Business fields {business_fields} should not be in 'data' wrapper for {event_type}")
