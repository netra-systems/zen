...
class WebSocketMultiServiceTests:
            ...
            for event in events:
                ...
                for forbidden in forbidden_imports:
                    assert forbidden not in event_str, (
                        fService independence violation: {service} event references {forbidden}: {event}
                    )
    ...
    async def test_service_failure_isolation(self, websocket_client):
        ...
        if service_errors:
            ...
            assert len(working_services) > 0, (
                fAll services failed: {error_services}. 
                fService isolation should prevent complete failure.
            )
            ...
            for service in working_services:
                ...
                assert len(recent_events) > 0, (
                    fService {service} stopped working after other service failures
                )
    ...
    async def test_websocket_load_balancing_compatibility(self, websocket_client):
        ...
        if len(session_ids) > 1:
            pytest.fail(
                fLoad balancing broke session consistency: {session_ids}. 
                fSame WebSocket connection should maintain consistent session.""
            )
        ...
        if compatibility_errors:
            for error in compatibility_errors:
                ...
                assert 500 not in error_message and internal error not in error_message.lower(), (
                    fVersion mismatch should not cause internal server error: {error_message}
                )
