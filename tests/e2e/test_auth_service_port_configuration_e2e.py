...
class AuthServicePortConfigurationValidator:
    ...
    async def validate_port_configuration(self, config: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            result["error"] = f"Connection refused - no service running on port {port}"
        except httpx.TimeoutException:
            result["error"] = f"Timeout connecting to port {port}"
    ...
    def _generate_fix_recommendation(self, validation_results: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if working_configs:
            ...
            recommendation = {
                ...
                "recommended_action": f"Standardize all E2E tests to use port {working_port}",
                "files_to_update": []
            }
...
async def test_auth_service_port_consistency_validation():
    ...
    for config_result in results['configuration_results']:
        ...
        print(f"   Description: {config_result['description']}")
        if config_result['error']:
            print(f"   Error: {config_result['error']}")
        if config_result['test_files_using']:
            print(f"   Used by: {', '.join(config_result['test_files_using'][:3])}{'...' if len(config_result['test_files_using']) > 3 else ''}")
        print()
    ...
    if results['recommended_fix']:
        ...
        if 'working_port' in fix:
            print(f"Working port found: {fix['working_port']}")
            print(f"Failed ports: {fix['failed_ports']}")
    
    # This assertion is DESIGNED TO FAIL to expose the port mismatch issue
    assert results['success'], (
        f"AUTH SERVICE PORT MISMATCH DETECTED: "
        f"{results['failed_configurations']}/{results['configurations_tested']} configurations failed. "
        f"Port mismatch detected: {results['port_mismatch_detected']}. "
...
async def test_specific_port_8001_expectation():
    ...
    try:
        async with httpx.AsyncClient(timeout=5.0) as client:
            ...
            assert response.status_code == 200, f"Auth service not responding on expected port {expected_port}"
            
            response_data = response.json()
            assert "auth" in response_data.get("service", "").lower(), (
                f"Service on port {expected_port} is not the auth service: {response_data}"
            )
            
            print(f"✅ Auth service found on expected port {expected_port}")
            
    except httpx.ConnectError:
        pytest.fail(
            f"❌ EXPECTED FAILURE: Auth service not running on port {expected_port}. "
        ...
        pytest.fail(f"❌ TIMEOUT: Auth service on port {expected_port} not responding within 5 seconds")
