...
from test_framework.ssot.e2e_auth_helper import E2EAuthHelper, create_authenticated_user, create_authenticated_user_context
from test_framework.real_services_test_fixtures import real_services_fixture
from test_framework.websocket_helpers import assert_websocket_events_sent, WebSocketTestHelpers, create_test_websocket_connection
...
class CompleteAgentChatFlowE2ETests(BaseE2ETest):
    ...
    async def test_complete_agent_chat_flow_real_authentication(self, real_services_fixture):
        ...
        auth_user = await self.auth_helper.create_authenticated_user(email='complete_chat_test@example.com', permissions=['read', 'write', 'agent_execute'])
        ...
        try:
            ...
            chat_message = {'type': 'agent_request', 'agent': 'triage_agent', 'message': 'Please analyze my current infrastructure costs and provide optimization recommendations. I need actionable insights to reduce spending.', 'user_id': auth_user.user_id, 'thread_id': str(user_context.thread_id), 'request_id': str(user_context.request_id), 'timestamp': datetime.now(timezone.utc).isoformat()}
    ...
    async def test_agent_chat_flow_with_tool_execution_validation(self, real_services_fixture):
        ...
        auth_user = await self.auth_helper.create_authenticated_user(email='tool_execution_test@example.com', permissions=['read', 'write', 'agent_execute', 'tool_execute'])
        websocket_url = 'ws://localhost:8000/ws/chat'
        headers = self.auth_helper.get_websocket_headers(auth_user.jwt_token)
        received_events = []
        websocket_connection = None
        try:
            websocket_connection = await WebSocketTestHelpers.create_test_websocket_connection(websocket_url, headers=headers, timeout=15.0, user_id=auth_user.user_id)
            chat_message = {'type': 'agent_request', 'agent': 'data_helper_agent', 'message': 'Please search for information about cloud cost optimization best practices and analyze current trends.', 'user_id': auth_user.user_id, 'timestamp': datetime.now(timezone.utc).isoformat()}
            await WebSocketTestHelpers.send_test_message(websocket_connection, chat_message)
            start_time = time.time()
            ...
            assert len(tool_executing_events) == len(tool_completed_events), f'Tool events not paired: {len(tool_executing_events)} executing, {len(tool_completed_events)} completed'
    ...
    async def test_agent_chat_flow_response_quality_validation(self, real_services_fixture):
        ...
        auth_user = await self.auth_helper.create_authenticated_user(email='response_quality_test@example.com')
        websocket_url = 'ws://localhost:8000/ws/chat'
        headers = self.auth_helper.get_websocket_headers(auth_user.jwt_token)
        received_events = []
        websocket_connection = None
        try:
            websocket_connection = await WebSocketTestHelpers.create_test_websocket_connection(websocket_url, headers=headers, timeout=15.0, user_id=auth_user.user_id)
            chat_message = {'type': 'agent_request', 'agent': 'triage_agent', 'message': 'I need to reduce my AWS costs by 20% this quarter. What are the most effective strategies and specific actions I can take?', 'user_id': auth_user.user_id, 'timestamp': datetime.now(timezone.utc).isoformat()}
            await WebSocketTestHelpers.send_test_message(websocket_connection, chat_message)
            start_time = time.time()
