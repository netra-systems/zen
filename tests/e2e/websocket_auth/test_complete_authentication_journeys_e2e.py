...
class CompleteWebSocketAuthenticationJourneysTests:
    ...
    def setup_class(cls):
        ...
        if test_env == 'staging':
            ...
            cls.auth_config = E2EAuthConfig(auth_service_url='http://localhost:8083', backend_url='http://localhost:8002', websocket_url='ws://localhost:8002/ws', timeout=30.0)
    ...
    def _validate_real_services_available(cls):
        ...
        required_services = [('Authentication Service', cls.auth_config.auth_service_url), ('Backend Service', cls.auth_config.backend_url)]
    ...
    def test_new_user_registration_to_websocket_authentication_journey(self):
        ...
        new_user_data = {'email': f'e2e_new_user_{int(time.time())}@test.example.com', 'password': 'SecureTestPassword123!', 'full_name': 'E2E Test User', 'terms_accepted': True}
        start_time = time.time()
        authenticated_user = self.auth_helper.create_authenticated_user(email=new_user_data['email'], user_id=f'e2e_user_{int(time.time())}', full_name=new_user_data['full_name'], permissions=['websocket', 'chat', 'agent_execution'])
        ...
        websocket_headers = {'Authorization': f'Bearer {authenticated_user.jwt_token}', 'User-Agent': 'E2E Test Client', 'X-User-ID': authenticated_user.user_id}
        ...
        async def test_websocket_connection():
            ...
            try:
                async with websockets.connect(self.auth_config.websocket_url, extra_headers=websocket_headers, timeout=self.auth_config.timeout) as websocket:
                    auth_message = {'type': 'authenticate', 'token': authenticated_user.jwt_token, 'user_id': authenticated_user.user_id, 'timestamp': datetime.now(timezone.utc).isoformat()}
    ...
    def test_existing_user_login_to_chat_session_journey(self):
        ...
        existing_user = self.auth_helper.create_authenticated_user(email=f'existing_user_{int(time.time())}@test.example.com', user_id=f'existing_{int(time.time())}', full_name='Existing E2E User', permissions=['websocket', 'chat', 'agent_execution'])
        ...
        async def test_authenticated_chat_session():
            ...
            websocket_headers = {'Authorization': f'Bearer {existing_user.jwt_token}', 'X-Session-Type': 'chat', 'X-User-ID': existing_user.user_id}
            try:
                async with websockets.connect(self.auth_config.websocket_url, extra_headers=websocket_headers, timeout=self.auth_config.timeout) as websocket:
                    auth_message = {'type': 'authenticate', 'token': existing_user.jwt_token, 'user_id': existing_user.user_id}
                    await websocket.send(json.dumps(auth_message))
                    auth_response = await asyncio.wait_for(websocket.recv(), timeout=10.0)
                    auth_result = json.loads(auth_response)
                    assert auth_result.get('type') == 'auth_success'
                    assert auth_result.get('user_id') == existing_user.user_id
                    chat_message = {'type': 'agent_request', 'message': 'Help me optimize my AI costs', 'agent_type': 'cost_optimization', 'user_id': existing_user.user_id, 'session_id': f'chat_session_{int(time.time())}', 'timestamp': datetime.now(timezone.utc).isoformat()}
    ...
    def test_oauth_social_login_to_websocket_chat_journey(self):
        ...
        oauth_user = self.auth_helper.create_authenticated_user(email=f'oauth_user_{int(time.time())}@gmail.com', user_id=f'oauth_{int(time.time())}', full_name='OAuth E2E User', permissions=['websocket', 'chat', 'agent_execution', 'oauth'])
        oauth_token_data = {'provider': 'google', 'provider_user_id': 'google_123456789', 'access_token': f'oauth_access_token_{int(time.time())}', 'refresh_token': f'oauth_refresh_token_{int(time.time())}', 'expires_in': 3600}
        ...
        async def test_oauth_websocket_chat():
            ...
            websocket_headers = {'Authorization': f'Bearer {oauth_user.jwt_token}', 'X-Auth-Provider': 'google', 'X-OAuth-User-ID': oauth_token_data['provider_user_id'], 'X-User-ID': oauth_user.user_id}
            try:
                async with websockets.connect(self.auth_config.websocket_url, extra_headers=websocket_headers, timeout=self.auth_config.timeout) as websocket:
                    oauth_auth_message = {'type': 'oauth_authenticate', 'token': oauth_user.jwt_token, 'user_id': oauth_user.user_id, 'provider': 'google', 'provider_user_id': oauth_token_data['provider_user_id']}
                    ...
                    multi_user_message = {'type': 'multi_user_chat', 'message': 'Hello from OAuth user!', 'user_id': oauth_user.user_id, 'chat_room': 'general', 'timestamp': datetime.now(timezone.utc).isoformat()}
    ...
    def test_session_expiration_and_token_refresh_journey(self):
        ...
        short_lived_user = self.auth_helper.create_authenticated_user(email=f'refresh_user_{int(time.time())}@test.example.com', user_id=f'refresh_{int(time.time())}', full_name='Token Refresh E2E User', permissions=['websocket', 'chat', 'token_refresh'])

        async def test_token_refresh_journey():
            """Test complete token refresh journey."""
            initial_token = short_lived_user.jwt_token
            websocket_headers = {'Authorization': f'Bearer {initial_token}', 'X-Token-Refresh': 'enabled', 'X-User-ID': short_lived_user.user_id}
            try:
                async with websockets.connect(self.auth_config.websocket_url, extra_headers=websocket_headers, timeout=self.auth_config.timeout) as websocket:
                    auth_message = {'type': 'authenticate', 'token': initial_token, 'user_id': short_lived_user.user_id, 'refresh_enabled': True}
                    await websocket.send(json.dumps(auth_message))
                    ...
                    refresh_request = {'type': 'token_refresh', 'current_token': initial_token, 'user_id': short_lived_user.user_id, 'timestamp': datetime.now(timezone.utc).isoformat()}
                    ...
                    continued_message = {'type': 'test_message', 'message': 'Testing with refreshed token', 'user_id': short_lived_user.user_id, 'token': new_token}
    ...
    def test_authentication_failure_recovery_journey(self):
        ...
        test_user = self.auth_helper.create_authenticated_user(email=f'failure_test_{int(time.time())}@test.example.com', user_id=f'failure_{int(time.time())}', full_name='Failure Recovery User', permissions=['websocket', 'chat'])

        async def test_authentication_failure_recovery():
            """Test complete authentication failure recovery."""
            failure_scenarios = [{'name': 'invalid_token', 'token': 'invalid.jwt.token.here', 'expected_error': 'INVALID_TOKEN'}, {'name': 'expired_token', 'token': 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJzdWIiOiIxMjM0NTY3ODkwIiwibmFtZSI6IkpvaG4gRG9lIiwiaWF0IjoxNTE2MjM5MDIyLCJleHAiOjE1MTYyMzkwMjJ9.invalid', 'expected_error': 'EXPIRED_TOKEN'}]
            successful_recovery = False
            for scenario in failure_scenarios:
                try:
                    invalid_headers = {'Authorization': f"Bearer {scenario['token']}", 'X-User-ID': test_user.user_id}
                    async with websockets.connect(self.auth_config.websocket_url, extra_headers=invalid_headers, timeout=10.0) as websocket:
                        invalid_auth = {'type': 'authenticate', 'token': scenario['token'], 'user_id': test_user.user_id}
                        ...
                        recovery_auth = {'type': 'authenticate', 'token': test_user.jwt_token, 'user_id': test_user.user_id, 'recovery_attempt': True}
            ...
            if not successful_recovery:
                try:
                    valid_headers = {'Authorization': f'Bearer {test_user.jwt_token}', 'X-User-ID': test_user.user_id}
                    async with websockets.connect(self.auth_config.websocket_url, extra_headers=valid_headers, timeout=self.auth_config.timeout) as websocket:
                        valid_auth = {'type': 'authenticate', 'token': test_user.jwt_token, 'user_id': test_user.user_id}
