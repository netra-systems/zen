...
from test_framework.ssot.real_websocket_test_client import RealWebSocketTestClient, WebSocketEvent, SecurityError
from test_framework.ssot.real_websocket_connection_manager import RealWebSocketConnectionManager, IsolationTestType
...
class TestIsolationViolationDetection:
    ...
    async def test_client_detects_user_id_mismatch(self):
        ...
        client = RealWebSocketTestClient(backend_url='ws://localhost:8000', environment='test', auth_required=False)
        client.expected_user_id = 'user_123'
        malicious_event = WebSocketEvent(event_type='test_event', data={'user_id': 'user_456'}, timestamp=time.time(), user_id='user_456')
    ...
    async def test_client_allows_matching_user_id(self):
        ...
        client = RealWebSocketTestClient(backend_url='ws://localhost:8000', environment='test', auth_required=False)
        client.expected_user_id = 'user_123'
        valid_event = WebSocketEvent(event_type='test_event', data={'user_id': 'user_123'}, timestamp=time.time(), user_id='user_123')
    ...
    async def test_assert_events_received_fails_when_missing(self):
        ...
        client = RealWebSocketTestClient(backend_url='ws://localhost:8000', environment='test', auth_required=False)
        client.received_events = [WebSocketEvent('agent_started', {}, time.time()), WebSocketEvent('agent_thinking', {}, time.time())]
        required_events = {'agent_started', 'agent_thinking', 'agent_completed', 'tool_executing', 'tool_completed'}
        with pytest.raises(AssertionError) as exc_info:
    ...
    async def test_connection_manager_detects_violations(self):
        ...
        manager = RealWebSocketConnectionManager(backend_url='ws://localhost:8000', environment='test')
        connection_id = 'test_conn_123'
        manager.connections[connection_id] = type('MockProfile', (), {'connection_id': connection_id, 'user_id': 'user_123', 'isolation_violations': ['Test violation 1', 'Test violation 2'], 'record_violation': lambda self, v: None})()
    ...
    async def test_isolation_test_result_indicates_failure(self):
        ...
        failed_result = IsolationTestResult(test_type=IsolationTestType.EVENT_ISOLATION, test_passed=False, connections_tested=5, total_violations=3, violation_details=["User A received User B's data", 'Cross-user event leak detected', 'Authentication boundary crossed'], test_duration=15.5, events_validated=100)
    ...
    async def test_websocket_events_isolation_function_detects_violations(self):
        ...
        with pytest.raises(SecurityError) as exc_info:
            await test_websocket_events_isolation(clients=mock_clients, event_types={'test_event'}, timeout=1.0)
...
class TestIsolationViolationScenarios:
    ...
    async def test_event_data_contamination_detection(self):
        ...
        contaminated_event = WebSocketEvent(event_type='message_notification', data={'user_id': 'user_bob', 'message': 'Private message for Bob', 'recipient': 'user_bob'}, timestamp=time.time(), user_id='user_bob')
    ...
    async def test_empty_user_context_handling(self):
        ...
        client = RealWebSocketTestClient(backend_url='ws://localhost:8000', environment='test', auth_required=False)
        client.expected_user_id = 'user_alice'
        neutral_event = WebSocketEvent(event_type='system_notification', data={'message': 'System maintenance scheduled'}, timestamp=time.time(), user_id=None)
    ...
    async def test_partial_user_id_in_data_only(self):
        ...
        client = RealWebSocketTestClient(backend_url='ws://localhost:8000', environment='test', auth_required=False)
        client.expected_user_id = 'user_alice'
        data_violation_event = WebSocketEvent(event_type='user_action', data={'user_id': 'user_charlie'}, timestamp=time.time(), user_id=None)
        with pytest.raises(SecurityError):
