...
class ChatCloudRunSimulationTests(BaseTestCase):
    ...
    async def test_chat_functionality_during_import_failures(self):
        ...
        chat_client = await self.ws_manager.create_authenticated_connection(user_token=user_token, endpoint='/ws/chat')
        try:
            ...
            initial_message = {'type': 'agent_request', 'data': {'agent_type': 'data_explorer', 'query': 'What data do we have available?'}}
            await chat_client.send_json(initial_message)
            events_received = await self.agent_validator.validate_agent_event_sequence(chat_client, expected_events=['agent_started', 'agent_thinking', 'tool_executing', 'tool_completed', 'agent_completed'], timeout=30.0)
            ...
            stress_message = {'type': 'agent_request', 'data': {'agent_type': 'optimization_agent', 'query': 'Optimize our data pipeline under Cloud Run conditions'}}
            await chat_client.send_json(stress_message)
            try:
                stress_events = await self.agent_validator.validate_agent_event_sequence(chat_client, expected_events=['agent_started', 'agent_thinking', 'tool_executing', 'tool_completed', 'agent_completed'], timeout=45.0)
                if stress_events:
    ...
    async def test_multi_user_chat_import_failure_isolation(self):
        ...
        try:
            for user_id in range(3):
                user_token = await self.auth_helper.get_authenticated_user_token()
                user_client = await self.ws_manager.create_authenticated_connection(user_token=user_token, endpoint='/ws/chat')
                await user_client.connect()
                user_connections.append({'client': user_client, 'user_id': user_id, 'token': user_token})
            ...
            chat_tasks = []
            for user_conn in user_connections:
                task = asyncio.create_task(self._execute_user_chat_flow(user_conn['client'], f"user_{user_conn['user_id']}"))
    ...
    async def test_chat_recovery_after_import_fix(self):
        ...
        try:
            await chat_client.connect()
            test_scenarios = [{'name': 'basic_chat', 'agent_type': 'data_explorer', 'query': 'Show me available data sources'}, {'name': 'complex_chat', 'agent_type': 'optimization_agent', 'query': 'Analyze performance bottlenecks in our data pipeline'}, {'name': 'error_recovery_chat', 'agent_type': 'data_explorer', 'query': 'Handle this gracefully even under resource pressure'}]
            for scenario in test_scenarios:
                await self._light_resource_pressure()
                message = {'type': 'agent_request', 'data': {'agent_type': scenario['agent_type'], 'query': scenario['query']}}
                await chat_client.send_json(message)
                events = await self.agent_validator.validate_agent_event_sequence(chat_client, expected_events=['agent_started', 'agent_thinking', 'tool_executing', 'tool_completed', 'agent_completed'], timeout=30.0)
                assert events, f"Chat scenario '{scenario['name']}' should work after fix"
                for event in events:
                    if 'error' in event:
                        error_msg = str(event['error'])
                        assert 'time' not in error_msg or 'not defined' not in error_msg, f'Import error still present after fix: {error_msg}'
    ...
    async def test_websocket_agent_events_with_import_stability(self):
        ...
        try:
            ...
            agent_message = {'type': 'agent_request', 'data': {'agent_type': 'data_explorer', 'query': 'Test all WebSocket events under import pressure'}}
            await chat_client.send_json(agent_message)
            required_events = ['agent_started', 'agent_thinking', 'tool_executing', 'tool_completed', 'agent_completed']
            received_events = await self.agent_validator.validate_all_required_events(chat_client, required_events=required_events, timeout=40.0)
    ...
    async def _execute_user_chat_flow(self, websocket_client, user_context):
        ...
        try:
            message = {'type': 'agent_request', 'data': {'agent_type': 'data_explorer', 'query': f'User-specific query from {user_context}'}}
            await websocket_client.send_json(message)
            events = await self.agent_validator.validate_agent_event_sequence(websocket_client, expected_events=['agent_started', 'agent_completed'], timeout=20.0)
    ...
    async def _corrupt_import_system_aggressively(self):
        ...
        for round_num in range(3):
            modules_to_remove = ['time', 'datetime', 'asyncio', 'shared.isolated_environment']
...
class WebSocketErrorRecoveryTests(BaseTestCase):
    ...
    async def test_websocket_connection_stability_post_fix(self):
        ...
        try:
            ...
            websocket_client = await ws_manager.create_authenticated_connection(user_token=user_token, endpoint='/ws/chat')
            await websocket_client.connect()
            error_scenarios = [{'type': 'invalid_message', 'data': {'malformed': 'structure'}}, {'type': 'agent_request', 'data': {'agent_type': 'INVALID_AGENT', 'query': 'test'}}, {'type': 'unknown_type', 'data': {'test': 'error_handling'}}]
            for scenario in error_scenarios:
                await websocket_client.send_json(scenario)
                assert websocket_client.is_connected(), f"Connection lost after error scenario: {scenario['type']}"
                try:
                    response = await websocket_client.receive_json(timeout=5.0)
                    if response and 'error' in response:
                        error_msg = str(response['error'])
                        assert 'time' not in error_msg or 'not defined' not in error_msg, f'Import error still present: {error_msg}'
                except asyncio.TimeoutError:
                    pass
