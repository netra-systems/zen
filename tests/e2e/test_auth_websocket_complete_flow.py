...
class AuthWebSocketFlowValidator:
    ...
    def start_flow_tracking(self, user_info: Dict[str, Any]):
        ...
        self.flow_steps.append({'step': 'flow_started', 'timestamp': self.flow_start_time, 'user_info': user_info})

    def record_flow_step(self, step_name: str, status: str, details: Optional[Dict]=None):
        """Record a step in the complete flow."""
        step_record = {'step': step_name, 'timestamp': time.time(), 'status': status, 'details': details or {}, 'flow_duration': time.time() - (self.flow_start_time or time.time())}
    ...
    def record_agent_event(self, event_data: Dict[str, Any]):
        ...
        event_record = {'event_type': event_data.get('type'), 'timestamp': time.time(), 'data': event_data, 'user_context': event_data.get('user_id') or self.user_context.get('user_id')}
    ...
    def record_auth_event(self, auth_data: Dict[str, Any]):
        ...
        auth_record = {'timestamp': time.time(), 'event': auth_data.get('event', 'unknown'), 'data': auth_data}
    ...
    def validate_complete_flow(self) -> Dict[str, Any]:
        ...
        validation = {'flow_completed': False, 'auth_successful': False, 'websocket_connected': False, 'agent_events_delivered': False, 'user_isolation_maintained': True, 'business_impact': '', 'flow_metrics': {}}
        ...
        if self.flow_steps:
            ...
            validation['flow_metrics'] = {'total_duration': total_duration, 'total_steps': len(self.flow_steps), 'agent_events_count': len(self.agent_events), 'auth_events_count': len(self.auth_events)}
...
class AuthWebSocketCompleteFlowTests(BaseE2ETest):
    ...
    async def test_complete_auth_to_agent_response_flow(self):
        ...
        user_info = {'user_id': user_id, 'email': user_email, 'flow_type': 'complete_auth_websocket_agent'}
        ...
        try:
            ...
            auth_token = self.auth_helper.create_test_jwt_token(user_id=user_id, email=user_email, permissions=['read', 'write', 'chat', 'websocket', 'agent:execute'], exp_minutes=60)
            ...
            try:
                ...
                self.validator.record_auth_event({'event': 'token_validated', 'user_id': user_id, 'permissions': decoded.get('permissions', [])})
                self.validator.record_flow_step('authentication_completed', 'success', {'token_created': True, 'token_validated': True, 'user_id': user_id})
            except Exception as e:
                self.validator.record_flow_step('authentication_completed', 'failed', {'error': str(e)})
            ...
            try:
                async with websockets.connect(websocket_url, additional_headers=self.ws_auth_helper.get_websocket_headers(auth_token), open_timeout=10.0) as websocket:
                    self.validator.record_flow_step('websocket_connection_completed', 'success', {'connected': True, 'auth_headers_sent': True})
                    ...
                    identity_message = {'type': 'user_identity_verify', 'user_id': user_id, 'email': user_email, 'timestamp': datetime.now().isoformat()}
                    ...
                    try:
                        ...
                        self.validator.record_auth_event({'event': 'identity_verified', 'response': identity_data})
                        self.validator.record_flow_step('identity_verification_completed', 'success', {'identity_confirmed': True, 'response_type': identity_data.get('type')})
                    ...
                    agent_request = {'type': 'agent_request', 'agent_name': 'triage_agent', 'query': 'Hello! This is a complete flow test. Please provide a helpful response to validate our auth-WebSocket integration.', 'user_id': user_id, 'thread_id': thread_id, 'metadata': {'test_type': 'complete_auth_websocket_flow', 'timestamp': datetime.now().isoformat()}}
                    await websocket.send(json.dumps(agent_request))
                    self.validator.record_flow_step('agent_request_completed', 'success', {'request_sent': True, 'agent_name': agent_request['agent_name'], 'thread_id': thread_id})
                    ...
                    while time.time() - event_start_time < event_timeout and (not agent_response_complete):
                        try:
                            ...
                            if event_type == 'agent_completed':
                                ...
                                if agent_result:
                                    self.validator.record_flow_step('agent_value_delivered', 'success', {'response_length': len(str(agent_result)), 'value_delivered': True})
                ...
                self.validator.record_flow_step('websocket_connection_completed', 'failed', {'error': str(e)})
    ...
    async def test_multi_user_auth_websocket_isolation(self):
        ...
        try:
            ...
            for i in range(user_count):
                ...
                user_token = self.auth_helper.create_test_jwt_token(user_id=user_id, email=user_email, permissions=['chat', 'websocket', 'agent:execute'])
                user_flows.append({'user_id': user_id, 'email': user_email, 'token': user_token, 'validator': AuthWebSocketFlowValidator()})
            logger.info('[U+1F517] STEP 2: WebSocket connections for all users')
            for i, user_flow in enumerate(user_flows):
                user_flow['validator'].start_flow_tracking({'user_id': user_flow['user_id'], 'email': user_flow['email'], 'user_index': i})
                websocket_url = f"{self.ws_auth_helper.config.websocket_url}?token={user_flow['token']}"
                websocket = await websockets.connect(websocket_url, additional_headers=self.ws_auth_helper.get_websocket_headers(user_flow['token']))
                websocket_connections.append(websocket)
                user_flow['websocket'] = websocket
                user_flow['validator'].record_flow_step('multi_user_connection', 'success', {'user_index': i, 'connected': True})
            logger.info('[U+1F4AC] STEP 3: Send isolated messages from each user')
            for i, user_flow in enumerate(user_flows):
                private_message = {'type': 'chat_message', 'content': f'Private message from user {i} - should not be seen by others', 'user_id': user_flow['user_id'], 'thread_id': f"private-{user_flow['user_id']}-{uuid.uuid4().hex[:8]}", 'privacy_level': 'private'}
                await user_flow['websocket'].send(json.dumps(private_message))
                user_flow['validator'].record_flow_step('private_message_sent', 'success', {'user_index': i, 'message_private': True})
                        ...
                        for j, other_user in enumerate(user_flows):
                            if i != j:
                                ...
                                if other_user_id in response_str:
                                    isolation_violations.append({'user_receiving': user_flow['user_id'], 'other_user_data': other_user_id, 'response': response})
                    user_flow['validator'].record_flow_step('isolation_validated', 'success', {'responses_received': len(user_responses), 'violations_detected': len([v for v in isolation_violations if v['user_receiving'] == user_flow['user_id']])})
                except Exception as e:
                    user_flow['validator'].record_flow_step('isolation_validation', 'failed', {'error': str(e)})
    ...
    async def test_auth_websocket_error_recovery(self):
        ...
        try:
            ...
            try:
                async with websockets.connect(websocket_url, additional_headers={'Authorization': f'Bearer {invalid_token}'}, open_timeout=5.0) as websocket:
                    test_message = {'type': 'ping', 'user_id': user_id}
        ...
        try:
            async with websockets.connect(recovery_url, additional_headers=self.ws_auth_helper.get_websocket_headers(valid_token)) as recovery_websocket:
                recovery_message = {'type': 'chat_message', 'content': 'Recovery test with valid token', 'user_id': user_id}
...
class AuthWebSocketPerformanceTests(BaseE2ETest):
    ...
    async def test_auth_websocket_concurrent_load(self):
        ...
        async def simulate_user_session(user_index: int) -> Dict[str, Any]:
            ...
            try:
                token = auth_helper.create_test_jwt_token(user_id=user_id, email=f'loaduser{user_index}@netra.test')
                websocket_url = f'{auth_helper.config.websocket_url}?token={token}'
                async with websockets.connect(websocket_url, additional_headers=auth_helper.get_websocket_headers(token), open_timeout=5.0) as websocket:
                    test_message = {'type': 'chat_message', 'content': f'Load test message from user {user_index}', 'user_id': user_id}
                    await websocket.send(json.dumps(test_message))
                    try:
                        response = await asyncio.wait_for(websocket.recv(), timeout=3.0)
                    ...
                    return {'user_index': user_index, 'user_id': user_id, 'success': True, 'duration': session_duration, 'response_received': response_received}
            except Exception as e:
                return {'user_index': user_index, 'user_id': user_id, 'success': False, 'error': str(e), 'duration': time.time() - start_time}
