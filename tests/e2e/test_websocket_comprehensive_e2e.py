...
class WebSocketE2ETester:
    ...
    async def send_message(self, conn_id: str, message: Dict):
        ...
        if not ws:
            raise ValueError(fConnection {conn_id} not found)
            
        # Check if websocket is open before sending - RAISE ON FAILURE
        if hasattr(ws, 'closed') and ws.closed:
            raise ConnectionError(fWebSocket connection {conn_id} is closed)
        elif hasattr(ws, 'state') and ws.state.name != 'OPEN':
            raise ConnectionError(fWebSocket connection {conn_id} state: {ws.state.name}")"
...
class WebSocketComprehensiveE2ETests:
    ...
    async def test_connection_health_monitoring(self, ws_tester):
        ...
        assert pong in pong_str or ping in pong_str, fExpected ping/pong response, got: {pong}""
