...
class TestDevModeer:
    ...
    async def _test_websocket_connection(self) -> DevModeTestResult:
        ...
        try:
            ...
            ws_url = f"ws://localhost:{self.backend_port}/ws"
    ...
    def _print_test_result(self, result: DevModeTestResult):
        ...
        print(f"{icon} {result.test_name}: {status} ({result.duration:.2f}s)")
...
async def test_dev_mode_comprehensive():
    ...
    assert report["failed"] == 0, f"Some tests failed: {report}"
    assert report["success_rate"] == 1.0, f"Not all tests passed: {report}"
