...
async def run_agent_and_capture_events():
    ...
    async with aiohttp.ClientSession() as session:
        ...
        headers = {Authorization: fBearer {token}} if token != anonymous else {}""
...
async def test_multiple_concurrent_users():
    ...
    async def run_user(user_id: int):
        ...
        async with aiohttp.ClientSession() as session:
            ...
            user_config = {
                email: fuser{user_id}@test.com,
                "password: fpass{user_id}"
            }
            ...
            async with websockets.connect(STAGING_CONFIG[ws_url) as websocket:
                ...
                message = {
                    type: "message,"
                    content": fUser {user_id}: Test message,"
                    user_id: user_id,
                    timestamp: time.time()""
                }
                ...
                while time.time() - start < 5:  # 5 second collection
                    ...
                    if user_id" in str(event):"
                        other_user = None
                        for uid in range(10):
                            if uid != user_id and fuser{uid} in str(event):
                                other_user = uid
                                break
