...
class MockWebSocketConnection:
    ...
    async def send_json(self, message: dict):
        ...
        self.messages_sent.append({'timestamp': time.time(), 'message': message})
...
class ActionsToMeetGoalsUserExperienceFailuresTests(SSotAsyncTestCase):
    ...
    async def test_complete_user_journey_action_plan_request_failure(self):
        ...
        user_request = {'message': 'Please create an action plan to optimize my AI infrastructure costs and improve performance', 'thread_id': self.test_thread_id, 'user_id': self.test_user_id, 'request_type': 'action_plan_generation'}
        websocket_client = MockWebSocketConnection(f'client_{self.test_user_id}')
        try:
            result = await self._simulate_backend_orchestration(user_request=user_request, user_session=user_session, websocket_connection=websocket_client)
            ...
            error_message = str(e)
            assert any(['Agent execution failed' in error_message, 'LLM manager is None' in error_message, 'incomplete architectural migration' in error_message]), f'Expected user-facing error, got: {error_message}'
    ...
    async def test_websocket_event_sequence_during_agent_failure(self):
        ...
        async def capture_event(event_type: str, data: Any):
            captured_events.append({'event_type': event_type, 'data': data, 'timestamp': time.time()})
            await websocket_client.send_json({'event_type': event_type, 'data': data})
        ...
        context = UserExecutionContext.from_request_supervisor(user_id=self.test_user_id, thread_id=self.test_thread_id, run_id=self.test_run_id, metadata={'user_request': 'Create action plan for infrastructure optimization', 'websocket_client_id': websocket_client.client_id})
        try:
    ...
    async def test_frontend_backend_integration_failure_handling(self):
        ...
        """
        api_request = {'method': 'POST', 'url': '/api/chat/request-action-plan', 'headers': {'Authorization': f'Bearer test_token_{self.test_user_id}', 'Content-Type': 'application/json'}, 'json': {'user_id': self.test_user_id, 'thread_id': self.test_thread_id, 'message': 'I need an action plan to optimize my AI infrastructure', 'context': {'optimization_focus': 'cost_and_performance', 'urgency': 'medium'}}}
        ...
        if backend_response.get('success') is False:
            ...
            assert not any(['LLM manager is None' in error_msg, 'architectural migration' in error_msg, 'FIVE_WHYS_ANALYSIS' in error_msg]), f'Internal error exposed to user: {error_msg}'
    ...
    async def test_database_consistency_during_agent_failure(self):
        ...
        contexts = []
        for i in range(3):
            context = UserExecutionContext.from_request_supervisor(user_id=self.test_user_id, thread_id=f'{self.test_thread_id}_{i}', run_id=f'{self.test_run_id}_{i}', metadata={'user_request': f'Test request {i} for database consistency', 'sequence_number': i})
        ...
        self.record_metric('database_consistency_maintained', True)
    ...
    async def test_production_like_load_during_agent_failures(self):
        ...
        for user_num in range(concurrent_users):
            ...
            request_task = self._simulate_concurrent_user_request(user_id=user_id, thread_id=thread_id, run_id=run_id, request_message=f'User {user_num} requests action plan')
    ...
    async def _create_authenticated_user_session(self) -> Dict[str, Any]:
        ...
        return {'user_id': self.test_user_id, 'session_id': f'session_{uuid.uuid4().hex[:12]}', 'authenticated': True, 'token': f'test_token_{self.test_user_id}', 'expires_at': time.time() + 3600}

    async def _simulate_backend_orchestration(self, user_request: Dict[str, Any], user_session: Dict[str, Any], websocket_connection: MockWebSocketConnection) -> Dict[str, Any]:
        """Helper: Simulate complete backend orchestration flow."""
        context = UserExecutionContext.from_request_supervisor(user_id=user_request['user_id'], thread_id=user_request['thread_id'], run_id=self.test_run_id, metadata={'user_request': user_request['message'], 'request_type': user_request['request_type'], 'websocket_client_id': websocket_connection.client_id, 'session_id': user_session['session_id']})
        agent = ActionsToMeetGoalsSubAgent(llm_manager=None, tool_dispatcher=None)
    ...
    async def _simulate_api_request_handling(self, api_request: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            context = UserExecutionContext.from_request_supervisor(user_id=request_data['user_id'], thread_id=request_data['thread_id'], run_id=f'api_{uuid.uuid4().hex[:8]}', metadata={'user_request': request_data['message'], 'context': request_data.get('context', {})})
            agent = ActionsToMeetGoalsSubAgent(llm_manager=None, tool_dispatcher=None)
            result = await agent.execute(context, stream_updates=False)
            return {'success': True, 'action_plan': result.get('action_plan_result'), 'request_id': context.run_id}
        except Exception as e:
            return {'success': False, 'error': 'Unable to generate action plan at this time. Please try again later.', 'error_type': 'agent_execution_failure', 'request_id': context.run_id if 'context' in locals() else None}

    async def _simulate_concurrent_user_request(self, user_id: str, thread_id: str, run_id: str, request_message: str) -> Dict[str, Any]:
        """Helper: Simulate individual concurrent user request."""
        await asyncio.sleep(0.1 * (hash(user_id) % 5))
        context = UserExecutionContext.from_request_supervisor(user_id=user_id, thread_id=thread_id, run_id=run_id, metadata={'user_request': request_message, 'concurrent_test': True})
        agent = ActionsToMeetGoalsSubAgent(llm_manager=None, tool_dispatcher=None)
