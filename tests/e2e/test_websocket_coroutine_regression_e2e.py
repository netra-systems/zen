    ...
    """"
        ...
        try:
            async with websockets.connect(
                websocket_url,
                extra_headers={Authorization: fBearer {user.token}},
                ping_interval=10,
                ping_timeout=5
            ) as websocket:
                ...
                logger.info(fE2E chat flow completed successfully with {len(responses)} responses)
...
""
        ...
        for i in range(3):  # Test with 3 concurrent users
            user = await self.auth_helper.create_authenticated_user(
                email=fmultiuser.e2e.{i}@netrasystems.ai,
                environment=staging
            )
            users.append(user)
        ...
        async def user_websocket_flow(user: AuthenticatedUser, user_index: int):
            ...
            try:
                ...
                if coroutine" in str(e).lower():"
                    self.fail(fMulti-user WebSocket failed with coroutine error: {e})
                raise
        ...
        try:
            ...
            if coroutine" in str(e).lower():"
                self.fail(fMulti-user E2E test failed with coroutine regression: {e})
                    ...
                    try:
                        ...
                        if event_type:
                            ...
                            self.assertNotIn(
                                coroutine","
                                response_str.lower(),
                                fAgent event {event_type} contains coroutine error
                            )
