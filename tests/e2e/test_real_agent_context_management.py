...
class ContextIsolationValidation:
    ...
    cross_user_contamination: List[Tuple[str, str]] = field(default_factory=list)
...
class RealAgentContextManagementTester:
    ...
    CONTEXT_ISOLATION_SCENARIOS = [{'scenario_name': 'concurrent_user_sessions', 'user_count': 3, 'test_message': 'Analyze my personal data and remember my preferences', 'expected_isolation': 'complete_separation', 'validation_criteria': ['no_data_leakage', 'separate_contexts', 'independent_execution']}, {'scenario_name': 'sequential_user_handover', 'user_count': 2, 'test_message': 'Continue the previous analysis from my last session', 'expected_isolation': 'session_continuity', 'validation_criteria': ['context_persistence', 'user_boundary_respect', 'state_isolation']}, {'scenario_name': 'high_concurrency_stress', 'user_count': 5, 'test_message': 'Process my confidential business data', 'expected_isolation': 'stress_test_isolation', 'validation_criteria': ['no_resource_conflicts', 'maintained_isolation', 'performance_stability']}, {'scenario_name': 'context_state_persistence', 'user_count': 2, 'test_message': 'Remember this important information for future sessions', 'expected_isolation': 'persistent_separation', 'validation_criteria': ['state_persistence', 'cross_session_isolation', 'data_integrity']}]
    ...
    async def create_user_context(self, user_prefix: str) -> UserContext:
        ...
        if hasattr(self.config, 'environment') and self.config.environment == 'staging':
            ...
            if not token:
                token = self.jwt_helper.create_access_token(user_id, email, permissions=['agents:use', 'context:isolate', 'session:manage'])
        else:
            token = self.jwt_helper.create_access_token(user_id, email, permissions=['agents:use', 'context:isolate', 'session:manage'])
        ...
        user_context = UserContext(user_id=user_id, email=email, token=token, thread_id=str(uuid.uuid4()))
        user_context.session_data['ws_client'] = ws_client
        logger.info(f'Created user context: {user_id} ({email})')
        return user_context

    async def execute_context_isolation_scenario(self, scenario: Dict[str, Any], timeout: float=120.0) -> ContextIsolationValidation:
        ...
        """
        validation = ContextIsolationValidation(test_scenario=scenario['scenario_name'])
    ...
    async def _execute_concurrent_sessions(self, validation: ContextIsolationValidation, scenario: Dict[str, Any], timeout: float):
        ...
        async def execute_user_session(user_context: UserContext):
            ...
            agent_request = {'type': 'agent_request', 'agent': 'triage_agent', 'message': user_specific_message, 'thread_id': user_context.thread_id, 'context': {'user_id': user_context.user_id, 'isolation_test': True, 'user_data': f'confidential_data_for_{user_context.user_id}'}, 'optimistic_id': str(uuid.uuid4())}
        ...
        tasks = [execute_user_session(user_context) for user_context in validation.user_contexts]
        await asyncio.gather(*tasks, return_exceptions=True)
        logger.info(f'Completed concurrent execution for {len(validation.user_contexts)} users')

    async def _execute_sequential_handover(self, validation: ContextIsolationValidation, scenario: Dict[str, Any], timeout: float):
        ...
        for i, user_context in enumerate(validation.user_contexts):
            ...
            agent_request = {'type': 'agent_request', 'agent': 'triage_agent', 'message': handover_message, 'thread_id': user_context.thread_id, 'context': {'user_id': user_context.user_id, 'session_sequence': i + 1, 'previous_context_test': True}, 'optimistic_id': str(uuid.uuid4())}
            await ws_client.send_json(agent_request)
    ...
    async def _execute_concurrency_stress(self, validation: ContextIsolationValidation, scenario: Dict[str, Any], timeout: float):
        ...
        async def stress_user_session(user_context: UserContext, iteration: int):
            ...
            for request_num in range(2):
                ...
                agent_request = {'type': 'agent_request', 'agent': 'triage_agent', 'message': stress_message, 'thread_id': f'{user_context.thread_id}_{request_num}', 'context': {'user_id': user_context.user_id, 'stress_test': True, 'iteration': iteration, 'request_number': request_num, 'sensitive_data': f'stress_data_{user_context.user_id}_{request_num}'}, 'optimistic_id': str(uuid.uuid4())}
                await ws_client.send_json(agent_request)
                await asyncio.sleep(0.5)
            stress_start = time.time()
            event_count = 0
            while time.time() - stress_start < timeout and event_count < 20:
        ...
        stress_tasks = [stress_user_session(user_context, i) for i, user_context in enumerate(validation.user_contexts)]
    ...
    async def _execute_general_context_test(self, validation: ContextIsolationValidation, scenario: Dict[str, Any], timeout: float):
        """Execute general context isolation test."""
        await self._execute_concurrent_sessions(validation, scenario, timeout)

    def _validate_context_isolation(self, validation: ContextIsolationValidation, scenario: Dict[str, Any]):
        ...
        validation.all_users_isolated = users_with_events == len(user_ids) and (not validation.data_leakage_detected)
        validation.session_integrity_maintained = all((len(validation.user_event_types.get(user_id, set())) > 0 for user_id in user_ids))
        event_sequences = [tuple(validation.user_event_types.get(user_id, set())) for user_id in user_ids]
    ...
    def generate_context_management_report(self) -> str:
        ...
        if self.validations:
            isolation_scores = [sum([val.all_users_isolated, val.session_integrity_maintained, val.agent_state_separated, val.resource_isolation_verified, val.context_boundaries_maintained]) for val in self.validations]
...
class TestRealAgentContextManagement:
    ...
    async def test_concurrent_user_session_isolation(self, context_management_tester):
        ...
        validation = await context_management_tester.execute_context_isolation_scenario(scenario, timeout=90.0)
        assert not validation.data_leakage_detected, f'Data leakage detected between users: {validation.cross_user_contamination}'
    ...
    async def test_sequential_user_handover_boundaries(self, context_management_tester):
        ...
        scenario = context_management_tester.CONTEXT_ISOLATION_SCENARIOS[1]
        validation = await context_management_tester.execute_context_isolation_scenario(scenario, timeout=80.0)
    ...
    async def test_high_concurrency_stress_isolation(self, context_management_tester):
        ...
        scenario = context_management_tester.CONTEXT_ISOLATION_SCENARIOS[2]
        validation = await context_management_tester.execute_context_isolation_scenario(scenario, timeout=100.0)
        ...
        completed_users = sum((1 for user_id in validation.user_events.keys() if len(validation.user_events[user_id]) > 0))
        assert completed_users >= len(validation.user_contexts) * 0.8, f'Most users should complete under stress: {completed_users}/{len(validation.user_contexts)}'
    ...
    async def test_context_state_persistence_isolation(self, context_management_tester):
        ...
        scenario = context_management_tester.CONTEXT_ISOLATION_SCENARIOS[3]
        validation = await context_management_tester.execute_context_isolation_scenario(scenario, timeout=85.0)
    ...
    async def test_context_isolation_websocket_events(self, context_management_tester):
        ...
        scenario = context_management_tester.CONTEXT_ISOLATION_SCENARIOS[0]
        validation = await context_management_tester.execute_context_isolation_scenario(scenario, timeout=75.0)
    ...
    async def test_context_management_performance_benchmarks(self, context_management_tester):
        ...
        performance_results = []
        for scenario in context_management_tester.CONTEXT_ISOLATION_SCENARIOS[:2]:
            validation = await context_management_tester.execute_context_isolation_scenario(scenario, timeout=70.0)
    ...
    async def test_context_isolation_quality_metrics(self, context_management_tester):
        ...
        scenario = context_management_tester.CONTEXT_ISOLATION_SCENARIOS[0]
        validation = await context_management_tester.execute_context_isolation_scenario(scenario, timeout=80.0)
        isolation_score = sum([validation.all_users_isolated, validation.session_integrity_maintained, validation.agent_state_separated, validation.resource_isolation_verified, validation.context_boundaries_maintained])
    ...
    async def test_comprehensive_context_management_report(self, context_management_tester):
        ...
        for scenario in context_management_tester.CONTEXT_ISOLATION_SCENARIOS:
            await context_management_tester.execute_context_isolation_scenario(scenario, timeout=90.0)
        ...
        isolated_tests = sum((1 for v in context_management_tester.validations if v.all_users_isolated and (not v.data_leakage_detected)))
