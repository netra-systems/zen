...
class TestCrossServiceAuthSessionMgmt(SSotAsyncTestCase):
    ...
    async def test_session_creation_consistency_across_services(self):
        ...
        # Step 1: Create session through auth service
        ...
        # Step 2: Validate session recognized by backend API
        ...
        # Step 3: Validate session recognized by WebSocket service
        ...
        # Step 4: Cross-validate session consistency
    ...
    async def _validate_backend_session_recognition(self):
        ...
        # Test backend API session recognition

        async with self.http_session.get(

            f"{self.backend_api_url}/api/v1/user/profile",
            ...
            # Validate session consistency in backend response

            assert profile_data.get("session_id") == self.session_data["session_id"], \

                "Backend uses same session ID as auth service"
    ...
    async def _validate_websocket_session_recognition(self):
        ...
        try:
            ...
            # Wait for connection acknowledgment with session info
            ...
            # Validate WebSocket session recognition

            assert connection_data["type"] == "connection_established", "WebSocket connection established"

            assert connection_data.get("session_id") == self.session_data["session_id"], \

                "WebSocket uses same session ID as auth service"
    ...
    async def _validate_cross_service_session_consistency(self):
        ...
        # Get session info from auth service
        ...
        async with self.http_session.get(
            ...
            f"{self.auth_service_url}/auth/session/info",
        ...
        async with self.http_session.get(
            ...
            f"{self.backend_api_url}/api/v1/session/info",
        ...
        assert auth_session_info["session_id"] == backend_session_info["session_id"], \

            "Session ID consistent between auth service and backend"

        assert auth_session_info["user_id"] == backend_session_info["user_id"], \

            "User ID consistent between services"

        assert auth_session_info["expires_at"] == backend_session_info["expires_at"], \

            "Session expiration consistent between services"
    ...
    async def test_session_refresh_consistency_across_services(self):
        ...
        # Step 2: Refresh session through auth service
        ...
        # Step 3: Validate backend recognizes refreshed session
        ...
        # Step 4: Validate WebSocket recognizes refreshed session
    ...
    async def _refresh_auth_service_session(self):
        ...
        async with self.http_session.post(
            ...
            f"{self.auth_service_url}/auth/session/refresh",
    ...
    async def _validate_backend_recognizes_refreshed_session(self, old_session_id: str):
        ...
        # Test backend with refreshed session
        ...
        async with self.http_session.get(
            ...
            assert profile_data.get("session_id") == self.session_data["session_id"], \
                ...
                "Backend uses refreshed session ID"



        # Verify old session no longer valid in backend
    ...
    async def _validate_websocket_recognizes_refreshed_session(self):
        ...
        try:
            ...
            assert connection_data.get("session_id") == self.session_data["session_id"], \
                ...
                "WebSocket recognizes refreshed session"
    ...
    async def test_session_expiration_handling_across_services(self):
        ...
        # Step 1: Create session with short expiration (test environment)

        await self._create_short_lived_session()



        # Step 2: Wait for session expiration

        logger.info("Waiting for session expiration")

        await asyncio.sleep(5)  # Assuming short-lived session for testing



        # Step 3: Validate auth service rejects expired session

        await self._validate_auth_service_rejects_expired_session()



        # Step 4: Validate backend rejects expired session

        await self._validate_backend_rejects_expired_session()



        # Step 5: Validate WebSocket rejects expired session
    ...
    async def _create_short_lived_session(self):
        ...
        login_payload = {
            ...
            "session_duration": 3  # 3 seconds for testing
    ...
    async def _validate_websocket_rejects_expired_session(self):
        ...
        # WebSocket should reject connection with expired session
        ...
        with pytest.raises(websockets.exceptions.ConnectionClosedError):
            ...
            await websocket_connection.recv()  # Should fail



if __name__ == "__main__":

    # Run with: python -m pytest tests/e2e/golden_path_auth/test_cross_service_auth_session_mgmt.py -v
