...
class WebSocketDevConnectivityTests:
    ...
    """"
        ...
        try:
            ...
            raise AssertionError(fWebSocket connection failed: {str(e)})
        except Exception as e:
            raise AssertionError(fUnexpected WebSocket error: {str(e)}")"
    ...
    async def test_dev_websocket_message_echo(self):
        ...
        async with websockets.connect(ws_url) as websocket:
            ...
            print(fEcho test passed - payload preserved, server timestamp added)
        ...
        async with websockets.connect(ws_url) as websocket:
            ...
            for i, response in enumerate(received_responses):
                ...
                assert original.get(payload) == fMessage {i}, \
                    fMessage content not preserved for message {i}
            
            print(fMultiple messages handled correctly - {message_count} messages processed in order"")
