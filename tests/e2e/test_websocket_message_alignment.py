    ...
    async def simulate_frontend_validation(self, message: Dict[str, Any]) -> bool:
        ...
        if msg_type in agent_types:
            if 'payload' not in message or not isinstance(message.get('payload'), dict):
                self.parse_errors.append({
                    'error': f'Agent message missing payload: {msg_type}',
                    'message': message
                })
                return False
        ...
        if msg_type in thread_types:
            if 'payload' not in message or not isinstance(message.get('payload'), dict):
                self.parse_errors.append({
                    'error': f'Thread message missing payload: {msg_type}',
                    'message': message
                })
                return False
...
Testing Welcome Message Format:")
    ...
    if not success:
        ...
        print(f"Parse errors: {tester.parse_errors}")
...
async def test_data_to_payload_conversion():
    ...
    if success and tester.received_messages:
        ...
        print(f"WARNING: Message validation failed - this is expected before the fix")
        print(f"Errors: {tester.parse_errors}")
