...
def create_test_user() -> Dict:
    ...
    return {
        "id": user_id,
        "email": f"test_user_{user_id[:8]}@netra.ai",
        ...
        async with httpx.AsyncClient(follow_redirects=True) as client:
            ...
            for endpoint in auth_endpoints:
                try:
                    ...
                    print(f"Auth attempt failed for {endpoint}: {e}")
                    continue
            
            return None
            
    except Exception as e:
        print(f"Authentication completely failed: {e}")
        return None
...
class TestWebSocketConnectionEstablishment:
        ...
        try:
            async with httpx.AsyncClient(follow_redirects=True) as client:
                ...
                if response.status_code == 200:
                    ...
                    pytest.fail(f"WebSocket config endpoint returned {response.status_code}")
    ...
    async def test_backend_service_discovery(self):
        ...
        for service_name, status in service_status.items():
            if status["status"] == "available":
                print(f"  [OK] {service_name}: {status['status']} ({status.get('response_time', 0):.3f}s)")
            else:
                print(f"  [FAIL] {service_name}: {status['status']} - {status.get('error', 'Unknown error')}")
                all_available = False
    ...
    async def test_websocket_automatic_connection_after_login(self, test_user_with_token):
        ...
        try:
            ...
            assert connected, f"WebSocket should connect automatically after login. Events: {client.connection_events}"
    ...
    async def test_websocket_authentication_header_method(self, test_user_with_token):
        ...
        try:
            ...
            assert connected, f"Authorization header auth should work. Events: {client.connection_events}"
    ...
    async def test_websocket_authentication_subprotocol_method(self, test_user_with_token):
        ...
        try:
            ...
            assert connected, f"Subprotocol auth should work as backup. Events: {client.connection_events}"
    ...
    async def test_websocket_invalid_token_rejection(self):
        ...
        try:
            ...
            assert not connected, f"Invalid token should be rejected. Events: {client.connection_events}"
            
            # Check that proper error was logged
            auth_failed_events = [e for e in client.connection_events if e[0] == "auth_failed"]
            connection_errors = [e for e in client.connection_events if e[0] == "connection_error"]
    ...
    async def test_message_delivery_guarantees(self, test_user_with_token):
        ...
        try:
            ...
            for i in range(5):
                ...
                assert sent, f"Message {i} should be sent"
            ...
            assert len(responses) >= len(message_ids), \
                f"Should receive response to all {len(message_ids)} messages, got {len(responses)}"
    ...
    async def test_concurrent_connections_same_user(self, test_user_with_token):
        ...
        try:
            ...
            for i in range(3):
                ...
                assert connected, f"Connection {i} should be established for same user"
            ...
            for i, client in enumerate(clients[1:], 1):
                ...
                assert received is not None, \
                    f"Connection {i} should receive message sent to user"
...
class TestWebSocketReconnectionResilience:
    ...
    async def test_automatic_reconnection_after_network_interruption(self, test_user_with_token):
        ...
        try:
            ...
            assert reconnected, f"Should automatically reconnect after interruption. Events: {client.connection_events}"
    ...
    async def test_message_queue_persistence_during_disconnection(self, test_user_with_token):
        ...
        try:
            ...
            print(f"Received {len(queued_messages)} queued messages after reconnection")
...
class TestWebSocketErrorHandling:
        ...
        try:
            ...
            if client.websocket:
                try:
                    ...
                    print(f"Exception during malformed message test: {e}")
    ...
    async def test_rate_limiting_enforcement(self, test_user_with_token):
        ...
        try:
            ...
            if messages_sent >= 40:  # If we sent almost all messages
                print(f"WARNING: Rate limiting might not be working - sent {messages_sent} messages")
            ...
            print(f"Rate limiting status: Hit={rate_limit_hit}, Messages sent={messages_sent}")
    ...
    async def test_connection_cleanup_on_error(self, test_user_with_token):
        ...
        try:
            ...
            print(f"Surviving connections after errors: {surviving_clients}/{len(clients)}")
...
class TestWebSocketHeartbeatMonitoring:
    ...
    async def test_zombie_connection_detection(self, test_user_with_token):
        ...
        try:
                ...
                try:
                    ...
                    if message:
                        if message.get("type") in ["ping", "heartbeat"]:
                            ...
                            print(f"Ignoring ping/heartbeat {ping_count} (simulating zombie)")
                        else:
                            print(f"Received non-ping message: {message.get('type')}")
                    
                    # Check if connection was forcibly closed
                    if client.websocket and client.websocket.closed:
                        print(f"Connection closed after {ping_count} ignored pings")
                        break
            ...
            print(f"Zombie test results: {ping_count} pings ignored over {elapsed:.1f} seconds")
...
async def test_websocket_memory_leak_detection(test_user_with_token):
    ...
    for cycle in range(10):
        try:
            ...
            if connected:
                ...
                # Send a few messages
                for i in range(3):
                    await client.send_message("cycle_test", {
                        "cycle": cycle,
                        "message": i,
    ...
    print(f"  Successful: {connection_stats['successful_connections']}")
    print(f"  Failed: {connection_stats['failed_connections']}")
    print(f"  Errors: {len(connection_stats['connection_errors'])}")
    
    if connection_stats["connection_errors"]:
        print(f"  Sample errors: {connection_stats['connection_errors'][:3]}")
                ...
                for service_name, url in services.items():
                    try:
                        ...
                        if response.status_code == 200:
                            ...
                            print(f"   [ISSUE] {service_name}: Returns {response.status_code}")
                            issues_discovered.append(f"{service_name} service returns HTTP {response.status_code}")
                    except Exception:
                        print(f"   [ISSUE] {service_name}: Not available")
                        issues_discovered.append(f"{service_name} service is not running or not accessible")
