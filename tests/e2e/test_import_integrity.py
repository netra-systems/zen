...
"):
            ...
            self.warnings.setdefault("circular_analysis_warnings", []).append(
                f"Could not analyze imports for {module_name}: {e}"
            )
    ...
    def attempt_import(self, file_path: Path) -> Tuple[bool, Optional[str], Optional[str]]:
        ...
        try:
            ...
            if spec is None or spec.loader is None:
                return False, "ModuleSpecError", f"Could not create module spec for {module_name}"
            ...
            if (exception_name == "Skipped" or 
                ...
                return True, None, f"Skipped test module: {str(e)}"
    ...
    def run_comprehensive_validation(self) -> Dict[str, Any]:
        ...
        print(f"Found {self.total_files} Python files to validate")
...
"
            ...
            for module_name, error_info in import_failures.items():
                ...
                error_groups.setdefault(error_type, []).append(
                    f"  {module_name}: {error_info['error_message']}"
                )
...
"
    ...
    pass
        ...
        if elapsed_time > max_allowed_time:
            pytest.fail(
                f"Import validation took {elapsed_time}s, exceeding {max_allowed_time}s limit. "
    ...
    def test_minimum_success_rate(self, validation_results):
        ...
        if success_rate < minimum_success_rate:
            pytest.fail(
                f"Import success rate {success_rate}% is below minimum {minimum_success_rate}%. "
    ...
    def test_validation_report_generation(self, validation_results):
        ...
        if missing_keys:
            pytest.fail(f"Validation report missing required keys: {missing_keys}")
        ...
        if successful + failed != total_files:
            pytest.fail(
                f"Data integrity error: successful ({successful}) + failed ({failed}) "
...
" + "=" * 80)
        ...
        print(f"Missing packages: {validation_results['missing_packages']}")
        print(f"Circular dependencies: {validation_results['circular_dependencies']}")
