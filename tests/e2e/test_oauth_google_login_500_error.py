...
class TestOAuthGoogleLogin500Error:
    ...
    async def wait_for_auth_service(self, auth_service_url: str, max_retries: int = 30) -> bool:
            ...
            for i in range(max_retries):
                try:
                    ...
                    if response.status_code == 200:
                        logger.info(f"Auth service is ready at {auth_service_url}")
                        return True
        ...
        logger.error(f"Auth service not ready after {max_retries} seconds")
        return False
    
    @pytest.mark.asyncio
    async def test_google_oauth_login_without_credentials_returns_500(self, auth_service_url: str):
        """
                    ...
                    if isinstance(response_data, dict):
                        if "detail" in response_data:
                            ...
                            if isinstance(detail, dict) and "error" in detail:
                                ...
                                logger.info(f"500 error with simple detail: {detail}")
    ...
    async def test_google_oauth_login_with_placeholder_credentials_returns_500(self, auth_service_url: str):
        ...
        try:
            async with httpx.AsyncClient(follow_redirects=False) as client:
                ...
                assert response.status_code == 500, f"Expected 500 error for placeholders, got {response.status_code}"
                
                try:
                    response_data = response.json()
                    logger.info(f"Placeholder credentials correctly rejected with response: {response_data}")
                    ...
                    logger.warning(f"Could not fully parse placeholder error response: {e}")
    ...
    @pytest.mark.asyncio
    async def test_google_oauth_login_with_valid_credentials_succeeds(self, auth_service_url: str):
        """
        ...
        try:
            ...
            async with httpx.AsyncClient(follow_redirects=False) as client:
                ...
                logger.info(f"Valid credentials allow OAuth flow to proceed: redirect to {location[:100]}...")
