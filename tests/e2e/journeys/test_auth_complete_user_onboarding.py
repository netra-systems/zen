...
class UserOnboardingJourneyValidator:
    ...
    def start_journey(self, user_info: Dict[str, Any]):
        ...
        self.journey_steps.append({'step': 'journey_started', 'timestamp': self.journey_start_time, 'user_info': user_info, 'status': 'started'})

    def record_step(self, step_name: str, status: str, details: Optional[Dict]=None):
        """Record a step in the onboarding journey."""
        step_record = {'step': step_name, 'timestamp': time.time(), 'status': status, 'details': details or {}, 'duration_from_start': time.time() - (self.journey_start_time or time.time())}
    ...
    def complete_journey(self):
        ...
        self.journey_steps.append({'step': 'journey_completed', 'timestamp': self.journey_end_time, 'total_duration': total_duration, 'status': 'completed'})
        return total_duration

    def validate_journey_success(self) -> Dict[str, Any]:
        """Validate that onboarding journey was successful."""
        validation = {'journey_completed': False, 'all_steps_successful': True, 'total_duration': 0, 'failed_steps': [], 'business_impact': '', 'conversion_points': {}}
        ...
        elif validation['total_duration'] > 120:
...
class TestCompleteUserOnboardingJourney(BaseE2ETest):
    ...
    async def test_new_user_complete_onboarding_journey(self):
        ...
        user_info = {'email': user_email, 'name': user_name, 'timestamp': timestamp}
        ...
        try:
            ...
            async with httpx.AsyncClient() as client:
                registration_data = {'email': user_email, 'password': password, 'full_name': user_name}
                register_response = await client.post(f'{self.auth_url}/auth/register', json=registration_data, timeout=10.0)
                if register_response.status_code in [200, 201]:
                    ...
                    self.validator.record_step('registration_completed', 'success', {'user_id': user_data.get('id'), 'has_token': bool(access_token)})
                    logger.info(f' PASS:  Registration successful for {user_email}')
                else:
                    error_text = register_response.text
                    self.validator.record_step('registration_completed', 'failed', {'status_code': register_response.status_code, 'error': error_text})
            ...
            try:
                ...
                async with httpx.AsyncClient() as client:
                    profile_response = await client.get(f'{self.backend_url}/api/user/profile', headers=validation_headers, timeout=5.0)
                    if profile_response.status_code == 200:
                        profile_data = profile_response.json()
                        assert profile_data.get('email') == user_email, 'Profile data must match registered user'
                        self.validator.record_step('token_validation_completed', 'success', {'profile_accessible': True})
                    else:
                        self.validator.record_step('token_validation_completed', 'failed', {'status_code': profile_response.status_code})
            ...
            try:
                ...
                async with websockets.connect(websocket_url, additional_headers=self.ws_auth_helper.get_websocket_headers(access_token), open_timeout=10.0) as websocket:
                    connection_message = {'type': 'connection_verify', 'user_email': user_email, 'timestamp': datetime.now().isoformat()}
                    await websocket.send(json.dumps(connection_message))
                    response = await asyncio.wait_for(websocket.recv(), timeout=5.0)
                    response_data = json.loads(response)
                    self.validator.record_step('chat_access_completed', 'success', {'websocket_connected': True, 'response_type': response_data.get('type')})
                    ...
                    first_message = {'type': 'chat_message', 'content': 'Hello! This is my first message. Can you help me get started?', 'user_email': user_email, 'thread_id': f'onboarding-{uuid.uuid4().hex[:8]}'}
                    await websocket.send(json.dumps(first_message))
                    self.validator.record_step('first_chat_message_completed', 'success', {'message_sent': True})
                    ...
                    try:
                        ...
                        while time.time() - start_time < response_timeout:
                            ...
                            if response_data.get('type') == 'agent_completed':
                                ...
                                if agent_result:
                                    self.validator.record_step('first_agent_response_completed', 'success', {'agent_responded': True, 'response_length': len(str(agent_result)), 'agent_events': agent_events, 'response_time': time.time() - start_time})
                ...
                self.validator.record_step('chat_access_completed', 'failed', {'error': str(e)})
            ...
            if validation['total_duration'] > 60:
    ...
    async def test_returning_user_login_to_chat_journey(self):
        ...
        user_token = self.auth_helper.create_test_jwt_token(user_id=f'returning-user-{timestamp}', email=user_email, permissions=['chat', 'agent:execute'])
        user_info = {'email': user_email, 'returning_user': True, 'timestamp': timestamp}
        self.validator.start_journey(user_info)
        try:
            ...
            self.validator.record_step('login_completed', 'success', {'has_token': bool(login_token)})
            ...
            try:
                ...
                async with websockets.connect(websocket_url, additional_headers=self.ws_auth_helper.get_websocket_headers(login_token), open_timeout=5.0) as websocket:
                    welcome_message = {'type': 'chat_message', 'content': "I'm back! Can you help me continue where I left off?", 'user_email': user_email, 'returning_user': True}
                    await websocket.send(json.dumps(welcome_message))
                    response = await asyncio.wait_for(websocket.recv(), timeout=3.0)
                    response_data = json.loads(response)
                    self.validator.record_step('returning_chat_access_completed', 'success', {'immediate_access': True, 'response_received': True})
            except Exception as e:
                self.validator.record_step('returning_chat_access_completed', 'failed', {'error': str(e)})
...
class TestOnboardingEdgeCases(BaseE2ETest):
    ...
    async def test_duplicate_registration_handling(self):
        ...
        registration_data = {'email': user_email, 'password': password, 'full_name': f'Duplicate Test User {timestamp}'}
        async with httpx.AsyncClient() as client:
            first_response = await client.post(f"{self.services['auth_url']}/auth/register", json=registration_data, timeout=10.0)
            assert first_response.status_code in [200, 201], 'First registration should succeed'
            second_response = await client.post(f"{self.services['auth_url']}/auth/register", json=registration_data, timeout=10.0)
            assert second_response.status_code in [200, 201, 400, 409], f'Duplicate registration should be handled gracefully, got {second_response.status_code}'
            if second_response.status_code in [400, 409]:
                error_data = second_response.json()
                assert 'email' in error_data.get('detail', '').lower() or 'exists' in error_data.get('detail', '').lower(), 'Error message should clearly indicate duplicate email'
    ...
    async def test_network_interruption_recovery(self):
        ...
        user_token = self.auth_helper.create_test_jwt_token(user_id=f'network-user-{timestamp}', email=user_email)
