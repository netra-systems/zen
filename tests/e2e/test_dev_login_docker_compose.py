...
class TestDevLoginDockerCompose:
    ...
    pass
        try:
            ...
            pytest.skip(f"Docker not available or services failed to start: {e}")
...
print("✓ Database URL correctly formatted")
        ...
        assert "✓ Database URL correctly formatted" in result.stdout
...
print(f"Connection to dev-postgres:5432: {'Success' if result==0 else f'Failed ({result})'}")
             """],
            capture_output=True,
            text=True
        )
...
print(f"Connection to dev-redis:6379: {'Success' if result==0 else f'Failed ({result})'}")
             """],
            capture_output=True,
            text=True
        )
