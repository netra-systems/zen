...
class TestFrontendConnectivity404Reproduction(BaseE2ETest):
    ...
    def setUp(self):
        ...
        self.critical_frontend_endpoints = [{'endpoint': '/api/agent/v2/execute', 'method': 'POST', 'description': 'Agent execution v2 API', 'payload': {'type': 'triage', 'message': 'Test agent execution from frontend', 'context': {'source': 'e2e_test'}}}, {'endpoint': '/api/agents/v2/execute', 'method': 'POST', 'description': 'Alternative agent execution v2 API', 'payload': {'type': 'data', 'message': 'Test data agent from frontend', 'context': {'source': 'e2e_test'}}}, {'endpoint': '/api/threads/{thread_id}/messages', 'method': 'GET', 'description': 'Thread messages retrieval', 'payload': None}, {'endpoint': '/api/threads/{thread_id}/messages', 'method': 'POST', 'description': 'Thread message creation', 'payload': {'content': 'Test message from frontend', 'role': 'user'}}]
    ...
    async def test_agent_v2_execute_404_reproduction(self, real_services):
        ...
        """
        user_context = await create_authenticated_user_context(user_email='e2e_404_test@example.com', environment=self.test_environment, websocket_enabled=True)
        ...
        test_payload = {'type': 'triage', 'message': 'E2E test: Reproduce 404 error on agent execution', 'context': {'source': 'frontend_connectivity_test', 'user_id': str(user_context.user_id), 'thread_id': str(user_context.thread_id), 'test_environment': self.test_environment}}
        async with aiohttp.ClientSession() as session:
            try:
                ...
                async with session.post(endpoint_url, json=test_payload, headers=headers, timeout=30) as response:
                    response_text = await response.text()
                    print(f' CHART:  Response status: {response.status}')
                    print(f'[U+1F4C4] Response: {response_text[:500]}...')
                    self.assertNotEqual(response.status, 404, f'CRITICAL 404 ERROR REPRODUCED: {endpoint_url} returned 404. This is the exact error users experience when frontend tries to execute agents. Environment: {self.test_environment}. Response: {response_text}')
                    if response.status not in [200, 201]:
                        self.fail(f'AGENT EXECUTION ENDPOINT ERROR: {endpoint_url} returned {response.status}. Users cannot execute agents. Response: {response_text}')
                    try:
                        response_data = await response.json()
                        required_fields = ['id', 'status']
                        missing_fields = [f for f in required_fields if f not in response_data]
                        if missing_fields:
                            self.fail(f'RESPONSE FORMAT ERROR: Missing fields {missing_fields}. Frontend expects: {required_fields}')
                    except json.JSONDecodeError:
                        self.fail(f'INVALID JSON RESPONSE: {response_text}')
            except aiohttp.ClientError as e:
                self.fail(f'CONNECTION ERROR to {endpoint_url}: {e}. Users cannot connect to backend from frontend. Environment: {self.test_environment}')
    ...
    async def test_thread_messages_404_reproduction(self, real_services):
        ...
        EXPECTED RESULT: This test should FAIL with 404, demonstrating the issue.
        """
        user_context = await create_authenticated_user_context(user_email='e2e_messages_404_test@example.com', environment=self.test_environment, websocket_enabled=True)
        jwt_token = user_context.agent_context['jwt_token']
        headers = self.auth_helper.get_auth_headers(jwt_token)
        ...
        async with aiohttp.ClientSession() as session:
            try:
                ...
                async with session.get(get_endpoint_url, headers=headers, timeout=30) as response:
                    response_text = await response.text()
                    print(f' CHART:  GET Response status: {response.status}')
                    print(f'[U+1F4C4] GET Response: {response_text[:500]}...')
                    self.assertNotEqual(response.status, 404, f'CRITICAL 404 ERROR REPRODUCED: GET {get_endpoint_url} returned 404. This is the exact error users experience when frontend tries to retrieve messages. Environment: {self.test_environment}. Thread ID: {test_thread_id}. Response: {response_text}')
                    if response.status not in [200]:
                        self.fail(f'THREAD MESSAGES GET ERROR: {get_endpoint_url} returned {response.status}. Users cannot retrieve conversation history. Response: {response_text}')
            except aiohttp.ClientError as e:
                self.fail(f'CONNECTION ERROR to {get_endpoint_url}: {e}. Users cannot retrieve messages from frontend.')
        post_endpoint_url = f'{self.backend_url}/api/threads/{test_thread_id}/messages'
        test_message_payload = {'content': 'E2E test: Reproduce 404 error on message creation', 'role': 'user', 'metadata': {'source': 'frontend_connectivity_test', 'test_environment': self.test_environment}}
        async with aiohttp.ClientSession() as session:
            try:
                ...
                async with session.post(post_endpoint_url, json=test_message_payload, headers=headers, timeout=30) as response:
                    response_text = await response.text()
                    print(f' CHART:  POST Response status: {response.status}')
                    print(f'[U+1F4C4] POST Response: {response_text[:500]}...')
                    self.assertNotEqual(response.status, 404, f'CRITICAL 404 ERROR REPRODUCED: POST {post_endpoint_url} returned 404. This is the exact error users experience when frontend tries to send messages. Environment: {self.test_environment}. Thread ID: {test_thread_id}. Response: {response_text}')
                    if response.status not in [200, 201]:
                        self.fail(f'THREAD MESSAGE POST ERROR: {post_endpoint_url} returned {response.status}. Users cannot send messages. Response: {response_text}')
            except aiohttp.ClientError as e:
                self.fail(f'CONNECTION ERROR to {post_endpoint_url}: {e}. Users cannot send messages from frontend.')
    ...
    async def test_websocket_connectivity_with_real_auth(self, real_services):
        ...
        """
        user_context = await create_authenticated_user_context(user_email='e2e_websocket_test@example.com', environment=self.test_environment, websocket_enabled=True)
        try:
            ...
            test_message = {'type': 'agent_request', 'agent': 'triage', 'message': 'E2E test: Reproduce WebSocket agent execution', 'context': {'user_id': str(user_context.user_id), 'thread_id': str(user_context.thread_id), 'test_environment': self.test_environment, 'source': 'frontend_websocket_test'}, 'timestamp': datetime.now(timezone.utc).isoformat()}
            ...
            try:
                ...
                while len(events_received) < 10:
                    try:
                        event_json = await asyncio.wait_for(websocket.recv(), timeout=timeout_duration)
                ...
                self.fail(f'WEBSOCKET EVENT ERROR: Failed to receive agent events: {e}. Users cannot see agent progress. Events received: {len(events_received)}')
            finally:
            ...
            if missing_critical_events:
                self.fail(f'CRITICAL WEBSOCKET EVENTS MISSING: {missing_critical_events}. Users cannot see agent progress. Received events: {event_types}. Environment: {self.test_environment}')
            print(f' PASS:  WebSocket connectivity successful. Events received: {event_types}')
        except Exception as e:
            self.fail(f'WEBSOCKET CONNECTION FAILURE: {e}. Users cannot connect to backend via WebSocket. Environment: {self.test_environment}. URL: {self.websocket_url}')
    ...
    async def test_comprehensive_404_reproduction(self, real_services):
        ...
        """
        user_context = await create_authenticated_user_context(user_email='e2e_comprehensive_test@example.com', environment=self.test_environment, websocket_enabled=True)
        jwt_token = user_context.agent_context['jwt_token']
        headers = self.auth_helper.get_auth_headers(jwt_token)
        test_thread_id = str(user_context.thread_id)
        failed_endpoints = []
                ...
                try:
                    ...
                    if method == 'GET':
                        async with session.get(endpoint_url, headers=headers, timeout=30) as response:
                            result = await self._process_endpoint_response(endpoint_url, method, description, response)
                    elif method == 'POST':
        ...
        if failed_endpoints:
            failure_message = f'COMPREHENSIVE 404 REPRODUCTION - FRONTEND CONNECTIVITY FAILURES:\nEnvironment: {self.test_environment}\nFailed endpoints ({len(failed_endpoints)}):\n' + '\n'.join((f'   FAIL:  {failure}' for failure in failed_endpoints))
            if successful_endpoints:
                failure_message += f'\n\nWorking endpoints ({len(successful_endpoints)}):\n' + '\n'.join((f'   PASS:  {success}' for success in successful_endpoints))
            failure_message += f'\n\nThis demonstrates the complete scope of 404 errors affecting user experience. Users cannot access critical frontend functionality due to missing backend endpoints.'
    ...
    async def _process_endpoint_response(self, url: str, method: str, description: str, response) -> Dict[str, Any]:
        ...
        if response.status == 404:
            return {'success': False, 'error': f'404 ERROR - {description} endpoint missing'}
        if response.status not in [200, 201]:
            return {'success': False, 'error': f'{response.status} ERROR - {response_text[:200]}'}
