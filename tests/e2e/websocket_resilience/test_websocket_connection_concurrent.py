...
async def test_concurrent_expired_token_connections(jwt_generator, audit_logger):
    ...
    expired_tokens = [jwt_generator.create_expired_token(f'concurrent_user_{i}', expired_minutes_ago=5) for i in range(concurrent_users)]
    ...
    async def attempt_connection_with_expired_token(user_index: int) -> List[Dict]:
        ...
        for attempt in range(attempts_per_user):
            ...
            headers = {'X-Forwarded-For': f'10.0.{user_index // 256}.{user_index % 256}', 'User-Agent': f'ConcurrentClient/{user_index}.{attempt}', 'X-User-Index': str(user_index), 'X-Attempt': str(attempt)}
            start_time = time.time()
            success, result = await client.attempt_connection(headers)
            response_time = time.time() - start_time
            user_results.append({'user_index': user_index, 'attempt': attempt, 'success': success, 'response_time': response_time, 'result': result})
            await client.disconnect()
            await asyncio.sleep(0.05)
        return user_results
    logger.info(f'Starting {concurrent_users} concurrent users with {attempts_per_user} attempts each')
    start_time = time.time()
    concurrent_tasks = [attempt_connection_with_expired_token(i) for i in range(concurrent_users)]
...
async def test_mixed_token_concurrent_connections(jwt_generator, audit_logger):
    ...
    for i in range(valid_users):
        ...
        test_tokens.append({'user_id': user_id, 'token': token, 'expected_success': True, 'token_type': 'valid'})
    for i in range(expired_users):
        ...
        test_tokens.append({'user_id': user_id, 'token': token, 'expected_success': False, 'token_type': 'expired'})
    import random
    ...
    async def attempt_mixed_connection(token_info: Dict, index: int) -> Dict[str, Any]:
        ...
        client = SecureWebSocketTestClient('ws://mock-server/ws', token_info['token'], jwt_generator, audit_logger)
        headers = {'X-Forwarded-For': f'172.16.{index // 256}.{index % 256}', 'User-Agent': f'MixedTestClient/{index}.0', 'X-Token-Type': token_info['token_type']}
        start_time = time.time()
        success, result = await client.attempt_connection(headers)
        response_time = time.time() - start_time
        await client.disconnect()
        return {'user_id': token_info['user_id'], 'token_type': token_info['token_type'], 'expected_success': token_info['expected_success'], 'actual_success': success, 'response_time': response_time, 'result': result}
    logger.info(f'Starting {total_users} mixed connections ({valid_users} valid, {expired_users} expired)')
    mixed_start_time = time.time()
    mixed_tasks = [attempt_mixed_connection(token_info, i) for i, token_info in enumerate(test_tokens)]
...
async def test_sustained_concurrent_load_stability(jwt_generator, audit_logger):
    ...
    for batch_num in range(total_batches):
        ...
        batch_tokens = [jwt_generator.create_expired_token(f'sustained_user_{batch_num}_{i}', expired_minutes_ago=1) for i in range(batch_size)]
        ...
        async def sustained_connection_attempt(batch_idx: int, token_idx: int) -> Dict[str, Any]:
            ...
            client = SecureWebSocketTestClient('ws://mock-server/ws', token, jwt_generator, audit_logger)
            headers = {'X-Forwarded-For': f'198.51.100.{(batch_idx * batch_size + token_idx) % 255}', 'User-Agent': f'SustainedClient/{batch_idx}.{token_idx}', 'X-Batch': str(batch_idx), 'X-Token-Index': str(token_idx)}
            attempt_start = time.time()
            success, result = await client.attempt_connection(headers)
            attempt_time = time.time() - attempt_start
            await client.disconnect()
            return {'batch': batch_idx, 'token_index': token_idx, 'success': success, 'response_time': attempt_time, 'batch_time': batch_start}
        batch_tasks = [sustained_connection_attempt(batch_num, i) for i in range(batch_size)]
