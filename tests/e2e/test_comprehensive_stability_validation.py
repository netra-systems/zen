...
class StabilityMetrics:
    ...
    def get_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "recovery_rate_percent": self.recovery_rate,
...
class SystemStabilityValidator:
    ...
    async def validate_circuit_breaker_resilience(
        ...
        try:
            ...
            # Phase 1: Normal operations (circuit should remain closed)
            ...
            for i in range(config.failure_threshold + 1):
                ...
                try:
                    ...
                    if "circuit breaker" not in str(e).lower():
                        ...
                        # Expected application failure, not circuit breaker rejection
            ...
            # Phase 3: Test rejection behavior in open state
            ...
            # Phase 6: Verify stability after recovery
    ...
    async def validate_service_startup_resilience(

        self,

        service_name: str,

        startup_simulator: Callable,

        health_check_simulator: Callable,

        dependency_checks: List[Callable] = None
        ...
        try:
            ...
            # Phase 1: Normal startup sequence
            ...
            # Phase 4: Restart resilience testing
            ...
            for restart_attempt in range(2):
                ...
                await asyncio.sleep(0.5)  # Delay between restarts
    ...
    async def validate_system_recovery_patterns(
        ...
        try:
            ...
            # Phase 3: Test degraded operation
            ...
            # Phase 5: Validate full recovery

            logger.info(f"Phase 5: Validating full recovery for {recovery_scenario}")

            await asyncio.sleep(1.0)  # Allow time for full recovery
            ...
            logger.info(f"Recovery validation: {recovery_checks}/{len(validation_checks)} checks passed "
    ...
    def get_comprehensive_report(self) -> Dict[str, Any]:
        ...
        if report["summary"]["overall_success_rate"] < 95:
            ...
            report["recommendations"].append(
                ...
                "Overall success rate is below 95%. Consider reviewing failure handling mechanisms."
        ...
        if report["summary"]["average_response_time"] > 1000:
            ...
            report["recommendations"].append(
                ...
                "Average response time exceeds 1 second. Consider optimizing performance."
        ...
        if report["summary"]["total_circuit_breaker_trips"] == 0:
            ...
            report["recommendations"].append(
                ...
                "No circuit breaker activations detected. Verify circuit breaker configuration."
...
class TestComprehensiveStabilityValidation:
    ...
    async def test_database_circuit_breaker_resilience(self, stability_validator):
        ...
        async def failure_operation():
            ...
            await asyncio.sleep(0.02)  # Simulate attempt latency

            raise ConnectionError("Database connection failed")

        

        metrics = await stability_validator.validate_circuit_breaker_resilience(

            circuit_name="test_database",

            failure_simulator=failure_operation,

            success_simulator=success_operation,
        ...
        # Circuit breaker tests expect some failures as part of the testing pattern

        # The success rate should be reasonable considering we're testing failure scenarios

        assert metrics.success_rate >= 30, f"Success rate too low: {metrics.success_rate}%"

        

        # Recovery attempts should be made even if not all succeed

        assert metrics.recovery_attempts > 0, f"Should have attempted recovery: {metrics.recovery_attempts}"

        

        # Max consecutive failures should be reasonable for circuit breaker testing
    ...
    async def test_auth_service_startup_resilience(self, stability_validator):
        ...
        metrics = await stability_validator.validate_service_startup_resilience(

            service_name="auth_service",

            startup_simulator=startup_operation,

            health_check_simulator=health_check_operation,

            dependency_checks=[database_dependency_check, redis_dependency_check]

        )

        

        # Validate startup resilience
    ...
    async def test_system_wide_recovery_patterns(self, stability_validator):
        ...
        metrics = await stability_validator.validate_system_recovery_patterns(

            recovery_scenario="system_wide_failure",

            failure_simulator=failure_simulation,

            recovery_simulator=recovery_simulation,

            validation_checks=[service_health_check, database_health_check]
    ...
    async def test_concurrent_stress_stability(self, stability_validator):
        ...
        async def mixed_load_operation(operation_id: int):
            ...
            # 20% failure rate to test resilience
        ...
        metrics = await stability_validator.validate_circuit_breaker_resilience(
            ...
            failure_simulator=lambda: mixed_load_operation(0),  # Always fails
    ...
    async def test_comprehensive_stability_report(self, stability_validator):
        ...
        await stability_validator.validate_circuit_breaker_resilience(

            circuit_name="report_test",

            failure_simulator=quick_failure,

            success_simulator=quick_success
