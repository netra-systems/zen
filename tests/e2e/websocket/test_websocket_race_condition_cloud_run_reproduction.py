...
from netra_backend.app.websocket_core.gcp_initialization_validator import GCPWebSocketInitializationValidator, GCPReadinessState, create_gcp_websocket_validator, gcp_websocket_readiness_check
...
class WebSocketRaceConditionTestFramework:
    ...
    CLOUD_RUN_LATENCY_PROFILES = {'fast': CloudRunLatencyProfile(name='fast', base_delay_ms=50, jitter_ms=25, connection_timeout_ms=5000, auth_delay_ms=100, description='Fast Cloud Run instance (optimal conditions)'), 'typical': CloudRunLatencyProfile(name='typical', base_delay_ms=150, jitter_ms=50, connection_timeout_ms=8000, auth_delay_ms=200, description='Typical Cloud Run instance (normal conditions)'), 'slow': CloudRunLatencyProfile(name='slow', base_delay_ms=300, jitter_ms=100, connection_timeout_ms=12000, auth_delay_ms=400, description='Slow Cloud Run instance (high load conditions)'), 'stress': CloudRunLatencyProfile(name='stress', base_delay_ms=500, jitter_ms=200, connection_timeout_ms=15000, auth_delay_ms=600, description='Stressed Cloud Run instance (extreme conditions)')}
    ...
    def __init__(self, auth_helper: E2EWebSocketAuthHelper, environment: str='test'):
        ...
        self.race_condition_patterns = ['need to call accept first', 'connection closed before', 'server error', 'timeout', '502 bad gateway', '503 service unavailable', '1011']
    ...
    async def test_websocket_connection_with_latency(self, profile: CloudRunLatencyProfile, test_name: str='latency_test') -> RaceConditionTestResult:
        """
        ...
        try:
            ...
            websocket = await self.auth_helper.connect_authenticated_websocket(timeout=connection_timeout)
            connection_time = time.time() - start_time
            test_message = {'type': 'test_race_condition', 'profile': profile.name, 'timestamp': datetime.now(timezone.utc).isoformat(), 'test_name': test_name}
            ...
            while time.time() < event_collection_timeout:
                try:
                    ...
                    continue
            return RaceConditionTestResult(test_name=test_name, success=True, race_condition_detected=False, race_condition_type=None, connection_time=connection_time, error_message=None, websocket_events_received=events_received, auth_method='jwt_with_e2e_headers', environment=self.environment, latency_profile=profile.name, additional_details={'simulated_delay': profile.get_simulated_delay(), 'events_count': len(events_received)})
        except Exception as e:
            connection_time = time.time() - start_time
            error_message = str(e)
            is_race_condition, race_type = self.detect_race_condition(error_message)
            return RaceConditionTestResult(test_name=test_name, success=False, race_condition_detected=is_race_condition, race_condition_type=race_type, connection_time=connection_time, error_message=error_message, websocket_events_received=events_received, auth_method='jwt_with_e2e_headers', environment=self.environment, latency_profile=profile.name, additional_details={'simulated_delay': profile.get_simulated_delay(), 'exception_type': type(e).__name__})
        finally:
            if websocket:
                try:
    ...
    async def test_concurrent_connections_under_latency(self, profile: CloudRunLatencyProfile, concurrent_count: int=3) -> List[RaceConditionTestResult]:
        """
        ...
        async def single_connection_test(connection_id: int) -> RaceConditionTestResult:
            return await self.test_websocket_connection_with_latency(profile=profile, test_name=f'concurrent_{connection_id}')
        ...
        for i, task in enumerate(tasks):
            ...
            try:
                ...
                error_result = RaceConditionTestResult(test_name=f'concurrent_{i}', success=False, race_condition_detected=True, race_condition_type=RaceConditionType.CONCURRENT_CONFLICT, connection_time=0.0, error_message=str(e), websocket_events_received=[], auth_method='jwt_with_e2e_headers', environment=self.environment, latency_profile=profile.name)
                results.append(error_result)
        return results

    def validate_websocket_events(self, received_events: List[str], expected_events: Optional[List[str]]=None) -> WebSocketEventValidationResult:
        """
        ...
        return WebSocketEventValidationResult(expected_events=expected_events, received_events=received_events, missing_events=missing_events, unexpected_events=unexpected_events, event_timing=event_timing, validation_success=validation_success)
...
class WebSocketRaceConditionCloudRunReproductionTests(SSotBaseTestCase):
    ...
    def setup_method(self, method):
        ...
        self.race_framework = WebSocketRaceConditionTestFramework(auth_helper=self.websocket_auth_helper, environment=self.test_environment)
    ...
    async def test_reproduce_accept_first_race_condition(self):
        ...
        user_context = await create_authenticated_user_context(environment=self.test_environment, websocket_enabled=True)
        ...
        for profile_name in latency_profiles:
            ...
            with patch.dict('os.environ', {'ENVIRONMENT': 'staging', 'K_SERVICE': 'netra-backend', 'GCP_PROJECT': 'netra-staging'}):
                result = await self.race_framework.test_websocket_connection_with_latency(profile=profile, test_name=f'accept_first_race_{profile_name}')
    ...
    async def test_progressive_cloud_run_latency_simulation(self):
        ...
        for profile_name in profile_names:
            ...
            with patch.dict('os.environ', {'ENVIRONMENT': 'staging', 'K_SERVICE': 'netra-backend', 'CLOUD_RUN_SERVICE': 'netra-backend', 'PORT': '8080'}):
                result = await self.race_framework.test_websocket_connection_with_latency(profile=profile, test_name=f'progressive_latency_{profile_name}')
    ...
    async def test_concurrent_websocket_connections_with_race_detection(self):
        ...
        for profile_name in test_profiles:
            ...
            with patch.dict('os.environ', {'ENVIRONMENT': 'staging', 'K_SERVICE': 'netra-backend'}):
                concurrent_results = await self.race_framework.test_concurrent_connections_under_latency(profile=profile, concurrent_count=concurrent_count)
        ...
        catastrophic_races = [r for r in total_race_conditions if r.race_condition_type in [RaceConditionType.ACCEPT_FIRST_ERROR, RaceConditionType.SERVICE_NOT_READY]]
    ...
    async def test_websocket_event_delivery_under_race_conditions(self):
        ...
        user_context = await create_authenticated_user_context(environment=self.test_environment, websocket_enabled=True)
        test_scenarios = [('optimal', 'fast'), ('normal', 'typical'), ('degraded', 'slow')]
        ...
        for scenario_name, profile_name in test_scenarios:
            ...
            with patch.dict('os.environ', {'ENVIRONMENT': 'staging', 'K_SERVICE': 'netra-backend'}):
                ...
                try:
                    ...
                    websocket = await self.websocket_auth_helper.connect_authenticated_websocket(timeout=self.base_timeout)
                    agent_trigger_message = {'type': 'chat_message', 'content': f'Test agent execution for {scenario_name} conditions', 'user_id': str(user_context.user_id), 'thread_id': str(user_context.thread_id), 'timestamp': datetime.now(timezone.utc).isoformat()}
                    ...
                    event_delivery_results.append({'scenario': scenario_name, 'profile': profile_name, 'success': True, 'events_received': events_received, 'validation': event_validation, 'error': None})
                    ...
                    event_delivery_results.append({'scenario': scenario_name, 'profile': profile_name, 'success': False, 'events_received': events_received, 'validation': None, 'error': error_message, 'race_condition': race_detected, 'race_type': race_type.value if race_type else None})
        ...
        for optimal_result in optimal_results:
            ...
            assert validation.validation_success or len(validation.missing_events) <= 2, f'Too many missing critical events in optimal conditions: {validation.missing_events}'
    ...
    async def test_race_condition_recovery_and_reconnection(self):
        ...
        recovery_scenarios = [{'name': 'immediate_reconnect', 'initial_profile': 'stress', 'recovery_profile': 'typical', 'delay_between': 0.5}, {'name': 'delayed_reconnect', 'initial_profile': 'slow', 'recovery_profile': 'fast', 'delay_between': 2.0}]
        recovery_results = []
        for scenario in recovery_scenarios:
            ...
            with patch.dict('os.environ', {'ENVIRONMENT': 'staging', 'K_SERVICE': 'netra-backend'}):
                ...
                initial_result = await self.race_framework.test_websocket_connection_with_latency(profile=initial_profile, test_name=f"{scenario['name']}_initial")
                ...
                recovery_result = await self.race_framework.test_websocket_connection_with_latency(profile=recovery_profile, test_name=f"{scenario['name']}_recovery")
                print(f"     Recovery connection ({recovery_profile.name}): {(' PASS: ' if recovery_result.success else ' FAIL: ')}")
                recovery_results.append({'scenario_name': scenario['name'], 'initial_success': initial_result.success, 'initial_race_detected': initial_result.race_condition_detected, 'recovery_success': recovery_result.success, 'recovery_race_detected': recovery_result.race_condition_detected, 'total_time': initial_result.connection_time + scenario['delay_between'] + recovery_result.connection_time})
        ...
        patterns = {'race_to_success': 0, 'success_to_success': 0, 'race_to_race': 0, 'other': 0}
    ...
    async def test_websocket_connection_timing_benchmarks(self, benchmark_framework):
        ...
        for profile_name in profile_names:
            ...
            for iteration in range(iterations):
                result = await benchmark_framework.test_websocket_connection_with_latency(profile=profile, test_name=f'benchmark_{profile_name}_{iteration}')
                timing_results.append({'success': result.success, 'connection_time': result.connection_time, 'race_detected': result.race_condition_detected, 'events_count': len(result.websocket_events_received)})
            ...
            if successful_times:
                ...
                benchmark_results[profile_name] = {'iterations': iterations, 'successful': len(successful_times), 'success_rate': len(successful_times) / iterations, 'avg_connection_time': avg_time, 'min_connection_time': min_time, 'max_connection_time': max_time, 'race_conditions': race_count, 'race_rate': race_count / iterations}
                ...
                benchmark_results[profile_name] = {'iterations': iterations, 'successful': 0, 'success_rate': 0.0, 'race_conditions': race_count, 'race_rate': race_count / iterations}
