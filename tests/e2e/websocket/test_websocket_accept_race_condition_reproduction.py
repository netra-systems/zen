...
class WebSocketAcceptRaceConditionE2ETest(BaseE2ETest):
    ...
    def setUp(self):
        ...
        self.production_message_burst_size = 10
        self.concurrent_user_count = 5
        self.agent_execution_duration = 30.0
        self.base_ws_url = f'ws://localhost:8000'

    def _record_e2e_race_event(self, event_type: str, user_id: str, connection_id: str, success: bool, error: Optional[str]=None, timing_data: Optional[Dict[str, float]]=None, agent_data: Optional[Dict[str, Any]]=None, websocket_state: Optional[str]=None, production_scenario: Optional[str]=None):
        """Record E2E race condition event for production scenario analysis."""
        event = E2ERaceConditionEvent(timestamp=time.time(), event_type=event_type, user_id=user_id, connection_id=connection_id, success=success, error=error, timing_data=timing_data or {}, agent_data=agent_data or {}, websocket_state=websocket_state, production_scenario=production_scenario)
    ...
    def test_first_time_user_websocket_connection_race_condition(self):
        ...
        async def simulate_first_time_user_connection(user_id: str):
            ...
            try:
                ...
                async with websockets.connect(websocket_uri, extra_headers={'Authorization': f'Bearer {auth_token}'}, timeout=self.gcp_accept_timeout) as websocket:
                    ...
                    self._record_e2e_race_event('accept_attempt', user_id, connection_id, True, timing_data={'accept_complete': accept_complete_time, 'accept_duration': accept_duration}, websocket_state='accepted', production_scenario='first_time_user')
                    message_send_start = time.time()
                    for i in range(self.production_message_burst_size):
                        try:
                            message = {'type': 'user_message', 'content': f'First time user message {i}', 'user_id': user_id, 'timestamp': time.time(), 'message_index': i}
                            await websocket.send(json.dumps(message))
                            self._record_e2e_race_event('message_send', user_id, connection_id, True, timing_data={'message_send_time': time.time(), 'message_index': i}, production_scenario='first_time_user')
                            await asyncio.sleep(0.05)
                        except Exception as message_error:
                            error_msg = str(message_error)
                            if 'accept' in error_msg.lower() or 'connected' in error_msg.lower():
                                self.production_errors.append(f'RACE CONDITION REPRODUCED: {error_msg}')
                                first_time_user_errors.append({'message_index': i, 'error': error_msg, 'timing': time.time() - message_send_start, 'race_condition': True})
                            self._record_e2e_race_event('message_send', user_id, connection_id, False, error=error_msg, timing_data={'message_failure_time': time.time()}, production_scenario='first_time_user')
                    try:
                        response = await asyncio.wait_for(websocket.recv(), timeout=5.0)
                        self._record_e2e_race_event('agent_response', user_id, connection_id, True, agent_data={'response_received': True}, production_scenario='first_time_user')
                    except asyncio.TimeoutError:
                        self.user_journey_failures.append(f"First-time user didn't receive response - potential race condition")
                        self._record_e2e_race_event('agent_response', user_id, connection_id, False, error='Response timeout - potential race condition', production_scenario='first_time_user')
                ...
                if 'accept' in error_msg.lower() or 'handshake' in error_msg.lower():
                    self.production_errors.append(f'Connection race condition: {error_msg}')
                self._record_e2e_race_event('connection_start', user_id, connection_id, False, error=error_msg, production_scenario='first_time_user')
        ...
        if self.production_errors:
            ...
            for error in self.production_errors:
                print(f'  - {error}')
        race_condition_reproduced = race_condition_errors > 0 or message_failures > 0 or len(self.user_journey_failures) > 0 or connection_timing.get('connection_failed', False)
        if not race_condition_reproduced:
            self.user_journey_failures.append('CRITICAL: First-time user race condition not reproduced - test may not be simulating production conditions properly')
        pytest.fail(f'First-time user WebSocket race condition E2E test completed. Production race condition reproduced: {race_condition_reproduced}, Race errors: {race_condition_errors}, Message failures: {message_failures}, Journey failures: {len(self.user_journey_failures)}')
    ...
    def test_concurrent_users_websocket_race_condition(self):
        ...
        async def concurrent_user_worker(user_data: Dict[str, Any], worker_id: int):
            ...
            try:
                async with websockets.connect(websocket_uri, extra_headers={'Authorization': f'Bearer {auth_token}'}, timeout=self.gcp_accept_timeout) as websocket:
                    ...
                    for i in range(5):
                        try:
                            message = {'type': 'concurrent_message', 'user_id': user_id, 'worker_id': worker_id, 'message_index': i, 'timestamp': time.time()}
                            await websocket.send(json.dumps(message))
                            self._record_e2e_race_event('message_send', user_id, connection_id, True, timing_data={'message_time': time.time()}, production_scenario='concurrent_users')
                        except Exception as msg_error:
                            error_msg = str(msg_error)
                            worker_result['errors'].append(error_msg)
                            if 'accept' in error_msg.lower():
                                concurrent_errors.append({'worker_id': worker_id, 'user_id': user_id[:8], 'error': error_msg, 'race_condition': True})
                                self.production_errors.append(f'Concurrent user race condition (worker {worker_id}): {error_msg}')
                            self._record_e2e_race_event('message_send', user_id, connection_id, False, error=error_msg, production_scenario='concurrent_users')
                ...
                if 'accept' in error_msg.lower() or 'handshake' in error_msg.lower():
                    concurrent_errors.append({'worker_id': worker_id, 'user_id': user_id[:8], 'error': error_msg, 'race_condition': True})
                self._record_e2e_race_event('connection_start', user_id, connection_id, False, error=error_msg, production_scenario='concurrent_users')
        ...
        if concurrent_errors:
            ...
            for error in concurrent_errors[:5]:
                print(f"  - Worker {error['worker_id']} (User {error['user_id']}): {error['error']}")
        concurrent_race_reproduced = race_condition_detected or failed_connections > successful_connections or total_errors > self.concurrent_user_count
        pytest.fail(f'Concurrent users WebSocket race condition E2E test completed. Race condition reproduced: {concurrent_race_reproduced}, Success rate: {successful_connections}/{self.concurrent_user_count}, Race errors: {len(concurrent_errors)}')
    ...
    def test_agent_execution_websocket_events_race_condition(self):
        ...
        async def simulate_agent_execution_with_websocket_events(user_id: str):
            ...
            try:
                async with websockets.connect(websocket_uri, extra_headers={'Authorization': f'Bearer {auth_token}'}, timeout=self.gcp_accept_timeout) as websocket:
                    ...
                    critical_agent_events = [{'type': 'agent_started', 'status': 'Agent execution initiated'}, {'type': 'agent_thinking', 'status': 'Analyzing market trends'}, {'type': 'tool_executing', 'tool': 'market_analysis', 'status': 'Gathering data'}, {'type': 'tool_completed', 'tool': 'market_analysis', 'status': 'Data analysis complete'}, {'type': 'agent_completed', 'status': 'Analysis complete', 'result': 'AI market trends analyzed'}]
                    for i, event in enumerate(critical_agent_events):
                        try:
                            ...
                            event_message = {**event, 'user_id': user_id, 'connection_id': connection_id, 'timestamp': time.time(), 'event_index': i}
                            await websocket.send(json.dumps(event_message))
                            agent_execution_events.append({'event_type': event['type'], 'success': True, 'timestamp': time.time(), 'event_index': i})
                            self._record_e2e_race_event('agent_event', user_id, connection_id, True, agent_data=event, production_scenario='agent_execution')
                        except Exception as event_error:
                            error_msg = str(event_error)
                            agent_execution_events.append({'event_type': event['type'], 'success': False, 'error': error_msg, 'timestamp': time.time(), 'event_index': i})
                            if 'accept' in error_msg.lower() or 'connected' in error_msg.lower():
                                agent_race_errors.append({'event_type': event['type'], 'error': error_msg, 'event_index': i, 'race_condition': True})
                                self.agent_execution_failures.append(f"Agent event race condition ({event['type']}): {error_msg}")
                                self.production_errors.append(f"CRITICAL BUSINESS IMPACT: Agent event {event['type']} failed due to race condition")
                            self._record_e2e_race_event('agent_event', user_id, connection_id, False, error=error_msg, agent_data=event, production_scenario='agent_execution')
                    try:
                        response = await asyncio.wait_for(websocket.recv(), timeout=5.0)
                        self._record_e2e_race_event('server_response', user_id, connection_id, True, agent_data={'response': 'Server response received'}, production_scenario='agent_execution')
                    except asyncio.TimeoutError:
                        self.agent_execution_failures.append('Server response timeout - may indicate race condition prevented processing')
                        self._record_e2e_race_event('server_response', user_id, connection_id, False, error='Response timeout', production_scenario='agent_execution')
            except Exception as execution_error:
                error_msg = str(execution_error)
                self.agent_execution_failures.append(f'Agent execution connection failed: {error_msg}')
                if 'accept' in error_msg.lower():
                    self.production_errors.append(f'Agent execution race condition: {error_msg}')
                self._record_e2e_race_event('agent_execution', user_id, connection_id, False, error=error_msg, production_scenario='agent_execution')
        user_id = user_data['user_id']
        ...
        if self.agent_execution_failures:
            ...
            for failure in self.agent_execution_failures:
                print(f'  - {failure}')
        agent_race_reproduced = critical_race_errors > 0 or failed_agent_events > 0 or business_impact_failures > 0 or (successful_agent_events < 5)
        event_delivery_rate = successful_agent_events / max(total_agent_events, 1) * 100
        pytest.fail(f'Agent execution WebSocket race condition E2E test completed. BUSINESS CRITICAL race condition reproduced: {agent_race_reproduced}, Event delivery rate: {event_delivery_rate:.1f}%, Critical errors: {critical_race_errors}, Business impact failures: {business_impact_failures}')
    ...
    def tearDown(self):
        ...
        business_critical_failures = [error for error in self.production_errors if 'CRITICAL BUSINESS IMPACT' in error or 'agent_event' in error]
