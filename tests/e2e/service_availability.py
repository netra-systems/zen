            ...
            try:
                ...
                if result == 1:
                    ...
                    return ServiceStatus(
                        name="postgresql",
                        available=True,
                        details=f"Connected to PostgreSQL at {parsed.hostname}:{parsed.port or 5432}",
                        connection_info=connection_info
                    )
                    
            except asyncio.TimeoutError:
                self.logger.debug(f"PostgreSQL connection timeout for URL: {db_url}")
                continue
            except Exception as e:
                self.logger.debug(f"PostgreSQL connection failed for URL {db_url}: {e}")
                continue
        ...
        for config in redis_configs:
            ...
            try:
                ...
                return ServiceStatus(
                    name="redis",
                    available=True,
                    details=f"Connected to Redis at {connection_info['host']}:{connection_info['port']}",
                    connection_info=connection_info
                )
                    
            except asyncio.TimeoutError:
                self.logger.debug(f"Redis connection timeout for config: {config}")
                continue
            except Exception as e:
                self.logger.debug(f"Redis connection failed for config {config}: {e}")
                continue
        ...
        for config in clickhouse_configs:
            try:
                ...
                return ServiceStatus(
                    name="clickhouse",
                    available=True,
                    details=f"Connected to ClickHouse at {config['host']}:{config['port']}",
                    connection_info=config
                )
                    
            except asyncio.TimeoutError:
                self.logger.debug(f"ClickHouse connection timeout for config: {config}")
                continue
            except Exception as e:
                self.logger.debug(f"ClickHouse connection failed for config {config}: {e}")
                continue
            ...
            async with httpx.AsyncClient(timeout=self.timeout, follow_redirects=True) as client:
                ...
                if response.status_code == 200:
                    ...
                    return ServiceStatus(
                        name="openai",
                        available=False,
                        details=f"OpenAI API returned status {response.status_code}",
            ...
            async with httpx.AsyncClient(timeout=self.timeout, follow_redirects=True) as client:
                ...
                if response.status_code in [400, 405]:  # Method not allowed or bad request
                    ...
                    return ServiceStatus(
                        name="anthropic",
                        available=False,
                        details=f"Anthropic API returned unexpected status {response.status_code}",
                        error=f"HTTP {response.status_code}"
                    )
...
async def main():
    ...
    for service in [availability.postgresql, availability.redis, availability.clickhouse]:
        ...
        print(f"  {status} {service.name}: {service.details}")
        if service.error:
