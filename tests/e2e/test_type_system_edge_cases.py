        ...
        for ts_file in self.types_path.rglob("*.ts"):
            try:
                ...
                for type_name in interfaces + types:
                    ...
                    type_origins[type_name].append(f"{origin_type}:{file_path}")
                        ...
                        if export_name:
                            ...
                            for source_file in registry_dir.rglob("*.ts"):
                                ...
                                try:
                                    ...
                                    if f"interface {export_name}" in source_content or f"type {export_name}" in source_content:
                                        source_found = True
                                        break
                                except Exception:
                                    continue
                    ...
                    if matching_interface:
                        ...
                        if 'optional' in guard_name.lower() and '?' not in interface_body:
                            mismatches.append({
                                ...
                                'issue': f'Type guard {guard_name} suggests optional but interface has required fields'
                            })
        ...
        for ts_file in self.types_path.rglob("*.ts"):
            try:
                ...
                for promise_type in promise_types:
                    ...
                    if nesting_level > 3:
                        issues.append({
                            'type_name': f'Promise<{promise_type}>',
                    ...
                    if type_parts in content and f'{type_parts}>' in promise_type:
                        issues.append({
                            'type_name': f'Promise<{promise_type}>',
                            'has_circular_reference': True,
                            'circular_path': f'{type_parts} -> Promise<{promise_type}>',
                            'file': str(ts_file.relative_to(self.types_path))
                        })
