...
class TestCloudSQLProxyURLConstruction:
    ...
    def test_cloud_sql_detection(self):
        ...
        cloud_env = {'POSTGRES_HOST': '/cloudsql/project:region:instance', 'POSTGRES_USER': 'user', 'POSTGRES_PASSWORD': 'pass', 'POSTGRES_DB': 'db'}
        builder = DatabaseURLBuilder(cloud_env)
        assert builder.cloud_sql.is_cloud_sql is True
        tcp_env = {'POSTGRES_HOST': 'localhost', 'POSTGRES_USER': 'user', 'POSTGRES_PASSWORD': 'pass', 'POSTGRES_DB': 'db'}
    ...
    def test_cloud_sql_async_url_format(self):
        ...
        env = {'ENVIRONMENT': 'staging', 'POSTGRES_HOST': '/cloudsql/netra-staging:us-central1:postgres', 'POSTGRES_USER': 'netra_user', 'POSTGRES_PASSWORD': 'secure_pass', 'POSTGRES_DB': 'netra_staging'}
    ...
    def test_staging_environment_prefers_cloud_sql(self):
        ...
        env = {'ENVIRONMENT': 'staging', 'POSTGRES_HOST': '/cloudsql/netra-staging:us-central1:postgres', 'POSTGRES_USER': 'user', 'POSTGRES_PASSWORD': 'pass', 'POSTGRES_DB': 'db'}
        builder = DatabaseURLBuilder(env)
    ...
    def test_url_encoding_in_cloud_sql(self):
        ...
        env = {'POSTGRES_HOST': '/cloudsql/project:region:instance', 'POSTGRES_USER': 'user@example.com', 'POSTGRES_PASSWORD': 'p@ss#word&special', 'POSTGRES_DB': 'db_name'}
        builder = DatabaseURLBuilder(env)
...
class TestDeploymentScriptCompliance:
    ...
    def test_deploy_script_no_database_url_construction(self):
        ...
        with open(deploy_script, 'r', encoding='utf-8') as f:
            content = f.read()
        assert 'env_vars["DATABASE_URL"]' not in content, 'Deployment script should not construct DATABASE_URL'
        assert 'DatabaseURLBuilder' in content, 'Deployment script should reference DatabaseURLBuilder'
        assert 'SSOT' in content, 'Deployment script should document SSOT principle'
...
class TestCloudSQLValidation:
    ...
    def test_cloud_sql_path_validation(self):
        ...
        valid_paths = ['/cloudsql/project:region:instance', '/cloudsql/netra-staging:us-central1:postgres', '/cloudsql/my-project:europe-west1:my-instance']
        for path in valid_paths:
            env = {'ENVIRONMENT': 'staging', 'POSTGRES_HOST': path, 'POSTGRES_USER': 'user', 'POSTGRES_PASSWORD': 'pass', 'POSTGRES_DB': 'db'}
            builder = DatabaseURLBuilder(env)
    ...
    def test_invalid_cloud_sql_format_detection(self):
        ...
        invalid_env = {'ENVIRONMENT': 'staging', 'POSTGRES_HOST': '/cloudsql/invalid-format', 'POSTGRES_USER': 'user', 'POSTGRES_PASSWORD': 'pass', 'POSTGRES_DB': 'db'}
...
class TestURLMasking:
    ...
    def test_cloud_sql_url_masking(self):
        ...
        assert '***' in masked
        assert '/cloudsql/project:region:instance' in masked
