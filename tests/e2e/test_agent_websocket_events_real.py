...
ADDITIONAL_EVENTS = {'partial_result', 'final_report'}
...
class RealWebSocketEventCollector:
    ...
    async def start_listening(self, timeout: float=30.0):
        ...
        while self._listening and time.time() < end_time:
            try:
                message_str = await asyncio.wait_for(self.websocket.recv(), timeout=2.0)
...
class AgentWebSocketEventsRealTests(SSotAsyncTestCase):
    ...
    async def test_websocket_manager_real_functionality(self):
        ...
        context = AgentExecutionContext(run_id=f'manager_test_run_{user_data.user_id}', thread_id=conn_id, user_id=user_data.user_id, agent_name='manager_test_agent', retry_count=0, max_retries=1)
    ...
    async def test_agent_websocket_events_mission_critical_real(self):
        ...
        user_data = await self.auth_helper.create_authenticated_user(email_prefix='mission_critical_ws_events', password='MissionCritical123!', name='Mission Critical WebSocket Events User')
        websocket = await self._create_real_authenticated_websocket(user_data)
        self.active_connections.append(websocket)
        ...
        try:
            ...
            context = AgentExecutionContext(run_id=f'mission_critical_run_{user_data.user_id}', thread_id=conn_id, user_id=user_data.user_id, agent_name='mission_critical_agent', retry_count=0, max_retries=1)
            ...
            await asyncio.sleep(0.5)
            await notifier.send_tool_completed(context, 'critical_analysis_tool', {'analysis_result': 'success', 'critical_data': 'protected'})
            await asyncio.sleep(0.5)
            await notifier.send_agent_completed(context, {'mission_status': 'completed', 'critical_events_sent': True})
    ...
    async def test_multi_user_agent_event_isolation_real(self):
        ...
        users = []
        for i in range(2):
            user_data = await self.auth_helper.create_authenticated_user(email_prefix=f'multi_user_agent_events_{i}', password=f'MultiUser{i}Agent123!', name=f'Multi User Agent Events Test User {i}')
            websocket = await self._create_real_authenticated_websocket(user_data)
            self.active_connections.append(websocket)
            collector = RealWebSocketEventCollector(websocket, user_data.user_id)
        ...
        for i, user in enumerate(users):
            ...
            await websocket_manager.connect_user(user['user_data'].user_id, user['websocket'], conn_id)
        ...
        try:
            ...
            await asyncio.sleep(0.5)
            for i, user in enumerate(users):
                context = AgentExecutionContext(run_id=f"multi_user_run_{i}_{user['user_data'].user_id}", thread_id=user['conn_id'], user_id=user['user_data'].user_id, agent_name=f'multi_user_agent_{i}', retry_count=0, max_retries=1)
                ...
                await asyncio.sleep(0.3)
                await notifier.send_tool_completed(context, f'user_{i}_specific_tool', {'user_specific_result': f'user_{i}_data', 'isolation_test': True})
                await asyncio.sleep(0.3)
                await notifier.send_agent_completed(context, {'multi_user_test': f'completed_for_user_{i}', 'isolation_verified': True})
                await asyncio.sleep(0.5)
            ...
            for user in users:
                try:
                    await websocket_manager.disconnect_user(user['user_data'].user_id, user['websocket'], user['conn_id'])
                except Exception:
                    pass
        execution_time = time.time() - start_time
        ...
        for i, user_report in enumerate(reports):
            ...
            for event in user_events:
                if 'payload' in event and 'user_id' in event['payload']:
                    ...
                    assert actual_user_id == expected_user_id, f'User isolation violated! User {i + 1} received event for different user: {actual_user_id} != {expected_user_id}'
