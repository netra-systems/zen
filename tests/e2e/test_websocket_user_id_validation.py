...
class TestWebSocketUserIDValidationE2E(BaseE2ETest):
    ...
    async def test_complete_chat_flow_e2e_staging_user(self, auth_config):
        ...
        try:
            user_result = await create_authenticated_user(user_id=failing_user_data['user_id'], email=failing_user_data['email'], password=failing_user_data['password'], auth_config=auth_config)
            ...
            if 'Invalid user_id format' in str(e):
                pytest.fail(f"CRITICAL: User creation failed for '{failing_user_data['user_id']}' due to ID validation bug: {e}. This blocks complete deployment workflows.")
            else:
                raise
        ...
        try:
            async with websockets.connect(websocket_url, timeout=auth_config.timeout, extra_headers={'Authorization': f'Bearer {jwt_token}'}) as websocket:
                ...
                agent_request = {'type': 'agent_execution', 'data': {'message': 'Test deployment user agent execution', 'agent_type': 'data_agent', 'run_id': str(uuid.uuid4())}}
            ...
            if 'Invalid user_id format' in str(e):
                pytest.fail(f"EXPECTED FAILURE CONFIRMED: Complete chat flow fails for deployment user '{failing_user_data['user_id']}' due to ID validation bug: {e}")
            else:
                raise
        ...
        for user_data in failing_deployment_users[:2]:
            ...
            try:
                ...
                async with websockets.connect(websocket_url, timeout=15.0, extra_headers={'Authorization': f'Bearer {jwt_token}'}) as websocket:
                    ...
                    agent_request = {'type': 'agent_execution', 'data': {'message': f'Test execution for deployment user {user_id}', 'agent_type': 'data_agent', 'run_id': str(uuid.uuid4())}}
                    await websocket.send(json.dumps(agent_request))
                    events = []
                    timeout_count = 0
                ...
                if 'Invalid user_id format' in str(e):
                    pytest.fail(f"Deployment user '{user_id}' should work after fix but still fails with ID validation error: {e}")
                else:
                    raise
    ...
    async def test_multi_user_websocket_connections(self, valid_deployment_users, auth_config):
        ...
        try:
            ...
            pytest.fail(f'REGRESSION: Valid user patterns should work but failed: {e}')
        finally:
    ...
    async def test_websocket_pipeline_end_to_end(self, auth_config):
        ...
        async with websockets.connect(websocket_url, timeout=15.0, extra_headers={'Authorization': f'Bearer {jwt_token}'}) as websocket:
            ...
            assert connection_result.get('status') == 'connected'
            pipeline_tasks = [{'message': 'Analyze deployment configuration', 'agent_type': 'data_agent', 'expected_events': ['agent_started', 'agent_thinking', 'agent_completed']}, {'message': 'Validate system health', 'agent_type': 'optimization_agent', 'expected_events': ['agent_started', 'agent_completed']}]
            for task in pipeline_tasks:
                agent_request = {'type': 'agent_execution', 'data': {'message': task['message'], 'agent_type': task['agent_type'], 'run_id': str(uuid.uuid4())}}
                await websocket.send(json.dumps(agent_request))
                task_events = []
                timeout_count = 0
                while timeout_count < 25:
                    try:
                        message = await asyncio.wait_for(websocket.recv(), timeout=1.0)
                        event = json.loads(message)
