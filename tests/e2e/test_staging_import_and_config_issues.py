        ...
        for file_path in critical_files:
            if Path(file_path).exists():
                ...
                if 'from netra_backend.app.db import *' in content:
                    pytest.fail(f"Wildcard import in {file_path} could hide missing dependencies")
    ...
    def test_backend_service_core_imports(self):
        ...
        for module_path, attr_name in required_imports:
            try:
                ...
                assert hasattr(module, attr_name), f"Module {module_path} missing attribute {attr_name}"
            except ImportError as e:
