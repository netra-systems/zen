...
class ErrorRecoveryE2ETests(SSotAsyncTestCase):
    ...
    def setup_class(cls):
        ...
        cls.error_scenarios = {'invalid_agent_request': {'description': 'Invalid agent type request', 'expected_error_type': 'agent_error', 'recovery_possible': True, 'severity': 'medium'}, 'malformed_message': {'description': 'Malformed message structure', 'expected_error_type': 'validation_error', 'recovery_possible': True, 'severity': 'low'}, 'empty_message_content': {'description': 'Empty message content', 'expected_error_type': 'validation_error', 'recovery_possible': True, 'severity': 'low'}, 'oversized_message': {'description': 'Message exceeding size limits', 'expected_error_type': 'size_limit_error', 'recovery_possible': True, 'severity': 'medium'}, 'invalid_user_context': {'description': 'Invalid user context data', 'expected_error_type': 'context_error', 'recovery_possible': True, 'severity': 'medium'}}
        cls.recovery_criteria = {'error_acknowledgment': 'System acknowledges error occurred', 'error_messaging': 'Clear error message provided to user', 'connection_maintained': 'WebSocket connection remains active', 'subsequent_request_success': 'Next valid request processes normally', 'no_system_corruption': 'System state remains consistent'}
    ...
    def _create_error_scenario_message(self, scenario_name: str) -> Dict[str, Any]:
        ...
        """
        error_messages = {'invalid_agent_request': {'type': 'agent_request', 'agent': 'nonexistent_agent_that_should_fail', 'message': 'This should trigger an agent error', 'thread_id': f'error_test_{scenario_name}_{int(time.time())}', 'user_id': self.__class__.test_user_id}, 'malformed_message': {'type': 'agent_request', 'invalid_field': 'This message is missing required fields', 'malformed_structure': True}, 'empty_message_content': {'type': 'agent_request', 'agent': 'triage_agent', 'message': '', 'thread_id': f'empty_test_{scenario_name}_{int(time.time())}', 'user_id': self.__class__.test_user_id}, 'oversized_message': {'type': 'agent_request', 'agent': 'supervisor_agent', 'message': 'A' * 10000, 'thread_id': f'oversized_test_{scenario_name}_{int(time.time())}', 'user_id': self.__class__.test_user_id, 'context': {'oversized_test': True}}, 'invalid_user_context': {'type': 'agent_request', 'agent': 'triage_agent', 'message': 'Test message with invalid context', 'thread_id': None, 'user_id': None, 'context': {'invalid_context_test': True}}}
        return error_messages.get(scenario_name, {})

    def _validate_error_recovery(self, scenario_name: str, error_events: List[Dict[str, Any]], recovery_events: List[Dict[str, Any]]) -> Dict[str, Any]:
        """
        ...
        validation = {'scenario': scenario_name, 'error_acknowledged': False, 'error_message_provided': False, 'expected_error_type_received': False, 'recovery_successful': False, 'connection_maintained': True, 'system_corruption_detected': False, 'recovery_score': 0.0, 'validation_details': []}
        ...
        if scenario_config.get('recovery_possible', False):
            recovery_success_indicators = ['agent_started', 'agent_thinking', 'agent_completed']
            recovery_event_types = [event.get('type', 'unknown') for event in recovery_events]
            recovery_indicators_found = [indicator for indicator in recovery_success_indicators if indicator in recovery_event_types]
        ...
        validation['validation_details'] = [f"Error acknowledged: {validation['error_acknowledged']}", f"Error message provided: {validation['error_message_provided']}", f"Expected error type: {validation['expected_error_type_received']}", f"Recovery successful: {validation['recovery_successful']}", f"Recovery score: {validation['recovery_score']:.2f}/1.0"]
        return validation
    ...
    async def test_comprehensive_error_handling_and_recovery_scenarios(self):
        ...
        try:
            ...
            for scenario_name, scenario_config in self.__class__.error_scenarios.items():
                ...
                recovery_metrics.append({'metric': f'scenario_{scenario_name}', 'duration': scenario_duration, 'recovery_score': recovery_validation['recovery_score'], 'recovery_successful': recovery_validation['recovery_successful']})
                self.logger.info(f"🔄 {scenario_name}: Recovery score {recovery_validation['recovery_score']:.2f}, Duration {scenario_duration:.1f}s")
            ...
            recovery_metrics.append({'metric': 'overall_error_recovery', 'scenarios_tested': len(scenario_results), 'successful_scenarios': len(successful_scenarios), 'failed_scenarios': len(failed_scenarios), 'success_rate': success_rate, 'avg_recovery_score': avg_recovery_score})
            assert success_rate >= 0.75, f"Error recovery success rate too low: {success_rate:.1%} (required ≥75%). Failed scenarios: {[r['scenario'] for r in failed_scenarios]}"
            assert avg_recovery_score >= 0.65, f'Average recovery score too low: {avg_recovery_score:.2f} (required ≥0.65). System recovery capability insufficient.'
            for result in scenario_results:
                validation = result['recovery_validation']
                if self.__class__.error_scenarios[result['scenario']]['severity'] in ['high', 'medium']:
                    assert validation['error_acknowledged'], f"Critical error scenario {result['scenario']} not properly acknowledged"
                if self.__class__.error_scenarios[result['scenario']]['recovery_possible']:
                    assert validation['recovery_successful'], f"Recoverable scenario {result['scenario']} failed to recover properly"
            total_recovery_test_time = time.time() - error_recovery_start_time
            recovery_metrics.append({'metric': 'total_error_recovery_test_time', 'value': total_recovery_test_time, 'all_scenarios_validated': True, 'timestamp': time.time(), 'success': True})
            ...
            raise AssertionError(f'Error recovery validation failed after {total_time:.1f}s: {e}. Poor error recovery threatens user experience and platform reliability. Recovery metrics: {recovery_metrics}')
    ...
    async def test_network_interruption_resilience(self):
        ...
        try:
            ...
            websocket = await asyncio.wait_for(websockets.connect(self.__class__.staging_config.urls.websocket_url, additional_headers={'Authorization': f'Bearer {self.access_token}', 'X-Environment': 'staging', 'X-Test-Suite': 'network-resilience-e2e', 'X-Network-Testing': 'enabled'}, ssl=ssl_context, ping_interval=20, ping_timeout=5), timeout=20.0)
            initial_connection_time = time.time() - initial_connection_start
            network_metrics.append({'metric': 'initial_connection_time', 'value': initial_connection_time, 'timestamp': time.time(), 'success': True})
            initial_message = {'type': 'agent_request', 'agent': 'triage_agent', 'message': 'Network resilience test: Please analyze this request and provide a response. This test will simulate network interruptions during processing.', 'thread_id': f'network_test_{int(time.time())}', 'run_id': f'network_run_{int(time.time())}', 'user_id': self.__class__.test_user_id, 'context': {'network_resilience_test': True, 'expect_interruptions': True}}
            await websocket.send(json.dumps(initial_message))
            initial_events = []
            interruption_timeout = 15.0
            ...
            network_metrics.append({'metric': 'connection_interruption', 'duration': interruption_duration, 'events_before_interruption': len(initial_events), 'timestamp': time.time(), 'success': True})
            recovery_start = time.time()
            try:
                recovery_websocket = await asyncio.wait_for(websockets.connect(self.__class__.staging_config.urls.websocket_url, additional_headers={'Authorization': f'Bearer {self.access_token}', 'X-Environment': 'staging', 'X-Test-Suite': 'network-recovery-e2e', 'X-Recovery-Attempt': 'true'}, ssl=ssl_context, ping_interval=30, ping_timeout=10), timeout=25.0)
                recovery_connection_time = time.time() - recovery_start
                network_metrics.append({'metric': 'recovery_connection_time', 'value': recovery_connection_time, 'timestamp': time.time(), 'success': True})
                self.logger.info(f'🔄 Successfully reconnected after interruption in {recovery_connection_time:.2f}s')
                recovery_message = {'type': 'agent_request', 'agent': 'triage_agent', 'message': 'Post-interruption recovery test: Please provide a response to validate that the system has recovered properly from the network interruption.', 'thread_id': f'recovery_test_{int(time.time())}', 'run_id': f'recovery_run_{int(time.time())}', 'user_id': self.__class__.test_user_id, 'context': {'post_interruption_recovery': True, 'recovery_validation': True}}
                ...
                recovery_successful = any((event.get('type') == 'agent_completed' for event in recovery_events))
                network_metrics.append({'metric': 'recovery_processing', 'duration': recovery_processing_time, 'recovery_events': len(recovery_events), 'recovery_successful': recovery_successful, 'timestamp': time.time(), 'success': recovery_successful})
                assert recovery_successful, f'System failed to recover properly after network interruption. Recovery events: {len(recovery_events)}, Processing time: {recovery_processing_time:.1f}s'
                assert recovery_connection_time < 30.0, f'Recovery connection too slow: {recovery_connection_time:.1f}s (max 30s)'
                assert recovery_processing_time < 60.0, f'Recovery processing too slow: {recovery_processing_time:.1f}s (max 60s)'
            except Exception as recovery_error:
                raise AssertionError(f'Network recovery failed: {recovery_error}')
            total_network_test_time = time.time() - network_test_start_time
            network_metrics.append({'metric': 'total_network_resilience_test', 'value': total_network_test_time, 'network_interruption_handled': True, 'recovery_validated': recovery_successful, 'timestamp': time.time(), 'success': True})
