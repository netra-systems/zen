...
class TestAdvancedErrorRecoveryE2E(SSotAsyncTestCase):
    ...
    def setup_method(self, method):
        ...
        # Create JWT token for error testing
    ...
    async def _establish_websocket_connection(self, headers_override: Dict[str, str] = None) -> websockets.WebSocketServerProtocol:
        ...
        headers = {
            ...
            "X-Test-Suite": "advanced-error-recovery",
            "X-Session-Id": self.error_test_session
        }
    ...
    async def _send_request_and_collect_events(self, websocket, message_data: Dict[str, Any],
        ...
        try:
            ...
            return {
                ...
                "events": [],
                "event_types": set(),
                "error_events": [{"error": "exception", "details": str(e)}],
                "events_count": 0,
                "critical_events": set(),
                "exception": e
            }
    ...
    async def test_invalid_agent_type_error_recovery(self):
        ...
        try:
            # Send request with invalid agent type
            invalid_request = {
                "type": "agent_request",
                "agent": "nonexistent_quantum_ai_agent",  # Invalid agent type
                "message": "Test invalid agent error recovery",
                "thread_id": self.thread_id,
                "run_id": self.run_id,
                "user_id": self.test_user_id,
                "context": {
                    "error_test": "invalid_agent_type",
                    "expected_error": True
                }
            }

            # Process invalid request
            invalid_result = await self._send_request_and_collect_events(
            ...
            assert not invalid_result["success"], (
                f"Invalid agent request should not succeed"
            )

            # Should receive some form of error indication
            has_error_indication = (
                len(invalid_result["error_events"]) > 0 or
                "error" in invalid_result["event_types"] or
                any("error" in str(event).lower() for event in invalid_result["events"])
            )

            assert has_error_indication, (
                f"Should receive error indication for invalid agent type. "
                f"Events: {invalid_result['event_types']}, "
                f"Error events: {invalid_result['error_events']}"
            )

            # Error should be returned quickly
            assert invalid_result["total_time"] < 15.0, (
                f"Error response should be quick: {invalid_result['total_time']:.1f}s (max 15s)"
            )

            # Wait brief moment to ensure connection stability
            await asyncio.sleep(2)

            # Send valid follow-up request to test recovery
            recovery_request = {
                "type": "agent_request",
                "agent": "triage_agent",  # Valid agent type
                "message": "Recovery test after invalid agent request",
                "thread_id": f"{self.thread_id}_recovery",
                "run_id": f"{self.run_id}_recovery",
                "user_id": self.test_user_id,
                "context": {
                    "error_test": "recovery_after_invalid_agent",
                    "recovery_validation": True
                }
            }

            recovery_result = await self._send_request_and_collect_events(
            ...
            assert recovery_result["success"], (
                f"Recovery request should succeed after error. "
                f"Error events: {recovery_result['error_events']}"
            )

            # Should receive proper agent response
            assert len(recovery_result["critical_events"]) >= 2, (
                f"Recovery should deliver critical events. "
                f"Got: {recovery_result['critical_events']}"
            )

            assert recovery_result["events_count"] >= 3, (
    ...
    async def test_malformed_request_error_recovery(self):
        ...
        try:
            ...
            try:
                ...
                # Try to receive response for malformed JSON
                try:
            ...
            missing_result = await self._send_request_and_collect_events(
                websocket, missing_fields_request, timeout=15.0
            )
            ...
            invalid_types_request = {
                ...
                "message": ["invalid", "array"],  # Should be string
                "user_id": {"invalid": "object"},  # Should be string
                "thread_id": True,  # Should be string
                "context": "should_be_object"  # Should be object
            }

            invalid_types_result = await self._send_request_and_collect_events(
                websocket, invalid_types_request, timeout=15.0
            )

            error_scenarios.append({
                "scenario": "invalid_types",
                "result": invalid_types_result
            })
            ...
            # Test connection stability with valid request
            stability_request = {
                "type": "agent_request",
                "agent": "triage_agent",
                "message": "Connection stability test after malformed requests",
                "thread_id": f"{self.thread_id}_stability",
                "run_id": f"{self.run_id}_stability",
                "user_id": self.test_user_id,
                "context": {
                    "stability_test": True,
                    "after_malformed_requests": True
                }
            }

            stability_result = await self._send_request_and_collect_events(
            ...
            assert stability_result["success"], (
                f"Connection should remain stable after malformed requests. "
                f"Stability test failed: {stability_result['error_events']}"
            )

            assert len(stability_result["critical_events"]) >= 2, (
                f"Should receive critical events after error recovery. "
                f"Got: {stability_result['critical_events']}"
            )

            # Validate that malformed requests were handled appropriately
            # (either rejected with errors or ignored without crashing)
            for scenario in error_scenarios[1:]:  # Skip malformed JSON
                if "result" in scenario:
                    result = scenario["result"]
                    # System should either handle gracefully or return quick error
                    assert result["total_time"] < 20.0, (
                        f"Malformed request handling should be quick for {scenario['scenario']}: "
    ...
    async def test_network_interruption_recovery(self):
        ...
        try:
            # Start a longer-running request
            initial_request = {
                "type": "agent_request",
                "agent": "apex_optimizer_agent",  # Likely to take longer
                "message": (
                    "Please provide comprehensive AI cost optimization analysis with "
                    "detailed market research and implementation recommendations. "
                    "Include specific calculations and step-by-step guidance."
                ),
                "thread_id": self.thread_id,
                "run_id": self.run_id,
                "user_id": self.test_user_id,
                "context": {
                    ...
                    "expects_lengthy_processing": True
                }
            }
            ...
            initial_events = []
            interruption_time = 15.0  # Allow some processing before interruption
            ...
            while time.time() - start_time < interruption_time:
                try:
                    ...
                    # If we get completion before interruption, that's fine too
            ...
            websocket2 = await self._establish_websocket_connection({
                "X-Connection-Type": "recovery",
                "X-Previous-Session": self.error_test_session
            })

            # Test that new connection works properly
            recovery_request = {
                "type": "agent_request",
                "agent": "triage_agent",
                "message": "Recovery test: system should respond normally after interruption",
                "thread_id": f"{self.thread_id}_recovery",
                "run_id": f"{self.run_id}_recovery",
                "user_id": self.test_user_id,
                "context": {
            ...
            assert recovery_result["success"], (
                f"Recovery after network interruption should succeed. "
                f"Error events: {recovery_result['error_events']}"
            )

            # Recovery should deliver proper agent response
            assert len(recovery_result["critical_events"]) >= 2, (
                f"Recovery should deliver critical events. "
                f"Got: {recovery_result['critical_events']}"
            )
            ...
            assert recovery_result["total_time"] < 60.0, (
                f"Recovery should not be excessively slow: {recovery_result['total_time']:.1f}s"
            )

            # Validate recovery response contains expected content
            recovery_events = recovery_result["events"]
            has_meaningful_response = any(
                len(str(event.get("data", {}))) > 50 for event in recovery_events
                if event.get("type") == "agent_completed"
            )
    ...
    async def test_concurrent_error_scenarios(self):
        ...
        concurrent_scenarios = [
            {
                ...
                "request": {
                    "type": "agent_request",
                    "agent": "triage_agent",
                    "message": f"Valid request 1: AI cost optimization help. ID: VALID1-{int(time.time())}",
                    "user_id": f"valid_user_1_{int(time.time())}"
                }
            },
            {
                "type": "invalid_agent",
                "request": {
                    ...
                    "message": f"Invalid agent request. ID: INVALID1-{int(time.time())}",
                    "user_id": f"invalid_user_1_{int(time.time())}"
                }
            },
            {
                "type": "valid",
                "request": {
                    ...
                    "message": f"Valid request 2: Comprehensive optimization analysis. ID: VALID2-{int(time.time())}",
                    "user_id": f"valid_user_2_{int(time.time())}"
                }
            },
            {
                "type": "missing_fields",
            ...
            },
            {
                ...
                "request": {
                    ...
                    "message": f"Valid request 3: Data analysis help. ID: VALID3-{int(time.time())}",
                    "user_id": f"valid_user_3_{int(time.time())}"
                }
            }
        ]

        # Create concurrent connections and execute scenarios
        async def execute_concurrent_scenario(scenario_index: int, scenario: Dict[str, Any]) -> Dict[str, Any]:
            ...
            try:
                ...
                headers = {
                    "X-Scenario-Index": str(scenario_index),
                    "X-Scenario-Type": scenario["type"]
                }

                websocket = await self._establish_websocket_connection(headers)

                # Add required fields for incomplete requests
                request = scenario["request"].copy()
                if scenario["type"] != "missing_fields":
                    request.update({
                        "thread_id": f"concurrent_{scenario_index}_{int(time.time())}",
                        "run_id": f"run_concurrent_{scenario_index}_{int(time.time())}"
                    })
                    if "user_id" not in request:
                        request["user_id"] = user_context["user_id"]

                # Execute request
                result = await self._send_request_and_collect_events(
                    websocket, request, timeout=30.0
                )
                ...
                return {
                    ...
                    "result": result,
                    "expected_success": scenario["type"] == "valid"
                }
        ...
        async def _create_isolated_user_context(self, user_index: int) -> Dict[str, Any]:
            ...
            user_context = {
                "user_id": f"concurrent_error_user_{user_index}_{int(time.time())}",
                "email": f"concurrent_error_{user_index}_{int(time.time())}@netra-testing.ai"
            }
        ...
        concurrent_tasks = [
            execute_concurrent_scenario(i, scenario)
        ...
        # Analyze concurrent error handling results
        ...
        assert valid_success_rate >= 0.8, (
            f"Valid requests should succeed despite concurrent errors. "
            f"Success rate: {valid_success_rate:.1%} ({len(valid_successes)}/{len(valid_scenarios)})"
        )
        ...
        for invalid_result in invalid_scenarios:
            if "result" in invalid_result:
                ...
                # Should not succeed, but should not crash either
                assert not result.get("success"), (
                    f"Invalid scenario should not succeed: {invalid_result['scenario_type']}"
                )

                # Should handle errors quickly
                assert result.get("total_time", 0) < 25.0, (
                    f"Error handling should be quick for {invalid_result['scenario_type']}: "
                    f"{result.get('total_time', 0):.1f}s"
                )

        # System should remain stable (no exceptions from connection handling)
        assert len(exception_scenarios) == 0, (
            f"Should not have connection exceptions during concurrent errors: {exception_scenarios}"
        )

        # Performance should remain reasonable despite errors
