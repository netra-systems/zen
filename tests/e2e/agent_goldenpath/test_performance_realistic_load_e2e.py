...
class TestPerformanceRealisticLoadE2E(SSotAsyncTestCase):
    ...
    async def _create_concurrent_user(self, user_index: int, total_users: int) -> PerformanceMetrics:
        ...
        try:
            ...
            websocket = await asyncio.wait_for(
                websockets.connect(
                    ...
                    additional_headers={
                        ...
                        "X-Concurrent-Users": str(total_users)
                    },
                    ssl=ssl_context,
                    ping_interval=30,
                    ping_timeout=10
            ...
            # Send realistic business question
            business_questions = [
                f"I'm optimizing AI costs for user {user_index}. My current spend is ${5000 + user_index * 100}/month. How can I reduce costs by 20%?",
                f"User {user_index} needs help with AI model selection. Current latency is {200 + user_index * 10}ms. What optimizations do you recommend?",
                f"Analyzing performance for client {user_index}. We have {10000 + user_index * 500} daily users. How can we scale efficiently?",
                f"Cost analysis for user {user_index}: spending ${15000 + user_index * 200}/month on LLM APIs. Need 30% reduction strategy.",
            ]
            ...
            events = []
            response_timeout = 60.0  # Reasonable timeout under load
            ...
            # Validate basic quality under load
    ...
    async def test_moderate_concurrent_user_load(self):
        ...
        assert results.success_rate >= 0.80, (
            f"Success rate too low under moderate load: {results.success_rate:.1%} "
            f"(expected ≥80%). Failed users: {results.failed_users}, "
            f"Errors: {[m.error_message for m in valid_metrics if not m.success][:3]}"
        )

        assert results.avg_response_time <= 30.0, (
            f"Average response time too slow under moderate load: {results.avg_response_time:.1f}s "
            f"(expected ≤30s). This impacts user experience."
        )

        assert results.p95_response_time <= 45.0, (
            f"P95 response time too slow under moderate load: {results.p95_response_time:.1f}s "
            f"(expected ≤45s). Users expect consistent performance."
        )

        # Validate user isolation - no cross-contamination
        user_ids = {m.user_id for m in valid_metrics}
        assert len(user_ids) == len(valid_metrics), (
            f"User isolation failure detected under load. "
            f"Unique users: {len(user_ids)}, Total metrics: {len(valid_metrics)}"
        )

        # Validate reasonable response quality under load
        ...
        if successful_metrics:
            ...
            assert avg_response_length >= 80, (
                f"Response quality degraded under load: {avg_response_length:.0f} avg chars "
    ...
    async def test_high_concurrent_user_load(self):
        ...
        assert results.success_rate >= 0.70, (
            f"Success rate too low under high load: {results.success_rate:.1%} "
            f"(expected ≥70%). System should handle higher concurrency gracefully."
        )

        assert results.error_rate <= 0.30, (
            f"Error rate too high under high load: {results.error_rate:.1%} "
            f"(expected ≤30%). Error handling should be robust."
        )

        if results.successful_users > 0:
            assert results.avg_response_time <= 45.0, (
                f"Average response time too slow under high load: {results.avg_response_time:.1f}s "
                f"(expected ≤45s for high concurrency scenario)"
            )

            assert results.p95_response_time <= 75.0, (
                f"P95 response time too slow under high load: {results.p95_response_time:.1f}s "
                f"(expected ≤75s). Even under pressure, most users should get reasonable performance."
            )

        # Validate throughput is reasonable
        assert results.throughput >= 0.1, (
            f"Throughput too low under high load: {results.throughput:.2f} requests/sec "
            f"(expected ≥0.1). System should maintain reasonable processing rate."
        )

        # Analyze performance distribution across agent types
        agent_performance = {}
    ...
    async def test_sustained_load_performance(self):
        ...
        concurrent_users = 3
        test_rounds = 4
        round_duration = 60  # seconds between rounds
        ...
        for round_num in range(test_rounds):
            ...
            user_tasks = []
            for user_index in range(concurrent_users):
                task = self._create_concurrent_user(
                    user_index + (round_num * concurrent_users),  # Unique user indices
                    concurrent_users
                )
                user_tasks.append(task)
        ...
        assert avg_success_rate >= 0.75, (
            f"Average sustained success rate too low: {avg_success_rate:.1%} "
            f"(expected ≥75%). System should maintain reliability over time."
        )

        # Check for significant performance degradation over time
        if len(avg_response_times) >= 3:
            ...
            assert degradation_ratio <= 1.5, (
                f"Significant performance degradation detected over time: "
                f"First half avg: {first_half_avg:.1f}s, Second half avg: {second_half_avg:.1f}s "
                f"(degradation ratio: {degradation_ratio:.2f}, expected ≤1.5)"
            )
        ...
        if avg_response_times:
            ...
            assert coefficient_of_variation <= 0.4, (
                f"Response time too inconsistent across rounds: CV={coefficient_of_variation:.2f} "
                f"(expected ≤0.4). System should provide consistent performance over time."
            )

        # Check for any rounds with complete failure
        failed_rounds = [r for r in all_round_results if r.success_rate == 0]
        assert len(failed_rounds) == 0, (
            f"Complete failure detected in {len(failed_rounds)} rounds. "
    ...
    async def test_mixed_agent_load_distribution(self):
        ...
        assert overall_results.success_rate >= 0.70, (
            f"Overall mixed agent success rate too low: {overall_results.success_rate:.1%} "
            f"(expected ≥70%). Mixed agent load should be handled effectively."
        )

        # Validate each agent type maintains reasonable performance
        for agent_type, results in agent_performance.items():
            assert results.success_rate >= 0.70, (
                f"{agent_type} success rate too low under mixed load: {results.success_rate:.1%} "
                f"(expected ≥70%). All agent types should perform well concurrently."
            )

            if results.successful_users > 0:
                assert results.avg_response_time <= 50.0, (
                    f"{agent_type} average response time too slow: {results.avg_response_time:.1f}s "
                    f"(expected ≤50s under mixed agent load)"
                )

        # Check for resource fairness across agent types
        successful_response_times = {}
        for agent_type in agent_types:
            agent_times = [m.response_time for m in valid_metrics
                          if m.agent_type == agent_type and m.success]
            if agent_times:
        ...
        if len(successful_response_times) >= 2:
            ...
            assert fairness_ratio <= 2.5, (
                f"Resource fairness issue detected across agent types. "
                f"Performance ratio: {fairness_ratio:.2f} (expected ≤2.5). "
                f"Times: {successful_response_times}"
            )

        # Validate no single agent type completely dominates
        success_distribution = {agent_type: results.successful_users
                              for agent_type, results in agent_performance.items()}
        ...
        if total_successes > 0:
            for agent_type, successes in success_distribution.items():
                ...
                assert success_percentage >= 0.1, (
                    f"{agent_type} has too few successes under mixed load: "
