...
class TestConcurrentUserScenariosE2E(SSotAsyncTestCase):
    ...
    async def _create_isolated_user_context(self, user_index: int) -> Dict[str, Any]:
        ...
        user_context = {
            ...
            "email": f"concurrent_user_{user_index}_{int(time.time())}@netra-testing.ai",
            "thread_id": f"thread_{user_index}_{self.test_session_id}",
            "run_id": f"run_{user_index}_{self.test_session_id}",
            "conversation_id": str(uuid.uuid4()),
            "user_index": user_index
        }

        # Create JWT token for this specific user
        user_context["access_token"] = self.__class__.auth_helper.create_test_jwt_token(
        ...
        websocket = await asyncio.wait_for(
            websockets.connect(
                ...
                additional_headers={
                    ...
                    "X-Test-Suite": "concurrent-user-scenarios",
                    "X-User-Index": str(user_context["user_index"]),
    ...
    async def _process_concurrent_agent_request(self, user_context: Dict[str, Any],
                                              message: str, agent_type: str = "triage_agent") -> Dict[str, Any]:
    ...
    async def test_basic_concurrent_user_isolation(self):
        ...
        # Define user-specific messages for isolation validation
        user_specific_messages = [
            f"I'm User Alpha and I need help with AI optimization for my e-commerce platform. My unique identifier is ALPHA-{int(time.time())}.",
            f"I'm User Beta working on marketing automation cost reduction. My unique identifier is BETA-{int(time.time())}.",
            f"I'm User Gamma optimizing customer support chatbot expenses. My unique identifier is GAMMA-{int(time.time())}."
        ]

        # Execute concurrent agent requests
        concurrent_tasks = []
        for i, user_context in enumerate(user_contexts):
            task = self._process_concurrent_agent_request(
                user_context,
                user_specific_messages[i],
                "triage_agent"
            )
        ...
        assert len(successful_results) == concurrent_users, (
            f"All {concurrent_users} concurrent users should succeed. "
            f"Successful: {len(successful_results)}, "
            f"Failed: {failed_results}, "
            f"Exceptions: {exception_results}"
        )

        # Validate user isolation - each user should have unique response content
        ...
        for i, result in enumerate(successful_results):
            ...
            # User should receive response containing their identifier
            assert user_identifier in user_response, (
                f"User {i} should receive response with their identifier '{user_identifier}'. "
                f"Response: {result['response_content'][:200]}..."
            )

            # User should NOT receive other users' identifiers
            other_identifiers = [uid for uid in user_identifiers if uid != user_identifier]
            for other_id in other_identifiers:
                assert other_id not in user_response, (
                    f"User {i} response should not contain other user identifier '{other_id}'. "
                    f"Response: {result['response_content'][:200]}..."
                )
        ...
        assert avg_response_time < 45.0, (
            f"Average response time under concurrent load too high: {avg_response_time:.2f}s (max 45s)"
        )

        assert max_response_time < 60.0, (
            f"Maximum response time under concurrent load too high: {max_response_time:.2f}s (max 60s)"
        )

        # Validate all users received critical events
        for i, result in enumerate(successful_results):
            critical_events_received = len(result["critical_events"])
            assert critical_events_received >= 3, (
                f"User {i} should receive at least 3 critical events, got {critical_events_received}. "
                f"Events: {result['event_types']}"
            )

        # Validate response quality
        for i, result in enumerate(successful_results):
            assert result["response_length"] >= 50, (
    ...
    async def test_high_concurrency_user_load(self):
        ...
        # Define varied complexity messages to test different load patterns
        complexity_messages = [
            # Simple request
            f"What is AI cost optimization? User ID: SIMPLE-{int(time.time())}",

            # Medium complexity
            f"I need recommendations for reducing my $5,000/month OpenAI costs for customer support. "
            f"User ID: MEDIUM-{int(time.time())}",

            # High complexity with tool usage
            f"Please analyze my AI infrastructure for comprehensive cost optimization. "
            f"I'm spending $20,000/month across GPT-4, Claude, and custom models. "
            f"Provide detailed analysis with market comparisons and implementation steps. "
            f"User ID: COMPLEX-{int(time.time())}",

            # Business scenario
            f"As CTO of a B2B SaaS company, I need to optimize our AI costs while scaling from "
            f"1,000 to 10,000 customers. Current spend is $15K/month. "
            f"User ID: BUSINESS-{int(time.time())}",

            # Technical deep dive
            f"I need technical implementation details for reducing API costs through prompt optimization, "
            f"caching, and model selection strategies. Include specific code examples. "
            f"User ID: TECHNICAL-{int(time.time())}"
        ]

        # Assign different agent types for varied load patterns
        agent_types = ["triage_agent", "triage_agent", "apex_optimizer_agent", "supervisor_agent", "apex_optimizer_agent"]

        # Execute high concurrent requests
        concurrent_tasks = []
        ...
        assert success_rate >= 0.8, (
            f"High concurrency success rate too low: {success_rate:.1%} (min 80%). "
            f"Successful: {len(successful_results)}/{concurrent_users}"
        )

        # Validate user isolation under high load
        ...
        for i, result in enumerate(successful_results):
            ...
            # Each user should receive their own identifier
            assert expected_id in user_response, (
                f"User {i} should receive their identifier '{expected_id}' under high load. "
                f"Response: {result['response_content'][:200]}..."
            )

        # Performance analysis under high load
        response_times = [r["total_time"] for r in successful_results]
        if response_times:
            avg_response_time = sum(response_times) / len(response_times)
            max_response_time = max(response_times)
            ...
            assert avg_response_time < 75.0, (
                f"Average response time under high load excessive: {avg_response_time:.2f}s (max 75s)"
            )

            assert max_response_time < 120.0, (
                f"Maximum response time under high load excessive: {max_response_time:.2f}s (max 120s)"
            )

        # Validate event delivery under high load
        total_events = sum(r["events_count"] for r in successful_results)
        assert total_events >= len(successful_results) * 3, (
            f"Should receive adequate events under high load. "
    ...
    async def test_concurrent_agent_type_isolation(self):
        ...
        agent_scenarios = [
            {
                ...
                "message": f"Quick question: What's the best way to reduce AI API costs? User: TRIAGE-{int(time.time())}",
                "expected_content": ["triage", "quick", "recommendation"],
                "user_id": "triage_user"
            },
            {
                "agent_type": "apex_optimizer_agent",
                "message": f"I need comprehensive AI cost optimization analysis with detailed recommendations and market data. User: APEX-{int(time.time())}",
                "expected_content": ["optimization", "analysis", "detailed"],
                "user_id": "apex_user"
            },
            {
                "agent_type": "supervisor_agent",
                "message": f"Please coordinate a complete review of my AI infrastructure costs and provide strategic guidance. User: SUPERVISOR-{int(time.time())}",
                "expected_content": ["strategic", "coordinate", "comprehensive"],
                "user_id": "supervisor_user"
            },
            {
                "agent_type": "data_helper_agent",
                "message": f"Help me analyze the cost data for my AI operations and create performance metrics. User: DATA-{int(time.time())}",
                "expected_content": ["data", "analyze", "metrics"],
                "user_id": "data_user"
            }
        ]

        # Create user contexts for each agent scenario
        user_contexts = []
        ...
        # Execute concurrent requests to different agent types
        concurrent_tasks = []
        ...
        assert len(successful_results) == len(agent_scenarios), (
            f"All {len(agent_scenarios)} agent types should execute successfully. "
            f"Successful: {len(successful_results)}, Failed: {failed_results}, Exceptions: {exception_results}"
        )

        # Validate agent-specific isolation
        ...
        for i, result in enumerate(successful_results):
            ...
            # User should receive response with their identifier
            assert expected_identifier in response, (
                f"Agent {scenario['agent_type']} should return user-specific content with '{expected_identifier}'. "
                f"Response: {result['response_content'][:200]}..."
            )

            # Response should not contain other users' identifiers
            other_identifiers = [uid for uid in user_identifiers if uid != expected_identifier]
            for other_id in other_identifiers:
                assert other_id not in response, (
                    f"Agent {scenario['agent_type']} response should not contain '{other_id}'. "
                    f"Agent isolation failure. Response: {result['response_content'][:200]}..."
                )

        # Validate agent-specific response characteristics
        for result in successful_results:
            scenario = result["user_context"]["agent_scenario"]
            agent_type = scenario["agent_type"]
            response = result["response_content"].lower()

            # Different agents should produce appropriate response styles
            if agent_type == "triage_agent":
                # Triage should provide quick, direct responses
                assert result["response_length"] >= 30, (
                    f"Triage agent should provide substantive response: {result['response_length']} chars"
                )
            elif agent_type == "apex_optimizer_agent":
                # APEX should provide detailed optimization analysis
                assert result["response_length"] >= 100, (
                    f"APEX agent should provide detailed analysis: {result['response_length']} chars"
                )
            elif agent_type == "supervisor_agent":
                # Supervisor should provide strategic guidance
                assert result["response_length"] >= 80, (
                    f"Supervisor agent should provide strategic guidance: {result['response_length']} chars"
                )
            elif agent_type == "data_helper_agent":
                # Data helper should provide analytical responses
                assert result["response_length"] >= 60, (
                    f"Data helper agent should provide analytical response: {result['response_length']} chars"
                )

        # Validate concurrent performance with different agent types
        response_times = [r["total_time"] for r in successful_results]
        avg_response_time = sum(response_times) / len(response_times)

        assert avg_response_time < 60.0, (
            f"Average response time for concurrent agent types too high: {avg_response_time:.2f}s (max 60s)"
        )

        # Validate all agents delivered critical events
        for i, result in enumerate(successful_results):
            scenario = result["user_context"]["agent_scenario"]
            critical_events = len(result["critical_events"])

            assert critical_events >= 2, (
                f"Agent {scenario['agent_type']} should deliver critical events. "
