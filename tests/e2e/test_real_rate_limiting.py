...
class RedisRateLimiterTests:
    ...
    async def _execute_all_test_steps(self, results: Dict[str, Any]) -> None:
        ...
        steps = [('user_setup', self._setup_free_user_with_redis), ('normal_usage', self._send_messages_within_limit), ('rate_limited', self._hit_rate_limit), ('rate_reset', self._wait_for_rate_limit_reset), ('post_reset', self._send_after_reset), ('upgrade', self._upgrade_to_paid_tier), ('unlimited', self._verify_unlimited_usage)]
...
async def _test_api_rate_limits(self) -> Dict[str, Any]:
    ...
    return {'success': True, 'auth_service_limited': auth_results.get('rate_limited_at_request', 0) > 0, 'backend_api_limited': backend_results.get('rate_limited_at_request', 0) > 0, 'steps': [{'step': 'auth_api_test', 'success': True, 'data': auth_results}, {'step': 'backend_api_test', 'success': True, 'data': backend_results}]}
...
async def test_comprehensive_rate_limiting_system(unified_test_harness):
    ...
    try:
        ...
        test_results['api_limits'] = {'auth_limited': auth_results.get('rate_limited_at_request', 0) > 0, 'backend_limited': backend_results.get('rate_limited_at_request', 0) > 0}
        ...
        try:
            ...
            test_results['websocket_limits'] = {'tested': ws_results.get('websocket_tested', False), 'limited': ws_results.get('rate_limited', False)}
        ...
        test_results['agent_throttling'] = {'tested': agent_results.get('agent_throttling_tested', False), 'throttled': agent_results.get('throttling_active', False)}
        print('Testing tier-based limits...')
        tier_results = await tier_tester.test_tier_based_limits()
        test_results['tier_limits'] = {'tested': tier_results.get('tier_testing_completed', False), 'free_limited': tier_results.get('free_tier_limit', 0) > 0, 'paid_higher': tier_results.get('paid_tier_higher', False)}
        print('Testing distributed rate limiting...')
        distributed_results = await distributed_validator.test_distributed_rate_limiting()
        test_results['distributed'] = {'tested': distributed_results.get('distributed_testing_completed', False), 'synced': distributed_results.get('counters_synced', False)}
        print('Testing 429 response headers...')
        header_results = await header_validator.test_429_responses_and_headers()
        test_results['response_headers'] = {'tested': header_results.get('response_header_tested', False), 'retry_after': header_results.get('retry_after_header', False)}
