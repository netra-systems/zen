...
from netra_backend.app.agents.optimizations_core_sub_agent import OptimizationsCoreSubAgent as OptimizationsSubAgent
...
class TestE2EConcurrentIsolation:
    ...
    async def test_200_concurrent_users_full_workflow_isolation(self):
        ...
        def track_violation(violation_type: str, user_id: str, details: Dict[str, Any]):
            ...
            with violation_lock:
                isolation_violations.append({'type': violation_type, 'user_id': user_id, 'details': details, 'timestamp': time.time(), 'thread_id': threading.get_ident()})
        ...
        async def full_workflow_user_scenario(user_id: str) -> Dict[str, Any]:
            ...
            base_context = UserExecutionContext(user_id=user_id, thread_id=f'e2e_thread_{user_id}', run_id=f'e2e_run_{uuid.uuid4()}')
            user_signature = f'e2e_signature_{user_id}_{uuid.uuid4()}'
            workflow_data = {'user_id': user_id, 'signature': user_signature, 'workflow_type': workflow_complexity, 'start_time': workflow_start}
            try:
                triage_context = UserExecutionContext(user_id=user_id, thread_id=f'triage_thread_{user_id}', run_id=f'triage_run_{uuid.uuid4()}')
                with patch.object(factory, '_get_agent_class') as mock_get_triage:
                    ...
                    if hasattr(triage_agent, '_user_signature'):
                        if user_signature not in triage_agent._user_signature:
                            track_violation('triage_contamination', user_id, {'expected': user_signature, 'actual': getattr(triage_agent, '_user_signature', None)})
                    await asyncio.sleep(random.uniform(0.01, 0.05))
                    triage_result = {'phase': 'triage', 'status': 'completed', 'user_id': user_id}
                data_context = UserExecutionContext(user_id=user_id, thread_id=f'data_thread_{user_id}', run_id=f'data_run_{uuid.uuid4()}')
                with patch.object(factory, '_get_agent_class') as mock_get_data:
                    ...
                    if hasattr(data_agent, '_user_signature'):
                        if user_signature not in data_agent._user_signature:
                            track_violation('data_contamination', user_id, {'expected': user_signature, 'actual': getattr(data_agent, '_user_signature', None)})
                    if hasattr(data_agent, '_triage_input'):
                        input_user = data_agent._triage_input.get('user_id')
                        if input_user and input_user != user_id:
                            track_violation('cross_user_data_leak', user_id, {'expected_user': user_id, 'leaked_from_user': input_user})
                    await asyncio.sleep(random.uniform(0.02, 0.08))
                    data_result = {'phase': 'data', 'status': 'completed', 'user_id': user_id}
                opt_context = UserExecutionContext(user_id=user_id, thread_id=f'opt_thread_{user_id}', run_id=f'opt_run_{uuid.uuid4()}')
                with patch.object(factory, '_get_agent_class') as mock_get_opt:
                    ...
                    if hasattr(opt_agent, '_user_signature'):
                        if user_signature not in opt_agent._user_signature:
                            track_violation('optimization_contamination', user_id, {'expected': user_signature, 'actual': getattr(opt_agent, '_user_signature', None)})
                    await asyncio.sleep(random.uniform(0.02, 0.06))
                    opt_result = {'phase': 'optimizations', 'status': 'completed', 'user_id': user_id}
                workflow_end = time.time()
                total_time = workflow_end - workflow_start
                workflow_results[user_id] = {'triage': triage_result, 'data': data_result, 'optimizations': opt_result, 'total_time': total_time, 'status': 'success', 'user_signature': user_signature}
                performance_metrics.append(total_time)
                return {'user_id': user_id, 'status': 'workflow_success', 'phases_completed': 3, 'total_time': total_time, 'signature': user_signature}
            except Exception as e:
                workflow_end = time.time()
                track_violation('workflow_exception', user_id, {'error': str(e), 'error_type': type(e).__name__, 'duration': workflow_end - workflow_start})
                return {'user_id': user_id, 'status': 'workflow_failure', 'error': str(e), 'total_time': workflow_end - workflow_start}
        logger.info(f'Starting E2E test with {concurrent_users} concurrent users')
        start_time = time.time()
        initial_memory = psutil.Process().memory_info().rss / 1024 / 1024
        results = await asyncio.gather(*[full_workflow_user_scenario(f'e2e_user_{i:04d}') for i in range(concurrent_users)], return_exceptions=False)
        end_time = time.time()
        ...
        assert len(isolation_violations) == 0, f'CRITICAL: Isolation violations detected: {len(isolation_violations)} violations'
    ...
    async def test_websocket_event_isolation_under_extreme_load(self):
        ...
        def track_websocket_event(connection_id: str, event_type: str, data: Dict[str, Any]):
            ...
            with event_lock:
                connection_events[connection_id].append({'event_type': event_type, 'data': data, 'timestamp': time.time(), 'thread_id': threading.get_ident()})
                if 'user_id' in data and data['user_id'] not in connection_id:
                    cross_contamination.append({'connection_id': connection_id, 'event_type': event_type, 'expected_user': connection_id.split('_')[-1] if '_' in connection_id else connection_id, 'actual_user': data['user_id'], 'timestamp': time.time()})
        ...
        async def websocket_intensive_scenario(connection_id: str) -> Dict[str, Any]:
            ...
            try:
                for agent_type in ['triage', 'data', 'optimizations']:
                    context = UserExecutionContext(user_id=user_id, thread_id=f'ws_{agent_type}_thread_{user_id}', run_id=f'ws_{agent_type}_run_{uuid.uuid4()}')
                    ...
                    with patch.object(factory, '_get_agent_class') as mock_get_class:
                        ...
                        agent_events = [('agent_started', {'message': f'{agent_type} started', 'user_id': user_id}), ('agent_thinking', {'message': f'{agent_type} processing', 'user_id': user_id}), ('tool_executing', {'message': f'{agent_type} tool exec', 'user_id': user_id}), ('agent_progress', {'message': f'{agent_type} progress', 'user_id': user_id}), ('agent_completed', {'message': f'{agent_type} completed', 'user_id': user_id})]
                ...
                return {'connection_id': connection_id, 'user_id': user_id, 'status': 'websocket_success', 'events_sent': events_sent, 'events_failed': events_failed}
            except Exception as e:
                return {'connection_id': connection_id, 'user_id': user_id, 'status': 'websocket_failure', 'error': str(e), 'events_sent': events_sent, 'events_failed': events_failed}
        logger.info(f'Starting WebSocket isolation test with {concurrent_connections} connections')
        ws_results = await asyncio.gather(*[websocket_intensive_scenario(f'ws_conn_{i:04d}') for i in range(concurrent_connections)], return_exceptions=False)
        ...
        assert len(cross_contamination) == 0, f'CRITICAL: WebSocket cross-contamination detected: {len(cross_contamination)} violations'
        ...
        for connection_id, events in connection_events.items():
            ...
            for event in events:
                if 'user_id' in event['data']:
                    assert event['data']['user_id'] == expected_user, f"Event contamination in {connection_id}: expected {expected_user}, got {event['data']['user_id']}"
    ...
    async def test_database_session_isolation_extreme_load(self):
        ...
        def track_db_session(user_id: str, session_id: str, operation: str, table: str=None):
            ...
            with session_lock:
                session_tracker[user_id].append({'session_id': session_id, 'operation': operation, 'table': table, 'timestamp': time.time(), 'thread_id': threading.get_ident()})
                for other_user, other_sessions in session_tracker.items():
                    if other_user != user_id:
                        for other_session in other_sessions:
                            if other_session['session_id'] == session_id:
                                session_violations.append({'shared_session': session_id, 'user1': user_id, 'user2': other_user, 'timestamp': time.time()})
        ...
        async def database_intensive_scenario(user_id: str) -> Dict[str, Any]:
            ...
            try:
                for agent_type in ['triage', 'data', 'optimizations']:
                    context = UserExecutionContext(user_id=user_id, thread_id=f'db_{agent_type}_thread_{user_id}', run_id=f'db_{agent_type}_run_{uuid.uuid4()}')
                    agent_class = {'triage': UnifiedTriageAgent, 'data': DataSubAgent, 'optimizations': OptimizationsSubAgent}[agent_type]
                    with patch.object(factory, '_get_agent_class') as mock_get_class:
                        mock_get_class.return_value = agent_class
                        agent = await factory.create_agent_instance(agent_type, context)
                        for i in range(operations_per_user // 3):
                            ...
                            for op in operations[:2]:
                                ...
                                await asyncio.sleep(0.001)
                return {'user_id': user_id, 'status': 'db_success', 'operations_completed': operations_completed, 'unique_sessions': len(sessions_used), 'agent_types_used': ['triage', 'data', 'optimizations']}
            except Exception as e:
                return {'user_id': user_id, 'status': 'db_failure', 'error': str(e), 'operations_completed': operations_completed, 'unique_sessions': len(sessions_used)}
        logger.info(f'Starting database isolation test with {concurrent_db_users} concurrent users')
        db_results = await asyncio.gather(*[database_intensive_scenario(f'db_user_{i:04d}') for i in range(concurrent_db_users)], return_exceptions=False)
        ...
        assert len(session_violations) == 0, f'CRITICAL: Database session sharing detected: {len(session_violations)} violations'
        all_sessions = set()
        ...
        expected_operations = concurrent_db_users * operations_per_user * 2
    ...
    async def test_memory_stability_under_extreme_concurrent_load(self):
        ...
        async def memory_intensive_scenario(user_id: str) -> Dict[str, Any]:
            ...
            try:
                for operation in range(memory_intensive_operations):
                    context = UserExecutionContext(user_id=user_id, thread_id=f'mem_thread_{user_id}_{operation}', run_id=f'mem_run_{uuid.uuid4()}')
                    ...
                    with patch.object(factory, '_get_agent_class') as mock_get_class:
                        ...
                        agent._large_memory_structure = {'data_matrix': [[random.random() for _ in range(200)] for _ in range(100)], 'user_cache': {f'key_{i}': f'value_{user_id}_{operation}_{i}' for i in range(500)}, 'processing_results': [f'result_{user_id}_{operation}_{i}' for i in range(1000)], 'metadata': {'user_id': user_id, 'operation': operation, 'memory_signature': f'mem_sig_{user_id}_{operation}'}}
                        await asyncio.sleep(0.002)
                        current_memory = psutil.Process().memory_info().rss / 1024 / 1024
                        memory_samples.append({'user_id': user_id, 'operation': operation, 'memory_mb': current_memory, 'phase': 'processing'})
                final_user_memory = psutil.Process().memory_info().rss / 1024 / 1024
                user_memory_growth = final_user_memory - scenario_start_memory
                if user_memory_growth > 50:
                    memory_violations.append({'user_id': user_id, 'memory_growth_mb': user_memory_growth, 'violation_type': 'excessive_growth'})
                return {'user_id': user_id, 'status': 'memory_success', 'operations_completed': memory_intensive_operations, 'memory_growth_mb': user_memory_growth}
            except Exception as e:
                return {'user_id': user_id, 'status': 'memory_failure', 'error': str(e), 'memory_growth_mb': psutil.Process().memory_info().rss / 1024 / 1024 - scenario_start_memory}
        logger.info(f'Starting memory stability test with {concurrent_memory_users} concurrent users')
        memory_results = await asyncio.gather(*[memory_intensive_scenario(f'mem_user_{i:04d}') for i in range(concurrent_memory_users)], return_exceptions=False)
        gc.collect()
        ...
        expected_max_growth = concurrent_memory_users * 5
        ...
        assert len(memory_violations) == 0, f'Memory violations detected: {len(memory_violations)} users with excessive growth'
