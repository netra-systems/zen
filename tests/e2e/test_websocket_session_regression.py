...
class TestWebSocketSessionRegression:
    ...
    async def test_concurrent_agent_execution_no_session_conflict(self):
        ...
        tasks = [
            simulate_agent_request(f"user_{i}", f"Request {i}")
            for i in range(5)
        ]
        ...
        for result in results:
            if isinstance(result, dict):
                assert result["success"], f"Agent failed for {result.get('user_id')}: {result.get('error')}"
            else:
    ...
    async def test_session_lifecycle_with_long_running_agent(self):
        ...
        async with get_db() as session:
            ...
            session.close = lambda: track_session_close(session)
    ...
    async def test_no_session_in_async_supervisor_execution(self):
        ...
        try:
            ...
            if "db_session" in str(e):
                pytest.fail(f"Supervisor tried to access db_session: {e}")
...
class TestAgentPerformanceRegression:
    ...
    async def test_agent_response_time_under_threshold(self):
        ...
        try:
            ...
            assert elapsed < 5.0, f"Agent took {elapsed}s, should be < 5s"
    ...
    async def test_multiple_sequential_agents_no_degradation(self):
        ...
        for i in range(3):
            message = WebSocketMessage(
                type=MessageType.START_AGENT,
                payload={"user_request": f"Sequential request {i}"},
                user_id=f"seq_user_{i}",
                thread_id=f"seq_thread_{i}"
            )
            
            start_time = time.time()
            try:
                await asyncio.wait_for(
                    handler.handle_message(f"seq_user_{i}", None, message),
                    timeout=5.0
                )
                elapsed = time.time() - start_time
        ...
        if len(response_times) >= 2:
            ...
            assert degradation < 2.0, f"Performance degraded by {degradation}s"
...
class TestSessionCleanup:
    ...
    async def test_no_session_leaks_after_agent_execution(self):
        ...
        for i in range(3):
            message = WebSocketMessage(
                type=MessageType.START_AGENT,
                payload={"user_request": f"Leak test {i}"},
                user_id=f"leak_user_{i}",
                thread_id=f"leak_thread_{i}"
            )
            await handler.handle_message(f"leak_user_{i}", None, message)
