...
class TestAuthenticationFlow:
    ...
    async def test_jwt_token_generation(self):
        ...
        async with aiohttp.ClientSession() as session:
            ...
            await session.post(f"{backend_url}/api/auth/register", json=signup_data)
        ...
        async with aiohttp.ClientSession() as session:
            ...
            for session_info in sessions:
                session_response = await session.get(
                    f"{backend_url}/api/auth/session/{session_info['session_id']}",
            ...
            revoke_response = await session.delete(
                f"{backend_url}/api/auth/session/{sessions[1]['session_id']}",
                headers={"Authorization": f"Bearer {sessions[0]['access_token']}"}
            )
