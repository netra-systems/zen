...
class WebSocketEventMonitor:
    ...
    async def capture_event(self, event_type: str, *args, **kwargs):
        ...
        async with self.event_lock:
            event_data = {'type': event_type, 'timestamp': time.time(), 'relative_time': time.time() - self.start_time, 'args': args, 'kwargs': kwargs, 'thread_id': asyncio.current_task().get_name() if asyncio.current_task() else 'unknown'}
...
class WebSocketEventIntegrityTests(SSotAsyncTestCase):
    ...
    async def test_websocket_events_during_migration(self):
        ...
        test_patterns = [{'name': 'UserExecutionEngine_Direct', 'description': 'Direct UserExecutionEngine usage (post-migration)'}, {'name': 'ExecutionEngine_Compatibility', 'description': 'Compatibility layer usage (during migration)'}]
    ...
    async def test_critical_websocket_event_sequence(self):
        ...
        agent_context = AgentExecutionContext(agent_name='supervisor_agent', user_id=user_context.user_id, thread_id=user_context.thread_id, run_id=user_context.run_id, metadata={'user_input': 'Execute comprehensive workflow with all WebSocket events', 'test_case': 'critical_sequence_validation', 'requires_all_events': True, 'websocket_critical': True})
        result = await self._execute_with_critical_event_sequence(engine, agent_context, user_context, monitor)
    ...
    async def test_websocket_event_isolation_per_user(self):
        ...
        num_users = 3
        user_contexts = [self._create_websocket_test_user_context(f'isolation_user_{i}') for i in range(num_users)]
        ...
        for i, (engine, context, monitor) in enumerate(zip(user_engines, user_contexts, user_monitors)):
            task = asyncio.create_task(self._execute_isolated_user_websocket_session(engine, context, monitor, i))
            tasks.append(task)
    ...
    async def test_websocket_event_error_recovery(self):
        ...
        agent_context = AgentExecutionContext(agent_name='error_recovery_agent', user_id=user_context.user_id, thread_id=user_context.thread_id, run_id=user_context.run_id, metadata={'test_case': 'error_recovery', 'simulate_errors': True})
        result = await self._execute_with_websocket_error_simulation(engine, agent_context, user_context, monitor)
    ...
    async def test_websocket_event_performance_during_migration(self):
        ...
        engine = await self._create_best_available_engine(user_context, monitor)
        num_events = 10
        performance_metrics = {'event_count': 0, 'total_time': 0.0, 'avg_time_per_event': 0.0, 'max_event_gap': 0.0, 'events_per_second': 0.0}
        start_time = time.time()
        for i in range(num_events):
            agent_context = AgentExecutionContext(agent_name='performance_test_agent', user_id=user_context.user_id, thread_id=user_context.thread_id, run_id=f'{user_context.run_id}_perf_{i}', metadata={'performance_test': i, 'batch_size': num_events})
    ...
    def _create_websocket_test_user_context(self, test_name: str) -> UserExecutionContext:
        ...
        return UserExecutionContext(user_id=f'websocket_{test_name}_user', thread_id=f'websocket_{test_name}_thread', run_id=f'websocket_{test_name}_run_{int(time.time())}', request_id=f'websocket_{test_name}_req_{int(time.time())}', audit_metadata={'websocket_test': True, 'test_name': test_name, 'requires_events': ['agent_started', 'agent_thinking', 'agent_completed'], 'test_timestamp': time.time()})

    async def _create_websocket_test_engine(self, user_context: UserExecutionContext, monitor: WebSocketEventMonitor, pattern: Dict[str, str]):
        ...
        try:
            if 'UserExecutionEngine' in pattern['name']:
                ...
                try:
                    engine = await UserExecutionEngine.create_from_legacy(registry=Mock(), websocket_bridge=websocket_bridge, user_context=user_context)
    ...
    async def _create_best_available_engine(self, user_context: UserExecutionContext, monitor: WebSocketEventMonitor):
        """Create best available execution engine."""
        try:
            from netra_backend.app.agents.supervisor.user_execution_engine import UserExecutionEngine
            mock_registry = Mock()
            websocket_bridge = self._create_monitored_websocket_bridge(monitor)
            engine = await UserExecutionEngine.create_from_legacy(registry=mock_registry, websocket_bridge=websocket_bridge, user_context=user_context)
    ...
    def _create_websocket_test_agent_context(self, user_context: UserExecutionContext, pattern: Dict[str, str]) -> AgentExecutionContext:
        ...
        return AgentExecutionContext(agent_name='websocket_test_agent', user_id=user_context.user_id, thread_id=user_context.thread_id, run_id=user_context.run_id, metadata={'test_pattern': pattern['name'], 'websocket_test': True, 'requires_events': True})

    async def _execute_agent_with_comprehensive_monitoring(self, engine, agent_context: AgentExecutionContext, user_context: UserExecutionContext, monitor: WebSocketEventMonitor):
        ...
        async def mock_execution_with_events(*args, **kwargs):
            ...
            if websocket_bridge:
                await websocket_bridge.notify_agent_started(agent_context.run_id, agent_context.agent_name, {'status': 'started'})
                await asyncio.sleep(0.05)
                await websocket_bridge.notify_agent_thinking(agent_context.run_id, agent_context.agent_name, 'Processing WebSocket test...', step_number=1)
                await asyncio.sleep(0.05)
                await websocket_bridge.notify_tool_executing(agent_context.run_id, agent_context.agent_name, 'websocket_tester', {'test': True})
                await asyncio.sleep(0.05)
                await websocket_bridge.notify_tool_completed(agent_context.run_id, agent_context.agent_name, 'websocket_tester', {'result': 'success'})
                await asyncio.sleep(0.05)
                await websocket_bridge.notify_agent_completed(agent_context.run_id, agent_context.agent_name, {'response': 'WebSocket test completed'}, 250.0)
            return AgentExecutionResult(success=True, agent_name=agent_context.agent_name, execution_time=0.25, data={'response': 'WebSocket test completed successfully', 'events_triggered': 5, 'websocket_test': True})
    ...
    async def _validate_websocket_event_flow(self, monitor: WebSocketEventMonitor, pattern_name: str):
        ...
        if event_types[0] != 'agent_started':
            pytest.fail(f"WEBSOCKET SEQUENCE FAILURE [{pattern_name}]: First event must be 'agent_started', got '{event_types[0]}'. Timeline: {event_timeline}")
        if event_types[-1] != 'agent_completed':
            pytest.fail(f"WEBSOCKET SEQUENCE FAILURE [{pattern_name}]: Last event must be 'agent_completed', got '{event_types[-1]}'. Timeline: {event_timeline}")
    ...
    async def _validate_critical_event_sequence(self, monitor: WebSocketEventMonitor, critical_events: List[str]):
        ...
        for critical_event in critical_events:
            if critical_event not in captured_event_types:
                pytest.fail(f"CRITICAL EVENT MISSING: '{critical_event}' not found in sequence. Captured: {captured_event_types}. Critical chat functionality broken.")
        critical_positions = []
        for critical_event in critical_events:
    ...
    def _create_user_isolated_websocket_bridge(self, monitor: WebSocketEventMonitor, user_context: UserExecutionContext, user_index: int):
    ...
    async def _execute_isolated_user_websocket_session(self, engine, user_context: UserExecutionContext, monitor: WebSocketEventMonitor, user_index: int):
        """Execute isolated WebSocket session for specific user."""
        agent_context = AgentExecutionContext(agent_name='isolated_websocket_agent', user_id=user_context.user_id, thread_id=user_context.thread_id, run_id=user_context.run_id, metadata={'user_index': user_index, 'isolation_test': True, 'websocket_test': True})
        return await self._execute_agent_with_comprehensive_monitoring(engine, agent_context, user_context, monitor)

    async def _validate_no_cross_user_events(self, user_monitors: List[WebSocketEventMonitor], user_contexts: List[UserExecutionContext]):
    ...
    async def _create_error_recovery_websocket_engine(self, user_context: UserExecutionContext, monitor: WebSocketEventMonitor):
        ...
        try:
            from netra_backend.app.agents.supervisor.user_execution_engine import UserExecutionEngine
            engine = await UserExecutionEngine.create_from_legacy(registry=Mock(), websocket_bridge=error_bridge, user_context=user_context)
            return engine
        except ImportError:
            from netra_backend.app.agents.supervisor.user_execution_engine import UserExecutionEngine as ExecutionEngine
    ...
    async def _execute_with_websocket_error_simulation(self, engine, agent_context: AgentExecutionContext, user_context: UserExecutionContext, monitor: WebSocketEventMonitor):
    ...
    async def _execute_performance_test_agent(self, engine, agent_context: AgentExecutionContext, user_context: UserExecutionContext):
        ...
        if hasattr(engine, 'websocket_bridge'):
            await engine.websocket_bridge.notify_agent_started(agent_context.run_id, agent_context.agent_name, {'performance_test': True})
            await asyncio.sleep(0.01)
            await engine.websocket_bridge.notify_agent_completed(agent_context.run_id, agent_context.agent_name, {'result': 'performance_test'}, 10.0)
