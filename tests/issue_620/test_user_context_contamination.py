...
class UserContextContaminationTests(SSotAsyncTestCase):
    ...
    async def test_user_context_contamination_reproduction(self):
        ...
        user1_context = UserExecutionContext(user_id='user1_contamination_test', thread_id='thread1_secure', run_id='run1_isolated', request_id='req1_safe', audit_metadata={'secret_user1_data': 'CONFIDENTIAL_USER1_SECRET', 'user1_account_balance': '$50000', 'user1_ssn': '123-45-6789'})
        user2_context = UserExecutionContext(user_id='user2_contamination_test', thread_id='thread2_secure', run_id='run2_isolated', request_id='req2_safe', audit_metadata={'secret_user2_data': 'CONFIDENTIAL_USER2_SECRET', 'user2_account_balance': '$75000', 'user2_ssn': '987-65-4321'})
        ...
        try:
            ...
            task1 = asyncio.create_task(self._execute_user_agent(engine1, user1_context, 'user1_sensitive_query', 'USER1_PRIVATE_RESPONSE'))
            task2 = asyncio.create_task(self._execute_user_agent(engine2, user2_context, 'user2_sensitive_query', 'USER2_PRIVATE_RESPONSE'))
    ...
    async def test_concurrent_user_state_isolation(self):
        ...
        for i in range(num_users):
            context = UserExecutionContext(user_id=f'concurrent_user_{i}', thread_id=f'thread_{i}', run_id=f'run_{i}', request_id=f'req_{i}', audit_metadata={f'user_{i}_secret': f'SECRET_DATA_USER_{i}', f'user_{i}_id': i, 'isolation_test': True})
        ...
        for i, (engine, context) in enumerate(zip(engines, user_contexts)):
            task = asyncio.create_task(self._execute_isolated_user_agent(engine, context, f'query_from_user_{i}', f'RESPONSE_FOR_USER_{i}'))
            tasks.append(task)
        ...
        if contamination_found and execution_engine_class.__name__ == 'ExecutionEngine':
            pytest.fail('ISOLATION VIOLATION: User data contamination detected in concurrent execution. Multiple users sharing execution state - critical security vulnerability.')
    ...
    async def test_memory_state_sharing_vulnerability(self):
        ...
        user1_context = UserExecutionContext(user_id='memory_test_user1', thread_id='memory_thread1', run_id='memory_run1', audit_metadata={'memory_test': 'user1_memory_data'})
        user2_context = UserExecutionContext(user_id='memory_test_user2', thread_id='memory_thread2', run_id='memory_run2', audit_metadata={'memory_test': 'user2_memory_data'})
        mock_registry = self._create_mock_registry()
        mock_websocket_bridge = self._create_mock_websocket_bridge()
        try:
            from netra_backend.app.agents.supervisor.user_execution_engine import UserExecutionEngine as ExecutionEngine
    ...
    async def _execute_user_agent(self, engine, user_context, query, expected_response):
        ...
        agent_context = AgentExecutionContext(agent_name='test_agent', user_id=user_context.user_id, thread_id=user_context.thread_id, run_id=user_context.run_id, metadata={'contamination_test': True})
        with patch.object(engine, 'execute_agent', new_callable=AsyncMock) as mock_execute:
            mock_result = AgentExecutionResult(success=True, agent_name='test_agent', execution_time=1.0, data={'response': expected_response, 'user_context': user_context.audit_metadata})
    ...
    async def _execute_isolated_user_agent(self, engine, user_context, query, expected_response):
        ...
        agent_context = AgentExecutionContext(agent_name='isolation_test_agent', user_id=user_context.user_id, thread_id=user_context.thread_id, run_id=user_context.run_id, metadata=user_context.audit_metadata)
        if hasattr(engine, 'execute_agent'):
            with patch.object(engine, 'execute_agent', new_callable=AsyncMock) as mock_execute:
                mock_result = AgentExecutionResult(success=True, agent_name='isolation_test_agent', execution_time=0.5, data={'response': expected_response, 'user_id': user_context.user_id, 'metadata': user_context.audit_metadata})
                mock_execute.return_value = mock_result
                result = await engine.execute_agent(agent_context, user_context)
                return result
        else:
            return AgentExecutionResult(success=True, agent_name='isolation_test_agent', execution_time=0.5, data={'response': expected_response, 'user_id': user_context.user_id, 'metadata': user_context.metadata})
    ...
    async def _populate_engine_state(self, engine, user_context, history_data):
        ...
        if hasattr(engine, 'run_history'):
            engine.run_history.append({'user_id': user_context.user_id, 'data': history_data, 'timestamp': time.time()})
