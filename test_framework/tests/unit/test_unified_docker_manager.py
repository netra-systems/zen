...
import pytest
from test_framework.unified_docker_manager import UnifiedDockerManager, OrchestrationConfig, EnvironmentType, ServiceMode, ServiceStatus, ContainerState, ServiceHealth, ContainerInfo
...
class TestUnifiedDockerManagerInitialization:
    ...
    def test_init_with_custom_config(self):
        ...
        config = OrchestrationConfig(environment='staging', startup_timeout=120.0, required_services=['backend', 'auth'])
        manager = UnifiedDockerManager(config=config, environment_type=EnvironmentType.STAGING, test_id='test-123', use_production_images=False, mode=ServiceMode.LOCAL, use_alpine=False, rebuild_images=False, rebuild_backend_only=False, pull_policy='always', no_cache_app_code=False)
    ...
    def test_get_database_credentials_default(self, mock_get_env):
        ...
        expected = {'user': 'test_user', 'password': 'test_pass', 'database': 'netra_test'}
    ...
    def test_get_database_credentials_alpine(self, mock_get_env):
        ...
        credentials = manager.get_database_credentials()
        expected = {'user': 'test', 'password': 'test', 'database': 'netra_test'}
        assert credentials == expected

    def test_get_database_credentials_environment_types(self):
        """Test database credentials for different environment types."""
        test_cases = [(EnvironmentType.DEVELOPMENT, 'netra', 'netra123', 'netra_dev'), (EnvironmentType.TEST, 'test_user', 'test_pass', 'netra_test'), (EnvironmentType.STAGING, 'staging_user', 'staging_pass', 'netra_staging'), (EnvironmentType.PRODUCTION, 'netra', 'netra123', 'netra_production'), (EnvironmentType.DEDICATED, 'test_user', 'test_pass', 'netra_test')]
...
class TestEnvironmentDetection:
    ...
    def test_detect_environment_from_running_containers(self, mock_run):
        ...
        mock_run.return_value = Mock(returncode=0, stdout='netra-alpine-test-abc123-backend-1\nnetra-alpine-test-abc123-redis-1')
        manager = UnifiedDockerManager()
    ...
    def test_detect_environment_development_containers(self, mock_run):
        ...
        mock_run.return_value = Mock(returncode=0, stdout='dev-backend\ndev-redis\ndev-postgres')
        manager = UnifiedDockerManager()
        env_type = manager.detect_environment()
    ...
    def test_detect_environment_test_containers(self, mock_run):
        ...
        mock_run.return_value = Mock(returncode=0, stdout='test-backend\ntest-redis')
        manager = UnifiedDockerManager()
        env_type = manager.detect_environment()
        assert env_type == EnvironmentType.TEST
...
class TestEnvironmentNaming:
    ...
    def test_get_environment_name_dedicated_alpine(self):
        ...
        manager = UnifiedDockerManager(environment_type=EnvironmentType.DEDICATED, test_id='abc123', use_alpine=True)
    ...
    def test_get_environment_name_dedicated_regular(self):
        ...
        manager = UnifiedDockerManager(environment_type=EnvironmentType.DEDICATED, test_id='abc123', use_alpine=False)
        name = manager._get_environment_name()
    ...
    def test_get_environment_name_test_alpine(self):
        ...
        manager = UnifiedDockerManager(environment_type=EnvironmentType.TEST, use_alpine=True)
        name = manager._get_environment_name()
    ...
    def test_get_environment_name_test_regular(self):
        ...
        manager = UnifiedDockerManager(environment_type=EnvironmentType.TEST, use_alpine=False)
        name = manager._get_environment_name()
        assert name == 'netra_test'

    def test_get_environment_name_other_types(self):
        """Test environment naming for other environment types."""
        test_cases = [(EnvironmentType.DEVELOPMENT, True, 'netra_alpine_development'), (EnvironmentType.DEVELOPMENT, False, 'netra_development'), (EnvironmentType.STAGING, True, 'netra_alpine_staging'), (EnvironmentType.STAGING, False, 'netra_staging'), (EnvironmentType.PRODUCTION, True, 'netra_alpine_production'), (EnvironmentType.PRODUCTION, False, 'netra_production')]
        for env_type, use_alpine, expected in test_cases:
            manager = UnifiedDockerManager(environment_type=env_type, use_alpine=use_alpine)
            name = manager._get_environment_name()
    ...
    def test_get_project_name_caching(self):
        ...
        manager = UnifiedDockerManager(environment_type=EnvironmentType.DEDICATED, test_id='abc123')
...
class TestPortAllocation:
    ...
    def test_allocate_service_ports_new_allocation(self, mock_allocate):
        ...
        mock_allocate.return_value = {'backend': 8001, 'auth': 8082, 'postgres': 5435, 'redis': 6382}
        manager = UnifiedDockerManager()
...
class TestServiceMapping:
    ...
    def test_map_service_name_development(self):
        ...
        manager = UnifiedDockerManager(environment_type=EnvironmentType.DEVELOPMENT)
        test_cases = [('postgres', 'dev-postgres'), ('redis', 'dev-redis'), ('backend', 'dev-backend'), ('auth', 'dev-auth'), ('frontend', 'dev-frontend'), ('unknown', 'unknown')]
...
class TestNetworkManagement:
    ...
    def test_setup_network_already_exists(self, mock_run):
        ...
        mock_run.side_effect = [Mock(returncode=1, stderr='already exists'), Mock(returncode=0)]
        manager = UnifiedDockerManager()
        result = manager._setup_network()
        assert result is True
...
class TestServiceStatusChecking:
    ...
    def test_is_service_healthy_with_health_data(self):
        ...
        manager = UnifiedDockerManager()
        manager.service_health['backend'] = ServiceHealth(service_name='backend', is_healthy=True, port=8000, response_time_ms=50.0)
...
class TestAsyncHealthChecking:
    ...
    async def test_check_service_health_async(self):
        ...
        manager = UnifiedDockerManager()
        mapping = ServicePortMapping(service_name='backend', host_port=8000, container_port=8000, host='localhost', is_healthy=True, health_url='http://localhost:8000/health')
...
class TestEnvironmentManagement:
    ...
    def test_acquire_environment_existing_available(self, mock_save, mock_load):
        ...
        existing_ports = {'backend': 8001, 'auth': 8082}
        mock_load.return_value = {'environments': {'test-env': {'users': 0, 'ports': existing_ports, 'type': 'dedicated'}}}
        manager = UnifiedDockerManager()
        env_name, ports = manager.acquire_environment()
        assert ports == existing_ports
        mock_save.assert_called()
    ...
    def test_release_environment(self, mock_save, mock_load):
        ...
        mock_load.return_value = {'environments': {'test-env': {'users': 1, 'ports': {'backend': 8001}}}}
        manager = UnifiedDockerManager()
...
class TestServiceEnvironmentVariables:
    ...
    def test_service_environment_variables_include_common_vars(self):
        ...
        manager = UnifiedDockerManager()
        env_vars = manager._get_service_environment_variables()
        required_vars = ['POSTGRES_HOST', 'POSTGRES_USER', 'POSTGRES_PASSWORD', 'POSTGRES_DB', 'REDIS_HOST', 'AUTH_SERVICE_URL', 'BACKEND_SERVICE_URL']
...
class TestMemoryMonitoring:
    ...
    def test_get_container_memory_info_success(self, mock_run):
        ...
        mock_run.return_value = Mock(returncode=0, stdout='50MiB / 1GiB')
        manager = UnifiedDockerManager()
    ...
    def test_check_container_memory_thresholds(self, mock_run):
        ...
        mock_run.return_value = Mock(returncode=0, stdout='backend,85.5\nauth,45.2\npostgres,92.1')
        manager = UnifiedDockerManager()
...
class TestContainerIntrospection:
    ...
    def test_get_enhanced_container_status_success(self, mock_run):
        ...
        mock_inspect_data = {'Name': '/test-backend', 'Config': {'Image': 'netra/backend:latest'}, 'State': {'Status': 'running', 'Health': {'Status': 'healthy'}, 'StartedAt': '2023-01-01T00:00:00Z', 'ExitCode': 0}, 'NetworkSettings': {'Ports': {'8000/tcp': [{'HostPort': '8000'}]}}}
        mock_run.return_value = Mock(returncode=0, stdout=json.dumps([mock_inspect_data]))
        manager = UnifiedDockerManager()
...
class TestCleanupOperations:
    ...
    def test_cleanup_orphaned_containers_success(self, mock_run):
        ...
        mock_run.side_effect = [Mock(returncode=0, stdout='orphaned-container-1\norphaned-container-2'), Mock(returncode=0), Mock(returncode=0), Mock(returncode=0), Mock(returncode=0)]
        manager = UnifiedDockerManager()
    ...
    def test_cleanup_old_environments(self, mock_save, mock_load):
        ...
        mock_load.return_value = {'environments': {'old-env': {'type': 'dedicated', 'created': old_time, 'users': 0}, 'recent-env': {'type': 'dedicated', 'created': recent_time, 'users': 0}, 'test-env': {'type': 'test', 'created': old_time, 'users': 0}}}
        manager = UnifiedDockerManager()
...
class TestWindowsSupport:
    ...
    def test_get_docker_service_status_windows(self, mock_run, mock_platform):
        ...
        mock_run.return_value = Mock(returncode=0, stdout='Docker Desktop Service,Running')
        manager = UnifiedDockerManager()
    ...
    def test_get_windows_event_logs(self, mock_run, mock_platform):
        ...
        mock_run.return_value = Mock(returncode=0, stdout='Event log entry 1\nEvent log entry 2')
        manager = UnifiedDockerManager()
...
class TestServiceUrlGeneration:
    ...
    def test_get_e2e_service_urls_with_allocated_ports(self):
        ...
        manager = UnifiedDockerManager()
        manager.allocated_ports = {'backend': 8001, 'auth': 8082, 'frontend': 3001}
    ...
    def test_build_service_url_from_mapping(self):
        ...
        manager = UnifiedDockerManager()
        mapping = ServicePortMapping(service_name='backend', host_port=8001, container_port=8000, host='localhost', is_healthy=True, health_url='http://localhost:8001/health')
...
class TestComplexWorkflows:
    ...
    def test_container_discovery_workflow(self, mock_run):
        ...
        mock_run.return_value = Mock(returncode=0, stdout='netra-test-backend-1:8000->8001/tcp\nnetra-test-auth-1:8081->8082/tcp')
        manager = UnifiedDockerManager()
