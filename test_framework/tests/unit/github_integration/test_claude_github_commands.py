...
class TestClaudeGitHubCommands(SSotBaseTestCase):
    ...
    def mock_github_issue_manager(self):
        ...
        manager = Mock()
        manager.create_issue_from_error_context.return_value = Mock(number=123, html_url='https://github.com/test/repo/issues/123')
    ...
    def mock_command_context(self):
        ...
        return {'user_id': 'user_123', 'thread_id': 'thread_456', 'session_id': 'session_789', 'current_error': 'Test error occurred', 'error_context': {'error_type': 'ValidationError', 'stack_trace': 'Traceback...', 'context_data': {'step': 'validation'}}}
    ...
    def test_create_github_issue_command_fails(self, mock_github_issue_manager, mock_command_context):
        ...
        with pytest.raises((ImportError, NameError, ModuleNotFoundError)):
            ...
            command = MockClaudeCommand(command='create-github-issue', args=['Bug in agent execution'], flags={'priority': 'high', 'labels': ['bug', 'agent']}, user_id='user_123', thread_id='thread_456', timestamp='2024-01-01T10:00:00Z')
    ...
    def test_link_github_issue_command_fails(self, mock_github_issue_manager, mock_command_context):
        ...
        with pytest.raises((ImportError, NameError, ModuleNotFoundError)):
            ...
            command = MockClaudeCommand(command='link-github-issue', args=['123'], flags={}, user_id='user_123', thread_id='thread_456', timestamp='2024-01-01T10:00:00Z')
            mock_github_issue_manager.get_issue.return_value = Mock(number=123, html_url='https://github.com/test/repo/issues/123', title='Test Issue')
            result = handler.execute_command(command, mock_command_context)
            assert result.success is True
            assert 'Linked to GitHub issue #123' in result.message
            assert result.github_issue_url == 'https://github.com/test/repo/issues/123'
            handler.thread_linker.link_thread_to_github_issue.assert_called_once_with('thread_456', 123)
    ...
    def test_search_github_issues_command_fails(self, mock_github_issue_manager, mock_command_context):
        ...
        with pytest.raises((ImportError, NameError, ModuleNotFoundError)):
            ...
            handler = GitHubCommandHandler()
            handler.github_manager = mock_github_issue_manager
            command = MockClaudeCommand(command='search-github-issues', args=['agent execution error'], flags={'state': 'open', 'labels': 'bug'}, user_id='user_123', thread_id='thread_456', timestamp='2024-01-01T10:00:00Z')
            mock_github_issue_manager.search_issues.return_value = [Mock(number=100, title='Agent execution failure', html_url='https://github.com/test/repo/issues/100', state='open'), Mock(number=101, title='Similar agent error', html_url='https://github.com/test/repo/issues/101', state='open')]
            result = handler.execute_command(command, mock_command_context)
            assert result.success is True
            assert 'Found 2 related issues' in result.message
            assert len(result.data['issues']) == 2
            mock_github_issue_manager.search_issues.assert_called_once_with(query='agent execution error', state='open', labels='bug')
    ...
    def test_command_validation_fails(self):
        ...
        with pytest.raises((ImportError, NameError, ModuleNotFoundError)):
            ...
            handler = GitHubCommandHandler()
            invalid_command = MockClaudeCommand(command='create-github-issue', args=[], flags={}, user_id='user_123', thread_id='thread_456', timestamp='2024-01-01T10:00:00Z')
            with pytest.raises(CommandValidationError):
                handler.validate_command(invalid_command)
            invalid_link_command = MockClaudeCommand(command='link-github-issue', args=[], flags={}, user_id='user_123', thread_id='thread_456', timestamp='2024-01-01T10:00:00Z')
            with pytest.raises(CommandValidationError):
    ...
    def test_command_permissions_fails(self, mock_command_context):
        ...
        with pytest.raises((ImportError, NameError, ModuleNotFoundError)):
            ...
            handler = GitHubCommandHandler()
            command = MockClaudeCommand(command='create-github-issue', args=['Test issue'], flags={}, user_id='unauthorized_user', thread_id='thread_456', timestamp='2024-01-01T10:00:00Z')
    ...
    def test_command_error_handling_fails(self, mock_github_issue_manager, mock_command_context):
        ...
        with pytest.raises((ImportError, NameError, ModuleNotFoundError)):
            ...
            handler = GitHubCommandHandler()
            handler.github_manager = mock_github_issue_manager
            mock_github_issue_manager.create_issue_from_error_context.side_effect = GitHubAPIError('API rate limit exceeded')
            command = MockClaudeCommand(command='create-github-issue', args=['Test issue'], flags={}, user_id='user_123', thread_id='thread_456', timestamp='2024-01-01T10:00:00Z')
            result = handler.execute_command(command, mock_command_context)
...
class TestGitHubCommandIntegration(SSotBaseTestCase):
    ...
    def test_command_execution_through_claude_system_fails(self):
        ...
        with pytest.raises((ImportError, NameError, ModuleNotFoundError)):
            ...
            result = processor.process_command(user_input, user_id='user_123', thread_id='thread_456')
            assert result.success is True
            assert 'GitHub issue created' in result.message
    ...
    def test_command_history_tracking_fails(self):
        ...
        with pytest.raises((ImportError, NameError, ModuleNotFoundError)):
            ...
            command = MockClaudeCommand(command='create-github-issue', args=['Test issue'], flags={'priority': 'high'}, user_id='user_123', thread_id='thread_456', timestamp='2024-01-01T10:00:00Z')
