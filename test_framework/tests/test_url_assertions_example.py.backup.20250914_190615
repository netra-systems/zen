...
class TestConfigURLAssertions:
    ...
    def test_config_structure_flexible_ports(self):
        ...
        """
        # Simulate config that might have dynamic ports
        config = Mock()
        config.api_url = "http://localhost:8001"  # Might be 8000, 8001, etc.
        config.auth_url = "http://localhost:8082"  # Might be 8081, 8082, etc.
        
        # Test structure without asserting exact ports
        assert_is_localhost_url(config.api_url)
        assert_is_localhost_url(config.auth_url)
        
        # Check base URLs match expected pattern
        ...
        # Can still extract and validate ports are present
    ...
    def test_development_config_exact_ports(self):
        ...
        with patch.dict(os.environ, {'ENVIRONMENT': 'development'}):
            # Simulate getting development config
            config = Mock()
            config.api_url = "http://localhost:8000"
            config.auth_url = "http://localhost:8081"
            config.frontend_url = "http://localhost:3000"
            
            # Development should use exact standard ports
    ...
    def test_staging_config_no_localhost(self):
        ...
        with patch.dict(os.environ, {'ENVIRONMENT': 'staging'}):
            ...
            # Exact URLs for staging are critical
    ...
    def test_dynamic_port_discovery(self):
        ...
        """
        # Simulate dynamic port discovery
        discovered_ports = {
            'backend': 8005,
            'auth': 8086,
            'frontend': 3002
        ...
        # Build config with discovered ports
        config = Mock()
        ...
        # Verify ports match discovered values
    ...
    def test_url_assertion_context_manager(self):
        ...
        """
        configs = [
            Mock(api_url="http://localhost:8000", auth_url="http://localhost:8081"),
            Mock(api_url="http://localhost:8002", auth_url="http://localhost:8083"),
            Mock(api_url="http://localhost:8004", auth_url="http://localhost:8085"),
        ]
        
        # Use context manager to ignore ports for all assertions
        with URLAssertion(ignore_port=True) as url_assert:
            for config in configs:
                # These will only check scheme and hostname, not port
    ...
    def test_mixed_assertion_patterns(self):
        ...
        # Exact assertions for localhost services (known ports)
        assert config.api_url == "http://localhost:8000"
        assert config.auth_url == "http://localhost:8081"
        
        # Flexible assertion for external URLs (don't care about port)
        assert_base_url_matches(config.external_api, "https://api.example.com", ignore_port=True)
        
        # Check webhook has correct base and path
        assert config.webhook_url.startswith("https://webhook.example.com")
        assert "/callback" in config.webhook_url
        
        # Extract and validate webhook port separately if needed
...
class TestURLAssertionHelpers:
    ...
    def test_extract_port(self):
        ...
        assert extract_port("http://example.com") is None  # Default port
        assert extract_port("https://example.com") is None  # Default port
