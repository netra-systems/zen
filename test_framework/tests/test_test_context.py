...
from test_framework.test_context import TestContext, TestUserContext, WebSocketEventCapture, create_test_context, create_isolated_test_contexts
...
class TestTestUserContext:
    ...
    def test_user_context_custom_values(self):
        ...
        context = TestUserContext(user_id='custom_user', email='custom@example.com', name='Custom User', role='admin', thread_id='custom_thread', session_id='custom_session')
...
class TestWebSocketEventCapture:
    ...
    def test_get_events_by_type(self):
        ...
        capture = WebSocketEventCapture()
        events = [{'type': 'agent_started', 'id': 1}, {'type': 'agent_thinking', 'id': 2}, {'type': 'agent_started', 'id': 3}, {'type': 'tool_executing', 'id': 4}]
        for event in events:
...
class TestTestContext:
    ...
    def test_get_captured_events(self):
        ...
        context = TestContext()
        events = [{'type': 'agent_started', 'id': 1}, {'type': 'agent_thinking', 'id': 2}, {'type': 'agent_started', 'id': 3}]
        for event in events:
...
class TestFactoryFunctions:
    ...
    def test_create_test_context_custom_params(self):
        ...
        context = create_test_context(user_id='custom_user_456', jwt_token='test_token_123', websocket_timeout=20.0)
...
class TestIntegrationScenarios:
    ...
    async def test_multiple_contexts_isolation(self):
        ...
        contexts = create_isolated_test_contexts(count=3)
        event_types = [['agent_started', 'agent_thinking'], ['tool_executing', 'tool_completed'], ['agent_completed', 'websocket_connected']]
    ...
    async def test_agent_event_validation_workflow(self):
        ...
        context = TestContext()
        agent_events_sequence = [{'type': 'agent_started', 'agent': 'test_agent'}, {'type': 'agent_thinking', 'thought': 'Processing request'}, {'type': 'tool_executing', 'tool': 'search_tool'}, {'type': 'tool_completed', 'tool': 'search_tool', 'result': 'data'}, {'type': 'agent_completed', 'response': 'Task completed'}]
    ...
    async def test_event_capture_timing_validation(self):
        ...
        events_with_delays = [('agent_started', 0.1), ('agent_thinking', 0.1), ('tool_executing', 0.2), ('tool_completed', 0.1), ('agent_completed', 0.1)]
