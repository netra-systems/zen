...
class TestCategorySystemValidation:
    ...
    def test_core_test_categories_have_registrations(self):
        ...
        core_categories = {'mission_critical', 'unit', 'integration', 'api', 'websocket', 'e2e', 'smoke', 'database', 'security'}
        existing_core_dirs = []
        if tests_dir.exists():
        ...
        assert not missing_core_registrations, f'Core test directories found without registered categories: {missing_core_registrations}. These core directories exist but are not registered in CategorySystem. This can cause test runner status reporting inconsistency like Issue #155. Please add them to CategorySystem._initialize_default_categories()'
    ...
    def test_mission_critical_category_registered(self):
        ...
        assert mission_critical is not None, 'mission_critical category must be registered in CategorySystem. This category has a test directory but was missing from registration.'
    ...
    def test_execution_plan_tracks_requested_categories(self):
        ...
        assert hasattr(execution_plan, 'requested_categories'), 'ExecutionPlan must have requested_categories field to support proper exit code logic'
        assert execution_plan.requested_categories == {'unit'}, f"ExecutionPlan should track requested categories. Expected {{'unit'}}, got {execution_plan.requested_categories}"
