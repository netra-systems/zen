#!/usr/bin/env python3
"""
Test suite for LayerExecutionManager
...
# Import the LayerExecutionManager and related components
from test_framework.orchestration.layer_execution_manager import (
    LayerExecutionManager, LayerExecutionConfig, LayerExecutionResult,
...
class TestLayerExecutionManager:
    """Test suite for LayerExecutionManager functionality"""
...
'''
    ...
    @pytest.fixture
    def layer_execution_manager(self, temp_project_root):
        """Create LayerExecutionManager for testing"""
        return LayerExecutionManager(temp_project_root)
    ...
    def test_agent_initialization(self, temp_project_root):
        """Test LayerExecutionManager initialization"""
        agent = LayerExecutionManager(temp_project_root)
    ...
    def test_get_available_layers(self, layer_execution_manager):
        """Test getting available layers"""
        layers = layer_execution_manager.get_available_layers()
    ...
    def test_get_layer_categories(self, layer_execution_manager):
        """Test getting categories for a layer"""
        categories = layer_execution_manager.get_layer_categories("fast_feedback")
        
        # Should have smoke and unit categories
        assert "smoke" in categories
        assert "unit" in categories
        
    def test_validate_layer_configuration(self, layer_execution_manager):
        """Test layer configuration validation"""
        # Valid layer
        issues = layer_execution_manager.validate_layer_configuration("fast_feedback")
        assert isinstance(issues, list)
        
        # Invalid layer
        issues = layer_execution_manager.validate_layer_configuration("nonexistent")
        assert len(issues) > 0
    ...
    @pytest.mark.asyncio
    async def test_execute_sequential_strategy(self, layer_execution_manager, mock_layer, sample_layer_config):
        ...
        layer_execution_manager.layer_system.layers["test_layer"] = mock_layer
        
        # Mock command execution
        with patch.object(layer_execution_manager, '_execute_command') as mock_execute:
            ...
            result = await layer_execution_manager._execute_sequential(mock_layer, sample_layer_config)
    ...
    @pytest.mark.asyncio
    async def test_execute_parallel_strategy(self, layer_execution_manager, mock_layer, sample_layer_config):
        ...
        with patch.object(layer_execution_manager, '_execute_command') as mock_execute:
            ...
            result = await layer_execution_manager._execute_parallel(mock_layer, sample_layer_config)
    ...
    @pytest.mark.asyncio
    async def test_execute_hybrid_strategy(self, layer_execution_manager, mock_layer, sample_layer_config):
        ...
        with patch.object(layer_execution_manager, '_execute_command') as mock_execute:
            ...
            result = await layer_execution_manager._execute_hybrid(mock_layer, sample_layer_config)
    ...
    @pytest.mark.asyncio
    async def test_execute_single_category_success(self, layer_execution_manager, mock_layer, sample_layer_config):
        ...
        with patch.object(layer_execution_manager, '_execute_command') as mock_execute:
            ...
            result = await layer_execution_manager._execute_single_category("smoke", mock_layer, sample_layer_config)
    ...
    @pytest.mark.asyncio 
    async def test_execute_single_category_failure(self, layer_execution_manager, mock_layer, sample_layer_config):
        ...
        with patch.object(layer_execution_manager, '_execute_command') as mock_execute:
            ...
            result = await layer_execution_manager._execute_single_category("unit", mock_layer, sample_layer_config)
    ...
    def test_build_category_command_basic(self, layer_execution_manager):
        ...
        cmd = layer_execution_manager._build_category_command("unit", config)
    ...
    def test_build_category_command_with_flags(self, layer_execution_manager):
        ...
        cmd = layer_execution_manager._build_category_command("integration", config)
    ...
    def test_parse_test_counts_pytest(self, layer_execution_manager):
        ...
        counts = layer_execution_manager._parse_test_counts(pytest_output)
        
        assert counts["passed"] == 12
        assert counts["failed"] == 3
        assert counts["total"] == 15
        
    def test_parse_test_counts_jest(self, layer_execution_manager):
        ...
        counts = layer_execution_manager._parse_test_counts(jest_output)
    ...
    @pytest.mark.asyncio
    async def test_allocate_resources(self, layer_execution_manager, mock_layer, sample_layer_config):
        """Test resource allocation for layer execution"""
        result = await layer_execution_manager._allocate_resources(mock_layer, sample_layer_config)
        
        assert result is True
        assert len(layer_execution_manager.allocated_resources) == 1
        
        # Check resource allocation details
        resource_key = list(layer_execution_manager.allocated_resources.keys())[0]
        allocated = layer_execution_manager.allocated_resources[resource_key]
    ...
    @pytest.mark.asyncio
    async def test_release_resources(self, layer_execution_manager, mock_layer, sample_layer_config):
        """Test resource release after layer execution"""
        # First allocate resources
        await layer_execution_manager._allocate_resources(mock_layer, sample_layer_config)
        assert len(layer_execution_manager.allocated_resources) == 1
        
        # Then release them
        await layer_execution_manager._release_resources(mock_layer)
        assert len(layer_execution_manager.allocated_resources) == 0
        
    # Test execution flow
    
    @pytest.mark.asyncio
    async def test_full_layer_execution_success(self, layer_execution_manager, sample_layer_config, temp_project_root):
        ...
        layer_execution_manager.layer_system.layers["test_layer"] = test_layer
        ...
        with patch.object(layer_execution_manager, '_execute_command') as mock_execute:
            ...
            result = await layer_execution_manager.execute_layer("test_layer", sample_layer_config)
    ...
    @pytest.mark.asyncio
    async def test_full_layer_execution_failure(self, layer_execution_manager, sample_layer_config):
        ...
        layer_execution_manager.layer_system.layers["failing_layer"] = test_layer
        ...
        with patch.object(layer_execution_manager, '_execute_command') as mock_execute:
            ...
            result = await layer_execution_manager.execute_layer("failing_layer", sample_layer_config)
    ...
    @pytest.mark.asyncio
    async def test_execution_with_nonexistent_layer(self, layer_execution_manager, sample_layer_config):
        """Test execution with nonexistent layer"""
        sample_layer_config.layer_name = "nonexistent_layer"
        
        result = await layer_execution_manager.execute_layer("nonexistent_layer", sample_layer_config)
    ...
    @pytest.mark.asyncio
    async def test_fail_fast_enabled(self, layer_execution_manager, sample_layer_config):
        ...
        layer_execution_manager.layer_system.layers["fail_fast_layer"] = test_layer
        ...
        with patch.object(layer_execution_manager, '_execute_command', side_effect=mock_execute_command):
            result = await layer_execution_manager.execute_layer("fail_fast_layer", sample_layer_config)
    ...
    @pytest.mark.asyncio
    async def test_cancel_execution(self, layer_execution_manager, sample_layer_config):
        """Test execution cancellation"""
        ...
        with patch.object(layer_execution_manager, '_execute_command', side_effect=long_running_execution):
            # Start execution in background
            execution_task = asyncio.create_task(
                layer_execution_manager.execute_layer("fast_feedback", sample_layer_config)
            )
            
            # Wait a moment then cancel
            await asyncio.sleep(0.1)
            layer_execution_manager.cancel_execution()
    ...
    def test_get_execution_status(self, layer_execution_manager, sample_layer_config):
        """Test execution status reporting"""
        # Initially no execution
        status = layer_execution_manager.get_execution_status()
        assert status["active"] is False
        assert status["current_layer"] is None
        
        # Mock active execution
        with layer_execution_manager.execution_lock:
            layer_execution_manager.current_execution = sample_layer_config
            
        status = layer_execution_manager.get_execution_status()
        assert status["active"] is True
        assert status["current_layer"] == "fast_feedback"
        
    def test_get_execution_summary(self, layer_execution_manager):
        """Test comprehensive execution summary"""
        summary = layer_execution_manager.get_execution_summary()
    ...
    @pytest.mark.asyncio
    async def test_health_check(self, layer_execution_manager):
        """Test agent health check functionality"""
        health = await layer_execution_manager.health_check()
    ...
    @pytest.mark.asyncio
    async def test_command_timeout_handling(self, layer_execution_manager, sample_layer_config):
        ...
        with patch.object(layer_execution_manager, '_execute_command', side_effect=timeout_command):
            # Set short timeout
            sample_layer_config.timeout_multiplier = 0.001  # Very short timeout
            
            result = await layer_execution_manager._execute_single_category("smoke", 
                                                                        layer_execution_manager.layer_system.layers["fast_feedback"], 
                                                                        sample_layer_config)
    ...
    def test_layer_system_integration(self, layer_execution_manager):
        """Test integration with LayerSystem"""
        # Should have access to layer system
        assert layer_execution_manager.layer_system is not None
        assert hasattr(layer_execution_manager.layer_system, 'layers')
        assert hasattr(layer_execution_manager.layer_system, 'create_execution_plan')
        
    def test_category_system_integration(self, layer_execution_manager):
        """Test integration with CategorySystem"""
        # Should have access to category system
        assert layer_execution_manager.category_system is not None
        assert hasattr(layer_execution_manager.category_system, 'categories')
        assert hasattr(layer_execution_manager.category_system, 'get_category')
        
    # Test execution group analysis
    
    def test_analyze_execution_groups(self, layer_execution_manager):
        ...
        groups = layer_execution_manager._analyze_execution_groups(categories)
    ...
    @pytest.mark.asyncio
    async def test_execution_exception_handling(self, layer_execution_manager, sample_layer_config):
        """Test handling of execution exceptions"""
        # Mock command execution that raises exception
        with patch.object(layer_execution_manager, '_execute_command', side_effect=RuntimeError("Test error")):
            result = await layer_execution_manager._execute_single_category("smoke", 
                                                                        layer_execution_manager.layer_system.layers["fast_feedback"],
                                                                        sample_layer_config)
    ...
    def test_concurrent_execution_prevention(self, layer_execution_manager, sample_layer_config):
        ...
        with pytest.raises(RuntimeError, match="execution already in progress"):
            asyncio.run(layer_execution_manager.execute_layer("other_layer", new_config))
            
    # Test layer-specific behavior
    
    def test_fast_feedback_layer_behavior(self, layer_execution_manager):
    ...
    def test_e2e_background_layer_behavior(self, layer_execution_manager):
        ...
        layer_info = layer_execution_manager.layer_system.layers.get("e2e_performance")
        if layer_info:
    ...
    def test_layer_execution_config_validation(self, layer_execution_manager):
    ...
    def test_test_runner_integration_setup(self, layer_execution_manager, temp_project_root):
        """Test setup of integration with existing test runner"""
        assert layer_execution_manager.test_runner_path == temp_project_root / "scripts" / "unified_test_runner.py"
        assert layer_execution_manager.python_executable == sys.executable
    ...
    async def test_execute_layer_async_function(self, temp_project_root):
        ...
        with patch('test_framework.orchestration.layer_execution_manager.LayerExecutionManager') as mock_agent_class:
            mock_agent = Mock()
    ...
    def test_execute_layer_sync_function(self, temp_project_root):
        ...
        # Mock successful execution
        with patch('test_framework.orchestration.layer_execution_manager.execute_layer_async') as mock_async:
            mock_result = LayerExecutionResult(
                success=True,
                layer_name="fast_feedback", 
                categories_executed=["smoke"],
    ...
    def test_execution_stats_tracking(self, layer_execution_manager):
        """Test execution statistics tracking"""
        initial_stats = layer_execution_manager.execution_stats.copy()
        ...
        layer_execution_manager._update_execution_stats(mock_result)
        
        # Check stats were updated
        assert layer_execution_manager.execution_stats["layers_executed"] == initial_stats["layers_executed"] + 1
        assert layer_execution_manager.execution_stats["categories_executed"] == initial_stats["categories_executed"] + 2
        
    # Test layer-specific logic implementation
    
    def test_fast_feedback_layer_characteristics(self, layer_execution_manager):
        """Test fast_feedback layer meets requirements"""
        layer = layer_execution_manager.layer_system.layers.get("fast_feedback")
        if layer:
    ...
    def test_core_integration_layer_characteristics(self, layer_execution_manager):
        """Test core_integration layer meets requirements"""
        layer = layer_execution_manager.layer_system.layers.get("core_integration")
        if layer:
    ...
    def test_service_integration_layer_characteristics(self, layer_execution_manager):
        """Test service_integration layer meets requirements"""
        layer = layer_execution_manager.layer_system.layers.get("service_integration")
        if layer:
    ...
    def test_e2e_background_layer_characteristics(self, layer_execution_manager):
        """Test e2e_background layer meets requirements"""
        layer = layer_execution_manager.layer_system.layers.get("e2e_performance")  # Maps to e2e_background
        if layer:
            # Should support background execution
...
class TestLayerExecutionIntegration:
    """Integration tests for LayerExecutionManager with existing systems"""
    
    @pytest.fixture
    def integration_agent(self):
        """Create agent for integration testing"""
        project_root = Path(__file__).parent.parent.parent
        return LayerExecutionManager(project_root)
...
class TestLayerExecutionPerformance:
    """Performance tests for LayerExecutionManager"""
    
    @pytest.mark.asyncio
    async def test_parallel_execution_performance(self, layer_execution_manager, temp_project_root):
        ...
        with patch.object(layer_execution_manager, '_execute_command', side_effect=mock_delay_execute):
            # Test parallel execution
            start_time = time.time()
            parallel_result = await layer_execution_manager._execute_parallel(parallel_layer, 
                                                                           LayerExecutionConfig(layer_name="parallel_test"))
            parallel_duration = time.time() - start_time
            
            # Test sequential execution  
            start_time = time.time()
            sequential_result = await layer_execution_manager._execute_sequential(sequential_layer,
