...
from test_framework.orchestration.layer_execution_agent import LayerExecutionAgent, LayerExecutionConfig, ExecutionStrategy
...
class TestLayerExecutionIntegration:
    ...
    async def test_command_building_integration(self, integration_agent):
        ...
        config = LayerExecutionConfig(layer_name='fast_feedback', environment='test', execution_strategy=ExecutionStrategy.SEQUENTIAL)
    ...
    def test_execution_summary_integration(self, integration_agent):
        ...
        required_sections = ['agent_id', 'project_root', 'execution_stats', 'available_layers', 'current_status', 'layer_system_summary']
    ...
    def test_layer_system_configuration_integrity(self, integration_agent):
        ...
        critical_issues = [issue for issue in config_issues if any((word in issue.lower() for word in ['circular', 'dependency', 'error']))]
        if config_issues:
    ...
    async def test_mock_layer_execution_flow(self, integration_agent):
        ...
        config = LayerExecutionConfig(layer_name='fast_feedback', execution_strategy=ExecutionStrategy.SEQUENTIAL, environment='test', timeout_multiplier=0.1, fail_fast_enabled=False)
        ...
        async def mock_successful_command(cmd, config):
            ...
            for i, arg in enumerate(cmd):
                if arg == '--category' and i + 1 < len(cmd):
                    ...
                    break
            if category == 'smoke':
                return {'success': True, 'output': '5 passed in 0.5s', 'errors': '', 'return_code': 0}
            elif category == 'unit':
                return {'success': True, 'output': '25 passed in 2.1s', 'errors': '', 'return_code': 0}
            else:
                return {'success': True, 'output': '10 passed in 1.0s', 'errors': '', 'return_code': 0}
    ...
    async def test_mock_layer_execution_with_failure(self, integration_agent):
        ...
        config = LayerExecutionConfig(layer_name='core_integration', execution_strategy=ExecutionStrategy.PARALLEL, environment='test', timeout_multiplier=0.1, fail_fast_enabled=False)
        call_count = 0
        ...
        async def mock_mixed_results_command(cmd, config):
            ...
            if category == 'database':
                return {'success': False, 'output': '8 passed, 2 failed in 3.2s', 'errors': 'Database connection tests failed', 'return_code': 1}
            else:
                return {'success': True, 'output': '15 passed in 2.0s', 'errors': '', 'return_code': 0}
    ...
    def test_background_execution_layer_detection(self, integration_agent):
        ...
        layers = integration_agent.layer_system.layers
        background_layers = [name for name, layer in layers.items() if layer.background_execution]
        ...
        found_background_pattern = any((any((pattern in layer_name.lower() for pattern in background_layer_patterns)) for layer_name in background_layers))
    ...
    async def test_timeout_handling_integration(self, integration_agent):
        ...
        config = LayerExecutionConfig(layer_name='fast_feedback', timeout_multiplier=0.01, environment='test')
        ...
        with patch.object(integration_agent, '_execute_command', side_effect=slow_command):
            result = await integration_agent._execute_single_category('smoke', integration_agent.layer_system.layers['fast_feedback'], config)
...
class TestRealSystemCompatibility:
    ...
    def test_unified_test_runner_compatibility(self, project_root):
        ...
        if test_runner_path.exists():
            try:
                result = subprocess.run([sys.executable, str(test_runner_path), '--help'], capture_output=True, text=True, timeout=10, cwd=str(project_root))
