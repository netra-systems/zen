#!/usr/bin/env python3
"""
Test suite for ResourceManagementAgent
...
from test_framework.orchestration.resource_management_agent import (
    ResourceManagementAgent, ResourceStatus, ServiceStatus,
...
class TestResourceManagementAgent:
    """Test suite for ResourceManagementAgent core functionality."""
    ...
    def resource_manager(self, mock_env):
        """Create ResourceManagementAgent for testing."""
        # Disable monitoring for tests to avoid thread interference
        manager = ResourceManagementAgent(
    ...
    def mock_psutil(self):
        ...
        with patch('test_framework.orchestration.resource_management_agent.psutil') as mock:
            # Mock CPU metrics
    ...
    def test_initialization(self, resource_manager):
        """Test ResourceManagementAgent initialization."""
    ...
    @patch('test_framework.orchestration.resource_management_agent.DockerPortDiscovery')
    @patch('test_framework.orchestration.resource_management_agent.ServiceAvailabilityChecker')
    ...
    def test_context_manager(self):
        """Test ResourceManagementAgent as context manager."""
...
class TestResourceManagerUtilities:
    """Test utility functions for ResourceManagementAgent."""
    ...
    def test_create_resource_manager(self):
        ...
        assert isinstance(rm, ResourceManagementAgent)
        assert not rm.enable_monitoring
        rm.shutdown()
    
    @patch('test_framework.orchestration.resource_management_agent.time.sleep')
    def test_ensure_layer_resources_available_success(self, mock_sleep):
        """Test successful layer resource availability check."""
        with patch('test_framework.orchestration.resource_management_agent.ResourceManagementAgent') as MockRM:
            mock_rm = Mock()
...
class TestResourceManagerIntegration:
    """Integration tests for ResourceManagementAgent with real components."""
