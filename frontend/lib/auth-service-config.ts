...
/**
 * Auth service API client - delegates to unified client
 */
export class AuthServiceClient {
  ...
  /**
   * Initialize with dynamic configuration (no-op in unified system)
   */
  async initWithDynamicConfig(): Promise<void> {
    logger.debug('Auth service client using unified configuration');
  }
   ...
   */
  async getAuthConfig() {
    return authServiceClient.getAuthConfig();
  }
   ...
   */
  async getSession() {
    return authServiceClient.getSession();
  }
  ...
  async getCurrentUser() {
    return authServiceClient.getCurrentUser();
  }
   ...
   */
  async refreshToken() {
    return authServiceClient.refreshToken();
  }
}
