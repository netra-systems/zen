...
class UnifiedTokenTester:
    ...
    async def dev_login_get_token(self) -> Optional[str]:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                print(f"[PASS] Dev login successful: {user_info.get('email')} (ID: {user_info.get('id')})")
                print(f"[INFO] Token: {token[:30]}..." if token else "[FAIL] No token received")
                return token
            else:
                print(f"[FAIL] Dev login failed: {response.status_code}")
                print(f"[INFO] Response: {response.text}")
                return None
        except Exception as e:
            print(f"[FAIL] Dev login error: {e}")
            return None
    ...
    async def validate_token_auth_service(self, token: str) -> Dict:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                print(f"[PASS] Auth validation success: valid={result.get('valid')}, user={result.get('user_id')}")
                return result
            else:
                print(f"[FAIL] Auth validation failed: {response.status_code}")
                return {"valid": False, "error": f"HTTP {response.status_code}"}
        except Exception as e:
            print(f"[FAIL] Auth validation error: {e}")
    ...
    async def test_token_in_backend(self, token: str) -> Dict:
        ...
        try:
            ...
            if success:
                print("[PASS] Backend accepts token at /health")
            else:
                print(f"[FAIL] Backend rejects token at /health: {response.status_code}")
            ...
            print(f"[FAIL] Backend token test error: {e}")
...
async def run_comprehensive_token_tests():
    ...
    try:
        ...
        if not auth_healthy:
            print("[FAIL] Auth service not available - cannot continue")
            return False
        ...
        if not token_data:
            print("[FAIL] Failed to obtain token - cannot continue")
            return False
        ...
        if not validation_result.get("valid"):
            print("[FAIL] Token validation failed - token may be invalid")
            return False
        ...
        if backend_healthy:
            ...
            print("[SKIP] Backend not healthy - skipping backend tests")
        ...
        try:
            ...
            if fresh_response.status_code == 200:
                ...
                if refresh_token:
                    ...
                    print("[INFO] No refresh token available from dev login")
            else:
                print("[FAIL] Could not get refresh token for testing")
        except Exception as e:
            print(f"[FAIL] Refresh test setup failed: {e}")
