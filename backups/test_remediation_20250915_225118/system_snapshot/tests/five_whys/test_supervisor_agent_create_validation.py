...
class SupervisorAgentCreateValidationTests:
    ...
    def test_why_1_supervisor_create_rejects_old_parameters(self):
        ...
        """
        deprecated_params = [{'llm_client': self.mock_llm_manager, 'websocket_bridge': self.mock_websocket_bridge}, {'llm_manager': self.mock_llm_manager, 'tool_dispatcher': Mock()}, {'llm_manager': self.mock_llm_manager, 'agent_registry': Mock()}, {'name': 'test_supervisor', 'llm_manager': self.mock_llm_manager}]
    ...
    def test_why_3_supervisor_parameter_validation_system(self):
        ...
        with pytest.raises(TypeError) as exc_info:
            SupervisorAgent.create(llm_client=self.mock_llm_manager, websocket_bridge=self.mock_websocket_bridge)
        assert 'unexpected keyword' in str(exc_info.value).lower()
        with pytest.raises(TypeError) as exc_info:
            SupervisorAgent.create(llm_manager=self.mock_llm_manager, tool_dispatcher=Mock())
        assert 'unexpected keyword' in str(exc_info.value).lower()
        with pytest.raises(TypeError) as exc_info:
            SupervisorAgent.create(llm_manager=self.mock_llm_manager, agent_registry=Mock())
        assert 'unexpected keyword' in str(exc_info.value).lower()
        with pytest.raises(TypeError) as exc_info:
            SupervisorAgent.create(name='test_supervisor', llm_manager=self.mock_llm_manager)
        assert 'unexpected keyword' in str(exc_info.value).lower()
        supervisor_no_ws = SupervisorAgent.create(llm_manager=self.mock_llm_manager, websocket_bridge=None)
    ...
    def test_why_4_supervisor_create_usage_coverage(self):
        ...
        """
        supervisor1 = SupervisorAgent.create(llm_manager=self.mock_llm_manager, websocket_bridge=self.mock_websocket_bridge)
        ...
        supervisor2 = SupervisorAgent.create(llm_manager=fresh_llm, websocket_bridge=fresh_bridge)
    ...
    def test_why_5_supervisor_interface_governance_standards(self):
        ...
        expected_return = 'SupervisorAgent'
        assert return_annotation == expected_return or return_annotation == SupervisorAgent or str(return_annotation).endswith('SupervisorAgent'), f'SupervisorAgent.create() return annotation should be SupervisorAgent, got {return_annotation}'
        for param_name, param in create_signature.parameters.items():
            if param_name != 'cls':
                assert param.annotation != inspect.Parameter.empty, f"SupervisorAgent.create() parameter '{param_name}' missing type annotation"
    ...
    def test_end_to_end_supervisor_websocket_integration(self):
        ...
        """
        supervisor = SupervisorAgent.create(llm_manager=self.mock_llm_manager, websocket_bridge=self.mock_websocket_bridge)
        context = UserExecutionContext(user_id='integration_test_user', thread_id='integration_test_thread', run_id='integration_test_run', websocket_client_id='integration_ws_client_123', db_session=None)
        assert supervisor is not None
    ...
    def test_regression_prevention_comprehensive_supervisor_validation(self):
        ...
        """
        validation_results = []
        validation_results.append({'check': 'create_method_exists', 'result': hasattr(SupervisorAgent, 'create') and callable(SupervisorAgent.create), 'critical': True})
        create_works = False
        try:
            supervisor = SupervisorAgent.create(llm_manager=self.mock_llm_manager, websocket_bridge=self.mock_websocket_bridge)
            ...
            create_works = False
        validation_results.append({'check': 'create_accepts_correct_params', 'result': create_works, 'critical': True})
        ...
        try:
            ...
            rejects_invalid = True
        validation_results.append({'check': 'create_rejects_invalid_params', 'result': rejects_invalid, 'critical': True})
        create_signature = inspect.signature(SupervisorAgent.create)
        required_params = {'llm_manager', 'websocket_bridge'}
        actual_params = set(create_signature.parameters.keys()) - {'cls'}
        validation_results.append({'check': 'interface_signature_consistent', 'result': required_params.issubset(actual_params), 'critical': True})
        ...
        validation_results.append({'check': 'no_deprecated_parameters', 'result': not has_deprecated, 'critical': True})
