...
class SSOTUserContextValidationTests:
    ...
    def test_valid_ssot_user_context_passes_validation(self):
        ...
        user_context = UserExecutionContext(user_id='test_user_123', thread_id='thread_456', run_id='run_789', websocket_client_id='ws_connection_abc')
    ...
    def test_missing_required_attributes_raises_error(self):
        ...
        user_context = UserExecutionContext(user_id='test_user_123', thread_id='thread_456', run_id='run_789')
    ...
    def test_empty_string_attributes_raise_error(self):
        ...
        user_context = UserExecutionContext(user_id='', thread_id='thread_456', run_id='run_789')
        with pytest.raises(ValueError) as exc_info:
            _validate_ssot_user_context(user_context)
        error_message = str(exc_info.value)
    ...
    def test_websocket_client_id_can_be_none(self):
        ...
        user_context = UserExecutionContext(user_id='test_user_123', thread_id='thread_456', run_id='run_789', websocket_client_id=None)
        _validate_ssot_user_context(user_context)

    def test_websocket_client_id_empty_string_raises_error(self):
        """Test that empty string websocket_client_id raises error."""
        user_context = UserExecutionContext(user_id='test_user_123', thread_id='thread_456', run_id='run_789', websocket_client_id='')
        with pytest.raises(ValueError) as exc_info:
            _validate_ssot_user_context(user_context)
        error_message = str(exc_info.value)
        assert 'SSOT CONTEXT VALIDATION FAILED' in error_message
...
@pytest.mark.websocket
class WebSocketFactoryExceptionHandlingTests:
...
@pytest.mark.websocket
class WebSocketFactoryManagerCreationTests:
    ...
    def test_factory_creates_isolated_managers(self):
        ...
        user_context_1 = UserExecutionContext(user_id='user_1', thread_id='thread_1', run_id='run_1', websocket_client_id='ws_1')
        user_context_2 = UserExecutionContext(user_id='user_2', thread_id='thread_2', run_id='run_2', websocket_client_id='ws_2')
...
@pytest.mark.websocket
class WebSocketFactoryErrorRecoveryTests:
...
@pytest.mark.websocket
class WebSocketFactoryIntegrationTests:
    ...
    async def test_factory_cleanup_operations(self):
        ...
        user_context = UserExecutionContext(user_id='cleanup_test_user', thread_id='cleanup_thread', run_id='cleanup_run', websocket_client_id='cleanup_ws_conn')
        manager = create_websocket_manager(user_context)
        assert manager is not None
    ...
    def test_factory_ssot_compliance(self):
        ...
        contexts = []
        for i in range(3):
            context = UserExecutionContext(user_id=f'ssot_user_{i}', thread_id=f'ssot_thread_{i}', run_id=f'ssot_run_{i}', websocket_client_id=f'ssot_ws_{i}')
