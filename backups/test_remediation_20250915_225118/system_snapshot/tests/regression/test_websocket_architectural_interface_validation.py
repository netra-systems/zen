...
class ComponentInterface:
    ...
    STARTUP_MANAGER_INTERFACE = {'add_startup_phase': ['phase_name', 'callback', 'dependencies'], 'wait_for_phase': ['phase_name', 'timeout'], 'mark_phase_complete': ['phase_name', 'status'], 'register_readiness_check': ['service_name', 'check_callback'], 'get_startup_status': [], 'coordinate_service_startup': ['service_configs']}
    WEBSOCKET_MANAGER_INTERFACE = {'is_ready_for_connections': [], 'register_readiness_callback': ['callback'], 'get_connection_readiness': [], 'validate_websocket_health': [], 'coordinate_with_startup': ['startup_manager'], 'set_readiness_status': ['ready', 'metadata']}
...
class WebSocketArchitecturalInterfaceValidationTests(SSotAsyncTestCase):
    ...
    async def test_startup_manager_websocket_coordination_interface(self):
        ...
        startup_manager_result = await self._validate_component_interface('startup_manager', self.interface_validator.STARTUP_MANAGER_INTERFACE)
        self.validation_results.append(startup_manager_result)
        coordination_methods = ['register_readiness_check', 'coordinate_service_startup', 'wait_for_phase']
        ...
        if missing_coordination:
            self.assertEqual(len(missing_coordination), 0, f'EXPECTED FAILURE: Startup manager missing coordination methods: {missing_coordination}. Without these interfaces, WebSocket manager cannot coordinate startup, causing 1011 errors when clients connect before WebSocket is ready.')
        else:
            self.assertTrue(coordination_possible, 'EXPECTED FAILURE: No coordination mechanism between startup manager and WebSocket. WebSocket cannot signal readiness or coordinate with startup sequence, causing 1011 errors.')
    ...
    async def test_websocket_manager_readiness_interface(self):
        ...
        websocket_manager_result = await self._validate_component_interface('websocket_manager', self.interface_validator.WEBSOCKET_MANAGER_INTERFACE)
        self.validation_results.append(websocket_manager_result)
        readiness_methods = ['is_ready_for_connections', 'get_connection_readiness', 'validate_websocket_health', 'set_readiness_status']
        missing_readiness = []
        ...
        if missing_readiness:
            self.assertEqual(len(missing_readiness), 0, f'EXPECTED FAILURE: WebSocket manager missing readiness methods: {missing_readiness}. Without these interfaces, other components cannot validate WebSocket readiness, causing 1011 errors when connections are attempted before WebSocket is ready.')
        else:
            self.assertTrue(external_validation_possible, 'EXPECTED FAILURE: Other components cannot validate WebSocket readiness. No interface exists for health checks or startup coordination to verify WebSocket is ready, causing 1011 errors.')
    ...
    async def test_health_check_websocket_integration_interface(self):
        ...
        health_check_result = await self._validate_component_interface('health_check', self.interface_validator.HEALTH_CHECK_INTERFACE)
        self.validation_results.append(health_check_result)
        integration_methods = ['include_websocket_readiness', 'validate_websocket_integration', 'overall_health_includes_websocket']
        missing_integration = []
        ...
        if missing_integration:
            self.assertEqual(len(missing_integration), 0, f'EXPECTED FAILURE: Health check missing WebSocket integration methods: {missing_integration}. Without these interfaces, health checks cannot include WebSocket readiness, causing load balancers to route traffic to instances with unready WebSockets (1011 errors).')
        else:
            self.assertTrue(websocket_in_health, 'EXPECTED FAILURE: WebSocket readiness not included in health checks. Health checks pass while WebSocket is not ready, causing load balancers to route traffic to unready instances, resulting in 1011 errors.')
    ...
    async def test_app_state_websocket_coordination_interface(self):
        ...
        app_state_result = await self._validate_component_interface('app_state', self.interface_validator.APP_STATE_INTERFACE)
        self.validation_results.append(app_state_result)
        coordination_methods = ['register_websocket_manager', 'get_websocket_readiness', 'coordinate_startup_sequence', 'get_startup_coordination']
        missing_coordination = []
        for method in coordination_methods:
        ...
        if missing_coordination:
            self.assertEqual(len(missing_coordination), 0, f'EXPECTED FAILURE: App state missing WebSocket coordination methods: {missing_coordination}. Without these interfaces, app state cannot track WebSocket readiness or coordinate startup sequence, contributing to 1011 errors during startup.')
        else:
            self.assertTrue(app_state_tracks_websocket, "EXPECTED FAILURE: App state doesn't track WebSocket readiness. Application state management cannot coordinate with WebSocket lifecycle, causing timing issues that lead to 1011 errors.")
    ...
    async def test_comprehensive_architectural_gap_analysis(self):
        ...
        self.assertLessEqual(len(critical_gaps), 3, f'Too many critical architectural gaps contributing to 1011 errors: {len(critical_gaps)}. Critical gaps: {[gap.description for gap in critical_gaps]}. These gaps must be addressed to prevent WebSocket handshake failures.')

    async def _validate_component_interface(self, component_name: str, expected_interface: Dict[str, List[str]]) -> InterfaceValidationResult:
        ...
        try:
            ...
            if component_class is None:
                return InterfaceValidationResult(component_name=component_name, interface_name=f'{component_name}_interface', exists=False, methods=[], missing_methods=list(expected_interface.keys()), signature_issues=[], coordination_support=False, error_details='Component class not found')
            available_methods = [method for method in dir(component_class) if not method.startswith('_') and callable(getattr(component_class, method, None))]
            missing_methods = []
            signature_issues = []
            for method_name, expected_params in expected_interface.items():
                if method_name not in available_methods:
                    ...
                    if len(expected_params) > 0 and len(actual_params) < len(expected_params):
                        signature_issues.append(f'{method_name}: expected {expected_params}, got {actual_params}')
            coordination_methods = ['coordinate', 'register', 'wait_for', 'notify']
            coordination_support = any((coord_word in method.lower() for method in available_methods for coord_word in coordination_methods))
            return InterfaceValidationResult(component_name=component_name, interface_name=f'{component_name}_interface', exists=True, methods=available_methods, missing_methods=missing_methods, signature_issues=signature_issues, coordination_support=coordination_support)
        except Exception as e:
            return InterfaceValidationResult(component_name=component_name, interface_name=f'{component_name}_interface', exists=False, methods=[], missing_methods=list(expected_interface.keys()), signature_issues=[], coordination_support=False, error_details=str(e))

    async def _get_component_class(self, component_name: str):
        """Get component class by name."""
        component_mappings = {'startup_manager': ('netra_backend.app.core.startup_manager', 'StartupManager'), 'websocket_manager': ('netra_backend.app.websocket_core.manager', 'WebSocketManager'), 'health_check': ('netra_backend.app.health.health_check', 'HealthCheck'), 'app_state': ('netra_backend.app.core.app_state', 'AppState')}
        ...
        try:
            ...
            alternative_paths = {'startup_manager': [('netra_backend.app.startup.manager', 'StartupManager'), ('netra_backend.app.core.startup', 'StartupManager')], 'websocket_manager': [('netra_backend.app.websocket.manager', 'WebSocketManager'), ('netra_backend.app.websockets.manager', 'UnifiedWebSocketManager')], 'health_check': [('netra_backend.app.health', 'HealthChecker'), ('netra_backend.app.core.health', 'HealthCheck')], 'app_state': [('netra_backend.app.state', 'AppState'), ('netra_backend.app.core.state', 'ApplicationState')]}
    ...
    async def _check_websocket_startup_coordination(self) -> bool:
        ...
        try:
            ...
            coordination_indicators = ['coordinate_startup', 'register_with_startup', 'startup_callback', 'wait_for_startup', 'notify_ready', 'set_ready']
    ...
    async def _check_external_websocket_readiness_validation(self) -> bool:
        ...
        try:
            from netra_backend.app.websocket_core import manager as websocket_module
            readiness_indicators = ['is_ready', 'ready_for_connections', 'connection_ready', 'validate_ready', 'check_readiness', 'get_status']
            websocket_methods = dir(websocket_module)
    ...
    async def _check_websocket_in_health_checks(self) -> bool:
        ...
        try:
            ...
            websocket_in_health = any(('websocket' in method.lower() for method in health_methods))
    ...
    async def _check_app_state_websocket_tracking(self) -> bool:
        ...
        try:
            ...
            websocket_tracking = any(('websocket' in method.lower() for method in app_state_methods))
    ...
    async def _analyze_architectural_gaps(self) -> List[ArchitecturalGap]:
        ...
        for result in self.validation_results:
            if result.missing_methods:
                gap = ArchitecturalGap(gap_type='missing_interface_methods', component_a=result.component_name, component_b='system', description=f'{result.component_name} missing methods: {result.missing_methods}', impact='Cannot coordinate with other components', contributes_to_1011=True)
                gaps.append(gap)
            if not result.coordination_support:
                gap = ArchitecturalGap(gap_type='coordination_interface_missing', component_a=result.component_name, component_b='startup_system', description=f'{result.component_name} lacks coordination interfaces', impact='Cannot participate in coordinated startup sequence', contributes_to_1011=True)
                gaps.append(gap)
        known_gaps = [ArchitecturalGap(gap_type='startup_websocket_coordination', component_a='startup_manager', component_b='websocket_manager', description='No coordination mechanism between startup and WebSocket', impact='WebSocket may start before dependencies ready', contributes_to_1011=True), ArchitecturalGap(gap_type='health_check_websocket_integration', component_a='health_check', component_b='websocket_manager', description="Health checks don't validate WebSocket readiness", impact='Load balancers route to unready instances', contributes_to_1011=True), ArchitecturalGap(gap_type='app_state_websocket_tracking', component_a='app_state', component_b='websocket_manager', description="App state doesn't track WebSocket lifecycle", impact='Application state inconsistent with WebSocket state', contributes_to_1011=True)]
