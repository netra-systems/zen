...
@pytest.mark.unit
class SSotMockPerformanceBaselineTests(SSotBaseTestCase):
    """
    ...
    """
    MAX_MOCK_CREATION_TIME_MS = 5.0
    MAX_PERFORMANCE_OVERHEAD_PERCENT = 110
    MIN_THROUGHPUT_MOCKS_PER_SECOND = 1000
    ...
    def _measure_execution_time(self, operation_name: str, operation_func, iterations: int=100) -> Dict[str, float]:
        ...
        stats = {'operation': operation_name, 'iterations': iterations, 'mean_ms': statistics.mean(execution_times), 'median_ms': statistics.median(execution_times), 'min_ms': min(execution_times), 'max_ms': max(execution_times), 'stdev_ms': statistics.stdev(execution_times) if len(execution_times) > 1 else 0, 'p95_ms': sorted(execution_times)[int(len(execution_times) * 0.95)], 'p99_ms': sorted(execution_times)[int(len(execution_times) * 0.99)]}
    ...
    def test_websocket_mock_creation_performance(self):
        ...
        def create_ssot_websocket_mock():
            return SSotMockFactory.create_websocket_mock(connection_id=f'perf-test-{time.time()}', user_id=f'user-{time.time()}')
        ssot_stats = self._measure_execution_time('ssot_websocket_mock_creation', create_ssot_websocket_mock, iterations=200)
        ...
        direct_stats = self._measure_execution_time('direct_websocket_mock_creation', create_direct_websocket_mock, iterations=200)
        self.assertLess(ssot_stats['mean_ms'], self.MAX_MOCK_CREATION_TIME_MS, f"SSOT WebSocket mock creation too slow: {ssot_stats['mean_ms']:.2f}ms > {self.MAX_MOCK_CREATION_TIME_MS}ms")
        overhead_percent = ssot_stats['mean_ms'] / direct_stats['mean_ms'] * 100
        self.assertLess(overhead_percent, self.MAX_PERFORMANCE_OVERHEAD_PERCENT, f'SSOT overhead too high: {overhead_percent:.1f}% > {self.MAX_PERFORMANCE_OVERHEAD_PERCENT}%')
    ...
    def test_agent_mock_creation_performance(self):
        ...
        def create_ssot_agent_mock():
            return SSotMockFactory.create_agent_mock(agent_type='supervisor', execution_result={'status': 'completed', 'result': 'test'}, execution_time=0.1)
        ssot_stats = self._measure_execution_time('ssot_agent_mock_creation', create_ssot_agent_mock, iterations=200)
        ...
        def create_direct_agent_mock():
            ...
            return mock_agent
        direct_stats = self._measure_execution_time('direct_agent_mock_creation', create_direct_agent_mock, iterations=200)
        self.assertLess(ssot_stats['mean_ms'], self.MAX_MOCK_CREATION_TIME_MS, f"SSOT agent mock creation too slow: {ssot_stats['mean_ms']:.2f}ms > {self.MAX_MOCK_CREATION_TIME_MS}ms")
        overhead_percent = ssot_stats['mean_ms'] / direct_stats['mean_ms'] * 100
        self.assertLess(overhead_percent, self.MAX_PERFORMANCE_OVERHEAD_PERCENT, f'SSOT overhead too high: {overhead_percent:.1f}% > {self.MAX_PERFORMANCE_OVERHEAD_PERCENT}%')
    ...
    def test_database_mock_creation_performance(self):
        ...
        ssot_stats = self._measure_execution_time('ssot_database_mock_creation', create_ssot_database_mock, iterations=200)
        ...
        def create_direct_database_mock():
            ...
            return mock_session
        direct_stats = self._measure_execution_time('direct_database_mock_creation', create_direct_database_mock, iterations=200)
        self.assertLess(ssot_stats['mean_ms'], self.MAX_MOCK_CREATION_TIME_MS, f"SSOT database mock creation too slow: {ssot_stats['mean_ms']:.2f}ms > {self.MAX_MOCK_CREATION_TIME_MS}ms")
        overhead_percent = ssot_stats['mean_ms'] / direct_stats['mean_ms'] * 100
        self.assertLess(overhead_percent, self.MAX_PERFORMANCE_OVERHEAD_PERCENT, f'SSOT overhead too high: {overhead_percent:.1f}% > {self.MAX_PERFORMANCE_OVERHEAD_PERCENT}%')
    ...
    def test_mock_suite_creation_performance(self):
        ...
        """
        mock_types = ['agent', 'websocket', 'database_session', 'execution_context', 'tool', 'llm_client', 'configuration']

        def create_ssot_mock_suite():
            return SSotMockFactory.create_mock_suite(mock_types)
        ssot_stats = self._measure_execution_time('ssot_mock_suite_creation', create_ssot_mock_suite, iterations=50)
        ...
        def create_direct_mock_suite():
            ...
            return mock_suite
        direct_stats = self._measure_execution_time('direct_mock_suite_creation', create_direct_mock_suite, iterations=50)
        max_suite_creation_time = self.MAX_MOCK_CREATION_TIME_MS * len(mock_types)
        self.assertLess(ssot_stats['mean_ms'], max_suite_creation_time, f"SSOT mock suite creation too slow: {ssot_stats['mean_ms']:.2f}ms > {max_suite_creation_time}ms")
        overhead_percent = ssot_stats['mean_ms'] / direct_stats['mean_ms'] * 100
        self.assertLess(overhead_percent, self.MAX_PERFORMANCE_OVERHEAD_PERCENT, f'SSOT suite overhead too high: {overhead_percent:.1f}% > {self.MAX_PERFORMANCE_OVERHEAD_PERCENT}%')
    ...
    def test_mock_throughput_performance(self):
        ...
        for i in range(iterations):
            SSotMockFactory.create_websocket_mock(connection_id=f'throughput-test-{i}', user_id=f'user-{i}')
        end_time = time.perf_counter()
        total_time_seconds = end_time - start_time
        throughput_per_second = iterations / total_time_seconds
        self.assertGreater(throughput_per_second, self.MIN_THROUGHPUT_MOCKS_PER_SECOND, f'WebSocket mock throughput too low: {throughput_per_second:.0f} < {self.MIN_THROUGHPUT_MOCKS_PER_SECOND} mocks/sec')
    ...
    async def test_async_mock_operation_performance(self):
        ...
        max_async_operation_time = 1.0
        self.assertLess(mean_time, max_async_operation_time, f'Async mock operations too slow: {mean_time:.2f}ms > {max_async_operation_time}ms')
    ...
    def test_memory_usage_baseline(self):
        ...
        for i in range(mock_batch_size):
            mocks.append(SSotMockFactory.create_websocket_mock(connection_id=f'memory-test-{i}', user_id=f'user-{i}'))
        ...
        self.assertLess(memory_per_mock_kb, max_memory_per_mock_kb, f'Memory usage too high: {memory_per_mock_kb:.2f}KB > {max_memory_per_mock_kb}KB per mock')
    ...
    def test_performance_regression_detection(self):
        ...
        """
        performance_baselines = {'websocket_mock_creation_ms': 2.0, 'agent_mock_creation_ms': 2.0, 'database_mock_creation_ms': 3.0, 'mock_suite_creation_ms': 15.0}
        if 'ssot_websocket_mock_creation' in self.performance_results:
            ...
            if current_websocket_time > baseline_websocket_time * 1.5:
