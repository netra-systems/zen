...
@pytest.mark.unit
class UnauthorizedTestRunnerDetectionTests(BaseTestCase):
    """Unit tests to detect unauthorized test runners violating SSOT patterns"""

    def setUp(self):
        """Setup test detection infrastructure"""
        self.project_root = Path(__file__).parent.parent.parent.absolute()
        self.test_directories = [self.project_root / 'tests', self.project_root / 'netra_backend' / 'tests', self.project_root / 'auth_service' / 'tests', self.project_root / 'analytics_service' / 'tests']
        self.violations = {'pytest_main_calls': [], 'subprocess_pytest_calls': [], 'standalone_test_runners': [], 'unauthorized_test_scripts': []}
    ...
    def test_detect_direct_pytest_main_violations(self):
        ...
        self.assertEqual(len(pytest_main_violations), 0, f'CRITICAL VIOLATION: Found {len(pytest_main_violations)} unauthorized pytest.main() calls bypassing unified test runner SSOT. This blocks Golden Path reliability. Violations: {pytest_main_violations[:10]}...')
            ...
            if test_dir.exists():
                for py_file in test_dir.rglob('*.py'):
                    try:
                        with open(py_file, 'r', encoding='utf-8') as f:
                            content = f.read()
                            if re.search('subprocess\\.(run|call|Popen).*pytest', content):
                                subprocess_violations.append(str(py_file))
                    except (UnicodeDecodeError, PermissionError):
                        continue
        self.violations['subprocess_pytest_calls'] = subprocess_violations
        self.assertEqual(len(subprocess_violations), 0, f'CRITICAL VIOLATION: Found {len(subprocess_violations)} unauthorized subprocess pytest calls bypassing unified test runner. This creates test infrastructure chaos. Violations: {subprocess_violations[:5]}...')
            ...
            if test_dir.exists():
                for py_file in test_dir.rglob('*.py'):
                    ...
                    try:
                        with open(py_file, 'r', encoding='utf-8') as f:
                            content = f.read()
                            if 'if __name__ == "__main__"' in content and ('pytest' in content or 'unittest.main' in content):
                                standalone_runners.append(str(py_file))
                    except (UnicodeDecodeError, PermissionError):
                        continue
        self.violations['standalone_test_runners'] = standalone_runners
        self.assertEqual(len(standalone_runners), 0, f'CRITICAL VIOLATION: Found {len(standalone_runners)} standalone test runner scripts bypassing unified test runner SSOT. This fragments test execution. Violations: {standalone_runners[:5]}...')
    ...
    def test_verify_unified_test_runner_ssot_compliance(self):
        ...
        self.assertTrue(unified_runner_path.exists(), f'CRITICAL MISSING: Unified test runner not found at {unified_runner_path}. SSOT test infrastructure incomplete.')
        with open(unified_runner_path, 'r', encoding='utf-8') as f:
            content = f.read()
        self.assertIn('UNIFIED TEST RUNNER', content, 'Unified test runner missing SSOT identification')
    ...
    def test_quantify_golden_path_reliability_impact(self):
        ...
        """
        total_violations = len(self.violations['pytest_main_calls']) + len(self.violations['subprocess_pytest_calls']) + len(self.violations['standalone_test_runners'])
        target_reliability = 95.0
        current_reliability = 60.0
        reliability_gap = target_reliability - current_reliability
        self.assertLess(total_violations, 50, f'BUSINESS CRITICAL: {total_violations} unauthorized test runners detected. Golden Path reliability: {current_reliability}% (target: {target_reliability}%). Reliability gap: {reliability_gap}% affects $500K+ ARR. Deployment blocking in progress.')
    ...
    def test_unauthorized_test_script_patterns(self):
        ...
        violation_patterns = ['pytest\\.main\\(', 'unittest\\.main\\(', 'subprocess\\.run\\(.*pytest', 'subprocess\\.call\\(.*pytest', 'os\\.system\\(.*pytest']
        for test_dir in self.test_directories:
            if test_dir.exists():
                for py_file in test_dir.rglob('*.py'):
                    try:
                        with open(py_file, 'r', encoding='utf-8') as f:
                            ...
                            for pattern in violation_patterns:
                                if re.search(pattern, content):
                                    unauthorized_patterns.append({'file': str(py_file), 'pattern': pattern})
                                    break
                    except (UnicodeDecodeError, PermissionError):
                        continue
...
@pytest.mark.unit
class GoldenPathTestReliabilityMetricsTests(BaseTestCase):
    ...
    def test_golden_path_test_success_rate_baseline(self):
        ...
        self.assertGreaterEqual(current_success_rate, target_success_rate, f'GOLDEN PATH CRITICAL: Success rate {current_success_rate}% below target {target_success_rate}%. Unauthorized test runners causing {target_success_rate - current_success_rate}% reliability loss. $500K+ ARR at risk from deployment blocks.')
    ...
    def test_mission_critical_websocket_events_reliability(self):
        ...
        """
        websocket_test_reliability = 60.0
        required_reliability = 99.0
        self.assertGreaterEqual(websocket_test_reliability, required_reliability, f'MISSION CRITICAL FAILURE: WebSocket events testing reliability {websocket_test_reliability}% below required {required_reliability}%. Chat functionality testing unreliable. Unauthorized test runners causing infrastructure chaos.')
    ...
    def test_multi_user_security_test_consistency(self):
        ...
        """
        security_test_consistency = 55.0
        required_consistency = 95.0
        self.assertGreaterEqual(security_test_consistency, required_consistency, f'SECURITY CRITICAL: Multi-user security test consistency {security_test_consistency}% below required {required_consistency}%. Different test runners producing different results. Enterprise compliance at risk.')
