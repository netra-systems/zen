...
class DockerComposePathValidationTests(SSotBaseTestCase):
    ...
    def setUpClass(cls):
        ...
        cls.expected_compose_files = ['docker-compose.yml', 'docker-compose.staging.yml', 'docker-compose.minimal-test.yml', 'docker-compose.alpine-test.yml', 'docker-compose.alpine-dev.yml', 'docker-compose.staging.alpine.yml']
    ...
    def test_all_docker_compose_files_exist(self):
        ...
        assert not missing_files, f'Missing Docker compose files: {missing_files}. This breaks Docker infrastructure and prevents service startup.'
    ...
    def test_docker_compose_files_are_valid_yaml(self):
        ...
        for compose_file in self.expected_compose_files:
            ...
            try:
                ...
                yaml_errors.append({'file': compose_file, 'error': str(e)})
        assert not yaml_errors, f'YAML syntax errors in Docker compose files: {yaml_errors}. This prevents Docker from parsing configurations.'
                ...
                for service_name, service_config in compose_config['services'].items():
                    if 'build' in service_config:
                        ...
                        if not resolved_path.exists():
                            build_context_errors.append({'compose_file': compose_file, 'service': service_name, 'context_path': context_path, 'resolved_path': str(resolved_path), 'error': 'Build context directory does not exist'})
            except Exception as e:
                build_context_errors.append({'compose_file': compose_file, 'service': 'unknown', 'error': f'Failed to parse compose file: {e}'})
        if build_context_errors:
            self.logger.error(f'Build context path errors: {build_context_errors}')
        assert not build_context_errors, f'Build context path errors: {build_context_errors}. CRITICAL: This is likely the root cause of Issue #426 Docker failures. Services cannot build because build contexts point to non-existent directories.'
                ...
                for service_name, service_config in compose_config['services'].items():
                    if 'build' in service_config:
                        ...
                        if not resolved_dockerfile.exists():
                            dockerfile_errors.append({'compose_file': compose_file, 'service': service_name, 'dockerfile_path': dockerfile_path, 'context_path': context_path, 'resolved_path': str(resolved_dockerfile), 'error': 'Dockerfile does not exist'})
            except Exception as e:
                dockerfile_errors.append({'compose_file': compose_file, 'service': 'unknown', 'error': f'Failed to parse compose file: {e}'})
        if dockerfile_errors:
            self.logger.error(f'Dockerfile path errors: {dockerfile_errors}')
        assert not dockerfile_errors, f'Dockerfile path errors: {dockerfile_errors}. Services cannot build because Dockerfiles are missing or incorrectly referenced.'
    ...
    def test_service_dependency_consistency(self):
        ...
        for compose_file in self.expected_compose_files:
            ...
            try:
                    ...
                    if 'depends_on' in service_config:
                        ...
                        for dep_name in dependency_names:
                            if dep_name not in service_names:
                                dependency_errors.append({'compose_file': compose_file, 'service': service_name, 'missing_dependency': dep_name, 'available_services': list(service_names)})
                ...
                dependency_errors.append({'compose_file': compose_file, 'service': 'unknown', 'error': f'Failed to parse compose file: {e}'})
        assert not dependency_errors, f'Service dependency errors: {dependency_errors}. Services have dependencies on non-existent services, preventing startup.'
                                ...
                                if ':' in port_mapping:
                                    ...
                                    if '-' not in host_port and '${' not in host_port:
                                        ...
                                        if port_key in port_mappings:
                                            port_conflicts.append({'port': port_key, 'conflict_services': port_mappings[port_key] + [(compose_file, service_name)]})
                                        else:
    ...
    def test_environment_variable_consistency(self):
        ...
        for var_name, usages in env_var_usage.items():
            ...
            if len(default_values) > 1:
                inconsistent_defaults.append({'variable': var_name, 'different_defaults': list(default_values), 'usages': usages})
        ...
        for compose_file in self.expected_compose_files:
            ...
            try:
                ...
                if not compose_config:
                    structure_issues.append({'compose_file': compose_file, 'issue': 'Empty or invalid YAML'})
                    continue
                for section in required_sections:
                    if section not in compose_config:
                        structure_issues.append({'compose_file': compose_file, 'issue': f'Missing required section: {section}'})
                if 'services' in compose_config and (not compose_config['services']):
                    structure_issues.append({'compose_file': compose_file, 'issue': 'Services section is empty'})
            except Exception as e:
                structure_issues.append({'compose_file': compose_file, 'issue': f'Failed to parse: {e}'})
        assert not structure_issues, f'Compose file structure issues: {structure_issues}. Files are missing required sections or have invalid structure.'
