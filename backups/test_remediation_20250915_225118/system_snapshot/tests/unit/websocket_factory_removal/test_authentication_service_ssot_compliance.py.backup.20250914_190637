...
class TestAuthenticationServiceSSOTCompliance(SSotAsyncTestCase):
    ...
    async def test_authentication_service_deprecated_factory_usage_MUST_FAIL_INITIALLY(self):
        ...
        # Import authentication service to check for deprecated imports
        try:
            import ast
            import inspect
            from netra_backend.app.services.unified_authentication_service import UnifiedAuthenticationService
            
            # Get source code of the authentication service
            source = inspect.getsource(UnifiedAuthenticationService)
            
            # Parse the source to detect deprecated factory imports
            ...
            # TEST SHOULD FAIL if deprecated imports are found
            if deprecated_imports_found:
                self.fail(
                    f"SSOT VIOLATION: Authentication service still uses deprecated factory imports: "
                    f"{deprecated_imports_found}. Migration to SSOT patterns incomplete."
                )
            
            # If no deprecated imports found, factory removal is complete
    ...
    async def test_authentication_service_user_context_creation_ssot(self):
        ...
        try:
            ...
            with patch('netra_backend.app.services.unified_authentication_service.AuthClient') as mock_auth_client, \
                 patch('netra_backend.app.services.unified_authentication_service.get_logger') as mock_logger:
                ...
                # Test user context creation without factory
                # This should work via direct SSOT patterns
                user_context = UserExecutionContext(
                    user_id=self.test_user_id,
                    websocket_client_id="test_auth_validation_client"
                )
    ...
    async def test_authentication_fallback_without_factory(self):
        ...
        try:
            ...
            # Test fallback context creation (simulating what auth service should do)
            fallback_user_id = f"fallback_{self.test_user_id}"
            
            # Create fallback context via direct SSOT patterns
            fallback_context = UserExecutionContext(
                user_id=fallback_user_id,
                websocket_client_id=None  # Auto-generated via SSOT
            )
            
            # Validate fallback context
            self.assertIsNotNone(fallback_context)
            self.assertEqual(fallback_context.user_id, fallback_user_id)
            self.assertIsNotNone(fallback_context.websocket_client_id)  # Should be auto-generated
    ...
    async def test_authentication_service_websocket_integration_ssot(self):
        ...
        try:
            ...
            # Create user context for WebSocket authentication
            user_context = UserExecutionContext(
                user_id=self.test_user_id,
                websocket_client_id="test_auth_websocket_integration"
            )
            
            # Test WebSocket manager creation with authenticated context
    ...
    async def test_authentication_service_memory_efficiency_without_factory(self):
        ...
        try:
            ...
            # Create multiple contexts to test memory efficiency
            contexts = []
            for i in range(10):
                context = UserExecutionContext(
                    user_id=f"{self.test_user_id}_{i}",
                    websocket_client_id=f"test_memory_client_{i}"
                )
            ...
            # Memory growth should be reasonable (less than 50MB for 10 contexts)
