...
class ConfigEnvironmentAccessSSotTests(SSotBaseTestCase):
    ...
    def test_config_managers_use_isolated_environment_only(self):
        ...
        """
        config_manager_modules = ['netra_backend.app.core.configuration.base', 'netra_backend.app.core.configuration.base', 'netra_backend.app.services.configuration_service']
            ...
            try:
                ...
                if hasattr(module, '__file__') and module.__file__:
                    ...
                    with open(file_path, 'r', encoding='utf-8') as f:
                        source_code = f.read()
                    has_isolated_env_import = 'from shared.isolated_environment import' in source_code or 'import shared.isolated_environment' in source_code or 'from dev_launcher.isolated_environment import' in source_code
                    ...
                    try:
                        ...
                        os_environ_violations.append({'file': str(file_path), 'line': e.lineno, 'violation': f'Syntax error during AST parsing: {str(e)}'})
            except ImportError as e:
                pass
            except Exception as e:
                os_environ_violations.append({'module': module_name, 'error': f'Failed to analyze: {str(e)}'})
        assert len(os_environ_violations) == 0, f'SSOT VIOLATION: Found {len(os_environ_violations)} direct os.environ access violations in config managers: {os_environ_violations}. Missing IsolatedEnvironment imports: {missing_isolated_env_imports}. Config managers MUST use IsolatedEnvironment for SSOT compliance.'
    ...
    def test_no_direct_os_getenv_usage_in_config_managers(self):
        ...
        """
        config_files = ['netra_backend/app/core/configuration/base.py', 'netra_backend/app/core/managers/unified_configuration_manager.py', 'netra_backend/app/services/configuration_service.py', 'netra_backend/app/core/configuration/loader.py', 'netra_backend/app/core/configuration/validator.py']
                ...
                try:
                    ...
                    for line_num, line in enumerate(lines, 1):
                        ...
                        if 'os.getenv' in line_stripped and (not line_stripped.startswith('#')):
                            os_getenv_violations.append({'file': file_path, 'line': line_num, 'content': line_stripped, 'violation': 'Direct os.getenv usage'})
                        if 'os.environ[' in line_stripped and (not line_stripped.startswith('#')):
                            os_getenv_violations.append({'file': file_path, 'line': line_num, 'content': line_stripped, 'violation': 'Direct os.environ[] access'})
                        if 'os.environ.get(' in line_stripped and (not line_stripped.startswith('#')):
                            os_getenv_violations.append({'file': file_path, 'line': line_num, 'content': line_stripped, 'violation': 'Direct os.environ.get() access'})
                except Exception as e:
                    os_getenv_violations.append({'file': file_path, 'error': f'Failed to read file: {str(e)}'})
        assert len(os_getenv_violations) == 0, f'SSOT VIOLATION: Found {len(os_getenv_violations)} direct os environment access violations: {os_getenv_violations}. All environment access must use IsolatedEnvironment.get_env() pattern.'
    ...
    def test_config_managers_use_consistent_env_access_patterns(self):
        ...
        for manager_name, patterns in env_access_patterns.items():
            ...
            if not has_ssot_pattern or has_non_ssot_pattern:
                non_ssot_managers.append({'manager': manager_name, 'patterns': patterns, 'has_ssot': has_ssot_pattern, 'has_violations': has_non_ssot_pattern})
        assert len(non_ssot_managers) == 0, f'SSOT VIOLATION: Found {len(non_ssot_managers)} config managers with non-SSOT environment access patterns: {non_ssot_managers}. All managers must use consistent IsolatedEnvironment pattern.'
    ...
    def test_environment_variables_loaded_consistently_across_managers(self):
        ...
        """
        critical_env_vars = ['ENVIRONMENT', 'DATABASE_URL', 'REDIS_URL', 'JWT_SECRET_KEY', 'AUTH_SERVICE_URL']
        config_managers = []
        try:
            from netra_backend.app.core.configuration.base import UnifiedConfigManager
        ...
        for env_var in critical_env_vars:
            ...
            if len(unique_values) > 1:
                env_var_conflicts.append({'env_var': env_var, 'values': values, 'unique_values': list(unique_values)})
        assert len(env_var_conflicts) == 0, f'SSOT VIOLATION: Found {len(env_var_conflicts)} environment variable loading conflicts: {env_var_conflicts}. Config managers must load environment variables consistently using SSOT patterns.'
        ...
        for node in ast.walk(tree):
            if isinstance(node, ast.Attribute):
                if isinstance(node.value, ast.Name) and node.value.id == 'os' and (node.attr == 'environ'):
                    violations.append({'file': file_path, 'line': node.lineno, 'violation': 'Direct os.environ access'})
                ...
                if isinstance(node.func, ast.Attribute) and isinstance(node.func.value, ast.Name) and (node.func.value.id == 'os') and (node.func.attr == 'getenv'):
                    violations.append({'file': file_path, 'line': node.lineno, 'violation': 'Direct os.getenv access'})
        return violations
