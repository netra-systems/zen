...
class SSOTRefactoringCompleteTests:
    ...
    def test_factory_consolidation_complete(self):
        ...
        logger.info('=' * 80)
        factory_files = ['netra_backend/app/agents/agent_instance_factory.py', 'netra_backend/app/agents/agent_instance_factory_optimized.py', 'netra_backend/app/agents/agent_instance_factory_legacy.py', 'netra_backend/app/agents/agent_factory_v1.py', 'netra_backend/app/agents/agent_factory_v2.py']
        existing_factories = []
        for factory_path in factory_files:
            full_path = self.project_root / factory_path
            if full_path.exists():
                existing_factories.append(factory_path)
        assert len(existing_factories) == 1, f'Expected 1 factory, found {len(existing_factories)}: {existing_factories}'
        assert existing_factories[0] == 'netra_backend/app/agents/agent_instance_factory.py', f'Wrong factory file exists: {existing_factories[0]}'
    ...
    def test_agent_consolidation_complete(self):
        ...
        logger.info('=' * 80)
        expected_agents = ['data_sub_agent', 'optimization_sub_agent', 'analysis_sub_agent', 'research_sub_agent', 'forecasting_sub_agent', 'simulation_sub_agent', 'visualization_sub_agent', 'etl_sub_agent', 'statistical_sub_agent', 'ml_sub_agent', 'nlp_sub_agent', 'computer_vision_sub_agent', 'time_series_sub_agent', 'graph_analytics_sub_agent', 'real_time_sub_agent', 'batch_processing_sub_agent', 'stream_processing_sub_agent', 'data_quality_sub_agent', 'data_governance_sub_agent', 'metadata_management_sub_agent', 'data_catalog_sub_agent', 'lineage_tracking_sub_agent', 'compliance_sub_agent', 'privacy_sub_agent', 'security_sub_agent', 'audit_sub_agent', 'monitoring_sub_agent', 'alerting_sub_agent', 'reporting_sub_agent', 'dashboard_sub_agent', 'collaboration_sub_agent', 'workflow_sub_agent', 'orchestration_sub_agent', 'scheduling_sub_agent', 'resource_management_sub_agent', 'cost_optimization_sub_agent', 'performance_tuning_sub_agent']
        duplicate_agents = []
        missing_agents = []
        for agent_name in expected_agents:
            ...
            if legacy_files:
                duplicate_agents.append({'agent': agent_name, 'duplicates': [f.name for f in legacy_files]})
    ...
    def test_tool_execution_consolidation_complete(self):
        ...
        logger.info('=' * 80)
        dispatcher_files = ['netra_backend/app/agents/tool_dispatcher.py', 'netra_backend/app/agents/tool_dispatcher_core.py', 'netra_backend/app/agents/request_scoped_tool_dispatcher.py', 'netra_backend/app/agents/tool_dispatcher_legacy.py']
        ...
        assert len(existing_dispatchers) == 1, f'Expected 1 dispatcher, found {len(existing_dispatchers)}: {existing_dispatchers}'
    ...
    def test_infrastructure_consolidation_complete(self):
        ...
        logger.info('=' * 80)
        legacy_infra = ['netra_backend/app/infrastructure/legacy_managers', 'netra_backend/app/infrastructure/old_session_manager.py', 'netra_backend/app/infrastructure/duplicate_id_generator.py']
    ...
    def test_no_regressions(self):
        ...
        assert regression_check['no_regressions'], f"Regressions detected: {regression_check['regressions']}"
    ...
    def test_all_legacy_files_removed(self):
        ...
        assert verification['all_removed'], f"Legacy files still exist: {verification['remaining_files']}"
    ...
    def test_mission_critical_websocket_events(self):
        ...
        logger.info('=' * 80)
        import subprocess
        result = subprocess.run([sys.executable, str(self.project_root / 'tests/mission_critical/test_websocket_agent_events_suite.py')], capture_output=True, text=True, timeout=300)
        assert result.returncode == 0, f'Mission critical WebSocket tests failed:\n{result.stdout}\n{result.stderr}'
    ...
    def test_performance_maintained(self):
        ...
        assert not perf_check['degradation_detected'], 'Performance degradation detected'
