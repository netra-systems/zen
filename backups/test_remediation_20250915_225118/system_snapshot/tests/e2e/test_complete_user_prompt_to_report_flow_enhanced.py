...
class BusinessValueMetrics:
    ...
    def to_dict(self) -> Dict[str, float]:
        return {'overall_score': self.score, 'keyword_density': self.keyword_density, 'quantitative_score': self.quantitative_score, 'industry_relevance': self.industry_relevance, 'roi_analysis': self.roi_analysis, 'structure_quality': self.structure_quality, 'content_depth': self.content_depth}
...
class EnhancedBusinessValueReportValidator:
    ...
    """
    BUSINESS_VALUE_KEYWORDS = {'recommendations', 'optimize', 'reduce', 'improve', 'save', 'savings', 'cost', 'efficiency', 'performance', 'strategy', 'opportunity', 'action', 'implement', 'upgrade', 'migrate', 'configure', 'best practices', 'guidelines', 'steps', 'plan', 'solution', 'prioritize', 'eliminate', 'consolidate', 'streamline', 'automate'}
    QUANTITATIVE_KEYWORDS = {'$', '%', 'percent', 'dollars', 'cost', 'hours', 'minutes', 'reduction', 'increase', 'improvement', 'roi', 'return on investment', 'budget', 'expense', 'annually', 'monthly', 'quarterly', 'per year', 'faster', 'slower', 'efficiency gain', 'cost savings', 'time savings'}
    INDUSTRY_RELEVANCE_KEYWORDS = {'cloud', 'aws', 'azure', 'gcp', 'kubernetes', 'docker', 'serverless', 'infrastructure', 'database', 'storage', 'compute', 'networking', 'security', 'compliance', 'monitoring', 'logging', 'backup', 'scalability', 'availability', 'disaster recovery', 'devops'}
    ROI_INDICATORS = {'payback period', 'break even', 'profit margin', 'cost benefit', 'investment return', 'savings per month', 'annual savings', 'cost avoidance', 'efficiency gains', 'productivity increase'}
    ...
    def validate_report(self, report_content: str) -> Tuple[bool, BusinessValueMetrics, List[str]]:
        ...
        self.business_metrics = self._calculate_enhanced_metrics(business_analysis, quantitative_analysis, industry_analysis, roi_analysis, structural_analysis, depth_analysis)
        self.detailed_analysis = {'business_keywords': business_analysis, 'quantitative_content': quantitative_analysis, 'industry_relevance': industry_analysis, 'roi_analysis': roi_analysis, 'structure_analysis': structural_analysis, 'content_depth': depth_analysis, 'content_length': len(report_content), 'word_count': len(report_content.split()), 'sentence_count': report_content.count('.') + report_content.count('!') + report_content.count('?')}
    ...
    def _analyze_business_keywords(self, content_lower: str) -> Dict[str, Any]:
        ...
        return {'found_keywords': found_keywords, 'density': density, 'high_value_count': high_value_count, 'keyword_frequency': keyword_frequency, 'unique_keyword_count': len(found_keywords), 'score': min(35, density * 35 + high_value_count * 2)}
    ...
    def _analyze_quantitative_content(self, content_lower: str) -> Dict[str, Any]:
        ...
        return {'found_keywords': found_quantitative, 'has_data': len(found_quantitative) > 0, 'has_percentages': has_percentages, 'has_monetary': has_monetary, 'has_time_metrics': has_time_metrics, 'has_improvement_metrics': has_improvement_metrics, 'numeric_patterns': numeric_patterns, 'score': base_score + bonus_score}
    ...
    def _analyze_industry_relevance(self, content_lower: str) -> Dict[str, Any]:
        ...
        return {'found_keywords': found_industry, 'has_relevance': len(found_industry) > 0, 'has_cloud_focus': has_cloud_focus, 'has_containerization': has_containerization, 'has_database_focus': has_database_focus, 'has_security_focus': has_security_focus, 'has_technical_depth': has_technical_depth, 'has_specific_technologies': has_specific_technologies, 'score': min(18, len(found_industry) * 2.5 + (3 if has_specific_technologies else 0))}
    ...
    def _analyze_roi_content(self, content_lower: str) -> Dict[str, Any]:
        ...
        return {'found_keywords': found_roi, 'has_roi': len(found_roi) > 0, 'has_timeframe': has_timeframe, 'has_cost_benefit': has_cost_benefit, 'has_investment_analysis': has_investment_analysis, 'score': min(12, len(found_roi) * 4 + (4 if has_timeframe else 0) + (2 if has_investment_analysis else 0))}
    ...
    def _analyze_report_structure(self, report_content: str) -> Dict[str, Any]:
        ...
        has_recommendations_section = any((marker in content_lower for marker in ['recommendations:', 'suggested actions:', 'next steps:', 'action items:', 'implementation plan:', 'key recommendations']))
        specific_actions = ['configure', 'set to', 'change to', 'upgrade to', 'migrate from', 'implement', 'deploy', 'enable', 'disable', 'replace with', 'switch to', 'consolidate', 'eliminate', 'automate', 'scale up', 'scale down', 'optimize for', 'tune', 'adjust']
        ...
        has_prioritization = any((marker in content_lower for marker in ['priority', 'first', 'second', 'third', 'immediate', 'short-term', 'long-term', 'critical', 'important', 'quick wins', 'phase']))
        ...
        if has_bullet_points or has_numbered_list:
            structure_score += 3
        return {'sections_count': len(sections), 'has_recommendations_section': has_recommendations_section, 'has_specific_recommendations': has_specific_recommendations, 'specific_action_count': specific_action_count, 'has_prioritization': has_prioritization, 'has_bullet_points': has_bullet_points, 'has_numbered_list': has_numbered_list, 'score': structure_score}
    ...
    def _analyze_content_depth(self, report_content: str) -> Dict[str, Any]:
        ...
        if 10 <= avg_sentence_length <= 25:
            depth_score += 2
        return {'word_count': word_count, 'sentence_count': sentence_count, 'paragraph_count': paragraph_count, 'avg_sentence_length': avg_sentence_length, 'has_examples': has_examples, 'has_explanations': has_explanations, 'score': depth_score}

    def _extract_numeric_patterns(self, content_lower: str) -> Dict[str, Any]:
        """Extract and analyze numeric patterns in content."""
        import re
        numeric_patterns = re.findall('\\b\\d+(?:\\.\\d+)?(?:\\s*[%$]|\\s*percent|\\s*dollars?|\\s*hours?|\\s*minutes?)\\b', content_lower)
        standalone_numbers = re.findall('\\b\\d+(?:\\.\\d+)?\\b', content_lower)
        return {'numeric_with_units': numeric_patterns, 'standalone_numbers': standalone_numbers, 'count': len(numeric_patterns) + len(standalone_numbers), 'has_percentages': any(('%' in pattern or 'percent' in pattern for pattern in numeric_patterns)), 'has_currency': any(('$' in pattern or 'dollar' in pattern for pattern in numeric_patterns))}

    def _calculate_enhanced_metrics(self, business_analysis: Dict, quantitative_analysis: Dict, industry_analysis: Dict, roi_analysis: Dict, structural_analysis: Dict, depth_analysis: Dict) -> BusinessValueMetrics:
        """Calculate enhanced business value metrics with weighted components."""
        keyword_score = business_analysis['score']
        quantitative_score = quantitative_analysis['score']
        industry_score = industry_analysis['score']
        roi_score = roi_analysis['score']
        structure_score = structural_analysis['score']
        ...
        return BusinessValueMetrics(score=total_score, keyword_density=business_analysis['density'], quantitative_score=quantitative_score, industry_relevance=industry_score, roi_analysis=roi_score, structure_quality=structure_score, content_depth=depth_score)
    ...
    def _validate_enhanced_criteria(self) -> bool:
        ...
        meets_minimum = score >= 65 and structural_analysis.get('has_recommendations_section', False) and (content_length >= 350)
        meets_quality = score >= 75 and structural_analysis.get('has_specific_recommendations', False) and (structural_analysis.get('specific_action_count', 0) >= 2)
...
class EnhancedCompleteFlowEventValidator:
    ...
    BUSINESS_VALUE_EVENTS = {'report_generated', 'final_report', 'analysis_complete', 'recommendations_ready', 'insights_generated', 'agent_completed'}
    EXPECTED_ORDER_PATTERNS = [['agent_started', 'agent_thinking'], ['tool_executing', 'tool_completed'], ['agent_completed']]
    ...
    def record_event(self, event: Dict[str, Any]) -> None:
        ...
        enhanced_event = {**event.copy(), 'recorded_at': timestamp, 'sequence_position': len(self.events), 'content_size': self._calculate_content_size(event)}
    ...
    def _validate_enhanced_event_order(self) -> List[str]:
        ...
        if 'agent_completed' in event_sequence:
            ...
            if completion_idx < len(event_sequence) - 3:
    ...
    def _validate_enhanced_tool_pairing(self) -> List[str]:
        ...
        if executing_count > 0:
            ...
            if exec_times and comp_times:
                avg_tool_duration = statistics.mean([comp_times[i] - exec_times[i] for i in range(min(len(exec_times), len(comp_times)))])
    ...
    def _validate_enhanced_business_content(self) -> List[str]:
        ...
        if len(self.business_content_events) == 0:
            ...
            return issues
        min_business_events = max(2, len(self.events) // 4)
    ...
    def _validate_enhanced_performance_timing(self) -> List[str]:
        ...
        expected_max_time = 60.0 + tool_count * 20.0
        ...
        min_expected_time = 3.0 + tool_count * 1.0
    ...
    def _generate_performance_metrics(self) -> Dict[str, Any]:
        ...
        timing_stats = {}
        if gaps:
            timing_stats = {'min_gap': min(gaps), 'max_gap': max(gaps), 'avg_gap': statistics.mean(gaps), 'median_gap': statistics.median(gaps)}
        return {'total_duration': total_time, 'total_events': event_count, 'events_per_second': event_count / total_time if total_time > 0 else 0, 'event_frequencies': event_frequencies, 'timing_statistics': timing_stats, 'business_content_ratio': len(self.business_content_events) / event_count if event_count > 0 else 0, 'required_events_ratio': len(set(self.event_counts.keys()) & self.REQUIRED_EVENTS) / len(self.REQUIRED_EVENTS)}
    ...
    def _calculate_sequence_metrics(self) -> EventSequenceMetrics:
        ...
        return EventSequenceMetrics(total_events=len(self.events), required_events_count=required_present, event_timing_analysis=self.performance_metrics.get('timing_statistics', {}), sequence_integrity=required_present / len(self.REQUIRED_EVENTS) if self.REQUIRED_EVENTS else 0, business_content_ratio=len(self.business_content_events) / len(self.events) if self.events else 0)
...
class ResourceMonitor:
    ...
    def stop_monitoring(self) -> Dict[str, Any]:
        ...
        return {'start_metrics': self.start_metrics, 'final_metrics': final_metrics, 'peak_metrics': self.peak_metrics, 'resource_usage': {'memory_increase_mb': final_metrics['memory_mb'] - self.start_metrics['memory_mb'], 'cpu_peak_percent': self.peak_metrics['cpu_percent'], 'memory_peak_mb': self.peak_metrics['memory_mb'], 'open_files_increase': final_metrics['open_files'] - self.start_metrics['open_files']}}
    ...
    def _capture_metrics(self) -> Dict[str, float]:
        ...
        try:
            ...
            return {'memory_mb': process.memory_info().rss / 1024 / 1024, 'cpu_percent': process.cpu_percent(), 'open_files': process.num_fds() if hasattr(process, 'num_fds') else len(process.open_files()), 'threads': process.num_threads()}
        except Exception as e:
...
class EnhancedCompleteUserPromptToReportTester:
    ...
    async def _validate_websocket_connection(self) -> None:
        ...
        test_ping = {'type': 'ping', 'timestamp': datetime.now(timezone.utc).isoformat()}
    ...
    async def test_complete_prompt_to_report_flow(self, user_prompt: str, timeout: float=120.0, expected_complexity: str='medium') -> Tuple[bool, EnhancedCompleteFlowEventValidator, Optional[str], Dict[str, Any]]:
        """
        ...
        event_task = asyncio.create_task(self._collect_enhanced_flow_events(validator, adjusted_timeout, expected_complexity))
        prompt_message = {'type': 'chat_message', 'text': user_prompt, 'timestamp': datetime.now(timezone.utc).isoformat(), 'metadata': {'test_id': str(uuid.uuid4()), 'expected_complexity': expected_complexity}}
        ...
        performance_metrics = {'execution_time': execution_time, 'expected_complexity': expected_complexity, 'sequence_metrics': sequence_metrics.to_dict() if hasattr(sequence_metrics, 'to_dict') else sequence_metrics.__dict__, 'resource_usage': resource_metrics, 'validator_metrics': validator.performance_metrics, 'event_count': len(validator.events), 'business_content_events': len(validator.business_content_events)}
    ...
    async def _collect_enhanced_flow_events(self, validator: EnhancedCompleteFlowEventValidator, timeout: float, expected_complexity: str) -> None:
        ...
        complexity_expectations = {'simple': {'min_events': 5, 'min_business_events': 2}, 'medium': {'min_events': 8, 'min_business_events': 3}, 'complex': {'min_events': 12, 'min_business_events': 5}}
        expectations = complexity_expectations.get(expected_complexity, complexity_expectations['medium'])
        event_count_at_last_check = 0
        stall_detection_interval = 10.0
    ...
    def _extract_enhanced_final_report(self, events: List[Dict[str, Any]]) -> Optional[str]:
        ...
        completion_events = [e for e in reversed(events) if e.get('type') in {'agent_completed', 'final_report', 'report_generated', 'analysis_complete'}]
        extraction_strategies = [self._extract_from_specific_fields, self._extract_from_structured_content, self._extract_from_concatenated_content]
...
class EnhancedCompleteUserPromptToReportFlowTests:
    ...
    async def test_enhanced_basic_optimization_query(self, isolated_test_environment):
        ...
        try:
            ...
            optimization_query = "I'm spending $15,000/month on AWS infrastructure across 25 EC2 instances, RDS databases, and S3 storage. My team is complaining about slow performance and high costs. Please analyze my setup and provide specific cost optimization recommendations with expected savings and implementation timeframes."
            start_time = time.time()
            is_valid, validator, final_report, performance_metrics = await tester.test_complete_prompt_to_report_flow(user_prompt=optimization_query, timeout=120.0, expected_complexity='medium')
    ...
    async def test_enhanced_complex_infrastructure_analysis(self, isolated_test_environment):
        ...
        try:
            await tester.setup_real_services(isolated_test_environment)
            complex_query = "I'm running a microservices architecture on AWS with 50+ EC2 instances across multiple availability zones, using EKS for container orchestration, RDS Aurora for databases, ElastiCache for caching, and CloudFront for CDN. We're experiencing performance bottlenecks, high costs ($25K/month), and security compliance challenges. Please analyze our entire infrastructure and provide a comprehensive optimization plan with security improvements, cost reduction strategies, and performance enhancements. Include specific recommendations for each service, expected ROI, and implementation phases."
            start_time = time.time()
            is_valid, validator, final_report, performance_metrics = await tester.test_complete_prompt_to_report_flow(user_prompt=complex_query, timeout=180.0, expected_complexity='complex')
            execution_time = time.time() - start_time
    ...
    async def test_enhanced_error_recovery_scenarios(self, isolated_test_environment):
        ...
        try:
            await tester.setup_real_services(isolated_test_environment)
            error_test_cases = [{'name': 'Very short query', 'query': 'Help', 'expected_behavior': 'should_complete_with_clarification', 'timeout': 30.0}, {'name': 'Extremely long query', 'query': 'Analyze my AWS infrastructure ' * 100, 'expected_behavior': 'should_complete_within_timeout', 'timeout': 90.0}, {'name': 'Mixed language query', 'query': 'Optimize my cloud costs. Como puedo reducir costos en la nube?', 'expected_behavior': 'should_handle_gracefully', 'timeout': 60.0}]
            results = []
            for test_case in error_test_cases:
                logger.info(f"Testing error scenario: {test_case['name']}")
                try:
                    start_time = time.time()
                    is_valid, validator, final_report, performance_metrics = await tester.test_complete_prompt_to_report_flow(user_prompt=test_case['query'], timeout=test_case['timeout'], expected_complexity='simple')
                    execution_time = time.time() - start_time
                    test_result = {'test_name': test_case['name'], 'completed': is_valid, 'execution_time': execution_time, 'final_report_length': len(final_report) if final_report else 0, 'event_count': len(validator.events), 'errors': validator.errors[:3]}
                    ...
                    results.append({'test_name': test_case['name'], 'completed': False, 'error': str(e)})
    ...
    async def test_enhanced_performance_profiling_validation(self, isolated_test_environment):
        ...
        try:
            await tester.setup_real_services(isolated_test_environment)
            performance_queries = [{'name': 'Simple query', 'query': 'What are 3 quick wins for AWS cost optimization?', 'complexity': 'simple', 'max_time': 45.0, 'min_events': 5}, {'name': 'Medium query', 'query': 'Analyze my EC2 instances and provide cost optimization recommendations with ROI calculations.', 'complexity': 'medium', 'max_time': 90.0, 'min_events': 8}]
            performance_results = []
            for query_test in performance_queries:
                logger.info(f"Performance testing: {query_test['name']}")
                start_time = time.time()
                is_valid, validator, final_report, performance_metrics = await tester.test_complete_prompt_to_report_flow(user_prompt=query_test['query'], timeout=query_test['max_time'] + 30, expected_complexity=query_test['complexity'])
                execution_time = time.time() - start_time
                ...
                sequence_metrics = performance_metrics.get('sequence_metrics', {})
                resource_usage = performance_metrics.get('resource_usage', {})
                perf_result = {'query_type': query_test['name'], 'complexity': query_test['complexity'], 'execution_time': execution_time, 'events_generated': len(validator.events), 'business_content_ratio': sequence_metrics.get('business_content_ratio', 0), 'memory_usage_mb': resource_usage.get('memory_increase_mb', 0), 'event_sequence_integrity': sequence_metrics.get('sequence_integrity', 0)}
