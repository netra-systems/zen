...
class TestCSPRegression:
    ...
    def test_staging_csp_blob_support(self, staging_url):
        ...
        assert 'blob:' in directives['script-src'], "script-src must include 'blob:' to support worker creation"
        if 'worker-src' in directives:
            assert 'blob:' in directives['worker-src'], "worker-src must include 'blob:' when present"
    ...
    def test_staging_csp_external_apis(self, staging_url):
        ...
        required_domains = ['https://featureassets.org', 'https://cloudflare-dns.com', 'https://*.staging.netrasystems.ai', 'wss://*.staging.netrasystems.ai']
        for domain in required_domains:
            assert domain in connect_src_values or domain.replace('https://', '') in connect_src_values, f'connect-src must include {domain}'
    ...
    def test_staging_csp_complete_configuration(self, staging_url):
        ...
        directives = self.parse_csp_directives(csp_header)
        required_directives = ['default-src', 'script-src', 'style-src', 'img-src', 'connect-src', 'font-src', 'media-src', 'frame-src']
        for directive in required_directives:
            assert directive in directives, f'{directive} must be present in CSP'
        assert 'blob:' in directives.get('img-src', []), "img-src must include 'blob:' for image handling"
        assert 'data:' in directives.get('img-src', []), "img-src must include 'data:' for inline images"
        assert 'data:' in directives.get('font-src', []), "font-src must include 'data:' for inline fonts"

    @pytest.mark.skipif(not os.getenv('TEST_LOCAL_CSP', '').lower() == 'true', reason='Local CSP testing requires TEST_LOCAL_CSP=true and running local server')
    def test_local_csp_configuration(self, local_url):
        ...
        if 'script-src' in directives:
            assert 'blob:' in directives['script-src'], "Local script-src should include 'blob:' for development"
        if 'connect-src' in directives:
            connect_src_values = ' '.join(directives['connect-src'])
            assert 'localhost' in connect_src_values or '127.0.0.1' in connect_src_values, 'Local connect-src should allow localhost connections'
    ...
    def test_csp_not_too_permissive(self, staging_url):
        ...
        csp_header = self.get_csp_header(staging_url)
        assert csp_header, 'CSP header must be present'
        dangerous_patterns = ["'unsafe-eval'", '*', 'http:']
        for pattern in dangerous_patterns:
            if pattern == '*':
                assert not ' * ' in f' {csp_header} ', 'CSP should not contain bare wildcard (*) sources'
            elif pattern == "'unsafe-eval'":
                assert pattern not in csp_header, f'Staging CSP should not contain {pattern} for security'
            elif pattern == 'http:':
                parts = csp_header.split()
                for part in parts:
