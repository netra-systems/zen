...
class TestDatabaseConnectionPoolMonitoring:
    ...
    def database_config(self):
        ...
        config = get_config()
        return {'database_url': config.DATABASE_URL, 'pool_size': 10, 'max_overflow': 5, 'pool_timeout': 30}

    @pytest.fixture
    def monitored_engine(self, database_config):
        """Create REAL SQLAlchemy engine with connection pool monitoring."""
        engine = create_engine(database_config['database_url'], poolclass=QueuePool, pool_size=database_config['pool_size'], max_overflow=database_config['max_overflow'], pool_timeout=database_config['pool_timeout'], pool_pre_ping=True, echo=False)
        yield engine
        engine.dispose()
    ...
    def test_real_connection_pool_metrics_collection(self, monitored_engine, database_config):
        ...
        logger.info(f'Initial pool metrics - Size: {initial_size}, Checked In: {initial_checked_in}, Checked Out: {initial_checked_out}, Overflow: {initial_overflow}, Invalid: {initial_invalid}')
    ...
    def test_real_pool_configuration_validation(self, database_config):
        ...
        start_time = time.time()
        test_configs = [{'name': 'minimal', 'config': {'pool_size': 2, 'max_overflow': 1, 'pool_timeout': 10}, 'should_work': True}, {'name': 'standard', 'config': {'pool_size': 5, 'max_overflow': 5, 'pool_timeout': 30}, 'should_work': True}, {'name': 'high_performance', 'config': {'pool_size': 15, 'max_overflow': 10, 'pool_timeout': 60}, 'should_work': True}]
        validation_results = {}
        for test_config in test_configs:
            ...
            try:
                test_engine = create_engine(database_config['database_url'], poolclass=QueuePool, pool_size=pool_config['pool_size'], max_overflow=pool_config['max_overflow'], pool_timeout=pool_config['pool_timeout'], echo=False)
                test_connections = []
                ...
                validation_results[config_name] = {'success': True, 'pool_size': final_size, 'max_overflow': final_overflow, 'connections_tested': len(test_connections)}
                test_engine.dispose()
                if should_work:
                ...
                validation_results[config_name] = {'success': False, 'error': str(e)}
                if should_work:
        ...
        assert validation_results.get('standard', {}).get('success', False), 'Standard pool configuration should always work'
        execution_time = time.time() - start_time
    ...
    async def test_real_concurrent_pool_monitoring(self, monitored_engine):
        ...
        async def concurrent_database_worker(worker_id: int, operations: int):
            ...
            worker_results = {'worker_id': worker_id, 'operations_completed': 0, 'errors': [], 'connection_times': [], 'query_times': []}
        ...
        tasks = [asyncio.create_task(concurrent_database_worker(worker_id, operations_per_worker)) for worker_id in range(num_workers)]
        ...
        for i, result in enumerate(worker_results):
            if isinstance(result, Exception):
                ...
                logger.info(f"Worker {i}: {result['operations_completed']} ops, {len(result['errors'])} errors, avg conn time: {sum(result['connection_times']) / len(result['connection_times']):.4f}s")
        ...
        logger.info(f'Concurrent test results: {total_operations}/{expected_operations} operations succeeded ({success_rate:.1f}%), {total_errors} errors')
