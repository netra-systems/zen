...
@pytest.mark.e2e
class Issue598HealthEndpointsSimpleTests:
    """
    ...
    async def test_health_endpoint_accessibility_staging(self):
        ...
        expected_health_endpoints = ['/health', '/api/health', '/health/ready', '/health/live', '/health/startup']
        health_results = {}
        all_successful = True
        async with httpx.AsyncClient(timeout=30.0) as client:
            for endpoint in expected_health_endpoints:
                ...
                try:
                    ...
                    health_results[endpoint] = {'status_code': response.status_code, 'accessible': response.status_code in [200, 503], 'response_time_ms': response.elapsed.total_seconds() * 1000, 'url': url, 'user_agent': 'test-client/1.0', 'timestamp': datetime.now(timezone.utc).isoformat()}
                    try:
                        if response.headers.get('content-type', '').startswith('application/json'):
                            health_results[endpoint]['response_body'] = response.json()
                        else:
                            health_results[endpoint]['response_text'] = response.text[:200]
                    ...
                    health_results[endpoint] = {'status_code': None, 'accessible': False, 'error': 'timeout', 'url': url, 'timestamp': datetime.now(timezone.utc).isoformat()}
                    all_successful = False
                except Exception as e:
                    health_results[endpoint] = {'status_code': None, 'accessible': False, 'error': str(e), 'url': url, 'timestamp': datetime.now(timezone.utc).isoformat()}
                    all_successful = False
        print('=== ISSUE #598 HEALTH ENDPOINT TEST RESULTS ===')
        for endpoint, result in health_results.items():
            print(f'{endpoint}: {result}')
        endpoints_with_404 = [endpoint for endpoint, result in health_results.items() if result.get('status_code') == 404]
        if endpoints_with_404:
            print(f'ISSUE #598 REPRODUCED: These endpoints returned 404: {endpoints_with_404}')
            error_details = {'issue': 'Health endpoints returning 404 in staging', 'affected_endpoints': endpoints_with_404, 'staging_url': staging_base_url, 'test_timestamp': datetime.now(timezone.utc).isoformat(), 'full_results': health_results}
            pytest.fail(f'Issue #598 reproduced: {len(endpoints_with_404)} health endpoints returned 404. Details: {error_details}')
        successful_endpoints = [endpoint for endpoint, result in health_results.items() if result.get('accessible', False)]
    ...
    async def test_curl_user_agent_compatibility(self):
        ...
        """
        staging_base_url = 'https://netra-backend-staging-service-672236085899.us-central1.run.app'
        curl_headers = {'User-Agent': 'curl/7.81.0'}
        health_results = {}
        async with httpx.AsyncClient(timeout=30.0, headers=curl_headers) as client:
            for endpoint in ['/health', '/api/health']:
                ...
                try:
                    response = await client.get(url)
                    health_results[endpoint] = {'status_code': response.status_code, 'user_agent': 'curl/7.81.0', 'accessible': response.status_code != 404, 'url': url}
                    if response.status_code == 404:
                        print(f'ISSUE #598: {endpoint} returned 404 with curl user agent')
                except Exception as e:
                    health_results[endpoint] = {'error': str(e), 'accessible': False, 'url': url}
        curl_404_endpoints = [endpoint for endpoint, result in health_results.items() if result.get('status_code') == 404]
