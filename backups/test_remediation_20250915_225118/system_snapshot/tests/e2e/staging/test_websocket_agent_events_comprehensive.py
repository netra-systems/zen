...
class WebSocketEventSequence:
    ...
    def add_event(self, event: Dict[str, Any]) -> None:
        ...
        event_with_timing = {**event, 'capture_timestamp': time.time(), 'relative_time': time.time() - self.start_time}
...
class WebSocketAgentEventsComprehensiveTests(BaseE2ETest):
    ...
    async def test_all_five_required_websocket_events_authenticated(self):
        ...
        event_sequence = WebSocketEventSequence(user_id=user_context.user_id, thread_id=user_context.thread_id)
        ...
        comprehensive_request = {'type': 'execute_agent', 'agent_type': 'comprehensive_analyzer', 'user_id': user_context.user_id, 'thread_id': user_context.thread_id, 'request_id': user_context.request_id, 'data': {'analysis_type': 'full_spectrum', 'require_tools': True, 'detailed_reasoning': True, 'progress_updates': True, 'expected_duration': 20}, 'auth': {'user_id': user_context.user_id, 'permissions': user_context.agent_context.get('permissions', [])}}
    ...
    async def test_websocket_event_timing_and_distribution(self):
        ...
        timing_request = {'type': 'execute_agent', 'agent_type': 'timing_analysis', 'user_id': user_context.user_id, 'thread_id': user_context.thread_id, 'request_id': user_context.request_id, 'data': {'execution_duration': 'extended', 'progress_frequency': 'regular', 'thinking_updates': 'frequent'}}
    ...
    async def test_multi_user_websocket_event_isolation(self):
        ...
        async def run_user_websocket_session(user_context, user_index):
            ...
            try:
                ...
                user_request = {'type': 'execute_agent', 'agent_type': 'isolation_test', 'user_id': user_context.user_id, 'thread_id': user_context.thread_id, 'request_id': user_context.request_id, 'data': {'user_identifier': f'user_{user_index}', 'isolation_test': True, 'unique_data': f'data_for_user_{user_index}'}}
                await websocket.send(json.dumps(user_request))
                await asyncio.wait_for(event_task, timeout=60.0)
                await websocket.close()
                return {'user_id': user_context.user_id, 'user_index': user_index, 'event_sequence': event_sequence, 'success': True}
            except Exception as e:
                return {'user_id': user_context.user_id, 'user_index': user_index, 'error': str(e), 'success': False}
        user_tasks = [run_user_websocket_session(user_context, i) for i, user_context in enumerate(self.test_users[:3])]
    ...
    async def test_websocket_reconnection_and_event_continuity(self):
        ...
        websocket = await self.ws_auth_helper.connect_authenticated_websocket(timeout=20.0)
        initial_events = []
        long_execution_request = {'type': 'execute_agent', 'agent_type': 'resilience_test', 'user_id': user_context.user_id, 'thread_id': user_context.thread_id, 'request_id': user_context.request_id, 'data': {'execution_time': 'extended', 'checkpoint_support': True, 'interruption_recovery': True}}
        ...
        resume_request = {'type': 'resume_agent', 'user_id': user_context.user_id, 'thread_id': user_context.thread_id, 'request_id': user_context.request_id, 'data': {'resume_from_checkpoint': True}}
    ...
    async def test_websocket_event_content_quality_validation(self):
        ...
        quality_request = {'type': 'execute_agent', 'agent_type': 'content_quality_analyzer', 'user_id': user_context.user_id, 'thread_id': user_context.thread_id, 'request_id': user_context.request_id, 'data': {'content_focus': 'high_quality', 'detailed_explanations': True, 'actionable_insights': True, 'business_context': True}}
