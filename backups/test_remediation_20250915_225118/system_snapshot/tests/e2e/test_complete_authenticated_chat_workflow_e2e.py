...
from netra_backend.app.websocket_core.event_validator import AgentEventValidator, CriticalAgentEventType, assert_critical_events_received
...
class CompleteAuthenticatedChatWorkflowE2ETests(SSotBaseTestCase):
    ...
    async def test_complete_authenticated_chat_workflow_golden_path(self):
        ...
        user_context = await create_authenticated_user_context(user_email=f'golden_path_e2e_{uuid.uuid4().hex[:8]}@example.com', environment=self.environment, permissions=['read', 'write', 'chat', 'agent_execution', 'websocket'], websocket_enabled=True)
        ...
        ws_headers.update({'X-User-ID': str(user_context.user_id), 'X-Thread-ID': str(user_context.thread_id), 'X-Request-ID': str(user_context.request_id), 'X-WebSocket-Client-ID': str(user_context.websocket_client_id)})
        websocket_url = self.ws_auth_helper.config.websocket_url
        try:
            websocket = await asyncio.wait_for(websockets.connect(websocket_url, additional_headers=ws_headers, ping_interval=30, ping_timeout=15, close_timeout=10), timeout=self.config.connection_timeout)
        ...
        try:
            ...
            chat_request = {'type': 'chat_message', 'content': 'Please analyze sample business data and provide strategic insights with actionable recommendations for improving customer retention and revenue growth', 'user_id': str(user_context.user_id), 'thread_id': str(user_context.thread_id), 'request_id': str(user_context.request_id), 'timestamp': datetime.now(timezone.utc).isoformat(), 'message_id': f'golden_path_msg_{uuid.uuid4().hex[:8]}', 'business_context': 'E2E test validating revenue-generating chat workflow'}
            ...
            event_validator = AgentEventValidator(user_context=user_context, strict_mode=True, timeout_seconds=self.config.event_timeout)
            required_events = {CriticalAgentEventType.AGENT_STARTED.value, CriticalAgentEventType.AGENT_THINKING.value, CriticalAgentEventType.TOOL_EXECUTING.value, CriticalAgentEventType.TOOL_COMPLETED.value, CriticalAgentEventType.AGENT_COMPLETED.value}
            received_events = []
    ...
    async def test_authenticated_chat_workflow_with_real_llm_integration(self):
        ...
        user_context = await create_authenticated_user_context(user_email=f'llm_integration_e2e_{uuid.uuid4().hex[:8]}@example.com', environment=self.environment, permissions=['read', 'write', 'chat', 'llm_access', 'agent_execution'], websocket_enabled=True)
        async with self.golden_path_helper.authenticated_websocket_connection(user_context):
            business_request = 'Please analyze the following business scenario and provide strategic recommendations: A SaaS company has 1000 customers, $100K MRR, 5% monthly churn rate, and wants to expand into enterprise market. What are the top 3 strategic priorities and implementation steps for the next 6 months?'
            result = await self.golden_path_helper.execute_golden_path_flow(user_message=business_request, user_context=user_context, timeout=60.0)
            ...
            events_with_responses = [event for event in result.events_received if event.event_type == 'agent_completed' and hasattr(event, 'data') and ('response' in event.data or 'message' in event.data)]
    ...
    async def test_chat_workflow_error_recovery_patterns(self):
        ...
        user_context = await create_authenticated_user_context(user_email=f'error_recovery_e2e_{uuid.uuid4().hex[:8]}@example.com', environment=self.environment, permissions=['read', 'write', 'chat', 'error_recovery_test'], websocket_enabled=True)
        challenging_request = "Please analyze a dataset that doesn't exist and provide insights on quantum computing applications for medieval agriculture while calculating the ROI of unicorn investments"
        async with self.golden_path_helper.authenticated_websocket_connection(user_context):
            result = await self.golden_path_helper.execute_golden_path_flow(user_message=challenging_request, user_context=user_context, timeout=45.0)
            if result.success:
                ...
                print(f' SEARCH:  Error details: {result.errors_encountered[:2]}')
