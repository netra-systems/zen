...
import pytest
import websockets
from tests.e2e.token_lifecycle_helpers import PerformanceBenchmark, TokenLifecycleManager, WebSocketSessionManager
from tests.e2e.jwt_token_helpers import JWTSecurityTester, JWTTestHelper
from tests.e2e.real_services_manager import RealServicesManager
from tests.e2e.test_data_factory import create_test_service_credentials, create_test_user
...
class CrossServiceJWTValidator:
    ...
    async def validate_single_jwt_across_services(self, token: str) -> Dict[str, Any]:
        ...
        return {'auth_service_valid': auth_result['success'], 'backend_rest_valid': backend_result['success'], 'websocket_valid': websocket_result['success'], 'database_context_valid': db_result['success'], 'user_id_consistent': self._check_user_id_consistency([auth_result, backend_result, websocket_result, db_result]), 'auth_duration': auth_result['duration'], 'backend_duration': backend_result['duration'], 'websocket_duration': websocket_result['duration'], 'db_duration': db_result['duration'], 'total_duration': total_duration, 'performance_ok': total_duration < 5.0, 'details': {'auth': auth_result, 'backend': backend_result, 'websocket': websocket_result, 'database': db_result}}
            ...
            async with httpx.AsyncClient(timeout=5.0, follow_redirects=True) as client:
                ...
                if response.status_code == 200:
                    try:
                        ...
                        pass
                return {'success': response.status_code in [200, 401], 'status_code': response.status_code, 'user_id': user_id, 'duration': self.performance.get_duration(start_time), 'service': 'auth'}
        except Exception as e:
            return {'success': False, 'error': str(e), 'duration': self.performance.get_duration(start_time), 'service': 'auth'}
                        ...
                        if response.status_code in [200, 401, 404]:
                            ...
                            if not user_id:
                                try:
                                    ...
                                    pass
                            best_result = {'success': True, 'status_code': response.status_code, 'endpoint': endpoint, 'user_id': user_id, 'duration': self.performance.get_duration(start_time), 'service': 'backend'}
                            break
                    except Exception:
                        continue
                if best_result:
                    ...
                    return {'success': False, 'error': 'No backend endpoints responded', 'duration': self.performance.get_duration(start_time), 'service': 'backend'}
        except Exception as e:
            return {'success': False, 'error': str(e), 'duration': self.performance.get_duration(start_time), 'service': 'backend'}
        ...
        try:
            ...
            try:
                async with websockets.connect(ws_url, open_timeout=3) as websocket:
                    test_message = {'type': 'ping', 'timestamp': datetime.now(timezone.utc).isoformat()}
                    ...
                    try:
                        ...
                        try:
                            ...
                            pass
                        return {'success': True, 'connected': True, 'response_received': True, 'user_id': user_id, 'duration': self.performance.get_duration(start_time), 'service': 'websocket'}
                ...
                if '401' in str(e) or 'Unauthorized' in str(e):
                    return {'success': True, 'connected': False, 'reason': 'unauthorized', 'duration': self.performance.get_duration(start_time), 'service': 'websocket'}
                raise
        except Exception as e:
            return {'success': False, 'error': str(e), 'duration': self.performance.get_duration(start_time), 'service': 'websocket'}
    ...
    async def _test_database_user_context(self, token: str) -> Dict[str, Any]:
        ...
        try:
            ...
            try:
                ...
                pass
            if not user_id:
                return {'success': False, 'error': 'Could not extract user_id from token', 'duration': self.performance.get_duration(start_time), 'service': 'database'}
            async with httpx.AsyncClient(timeout=5.0, follow_redirects=True) as client:
                headers = {'Authorization': f'Bearer {token}'}
                try:
                    ...
                    if response.status_code == 200:
                        ...
                        return {'success': True, 'user_id_matches': returned_user_id == user_id, 'token_user_id': user_id, 'db_user_id': returned_user_id, 'duration': self.performance.get_duration(start_time), 'service': 'database'}
                    elif response.status_code in [401, 404]:
                        return {'success': True, 'user_id_matches': None, 'token_user_id': user_id, 'status_code': response.status_code, 'duration': self.performance.get_duration(start_time), 'service': 'database'}
                    else:
                        return {'success': False, 'error': f'Unexpected status code: {response.status_code}', 'duration': self.performance.get_duration(start_time), 'service': 'database'}
                except Exception as e:
                    return {'success': False, 'error': str(e), 'duration': self.performance.get_duration(start_time), 'service': 'database'}
        except Exception as e:
            return {'success': False, 'error': str(e), 'duration': self.performance.get_duration(start_time), 'service': 'database'}
    ...
    async def validate_token_expiry_handling(self, user_id: str) -> Dict[str, Any]:
        ...
        total_duration = self.performance.get_duration(start_time)
        return {'initial_token_worked': initial_result['auth_service_valid'] or initial_result['backend_rest_valid'], 'expired_token_rejected': not (expired_result['auth_service_valid'] and expired_result['backend_rest_valid']), 'consistent_expiry_handling': self._check_consistent_expiry_handling(expired_result), 'total_duration': total_duration, 'performance_ok': total_duration < 10.0, 'initial_details': initial_result, 'expired_details': expired_result}
    ...
    async def validate_refresh_token_flow(self, user_id: str) -> Dict[str, Any]:
        ...
        if refresh_response and 'access_token' in refresh_response:
            ...
            total_duration = self.performance.get_duration(start_time)
            return {'refresh_successful': True, 'new_token_valid': validation_result['auth_service_valid'] or validation_result['backend_rest_valid'], 'cross_service_validation': validation_result, 'total_duration': total_duration, 'performance_ok': total_duration < 10.0}
        else:
            total_duration = self.performance.get_duration(start_time)
            return {'refresh_successful': False, 'error': 'Refresh token API failed', 'total_duration': total_duration, 'performance_ok': total_duration < 10.0}
...
async def test_jwt_single_token_all_services():
    ...
    try:
        ...
        token = validator.jwt_helper.create_access_token(test_user.id, test_user.email, ['read', 'write'])
        ...
        services_responding = result['auth_service_valid'] or result['backend_rest_valid'] or result['websocket_valid']
...
async def test_jwt_invalid_token_rejection():
    ...
    try:
        ...
        services_responding = tampered_result['auth_service_valid'] or tampered_result['backend_rest_valid']
...
async def test_jwt_service_to_service_authentication():
    ...
    try:
        ...
        backend_token = validator.jwt_helper.create_token({'sub': backend_creds['service_id'], 'service': 'netra-backend', 'token_type': 'service', 'iat': int(datetime.now(timezone.utc).timestamp()), 'exp': int((datetime.now(timezone.utc) + timedelta(minutes=5)).timestamp()), 'iss': 'netra-auth-service'})
        worker_token = validator.jwt_helper.create_token({'sub': worker_creds['service_id'], 'service': 'netra-worker', 'token_type': 'service', 'iat': int(datetime.now(timezone.utc).timestamp()), 'exp': int((datetime.now(timezone.utc) + timedelta(minutes=5)).timestamp()), 'iss': 'netra-auth-service'})
