...
class RealAgentOrchestrationCore:
    ...
    REQUIRED_ORCHESTRATION_EVENTS = {'agent_started', 'supervisor_routing', 'agent_thinking', 'tool_executing', 'tool_completed', 'agent_handoff', 'agent_completed'}
    ...
    async def setup_real_orchestration_infrastructure(self, isolated_env) -> Dict[str, Any]:
        ...
        return {'auth_client': self.auth_client, 'backend_client': self.backend_client, 'ws_client': self.ws_client, 'user_data': self.test_user, 'env': isolated_env}
    ...
    async def _setup_real_test_user(self):
        ...
        register_response = await self.auth_client.register(email=test_email, password=test_password, name='Real Orchestration Test User')
        ...
        self.test_user = {'user_id': login_response.get('user_id'), 'email': test_email, 'token': login_response['access_token']}
    ...
    async def _setup_real_websocket_connection(self, backend_host: str, backend_port: str):
        ...
        connection_success = await self.ws_client.connect(token=self.test_user['token'], timeout=10.0)
        assert connection_success and self.ws_client.is_connected, 'Real WebSocket connection failed for orchestration testing'

    async def execute_real_orchestration_request(self, request_message: str, expected_agents: Optional[List[str]]=None, timeout: float=60.0) -> Dict[str, Any]:
        """Execute real agent orchestration request and track events."""
        self.orchestration_events.clear()
        self.agent_hierarchy.clear()
        collection_task = asyncio.create_task(self._collect_real_orchestration_events(timeout))
    ...
    async def _collect_real_orchestration_events(self, timeout: float):
        ...
        while time.time() - start_time < timeout and (not orchestration_complete):
            ...
            if event:
                self.orchestration_events.append({'event': event, 'timestamp': time.time() - start_time, 'event_type': event.get('type', 'unknown')})
                if event.get('type') == 'agent_started':
                    agent_info = {'agent_type': event.get('agent_type', 'unknown'), 'agent_id': event.get('agent_id'), 'parent_agent': event.get('parent_agent'), 'started_at': time.time() - start_time}
    ...
    def _analyze_real_orchestration_results(self, total_time: float, expected_agents: Optional[List[str]]) -> Dict[str, Any]:
        ...
        return {'total_time': total_time, 'event_count': len(self.orchestration_events), 'unique_event_types': len(event_types), 'agent_count': len(self.agent_hierarchy), 'agent_types': list(agent_types), 'orchestration_valid': orchestration_valid, 'missing_events': list(missing_events), 'expected_agents_present': expected_agents_present, 'events_timeline': self.orchestration_events, 'agent_hierarchy': self.agent_hierarchy, 'performance_acceptable': total_time < 45.0}
    ...
    async def validate_real_agent_handoff(self, source_agent: str, target_agent: str) -> Dict[str, bool]:
        ...
        handoff_events = [event for event in self.orchestration_events if event['event_type'] == 'agent_handoff']
        target_handoff = None
        for event in handoff_events:
            event_data = event['event']
            if event_data.get('source_agent') == source_agent and event_data.get('target_agent') == target_agent:
                target_handoff = event
                break
        return {'handoff_occurred': target_handoff is not None, 'handoff_data_complete': target_handoff is not None and target_handoff['event'].get('context_preserved', False), 'handoff_timing_valid': target_handoff is not None and target_handoff['timestamp'] < 30.0}
...
class TestRealAgentOrchestration:
    ...
    async def test_real_supervisor_agent_orchestration(self, orchestration_core):
        ...
        orchestration_result = await orchestration_core.execute_real_orchestration_request(request_message, expected_agents, timeout=60.0)
        assert orchestration_result['orchestration_valid'], f"Real orchestration invalid. Missing events: {orchestration_result['missing_events']}"
        assert orchestration_result['expected_agents_present'], f"Expected agents not present. Found: {orchestration_result['agent_types']}"
        assert orchestration_result['performance_acceptable'], f"Real orchestration too slow: {orchestration_result['total_time']:.2f}s"
        assert orchestration_result['agent_count'] >= 3, f"Insufficient agents in orchestration: {orchestration_result['agent_count']}"
        assert 'supervisor' in orchestration_result['agent_types'], 'Supervisor agent not found in orchestration'
        assert orchestration_result['event_count'] >= 10, f"Insufficient orchestration events: {orchestration_result['event_count']}"
    ...
    async def test_real_agent_context_preservation(self, orchestration_core):
        ...
        orchestration_result = await orchestration_core.execute_real_orchestration_request(context_test_message, ['supervisor', 'triage', 'data'], timeout=45.0)
    ...
    async def test_real_orchestration_error_recovery(self, orchestration_core):
        ...
        orchestration_result = await orchestration_core.execute_real_orchestration_request(error_test_message, timeout=30.0)
        assert orchestration_result['event_count'] > 0, 'No orchestration events received during error test'
        error_events = [event for event in orchestration_result['events_timeline'] if 'error' in event['event_type'].lower() or 'error' in json.dumps(event['event']).lower()]
        ...
        recovery_result = await orchestration_core.execute_real_orchestration_request(recovery_message, timeout=30.0)
        assert recovery_result['event_count'] > 0, 'Orchestration failed to recover after error'
        assert recovery_result['orchestration_valid'] or recovery_result['event_count'] >= 3, 'Orchestration recovery insufficient'
    ...
    async def test_real_concurrent_orchestration_requests(self, orchestration_core):
        ...
        """
        concurrent_requests = ['Quick cost analysis for Project Alpha', 'Performance optimization recommendations', 'Infrastructure scaling analysis']
        results = []
        for i, request in enumerate(concurrent_requests):
            result = await orchestration_core.execute_real_orchestration_request(request, timeout=25.0)
            results.append(result)
            if i < len(concurrent_requests) - 1:
                await asyncio.sleep(2.0)
        for i, result in enumerate(results):
            assert result['event_count'] > 0, f'Concurrent request {i + 1} received no events'
            assert result['performance_acceptable'] or result['total_time'] < 30.0, f"Concurrent request {i + 1} too slow: {result['total_time']:.2f}s"
    ...
    async def test_real_orchestration_performance_validation(self, orchestration_core):
        ...
        """
        performance_test_requests = ['Performance test 1: Quick infrastructure analysis', 'Performance test 2: Basic cost optimization', 'Performance test 3: Simple recommendations']
        total_start = time.time()
        performance_results = []
        for request in performance_test_requests:
            result = await orchestration_core.execute_real_orchestration_request(request, timeout=20.0)
    ...
    async def test_real_orchestration_agent_types_validation(self, orchestration_core):
        ...
        """
        agent_type_tests = [('Data analysis request for cost optimization', ['supervisor', 'triage', 'data']), ('Action recommendations for infrastructure improvements', ['supervisor', 'triage', 'actions']), ('Comprehensive analysis with implementation plan', ['supervisor', 'triage', 'data', 'actions'])]
        for request_message, expected_agents in agent_type_tests:
            result = await orchestration_core.execute_real_orchestration_request(request_message, expected_agents, timeout=40.0)
            assert result['expected_agents_present'], f"Expected agents {expected_agents} not found. Got: {result['agent_types']}"
            assert result['orchestration_valid'], f'Orchestration invalid for agent types {expected_agents}'
    ...
    async def test_real_orchestration_websocket_stability(self, orchestration_core):
        ...
        result = await orchestration_core.execute_real_orchestration_request(extended_request, timeout=90.0)
        ...
        followup_result = await orchestration_core.execute_real_orchestration_request(followup_request, timeout=15.0)
