...
import httpx
import pytest
from tests.e2e.service_failure_recovery_helpers import AuthServiceFailureSimulator, GracefulDegradationTester, RecoveryTimeValidator, StatePreservationValidator
...
class TestAuthServiceRecoveryer:
    ...
    async def _create_authenticated_session(self, user_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            return {'success': True, 'user_id': user_id, 'token': token, 'ws_connection': ws_connection, 'backend_session': backend_session, 'session_established_at': time.time()}
        except Exception as e:
    ...
    async def _get_auth_token(self, user_id: str) -> str:
        ...
        if real_token:
            return real_token
        return self.jwt_helper.create_access_token(user_id=user_id, email=f'{user_id}@test.com', permissions=['read', 'write'])
    ...
    async def _stop_auth_process(self) -> bool:
        ...
        try:
            ...
            if sys.platform == 'win32':
                ...
                result = subprocess.run(['lsof', '-t', f'-i:{auth_port}'], capture_output=True, text=True)
        ...
        for user_id, session_data in sessions.items():
            try:
                ...
                if ws_result['working'] and backend_result['working']:
                    ...
                    results['failures'].append({'user_id': user_id, 'ws_working': ws_result['working'], 'backend_working': backend_result['working']})
            except Exception as e:
    ...
    async def test_new_login_fails_gracefully(self) -> Dict[str, Any]:
        ...
        try:
            with patch.object(self.jwt_helper, 'get_real_token_from_auth') as mock_auth:
                ...
                return {'failed_gracefully': not session_result['success'], 'error_handled': 'error' in session_result, 'error_message': session_result.get('error', '')}
        except Exception as e:
    ...
    async def _manual_auth_restart(self) -> bool:
        ...
        try:
            ...
            if not auth_main.exists():
                ...
                return False
            cmd = [sys.executable, str(auth_main), '--host', '0.0.0.0', '--port', str(auth_port)]
            process = subprocess.Popen(cmd, cwd=str(project_root), stdout=subprocess.PIPE, stderr=subprocess.PIPE, creationflags=subprocess.CREATE_NEW_CONSOLE if sys.platform == 'win32' else 0)
    ...
    async def test_circuit_breaker_activation(self) -> Dict[str, Any]:
        ...
        try:
            ...
            async with httpx.AsyncClient(timeout=5.0, follow_redirects=True) as client:
                ...
                return {'degraded_properly': response.status_code in [200, 503], 'response_code': response.status_code, 'graceful': True}
        except Exception as e:
