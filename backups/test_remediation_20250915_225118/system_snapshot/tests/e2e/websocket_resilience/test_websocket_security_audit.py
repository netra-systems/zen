...
async def test_security_logging_comprehensive_audit_trail(jwt_generator, audit_logger):
    ...
    test_scenarios = [{'user': 'user_001', 'expired_minutes': 1, 'ip': '192.168.1.10', 'agent': 'Browser/1.0'}, {'user': 'user_002', 'expired_minutes': 30, 'ip': '10.0.0.50', 'agent': 'Mobile/2.0'}, {'user': 'user_001', 'expired_minutes': 5, 'ip': '192.168.1.10', 'agent': 'Browser/1.0'}, {'user': 'user_003', 'expired_minutes': 120, 'ip': '172.16.0.100', 'agent': 'API/1.0'}, {'user': 'user_001', 'expired_minutes': 2, 'ip': '192.168.1.10', 'agent': 'Browser/1.0'}]
    clients = []
    for i, scenario in enumerate(test_scenarios):
        expired_token = jwt_generator.create_expired_token(scenario['user'], expired_minutes_ago=scenario['expired_minutes'])
        client = SecureWebSocketTestClient('ws://mock-server/ws', expired_token, jwt_generator, audit_logger)
        headers = {'X-Forwarded-For': scenario['ip'], 'User-Agent': scenario['agent'], 'X-Request-ID': f'req_{i + 1}_{uuid.uuid4().hex[:8]}'}
    ...
    for i, event in enumerate(expired_events):
        ...
        required_fields = ['timestamp', 'event_type', 'severity', 'user_id', 'source_ip', 'token_fingerprint', 'user_agent']
...
async def test_security_event_correlation_and_pattern_detection(jwt_generator, audit_logger):
    ...
    attack_patterns = [{'pattern': 'distributed_attack', 'users': ['target_1', 'target_2', 'target_3'], 'ips': ['203.0.113.10', '203.0.113.11', '203.0.113.12'], 'attempts_per_user': 3}, {'pattern': 'single_source_multi_target', 'users': ['victim_a', 'victim_b', 'victim_c'], 'ips': ['198.51.100.50'] * 3, 'attempts_per_user': 2}]
    all_clients = []
    pattern_results = []
    for pattern_info in attack_patterns:
        ...
        for user_idx, user_id in enumerate(pattern_info['users']):
            ...
            for attempt in range(pattern_info['attempts_per_user']):
                expired_token = jwt_generator.create_expired_token(user_id, expired_minutes_ago=5 + attempt)
                client = SecureWebSocketTestClient('ws://mock-server/ws', expired_token, jwt_generator, audit_logger)
                headers = {'X-Forwarded-For': user_ip, 'User-Agent': f"AttackBot/{pattern_info['pattern']}/{user_idx}.{attempt}", 'X-Attack-Pattern': pattern_info['pattern']}
                success, result = await client.attempt_connection(headers)
                assert not success, f'Attack attempt should be blocked'
                pattern_clients.append(client)
                await asyncio.sleep(0.05)
        pattern_time = time.time() - pattern_start
        pattern_results.append({'pattern': pattern_info['pattern'], 'total_attempts': len(pattern_info['users']) * pattern_info['attempts_per_user'], 'pattern_time': pattern_time, 'clients': pattern_clients})
    ...
    recent_events = [e for e in security_events if (datetime.now(timezone.utc) - datetime.fromisoformat(e['timestamp'])).total_seconds() < 120]
    ...
    if alert_count > 0:
        ...
        for pattern_info in attack_patterns:
            for user_id in pattern_info['users']:
                ...
                if pattern_info['attempts_per_user'] >= 3:
    ...
    single_source_events = [e for e in expired_events if e.get('source_ip') == single_source_ip]
...
async def test_audit_trail_data_integrity_and_retention(jwt_generator, audit_logger):
    ...
    for i, user_id in enumerate(test_users):
        ...
        client = SecureWebSocketTestClient('ws://mock-server/ws', expired_token, jwt_generator, audit_logger)
        headers = {'X-Forwarded-For': f'192.168.100.{10 + i}', 'User-Agent': f'IntegrityTestClient/{i + 1}.0', 'X-Test-Sequence': str(i + 1)}
        success, result = await client.attempt_connection(headers)
        assert not success, f'Expired token should be rejected for {user_id}'
        after_attempt = datetime.now(timezone.utc)
        event_timestamps.append({'user_id': user_id, 'before': before_attempt, 'after': after_attempt, 'sequence': i + 1})
        await client.disconnect()
        await asyncio.sleep(0.5)
    security_events = audit_logger.security_events
    recent_events = [e for e in security_events if e['event_type'] == 'expired_token_attempt' and any((e['user_id'] == ts['user_id'] for ts in event_timestamps))]
    ...
    for event in recent_events:
        ...
        assert timing_info['before'] <= event_time <= timing_info['after'], f'Event timestamp {event_time} outside expected window for {user_id}'
    ...
    for event in recent_events:
        ...
        assert event['severity'] in ['LOW', 'MEDIUM', 'HIGH', 'CRITICAL'], f"Invalid severity level: {event['severity']}"
        if 'token_fingerprint' in event:
            ...
            assert all((c in '0123456789abcdef' for c in fingerprint)), f'Invalid fingerprint format: {fingerprint}'
    one_hour_ago = datetime.now(timezone.utc) - timedelta(hours=1)
    old_events = [e for e in security_events if datetime.fromisoformat(e['timestamp']) < one_hour_ago]
    recent_threshold = datetime.now(timezone.utc) - timedelta(minutes=5)
    truly_recent_events = [e for e in security_events if datetime.fromisoformat(e['timestamp']) > recent_threshold]
