...
async def test_token_refresh_prompt_and_flow(jwt_generator, audit_logger, token_refresh_service, expired_token):
    ...
    if 'Token expired' in result['message']:
        ...
        new_client = SecureWebSocketTestClient('ws://mock-server/ws', new_access_token, jwt_generator, audit_logger)
...
async def test_refresh_token_validation_and_security(jwt_generator, audit_logger, token_refresh_service):
    ...
    invalid_refresh_scenarios = [{'name': 'Empty refresh token', 'refresh_token': '', 'should_succeed': False}, {'name': 'Too short refresh token', 'refresh_token': 'short', 'should_succeed': False}, {'name': 'Valid length refresh token', 'refresh_token': f'refresh_{secrets.token_urlsafe(32)}', 'should_succeed': True}, {'name': 'Malformed refresh token', 'refresh_token': 'malformed_refresh_token_12345', 'should_succeed': True}]
    test_results = []
    for i, scenario in enumerate(invalid_refresh_scenarios):
        logger.info(f"Testing refresh scenario: {scenario['name']}")
        refresh_start = time.time()
        refresh_result = await token_refresh_service.refresh_expired_token(expired_token, scenario['refresh_token'])
        ...
        test_results.append({'scenario': scenario['name'], 'success': refresh_result is not None, 'refresh_time': refresh_time, 'expected_success': scenario['should_succeed']})
...
async def test_refresh_token_lifecycle_and_cleanup(jwt_generator, audit_logger, token_refresh_service):
    ...
    first_refresh = await token_refresh_service.refresh_expired_token(expired_token, initial_refresh_token)
    ...
    second_refresh = await token_refresh_service.refresh_expired_token(short_lived_token, first_refresh_token)
    ...
    reuse_attempt = await token_refresh_service.refresh_expired_token(expired_token, initial_refresh_token)
    security_events = audit_logger.security_events
    refresh_events = [e for e in security_events if e['event_type'] == 'token_refresh_success']
    assert len(refresh_events) >= 2, 'Both refresh attempts should be logged'
    for event in refresh_events[-2:]:
    ...
    final_client = SecureWebSocketTestClient('ws://mock-server/ws', second_access_token, jwt_generator, audit_logger)
