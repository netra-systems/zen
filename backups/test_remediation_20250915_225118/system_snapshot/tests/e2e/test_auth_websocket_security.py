...
class WebSocketEventCollector:
    ...
    def record_event(self, event: Dict[str, Any]) -> None:
        ...
        timestamp = time.time()
        event_with_time = {**event, 'timestamp': timestamp, 'relative_time': timestamp - self.start_time}
    ...
    def get_missing_critical_events(self) -> List[str]:
        ...
        return [event_type for event_type in CRITICAL_WEBSOCKET_EVENTS if self.event_counts[event_type] == 0]
...
class WebSocketAuthSecurityTests(SSotBaseTestCase):
    ...
    async def test_websocket_authentication_flow_with_critical_events(self):
        ...
        token = self.ws_auth_helper.create_test_jwt_token(user_id=test_user_id, email=test_email, permissions=['read', 'write', 'websocket', 'agents'])
        ...
        websocket_auth_result = await self._simulate_websocket_agent_interaction(user_id=test_user_id, token=token, event_collector=event_collector)
        ...
        assert event_collector.has_all_critical_events(), f'Missing critical WebSocket events: {missing_events} - These events are REQUIRED for chat business value'
        total_events = len(event_collector.events)
        assert total_events >= len(CRITICAL_WEBSOCKET_EVENTS), f'Insufficient events received: {total_events} < {len(CRITICAL_WEBSOCKET_EVENTS)}'
    ...
    async def test_websocket_multi_user_security_isolation(self):
        ...
        for i in range(num_users):
            user_data = {'user_id': f'ws_isolation_user_{i}_{int(time.time())}', 'email': f'ws_isolation_{i}_{int(time.time())}@example.com', 'permissions': ['read', 'write', 'websocket'], 'thread_id': str(uuid.uuid4())}
            users.append(user_data)
        user_tokens = []
        user_collectors = {}
        for user in users:
            token = self.ws_auth_helper.create_test_jwt_token(user_id=user['user_id'], email=user['email'], permissions=user['permissions'])
        ...
        auth_tasks = []
        for user in users:
            task = asyncio.create_task(self._simulate_websocket_agent_interaction(user_id=user['user_id'], token=user['token'], event_collector=user_collectors[user['user_id']]))
        ...
        for i, user in enumerate(users):
            ...
            assert collector.has_all_critical_events(), f'User {i} missing critical events: {collector.get_missing_critical_events()}'
            for event in collector.events:
                for j, other_user in enumerate(users):
                    if i != j:
                        event_str = str(event)
                        assert other_user['user_id'] not in event_str, f'User {i} received event with user {j} context: {event_str[:100]}'
        all_event_types = set()
        ...
        for user_id, collector in user_collectors.items():
            ...
            assert CRITICAL_WEBSOCKET_EVENTS[0] in user_event_types, f'User {user_id} missing agent_started event'
            assert CRITICAL_WEBSOCKET_EVENTS[-1] in user_event_types, f'User {user_id} missing agent_completed event'
        execution_time = time.time() - start_time
    ...
    async def test_websocket_authentication_security_boundaries(self):
        ...
        valid_token = self.ws_auth_helper.create_test_jwt_token(user_id=valid_user_id, email=valid_email, permissions=['websocket'])
        ...
        expired_token = self.ws_auth_helper.create_test_jwt_token(user_id=valid_user_id, email=valid_email, permissions=['websocket'], exp_minutes=-1)
        ...
        malformed_tokens = ['invalid.token.format', valid_token + 'tampered', 'Bearer malformed_token', '', 'eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9.invalid.payload']
        ...
        no_ws_token = self.ws_auth_helper.create_test_jwt_token(user_id=valid_user_id, email=valid_email, permissions=['read'])
        ...
        for header_name, header_value in security_headers.items():
            ...
            for term in sensitive_terms:
                assert term not in header_value_lower, f"Sensitive term '{term}' found in header {header_name}"
        security_tasks = []
    ...
    async def test_websocket_agent_event_timing_and_order(self):
        ...
        token = self.ws_auth_helper.create_test_jwt_token(user_id=timing_user_id, email=timing_email, permissions=['websocket', 'agents'])
        timing_collector = WebSocketEventCollector()
        self.event_collectors[timing_user_id] = timing_collector
        interaction_result = await self._simulate_websocket_agent_interaction(user_id=timing_user_id, token=token, event_collector=timing_collector, interaction_type='complex')
        ...
        for i in range(3):
            ...
            task = asyncio.create_task(self._simulate_websocket_agent_interaction(user_id=rapid_user_id, token=token, event_collector=rapid_collector, interaction_type='quick'))
        ...
        for i, collector in enumerate(rapid_collectors):
            assert collector.has_all_critical_events(), f'Rapid interaction {i} missing critical events: {collector.get_missing_critical_events()}'
    ...
    async def test_websocket_connection_timeout_and_resilience(self):
        ...
        token = self.ws_auth_helper.create_test_jwt_token(user_id=resilience_user_id, email=resilience_email, permissions=['websocket', 'agents'])
        is_valid = await self.ws_auth_helper.validate_token(token)
        ...
        for attempt in range(3):
            result = await self._simulate_websocket_agent_interaction(user_id=f'{resilience_user_id}_attempt_{attempt}', token=token, event_collector=timeout_collector, interaction_type='quick')
    ...
    async def _simulate_websocket_agent_interaction(self, user_id: str, token: str, event_collector: WebSocketEventCollector, interaction_type: str='standard') -> Dict[str, Any]:
        """
        ...
        try:
            ...
            event_collector.record_event({'type': 'agent_started', 'user_id': user_id, 'data': {'agent': 'test_agent', 'request_id': str(uuid.uuid4()), 'message': 'Starting agent processing...'}})
            for think_step in range(1 if interaction_type == 'quick' else 2):
                await asyncio.sleep(base_delay)
                event_collector.record_event({'type': 'agent_thinking', 'user_id': user_id, 'data': {'step': think_step + 1, 'thought': f'Analyzing request step {think_step + 1}...', 'reasoning': 'Processing user input and determining approach'}})
            if interaction_type in ['standard', 'complex']:
                await asyncio.sleep(base_delay)
                event_collector.record_event({'type': 'tool_executing', 'user_id': user_id, 'data': {'tool': 'data_analyzer', 'status': 'executing', 'parameters': {'query': 'test_analysis'}}})
                await asyncio.sleep(base_delay)
                event_collector.record_event({'type': 'tool_completed', 'user_id': user_id, 'data': {'tool': 'data_analyzer', 'status': 'completed', 'result': {'insights': 'Test analysis results'}}})
            await asyncio.sleep(base_delay)
            event_collector.record_event({'type': 'agent_completed', 'user_id': user_id, 'data': {'status': 'completed', 'result': {'message': 'Agent processing completed successfully', 'value': 'Test response with business value', 'user_context_verified': True}, 'execution_time': time.time() - event_collector.start_time, 'auth_context': {'user_id': user_id, 'authenticated': True, 'session_valid': True}}})
            return {'auth_successful': True, 'user_id': user_id, 'websocket_url': ws_url, 'events_sent': len(event_collector.events), 'interaction_type': interaction_type}
        except Exception as e:
            logger.error(f'WebSocket agent interaction simulation failed: {e}')
            return {'auth_successful': False, 'user_id': user_id, 'error': str(e)}
