...
class AgentWebSocketEventTracker:
    ...
    def __init__(self, environment: str='test'):
        ...
        self.thread_timelines: Dict[str, Dict] = {}
        self.event_routing_errors: List[Dict] = []
        self.agent_execution_results: Dict[str, Dict] = {}
        self.user_thread_mapping: Dict[str, Set[str]] = {}
        self.thread_user_mapping: Dict[str, str] = {}
    ...
    def register_thread(self, user_id: str, thread_id: str):
        ...
        self.thread_timelines[thread_id] = {'created_at': time.time(), 'events': {}, 'agent_started_at': None, 'agent_completed_at': None, 'total_duration': None}
    ...
    def track_agent_event(self, user_id: str, thread_id: str, event_type: str, event_data: Dict):
        ...
        if expected_user and expected_user != user_id:
            routing_error = {'timestamp': timestamp, 'thread_id': thread_id, 'event_type': event_type, 'expected_user': expected_user, 'received_by_user': user_id, 'error_type': 'cross_user_event_routing'}
        ...
        if thread_id not in self.thread_events:
            self.thread_events[thread_id] = []
        event_entry = {'timestamp': timestamp, 'relative_time': relative_time, 'user_id': user_id, 'thread_id': thread_id, 'event_type': event_type, 'is_critical': event_type in self.critical_agent_events, 'event_data': event_data}
    ...
    def validate_thread_events(self, thread_id: str, timeout_seconds: float=30.0) -> Dict:
        ...
        if thread_id not in self.thread_events:
            return {'success': False, 'error': 'Thread not found in event tracker', 'events_found': [], 'missing_events': list(self.critical_agent_events)}
        ...
        return {'success': success, 'events_found': list(received_event_types), 'missing_events': list(missing_events), 'event_count': len(thread_events), 'event_order_valid': event_order_valid, 'timing_valid': timing_valid, 'timeline': self.thread_timelines.get(thread_id, {}), 'critical_events_complete': len(missing_events) == 0}
    ...
    def generate_comprehensive_report(self) -> Dict:
        ...
        if self.thread_timelines:
            durations = [timeline.get('total_duration', 0) for timeline in self.thread_timelines.values() if timeline.get('total_duration')]
            if durations:
                avg_agent_duration = sum(durations) / len(durations)
        return {'test_duration': time.time() - self.start_time, 'total_threads': total_threads, 'total_events': total_events, 'routing_errors': total_routing_errors, 'critical_event_delivery_rate': critical_event_delivery_rate, 'threads_with_complete_events': threads_with_complete_events, 'avg_agent_execution_duration': avg_agent_duration, 'event_routing_success_rate': (total_events - total_routing_errors) / max(total_events, 1), 'detailed_routing_errors': self.event_routing_errors, 'thread_timelines': self.thread_timelines, 'thread_events': self.thread_events}

class ThreadEventWebSocketClient:
    """WebSocket client focused on thread-specific agent event collection."""

    def __init__(self, user_context: StronglyTypedUserExecutionContext, auth_helper: E2EWebSocketAuthHelper, event_tracker: AgentWebSocketEventTracker):
                ...
                try:
                    ...
                    try:
                        ...
                        if event_type in self.event_tracker.critical_agent_events:
                            self.event_tracker.track_agent_event(self.user_id, thread_id, event_type, event_data)
    ...
    async def execute_agent_in_thread(self, thread_id: str, query: str, agent_type: str='triage_agent') -> str:
        ...
        agent_request = {'type': 'agent_request', 'user_id': self.user_id, 'thread_id': thread_id, 'request_id': request_id, 'run_id': run_id, 'agent': agent_type, 'message': query, 'timestamp': datetime.now(timezone.utc).isoformat(), 'expect_all_events': True}
    ...
    async def wait_for_thread_agent_completion(self, thread_id: str, timeout: float=35.0) -> Dict:
...
class AgentWebSocketThreadEventsE2ETests:
    ...
    async def test_single_thread_all_agent_events_delivery(self):
        ...
        try:
            user_context = await create_authenticated_user_context(user_email=f'single_thread_agent_{int(time.time())}@example.com', environment=self.test_environment, websocket_enabled=True)
            client = ThreadEventWebSocketClient(user_context, self.auth_helper, self.event_tracker)
            ...
            async with client.connect(timeout=30.0):
                ...
                if not result['critical_events_complete']:
                    ...
                    pytest.fail(f"AGENT EVENT DELIVERY FAILURE: Missing {len(result['missing_events'])} critical events. Missing: {result['missing_events']}. Agent event pipeline not delivering complete Chat business value.")
                ...
                assert len(result['events_found']) >= 5, f"Insufficient events received: {len(result['events_found'])}"
                missing_critical = self.event_tracker.critical_agent_events - set(result['events_found'])
                assert len(missing_critical) == 0, f'Missing critical events: {missing_critical}'
                if result['timeline'].get('total_duration'):
                    assert result['timeline']['total_duration'] <= 30.0, f"Agent execution too slow: {result['timeline']['total_duration']}s"
        except Exception as e:
        ...
        assert actual_duration > 8.0, f'Agent event test completed too quickly ({actual_duration:.2f}s)'
    ...
    async def test_multi_thread_agent_event_routing(self):
        ...
        try:
            user_context = await create_authenticated_user_context(user_email=f'multi_thread_agent_{int(time.time())}@example.com', environment=self.test_environment, websocket_enabled=True)
            client = ThreadEventWebSocketClient(user_context, self.auth_helper, self.event_tracker)
            self.test_clients.append(client)
            async with client.connect(timeout=35.0):
                ...
                for i, result in enumerate(execution_results):
                    if isinstance(result, Exception):
                        ...
                        logger.info(f"[U+1F9F5] Thread {i}: {result.get('events_received', 0)} events, Complete: {result['success']}")
                ...
                if len(routing_violations) > 0:
                    ...
                    pytest.fail(f'THREAD EVENT ROUTING FAILURE: {len(routing_violations)} routing violations. Agent events being delivered to wrong threads. Critical thread isolation bug in event pipeline.')
                if report['critical_event_delivery_rate'] < 0.8:
                    ...
                    pytest.fail(f"CRITICAL EVENT DELIVERY FAILURE: Only {report['critical_event_delivery_rate']:.2%} of threads received complete agent events. Multi-thread event delivery pipeline broken.")
                logger.info(' PASS:  SUCCESS: Perfect multi-thread agent event routing!')
                assert successful_threads >= num_concurrent_threads * 0.8, f'Too few successful thread executions: {successful_threads}/{num_concurrent_threads}'
                expected_total_events = num_concurrent_threads * len(self.event_tracker.critical_agent_events)
                assert total_events_delivered >= expected_total_events * 0.8, f'Too few events delivered: {total_events_delivered}/{expected_total_events}'
                assert report['event_routing_success_rate'] >= 0.95, f"Event routing success rate too low: {report['event_routing_success_rate']:.2%}"
        except Exception as e:
            report = self.event_tracker.generate_comprehensive_report()
            logger.error(f' CHART:  Partial results: {report}')
            ...
            raise
        actual_duration = time.time() - test_start_time
        assert actual_duration > 12.0, f'Multi-thread agent test completed too quickly ({actual_duration:.2f}s)'
        logger.info(f' PASS:  Multi-thread agent event routing test completed in {actual_duration:.2f}s')

    async def _execute_and_track_thread_agent(self, client: ThreadEventWebSocketClient, thread_id: str, query: str, thread_index: int) -> Dict:
        ...
        try:
            ...
            thread_routing_errors = [error for error in self.event_tracker.get_thread_routing_violations() if error.get('thread_id') == thread_id]
            routing_errors = len(thread_routing_errors)
            return {'thread_index': thread_index, 'thread_id': thread_id, 'success': result['critical_events_complete'], 'events_received': events_received, 'routing_errors': routing_errors, 'timeline': result.get('timeline', {})}
        except Exception as e:
    ...
    async def test_multi_user_multi_thread_agent_event_isolation(self):
        ...
        try:
            ...
            for user_idx in range(num_users):
                user_context = await create_authenticated_user_context(user_email=f'multi_user_thread_{user_idx}_{int(time.time())}@example.com', environment=self.test_environment, websocket_enabled=True)
                client = ThreadEventWebSocketClient(user_context, self.auth_helper, self.event_tracker)
            ...
            for user_idx, client in enumerate(user_clients):
                task = asyncio.create_task(self._execute_multi_thread_user_scenario(client, threads_per_user, user_idx))
            ...
            if len(routing_violations) > 0 or isolation_violations > 0:
                ...
                pytest.fail(f'COMPLEX ISOLATION FAILURE: {len(routing_violations)} routing + {isolation_violations} isolation violations in multi-user multi-thread scenario. System cannot maintain event isolation under realistic complex load.')
            if report['critical_event_delivery_rate'] < 0.7:
                logger.error(' ALERT:  COMPLEX EVENT DELIVERY RATE FAILURE')
                pytest.fail(f"COMPLEX EVENT DELIVERY FAILURE: Only {report['critical_event_delivery_rate']:.2%} event delivery rate in complex scenario. System breaks under load.")
            logger.info(' PASS:  SUCCESS: Perfect isolation in complex multi-user multi-thread scenario!')
            assert successful_users >= num_users * 0.8, f'Too few successful users: {successful_users}/{num_users}'
            expected_threads = num_users * threads_per_user
            assert total_successful_threads >= expected_threads * 0.7, f'Too few successful threads: {total_successful_threads}/{expected_threads}'
        except Exception as e:
            report = self.event_tracker.generate_comprehensive_report()
            ...
            raise
        actual_duration = time.time() - test_start_time
        assert actual_duration > 15.0, f'Complex isolation test completed too quickly ({actual_duration:.2f}s)'
        logger.info(f' PASS:  Multi-user multi-thread isolation test completed in {actual_duration:.2f}s')

    async def _execute_multi_thread_user_scenario(self, client: ThreadEventWebSocketClient, num_threads: int, user_idx: int) -> Dict:
        ...
        try:
            async with client.connect(timeout=40.0):
                ...
                for thread_idx in range(num_threads):
                    ...
                    task = asyncio.create_task(self._execute_and_track_thread_agent(client, thread_id, query, thread_idx))
            ...
            raise
        return {'user_idx': user_idx, 'user_id': client.user_id, 'successful_threads': successful_threads, 'total_events': total_events, 'isolation_violations': isolation_violations}
