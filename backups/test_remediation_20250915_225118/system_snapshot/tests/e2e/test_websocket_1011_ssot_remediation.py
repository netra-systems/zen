...
class WebSocket1011SSoTRemediationTests(SSotAsyncTestCase):
    ...
    async def test_golden_path_with_ssot_configuration(self):
        ...
        try:
            ...
            if websocket_result.get('status_code') == 1011:
                self.fail(f"GOLDEN PATH FAILURE: WebSocket 1011 error for {primary_user['email']} indicates SSOT configuration violation. Error: {websocket_result.get('error_message')}")
            ...
            if chat_result.get('status') != 'success':
                self.fail(f"GOLDEN PATH FAILURE: Agent execution failed for {primary_user['email']} Chat functionality broken. Error: {chat_result.get('error_message')}")
    ...
    async def test_websocket_connection_success_rate_after_fix(self):
        ...
        self.assertGreaterEqual(success_rate, 0.9, f"WebSocket connection success rate should be  >= 90% after SSOT remediation. Current: {success_rate:.1%} ({connection_stats['successes']}/{connection_stats['attempts']})")
        if connection_times:
