...
class StagingServiceAuthenticationTests:
    ...
    def test_staging_has_service_credentials(self):
        ...
        assert 'SERVICE_ID' in staging_vars, 'SERVICE_ID must be configured in .env.staging'
        assert staging_vars['SERVICE_ID'] == 'netra-backend', f"SERVICE_ID must be 'netra-backend' in staging, got '{staging_vars.get('SERVICE_ID')}'"
        assert 'SERVICE_SECRET' in staging_vars, 'SERVICE_SECRET must be configured in .env.staging'
        assert len(staging_vars['SERVICE_SECRET']) >= 32, 'SERVICE_SECRET must be at least 32 characters in staging'
        jwt_secret = staging_vars.get('JWT_SECRET_KEY', '') or staging_vars.get('JWT_SECRET_STAGING', '')
        assert staging_vars['SERVICE_SECRET'] != jwt_secret, 'SERVICE_SECRET must be different from JWT_SECRET for security'
    ...
    def test_staging_config_loads_service_credentials(self):
        ...
        with patch.dict(os.environ, {'ENVIRONMENT': 'staging', 'SERVICE_ID': 'netra-backend', 'SERVICE_SECRET': 'staging-test-secret-for-verification-purposes-32chars', 'POSTGRES_HOST': 'test-host', 'POSTGRES_USER': 'test-user', 'POSTGRES_PASSWORD': 'test-pass', 'POSTGRES_DB': 'test-db'}):
            try:
                config = StagingConfig()
                assert config.service_id == 'netra-backend', "StagingConfig must use 'netra-backend' as service_id"
                assert config.environment == 'staging', "StagingConfig must set environment to 'staging'"
    ...
    def test_auth_client_sends_correct_headers_in_staging(self):
        ...
        with patch('netra_backend.app.clients.auth_client_core.get_configuration') as mock_config:
            ...
            with patch('netra_backend.app.clients.auth_client_core.get_current_environment') as mock_env:
                ...
                assert headers['X-Service-ID'] == 'netra-backend', "Auth client must send 'netra-backend' as X-Service-ID in staging"
                assert 'X-Service-Secret' in headers, 'Auth client must include X-Service-Secret header in staging'
                assert headers['X-Service-Secret'] == mock_conf.service_secret, 'Auth client must send correct service secret in staging'
    ...
    def test_auth_service_validates_staging_credentials(self):
        ...
        try:
            ...
            assert expected_service_secret, 'Auth service must have SERVICE_SECRET configured in staging'
            assert incoming_service_id == expected_service_id, f"Service ID mismatch in staging: expected '{expected_service_id}', got '{incoming_service_id}'"
            assert incoming_service_secret == expected_service_secret, 'Service secret mismatch in staging'
        finally:
    ...
    def test_error_messages_are_descriptive(self):
        ...
        env = get_env()
        test_cases = [{'incoming_id': 'backend', 'incoming_secret': 'correct-secret', 'expected_id': 'netra-backend', 'expected_secret': 'correct-secret', 'error_contains': ['Service ID mismatch', 'netra-backend', 'backend']}, {'incoming_id': 'netra-backend', 'incoming_secret': 'wrong-secret', 'expected_id': 'netra-backend', 'expected_secret': 'correct-secret', 'error_contains': ['Service secret mismatch']}, {'incoming_id': None, 'incoming_secret': 'some-secret', 'expected_id': 'netra-backend', 'expected_secret': 'correct-secret', 'error_contains': ['X-Service-ID']}]
        for case in test_cases:
            if not case['incoming_id']:
                ...
                error_msg = f"Service ID mismatch: received '{case['incoming_id']}', expected '{case['expected_id']}'. Backend should use SERVICE_ID='{case['expected_id']}'"
            elif case['incoming_secret'] != case['expected_secret']:
                error_msg = f"Service secret mismatch for service '{case['incoming_id']}'. Check that SERVICE_SECRET environment variable matches between services."
            else:
                error_msg = None
            if error_msg:
                for expected_text in case['error_contains']:
                    assert expected_text in error_msg, f"Error message should contain '{expected_text}' for clear debugging"
