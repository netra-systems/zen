...
from netra_backend.app.services.user_execution_context import UserExecutionContext, validate_user_context, InvalidContextError, ContextIsolationError
...
class TestUserIsolationValidation:
    ...
    def user1_context(self) -> UserExecutionContext:
        ...
        return UserExecutionContext.from_request(user_id='test_user_1', thread_id='thread_1', run_id='run_1', agent_context={'user_request': 'User 1 private request', 'sensitive_data': 'user1_secret_data', 'user_preferences': {'theme': 'dark', 'api_key': 'user1_api_key'}}, audit_metadata={'test_scenario': 'user_isolation_validation', 'user_tier': 'enterprise'})

    @pytest.fixture
    def user2_context(self) -> UserExecutionContext:
        """Create isolated context for user 2."""
        return UserExecutionContext.from_request(user_id='test_user_2', thread_id='thread_2', run_id='run_2', agent_context={'user_request': 'User 2 confidential request', 'sensitive_data': 'user2_secret_data', 'user_preferences': {'theme': 'light', 'api_key': 'user2_api_key'}}, audit_metadata={'test_scenario': 'user_isolation_validation', 'user_tier': 'free'})

    @pytest.fixture
    def mock_execution_context(self) -> AgentExecutionContext:
        """Create mock execution context."""
        return AgentExecutionContext(run_id='test_run', agent_name='test_agent', user_id='test_user', thread_id='test_thread')

    async def test_concurrent_agent_execution_isolation(self, user1_context: UserExecutionContext, user2_context: UserExecutionContext, mock_execution_context: AgentExecutionContext):
        """
        ...
        with patch.object(execution_core, 'agent_registry') as mock_registry:
            ...
            async def mock_execute(context, exec_context):
                received_contexts.append({'user_id': context.user_id, 'agent_context': context.agent_context.copy(), 'thread_id': context.thread_id, 'request_id': context.request_id})
                return MagicMock(success=True)
            mock_agent = AsyncMock()
            mock_agent.execute = mock_execute
            mock_registry.get_agent.return_value = mock_agent
            task1 = asyncio.create_task(execution_core.execute_agent_safely('test_agent', user1_context, mock_execution_context))
            task2 = asyncio.create_task(execution_core.execute_agent_safely('test_agent', user2_context, mock_execution_context))
            ...
            with patch.object(connection_executor, '_process_user_request') as mock_process:
                ...
                async def mock_process_request(user_context):
                    if user_context.websocket_client_id:
                        mock_send_event(user_context.websocket_client_id, {'event': 'agent_response', 'user_id': user_context.user_id, 'data': f'Response for {user_context.user_id}', 'sensitive_info': user_context.agent_context.get('sensitive_data')})
                mock_process.side_effect = mock_process_request
                user1_request = {'user_id': 'ws_user_1', 'message': 'User 1 private message', 'sensitive_data': 'user1_websocket_secret'}
                user2_request = {'user_id': 'ws_user_2', 'message': 'User 2 private message', 'sensitive_data': 'user2_websocket_secret'}
                await asyncio.gather(connection_executor.execute_with_websocket(user1_request, 'conn_1'), connection_executor.execute_with_websocket(user2_request, 'conn_2'))
    ...
    async def test_tool_dispatcher_isolation(self, user1_context: UserExecutionContext, user2_context: UserExecutionContext):
        ...
        async def mock_tool_execution(tool_name: str, params: Dict, context: UserExecutionContext):
            tool_results[context.request_id] = {'user_id': context.user_id, 'tool_name': tool_name, 'user_data': context.agent_context.copy(), 'params': params}
        ...
        with patch.object(tool_dispatcher, '_execute_tool_with_context', side_effect=mock_tool_execution):
            ...
            await asyncio.gather(tool_dispatcher.dispatch_tool('search_tool', tool_params_user1, user1_context), tool_dispatcher.dispatch_tool('search_tool', tool_params_user2, user2_context))
...
class TestMigrationFunctionalityValidation:
    ...
    def sample_context(self) -> UserExecutionContext:
        ...
        return UserExecutionContext.from_request(user_id='func_test_user', thread_id='func_thread', run_id='func_run', agent_context={'user_request': 'Analyze my usage patterns', 'user_tier': 'enterprise', 'preferences': {'analysis_depth': 'detailed'}})

    async def test_agent_execution_core_functionality_preserved(self, sample_context: UserExecutionContext):
        """Test that agent execution core functionality is preserved after migration."""
        execution_core = AgentExecutionCore()
        mock_execution_context = AgentExecutionContext(run_id='func_test_run', agent_name='test_agent', user_id='func_test_user', thread_id='func_thread')
        with patch.object(execution_core, 'agent_registry') as mock_registry:
            mock_agent = AsyncMock()
            mock_agent.execute.return_value = MagicMock(success=True, result_data={'analysis': 'usage analysis complete'}, execution_time=1.5)
            mock_registry.get_agent.return_value = mock_agent
            result = await execution_core.execute_agent_safely('analysis_agent', sample_context, mock_execution_context)
    ...
    async def test_workflow_orchestration_functionality_preserved(self, sample_context: UserExecutionContext):
        """Test workflow orchestration functionality is preserved after migration."""
        orchestrator = WorkflowOrchestrator()
        mock_execution_context = AgentExecutionContext(run_id='workflow_test_run', agent_name='supervisor', user_id='func_test_user', thread_id='func_thread')
        with patch.object(orchestrator, '_execute_workflow_steps') as mock_execute:
            mock_execute.return_value = MagicMock(success=True, workflow_result={'steps_completed': 3, 'final_result': 'workflow success'})
            result = await orchestrator.orchestrate_workflow('analysis_workflow', sample_context, mock_execution_context)
            assert result.success == True
...
class TestContextValidationAndSecurity:
    ...
    def test_invalid_context_detection(self):
        ...
        with pytest.raises(InvalidContextError, match='user_id must be a non-empty string'):
            UserExecutionContext.from_request(user_id='', thread_id='thread', run_id='run')
    ...
    def test_context_isolation_verification(self):
        ...
        context = UserExecutionContext.from_request(user_id='isolation_test_user', thread_id='isolation_thread', run_id='isolation_run')
    ...
    def test_child_context_creation_isolation(self):
        ...
        child_context = parent_context.create_child_context('child_operation', additional_agent_context={'child_data': 'child_specific_info'})
...
class TestPerformanceAndResourceManagement:
    ...
    async def test_concurrent_execution_performance(self):
        ...
        contexts = []
        for i in range(10):
            context = UserExecutionContext.from_request(user_id=f'perf_user_{i}', thread_id=f'perf_thread_{i}', run_id=f'perf_run_{i}', agent_context={'user_data': f'data_for_user_{i}'})
        ...
        with patch.object(execution_core, 'agent_registry') as mock_registry:
            ...
            mock_registry.get_agent.return_value = mock_agent
            mock_execution_context = AgentExecutionContext(run_id='perf_test', agent_name='test_agent', user_id='test_user', thread_id='test_thread')
            start_time = time.time()
            tasks = [execution_core.execute_agent_safely('test_agent', context, mock_execution_context) for context in contexts]
    ...
    def test_memory_usage_isolation(self):
        ...
        contexts = []
        for i in range(100):
            context = UserExecutionContext.from_request(user_id=f'mem_user_{i}', thread_id=f'mem_thread_{i}', run_id=f'mem_run_{i}', agent_context={'large_data': 'x' * 1000})
            contexts.append(context)
        gc.collect()
...
class TestBusinessContinuityValidation:
    ...
    async def test_golden_path_user_flow_preserved(self):
        ...
        connection_executor = ConnectionExecutor()
        user_request = {'user_id': 'golden_path_user', 'message': 'Help me optimize my AI costs by 30%', 'type': 'chat'}
        with patch.multiple(connection_executor, _process_user_request=AsyncMock(), _send_success_response=AsyncMock(), websocket_bridge=AsyncMock()):
    ...
    async def test_enterprise_customer_workflow_preserved(self):
        ...
        enterprise_context = UserExecutionContext.from_request(user_id='enterprise_customer_001', thread_id='ent_thread_001', run_id='ent_run_001', agent_context={'user_request': 'Generate comprehensive cost analysis report', 'user_tier': 'enterprise', 'customer_features': ['advanced_analytics', 'priority_support'], 'data_retention': '7_years'}, audit_metadata={'compliance_level': 'SOC2', 'data_classification': 'confidential', 'customer_id': 'CUST_ENT_001'})
        execution_core = AgentExecutionCore()
        mock_execution_context = AgentExecutionContext(run_id='enterprise_run', agent_name='enterprise_agent', user_id='enterprise_customer_001', thread_id='ent_thread_001')
        with patch.object(execution_core, 'agent_registry') as mock_registry:
            mock_agent = AsyncMock()
            mock_agent.execute.return_value = MagicMock(success=True, result_data={'report_generated': True, 'compliance_validated': True, 'enterprise_features_used': ['advanced_analytics']})
            mock_registry.get_agent.return_value = mock_agent
            result = await execution_core.execute_agent_safely('enterprise_agent', enterprise_context, mock_execution_context)
            assert result.success == True
