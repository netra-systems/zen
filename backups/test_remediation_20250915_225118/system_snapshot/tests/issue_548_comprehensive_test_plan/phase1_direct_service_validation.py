...
class TestPhase1DirectServiceValidation(SSotAsyncTestCase):
    ...
    def test_docker_redis_service_availability_required(self, real_services_fixture):
        ...
        try:
            ...
            raise AssertionError(f'ISSUE #548 DEMONSTRATED: Docker dependency blocking test execution. Redis service not available (requires Docker): {e}')
    ...
    def test_docker_postgres_service_connectivity_required(self, real_services_fixture):
        ...
        try:
            ...
            connection = psycopg2.connect(host='localhost', port=5432, database='test_db', user='test_user', password='test_password')
            ...
            cursor.execute(sql.SQL('CREATE TABLE {} (id SERIAL PRIMARY KEY, data TEXT)').format(sql.Identifier(test_table)))
            cursor.execute(sql.SQL('INSERT INTO {} (data) VALUES (%s)').format(sql.Identifier(test_table)), ('docker_test_data',))
            connection.commit()
            ...
            self.record_metric('error_type', type(e).__name__)
            self.record_metric('error_message', str(e))
            raise AssertionError(f'ISSUE #548 DEMONSTRATED: Docker dependency blocking database tests. PostgreSQL service not available (requires Docker): {e}')
    ...
    def test_docker_services_health_check_required(self, real_services_fixture):
        ...
        try:
            ...
            self.record_metric('error_type', type(e).__name__)
            self.record_metric('error_message', str(e))
            raise AssertionError(f'ISSUE #548 DEMONSTRATED: Docker services not healthy, blocking Golden Path execution. Service health check failed: {e}')
    ...
    def test_golden_path_websocket_docker_dependency(self, real_services_fixture):
        ...
        try:
            ...
            self.record_metric('error_type', type(e).__name__)
            self.record_metric('error_message', str(e))
            raise AssertionError(f'ISSUE #548 DEMONSTRATED: WebSocket services require Docker backend dependencies. Service dependency check failed: {e}')
    ...
    def test_golden_path_agent_execution_docker_requirements(self, real_services_fixture):
        ...
        """
        try:
            service_requirements = {'redis': 6379, 'postgres': 5432, 'backend': 8000}
            failed_services = []
            ...
            self.record_metric('error_type', type(e).__name__)
            self.record_metric('error_message', str(e))
            raise AssertionError(f'ISSUE #548 DEMONSTRATED: Agent execution requires Docker-orchestrated services. Service dependency validation failed: {e}')
...
class TestPhase1ServiceConnectivityValidation(SSotAsyncTestCase):
    ...
    def test_multi_service_orchestration_required(self, real_services_fixture):
        ...
        """
        try:
            service_chain = [('postgres', 5432, 'database persistence'), ('redis', 6379, 'session and caching'), ('backend', 8000, 'API and WebSocket services')]
            service_statuses = []
            for service_name, port, description in service_chain:
                try:
                    if service_name == 'backend':
                        ...
                        if response.status_code == 200:
                            health_data = response.json() if response.headers.get('content-type', '').startswith('application/json') else {}
                            service_statuses.append({'name': service_name, 'status': 'healthy', 'description': description, 'details': health_data})
                        else:
                            service_statuses.append({'name': service_name, 'status': 'unhealthy', 'description': description, 'error': f'HTTP {response.status_code}'})
                    else:
                        sock = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
                        sock.settimeout(2)
                        result = sock.connect_ex(('localhost', port))
                        sock.close()
                        if result == 0:
                            service_statuses.append({'name': service_name, 'status': 'connected', 'description': description})
                        else:
                            service_statuses.append({'name': service_name, 'status': 'connection_failed', 'description': description, 'error': f'Connection refused on port {port}'})
                except Exception as e:
                    service_statuses.append({'name': service_name, 'status': 'error', 'description': description, 'error': str(e)})
            ...
            self.record_metric('error_type', type(e).__name__)
            self.record_metric('error_message', str(e))
            raise AssertionError(f'ISSUE #548 DEMONSTRATED: Golden Path requires Docker service orchestration. Multi-service coordination failed: {e}')
    ...
    def test_docker_compose_service_dependencies(self, real_services_fixture):
        ...
        """
        try:
            result = subprocess.run(['docker-compose', 'ps'], capture_output=True, text=True, timeout=10, cwd='.')
            if result.returncode != 0:
                raise Exception(f'Docker Compose not running or configured: {result.stderr}')
            compose_output = result.stdout
            expected_services = ['redis', 'postgres', 'backend']
            running_services = []
            missing_services = []
            ...
            self.record_metric('error_type', type(e).__name__)
            self.record_metric('error_message', str(e))
            raise AssertionError(f'ISSUE #548 DEMONSTRATED: Golden Path requires Docker Compose orchestration. Docker Compose service dependency validation failed: {e}')
