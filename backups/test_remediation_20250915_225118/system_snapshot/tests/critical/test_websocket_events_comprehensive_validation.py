...
REQUIRED_EVENT_FIELDS = {'agent_started': ['agent_name', 'run_id', 'timestamp'], 'agent_thinking': ['thought', 'agent_name', 'timestamp'], 'tool_executing': ['tool_name', 'agent_name', 'timestamp'], 'tool_completed': ['tool_name', 'agent_name', 'result', 'timestamp'], 'agent_completed': ['agent_name', 'run_id', 'timestamp']}
MAX_EVENT_DELAY_MS = 2000
MAX_CONNECTION_TIME_MS = 10000
MIN_CONCURRENT_USERS = 10
STRESS_TEST_USERS = 25
PERFORMANCE_TEST_EVENTS_PER_USER = 50
...
class ComprehensiveEventCapture:
    ...
    def __init__(self):
        ...
        self.event_sequences: Dict[str, List[str]] = {}
        self.expected_sequences: Dict[str, List[str]] = {}
    ...
    def add_session(self, user_id: str, connection_id: str, websocket=None) -> UserSession:
        ...
        with self.lock:
            session = UserSession(user_id=user_id, connection_id=connection_id, websocket=websocket, connection_start_time=time.time())
            self.sessions[user_id] = session
            self.event_sequences[user_id] = []
            self.expected_sequences[user_id] = []
            return session

    def record_event(self, user_id: str, event_type: str, payload: Dict[str, Any], thread_id: str=None, run_id: str=None, connection_id: str=None) -> WebSocketEvent:
        """Record a WebSocket event with comprehensive validation."""
        timestamp = time.time()
        with self.lock:
            event = WebSocketEvent(event_type=event_type, payload=payload, timestamp=timestamp, user_id=user_id, thread_id=thread_id, run_id=run_id, connection_id=connection_id)
        ...
        if event_type in CRITICAL_EVENTS:
            ...
            for field in required_fields:
                if field not in payload:
                    ...
                    self.content_violations.append({'event': event, 'violation_type': 'missing_field', 'field': field, 'timestamp': event.timestamp})
    ...
    def _detect_cross_user_violations(self, event: WebSocketEvent) -> None:
        ...
        if 'user_id' in payload and payload['user_id'] != event.user_id:
            violation = {'violation_type': 'cross_user_data', 'event': event, 'intended_user': payload['user_id'], 'actual_user': event.user_id, 'timestamp': event.timestamp, 'severity': 'CRITICAL'}
    ...
    def _detect_timing_violations(self, event: WebSocketEvent) -> None:
        ...
        if event.delivery_latency_ms and event.delivery_latency_ms > MAX_EVENT_DELAY_MS:
            violation = {'violation_type': 'excessive_latency', 'event': event, 'latency_ms': event.delivery_latency_ms, 'max_allowed_ms': MAX_EVENT_DELAY_MS, 'timestamp': event.timestamp}
        ...
        with self.lock:
            for user_id, expected in self.expected_sequences.items():
                ...
                if len(actual) != len(expected):
                    violations.append({'user_id': user_id, 'violation_type': 'sequence_length_mismatch', 'expected_count': len(expected), 'actual_count': len(actual), 'expected': expected, 'actual': actual})
                    continue
                for i, (exp_event, act_event) in enumerate(zip(expected, actual)):
                    if exp_event != act_event:
                        violations.append({'user_id': user_id, 'violation_type': 'sequence_order_violation', 'position': i, 'expected_event': exp_event, 'actual_event': act_event, 'expected_sequence': expected, 'actual_sequence': actual})
                        break
        return violations
    ...
    def get_performance_metrics(self) -> Dict[str, Any]:
        ...
        with self.lock:
            ...
            return {'total_events': total_events, 'total_sessions': total_sessions, 'test_duration_s': test_duration, 'events_per_second': total_events / test_duration if test_duration > 0 else 0, 'avg_latency_ms': avg_latency, 'max_latency_ms': max_latency, 'min_latency_ms': min_latency, 'p95_latency_ms': p95_latency, 'cross_user_violations': len(self.cross_user_violations), 'timing_violations': len(self.timing_violations), 'content_violations': len(self.content_violations), 'critical_events_received': len([e for e in self.all_events if e.event_type in CRITICAL_EVENTS])}
    ...
    def get_session_summary(self) -> Dict[str, Dict[str, Any]]:
        ...
        with self.lock:
            ...
            for user_id, session in self.sessions.items():
                summary[user_id] = {'events_received': len(session.events), 'event_types': list(session.received_events), 'errors': session.errors, 'connection_duration_s': time.time() - session.connection_start_time if session.connection_start_time else 0, 'last_event_ago_s': time.time() - session.last_event_time if session.last_event_time else None, 'expected_events': list(session.expected_events), 'missing_events': list(session.expected_events - session.received_events)}
            return summary
...
class RealWebSocketConnectionManager:
    ...
    async def create_user_connection(self, user_id: str) -> bool:
        ...
        try:
            ...
            self.connection_tasks[user_id] = asyncio.create_task(self._listen_for_events(user_id, websocket))
        ...
        try:
            async for message in websocket:
                ...
                try:
                    ...
                    self.event_capture.record_event(user_id=user_id, event_type=event_type, payload=payload, thread_id=payload.get('thread_id'), run_id=payload.get('run_id'))
...
class MockAgentSystemForTesting:
    ...
    async def simulate_agent_execution(self, user_id: str, agent_name: str='TestAgent') -> Dict[str, Any]:
        ...
        execution = {'user_id': user_id, 'agent_name': agent_name, 'run_id': run_id, 'thread_id': thread_id, 'start_time': time.time(), 'events_sent': []}
    ...
    async def _send_agent_started(self, user_id: str, agent_name: str, run_id: str, thread_id: str):
        ...
        message = {'type': 'agent_started', 'payload': {'agent_name': agent_name, 'run_id': run_id, 'thread_id': thread_id, 'timestamp': time.time(), 'user_id': user_id}}
    ...
    async def _send_agent_thinking(self, user_id: str, agent_name: str, run_id: str, thread_id: str):
        ...
        message = {'type': 'agent_thinking', 'payload': {'thought': f'Analyzing the request and planning approach...', 'agent_name': agent_name, 'run_id': run_id, 'thread_id': thread_id, 'step_number': 1, 'timestamp': time.time(), 'user_id': user_id}}
        await self.websocket_manager.send_test_message(user_id, message)
    ...
    async def _send_tool_executing(self, user_id: str, agent_name: str, tool_name: str, run_id: str, thread_id: str):
        ...
        message = {'type': 'tool_executing', 'payload': {'tool_name': tool_name, 'agent_name': agent_name, 'run_id': run_id, 'thread_id': thread_id, 'timestamp': time.time(), 'user_id': user_id, 'tool_purpose': 'Analysis and data processing'}}
        await self.websocket_manager.send_test_message(user_id, message)

    async def _send_tool_completed(self, user_id: str, agent_name: str, tool_name: str, result: Dict[str, Any], run_id: str, thread_id: str):
        """Send tool_completed event through WebSocket."""
        message = {'type': 'tool_completed', 'payload': {'tool_name': tool_name, 'agent_name': agent_name, 'result': result, 'run_id': run_id, 'thread_id': thread_id, 'timestamp': time.time(), 'user_id': user_id}}
        await self.websocket_manager.send_test_message(user_id, message)

    async def _send_agent_completed(self, user_id: str, agent_name: str, result: Dict[str, Any], run_id: str, thread_id: str):
        """Send agent_completed event through WebSocket."""
        message = {'type': 'agent_completed', 'payload': {'agent_name': agent_name, 'run_id': run_id, 'thread_id': thread_id, 'result': result, 'timestamp': time.time(), 'duration_ms': 800, 'user_id': user_id}}
        await self.websocket_manager.send_test_message(user_id, message)
...
def websocket_config():
    ...
    if environment == 'staging':
        config = WebSocketTestConfig(websocket_url='wss://netra-backend-staging-701982941522.us-central1.run.app/ws', backend_url='https://netra-backend-staging-701982941522.us-central1.run.app', auth_url='https://netra-auth-service-701982941522.us-central1.run.app', environment='staging', use_ssl=True)
    else:
        config = WebSocketTestConfig(websocket_url=env.get('E2E_WEBSOCKET_URL', 'ws://localhost:8888/ws'), backend_url=env.get('E2E_BACKEND_URL', 'http://localhost:8888'), auth_url=env.get('E2E_AUTH_SERVICE_URL', 'http://localhost:8001'), environment='development', use_ssl=False)
    return config
...
class BasicEventTransmissionTests:
    ...
    async def test_single_user_all_critical_events(self, websocket_manager, mock_agent_system):
        ...
        for critical_event in CRITICAL_EVENTS:
            assert critical_event in received_event_types, f"Critical event '{critical_event}' was not received"
        sequence_violations = websocket_manager.event_capture.validate_event_sequences()
        assert len(sequence_violations) == 0, f'Event sequence violations detected: {sequence_violations}'
        for event in session.events:
            if event.event_type in CRITICAL_EVENTS:
                assert len(event.validation_errors) == 0, f'Event validation errors: {event.validation_errors}'
    ...
    async def test_event_content_structure_validation(self, websocket_manager, mock_agent_system):
        ...
        for event in critical_events:
            ...
            required_fields = REQUIRED_EVENT_FIELDS.get(event_type, [])
            for field in required_fields:
                assert field in payload, f"Event {event_type} missing required field '{field}'. Payload: {payload}"
            if 'timestamp' in payload:
                assert isinstance(payload['timestamp'], (int, float)), f'Timestamp must be numeric in {event_type}'
                assert payload['timestamp'] > 0, f'Timestamp must be positive in {event_type}'
            if 'agent_name' in payload:
                assert payload['agent_name'], f'Agent name cannot be empty in {event_type}'
...
class ConcurrentUserIsolationTests:
    ...
    async def test_concurrent_users_event_isolation(self, websocket_manager, mock_agent_system):
        ...
        connection_tasks = [websocket_manager.create_user_connection(user_id) for user_id in user_ids]
        ...
        execution_tasks = [mock_agent_system.simulate_agent_execution(user_id, f'Agent_{user_id}') for user_id in user_ids]
        executions = await asyncio.gather(*execution_tasks)
        await asyncio.sleep(3)
        for user_id in user_ids:
            session = websocket_manager.event_capture.sessions[user_id]
            for critical_event in CRITICAL_EVENTS:
                assert critical_event in session.received_events, f'User {user_id} missing event {critical_event}'
            for event in session.events:
                if 'user_id' in event.payload:
                    assert event.payload['user_id'] == user_id, f"Cross-user event detected: event for {event.payload['user_id']} delivered to {user_id}"
        violations = websocket_manager.event_capture.cross_user_violations
        assert len(violations) == 0, f'Cross-user violations detected: {violations}'
    ...
    async def test_concurrent_race_condition_prevention(self, websocket_manager, mock_agent_system):
        ...
        for user_id in user_ids:
            ...
            for event in user_specific_events:
                assert event.payload['user_id'] == user_id, f"Race condition caused cross-user event: {event.payload['user_id']} -> {user_id}"
...
class PerformanceAndLoadTests:
    ...
    async def test_high_load_event_delivery_performance(self, websocket_manager, mock_agent_system):
        ...
        assert successful_connections >= num_users * 0.9, f'Too many connection failures: {successful_connections}/{num_users}'
        ...
        all_tasks = []
        for user_id in user_ids[:successful_connections]:
            for i in range(events_per_user):
                task = mock_agent_system.simulate_agent_execution(user_id, f'LoadAgent_{user_id}_{i}')
        ...
        assert metrics['avg_latency_ms'] < MAX_EVENT_DELAY_MS, f"Average latency too high: {metrics['avg_latency_ms']}ms > {MAX_EVENT_DELAY_MS}ms"
        assert metrics['p95_latency_ms'] < MAX_EVENT_DELAY_MS * 2, f"95th percentile latency too high: {metrics['p95_latency_ms']}ms"
        assert metrics['events_per_second'] > 10, f"Event throughput too low: {metrics['events_per_second']} events/sec"
        ...
        assert delivery_rate > 0.95, f'Event delivery rate too low: {delivery_rate:.2%} (expected >95%)'
    ...
    async def test_connection_timeout_and_recovery(self, websocket_manager, mock_agent_system):
        ...
        execution_task = asyncio.create_task(mock_agent_system.simulate_agent_execution(user_id))
...
class ErrorHandlingAndEdgeCasesTests:
    ...
    async def test_malformed_message_handling(self, websocket_manager):
        ...
        await websocket_manager.create_user_connection(user_id)
        malformed_messages = ['{"incomplete": json', '{"type": null, "payload": {}}', '{"payload": {"test": true}}', '{"type": "", "payload": {}}', '{"type": "test", "payload": null}', '{"type": "agent_started", "payload": {"incomplete": true}}']
        ...
        await asyncio.sleep(1)
        test_message = {'type': 'test_message', 'payload': {'test': True}}
    ...
    async def test_event_ordering_under_stress(self, websocket_manager, mock_agent_system):
        ...
        for i in range(num_executions):
            task = mock_agent_system.simulate_agent_execution(user_id, f'OrderingAgent_{i}')
            execution_tasks.append(task)
        ...
        for run_id, events in events_by_run.items():
            ...
            if len(critical_events_in_run) == len(expected_order):
                assert critical_events_in_run == expected_order, f'Event order violation for run {run_id}: {critical_events_in_run} != {expected_order}'
    ...
    async def test_missing_event_detection(self, websocket_manager):
        ...
        partial_events = [{'type': 'agent_started', 'payload': {'agent_name': 'PartialAgent', 'run_id': 'partial_run_001', 'timestamp': time.time(), 'user_id': user_id}}, {'type': 'tool_executing', 'payload': {'tool_name': 'partial_tool', 'agent_name': 'PartialAgent', 'timestamp': time.time(), 'user_id': user_id}}]
        ...
        assert violation['violation_type'] == 'sequence_length_mismatch', f"Expected sequence_length_mismatch, got {violation['violation_type']}"
...
class ComprehensiveIntegrationTests:
    ...
    async def test_comprehensive_websocket_validation(self, websocket_manager, mock_agent_system):
        ...
        assert successful_normal >= len(normal_users) * 0.9, f'Too many normal connection failures: {successful_normal}/{len(normal_users)}'
        ...
        for user_id in normal_users[:successful_normal]:
            for i in range(executions_per_user):
                task = mock_agent_system.simulate_agent_execution(user_id, f'CompAgent_{user_id}_{i}')
        ...
        for user_id in stress_users[:successful_stress]:
            for i in range(executions_per_user * 2):
                task = mock_agent_system.simulate_agent_execution(user_id, f'StressAgent_{user_id}_{i}')
        ...
        assert metrics['avg_latency_ms'] < MAX_EVENT_DELAY_MS, f"Average latency too high: {metrics['avg_latency_ms']}ms"
        ...
        assert delivery_rate > 0.9, f'Event delivery rate too low: {delivery_rate:.1%}'
        assert metrics['cross_user_violations'] == 0, f"Cross-user violations detected: {metrics['cross_user_violations']}"
        for user_id, summary in session_summary.items():
            assert summary['events_received'] > 0, f'User {user_id} received no events'
            critical_received = len([et for et in summary['event_types'] if et in CRITICAL_EVENTS])
            assert critical_received >= len(CRITICAL_EVENTS), f"User {user_id} missing critical events: {summary['event_types']}"
        assert len(websocket_manager.connections) == total_users, 'Connection count mismatch - possible connection leaks'
