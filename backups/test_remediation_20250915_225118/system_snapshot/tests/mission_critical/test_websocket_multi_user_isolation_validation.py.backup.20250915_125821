...
class TestWebSocketMultiUserIsolationValidation(SSotAsyncTestCase):
    ...
    async def _analyze_isolation_security(self):
        ...
        logger.info(f"Multi-User Isolation Security Analysis:")
        ...
        if contamination_incidents > 0:
            logger.critical(f"CRITICAL SECURITY ALERT: {contamination_incidents} cross-contamination incidents")
    ...
    async def test_user_context_isolation_validation(self):
        ...
        try:
            ...
            assert len(isolation_violations) == 0, (
                f"CRITICAL SECURITY FAILURE: User isolation violations detected:\n" +
            ...
            assert len(unique_thread_ids) == len(all_thread_ids), (
                f"SECURITY FAILURE: Thread ID collision detected. "
                f"Expected {len(all_thread_ids)} unique thread IDs, got {len(unique_thread_ids)}. "
                f"Thread IDs: {all_thread_ids}"
            )
            ...
            assert len(unique_run_ids) == len(all_run_ids), (
                f"SECURITY FAILURE: Run ID collision detected. "
                f"Expected {len(all_run_ids)} unique run IDs, got {len(unique_run_ids)}. "
                f"Run IDs: {all_run_ids}"
            )
            ...
            pytest.fail(f"Unexpected exception during user isolation validation: {e}")
    ...
    async def _send_user_events(self, user_test: UserIsolationTest) -> None:
        ...
        for event_data in user_events:
            try:
                ...
                logger.debug(f"User {user_test.user_id} event: {validated_event.event_type}")
                
            except Exception as e:
                logger.error(f"Failed to process event for user {user_test.user_id}: {e}")
                user_test.isolation_violations.append(f"Event processing error: {str(e)}")
        ...
        for event in user_events:
            ...
            if event.thread_id != user_test.thread_id:
                violation = IsolationViolation(
                    ...
                    severity="CRITICAL",
                    timestamp=time.time(),
                    message=f"User {user_test.user_id} received event with wrong thread ID: {event.thread_id} != {user_test.thread_id}"
                )
            ...
            if event.run_id and event.run_id != user_test.run_id:
                violation = IsolationViolation(
                    ...
                    severity="CRITICAL",
                    timestamp=time.time(),
                    message=f"User {user_test.user_id} received event with wrong run ID: {event.run_id} != {user_test.run_id}"
                )
                violations.append(violation)
            ...
            for other_user in all_users:
                ...
                if (other_user.user_id in event_content_str or 
                    ...
                    violation = IsolationViolation(
                        ...
                        severity="CRITICAL",
                        timestamp=time.time(),
                        message=f"User {user_test.user_id} event contains data from user {other_user.user_id}"
                    )
                    violations.append(violation)
        ...
        return [
            {
                ...
                "payload": {
                    ...
                    "run_id": user_test.run_id,
                    "timestamp": base_time,
                    "user_context": f"Processing request for user {user_test.user_id}"
                }
            },
            {
                "type": "agent_thinking",
                ...
                "payload": {
                    ...
                    "run_id": user_test.run_id,
                    "timestamp": base_time + 1,
                    "thought": f"Analyzing request from user {user_test.user_id} with thread {user_test.thread_id}"
                }
            },
            {
                ...
                "payload": {
                    ...
                    "run_id": user_test.run_id,
                    "timestamp": base_time + 2,
                    "result": f"Task completed for user {user_test.user_id}",
        ...
        try:
            ...
            async def process_concurrent_user(user_test: UserIsolationTest):
                try:
                    ...
                    logger.error(f"Concurrent user {user_test.user_id} failed: {e}")
                    user_test.isolation_violations.append(f"Concurrent processing error: {str(e)}")
                    return user_test
            ...
            for result in completed_users:
                if isinstance(result, Exception):
                    logger.error(f"Concurrent user processing exception: {result}")
                    pytest.fail(f"Concurrent user processing failed: {result}")
                else:
            ...
            assert len(isolation_violations) == 0, (
                f"CRITICAL CONCURRENT FAILURE: Isolation violations during concurrent operations:\n" +
                "\n".join([f"  - {v.message}" for v in isolation_violations]) +
            ...
            assert len(successful_users) == len(concurrent_users), (
                f"CONCURRENT FAILURE: Only {len(successful_users)}/{len(concurrent_users)} users completed successfully"
            )
            ...
            assert total_events_received >= total_events_expected * 0.9, (
                ...
                f"Expected at least {int(total_events_expected * 0.9)} events, got {total_events_received}"
            )
            ...
            pytest.fail(f"Unexpected exception during concurrent user validation: {e}")
    ...
    async def test_cross_user_contamination_detection(self):
        ...
        try:
            ...
            for event in user_a.events_received:
                ...
                if (user_b.user_id in event_content or 
                    ...
                    contamination_events.append({
                        ...
                        "contamination": f"Contains user B data: {user_b.user_id}"
                    })
            ...
            for event in user_b.events_received:
                ...
                if (user_a.user_id in event_content or 
                    ...
                    contamination_events.append({
                        "user": user_b.user_id,
                        "event": event.event_type,
                        "contamination": f"Contains user A data: {user_a.user_id}"
                    })
            
            # CRITICAL ASSERTIONS: No contamination should be detected
            assert not contamination_detected, (
                f"CRITICAL CONTAMINATION FAILURE: Cross-user data contamination detected:\n" +
            ...
            assert sequence_a is not None, f"User A sequence not found: {user_a.thread_id}"
            ...
            pytest.fail(f"Unexpected exception during contamination detection: {e}")
