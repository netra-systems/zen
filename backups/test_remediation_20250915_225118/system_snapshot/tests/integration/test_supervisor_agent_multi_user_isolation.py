...
class SupervisorAgentMultiUserIsolationTests(SSotAsyncTestCase):
    ...
    def setUp(self):
        ...
        super().setUp()
        self.enterprise_user_context = UserExecutionContext(user_id='enterprise_user_001', session_id=str(uuid.uuid4()), request_id=str(uuid.uuid4()), websocket_manager=Mock(), execution_metadata={'user_name': 'Enterprise Admin', 'role': 'admin', 'organization': 'Acme Corp', 'data_classification': 'confidential'})
        self.startup_user_context = UserExecutionContext(user_id='startup_user_002', session_id=str(uuid.uuid4()), request_id=str(uuid.uuid4()), websocket_manager=Mock(), execution_metadata={'user_name': 'Startup Founder', 'role': 'owner', 'organization': 'Tech Startup', 'data_classification': 'internal'})
        self.enterprise_events = []
        self.startup_events = []
        self.enterprise_user_context.websocket_manager.send_message = AsyncMock(side_effect=lambda event, data: self.enterprise_events.append({'event': event, 'data': data, 'timestamp': time.time(), 'thread_id': threading.current_thread().ident}))
        self.startup_user_context.websocket_manager.send_message = AsyncMock(side_effect=lambda event, data: self.startup_events.append({'event': event, 'data': data, 'timestamp': time.time(), 'thread_id': threading.current_thread().ident}))
    ...
    def test_websocket_event_contamination_vulnerability(self):
        ...
        for content in enterprise_event_content:
            self.assertNotIn('series a', content, f'VULNERABILITY: Enterprise user received startup-related event: {content}')
            self.assertNotIn('product roadmap', content, f'VULNERABILITY: Enterprise user received startup task data: {content}')
        for content in startup_event_content:
            self.assertNotIn('confidential financial', content, f'VULNERABILITY: Startup user received enterprise-sensitive event: {content}')
            self.assertNotIn('q4 projections', content, f'VULNERABILITY: Startup user received enterprise task data: {content}')
    ...
    def test_concurrent_agent_execution_isolation_vulnerability(self):
        ...
        def execute_agent_workflow(user_id, user_context, task_description):
            ...
            try:
                ...
                execution_results[user_id] = {'initial_user_id': initial_user_id, 'final_user_id': final_user_id, 'user_id_stable': initial_user_id == final_user_id, 'initial_metadata': initial_metadata, 'final_metadata': final_metadata, 'metadata_stable': initial_metadata == final_metadata, 'thread_id': threading.current_thread().ident, 'task': task_description}
            except Exception as e:
                execution_errors.append(f'User {user_id}: {str(e)}')
        user_workflows = [('enterprise_001', self.enterprise_user_context, 'Process HIPAA-compliant patient data'), ('startup_002', self.startup_user_context, 'Analyze competitor pricing strategy'), ('enterprise_003', self.enterprise_user_context, 'Generate SOC2 compliance report'), ('startup_004', self.startup_user_context, 'Review investor term sheet'), ('enterprise_005', self.enterprise_user_context, 'Audit financial transactions')]
        with ThreadPoolExecutor(max_workers=5) as executor:
            ...
            for future in futures:
                future.result()
        self.assertEqual(len(execution_errors), 0, f'Errors during concurrent execution: {execution_errors}')
        context_violations = []
        for user_id, result in execution_results.items():
            if not result['user_id_stable']:
                context_violations.append({'user_id': user_id, 'initial_user_id': result['initial_user_id'], 'final_user_id': result['final_user_id'], 'thread': result['thread_id']})
            if not result['metadata_stable']:
                context_violations.append({'user_id': user_id, 'metadata_change': {'initial': result['initial_metadata'], 'final': result['final_metadata']}})
        self.assertEqual(len(context_violations), 0, f'ISOLATION VULNERABILITY: {len(context_violations)} context violations detected during concurrent execution: {context_violations}')
    ...
    def test_agent_tool_execution_result_contamination(self):
        ...
        enterprise_agent = factory.create_supervisor_agent(self.enterprise_user_context)
        startup_agent = factory.create_supervisor_agent(self.startup_user_context)
        enterprise_tool_result = {'tool': 'financial_analyzer', 'result': 'CONFIDENTIAL: Q4 revenue $2.3M, classified information', 'user_id': 'enterprise_user_001', 'data_classification': 'confidential'}
        startup_tool_result = {'tool': 'market_analyzer', 'result': 'Market analysis: 15% growth potential identified', 'user_id': 'startup_user_002', 'data_classification': 'internal'}
        ...
        for event_str in all_enterprise_events:
            self.assertNotIn('15% growth potential', event_str, f'VULNERABILITY: Enterprise user received startup tool result: {event_str}')
            self.assertNotIn('market_analyzer', event_str, f'VULNERABILITY: Enterprise user received wrong tool result: {event_str}')
        for event_str in all_startup_events:
            self.assertNotIn('CONFIDENTIAL: Q4 revenue', event_str, f'VULNERABILITY: Startup user received confidential enterprise data: {event_str}')
            self.assertNotIn('financial_analyzer', event_str, f'VULNERABILITY: Startup user received enterprise tool result: {event_str}')
    ...
    def test_agent_workflow_state_persistence_vulnerability(self):
        ...
        factory = get_agent_instance_factory()
        enterprise_agent = factory.create_supervisor_agent(self.enterprise_user_context)
        enterprise_workflow_state = {'current_phase': 'financial_analysis', 'completed_steps': ['data_validation', 'compliance_check', 'risk_assessment'], 'pending_approvals': ['cfo_approval', 'board_review'], 'confidential_findings': ['revenue_decline_q3', 'cost_overrun_detected'], 'classification_level': 'top_secret'}
        ...
        for key, value in enterprise_workflow_state.items():
            if key in startup_workflow_state:
                contamination_found.append({'key': key, 'enterprise_value': value, 'startup_value': startup_workflow_state[key]})
        ...
        self.assertEqual(len(contamination_found), 0, f'WORKFLOW STATE VULNERABILITY: Enterprise state contaminated startup agent: {contamination_found}')
        self.assertEqual(len(leaked_terms), 0, f'CONFIDENTIAL DATA LEAK: Enterprise confidential terms found in startup agent: {leaked_terms} in {startup_state_str}')
