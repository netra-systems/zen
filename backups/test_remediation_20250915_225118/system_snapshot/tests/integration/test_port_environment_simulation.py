...
class PortEnvironmentSimulationTests(BaseTestCase):
    ...
    def setup_method(self, method=None):
        ...
        self.test_ports = [8080, 8090, 9000, 9090]
    ...
    def test_cloud_run_port_assignment_simulation(self):
        ...
        for test_port in self.test_ports:
            with patch.dict(os.environ, {'PORT': str(test_port)}):
                ...
                self.assertEqual(gcp_effective_port, test_port, f'GCP Dockerfile should use PORT={test_port} from environment')
                ...
                self.assertEqual(alpine_effective_port, test_port, f'Alpine Dockerfile should use PORT={test_port} from environment, but currently ignores it and uses hardcoded port 8000')
        ...
        for test_port in self.test_ports:
            with patch.dict(os.environ, {'PORT': str(test_port)}):
                ...
                if health_check_config:
                    effective_health_port = self._simulate_health_check_execution(health_check_config, test_port)
                    self.assertEqual(effective_health_port, test_port, f'Health check should use PORT={test_port} from environment, but currently uses hardcoded localhost:8000')
    ...
    def test_multiple_container_port_conflict_simulation(self):
        ...
        """
        backend_alpine_dockerfile = self.project_root / 'dockerfiles' / 'backend.alpine.Dockerfile'
        container_configs = [{'instance': 'backend-1', 'port': 8080}, {'instance': 'backend-2', 'port': 8090}, {'instance': 'backend-3', 'port': 9000}]
        used_ports = set()
        conflicts = []
        for config in container_configs:
            with patch.dict(os.environ, {'PORT': str(config['port'])}):
                ...
                if effective_port in used_ports:
                    conflicts.append(f"Port conflict: {config['instance']} tried to use port {effective_port} (already used by another instance)")
                used_ports.add(effective_port)
        self.assertEqual(len(conflicts), 0, f'Alpine Dockerfiles cause port conflicts in multi-container environments:\n' + '\n'.join(conflicts))
    ...
    def test_environment_variable_precedence_simulation(self):
        ...
        backend_alpine_dockerfile = self.project_root / 'dockerfiles' / 'backend.alpine.Dockerfile'
        backend_gcp_dockerfile = self.project_root / 'deployment' / 'docker' / 'backend.gcp.Dockerfile'
        test_scenarios = [{'PORT': '8080', 'expected': 8080}, {'PORT': '9000', 'expected': 9000}, {None: None, 'expected': 8000}]
        for scenario in test_scenarios:
            ...
            with patch.dict(os.environ, env_patch, clear=False):
                ...
                self.assertEqual(gcp_port, scenario['expected'], f'GCP Dockerfile should respect PORT environment variable precedence')
                alpine_command = self._extract_command_from_dockerfile(backend_alpine_dockerfile)
                alpine_port = self._simulate_command_execution(alpine_command, scenario['expected'])
                self.assertEqual(alpine_port, scenario['expected'], f"Alpine Dockerfile should respect PORT environment variable precedence. Expected {scenario['expected']}, but Alpine ignores PORT and uses hardcoded 8000")
    ...
    def _extract_command_from_dockerfile(self, dockerfile_path: Path) -> str:
        ...
        with open(dockerfile_path, 'r', encoding='utf-8') as f:
            content = f.read()
        import re
        cmd_match = re.search('CMD\\s*\\[(.*?)\\]', content, re.DOTALL)
        if cmd_match:
            return cmd_match.group(1)
        cmd_match = re.search('CMD\\s+(.+)', content, re.MULTILINE)
        if cmd_match:
            return cmd_match.group(1)
        return ''
    ...
    def _extract_health_check_from_dockerfile(self, dockerfile_path: Path) -> str:
        ...
        with open(dockerfile_path, 'r', encoding='utf-8') as f:
            content = f.read()
        import re
        health_match = re.search('HEALTHCHECK.*CMD\\s+(.+)', content, re.MULTILINE)
        if health_match:
    ...
    def _simulate_command_execution(self, command: str, expected_port: int) -> int:
        ...
        import re
        bind_match = re.search('--bind\\s+[^:]+:(\\d+)', command)
        if bind_match:
            return int(bind_match.group(1))
        port_match = re.search('--port\\s+(\\d+)', command)
        if port_match:
    ...
    def _simulate_health_check_execution(self, health_check: str, expected_port: int) -> int:
        ...
        if '${PORT' in health_check:
            return expected_port
        import re
        port_match = re.search('localhost:(\\d+)', health_check)
        if port_match:
            return int(port_match.group(1))
        return expected_port
