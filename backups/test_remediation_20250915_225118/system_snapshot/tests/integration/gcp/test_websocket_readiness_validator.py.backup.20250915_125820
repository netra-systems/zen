...
from netra_backend.app.websocket_core.gcp_initialization_validator import GCPWebSocketInitializationValidator, create_gcp_websocket_validator, GCPReadinessState, GCPReadinessResult, ServiceReadinessCheck
...
def mock_app_state_redis_unavailable():
    ...
    mock_state.startup_phase = 'cache'
    return mock_state
...
class TestWebSocketReadinessValidatorGCPRedis:
    ...
    async def test_validator_detects_redis_failure_in_gcp_staging(self, gcp_validator_staging):
        """
        ...
        result = await gcp_validator_staging.validate_gcp_readiness_for_websocket(timeout_seconds=120.0)
        ...
        assert not result.ready, f'Validation should fail when Redis unavailable, but ready={result.ready}'
        assert 'redis' in result.failed_services, f'Redis should be in failed services list, got: {result.failed_services}'
        assert result.state == GCPReadinessState.FAILED, f'Expected FAILED state when Redis unavailable, got: {result.state.value}'
        assert elapsed_time > 0.1, f'Validation completed too quickly ({elapsed_time:.3f}s) - may not be testing properly'
        assert gcp_validator_staging.is_gcp_environment, 'Validator should detect GCP environment'
        logger.info(' PASS:  PASS: Validator correctly detected Redis failure')

    @pytest.mark.integration
    @pytest.mark.gcp
    @pytest.mark.critical
    async def test_validator_passes_when_redis_working(self, gcp_validator_working):
        """
        ...
        start_time = time.time()
        result = await gcp_validator_working.validate_gcp_readiness_for_websocket(timeout_seconds=120.0)
        elapsed_time = time.time() - start_time
        logger.info(f'Validation completed in {elapsed_time:.2f}s')
        logger.info(f'  Ready: {result.ready}')
        logger.info(f'  State: {result.state.value}')
        logger.info(f'  Failed Services: {result.failed_services}')
        assert result.ready, f'Validation should succeed when Redis available, but ready={result.ready}'
        assert len(result.failed_services) == 0, f'No services should fail when Redis working, got: {result.failed_services}'
        assert result.state == GCPReadinessState.WEBSOCKET_READY, f'Expected WEBSOCKET_READY state when Redis working, got: {result.state.value}'
        assert elapsed_time > 0.1, f'Validation completed too quickly ({elapsed_time:.3f}s) - may not be testing properly'
    ...
    @pytest.mark.gcp
    @pytest.mark.critical
    async def test_gcp_timeout_configuration_for_redis(self, gcp_validator_staging):
        """
        ...
        assert gcp_validator_staging.is_gcp_environment, 'Validator should detect GCP environment for timeout configuration'
        redis_check = gcp_validator_staging.readiness_checks.get('redis')
        assert redis_check is not None, 'Redis readiness check should be configured'
        assert redis_check.timeout_seconds == 60.0, f'GCP Redis timeout should be 60.0s, got: {redis_check.timeout_seconds}s'
        assert redis_check.retry_count == 5, f'GCP Redis retry count should be 5, got: {redis_check.retry_count}'
        assert redis_check.retry_delay == 1.5, f'GCP Redis retry delay should be 1.5s, got: {redis_check.retry_delay}s'
        assert redis_check.is_critical, 'Redis readiness check should be marked as critical'
    ...
    @pytest.mark.gcp
    @pytest.mark.critical
    async def test_startup_phase_progression_with_redis_failure(self, gcp_validator_staging):
        """
        ...
        try:
            result = await gcp_validator_staging.validate_gcp_readiness_for_websocket(timeout_seconds=60.0)
            ...
            assert dependency_phase_reached, 'Phase 1 (Dependencies) should be reached and logged'
            assert not result.ready, 'Validation should fail when Redis unavailable in dependencies'
            assert 'redis' in result.failed_services, f'Redis should cause dependency failure, got: {result.failed_services}'
            if services_phase_reached or websocket_phase_reached:
                assert result.state == GCPReadinessState.FAILED, f'If later phases reached, final state should be FAILED, got: {result.state.value}'
    ...
    @pytest.mark.gcp
    @pytest.mark.critical
    async def test_redis_grace_period_behavior_in_gcp(self, mock_app_state_redis_available):
        """
        ...
        if redis_result:
            assert elapsed_time >= 0.4, f'Expected at least 500ms for grace period, got {elapsed_time:.3f}s'
            assert elapsed_time <= 1.0, f'Grace period too long, expected ~500ms, got {elapsed_time:.3f}s'
    ...
    async def test_validator_timing_consistency_multiple_runs(self, gcp_validator_staging):
        """
        ...
        for run in range(3):
            ...
            start_time = time.time()
            result = await gcp_validator_staging.validate_gcp_readiness_for_websocket(timeout_seconds=30.0)
            elapsed_time = time.time() - start_time
        ...
        assert len(set(ready_states)) == 1, f'Validation results should be consistent, got: {ready_states}'
        assert timing_spread <= 2.0, f'Timing spread too large: {timing_spread:.3f}s indicates inconsistent behavior'
        assert all((t > 0.1 for t in timing_results)), f'Some validations completed too quickly: {timing_results}'
        logger.info(' PASS:  PASS: Validator timing is consistent across multiple runs')

    @pytest.mark.integration
    @pytest.mark.gcp
    @pytest.mark.error_handling
    async def test_validator_error_handling_and_logging(self, gcp_validator_staging):
        """
        ...
        try:
            ...
            assert len(info_messages) > 0, 'Validator should log informational messages about validation progress'
            if not result.ready:
                assert len(error_messages) > 0, 'Validator should log error messages when validation fails'
            if not result.ready:
                assert len(result.failed_services) > 0, 'Failed validation should identify which services failed'
                assert 'redis' in result.failed_services, f'Redis should be identified as failing service: {result.failed_services}'
            assert isinstance(result.details, dict), 'Result details should be a dictionary with debugging information'
            assert 'environment' in result.details, 'Result details should include environment information'
            logger.info(' PASS:  PASS: Validator error handling and logging is comprehensive')
        finally:
            gcp_validator_staging.logger = original_logger
INTEGRATION_TEST_METADATA = {'suite_name': 'WebSocket Readiness Validator GCP Redis Integration', 'test_layer': 'Integration', 'dependencies': ['GCP WebSocket Initialization Validator', 'Redis connectivity'], 'business_impact': 'Prevents WebSocket 1011 errors by detecting Redis failures', 'test_strategy': {'real_services': 'Controlled Redis availability testing', 'mocking': 'Only app_state components, not Redis operations', 'timing': 'Validates GCP-specific timeout configurations'}, 'expected_behavior': {'redis_unavailable': 'Validation fails with Redis in failed_services', 'redis_available': 'Validation succeeds with WEBSOCKET_READY state', 'timing': 'Consistent results with GCP timeout configuration'}}
