...
class MockUser:
    ...
    def create_test_user(cls, user_id: Optional[str]=None) -> 'MockUser':
        ...
        return cls(id=user_id, email=f'{user_id}@test.com', session_id=f'session_{uuid.uuid4().hex[:8]}')
...
class MockWebSocketEvent:
    ...
    def create_agent_event(cls, event_type: str, user_id: str, session_id: str, **data) -> 'MockWebSocketEvent':
        ...
        import time
        return cls(event_type=event_type, user_id=user_id, session_id=session_id, data=data, timestamp=time.time())
...
class SsotToolDispatcherConsolidationValidationTests(SSotAsyncTestCase):
    ...
    async def test_consolidated_dispatcher_api_compatibility(self):
        ...
        mock_tool_result = {'tool_name': 'test_tool', 'result': 'test_result', 'execution_time': 0.1, 'user_id': test_user.id}
        try:
            with patch('netra_backend.app.tools.enhanced_dispatcher.get_websocket_manager') as mock_get_ws:
                ...
                dispatcher = RequestScopedToolDispatcher(user_id=test_user.id, session_id=test_user.session_id, websocket_manager=self.mock_websocket_manager)
            ...
            with patch('netra_backend.app.tools.enhanced_dispatcher.get_websocket_manager') as mock_get_ws:
                ...
                with patch.object(dispatcher, '_execute_tool_internal') as mock_execute:
                    ...
                    for event_type in expected_events:
                        await self.mock_websocket_manager.send_agent_event(event_type=event_type, user_id=test_user.id, session_id=test_user.session_id, tool_name='test_tool', status='running' if event_type != 'agent_completed' else 'completed')
    ...
    async def test_user_isolation_maintained_post_consolidation(self):
        ...
        try:
            with patch('netra_backend.app.tools.enhanced_dispatcher.get_websocket_manager') as mock_get_ws:
                mock_get_ws.return_value = self.mock_websocket_manager
                dispatcher_a = RequestScopedToolDispatcher(user_id=user_a.id, session_id=user_a.session_id, websocket_manager=self.mock_websocket_manager)
                dispatcher_b = RequestScopedToolDispatcher(user_id=user_b.id, session_id=user_b.session_id, websocket_manager=self.mock_websocket_manager)
                assert dispatcher_a.user_id != dispatcher_b.user_id
                assert dispatcher_a.session_id != dispatcher_b.session_id
                assert id(dispatcher_a) != id(dispatcher_b)
                await self.mock_websocket_manager.send_agent_event(event_type='tool_executing', user_id=user_a.id, session_id=user_a.session_id, tool_name='user_a_tool')
                await self.mock_websocket_manager.send_agent_event(event_type='tool_executing', user_id=user_b.id, session_id=user_b.session_id, tool_name='user_b_tool')
    ...
    async def test_memory_usage_improvement_indicators(self):
        ...
        """
        expected_improvements = {'duplicate_classes_eliminated': True, 'shared_state_reduced': True, 'factory_instances_consolidated': True}
