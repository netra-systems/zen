...
class TestIdFormatConsistencyValidation(SSotBaseTestCase):
    ...
    def test_session_id_format_consistency_post_migration(self):
        ...
        # Test multiple session ID generations for consistency
        test_cases = [
            ("user_123", "req_456"),
            ("test_user_789", "test_request_012"),
            ("admin_user", "admin_req_345"),
            ("guest_678", "guest_request_901")
        ]

        session_pattern = re.compile(r'^sess_[a-zA-Z0-9_]+_[a-zA-Z0-9_]+_\d+_[a-f0-9]{8}$')
        ...
        for user_id, request_id in test_cases:
            ...
            assert session_pattern.match(session_id), (
                f"SESSION FORMAT INCONSISTENCY: '{session_id}' doesn't match SSOT pattern. "
                f"Expected: sess_[user]_[request]_[timestamp]_[8-char-hex]"
            )
            ...
            # Validate timestamp component (should be recent Unix timestamp)
            timestamp_found = False
            for part in parts:
                if part.isdigit() and len(part) >= 10:  # Unix timestamp
                    timestamp = int(part)
                    current_time = int(time.time())
                    assert abs(current_time - timestamp) < 300, (  # Within 5 minutes
                        f"SESSION TIMESTAMP ERROR: Timestamp {timestamp} too old/future"
                    )
        ...
        # Validate uniqueness across all generated IDs
        session_id_strings = [sid for sid, _, _ in generated_session_ids]
        unique_session_ids = set(session_id_strings)

        assert len(unique_session_ids) == len(session_id_strings), (
            f"SESSION UNIQUENESS FAILURE: Generated {len(unique_session_ids)}/{len(session_id_strings)} unique IDs"
        )
    ...
    def test_connection_id_format_consistency_post_migration(self):
        ...
        # Generate session IDs first, then connection IDs
        test_cases = [
        ...
        connection_pattern = re.compile(r'^conn_[a-zA-Z0-9_]+_[a-zA-Z0-9_]+_\d+_[a-f0-9]{8}$')
        ...
        for user_id, session_id in test_cases:
            ...
            assert connection_pattern.match(connection_id), (
                f"CONNECTION FORMAT INCONSISTENCY: '{connection_id}' doesn't match SSOT pattern. "
                f"Expected: conn_[user]_[session]_[timestamp]_[8-char-hex]"
            )
            ...
            for part in parts:
                if part.isdigit() and len(part) >= 10:  # Unix timestamp
                    ...
                    current_time = int(time.time())
                    assert abs(current_time - timestamp) < 300, (  # Within 5 minutes
                        f"CONNECTION TIMESTAMP ERROR: Timestamp {timestamp} too old/future"
                    )
                    timestamp_found = True
                    break
        ...
        assert len(unique_connection_ids) == len(connection_id_strings), (
            f"CONNECTION UNIQUENESS FAILURE: Generated {len(unique_connection_ids)}/{len(connection_id_strings)} unique IDs"
        )
    ...
    def test_client_id_format_consistency_post_migration(self):
        ...
        # Test client IDs for different services
        test_cases = [
            ("redis", "client_user_123", "client_req_456"),
            ("clickhouse", "analytics_user_789", "analytics_req_012"),
            ("redis", "cache_user", "cache_request_345"),
            ("clickhouse", "metrics_678", "metrics_request_901")
        ]
        ...
        for service_type, user_id, request_id in test_cases:
            ...
            assert client_pattern.match(client_id), (
                f"CLIENT FORMAT INCONSISTENCY: '{client_id}' doesn't match SSOT pattern. "
                f"Expected: client_[service]_[user]_[request]_[timestamp]_[8-char-hex]"
            )
            ...
            for part in parts:
                if part.isdigit() and len(part) >= 10:  # Unix timestamp
                    ...
                    current_time = int(time.time())
                    assert abs(current_time - timestamp) < 300, (  # Within 5 minutes
                        f"CLIENT TIMESTAMP ERROR: Timestamp {timestamp} too old/future"
                    )
                    timestamp_found = True
                    break
        ...
        assert len(unique_client_ids) == len(client_id_strings), (
            f"CLIENT UNIQUENESS FAILURE: Generated {len(unique_client_ids)}/{len(client_id_strings)} unique IDs"
        )
    ...
    def test_audit_id_format_consistency_post_migration(self):
        ...
        # Test audit IDs for different record types
        test_cases = [
            ("corpus", "audit_user_123", "audit_resource_456"),
            ("access", "compliance_user_789", "access_resource_012"),
            ("security", "admin_user", "security_resource_345"),
            ("performance", "metrics_678", "perf_resource_901")
        ]

        audit_pattern = re.compile(r'^audit_[a-z]+_[a-zA-Z0-9_]+_\d+_[a-f0-9]{8}$')
        generated_audit_ids = []
        ...
        for record_type, user_id, resource_id in test_cases:
            ...
            assert audit_pattern.match(audit_id), (
                f"AUDIT FORMAT INCONSISTENCY: '{audit_id}' doesn't match SSOT pattern. "
                f"Expected: audit_[type]_[user]_[timestamp]_[8-char-hex]"
            )
            ...
            assert parts[1] in ['corpus', 'access', 'security', 'performance'], (
                f"AUDIT TYPE ERROR: Invalid record type '{parts[1]}'"
            )
            ...
            for part in parts:
                if part.isdigit() and len(part) >= 10:  # Unix timestamp
                    ...
                    current_time = int(time.time())
                    assert abs(current_time - timestamp) < 300, (  # Within 5 minutes
                        f"AUDIT TIMESTAMP ERROR: Timestamp {timestamp} too old/future"
                    )
                    timestamp_found = True
                    break
        ...
        assert len(unique_audit_ids) == len(audit_id_strings), (
            f"AUDIT UNIQUENESS FAILURE: Generated {len(unique_audit_ids)}/{len(audit_id_strings)} unique IDs"
        )
    ...
    def test_cross_format_compatibility_validation(self):
        ...
        # Generate one ID of each type for the same user
        ...
        audit_id = unified_generator.generate_audit_id(
            ...
            user_id=test_user_id,
            resource_id="compatibility_resource"
        )
        ...
        # Test cross-format compatibility features

        # 1. User extraction compatibility
        def extract_user_info(id_string: str) -> Tuple[str, str]:
            parts = id_string.split('_')
            if len(parts) >= 2:
                return parts[0], parts[1]  # prefix, user_part
        ...
        for id_type, id_string in all_ids.items():
            ...
            assert prefix == expected_prefixes[id_type], (
                f"PREFIX INCOMPATIBILITY: {id_type} has prefix '{prefix}', expected '{expected_prefixes[id_type]}'"
            )

        # 2. Timestamp extraction compatibility
        ...
        assert timestamp_range <= 10, (
            f"TIMESTAMP CORRELATION FAILURE: {timestamp_range}s range exceeds compatibility limit"
        )

        # 3. Random component compatibility (8-char hex)
        def extract_random_component(id_string: str) -> str:
            parts = id_string.split('_')
            # Last component should be 8-char hex
            if parts and re.match(r'^[a-f0-9]{8}$', parts[-1]):
                return parts[-1]
            return ""
        ...
        for id_type, id_string in all_ids.items():
            ...
            assert len(random_comp) == 8, (
                f"RANDOM COMPONENT INCOMPATIBILITY: {id_type} random component '{random_comp}' not 8 chars"
            )

            assert re.match(r'^[a-f0-9]{8}$', random_comp), (
                f"RANDOM COMPONENT FORMAT: {id_type} component '{random_comp}' not valid hex"
            )
    ...
    def test_format_migration_regression_prevention(self):
        ...
        # Test format stability over multiple generations
        stability_test_cases = [
            ("stability_user_1", "stability_req_1"),
            ("stability_user_2", "stability_req_2"),
            ("stability_user_3", "stability_req_3")
        ]

        format_patterns = {
            'session': re.compile(r'^sess_[a-zA-Z0-9_]+_[a-zA-Z0-9_]+_\d+_[a-f0-9]{8}$'),
            'connection': re.compile(r'^conn_[a-zA-Z0-9_]+_[a-zA-Z0-9_]+_\d+_[a-f0-9]{8}$'),
            'client': re.compile(r'^client_[a-z]+_[a-zA-Z0-9_]+_[a-zA-Z0-9_]+_\d+_[a-f0-9]{8}$'),
            'audit': re.compile(r'^audit_[a-z]+_[a-zA-Z0-9_]+_\d+_[a-f0-9]{8}$')
        }

        # Generate multiple IDs and validate consistent patterns
        all_generated_ids = []

        for user_id, request_id in stability_test_cases:
            # Generate all ID types for each test case
            session_id = unified_generator.generate_session_id(
                user_id=user_id,
                request_id=request_id
            )
            ...
            audit_id = unified_generator.generate_audit_id(
                ...
                user_id=user_id,
                resource_id=f"resource_{request_id}"
            )

            test_ids = {
                'session': session_id,
                'connection': connection_id,
                'client': redis_client_id,
                'audit': audit_id
            }

            all_generated_ids.append(test_ids)

            # Validate each ID against its expected pattern
            for id_type, id_string in test_ids.items():
                pattern = format_patterns[id_type]
                assert pattern.match(id_string), (
                    f"FORMAT REGRESSION: {id_type} ID '{id_string}' doesn't match stable pattern"
                )

        # Validate stability across all generations
        all_ids_flat = []
        ...
        assert len(unique_ids) == len(all_ids_flat), (
            f"STABILITY REGRESSION: ID collisions detected. "
            f"Generated {len(all_ids_flat)} IDs, {len(unique_ids)} unique"
        )

        # Validate consistent component counts
        component_counts = {}
        ...
        # Each prefix should have consistent component counts
        for prefix, counts in component_counts.items():
            assert len(counts) == 1, (
                f"COMPONENT COUNT REGRESSION: {prefix} has inconsistent component counts: {counts}"
            )
