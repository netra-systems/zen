...
class Issue845RegistryDuplicationTests(SSotAsyncTestCase):
    ...
    async def asyncTearDown(self):
        ...
        modules_to_clear = ['netra_backend.app.agents.registry', 'netra_backend.app.agents.supervisor.agent_registry']
    ...
    async def test_duplicate_registry_imports_exist(self):
        ...
        try:
            ...
            self.assertNotEqual(BasicRegistry, AdvancedRegistry, 'SSOT VIOLATION: Two different AgentRegistry classes exist')
            self.assertNotEqual(basic_registry_file, advanced_registry_file, 'SSOT VIOLATION: Registries exist in different modules')
            ...
            self.assertGreater(len(method_differences), 10, f'Advanced registry should have significantly more methods. Found differences: {method_differences}')
    ...
    async def test_agent_creation_pattern_differences(self):
        ...
        user_context = UserExecutionContext(user_id='test_user_845', request_id='test_request_845', thread_id='test_thread_845', run_id='test_run_845')
        basic_registry = BasicRegistry()
        basic_agent_types = basic_registry.get_all_agents()
        import inspect
    ...
    async def test_import_resolution_conflicts(self):
        ...
        if basic_registry_class and advanced_registry_class:
            self.assertNotEqual(basic_registry_class, advanced_registry_class, 'CRITICAL: Same import name gives different classes')
            import_scenarios.append({'import_path': 'netra_backend.app.agents.registry', 'class': basic_registry_class, 'capabilities': 'basic'})
            import_scenarios.append({'import_path': 'netra_backend.app.agents.supervisor.agent_registry', 'class': advanced_registry_class, 'capabilities': 'advanced'})
        self.assertEqual(len(import_scenarios), 2, 'SSOT VIOLATION: Multiple import paths for AgentRegistry')
    ...
    async def test_websocket_event_delivery_failure_demonstration(self):
        ...
        basic_registry.register_agent(agent_type=AgentType.TRIAGE, name='Test Agent', description='Test agent for WebSocket events', agent_instance=test_agent)
        ...
        user_context = UserExecutionContext(user_id='test_user_845_advanced', request_id='test_request_845_advanced', thread_id='test_thread_845_advanced', run_id='test_run_845_advanced')
        mock_llm_manager = Mock()
        advanced_registry = AdvancedRegistry(llm_manager=mock_llm_manager)
        advanced_registry.set_websocket_manager(mock_websocket_manager)
        advanced_supports_user_events = hasattr(advanced_registry, 'get_user_session')
        self.assertTrue(advanced_supports_user_events, 'Advanced registry should support user-isolated events')
