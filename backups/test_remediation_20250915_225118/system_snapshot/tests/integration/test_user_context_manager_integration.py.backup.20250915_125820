...
try:
    ...
    IMPORT_ERROR = str(e)
from netra_backend.app.services.user_execution_context import UserExecutionContext, InvalidContextError, ContextIsolationError, create_isolated_execution_context, managed_user_context, validate_user_context
...
class TestUserContextManagerSSotIntegration(SSotBaseTestCase):
    ...
    def test_ssot_factory_integration(self):
        ...
        with patch('netra_backend.app.services.user_execution_context.create_isolated_execution_context') as mock_factory:
            mock_context = UserExecutionContext.from_request(user_id=self.user_id, thread_id=self.thread_id, run_id=self.run_id)
            mock_factory.return_value = mock_context
            created_context = manager.create_isolated_context(user_id=self.user_id, request_id='ssot_request_123')
    ...
    def test_isolated_environment_integration(self):
        ...
        with patch('shared.isolated_environment.IsolatedEnvironment') as mock_env:
            ...
            context = UserExecutionContext.from_request(user_id=self.user_id, thread_id=self.thread_id, run_id=self.run_id)
    ...
    def test_audit_trail_ssot_compliance(self):
        ...
        required_fields = ['context_set_at', 'user_id', 'context_source', 'isolation_verified', 'compliance_version']
...
class TestUserContextManagerWebSocketIntegration(SSotBaseTestCase):
    ...
    async def test_websocket_manager_integration(self):
        ...
        with patch('netra_backend.app.websocket_core.manager.WebSocketManager') as mock_ws_manager:
            ...
            mock_ws_manager.get_connection.side_effect = lambda user_id: {self.user_a: mock_connection_a, self.user_b: mock_connection_b}.get(user_id)
            context_a = UserExecutionContext.from_request(user_id=self.user_a, thread_id='ws_thread_a', run_id='ws_run_a', websocket_client_id='ws_client_a')
            context_b = UserExecutionContext.from_request(user_id=self.user_b, thread_id='ws_thread_b', run_id='ws_run_b', websocket_client_id='ws_client_b')
    ...
    async def test_websocket_event_isolation(self):
        ...
        manager = UserContextManager()
        context_a = UserExecutionContext.from_request(user_id=self.user_a, thread_id='event_thread_a', run_id='event_run_a', websocket_client_id='event_ws_a')
        context_b = UserExecutionContext.from_request(user_id=self.user_b, thread_id='event_thread_b', run_id='event_run_b', websocket_client_id='event_ws_b')
        manager.set_context(self.user_a, context_a)
        manager.set_context(self.user_b, context_b)
...
class TestUserContextManagerAgentExecution(SSotBaseTestCase):
    ...
    async def test_agent_registry_integration(self):
        ...
        with patch('netra_backend.app.agents.supervisor.agent_registry.UserAgentSession') as mock_registry:
            ...
            context = UserExecutionContext.from_request(user_id=self.user_id, thread_id='agent_thread', run_id='agent_run')
            manager.set_context(self.user_id, context)
            await manager.execute_with_agent(user_id=self.user_id, agent_name='test_agent', parameters={'test': 'data'})
    ...
    async def test_multi_agent_isolation(self):
        ...
        async def run_agent(user_id: str, agent_name: str, data: dict):
            ...
            try:
                context = UserExecutionContext.from_request(user_id=user_id, thread_id=f'agent_{agent_name}_thread', run_id=f'agent_{agent_name}_run', agent_context={'agent_data': data})
...
class TestUserContextManagerDatabaseIntegration(SSotBaseTestCase):
    ...
    async def test_database_session_management(self):
        ...
        context_a = UserExecutionContext.from_request(user_id='db_user_a', thread_id='db_thread_a', run_id='db_run_a', db_session=mock_session_a)
        context_b = UserExecutionContext.from_request(user_id='db_user_b', thread_id='db_thread_b', run_id='db_run_b', db_session=mock_session_b)
    ...
    async def test_transaction_isolation(self):
        ...
        with patch('netra_backend.app.db.database_manager.DatabaseManager') as mock_db_manager:
            ...
            self.assertIsNot(context_a.db_session, context_b.db_session)
...
class TestUserContextManagerPerformance(SSotBaseTestCase):
    ...
    def test_memory_usage_bounds(self):
        ...
        for user_id in user_ids:
            context = UserExecutionContext.from_request(user_id=user_id, thread_id=f'perf_thread_{user_id}', run_id=f'perf_run_{user_id}')
            manager.set_context(user_id, context)
        self.assertEqual(len(manager.get_active_contexts()), initial_contexts + num_contexts)
    ...
    async def test_concurrent_performance(self):
        ...
        async def concurrent_operation(user_id: str, operation_id: int):
            ...
            try:
                context = UserExecutionContext.from_request(user_id=f'{user_id}_{operation_id}', thread_id=f'concurrent_thread_{operation_id}', run_id=f'concurrent_run_{operation_id}')
    ...
    def test_resource_cleanup_effectiveness(self):
        ...
        for i in range(10):
            ...
            context = UserExecutionContext.from_request(user_id=f'cleanup_user_{i}', thread_id=f'cleanup_thread_{i}', run_id=f'cleanup_run_{i}', db_session=mock_session, websocket_client_id=f'ws_{i}')
...
class TestUserContextManagerErrorHandling(SSotBaseTestCase):
    ...
    def test_graceful_degradation(self):
        ...
        with patch.object(UserExecutionContext, 'verify_isolation', side_effect=ContextIsolationError('Test error')):
            context = UserExecutionContext.from_request(user_id='error_user', thread_id='error_thread', run_id='error_run')
    ...
    def test_error_isolation(self):
        ...
        manager = UserContextManager()
        good_context = UserExecutionContext.from_request(user_id='good_user', thread_id='good_thread', run_id='good_run')
        ...
        with patch.object(UserExecutionContext, '__post_init__', side_effect=Exception('Initialization error')):
            with self.assertRaises(Exception):
                bad_context = UserExecutionContext.from_request(user_id='bad_user', thread_id='bad_thread', run_id='bad_run')
    ...
    async def test_cleanup_error_handling(self):
        ...
        context = UserExecutionContext.from_request(user_id='cleanup_error_user', thread_id='cleanup_error_thread', run_id='cleanup_error_run', db_session=mock_session)
