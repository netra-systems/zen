...
class ThreadStateTracker:
    ...
    def record_session(self, session_id: str, user_id: str, thread_id: str, session_data: Dict[str, Any]):
        """Record session metadata and state"""
        with self._lock:
            self.sessions[session_id] = {'user_id': user_id, 'thread_id': thread_id, 'created_at': datetime.now().isoformat(), 'session_data': session_data.copy()}

    def record_thread_state(self, thread_id: str, session_id: str, state: Dict[str, Any]):
        """Record thread state at specific session"""
        with self._lock:
            state_record = {'session_id': session_id, 'timestamp': datetime.now().isoformat(), 'state': state.copy()}
    ...
    def record_message(self, thread_id: str, session_id: str, message_data: Dict[str, Any]):
        ...
        with self._lock:
            message_record = {'session_id': session_id, 'timestamp': datetime.now().isoformat(), 'message': message_data.copy()}
            self.messages_by_thread[thread_id].append(message_record)

    def record_cross_session_event(self, event_type: str, thread_id: str, from_session: str, to_session: str, data: Dict[str, Any]):
        """Record cross-session continuity events"""
        with self._lock:
            event = {'event_type': event_type, 'thread_id': thread_id, 'from_session': from_session, 'to_session': to_session, 'timestamp': datetime.now().isoformat(), 'data': data.copy()}
    ...
    def get_thread_continuity_analysis(self, thread_id: str) -> Dict[str, Any]:
        ...
        with self._lock:
            ...
            return {'thread_id': thread_id, 'sessions_count': len(sessions_involved), 'sessions_involved': list(sessions_involved), 'total_messages': len(thread_messages), 'state_transitions': len(thread_states), 'messages_timeline': thread_messages.copy(), 'states_timeline': thread_states.copy(), 'cross_session_events': [e for e in self.cross_session_events if e['thread_id'] == thread_id]}
...
class TestThreadContinuityMultiSession(DockerTestBase):
    ...
    async def _create_test_user_and_authenticate(self) -> str:
        ...
        register_payload = {'email': f'{self.user_id}@test.netra.com', 'password': 'TestPassword123!', 'full_name': f'Thread Continuity Test User {self.user_id[:8]}'}
        response = requests.post(f'{self.auth_url}/auth/register', json=register_payload, timeout=30)
        ...
        login_response = requests.post(f'{self.auth_url}/auth/login', json=login_payload, timeout=30)
    ...
    async def _create_conversation_thread(self, session_id: str) -> Dict[str, Any]:
        ...
        thread_payload = {'user_id': self.user_id, 'session_id': session_id, 'thread_id': self.thread_id, 'title': f'Multi-Session Thread Test - {session_id[:8]}', 'metadata': {'test_type': 'thread_continuity', 'session_id': session_id, 'created_by': 'integration_test'}}
        headers = {'Authorization': f'Bearer {auth_token}', 'Content-Type': 'application/json'}
        response = requests.post(f'{self.backend_url}/api/conversation/threads', json=thread_payload, headers=headers, timeout=30)
        if response.status_code in [200, 201, 409]:
            if response.status_code == 409:
                get_response = requests.get(f'{self.backend_url}/api/conversation/threads/{self.thread_id}', headers=headers, timeout=30)
    ...
    async def _send_message_in_session(self, session_id: str, message_content: str, context: Dict[str, Any]=None) -> Dict[str, Any]:
        """Send message in specific session context"""
        auth_token = await self._create_test_user_and_authenticate()
        message_payload = {'user_id': self.user_id, 'session_id': session_id, 'thread_id': self.thread_id, 'message': message_content, 'message_type': 'user', 'context': context or {}}
        headers = {'Authorization': f'Bearer {auth_token}', 'Content-Type': 'application/json'}
        response = requests.post(f'{self.backend_url}/api/conversation/messages', json=message_payload, headers=headers, timeout=60)
    ...
    async def _get_thread_state(self, session_id: str) -> Dict[str, Any]:
        ...
        auth_token = await self._create_test_user_and_authenticate()
        headers = {'Authorization': f'Bearer {auth_token}', 'Content-Type': 'application/json'}
        response = requests.get(f'{self.backend_url}/api/conversation/threads/{self.thread_id}/state', headers=headers, params={'session_id': session_id}, timeout=30)
    ...
    async def _execute_agent_in_session(self, session_id: str, query: str) -> Dict[str, Any]:
        ...
        auth_token = await self._create_test_user_and_authenticate()
        agent_payload = {'user_id': self.user_id, 'session_id': session_id, 'thread_id': self.thread_id, 'agent_type': 'supervisor', 'query': query, 'context': {'session_continuity': True, 'thread_context_required': True}}
        headers = {'Authorization': f'Bearer {auth_token}', 'Content-Type': 'application/json'}
        response = requests.post(f'{self.backend_url}/api/agent/execute', json=agent_payload, headers=headers, timeout=120)
    ...
    async def _get_thread_history(self, session_id: str) -> Dict[str, Any]:
        ...
        response = requests.get(f'{self.backend_url}/api/conversation/threads/{self.thread_id}/history', headers=headers, params={'session_id': session_id, 'include_context': 'true'}, timeout=30)
    ...
    async def test_thread_continuity_across_sessions(self):
        ...
        await self._send_message_in_session(session_1, "Hello! I'm starting a conversation that should continue across sessions.")
        state_1 = await self._get_thread_state(session_1)
        agent_result_1 = await self._execute_agent_in_session(session_1, 'Please analyze my request and remember this context for our ongoing conversation.')
        await asyncio.sleep(1.0)
        logger.info('Phase 2: Continuing conversation in second session')
        session_2 = self.session_ids[1]
        thread_data_2 = await self._create_conversation_thread(session_2)
        await self._send_message_in_session(session_2, "I'm continuing our conversation from before. Do you remember what we discussed?")
        state_2 = await self._get_thread_state(session_2)
        self.state_tracker.record_cross_session_event('session_continuation', self.thread_id, session_1, session_2, {'context_preserved': True})
        agent_result_2 = await self._execute_agent_in_session(session_2, 'Based on our previous discussion, provide a continuation of your analysis.')
        await asyncio.sleep(1.0)
        ...
        await self._send_message_in_session(session_3, 'This is my third session. Please summarize our entire conversation history.')
        state_3 = await self._get_thread_state(session_3)
        history_3 = await self._get_thread_history(session_3)
        agent_result_3 = await self._execute_agent_in_session(session_3, 'Please provide a comprehensive summary that demonstrates you have access to our complete conversation history across all sessions.')
    ...
    def _validate_thread_continuity(self, state_1: Dict[str, Any], state_2: Dict[str, Any], state_3: Dict[str, Any]):
    ...
    def _validate_cross_session_context(self, result_1: Dict[str, Any], result_2: Dict[str, Any], result_3: Dict[str, Any]):
