...
class SyntaxErrorDetector:
    ...
    def __init__(self):
        self.error_patterns = [('^\\s*except\\s*:\\s*$', 'Bare except clause (should specify exception)'), ('^\\s*except\\s*:\\s*#.*$', 'Bare except clause (should specify exception)')]
    ...
    def analyze_file(self, file_path: str) -> Dict[str, Any]:
        ...
        """
        result = {'file_path': file_path, 'has_syntax_errors': False, 'ast_error': None, 'tokenization_error': None, 'pattern_issues': [], 'line_issues': [], 'error_summary': []}
        try:
        ...
        try:
            ...
            result['ast_error'] = {'line': e.lineno, 'offset': e.offset, 'message': e.msg, 'text': e.text}
            result['has_syntax_errors'] = True
        ...
        try:
            ...
            result['tokenization_error'] = {'message': str(e), 'error_type': 'TokenError'}
            result['has_syntax_errors'] = True
        ...
        for line_num, line in enumerate(lines, 1):
            ...
            for pattern, description in self.error_patterns:
                if re.search(pattern, line):
                    issue = {'line_number': line_num, 'line_content': line.strip(), 'pattern': pattern, 'description': description}
        ...
        if result['pattern_issues'] and (not result.get('ast_valid', False)):
            result['has_syntax_errors'] = True
            result['error_summary'].extend([f"Pattern Issue Line {issue['line_number']}: {issue['description']}" for issue in result['pattern_issues']])
        return result
    ...
    def scan_directory(self, directory: str, pattern: str='*.py') -> Dict[str, Any]:
        ...
        """
        results = {'directory': directory, 'files_scanned': 0, 'files_with_errors': 0, 'file_results': {}, 'error_summary': []}
...
class SyntaxErrorDetectionTests(SSotBaseTestCase):
    ...
    def test_integration_files_syntax_validation(self):
        ...
        scan_results = self.syntax_detector.scan_directory(str(self.integration_tests_dir), '**/*.py')
        ...
        error_report = {'scan_timestamp': asyncio.get_event_loop().time(), 'directory_scanned': str(self.integration_tests_dir), 'total_files': scan_results['files_scanned'], 'files_with_errors': scan_results['files_with_errors'], 'detailed_errors': []}
        for file_path, file_result in scan_results['file_results'].items():
            if file_result['has_syntax_errors']:
                error_report['detailed_errors'].append({'file': file_path, 'ast_error': file_result.get('ast_error'), 'tokenization_error': file_result.get('tokenization_error'), 'pattern_issues': file_result.get('pattern_issues', []), 'error_summary': file_result.get('error_summary', [])})
        ...
        assert scan_results['files_with_errors'] == 0, f"CRITICAL: Found {scan_results['files_with_errors']} integration test files with syntax errors. See detailed report at {report_path}. Errors: {'; '.join(scan_results['error_summary'][:5])}"
    ...
    def test_unterminated_string_literal_detection(self):
        ...
        for file_path in glob.glob(str(self.integration_tests_dir / '*.py')):
            try:
                ...
                if analysis_result.get('ast_error'):
                    ...
                    if any((indicator in error_msg for indicator in ['unterminated string', 'eol while scanning', 'eof while scanning', 'invalid token', 'unexpected eof'])):
                        detection_results.append({'file': file_path, 'line': ast_error.get('line', 0), 'content': ast_error.get('text', '').strip() if ast_error.get('text') else '', 'issue': f"AST Error: {ast_error.get('message', 'Unknown syntax error')}"})
                        logger.warning(f"Potential unterminated string in AST: {file_path}:{ast_error.get('line', 0)}")
            except Exception as e:
                logger.error(f'Error scanning {file_path}: {e}')
                detection_results.append({'file': file_path, 'line': 0, 'content': str(e), 'issue': 'File analysis error'})
        ...
        assert len(detection_results) == 0, f'CRITICAL: Found {len(detection_results)} unterminated string literals in integration tests. Details: {detection_results[:3]}'
    ...
    def test_literal_newline_sequence_detection(self):
                ...
                for line_num, line in enumerate(lines, 1):
                    ...
                    for pattern, description in literal_newline_patterns:
                        ...
                        for match in matches:
                            literal_newline_issues.append({'file': file_path, 'line': line_num, 'position': match.start(), 'content': line.strip(), 'matched_text': match.group(), 'issue': description})
        ...
        assert len(literal_newline_issues) == 0, f'CRITICAL: Found {len(literal_newline_issues)} literal newline sequences in integration tests. These can cause syntax errors. Details: {literal_newline_issues[:2]}'
    ...
    def test_test_runner_syntax_pre_validation(self):
        ...
        try:
            ...
            assert analysis_result['has_syntax_errors'], 'Syntax detector should detect the intentional syntax error'
            assert analysis_result.get('ast_error') or analysis_result.get('tokenization_error'), 'Should detect either AST or tokenization error'
            try:
                subprocess.run([sys.executable, '-m', 'py_compile', temp_file_path], check=True, capture_output=True, text=True)
