...
from netra_backend.app.core.auth_constants import OAuthConstants, AuthConstants, CredentialConstants, JWTConstants, AuthErrorConstants
...
class RealOAuthFlowTests:
    ...
    def generate_pkce_challenge(self) -> tuple[str, str]:
        ...
        code_challenge_b64 = secrets.token_urlsafe(len(code_challenge))[:43]
    ...
    async def test_oauth_authorization_url_generation(self, oauth_credentials: Dict[str, str], oauth_redirect_uri: str):
        ...
        auth_params = {OAuthConstants.CLIENT_ID: oauth_credentials['client_id'], OAuthConstants.REDIRECT_URI: oauth_redirect_uri, OAuthConstants.SCOPE: ' '.join(AuthConstants.OAUTH_SCOPES), OAuthConstants.STATE: state, OAuthConstants.CODE_CHALLENGE: code_challenge, OAuthConstants.CODE_CHALLENGE_METHOD: 'S256', 'response_type': OAuthConstants.CODE, 'access_type': 'offline', 'prompt': 'consent'}
    ...
    async def test_oauth_pkce_flow_security(self, oauth_credentials: Dict[str, str]):
        ...
        import base64
        import hashlib
        expected_challenge = base64.urlsafe_b64encode(hashlib.sha256(code_verifier.encode()).digest()).decode().rstrip('=')
    ...
    async def test_oauth_callback_handling(self, async_client: AsyncClient, oauth_credentials: Dict[str, str]):
        ...
        callback_params = {OAuthConstants.CODE: mock_auth_code, OAuthConstants.STATE: state}
        try:
    ...
    async def test_oauth_token_exchange_simulation(self, oauth_credentials: Dict[str, str], oauth_redirect_uri: str):
        ...
        token_data = {'grant_type': OAuthConstants.AUTHORIZATION_CODE, OAuthConstants.CLIENT_ID: oauth_credentials['client_id'], OAuthConstants.CLIENT_SECRET: oauth_credentials['client_secret'], OAuthConstants.REDIRECT_URI: oauth_redirect_uri, OAuthConstants.CODE: mock_auth_code, OAuthConstants.CODE_VERIFIER: code_verifier}
        ...
        mock_token_response = {JWTConstants.ACCESS_TOKEN: 'mock_access_token_abcd1234', JWTConstants.REFRESH_TOKEN: 'mock_refresh_token_efgh5678', JWTConstants.TOKEN_TYPE: 'Bearer', JWTConstants.EXPIRES_IN: 3600, 'scope': ' '.join(AuthConstants.OAUTH_SCOPES)}
    ...
    async def test_oauth_user_profile_fetching(self, oauth_credentials: Dict[str, str]):
        ...
        profile_headers = {'Authorization': f'Bearer {mock_access_token}', 'Accept': 'application/json'}
        mock_profile_response = {'id': 'oauth_user_123456', 'email': 'oauth.user@gmail.com', 'verified_email': True, 'name': 'OAuth Test User', 'given_name': 'OAuth', 'family_name': 'User', 'picture': 'https://example.com/photo.jpg', 'locale': 'en'}
        ...
        internal_user = {'external_id': mock_profile_response['id'], 'email': mock_profile_response['email'], 'full_name': mock_profile_response['name'], 'first_name': mock_profile_response['given_name'], 'last_name': mock_profile_response['family_name'], 'avatar_url': mock_profile_response['picture'], 'email_verified': mock_profile_response['verified_email'], 'oauth_provider': OAuthConstants.GOOGLE}
    ...
    async def test_oauth_error_handling_scenarios(self, async_client: AsyncClient):
        ...
        error_scenarios = [{'error': 'invalid_request', 'state': 'invalid_state_123'}, {'error': 'access_denied', 'state': 'valid_state_456'}, {'error': 'unauthorized_client', 'state': 'valid_state_789'}, {'error': 'server_error', 'state': 'valid_state_012'}]
    ...
    async def test_oauth_token_refresh_flow(self, oauth_credentials: Dict[str, str]):
        ...
        refresh_data = {'grant_type': OAuthConstants.REFRESH_TOKEN_GRANT, OAuthConstants.CLIENT_ID: oauth_credentials['client_id'], OAuthConstants.CLIENT_SECRET: oauth_credentials['client_secret'], JWTConstants.REFRESH_TOKEN: mock_refresh_token}
        ...
        mock_refresh_response = {JWTConstants.ACCESS_TOKEN: 'new_access_token_xyz789', JWTConstants.TOKEN_TYPE: 'Bearer', JWTConstants.EXPIRES_IN: 3600, 'scope': ' '.join(AuthConstants.OAUTH_SCOPES)}
    ...
    async def test_oauth_environment_configuration_validation(self, oauth_credentials: Dict[str, str]):
        ...
        if current_env == 'test':
            assert 'test' in client_id.lower() or 'localhost' in client_id.lower(), 'TEST environment must use test-specific OAuth credentials'
        elif current_env == 'staging':
            assert 'staging' in client_id.lower(), 'STAGING environment must use staging-specific OAuth credentials'
        elif current_env == 'production':
            assert 'test' not in client_id.lower() and 'staging' not in client_id.lower(), 'PRODUCTION environment must not use test/staging OAuth credentials'
        ...
        if current_env == 'test':
            assert 'localhost' in oauth_callback or '127.0.0.1' in oauth_callback, 'TEST environment should use localhost callback URLs'
        elif current_env == 'production':
            assert 'localhost' not in oauth_callback and '127.0.0.1' not in oauth_callback, 'PRODUCTION environment must not use localhost callback URLs'
