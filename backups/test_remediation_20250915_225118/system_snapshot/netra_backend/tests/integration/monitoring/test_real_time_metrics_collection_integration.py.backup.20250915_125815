...
class TestRealTimeMetricsCollectionIntegration(SSotBaseTestCase):
    ...
    async def test_system_performance_metrics_collection(self):
        ...
        """
        collection_id = await self.metrics_collector.start_collection(collection_type='system_performance', interval_seconds=1, test_prefix=self.test_prefix)
        try:
            await asyncio.sleep(10)
            collected_metrics = await self.metrics_collector.get_collected_metrics(collection_id=collection_id, metric_types=['cpu', 'memory', 'disk', 'network'])
            ...
            aggregation_result = await self.aggregator.aggregate_metrics(metrics=collected_metrics, aggregation_window_seconds=5, aggregation_functions=['mean', 'max', 'min', 'p95'])
    ...
    async def test_database_performance_metrics_collection(self):
        ...
        """
        pg_collection_id = await self.metrics_collector.start_database_collection(database_type='postgresql', connection_info=await self.db_helper.get_connection_info(), test_prefix=self.test_prefix)
        try:
            ...
            db_metrics = await self.metrics_collector.get_collected_metrics(collection_id=pg_collection_id, metric_types=['connection_count', 'query_performance', 'locks', 'transactions'])
            ...
            alert_config = {'metric_type': 'query_performance', 'threshold_value': 500, 'comparison': 'greater_than', 'severity': 'warning'}
            alert_result = await self.alert_manager.evaluate_metrics_for_alerts(metrics=query_metrics, alert_configs=[alert_config], test_prefix=self.test_prefix)
            assert alert_result.alerts_evaluated > 0
            slow_queries = [m for m in query_metrics if m.attributes.get('avg_query_time_ms', 0) > 500]
    ...
    async def test_application_metrics_collection_real_api_usage(self):
        ...
        """
        app_collection_id = await self.metrics_collector.start_application_collection(application_name='netra_backend', endpoints_to_monitor=['/*'], test_prefix=self.test_prefix)
        try:
            import httpx
            backend_url = self.env.get('BACKEND_URL', 'http://localhost:8000')
            api_scenarios = [{'endpoint': '/health', 'expected_status': 200, 'expected_max_time_ms': 100, 'count': 10}, {'endpoint': '/docs', 'expected_status': 200, 'expected_max_time_ms': 500, 'count': 5}, {'endpoint': '/metrics', 'expected_status': [200, 404], 'expected_max_time_ms': 200, 'count': 3}]
            request_results = []
                ...
                for scenario in api_scenarios:
                    ...
                    for i in range(count):
                        ...
                        try:
                            ...
                            request_results.append({'endpoint': endpoint, 'status_code': response.status_code, 'response_time_ms': response_time_ms, 'success': response.status_code in scenario.get('expected_status', [200])})
                            await asyncio.sleep(0.1)
                        except Exception as e:
                            request_results.append({'endpoint': endpoint, 'status_code': 0, 'response_time_ms': 0, 'success': False, 'error': str(e)})
            await asyncio.sleep(3)
            app_metrics = await self.metrics_collector.get_collected_metrics(collection_id=app_collection_id, metric_types=['http_requests', 'response_times', 'error_rates', 'throughput'])
            http_request_metrics = [m for m in app_metrics if m.metric_type == 'http_requests']
            if len(http_request_metrics) > 0:
            ...
            storage_result = await self.metrics_service.store_metrics(metrics=app_metrics, storage_config={'retention_days': 30, 'compression': True, 'indexing': ['timestamp', 'metric_type', 'endpoint']}, test_prefix=self.test_prefix)
            ...
            if storage_result.metrics_stored > 0:
                retrieved_metrics = await self.metrics_service.query_metrics(query_filters={'metric_type': 'http_requests', 'time_range': {'start': datetime.now(timezone.utc) - timedelta(minutes=10), 'end': datetime.now(timezone.utc)}}, test_prefix=self.test_prefix)
    ...
    async def test_business_metrics_collection_integration(self):
        ...
        """
        business_collection_id = await self.metrics_collector.start_business_collection(metric_categories=['user_activity', 'revenue', 'conversions', 'feature_usage'], test_prefix=self.test_prefix)
        try:
            business_events = [{'event_type': 'user_login', 'user_id': f'user_{i}', 'user_tier': 'free' if i % 4 == 0 else 'mid' if i % 4 == 1 else 'enterprise', 'timestamp': datetime.now(timezone.utc), 'attributes': {'source': 'web', 'session_duration': 300 + i * 10}} for i in range(20)]
            revenue_events = [{'event_type': 'subscription_upgrade', 'user_id': f'user_{i}', 'revenue_amount': 50.0 if i % 2 == 0 else 500.0, 'currency': 'USD', 'timestamp': datetime.now(timezone.utc), 'attributes': {'from_tier': 'free', 'to_tier': 'mid' if i % 2 == 0 else 'enterprise'}} for i in range(5)]
            feature_events = [{'event_type': 'feature_usage', 'user_id': f'user_{i}', 'feature_name': 'cost_optimizer' if i % 3 == 0 else 'security_analyzer' if i % 3 == 1 else 'data_processor', 'usage_duration': 120 + i * 20, 'timestamp': datetime.now(timezone.utc), 'attributes': {'success': True, 'complexity': 'medium'}} for i in range(30)]
            all_events = business_events + revenue_events + feature_events
            for event in all_events:
                await self.metrics_collector.record_business_event(event=event, collection_id=business_collection_id)
                await asyncio.sleep(0.01)
            await asyncio.sleep(3)
            business_metrics = await self.metrics_collector.get_collected_metrics(collection_id=business_collection_id, metric_types=['user_activity', 'revenue', 'feature_usage', 'conversions'])
            ...
            kpi_result = await self.aggregator.calculate_business_kpis(metrics=business_metrics, kpi_definitions={'daily_active_users': {'metric_type': 'user_activity', 'aggregation': 'unique_count'}, 'revenue_per_user': {'metric_type': 'revenue', 'aggregation': 'average'}, 'feature_adoption_rate': {'metric_type': 'feature_usage', 'aggregation': 'adoption_rate'}}, time_window_hours=1, test_prefix=self.test_prefix)
    ...
    async def test_alert_integration_on_real_metrics(self):
        ...
        """
        alert_rules = [{'rule_id': f'{self.test_prefix}_high_cpu', 'metric_type': 'cpu', 'threshold': 90.0, 'comparison': 'greater_than', 'duration_seconds': 5, 'severity': 'critical', 'notification_channels': ['email', 'slack']}, {'rule_id': f'{self.test_prefix}_high_memory', 'metric_type': 'memory', 'threshold': 85.0, 'comparison': 'greater_than', 'duration_seconds': 3, 'severity': 'warning', 'notification_channels': ['slack']}, {'rule_id': f'{self.test_prefix}_slow_response', 'metric_type': 'response_times', 'threshold': 1000.0, 'comparison': 'greater_than', 'duration_seconds': 2, 'severity': 'warning', 'notification_channels': ['email']}]
        for rule in alert_rules:
            await self.alert_manager.register_alert_rule(rule)
        try:
            alert_collection_id = await self.metrics_collector.start_collection(collection_type='comprehensive', interval_seconds=1, test_prefix=self.test_prefix)
            await asyncio.sleep(8)
            alert_metrics = await self.metrics_collector.get_collected_metrics(collection_id=alert_collection_id, metric_types=['cpu', 'memory', 'response_times'])
            alert_evaluation = await self.alert_manager.evaluate_alert_rules(metrics=alert_metrics, rules=alert_rules, test_prefix=self.test_prefix)
            ...
            if alert_evaluation.alerts_triggered > 0:
                await asyncio.sleep(3)
                second_evaluation = await self.alert_manager.evaluate_alert_rules(metrics=alert_metrics, rules=alert_rules, test_prefix=self.test_prefix)
            ...
            recovery_evaluation = await self.alert_manager.evaluate_alert_recovery(current_metrics=alert_metrics, active_alerts=alert_evaluation.active_alerts, test_prefix=self.test_prefix)
