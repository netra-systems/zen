...
class TestWebSocketStateMachineTiming(SSotBaseTestCase):
            ...
            for i, result in enumerate(results):
                if isinstance(result, Exception):
                    ...
                    if 'get_connection_state_machine' in error_str and 'not defined' in error_str:
                        import_failures.append({'connection_id': f'test-timing-{i}', 'error': error_str, 'error_type': type(result).__name__})
                        print(f' PASS:  IMPORT SCOPE BUG REPRODUCED in connection {i}: {error_str}')
            if import_failures:
                failure_details = json.dumps(import_failures, indent=2)
                raise AssertionError(f'INTEGRATION TEST SUCCESS: Import scope bugs reproduced in {len(import_failures)} connections:\n{failure_details}')
            pytest.fail('Import scope timing bug not reproduced in integration test. Real WebSocket connections did not trigger the expected NameError.')
    ...
    async def _create_timed_websocket_connection(self, connection_id: str) -> Dict[str, Any]:
        ...
        connection_result = {'connection_id': connection_id, 'start_time': start_time, 'success': False, 'error': None, 'timing_ms': 0}
        try:
            headers = self.auth_helper.get_websocket_headers(self.jwt_token)
            connection_timeout = 5.0
            websocket = await asyncio.wait_for(websockets.connect(self.websocket_url, additional_headers=headers, open_timeout=connection_timeout, close_timeout=2.0), timeout=connection_timeout)
            test_message = {'type': 'agent_request', 'data': {'message': f'Test message from {connection_id}', 'timestamp': datetime.now(timezone.utc).isoformat()}}
    ...
    async def test_concurrent_websocket_state_machine_race_conditions(self):
        ...
        async def create_racing_connection(conn_id: int) -> Dict[str, Any]:
            ...
            try:
                headers = self.auth_helper.get_websocket_headers(self.jwt_token)
                websocket = await asyncio.wait_for(websockets.connect(self.websocket_url, additional_headers=headers, open_timeout=2.0), timeout=2.0)
                rapid_message = {'type': 'ping', 'connection_id': f'race-{conn_id}', 'timestamp': time.time()}
                await websocket.send(json.dumps(rapid_message))
                await websocket.close()
                return {'success': True, 'connection_id': conn_id}
            except Exception as e:
                error_info = {'connection_id': conn_id, 'error': str(e), 'error_type': type(e).__name__}
                if 'get_connection_state_machine' in str(e) and 'not defined' in str(e):
                    error_info['import_scope_race'] = True
                    race_condition_errors.append(error_info)
                return error_info
        try:
            connection_tasks = [create_racing_connection(i) for i in range(concurrent_connections)]
            ...
            for result in results:
                if isinstance(result, dict) and result.get('import_scope_race'):
                    ...
                    if 'get_connection_state_machine' in str(result):
                        race_condition_errors.append({'error': str(result), 'error_type': type(result).__name__, 'import_scope_race': True})
            if race_condition_errors:
                error_summary = json.dumps(race_condition_errors, indent=2)
                raise AssertionError(f'RACE CONDITION IMPORT SCOPE BUGS REPRODUCED in {len(race_condition_errors)} connections:\n{error_summary}')
            pytest.fail(f'Race condition import scope bugs not reproduced. All {concurrent_connections} concurrent connections succeeded without timing errors.')
    ...
    async def test_websocket_exception_handler_import_scope_timing(self):
        ...
        test_scenarios = [{'invalid_url': 'ws://localhost:8000/ws/invalid', 'expected': 'Invalid endpoint'}, {'malformed_headers': True, 'expected': 'Header processing'}, {'rapid_disconnect': True, 'expected': 'Rapid disconnection'}]
        for i, scenario in enumerate(test_scenarios):
            try:
                if scenario.get('invalid_url'):
                    headers = self.auth_helper.get_websocket_headers(self.jwt_token)
                    try:
                        websocket = await asyncio.wait_for(websockets.connect(scenario['invalid_url'], additional_headers=headers), timeout=3.0)
                        await websocket.close()
                    except Exception as url_error:
                        if 'get_connection_state_machine' in str(url_error):
                            exception_handler_failures.append({'scenario': 'invalid_url', 'error': str(url_error), 'import_scope_error': True})
                elif scenario.get('malformed_headers'):
                    malformed_headers = {'Authorization': 'Bearer invalid-token-format', 'X-Invalid-Header': 'malformed-value-to-trigger-error'}
                    try:
                        websocket = await asyncio.wait_for(websockets.connect(self.websocket_url, additional_headers=malformed_headers), timeout=3.0)
                        await websocket.close()
                    except Exception as header_error:
                        if 'get_connection_state_machine' in str(header_error):
                            exception_handler_failures.append({'scenario': 'malformed_headers', 'error': str(header_error), 'import_scope_error': True})
                    ...
                    try:
                        ...
                        if 'get_connection_state_machine' in str(disconnect_error):
                            exception_handler_failures.append({'scenario': 'rapid_disconnect', 'error': str(disconnect_error), 'import_scope_error': True})
            except Exception as scenario_error:
                if 'get_connection_state_machine' in str(scenario_error) and 'not defined' in str(scenario_error):
                    exception_handler_failures.append({'scenario': f'scenario_{i}', 'error': str(scenario_error), 'import_scope_error': True})
        if exception_handler_failures:
            failure_summary = json.dumps(exception_handler_failures, indent=2)
            raise AssertionError(f'EXCEPTION HANDLER IMPORT SCOPE BUGS REPRODUCED in {len(exception_handler_failures)} scenarios:\n{failure_summary}')
        pytest.fail('Exception handler import scope timing bugs not reproduced. All exception scenarios completed without triggering import scope errors.')
