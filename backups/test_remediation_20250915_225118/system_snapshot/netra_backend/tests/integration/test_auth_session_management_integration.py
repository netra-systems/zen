...
class AuthSessionManagementIntegrationTests(BaseIntegrationTest):
    ...
    async def test_session_persistence_across_multiple_api_calls(self, real_services_fixture):
        ...
        email = f"session-persist-{user_id}@netrasystems.ai"
    ...
    async def test_session_expiration_invalidates_api_access(self, real_services_fixture):
        ...
        email = f"session-expire-{user_id}@netrasystems.ai"
    ...
    async def test_redis_session_cache_integration(self, real_services_fixture):
        ...
        email = f"redis-session-{user_id}@netrasystems.ai"
        ...
        await redis_client.hset(session_key, "last_accessed", updated_access_time)
        
        updated_data = await redis_client.hgetall(session_key)
    ...
    async def test_concurrent_session_management_maintains_isolation(self, real_services_fixture):
        ...
        for i in range(num_users):
            ...
            email = f"concurrent-session-{i}-{user_id}@netrasystems.ai"
        ...
        async def perform_session_operations(session):
            ...
            try:
                await redis_client.hset(session_key, mapping=session_redis_data)
                await redis_client.expire(session_key, 180)  # 3 minute expiry
                redis_success = True
            except Exception as e:
                redis_success = False
    ...
    async def test_session_cleanup_removes_expired_data_from_redis(self, real_services_fixture):
        ...
        for config in session_configs:
            ...
            session_data = {
                ...
                "email": f"{config['user_id']}@netrasystems.ai",
                "created_at": datetime.now(timezone.utc).isoformat(),
            ...
            if config["expiry_seconds"] > 0:
                ...
                await redis_client.expire(session_key, config["expiry_seconds"])
            else:
        ...
        for i, session_key in enumerate(created_keys):
            ...
            ttl = await redis_client.ttl(session_key) if exists else -2
