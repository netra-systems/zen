...
class ToolRegistryRegressionValidationTests(SSotAsyncTestCase):
    ...
    def setUp(self):
        ...
        self.baseline_tools = [RegressionTestTool('baseline_tool_1'), RegressionTestTool('baseline_tool_2'), RegressionTestTool('baseline_tool_3')]
    ...
    def test_existing_tool_registration_still_works(self):
        ...
        for i, tool in enumerate(self.baseline_tools):
            try:
                ...
                registration_results.append({'tool': tool.name, 'category': category, 'registered': len(registered_tools) > 0, 'status': 'success'})
            except Exception as e:
                registration_results.append({'tool': tool.name, 'category': f'category_{i}', 'registered': False, 'status': 'error', 'error': str(e)})
        ...
        self.assertEqual(len(successful_registrations), len(self.baseline_tools), 'REGRESSION: Existing tool registration should continue working')
        self.assertEqual(len(failed_registrations), 0, 'REGRESSION: No failures expected for valid tools')
    ...
    async def test_existing_tool_execution_still_works(self):
        ...
        for i, baseline_tool in enumerate(self.baseline_tools):
            unified_tool = UnifiedTool(id=f'unified_{baseline_tool.name}', name=baseline_tool.name, description=baseline_tool.description, category='regression_test')
            ...
            try:
                result = await self.unified_registry.execute_tool(unified_tool.id, {'test_param': f'value_{i}'}, {'user': 'regression_user', 'request_id': f'req_{i}'})
                execution_results.append({'tool_id': unified_tool.id, 'success': result.success, 'has_result': result.result is not None, 'status': 'success'})
            except Exception as e:
                execution_results.append({'tool_id': unified_tool.id, 'success': False, 'has_result': False, 'status': 'error', 'error': str(e)})
        ...
        self.assertEqual(len(successful_executions), len(self.baseline_tools), 'REGRESSION: Existing tool execution should continue working')
    ...
    def test_existing_validation_behavior_preserved(self):
        ...
        try:
            ...
            validation_results.append({'scenario': 'validation_disabled_empty_name', 'expected': 'success', 'actual': 'success', 'status': 'pass'})
        except Exception as e:
            validation_results.append({'scenario': 'validation_disabled_empty_name', 'expected': 'success', 'actual': f'error: {e}', 'status': 'regression_failure'})
        self.agent_registry.enable_validation = True
        try:
            invalid_tool = RegressionTestTool('')
            self.agent_registry.register_tool('test_invalid_2', invalid_tool)
            validation_results.append({'scenario': 'validation_enabled_empty_name', 'expected': 'error', 'actual': 'success', 'status': 'unexpected_success'})
        except Exception as e:
            validation_results.append({'scenario': 'validation_enabled_empty_name', 'expected': 'error', 'actual': f'error: {type(e).__name__}', 'status': 'pass'})
        ...
        self.assertEqual(len(regression_failures), 0, 'REGRESSION: Existing validation behavior should be preserved')
    ...
    def test_exception_type_improvements_while_preserving_catching(self):
        ...
        try:
            ...
            exception_compatibility_results.append({'test': 'netra_exception_catch', 'caught': False, 'exception_type': None})
        except NetraException as e:
            exception_compatibility_results.append({'test': 'netra_exception_catch', 'caught': True, 'exception_type': type(e).__name__, 'compatible': True})
        except Exception as e:
            exception_compatibility_results.append({'test': 'netra_exception_catch', 'caught': True, 'exception_type': type(e).__name__, 'compatible': isinstance(e, NetraException)})
        try:
            from netra_backend.app.services.unified_tool_registry.models import UnifiedTool
            invalid_tool_data = 'not_a_unified_tool'
            self.unified_registry.register_tool(invalid_tool_data, None)
            exception_compatibility_results.append({'test': 'generic_exception_catch', 'caught': False, 'exception_type': None})
        except Exception as e:
            exception_compatibility_results.append({'test': 'generic_exception_catch', 'caught': True, 'exception_type': type(e).__name__, 'compatible': True})
        print(f'EXCEPTION COMPATIBILITY RESULTS: {exception_compatibility_results}')
        incompatible_exceptions = [r for r in exception_compatibility_results if 'compatible' in r and (not r['compatible'])]
        if incompatible_exceptions:
            print(f'EXCEPTION COMPATIBILITY FAILURES: {incompatible_exceptions}')
        self.assertEqual(len(incompatible_exceptions), 0, 'REGRESSION: New exceptions should maintain backward compatibility')
    ...
    def test_performance_regression_check(self):
        ...
        performance_results.append({'scenario': 'baseline_successful_operations', 'operations': successful_operations, 'time_seconds': baseline_time, 'ops_per_second': baseline_ops_per_second})
        start_time = time.time()
        error_operations = 0
        self.agent_registry.enable_validation = True
        for i in range(50):
        ...
        performance_results.append({'scenario': 'error_handling_operations', 'operations': error_operations, 'time_seconds': error_time, 'ops_per_second': error_ops_per_second})
        ...
        if baseline_ops_per_second > 0 and error_ops_per_second > 0:
            ...
            performance_acceptable = performance_ratio <= 10.0
            print(f'PERFORMANCE RATIO (baseline/error): {performance_ratio:.2f}x')
            self.assertTrue(performance_acceptable, f'PERFORMANCE REGRESSION: Error handling should not be more than 10x slower (actual: {performance_ratio:.2f}x)')
        self.assertGreater(baseline_ops_per_second, 10, 'PERFORMANCE REGRESSION: Should handle at least 10 successful operations per second')
    ...
    async def test_existing_async_patterns_still_work(self):
        ...
        async def execute_tool_concurrently(tool_id: str, iteration: int):
            try:
                result = await self.unified_registry.execute_tool(tool_id, {'iteration': iteration}, {'user': f'async_user_{iteration}'})
                return {'tool_id': tool_id, 'iteration': iteration, 'success': result.success, 'status': 'completed'}
            except Exception as e:
                return {'tool_id': tool_id, 'iteration': iteration, 'success': False, 'status': 'error', 'error': str(e)}
        async_tools = []
        for i in range(5):
            tool_id = f'async_regression_tool_{i}'
            unified_tool = UnifiedTool(id=tool_id, name=f'Async Regression Tool {i}', description=f'Async regression test tool {i}', category='async_regression')
        ...
        total_expected = len(async_tools) * 3
        self.assertEqual(len(successful_async), total_expected, f'ASYNC REGRESSION: Expected {total_expected} successful async operations')
        self.assertEqual(len(exceptions_async), 0, 'ASYNC REGRESSION: No exceptions should occur during valid async operations')
...
class ToolRegistryExceptionImprovementValidationTests(SSotAsyncTestCase):
    ...
    def test_improved_error_messages_provide_better_diagnostics(self):
        ...
        try:
            ...
            diagnostic_improvements.append({'scenario': 'empty_tool_name_validation', 'current_message': str(e), 'current_type': type(e).__name__, 'has_error_code': hasattr(e, 'error_details'), 'has_context': hasattr(e, 'error_details') and hasattr(e.error_details, 'context'), 'has_severity': hasattr(e, 'error_details') and hasattr(e.error_details, 'severity')})
        print(f'DIAGNOSTIC IMPROVEMENTS: {diagnostic_improvements}')
        self.assertGreater(len(diagnostic_improvements), 0, 'Should have captured diagnostic information')
    ...
    def test_exception_categorization_enables_better_handling(self):
        ...
        error_scenarios = [('empty_name', lambda: RegressionTestTool('')), ('type_error', lambda: 'not_a_tool'), ('execution_error', lambda: RegressionTestTool('exec_error', simulate_error=True))]
        for scenario_name, error_generator in error_scenarios:
            try:
                ...
                exception_categories[scenario_name] = {'exception_type': exception_type, 'message': str(e), 'is_specific': exception_type not in ['Exception', 'RuntimeError', 'ValueError'], 'is_netra_exception': isinstance(e, NetraException)}
    ...
    def test_exception_context_enables_better_debugging(self):
        ...
        """
        context_analysis = []
        debug_context = {'operation': 'test_tool_registration', 'user_id': 'debug_user', 'request_id': 'debug_req_123', 'tool_source': 'regression_test', 'validation_level': 'strict'}
        try:
            self.registry.enable_validation = True
            invalid_tool = RegressionTestTool('')
            self.registry.register_tool('debug_test', invalid_tool)
        except Exception as e:
            context_analysis.append({'has_error_details': hasattr(e, 'error_details'), 'has_trace_id': hasattr(e, 'error_details') and hasattr(e.error_details, 'trace_id'), 'has_context': hasattr(e, 'error_details') and hasattr(e.error_details, 'context'), 'has_user_message': hasattr(e, 'error_details') and hasattr(e.error_details, 'user_message'), 'exception_str': str(e), 'exception_type': type(e).__name__})
