...
from netra_backend.app.websocket_core.connection_state_machine import ApplicationConnectionState, ConnectionStateTransition, WebSocketConnectionStateMachine
...
class WebSocketStateMachineAuthenticationIntegrationTests(SSotBaseTestCase):
    ...
    def setUpClass(cls):
        ...
        cls.assertTrue(cls.db_fixture.is_available(), 'Database fixture required for state persistence')
        cls.assertTrue(cls.redis_fixture.is_available(), 'Redis fixture required for state caching')
    ...
    def setUp(self) -> None:
        ...
        self.state_machine = WebSocketConnectionStateMachine(connection_id=self.connection_id)
    ...
    def create_authenticated_websocket(self, user_id: str=None) -> tuple[Mock, Any]:
        ...
        auth_user = self.auth_helper.create_authenticated_user(email=f'{user_id}@integration.test', user_id=user_id, permissions=['websocket', 'chat'])
        websocket = Mock()
        websocket.state = 'connected'
        websocket.headers = {'authorization': f'Bearer {auth_user.jwt_token}', 'connection-id': self.connection_id, 'user-id': user_id}
    ...
    def test_authentication_state_flow_with_real_services(self):
        ...
        accepted_transition = ConnectionStateTransition(connection_id=self.connection_id, from_state=ApplicationConnectionState.CONNECTING, to_state=ApplicationConnectionState.ACCEPTED, user_id=self.user_id)
        ...
        self.assertEqual(self.state_machine.get_current_state(), ApplicationConnectionState.ACCEPTED)
        ...
        authenticated_transition = ConnectionStateTransition(connection_id=self.connection_id, from_state=ApplicationConnectionState.ACCEPTED, to_state=ApplicationConnectionState.AUTHENTICATED, user_id=self.user_id)
        ...
        self.assertEqual(self.state_machine.get_current_state(), ApplicationConnectionState.AUTHENTICATED)
    ...
    def test_authentication_failure_state_rollback(self):
        ...
        invalid_websocket.headers = {'authorization': 'Bearer invalid.jwt.token.here', 'connection-id': self.connection_id}
        ...
        auth_transition = ConnectionStateTransition(connection_id=self.connection_id, from_state=ApplicationConnectionState.ACCEPTED, to_state=ApplicationConnectionState.AUTHENTICATED, user_id=self.user_id)
    ...
    def test_concurrent_authentication_with_state_consistency(self):
        ...
        def concurrent_authentication_attempt(thread_id: int):
            ...
            try:
                ...
                auth_transition = ConnectionStateTransition(connection_id=f'{self.connection_id}_{thread_id}', from_state=ApplicationConnectionState.ACCEPTED, to_state=ApplicationConnectionState.AUTHENTICATED, user_id=ensure_user_id(thread_user_id))
                thread_state_machine = WebSocketConnectionStateMachine(connection_id=f'{self.connection_id}_{thread_id}')
                ...
                with auth_lock:
                    auth_results.append({'thread_id': thread_id, 'user_id': thread_user_id, 'success': success, 'final_state': final_state, 'connection_id': f'{self.connection_id}_{thread_id}'})
            except Exception as e:
                with auth_lock:
                    auth_results.append({'thread_id': thread_id, 'error': str(e), 'success': False})
        ...
        authenticated_states = [r for r in successful_auths if r.get('final_state') == ApplicationConnectionState.AUTHENTICATED]
    ...
    def test_state_persistence_during_authentication_timeout(self):
        ...
        timeout_websocket.headers = {'authorization': 'Bearer timeout.simulation.token', 'connection-id': self.connection_id}
    ...
    def test_message_queuing_coordination_during_authentication(self):
        ...
        test_states = [ApplicationConnectionState.CONNECTING, ApplicationConnectionState.ACCEPTED, ApplicationConnectionState.AUTHENTICATED, ApplicationConnectionState.SERVICES_READY, ApplicationConnectionState.PROCESSING_READY]
        for test_state in test_states:
            self.state_machine._current_state = test_state
            current_state = self.state_machine.get_current_state()
            test_message = {'type': 'test_message', 'content': f'Message for state {test_state}', 'timestamp': datetime.now(timezone.utc).isoformat(), 'connection_id': self.connection_id}
            should_queue = current_state in [ApplicationConnectionState.CONNECTING, ApplicationConnectionState.ACCEPTED]
            ...
            if should_queue:
                ...
                self.assertGreater(len(queued_messages), 0, f'Messages should be queued in {test_state} state')
            elif should_process:
    ...
    def test_performance_under_concurrent_state_and_auth_operations(self):
        ...
        def concurrent_state_auth_operation(operation_id: int):
            ...
            try:
                ...
                op_state_machine = WebSocketConnectionStateMachine(connection_id=op_connection_id)
                op_user_id = f'perf_user_{operation_id}'
                websocket, auth_user = self.create_authenticated_websocket(op_user_id)
                transitions = [(ApplicationConnectionState.CONNECTING, ApplicationConnectionState.ACCEPTED), (ApplicationConnectionState.ACCEPTED, ApplicationConnectionState.AUTHENTICATED), (ApplicationConnectionState.AUTHENTICATED, ApplicationConnectionState.SERVICES_READY)]
                for from_state, to_state in transitions:
                    transition = ConnectionStateTransition(connection_id=op_connection_id, from_state=from_state, to_state=to_state, user_id=ensure_user_id(op_user_id))
                ...
                with operation_lock:
                    operation_results.append({'operation_id': operation_id, 'execution_time_ms': execution_time, 'final_state': op_state_machine.get_current_state(), 'success': True})
            except Exception as e:
                with operation_lock:
                    operation_results.append({'operation_id': operation_id, 'error': str(e), 'success': False})
        ...
        correct_final_state = [r for r in successful_ops if r.get('final_state') == ApplicationConnectionState.SERVICES_READY]
