...
from netra_backend.app.agents.supervisor.user_execution_engine import UserExecutionEngine, MinimalPeriodicUpdateManager, MinimalFallbackManager
...
class UserExecutionEngineInitializationTests(SSotAsyncTestCase):
    ...
    def test_engine_initialization_with_valid_parameters(self):
        ...
        engine = UserExecutionEngine(context=context, agent_factory=agent_factory, websocket_emitter=websocket_emitter)
        ...
        self.assertEqual(engine.max_concurrent, 3)
    ...
    def test_engine_initialization_with_invalid_context(self):
        ...
        with self.assertRaises(TypeError):
            UserExecutionEngine(context=None, agent_factory=agent_factory, websocket_emitter=websocket_emitter)
        with self.assertRaises(TypeError):
            UserExecutionEngine(context='invalid_context', agent_factory=agent_factory, websocket_emitter=websocket_emitter)
    ...
    def test_engine_initialization_with_missing_parameters(self):
        ...
        with self.assertRaises(ValueError) as exc_context:
            UserExecutionEngine(context=context, agent_factory=None, websocket_emitter=MockUserWebSocketEmitter())
        self.assertIn('AgentInstanceFactory cannot be None', str(exc_context.exception))
        with self.assertRaises(ValueError) as exc_context:
            UserExecutionEngine(context=context, agent_factory=MockAgentInstanceFactory(), websocket_emitter=None)
...
class UserExecutionEngineIsolationTests(SSotAsyncTestCase):
    ...
    def create_test_context(self, user_id: str, run_id: str=None) -> UserExecutionContext:
        """Create test user execution context."""
        return UserExecutionContext(user_id=UserID(user_id), thread_id=ThreadID(f'thread_{user_id}'), run_id=RunID(run_id or f'run_{user_id}_{int(time.time())}'), request_id=f'req_{user_id}_{int(time.time())}', operation_depth=1, agent_context={'user_request': f'Test request from {user_id}'}, metadata={})
    ...
    def create_engine_for_user(self, user_id: str) -> UserExecutionEngine:
        ...
        agent_factory = MockAgentInstanceFactory()
        websocket_emitter = MockUserWebSocketEmitter()
        return UserExecutionEngine(context=context, agent_factory=agent_factory, websocket_emitter=websocket_emitter)
    ...
    def test_execution_summary_per_user(self):
        ...
        self.assertEqual(summary['completed_agents'], 2)
...
class UserExecutionEngineAgentExecutionTests(SSotAsyncTestCase):
    ...
    def create_agent_execution_context(self, user_context: UserExecutionContext, agent_name: str='test_agent'):
        ...
        return AgentExecutionContext(user_id=user_context.user_id, thread_id=user_context.thread_id, run_id=user_context.run_id, request_id=user_context.request_id, agent_name=agent_name, step=PipelineStep.INITIALIZATION, execution_timestamp=datetime.now(timezone.utc), pipeline_step_num=1, metadata={'test': 'data'})
    ...
    async def test_agent_execution_error_handling(self):
        ...
        async def create_fallback(ctx, state, error, start_time):
            return AgentExecutionResult(success=False, agent_name=ctx.agent_name, duration=0.1, error=str(error), data=None, metadata={'fallback_result': True})
    ...
    async def test_agent_execution_concurrency_limits(self):
        ...
        async def slow_execution(ctx, user_ctx):
            ...
            return AgentExecutionResult(success=True, agent_name=ctx.agent_name, duration=0.1, error=None, data={'result': 'success'}, metadata={})
        engine.agent_core.execute_agent = slow_execution
...
class UserExecutionEnginePipelineExecutionTests(SSotAsyncTestCase):
    ...
    async def test_execute_agent_pipeline_success(self):
        ...
        input_data = {'message': 'Test pipeline execution', 'data': {'key': 'value'}}
        result = await engine.execute_agent_pipeline(agent_name='pipeline_agent', execution_context=execution_context, input_data=input_data)
        self.assertIsInstance(result, AgentExecutionResult)
        self.assertTrue(result.success)
    ...
    async def test_execute_agent_pipeline_with_string_input(self):
        ...
        execution_context = engine.context
        result = await engine.execute_agent_pipeline(agent_name='string_agent', execution_context=execution_context, input_data='Simple string input')
        self.assertIsInstance(result, AgentExecutionResult)
        self.assertTrue(result.success)
    ...
    async def test_execute_agent_pipeline_error_handling(self):
        ...
        engine.agent_core.execute_agent = failing_execution
        result = await engine.execute_agent_pipeline(agent_name='failing_agent', execution_context=execution_context, input_data={'test': 'data'})
        self.assertIsInstance(result, AgentExecutionResult)
        self.assertFalse(result.success)
    ...
    async def test_execute_pipeline_multiple_steps(self):
        ...
        user_context = engine.context
        steps = [PipelineStep(agent_name='step1_agent', metadata={'step': 1}), PipelineStep(agent_name='step2_agent', metadata={'step': 2}), PipelineStep(agent_name='step3_agent', metadata={'step': 3})]
        base_context = AgentExecutionContext(user_id=user_context.user_id, thread_id=user_context.thread_id, run_id=user_context.run_id, request_id=user_context.request_id, agent_name='pipeline', step=PipelineStep.INITIALIZATION, execution_timestamp=datetime.now(timezone.utc), pipeline_step_num=0, metadata={'pipeline': 'test'})
    ...
    async def test_execute_pipeline_with_failure_stop(self):
        ...
        async def selective_failure(ctx, user_ctx):
            ...
            if execution_count == 2:
                return AgentExecutionResult(success=False, agent_name=ctx.agent_name, duration=0.1, error='Step 2 failed', data=None, metadata={})
            else:
                return AgentExecutionResult(success=True, agent_name=ctx.agent_name, duration=0.1, error=None, data={'result': 'success'}, metadata={})
        engine.agent_core.execute_agent = selective_failure
        steps = [PipelineStep(agent_name='step1_agent', metadata={}), PipelineStep(agent_name='step2_agent', metadata={}), PipelineStep(agent_name='step3_agent', metadata={})]
        base_context = AgentExecutionContext(user_id=user_context.user_id, thread_id=user_context.thread_id, run_id=user_context.run_id, request_id=user_context.request_id, agent_name='pipeline', step=PipelineStep.INITIALIZATION, execution_timestamp=datetime.now(timezone.utc), pipeline_step_num=0, metadata={})
        results = await engine.execute_pipeline(steps, base_context, user_context)
        self.assertEqual(len(results), 2)
...
class UserExecutionEngineResourceManagementTests(SSotAsyncTestCase):
    ...
    def create_test_engine(self, user_id: str='resource_user') -> UserExecutionEngine:
        """Create test engine."""
        context = UserExecutionContext(user_id=UserID(user_id), thread_id=ThreadID(f'thread_{user_id}'), run_id=RunID(f'run_{user_id}_{int(time.time())}'), request_id=f'req_{user_id}_{int(time.time())}', operation_depth=1, agent_context={'user_request': f'Test request from {user_id}'}, metadata={})
        agent_factory = MockAgentInstanceFactory()
        websocket_emitter = MockUserWebSocketEmitter()
        return UserExecutionEngine(context=context, agent_factory=agent_factory, websocket_emitter=websocket_emitter)
    ...
    async def test_inactive_engine_rejects_execution(self):
        ...
        user_context = engine.context
        engine._is_active = False
        agent_context = AgentExecutionContext(user_id=user_context.user_id, thread_id=user_context.thread_id, run_id=user_context.run_id, request_id=user_context.request_id, agent_name='test_agent', step=PipelineStep.INITIALIZATION, execution_timestamp=datetime.now(timezone.utc), pipeline_step_num=1, metadata={})
        with self.assertRaises(ValueError) as exc_context:
