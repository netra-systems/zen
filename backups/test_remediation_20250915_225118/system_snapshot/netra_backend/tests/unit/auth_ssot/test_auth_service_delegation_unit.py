...
class AuthServiceDelegationUnitTests(SSotAsyncTestCase):
    ...
    async def asyncSetUp(self):
        ...
        self.mock_auth_service_valid_response = {'valid': True, 'user_id': 'test-user-123', 'email': 'test@example.com', 'tier': 'enterprise', 'permissions': ['chat', 'api', 'admin']}
        self.mock_auth_service_invalid_response = {'valid': False, 'error': 'Invalid token', 'code': 'TOKEN_INVALID'}
    ...
    async def test_user_context_extractor_uses_auth_service(self):
        ...
        """
        with patch('netra_backend.app.auth_integration.auth.auth_client') as mock_auth_client:
            mock_auth_client.get_user_context.return_value = {'user_id': 'context-user-456', 'email': 'context@example.com', 'tier': 'free', 'session_id': 'session-789'}
    ...
    async def test_no_jwt_imports_in_auth_integration(self):
        ...
        assert len(jwt_violations) == 0, f'Auth integration SSOT violation: {jwt_violations}. Auth integration must delegate to auth service, never handle JWT directly.'
