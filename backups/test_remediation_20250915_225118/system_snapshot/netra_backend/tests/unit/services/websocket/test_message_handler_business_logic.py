...
from netra_backend.app.services.websocket.message_handler import BaseMessageHandler, StartAgentHandler, UserMessageHandler, ThreadHistoryHandler, StopAgentHandler, MessageHandlerService
from netra_backend.app.services.websocket.message_queue import MessagePriority, QueuedMessage
...
class StartAgentHandlerBusinessLogicTests(BaseUnitTest):
    ...
    def setUp(self):
        ...
        self.mock_supervisor.websocket_bridge = None
    ...
    async def test_start_agent_handler_processes_agent_request_successfully(self):
        ...
        payload = {'request': {'query': 'Optimize my AWS infrastructure costs', 'user_request': 'Help me reduce my cloud spending by 30%'}}
        ...
        self.mock_uow.runs.create_run.assert_called_once_with(self.mock_uow.session, thread_id=self.mock_thread.id, assistant_id='netra-assistant', model='gemini-2.5-flash', instructions='You are Netra AI Workload Optimization Assistant')
        ...
        self.mock_supervisor.run.assert_called_once_with('Optimize my AWS infrastructure costs', self.mock_thread.id, user_id, self.mock_run.id)
    ...
    async def test_start_agent_handler_creates_websocket_bridge_when_missing(self):
        ...
        with patch('netra_backend.app.services.websocket.message_handler.create_websocket_manager'), patch('netra_backend.app.services.websocket.message_handler.create_agent_websocket_bridge') as mock_create_bridge, patch('netra_backend.app.services.websocket.message_handler.get_user_execution_context') as mock_get_context:
            mock_context = Mock()
    ...
    async def test_start_agent_handler_handles_thread_creation_failure(self):
        ...
        with patch('netra_backend.app.services.websocket.message_handler.create_websocket_manager') as mock_create_manager:
            ...
            with patch('netra_backend.app.services.websocket.message_handler.get_user_execution_context'):
                await self.handler.handle(user_id, payload)
            mock_websocket_manager.send_error.assert_called_once_with(user_id, 'Failed to create or retrieve thread')
    ...
    async def test_start_agent_handler_saves_assistant_response_properly(self):
        ...
        agent_response = {'analysis': 'Your AWS costs can be optimized by 25%', 'recommendations': ['Resize EC2 instances', 'Use Reserved Instances'], 'potential_savings': '$2,500/month'}
        ...
        with patch('netra_backend.app.services.websocket.message_handler.create_websocket_manager'):
            with patch('netra_backend.app.services.websocket.message_handler.create_agent_websocket_bridge'):
                await self.handler.handle(user_id, payload)
        assistant_message_calls = [call for call in self.mock_uow.messages.create_message.call_args_list if len(call.kwargs) > 0 and call.kwargs.get('role') == 'assistant']
        assert len(assistant_message_calls) >= 1, 'Must save assistant response message'
        self.mock_uow.runs.update_status.assert_called_with(self.mock_uow.session, self.mock_run.id, 'completed')
...
class UserMessageHandlerBusinessLogicTests(BaseUnitTest):
    ...
    async def test_user_message_handler_processes_user_message_with_context(self):
        ...
        payload = {'text': 'Can you help me analyze my current cloud spending patterns?', 'references': [{'type': 'file', 'name': 'aws-bill.pdf'}]}
        self.mock_uow.threads.get_or_create_for_user.return_value = self.mock_thread
        self.mock_uow.messages.create_message.return_value = None
        self.mock_uow.runs.create_run.return_value = self.mock_run
        self.mock_uow.runs.update_status.return_value = None
        ...
        user_message_call = next((call for call in message_calls if call.kwargs.get('role') == 'user'))
        ...
        self.mock_supervisor.run.assert_called_once_with('Can you help me analyze my current cloud spending patterns?', self.mock_thread.id, user_id, self.mock_run.id)
    ...
    async def test_user_message_handler_handles_messages_without_references(self):
        ...
        payload = {'text': 'What are the top 3 ways to reduce AWS costs?', 'references': []}
        self.mock_uow.threads.get_or_create_for_user.return_value = self.mock_thread
        self.mock_uow.runs.create_run.return_value = self.mock_run
        with patch('netra_backend.app.services.websocket.message_handler.create_websocket_manager'):
            await self.handler.handle(user_id, payload)
    ...
    async def test_user_message_handler_saves_assistant_response_with_metadata(self):
        ...
        with patch('netra_backend.app.services.websocket.message_handler.create_websocket_manager'):
            await self.handler.handle(user_id, payload)
        message_calls = self.mock_uow.messages.create_message.call_args_list
        assistant_message_calls = [call for call in message_calls if call.kwargs.get('role') == 'assistant']
...
class ThreadHistoryHandlerBusinessLogicTests(BaseUnitTest):
    ...
    async def test_thread_history_handler_retrieves_conversation_history(self):
        ...
        payload = {'limit': 25}
        self.mock_uow.threads.get_or_create_for_user.return_value = self.mock_thread
        mock_messages = [Mock(id='msg-1', role='user', content=[{'text': {'value': 'How can I optimize my costs?'}}], created_at='2024-01-15T10:30:00Z'), Mock(id='msg-2', role='assistant', content=[{'text': {'value': 'I recommend analyzing your EC2 usage patterns...'}}], created_at='2024-01-15T10:31:30Z'), Mock(id='msg-3', role='user', content=[{'text': {'value': 'Can you be more specific about EC2 optimization?'}}], created_at='2024-01-15T10:32:15Z')]
        ...
        with patch('netra_backend.app.services.websocket.message_handler.create_websocket_manager') as mock_create_manager:
            ...
            mock_create_manager.return_value = mock_websocket_manager
            await self.handler.handle(user_id, payload)
        self.mock_uow.messages.get_thread_messages.assert_called_once_with(self.mock_uow.session, self.mock_thread.id, limit=25)
    ...
    async def test_thread_history_handler_uses_default_limit_when_not_specified(self):
        ...
        with patch('netra_backend.app.services.websocket.message_handler.create_websocket_manager'):
            await self.handler.handle(user_id, payload)
        self.mock_uow.messages.get_thread_messages.assert_called_once_with(self.mock_uow.session, self.mock_thread.id, limit=50)
...
class MessageHandlerServiceBusinessLogicTests(BaseUnitTest):
    ...
    def test_message_handler_service_registers_all_required_handlers(self):
        ...
        expected_handlers = ['start_agent', 'user_message', 'get_thread_history', 'stop_agent']
    ...
    async def test_message_handler_service_validates_message_format_before_processing(self):
        ...
        valid_message = {'type': 'user_message', 'payload': {'text': 'Test message'}, 'timestamp': 1640995200}
        with patch('netra_backend.app.services.websocket.message_handler.create_websocket_manager') as mock_create_manager:
            mock_websocket_manager = Mock()
    ...
    async def test_message_handler_service_rejects_invalid_message_format(self):
        ...
        invalid_message = {'payload': {'text': 'Missing type field'}, 'timestamp': 1640995200}
        with patch('netra_backend.app.services.websocket.message_handler.create_websocket_manager') as mock_create_manager:
            mock_websocket_manager = Mock()
    ...
    async def test_message_handler_service_handles_unknown_message_types(self):
        ...
        unknown_message = {'type': 'unknown_message_type', 'payload': {'data': 'test'}, 'timestamp': 1640995200}
        with patch('netra_backend.app.services.websocket.message_handler.create_websocket_manager') as mock_create_manager:
            mock_websocket_manager = Mock()
            mock_websocket_manager.validate_message = Mock(return_value=True)
            mock_websocket_manager.send_error = AsyncMock()
            ...
            mock_websocket_manager.send_error.assert_called_once_with(user_id, 'Unknown message type: unknown_message_type')
