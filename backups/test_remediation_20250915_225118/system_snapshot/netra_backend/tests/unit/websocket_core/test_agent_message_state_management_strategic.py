...
class AgentMessageStateManagementStrategicTests(SSotAsyncTestCase):
    ...
    async def _track_emission(self, run_id, event_type, data, **kwargs):
        ...
        self.emitted_messages.append({'timestamp': time.time(), 'run_id': run_id, 'event_type': event_type, 'data': data.copy() if isinstance(data, dict) else data, 'connection_state': getattr(self, '_current_connection_state', ConnectionState.CONNECTED), 'agent_state': getattr(self, '_current_agent_state', AgentExecutionState.STARTED)})
        return True
    ...
    async def test_message_state_during_websocket_reconnection_transition(self):
        ...
        with patch.object(type(self.bridge), 'websocket_manager', new_callable=PropertyMock) as mock_ws_property:
            ...
            result1 = await self.bridge.notify_agent_started(run_id='run_123', agent_name='CriticalAgent', context={'operation': 'business_critical_analysis'})
            self._current_connection_state = ConnectionState.RECONNECTING
            result2 = await self.bridge.notify_agent_thinking(run_id='run_123', agent_name='CriticalAgent', reasoning='Processing critical business data')
            self._current_connection_state = ConnectionState.CONNECTED
            result3 = await self.bridge.notify_agent_completed(run_id='run_123', agent_name='CriticalAgent', result={'critical_insights': 'data'})
            ...
            assert message_types == ['agent_started', 'agent_thinking', 'agent_completed'], 'Message order must be preserved during connection transitions'
    ...
    async def test_concurrent_agent_state_transitions_isolation(self):
        ...
        """
        users = [{'user_id': 'user_1', 'run_id': 'run_1', 'agent': 'Agent1'}, {'user_id': 'user_2', 'run_id': 'run_2', 'agent': 'Agent2'}, {'user_id': 'user_3', 'run_id': 'run_3', 'agent': 'Agent3'}]
        with patch.object(type(self.bridge), 'websocket_manager', new_callable=PropertyMock) as mock_ws_property:
            mock_ws_property.return_value = self.mock_websocket_manager
            tasks = []
            for user in users:
                tasks.extend([self.bridge.notify_agent_started(run_id=user['run_id'], agent_name=user['agent'], context={'user_id': user['user_id'], 'operation': f"analysis_{user['user_id']}"}), self.bridge.notify_agent_thinking(run_id=user['run_id'], agent_name=user['agent'], reasoning=f"Processing for {user['user_id']}"), self.bridge.notify_agent_completed(run_id=user['run_id'], agent_name=user['agent'], result={'user_specific_data': user['user_id']})])
            ...
            for user in users:
                ...
                for msg in user_messages:
                    if 'context' in msg['data']:
                        assert msg['data']['context']['user_id'] == user['user_id'], 'User context must be isolated'
                    elif 'result' in msg['data']:
                        assert msg['data']['result']['user_specific_data'] == user['user_id'], 'User results must be isolated'
    ...
    async def test_agent_state_recovery_during_partial_execution_failure(self):
        ...
        with patch.object(type(self.bridge), 'websocket_manager', new_callable=PropertyMock) as mock_ws_property:
            mock_ws_property.return_value = self.mock_websocket_manager
            result1 = await self.bridge.notify_agent_started(run_id=run_id, agent_name=agent_name, context={'operation': 'complex_analysis'})
            assert result1, 'Agent should start successfully'
            result2 = await self.bridge.notify_agent_thinking(run_id=run_id, agent_name=agent_name, reasoning='Processing complex calculations')
            assert result2, 'Agent thinking should work'
            result3 = await self.bridge.notify_agent_error(run_id=run_id, agent_name=agent_name, error='Tool execution failed: memory exhausted', error_context={'tool': 'data_analyzer', 'memory_used': '8GB'})
            assert result3, 'Agent error notification should work'
            result4 = await self.bridge.notify_agent_started(run_id=run_id, agent_name=agent_name, context={'operation': 'complex_analysis', 'recovery_attempt': True})
            assert result4, 'Agent recovery start should work'
            result5 = await self.bridge.notify_agent_completed(run_id=run_id, agent_name=agent_name, result={'status': 'recovered', 'analysis': 'completed'})
            ...
            assert message_sequence == expected_sequence, f'Recovery sequence should be preserved: expected {expected_sequence}, got {message_sequence}'
            recovery_start = [msg for msg in run_messages if msg['event_type'] == 'agent_started' and 'recovery_attempt' in str(msg['data'])]
    ...
    async def test_system_startup_message_state_initialization(self):
        ...
        mock_manager.emit_to_run.side_effect = [Exception('WebSocket manager not ready'), True, True]
        with patch.object(type(startup_bridge), 'websocket_manager', new_callable=PropertyMock) as mock_ws_property:
            ...
            result1 = await startup_bridge.notify_agent_started(run_id='startup_run', agent_name='StartupAgent', context={'phase': 'system_startup'})
            startup_bridge.state = IntegrationState.ACTIVE
            result2 = await startup_bridge.notify_agent_thinking(run_id='startup_run', agent_name='StartupAgent', reasoning='System now active, processing normally')
            result3 = await startup_bridge.notify_agent_completed(run_id='startup_run', agent_name='StartupAgent', result={'startup_handling': 'success'})
    ...
    async def test_message_state_consistency_during_high_frequency_transitions(self):
        ...
        with patch.object(type(self.bridge), 'websocket_manager', new_callable=PropertyMock) as mock_ws_property:
            ...
            rapid_tasks = []
            for i in range(3):
                rapid_tasks.append(self.bridge.notify_agent_started(run_id=f'{run_id}_{i}', agent_name=f'{agent_name}_{i}', context={'batch_id': 'high_freq_batch', 'agent_index': i}))
            for i in range(3):
                for step in range(5):
                    rapid_tasks.append(self.bridge.notify_agent_thinking(run_id=f'{run_id}_{i}', agent_name=f'{agent_name}_{i}', reasoning=f'Rapid processing step {step}', step_number=step))
            for i in range(3):
                rapid_tasks.append(self.bridge.notify_agent_completed(run_id=f'{run_id}_{i}', agent_name=f'{agent_name}_{i}', result={'batch_result': f'agent_{i}_completed'}))
    ...
    async def test_message_state_preservation_during_error_cascade(self):
        ...
        failure_sequence = [True, Exception('Network error'), Exception('Memory error'), True, Exception('Database error'), True]
        mock_manager = AsyncMock()
        ...
        with patch.object(self.bridge, '_get_websocket_manager', return_value=mock_manager):
            result1 = await self.bridge.notify_agent_started(run_id=run_id, agent_name=agent_name, context={'test_scenario': 'error_cascade'})
            result2 = await self.bridge.notify_agent_thinking(run_id=run_id, agent_name=agent_name, reasoning='Processing before network failure')
            result3 = await self.bridge.notify_agent_error(run_id=run_id, agent_name=agent_name, error='First error: processing failed', error_context={'error_type': 'processing'})
            result4 = await self.bridge.notify_agent_error(run_id=run_id, agent_name=agent_name, error='Second error: system overload', error_context={'error_type': 'system'})
            result5 = await self.bridge.notify_agent_death(run_id=run_id, agent_name=agent_name, death_cause='Cascading system failure', death_context={'cascade_count': 3})
            result6 = await self.bridge.notify_agent_death(run_id=run_id, agent_name=agent_name, death_cause='Final system shutdown', death_context={'cascade_count': 4, 'final': True})
