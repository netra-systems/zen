...
def multiple_test_users():
    ...
    for i in range(15):
        ...
        context = UserExecutionContext(user_id=user_id, request_id=f'request_{i}_{uuid.uuid4().hex[:6]}', thread_id=f'thread_{i}_{uuid.uuid4().hex[:6]}', run_id=f'run_{i}_{uuid.uuid4().hex[:6]}')
...
class TestUserAgentSessionComplete(SSotBaseTestCase):
    ...
    def test_user_session_initialization_validates_user_id(self):
        ...
        invalid_inputs = [('', 'user_id must be a non-empty string'), (None, 'user_id must be a non-empty string'), (123, 'user_id must be a non-empty string'), ([], 'user_id must be a non-empty string'), ({}, 'user_id must be a non-empty string'), ('   ', 'user_id must be a non-empty string')]
    ...
    async def test_user_session_websocket_manager_without_context(self, test_user_id):
        ...
        with patch('netra_backend.app.services.agent_websocket_bridge.create_agent_websocket_bridge') as mock_create_bridge:
            with patch('netra_backend.app.services.user_execution_context.UserExecutionContext') as mock_context_class:
                ...
                mock_context_instance.user_id = test_user_id
                mock_create_bridge.return_value = mock_bridge
...
class TestAgentLifecycleManagerComplete(SSotBaseTestCase):
    ...
    async def test_monitor_memory_usage_healthy_session(self, test_user_id):
        ...
        mock_registry = Mock()
        mock_user_session = Mock()
        metrics = {'user_id': test_user_id, 'agent_count': 10, 'uptime_seconds': 3600}
    ...
    async def test_monitor_memory_usage_detects_issues(self, test_user_id):
        ...
        mock_registry = Mock()
        mock_user_session = Mock()
        metrics = {'user_id': test_user_id, 'agent_count': 60, 'uptime_seconds': 90000}
        mock_user_session.get_metrics = Mock(return_value=metrics)
        mock_registry._user_sessions = {test_user_id: mock_user_session}
        manager = AgentLifecycleManager(registry=mock_registry)
        result = await manager.monitor_memory_usage(test_user_id)
...
class TestAgentRegistryComplete(SSotBaseTestCase):
    ...
    @pytest.mark.asyncio
    async def test_get_user_session_with_websocket_manager_propagation(self, mock_llm_manager, test_user_id, mock_websocket_manager):
    ...
    @pytest.mark.asyncio
    async def test_create_agent_for_user_comprehensive(self, mock_llm_manager, test_user_context, mock_websocket_manager):
    ...
    @pytest.mark.asyncio
    async def test_websocket_manager_integration_async_method(self, mock_llm_manager, mock_websocket_manager, test_user_id):
...
class TestToolDispatcherIntegration(SSotBaseTestCase):
    ...
    async def test_create_tool_dispatcher_for_user(self, mock_llm_manager, test_user_context):
        ...
        with patch('netra_backend.app.core.tools.unified_tool_dispatcher.UnifiedToolDispatcher') as mock_dispatcher_class:
            ...
            result = await registry.create_tool_dispatcher_for_user(user_context=test_user_context, websocket_bridge=None, enable_admin_tools=False)
...
class TestConcurrentUserScenarios(SSotBaseTestCase):
    ...
    async def test_memory_pressure_monitoring_concurrent_users(self, mock_llm_manager, multiple_test_users):
        ...
        registry = AgentRegistry(mock_llm_manager)
        extended_users = multiple_test_users + [UserExecutionContext(user_id=f'pressure_user_{i}', request_id=f'pressure_req_{i}', thread_id=f'pressure_thread_{i}', run_id=f'pressure_run_{i}') for i in range(50)]
        ...
        for user_context in extended_users[:5]:
...
class TestAgentFactoryRegistration(SSotBaseTestCase):
    ...
    async def test_register_agent_safely_success_scenario(self, mock_llm_manager):
        ...
        with patch.object(registry, 'create_tool_dispatcher_for_user') as mock_create_dispatcher:
            ...
            result = await registry.register_agent_safely(name='test_agent', agent_class=mock_agent_class, test_param='test_value')
            assert result is True
            assert 'test_agent' not in registry.registration_errors
            mock_create_dispatcher.assert_called_once()
            mock_agent_class.assert_called_once_with(registry.llm_manager, mock_dispatcher, test_param='test_value')
    ...
    async def test_register_agent_safely_failure_scenario(self, mock_llm_manager):
        ...
        result = await registry.register_agent_safely(name='failing_agent', agent_class=mock_agent_class)
        assert result is False
...
class TestRegistryHealthAndDiagnostics(SSotBaseTestCase):
    ...
    def test_get_registry_health_comprehensive_metrics(self, mock_llm_manager):
        ...
        expected_keys = ['total_agents', 'failed_registrations', 'registration_errors', 'death_detection_enabled', 'using_universal_registry', 'hardened_isolation', 'total_user_sessions', 'total_user_agents', 'memory_leak_prevention', 'thread_safe_concurrent_execution', 'uptime_seconds', 'issues']
    ...
    async def test_diagnose_websocket_wiring_comprehensive(self, mock_llm_manager, mock_websocket_manager):
        ...
        expected_keys = ['registry_has_websocket_bridge', 'registry_has_websocket_manager', 'total_agents', 'using_universal_registry', 'total_user_sessions', 'users_with_websocket_bridges', 'critical_issues', 'user_details', 'websocket_health']
        for key in expected_keys:
    ...
    def test_get_factory_integration_status_complete_metrics(self, mock_llm_manager):
        ...
        expected_keys = ['using_universal_registry', 'factory_patterns_enabled', 'total_factories', 'thread_safe', 'metrics_enabled', 'hardened_isolation_enabled', 'user_isolation_enforced', 'memory_leak_prevention', 'thread_safe_concurrent_execution', 'total_user_sessions', 'global_state_eliminated', 'websocket_isolation_per_user', 'timestamp']
        for key in expected_keys:
...
class TestBackwardCompatibilityAndLegacySupport(SSotBaseTestCase):
    ...
    async def test_reset_all_agents_returns_success_report(self, mock_llm_manager):
        ...
        expected_keys = ['total_agents', 'successful_resets', 'failed_resets', 'agents_without_reset', 'reset_details', 'using_universal_registry']
        for key in expected_keys:
...
class TestMemoryLeakPreventionAndResourceManagement(SSotBaseTestCase):
    ...
    async def test_monitor_all_users_comprehensive_report(self, mock_llm_manager, multiple_test_users):
        ...
        assert monitoring_report['total_agents'] == sum(range(1, 11))
...
class TestErrorHandlingAndEdgeCases(SSotBaseTestCase):
    ...
    async def test_agent_creation_with_invalid_context_data(self, mock_llm_manager):
        ...
        registry = AgentRegistry(mock_llm_manager)
        invalid_contexts = [None, 'not_a_context', {'user_id': 'test', 'invalid': True}]
        for invalid_context in invalid_contexts:
            if invalid_context is None or isinstance(invalid_context, str):
                with pytest.raises((ValueError, TypeError, AttributeError)):
                    await registry.create_agent_for_user(user_id='test_user', agent_type='test_agent', user_context=invalid_context)
    ...
    async def test_registry_with_extremely_high_user_load(self, mock_llm_manager):
        ...
        user_contexts = []
        for i in range(user_count):
            context = UserExecutionContext(user_id=f'load_test_user_{i}', request_id=f'req_{i}', thread_id=f'thread_{i}', run_id=f'run_{i}')
