...
import random
from netra_backend.app.websocket_core.connection_state_machine import ApplicationConnectionState, ConnectionStateMachine, StateTransitionError
...
class WebSocketCloudRunRaceConditionsTests:
    ...
    async def test_gcp_load_balancer_routing_race_condition(self):
        ...
        """
        race_condition = CloudRunRaceCondition(test_name='gcp_load_balancer_routing_race', cloud_run_phase='routing', timing_issue='Load balancer delay vs application readiness', start_time=time.time())
        ...
        try:
            ...
            app_processing_task = asyncio.create_task(self._simulate_immediate_application_processing(websocket_mock, race_condition))
            load_balancer_delay = random.uniform(0.1, 0.5)
            ...
            handshake_task = asyncio.create_task(self._simulate_delayed_websocket_handshake(websocket_mock, race_condition))
            results = await asyncio.wait_for(asyncio.gather(app_processing_task, handshake_task, return_exceptions=True), timeout=2.0)
        ...
        assert not race_condition.success, f'RACE CONDITION NOT DETECTED: Application processing succeeded despite load balancer delay. This indicates the race condition is not being properly reproduced or has been fixed. Details: {race_condition.error_details}'
    ...
    async def test_cloud_run_container_startup_service_discovery_race(self):
        ...
        """
        race_condition = CloudRunRaceCondition(test_name='container_startup_service_discovery_race', cloud_run_phase='scaling', timing_issue='Service discovery lag during container startup', start_time=time.time())
        ...
        try:
            ...
            startup_sequence = [(ApplicationConnectionState.ACCEPTED, 'WebSocket accepted'), (ApplicationConnectionState.AUTHENTICATED, 'User authenticated'), (ApplicationConnectionState.SERVICES_READY, 'Services initializing...')]
            for target_state, reason in startup_sequence:
                if target_state == ApplicationConnectionState.SERVICES_READY:
                    service_discovery_delay = random.uniform(0.2, 1.0)
                    ...
                    services_ready = await self._check_services_availability_during_startup(race_condition, service_discovery_delay)
            ...
            race_condition.end_time = time.time()
            self.race_conditions.append(race_condition)
        assert not race_condition.success, f'STARTUP RACE CONDITION NOT DETECTED: Services were ready during container startup. This indicates either the race condition is not being reproduced properly, or startup coordination has been implemented. Details: {race_condition.error_details}'
    ...
    async def test_cloud_run_auto_scaling_concurrent_connection_race(self):
        ...
        """
        race_condition = CloudRunRaceCondition(test_name='auto_scaling_concurrent_connection_race', cloud_run_phase='scaling', timing_issue='Concurrent container startup with multiple connections', start_time=time.time(), concurrent_operations=4)
        ...
        try:
            ...
            for container_idx in range(num_containers):
                task = asyncio.create_task(self._simulate_container_with_concurrent_connections(container_idx, connections_per_container, race_condition))
                container_tasks.append(task)
                container_startup_delay = random.uniform(0.05, 0.15)
                race_condition.infrastructure_delays.append(container_startup_delay)
                await asyncio.sleep(container_startup_delay)
            results = await asyncio.wait_for(asyncio.gather(*container_tasks, return_exceptions=True), timeout=5.0)
            ...
            if failed_containers or state_conflicts > 0:
                race_condition.success = False
                race_condition.error_details = f'Race conditions detected: {len(failed_containers)} failed containers, {state_conflicts} state conflicts'
                logger.info(f' PASS:  Auto-scaling race conditions detected: {race_condition.error_details}')
            else:
                race_condition.success = True
                race_condition.error_details = f'NO RACE CONDITION: All {num_containers} containers succeeded without conflicts'
        except Exception as e:
            race_condition.success = False
            ...
            race_condition.end_time = time.time()
            self.race_conditions.append(race_condition)
        assert not race_condition.success, f'AUTO-SCALING RACE CONDITION NOT DETECTED: All containers succeeded without conflicts. This indicates auto-scaling coordination is working properly or race conditions are not being triggered. Details: {race_condition.error_details}'
    ...
    async def test_cloud_run_network_partition_message_ordering_race(self):
        ...
        """
        race_condition = CloudRunRaceCondition(test_name='network_partition_message_ordering_race', cloud_run_phase='processing', timing_issue='Network delays causing message arrival during state transitions', start_time=time.time())
        ...
        try:
            ...
            state_machine = ConnectionStateMachine(connection_id, user_id)
            messages = [{'type': 'user_message', 'text': 'Message 1', 'sequence': 1}, {'type': 'user_message', 'text': 'Message 2', 'sequence': 2}, {'type': 'user_message', 'text': 'Message 3', 'sequence': 3}]
            transition_task = asyncio.create_task(self._simulate_slow_state_transitions(state_machine, race_condition))
            message_task = asyncio.create_task(self._simulate_messages_during_network_partition(messages, race_condition))
            ...
            if isinstance(transition_result, Exception) or isinstance(message_result, Exception):
                race_condition.success = False
                race_condition.error_details = f'Transition: {transition_result}, Messages: {message_result}'
            ...
            race_condition.end_time = time.time()
            self.race_conditions.append(race_condition)
        assert not race_condition.success, f'NETWORK PARTITION RACE CONDITION NOT DETECTED: Operations completed without issues. This indicates message ordering is properly coordinated or race conditions are not being triggered. Details: {race_condition.error_details}'
    ...
    async def _check_services_availability_during_startup(self, race_condition: CloudRunRaceCondition, delay: float) -> bool:
        ...
        supervisor_available = random.random() > 0.7
        thread_service_available = random.random() > 0.6
    ...
    async def _simulate_container_with_concurrent_connections(self, container_idx: int, num_connections: int, race_condition: CloudRunRaceCondition) -> Dict[str, Any]:
        ...
        try:
            ...
            for conn_idx in range(num_connections):
                ...
                task = asyncio.create_task(self._create_connection_in_container(connection_id, user_id, container_idx))
            ...
            container_result = {'container_idx': container_idx, 'connections': successful_connections, 'failures': failed_connections, 'success': len(failed_connections) == 0}
    ...
    async def _create_connection_in_container(self, connection_id: str, user_id: str, container_idx: int) -> Dict[str, Any]:
        ...
        try:
            ...
            state_machine = ConnectionStateMachine(connection_id, user_id)
            states = [ApplicationConnectionState.ACCEPTED, ApplicationConnectionState.AUTHENTICATED, ApplicationConnectionState.SERVICES_READY]
            for state in states:
            ...
            return {'connection_id': connection_id, 'user_id': user_id, 'container_idx': container_idx, 'final_state': state_machine.current_state.value, 'success': True}
        except Exception as e:
            logger.info(f'Connection creation failed in container {container_idx}: {e}')
            raise

    async def _simulate_slow_state_transitions(self, state_machine: ConnectionStateMachine, race_condition: CloudRunRaceCondition):
        """Simulate slow state transitions during network issues."""
        try:
            states = [(ApplicationConnectionState.ACCEPTED, 'Accept with network delay'), (ApplicationConnectionState.AUTHENTICATED, 'Auth with network delay'), (ApplicationConnectionState.SERVICES_READY, 'Services with network delay')]
    ...
    async def _simulate_messages_during_network_partition(self, messages: List[Dict[str, Any]], race_condition: CloudRunRaceCondition):
        ...
        try:
            ...
            for message in messages:
                ...
                processed_messages.append({'sequence': message['sequence'], 'processed_at': time.time(), 'delay': partition_delay})
            ...
            raise
pytestmark = [pytest.mark.race_condition, pytest.mark.cloud_run, pytest.mark.critical, pytest.mark.xfail(reason='Cloud Run race condition tests designed to FAIL until proper coordination is implemented')]
