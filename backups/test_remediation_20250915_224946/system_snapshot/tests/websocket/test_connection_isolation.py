...
class WebSocketIsolationValidator:
    ...
    async def create_isolated_connection(self, user_id: str, token: str, base_url: str='ws://localhost:8000') -> str:
        ...
        try:
            async for message in websocket:
                ...
                with self.event_lock:
                    event_with_metadata = {**event, 'received_by_connection': connection_id, 'received_by_user': user_id, 'received_at': datetime.now(timezone.utc).isoformat()}
                    self.received_events[connection_id].append(event_with_metadata)
                    event_user_id = event.get('user_id')
                    if event_user_id and event_user_id != user_id:
                        violation = {'violation_type': 'cross_user_event_leakage', 'event': event, 'intended_user': event_user_id, 'received_by_user': user_id, 'received_by_connection': connection_id, 'timestamp': datetime.now(timezone.utc).isoformat()}
                        self.isolation_violations.append(violation)
                        print(f' ALERT:  ISOLATION VIOLATION: Event for user {event_user_id} received by user {user_id}')
        except ConnectionClosed:
            print(f'[U+1F50C] Connection {connection_id} closed')
        except Exception as e:
            print(f' FAIL:  Error listening on connection {connection_id}: {e}')

    async def send_user_specific_event(self, target_user_id: str, event_type: str, payload: Dict[str, Any]) -> bool:
        ...
        if not target_connection:
            ...
            return False
        event = {'type': event_type, 'user_id': target_user_id, 'payload': payload, 'timestamp': datetime.now(timezone.utc).isoformat(), 'test_event': True}
        try:
    ...
    async def trigger_agent_execution(self, user_id: str, agent_name: str) -> Optional[str]:
        ...
        agent_request = {'type': 'agent_execute', 'user_id': user_id, 'agent_name': agent_name, 'run_id': run_id, 'data': {'request': f'Test request from user {user_id}', 'isolation_test': True}, 'timestamp': datetime.now(timezone.utc).isoformat()}
        try:
            websocket = connection['websocket']
...
class WebSocketConnectionIsolationTests(RealWebSocketTestBase):
    ...
    def test_users(self):
        ...
        return [{'user_id': f'user_{i}', 'token': f'test_token_{i}'} for i in range(1, 6)]
    ...
    async def test_no_cross_user_event_leakage(self, isolation_validator, test_users):
        ...
        for user in test_users:
            conn_id = await isolation_validator.create_isolated_connection(user['user_id'], user['token'])
            connections.append(conn_id)
        await asyncio.sleep(2)
        target_user = test_users[0]['user_id']
        await isolation_validator.send_user_specific_event(target_user, 'test_private_event', {'message': f'Private message for {target_user}', 'confidential': True})
        ...
        assert len(violations) == 0, f'CRITICAL: Found {len(violations)} isolation violations! Events leaked between users: {violations}'
        ...
        for user in test_users[1:]:
            ...
            leaked_events = [event for event in user_events if event.get('user_id') == target_user or target_user in str(event.get('payload', {}))]
            assert len(leaked_events) == 0, f"ISOLATION VIOLATION: User {user['user_id']} received {len(leaked_events)} events intended for {target_user}: {leaked_events}"
    ...
    async def test_concurrent_agent_executions_isolated(self, isolation_validator, test_users):
        ...
        for user in test_users:
            await isolation_validator.create_isolated_connection(user['user_id'], user['token'])
        await asyncio.sleep(2)
        agent_tasks = []
        run_ids = {}
        for user in test_users:
        ...
        violations = isolation_validator.get_isolation_violations()
        assert len(violations) == 0, f'CONCURRENT ISOLATION VIOLATION: Found {len(violations)} violations during concurrent agent execution: {violations}'
        for user in test_users:
            user_id = user['user_id']
            user_events = isolation_validator.get_events_for_user(user_id)
            agent_events = [event for event in user_events if event.get('type', '').startswith(('agent_', 'tool_'))]
            if user_id in run_ids:
                expected_run_id = run_ids[user_id]
                for event in agent_events:
                    event_run_id = event.get('run_id') or event.get('payload', {}).get('run_id')
                    assert event_run_id == expected_run_id, f'ISOLATION VIOLATION: User {user_id} received agent event for run_id {event_run_id}, expected {expected_run_id}'
    ...
    async def test_connection_cleanup_isolation(self, isolation_validator, test_users):
        ...
        for user in test_users[1:]:
            user_id = user['user_id']
            await isolation_validator.send_user_specific_event(user_id, 'cleanup_test_event', {'message': f'Testing connection after user 1 cleanup'})
        await asyncio.sleep(2)
        for user in test_users[1:]:
            user_id = user['user_id']
            user_events = isolation_validator.get_events_for_user(user_id)
            cleanup_events = [event for event in user_events if event.get('type') == 'cleanup_test_event']
            assert len(cleanup_events) > 0, f"User {user_id} didn't receive cleanup test event - connection may have been affected by user 1 cleanup"
        violations = isolation_validator.get_isolation_violations()
        assert len(violations) == 0, f'Connection cleanup caused isolation violations: {violations}'
    ...
    async def test_event_filtering_by_user_id(self, isolation_validator, test_users):
        ...
        for i, user in enumerate(active_users):
            ...
            malicious_event = {'type': 'malicious_event', 'user_id': other_user_id, 'payload': {'message': f'Trying to impersonate {other_user_id}', 'sent_through_connection': user_id}, 'timestamp': datetime.now(timezone.utc).isoformat()}
            try:
        ...
        for user in active_users:
            user_id = user['user_id']
            user_events = isolation_validator.get_events_for_user(user_id)
            malicious_events = [event for event in user_events if event.get('type') == 'malicious_event' and event.get('payload', {}).get('sent_through_connection') != user_id]
            assert len(malicious_events) == 0, f'SECURITY VIOLATION: User {user_id} received {len(malicious_events)} malicious events intended for other users: {malicious_events}'
