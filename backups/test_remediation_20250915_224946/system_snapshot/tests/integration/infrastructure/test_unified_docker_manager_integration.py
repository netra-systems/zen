...
from test_framework.unified_docker_manager import UnifiedDockerManager, EnvironmentType, ServiceMode, ServiceStatus, ServiceHealth, OrchestrationConfig, ContainerInfo
from test_framework.resource_monitor import DockerResourceMonitor, ResourceThresholdLevel, DockerResourceSnapshot
...
class UnifiedDockerManagerOrchestrationTests(SSotBaseTestCase):
    ...
    def setup_method(self, method):
        ...
        self.docker_manager = UnifiedDockerManager(config=self.config, environment_type=EnvironmentType.DEDICATED, test_id=self.test_id, use_production_images=True, mode=ServiceMode.DOCKER, use_alpine=True, rebuild_images=False, rebuild_backend_only=True, pull_policy='missing')
    ...
    async def _cleanup_services(self):
        ...
        try:
            await self.docker_manager.graceful_shutdown(services=list(self.started_services), timeout=30)
        except Exception as e:
            logger.warning(f'Graceful shutdown failed, forcing: {e}')
            await self.docker_manager.force_shutdown(services=list(self.started_services))
    ...
    async def test_service_startup_coordination_with_dependencies(self):
        ...
        success = await self.docker_manager.start_services_smart(services=database_services, wait_healthy=True)
        ...
        success = await self.docker_manager.start_services_smart(services=app_services, wait_healthy=True)
    ...
    async def test_cross_service_communication_validation(self):
        ...
        for service in services:
            ...
            if service_mapping:
                ...
                connectivity = await self.docker_manager._check_port_connectivity(host, port, timeout=5.0)
    ...
    def test_health_monitoring_real_time_updates(self):
        ...
        for i in range(3):
            ...
            time.sleep(1)
        assert all((status in [ServiceStatus.RUNNING, ServiceStatus.HEALTHY] for status in health_checks)), 'Service health should remain stable'
        health_report = self.docker_manager.get_health_report()
    ...
    def test_service_failure_detection_and_recovery(self):
        ...
        try:
            result = self.docker_manager._execute_command(['docker', 'stop', f'netra-{service_name}'], capture_output=True)
            time.sleep(2)
...
class UnifiedDockerManagerResourceManagementTests(SSotBaseTestCase):
    ...
    def setup_method(self, method):
        ...
        self.docker_manager = UnifiedDockerManager(environment_type=EnvironmentType.DEDICATED, test_id=self.test_id, use_production_images=True, mode=ServiceMode.DOCKER)
        self.resource_monitor = DockerResourceMonitor(enable_auto_cleanup=True, cleanup_aggressive=False)
    ...
    async def test_memory_limit_enforcement_under_load(self):
        ...
        try:
            result = self.docker_manager._execute_command(['docker', 'stats', container_name, '--no-stream', '--format', 'table {{.MemUsage}}\t{{.MemPerc}}'], capture_output=True)
    ...
    def test_disk_space_management_and_cleanup(self):
        ...
        self.started_services.add(service_name)
        try:
            result = self.docker_manager._execute_command(['docker', 'system', 'df'], capture_output=True)
            if result.returncode == 0:
                disk_info = result.stdout
                assert disk_info, 'Should get Docker disk usage information'
                cleanup_result = self.docker_manager._execute_command(['docker', 'system', 'prune', '-f', '--volumes'], capture_output=True)
    ...
    def test_cpu_resource_monitoring(self):
        ...
        container_name = f'netra-{service_name}'
        try:
            result = self.docker_manager._execute_command(['docker', 'stats', container_name, '--no-stream', '--format', 'table {{.CPUPerc}}'], capture_output=True)
            if result.returncode == 0:
...
class UnifiedDockerManagerCrossPlatformTests(SSotBaseTestCase):
    ...
    def setup_method(self, method):
        ...
        self.docker_manager = UnifiedDockerManager(environment_type=EnvironmentType.DEDICATED, test_id=self.test_id, mode=ServiceMode.DOCKER)
        self.started_services: Set[str] = set()
        if not self._is_docker_available():
    ...
    def test_linux_container_orchestration(self):
        ...
        if hasattr(self.docker_manager, '_unix_socket_path'):
            pass
        container_name = f'netra-{service_name}'
        result = self.docker_manager._execute_command(['docker', 'inspect', container_name, '--format', '{{.NetworkSettings.IPAddress}}'], capture_output=True)
        if result.returncode == 0:
    ...
    def test_file_system_permission_handling(self):
        ...
        container_name = f'netra-{service_name}'
        result = self.docker_manager._execute_command(['docker', 'inspect', container_name, '--format', '{{.Mounts}}'], capture_output=True)
        if result.returncode == 0:
...
class UnifiedDockerManagerEnvironmentIsolationTests(SSotBaseTestCase):
    ...
    def setup_method(self, method):
        ...
        self.docker_manager_1 = UnifiedDockerManager(environment_type=EnvironmentType.DEDICATED, test_id=self.test_id_1, mode=ServiceMode.DOCKER)
        self.docker_manager_2 = UnifiedDockerManager(environment_type=EnvironmentType.DEDICATED, test_id=self.test_id_2, mode=ServiceMode.DOCKER)
    ...
    async def _cleanup_services(self, docker_manager, services):
        ...
        try:
            await docker_manager.graceful_shutdown(services=list(services), timeout=30)
        except Exception as e:
    ...
    def test_environment_variable_management(self):
        ...
        assert success_2
        self.started_services_2.add(service_name)
        credentials_1 = self.docker_manager_1.ENVIRONMENT_CREDENTIALS[self.docker_manager_1.environment_type]
        credentials_2 = self.docker_manager_2.ENVIRONMENT_CREDENTIALS[self.docker_manager_2.environment_type]
    ...
    async def test_container_cleanup_and_resource_reclaim(self):
        ...
        assert success
        self.started_services_1.add(service_name)
        result_before = self.docker_manager_1._execute_command(['docker', 'ps', '-a', '--format', '{{.Names}}'], capture_output=True)
        ...
        result_after = self.docker_manager_1._execute_command(['docker', 'ps', '-a', '--format', '{{.Names}}'], capture_output=True)
        containers_after = set()
        if result_after.returncode == 0:
            containers_after = set(result_after.stdout.strip().split('\n'))
        container_name = f'netra-{service_name}'
        running_result = self.docker_manager_1._execute_command(['docker', 'ps', '--format', '{{.Names}}', '--filter', f'name={container_name}'], capture_output=True)
...
class UnifiedDockerManagerCIPipelineTests(SSotBaseTestCase):
    ...
    def setup_method(self, method):
        ...
        self.docker_manager = UnifiedDockerManager(environment_type=EnvironmentType.DEDICATED, test_id=self.test_id, use_production_images=True, mode=ServiceMode.DOCKER, use_alpine=True, rebuild_images=False, pull_policy='missing')
        self.started_services: Set[str] = set()
        if not self._is_docker_available():
    ...
    async def test_automated_service_orchestration(self):
        ...
        success = await self.docker_manager.start_services_smart(services=services, wait_healthy=True)
    ...
    def test_build_environment_consistency(self):
        ...
        self.started_services.add(service_name)
        container_name = f'netra-{service_name}'
        result = self.docker_manager._execute_command(['docker', 'inspect', container_name, '--format', '{{.Config.Env}}'], capture_output=True)
        if result.returncode == 0:
        ...
        image_result = self.docker_manager._execute_command(['docker', 'inspect', container_name, '--format', '{{.Config.Image}}'], capture_output=True)
    ...
    async def test_test_environment_provisioning(self):
        ...
        start_time = time.time()
        success = await self.docker_manager.ensure_services_running(services=test_services, timeout=120)
    ...
    async def test_ci_pipeline_failure_recovery(self):
        ...
        recovery_success = await self.docker_manager.start_services_smart([service_name], wait_healthy=True)
    ...
    def test_ci_resource_optimization(self):
        ...
        for service in services:
            status = self.docker_manager.get_service_status(service)
            assert status in [ServiceStatus.RUNNING, ServiceStatus.HEALTHY]
pytest_markers = ['integration', 'infrastructure', 'docker', 'no_mocks', 'real_services']
