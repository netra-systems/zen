...
class ImprovedReadinessVerifier:
    ...
    def __init__(self):
        self.readiness_configs = {'auth_service': ReadinessConfiguration(service_name='auth_service', base_url=get_env().get('AUTH_SERVICE_URL', 'http://localhost:8081'), readiness_endpoints=['/health', '/health/ready', '/status'], dependencies=['postgres'], required_checks=['endpoint_responsive', 'database_connected'], optional_checks=['oauth_configured']), 'backend': ReadinessConfiguration(service_name='backend', base_url=get_env().get('BACKEND_URL', 'http://localhost:8000'), readiness_endpoints=['/health', '/health/ready', '/api/health'], dependencies=['postgres', 'redis', 'auth_service'], required_checks=['endpoint_responsive', 'database_connected'], optional_checks=['llm_providers_available', 'websocket_ready']), 'frontend': ReadinessConfiguration(service_name='frontend', base_url=get_env().get('FRONTEND_URL', 'http://localhost:3000'), readiness_endpoints=['/health', '/api/health', '/'], dependencies=['backend'], required_checks=['endpoint_responsive'], optional_checks=['static_assets_loaded'])}
        self.check_history: Dict[str, List[ReadinessCheckResult]] = {}

    async def perform_readiness_check(self, service_name: str, config: Optional[ReadinessConfiguration]=None) -> ReadinessCheckResult:
        """Perform comprehensive readiness check for a service."""
        config = config or self.readiness_configs.get(service_name)
        if not config:
            return ReadinessCheckResult(service_name=service_name, state=ReadinessState.UNKNOWN, response_time=0.0, timestamp=time.time(), success=False, error='No configuration found for service')
        start_time = time.time()
        result = ReadinessCheckResult(service_name=service_name, state=ReadinessState.INITIALIZING, response_time=0.0, timestamp=start_time, success=False, checks_performed=[])
        try:
                ...
                try:
                    async with httpx.AsyncClient(timeout=config.timeout) as client:
                        ...
                        if response.status_code < 500:
                            return {'success': True, 'endpoint': endpoint, 'status_code': response.status_code, 'attempt': attempt + 1, 'response_size': len(response.content) if response.content else 0}
                    ...
                    if attempt < config.retry_attempts - 1:
                        await asyncio.sleep(config.retry_delay)
                        continue
        return {'success': False, 'error': f'All endpoints failed after {config.retry_attempts} attempts', 'endpoints_attempted': config.readiness_endpoints}
    ...
    async def _check_service_specific_readiness(self, config: ReadinessConfiguration) -> Optional[Dict[str, Any]]:
        ...
        service_checks = {'auth_service': self._check_auth_service_readiness, 'backend': self._check_backend_service_readiness, 'frontend': self._check_frontend_service_readiness}
        ...
        if check_function:
            try:
                ...
                return {'error': str(e), 'partially_ready': True}
        return None
    ...
    async def _check_auth_service_readiness(self, config: ReadinessConfiguration) -> Dict[str, Any]:
        ...
        checks = {'database_connected': False, 'oauth_configured': False, 'jwt_functionality': False}
        ...
        return {**checks, 'ready': ready, 'partially_ready': any(checks.values())}
    ...
    async def _check_backend_service_readiness(self, config: ReadinessConfiguration) -> Dict[str, Any]:
        ...
        checks = {'database_connected': False, 'redis_connected': False, 'websocket_ready': False, 'auth_integration': False}
        base_url = config.base_url
        try:
        ...
        return {**checks, 'ready': ready, 'partially_ready': partially_ready}
    ...
    async def _check_frontend_service_readiness(self, config: ReadinessConfiguration) -> Dict[str, Any]:
        ...
        checks = {'static_assets_loaded': False, 'backend_connectivity': False, 'routing_working': False}
        base_url = config.base_url
        try:
            async with httpx.AsyncClient(timeout=5.0) as client:
        ...
        return {**checks, 'ready': ready, 'partially_ready': ready}
    ...
    async def _check_dependencies_readiness(self, config: ReadinessConfiguration) -> Dict[str, Any]:
        ...
        return {'dependencies': dependency_status, 'all_dependencies_ready': dependencies_ready, 'critical_dependencies_ready': any(dependency_status.values()) if dependency_status else True}
    ...
    def _determine_readiness_state(self, result: ReadinessCheckResult, config: ReadinessConfiguration) -> ReadinessState:
    ...
    def get_readiness_trends(self, service_name: str) -> Dict[str, Any]:
        ...
        return {'trend': trend, 'recent_success_rate': success_rate, 'total_checks': len(history), 'last_successful_check': next((r.timestamp for r in reversed(history) if r.success), None)}
...
class ReadinessVerificationImprover:
    ...
    def create_resilient_readiness_check(service_name: str, base_url: str) -> Callable:
        ...
        async def resilient_readiness_check() -> Dict[str, Any]:
            ...
            strategies = [{'name': 'fast_health_check', 'endpoints': ['/health/ready', '/ready'], 'timeout': 2.0, 'required': True}, {'name': 'standard_health_check', 'endpoints': ['/health', '/status'], 'timeout': 5.0, 'required': False}, {'name': 'basic_connectivity', 'endpoints': ['/'], 'timeout': 3.0, 'required': False}]
            results = {}
                    ...
                    try:
                        ...
                        async with httpx.AsyncClient(timeout=strategy['timeout']) as client:
                            ...
                            if response.status_code < 500:
                                ...
                                results[strategy['name']] = {'success': True, 'endpoint': endpoint, 'status_code': response.status_code}
                                break
                    except:
                        continue
            ...
            return {'ready': ready, 'service_name': service_name, 'strategy_results': results, 'evaluation': 'required_passed' if required_success else 'optional_passed' if optional_success else 'failed'}
...
class TestServiceReadinessVerificationImprovements:
            ...
            try:
                ...
                async with httpx.AsyncClient(timeout=5.0) as client:
                    ...
                    if response.status_code == 200:
                        ...
                        return {'healthy': True, 'endpoint': endpoint, 'response_time': time.time() - start_time, 'status_code': response.status_code, 'details': health_data}
            except:
                continue
        return {'healthy': False, 'response_time': time.time() - start_time, 'error': 'All health endpoints failed'}
    ...
    async def test_resilient_readiness_check_improvements(self):
        ...
        test_services = [{'name': 'auth_service', 'url': get_env().get('AUTH_SERVICE_URL', 'http://localhost:8081')}, {'name': 'backend', 'url': get_env().get('BACKEND_URL', 'http://localhost:8000')}]
        for service_config in test_services:
            print(f"\nTesting resilient check for {service_config['name']}...")
            resilient_check = ReadinessVerificationImprover.create_resilient_readiness_check(service_config['name'], service_config['url'])
            start_time = time.time()
