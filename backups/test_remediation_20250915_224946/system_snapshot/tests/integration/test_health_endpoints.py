...
class HealthEndpointValidator:
    ...
    async def validate_health_endpoint(self, url: str, timeout: int=30) -> Dict[str, Any]:
        ...
        """
        result = {'url': url, 'accessible': False, 'status_code': None, 'response_time_ms': 0, 'response_data': None, 'headers': {}, 'errors': [], 'cloud_run_compatible': False}
    ...
    async def test_health_endpoint_dependencies(self, base_url: str) -> Dict[str, Any]:
        ...
        """
        dependency_tests = {'base_url': base_url, 'tests_performed': [], 'dependency_failures': [], 'overall_health_resilient': True}
        health_endpoints = ['/health', '/api/health']
        for endpoint in health_endpoints:
            url = f"{base_url.rstrip('/')}{endpoint}"
            normal_result = await self.validate_health_endpoint(url, timeout=10)
            dependency_tests['tests_performed'].append({'endpoint': endpoint, 'test': 'normal_health_check', 'result': normal_result})
            load_results = []
            try:
                tasks = []
                for i in range(5):
                ...
                successful_responses = [r for r in load_results if isinstance(r, dict) and r.get('accessible')]
                if len(successful_responses) < 3:
                    dependency_tests['dependency_failures'].append({'endpoint': endpoint, 'test': 'load_test', 'issue': 'poor_success_rate_under_load', 'success_rate': len(successful_responses) / 5})
                    dependency_tests['overall_health_resilient'] = False
            except Exception as e:
                dependency_tests['dependency_failures'].append({'endpoint': endpoint, 'test': 'load_test', 'issue': f'load_test_exception_{type(e).__name__}', 'error': str(e)})
            dependency_tests['tests_performed'].append({'endpoint': endpoint, 'test': 'load_simulation', 'results': load_results if isinstance(load_results, list) else str(load_results)})
    ...
    def analyze_health_response_format(self, response_data: Any) -> Dict[str, Any]:
        ...
        """
        analysis = {'format_valid': False, 'has_status': False, 'has_timestamp': False, 'has_dependencies': False, 'has_version': False, 'format_issues': [], 'recommendations': []}
...
class HealthEndpointsTests(SSotBaseTestCase):
    ...
    def setup_method(self):
        ...
        self.env = IsolatedEnvironment()
        self.service_urls = {'backend': f"http://localhost:{self.env.get('NETRA_BACKEND_PORT', '8000')}", 'auth': f"http://localhost:{self.env.get('NETRA_AUTH_PORT', '8081')}"}
    ...
    async def test_backend_health_endpoints_accessibility(self):
        ...
        for endpoint, result in endpoint_results.items():
            assert result['accessible'], f"CRITICAL: Backend health endpoint {endpoint} is not accessible\nURL: {result['url']}\nErrors: {result['errors']}"
            assert result['status_code'] == 200, f"CRITICAL: Backend health endpoint {endpoint} returned {result['status_code']}\nExpected: 200\nResponse: {result['response_data']}"
        for endpoint, result in endpoint_results.items():
            assert result['response_time_ms'] < 30000, f"Health endpoint {endpoint} too slow: {result['response_time_ms']}ms\nCloud Run timeout: 30000ms"
    ...
    async def test_auth_service_health_endpoints_accessibility(self):
        ...
        for endpoint, result in endpoint_results.items():
            assert result['accessible'], f"CRITICAL: Auth health endpoint {endpoint} is not accessible\nURL: {result['url']}\nErrors: {result['errors']}"
        successful_endpoints = [ep for ep, res in endpoint_results.items() if res['status_code'] == 200]
        assert len(successful_endpoints) > 0, f"CRITICAL: No auth health endpoints returned 200 status\nResults: {[(ep, res['status_code']) for ep, res in endpoint_results.items()]}"
    ...
    async def test_health_endpoint_response_formats(self):
        ...
        for service, base_url in self.service_urls.items():
            ...
            for endpoint in ['/health', '/api/health']:
                ...
                if health_result['accessible'] and health_result['status_code'] == 200:
                    format_analysis = self.validator.analyze_health_response_format(health_result['response_data'])
                    service_results[endpoint] = {'health_result': health_result, 'format_analysis': format_analysis}
        ...
        assert len(services_with_valid_format) > 0, f'No services have properly formatted health responses\nServices tested: {list(all_results.keys())}\nAll responses need status information for Cloud Run compatibility'
    ...
    async def test_health_endpoint_dependency_resilience(self):
        ...
        resilient_services = [service for service, result in dependency_test_results.items() if result['overall_health_resilient']]
        min_resilient = len(self.service_urls) // 2
        assert len(resilient_services) >= min_resilient, f'Only {len(resilient_services)} services are health-resilient, need at least {min_resilient}\nResilient services: {resilient_services}\nNon-resilient services may cause Cloud Run startup failures'
                ...
                for method in methods_to_test:
                    try:
                        async with httpx.AsyncClient(timeout=10) as client:
                            ...
                            method_results[method] = {'status_code': response.status_code, 'accessible': response.status_code < 500, 'headers': dict(response.headers), 'supports_cors': 'Access-Control-Allow-Origin' in response.headers}
                    except Exception as e:
                        method_results[method] = {'status_code': None, 'accessible': False, 'error': str(e), 'supports_cors': False}
        ...
        for service, endpoints in cors_test_results.items():
            for endpoint, methods in endpoints.items():
                if 'GET' in methods:
                    ...
                    if get_result['accessible']:
                        assert get_result['status_code'] in [200, 503], f"Health endpoint {service}{endpoint} GET method failed\nStatus: {get_result['status_code']}\nCloud Run requires GET method support for health checks"
    ...
    async def test_health_endpoints_startup_probe_simulation(self):
        ...
        startup_config = {'initial_delay_seconds': 0, 'timeout_seconds': 1, 'period_seconds': 1, 'failure_threshold': 30}
        for service, base_url in self.service_urls.items():
            service_startup_results = {'service': service, 'startup_config': startup_config, 'probe_results': [], 'startup_successful': False, 'time_to_healthy_seconds': 0}
            start_time = time.time()
            failures = 0
            max_attempts = startup_config['failure_threshold']
            for attempt in range(max_attempts):
                health_url = f'{base_url}/health'
                probe_result = await self.validator.validate_health_endpoint(health_url, timeout=startup_config['timeout_seconds'])
        ...
        successful_startups = [service for service, result in startup_simulation_results.items() if result['startup_successful']]
        assert len(successful_startups) > 0, f'No services passed startup probe simulation\nServices tested: {list(startup_simulation_results.keys())}\nCloud Run startup probes will fail for all services'
        for service, result in startup_simulation_results.items():
            if result['startup_successful']:
                assert result['time_to_healthy_seconds'] < 30, f"Service {service} takes too long to start: {result['time_to_healthy_seconds']}s\nCloud Run default startup timeout: 30 seconds"
