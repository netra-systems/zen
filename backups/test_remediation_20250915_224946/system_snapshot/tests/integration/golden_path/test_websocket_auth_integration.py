...
class WebSocketAuthIntegrationTests(SSotAsyncTestCase):
    ...
    async def test_jwt_token_validation_with_real_auth_service(self, real_services_fixture):
        ...
        user_context = await create_authenticated_user_context(user_email='auth_test_user@example.com', environment=self.environment, permissions=['read', 'write', 'execute_agents'], websocket_enabled=True)
        start_time = time.time()
        jwt_token = await self.auth_helper.get_staging_token_async(email=user_context.agent_context.get('user_email'))
        ...
        assert auth_headers['Authorization'].startswith('Bearer '), 'Should use Bearer token format'
    ...
    async def test_websocket_connection_with_real_authentication(self, real_services_fixture):
        ...
        user_context = await create_authenticated_user_context(user_email='websocket_auth_test@example.com', environment=self.environment, websocket_enabled=True)
        jwt_token = await self.auth_helper.get_staging_token_async(email=user_context.agent_context.get('user_email'))
        ...
        websocket_connection = await WebSocketTestHelpers.create_test_websocket_connection(url=self.websocket_url, headers=ws_headers, timeout=10.0, user_id=str(user_context.user_id))
        ...
        try:
            test_message = {'type': 'connection_test', 'user_id': str(user_context.user_id), 'timestamp': time.time()}
    ...
    async def test_user_context_creation_with_factory_initialization(self, real_services_fixture):
        ...
        user_contexts = []
        for i in range(3):
            user_context = await create_authenticated_user_context(user_email=f'factory_test_user_{i}@example.com', environment=self.environment, websocket_enabled=True, permissions=['read', 'write', 'execute_agents'])
        ...
        for i, context in enumerate(user_contexts):
            assert isinstance(context, StronglyTypedUserExecutionContext), f'Context {i} should be properly typed'
            ...
            assert 'user_email' in context.agent_context, f'Context {i} missing user email in agent context'
            assert 'permissions' in context.agent_context, f'Context {i} missing permissions in agent context'
    ...
    async def test_authentication_failure_handling(self, real_services_fixture):
        ...
        with pytest.raises(Exception) as exc_info:
            await WebSocketTestHelpers.create_test_websocket_connection(url=self.websocket_url, headers=invalid_headers, timeout=5.0, user_id='test_user')
        ...
        with pytest.raises(Exception):
            await WebSocketTestHelpers.create_test_websocket_connection(url=self.websocket_url, headers=expired_headers, timeout=5.0, user_id='test_user')
        empty_headers = {}
        with pytest.raises(Exception):
            await WebSocketTestHelpers.create_test_websocket_connection(url=self.websocket_url, headers=empty_headers, timeout=5.0, user_id='test_user')
    ...
    async def test_concurrent_websocket_authentication(self, real_services_fixture):
        ...
        for i in range(concurrent_users):
            task = self._create_concurrent_auth_connection(f'concurrent_auth_user_{i}@example.com', i)
        ...
        assert success_rate >= 0.8, f'Should have 80%+ success rate for concurrent auth: {success_rate:.1%}'
        if connection_times:
            avg_connection_time = sum(connection_times) / len(connection_times)
            assert avg_connection_time < 10.0, f'Average connection time should be reasonable: {avg_connection_time:.2f}s'
        assert total_time < 30.0, f'All concurrent auths should complete within 30s: {total_time:.2f}s'
    ...
    async def _create_concurrent_auth_connection(self, user_email: str, user_index: int) -> Dict[str, Any]:
        """Create a concurrent authentication connection for testing."""
        try:
            user_context = await create_authenticated_user_context(user_email=user_email, environment=self.environment, websocket_enabled=True)
            jwt_token = await self.auth_helper.get_staging_token_async(email=user_context.agent_context.get('user_email'))
            ws_headers = self.websocket_helper.get_websocket_headers(jwt_token)
            connection_start = time.time()
            ...
            connection_time = time.time() - connection_start
            await WebSocketTestHelpers.send_test_message(connection, {'type': 'ping', 'user_index': user_index}, timeout=3.0)
            await WebSocketTestHelpers.close_test_connection(connection)
            return {'success': True, 'user_index': user_index, 'connection_time': connection_time, 'user_id': str(user_context.user_id)}
    ...
    async def test_authentication_performance_benchmarks(self, real_services_fixture):
        ...
        performance_metrics = {'token_generation_times': [], 'connection_establishment_times': [], 'total_auth_times': []}
        test_cycles = 10
        for i in range(test_cycles):
            total_start = time.time()
            user_context = await create_authenticated_user_context(user_email=f'perf_test_user_{i}@example.com', environment=self.environment, websocket_enabled=True)
        ...
        assert avg_token_time < 3.0, f'Average token generation should be < 3s: {avg_token_time:.2f}s'
        assert avg_connection_time < 5.0, f'Average connection time should be < 5s: {avg_connection_time:.2f}s'
        assert avg_total_time < 8.0, f'Average total auth time should be < 8s: {avg_total_time:.2f}s'
        assert max_token_time < 5.0, f'Max token generation should be < 5s: {max_token_time:.2f}s'
        assert max_connection_time < 10.0, f'Max connection time should be < 10s: {max_connection_time:.2f}s'
        assert max_total_time < 15.0, f'Max total auth time should be < 15s: {max_total_time:.2f}s'
    ...
    async def test_authentication_with_different_user_permissions(self, real_services_fixture):
        ...
        permission_test_cases = [{'permissions': ['read'], 'description': 'Read-only user'}, {'permissions': ['read', 'write'], 'description': 'Read-write user'}, {'permissions': ['read', 'write', 'execute_agents'], 'description': 'Full permissions user'}, {'permissions': ['read', 'write', 'execute_agents', 'admin'], 'description': 'Admin user'}]
        for i, test_case in enumerate(permission_test_cases):
            permissions = test_case['permissions']
            description = test_case['description']
            user_context = await create_authenticated_user_context(user_email=f'permissions_test_{i}@example.com', environment=self.environment, permissions=permissions, websocket_enabled=True)
            assert user_context.agent_context.get('permissions') == permissions, f'Permissions should match for {description}'
            jwt_token = await self.auth_helper.get_staging_token_async(email=user_context.agent_context.get('user_email'))
            assert jwt_token is not None, f'Should authenticate {description}'
            ws_headers = self.websocket_helper.get_websocket_headers(jwt_token)
            connection = await WebSocketTestHelpers.create_test_websocket_connection(url=self.websocket_url, headers=ws_headers, timeout=10.0, user_id=str(user_context.user_id))
            assert connection is not None, f'Should connect {description}'
            test_message = {'type': 'permission_test', 'permissions': permissions, 'user_type': description}
