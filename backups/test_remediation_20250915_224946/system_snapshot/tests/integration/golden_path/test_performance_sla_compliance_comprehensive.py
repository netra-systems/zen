...
class PerformanceSLAComplianceComprehensiveTests(SSotAsyncTestCase):
    ...
    def setup_method(self, method=None):
        ...
        self.SLA_THRESHOLDS = {'first_response_time': 3.0, 'complete_response_time': 60.0, 'websocket_connection_time': 5.0, 'concurrent_users_supported': 10, 'memory_growth_limit': 100, 'throughput_min_requests': 5, 'availability_threshold': 0.99}
        self.performance_metrics = {'response_times': [], 'connection_times': [], 'memory_usage': [], 'throughput_measurements': [], 'error_rates': [], 'concurrent_performance': []}
    ...
    def _record_final_performance_metrics(self):
        ...
        try:
            if self.performance_metrics['response_times']:
                ...
                self.record_metric('p95_response_time', statistics.quantiles(response_times, n=20)[18])
    ...
    async def test_first_response_time_sla_compliance(self):
        ...
        for test_num in range(5):
            ...
            try:
                user_context = await create_authenticated_user_context(user_email=f'first_response_test_{test_num}@example.com', environment=self.environment, websocket_enabled=True)
                ...
                test_message = {'type': 'chat_message', 'content': f'Performance test {test_num}: Quick response needed', 'user_id': str(user_context.user_id), 'performance_test': True, 'test_number': test_num}
                ...
                while time.time() - response_start < 10.0:
                    try:
                        response = await WebSocketTestHelpers.receive_test_message(connection, timeout=1.0)
                        ...
                        continue
                test_result = {'test_number': test_num, 'success': first_response_received, 'first_response_time': first_response_time, 'connection_time': connection_time, 'sla_compliant': first_response_time is not None and first_response_time <= self.SLA_THRESHOLDS['first_response_time'], 'total_test_time': time.time() - test_start}
                ...
                print(f"   Test {test_num}: {(' PASS: ' if test_result['sla_compliant'] else ' FAIL: ')} ({first_response_time:.2f}s)" if first_response_time else f'   Test {test_num}:  FAIL:  No response')
                await WebSocketTestHelpers.close_test_connection(connection)
                self.active_connections.remove(connection)
            except Exception as test_error:
                response_time_tests.append({'test_number': test_num, 'success': False, 'error': str(test_error), 'sla_compliant': False, 'total_test_time': time.time() - test_start})
        ...
        if sla_compliance_rate < 0.95:
            pytest.fail(f" ALERT:  FIRST RESPONSE SLA FAILURE\nSLA Compliance: {sla_compliance_rate:.1%} (< 95% required)\nSLA Threshold: {self.SLA_THRESHOLDS['first_response_time']}s\nAverage Response Time: {avg_response_time:.2f}s\nThis violates user engagement SLA requirements!")
        if success_rate < 0.9:
            pytest.fail(f' ALERT:  FIRST RESPONSE RELIABILITY FAILURE\nSuccess Rate: {success_rate:.1%} (< 90% required)\nSystem not reliably delivering first responses!')
    ...
    async def test_complete_response_time_sla_compliance(self):
        ...
        for test_num in range(3):
            test_start = time.time()
            try:
                user_context = await create_authenticated_user_context(user_email=f'complete_response_test_{test_num}@example.com', environment=self.environment, websocket_enabled=True)
                jwt_token = self.auth_helper.create_test_jwt_token(user_id=str(user_context.user_id), email=user_context.agent_context.get('user_email'))
                ws_headers = self.websocket_helper.get_websocket_headers(jwt_token)
                connection = await WebSocketTestHelpers.create_test_websocket_connection(url=self.websocket_url, headers=ws_headers, timeout=10.0, user_id=str(user_context.user_id))
                self.active_connections.append(connection)
                request_send_time = time.time()
                comprehensive_request = {'type': 'chat_message', 'content': f'Performance Test {test_num}: Provide comprehensive cost optimization analysis with detailed recommendations and specific savings opportunities', 'user_id': str(user_context.user_id), 'comprehensive_test': True, 'test_number': test_num}
                await WebSocketTestHelpers.send_test_message(connection, comprehensive_request, timeout=5.0)
                complete_response_time = None
                complete_response_received = False
                events_received = []
                collection_start = time.time()
                max_wait = self.SLA_THRESHOLDS['complete_response_time'] + 10.0
                ...
                test_result = {'test_number': test_num, 'success': complete_response_received, 'complete_response_time': complete_response_time, 'events_received': len(events_received), 'response_quality': response_quality, 'sla_compliant': complete_response_time is not None and complete_response_time <= self.SLA_THRESHOLDS['complete_response_time'], 'total_test_time': time.time() - test_start}
                ...
                print(f"   Test {test_num}: {(' PASS: ' if test_result['sla_compliant'] else ' FAIL: ')} ({complete_response_time:.2f}s, {len(events_received)} events)" if complete_response_time else f'   Test {test_num}:  FAIL:  No completion')
                await WebSocketTestHelpers.close_test_connection(connection)
                self.active_connections.remove(connection)
            except Exception as test_error:
                complete_response_tests.append({'test_number': test_num, 'success': False, 'error': str(test_error), 'sla_compliant': False, 'total_test_time': time.time() - test_start})
                print(f'   Test {test_num}:  FAIL:  Error: {test_error}')
        ...
        if sla_compliance_rate < 0.9:
            pytest.fail(f" ALERT:  COMPLETE RESPONSE SLA FAILURE\nSLA Compliance: {sla_compliance_rate:.1%} (< 90% required)\nSLA Threshold: {self.SLA_THRESHOLDS['complete_response_time']}s\nAverage Complete Time: {avg_complete_time:.2f}s\nThis violates business expectation SLA requirements!")
    ...
    def _assess_response_quality(self, events: List[Dict]) -> Dict[str, Any]:
        ...
        quality_metrics = {'events_count': len(events), 'has_agent_started': any((e.get('type') == 'agent_started' for e in events)), 'has_agent_completed': any((e.get('type') == 'agent_completed' for e in events)), 'has_tool_execution': any((e.get('type') in ['tool_executing', 'tool_completed'] for e in events)), 'response_content_length': 0, 'quality_score': 0}
    ...
    async def test_concurrent_users_performance_sla(self):
        ...
        async def concurrent_user_test(user_index: int) -> Dict[str, Any]:
            ...
            try:
                user_context = await create_authenticated_user_context(user_email=f'concurrent_user_{user_index}@example.com', environment=self.environment, websocket_enabled=True)
                jwt_token = self.auth_helper.create_test_jwt_token(user_id=str(user_context.user_id), email=user_context.agent_context.get('user_email'))
                connection_start = time.time()
                ws_headers = self.websocket_helper.get_websocket_headers(jwt_token)
                connection = await WebSocketTestHelpers.create_test_websocket_connection(url=self.websocket_url, headers=ws_headers, timeout=15.0, user_id=str(user_context.user_id))
                connection_time = time.time() - connection_start
                request_start = time.time()
                test_message = {'type': 'chat_message', 'content': f'Concurrent user {user_index} performance test', 'user_id': str(user_context.user_id), 'concurrent_test': True, 'user_index': user_index}
                await WebSocketTestHelpers.send_test_message(connection, test_message, timeout=5.0)
                first_response_time = None
                ...
                return {'user_index': user_index, 'success': response_received, 'connection_time': connection_time, 'first_response_time': first_response_time, 'total_execution_time': user_execution_time, 'connection_sla_met': connection_time <= self.SLA_THRESHOLDS['websocket_connection_time'], 'response_sla_met': first_response_time is not None and first_response_time <= self.SLA_THRESHOLDS['first_response_time']}
            except Exception as user_error:
                return {'user_index': user_index, 'success': False, 'error': str(user_error), 'total_execution_time': time.time() - user_start, 'connection_sla_met': False, 'response_sla_met': False}
        concurrent_tasks = [concurrent_user_test(i) for i in range(concurrent_users)]
        ...
        if success_rate < 0.8:
            pytest.fail(f' ALERT:  CONCURRENT USERS PERFORMANCE FAILURE\nSuccess Rate: {success_rate:.1%} (< 80% required)\nSystem cannot handle {concurrent_users} concurrent users!')
        if connection_sla_compliance < 0.8:
            pytest.fail(f' ALERT:  CONCURRENT CONNECTION SLA FAILURE\nConnection SLA Compliance: {connection_sla_compliance:.1%} (< 80% required)\nConnection times degrading significantly under load!')
        if response_sla_compliance < 0.7:
            pytest.fail(f' ALERT:  CONCURRENT RESPONSE SLA FAILURE\nResponse SLA Compliance: {response_sla_compliance:.1%} (< 70% required)\nResponse times degrading significantly under load!')
    ...
    async def test_memory_usage_stability_sla(self):
        ...
        def record_memory():
            ...
            try:
                ...
                memory_measurements.append({'timestamp': time.time() - memory_test_start, 'memory_mb': current_memory, 'growth_mb': current_memory - self.baseline_memory})
        ...
        try:
            connections = []
            for i in range(5):
                user_context = await create_authenticated_user_context(user_email=f'memory_test_user_{i}@example.com', environment=self.environment, websocket_enabled=True)
                jwt_token = self.auth_helper.create_test_jwt_token(user_id=str(user_context.user_id), email=user_context.agent_context.get('user_email'))
                ws_headers = self.websocket_helper.get_websocket_headers(jwt_token)
                connection = await WebSocketTestHelpers.create_test_websocket_connection(url=self.websocket_url, headers=ws_headers, timeout=10.0, user_id=str(user_context.user_id))
            ...
            for i, connection in enumerate(connections):
                for msg_num in range(3):
                    message = {'type': 'chat_message', 'content': f'Memory test - Connection {i}, Message {msg_num}', 'memory_test': True}
            ...
            stress_operations.append({'operation': 'websocket_stress_test', 'success': True, 'connections_created': len(connections), 'messages_sent': len(connections) * 3})
        except Exception as stress_error:
            stress_operations.append({'operation': 'websocket_stress_test', 'success': False, 'error': str(stress_error)})
        ...
        if memory_measurements:
            ...
            memory_leak_suspected = final_growth > self.SLA_THRESHOLDS['memory_growth_limit'] * 0.5
        else:
        ...
        if max_growth > self.SLA_THRESHOLDS['memory_growth_limit']:
            pytest.fail(f" ALERT:  MEMORY GROWTH SLA FAILURE\nMax Memory Growth: {max_growth:.2f}MB (> {self.SLA_THRESHOLDS['memory_growth_limit']}MB limit)\nPeak Memory: {peak_memory:.2f}MB\nSystem using excessive memory under load!")
        if memory_leak_suspected:
            pytest.fail(f" ALERT:  MEMORY LEAK SUSPECTED\nFinal Growth: {final_growth:.2f}MB (> {self.SLA_THRESHOLDS['memory_growth_limit'] * 0.5:.2f}MB threshold)\nMemory not recovering properly after operations!")
