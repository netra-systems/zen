...
class CloudRunPortConfigurationValidator:
            ...
            if isinstance(env_vars, list):
                for env_var in env_vars:
                    if isinstance(env_var, str) and env_var.startswith('PORT='):
                        issues.append({'service': service_name, 'issue': 'manual_port_env_var', 'details': f'Manual PORT environment variable: {env_var}', 'severity': 'CRITICAL', 'cloud_run_conflict': True})
            elif isinstance(env_vars, dict):
                if 'PORT' in env_vars:
                    issues.append({'service': service_name, 'issue': 'manual_port_env_var', 'details': f"Manual PORT environment variable: PORT={env_vars['PORT']}", 'severity': 'CRITICAL', 'cloud_run_conflict': True})
            ports = service_config.get('ports', [])
            if ports:
                for port_mapping in ports:
                    if isinstance(port_mapping, str):
                        if ':8888' in port_mapping:
                            issues.append({'service': service_name, 'issue': 'hardcoded_port_8888', 'details': f'Hardcoded port 8888 mapping: {port_mapping}', 'severity': 'HIGH', 'cloud_run_conflict': True})
        return {'valid': len(issues) == 0, 'services_checked': services_checked, 'issues': issues, 'total_issues': len(issues), 'critical_issues': len([i for i in issues if i['severity'] == 'CRITICAL'])}
    ...
    def check_environment_port_conflicts(self) -> Dict[str, Any]:
        ...
        """
        env_files = [self.project_root / '.env', self.project_root / '.env.staging', self.project_root / '.env.production.template', self.project_root / '.env.development', self.project_root / '.env.test']
        port_conflicts = []
        port_definitions = {}
            ...
            try:
                ...
                for line_num, line in enumerate(lines, 1):
                    ...
                    if line.startswith('PORT=') and (not line.startswith('#')):
                        ...
                        port_definitions[str(env_file.name)] = {'value': port_value, 'line': line_num, 'file': str(env_file)}
                        if port_value == '8888':
                            port_conflicts.append({'file': str(env_file.name), 'line': line_num, 'value': port_value, 'issue': 'cloud_run_reserved_port', 'severity': 'CRITICAL'})
            except (UnicodeDecodeError, PermissionError) as e:
                continue
        return {'conflicts_found': len(port_conflicts) > 0, 'port_conflicts': port_conflicts, 'port_definitions': port_definitions, 'total_conflicts': len(port_conflicts)}
    ...
    async def test_port_binding_simulation(self, port: int=8888) -> Dict[str, Any]:
        ...
        """
        binding_results = {'port': port, 'binding_successful': False, 'binding_time_ms': 0, 'conflicts_detected': [], 'error': None}
        ...
        try:
            ...
            try:
                ...
                binding_results['conflicts_detected'].append({'type': 'port_in_use', 'details': str(e), 'port': port})
            finally:
                sock.close()
...
class TestCloudRunPortConfiguration(SSotBaseTestCase):
    ...
    async def test_docker_compose_staging_port_config(self):
        ...
        self.log_test_result('docker_compose_staging_validation', {'file': str(staging_compose), 'validation_result': validation_result})
        critical_issues = [i for i in validation_result['issues'] if i['severity'] == 'CRITICAL' and i.get('cloud_run_conflict')]
        assert len(critical_issues) == 0, f'CRITICAL: Found {len(critical_issues)} Cloud Run port conflicts in staging compose:\n' + '\n'.join([f"- {issue['service']}: {issue['details']}" for issue in critical_issues])
        assert len(validation_result['services_checked']) > 0, 'No services found in staging compose file'
        self.log_test_result('staging_port_config_validation', {'status': 'PASS', 'services_validated': validation_result['services_checked'], 'issues_found': validation_result['total_issues']})
    ...
    async def test_environment_files_port_conflicts(self):
        ...
        critical_conflicts = [c for c in conflict_analysis['port_conflicts'] if c['severity'] == 'CRITICAL']
        assert len(critical_conflicts) == 0, f'CRITICAL: Found {len(critical_conflicts)} PORT conflicts in environment files:\n' + '\n'.join([f"- {conflict['file']}:{conflict['line']} PORT={conflict['value']} ({conflict['issue']})" for conflict in critical_conflicts])
        assert len(conflict_analysis['port_definitions']) > 0, 'No PORT definitions found in any environment files'
    ...
    async def test_cloud_run_port_binding_simulation(self):
        ...
        if binding_result['conflicts_detected']:
            ...
            self.log_test_result('port_binding_conflicts', {'port': 8888, 'conflicts': conflict_details, 'binding_time_ms': binding_result['binding_time_ms']})
            print(f'DEBUG: Port 8888 conflicts detected: {conflict_details}')
        assert binding_result['error'] is None, f"Port binding simulation failed with error: {binding_result['error']}"
        assert binding_result['binding_time_ms'] < 10000, f"Port binding took too long: {binding_result['binding_time_ms']}ms (Cloud Run timeout concern)"
    ...
    async def test_alpine_compose_port_config(self):
        ...
        assert len(critical_issues) == 0, f'CRITICAL: Alpine compose has {len(critical_issues)} port configuration issues:\n' + '\n'.join([f"- {issue['service']}: {issue['details']}" for issue in critical_issues])
    ...
    async def test_service_port_environment_isolation(self):
        ...
        for service in services_to_check:
            service_result = {'service': service, 'expected_ports': {}, 'conflicts': [], 'isolation_valid': True}
            if service == 'backend':
                service_result['expected_ports'] = {'local': 8000, 'test': 8000, 'staging': 'cloud_run_assigned', 'production': 'cloud_run_assigned'}
            elif service == 'auth':
                service_result['expected_ports'] = {'local': 8081, 'test': 8081, 'staging': 'cloud_run_assigned', 'production': 'cloud_run_assigned'}
            elif service == 'frontend':
                service_result['expected_ports'] = {'local': 3000, 'test': 3000, 'staging': 'cloud_run_assigned', 'production': 'cloud_run_assigned'}
            for env_type, expected_port in service_result['expected_ports'].items():
                if env_type in ['staging', 'production'] and expected_port != 'cloud_run_assigned':
                    service_result['conflicts'].append({'environment': env_type, 'issue': 'hardcoded_cloud_run_port', 'expected': 'cloud_run_assigned', 'found': expected_port})
        ...
        failed_services = [service for service, result in isolation_results.items() if not result['isolation_valid']]
        assert len(failed_services) == 0, f'Port isolation failures in services: {failed_services}\n' + 'Services must use Cloud Run assigned ports in staging/production'
    ...
    async def test_cloud_run_deployment_config_generation(self):
        ...
        """
        deployment_files = [self.project_root / 'scripts' / 'deploy_to_gcp.py', self.project_root / 'cloudbuild-backend.yaml', self.project_root / 'cloudbuild-auth.yaml']
        config_analysis = {'files_checked': [], 'port_violations': [], 'deployment_valid': True}
            ...
            try:
                ...
                for line_num, line in enumerate(lines, 1):
                    if 'PORT=8888' in line or '--set-env-vars=PORT=' in line:
                        config_analysis['port_violations'].append({'file': config_file.name, 'line': line_num, 'content': line.strip(), 'issue': 'manual_port_in_deployment'})
                        config_analysis['deployment_valid'] = False
            except (UnicodeDecodeError, PermissionError):
                continue
        self.log_test_result('deployment_config_analysis', config_analysis)
        assert len(config_analysis['port_violations']) == 0, f"Found {len(config_analysis['port_violations'])} PORT violations in deployment configs:\n" + '\n'.join([f"- {violation['file']}:{violation['line']}: {violation['content']}" for violation in config_analysis['port_violations']])
        assert len(config_analysis['files_checked']) > 0, 'No deployment configuration files found to validate'
