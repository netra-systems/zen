...
class AuthLifecycleMismatchesPOCTests:
    ...
    def setup_method(self, method):
        ...
        self.agent_context_duration = 30
    ...
    def _simulate_agent_context_creation(self, connection_info: dict, attempt_time: float) -> dict:
        ...
        attempt_result = {'attempt_time': current_time, 'seconds_since_connection': attempt_time, 'jwt_expired': jwt_expired, 'user_id': connection_info['user_id'], 'connection_id': connection_info['connection_id']}
        if jwt_expired:
            attempt_result['status'] = 'FAILED'
            attempt_result['error'] = 'JWT_EXPIRED_DURING_WEBSOCKET_SESSION'
            attempt_result['error_details'] = f"Agent context creation failed: JWT expired at {connection_info['jwt_expires_at']}, but WebSocket connection established at {connection_info['created_at']} is still active. This is the LIFECYCLE MISMATCH causing Golden Path failures."
    ...
    def test_websocket_connection_outlives_agent_context_timing(self):
        ...
        early_attempt = self._simulate_agent_context_creation(connection_info, 30)
        ...
        jwt_expiry_time = self.jwt_expiry_duration + 5
        ...
        assert len(successful_attempts) >= 1, f'Expected at least one successful context creation (before JWT expiry), but got {len(successful_attempts)} successes. Context attempts: {self.agent_context_attempts}'
        assert len(failed_attempts) >= 1, f'CRITICAL: Expected at least one failed context creation (after JWT expiry), but got {len(failed_attempts)} failures. This means the lifecycle mismatch is NOT being detected. The test should expose JWT expiry during active WebSocket sessions. Context attempts: {self.agent_context_attempts}'
        jwt_expiry_failures = [a for a in failed_attempts if a.get('error') == 'JWT_EXPIRED_DURING_WEBSOCKET_SESSION']
        assert len(jwt_expiry_failures) >= 1, f"Expected JWT expiry failures during WebSocket session, but got failures: {[a.get('error') for a in failed_attempts]}. The lifecycle mismatch should specifically be JWT_EXPIRED_DURING_WEBSOCKET_SESSION errors."
    ...
    def test_concurrent_agent_execution_auth_timing_conflicts(self):
        ...
        connection_info = self._simulate_websocket_connection_start(self.test_user_id)
        timing_intervals = [10, 30, 50, 70, 90]
        concurrent_results = []
        ...
        assert len(successful_contexts) > 0 and len(failed_contexts) > 0, f'Expected both successful and failed auth attempts to demonstrate timing conflicts, but got {len(successful_contexts)} successes and {len(failed_contexts)} failures. Results: {concurrent_results}'
    ...
    def test_auth_state_persistence_simulation(self):
        ...
        connection_info = self._simulate_websocket_connection_start(self.test_user_id)
        message_sequence = [{'time': 15, 'type': 'chat', 'content': 'Hello, start conversation'}, {'time': 35, 'type': 'agent_request', 'content': 'Process this request'}, {'time': 55, 'type': 'tool_execution', 'content': 'Execute tool'}, {'time': 75, 'type': 'chat', 'content': 'Continue conversation'}, {'time': 95, 'type': 'agent_response', 'content': 'Agent response'}]
        message_results = []
        ...
        assert len(failed_messages) > 0, f'Expected some message processing failures due to auth state persistence issues, but all {len(message_results)} messages succeeded. This suggests auth state persistence problems are not being detected. Message results: {message_results}'
