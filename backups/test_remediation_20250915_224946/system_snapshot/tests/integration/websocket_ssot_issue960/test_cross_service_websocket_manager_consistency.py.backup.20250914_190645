...
class TestCrossServiceWebSocketManagerConsistency(SSotAsyncTestCase):
    ...
    async def test_agent_registry_uses_ssot_websocket_manager(self):
        ...
        try:
            # Import agent registry and WebSocket manager
            from netra_backend.app.agents.registry import AgentRegistry
            from netra_backend.app.websocket_core.websocket_manager import get_websocket_manager

            user_context = {"user_id": "registry_test_user", "thread_id": "registry_test_thread"}

            # Get WebSocket manager directly
            direct_websocket_manager = await get_websocket_manager(user_context=user_context)

            # Create agent registry and check its WebSocket manager
            agent_registry = AgentRegistry()

            # Check if agent registry has WebSocket manager
            if hasattr(agent_registry, 'websocket_manager'):
                registry_websocket_manager = agent_registry.websocket_manager

                # CRITICAL TEST: Should be the same instance (SSOT)
                # This will FAIL if agent registry creates its own manager
                self.assertIs(
                    direct_websocket_manager,
                    registry_websocket_manager,
                    "SSOT VIOLATION: Agent registry uses different WebSocket manager instance. "
                    "This causes event delivery inconsistencies across services."
                )

            elif hasattr(agent_registry, 'set_websocket_manager'):
                # Check if we can set the manager and it's consistent
                ...
                if hasattr(agent_registry, 'get_websocket_manager'):
                    ...
                    self.assertIs(
                        direct_websocket_manager,
                        retrieved_manager,
                        "SSOT VIOLATION: Agent registry does not maintain WebSocket manager reference. "
                        "This indicates fragmented manager handling."
                    )
                else:
                    raise AssertionError(
                        "SSOT VIOLATION: Agent registry cannot retrieve WebSocket manager. "
                        "This indicates incomplete SSOT integration."
                    )
            else:
                raise AssertionError(
                    "SSOT VIOLATION: Agent registry has no WebSocket manager interface. "
                    "This indicates services are not properly integrated."
                )
    ...
    async def test_websocket_manager_startup_uses_single_instance(self):
        ...
        try:
            ...
            with patch('netra_backend.app.websocket_core.websocket_manager.get_websocket_manager') as mock_get_manager:
                # Create different manager instances for different calls
                manager1 = MagicMock()
                manager2 = MagicMock()
                manager3 = MagicMock()

                # This simulates fragmented startup behavior
                ...
                # Simulate multiple service startup calls
            ...
            # CRITICAL TEST: All services should get the same manager instance (SSOT)
            # This will FAIL if startup creates multiple instances
            manager_ids = [id(manager) for _, manager in startup_managers]
            unique_manager_ids = set(manager_ids)

            if len(unique_manager_ids) > 1:
                raise AssertionError(
                    f"SSOT VIOLATION: Startup process creates {len(unique_manager_ids)} different WebSocket manager instances: {unique_manager_ids}. "
                    f"Services: {[service for service, _ in startup_managers]}. "
                    f"SSOT requires exactly ONE instance shared across all services."
                )
    ...
    async def test_websocket_event_delivery_cross_service_consistency(self):
        ...
        try:
            ...
            service_events = [
                ("agent_service", {"type": "agent_started", "agent": "test_agent"}),
                ("tool_service", {"type": "tool_executing", "tool": "test_tool"}),
                ("supervisor_service", {"type": "agent_completed", "result": "success"}),
            ]
            ...
            # CRITICAL TEST: All services should successfully deliver events (SSOT)
            # This will FAIL if event delivery is inconsistent across services
            failed_deliveries = [(service, result) for service, result in event_delivery_results if result != "SUCCESS"]

            if failed_deliveries:
                raise AssertionError(
                    f"SSOT VIOLATION: Cross-service event delivery is inconsistent. "
                    f"Failed deliveries: {failed_deliveries}. "
                    f"All services should use the same WebSocket manager for consistent delivery."
                )

            # Additional check: Verify WebSocket received all events
            if hasattr(mock_websocket, 'send') and mock_websocket.send.called:
                ...
                if call_count != expected_calls:
                    raise AssertionError(
                        f"SSOT VIOLATION: Expected {expected_calls} WebSocket calls, got {call_count}. "
                        f"This indicates duplicate or missing event deliveries."
                    )
        ...
        try:
            ...
            class MockDependencyInjector:
                ...
                async def get_websocket_manager(self, user_context):
                    # This simulates broken DI that creates multiple instances
                    from netra_backend.app.websocket_core.websocket_manager import get_websocket_manager
                    # Each call creates a new manager (violates SSOT)
                    return await get_websocket_manager(user_context=user_context)

            # Create injector and user context
            injector = MockDependencyInjector()
            user_context = {"user_id": "di_test_user", "thread_id": "di_test_thread"}

            # Get WebSocket manager for multiple services
            service_managers = {}
            ...
            # CRITICAL TEST: All services should get the same manager instance (SSOT)
            # This will FAIL if DI creates multiple instances
            ...
            for service, manager in service_managers.items():
                if manager is not first_manager:
                    ...
                    raise AssertionError(
                        f"SSOT VIOLATION: Dependency injection provides different WebSocket manager instances. "
                        f"Service '{service}' got different instance. Manager IDs: {manager_ids}. "
                        f"SSOT requires exactly ONE instance shared across all services."
                    )
    ...
    async def test_websocket_manager_memory_cleanup_cross_service(self):
        ...
        try:
            ...
            # Add test connections to manager
            ...
            # Simulate cleanup from different services
            ...
            # CRITICAL TEST: Cleanup should be idempotent across services (SSOT)
            # This will FAIL if multiple cleanup calls cause issues
            ...
            if len(successful_cleanups) > 1 and final_connection_count != 0:
                raise AssertionError(
                    f"SSOT VIOLATION: Multiple service cleanups resulted in inconsistent state. "
                    f"Initial connections: {initial_connection_count}, Final: {final_connection_count}. "
                    f"Cleanup results: {cleanup_results}. SSOT requires idempotent cleanup."
                )
