...
class TestJwtSsotEnforcement(SSotBaseTestCase):
    ...
    def test_single_jwt_validation_authority(self):
        ...
        # Search for JWT validation implementations across services
        project_root = Path(__file__).parent.parent.parent

        # Check backend service for JWT validation
        backend_jwt_files = self._scan_for_jwt_implementations(
            project_root / "netra_backend",
            "backend"
        )
        jwt_validation_implementations.extend(backend_jwt_files)

        # Check auth service for JWT validation
        auth_jwt_files = self._scan_for_jwt_implementations(
            project_root / "auth_service",
            "auth_service"
        )
        jwt_validation_implementations.extend(auth_jwt_files)

        # Check frontend for JWT validation
        frontend_jwt_files = self._scan_for_jwt_implementations(
        ...
        # SSOT VIOLATION: Multiple JWT validation implementations found
        if len(jwt_validation_implementations) > 1:
            violation_details = [
                f"Multiple JWT validation found in: {impl['service']}/{impl['file']}"
                for impl in jwt_validation_implementations
            ]

            self.logger.critical(
                f"JWT SSOT VIOLATION DETECTED: {len(jwt_validation_implementations)} "
                f"separate JWT validation implementations found: {violation_details}"
            )

            # THIS TEST IS DESIGNED TO FAIL - proving violations exist
            self.assertTrue(
                False,
                f"JWT SSOT VIOLATION: Found {len(jwt_validation_implementations)} "
                f"separate JWT validation implementations. Expected: 1 (auth service only). "
    ...
    def test_backend_delegates_jwt_to_auth_service(self):
        ...
        if backend_jwt_violations:
            violation_details = [
                f"{violation['type']}: {violation['file']}:{violation['line']}"
                for violation in backend_jwt_violations
            ]

            self.logger.critical(
                f"BACKEND JWT DELEGATION VIOLATION: Backend performing direct JWT operations. "
                f"Violations: {violation_details}"
            )
            ...
            self.assertTrue(
                False,
                f"BACKEND JWT DELEGATION VIOLATION: Backend should delegate ALL JWT operations "
                f"to auth service. Found {len(backend_jwt_violations)} violations: {violation_details}"
            )
        else:
            # Should only reach here after SSOT remediation
    ...
    def test_websocket_uses_same_jwt_validation_as_http(self):
        ...
        # Compare HTTP vs WebSocket JWT validation paths
        project_root = Path(__file__).parent.parent.parent
        backend_path = project_root / "netra_backend"

        # Find HTTP JWT validation path
        http_jwt_path = self._find_http_jwt_validation_path(backend_path)

        # Find WebSocket JWT validation path
        ...
        if http_jwt_path and websocket_jwt_path:
            # Check if they use the same validation logic
            path_comparison = self._compare_jwt_validation_paths(
                http_jwt_path,
                websocket_jwt_path
            )
            ...
            if not path_comparison['identical']:
                ...
                self.logger.critical(
                    f"WEBSOCKET JWT VALIDATION DIVERGENCE: WebSocket uses different JWT "
                    f"validation than HTTP. Differences: {auth_path_differences}"
                )

                # THIS TEST IS DESIGNED TO FAIL - proving violations exist
                self.assertTrue(
                    False,
                    f"WEBSOCKET JWT VALIDATION DIVERGENCE: WebSocket and HTTP must use "
                    f"identical JWT validation. Differences found: {auth_path_differences}"
                )
    ...
    def test_frontend_delegates_jwt_decode_to_backend(self):
        ...
        # Scan frontend for direct JWT decode operations
        project_root = Path(__file__).parent.parent.parent
        frontend_path = project_root / "frontend"

        # Check for direct JWT imports in frontend
        jwt_imports = self._scan_frontend_jwt_imports(frontend_path)
        frontend_jwt_violations.extend(jwt_imports)

        # Check for JWT decode operations in frontend
        ...
        if frontend_jwt_violations:
            violation_details = [
                f"{violation['type']}: {violation['file']}"
                for violation in frontend_jwt_violations
            ]

            self.logger.critical(
                f"FRONTEND JWT DELEGATION VIOLATION: Frontend performing direct JWT operations. "
                f"Should delegate to backend. Violations: {violation_details}"
            )

            # THIS TEST IS DESIGNED TO FAIL - proving violations exist
            self.assertTrue(
                False,
                f"FRONTEND JWT DELEGATION VIOLATION: Frontend should delegate ALL JWT "
                f"decode operations to backend. Found {len(frontend_jwt_violations)} violations: {violation_details}"
            )
        else:
            # Should only reach here after SSOT remediation
            ...
            try:
                ...
                for pattern in jwt_patterns:
                    if pattern in content:
                        ...
                        break  # One implementation per file
    ...
    def _find_http_jwt_validation_path(self, backend_path: Path) -> Optional[Dict[str, Any]]:
        ...
        # Search for HTTP authentication handlers
    ...
    def _find_websocket_jwt_validation_path(self, backend_path: Path) -> Optional[Dict[str, Any]]:
        ...
        # Search for WebSocket authentication handlers
        for py_file in backend_path.rglob("*.py"):
    ...
    def _compare_jwt_validation_paths(self, http_path: Dict[str, Any], ws_path: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Simplified comparison - in real implementation would analyze actual validation logic
        differences = []
    ...
    def _scan_frontend_jwt_decode_operations(self, frontend_path: Path) -> List[Dict[str, Any]]:
        ...
        decode_patterns = [
            ...
            'parseJwt(',
            'decodeToken(',
            'JSON.parse(atob(',  # Manual base64 decode
            'jwt.decode('
        ]
