...
class TestCrossServiceIdConsistency(SSotBaseTestCase):
    ...
    def test_auth_service_backend_id_correlation_post_migration(self):
        ...
        auth_session_id = unified_generator.generate_session_id(
            user_id=auth_user_id,
            request_id=auth_request_id
        )

        # Simulate backend service operations using the same user context
        backend_connection_id = unified_generator.generate_connection_id(
            user_id=auth_user_id,
            session_id=auth_session_id
        )

        backend_redis_client_id = unified_generator.generate_client_id(
            service_type="redis",
            user_id=auth_user_id,
            request_id=auth_request_id
        )

        # Validate cross-service ID correlation
        def extract_user_context(id_string: str) -> str:
            """Extract user context for correlation analysis"""
            parts = id_string.split('_')
            if len(parts) >= 2:
                return parts[1]  # User component should be second
            return "unknown"

        # All IDs should contain the same user context
        ...
        assert auth_user_context == "cross", (
            f"AUTH USER CONTEXT ERROR: Expected 'cross' from user_id, got '{auth_user_context}'"
        )

        assert backend_conn_user_context == "cross", (
            f"BACKEND CONNECTION CONTEXT ERROR: Expected 'cross', got '{backend_conn_user_context}'"
        )

        assert backend_client_user_context == "cross", (
            f"BACKEND CLIENT CONTEXT ERROR: Expected 'cross', got '{backend_client_user_context}'"
        )

        # Validate timestamp correlation (should be within seconds)
        ...
        # Timestamps should be within 5 seconds for correlated operations
        ...
        assert timestamp_range <= 5, (
            f"CROSS-SERVICE TIMESTAMP CORRELATION FAILURE: {timestamp_range}s range exceeds 5s limit"
        )
    ...
    def test_websocket_auth_integration_id_consistency_post_migration(self):
        ...
        # Simulate auth service flow
        websocket_user_id = "ws_integration_user_789"
        websocket_request_id = "ws_request_012"

        # Auth service generates session ID
        auth_session_id = unified_generator.generate_session_id(
            user_id=websocket_user_id,
            request_id=websocket_request_id
        )

        # WebSocket service uses session ID for connection
        websocket_connection_id = unified_generator.generate_connection_id(
            user_id=websocket_user_id,
            session_id=auth_session_id
        )

        # WebSocket service may also need audit trail
        websocket_audit_id = unified_generator.generate_audit_id(
            record_type="websocket",
            user_id=websocket_user_id,
            resource_id=websocket_connection_id
        )

        # Validate integration consistency

        # 1. Session ID should be referenced in connection ID
        ...
        # Note: Direct session reference may not be in connection ID, but user consistency is key
        # Validate user consistency instead
        def extract_user_prefix(id_string: str) -> str:
            parts = id_string.split('_')
            if len(parts) >= 2:
                return parts[1].split('-')[0]  # Get user prefix
            return "unknown"
        ...
        assert auth_user_prefix == websocket_user_prefix, (
            f"USER CONSISTENCY FAILURE: Auth '{auth_user_prefix}' != WebSocket '{websocket_user_prefix}'"
        )

        assert websocket_user_prefix == audit_user_prefix, (
            f"AUDIT CONSISTENCY FAILURE: WebSocket '{websocket_user_prefix}' != Audit '{audit_user_prefix}'"
        )

        # 2. Validate service-specific prefixes are correct
        ...
        # 3. Validate all IDs are unique
        all_ids = [auth_session_id, websocket_connection_id, websocket_audit_id]
        unique_ids = set(all_ids)

        assert len(unique_ids) == 3, (
            f"ID UNIQUENESS FAILURE: Generated {len(unique_ids)}/3 unique IDs"
        )
    ...
    def test_factory_services_client_id_coordination_post_migration(self):
        ...
        # Generate additional client IDs for different operations
        redis_cache_client_id = unified_generator.generate_client_id(
            service_type="redis",
            user_id=factory_user_id,
            request_id=f"{factory_request_id}_cache"
        )

        clickhouse_analytics_client_id = unified_generator.generate_client_id(
            service_type="clickhouse",
            user_id=factory_user_id,
            request_id=f"{factory_request_id}_analytics"
        )

        all_client_ids = [
            redis_client_id,
            clickhouse_client_id,
            redis_cache_client_id,
            clickhouse_analytics_client_id
        ]

        # Validate coordination capabilities

        # 1. All client IDs should follow consistent format
        client_pattern = re.compile(r'^client_[a-z]+_[a-zA-Z0-9_]+_[a-zA-Z0-9_]+_\d+_[a-f0-9]{8}$')
        for client_id in all_client_ids:
            assert client_pattern.match(client_id), (
                f"CLIENT FORMAT ERROR: '{client_id}' doesn't match SSOT pattern"
            )

        # 2. Service type identification should be clear
        ...
        assert service_types == expected_services, (
            f"SERVICE TYPE COORDINATION ERROR: Expected {expected_services}, got {service_types}"
        )

        # 3. User coordination should be consistent
        ...
        expected_user_context = "factory"  # From factory_coord_user_345

        for user_context in user_contexts:
            assert user_context == expected_user_context, (
                f"USER COORDINATION ERROR: Expected '{expected_user_context}', got '{user_context}'"
            )

        # 4. Request correlation should be possible
        ...
        # First two should be same request, last two should be derived requests
        ...
        # 5. All client IDs should be unique
        unique_client_ids = set(all_client_ids)
        assert len(unique_client_ids) == 4, (
            f"CLIENT ID UNIQUENESS FAILURE: Generated {len(unique_client_ids)}/4 unique IDs"
        )
    ...
    def test_audit_trail_cross_service_traceability_post_migration(self):
        ...
        # Simulate cross-service operation flow
        ...
        auth_audit_id = unified_generator.generate_audit_id(
            record_type="auth",
            user_id=trace_user_id,
            resource_id=auth_session_id
        )

        # 2. WebSocket service: Connection established
        websocket_connection_id = unified_generator.generate_connection_id(
            user_id=trace_user_id,
            session_id=auth_session_id
        )
        ...
        # 3. Backend service: Data operations
        redis_client_id = unified_generator.generate_client_id(
            service_type="redis",
            user_id=trace_user_id,
            request_id=trace_request_id
        ...
        data_audit_id = unified_generator.generate_audit_id(
            record_type="data",
            user_id=trace_user_id,
            resource_id=f"{redis_client_id}|{clickhouse_client_id}"
        )

        # Collect all IDs for traceability analysis
        trace_chain = {
            'auth_session': auth_session_id,
            'auth_audit': auth_audit_id,
            'websocket_connection': websocket_connection_id,
            'websocket_audit': websocket_audit_id,
            'redis_client': redis_client_id,
            'clickhouse_client': clickhouse_client_id,
            'data_audit': data_audit_id
        }

        # Validate cross-service traceability

        # 1. User context consistency across all services
        def extract_user_context(id_string: str) -> str:
            parts = id_string.split('_')
            if len(parts) >= 2:
                # Extract base user identifier
                user_part = parts[1]
                return user_part.split('-')[0]  # Get prefix before any suffixes
            return "unknown"
        ...
        # All should have consistent user context
        expected_user_context = "trace"
        for service, user_context in user_contexts.items():
            assert user_context == expected_user_context, (
                f"TRACEABILITY USER CONTEXT ERROR: {service} has '{user_context}', expected '{expected_user_context}'"
            )

        # 2. Timestamp correlation for operation sequence
        def extract_timestamp(id_string: str) -> int:
            parts = id_string.split('_')
            for part in parts:
                if part.isdigit() and len(part) >= 10:
        ...
        # Validate operation sequence (auth should be first, then websocket, then data)
        auth_time = timestamps['auth_session']
        websocket_time = timestamps['websocket_connection']
        data_time = timestamps['redis_client']

        # Times should be in logical sequence (within reasonable bounds)
        time_range = max(timestamps.values()) - min(timestamps.values())
        assert time_range <= 30, (
            f"TRACEABILITY TIME RANGE ERROR: {time_range}s exceeds 30s operation window"
        )

        # 3. Service type identification for complete trace
        service_prefixes = {
            'auth_session': 'sess',
            'auth_audit': 'audit',
            'websocket_connection': 'conn',
            'websocket_audit': 'audit',
            'redis_client': 'client',
        ...
        for service, id_string in trace_chain.items():
            ...
            assert actual_prefix == expected_prefix, (
                f"SERVICE PREFIX ERROR: {service} has '{actual_prefix}', expected '{expected_prefix}'"
            )

        # 4. All IDs in trace chain should be unique
        all_trace_ids = list(trace_chain.values())
        unique_trace_ids = set(all_trace_ids)

        assert len(unique_trace_ids) == len(all_trace_ids), (
            f"TRACE UNIQUENESS FAILURE: Generated {len(unique_trace_ids)}/{len(all_trace_ids)} unique IDs"
        )
