...
class TestMessageRouterConsolidatedFunctionality(SSotAsyncTestCase):
    ...
    async def test_consolidated_message_routing_functionality(self):
        ...
        test_messages = [
            {
                "type": "user_message",
                "payload": {"content": "Hello, this is a user message"},
                "timestamp": time.time(),
                "user_id": self.test_user_id,
                "thread_id": self.test_thread_id
            },
        ...
        for message in test_messages:
            ...
            try:
                ...
                self.functionality_tests["message_routing"].append({
                    "message_type": message["type"],
                    "success": result,
                    "route_time": route_time,
                    "response_count": len(mock_websocket.sent_messages)
                })
        ...
        self.assertGreaterEqual(success_rate, 75.0,
                               f"Message routing success rate too low: {success_rate}%")
    ...
    async def test_handler_registration_and_execution_functionality(self):
        ...
        self.functionality_tests["handler_management"].append({
        ...
        test_message = {
            "type": "user_message",
            "payload": {"content": "Test handler execution"},
            "timestamp": time.time()
        }
        ...
        self.functionality_tests["handler_management"].append({
            ...
            "messages_handled": len(test_handler.handled_messages)
        })
        ...
        self.functionality_tests["handler_management"].append({
            ...
            "final_handler_count": len(router.handlers)
        })
    ...
    async def test_websocket_integration_functionality(self):
        ...
        # Test with WebSocket manager integration
        ws_manager = WebSocketManager(user_context=self.user_context)
        
        # Verify WebSocket manager works with router
        ...
        # Test critical WebSocket events (Golden Path validation)
        critical_events = [
            {
                "type": "agent_request",
                "payload": {
                    "message": "Test critical event sequence",
                    "turn_id": "critical_turn_123",
                    "real_llm": False
                },
                "timestamp": time.time()
            }
        ...
        for event in critical_events:
            ...
            try:
                result = await router.route_message(
                    self.test_user_id, mock_websocket, event
                )
                
                integration_time = time.time() - start_time
                
                # Check for expected WebSocket responses
                ...
                self.functionality_tests["websocket_integration"].append({
                    "event_type": event["type"],
                    "success": result,
                    "integration_time": integration_time,
                    "expected_events": len(expected_events),
                    "delivered_events": events_delivered,
                    "delivery_rate": (events_delivered / len(expected_events)) * 100
                })
    ...
    async def test_concurrent_routing_functionality(self):
        ...
        async def route_task(task_id):
            ...
            message = {
                "type": "user_message",
                "payload": {"content": f"Concurrent message {task_id}"},
                "timestamp": time.time()
            }
            
            start_time = time.time()
            result = await router.route_message(
                f"{self.test_user_id}_{task_id}", mock_websocket, message
            )
            route_time = time.time() - start_time
            ...
            return {
                ...
                "success": result,
                "route_time": route_time,
                "responses": len(mock_websocket.sent_messages)
            }
        ...
        self.functionality_tests["performance_metrics"]["concurrent_routing"] = {
            "total_tasks": num_concurrent_tasks,
            "successful_tasks": len(successful_tasks),
            "failed_tasks": len(failed_tasks),
            "total_time": total_time,
            "average_time": sum(r["route_time"] for r in successful_tasks) / len(successful_tasks) if successful_tasks else 0,
            "success_rate": (len(successful_tasks) / num_concurrent_tasks) * 100
        }
        
        # Verify concurrent functionality
        success_rate = (len(successful_tasks) / num_concurrent_tasks) * 100
        self.assertGreaterEqual(success_rate, 80.0,
                               f"Concurrent routing success rate too low: {success_rate}%")
    ...
    async def test_error_handling_functionality(self):
        ...
        error_scenarios = [
            ...
            {
                "name": "missing_type",
                "message": {"payload": {"content": "no type"}},
                "expected_behavior": "graceful_handling"
            },
            {
        ...
        for scenario in error_scenarios:
            ...
            try:
                result = await router.route_message(
                    self.test_user_id, mock_websocket, scenario["message"]
                )
                ...
                # Should not raise exceptions for malformed messages
                self.functionality_tests["message_routing"].append({
                    "scenario": scenario["name"],
                    "success": False,
                    "error": str(e),
    ...
    async def test_statistics_and_monitoring_functionality(self):
        ...
        test_messages = [
            ...
            {"type": "user_message", "payload": {"content": "test"}, "timestamp": time.time()},
            {"type": "heartbeat", "payload": {}, "timestamp": time.time()}
        ]
        ...
        self.functionality_tests["performance_metrics"]["statistics"] = {
            "initial_handler_count": initial_stats["handler_count"],
            "updated_handler_count": updated_stats["handler_count"],
            "messages_routed": updated_stats.get("messages_routed", 0),
            "statistics_keys": list(updated_stats.keys())
        }
    ...
    def test_functionality_summary(self):
        ...
        functionality_summary = {
            ...
            "functionality_success_rate": (successful_tests / total_tests) * 100 if total_tests > 0 else 0,
            "message_routing_tests": len(self.functionality_tests["message_routing"]),
            "handler_management_tests": len(self.functionality_tests["handler_management"]),
            "websocket_integration_tests": len(self.functionality_tests["websocket_integration"]),
            "performance_metrics": self.functionality_tests["performance_metrics"],
            "detailed_results": self.functionality_tests
        }
        
        logger.info(f"Consolidated Functionality Summary: {json.dumps(functionality_summary, indent=2)}")
        
        # Assert functionality preservation
        self.assertGreaterEqual(functionality_summary["functionality_success_rate"], 85.0,
                               "Consolidated functionality success rate should be at least 85%")
        
        # Verify critical functionality categories
        self.assertGreater(functionality_summary["message_routing_tests"], 0,
                          "Should have message routing tests")
        self.assertGreater(functionality_summary["handler_management_tests"], 0,
                          "Should have handler management tests")
