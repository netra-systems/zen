...
class UserExecutionContextCoreTests:
    ...
    def test_context_creation_with_valid_data(self):
        ...
        context = UserExecutionContext(user_id='user_123', thread_id='thread_456', run_id='run_789', request_id='req_012', websocket_client_id='conn_345')
    ...
    def test_context_validation_rejects_none_user_id(self):
        ...
        with pytest.raises(InvalidContextError, match="Required field 'user_id' must be a non-empty string"):
            UserExecutionContext(user_id=None, thread_id='thread_456', run_id='run_789', request_id='req_012')
    ...
    def test_context_validation_rejects_placeholder_run_id(self):
        ...
        from netra_backend.app.agents.supervisor.user_execution_context import InvalidContextError
        with pytest.raises(InvalidContextError, match='contains forbidden placeholder value'):
            UserExecutionContext(user_id='user_123', thread_id='thread_456', run_id='registry', request_id='req_012')
    ...
    def test_context_string_representation_security(self):
        ...
        context = UserExecutionContext(user_id=long_user_id, thread_id='thread_456', run_id='run_789', request_id='req_012')
...
class AgentExecuteMethodMigrationTests:
    ...
    class MigrationTestAgent(BaseAgent):
        ...
        async def execute_with_context(self, context: UserExecutionContext, stream_updates: bool=False) -> Any:
            ...
            self.execution_calls.append({'method': 'execute_with_context', 'context': context, 'stream_updates': stream_updates, 'timestamp': datetime.now(timezone.utc)})
    ...
    async def test_agent_execute_with_context_success(self):
        ...
        context = UserExecutionContext(user_id='test_user', thread_id='test_thread', run_id='test_run', request_id='test_request')
    ...
    async def test_concurrent_user_execution_isolation(self):
        ...
        class IsolationTestAgent(BaseAgent):
            ...
            async def execute_with_context(self, context: UserExecutionContext, stream_updates: bool=False) -> Any:
                ...
                return {'user_id': context.user_id, 'secret': user_secret, 'data_integrity': 'verified'}

        async def execute_for_user(user_id: str) -> Dict[str, Any]:
            agent = IsolationTestAgent()
            context = UserExecutionContext(user_id=user_id, thread_id=f'thread_{user_id}', run_id=f'run_{user_id}_{uuid.uuid4()}', request_id=f'req_{user_id}_{uuid.uuid4()}')
            return await agent.execute(context)
        user_count = 10
        tasks = [execute_for_user(f'user_{i}') for i in range(user_count)]
...
class PerformanceValidationTests:
    ...
    async def test_context_creation_performance(self):
        ...
        contexts = []
        for i in range(1000):
            context = UserExecutionContext(user_id=f'perf_user_{i}', thread_id=f'perf_thread_{i}', run_id=f'perf_run_{i}', request_id=f'perf_req_{i}')
            contexts.append(context)
        end_time = time.time()
        total_time = end_time - start_time
        avg_time_per_context = total_time / 1000 * 1000
    ...
    async def test_agent_execution_performance(self):
        ...
        class PerformanceTestAgent(BaseAgent):
            ...
            async def execute_with_context(self, context: UserExecutionContext, stream_updates: bool=False) -> Any:
                return {'user_id': context.user_id, 'execution_time': time.time()}
