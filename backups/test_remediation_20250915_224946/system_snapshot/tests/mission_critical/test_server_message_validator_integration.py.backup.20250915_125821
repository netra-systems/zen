...
class TestServerMessageValidatorIntegration(SSotBaseTestCase):
    ...
    def test_flat_messages_validation_batch(self):
        ...
        self.assertTrue(all_passed, f"Some flat messages failed validation: {failed_events}")
    ...
    def test_server_messages_validation_batch(self):
        ...
        self.assertTrue(all_passed, f"Some ServerMessage format messages failed validation: {failed_events}")
    ...
    def test_mixed_format_validation(self):
        ...
        self.assertTrue(all_passed, f"Mixed format validation failed: {failed_events}")
    ...
    def test_complete_validation_workflow(self):
        ...
        for message in execution_messages:
            ...
            self.assertTrue(result, 
                f"Message {event_type} failed validation: {self.validator.errors}")
        ...
        for required in required_events:
            self.assertIn(required, event_types, f"Missing required event: {required}")
        
        # Verify no validation errors
        self.assertEqual(len(self.validator.errors), 0, 
            f"Validation should pass with no errors, but got: {self.validator.errors}")
