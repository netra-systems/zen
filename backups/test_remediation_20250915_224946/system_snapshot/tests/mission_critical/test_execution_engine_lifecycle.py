...
class ExecutionEngineLifecycleTests(SSotAsyncTestCase):
    ...
    def setup_method(self, method=None):
        ...
        logger.info(f"Starting ExecutionEngine lifecycle testing - Initial memory: {self.initial_memory:.1f}MB, Threads: {self.initial_threads}")
        ...
        async def test_creation_lifecycle():
            try:
                ...
                if creation_time > 1.0:  # Should create in under 1 second
                    self.performance_issues.append(f"Slow engine creation: {creation_time:.3f}s")
                ...
                self.lifecycle_violations.append(f"Creation lifecycle test failed: {e}")
    ...
    def test_factory_lifecycle_management(self):
        ...
        async def test_factory_lifecycle():
            try:
                ...
                for i in range(3):
                    user_id = UnifiedIdGenerator.generate_base_id(f"factory_user_{i}", True, 8)
                ...
                self.lifecycle_violations.append(f"Factory lifecycle test failed: {e}")
        ...
        async def test_resource_limits():
            try:
                ...
                for i in range(max_engines):
                    user_id = UnifiedIdGenerator.generate_base_id(f"resource_user_{i}", True, 8)
                    ...
                    engine.set_agent_state(f"memory_agent_{i}", "x" * 1000)  # 1KB per engine
                    engine.set_agent_result(f"memory_result_{i}", {
                ...
                self.assertEqual(metrics['active_engines_count'], max_engines, f"Should have {max_engines} active engines")
                
                # Test that engines are properly tracked
                for i, engine in enumerate(engines):
                    stats = engine.get_user_execution_stats()
                    self.assertIn('memory_usage', stats, f"Engine {i} should track memory usage")
                    self.assertIn('created_at', stats, f"Engine {i} should track creation time")
                ...
                if memory_leak > 10:  # Allow 10MB overhead
                    self.resource_leaks.append(f"Possible memory leak: {memory_leak:.1f}MB not released")
                ...
                self.resource_leaks.append(f"Resource limits test failed: {e}")
    ...
    def test_error_condition_cleanup(self):
        ...
        async def test_error_cleanup():
            try:
                ...
                engines = []
                for i in range(3):
                    user_id = UnifiedIdGenerator.generate_base_id(f"error_user_{i}", True, 8)
                ...
                for i, engine in enumerate(engines):
                    try:
                        ...
                        cleanup_errors.append(f"Engine {i} cleanup error: {e}")
                ...
                self.cleanup_failures.append(f"Error cleanup test failed: {e}")
            ...
            try:
                ...
                for cycle in range(cycles):
                    ...
                    for i in range(engines_per_cycle):
                        ...
                        thread_id, run_id, _ = UnifiedIdGenerator.generate_user_context_ids(user_id, f"stability_{cycle}")
                        ...
                        engine.set_agent_state(f"stability_agent_{i}", f"cycle_{cycle}_state")
                        engine.set_agent_result(f"stability_result_{i}", {
                            "cycle": cycle,
                            "engine": i,
                            "data": list(range(10))
                        })
                    ...
                    for i, engine in enumerate(cycle_engines):
                        if not engine.is_active():
                            stability_issues.append(f"Cycle {cycle}, Engine {i}: Not active")
                        
                        stats = engine.get_user_execution_stats()
                        if stats['user_id'] != engine.get_user_context().user_id:
                            stability_issues.append(f"Cycle {cycle}, Engine {i}: User ID mismatch")
                ...
                self.assertEqual(final_metrics['total_engines_created'], expected_total_created, 
                               f"Should have created {expected_total_created} engines total")
                ...
                # Validate stability
                self.assertEqual(len(stability_issues), 0, f"Stability issues found: {stability_issues}")
                
                logger.info(f"✅ PASS: Stability test - {cycles} cycles, {expected_total_created} engines, 0 stability issues")
                return True
                
            except Exception as e:
                self.lifecycle_violations.append(f"Stability test failed: {e}")
    ...
    def test_comprehensive_lifecycle_report(self):
        ...
        if resource_leak_detected:
            self.resource_leaks.append(f"Significant resource leak: {memory_change:+.1f}MB, {thread_change:+d} threads")
        
        # This test should PASS if lifecycle management works correctly
        self.assertEqual(
            lifecycle_summary['total_issues'], 0,
            f"ExecutionEngine lifecycle management should work correctly. "
            f"Found {lifecycle_summary['total_issues']} issues. "
            f"Resource changes: {memory_change:+.1f}MB, {thread_change:+d} threads"
        )
