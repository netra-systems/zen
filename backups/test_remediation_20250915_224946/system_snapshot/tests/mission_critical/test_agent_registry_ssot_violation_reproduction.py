...
class AgentRegistrySSoTViolationReproductionTests(SSotAsyncTestCase):
    ...
    async def test_duplicate_registry_imports_conflict(self):
        ...
        for import_path in import_attempts:
            try:
                ...
                self.record_metric(f"import_successful_{module_path}", True)
        ...
        assert len(unique_classes) == 1, (
            f"SSOT VIOLATION: Found {len(unique_classes)} different AgentRegistry classes "
            f"from {len(registry_classes)} imports. Expected exactly 1 class. "
            f"Import paths: {registry_import_paths}"
        )
        ...
        if len(registry_classes) > 1:
            ...
            for i, registry_class in enumerate(registry_classes[1:], 1):
                assert first_class is registry_class, (
                    ...
                    f"is not the same as class from {registry_import_paths[i]}"
                )
    ...
    async def test_websocket_event_delivery_inconsistency(self):
        ...
        for capability in websocket_methods[1:]:
            assert capability == first_capability, (
                f"SSOT VIOLATION: WebSocket capabilities differ between registries. "
                f"Registry '{first_capability['name']}': {first_capability} "
                f"vs Registry '{capability['name']}': {capability}. "
                f"This breaks Golden Path user experience consistency."
            )
    ...
    async def test_multi_user_isolation_breaks_with_mixed_registries(self):
        ...
        for i, capability in enumerate(capability_values[1:], 1):
            ...
            assert capability == first_capability, (
                f"SSOT VIOLATION: User isolation capabilities differ between registries. "
                f"Registry '{registry_names[0]}': {first_capability} "
                f"vs Registry '{registry_names[i]}': {capability}. "
                f"This creates enterprise security risk and data contamination potential."
            )
    ...
    async def test_factory_pattern_inconsistency_reproduction(self):
        ...
        for i, pattern_name in enumerate(pattern_names[1:], 1):
            ...
            self.record_metric(f"factory_method_difference_{pattern_names[0]}_vs_{pattern_name}", len(method_diff))
            
            # This should FAIL initially - different registries have different factory patterns
            assert len(method_diff) == 0, (
                f"SSOT VIOLATION: Factory patterns differ between registries. "
                f"Registry '{pattern_names[0]}' has methods: {first_factory_methods} "
                f"vs Registry '{pattern_name}' has methods: {current_factory_methods}. "
                f"Differences: {method_diff}. This causes developer confusion and inconsistent patterns."
            )
            ...
            assert first_pattern['init_signature'] == pattern['init_signature'], (
                ...
                f"Registry '{pattern_names[0]}': {first_pattern['init_signature']} "
                f"vs Registry '{pattern_name}': {pattern['init_signature']}"
            )
    ...
    async def test_ssot_violation_summary_metrics(self):
        ...
        if len(registry_interfaces) > 1:
            ...
            if basic_methods != advanced_methods:
                ...
                violation_details.append(f"Interface inconsistency: {len(method_diff)} method differences")
        ...
        assert violation_count == 0, (
            f"SSOT VIOLATIONS DETECTED: {violation_count} violations found. "
