...
class MockWebSocketBridge:
    ...
    def __init__(self, collector: WebSocketEventCollector):
...
class CompleteWebSocketTestAgent(BaseAgent):
    ...
    async def execute_core_logic(self, context: ExecutionContext) -> Dict[str, Any]:
        ...
        if 'agent_started' not in self.skip_events:
            await self.emit_agent_started(f"Starting {self.name} to process user request")
...
class PartialWebSocketTestAgent(BaseAgent):
    ...
    def __init__(self, missing_events: List[str], **kwargs):
        super().__init__(**kwargs)
...
class WebSocketEventGuaranteesTests:
    ...
    async def test_all_five_critical_events_must_be_emitted(self, event_collector, mock_bridge):
        ...
        if event_sequence[0] != 'agent_started':
            pytest.fail(f"WEBSOCKET SEQUENCE VIOLATION: agent_started must be first event. "
                       f"Got sequence: {event_sequence}")
        
        # agent_completed should be last
        if event_sequence[-1] != 'agent_completed':
            pytest.fail(f"WEBSOCKET SEQUENCE VIOLATION: agent_completed must be last event. "
                       f"Got sequence: {event_sequence}")
            ...
            if event_type == 'tool_completed':
                ...
                if 'tool_executing' not in preceding_events:
                    pytest.fail(f"WEBSOCKET SEQUENCE VIOLATION: tool_completed without preceding "
                               f"tool_executing. Sequence: {event_sequence}")
    ...
    async def test_agent_thinking_event_violation_detection(self, event_collector, mock_bridge):
        ...
        for event in thinking_events:
            ...
            if not thought or len(thought) < 10:
                pytest.fail(f"WEBSOCKET CONTENT VIOLATION: agent_thinking event has inadequate content: "
    ...
    async def test_concurrent_websocket_event_integrity(self, event_collector, mock_bridge):
        ...
        for i, agent in enumerate(agents):
            agent.set_websocket_bridge(mock_bridge, f"concurrent_run_{i}")
        
        # Execute all agents concurrently
        async def execute_agent(agent, agent_id):
            context = ExecutionContext(
                run_id=f"concurrent_run_{agent_id}",
                agent_name=agent.name,
                state=DeepAgentState()
            )
        ...
        if failed_executions:
            pytest.fail(f"CONCURRENT EXECUTION VIOLATION: {len(failed_executions)} out of "
        ...
        for i in range(num_concurrent_agents):
            ...
            if not run_events:
                pytest.fail(f"CONCURRENT EVENT LOSS: No events captured for concurrent run {run_id}. "
                           f"Events may be lost under concurrent load.")
            
            run_event_types = {e.event_type for e in run_events}
            missing_events = required_events - run_event_types
            
            if missing_events:
                pytest.fail(f"CONCURRENT EVENT INTEGRITY VIOLATION: Run {run_id} missing events "
        ...
        if total_events < expected_events:
            pytest.fail(f"CONCURRENT EVENT LOSS: Expected at least {expected_events} events, "
                       f"got {total_events}. Events are being lost under concurrent load.")
        ...
        for i in range(num_concurrent_agents):
            ...
            if sequence[-1] != 'agent_completed':
                pytest.fail(f"CONCURRENT SEQUENCE VIOLATION: Run {run_id} wrong end event: {sequence}")
    ...
    async def test_websocket_bridge_failure_resilience(self, event_collector, mock_bridge):
        ...
        try:
            ...
            pytest.fail(f"RESILIENCE VIOLATION: Agent execution blocked by WebSocket failure: {e}. "
    ...
    async def test_websocket_event_content_validation(self, event_collector, mock_bridge):
        ...
        for event in started_events:
            ...
            if not message or len(message) < 10:
                pytest.fail(f"CONTENT QUALITY VIOLATION: agent_started event has inadequate "
                           f"message: '{message}'. Users need meaningful start notifications.")
        ...
        for event in thinking_events:
            ...
            if not thought or len(thought) < 15:
                pytest.fail(f"CONTENT QUALITY VIOLATION: agent_thinking event has inadequate "
            ...
            if 'analyzing' not in thought.lower() and 'planning' not in thought.lower() and 'determining' not in thought.lower():
                pytest.fail(f"CONTENT RELEVANCE VIOLATION: agent_thinking lacks problem-solving "
                           f"keywords: '{thought}'. Must show actual reasoning process.")
        ...
        for event in executing_events:
            ...
            if not tool_name or len(tool_name) < 3:
                pytest.fail(f"CONTENT QUALITY VIOLATION: tool_executing event has inadequate "
                           f"tool_name: '{tool_name}'. Users need clear tool identification.")
        
        for event in completed_events:
            result = event.data.get('result')
            if not result or not isinstance(result, dict):
                pytest.fail(f"CONTENT QUALITY VIOLATION: tool_completed event lacks meaningful "
                           f"result data: {result}. Users need actionable tool outcomes.")
        ...
        for event in completed_events:
            result = event.data.get('result')
            if not result or not isinstance(result, dict):
                pytest.fail(f"CONTENT QUALITY VIOLATION: agent_completed event lacks meaningful "
                           f"result: {result}. Users need comprehensive final results.")
            
            # Final result should contain substantive information
            if 'status' not in result or 'final_result' not in result:
                pytest.fail(f"CONTENT STRUCTURE VIOLATION: agent_completed result missing "
