...
class TestDatabaseManagerBackupFileViolations(SSotBaseTestCase):
    ...
    def test_no_backup_files_exist(self):
        ...
        if found_backups:
            ...
            pytest.fail(
                f"SSOT VIOLATION: DatabaseManager backup files found:\n{backup_list}\n"
                f"These files violate SSOT principles and must be removed."
            )
    ...
    def test_single_database_manager_source(self):
        ...
        if len(actual_implementations) == 0:
            ...
            pytest.fail(
                f"SSOT VIOLATION: Multiple DatabaseManager implementations found:\n{impl_list}\n"
    ...
    def test_database_manager_import_consistency(self):
        ...
        forbidden_patterns = [
            ...
            r"import.*database_manager_backup",
            r"from.*\.database_manager_backup",
            r"database_manager_original",
            r"database_manager_temp"
        ]
        ...
        if violations:
            ...
            pytest.fail(
                ...
                f"These imports reference non-existent backup files and must be updated."
            )
    ...
    def test_database_manager_canonical_location(self):
        ...
        if not expected_path.exists():
            pytest.fail(
                f"SSOT VIOLATION: Canonical DatabaseManager not found at expected location:\n"
                f"  Expected: {expected_path}\n"
                f"The DatabaseManager MUST exist at the canonical SSOT location."
            )
        ...
        try:
            with open(expected_path, 'r', encoding='utf-8') as f:
                ...
                if 'class DatabaseManager' not in content:
                    pytest.fail(
                        f"SSOT VIOLATION: File exists but does not contain DatabaseManager class:\n"
                        f"  File: {expected_path}\n"
                        f"The canonical file must contain the DatabaseManager implementation."
                    )
        except Exception as e:
            pytest.fail(f"ERROR reading canonical DatabaseManager file: {e}")
    ...
    def test_no_database_manager_duplicates_in_services(self):
        ...
        if all_implementations:
            ...
            pytest.fail(
                f"SSOT VIOLATION: Duplicate DatabaseManager implementations found:\n{impl_list}\n"
                f"Only canonical location allowed: {canonical_path}\n"
                f"These duplicates violate SSOT principles and must be removed."
            )