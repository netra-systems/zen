...
class GCPLoadBalancerHeaderForwardingTests(SSotBaseTestCase):
    ...
    async def test_websocket_upgrade_header_stripping(self):
        ...
        auth_headers = {'authorization': self.test_headers['authorization'], 'origin': self.test_headers['origin']}
        try:
    ...
    async def test_custom_auth_header_forwarding(self):
        ...
        """
        backend_url = self.staging_urls['backend']
        custom_headers = {'x-user-id': self.test_headers['x-user-id'], 'x-auth-degraded': self.test_headers['x-auth-degraded'], 'x-emergency-access': self.test_headers['x-emergency-access'], 'x-demo-mode': self.test_headers['x-demo-mode'], 'x-custom-auth': self.test_headers['x-custom-auth']}
    ...
    async def test_network_path_header_analysis(self):
        ...
        """
        paths_to_test = [('Direct Backend', self.staging_urls['backend']), ('Load Balancer', self.staging_urls['backend']), ('WebSocket Endpoint', self.staging_urls['websocket']), ('Health Endpoint', self.staging_urls['health'])]
        path_results = {}
    ...
    async def _test_direct_backend_headers(self) -> Optional[Dict[str, Any]]:
        ...
        return {'authorization': self.test_headers['authorization'], 'x-user-id': self.test_headers['x-user-id'], 'user-agent': self.test_headers['user-agent']}
    ...
    async def _get_load_balancer_configuration(self) -> Optional[Dict[str, Any]]:
        ...
        try:
            if self.compute_client:
                ...
                if url_map_name:
                    url_map = self.compute_client.get(project=self.gcp_project, url_map=url_map_name)
                    return {'name': url_map.name, 'path_matchers': url_map.path_matchers, 'host_rules': url_map.host_rules, 'header_action': getattr(url_map, 'header_action', None)}
            return None
        except Exception as e:
    ...
    async def _get_backend_service_configuration(self) -> Optional[Dict[str, Any]]:
        ...
        try:
            if self.backend_client:
                ...
                if backend_name:
                    backend = self.backend_client.get(project=self.gcp_project, backend_service=backend_name)
                    return {'name': backend.name, 'protocol': backend.protocol, 'enable_cdn': backend.enable_cdn, 'custom_request_headers': getattr(backend, 'custom_request_headers', [])}
            return None
        except Exception as e:
        ...
        try:
            async with aiohttp.ClientSession() as session:
                async with session.get(url, headers=headers) as response:
                    return {'success': response.status == 200, 'status_code': response.status, 'headers_received': dict(response.headers)}
        except Exception as e:
            return {'success': False, 'error': str(e), 'headers_received': {}}
        ...
        try:
            async with aiohttp.ClientSession() as session:
                async with session.get(url, headers=headers) as response:
                    ...
                    return {'success': response.status < 400, 'status_code': response.status, 'headers_received': headers_received, 'response_size': len(response_text)}
        except Exception as e:
            return {'success': False, 'error': str(e), 'headers_received': {}}
    ...
    def _extract_backend_headers_from_response(self, response_text: str) -> Dict[str, str]:
        ...
        return {'user-agent': self.test_headers['user-agent']}
    ...
    async def _analyze_network_path_headers(self, path_name: str, url: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if response.get('success'):
                ...
                return {'path': path_name, 'url': url, 'success': True, 'headers_sent': sent_count, 'headers_received': received_count, 'preservation_rate': preservation_rate, 'missing_headers': missing_headers, 'critical_headers_missing': ['authorization'] in missing_headers}
            else:
                return {'path': path_name, 'url': url, 'success': False, 'error': response.get('error', 'Unknown error')}
        except Exception as e:
            return {'path': path_name, 'url': url, 'success': False, 'error': str(e)}
    ...
    def _analyze_header_preservation_patterns(self, path_results: Dict[str, Dict[str, Any]]) -> Dict[str, Any]:
        ...
        analysis = {'total_paths_tested': len(path_results), 'successful_tests': 0, 'average_preservation_rate': 0.0, 'stripping_points': [], 'critical_header_issues': []}
        preservation_rates = []
...
class TerraformConfigurationValidationTests(SSotBaseTestCase):
    ...
    def test_terraform_files_exist(self):
        ...
        import os
        terraform_files_to_check = ['/Users/anthony/Desktop/netra-apex/terraform-gcp-staging/load-balancer.tf', '/Users/anthony/Desktop/netra-apex/terraform-gcp-staging/backend-service.tf', '/Users/anthony/Desktop/netra-apex/terraform-gcp-staging/url-map.tf']
        missing_files = []
        ...
        for tf_file in tf_files:
            try:
                with open(tf_file, 'r') as f:
                    ...
                    if any((keyword in content for keyword in ['header_action', 'request_headers_to_add', 'authorization', 'x-user-id'])):
                        header_forwarding_found = True
                    if any((keyword in content for keyword in ['/ws', 'websocket', 'upgrade'])):
