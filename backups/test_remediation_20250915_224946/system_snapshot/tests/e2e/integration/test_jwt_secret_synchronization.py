...
class JWTSecretSynchronizationValidator:
    ...
    async def validate_secret_consistency_across_services(self, secret: str) -> Dict[str, Any]:
        ...
        return {'auth_status': auth_result['status'], 'backend_status': backend_result['status'], 'websocket_connected': ws_result['connected'], 'auth_duration': auth_duration, 'backend_duration': backend_duration, 'websocket_duration': ws_duration, 'total_duration': total_duration, 'performance_ok': total_duration < 0.05, 'consistent_handling': self._check_consistency(auth_result, backend_result, ws_result)}

    def _create_token_with_secret(self, user_id: str, secret: str) -> str:
        """Create JWT token with specific secret."""
        payload = {'sub': user_id, 'email': 'test@netrasystems.ai', 'permissions': ['read', 'write'], 'iat': int(datetime.now(timezone.utc).timestamp()), 'exp': int((datetime.now(timezone.utc) + timedelta(minutes=15)).timestamp()), 'token_type': 'access', 'iss': 'netra-auth-service'}
        return jwt.encode(payload, secret, algorithm='HS256')

    async def _test_websocket_token_validation(self, token: str) -> Dict[str, Any]:
        """Test WebSocket token validation."""
        try:
            async with websockets.connect(f'{self.websocket_url}/ws?token={token}', timeout=3) as websocket:
    ...
    async def validate_mismatched_secret_rejection(self) -> Dict[str, Any]:
        ...
        total_duration = self.performance.get_duration(start_time)
        return {'auth_rejected': auth_result['status'] == 401, 'backend_rejected': backend_result['status'] == 401, 'websocket_rejected': not ws_result['connected'], 'consistent_rejection': self._check_consistent_rejection(auth_result, backend_result, ws_result), 'total_duration': total_duration, 'performance_ok': total_duration < 0.05}
    ...
    async def simulate_secret_rotation_scenario(self) -> Dict[str, Any]:
        ...
        total_duration = self.performance.get_duration(start_time)
        return {'old_token_rejected': old_auth_result['status'] == 401 and old_backend_result['status'] == 401, 'new_token_handled': new_auth_result['status'] in [200, 401, 500] and new_backend_result['status'] in [200, 401, 500], 'rotation_timing': total_duration, 'performance_ok': total_duration < 0.05, 'simulation_complete': True}
    ...
    async def validate_cross_service_token_flow(self) -> Dict[str, Any]:
        ...
        if not real_token:
            return {'test_skipped': True, 'reason': 'Auth service not available for real token creation', 'performance_ok': True}
        ...
        total_duration = self.performance.get_duration(start_time)
        return {'token_from_auth': True, 'backend_accepts': backend_result['status'] in [200, 500], 'websocket_accepts': ws_result['connected'] or 'timeout' in str(ws_result.get('error', '')), 'cross_service_flow': backend_result['status'] in [200, 500], 'total_duration': total_duration, 'performance_ok': total_duration < 0.05}
...
async def test_jwt_secret_consistency_across_services():
    ...
    consistency_result = await validator.validate_secret_consistency_across_services(validator.test_secret)
...
async def test_jwt_service_token_secret_consistency():
    ...
    backend_token = jwt_helper.create_token({'sub': backend_creds['service_id'], 'service': 'netra-backend', 'token_type': 'service', 'iat': int(datetime.now(timezone.utc).timestamp()), 'exp': int((datetime.now(timezone.utc) + timedelta(minutes=5)).timestamp()), 'iss': 'netra-auth-service'})
    worker_token = jwt_helper.create_token({'sub': worker_creds['service_id'], 'service': 'netra-worker', 'token_type': 'service', 'iat': int(datetime.now(timezone.utc).timestamp()), 'exp': int((datetime.now(timezone.utc) + timedelta(minutes=5)).timestamp()), 'iss': 'netra-auth-service'})
