...
class CompleteSystemHealthValidationTests:
    ...
    async def system_config(self):
        ...
        config = get_config()
        return {'backend_url': 'http://localhost:8000', 'auth_url': 'http://localhost:8081', 'database_enabled': True, 'redis_enabled': not config.TEST_DISABLE_REDIS, 'clickhouse_enabled': config.CLICKHOUSE_ENABLED}
    ...
    async def test_complete_system_startup_health(self, system_config, auth_helper):
        ...
        if system_config['redis_enabled']:
            try:
                ...
                await redis_client.set(test_key, 'health_test', ex=10)
        ...
        if failed_checks:
            failure_summary = '\n'.join([f"  - {check[0]}: {(check[2] if len(check) > 2 else 'Failed')}" for check in failed_checks])
    ...
    async def test_critical_endpoints_availability(self, system_config, auth_helper):
        ...
        start_time = time.time()
        critical_endpoints = [('GET', f"{system_config['backend_url']}/health", 'Backend Health'), ('GET', f"{system_config['auth_url']}/health", 'Auth Health'), ('GET', f"{system_config['backend_url']}/api/v1/threads", 'Threads API')]
        endpoint_results = []
        async with httpx.AsyncClient(timeout=10.0) as client:
        ...
        if failed_endpoints:
            failure_summary = '\n'.join([f'  - {result[0]}: {result[2]}' for result in failed_endpoints])
    ...
    def test_system_resource_availability(self):
        ...
        critical_failures = [check for check in resource_checks if len(check) > 2 and (not check[1]) and (check[0] in ['disk_space'])]
        execution_time = time.time() - start_time
        if critical_failures:
            failure_summary = '\n'.join([f'  - {check[0]}: {check[2]}' for check in critical_failures])
