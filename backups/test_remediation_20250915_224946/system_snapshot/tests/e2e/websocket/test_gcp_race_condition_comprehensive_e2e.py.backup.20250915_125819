...
class TestGCPRaceConditionComprehensiveE2E(SSotBaseTestCase):
    ...
    async def test_websocket_connection_with_race_condition_fix(self):
        ...
        for attempt in range(3):
            ...
            errors = []
            try:
                websocket = await self.websocket_auth_helper.connect_authenticated_websocket(timeout=self.connection_timeout)
                connection_time = time.time() - start_time
                test_message = {'type': 'ping', 'timestamp': datetime.now(timezone.utc).isoformat(), 'user_id': str(user_context.user_id), 'test_id': f'race-condition-test-{attempt}'}
                await websocket.send(json.dumps(test_message))
                response = await asyncio.wait_for(websocket.recv(), timeout=self.message_timeout)
                response_data = json.loads(response)
                connection_results.append(WebSocketTestResult(success=True, connection_time=connection_time, message_count=1, errors=[], websocket_events=[response_data], auth_method='jwt_with_e2e_headers', environment=self.test_environment))
                ...
                connection_results.append(WebSocketTestResult(success=False, connection_time=connection_time, message_count=0, errors=errors, websocket_events=[], auth_method='jwt_with_e2e_headers', environment=self.test_environment))
                ...
                connection_results.append(WebSocketTestResult(success=False, connection_time=time.time() - start_time, message_count=0, errors=errors, websocket_events=[], auth_method='jwt_with_e2e_headers', environment=self.test_environment))
        ...
        websocket_1011_errors = [r for r in failed_connections if any(('1011' in error for error in r.errors))]
        assert len(websocket_1011_errors) == 0, f'WebSocket 1011 errors detected - race condition fix failed: {[r.errors for r in websocket_1011_errors]}'
        assert len(successful_connections) >= 2, f'Too many connection failures: {len(successful_connections)}/3 successful. Errors: {[r.errors for r in failed_connections]}'
    ...
    async def test_message_routing_with_agent_events(self):
        ...
        try:
            websocket = await self.websocket_auth_helper.connect_authenticated_websocket(timeout=self.connection_timeout)
            test_messages = [{'type': 'chat_message', 'content': 'Hello, can you help me test the system?', 'user_id': str(user_context.user_id), 'thread_id': str(user_context.thread_id)}, {'type': 'ping', 'timestamp': datetime.now(timezone.utc).isoformat()}]
            for msg_idx, test_message in enumerate(test_messages):
                ...
                try:
                    ...
                    routing_results.append(MessageRoutingTestResult(message_sent=message_sent, response_received=response_received, routing_time=routing_time, agent_events_received=agent_events_received, errors=errors))
                    ...
                    routing_results.append(MessageRoutingTestResult(message_sent=False, response_received=False, routing_time=time.time() - routing_start, agent_events_received=[], errors=errors))
        finally:
            if websocket:
                await websocket.close()
        ...
        assert len(successful_routing) >= len(routing_results) // 2, f'Too many message routing failures: {len(successful_routing)}/{len(routing_results)}. Errors: {[r.errors for r in failed_routing]}'
    ...
    async def test_gcp_staging_environment_simulation(self):
        ...
        user_context = await create_authenticated_user_context(environment='staging', websocket_enabled=True)
        print('   Testing GCP readiness validation...')
        mock_app_state = type('MockAppState', (), {'db_session_factory': object(), 'database_available': True, 'redis_manager': type('MockRedisManager', (), {'is_connected': lambda: True})(), 'auth_validation_complete': True, 'key_manager': object(), 'agent_supervisor': object(), 'thread_service': object(), 'agent_websocket_bridge': type('MockBridge', (), {'notify_agent_started': lambda: None, 'notify_agent_completed': lambda: None, 'notify_tool_executing': lambda: None})(), 'startup_complete': True, 'startup_failed': False, 'startup_phase': 'complete'})()
        ...
        assert readiness_time >= 0.5 or not details.get('gcp_detected', False), f'GCP readiness should include grace period if GCP detected: {readiness_time}s'
        ...
        for attempt in range(2):
            ...
            websocket = None
            try:
                websocket = await staging_auth_helper.connect_authenticated_websocket(timeout=20.0)
                connection_time = time.time() - start_time
                test_message = {'type': 'staging_test', 'content': 'GCP race condition fix validation', 'timestamp': datetime.now(timezone.utc).isoformat(), 'user_id': str(user_context.user_id)}
                await websocket.send(json.dumps(test_message))
                response = await asyncio.wait_for(websocket.recv(), timeout=15.0)
                staging_connection_results.append({'success': True, 'connection_time': connection_time, 'response_received': True})
                print(f'        PASS:  Staging connection {attempt + 1} successful in {connection_time:.3f}s')
            except Exception as e:
                connection_time = time.time() - start_time
                error_msg = str(e)
                is_race_condition = any((indicator in error_msg.lower() for indicator in ['1011', 'server error', 'connection closed', 'timeout']))
                staging_connection_results.append({'success': False, 'connection_time': connection_time, 'error': error_msg, 'is_race_condition': is_race_condition})
        ...
        race_condition_failures = [r for r in failed_staging if r.get('is_race_condition', False)]
        ...
        assert len(race_condition_failures) == 0, f"Race condition failures in staging simulation: {[r['error'] for r in race_condition_failures]}"
        assert len(successful_staging) >= 1, f"No successful staging connections: {[r.get('error') for r in failed_staging]}"
    ...
    async def test_concurrent_websocket_connections_stress_test(self):
        ...
        for i in range(concurrent_users):
            user_context = await create_authenticated_user_context(user_email=f'stress_test_user_{i}@example.com', environment=self.test_environment, websocket_enabled=True)
        ...
        async def test_single_connection(user_idx: int, user_context) -> Dict[str, Any]:
            ...
            try:
                ...
                test_message = {'type': 'concurrent_test', 'user_id': str(user_context.user_id), 'user_index': user_idx, 'timestamp': datetime.now(timezone.utc).isoformat()}
                await websocket.send(json.dumps(test_message))
                response = await asyncio.wait_for(websocket.recv(), timeout=self.message_timeout)
                return {'user_idx': user_idx, 'success': True, 'connection_time': connection_time, 'response_received': True, 'error': None}
            except Exception as e:
                return {'user_idx': user_idx, 'success': False, 'connection_time': time.time() - connection_start, 'response_received': False, 'error': str(e)}
            finally:
                if websocket:
                    await websocket.close()
        print('   Starting concurrent connections...')
        concurrent_start = time.time()
        concurrent_tasks = [test_single_connection(i, user_contexts[i]) for i in range(concurrent_users)]
        ...
        for result in concurrent_results:
            if isinstance(result, Exception):
                failed_concurrent.append({'user_idx': -1, 'success': False, 'error': str(result)})
            elif result['success']:
        ...
        if successful_concurrent:
            ...
            print(f'   Average connection time: {avg_connection_time:.3f}s')
        race_condition_errors = [r for r in failed_concurrent if r.get('error') and any((indicator in r['error'].lower() for indicator in ['1011', 'race']))]
        print(f'   Race condition errors: {len(race_condition_errors)}')
        assert len(race_condition_errors) == 0, f"Race condition errors under concurrent load: {[r['error'] for r in race_condition_errors]}"
        success_rate = len(successful_concurrent) / concurrent_users
        assert success_rate >= 0.7, f"Too many concurrent connection failures: {success_rate:.1%} success rate. Errors: {[r.get('error') for r in failed_concurrent]}"
    ...
    async def test_websocket_reconnection_with_race_condition_fix(self):
        ...
        for cycle in range(2):
            ...
            try:
                ...
                initial_message = {'type': 'reconnection_test_initial', 'cycle': cycle, 'user_id': str(user_context.user_id)}
                ...
                reconnect_message = {'type': 'reconnection_test_reconnect', 'cycle': cycle, 'user_id': str(user_context.user_id)}
                await websocket.send(json.dumps(reconnect_message))
                response = await asyncio.wait_for(websocket.recv(), timeout=self.message_timeout)
                total_cycle_time = time.time() - cycle_start
                reconnection_results.append({'cycle': cycle, 'success': True, 'initial_connection_time': connection_time, 'reconnection_time': reconnect_time, 'total_cycle_time': total_cycle_time, 'error': None})
                print(f'      PASS:  Cycle {cycle + 1} successful: {total_cycle_time:.3f}s total')
            except Exception as e:
                total_cycle_time = time.time() - cycle_start
                error_msg = str(e)
                reconnection_results.append({'cycle': cycle, 'success': False, 'total_cycle_time': total_cycle_time, 'error': error_msg})
...
class TestWebSocketRaceConditionPerformanceE2E:
    ...
    async def test_websocket_connection_timing_benchmarks(self):
        ...
        timing_scenarios = [{'name': 'Fast Connection', 'iterations': 5}, {'name': 'Repeated Connection', 'iterations': 3}]
        benchmark_results = []
        for scenario in timing_scenarios:
            ...
            if scenario_times:
                ...
                benchmark_results.append({'scenario': scenario['name'], 'iterations': len(scenario_times), 'average_time': avg_time, 'min_time': min_time, 'max_time': max_time, 'success_rate': len(scenario_times) / scenario['iterations']})
