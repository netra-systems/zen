...
class StagingConfigurationValidationTests(BaseE2ETest):
    ...
    async def setup_staging_config_validation(self):
        ...
        self.test_user_context = await create_authenticated_user_context(user_email=f'e2e_config_test_{int(time.time())}@staging.netrasystems.ai', environment='staging', permissions=['read', 'write', 'admin', 'config_validation'])
