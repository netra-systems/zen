...
class AgentPerformanceUnderLoadE2ETests(BaseE2ETest):
    ...
    async def setup_test_environment(self):
        ...
        cleanup_tasks = [WebSocketTestHelpers.close_test_connection(conn) for conn in self.active_connections]
        if cleanup_tasks:
            try:
    ...
    async def create_concurrent_user_session(self, user_index: int) -> Tuple[Any, Any, Any]:
        ...
        websocket_connection = await WebSocketTestHelpers.create_test_websocket_connection(websocket_url, headers=headers, timeout=20.0, max_retries=2, user_id=auth_user.user_id)
        self.active_connections.append(websocket_connection)
        return (auth_user, websocket_connection, user_context)

    async def execute_user_agent_workflow(self, user_index: int, concurrent_user_count: int=1) -> UserLoadTestMetrics:
        """
        ...
        """
        metrics = UserLoadTestMetrics(user_id='', email=f'load_test_user_{user_index}@example.com', start_time=time.time())
        try:
            ...
            agent_message = {'type': 'agent_request', 'agent': 'triage_agent', 'message': f'Load test request #{user_index + 1} of {concurrent_user_count} concurrent users. Please provide system optimization insights for high-load scenarios.', 'user_id': auth_user.user_id, 'thread_id': str(user_context.thread_id), 'request_id': str(user_context.request_id) + f'_load_{user_index}', 'timestamp': datetime.now(timezone.utc).isoformat()}
            ...
            while time.time() - event_start_time < collection_timeout:
                try:
                    ...
                    if 'timeout' in str(e).lower():
                        completion_events = [e for e in metrics.events_received if e.get('type') == 'agent_completed']
    ...
    def measure_system_resources(self) -> Dict[str, float]:
        ...
        try:
            ...
            return {'memory_mb': memory_info.rss / 1024 / 1024, 'memory_delta_mb': memory_info.rss / 1024 / 1024 - self.initial_memory, 'cpu_percent': cpu_percent, 'num_connections': len(self.active_connections)}
        except Exception as e:
    ...
    async def test_concurrent_agent_execution_5_users(self, real_services_fixture):
        ...
        user_tasks = [asyncio.create_task(self.execute_user_agent_workflow(i, concurrent_users)) for i in range(concurrent_users)]
        ...
        for i, result in enumerate(user_metrics):
            if isinstance(result, Exception):
                ...
                failed_metric = UserLoadTestMetrics(user_id=f'failed_user_{i}', email=f'load_test_user_{i}@example.com', start_time=start_time, end_time=time.time(), error_message=str(result))
        ...
        assert results.average_response_time < 30.0, f'Average response time too high: {results.average_response_time:.2f}s (max: 30s)'
        assert results.max_response_time < 60.0, f'Max response time too high: {results.max_response_time:.2f}s (max: 60s)'
        successful_metrics = [m for m in valid_metrics if m.success]
        for metrics in successful_metrics:
            event_types = [event.get('type') for event in metrics.events_received]
            required_events = ['agent_started', 'agent_thinking', 'tool_executing', 'tool_completed', 'agent_completed']
            missing_events = [e for e in required_events if e not in event_types]
            assert len(missing_events) == 0, f'User {metrics.email} missing required events: {missing_events}'
        assert results.memory_usage_mb < 200.0, f'Memory usage too high: {results.memory_usage_mb:.2f} MB (max: 200 MB)'
    ...
    async def test_concurrent_agent_execution_10_users(self, real_services_fixture):
        ...
        for batch_start in range(0, concurrent_users, batch_size):
            ...
            batch_tasks = [asyncio.create_task(self.execute_user_agent_workflow(i, concurrent_users)) for i in range(batch_start, batch_end)]
            ...
            for i, result in enumerate(batch_metrics):
                if isinstance(result, Exception):
                    ...
                    failed_metric = UserLoadTestMetrics(user_id=f'failed_user_{batch_start + i}', email=f'load_test_user_{batch_start + i}@example.com', start_time=start_time, end_time=time.time(), error_message=str(result))
        ...
        assert results.average_response_time < 45.0, f'Average response time too high: {results.average_response_time:.2f}s (max: 45s)'
        assert results.max_response_time < 90.0, f'Max response time too high: {results.max_response_time:.2f}s (max: 90s)'
        assert results.memory_usage_mb < 400.0, f'Memory usage too high: {results.memory_usage_mb:.2f} MB (max: 400 MB)'
    ...
    async def test_rapid_sequential_requests_stress_test(self, real_services_fixture):
        ...
        for request_index in range(num_requests):
            ...
            message = {'type': 'agent_request', 'agent': 'triage_agent', 'message': f'Rapid request #{request_index + 1} - provide quick system status check for stress testing.', 'user_id': auth_user.user_id, 'thread_id': str(user_context.thread_id), 'request_id': str(user_context.request_id) + f'_rapid_{request_index}', 'timestamp': datetime.now(timezone.utc).isoformat()}
            ...
            has_completion = 'agent_completed' in event_types
            request_metrics.append({'request_index': request_index, 'response_time': request_time, 'events_count': len(events), 'success': has_completion, 'event_types': event_types})
        ...
        if first_half_successful and second_half_successful:
            ...
            assert degradation_ratio < 2.0, f'Significant performance degradation during stress test: {degradation_ratio:.2f}x'
    ...
    async def test_resource_cleanup_after_load(self, real_services_fixture):
        ...
        for session_round in range(3):
            ...
            for i in range(num_sessions):
                task = asyncio.create_task(self.execute_user_agent_workflow(i + session_round * num_sessions, num_sessions))
                session_tasks.append(task)
            session_results = await asyncio.gather(*session_tasks, return_exceptions=True)
            current_connections = len(self.active_connections)
            logger.info(f'[U+1F9F9] Cleaning up {current_connections} connections from round {session_round + 1}')
            cleanup_tasks = [WebSocketTestHelpers.close_test_connection(conn) for conn in self.active_connections[-current_connections:]]
            if cleanup_tasks:
                await asyncio.gather(*cleanup_tasks, return_exceptions=True)
            self.active_connections = self.active_connections[:-current_connections]
            await asyncio.sleep(2.0)
            round_end_resources = self.measure_system_resources()
            cleanup_metrics.append({'round': session_round + 1, 'start_memory': round_start_resources.get('memory_mb', 0), 'end_memory': round_end_resources.get('memory_mb', 0), 'memory_delta': round_end_resources.get('memory_mb', 0) - round_start_resources.get('memory_mb', 0), 'connections_cleaned': current_connections, 'final_connections': len(self.active_connections)})
        ...
        assert total_memory_delta < 300.0, f'Excessive memory usage after cleanup: {total_memory_delta:.2f} MB (max: 300 MB)'
        assert len(self.active_connections) < num_sessions, f'Too many connections remaining: {len(self.active_connections)} (should be < {num_sessions})'
        memory_deltas = [m['memory_delta'] for m in cleanup_metrics]
        if len(memory_deltas) >= 2:
            memory_growth = memory_deltas[-1] - memory_deltas[0]
            assert memory_growth < 100.0, f'Memory usage growing across rounds: {memory_growth:+.2f} MB'
