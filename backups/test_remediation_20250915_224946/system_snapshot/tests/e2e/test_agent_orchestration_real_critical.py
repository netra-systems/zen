...
class RealServiceChatTester:
    ...
    async def setup_real_services(self, isolated_env) -> None:
        ...
        self.test_user_token, self.test_user_data = await create_authenticated_user(environment=test_environment, email=unique_email)
    ...
    async def test_critical_chat_flow(self, user_message: str, timeout: float=5.0) -> tuple[bool, RealWebSocketEventValidator]:
        ...
        event_collection_task = asyncio.create_task(self._collect_websocket_events(validator, timeout))
    ...
    async def test_concurrent_chat_sessions_real(self, isolated_test_env):
        ...
        try:
            ...
            for i, (tester, query) in enumerate(zip(testers, queries)):
                task = asyncio.create_task(tester.test_critical_chat_flow(user_message=f'[Session {i + 1}] {query}', timeout=10.0))
                tasks.append(task)
