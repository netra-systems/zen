...
class MultiUserThreadIsolationFramework:
    ...
    def log_thread_access(self, user_id: str, thread_id: str, action: str, success: bool, details: Dict=None):
        ...
        entry = {'timestamp': time.time(), 'relative_time': time.time() - self.start_time, 'user_id': user_id, 'thread_id': thread_id, 'action': action, 'success': success, 'details': details or {}}
        ...
        if thread_owner and thread_owner != user_id:
            violation = {'timestamp': time.time(), 'violating_user': user_id, 'thread_owner': thread_owner, 'thread_id': thread_id, 'action': action, 'success': success, 'violation_type': 'cross_user_thread_access'}
    ...
    def log_websocket_event(self, user_id: str, event_type: str, thread_id: str, event_data: Dict):
        ...
        entry = {'timestamp': time.time(), 'relative_time': time.time() - self.start_time, 'user_id': user_id, 'event_type': event_type, 'thread_id': thread_id, 'is_critical': event_type in self.critical_agent_events, 'event_data': event_data}
        self.websocket_events.append(entry)
        thread_owner = self._find_thread_owner(thread_id)
        if thread_owner and thread_owner != user_id:
            violation = {'timestamp': time.time(), 'event_recipient': user_id, 'thread_owner': thread_owner, 'thread_id': thread_id, 'event_type': event_type, 'violation_type': 'cross_user_event_routing'}
            self.isolation_violations.append(violation)
    ...
    def detect_cross_user_message_leakage(self, received_events: List[Dict], expected_user_id: str) -> List[Dict]:
        ...
        for event in received_events:
            ...
            if event_user_id and event_user_id != expected_user_id:
                leaked_events.append({'event': event, 'expected_user': expected_user_id, 'actual_user': event_user_id, 'leak_type': 'user_id_mismatch'})
            if event_thread_id:
                thread_owner = self._find_thread_owner(event_thread_id)
                if thread_owner and thread_owner != expected_user_id:
                    leaked_events.append({'event': event, 'expected_user': expected_user_id, 'thread_owner': thread_owner, 'thread_id': event_thread_id, 'leak_type': 'thread_cross_contamination'})
    ...
    def generate_isolation_report(self) -> Dict:
        ...
        return {'test_duration': time.time() - self.start_time, 'total_users': total_users, 'total_threads': total_threads, 'isolation_violations': isolation_violations, 'critical_events_logged': critical_events_logged, 'isolation_success_rate': (total_users - isolation_violations) / max(total_users, 1), 'thread_access_attempts': len(self.thread_access_log), 'websocket_events_logged': len(self.websocket_events), 'detailed_violations': self.isolation_violations, 'thread_access_log': self.thread_access_log, 'websocket_events': self.websocket_events}

class IsolatedUserWebSocketClient:
    """WebSocket client with user context isolation and violation detection."""

    def __init__(self, user_context: StronglyTypedUserExecutionContext, auth_helper: E2EWebSocketAuthHelper, isolation_framework: MultiUserThreadIsolationFramework):
            ...
            while self.is_connected and self.websocket:
                try:
                    ...
                    try:
                        ...
                        self.isolation_framework.log_websocket_event(self.user_id, event_type, thread_id, event_data)
    ...
    async def create_thread(self, thread_title: str) -> str:
        ...
        self.isolation_framework.log_thread_access(self.user_id, thread_id, 'create', True, {'title': thread_title})
    ...
    async def send_agent_request(self, thread_id: str, message: str, agent_type: str='triage_agent') -> str:
        ...
        agent_request = {'type': 'agent_request', 'user_id': self.user_id, 'thread_id': thread_id, 'request_id': request_id, 'agent': agent_type, 'message': message, 'timestamp': datetime.now(timezone.utc).isoformat()}
        await self.websocket.send(json.dumps(agent_request))
        self.isolation_framework.log_thread_access(self.user_id, thread_id, 'send_message', True, {'agent': agent_type, 'message': message[:50]})
    ...
    async def wait_for_agent_completion(self, thread_id: str, timeout: float=30.0) -> Dict:
        ...
        while time.time() - start_time < timeout:
            for event in self.received_events:
                if event.get('thread_id') == thread_id and event.get('type') in self.isolation_framework.critical_agent_events:
                    collected_events.append(event)
                    if event.get('type') == 'agent_completed':
                        return {'success': True, 'events': collected_events, 'duration': time.time() - start_time}
...
class TestMultiUserThreadIsolationE2E:
    ...
    async def test_concurrent_user_thread_creation_isolation(self):
        ...
        try:
            ...
            for i in range(num_concurrent_users):
                user_context = await create_authenticated_user_context(user_email=f'isolation_test_user_{i}_{int(time.time())}@example.com', environment=self.test_environment, websocket_enabled=True)
                self.isolation_framework.register_user_context(user_context)
                client = IsolatedUserWebSocketClient(user_context, self.auth_helper, self.isolation_framework)
            ...
            if report['isolation_violations'] > 0:
                ...
                pytest.fail(f"MULTI-USER ISOLATION FAILURE: {report['isolation_violations']} violations detected. Users can access each other's threads. Critical security gap requiring fix.")
            else:
                logger.info(' PASS:  SUCCESS: Perfect multi-user thread isolation achieved!')
                assert successful_users >= num_concurrent_users * 0.8, f'Insufficient successful users: {successful_users}/{num_concurrent_users}'
                assert total_threads_created >= num_concurrent_users * threads_per_user * 0.8, f'Insufficient threads created: {total_threads_created}'
        except Exception as e:
            report = self.isolation_framework.generate_isolation_report()
        ...
        assert actual_duration > 5.0, f'E2E test completed too quickly ({actual_duration:.2f}s). Must test real concurrent scenarios.'
                ...
                for other_client in self.test_users:
                    if other_client != client:
                        for thread_id in other_client.isolation_framework.user_thread_mapping.get(str(other_client.user_id), set()):
                            try:
                                self.isolation_framework.log_thread_access(client.user_id, thread_id, 'unauthorized_access_attempt', False, {'target_user': str(other_client.user_id)})
                            except Exception:
                                pass
                user_violations = len([v for v in self.isolation_framework.isolation_violations if v.get('violating_user') == client.user_id])
                isolation_violations = user_violations
        except Exception as e:
            logger.error(f' FAIL:  User {client.user_id} thread creation test failed: {e}')
            raise
        return {'user_id': client.user_id, 'threads_created': threads_created, 'isolation_violations': isolation_violations}
    ...
    async def test_multi_user_agent_event_isolation(self):
        ...
        try:
            ...
            for i in range(num_users):
                user_context = await create_authenticated_user_context(user_email=f'agent_isolation_user_{i}_{int(time.time())}@example.com', environment=self.test_environment, websocket_enabled=True)
                self.isolation_framework.register_user_context(user_context)
                client = IsolatedUserWebSocketClient(user_context, self.auth_helper, self.isolation_framework)
                user_clients.append(client)
                self.test_users.append(client)
            ...
            agent_tasks = []
            for client in user_clients:
                task = asyncio.create_task(self._test_user_agent_event_isolation(client, agent_requests_per_user))
            ...
            if cross_user_event_leaks > 0 or report['isolation_violations'] > 0:
                ...
                if self.isolation_framework.isolation_violations:
                    for violation in self.isolation_framework.isolation_violations[:3]:
                        logger.error(f' SEARCH:  Violation: {violation}')
                pytest.fail(f"AGENT EVENT ISOLATION FAILURE: {cross_user_event_leaks} cross-user leaks + {report['isolation_violations']} violations. Users receiving events meant for other users. Critical routing bug.")
            else:
                ...
                assert event_delivery_rate >= 0.8, f'Event delivery rate too low: {event_delivery_rate:.2%} (need  >= 80%)'
                assert successful_agent_interactions >= num_users * 0.8, f'Too few successful agent interactions: {successful_agent_interactions}/{num_users}'
        except Exception as e:
            report = self.isolation_framework.generate_isolation_report()
            logger.error(f' CHART:  Partial results: {report}')
    ...
    async def _test_user_agent_event_isolation(self, client: IsolatedUserWebSocketClient, num_requests: int) -> Dict:
        ...
        try:
            async with client.connect(timeout=30.0):
                ...
                leaked_events = self.isolation_framework.detect_cross_user_message_leakage(client.received_events, client.user_id)
                ...
                if cross_user_leaks > 0:
                    ...
                    for leak in leaked_events[:3]:
                        logger.error(f' SEARCH:  Leaked event: {leak}')
        except Exception as e:
            logger.error(f' FAIL:  User {client.user_id} agent event test failed: {e}')
            raise
        return {'user_id': client.user_id, 'events_received': events_received, 'cross_user_leaks': cross_user_leaks}
    ...
    async def test_stress_multi_user_thread_access_patterns(self):
        ...
        concurrent_operation_delay = 0.1
        stress_test_clients = []
        stress_results = []
        try:
            logger.info(f' FIRE:  Setting up {num_stress_users} users for stress testing...')
            for i in range(num_stress_users):
                user_context = await create_authenticated_user_context(user_email=f'stress_test_user_{i}_{int(time.time())}@example.com', environment=self.test_environment, websocket_enabled=True)
                self.isolation_framework.register_user_context(user_context)
                client = IsolatedUserWebSocketClient(user_context, self.auth_helper, self.isolation_framework)
            ...
            for client in stress_test_clients:
                task = asyncio.create_task(self._execute_stress_operations(client, operations_per_user, concurrent_operation_delay))
            ...
            if report['isolation_violations'] > 0:
                ...
                pytest.fail(f"STRESS ISOLATION FAILURE: {report['isolation_violations']} violations under load. Race conditions in thread access patterns cause isolation breaks. System cannot maintain isolation under realistic load.")
            else:
                ...
                assert operation_completion_rate >= 0.8, f'Stress operation completion too low: {operation_completion_rate:.2%}'
                assert successful_stress_users >= num_stress_users * 0.8, f'Too few successful stress users: {successful_stress_users}/{num_stress_users}'
        except Exception as e:
            report = self.isolation_framework.generate_isolation_report()
            ...
            raise
        actual_duration = time.time() - test_start_time
        assert actual_duration > 10.0, f'Stress test completed too quickly ({actual_duration:.2f}s)'
        logger.info(f' PASS:  Stress multi-user thread access test completed in {actual_duration:.2f}s')

    async def _execute_stress_operations(self, client: IsolatedUserWebSocketClient, num_operations: int, operation_delay: float) -> Dict:
        ...
        try:
                ...
                for op_num in range(num_operations):
                    if op_num % 3 == 0:
                        ...
                        if user_threads:
                            ...
                            self.isolation_framework.log_thread_access(client.user_id, thread_id, 'rapid_access', True, {'operation': op_num})
                        operations_completed += 1
            ...
            raise
        return {'user_id': client.user_id, 'operations_completed': operations_completed, 'isolation_violations': isolation_violations}
