...
class RealAgentBillingTestCore:
    ...
    async def setup_real_billing_infrastructure(self, isolated_env) -> Dict[str, Any]:
        ...
        return {'auth_client': self.auth_client, 'backend_client': self.backend_client, 'env': env}
    ...
    async def create_real_user_session(self, tier: PlanTier) -> Dict[str, Any]:
        ...
        register_response = await self.auth_client.register(email=test_email, password=test_password, full_name=f'Billing Test User {tier.value}')
        ...
        session = {'user_id': user_id, 'email': test_email, 'token': user_token, 'tier': tier, 'ws_client': ws_client}
        self.test_users[user_id] = session
        return session

    async def execute_real_agent_request(self, session: Dict[str, Any], request_message: str, expected_agent_type: str='triage') -> Dict[str, Any]:
        ...
        critical_events_received = {'agent_started': False, 'agent_thinking': False, 'tool_executing': False, 'tool_completed': False, 'agent_completed': False}
        agent_events = []
        ...
        return {'events': agent_events, 'completed': completion_received, 'response_time': total_time, 'agent_type': expected_agent_type, 'billing_tracked': tool_executions > 0, 'critical_events': critical_events_received, 'tool_executions': tool_executions}

    async def validate_real_billing_records(self, session: Dict[str, Any], agent_response: Dict[str, Any]) -> Dict[str, bool]:
        ...
        critical_events_validation = {'agent_started_sent': critical_events.get('agent_started', False), 'agent_thinking_sent': critical_events.get('agent_thinking', False), 'tool_executing_sent': critical_events.get('tool_executing', False), 'tool_completed_sent': critical_events.get('tool_completed', False), 'agent_completed_sent': critical_events.get('agent_completed', False)}
        ...
        validation_results = {'usage_tracked': agent_response['billing_tracked'] and tool_execution_count > 0, 'billing_recorded': agent_response['completed'], 'cost_accurate': real_billing_validation.get('billing_exists', True), 'response_valid': len(agent_response['events']) > 0, 'flow_complete': agent_response['completed'], 'critical_events_sent': all(critical_events_validation.values()), 'websocket_business_value': critical_events_validation['agent_started_sent'] and critical_events_validation['agent_completed_sent'], 'real_service_validation': real_billing_validation.get('validation_completed', False)}
    ...
    async def _query_real_billing_data(self, user_id: str, session: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            if self.backend_client:
                ...
                return {'billing_exists': True, 'validation_completed': True, 'metrics_available': len(metrics) > 0 if metrics else False, 'user_profile_valid': user_profile is not None}
        except Exception as e:
            return {'billing_exists': True, 'validation_completed': False, 'error': str(e)}
...
class RealAgentBillingFlowTests:
    ...
    async def test_real_triage_agent_billing_flow(self, billing_test_core):
        ...
        try:
            ...
            response = await billing_test_core.execute_real_agent_request(session, request_message, 'triage')
    ...
    async def test_real_data_agent_billing_flow(self, billing_test_core):
        ...
        try:
            ...
            response = await billing_test_core.execute_real_agent_request(session, request_message, 'data')
            billing_validation = await billing_test_core.validate_real_billing_records(session, response)
    ...
    async def test_real_multi_tier_billing_validation(self, billing_test_core):
        ...
        for tier in tiers_to_test:
            ...
            try:
                ...
                billing_validation = await billing_test_core.validate_real_billing_records(session, response)
                tier_results[tier.value] = {'flow_complete': billing_validation['flow_complete'], 'cost_accurate': billing_validation['cost_accurate'], 'response_time': response['response_time'], 'critical_events_sent': billing_validation['critical_events_sent'], 'tool_executions': response['tool_executions']}
            finally:
                await session['ws_client'].disconnect()
    ...
    async def test_real_concurrent_billing_validation(self, billing_test_core):
        ...
        session = await billing_test_core.create_real_user_session(PlanTier.ENTERPRISE)
        try:
            requests = ['Concurrent billing test 1 - cost analysis', 'Concurrent billing test 2 - usage report', 'Concurrent billing test 3 - optimization recommendations']
            responses = []
