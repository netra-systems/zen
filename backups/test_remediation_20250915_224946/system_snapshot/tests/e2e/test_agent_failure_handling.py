...
try:
    ...
    from netra_backend.app.agents.execution_tracking.tracker import ExecutionTracker, AgentExecutionContext, AgentExecutionResult, ExecutionProgress
...
class MockChatUser:
    ...
    async def send_chat_message(self, message: str, agent_type: str='triage'):
        ...
        chat_message = {'type': 'chat_message', 'data': {'message': message, 'thread_id': self.thread_id, 'user_id': self.user_id, 'agent_type': agent_type, 'timestamp': datetime.now(timezone.utc).isoformat()}}
    ...
    async def wait_for_error_notification(self, timeout_seconds: int=15) -> Optional[Dict[str, Any]]:
        ...
        while time.time() - start_time < timeout_seconds:
            try:
                ...
                message_data = json.loads(message)
                self.received_messages.append(message_data)
                if message_data.get('type') in ['agent_failed', 'agent_death', 'execution_failed', 'error_notification']:
                    return message_data
            except asyncio.TimeoutError:
                continue
        return None
...
class AgentFailureHandlingE2ETests:
    ...
    async def test_user_experience_during_agent_death(self, mock_user):
        ...
        mock_user.websocket.queue_message({'type': 'agent_death', 'data': {'message': 'Agent execution encountered an issue and has been terminated', 'execution_id': 'test-execution-001', 'error': 'Agent process terminated unexpectedly'}})
    ...
    async def test_chat_ui_resilience_during_failures(self, mock_user):
        ...
        print('=' * 80)
        test_scenarios = [{'message': "What's my AWS spending this month?", 'should_fail': False}, {'message': 'Analyze my EC2 costs in detail', 'should_fail': True}, {'message': 'Show me my top 5 most expensive services', 'should_fail': True}, {'message': "What's my current monthly bill?", 'should_fail': False}, {'message': 'Help me understand my data transfer costs', 'should_fail': True}, {'message': 'Simple question: how much did I spend yesterday?', 'should_fail': False}]
        ...
        for i, scenario in enumerate(test_scenarios):
            ...
            if scenario['should_fail']:
                mock_user.websocket.queue_message({'type': 'agent_death', 'data': {'message': f'Agent failed processing message {i + 1}', 'execution_id': f'execution-{i + 1}'}})
        ...
        assert len(completion_messages) >= expected_successes * 0.8, f'Expected ~{expected_successes} completions, got {len(completion_messages)}'
