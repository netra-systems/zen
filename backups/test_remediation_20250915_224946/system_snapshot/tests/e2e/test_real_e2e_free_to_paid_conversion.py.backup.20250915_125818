...
class PaymentSimulator:
    ...
    @staticmethod
    async def process_payment(user_id: str, plan: str, amount: float, card_token: str) -> Dict[str, Any]:
        """Simulate payment processing."""
        await asyncio.sleep(0.5)
        return {'success': True, 'transaction_id': f'txn_{int(time.time())}', 'amount': amount, 'currency': 'USD', 'plan': plan, 'status': 'completed', 'timestamp': datetime.now().isoformat()}
...
class SubscriptionManager:
    ...
    async def create_subscription(self, user_id: str, plan: str, payment_info: Dict[str, Any]) -> Dict[str, Any]:
        ...
        subscription = {'subscription_id': f'sub_{int(time.time())}', 'user_id': user_id, 'plan': plan, 'status': 'active', 'start_date': datetime.now().isoformat(), 'billing_cycle': 'monthly' if 'monthly' in plan else 'annual', 'amount': payment_info['amount'], 'next_billing_date': (datetime.now() + timedelta(days=30)).isoformat(), 'features': self._get_plan_features(plan)}
...
class TestRealE2EFreeToPayConversion(BaseE2ETest):
    ...
    async def simulate_free_tier_usage(self, user_id: str, websocket: MockWebSocketConnection) -> int:
        """Simulate free tier usage until limitations encountered."""
        limitation_count = 0
        start_time = time.time()
        for i in range(5):
            request = {'type': 'agent_request', 'user_id': user_id, 'message': f'Optimize my costs - request {i + 1}', 'agent': 'cost_optimizer'}
            await websocket.send(json.dumps(request))
            response_str = await websocket.recv()
            response = json.loads(response_str)
            if i >= 2:
                limitation_count += 1
                assert response.get('type') == 'tier_limitation', 'Should encounter tier limitation after 3 requests'
                assert 'upgrade' in response.get('message', '').lower(), 'Limitation message should mention upgrade'
    ...
    async def view_upgrade_options(self, user_id: str, websocket: MockWebSocketConnection) -> Dict[str, Any]:
        """View upgrade options and pricing."""
        request = {'type': 'view_pricing', 'user_id': user_id}
        await websocket.send(json.dumps(request))
        response_str = await websocket.recv()
        ...
        assert pricing_info.get('type') == 'pricing_options', 'Should receive pricing options'
    ...
    async def initiate_checkout(self, user_id: str, websocket: MockWebSocketConnection, selected_plan: str) -> Dict[str, Any]:
        """Initiate checkout process."""
        self.conversion_metrics.checkout_initiated = True
        checkout_request = {'type': 'initiate_checkout', 'user_id': user_id, 'plan': selected_plan}
        await websocket.send(json.dumps(checkout_request))
        response_str = await websocket.recv()
        checkout_info = json.loads(response_str)
        assert checkout_info.get('type') == 'checkout_session', 'Should receive checkout session'
    ...
    async def complete_payment(self, user_id: str, websocket: MockWebSocketConnection, checkout_info: Dict[str, Any]) -> Dict[str, Any]:
        """Complete payment process."""
        payment_request = {'type': 'process_payment', 'user_id': user_id, 'session_id': checkout_info['session_id'], 'payment_method': {'type': 'card', 'token': 'tok_test_visa_4242'}}
        payment_result = await self.payment_simulator.process_payment(user_id=user_id, plan=checkout_info['plan_details']['id'], amount=checkout_info['amount'], card_token=payment_request['payment_method']['token'])
        if payment_result['success']:
            self.conversion_metrics.payment_completed = True
            subscription = await self.subscription_manager.create_subscription(user_id=user_id, plan=checkout_info['plan_details']['id'], payment_info=payment_result)
            confirmation = {'type': 'payment_success', 'user_id': user_id, 'subscription': subscription, 'payment': payment_result}
            await websocket.send(json.dumps(confirmation))
            response_str = await websocket.recv()
            confirmation_response = json.loads(response_str)
            assert confirmation_response.get('type') == 'subscription_activated', 'Should receive subscription activation confirmation'
            logger.info(f'Payment completed successfully for user {user_id}')
            return confirmation_response
        else:
            raise Exception('Payment failed in simulation')

    async def verify_premium_access(self, user_id: str, websocket: MockWebSocketConnection) -> bool:
        """Verify access to premium features."""
        premium_request = {'type': 'agent_request', 'user_id': user_id, 'message': 'Perform advanced cost analysis with ML predictions', 'agent': 'advanced_cost_analyzer', 'features': ['ml_predictions', 'custom_reports']}
    ...
    async def test_complete_free_to_paid_conversion_journey(self):
        ...
        try:
            ...
            self.conversion_metrics.annual_revenue_value = 199 * 12
            checkout_info = await self.initiate_checkout(user_id, websocket, selected_plan)
            assert checkout_info['amount'] == 199.0, 'Early plan should be $199/month'
            payment_confirmation = await self.complete_payment(user_id, websocket, checkout_info)
            assert payment_confirmation['subscription']['status'] == 'active', 'Subscription should be active'
            ...
            assert self.conversion_metrics.premium_features_accessed, 'Premium features must be accessible'
            assert self.conversion_metrics.annual_revenue_value >= 2000, 'Conversion must generate significant revenue'
            logger.info(f'Conversion completed successfully:\n  - Tier: {self.conversion_metrics.subscription_tier}\n  - Annual Revenue: ${self.conversion_metrics.annual_revenue_value:,.2f}\n  - Conversion Time: {self.conversion_metrics.conversion_time:.2f}s\n  - Limitation Encounters: {self.conversion_metrics.value_limitation_encounters}')
        finally:
    ...
    async def test_upgrade_triggers_and_value_proposition(self):
        ...
        try:
            limitation_messages = []
            for i in range(5):
                request = {'type': 'agent_request', 'user_id': user_id, 'message': f'Request {i + 1}'}
                await websocket.send(json.dumps(request))
                response = json.loads(await websocket.recv())
                if response.get('type') == 'tier_limitation':
                    limitation_messages.append(response.get('message', ''))
            advanced_request = {'type': 'agent_request', 'user_id': user_id, 'message': 'Generate custom optimization report', 'features': ['custom_reports']}
            ...
            for message in limitation_messages:
                assert any((keyword in message.lower() for keyword in ['upgrade', 'premium', 'unlock', 'pro', 'paid'])), f'Limitation message should mention upgrade: {message}'
            value_props_found = {'unlimited': any(('unlimited' in msg.lower() for msg in limitation_messages)), 'advanced': any(('advanced' in msg.lower() for msg in limitation_messages)), 'priority': any(('priority' in msg.lower() for msg in limitation_messages))}
            assert any(value_props_found.values()), 'At least one value proposition should be communicated'
    ...
    async def test_conversion_revenue_tracking(self):
        ...
        conversion_scenarios = [{'tier': 'early_monthly', 'price': 199, 'annual_value': 2388}, {'tier': 'mid_monthly', 'price': 499, 'annual_value': 5988}, {'tier': 'enterprise', 'price': 2499, 'annual_value': 29988}]
        total_revenue = 0
        ...
        for scenario in conversion_scenarios:
            ...
            websocket = MockWebSocketConnection(user_id=user_id)
            try:
                checkout_info = {'session_id': f'session_{int(time.time())}', 'amount': scenario['price'], 'plan_details': {'id': scenario['tier']}}
                payment_result = await self.payment_simulator.process_payment(user_id=user_id, plan=scenario['tier'], amount=scenario['price'], card_token='tok_test')
                if payment_result['success']:
                    subscription = await self.subscription_manager.create_subscription(user_id=user_id, plan=scenario['tier'], payment_info=payment_result)
                    successful_conversions.append({'user_id': user_id, 'tier': scenario['tier'], 'annual_revenue': scenario['annual_value']})
                    total_revenue += scenario['annual_value']
            finally:
                await websocket.close()
        assert len(successful_conversions) == len(conversion_scenarios), 'All conversions should succeed'
        assert total_revenue == sum((s['annual_value'] for s in conversion_scenarios)), 'Total revenue should match expected'
        assert total_revenue >= 38364, 'Combined revenue should exceed targets'
        logger.info(f'Revenue tracking validated:\n  - Conversions: {len(successful_conversions)}\n  - Total Annual Revenue: ${total_revenue:,}\n  - Average Revenue per User: ${total_revenue / len(successful_conversions):,.2f}')
