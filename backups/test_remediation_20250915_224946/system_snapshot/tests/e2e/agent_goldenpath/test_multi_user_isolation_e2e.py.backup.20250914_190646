...
class TestMultiUserIsolationE2E(SSotAsyncTestCase):
    ...
    def setup_class(cls):
        ...
        cls.max_concurrent_users = 4  # Reasonable load for staging
        cls.isolation_test_scenarios = [
            "cost_optimization",
    ...
    def _create_test_user(self, user_index: int, scenario: str) -> Dict[str, Any]:
        ...
        # Create unique JWT token for this user
        ...
        user_context = {
            ...
            "business_context": self._get_scenario_business_context(scenario, user_index),
            "expected_isolation_markers": [
                user_id, user_email, f"user_{user_index}", scenario
            ]
        }
    ...
    def _get_scenario_business_context(self, scenario: str, user_index: int) -> Dict[str, Any]:
        ...
        scenario_contexts = {
            "cost_optimization": {
                "company_size": f"Company_{user_index}_size",
                "monthly_spend": 5000 + (user_index * 2000),
                "growth_rate": 10 + (user_index * 5),
                "target_savings": 20 + (user_index * 5),
                "unique_identifier": f"cost_opt_user_{user_index}"
            },
            "performance_analysis": {
                "system_load": f"System_{user_index}_load",
                "response_time": 2.0 + (user_index * 0.5),
                "throughput": 1000 + (user_index * 500),
                "optimization_goal": f"perf_goal_{user_index}",
                "unique_identifier": f"perf_user_{user_index}"
            },
            "security_assessment": {
                "compliance_level": f"Level_{user_index}",
                "data_sensitivity": f"Sensitivity_{user_index}",
                "audit_requirements": f"Audit_{user_index}",
                "security_priority": user_index + 1,
                "unique_identifier": f"security_user_{user_index}"
            },
            "infrastructure_review": {
                "infrastructure_type": f"Infrastructure_{user_index}",
                "deployment_scale": f"Scale_{user_index}",
                "service_count": 10 + (user_index * 5),
                "review_focus": f"focus_area_{user_index}",
                "unique_identifier": f"infra_user_{user_index}"
            }
        }
    ...
    def _create_user_message(self, user_context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        message_templates = {
            "cost_optimization": (
                f"I'm from {business_context['company_size']} spending ${business_context['monthly_spend']}/month "
                f"on AI with {business_context['growth_rate']}% growth. My unique situation is "
                f"{business_context['unique_identifier']}. I need {business_context['target_savings']}% savings. "
                f"This is user {user_index} requesting cost optimization analysis."
            ),
            "performance_analysis": (
                f"Our {business_context['system_load']} has {business_context['response_time']}s response time "
                f"with {business_context['throughput']} requests/min. Optimization goal: {business_context['optimization_goal']}. "
                f"User identifier: {business_context['unique_identifier']}. This is user {user_index} analysis request."
            ),
            "security_assessment": (
                f"Security assessment needed for {business_context['compliance_level']} with "
                f"{business_context['data_sensitivity']} data and {business_context['audit_requirements']} requirements. "
                f"Priority: {business_context['security_priority']}. User context: {business_context['unique_identifier']}. "
                f"This is user {user_index} security review."
            ),
            "infrastructure_review": (
                f"Infrastructure review for {business_context['infrastructure_type']} at {business_context['deployment_scale']} "
                f"with {business_context['service_count']} services. Focus: {business_context['review_focus']}. "
                f"Context identifier: {business_context['unique_identifier']}. User {user_index} infrastructure analysis."
            )
        }
        ...
        return {
            ...
            "context": {
                ...
                "expected_isolation_markers": user_context["expected_isolation_markers"]
            }
        }
    ...
    def _validate_response_isolation(self, user_context: Dict[str, Any], response_text: str) -> Dict[str, Any]:
        ...
        """
        validation = {
            "contains_own_markers": False,
            "contains_other_user_data": False,
            "isolation_score": 0.0,
            "own_markers_found": [],
            "other_user_data_found": [],
            "isolation_violation_details": [],
            "isolation_validated": False
        }

        response_lower = response_text.lower()

        # Check for own isolation markers
        own_markers_found = [
            marker for marker in user_context["expected_isolation_markers"]
        ...
        # Check for other user data contamination
        # Look for other user indices, scenarios, or unique identifiers
        other_user_patterns = []

        for other_index in range(self.__class__.max_concurrent_users):
            if other_index != user_context["user_index"]:
                other_user_patterns.extend([
                    f"user_{other_index}",
                    f"user {other_index}",
                    f"isolation_user_{other_index}",
                ])

        # Check for other scenario contamination
        ...
        for other_scenario in other_scenarios:
            for other_index in range(self.__class__.max_concurrent_users):
                if other_index != user_context["user_index"]:
                    other_user_patterns.extend([
                        f"{other_scenario}_user_{other_index}",
                        f"unique_identifier_{other_scenario}_{other_index}"
                    ])
        ...
        if validation["contains_own_markers"] and not validation["contains_other_user_data"]:
            ...
            validation["isolation_score"] = 0.5  # Partial - has own data but contaminated
        elif not validation["contains_other_user_data"]:
            validation["isolation_score"] = 0.7  # Acceptable - no contamination but generic response
        else:
            validation["isolation_score"] = 0.0  # Failed - contaminated with other user data

        # Record violation details
        if validation["contains_other_user_data"]:
            validation["isolation_violation_details"] = [
                f"Response contains other user data: {contamination_found}",
                f"Expected isolation for user {user_context['user_index']} in scenario {user_context['scenario']}"
            ]
    ...
    async def _process_user_scenario(self, user_context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            # Step 1: Establish user-specific WebSocket connection
            ...
            websocket = await asyncio.wait_for(
                websockets.connect(
                    ...
                    additional_headers={
                        ...
                        "X-Test-Suite": "multi-user-isolation-e2e",
                        "X-User-Context": user_context["user_id"],
                        "X-Isolation-Test": "enabled"
                    },
                    ssl=ssl_context
                ),
                timeout=20.0
            ...
            # Step 2: Send user-specific message
            user_message = self._create_user_message(user_context)
            await websocket.send(json.dumps(user_message))

            # Step 3: Collect user-specific events
            user_events = []
            response_timeout = 75.0  # Reasonable timeout for concurrent processing
            ...
            while time.time() - collection_start < response_timeout:
                try:
                    ...
                    # Continue processing - single decode error shouldn't fail user
                    continue

            await websocket.close()

            # Step 4: Validate user isolation
            ...
            if user_results["final_response"]:
                # Extract response text for isolation validation
    ...
    async def test_concurrent_multi_user_agent_processing_isolation(self):
        ...
        try:
            # Step 1: Create multiple test users with different scenarios
            test_users = []
            ...
            # Step 2: Process all users concurrently
            concurrent_start = time.time()

            # Create tasks for concurrent processing
            user_tasks = [
                self._process_user_scenario(user_context)
            ...
            # Step 3: Validate concurrent processing results
            ...
            isolation_metrics.append({
                ...
                "exception_users": len(exception_results),
            ...
            # CRITICAL: Minimum success rate for concurrent processing
            success_rate = len(successful_results) / len(test_users)
            assert success_rate >= 0.75, (
                f"Concurrent processing success rate too low: {success_rate:.1%} "
                f"(required ≥75%). Failed: {len(failed_results)}, Exceptions: {len(exception_results)}"
            )

            # Step 4: Validate isolation across successful results
            ...
            for i, result in enumerate(successful_results):
                ...
                if not isolation_validation.get("isolation_validated", False):
                    isolation_violations.append({
                        ...
                        "violation_details": isolation_validation.get("isolation_violation_details", []),
                        "isolation_score": isolation_validation.get("isolation_score", 0.0),
                        "other_user_data": isolation_validation.get("other_user_data_found", [])
                    })

            assert len(isolation_violations) == 0, (
                f"Isolation violations detected across {len(isolation_violations)} users: "
                f"{isolation_violations}. Multi-user isolation is critical for enterprise security."
            )
            ...
            if successful_results:
                ...
                assert avg_duration < 120.0, (
                    f"Average response time too slow under concurrent load: {avg_duration:.1f}s (max 120s)"
                )
                assert max_duration < 180.0, (
                    f"Max response time too slow: {max_duration:.1f}s (max 180s)"
                )

                isolation_metrics.append({
                    "metric": "concurrent_performance",
            ...
            # Step 5: Validate isolation score distribution
            isolation_scores = [
                r["isolation_validation"]["isolation_score"]
                for r in successful_results
                if r.get("isolation_validation")
            ]
            ...
            if isolation_scores:
                ...
                assert avg_isolation_score >= 0.8, (
                    f"Average isolation score too low: {avg_isolation_score:.2f} (required ≥0.8)"
                )
                assert min_isolation_score >= 0.7, (
                    f"Minimum isolation score too low: {min_isolation_score:.2f} (required ≥0.7)"
                )

                isolation_metrics.append({
                    "metric": "isolation_quality",
                    "avg_isolation_score": avg_isolation_score,
                    "min_isolation_score": min_isolation_score,
                    "isolation_scores": isolation_scores
                })

            # Final concurrent isolation reporting
            ...
            # Log detailed results for each successful user
            for result in successful_results:
                isolation_val = result.get("isolation_validation", {})
                self.logger.info(f"   User {result['user_index']} ({result['scenario']}): "
                               f"Score {isolation_val.get('isolation_score', 0):.2f}, "
            ...
            # Fail with multi-user isolation impact context
            raise AssertionError(
                f"Multi-user isolation validation failed after {total_time:.1f}s: {e}. "
                f"Isolation failures threaten enterprise trust and regulatory compliance. "
                f"Isolation metrics: {isolation_metrics}"
            )
    ...
    async def test_user_context_boundary_validation(self):
        ...
        try:
            # Step 1: Create two distinct users with highly differentiated contexts
            user_a_context = self._create_test_user(1, "cost_optimization")
            user_b_context = self._create_test_user(2, "security_assessment")

            # Add unique sensitive data to each user context
            user_a_context["sensitive_data"] = {
                "company_name": "ACME_Corp_User_A",
                "budget_secret": "UserA_Budget_12345",
                "confidential_metric": "UserA_Metric_Secret"
            }

            user_b_context["sensitive_data"] = {
                "company_name": "Beta_Industries_User_B",
                "security_level": "UserB_Security_Level_9",
                "confidential_assessment": "UserB_Assessment_Secret"
            }

            boundary_metrics.append({
                "metric": "user_context_setup",
                "user_a_id": user_a_context["user_id"],
                "user_b_id": user_b_context["user_id"],
                "context_separation": "configured"
            })

            # Step 2: Process both users with sensitive context data
            user_a_message = self._create_user_message(user_a_context)
            user_a_message["message"] += (
                f" My company {user_a_context['sensitive_data']['company_name']} has budget constraints "
                f"with reference {user_a_context['sensitive_data']['budget_secret']}."
            )

            user_b_message = self._create_user_message(user_b_context)
            user_b_message["message"] += (
                f" Our organization {user_b_context['sensitive_data']['company_name']} requires "
                f"security level {user_b_context['sensitive_data']['security_level']} assessment."
            )

            # Process both users sequentially to validate complete isolation
            user_results = []

            for user_context, user_message in [(user_a_context, user_a_message), (user_b_context, user_b_message)]:
                user_start = time.time()

                # Establish user-specific connection
                ssl_context = ssl.create_default_context()
                ...
                websocket = await asyncio.wait_for(
                    websockets.connect(
                        ...
                        additional_headers={
                            "Authorization": f"Bearer {user_context['access_token']}",
                            "X-Environment": "staging",
                            "X-Test-Suite": "boundary-validation-e2e",
                            "X-User-Boundary-Test": user_context["user_id"]
                        },
                        ssl=ssl_context
                    ),
                    timeout=20.0
                ...
                # Send user message with sensitive context
                await websocket.send(json.dumps(user_message))
                ...
                # Extract response for boundary validation
                response_text = ""
                if final_response:
            ...
            # Step 3: Validate privacy boundaries
            ...
            for i, result in enumerate(user_results):
                ...
                # Check for other user's sensitive data in current response
                        ...
                        for sensitive_key, sensitive_value in other_user["sensitive_data"].items():
                            if sensitive_value.lower() in current_response.lower():
                                contamination_violations.append({
                                    "victim_user": current_user["user_id"],
                                    "leaked_from_user": other_user["user_id"],
                                    "leaked_data": sensitive_value,
                                    "data_type": sensitive_key,
                                    "response_excerpt": current_response[:200] + "..."
                                })

            # CRITICAL: No cross-user data contamination allowed
            assert len(contamination_violations) == 0, (
                f"PRIVACY VIOLATION: Cross-user data contamination detected: {contamination_violations}. "
                f"This is a critical security and compliance failure."
            )

            # Validate each user received appropriate context-specific responses
            ...
            for result in user_results:
                ...
                # Check that response contains user's own context markers
                own_markers_found = [
                    marker for marker in user_context["expected_isolation_markers"]
                ...
                if len(own_markers_found) == 0:
                    context_validation_failures.append({
                        "user_id": user_context["user_id"],
                        "scenario": user_context["scenario"],
                        "expected_markers": user_context["expected_isolation_markers"],
                        "response_excerpt": response_text[:200] + "..."
                    })

            # Users should receive context-relevant responses
            assert len(context_validation_failures) == 0, (
                f"Context validation failures: {context_validation_failures}. "
                f"Users should receive responses relevant to their specific context."
            )

            # Final boundary validation reporting
            total_boundary_test_time = time.time() - boundary_test_start_time

            boundary_metrics.append({
                "metric": "boundary_validation_complete",
                "total_test_time": total_boundary_test_time,
                "users_tested": len(user_results),
                "contamination_violations": len(contamination_violations),
                "context_validation_failures": len(context_validation_failures),
                "privacy_boundaries_validated": True
            })
            ...
            raise AssertionError(
                f"User context boundary validation failed after {total_time:.1f}s: {e}. "
                f"Privacy boundary failures create serious compliance and security risks."
            )
