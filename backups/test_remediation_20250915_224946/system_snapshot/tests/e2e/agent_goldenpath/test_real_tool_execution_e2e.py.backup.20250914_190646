...
class TestRealToolExecutionE2E(SSotAsyncTestCase):
    ...
    def setup_class(cls):
        ...
        # Tool execution test patterns
        cls.tool_execution_patterns = {
            "data_analysis": [
                "analyze", "calculate", "compute", "metrics", "statistics"
            ],
            "cost_optimization": [
                "cost", "savings", "optimization", "efficiency", "budget"
            ],
            "technical_lookup": [
                "documentation", "lookup", "search", "find", "retrieve"
            ],
            "calculation": [
        ...
        # Business value indicators for tool-assisted responses
        cls.tool_value_indicators = [
            "based on analysis", "calculated", "according to data",
            "tool results show", "analysis indicates", "computed"
        ]
    ...
    def _extract_tool_events(self, events: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        return {
            ...
            "tool_completion_count": len(tool_results)
        }
    ...
    def _analyze_tool_integration(self, response_text: str) -> Dict[str, Any]:
        ...
        data_patterns = [
            ...
            r'\d+\s+(bytes?|KB|MB|GB)',  # Data sizes
        ]
        ...
        # Count calculation-style results
        calculation_patterns = [
            r'calculated?\s+\w+',
            r'computed?\s+\w+',
            r'analysis shows?\s+\w+',
            r'results? indicate?\s+\w+'
        ]
        ...
        quality_components = [
            min(integration_analysis["tool_references"] / 2, 1.0) * 0.4,  # Tool references
            min(integration_analysis["data_points"] / 5, 1.0) * 0.3,      # Data points
            min(integration_analysis["calculation_results"] / 2, 1.0) * 0.3  # Calculations
        ]
    ...
    async def test_data_analysis_tool_execution(self):
        ...
        try:
            # Create user for data analysis testing
            data_user = await self.auth_helper.create_authenticated_user(
                email=f"data_tool_test_{self.test_id}@test.com",
                permissions=["read", "write", "chat", "tool_execution", "data_analysis"]
            )
            
            # Connect WebSocket for tool monitoring
            ...
            websocket = await asyncio.wait_for(
                websockets.connect(
                    ...
                    additional_headers=self.auth_helper.get_websocket_headers(data_user.jwt_token),
                    ssl=ssl_context,
                    ping_interval=30,
                    ping_timeout=10
                ),
            ...
            # Send data analysis request that should trigger tool execution
            analysis_request = {
                "type": "chat_message",
                "content": (
                    "I need a detailed analysis of my AI infrastructure costs. "
                    "Current monthly spend is $15,000 with 500,000 API calls. "
                    "Please analyze cost per call, identify optimization opportunities, "
                    "and calculate potential savings with different optimization strategies. "
                    "Use data analysis tools to provide specific metrics and recommendations."
                ),
                ...
                "context": {
                    "test_type": "data_analysis_tools",
                    "expects_tool_execution": True,
                    "tool_categories": ["data_analysis", "cost_optimization"]
                }
            }
            ...
            # Monitor for tool execution events
            all_events = []
            tool_execution_detected = False
            tool_completion_detected = False
            final_response = None
            
            timeout = 90.0  # Extended for tool execution
            ...
            assert tool_execution_detected or tool_analysis["tool_execution_count"] > 0, (
                f"Should detect tool execution for data analysis. "
                f"Tool events: {len(tool_analysis['tool_events'])}"
            )
            ...
            # Analyze tool integration in response
            ...
            assert len(response_text) > 200, (
                f"Data analysis response should be substantial: {len(response_text)} chars"
            )
            
            assert integration_analysis["integration_quality"] >= 0.3, (
                f"Tool results should be well integrated: {integration_analysis['integration_quality']:.3f} "
                f"(tool references: {integration_analysis['tool_references']}, "
                f"data points: {integration_analysis['data_points']})"
            )
            ...
            assert len(found_indicators) >= 2, (
                f"Should contain business value indicators: found {found_indicators}"
            )
            ...
            raise AssertionError(
                f"Data analysis tool execution failed: {e}. "
                f"This breaks advanced AI capabilities requiring tool integration."
            )
    ...
    async def test_calculation_tool_execution(self):
        ...
        try:
            # Create user for calculation testing
            calc_user = await self.auth_helper.create_authenticated_user(
                email=f"calc_tool_test_{self.test_id}@test.com",
                permissions=["read", "write", "chat", "tool_execution", "calculations"]
            )
            ...
            websocket = await asyncio.wait_for(
                websockets.connect(
                    self.staging_config.urls.websocket_url,
                    additional_headers=self.auth_helper.get_websocket_headers(calc_user.jwt_token),
                    ssl=ssl_context
                ),
                timeout=15.0
            )
            
            # Send calculation-heavy request
            calculation_request = {
                "type": "chat_message",
                "content": (
                    "Please calculate ROI for our AI optimization initiatives: "
                    "Current costs: $50,000/month, "
                    "Projected savings after optimization: 35% reduction, "
                    "Implementation cost: $25,000 one-time, "
                    "Timeline: 6 months implementation. "
                    "Calculate monthly savings, payback period, 12-month ROI, "
                    "and 3-year total value. Use calculation tools for precision."
                ),
                "thread_id": self.thread_id,
                "run_id": self.run_id,
                "user_id": calc_user.user_id,
                "timestamp": datetime.now(timezone.utc).isoformat(),
                "context": {
                    "test_type": "calculation_tools",
                    "expects_calculations": True,
                    "calculation_types": ["roi", "savings", "payback", "projections"]
                }
            }
            ...
            # Monitor for calculation tool execution
            all_events = []
            calculation_events = []
            final_response = None
            ...
            while time.time() - collection_start < timeout:
                try:
                    ...
                    # Track calculation-related events
            ...
            # Validate calculation execution
            assert final_response is not None, "Should receive calculation response"
            
            # Extract response and analyze calculations
            response_data = final_response.get("data", {})
            result = response_data.get("result", {})
            response_text = str(result) if result else ""
            
            # Count numeric results in response
            numeric_patterns = [
                r'\$[\d,]+',  # Dollar amounts
                r'\d+%',      # Percentages  
                r'\d+\.\d+',  # Decimal numbers
            ...
            # Validate calculation integration
            integration_analysis = self._analyze_tool_integration(response_text)
            
            # Validate calculation results
            assert len(response_text) > 150, (
                f"Calculation response should be detailed: {len(response_text)} chars"
            )
            
            assert calculation_metrics["numeric_results"] >= 3, (
                f"Should contain multiple calculated values: {calculation_metrics['numeric_results']} found"
            )
            
            # Validate calculation-specific terms
            calc_terms = ["calculated", "computed", "roi", "savings", "payback"]
            response_lower = response_text.lower()
            found_calc_terms = [term for term in calc_terms if term in response_lower]
            
            assert len(found_calc_terms) >= 2, (
                f"Should contain calculation terminology: found {found_calc_terms}"
            )
            
            # Check for specific expected calculations
            expected_values = ["$17,500", "35%", "payback", "roi"]  # Based on the request
            ...
            raise AssertionError(
                f"Calculation tool execution failed: {e}. "
                f"This breaks computational capabilities and business calculations."
            )
    ...
    async def test_multi_tool_workflow_execution(self):
        ...
        try:
            # Create user for workflow testing
            workflow_user = await self.auth_helper.create_authenticated_user(
                email=f"workflow_tool_test_{self.test_id}@test.com",
                permissions=["read", "write", "chat", "tool_execution", "advanced_analysis"]
            )
            ...
            websocket = await asyncio.wait_for(
                websockets.connect(
                    self.staging_config.urls.websocket_url,
                    additional_headers=self.auth_helper.get_websocket_headers(workflow_user.jwt_token),
                    ssl=ssl_context
                ),
                timeout=15.0
            )
            
            # Send complex request requiring multiple tool coordination
            workflow_request = {
                "type": "chat_message",
                "content": (
                    "I need a comprehensive AI infrastructure assessment for our enterprise: "
                    "1) Analyze current usage patterns from our data (10M+ API calls/month), "
                    "2) Calculate cost optimization potential across different models, "
                    "3) Research best practices for our industry (FinTech), "
                    "4) Generate implementation timeline with specific milestones, "
                    "5) Calculate ROI projections for 12-month optimization program. "
                    "Please use multiple analysis tools to provide detailed recommendations "
                    "with data-driven insights and actionable next steps."
                ),
                "thread_id": self.thread_id,
                "run_id": self.run_id,
                ...
                "timestamp": datetime.now(timezone.utc).isoformat(),
                "context": {
                    "test_type": "multi_tool_workflow",
                    "expects_multiple_tools": True,
                    "workflow_complexity": "high",
                    "expected_tools": ["data_analysis", "calculations", "research", "planning"]
                }
            }
            ...
            # Monitor for multi-tool execution workflow
            all_events = []
            tool_sequence = []
            workflow_events = []
            final_response = None
            
            timeout = 120.0  # Extended for complex workflow
            collection_start = time.time()
            ...
            # Validate multiple tools were used
            assert workflow_metrics["workflow_steps"] >= 2, (
                f"Should execute multiple workflow steps: {workflow_metrics['workflow_steps']} detected"
            )
            
            # Extract and analyze comprehensive response
            response_data = final_response.get("data", {})
            result = response_data.get("result", {})
            response_text = str(result) if result else ""
            ...
            assert len(response_text) > 500, (
                f"Multi-tool workflow should generate comprehensive response: {len(response_text)} chars"
            )
            
            # Analyze tool integration across workflow
            integration_analysis = self._analyze_tool_integration(response_text)
            workflow_metrics["coordination_score"] = integration_analysis["integration_quality"]
            
            # Validate workflow coordination quality
            assert integration_analysis["integration_quality"] >= 0.4, (
                f"Multi-tool workflow should show high integration: {integration_analysis['integration_quality']:.3f}"
            )
            
            # Validate comprehensive business coverage
            ...
            assert coverage_rate >= 0.6, (
                f"Should cover most business areas: {coverage_rate:.1%} ({covered_areas})"
            )
            ...
            raise AssertionError(
                f"Multi-tool workflow execution failed: {e}. "
                f"This breaks complex analysis capabilities requiring tool coordination."
            )
    ...
    async def test_tool_error_handling_and_recovery(self):
        ...
        try:
            # Create user for error handling testing
            error_user = await self.auth_helper.create_authenticated_user(
                email=f"tool_error_test_{self.test_id}@test.com",
                permissions=["read", "write", "chat", "tool_execution", "error_testing"]
            )
            ...
            websocket = await asyncio.wait_for(
                websockets.connect(
                    self.staging_config.urls.websocket_url,
                    additional_headers=self.auth_helper.get_websocket_headers(error_user.jwt_token),
                    ssl=ssl_context
                ),
                timeout=15.0
            )
            
            # Send request that might cause tool issues
            error_test_request = {
                "type": "chat_message",
                "content": (
                    "Please analyze data from the 'nonexistent_dataset_12345' and calculate "
                    "optimization metrics using invalid_analysis_tool. If tools fail, "
                    "please provide alternative analysis based on available information. "
                    "I still need actionable recommendations for AI cost optimization."
                ),
                "thread_id": self.thread_id,
                "run_id": self.run_id,
            ...
            # Monitor for error handling and recovery
            all_events = []
            error_events = []
            recovery_events = []
            final_response = None
            ...
            # Extract response and validate graceful degradation
            response_data = final_response.get("data", {})
            result = response_data.get("result", {})
            response_text = str(result) if result else ""
            
            # Validate response quality despite tool issues
            assert len(response_text) > 100, (
                f"Should provide meaningful response despite tool errors: {len(response_text)} chars"
            )
            
            # Check for graceful degradation indicators
            degradation_indicators = [
                "alternative", "available information", "based on", "recommendations",
                "despite", "however", "alternatively", "instead"
            ]
            response_lower = response_text.lower()
            ...
            # Validate that response still provides business value
            ...
            assert len(found_value) >= 1, (
                f"Should maintain business value despite tool errors: found {found_value}"
            )
            
            # Error handling shouldn't break user experience
            assert "error" not in response_lower or "sorry" in response_lower, (
                "User-facing response should handle errors gracefully"
            )
            ...
            raise AssertionError(
                f"Tool error handling and recovery failed: {e}. "
                f"This breaks system resilience and user experience."
            )
    ...
    async def test_tool_performance_and_efficiency(self):
        ...
        try:
            ...
            perf_user = await self.auth_helper.create_authenticated_user(
                email=f"tool_perf_test_{self.test_id}@test.com",
                permissions=["read", "write", "chat", "tool_execution", "performance_testing"]
            )
            ...
            websocket = await asyncio.wait_for(
                websockets.connect(
                    self.staging_config.urls.websocket_url,
                    additional_headers=self.auth_helper.get_websocket_headers(perf_user.jwt_token),
                    ssl=ssl_context
                ),
                timeout=15.0
            )
            ...
            performance_request = {
                "type": "chat_message",
                "content": (
                    "Quick analysis needed: Calculate savings from reducing GPT-4 usage by 25% "
                    "on current monthly spend of $8,000. Include implementation recommendations. "
                    "Please provide efficient tool-based analysis with fast turnaround."
                ),
                "thread_id": self.thread_id,
                "run_id": self.run_id,
                ...
                "context": {
                    ...
                    "expects_efficient_execution": True,
            ...
            if performance_metrics["tool_execution_times"]:
                ...
                # Calculate efficiency score (faster = better)
                efficiency_score = max(0, 1.0 - (avg_tool_time / 10.0))  # 10s baseline
            ...
            assert total_processing_time < 40.0, (
                f"Performance test should complete quickly: {total_processing_time:.2f}s (max 40s)"
            )
            
            if first_response_time:
                assert first_response_time < 8.0, (
                    f"First response should be fast: {first_response_time:.2f}s (max 8s)"
                )
            
            # Validate tool efficiency
            if performance_metrics["tool_execution_times"]:
                assert avg_tool_time < 8.0, (
                    f"Average tool execution should be efficient: {avg_tool_time:.2f}s (max 8s)"
                )
                
                assert max_tool_time < 15.0, (
                    f"Maximum tool execution should be reasonable: {max_tool_time:.2f}s (max 15s)"
                )
            
            # Validate response quality vs speed trade-off
            response_data = final_response.get("data", {})
            result = response_data.get("result", {})
            response_text = str(result) if result else ""
            
            # Should still provide valuable response despite speed focus
            assert len(response_text) > 80, (
                f"Should maintain response quality despite speed focus: {len(response_text)} chars"
            )
            ...
            assert len(found_terms) >= 1, (
                f"Should maintain business value despite speed focus: {found_terms}"
            )
            ...
            raise AssertionError(
                f"Tool performance and efficiency test failed: {e}. "
                f"This indicates tool execution performance issues."
            )
