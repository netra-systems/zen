...
class TestSecurityAuthorizationEdgeCasesE2E(SSotAsyncTestCase):
    ...
    def setup_method(self, method):
        ...
        # Create legitimate test user for baseline comparison
        ...
        self.legitimate_access_token = self.__class__.auth_helper.create_test_jwt_token(
            user_id=self.legitimate_user_id,
            email=self.legitimate_user_email,
            exp_minutes=60
        )
    ...
    async def _attempt_websocket_connection(self, headers: Dict[str, str],
        ...
        try:
            ...
            # If connection succeeded, try sending a test message
            if should_succeed:
                test_message = {
                    "type": "ping",
                    "timestamp": time.time()
            ...
            connection_result["success"] = not should_succeed  # Success if we expected failure
    ...
    async def _attempt_agent_request_with_auth(self, auth_token: str,
                                             should_succeed: bool = True,
        ...
        try:
            # Attempt connection with auth token
            connection_result = await self._attempt_websocket_connection({
                "Authorization": f"Bearer {auth_token}",
                "X-Environment": "staging",
                "X-Test-Suite": "security-authorization-edge-cases"
            }, should_succeed=True, timeout=15.0)
            ...
            test_request = {
                "type": "agent_request",
                "agent": "triage_agent",
                "message": f"Security test request - should {'succeed' if should_succeed else 'fail'}",
                "thread_id": f"security_test_{int(time.time())}",
                "run_id": f"security_run_{int(time.time())}",
                "user_id": self.legitimate_user_id,
                "context": {
                    "security_test": True,
                    "expected_outcome": "success" if should_succeed else "failure"
                }
            }
    ...
    async def test_expired_token_rejection(self):
        ...
        expired_token = self.__class__.auth_helper.create_test_jwt_token(
            user_id=f"expired_user_{int(time.time())}",
            email=f"expired_{int(time.time())}@netra-testing.ai",
        ...
        assert connection_result["success"], (
            ...
            f"Connection result: {connection_result}"
        )

        # Should not establish successful connection
        assert not connection_result["connected"], (
            f"Should not establish connection with expired token"
        )
        ...
        if connection_result["error"]:
            ...
            assert has_auth_error, (
                f"Should receive authentication-related error for expired token. "
    ...
    async def test_invalid_token_format_rejection(self):
        ...
        invalid_tokens = [
            "invalid-jwt-token",  # Plain string
            "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.invalid-payload.invalid-signature",  # Invalid parts
            "Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9",  # Missing parts
            "",  # Empty token
            "null",  # Null string
            base64.b64encode(b"not-a-jwt-token").decode(),  # Base64 but not JWT
            ...
            "malformed" * 100,  # Very long malformed token
        ]
        ...
        for i, invalid_token in enumerate(invalid_tokens):
            ...
            connection_result = await self._attempt_websocket_connection({
                ...
                "X-Environment": "staging",
                "X-Test-Suite": f"invalid-token-format-{i+1}"
            }, should_succeed=False, timeout=10.0)

            rejection_results.append({
                "token_type": f"invalid_format_{i+1}",
                "token_preview": invalid_token[:50] + "..." if len(invalid_token) > 50 else invalid_token,
        ...
        assert rejection_rate >= 0.9, (
            f"Invalid token rejection rate too low: {rejection_rate:.1%} (min 90%). "
            f"Failed rejections: {[r['token_type'] for r in failed_rejections]}"
        )

        # No connections should be established with invalid tokens
        established_connections = [r for r in rejection_results if r["result"]["connected"]]
        assert len(established_connections) == 0, (
            f"No connections should be established with invalid tokens. "
            f"Established: {[r['token_type'] for r in established_connections]}"
        )

        # Check for appropriate error handling
        for result in rejection_results:
            if result["result"]["error"]:
                # Should not cause system crashes or unexpected errors
                ...
                assert not has_crash_error, (
                    f"Invalid token should not cause system errors. "
    ...
    async def test_unauthorized_user_isolation(self):
        ...
        user_a_token = self.__class__.auth_helper.create_test_jwt_token(
            user_id=user_a_id,
            email=user_a_email,
            exp_minutes=30
        )
        ...
        user_b_token = self.__class__.auth_helper.create_test_jwt_token(
            user_id=user_b_id,
            email=user_b_email,
            exp_minutes=30
        )

        # User A creates conversation with sensitive data
        sensitive_thread_id = f"sensitive_thread_{user_a_id}"
        user_a_request_result = await self._attempt_agent_request_with_auth(
            user_a_token, should_succeed=True
        )

        assert user_a_request_result["success"], (
            f"User A should be able to create legitimate conversation. "
        ...
        unauthorized_attempts = [
            {
                "scenario": "thread_hijacking",
                "description": "User B attempts to access User A's thread",
                "request": {
                    "type": "agent_request",
                    "agent": "triage_agent",
                    "message": "Show me the previous conversation in this thread",
                    "thread_id": sensitive_thread_id,  # User A's thread
                    "user_id": user_b_id,  # User B's ID
                    "context": {"unauthorized_access_attempt": True}
                }
            },
            {
                "scenario": "user_id_spoofing",
                "description": "User B attempts to spoof User A's user_id",
                "request": {
                    "type": "agent_request",
                    "agent": "triage_agent",
                    "message": "Retrieve my previous AI cost analysis",
                    "thread_id": f"spoofing_thread_{int(time.time())}",
                    "user_id": user_a_id,  # Spoofing User A's ID
                    "context": {"spoofing_attempt": True}
                }
            },
            {
                "scenario": "cross_user_context",
                "description": "User B requests context from User A",
                "request": {
                    "type": "agent_request",
                    "agent": "data_helper_agent",
                    "message": f"Show me data and analysis history for user {user_a_id}",
                    "thread_id": f"cross_user_{int(time.time())}",
                    "user_id": user_b_id,
                    "context": {"cross_user_access_attempt": True}
                }
            }
        ]
            ...
            try:
                ...
                websocket = await asyncio.wait_for(
                    websockets.connect(
                        ...
                        additional_headers={
                            ...
                            "X-Environment": "staging",
                            "X-Test-Suite": f"user-isolation-{attempt['scenario']}"
                        },
                        ssl=ssl_context
                    ),
                    timeout=15.0
                ...
                while time.time() - start_time < 20.0:
                    try:
                        ...
                        # Check for auth errors (good)
                        if event_type in ["auth_error", "unauthorized", "forbidden"]:
                            auth_error_detected = True
                            break

                        # Check for unauthorized data access (bad)
                        if event_type == "agent_completed":
                            response_data = event.get("data", {})
                            result = response_data.get("result", {})
                            response_content = str(result).lower()

                            # Check if response contains User A's sensitive data
                ...
                isolation_results.append({
                    ...
                    "unauthorized_data_found": unauthorized_data_found,
                    "events_received": len(events),
                    "isolation_maintained": not unauthorized_data_found
                })

            except Exception as e:
                # Connection or processing failure is acceptable for security tests
                isolation_results.append({
                    "scenario": attempt["scenario"],
                    "auth_error_detected": True,
                    "unauthorized_data_found": False,
        ...
        # All unauthorized access attempts should be blocked
        isolation_success_rate = len(successful_isolations) / len(unauthorized_attempts)
        assert isolation_success_rate == 1.0, (
            f"User isolation must be 100% effective. Success rate: {isolation_success_rate:.1%}. "
            f"Data breaches: {[r['scenario'] for r in data_breaches]}"
        )

        # No unauthorized data access should occur
        assert len(data_breaches) == 0, (
            f"No unauthorized data access should be possible. "
    ...
    async def test_malicious_request_payload_rejection(self):
        ...
        malicious_payloads = [
            {
                ...
                "payload": {
                    "type": "agent_request",
                    "agent": "triage_agent'; DROP TABLE users; --",
                    "message": "SELECT * FROM sensitive_data WHERE user_id = '1' OR '1'='1'",
            ...
            },
            {
                ...
                "payload": {
                    ...
                    "message": "<img src=x onerror=alert('XSS')>Please optimize my AI costs</img>",
                    "user_id": "user<script>location.href='evil.com'</script>"
                }
            },
            {
                ...
                "payload": {
                    "type": "agent_request",
                    "agent": "triage_agent",
                    "message": "Optimize costs; rm -rf /; echo 'system compromised'",
                    "user_id": "user`whoami`",
            ...
            },
            {
                ...
                "payload": {
                    ...
                    "user_id": "A" * 10000,   # Very large user ID
                    "context": {"large_data": "B" * 50000}
                }
            },
            {
                ...
                "payload": {
                    "type": "agent_request",
                    "agent": "triage_agent\x00malicious",
                    "message": "Cost optimization\x00<script>alert()</script>",
                    "user_id": "user\x00admin"
                }
            },
            {
                ...
                "payload": {
                    "type": "agent_request",
                    "agent": "triage\u202emalicious\u202d_agent",
                    "message": "Normal request\u200b<script>evil()</script>",
                    "user_id": "user\ufeff_admin"
                }
            },
            {
            ...
            },
            {
                ...
                "payload": self._create_deeply_nested_payload(50)  # 50 levels deep
            }
        ]
            ...
            try:
                ...
                websocket = await asyncio.wait_for(
                    websockets.connect(
                        self.__class__.staging_config.urls.websocket_url,
                        additional_headers={
                            "Authorization": f"Bearer {self.legitimate_access_token}",
                            "X-Environment": "staging",
                            "X-Test-Suite": f"malicious-payload-{attack['name']}"
                        },
                        ssl=ssl_context
                    ),
                    timeout=15.0
                ...
                try:
                    ...
                    # Payload rejection at send level is good
                    rejection_results.append({
                        "attack": attack["name"],
                        "rejected_at_send": True,
                ...
                while time.time() - start_time < 15.0:
                    try:
                        ...
                        # Check for security compromise indicators
                        if event_type == "agent_completed":
                            response_data = event.get("data", {})
                            ...
                            compromise_indicators = [
                                "system compromised", "table dropped", "passwd", "whoami",
                                "alert('xss')", "script executed", "admin: true"
                            ]
                        ...
                        # Malformed response could indicate payload processing issues
                        continue
                ...
                rejection_results.append({
                    "attack": attack["name"],
                    "rejected_at_send": False,
                    "payload_sent": payload_sent,
                    "proper_rejection": proper_rejection,
                    "security_compromise": security_compromise,
                    "events_received": len(events),
                    "security_maintained": not security_compromise
                })

            except Exception as e:
                # Connection failure is acceptable for malicious payload tests
                rejection_results.append({
                    "attack": attack["name"],
                    "rejected_at_connection": True,
                    "error": str(e),
                    "security_maintained": True
                })

        # Analyze malicious payload rejection results
        ...
        assert security_success_rate == 1.0, (
            f"Malicious payload security must be 100% effective. "
            f"Success rate: {security_success_rate:.1%}. "
            f"Compromises: {[r['attack'] for r in security_compromises]}"
        )

        # No security compromises should occur
        assert len(security_compromises) == 0, (
            f"No security compromises should be possible through malicious payloads. "
    ...
    def _create_deeply_nested_payload(self, depth: int) -> Dict[str, Any]:
        ...
        if depth <= 0:
            return {
                "type": "agent_request",
                "agent": "triage_agent",
                "message": "Deep nesting attack payload",
                "user_id": "nested_attack_user"
            }

        return {
            "nested": self._create_deeply_nested_payload(depth - 1)
        }
    ...
    async def test_rate_limiting_and_abuse_prevention(self):
        ...
        for i in range(normal_rate_requests):
            result = await self._attempt_agent_request_with_auth(
                self.legitimate_access_token, should_succeed=True, timeout=30.0
            )
        ...
        rapid_tasks = [
            self._attempt_agent_request_with_auth(
                self.legitimate_access_token, should_succeed=False, timeout=15.0
            )
            for _ in range(rapid_rate_requests)
        ]
        ...
        # Normal rate should have high success rate
        assert normal_success_rate >= 0.8, (
            f"Normal rate requests should succeed: {normal_success_rate:.1%} (min 80%)"
        )

        # Rate limiting should activate under rapid requests
        # Either through blocking requests or degraded performance
        if rapid_success_rate >= 0.8:
            # If most rapid requests succeeded, check for performance degradation
            successful_times = [
                r.get("connection_time", 0) +
                sum(event.get("processing_time", 0) for event in r.get("events", []))
                for r in successful_rapid
            ]
        ...
        assert recovery_result["success"], (
            f"Should recover from rate limiting after cool-down period. "
