...
class MultiUserAgentIsolationE2ETests(BaseE2ETest):
    ...
    async def setup_multi_user_environment(self):
        ...
        for i in range(user_count):
            user_context = await create_authenticated_user_context(user_email=f'isolation_test_user_{i}_{int(time.time())}@staging.netrasystems.ai', environment='staging', permissions=['read', 'write', 'execute_agents'], user_metadata={'user_index': i, 'isolation_group': f'group_{i % 3}', 'secret_data': f'secret_for_user_{i}_only'})
            ...
            try:
                ...
                async def monitor_user_isolation():
                    ...
                    async for message in websocket:
                        ...
                        user_events.append({**event, 'user_index': user_index, 'user_id': user_context.user_id, 'timestamp': time.time()})
                        ...
                        for other_user in self.test_users:
                            if other_user.user_id != user_context.user_id:
                                ...
                                if other_secret and other_secret in str(event_data):
                                    other_user_data_detected.append({'leaked_from': other_user.user_id, 'leaked_to': user_context.user_id, 'secret_data': other_secret, 'event_type': event.get('type')})
                ...
                isolation_request = {'type': 'execute_agent', 'agent_type': 'isolation_test', 'user_id': user_context.user_id, 'thread_id': user_context.thread_id, 'request_id': user_context.request_id, 'data': {'user_index': user_index, 'isolation_validation': True, 'user_secret_data': user_secret, 'user_specific_task': f'task_for_user_{user_index}', 'cross_contamination_test': True, 'expected_isolation_group': user_context.agent_context.get('user_metadata', {}).get('isolation_group')}}
                ...
                return {'user_index': user_index, 'user_id': user_context.user_id, 'success': True, 'execution_duration': execution_duration, 'events': user_events, 'user_data': user_data, 'isolation_violations': isolation_violations, 'leaked_data': other_user_data_detected, 'event_count': len(user_events)}
            except Exception as e:
                return {'user_index': user_index, 'user_id': user_context.user_id, 'success': False, 'error': str(e)}
        tasks = [run_isolated_user_execution(user_context, i) for i, user_context in enumerate(self.test_users)]
    ...
    async def test_factory_pattern_instance_isolation(self):
        ...
        async def test_user_factory_isolation(user_context, user_index):
            ...
            try:
                ...
                factory_request = {'type': 'execute_agent', 'agent_type': 'factory_isolation_test', 'user_id': user_context.user_id, 'thread_id': user_context.thread_id, 'request_id': user_context.request_id, 'data': {'test_factory_isolation': True, 'user_index': user_index, 'require_unique_instance': True, 'state_isolation_test': True, 'instance_validation': {'memory_isolation': True, 'state_separation': True, 'resource_independence': True}}}
                await websocket.send(json.dumps(factory_request))
                await asyncio.wait_for(factory_task, timeout=30.0)
                await websocket.close()
                return {'user_index': user_index, 'user_id': user_context.user_id, 'instance_id': instance_id, 'shared_state_detected': shared_state_detected, 'events': factory_events, 'success': True}
            except Exception as e:
                return {'user_index': user_index, 'user_id': user_context.user_id, 'success': False, 'error': str(e)}
        tasks = [test_user_factory_isolation(user_context, i) for i, user_context in enumerate(self.test_users[:3])]
    ...
    async def test_websocket_event_stream_isolation(self):
        ...
        async def monitor_user_event_stream(websocket, user_context, user_index, event_collector):
            ...
            try:
                stream_test_request = {'type': 'execute_agent', 'agent_type': 'event_stream_isolation_test', 'user_id': user_context.user_id, 'thread_id': user_context.thread_id, 'request_id': user_context.request_id, 'data': {'user_index': user_index, 'generate_unique_events': True, 'event_isolation_test': True, 'user_identifier': f'user_{user_index}_stream', 'event_count_target': 10}}
                await websocket.send(json.dumps(stream_test_request))
                async for message in websocket:
                    event = json.loads(message)
                    event_collector.append({**event, 'received_by_user': user_context.user_id, 'user_index': user_index, 'timestamp': time.time()})
                    if event.get('type') == 'agent_completed':
                        break
            except Exception as e:
        ...
        for (websocket, user_context, user_index), event_collector in zip(websocket_connections, event_collectors):
            task = asyncio.create_task(monitor_user_event_stream(websocket, user_context, user_index, event_collector))
    ...
    async def test_resource_isolation_memory_cpu_separation(self):
        ...
        async def test_user_resource_isolation(user_context, user_index):
            ...
            try:
                ...
                async def monitor_user_resources():
                    ...
                    async for message in websocket:
                        ...
                        if event.get('type') == 'resource_update':
                            ...
                            resource_snapshots.append({'timestamp': time.time(), 'memory_mb': memory_mb, 'cpu_percent': cpu_percent, 'user_id': user_context.user_id, 'user_index': user_index})
                ...
                resource_request = {'type': 'execute_agent', 'agent_type': 'resource_isolation_test', 'user_id': user_context.user_id, 'thread_id': user_context.thread_id, 'request_id': user_context.request_id, 'data': {'user_index': user_index, 'resource_intensive': True, 'memory_test_mb': 100 + user_index * 20, 'cpu_test_duration': 10, 'resource_monitoring': True, 'isolation_validation': True}}
                await websocket.send(json.dumps(resource_request))
                await asyncio.wait_for(resource_task, timeout=30.0)
                await websocket.close()
                return {'user_index': user_index, 'user_id': user_context.user_id, 'resource_snapshots': resource_snapshots, 'peak_memory_mb': peak_memory_mb, 'peak_cpu_percent': peak_cpu_percent, 'success': True}
            except Exception as e:
                return {'user_index': user_index, 'user_id': user_context.user_id, 'success': False, 'error': str(e)}
        tasks = [test_user_resource_isolation(user_context, i) for i, user_context in enumerate(self.test_users[:3])]
