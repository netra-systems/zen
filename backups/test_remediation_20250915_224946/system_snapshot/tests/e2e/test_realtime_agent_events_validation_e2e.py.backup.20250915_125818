...
from netra_backend.app.websocket_core.event_validator import AgentEventValidator, CriticalAgentEventType, WebSocketEventMessage, validate_agent_events
...
class TestRealtimeAgentEventsValidationE2E(SSotBaseTestCase):
    ...
    def _analyze_event_content_quality(self, event: WebSocketEventMessage) -> Dict[str, float]:
        ...
        if hasattr(event, 'data'):
            content = (event.data.get('message', '') + ' ' + event.data.get('response', '') + ' ' + event.data.get('description', '') + ' ' + str(event.data.get('details', ''))).strip()
        quality_scores = {'content_length': 0.0, 'business_keywords': 0.0, 'specificity': 0.0, 'actionability': 0.0, 'technical_depth': 0.0}
        if not content:
        ...
        business_terms = ['analyze', 'insights', 'recommend', 'strategy', 'optimize', 'business', 'customer', 'revenue', 'growth', 'efficiency', 'data', 'trends', 'patterns', 'metrics', 'performance']
        ...
        specificity_indicators = ['%', '$', 'million', 'thousand', 'increase', 'decrease', 'step', 'process', 'method', 'approach', 'technique']
        ...
        action_words = ['should', 'recommend', 'suggest', 'implement', 'improve', 'focus', 'prioritize', 'consider', 'develop', 'create']
        ...
        technical_terms = ['algorithm', 'model', 'analysis', 'calculation', 'processing', 'evaluation', 'assessment', 'comparison', 'correlation', 'prediction']
    ...
    def _calculate_user_engagement_score(self, events: List[WebSocketEventMessage]) -> float:
        ...
        engagement_factors = []
        if len(events) >= 4:
            engagement_factors.append(25.0)
        elif len(events) >= 2:
        ...
        has_progression = any((event.event_type in ['agent_thinking', 'tool_executing', 'tool_completed'] for event in events))
        if has_progression:
    ...
    async def test_agent_thinking_event_quality_realtime(self):
        ...
        user_context = await create_authenticated_user_context(user_email=f'thinking_quality_{uuid.uuid4().hex[:8]}@example.com', environment=self.environment, permissions=['read', 'write', 'chat', 'agent_reasoning', 'websocket'], websocket_enabled=True)
        print(f'[U+1F464] User authenticated for thinking quality test: {user_context.user_id}')
        reasoning_request = 'I need to make a strategic decision about expanding our software company into the European market. We currently have $2M ARR in the US, 50 employees, and our main competitors are already established in Europe. Please analyze the key factors I should consider, evaluate the risks and opportunities, and help me think through this decision systematically.'
        async with self.golden_path_helper.authenticated_websocket_connection(user_context):
            result = await self.golden_path_helper.execute_golden_path_flow(user_message=reasoning_request, user_context=user_context, timeout=90.0)
            thinking_events = [event for event in result.events_received if event.event_type == 'agent_thinking']
            ...
            for event in thinking_events:
                ...
                thinking_quality = quality_scores['content_length'] * 0.3 + quality_scores['business_keywords'] * 0.25 + quality_scores['specificity'] * 0.2 + quality_scores['technical_depth'] * 0.25
            ...
            reasoning_indicators = ['consider', 'analyze', 'evaluate', 'factor', 'risk', 'opportunity', 'decision', 'strategy', 'approach']
    ...
    async def test_tool_execution_transparency_realtime(self):
        ...
        user_context = await create_authenticated_user_context(user_email=f'tool_transparency_{uuid.uuid4().hex[:8]}@example.com', environment=self.environment, permissions=['read', 'write', 'chat', 'tool_execution', 'transparency'], websocket_enabled=True)
        tool_request = 'Please analyze the following business scenario using data analysis tools: Calculate the customer lifetime value (CLV) for a SaaS business with average monthly revenue per user of $50, monthly churn rate of 3%, and customer acquisition cost of $200. Also provide recommendations for improving these metrics and their potential impact.'
        async with self.golden_path_helper.authenticated_websocket_connection(user_context):
            result = await self.golden_path_helper.execute_golden_path_flow(user_message=tool_request, user_context=user_context, timeout=120.0)
            tool_executing_events = [event for event in result.events_received if event.event_type == 'tool_executing']
            tool_completed_events = [event for event in result.events_received if event.event_type == 'tool_completed']
            ...
            for event in tool_completed_events:
                quality_scores = self._analyze_event_content_quality(event)
                completion_quality = quality_scores['content_length'] * 0.25 + quality_scores['business_keywords'] * 0.25 + quality_scores['specificity'] * 0.3 + quality_scores['actionability'] * 0.2
    ...
    async def test_comprehensive_event_quality_journey(self):
        ...
        user_context = await create_authenticated_user_context(user_email=f'comprehensive_quality_{uuid.uuid4().hex[:8]}@example.com', environment=self.environment, permissions=['read', 'write', 'chat', 'comprehensive_analysis', 'quality_validation'], websocket_enabled=True)
        comprehensive_request = "I'm the CEO of a growing B2B SaaS company and need comprehensive strategic analysis. We have $3M ARR, 150 customers, 8% monthly churn, and want to scale to $10M ARR. Please provide: 1) Current business health assessment, 2) Growth strategy analysis with specific tactics, 3) Risk assessment and mitigation plans, 4) Implementation roadmap with timelines and KPIs. Show your analytical thinking process and provide actionable recommendations."
        async with self.golden_path_helper.authenticated_websocket_connection(user_context):
            result = await self.golden_path_helper.execute_golden_path_flow(user_message=comprehensive_request, user_context=user_context, timeout=150.0)
            ...
            for event in all_events:
                quality_scores = self._analyze_event_content_quality(event)
                overall_quality = quality_scores['content_length'] * 0.2 + quality_scores['business_keywords'] * 0.3 + quality_scores['specificity'] * 0.2 + quality_scores['actionability'] * 0.2 + quality_scores['technical_depth'] * 0.1
            ...
            business_coverage_terms = ['strategy', 'growth', 'revenue', 'customers', 'churn', 'assessment', 'analysis', 'recommendations', 'implementation', 'kpi']
    ...
    async def test_event_quality_under_different_scenarios(self):
        ...
        user_context = await create_authenticated_user_context(user_email=f'quality_scenarios_{uuid.uuid4().hex[:8]}@example.com', environment=self.environment, permissions=['read', 'write', 'chat', 'scenario_testing'], websocket_enabled=True)
        quality_scenarios = [{'name': 'strategic_planning', 'request': 'Help me develop a 12-month strategic plan for expanding our AI consultancy from 5 to 25 employees while maintaining quality and profitability.', 'expected_quality_threshold': 60.0}, {'name': 'financial_analysis', 'request': 'Analyze the financial health of a business with $500K revenue, $350K expenses, and project growth scenarios for next 3 years.', 'expected_quality_threshold': 55.0}, {'name': 'operational_optimization', 'request': 'Our customer support team handles 200 tickets/day with 24h response time. How can we optimize to 4h response while scaling volume?', 'expected_quality_threshold': 50.0}]
        scenario_quality_scores = []
        async with self.golden_path_helper.authenticated_websocket_connection(user_context):
            for i, scenario in enumerate(quality_scenarios):
                print(f" CYCLE:  Testing scenario {i + 1}: {scenario['name']}")
                try:
                    result = await self.golden_path_helper.execute_golden_path_flow(user_message=scenario['request'], user_context=user_context, timeout=90.0)
                    if result.success and result.events_received:
                        ...
                        assert scenario_avg_quality >= scenario['expected_quality_threshold'], f"{scenario['name']} quality below threshold: {scenario_avg_quality:.1f}%"
                    else:
