...
class AgentExecutionTimeoutReproductionTests:
    ...
    async def test_windows_safe_wait_for_timeout_reproduction(self):
        ...
        with pytest.raises(asyncio.TimeoutError):
            await windows_safe_wait_for(mock_slow_operation(), timeout=1.0)
        elapsed = time.time() - start_time
    ...
    async def test_windows_safe_wait_for_with_default_value(self):
        ...
        start_time = time.time()
        try:
            result = await windows_safe_wait_for(mock_slow_websocket_recv(), timeout=0.5, default=None)
    ...
    async def test_progressive_timeout_pattern_reproduction(self):
        ...
        event_timeouts = [3.0, 2.0, 1.5, 1.0, 0.8]
        events_received = []
        ...
        for i in range(5):
            ...
            recv_delay = 0.1 if i < 3 else 2.0
            try:
                event_data = await windows_safe_wait_for(mock_websocket_recv(recv_delay), timeout=timeout, default=None)
            except (asyncio.TimeoutError, TimeoutError):
                event_data = None
    ...
    async def test_agent_execution_timeout_simulation(self):
        ...
        class MockAgent:
            ...
            async def _send_event(self, event_type: str, data: Dict[str, Any]):
                ...
                await windows_safe_wait_for(self._mock_websocket_send(event), timeout=1.0, default=None)
        ...
        start_time = time.time()
        result = await windows_safe_wait_for(fast_agent.execute('Test message for fast agent'), timeout=5.0)
        fast_duration = time.time() - start_time
        assert result['status'] == 'completed'
        assert len(fast_agent.events_sent) == 5
        assert fast_duration < 2.0, f'Fast agent took too long: {fast_duration:.3f}s'
        slow_agent = MockAgent(execution_delay=3.0)
        start_time = time.time()
        try:
            result = await windows_safe_wait_for(slow_agent.execute('Test message for slow agent'), timeout=2.0)
    ...
    async def test_websocket_chunked_streaming_timeout_reproduction(self):
        ...
        async def mock_streaming_websocket():
            ...
            chunks = ['{"type": "agent_started", "data": {"status": "beginning"}}', '{"type": "agent_thinking", "data": {"step": 1}}', '{"type": "tool_executing", "data": {"tool": "optimizer"}}', '{"type": "partial_result", "data": {"progress": 50}}', '{"type": "tool_completed", "data": {"result": "optimization_data"}}', '{"type": "agent_completed", "data": {"final_result": "success"}}']
            for i, chunk in enumerate(chunks):
                if i < 3:
                    await windows_safe_sleep(0.1)
                else:
                    await windows_safe_sleep(1.5)
                yield chunk
        chunks_received = []
        chunk_timeouts = [1.0, 1.0, 1.0, 0.8, 0.5]
        async_generator = mock_streaming_websocket()
        start_time = time.time()
        for i in range(len(chunk_timeouts)):
            timeout = chunk_timeouts[i]
            try:
                chunk = await windows_safe_wait_for(async_generator.__anext__(), timeout=timeout, default=None)
                if chunk is None:
    ...
    async def test_critical_event_delivery_timeout_patterns(self):
        ...
        class MockAgentWebSocketBridge:
            ...
            async def send_event(self, event_type: str, data: Dict[str, Any]):
                ...
                event = {'type': event_type, 'data': data, 'timestamp': time.time()}
                try:
                    await windows_safe_wait_for(self._mock_send_with_delay(event), timeout=1.0)
                    self.events_sent.append(event)
                    return True
                except asyncio.TimeoutError:
        ...
        slow_bridge = MockAgentWebSocketBridge(event_delay=1.2)
        start_time = time.time()
        for event_type in critical_event_types:
