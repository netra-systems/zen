...
class TestFrontendSentryDSNConfiguration:
    ...
    def test_staging_environment_dsn_exists(self):
        ...
        staging_env = {
            ...
            'NEXT_PUBLIC_SENTRY_DSN': None  # This should be set but isn't
        }
        
        with patch.dict(os.environ, staging_env, clear=True):
            # Check if DSN is available for staging
    ...
    def test_production_environment_dsn_exists(self):
        ...
        with patch.dict(os.environ, production_env, clear=True):
            # Check if DSN is available for production
            dsn = os.environ.get('NEXT_PUBLIC_SENTRY_DSN')
    ...
    def test_development_environment_dsn_optional(self):
        ...
        development_env = {
            ...
            # NEXT_PUBLIC_SENTRY_DSN intentionally not set
        }
        
        with patch.dict(os.environ, development_env, clear=True):
            # Development should work without DSN
            dsn = os.environ.get('NEXT_PUBLIC_SENTRY_DSN')
            # This is OK in development - no assertion failure expected
    ...
    def test_dsn_validation_logic(self):
        ...
        # Check if environment-specific logic exists
    ...
    def test_environment_detection_logic(self):
        ...
        # Check if both staging and production are handled
...
class TestSentryConfigurationSecrets:
    ...
    def test_staging_secret_manager_dsn(self):
        ...
        """
        # This test would verify DSN is in Secret Manager
        # Currently expected to fail as it's not configured
        ...
        with patch('google.cloud.secretmanager.SecretManagerServiceClient') as mock_client:
            ...
            # Try to access staging sentry DSN secret
            try:
    ...
    def test_production_secret_manager_dsn(self):
        ...
        """
        # This test would verify production DSN is in Secret Manager
        # Currently expected to fail as it's not configured
        ...
        with patch('google.cloud.secretmanager.SecretManagerServiceClient') as mock_client:
            ...
            # Try to access production sentry DSN secret
            try:
                from scripts.fetch_secrets_to_env import fetch_secret
    ...
    def test_secret_creation_scripts(self):
        ...
        """
        # Check if create_staging_secrets.py handles Sentry DSN
...
class TestSentryDeploymentConfiguration:
    ...
    def test_terraform_secret_configuration(self):
        ...
        # Check terraform files for Sentry configuration
    ...
    def test_cloud_run_environment_variables(self):
        ...
        """
        # Check deployment script for Sentry environment variables
    ...
    def test_staging_deployment_sentry_config(self):
        ...
        """
        # This would test actual staging deployment configuration
        # Currently expected to fail as Sentry isn't configured in staging
        
        # Mock environment for staging deployment test
        staging_env = {
            'ENVIRONMENT': 'staging',
            'NEXT_PUBLIC_ENVIRONMENT': 'staging',
            'NEXT_PUBLIC_SENTRY_DSN': 'https://staging@sentry.io/123456'
        }
        
        with patch.dict(os.environ, staging_env):
            # Test that staging deployment would have correct Sentry config
            from frontend.app.sentry_init import SentryInit
            
            # This should work in staging but currently doesn't due to missing DSN
...
class TestSentryErrorBoundaryIntegration:
    ...
    def test_error_boundary_context_setting(self):
        ...
        # Check if context is set for Sentry
...
class TestSentryPerformanceMonitoring:
    ...
    def test_environment_specific_sampling(self):
        ...
        # Check if environment-specific sampling is configured
