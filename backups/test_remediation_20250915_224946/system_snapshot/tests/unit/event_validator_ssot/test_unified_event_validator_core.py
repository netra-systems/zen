...
try:
    from netra_backend.app.websocket_core.event_validator import UnifiedEventValidator, ValidationResult as UnifiedValidationResult, EventCriticality as UnifiedEventCriticality, CriticalAgentEventType as UnifiedCriticalAgentEventType
    unified_available = True
except ImportError as e:
    unified_available = False
    unified_import_error = str(e)
try:
    from netra_backend.app.services.websocket_error_validator import WebSocketEventValidator as ProductionEventValidator, ValidationResult as ProductionValidationResult, EventCriticality as ProductionEventCriticality
    production_available = True
except ImportError as e:
    production_available = False
    production_import_error = str(e)
try:
    from netra_backend.app.websocket_core.event_validator import AgentEventValidator as SsotFrameworkEventValidator, AgentEventValidationResult as SsotFrameworkValidationResult, CriticalAgentEventType as SsotFrameworkCriticalAgentEventType
...
@pytest.mark.unit
class UnifiedEventValidatorCoreTests(SSotBaseTestCase, unittest.TestCase):
    """
    ...
    def setUp(self):
        ...
        self.sample_critical_event = {'type': 'agent_started', 'run_id': 'test-run-123', 'agent_name': 'test-agent', 'timestamp': '2025-01-09T10:00:00Z', 'payload': {'status': 'started', 'agent': 'test-agent'}}
    ...
    def test_import_availability_exposes_multiple_implementations(self):
        ...
        """
        available_implementations = sum([self.import_status['unified'], self.import_status['production'], self.import_status['ssot_framework']])
        self.assertEqual(available_implementations, 1, f'SSOT VIOLATION: Found {available_implementations} EventValidator implementations. Expected exactly 1 unified implementation. Import status: {self.import_status}. This demonstrates the SSOT consolidation issue - multiple validators exist!')
    ...
    def test_validation_method_signature_consistency_across_implementations(self):
        ...
        if unified_available:
            ...
            validation_methods['unified'] = {'has_validate_event': hasattr(validator, 'validate_event'), 'validate_event_params': self._get_method_params(validator.validate_event) if hasattr(validator, 'validate_event') else None, 'has_record_event': hasattr(validator, 'record_event'), 'has_perform_full_validation': hasattr(validator, 'perform_full_validation')}
        if production_available:
            validator = ProductionEventValidator()
            validation_methods['production'] = {'has_validate_event': hasattr(validator, 'validate_event'), 'validate_event_params': self._get_method_params(validator.validate_event) if hasattr(validator, 'validate_event') else None, 'has_record_event': hasattr(validator, 'record_event'), 'has_perform_full_validation': hasattr(validator, 'perform_full_validation')}
        if ssot_framework_available:
            validator = SsotFrameworkEventValidator()
            validation_methods['ssot_framework'] = {'has_validate_event': hasattr(validator, 'validate_event'), 'validate_event_params': self._get_method_params(validator.validate_event) if hasattr(validator, 'validate_event') else None, 'has_record_event': hasattr(validator, 'record_event'), 'has_perform_full_validation': hasattr(validator, 'perform_full_validation')}
        ...
        if len(method_signatures) > 1:
            ...
            for i, signature in enumerate(method_signatures[1:], 1):
                self.assertEqual(first_signature, signature, f'SSOT VIOLATION: validate_event method signatures differ between implementations. First: {first_signature}, Implementation {i}: {signature}. This demonstrates inconsistent APIs across EventValidator implementations!')
        method_availability = {}
        ...
        for method_name, availability_list in method_availability.items():
            ...
            if len(set(has_method_values)) > 1:
                ...
                self.fail(f"SSOT VIOLATION: Method '{method_name}' availability differs across implementations: {availability_details}. This demonstrates inconsistent interfaces!")
    ...
    def test_validation_result_type_consistency_across_implementations(self):
        ...
        if unified_available:
            ...
            try:
                ...
                validation_result_types['unified'] = {'type': type(result).__name__, 'module': type(result).__module__, 'has_is_valid': hasattr(result, 'is_valid'), 'has_error_message': hasattr(result, 'error_message'), 'has_business_value_score': hasattr(result, 'business_value_score'), 'has_revenue_impact': hasattr(result, 'revenue_impact')}
        ...
        if production_available:
            ...
            try:
                result = validator.validate_event(self.sample_critical_event, self.sample_user_id)
                validation_result_types['production'] = {'type': type(result).__name__, 'module': type(result).__module__, 'has_is_valid': hasattr(result, 'is_valid'), 'has_error_message': hasattr(result, 'error_message'), 'has_business_value_score': hasattr(result, 'business_value_score'), 'has_revenue_impact': hasattr(result, 'revenue_impact')}
            except Exception as e:
        ...
        if len(set(result_type_names)) > 1:
            self.fail(f'SSOT VIOLATION: ValidationResult types differ across implementations: Types: {set(result_type_names)}, Modules: {set(result_modules)}. Full details: {validation_result_types}. This demonstrates inconsistent return types!')
        field_availability = {}
        ...
        for field_name, availability_list in field_availability.items():
            ...
            if len(set(has_field_values)) > 1:
                ...
                self.fail(f"SSOT VIOLATION: ValidationResult field '{field_name}' availability differs: {availability_details}. This demonstrates inconsistent ValidationResult schemas!")
    ...
    def test_critical_event_type_definitions_consistency(self):
        ...
        if len(valid_definitions) > 1:
            ...
            for i, definition in enumerate(valid_definitions[1:], 1):
                self.assertEqual(first_definition, definition, f'SSOT VIOLATION: Critical event definitions differ between implementations. First: {first_definition}, Implementation {i}: {definition}. All definitions: {critical_event_definitions}. This demonstrates inconsistent critical event definitions!')
    ...
    def test_validation_behavior_consistency_for_same_input(self):
        ...
        demonstrating the need for SSOT consolidation.
        """
        test_cases = [{'name': 'valid_agent_started', 'event': self.sample_critical_event, 'user_id': self.sample_user_id, 'expected_valid': True}, {'name': 'missing_run_id', 'event': {'type': 'agent_started', 'agent_name': 'test-agent', 'timestamp': '2025-01-09T10:00:00Z', 'payload': {'status': 'started'}}, 'user_id': self.sample_user_id, 'expected_valid': False}, {'name': 'malformed_event', 'event': {'invalid': 'structure'}, 'user_id': self.sample_user_id, 'expected_valid': False}]
        ...
        for test_case in test_cases:
            ...
            if unified_available:
                try:
                    ...
                    case_results['unified'] = {'is_valid': result.is_valid, 'error_message': result.error_message, 'criticality': getattr(result, 'criticality', None)}
                except Exception as e:
            ...
            if production_available:
                try:
                    validator = ProductionEventValidator()
                    result = validator.validate_event(event, user_id)
                    case_results['production'] = {'is_valid': result.is_valid, 'error_message': result.error_message, 'criticality': getattr(result, 'criticality', None)}
                except Exception as e:
            ...
            if len(valid_results) > 1:
                ...
                if len(set(is_valid_values)) > 1:
                    self.fail(f"SSOT VIOLATION: Validation behavior differs for test case '{case_name}'. is_valid results: {is_valid_values}. Full results: {case_results}. Same input should produce same validation outcome across implementations!")
