...
class TestWebSocketUuid4Violations(SSotBaseTestCase):
    ...
    def test_unified_websocket_auth_line_1303_violation_EXPECT_FAILURE(self):
        ...
        """
        try:
            # Import the WebSocket auth module to inspect line 1303
            ...
            # Find the specific violation around line 1303
            ...
            assert violation_found, (
                "CRITICAL VIOLATION DETECTION FAILURE: unified_websocket_auth.py "
                "should still be using uuid.uuid4() for connection ID generation. "
            ...
            expected_pattern = r'connection_id\s*=.*str\(uuid\.uuid4\(\)\)'
            pattern_match = re.search(expected_pattern, source_code)

            assert pattern_match is not None, (
                f"SSOT VIOLATION PATTERN MISMATCH: Expected 'connection_id = str(uuid.uuid4())' "
                f"pattern at line ~1303, but found different pattern: {violation_content}"
            )
    ...
    def test_websocket_connection_routing_failure_EXPECT_FAILURE(self):
        ...
        """
        # Generate connection ID using current (violating) method
        current_connection_id = str(uuid.uuid4())

        # Check against SSOT format pattern for connection IDs
        ssot_connection_pattern = re.compile(r'^conn_[a-zA-Z0-9]+_[a-zA-Z0-9]+_\d+_[a-f0-9]{8}$')

        # This should fail - current format doesn't match SSOT pattern
        assert not ssot_connection_pattern.match(current_connection_id), (
            f"UNEXPECTED SSOT COMPLIANCE: Connection ID '{current_connection_id}' "
        ...
        ssot_connection_id = unified_generator.generate_connection_id(
            user_id="test_user_123",
            session_id="sess_456"
        )

        # SSOT format should match the pattern
        assert ssot_connection_pattern.match(ssot_connection_id), (
            f"SSOT GENERATOR MALFUNCTION: UnifiedIdGenerator produced "
            f"invalid connection format: '{ssot_connection_id}'"
        )

        # Document the routing impact
    ...
    def test_websocket_preliminary_connection_id_violation_EXPECT_FAILURE(self):
        ...
        try:
            from netra_backend.app.websocket_core.unified_websocket_auth import (
                consolidated_websocket_auth_with_e2e_context
            )

            # Mock WebSocket connection without preliminary_connection_id
            ...
            with patch('netra_backend.app.websocket_core.unified_websocket_auth._validate_token_with_auth_service') as mock_validate:
                ...
                try:
                    # Call the function without preliminary_connection_id to trigger UUID4 generation
                    result = consolidated_websocket_auth_with_e2e_context(
                        websocket=mock_websocket,
                        token="test.jwt.token",
                        preliminary_connection_id=None,  # This should trigger uuid.uuid4()
                        e2e_context=None
                    )

                    # Extract the generated connection ID
                    generated_connection_id = result.connection_id if hasattr(result, 'connection_id') else None

                    if generated_connection_id:
                        # This should be a raw UUID format (violation)
                        try:
                        ...
                        # Test should fail if it's NOT a raw UUID (meaning migration is done)
                        assert is_raw_uuid, (
                            f"UNEXPECTED SSOT COMPLIANCE: Connection ID '{generated_connection_id}' "
                            f"is not a raw UUID. Migration may be complete."
                        )

                        # Verify it doesn't match SSOT pattern
                        ssot_pattern = re.compile(r'^conn_[a-zA-Z0-9]+_[a-zA-Z0-9]+_\d+_[a-f0-9]{8}$')
                        assert not ssot_pattern.match(generated_connection_id), (
                            f"UNEXPECTED SSOT FORMAT: Connection ID '{generated_connection_id}' "
                            f"already follows SSOT pattern."
                        )
                    ...
                    # Even if auth fails, we want to detect the UUID4 pattern
    ...
    def test_websocket_event_delivery_id_mismatch_EXPECT_FAILURE(self):
        ...
        """
        # Simulate WebSocket event delivery scenario
        raw_connection_id = str(uuid.uuid4())

        # Simulate agent execution context that expects SSOT format
        unified_generator = UnifiedIdGenerator()
        expected_connection_format = unified_generator.generate_connection_id(
            user_id="test_user",
        ...
        # Validate format incompatibility
        ssot_pattern = re.compile(r'^conn_[a-zA-Z0-9]+_[a-zA-Z0-9]+_\d+_[a-f0-9]{8}$')

        # Current ID should NOT match SSOT pattern
        assert not ssot_pattern.match(test_event['connection_id']), (
            f"UNEXPECTED COMPLIANCE: Event connection ID '{test_event['connection_id']}' "
            f"already matches SSOT pattern."
        )

        # Expected format should match SSOT pattern
        assert ssot_pattern.match(expected_connection_format), (
            f"SSOT GENERATOR ERROR: Expected format '{expected_connection_format}' "
            f"doesn't match SSOT pattern."
        )

        # Demonstrate the format mismatch problem
        ...
        # Raw UUID should have low format compatibility score
        assert format_match_score < 2, (
            f"UNEXPECTED FORMAT COMPATIBILITY: Raw UUID connection ID "
            f"has high compatibility score: {format_match_score}/2"
        )
    ...
    def test_websocket_multi_user_isolation_violation_EXPECT_FAILURE(self):
        ...
        """
        # Generate connection IDs for multiple users using current method
        user1_connection = str(uuid.uuid4())
        user2_connection = str(uuid.uuid4())

        # Extract user information from connection IDs (should fail)
        def extract_user_from_connection_id(connection_id: str) -> Optional[str]:
            # This should fail for raw UUIDs - no user context embedded
            parts = connection_id.split('_')
            if len(parts) >= 2 and parts[0] == 'conn':
                return parts[1]  # User ID should be here in SSOT format
            return None

        # Current method should fail to provide user context
        ...
        assert user1_extracted is None, (
            f"UNEXPECTED USER CONTEXT: Raw UUID connection ID '{user1_connection}' "
            f"unexpectedly contains user context: '{user1_extracted}'"
        )

        assert user2_extracted is None, (
            f"UNEXPECTED USER CONTEXT: Raw UUID connection ID '{user2_connection}' "
            f"unexpectedly contains user context: '{user2_extracted}'"
        )

        # Compare with SSOT format that includes user context
        unified_generator = UnifiedIdGenerator()
        ssot_user1_connection = unified_generator.generate_connection_id(
            user_id="user_123",
            session_id="sess_456"
        )
        ssot_user2_connection = unified_generator.generate_connection_id(
            user_id="user_789",
            session_id="sess_012"
        )

        # SSOT format should provide user context
        ...
        assert ssot_user1_extracted == "user", (
            f"SSOT FORMAT ERROR: Expected user context in '{ssot_user1_connection}', "
            f"got: '{ssot_user1_extracted}'"
        )
