...
try:
    from shared.configuration.central_config_validator import CentralConfigurationValidator, LegacyConfigMarker, check_config_before_deletion, get_legacy_migration_report
except ImportError:
    ...
    class LegacyConfigMarker:
        ...
        def get_replacement_variables(var_name: str) -> list:
            replacements = {'DATABASE_URL': ['POSTGRES_HOST', 'POSTGRES_PORT', 'POSTGRES_DB', 'POSTGRES_USER', 'POSTGRES_PASSWORD'], 'JWT_SECRET': ['JWT_SECRET_KEY'], 'GOOGLE_OAUTH_CLIENT_ID': ['GOOGLE_OAUTH_CLIENT_ID_STAGING', 'GOOGLE_OAUTH_CLIENT_ID_PRODUCTION']}
...
try:
    ...
    class ConfigDependencyMap:
        ...
        def get_dependent_services(var_name: str) -> list:
            dependencies = {'GOOGLE_OAUTH_CLIENT_ID': ['backend', 'auth_service'], 'JWT_SECRET': ['backend', 'auth_service'], 'POSTGRES_HOST': ['backend', 'analytics_service']}
...
try:
    from shared.configuration.cross_service_validator import CrossServiceConfigValidator, validate_config_deletion_cross_service
except ImportError:
...
class LegacyConfigMarkerTests:
    ...
    def test_check_legacy_usage_warnings(self):
        ...
        configs = {'DATABASE_URL': 'postgresql://localhost/test', 'POSTGRES_HOST': 'localhost', 'JWT_SECRET': 'old-secret', 'JWT_SECRET_KEY': 'new-secret'}
...
class CrossServiceValidationTests:
    ...
    def test_unsafe_config_deletion(self):
        ...
        with patch('shared.configuration.cross_service_validator.validate_config_deletion_cross_service') as mock_validate:
            mock_validate.return_value = {'safe': False, 'affected_services': ['backend', 'auth_service']}
            result = validate_config_deletion_cross_service('GOOGLE_OAUTH_CLIENT_ID')
...
class ConfigRegressionPreventionTests:
    ...
    async def test_real_time_config_monitoring(self):
        ...
        config_changes = [{'variable': 'GOOGLE_OAUTH_CLIENT_ID', 'action': 'delete'}, {'variable': 'DEBUG_MODE', 'action': 'update'}, {'variable': 'JWT_SECRET', 'action': 'rotate'}]
        for change in config_changes:
            impact_level = ConfigDependencyMap.get_impact_level(change['variable'])
            if impact_level == ConfigImpactLevel.CRITICAL:
                await websocket.send_json({'type': 'config_change_alert', 'level': 'critical', 'variable': change['variable'], 'action': change['action']})
