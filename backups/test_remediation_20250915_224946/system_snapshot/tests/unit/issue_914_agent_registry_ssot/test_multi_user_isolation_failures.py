...
class AgentRegistryMultiUserIsolationFailuresTests(SSotAsyncTestCase):
    ...
    async def async_setup_method(self, method=None):
        ...
        self.test_users = [{'user_id': 'user-1-isolation-test', 'session_id': 'session-1-test'}, {'user_id': 'user-2-isolation-test', 'session_id': 'session-2-test'}, {'user_id': 'user-3-isolation-test', 'session_id': 'session-3-test'}]
    ...
    async def test_user_context_contamination_between_registries(self):
        ...
        registry_user_sessions = {'basic': {}, 'advanced': {}}
        for user_info in self.test_users:
            user_id = user_info['user_id']
            session_id = user_info['session_id']
            user_context_isolation_results[user_id] = {'basic_registry': {'success': False, 'context': None, 'error': None}, 'advanced_registry': {'success': False, 'context': None, 'error': None}, 'unique_data': f'PRIVATE_DATA_FOR_{user_id}_{uuid.uuid4().hex[:8]}'}
            try:
                if hasattr(self.basic_registry, 'create_user_session'):
                    ...
                    if hasattr(basic_context, 'set_user_data') and basic_context:
                        await basic_context.set_user_data('private_data', user_context_isolation_results[user_id]['unique_data'])
                    user_context_isolation_results[user_id]['basic_registry'] = {'success': True, 'context': basic_context, 'context_id': id(basic_context) if basic_context else None}
                    self.logger.info(f'Basic registry user session created for {user_id}')
                else:
                    user_context_isolation_results[user_id]['basic_registry'] = {'success': False, 'error': 'create_user_session method not available'}
            except Exception as e:
                user_context_isolation_results[user_id]['basic_registry'] = {'success': False, 'error': str(e)}
            ...
            try:
                if hasattr(self.advanced_registry, 'create_user_session'):
                    ...
                    if hasattr(advanced_context, 'set_user_data') and advanced_context:
                        await advanced_context.set_user_data('private_data', user_context_isolation_results[user_id]['unique_data'])
                    user_context_isolation_results[user_id]['advanced_registry'] = {'success': True, 'context': advanced_context, 'context_id': id(advanced_context) if advanced_context else None}
                    self.logger.info(f'Advanced registry user session created for {user_id}')
                else:
                    user_context_isolation_results[user_id]['advanced_registry'] = {'success': False, 'error': 'create_user_session method not available'}
            except Exception as e:
                user_context_isolation_results[user_id]['advanced_registry'] = {'success': False, 'error': str(e)}
                    ...
                    if context and hasattr(context, 'get_user_data'):
                        try:
                            ...
                            if user_data != expected_data:
                                contamination_evidence.append({'contamination_type': 'data_mismatch', 'registry': registry_type, 'user_id': user_id, 'expected_data': expected_data, 'actual_data': user_data, 'description': f'User {user_id} got wrong data from {registry_type} registry'})
                            for other_user in self.test_users:
                                if other_user['user_id'] != user_id:
                                    try:
                                        other_user_data = await context.get_user_data(f"private_data_for_{other_user['user_id']}")
                                        if other_user_data is not None:
                                            contamination_evidence.append({'contamination_type': 'cross_user_access', 'registry': registry_type, 'user_id': user_id, 'accessed_other_user': other_user['user_id'], 'leaked_data': other_user_data, 'description': f"User {user_id} accessed data from {other_user['user_id']}"})
                                    except:
                                        pass
                        except Exception as e:
                            self.logger.warning(f'Could not test user data for {user_id} in {registry_type}: {e}')
        registry_state_contamination = []
        successful_basic_sessions = [info['basic_registry']['context'] for info in user_context_isolation_results.values() if info['basic_registry']['success'] and info['basic_registry']['context']]
        successful_advanced_sessions = [info['advanced_registry']['context'] for info in user_context_isolation_results.values() if info['advanced_registry']['success'] and info['advanced_registry']['context']]
            ...
            for i, context_a in enumerate(successful_basic_sessions):
                for j, context_b in enumerate(successful_basic_sessions):
                    if i != j and id(context_a) == id(context_b):
                        registry_state_contamination.append({'contamination_type': 'shared_context_instance', 'registry': 'basic', 'context_id': id(context_a), 'description': f'Multiple users share same context instance in basic registry'})
        if len(successful_advanced_sessions) > 1:
            for i, context_a in enumerate(successful_advanced_sessions):
                for j, context_b in enumerate(successful_advanced_sessions):
                    if i != j and id(context_a) == id(context_b):
                        registry_state_contamination.append({'contamination_type': 'shared_context_instance', 'registry': 'advanced', 'context_id': id(context_a), 'description': f'Multiple users share same context instance in advanced registry'})
        failure_reasons = []
        if contamination_evidence:
            for evidence in contamination_evidence:
                failure_reasons.append(f"{evidence['contamination_type']} in {evidence['registry']} registry: {evidence['description']}")
        if registry_state_contamination:
            for contamination in registry_state_contamination:
                failure_reasons.append(f"{contamination['contamination_type']}: {contamination['description']}")
        ...
        if failure_reasons:
            pytest.fail(f"CRITICAL USER CONTEXT CONTAMINATION: Multi-user isolation failures detected. This causes privacy breaches and data leaks blocking Golden Path security. Contamination evidence: {'; '.join(failure_reasons)}. IMPACT: Users see other users' private data and agent responses.")
    ...
    async def test_memory_leaks_from_shared_state_violations(self):
        ...
        for obj_type in [BasicRegistry, AdvancedRegistry]:
            initial_object_counts[obj_type.__name__] = len([obj for obj in gc.get_objects() if type(obj) == obj_type])
        self.logger.info(f'Initial object counts: {initial_object_counts}')
        session_cycles = 5
        users_per_cycle = 3
        for cycle in range(session_cycles):
            cycle_registries = {'basic': [], 'advanced': []}
            ...
            for obj_type in [BasicRegistry, AdvancedRegistry]:
                current_object_counts[obj_type.__name__] = len([obj for obj in gc.get_objects() if type(obj) == obj_type])
            registry_instance_growth[cycle] = {'cycle': cycle, 'basic_instances': len(cycle_registries['basic']), 'advanced_instances': len(cycle_registries['advanced']), 'total_basic_objects': current_object_counts['BasicRegistry'], 'total_advanced_objects': current_object_counts['AdvancedRegistry']}
            self.logger.info(f"Cycle {cycle} - Basic: {current_object_counts['BasicRegistry']}, Advanced: {current_object_counts['AdvancedRegistry']}")
                        ...
                        for j, registry_b in enumerate(registries):
                            if i != j:
                                ...
                                if shared_attributes:
                                    shared_state_violations.append({'cycle': cycle, 'registry_type': registry_type, 'registry_a_id': id(registry_a), 'registry_b_id': id(registry_b), 'shared_attributes': shared_attributes, 'description': f'Registries share state attributes: {shared_attributes}'})
            await asyncio.sleep(0.1)
            gc.collect()
        final_object_counts = {}
        for obj_type in [BasicRegistry, AdvancedRegistry]:
            final_object_counts[obj_type.__name__] = len([obj for obj in gc.get_objects() if type(obj) == obj_type])
        ...
        for obj_type_name in initial_object_counts.keys():
            ...
            memory_growth_analysis[obj_type_name] = {'initial_count': initial_count, 'final_count': final_count, 'expected_count': expected_count, 'growth': final_count - initial_count, 'potential_leak': final_count > expected_count + 1}
        for cycle_data in registry_instance_growth.values():
            ...
            if cycle_data['total_basic_objects'] > expected_basic * 1.5:
                memory_leak_evidence.append({'cycle': cycle_num, 'registry_type': 'basic', 'expected_objects': expected_basic, 'actual_objects': cycle_data['total_basic_objects'], 'excess_objects': cycle_data['total_basic_objects'] - expected_basic, 'description': f"BasicRegistry object count exceeds expected by {cycle_data['total_basic_objects'] - expected_basic}"})
            if cycle_data['total_advanced_objects'] > expected_advanced * 1.5:
                memory_leak_evidence.append({'cycle': cycle_num, 'registry_type': 'advanced', 'expected_objects': expected_advanced, 'actual_objects': cycle_data['total_advanced_objects'], 'excess_objects': cycle_data['total_advanced_objects'] - expected_advanced, 'description': f"AdvancedRegistry object count exceeds expected by {cycle_data['total_advanced_objects'] - expected_advanced}"})
        failure_reasons = []
        if memory_leak_evidence:
            for evidence in memory_leak_evidence:
                failure_reasons.append(f"Memory leak in {evidence['registry_type']} registry cycle {evidence['cycle']}: {evidence['excess_objects']} excess objects ({evidence['description']})")
        if shared_state_violations:
            for violation in shared_state_violations:
                failure_reasons.append(f"Shared state violation in {violation['registry_type']} registry cycle {violation['cycle']}: {violation['description']}")
        persistent_growth = []
        for obj_type_name, analysis in memory_growth_analysis.items():
            if analysis['potential_leak']:
                persistent_growth.append(f"{obj_type_name}: {analysis['growth']} objects not cleaned up ({analysis['final_count']} final vs {analysis['expected_count']} expected)")
        if persistent_growth:
            failure_reasons.extend(persistent_growth)
        if failure_reasons:
            pytest.fail(f"CRITICAL MEMORY LEAKS FROM SHARED STATE: Memory leaks and state sharing violations detected. This causes service degradation and crashes blocking Golden Path reliability. Memory issues: {'; '.join(failure_reasons)}. IMPACT: Service performance degrades over time, affecting all users.")
        self.logger.info(f'No memory leaks detected - proper isolation maintained. Final object counts: {final_object_counts}')
    ...
    async def test_websocket_event_delivery_to_wrong_users(self):
        ...
        for user_info in self.test_users:
            ...
            user_event_isolation_results[user_id] = {'websocket_manager': websocket_manager, 'events_received': [], 'events_expected': [], 'basic_registry_setup': False, 'advanced_registry_setup': False}
        for user_info in self.test_users:
            user_id = user_info['user_id']
            session_id = user_info['session_id']
        ...
        agent_events = [{'event': 'agent_started', 'data': {'agent_id': 'test-agent', 'status': 'started'}}, {'event': 'agent_thinking', 'data': {'agent_id': 'test-agent', 'message': 'Processing request'}}, {'event': 'agent_completed', 'data': {'agent_id': 'test-agent', 'result': 'Task completed'}}]
        for user_info in self.test_users:
            user_id = user_info['user_id']
            session_id = user_info['session_id']
            for event in agent_events:
                user_specific_event = {'event': event['event'], 'data': {**event['data'], 'user_id': user_id, 'session_id': session_id, 'private_data': f'PRIVATE_EVENT_DATA_FOR_{user_id}_{uuid.uuid4().hex[:8]}'}}
                ...
                if user_event_isolation_results[user_id]['basic_registry_setup']:
                    try:
                        if hasattr(self.basic_registry, '_notify_agent_event'):
                            await self.basic_registry._notify_agent_event(user_specific_event['event'], user_specific_event['data'])
                        elif hasattr(self.basic_registry, 'emit_agent_event'):
                            await self.basic_registry.emit_agent_event(user_specific_event['event'], user_specific_event['data'])
                ...
                if user_event_isolation_results[user_id]['advanced_registry_setup']:
                    try:
                        if hasattr(self.advanced_registry, '_notify_agent_event'):
                            await self.advanced_registry._notify_agent_event(user_specific_event['event'], user_specific_event['data'])
                        elif hasattr(self.advanced_registry, 'emit_agent_event'):
                            await self.advanced_registry.emit_agent_event(user_specific_event['event'], user_specific_event['data'])
            ...
            if websocket_manager.emit_agent_event.called:
                for call in websocket_manager.emit_agent_event.call_args_list:
                    ...
                    if len(args) >= 2:
                        ...
                        continue
                    results['events_received'].append({'event': event_type, 'data': event_data})
                    if isinstance(event_data, dict):
                        ...
                        if event_user_id and event_user_id != user_id:
                            websocket_event_contamination.append({'contamination_type': 'wrong_user_event', 'receiving_user': user_id, 'event_from_user': event_user_id, 'event_type': event_type, 'private_data_leaked': private_data, 'description': f'User {user_id} received event for user {event_user_id}'})
                        if private_data and f'FOR_{user_id}_' not in private_data:
                            for other_user in self.test_users:
                                if f"FOR_{other_user['user_id']}_" in private_data:
                                    websocket_event_contamination.append({'contamination_type': 'private_data_leak', 'receiving_user': user_id, 'data_from_user': other_user['user_id'], 'event_type': event_type, 'leaked_private_data': private_data, 'description': f"User {user_id} received private data from {other_user['user_id']}"})
                                    break
        missing_events = []
        for user_id, results in user_event_isolation_results.items():
            ...
            for expected_event in expected_events:
                ...
                for received_event in received_events:
                    if received_event['event'] == expected_event['event'] and received_event['data'].get('user_id') == user_id:
                        event_received = True
                        break
                if not event_received:
                    missing_events.append({'user_id': user_id, 'missing_event': expected_event['event'], 'event_data': expected_event['data'], 'description': f"User {user_id} did not receive expected {expected_event['event']} event"})
        failure_reasons = []
        ...
        if failure_reasons:
            pytest.fail(f"CRITICAL WEBSOCKET EVENT CONTAMINATION: WebSocket events delivered to wrong users. This causes privacy breaches and user confusion blocking Golden Path security. Event contamination: {'; '.join(failure_reasons)}. IMPACT: Users see other users' private agent activities and responses.")
    ...
    async def test_concurrent_execution_isolation_failures(self):
        ...
        concurrent_users = [{'user_id': f'concurrent-user-{i}', 'session_id': f'concurrent-session-{i}'} for i in range(5)]
        for user_info in concurrent_users:
            user_id = user_info['user_id']
            execution_isolation_results[user_id] = {'execution_started': False, 'execution_completed': False, 'execution_results': [], 'execution_errors': [], 'context_contamination': [], 'basic_registry_results': None, 'advanced_registry_results': None}
            ...
            try:
                ...
                if user_context and hasattr(user_context, 'get_user_data'):
                    ...
                    if retrieved_data != unique_data:
                        execution_isolation_results[user_id]['context_contamination'].append({'contamination_type': 'execution_data_mismatch', 'expected': unique_data, 'actual': retrieved_data, 'registry_type': registry_type})
                        execution_data_intact = False
                    if retrieved_start_time != execution_start_time:
                        execution_isolation_results[user_id]['context_contamination'].append({'contamination_type': 'execution_time_mismatch', 'expected': execution_start_time, 'actual': retrieved_start_time, 'registry_type': registry_type})
                        execution_data_intact = False
                execution_result = {'registry_type': registry_type, 'user_id': user_id, 'session_id': session_id, 'execution_time': time.time() - execution_start_time, 'agents_retrieved': agents is not None, 'agents_count': len(agents) if isinstance(agents, list) else None, 'user_context_created': user_context is not None, 'execution_data_intact': execution_data_intact, 'unique_data': unique_data}
                ...
                execution_isolation_results[user_id]['execution_errors'].append({'registry_type': registry_type, 'error': str(e), 'error_type': type(e).__name__})
                self.logger.error(f'Concurrent execution failed for {user_id} in {registry_type} registry: {e}')
                return None
        basic_registry_tasks = [execute_user_task(user_info, self.basic_registry, 'basic') for user_info in concurrent_users]
        advanced_registry_tasks = [execute_user_task(user_info, self.advanced_registry, 'advanced') for user_info in concurrent_users]
        ...
        for user_id, results in execution_isolation_results.items():
            if results['context_contamination']:
                for contamination in results['context_contamination']:
                    concurrent_execution_failures.append({'failure_type': 'context_contamination', 'user_id': user_id, 'registry_type': contamination['registry_type'], 'contamination_type': contamination['contamination_type'], 'description': f"User {user_id} execution context contaminated in {contamination['registry_type']} registry"})
            if results['execution_errors']:
                for error in results['execution_errors']:
                    concurrent_execution_failures.append({'failure_type': 'execution_error', 'user_id': user_id, 'registry_type': error['registry_type'], 'error': error['error'], 'description': f"User {user_id} execution failed in {error['registry_type']} registry: {error['error']}"})
                ...
                if user_a_id != user_b_id:
                    for result_a in results_a['execution_results']:
                        for result_b in results_b['execution_results']:
                            if result_a['registry_type'] == result_b['registry_type'] and result_a['unique_data'] == result_b['unique_data']:
                                concurrent_execution_failures.append({'failure_type': 'cross_user_contamination', 'user_a': user_a_id, 'user_b': user_b_id, 'registry_type': result_a['registry_type'], 'contaminated_data': result_a['unique_data'], 'description': f"Users {user_a_id} and {user_b_id} share execution data in {result_a['registry_type']} registry"})
        registry_execution_consistency = {}
        for registry_type in ['basic', 'advanced']:
            registry_execution_consistency[registry_type] = {'successful_executions': 0, 'failed_executions': 0, 'execution_times': [], 'agents_counts': []}
            for user_id, results in execution_isolation_results.items():
        ...
        for registry_type, consistency in registry_execution_consistency.items():
            ...
            if total_attempts > 0:
                ...
                if failure_rate > 0.2:
                    concurrent_execution_failures.append({'failure_type': 'high_failure_rate', 'registry_type': registry_type, 'failure_rate': failure_rate * 100, 'description': f'{registry_type} registry has {failure_rate * 100:.1f}% execution failure rate'})
            if len(consistency['execution_times']) > 1:
                ...
                if max_time > min_time * 3:
                    concurrent_execution_failures.append({'failure_type': 'execution_time_variance', 'registry_type': registry_type, 'min_time': min_time, 'max_time': max_time, 'variance_ratio': max_time / min_time, 'description': f'{registry_type} registry shows high execution time variance ({max_time / min_time:.1f}x)'})
        failure_reasons = []
        ...
        if failure_reasons:
            pytest.fail(f"CRITICAL CONCURRENT EXECUTION ISOLATION FAILURES: Multiple users interfere during concurrent execution. This causes incorrect responses and system instability blocking Golden Path reliability. Isolation failures: {'; '.join(failure_reasons)}. IMPACT: Concurrent users experience incorrect agent responses and execution errors.")
