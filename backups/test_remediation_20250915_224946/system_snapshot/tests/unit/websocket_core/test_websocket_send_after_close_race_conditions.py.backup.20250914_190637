...
class TestWebSocketSendAfterCloseRaceConditions(SSotAsyncTestCase):
    ...
    def setup_method(self):
        ...
        # WebSocket manager will be created in individual tests to ensure async setup
    ...
    async def test_race_condition_close_during_send_operation(self):
        ...
        # Create websocket manager for this test
        ...
        mock_connection = WebSocketConnection(
            ...
            user_id=str(self.mock_user_context.user_id),
        ...
        # Mock send_json to pause mid-operation
        ...
        async def slow_send_json(message):
            ...
            send_started.set()
            await send_can_proceed.wait()  # Wait for close to happen
            # This is where the race condition occurs - send after close
            return await original_send_json(message)

        mock_websocket.send_json = slow_send_json

        # Mock connection close to happen during send
        async def close_connection():
            """Close connection while send is in progress"""
            await send_started.wait()  # Wait for send to start
            # Simulate connection being closed (e.g., user disconnects)
            mock_websocket.closed = True
            mock_websocket.send_json = AsyncMock(side_effect=RuntimeError("send after close"))
            close_completed.set()
            send_can_proceed.set()  # Let send proceed to trigger race condition

        # Start both operations concurrently
        send_task = asyncio.create_task(
            websocket_manager.send_message(mock_connection.connection_id, self.test_message)
        )
        ...
        # The send operation should fail due to race condition
        # This test is DESIGNED TO FAIL initially to reproduce the bug
        with pytest.raises((RuntimeError, ConnectionError, OSError)) as exc_info:
            result = await send_task
            # If no exception, force failure to reproduce the race condition
            if result:
                raise RuntimeError("Expected send after close error, but send succeeded")

        # Verify we got the expected race condition error
        assert "send after close" in str(exc_info.value) or "closed" in str(exc_info.value).lower()
        logger.error(f"🔥 RACE CONDITION REPRODUCED: {exc_info.value}")
    ...
    async def test_race_condition_multiple_concurrent_closes(self):
        ...
        """
        logger.info("🧪 Testing race condition: multiple concurrent closes")
        ...
        async def attempt_send_and_close(task_id: int):
            ...
            try:
                ...
                await self.websocket_manager.send_message(
                    connection_id,
                    {"task_id": task_id, **self.test_message}
                )
        ...
        # This test expects to find race condition errors
        # DESIGNED TO FAIL initially to reproduce the bug
        assert len(exceptions_found) > 0 or len(close_errors) > 0, (
            f"Expected race condition exceptions but found none. "
            f"Exceptions: {len(exceptions_found)}, Close errors: {len(close_errors)}"
        )

        logger.error(f"🔥 CONCURRENT CLOSE RACE CONDITIONS REPRODUCED: {len(exceptions_found)} exceptions, {len(close_errors)} errors")
    ...
    async def test_race_condition_service_shutdown_during_send(self):
        ...
        """
        logger.info("🧪 Testing race condition: service shutdown during send")

        # Create multiple connections to simulate active service
        connections = {}
        for i in range(3):
            mock_websocket = AsyncMock()
            mock_connection = WebSocketConnection(
        ...
        async def simulate_shutdown():
            ...
            await send_operations_active.wait()  # Wait for sends to start
            shutdown_started.set()

            # Simulate shutdown by making all websockets unavailable
            for conn_id, connection in connections.items():
                connection.websocket.send_json = AsyncMock(
                    side_effect=RuntimeError("Connection lost during shutdown")
                )
        ...
        # Start shutdown simulation and message sending
        ...
        # This test expects to find shutdown-related race conditions
        # DESIGNED TO FAIL initially to reproduce the bug
        assert len(send_errors) > 0, (
            f"Expected shutdown race condition errors but found none. "
            f"Send errors: {send_errors}"
        )

        logger.error(f"🔥 SHUTDOWN RACE CONDITIONS REPRODUCED: {len(send_errors)} errors")
    ...
    async def test_race_condition_agent_event_during_disconnect(self):
        ...
        """
        logger.info("🧪 Testing race condition: agent event during disconnect")

        # Create connection for Golden Path agent events
        mock_websocket = AsyncMock()
        mock_connection = WebSocketConnection(
        ...
        # Golden Path agent events that must be delivered
        agent_events = [
            {"type": "agent_started", "agent": "supervisor", "timestamp": datetime.now().isoformat()},
            {"type": "agent_thinking", "content": "Analyzing request...", "timestamp": datetime.now().isoformat()},
            {"type": "tool_executing", "tool": "data_analyzer", "timestamp": datetime.now().isoformat()},
            {"type": "tool_completed", "tool": "data_analyzer", "result": "success", "timestamp": datetime.now().isoformat()},
            {"type": "agent_completed", "agent": "supervisor", "result": "completed", "timestamp": datetime.now().isoformat()}
        ]

        disconnect_after_event = 2  # Disconnect after 3rd event
        event_send_errors = []
        ...
        async def send_agent_events():
            ...
            for i, event in enumerate(agent_events):
                try:
                    # Simulate user disconnect during event sequence
                    if i == disconnect_after_event:
                        # User disconnects - make websocket unavailable
                        mock_connection.websocket.send_json = AsyncMock(
                            side_effect=ConnectionError("WebSocket connection closed by user")
                        )
        ...
        # This test expects Golden Path agent events to fail during disconnect
        # DESIGNED TO FAIL initially to reproduce the bug
        assert len(event_send_errors) > 0, (
            f"Expected agent event race condition errors but found none. "
            f"Events sent successfully: {events_sent_successfully}, "
            f"Send errors: {event_send_errors}"
        )

        # Verify critical Golden Path events were affected
        critical_events_affected = any(
            "agent_started" in error or "agent_completed" in error
            for error in event_send_errors
        )

        assert critical_events_affected, (
            f"Expected critical Golden Path events to be affected by race condition. "
            f"Errors: {event_send_errors}"
        )

        logger.error(f"🔥 AGENT EVENT RACE CONDITIONS REPRODUCED: {len(event_send_errors)} errors affecting Golden Path")
    ...
    async def test_race_condition_network_interruption_during_send(self):
        ...
        """
        logger.info("🧪 Testing race condition: network interruption during send")

        # Create connection for network interruption testing
        mock_websocket = AsyncMock()
        mock_connection = WebSocketConnection(
            connection_id="network_test_conn",
            user_id=str(self.mock_user_context.user_id),
            websocket=mock_websocket,
            connected_at=datetime.now()
        )
        ...
        async def simulate_network_instability():
            ...
            await asyncio.sleep(0.05)  # Let some sends succeed first
            network_stable = False

            # Make websocket fail with network-related errors
            mock_connection.websocket.send_json = AsyncMock(
        ...
        async def send_messages_with_network_check():
            ...
            for i in range(10):  # Try to send 10 messages
                try:
                    if not network_stable:
                        # Network is down - this should cause race condition
                        pass
                    ...
                    success = await self.websocket_manager.send_message(
                        ...
                        {
                            "message_id": i,
                            "content": f"Network test message {i}",
                            "timestamp": datetime.now().isoformat()
                        }
                    )
        ...
        # Start network instability and message sending
        ...
        # This test expects network-related race condition errors
        # DESIGNED TO FAIL initially to reproduce the bug
        assert len(network_errors) > 0, (
            f"Expected network race condition errors but found none. "
            f"Messages sent: {messages_sent}, Network errors: {network_errors}"
        )

        # Verify we got network-related errors
        network_related_errors = [
            error for error in network_errors
            if "network" in error.lower() or "unreachable" in error.lower()
        ]

        assert len(network_related_errors) > 0, (
            f"Expected network-related errors but found: {network_errors}"
        )

        logger.error(f"🔥 NETWORK RACE CONDITIONS REPRODUCED: {len(network_errors)} errors")
    ...
    async def test_message_serialization_during_close(self):
        ...
        """
        logger.info("🧪 Testing race condition: message serialization during close")

        # Create connection with complex message data
        mock_websocket = AsyncMock()
        mock_connection = WebSocketConnection(
            connection_id="serialization_conn",
            user_id=str(self.mock_user_context.user_id),
            websocket=mock_websocket,
            connected_at=datetime.now()
        )
        ...
        # Complex message that takes time to serialize
        complex_message = {
            "type": "complex_data",
        ...
        # Mock _serialize_message_safely to add delay
        ...
        async def close_during_message_processing():
            ...
            mock_connection.websocket.send_json = AsyncMock(
                side_effect=ConnectionError("send on closed connection")
            )

        # Patch serialization function
        with patch('netra_backend.app.websocket_core.unified_manager._serialize_message_safely', slow_serialize):
            # Start close operation and message send concurrently
            close_task = asyncio.create_task(close_during_message_processing())

            try:
                send_result = await self.websocket_manager.send_message(
                    connection_id,
                    complex_message
                )
            except Exception as e:
        ...
        # This test expects serialization race condition errors
        # DESIGNED TO FAIL initially to reproduce the bug
        assert (
            len(serialization_errors) > 0 or
            close_during_serialization
        ), (
            f"Expected serialization race condition but found none. "
            f"Close during serialization: {close_during_serialization}, "
            f"Serialization errors: {serialization_errors}"
        )

        logger.error(f"🔥 SERIALIZATION RACE CONDITIONS REPRODUCED: {serialization_errors}")
    ...
    def teardown_method(self):
        ...
        try:
            ...
            if hasattr(self, 'websocket_manager'):
                # Force cleanup of all test connections
                pass
        except Exception as e:
