...
class DeploymentPipelineIntegrationTests(SSotBaseTestCase):
    ...
    def test_terraform_to_script_integration_chain(self):
        ...
        if integration_issues:
            ...
            for issue in integration_issues:
                print(f'  - {issue}')
        self.assertEqual(len(integration_issues), 0, f'Integration chain should be intact, found {len(integration_issues)} issues: {integration_issues}')
    ...
    def test_ci_cd_pipeline_deployment_consistency(self):
        ...
        if consistency_analysis['inconsistencies']:
            ...
            for issue in consistency_analysis['inconsistencies']:
                print(f'  - {issue}')
        self.assertGreater(consistency_analysis['consistency_score'], 80, f"CI/CD and manual deployments should be >80% consistent, found {consistency_analysis['consistency_score']}%")
    ...
    def test_environment_specific_configuration_validation(self):
        ...
        """
        env_configs = {'development': self._extract_development_config(), 'staging': self._extract_staging_config(), 'production': self._extract_production_config()}
        validation_results = {}
        ...
        self.assertEqual(len(invalid_envs), 0, f'All environments should be valid, found invalid: {invalid_envs}')
        self.assertEqual(len(isolation_issues), 0, f'Environments should be isolated, found {len(isolation_issues)} issues')
        return {'env_configs': env_configs, 'validation_results': validation_results, 'isolation_issues': isolation_issues}
    ...
    def test_deployment_rollback_capability_validation(self):
        ...
        self.assertGreater(len(rollback_sources), 0, f'At least one deployment source should support rollback, found {len(rollback_sources)}')
    ...
    def test_deployment_performance_impact_analysis(self):
        ...
        """
        performance_metrics = {}
        deployment_scripts = [self.scripts_dir / 'deploy_to_gcp.py', self.scripts_dir / 'deploy_to_gcp_actual.py']
        for script in deployment_scripts:
            if script.exists():
        ...
        self.assertGreater(avg_performance, 60, f'Average deployment performance should be > 60, found {avg_performance}')
        return {'performance_metrics': performance_metrics, 'average_performance': avg_performance}
    ...
    def _analyze_terraform_deploy_script(self, script_path: Path) -> Dict:
        ...
        try:
            with open(script_path, 'r') as f:
                content = f.read()
            analysis = {'calls_python_deploy': 'deploy_to_gcp.py' in content, 'has_project_validation': 'PROJECT_ID' in content, 'has_error_handling': 'set -e' in content or 'exit' in content, 'has_logging': 'echo' in content, 'script_type': 'bash' if script_path.suffix == '.sh' else 'unknown'}
            return analysis
    ...
    def _test_terraform_python_integration(self) -> Dict:
        ...
        integration_data = {'terraform_calls_python': False, 'python_supports_terraform_args': False, 'integration_working': False}
        ...
        for py_script in python_scripts:
            try:
                ...
                continue
        integration_data['integration_working'] = integration_data['terraform_calls_python'] and integration_data['python_supports_terraform_args']
        ...
        if var_file.exists():
            try:
                with open(var_file, 'r') as f:
                    content = f.read()
                variables = {'has_project_var': 'variable "project"' in content, 'has_region_var': 'variable "region"' in content, 'has_environment_var': 'variable "environment"' in content}
            except Exception as e:
    ...
    def _extract_ci_cd_configurations(self) -> Dict:
        ...
        ci_configs = {}
        ci_files = list(self.project_root.glob('.github/workflows/*.yml')) + list(self.project_root.glob('.gitlab-ci.yml')) + list(self.project_root.glob('Jenkinsfile'))
        for ci_file in ci_files:
            try:
                with open(ci_file, 'r') as f:
                    content = f.read()
                config = {'has_deployment_step': 'deploy' in content.lower(), 'uses_docker': 'docker' in content.lower(), 'calls_scripts': 'scripts/' in content, 'has_staging_deploy': 'staging' in content.lower()}
        ...
        for script in deployment_scripts:
            try:
                with open(script, 'r') as f:
                    content = f.read()
                config = {'supports_staging': '--env' in content or 'staging' in content.lower(), 'supports_docker': 'docker' in content.lower(), 'has_validation': 'check' in content.lower() or 'validate' in content.lower(), 'supports_rollback': '--rollback' in content}
    ...
    def _analyze_ci_manual_consistency(self, ci_configs: Dict, manual_configs: Dict) -> Dict:
        ...
        return {'inconsistencies': inconsistencies, 'consistency_score': int(consistency_score), 'ci_configs': ci_configs, 'manual_configs': manual_configs}
    ...
    def _test_rollback_capability(self, script_path: Path) -> Dict:
        ...
        capability = {'supports_rollback': False, 'method': None, 'issues': []}
        try:
            with open(script_path, 'r') as f:
                content = f.read()
    ...
    def _measure_deployment_performance(self, script_path: Path) -> Dict:
        ...
        metrics = {'startup_time': 0, 'memory_usage': 0, 'cpu_usage': 0, 'performance_score': 0}
        try:
            start_time = time.time()
            result = subprocess.run([sys.executable, str(script_path), '--help'], capture_output=True, text=True, timeout=30)
            end_time = time.time()
    ...
    def _measure_test_runner_deployment_performance(self, runner_path: Path) -> Dict:
        ...
        metrics = {'startup_time': 0, 'performance_score': 0}
        try:
            start_time = time.time()
            result = subprocess.run([sys.executable, str(runner_path), '--help'], capture_output=True, text=True, timeout=30)
            end_time = time.time()
            metrics['startup_time'] = round(end_time - start_time, 2)
    ...
    def _parse_compose_file(self, compose_path: Path) -> Dict:
        ...
        try:
            ...
            return {'services': list(data.get('services', {}).keys()) if data else [], 'has_networks': 'networks' in data if data else False, 'has_volumes': 'volumes' in data if data else False}
        except Exception as e:
            return {'error': str(e)}
    ...
    def _parse_terraform_config(self, tf_path: Path) -> Dict:
        ...
        try:
            with open(tf_path, 'r') as f:
                content = f.read()
            return {'has_resources': 'resource ' in content, 'has_google_provider': 'google' in content, 'has_cloud_run': 'google_cloud_run' in content}
        except Exception as e:
            return {'error': str(e)}
