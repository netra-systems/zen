...
from shared.cors_config_builder import CORSConfigurationBuilder, CORSEnvironment, CORSSecurityEvent, get_cors_origins, get_cors_config, is_origin_allowed, validate_content_type, is_service_to_service_request, get_cors_health_info, get_websocket_cors_origins, get_static_file_cors_headers, get_cdn_cors_config, get_fastapi_cors_config
...
class TestCORSConfigurationComprehensive(SSotBaseTestCase):
    ...
    def test_cors_origins_builder_allowed_origins(self):
        ...
        expected_dev_origins = ['http://localhost:3000', 'http://localhost:8000', 'https://localhost:3000']
    ...
    def test_cors_origins_builder_is_allowed_validation(self):
        ...
        """
        cors_builder = CORSConfigurationBuilder()
        test_cases = [('http://localhost:3000', True, 'Localhost should be allowed in development'), ('https://localhost:3000', True, 'HTTPS localhost should be allowed'), ('http://127.0.0.1:3000', True, '127.0.0.1 should be allowed in development'), ('http://frontend:3000', True, 'Docker container names should be allowed'), ('https://malicious-site.com', False, 'External sites should not be allowed'), ('', False, 'Empty origin should not be allowed')]
    ...
    def test_cors_origins_builder_environment_specific_origins(self):
        ...
        """
        test_environments = {'development': ['localhost', '127.0.0.1', 'frontend'], 'staging': ['staging.netrasystems.ai', 'run.app'], 'production': ['netrasystems.ai', 'secure-domain.com']}
    ...
    def test_cors_headers_builder_configuration(self):
        ...
        self.assertTrue(cors_builder.headers.is_header_allowed('Content-Type'), 'Should allow Content-Type header')
        self.assertTrue(cors_builder.headers.is_method_allowed('POST'), 'Should allow POST method')
    ...
    def test_cors_security_builder_content_type_validation(self):
        ...
        """
        cors_builder = CORSConfigurationBuilder()
        allowed_content_types = ['application/json', 'application/x-www-form-urlencoded', 'text/plain', 'text/html', 'multipart/form-data']
        ...
        blocked_content_types = ['application/x-msdownload', 'text/vbscript', 'application/vnd.ms-excel']
    ...
    def test_cors_security_builder_event_logging(self):
        ...
        cors_builder.security.log_security_event('test_event', 'http://malicious-site.com', '/api/sensitive', request_id='test-123', additional_info={'user_agent': 'malicious-bot'})
    ...
    def test_cors_service_detector_internal_request_detection(self):
        ...
        """
        cors_builder = CORSConfigurationBuilder()
        internal_headers = {'x-service-name': 'auth-service', 'user-agent': 'httpx/0.24.0'}
        ...
        self.assertEqual(should_bypass, expected_bypass, f'CORS bypass should be {expected_bypass} in {cors_builder.environment}')
    ...
    def test_cors_fastapi_builder_middleware_config(self):
        ...
        self.assertTrue(middleware_config['allow_credentials'], 'Should allow credentials for authenticated requests')
    ...
    def test_cors_websocket_builder_configuration(self):
        ...
        self.assertEqual(websocket_origins, regular_origins, 'WebSocket origins should match regular CORS origins')
    ...
    def test_cors_static_assets_builder_configuration(self):
        ...
        self.assertEqual(static_headers['Access-Control-Allow-Origin'], '*', 'Static assets should allow all origins')
    ...
    def test_cors_health_builder_configuration_info(self):
        ...
        required_fields = ['environment', 'origins_count', 'origins', 'allow_credentials', 'methods', 'headers_count', 'config_valid']
        for field in required_fields:
            self.assertIn(field, config_info, f'Config info should include {field}')
        self.assertEqual(config_info['environment'], cors_builder.environment, 'Should report correct environment')
    ...
    def test_cors_configuration_environment_specificity(self):
        ...
        for env_name in test_environments:
            with self.subTest(environment=env_name):
                with self.temp_env_vars(ENVIRONMENT=env_name):
                    cors_builder = CORSConfigurationBuilder()
                    self.assertEqual(cors_builder.environment, env_name, f'Should detect {env_name} environment')
                    allowed_origins = cors_builder.origins.allowed
                    self.assertGreater(len(allowed_origins), 0, f'Should have origins for {env_name}')
                    is_valid, error_msg = cors_builder.validate()
                    if env_name == 'production':
                        self.assertNotIn('*', allowed_origins, 'Production should not allow wildcard origins')
    ...
    def test_cors_configuration_custom_origins_override(self):
        ...
        with self.temp_env_vars(CORS_ORIGINS=custom_origins):
            cors_builder = CORSConfigurationBuilder()
            allowed_origins = cors_builder.origins.allowed
            self.assertIn('https://custom1.example.com', allowed_origins, 'Should include first custom origin')
            self.assertIn('https://custom2.example.com', allowed_origins, 'Should include second custom origin')
    ...
    def test_cors_configuration_invalid_environment_handling(self):
        ...
        with self.temp_env_vars(ENVIRONMENT='invalid_environment'):
            cors_builder = CORSConfigurationBuilder()
            self.assertEqual(cors_builder.environment, 'invalid_environment', 'Should preserve environment name')
            allowed_origins = cors_builder.origins.allowed
    ...
    def test_cors_configuration_performance_requirements(self):
        ...
        print(f'[U+2713] CORS configuration performance validated - Origins: {origins_time:.3f}s, Validation: {validation_time:.3f}s, Middleware: {middleware_time:.3f}s')
    ...
    def test_cors_business_value_metrics(self):
        ...
        metrics = {'cors_origin_validation_reliability': f'{validation_reliability:.1%}', 'cors_middleware_generation_reliability': f'{middleware_reliability:.1%}', 'cors_security_validation_reliability': f'{security_reliability:.1%}'}
