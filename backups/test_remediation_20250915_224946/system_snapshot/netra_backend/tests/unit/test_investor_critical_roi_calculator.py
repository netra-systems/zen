...
from netra_backend.app.services.cost_calculator import CostCalculatorService, CostTier, ModelCostInfo, calculate_cost_savings, create_cost_calculator
...
class ROICalculatorService:
    ...
    def calculate_instant_roi(self, usage_tokens: int, current_provider: LLMProvider, current_model: str, optimized_provider: LLMProvider, optimized_model: str) -> Dict[str, Decimal]:
    ...
    def calculate_annual_savings(self, monthly_tokens: int, growth_rate: Decimal, current_provider: LLMProvider, current_model: str, optimized_provider: LLMProvider, optimized_model: str) -> Dict[str, Decimal]:
        """Calculate annual savings with growth projections"""
        monthly_savings = []
        for month in range(12):
            projected_tokens = int(monthly_tokens * growth_rate ** month)
            monthly_roi = self.calculate_instant_roi(projected_tokens, current_provider, current_model, optimized_provider, optimized_model)
            monthly_savings.append(monthly_roi['dollar_savings'])
        return self._aggregate_annual_metrics(monthly_savings)

    def calculate_tier_specific_savings(self, tier: str, usage_multiplier: int) -> Dict[str, Dict[str, Decimal]]:
        ...
        return {scenario_name: self.calculate_instant_roi(actual_tokens, scenario['current_provider'], scenario['current_model'], scenario['optimized_provider'], scenario['optimized_model']) for scenario_name, scenario in scenarios.items()}

    def _create_usage_object(self, total_tokens: int) -> TokenUsage:
        """Create token usage object with 70/30 split"""
        return TokenUsage(prompt_tokens=int(total_tokens * 0.7), completion_tokens=int(total_tokens * 0.3), total_tokens=total_tokens)

    def _get_monthly_cost(self, usage: TokenUsage, provider: LLMProvider, model: str) -> Decimal:
    ...
    def _build_roi_metrics(self, current_cost: Decimal, optimized_cost: Decimal) -> Dict[str, Decimal]:
        ...
        return {'current_cost': current_cost, 'optimized_cost': optimized_cost, 'dollar_savings': savings, 'percentage_savings': percentage.quantize(Decimal('0.1'))}
    ...
    def _get_tier_scenarios(self, tier: str) -> Dict[str, Dict[str, any]]:
        ...
        if tier == 'free':
            return {'basic_optimization': {'current_provider': LLMProvider.OPENAI, 'current_model': LLMModel.GEMINI_2_5_FLASH.value, 'optimized_provider': LLMProvider.GOOGLE, 'optimized_model': 'gemini-2.5-flash'}}
        elif tier == 'growth':
            return {'premium_optimization': {'current_provider': LLMProvider.OPENAI, 'current_model': LLMModel.GEMINI_2_5_FLASH.value, 'optimized_provider': LLMProvider.ANTHROPIC, 'optimized_model': 'claude-3.5-sonnet'}}
        else:
...
class InvestorCriticalROICalculatorTests:
    ...
    def test_instant_roi_calculation_accuracy(self, roi_service):
        ...
        roi_metrics = roi_service.calculate_instant_roi(usage_tokens=1000000, current_provider=LLMProvider.OPENAI, current_model=LLMModel.GEMINI_2_5_FLASH.value, optimized_provider=LLMProvider.GOOGLE, optimized_model='gemini-2.5-flash')
        expected_current = roi_metrics['current_cost']
        expected_savings = roi_metrics['dollar_savings']
        accuracy_threshold = expected_current * Decimal('0.001')
    ...
    def test_real_time_calculation_performance(self, roi_service):
        ...
        roi_metrics = roi_service.calculate_instant_roi(usage_tokens=5000000, current_provider=LLMProvider.OPENAI, current_model=LLMModel.GEMINI_2_5_FLASH.value, optimized_provider=LLMProvider.ANTHROPIC, optimized_model='claude-3.5-sonnet')
        calculation_time_ms = (time.perf_counter() - start_time) * 1000
        assert calculation_time_ms < 100.0
        assert roi_metrics is not None
        assert len(roi_metrics) == 4
    ...
    def test_multiple_model_cost_comparisons(self, roi_service):
        ...
        scenarios = [(LLMProvider.OPENAI, LLMModel.GEMINI_2_5_FLASH.value, LLMProvider.GOOGLE, 'gemini-2.5-flash'), (LLMProvider.ANTHROPIC, LLMModel.GEMINI_2_5_FLASH.value, LLMProvider.ANTHROPIC, 'claude-3-haiku'), (LLMProvider.OPENAI, LLMModel.GEMINI_2_5_FLASH.value, LLMProvider.GOOGLE, 'gemini-2.5-pro')]
        for current_provider, current_model, opt_provider, opt_model in scenarios:
            roi = roi_service.calculate_instant_roi(2000000, current_provider, current_model, opt_provider, opt_model)
            assert isinstance(roi['percentage_savings'], Decimal)
    ...
    def test_annual_savings_projections(self, roi_service):
        ...
        annual_savings = roi_service.calculate_annual_savings(monthly_tokens=1000000, growth_rate=Decimal('1.15'), current_provider=LLMProvider.OPENAI, current_model=LLMModel.GEMINI_2_5_FLASH.value, optimized_provider=LLMProvider.GOOGLE, optimized_model='gemini-2.5-flash')
        assert annual_savings['annual_savings'] > Decimal('0.0')
        assert annual_savings['avg_monthly_savings'] > Decimal('0.0')
    ...
    def test_zero_usage_edge_case(self, roi_service):
        ...
        roi_metrics = roi_service.calculate_instant_roi(usage_tokens=0, current_provider=LLMProvider.OPENAI, current_model=LLMModel.GEMINI_2_5_FLASH.value, optimized_provider=LLMProvider.GOOGLE, optimized_model='gemini-2.5-flash')
        assert roi_metrics['current_cost'] == Decimal('0.0')
    ...
    def test_massive_usage_spike_handling(self, roi_service):
        ...
        massive_usage = 100000000
        roi_metrics = roi_service.calculate_instant_roi(usage_tokens=massive_usage, current_provider=LLMProvider.OPENAI, current_model=LLMModel.GEMINI_2_5_FLASH.value, optimized_provider=LLMProvider.GOOGLE, optimized_model='gemini-2.5-flash')
        assert roi_metrics['dollar_savings'] > Decimal('0.0')
        assert roi_metrics['percentage_savings'] > Decimal('80.0')
        assert isinstance(roi_metrics['current_cost'], Decimal)
    ...
    def test_negative_value_protection(self, roi_service):
        ...
        roi_metrics = roi_service.calculate_instant_roi(usage_tokens=100000, current_provider=LLMProvider.GOOGLE, current_model='gemini-2.5-flash', optimized_provider=LLMProvider.OPENAI, optimized_model=LLMModel.GEMINI_2_5_FLASH.value)
        assert isinstance(roi_metrics['dollar_savings'], Decimal)
