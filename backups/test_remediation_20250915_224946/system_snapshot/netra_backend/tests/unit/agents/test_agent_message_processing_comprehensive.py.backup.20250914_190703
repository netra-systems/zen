...
class TestAgentMessageRouting(SSotAsyncTestCase):
    ...
    async def test_message_routing_with_invalid_type(self):
        ...
        # Test with None message type
        with self.expect_exception(ValueError):
            invalid_message = WebSocketMessage(
                id=self.id_manager.generate_id(IDType.MESSAGE),
                type=None,
                user_id=self.user_id,
                data={"content": "test"}
            )
    ...
    async def test_message_normalization(self):
        ...
        # Test string to enum normalization
    ...
    async def test_message_routing_with_user_context(self):
        ...
        message = create_standard_message(
            message_type=MessageType.AGENT_REQUEST,
            user_id=self.user_id,
            data={"content": "test with context"}
        )
    ...
    async def test_message_routing_concurrent_messages(self):
        ...
        messages = [
            create_standard_message(
                message_type=MessageType.AGENT_REQUEST,
                user_id=f"user_{i}",
                data={"content": f"concurrent message {i}"}
            )
            for i in range(5)
        ]
...
class TestMessageTransformationProcessing(SSotAsyncTestCase):
    ...
    async def test_message_data_validation(self):
        ...
        # Test with None data
        message_none = create_standard_message(
            message_type=MessageType.AGENT_REQUEST,
            user_id=self.user_id,
            data=None
        )
    ...
    async def test_message_timestamp_processing(self):
        ...
        message_custom = create_standard_message(
            message_type=MessageType.AGENT_REQUEST,
            user_id=self.user_id,
            data={"content": "custom timestamp"},
    ...
    async def test_message_metadata_processing(self):
        ...
        message = create_standard_message(
            message_type=MessageType.AGENT_REQUEST,
            user_id=self.user_id,
            data={"content": "metadata test", "metadata": metadata}
        )
    ...
    async def test_message_size_validation(self):
        ...
        normal_message = create_standard_message(
            message_type=MessageType.AGENT_REQUEST,
            user_id=self.user_id,
            data={"content": normal_content}
        )
        ...
        large_message = create_standard_message(
            message_type=MessageType.AGENT_REQUEST,
            user_id=self.user_id,
            data={"content": large_content}
        )

        # Should still create message (size limits enforced elsewhere)
...
class TestMessageErrorHandling(SSotAsyncTestCase):
    ...
    async def test_malformed_message_data_handling(self):
        ...
        try:
            message = create_standard_message(
                message_type=MessageType.AGENT_REQUEST,
                user_id=self.user_id,
                data={"invalid": lambda x: x}  # Non-serializable data
            )
    ...
    async def test_message_processing_timeout_handling(self):
        ...
        try:
            await asyncio.wait_for(
                handler.handle_message(self.user_id, None, message),
                timeout=0.5
            )
        ...
        duration = time.time() - start_time
        self.assertLess(duration, 1.0)  # Should timeout before 1 second
    ...
    async def test_concurrent_error_handling(self):
        ...
        messages = [
            ...
            create_standard_message(
                message_type=MessageType.AGENT_REQUEST,
                user_id=self.user_id,
                data={"content": "another normal message"}
            )
        ]
    ...
    async def test_memory_error_handling(self):
        ...
        try:
            ...
            # If memory error occurs, it should be handled
    ...
    async def test_graceful_degradation(self):
        ...
        message = create_standard_message(
            message_type=MessageType.AGENT_REQUEST,
            user_id=self.user_id,
            data={"content": "degradation test"}
        )

        # Should handle missing optional components gracefully
...
class TestAgentResponseGeneration(SSotAsyncTestCase):
    ...
    async def test_agent_response_message_creation(self):
        ...
        response_msg = create_standard_message(
    ...
    async def test_streaming_response_generation(self):
        ...
        for i, chunk in enumerate(chunks):
            ...
            chunk_msg = create_standard_message(
                message_type=MessageType.AGENT_STREAM,
                user_id=self.user_id,
                data=chunk_data
            )
    ...
    async def test_error_response_generation(self):
        ...
        error_response = create_error_message(
            ...
            error_message="Agent processing timed out",
            user_id=self.user_id,
            details={"timeout_seconds": 30, "agent_id": "agent_123"}
        )
    ...
    async def test_response_correlation_tracking(self):
        ...
        request_msg = create_standard_message(
            message_type=MessageType.AGENT_REQUEST,
            user_id=self.user_id,
    ...
    async def test_multi_step_response_generation(self):
        ...
        steps = [
            {"step": 1, "action": "analyzing", "status": "in_progress"},
            {"step": 2, "action": "processing", "status": "in_progress"},
            {"step": 3, "action": "generating_response", "status": "completed"}
        ]

        step_messages = []
        for step in steps:
            step_msg = create_standard_message(
                message_type=MessageType.AGENT_STEP,
                user_id=self.user_id,
                data=step
            )
    ...
    async def test_response_formatting_and_validation(self):
        ...
        raw_response = {
            "content": "  Here's your answer with extra spaces  ",
