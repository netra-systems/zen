...
class MockAgentCore:
    ...
    async def execute_agent(self, context: AgentExecutionContext, state: DeepAgentState) -> AgentExecutionResult:
        ...
        if self.should_succeed:
            return AgentExecutionResult(success=True, agent_name=context.agent_name, execution_time=time.time() - start_time, state=state, metadata={'test': 'success'})
        else:
...
class TestExecutionEngineConstruction(SSotAsyncTestCase):
    ...
    def test_factory_construction_requires_user_context(self):
        ...
        engine = create_request_scoped_engine(user_context=user_context, registry=self.registry, websocket_bridge=self.websocket_bridge)
    ...
    def test_factory_construction_with_invalid_user_context(self):
        ...
        with self.assertRaises(Exception):
            create_request_scoped_engine(user_context=None, registry=self.registry, websocket_bridge=self.websocket_bridge)

    def test_factory_create_request_scoped_engine_creates_engine(self):
        """Test internal create_request_scoped_engine method."""
        user_context = UserExecutionContext.from_request(user_id='factory_user', thread_id='factory_thread', run_id='factory_run')
    ...
    def test_factory_init_creates_unique_instances(self):
        ...
        engine1 = ExecutionEngine.create_request_scoped_engine(registry=self.registry, websocket_bridge=self.websocket_bridge, user_context=context1)
        engine2 = ExecutionEngine.create_request_scoped_engine(registry=self.registry, websocket_bridge=self.websocket_bridge, user_context=context2)
    ...
    def test_websocket_bridge_validation(self):
        ...
        user_context = UserExecutionContext.from_request(user_id='validation_user', thread_id='validation_thread', run_id='validation_run')
        ...
        del invalid_bridge.notify_agent_started
        with self.assertRaises(RuntimeError) as cm:
    ...
    def test_websocket_bridge_security_validation(self):
        ...
        insecure_bridge.notify_agent_started = MagicMock()
        with self.assertRaises(RuntimeError) as cm:
            ExecutionEngine.create_request_scoped_engine(registry=self.registry, websocket_bridge=insecure_bridge, user_context=user_context)
...
class TestExecutionEngineInitialization(SSotAsyncTestCase):
    ...
    def create_engine(self) -> ExecutionEngine:
        ...
        return ExecutionEngine.create_request_scoped_engine(registry=self.registry, websocket_bridge=self.websocket_bridge, user_context=self.user_context)
    ...
    def test_initialization_with_and_without_user_context(self):
        ...
        with_context = ExecutionEngine.create_request_scoped_engine(registry=self.registry, websocket_bridge=self.websocket_bridge, user_context=self.user_context)
    ...
    def test_execution_stats_initialization(self):
        ...
        expected_keys = ['total_executions', 'concurrent_executions', 'queue_wait_times', 'execution_times', 'failed_executions', 'dead_executions', 'timeout_executions']
...
class TestExecutionEngineValidation(SSotAsyncTestCase):
    ...
    def test_valid_execution_context_passes(self):
        ...
        context = AgentExecutionContext(run_id='valid_run', thread_id='valid_thread', user_id='valid_user', agent_name='test_agent')
        self.engine._validate_execution_context(context)

    def test_empty_user_id_raises_error(self):
        """Test that empty user_id raises ValueError."""
        context = AgentExecutionContext(run_id='valid_run', thread_id='valid_thread', user_id='', agent_name='test_agent')
    ...
    def test_none_user_id_raises_error(self):
        ...
        context = AgentExecutionContext(run_id='valid_run', thread_id='valid_thread', user_id=None, agent_name='test_agent')
        with self.assertRaises(ValueError) as cm:
            self.engine._validate_execution_context(context)
        self.assertIn('user_id must be a non-empty string', str(cm.exception))
    ...
    def test_registry_placeholder_run_id_raises_error(self):
        ...
        context = AgentExecutionContext(run_id='registry', thread_id='valid_thread', user_id='valid_user', agent_name='test_agent')
        with self.assertRaises(ValueError) as cm:
            self.engine._validate_execution_context(context)
    ...
    def test_empty_run_id_raises_error(self):
        ...
        context = AgentExecutionContext(run_id='   ', thread_id='valid_thread', user_id='valid_user', agent_name='test_agent')
        with self.assertRaises(ValueError) as cm:
            self.engine._validate_execution_context(context)
    ...
    def test_user_context_consistency_validation(self):
        ...
        context = AgentExecutionContext(run_id='consistent_run', thread_id='consistent_thread', user_id='different_user', agent_name='test_agent')
        with self.assertRaises(ValueError) as cm:
            self.engine._validate_execution_context(context)
    ...
    def test_user_context_run_id_mismatch_warning(self):
        ...
        context = AgentExecutionContext(run_id='different_run', thread_id='validation_thread', user_id='validation_user', agent_name='test_agent')
    ...
    def test_validation_with_empty_agent_name(self):
        ...
        context = AgentExecutionContext(run_id='validation_run', thread_id='validation_thread', user_id='validation_user', agent_name='')
        try:
            self.engine._validate_execution_context(context)
        except ValueError:
    ...
    def test_validation_with_special_characters_in_ids(self):
        ...
        context = AgentExecutionContext(run_id='run-with-dashes_and_underscores.123', thread_id='thread@with#special$chars', user_id='user:with:colons', agent_name='test_agent')
        self.engine._validate_execution_context(context)

    def test_validation_with_very_long_ids(self):
        """Test validation with very long IDs."""
        long_id = 'a' * 1000
        context = AgentExecutionContext(run_id=long_id, thread_id=long_id, user_id=long_id, agent_name='test_agent')
        self.engine._validate_execution_context(context)

    def test_validation_with_unicode_characters(self):
        """Test validation with Unicode characters in IDs."""
        context = AgentExecutionContext(run_id='run_[U+6D4B][U+8BD5]_[U+1F680]', thread_id='thread_[U+65E5][U+672C][U+8A9E]', user_id='user_[U+0627][U+0644][U+0639][U+0631][U+0628][U+064A][U+0629]', agent_name='test_agent')
        self.engine._validate_execution_context(context)

    def test_validation_caching_performance(self):
        """Test validation performance with repeated calls."""
        context = AgentExecutionContext(run_id='validation_run', thread_id='validation_thread', user_id='validation_user', agent_name='test_agent')
        start_time = time.time()
        for _ in range(100):
            self.engine._validate_execution_context(context)
        end_time = time.time()
...
class TestExecutionEngineWebSocketEvents(SSotAsyncTestCase):
    ...
    async def test_send_final_report_event(self):
        ...
        report = {'status': 'completed', 'recommendations': ['Use spot instances', 'Right-size servers'], 'potential_savings': 15000}
        duration_ms = 2500.0
    ...
    async def test_send_via_user_emitter_fallback(self):
        ...
        success = await self.engine._send_via_user_emitter('notify_agent_started', 'test_agent', {'test': 'data'})
        self.assertFalse(success)
        engine_no_context = ExecutionEngine.create_request_scoped_engine(registry=self.registry, websocket_bridge=self.websocket_bridge, user_context=None)
        success = await engine_no_context._send_via_user_emitter('notify_agent_started', 'test_agent', {'test': 'data'})
        self.assertFalse(success)
    ...
    async def test_websocket_event_data_integrity(self):
        ...
        test_data = {'complex_data': {'nested': {'deeply': ['array', 'items']}, 'number': 42}, 'unicode': '[U+6D4B][U+8BD5][U+6570][U+636E] [U+1F680]', 'special_chars': '@#$%^&*()'}
    ...
    async def test_websocket_event_error_handling(self):
        ...
        engine_with_error_bridge = ExecutionEngine.create_request_scoped_engine(registry=self.registry, websocket_bridge=error_bridge, user_context=self.user_context)
        try:
...
class TestExecutionEngineAgentExecution(SSotAsyncTestCase):
    ...
    async def test_successful_agent_execution(self):
        ...
        context = AgentExecutionContext(run_id='execution_run', thread_id='execution_thread', user_id='execution_user', agent_name='test_agent')
    ...
    async def test_agent_execution_with_failure(self):
        ...
        context = AgentExecutionContext(run_id='execution_run', thread_id='execution_thread', user_id='execution_user', agent_name='failing_agent')
        state = DeepAgentState()
        with patch.object(engine, 'execution_tracker') as mock_tracker:
    ...
    async def test_agent_execution_timeout(self):
        ...
        slow_core = MockAgentCore(should_succeed=True, execution_time=35000)
        engine = ExecutionEngine.create_request_scoped_engine(registry=self.registry, websocket_bridge=self.websocket_bridge, user_context=self.user_context)
        engine.agent_core = slow_core
        context = AgentExecutionContext(run_id='execution_run', thread_id='execution_thread', user_id='execution_user', agent_name='slow_agent')
        state = DeepAgentState()
        with patch.object(engine, 'execution_tracker') as mock_tracker:
    ...
    async def test_agent_execution_concurrency_control(self):
        ...
        tasks = []
        for i in range(15):
            context = AgentExecutionContext(run_id=f'concurrent_run_{i}', thread_id=f'concurrent_thread_{i}', user_id='execution_user', agent_name=f'concurrent_agent_{i}')
    ...
    async def test_execution_stats_tracking(self):
        ...
        engine = self.create_engine_with_mock_core()
        context = AgentExecutionContext(run_id='stats_run', thread_id='stats_thread', user_id='execution_user', agent_name='stats_agent')
        state = DeepAgentState()
        with patch.object(engine, 'execution_tracker') as mock_tracker:
...
class TestExecutionEnginePipelineExecution(SSotAsyncTestCase):
    ...
    def create_pipeline_steps(self, count: int, strategy: AgentExecutionStrategy=AgentExecutionStrategy.SEQUENTIAL) -> List[PipelineStep]:
        ...
        for i in range(count):
            step = PipelineStep(agent_name=f'pipeline_agent_{i}', strategy=strategy, metadata={'step_index': i})
            steps.append(step)
        return steps
    ...
    async def test_sequential_pipeline_execution(self):
        ...
        context = AgentExecutionContext(run_id='pipeline_run', thread_id='pipeline_thread', user_id='pipeline_user', agent_name='pipeline_coordinator')
        state = DeepAgentState()
        with patch.object(self.engine, '_execute_step') as mock_execute:
            mock_results = [AgentExecutionResult(success=True, agent_name=f'pipeline_agent_{i}', execution_time=0.1) for i in range(3)]
    ...
    async def test_pipeline_step_condition_evaluation(self):
        ...
        conditional_step = PipelineStep(agent_name='conditional_agent', condition=test_condition)
    ...
    async def test_pipeline_early_termination_on_failure(self):
        ...
        context = AgentExecutionContext(run_id='termination_run', thread_id='termination_thread', user_id='pipeline_user', agent_name='termination_coordinator')
        state = DeepAgentState()
        with patch.object(self.engine, '_execute_step') as mock_execute:
            mock_results = [AgentExecutionResult(success=True, agent_name='pipeline_agent_0', execution_time=0.1), AgentExecutionResult(success=False, agent_name='pipeline_agent_1', execution_time=0.1, error='Mock failure')]
            mock_execute.side_effect = mock_results
            with patch.object(self.engine, '_should_execute_step', return_value=True):
                results = await self.engine.execute_pipeline(steps, context, state)
    ...
    async def test_pipeline_parallel_execution_with_gather(self):
        ...
        parallel_steps = self.create_pipeline_steps(3, AgentExecutionStrategy.PARALLEL)
        context = AgentExecutionContext(run_id='parallel_run', thread_id='parallel_thread', user_id='pipeline_user', agent_name='parallel_coordinator')
        state = DeepAgentState()
    ...
    async def test_pipeline_step_context_creation(self):
        ...
        base_context = AgentExecutionContext(run_id='base_run', thread_id='base_thread', user_id='base_user', agent_name='base_agent')
        step = PipelineStep(agent_name='step_agent', metadata={'step_type': 'analysis', 'priority': 'high'})
...
class TestExecutionEngineErrorHandling(SSotAsyncTestCase):
    ...
    async def test_execution_error_handling(self):
        ...
        context = AgentExecutionContext(run_id='error_run', thread_id='error_thread', user_id='error_user', agent_name='error_agent')
        state = DeepAgentState()
    ...
    async def test_retry_mechanism(self):
        ...
        context = AgentExecutionContext(run_id='retry_run', thread_id='retry_thread', user_id='error_user', agent_name='retry_agent', retry_count=0, max_retries=3)
        state = DeepAgentState()
    ...
    async def test_fallback_strategy_execution(self):
        ...
        context = AgentExecutionContext(run_id='fallback_run', thread_id='fallback_thread', user_id='error_user', agent_name='fallback_agent')
        state = DeepAgentState()
    ...
    async def test_timeout_result_creation(self):
        ...
        context = AgentExecutionContext(run_id='timeout_run', thread_id='timeout_thread', user_id='error_user', agent_name='timeout_agent')
    ...
    async def test_error_result_creation(self):
        ...
        context = AgentExecutionContext(run_id='error_result_run', thread_id='error_result_thread', user_id='error_user', agent_name='error_result_agent')
    ...
    async def test_user_notification_methods(self):
        ...
        context = AgentExecutionContext(run_id='notification_run', thread_id='notification_thread', user_id='error_user', agent_name='notification_agent')
...
class TestExecutionEnginePerformanceAndStats(SSotAsyncTestCase):
    ...
    async def test_history_size_limit_enforcement(self):
        ...
        for i in range(ExecutionEngine.MAX_HISTORY_SIZE + 50):
            result = AgentExecutionResult(success=True, agent_name=f'test_agent_{i}', execution_time=0.1)
...
class TestExecutionEngineFactoryMethods(SSotAsyncTestCase):
    ...
    def test_create_request_scoped_engine_with_concurrency_limit(self):
        ...
        max_concurrent = 5
        engine = create_request_scoped_engine(user_context=self.user_context, registry=self.registry, websocket_bridge=self.websocket_bridge, max_concurrent_executions=max_concurrent)
    ...
    def test_create_execution_context_manager_with_params(self):
        ...
        max_concurrent = 8
        timeout = 45.0
        context_manager = create_execution_context_manager(registry=self.registry, websocket_bridge=self.websocket_bridge, max_concurrent_per_request=max_concurrent, execution_timeout=timeout)
...
class TestExecutionEngineUserIsolation(SSotAsyncTestCase):
    ...
    def test_user_execution_context_integration(self):
        ...
        user_context = UserExecutionContext.from_request(user_id='isolation_user', thread_id='isolation_thread', run_id='isolation_run')
        engine = ExecutionEngine.create_request_scoped_engine(registry=self.registry, websocket_bridge=self.websocket_bridge, user_context=user_context)
    ...
    def test_engine_without_user_context(self):
        ...
        engine = ExecutionEngine.create_request_scoped_engine(registry=self.registry, websocket_bridge=self.websocket_bridge, user_context=None)
        isolation_status = engine.get_isolation_status()
    ...
    def test_has_user_context_method(self):
        ...
        with_context = ExecutionEngine.create_request_scoped_engine(registry=self.registry, websocket_bridge=self.websocket_bridge, user_context=UserExecutionContext.from_request('user1', 'thread1', 'run1'))
    ...
    async def test_user_isolation_in_concurrent_scenarios(self):
        ...
        engines = []
        for i in range(5):
            user_context = UserExecutionContext.from_request(user_id=f'concurrent_user_{i}', thread_id=f'concurrent_thread_{i}', run_id=f'concurrent_run_{i}')
            contexts.append(user_context)
            engine = ExecutionEngine.create_request_scoped_engine(registry=self.registry, websocket_bridge=MockWebSocketBridge(), user_context=user_context)
    ...
    async def test_user_context_metadata_isolation(self):
        ...
        context1 = UserExecutionContext.from_request(user_id='metadata_user_1', thread_id='metadata_thread_1', run_id='metadata_run_1', metadata={'priority': 'high', 'source': 'web_ui'})
        context2 = UserExecutionContext.from_request(user_id='metadata_user_2', thread_id='metadata_thread_2', run_id='metadata_run_2', metadata={'priority': 'low', 'source': 'api'})
        engine1 = ExecutionEngine.create_request_scoped_engine(registry=self.registry, websocket_bridge=MockWebSocketBridge(), user_context=context1)
        engine2 = ExecutionEngine.create_request_scoped_engine(registry=self.registry, websocket_bridge=MockWebSocketBridge(), user_context=context2)
    ...
    async def test_isolation_status_reporting_accuracy(self):
        ...
        complex_context = UserExecutionContext.from_request(user_id='complex_user', thread_id='complex_thread', run_id='complex_run', websocket_client_id='ws_conn_123', metadata={'env': 'production', 'features': ['feature_a', 'feature_b']})
        standard_engine = ExecutionEngine.create_request_scoped_engine(registry=self.registry, websocket_bridge=MockWebSocketBridge(), user_context=standard_context)
        complex_engine = ExecutionEngine.create_request_scoped_engine(registry=self.registry, websocket_bridge=MockWebSocketBridge(), user_context=complex_context)
    ...
    def test_user_context_validation_integration(self):
        ...
        try:
            invalid_context = UserExecutionContext.from_request(user_id='', thread_id='test_thread', run_id='test_run')
        ...
        engine = ExecutionEngine.create_request_scoped_engine(registry=self.registry, websocket_bridge=MockWebSocketBridge(), user_context=valid_context)
    ...
    async def test_static_user_isolation_method(self):
        ...
        user_context = UserExecutionContext.from_request(user_id='static_user', thread_id='static_thread', run_id='static_run')
        agent_context = AgentExecutionContext(run_id='static_run', thread_id='static_thread', user_id='static_user', agent_name='static_agent')
        state = DeepAgentState()
        with patch('netra_backend.app.agents.supervisor.execution_engine.user_execution_engine') as mock_context:
            mock_engine = AsyncMock()
            mock_engine.execute_agent.return_value = AgentExecutionResult(success=True, agent_name='static_agent', execution_time=1.0)
...
class TestExecutionEngineShutdownAndCleanup(SSotAsyncTestCase):
    ...
    def test_flow_logger_integration(self):
        ...
        context = AgentExecutionContext(run_id='flow_run', thread_id='flow_thread', user_id='cleanup_user', agent_name='flow_agent')
    ...
    async def test_memory_management_history_limit(self):
        ...
        for i in range(ExecutionEngine.MAX_HISTORY_SIZE + 20):
            result = AgentExecutionResult(success=True, agent_name=f'agent_{i}', execution_time=0.1)
            self.engine._update_history(result)
        self.assertEqual(len(self.engine.run_history), ExecutionEngine.MAX_HISTORY_SIZE)
        last_result = self.engine.run_history[-1]
    ...
    async def test_completion_event_sending(self):
        ...
        context = AgentExecutionContext(run_id='completion_run', thread_id='completion_thread', user_id='cleanup_user', agent_name='completion_agent')
        state = DeepAgentState()
        state.final_answer = 'Task completed successfully'
        success_result = AgentExecutionResult(success=True, agent_name='completion_agent', execution_time=2.5, metadata={'status': 'success'})
        success_result.duration = 2.5
        await self.engine._send_final_execution_report(context, success_result, state)
        failed_result = AgentExecutionResult(success=False, agent_name='completion_agent', execution_time=1.0, error='Mock execution failure', metadata={'fallback_used': True})
