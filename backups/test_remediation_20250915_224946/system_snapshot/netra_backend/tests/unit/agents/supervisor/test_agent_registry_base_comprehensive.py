...
class AgentRegistryInitializationTests(SSotAsyncTestCase):
    ...
    async def test_init_creates_registry_with_required_components(self):
        ...
        registry = AgentRegistry(llm_manager=mock_llm_manager, tool_dispatcher_factory=mock_tool_dispatcher_factory)
...
class UserAgentSessionBehaviorTests(SSotAsyncTestCase):
    ...
    async def test_create_agent_execution_context(self):
        ...
        test_user_id = f'unit_testing_user_{uuid.uuid4().hex[:8]}'
        user_session = UserAgentSession(test_user_id)
        test_user_context = UserExecutionContext(user_id=test_user_id, request_id=f'unit_testing_request_{uuid.uuid4().hex[:8]}', thread_id=f'unit_testing_thread_{uuid.uuid4().hex[:8]}', run_id=f'unit_testing_run_{uuid.uuid4().hex[:8]}')
        execution_context = await user_session.create_agent_execution_context('test_agent', test_user_context)
...
class AgentCreationAndManagementTests(SSotAsyncTestCase):
    ...
    async def test_create_agent_for_user_handles_unknown_agent_type(self):
        ...
        with self.expect_exception(KeyError, 'No factory registered for agent type'):
            await registry.create_agent_for_user(user_id=test_user_id, agent_type='nonexistent_agent', user_context=test_user_context)
...
class ToolDispatcherIntegrationTests(SSotAsyncTestCase):
    ...
    async def test_create_tool_dispatcher_for_user_creates_isolated_dispatcher(self):
        ...
        with patch('netra_backend.app.core.tools.unified_tool_dispatcher.UnifiedToolDispatcher') as mock_unified_dispatcher:
            ...
            mock_unified_dispatcher.create_for_user.assert_called_once_with(user_context=test_user_context, websocket_bridge=None, enable_admin_tools=False)
    ...
    async def test_default_dispatcher_factory_uses_unified_dispatcher(self):
        ...
        with patch('netra_backend.app.core.tools.unified_tool_dispatcher.UnifiedToolDispatcher') as mock_unified_dispatcher:
            mock_dispatcher = Mock()
            mock_unified_dispatcher.create_for_user = AsyncMock(return_value=mock_dispatcher)
            result = await registry._default_dispatcher_factory(user_context=test_user_context, websocket_bridge=None)
            assert result == mock_dispatcher
            mock_unified_dispatcher.create_for_user.assert_called_once_with(user_context=test_user_context, websocket_bridge=None, enable_admin_tools=False)
...
class AgentFactoryRegistrationTests(SSotAsyncTestCase):
    ...
    async def test_register_agent_safely_handles_success(self):
        ...
        result = await registry.register_agent_safely(name='test_agent', agent_class=mock_agent_class)
        assert result is True
...
class ConcurrencyAndThreadSafetyTests(SSotAsyncTestCase):
    ...
    async def test_concurrent_websocket_manager_setting(self):
        ...
        tasks = [registry.set_websocket_manager_async(manager) for manager in mock_websocket_managers]
...
class EdgeCasesAndErrorHandlingTests(SSotAsyncTestCase):
    ...
    async def test_concurrent_access_to_user_session_agents(self):
        ...
        register_tasks = [user_session.register_agent(name, agent) for name, agent in zip(agent_names, mock_agents)]
        await asyncio.gather(*register_tasks)
        get_tasks = [user_session.get_agent(name) for name in agent_names]
    ...
    async def test_registry_cleanup_with_partially_failed_sessions(self):
        ...
        assert len(cleanup_report['errors']) >= 1
