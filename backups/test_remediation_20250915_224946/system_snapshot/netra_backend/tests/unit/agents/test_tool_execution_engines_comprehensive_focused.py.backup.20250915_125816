...
from netra_backend.app.agents.unified_tool_execution import UnifiedToolExecutionEngine, EnhancedToolExecutionEngine, enhance_tool_dispatcher_with_notifications
...
from netra_backend.app.schemas.tool import ToolInput, ToolResult, ToolStatus, ToolExecuteResponse, SimpleToolPayload
from netra_backend.app.schemas.tool_permission import PermissionCheckResult, ToolExecutionContext
...
class RealMockTool:
    ...
    def __init__(self, name: str, execution_time: float=0.1, should_fail: bool=False, fail_mode: str='runtime_error', has_handler: bool=True):
        self.name = name
...
class RealMockUser:
    ...
    def __init__(self, user_id: str='test_user', plan_tier: str='free', is_developer: bool=False, roles: List[str]=None):
        self.id = user_id
...
class RealMockPermissionService:
    ...
    async def check_tool_permission(self, context: ToolExecutionContext) -> PermissionCheckResult:
        ...
        self.permission_checks.append({'user_id': context.user_id, 'tool_name': context.tool_name, 'timestamp': datetime.now(UTC)})
        if self.simulate_failures and (not self.allow_all):
            return PermissionCheckResult(allowed=False, reason=f'Permission denied for {context.tool_name}', user_id=context.user_id, tool_name=context.tool_name)
        return PermissionCheckResult(allowed=True, reason='Permission granted', user_id=context.user_id, tool_name=context.tool_name)

    async def record_tool_usage(self, user_id: str, tool_name: str, execution_time_ms: int, status: str):
        """Record tool usage for tracking."""
        self.usage_records.append({'user_id': user_id, 'tool_name': tool_name, 'execution_time_ms': execution_time_ms, 'status': status, 'timestamp': datetime.now(UTC)})
...
class RealMockWebSocketBridge:
    ...
    async def notify_tool_executing(self, run_id: str, agent_name: str, tool_name: str, parameters: Dict=None) -> bool:
        """Send tool executing notification."""
        if self.should_fail:
            raise ConnectionError('WebSocket notification failed')
        notification = {'type': 'tool_executing', 'run_id': run_id, 'agent_name': agent_name, 'tool_name': tool_name, 'parameters': parameters, 'timestamp': datetime.now(UTC).isoformat()}
        self.notifications_sent.append(notification)
        return True

    async def notify_tool_completed(self, run_id: str, agent_name: str, tool_name: str, result: Dict, execution_time_ms: float) -> bool:
        """Send tool completed notification."""
        if self.should_fail:
            raise ConnectionError('WebSocket notification failed')
        notification = {'type': 'tool_completed', 'run_id': run_id, 'agent_name': agent_name, 'tool_name': tool_name, 'result': result, 'execution_time_ms': execution_time_ms, 'timestamp': datetime.now(UTC).isoformat()}
        self.notifications_sent.append(notification)
        return True
...
class RealMockAgentExecutionContext:
    ...
    def __init__(self, user_id: str='test_user', thread_id: str='test_thread', run_id: str='test_run', agent_name: str='TestAgent'):
...
def enterprise_user():
    ...
    return RealMockUser(user_id='enterprise_user', plan_tier='enterprise', is_developer=True, roles=['admin', 'developer'])
...
class TestUnifiedToolExecutionEngineBasics(AsyncBaseTestCase):
    ...
    def test_unified_tool_execution_engine_initialization(self):
        ...
        assert engine.websocket_notifier is None
    ...
    def test_unified_tool_execution_engine_with_websocket_bridge(self, mock_websocket_bridge):
        ...
        assert engine.websocket_notifier is mock_websocket_bridge
...
class TestUnifiedToolExecutionEngineExecution(AsyncBaseTestCase):
    ...
    async def test_execute_tool_timeout_handling(self, mock_websocket_bridge, mock_execution_context):
        ...
        with patch.object(UnifiedToolExecutionEngine, 'default_timeout', 0.1):
            engine = UnifiedToolExecutionEngine(websocket_bridge=mock_websocket_bridge)
            slow_tool = RealMockTool('slow_tool', execution_time=0.5)
...
class TestUnifiedToolExecutionEngineWebSocketIntegration(AsyncBaseTestCase):
    ...
    async def test_websocket_event_parameter_summary_creation(self, mock_websocket_bridge, mock_execution_context):
        ...
        engine = UnifiedToolExecutionEngine(websocket_bridge=mock_websocket_bridge)
        complex_params = {'query': 'SELECT * FROM users WHERE active = true', 'table_name': 'users', 'limit': 100, 'filters': [{'field': 'status', 'value': 'active'}]}
    ...
    async def test_websocket_progress_updates(self, mock_websocket_bridge, mock_execution_context):
        ...
        await engine.send_progress_update(mock_execution_context, 'long_tool', 45.5, 'Processing data batch 2 of 4')
...
class TestUnifiedToolExecutionEngineSecurityValidation(AsyncBaseTestCase):
    ...
    async def test_execute_with_permissions_success(self, mock_user, mock_permission_service):
        ...
        engine = UnifiedToolExecutionEngine(permission_service=mock_permission_service)
        tool = UnifiedTool(name='authorized_tool', description='Tool for testing authorization', handler=lambda args, user: {'result': 'authorized'})
    ...
    async def test_execute_with_permissions_denied(self, mock_user, restrictive_permission_service):
        ...
        tool = UnifiedTool(name='restricted_tool', description='Tool with restricted access', handler=lambda args, user: {'result': 'should_not_execute'})
    ...
    async def test_tool_validation_missing_handler(self, mock_user, mock_permission_service):
        ...
        engine = UnifiedToolExecutionEngine(permission_service=mock_permission_service)
        tool = UnifiedTool(name='no_handler_tool', description='Tool without handler', handler=None)
        result = await engine.execute_with_permissions(tool, {}, mock_user)
    ...
    async def test_input_schema_validation(self, mock_user, mock_permission_service):
        ...
        engine = UnifiedToolExecutionEngine(permission_service=mock_permission_service)
        tool = UnifiedTool(name='schema_tool', description='Tool with input schema', handler=lambda args, user: {'result': 'valid'}, input_schema={'type': 'object', 'properties': {'required_field': {'type': 'string'}}, 'required': ['required_field']})
        result = await engine.execute_with_permissions(tool, {}, mock_user)
        assert result.status == 'error'
...
class TestUnifiedToolExecutionEngineMetricsAndPerformance(AsyncBaseTestCase):
    ...
    async def test_concurrent_execution_tracking(self, mock_websocket_bridge):
        ...
        engine = UnifiedToolExecutionEngine(websocket_bridge=mock_websocket_bridge)
        contexts = [RealMockAgentExecutionContext(f'user_{i}', f'thread_{i}', f'run_{i}') for i in range(3)]
        tools = [RealMockTool(f'concurrent_tool_{i}', execution_time=0.1) for i in range(3)]
        tasks = []
...
class TestUnifiedToolExecutionEngineRecoveryMechanisms(AsyncBaseTestCase):
    ...
    async def test_force_cleanup_user_executions(self, mock_websocket_bridge, mock_execution_context):
        ...
        engine._active_executions[execution_id] = {'tool_name': 'stuck_tool', 'start_time': time.time() - engine.default_timeout * 4, 'user_id': user_id, 'context': mock_execution_context}
    ...
    async def test_emergency_shutdown_all_executions(self, mock_websocket_bridge):
        ...
        for i in range(3):
            ...
            engine._active_executions[execution_id] = {'tool_name': f'tool_{i}', 'start_time': time.time(), 'user_id': f'user_{i}'}
...
class TestServicesToolExecutionEngine(AsyncBaseTestCase):
    ...
    async def test_execute_unified_tool_with_permissions(self, mock_permission_service, mock_user):
        ...
        engine = ServicesToolExecutionEngine(mock_permission_service)
        tool = UnifiedTool(name='services_test_tool', description='Tool for services testing', handler=lambda args, user: {'result': 'services_success'})
    ...
    def test_convert_execution_result_to_response(self, mock_permission_service):
        ...
        engine = ServicesToolExecutionEngine(mock_permission_service)
        success_result = ToolExecutionResult(tool_name='test_tool', user_id='test_user', status='success', result={'data': 'success_data'}, execution_time_ms=150)
        ...
        error_result = ToolExecutionResult(tool_name='error_tool', user_id='test_user', status='error', error_message='Tool execution failed', execution_time_ms=50)
...
class TestToolDispatcherEnhancement(AsyncBaseTestCase):
    ...
    async def test_enhance_tool_dispatcher_with_notifications(self, mock_websocket_bridge):
        ...
        enhanced_dispatcher = await enhance_tool_dispatcher_with_notifications(mock_dispatcher, websocket_manager=mock_websocket_bridge, enable_notifications=True)
    ...
    async def test_enhance_dispatcher_without_websocket_manager(self):
        ...
        mock_dispatcher = Mock()
        mock_dispatcher._websocket_enhanced = False
        enhanced_dispatcher = await enhance_tool_dispatcher_with_notifications(mock_dispatcher, websocket_manager=None, enable_notifications=False)
        assert enhanced_dispatcher._websocket_enhanced is True
        assert isinstance(enhanced_dispatcher.executor, UnifiedToolExecutionEngine)
...
class TestMultiUserExecutionIsolation(AsyncBaseTestCase):
    ...
    async def test_concurrent_user_tool_execution_isolation(self, mock_websocket_bridge):
        ...
        for i in range(5):
            ...
            context = RealMockAgentExecutionContext(user_id=f'user_{i}', thread_id=f'thread_{i}', run_id=f'run_{i}', agent_name=f'Agent_{i}')
        ...
        for result in results:
            assert result.status == ToolStatus.SUCCESS
        assert len(mock_websocket_bridge.notifications_sent) == 10
        user_events = {}
    ...
    async def test_user_context_prevents_data_leakage(self, mock_websocket_bridge):
        ...
        task1 = engine.execute_tool_with_input(ToolInput(tool_name='sensitive_tool_1', parameters={'secret': 'user1_secret'}), tool1, {'context': user1_context})
        task2 = engine.execute_tool_with_input(ToolInput(tool_name='sensitive_tool_2', parameters={'secret': 'user2_secret'}), tool2, {'context': user2_context})
    ...
    async def test_performance_under_concurrent_load(self, mock_websocket_bridge):
        ...
        for i in range(num_users):
            ...
            tool = RealMockTool(f'load_tool_{i}', execution_time=0.1)
...
class TestToolExecutionEngineIntegration(AsyncBaseTestCase):
    ...
    async def test_end_to_end_tool_execution_flow(self, mock_websocket_bridge, mock_permission_service, mock_user):
        ...
        tool = UnifiedTool(name='e2e_test_tool', description='End-to-end test tool', handler=lambda args, user: {'result': f'E2E success for {user.id}', 'processed_args': args}, input_schema={'type': 'object', 'properties': {'operation': {'type': 'string'}}, 'required': ['operation']})
        result = await services_engine.execute_unified_tool(tool, {'operation': 'test_e2e'}, mock_user)
        assert isinstance(result, ToolExecutionResult)
        assert result.status == 'success'
        assert result.user_id == mock_user.id
    ...
    async def test_dispatcher_to_unified_engine_integration(self, mock_websocket_bridge):
        ...
        result = await dispatcher.execute_with_state(tool, 'integration_tool', {'integration': 'test'}, state, 'integration_run')
        assert hasattr(result, 'success')
        assert result.success is True
        assert result.result is not None
...
class TestToolExecutionEngineRegressionTests(AsyncBaseTestCase):
    ...
    async def test_permission_service_integration_robustness(self, mock_user):
        ...
        engine = UnifiedToolExecutionEngine()
        tool = UnifiedTool(name='permission_test_tool', description='Test tool for permission edge cases', handler=lambda args, user: {'result': 'success'})
        result = await engine.execute_with_permissions(tool, {}, mock_user)
        assert result.status == 'success'
    ...
    async def test_execution_metrics_consistency_under_errors(self, mock_websocket_bridge, mock_execution_context):
        ...
        engine = UnifiedToolExecutionEngine(websocket_bridge=mock_websocket_bridge)
        initial_metrics = engine.get_execution_metrics()
        tools = [RealMockTool('success_1', should_fail=False), RealMockTool('fail_1', should_fail=True, fail_mode='runtime_error'), RealMockTool('success_2', should_fail=False), RealMockTool('fail_2', should_fail=True, fail_mode='permission_denied')]
        results = []
        for tool in tools:
        ...
        expected_successful = initial_metrics['successful_executions'] + 2
        expected_failed = initial_metrics['failed_executions'] + 2
