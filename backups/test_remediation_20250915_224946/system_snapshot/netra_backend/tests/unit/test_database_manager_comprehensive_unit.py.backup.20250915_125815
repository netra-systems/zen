...
class TestDatabaseManagerConfiguration(BaseTestCase):
    ...
    async def test_database_manager_initialization_creates_engines(self):
        ...
        with patch('netra_backend.app.core.config.get_config') as mock_config, patch.object(DatabaseManager, '_get_database_url', return_value=test_url), patch('netra_backend.app.db.database_manager.create_async_engine') as mock_create_engine:
    ...
    def test_database_manager_config_attribute_fallbacks(self):
        ...
        with patch('netra_backend.app.core.config.get_config') as mock_config, patch.object(DatabaseManager, '_get_database_url', return_value=test_url), patch('netra_backend.app.db.database_manager.create_async_engine') as mock_create_engine:
            ...
            mock_config_obj.database_echo = None
    ...
    async def test_database_manager_initialization_error_handling(self):
        ...
        """
        with patch('netra_backend.app.core.config.get_config') as mock_config, patch.object(DatabaseManager, '_get_database_url', side_effect=ValueError('Invalid database URL')):
            mock_config.return_value.database_echo = False
            mock_config.return_value.database_pool_size = 5
            mock_config.return_value.database_max_overflow = 10
            db_manager = DatabaseManager()
...
class TestDatabaseManagerConnectionPool(BaseTestCase):
    ...
    def test_get_engine_with_auto_initialization(self):
        ...
        """
        test_url = 'postgresql+asyncpg://test:test@localhost:5432/test_db'
        with patch('netra_backend.app.core.config.get_config') as mock_config, patch.object(DatabaseManager, '_get_database_url', return_value=test_url), patch('netra_backend.app.db.database_manager.create_async_engine') as mock_create_engine, patch('asyncio.create_task') as mock_create_task, patch('time.sleep') as mock_sleep:
            mock_config.return_value.database_echo = False
            mock_config.return_value.database_pool_size = 5
            mock_config.return_value.database_max_overflow = 10
            mock_engine = Mock(spec=AsyncEngine)
...
class TestDatabaseURLBuilderIntegration(BaseTestCase):
    ...
    def test_get_database_url_creates_url_builder(self):
        ...
        """
        mock_env_dict = {'POSTGRES_HOST': 'localhost', 'POSTGRES_PORT': '5432', 'POSTGRES_USER': 'test_user', 'POSTGRES_PASSWORD': 'test_pass', 'POSTGRES_DB': 'test_db', 'ENVIRONMENT': 'test'}
        with patch('netra_backend.app.core.config.get_config') as mock_config, patch('shared.isolated_environment.get_env') as mock_get_env, patch('shared.database_url_builder.DatabaseURLBuilder') as mock_url_builder_class:
            mock_env = Mock()
            ...
            mock_url_builder.format_url_for_driver.assert_called_once_with('postgresql://test:test@localhost:5432/test_db', 'asyncpg')
    ...
    def test_get_migration_url_sync_format(self):
        ...
        """
        mock_env_dict = {'POSTGRES_HOST': 'localhost', 'POSTGRES_PORT': '5432', 'POSTGRES_USER': 'migration_user', 'POSTGRES_PASSWORD': 'migration_pass', 'POSTGRES_DB': 'migration_db', 'ENVIRONMENT': 'development'}
        with patch('shared.isolated_environment.get_env') as mock_get_env, patch('shared.database_url_builder.DatabaseURLBuilder') as mock_url_builder_class:
            mock_env = Mock()
            mock_env.as_dict.return_value = mock_env_dict
            mock_get_env.return_value = mock_env
            mock_url_builder = Mock()
            ...
            assert 'postgresql://' in result_url
            assert '+asyncpg' not in result_url
    ...
    def test_create_application_engine_for_health_checks(self):
        ...
        """
        mock_env_dict = {'POSTGRES_HOST': 'localhost', 'POSTGRES_PORT': '5432', 'POSTGRES_USER': 'health_user', 'POSTGRES_PASSWORD': 'health_pass', 'POSTGRES_DB': 'health_db', 'ENVIRONMENT': 'staging'}
        with patch('netra_backend.app.core.config.get_config') as mock_config, patch('shared.isolated_environment.get_env') as mock_get_env, patch('shared.database_url_builder.DatabaseURLBuilder') as mock_url_builder_class, patch('netra_backend.app.db.database_manager.create_async_engine') as mock_create_engine:
            mock_config.return_value.database_url = None
            mock_env = Mock()
            mock_env.as_dict.return_value = mock_env_dict
            mock_get_env.return_value = mock_env
...
class TestDatabaseManagerSessionManagement(BaseTestCase):
    ...
    async def test_get_session_auto_initialization(self):
        ...
        """
        test_url = 'postgresql+asyncpg://test:test@localhost:5432/test_db'
        with patch('netra_backend.app.core.config.get_config') as mock_config, patch.object(DatabaseManager, '_get_database_url', return_value=test_url), patch('netra_backend.app.db.database_manager.create_async_engine') as mock_create_engine, patch('netra_backend.app.db.database_manager.AsyncSession') as mock_session_class:
            mock_config.return_value.database_echo = False
            mock_config.return_value.database_pool_size = 5
            mock_config.return_value.database_max_overflow = 10
            mock_engine = Mock(spec=AsyncEngine)
