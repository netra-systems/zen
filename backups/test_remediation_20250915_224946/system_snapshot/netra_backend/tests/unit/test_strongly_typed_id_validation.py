...
class StronglyTypedIDValidationTests(BaseIntegrationTest):
    ...
    def test_to_string_dict_conversion(self):
        ...
        """
        typed_data = {'user_id': UserID(self.valid_user_id), 'thread_id': ThreadID(self.valid_thread_id), 'run_id': RunID(self.valid_run_id), 'request_id': RequestID(self.valid_request_id), 'websocket_id': WebSocketID(self.valid_websocket_id), 'session_id': SessionID(self.valid_session_id), 'status': 'active'}
    ...
    def test_from_string_dict_conversion(self):
        ...
        """
        string_data = {'user_id': self.valid_user_id, 'thread_id': self.valid_thread_id, 'run_id': self.valid_run_id, 'request_id': self.valid_request_id, 'websocket_id': self.valid_websocket_id, 'status': 'active'}
        expected_types = {'user_id': UserID, 'thread_id': ThreadID, 'run_id': RunID, 'request_id': RequestID, 'websocket_id': WebSocketID}
    ...
    def test_conversion_round_trip_preservation(self):
        ...
        """
        original_typed_data = {'user_id': UserID(self.valid_user_id), 'thread_id': ThreadID(self.valid_thread_id), 'run_id': RunID(self.valid_run_id)}
        ...
        expected_types = {'user_id': UserID, 'thread_id': ThreadID, 'run_id': RunID}
    ...
    def test_auth_validation_result_with_valid_data(self):
        ...
        """
        result = AuthValidationResult(valid=True, user_id=self.valid_user_id, email='test@example.com', permissions=['read', 'write'], expires_at=datetime.now(timezone.utc))
    ...
    def test_auth_validation_result_with_string_user_id_conversion(self):
        ...
        """
        result = AuthValidationResult(valid=True, user_id=self.valid_user_id)
        assert isinstance(result.user_id, str), f'Pydantic should create string-based UserID, got {type(result.user_id)}'
    ...
    def test_auth_validation_result_invalid_data(self):
        ...
        """
        result = AuthValidationResult(valid=False, user_id=None, error_message='Invalid token')
    ...
    def test_auth_validation_result_with_typed_user_id_input(self):
        ...
        result = AuthValidationResult(valid=True, user_id=typed_user_id)
    ...
    def test_session_validation_result_with_valid_data(self):
        ...
        """
        result = SessionValidationResult(valid=True, user_id=self.valid_user_id, session_id=self.valid_session_id, permissions=['admin'], expires_at=datetime.now(timezone.utc))
        assert result.valid is True
        assert isinstance(result.user_id, str), f'UserID should be string at runtime, got {type(result.user_id)}'
    ...
    def test_session_validation_result_auto_conversion(self):
        ...
        """
        result = SessionValidationResult(valid=True, user_id=self.valid_user_id, session_id=self.valid_session_id)
        assert isinstance(result.user_id, str), f'UserID should be string at runtime, got {type(result.user_id)}'
        assert isinstance(result.session_id, str), f'SessionID should be string at runtime, got {type(result.session_id)}'
        assert result.user_id == self.valid_user_id, f'UserID value should be preserved during conversion'
    ...
    def test_token_response_with_valid_data(self):
        ...
        result = TokenResponse(access_token=self.valid_token, refresh_token=refresh_token, token_type='bearer', expires_in=3600, user_id=self.valid_user_id)
    ...
    def test_websocket_message_with_valid_data(self):
        ...
        NOTE: Pydantic field validators convert strings to typed IDs (strings at runtime).
        """
        result = WebSocketMessage(event_type=WebSocketEventType.AGENT_STARTED, user_id=self.valid_user_id, thread_id=self.valid_thread_id, request_id=self.valid_request_id, data={'message': 'Agent started processing'})
    ...
    def test_websocket_message_auto_conversion(self):
        ...
        NOTE: Pydantic field validators handle conversion - test validation behavior.
        """
        result = WebSocketMessage(event_type=WebSocketEventType.TOOL_EXECUTING, user_id=self.valid_user_id, thread_id=self.valid_thread_id, request_id=self.valid_request_id)
        assert isinstance(result.user_id, str), f'UserID should be string at runtime, got {type(result.user_id)}'
        assert isinstance(result.thread_id, str), f'ThreadID should be string at runtime, got {type(result.thread_id)}'
        assert isinstance(result.request_id, str), f'RequestID should be string at runtime, got {type(result.request_id)}'
        assert result.user_id == self.valid_user_id, f'UserID value should be preserved'
    ...
    def test_websocket_message_all_event_types(self):
        ...
        for event_type in critical_events:
            result = WebSocketMessage(event_type=event_type, user_id=self.valid_user_id, thread_id=self.valid_thread_id, request_id=self.valid_request_id)
    ...
    def test_agent_execution_context_with_valid_data(self):
        ...
        result = AgentExecutionContext(execution_id=execution_id, agent_id=agent_id, user_id=self.valid_user_id, thread_id=self.valid_thread_id, run_id=self.valid_run_id, request_id=self.valid_request_id, websocket_id=self.valid_websocket_id, state=ExecutionContextState.ACTIVE, metadata={'priority': 'high'})
    ...
    def test_agent_execution_context_minimal_data(self):
        ...
        execution_id = str(uuid.uuid4())
        agent_id = str(uuid.uuid4())
        result = AgentExecutionContext(execution_id=execution_id, agent_id=agent_id, user_id=self.valid_user_id, thread_id=self.valid_thread_id, run_id=self.valid_run_id, request_id=self.valid_request_id)
        assert isinstance(result.execution_id, str), f'ExecutionID should be string at runtime, got {type(result.execution_id)}'
        assert isinstance(result.agent_id, str), f'AgentID should be string at runtime, got {type(result.agent_id)}'
        assert isinstance(result.user_id, str), f'UserID should be string at runtime, got {type(result.user_id)}'
        assert isinstance(result.thread_id, str), f'ThreadID should be string at runtime, got {type(result.thread_id)}'
        assert isinstance(result.run_id, str), f'RunID should be string at runtime, got {type(result.run_id)}'
        assert isinstance(result.request_id, str), f'RequestID should be string at runtime, got {type(result.request_id)}'
        assert result.websocket_id is None
        assert result.state == ExecutionContextState.CREATED
    ...
    def test_websocket_connection_info_validation(self):
        ...
        user_id = UserID(self.valid_user_id)
        connection_info = WebSocketConnectionInfo(websocket_id=websocket_id, user_id=user_id, connection_state=ConnectionState.CONNECTED, connected_at=datetime.now(timezone.utc), last_ping=datetime.now(timezone.utc), message_count=5)
    ...
    def test_database_connection_info_validation(self):
        ...
        user_id = UserID(self.valid_user_id)
        db_info = DatabaseConnectionInfo(session_id=session_id, user_id=user_id, created_at=datetime.now(timezone.utc), last_activity=datetime.now(timezone.utc), connection_pool='primary', isolation_level='READ_COMMITTED')
    ...
    def test_execution_metrics_validation(self):
        ...
        metrics = ExecutionMetrics(execution_id=execution_id, start_time=datetime.now(timezone.utc), end_time=datetime.now(timezone.utc), tool_calls=3, websocket_events=5, errors=0, state=ExecutionContextState.COMPLETED)
    ...
    def test_ensure_functions_validate_and_convert_properly(self):
        ...
        """
        test_cases = [(ensure_user_id, 'user_123', 'user_123'), (ensure_thread_id, 'thread_456', 'thread_456'), (ensure_run_id, 'run_789', 'run_789'), (ensure_request_id, 'req_abc', 'req_abc'), (ensure_websocket_id, 'ws_def', 'ws_def')]
    ...
    def test_websocket_message_structure_validates_correctly(self):
        ...
        """
        message = WebSocketMessage(event_type=WebSocketEventType.AGENT_STARTED, user_id=self.valid_user_id, thread_id=self.valid_thread_id, request_id=self.valid_request_id)
        ...
        try:
            empty_user_msg = WebSocketMessage(event_type=WebSocketEventType.AGENT_STARTED, user_id='', thread_id=self.valid_thread_id, request_id=self.valid_request_id)
    ...
    def test_auth_validation_result_handles_user_isolation(self):
        ...
        user1_id = 'user_001'
        user2_id = 'user_002'
        auth1 = AuthValidationResult(valid=True, user_id=user1_id, email='user1@example.com', permissions=['read', 'write'])
        auth2 = AuthValidationResult(valid=False, user_id=None, error_message='Invalid credentials')
        ...
        assert len(auth2.permissions) == 0
    ...
    def test_agent_execution_context_ensures_multi_user_isolation(self):
        ...
        agent_id = str(uuid.uuid4())
        context1 = AgentExecutionContext(execution_id=execution_id1, agent_id=agent_id, user_id='user_001', thread_id='thread_001', run_id='run_001', request_id='req_001', state=ExecutionContextState.ACTIVE, metadata={'user_data': 'sensitive_user1_data'})
        context2 = AgentExecutionContext(execution_id=execution_id2, agent_id=agent_id, user_id='user_002', thread_id='thread_002', run_id='run_002', request_id='req_002', state=ExecutionContextState.ACTIVE, metadata={'user_data': 'sensitive_user2_data'})
