...
from netra_backend.app.core.registry.universal_registry import UniversalRegistry, RegistryItem
...
class ToolRegistryDynamicDiscoveryTests(SSotAsyncTestCase):
    ...
    def setUp(self):
        ...
        super().setUp()
        self.user1_context = UserExecutionContext(user_id='registry_user_001', run_id=f'run_{int(time.time() * 1000)}', thread_id='thread_registry_001', session_id='session_registry_001', metadata={'plan_tier': 'early', 'roles': ['user']})
        self.user2_context = UserExecutionContext(user_id='registry_user_002', run_id=f'run_{int(time.time() * 1000) + 1}', thread_id='thread_registry_002', session_id='session_registry_002', metadata={'plan_tier': 'mid', 'roles': ['user', 'analyst']})
    ...
    def test_tool_registration_and_retrieval(self):
        ...
        self.tool_registry.register('analytics', self.analytics_tool, description='Advanced analytics tool', tags=['analytics', 'reporting'])
        self.tool_registry.register('data_processing', self.data_tool, description='Data processing tool', tags=['data', 'processing'])
    ...
    def test_tool_discovery_by_category(self):
        ...
        self.tool_registry.register('advanced_analytics', self.analytics_tool, tags=['analytics', 'advanced', 'ml'])
        self.tool_registry.register('basic_analytics', self.basic_analytics, tags=['analytics', 'basic', 'reporting'])
        self.tool_registry.register('data_processor', self.data_tool, tags=['data', 'processing', 'etl'])
    ...
    def test_available_tools_listing(self):
        ...
        tools_to_register = {'tool_1': self.analytics_tool, 'tool_2': self.data_tool, 'tool_3': self.basic_analytics}
    ...
    def test_tool_factory_registration_and_creation(self):
        ...
        self.tool_registry.register_factory('user_tool', factory, description='User-specific tool factory', tags=['user', 'contextual'])
    ...
    async def test_concurrent_factory_instance_creation(self):
        ...
        contexts = []
        for i in range(10):
            context = UserExecutionContext(user_id=f'concurrent_user_{i}', run_id=f'concurrent_run_{i}', thread_id=f'concurrent_thread_{i}', session_id=f'concurrent_session_{i}')
    ...
    def test_thread_safe_registration(self):
        ...
        def register_tools(thread_id):
            for i in range(tools_per_thread):
                ...
                try:
                    self.tool_registry.register(tool_key, tool, tags=[f'thread_{thread_id}', 'concurrent'])
    ...
    def test_concurrent_access_patterns(self):
        ...
        for i in range(5):
            self.tool_registry.register(f'base_tool_{i}', MockAnalyticsTool(f'base_{i}'), tags=['base', 'concurrent'])
