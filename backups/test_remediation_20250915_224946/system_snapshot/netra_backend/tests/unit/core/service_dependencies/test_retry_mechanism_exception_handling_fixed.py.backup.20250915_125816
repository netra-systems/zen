...
class TestRetryMechanismExceptionHandlingFixed:
    ...
    def test_logging_behavior_for_fallback_cases(self):
        ...
        fallback_logged = any(('Failed to construct' in call and 'Using RuntimeError wrapper' in call for call in debug_calls))
...
class TestExceptionConstructorSafety:
    ...
    def test_safe_construction_attempt_patterns(self):
        ...
        simple_exceptions = [ValueError('test'), TypeError('test'), RuntimeError('test'), KeyError('test'), AttributeError('test')]
    ...
    def test_complex_exception_construction_detection(self):
        ...
        complex_exceptions = [OSError(2, 'No such file', '/path'), UnicodeDecodeError('utf-8', b'\xff\xfe', 0, 2, 'invalid'), PermissionError(13, 'Permission denied', '/protected')]
    ...
    def test_runtime_error_fallback_construction(self):
        ...
        test_messages = ['Simple message', 'Message with (parentheses) and special chars: @#$%', 'Multi-line\nmessage\nwith\nbreaks', 'Unicode message with [U+00E9]mojis: [U+1F680] FIRE:  WARNING: [U+FE0F]', '', 'Very ' + 'long ' * 100 + 'message']
