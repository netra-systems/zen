...
def create_basic_agent_result():
    ...
    return AgentResult(success=True, output={'test': 'data'}, execution_time_ms=100.0)

def create_agent_completed_instance(result, run_id='test-run-123'):
    """Create AgentCompleted instance for testing."""
    return AgentCompleted(run_id=run_id, result=result, execution_time_ms=100.0)
...
def create_triage_metadata():
    ...
    return TriageMetadata(triage_duration_ms=50, cache_hit=False, fallback_used=False, retry_count=0)

def create_triage_result(metadata, category='optimization', confidence=0.95):
    """Create TriageResult for testing."""
    return TriageResult(category=category, confidence_score=confidence, metadata=metadata)

def create_deep_agent_state(triage_result, request='Optimize my workload'):
    """Create DeepAgentState for testing."""
    return DeepAgentState(user_request=request, triage_result=triage_result)
...
def create_complete_test_metadata():
    ...
    return TriageMetadata(triage_duration_ms=75, cache_hit=True, fallback_used=False, retry_count=1)

def create_complete_agent_result(final_state):
    """Create complete AgentResult with final state."""
    return AgentResult(success=True, output=final_state.to_dict(), execution_time_ms=200.0, metrics={'tokens': 150, 'steps': 5})
...
def create_complete_final_state():
    ...
    return DeepAgentState(user_request='Analyze performance', triage_result=triage_result, step_count=5)
...
def create_completed_with_final_state(final_state):
    ...
    return AgentCompleted(run_id='test-456', result=result, execution_time_ms=200.0, final_state=final_state)
...
def test_model_handles_circular_dependencies():
    ...
    completed = AgentCompleted(run_id='rebuild-test', result=result, execution_time_ms=10.0)
