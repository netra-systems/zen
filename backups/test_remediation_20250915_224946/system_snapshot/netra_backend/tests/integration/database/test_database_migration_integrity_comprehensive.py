...
class DatabaseMigrationIntegrityComprehensiveTests(SSotBaseTestCase):
    """
    ...
    async def test_schema_evolution_without_data_loss(self):
        ...
        try:
            ...
            for i in range(5):
                ...
                user_data = {'email': user_email, 'name': f'Test User {i}', 'subscription_tier': 'mid', 'credits_remaining': 1000 + i * 100, 'created_at': datetime.now(timezone.utc)}
                user = await migrator.create_test_user(user_data)
                test_users.append(user)
                for j in range(2):
                    thread_data = {'user_id': user.id, 'title': f'Thread {j} for User {i}', 'created_at': datetime.now(timezone.utc) - timedelta(hours=j), 'metadata': {'test_data': True, 'user_sequence': i, 'thread_sequence': j}}
                    thread = await migrator.create_test_thread(thread_data)
                    test_threads.append(thread)
                    for k in range(3):
                        message_data = {'thread_id': thread.id, 'user_id': user.id, 'content': f'Message {k} in thread {j} from user {i}', 'message_type': 'user' if k % 2 == 0 else 'agent', 'created_at': datetime.now(timezone.utc) - timedelta(minutes=k), 'metadata': {'test_data': True, 'message_sequence': k}}
                        message = await migrator.create_test_message(message_data)
                        test_messages.append(message)
            pre_migration_checksums = await migrator.calculate_data_checksums(test_prefix=self.test_prefix)
            migration_script = '\n            ALTER TABLE users ADD COLUMN last_login_at TIMESTAMP;\n            ALTER TABLE threads ADD COLUMN priority_level INTEGER DEFAULT 1;\n            ALTER TABLE messages ADD COLUMN edited_at TIMESTAMP;\n            '
            migration_result = await migrator.execute_migration(script=migration_script, migration_name=f'add_columns_{self.test_prefix}', description='Add timestamp and priority columns')
            assert migration_result.success, f'Migration failed: {migration_result.error}'
            assert migration_result.affected_tables == ['users', 'threads', 'messages'], f'Unexpected affected tables: {migration_result.affected_tables}'
            post_migration_checksums = await migrator.calculate_data_checksums(test_prefix=self.test_prefix)
            ...
            for table in core_tables:
                if table in pre_migration_checksums and table in post_migration_checksums:
                    assert pre_migration_checksums[table] == post_migration_checksums[table], f'Data corruption detected in {table}: checksum mismatch'
            ...
            new_user_data = {'email': f'{self.test_prefix}_post_migration@example.com', 'name': 'Post Migration User', 'subscription_tier': 'enterprise', 'credits_remaining': 5000, 'last_login_at': datetime.now(timezone.utc), 'created_at': datetime.now(timezone.utc)}
    ...
    async def test_migration_rollback_safety_and_data_recovery(self):
        ...
        migrator = await self.setup_migration_environment()
        try:
            baseline_user = await migrator.create_test_user({'email': f'{self.test_prefix}_rollback@example.com', 'name': 'Rollback Test User', 'subscription_tier': 'early', 'credits_remaining': 500, 'created_at': datetime.now(timezone.utc)})
            baseline_thread = await migrator.create_test_thread({'user_id': baseline_user.id, 'title': 'Rollback Test Thread', 'created_at': datetime.now(timezone.utc)})
            pre_migration_state = await migrator.capture_database_state(test_prefix=self.test_prefix)
            ...
            forward_result = await migrator.execute_migration(script=forward_migration, migration_name=f'rollback_test_forward_{self.test_prefix}', description='Migration to be rolled back', rollback_script=rollback_migration)
            ...
            assert hasattr(user_with_new_column, 'test_rollback_column'), 'Forward migration column not applied'
            assert user_with_new_column.test_rollback_column == 'test_value', 'Forward migration data not applied'
            rollback_result = await migrator.execute_rollback(migration_name=f'rollback_test_forward_{self.test_prefix}')
            assert rollback_result.success, f'Rollback failed: {rollback_result.error}'
            post_rollback_state = await migrator.capture_database_state(test_prefix=self.test_prefix)
            ...
            assert rolled_back_user.subscription_tier == baseline_user.subscription_tier, 'User subscription corrupted in rollback'
            assert not hasattr(rolled_back_user, 'test_rollback_column'), 'Rollback did not remove added column'
            rolled_back_thread = await migrator.get_thread_by_id(baseline_thread.id)
            assert not hasattr(rolled_back_thread, 'test_priority'), 'Rollback did not remove thread column'
            ...
            await migrator.execute_migration(script=multi_migration_1, migration_name=f'multi_rollback_1_{self.test_prefix}', rollback_script='ALTER TABLE users DROP COLUMN rollback_test_1;')
            await migrator.execute_migration(script=multi_migration_2, migration_name=f'multi_rollback_2_{self.test_prefix}', rollback_script='ALTER TABLE users DROP COLUMN rollback_test_2;')
            rollback_2_result = await migrator.execute_rollback(migration_name=f'multi_rollback_2_{self.test_prefix}')
            assert rollback_2_result.success, 'Second rollback failed'
            rollback_1_result = await migrator.execute_rollback(migration_name=f'multi_rollback_1_{self.test_prefix}')
