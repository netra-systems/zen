...
class ObservableAgent(BaseAgent):
    ...
    async def execute(self, state: DeepAgentState, run_id: str, stream_updates: bool=True) -> Dict[str, Any]:
        ...
        await self.websocket_bridge.notify_agent_started(run_id, self.name, {'monitoring_enabled': True, 'observability_level': 'comprehensive', 'tracing_id': f'trace_{run_id}', 'metrics_collection': True})
        self.monitoring_metrics.append({'metric': 'execution_started', 'timestamp': execution_start, 'value': 1, 'labels': {'agent': self.name, 'run_id': run_id}})
        await self.websocket_bridge.notify_agent_thinking(run_id, self.name, 'Processing with full observability enabled...', step_number=1, progress_percentage=50, monitoring_data={'cpu_usage': 45, 'memory_mb': 128, 'latency_ms': 50})
        await self.websocket_bridge.notify_tool_executing(run_id, self.name, 'observable_tool', {'monitoring': True, 'trace_context': f'trace_{run_id}', 'metrics_enabled': True})
        tool_metrics = {'execution_time_ms': 85, 'resource_usage': {'cpu': 35, 'memory_mb': 64}, 'throughput': '1200_ops_sec', 'error_rate': 0.0}
        await self.websocket_bridge.notify_tool_completed(run_id, self.name, 'observable_tool', {'result': {'success': True, 'data': 'observable_result'}, 'performance_metrics': tool_metrics, 'observability_data': {'spans': 3, 'traces': 1, 'metrics_points': 12}})
        execution_end = datetime.now(timezone.utc)
        execution_duration = (execution_end - execution_start).total_seconds()
        monitoring_summary = {'total_metrics_collected': len(self.monitoring_metrics) + 12, 'execution_duration_seconds': execution_duration, 'performance_score': 95.5, 'resource_efficiency': 'optimal', 'observability_coverage': '100%', 'sla_compliance': 'verified'}
        result = {'success': True, 'agent_name': self.name, 'monitoring_summary': monitoring_summary, 'observability_data': {'traces_generated': 1, 'metrics_collected': monitoring_summary['total_metrics_collected'], 'spans_created': 3, 'logs_generated': 8}, 'enterprise_compliance': {'audit_trail': 'complete', 'performance_monitoring': 'enabled', 'resource_tracking': 'comprehensive', 'security_logging': 'verified'}}
        await self.websocket_bridge.notify_agent_completed(run_id, self.name, result, execution_time_ms=int(execution_duration * 1000), monitoring_complete=True, observability_score=95.5)
        return result
...
class ObservabilityEventCollector:
    ...
    async def create_observability_bridge(self, user_context: UserExecutionContext):
        ...
        async def track_observability(event_type: str, run_id: str, agent_name: str, data: Any=None, **kwargs):
            event = {'event_type': event_type, 'run_id': run_id, 'agent_name': agent_name, 'data': data, 'timestamp': datetime.now(timezone.utc), 'kwargs': kwargs}
            ...
            return True
        bridge.notify_agent_started = AsyncMock(side_effect=lambda run_id, name, ctx: track_observability('agent_started', run_id, name, ctx))
        bridge.notify_agent_thinking = AsyncMock(side_effect=lambda run_id, name, thinking, **kwargs: track_observability('agent_thinking', run_id, name, {'reasoning': thinking}, **kwargs))
        bridge.notify_tool_executing = AsyncMock(side_effect=lambda run_id, name, tool, params: track_observability('tool_executing', run_id, name, {'tool_name': tool, 'parameters': params}))
        bridge.notify_tool_completed = AsyncMock(side_effect=lambda run_id, name, tool, result: track_observability('tool_completed', run_id, name, {'tool_name': tool, **result}))
        bridge.notify_agent_completed = AsyncMock(side_effect=lambda run_id, name, result, **kwargs: track_observability('agent_completed', run_id, name, result, **kwargs))
        return bridge
...
class WebSocketAgentExecutionMonitoringObservabilityTests(BaseIntegrationTest):
    ...
    async def monitoring_user_context(self):
        return UserExecutionContext(user_id=f'monitor_user_{uuid.uuid4().hex[:8]}', thread_id=f'monitor_thread_{uuid.uuid4().hex[:8]}', run_id=f'monitor_run_{uuid.uuid4().hex[:8]}', request_id=f'monitor_req_{uuid.uuid4().hex[:8]}', metadata={'monitoring_required': True, 'enterprise_compliance': True})

    @pytest.mark.integration
    @pytest.mark.real_services
    async def test_comprehensive_agent_monitoring_and_observability_through_websocket(self, monitoring_user_context, mock_llm_manager):
        ...
        execution_engine = ExecutionEngine._init_from_factory(registry=registry, websocket_bridge=websocket_bridge, user_context=monitoring_user_context)
        exec_context = AgentExecutionContext(user_id=monitoring_user_context.user_id, thread_id=monitoring_user_context.thread_id, run_id=monitoring_user_context.run_id, request_id=monitoring_user_context.request_id, agent_name='monitoring_agent', step=PipelineStep.PROCESSING, execution_timestamp=datetime.now(timezone.utc), pipeline_step_num=1)
        agent_state = DeepAgentState(user_request={'comprehensive_monitoring': True, 'observability_required': True}, user_id=monitoring_user_context.user_id, chat_thread_id=monitoring_user_context.thread_id, run_id=monitoring_user_context.run_id)
        ...
        self.logger.info(f" PASS:  Monitoring and observability test PASSED - Events: {analysis['total_monitoring_events']}, Metrics: {analysis['performance_metrics_collected']}, Performance score: {monitoring_summary.get('performance_score', 0)}")
