...
class StatePersistenceAgent(BaseAgent):
    ...
    async def execute(self, state: DeepAgentState, run_id: str, stream_updates: bool=True) -> Dict[str, Any]:
        ...
        self.execution_state = {'phase': 'initialization', 'progress': 0, 'completed_operations': [], 'persistent_data': {'user_id': getattr(state, 'user_id', None)}, 'checkpoint_data': {}}
        try:
            await self.websocket_bridge.notify_agent_started(run_id, self.name, {'state_persistence_enabled': True, 'checkpointing': True})
            ...
            result = {'success': True, 'agent_name': self.name, 'state_persistence': {'checkpoints_created': len([op for op in self.execution_state['completed_operations'] if 'checkpoint' in op]), 'interruptions_handled': len(self.connection_interruptions), 'state_recovered': True, 'data_integrity': 'maintained'}, 'final_state': self.execution_state, 'business_continuity': 'ensured'}
            await self.websocket_bridge.notify_agent_completed(run_id, self.name, result, execution_time_ms=300)
            return result
    ...
    async def _execute_with_checkpoint(self, run_id: str, phase_name: str, target_progress: int):
        ...
        await self.websocket_bridge.notify_agent_thinking(run_id, self.name, f'Executing {phase_name} with state checkpointing...', step_number=len(self.execution_state['completed_operations']) + 1, progress_percentage=target_progress)
        checkpoint = {'timestamp': datetime.now(timezone.utc), 'phase': phase_name, 'progress': target_progress, 'data': f'checkpoint_data_{phase_name}'}
    ...
    async def _simulate_connection_interruption(self, run_id: str):
        ...
        interruption_event = {'timestamp': datetime.now(timezone.utc), 'type': 'connection_interruption', 'duration_ms': 500, 'recovery_strategy': 'state_restoration'}
        self.connection_interruptions.append(interruption_event)
        await asyncio.sleep(0.05)
        await self.websocket_bridge.notify_agent_thinking(run_id, self.name, 'Connection restored, resuming from persistent state...', step_number=len(self.execution_state['completed_operations']) + 1, progress_percentage=50, connection_recovered=True)
...
class WebSocketAgentExecutionStatePersistenceInterruptionTests(BaseIntegrationTest):
    ...
    async def persistence_user_context(self):
        return UserExecutionContext(user_id=f'persist_user_{uuid.uuid4().hex[:8]}', thread_id=f'persist_thread_{uuid.uuid4().hex[:8]}', run_id=f'persist_run_{uuid.uuid4().hex[:8]}', request_id=f'persist_req_{uuid.uuid4().hex[:8]}', metadata={'state_persistence_test': True, 'mission_critical': True})

    @pytest.mark.integration
    @pytest.mark.real_services
    async def test_agent_state_persistence_during_websocket_interruptions(self, real_services_fixture, persistence_user_context, mock_llm_manager):
        ...
        async def track_with_persistence(event_type: str, run_id: str, agent_name: str, data: Any=None, **kwargs):
            event = {'event_type': event_type, 'data': data, 'timestamp': datetime.now(timezone.utc), 'persistent': True}
            ...
            return True
        bridge.notify_agent_started = AsyncMock(side_effect=lambda run_id, agent_name, context=None: track_with_persistence('agent_started', run_id, agent_name, context))
        bridge.notify_agent_thinking = AsyncMock(side_effect=lambda run_id, agent_name, thinking, **kwargs: track_with_persistence('agent_thinking', run_id, agent_name, {'reasoning': thinking}, **kwargs))
        bridge.notify_agent_completed = AsyncMock(side_effect=lambda run_id, agent_name, result, **kwargs: track_with_persistence('agent_completed', run_id, agent_name, result, **kwargs))
        bridge.notify_agent_error = AsyncMock(side_effect=lambda run_id, agent_name, error, **kwargs: track_with_persistence('agent_error', run_id, agent_name, {'error': str(error)}, **kwargs))
        execution_engine = ExecutionEngine._init_from_factory(registry=registry, websocket_bridge=bridge, user_context=persistence_user_context)
        exec_context = AgentExecutionContext(user_id=persistence_user_context.user_id, thread_id=persistence_user_context.thread_id, run_id=persistence_user_context.run_id, request_id=persistence_user_context.request_id, agent_name='persistence_agent', step=PipelineStep.PROCESSING, execution_timestamp=datetime.now(timezone.utc), pipeline_step_num=1)
        agent_state = DeepAgentState(user_request={'mission_critical': True, 'require_persistence': True}, user_id=persistence_user_context.user_id, chat_thread_id=persistence_user_context.thread_id, run_id=persistence_user_context.run_id)
        ...
        self.logger.info(f" PASS:  State persistence test PASSED - Checkpoints: {state_persistence.get('checkpoints_created', 0)}, Interruptions: {state_persistence.get('interruptions_handled', 0)}")
