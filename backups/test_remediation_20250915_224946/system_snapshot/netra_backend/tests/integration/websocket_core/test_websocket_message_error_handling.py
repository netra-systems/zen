...
class WebSocketMessageErrorHandlingTests(SSotAsyncTestCase):
    ...
    def create_test_user_context(self):
        ...
        return UserExecutionContext(user_id='test_user_123', thread_id='test_thread_456', run_id='test_run_789')
            ...
            with patch('netra_backend.app.websocket_core.utils.safe_websocket_send') as mock_send:
                with patch('netra_backend.app.websocket_core.utils.safe_websocket_close') as mock_close:
                    ...
                    if mock_send.called:
                        ...
                        self.assertEqual(close_args[1], 1008)
                ...
                with patch('netra_backend.app.websocket_core.utils.safe_websocket_send') as mock_send:
                    with patch('netra_backend.app.websocket_core.utils.safe_websocket_close') as mock_close:
                        ...
                        if mock_send.called:
                            ...
                            self.assertTrue(mock_close.called)
                            close_args = mock_close.call_args[0]
                            self.assertEqual(close_args[1], 1011)
    ...
    async def test_error_message_serialization_for_websocket_transmission(self):
        ...
        test_cases = [('AUTH_FAILED', 'Authentication failed'), ('SERVICE_INIT_FAILED', 'Service initialization failed'), ('CLEANUP_FAILED', 'Error during cleanup'), ('INVALID_JSON', 'Invalid JSON format')]
