...
from test_framework.user_execution_context_fixtures import clean_context_registry, realistic_user_context, multi_user_contexts
...
class UserExecutionContextAuthenticationTests(BaseIntegrationTest):
    ...
    def setup_method(self):
        ...
        self.auth_users = {'user_auth_test_12345678901234567890': {'email': 'enterprise@example.com', 'subscription': 'enterprise', 'roles': ['user', 'admin'], 'permissions': ['read', 'write', 'execute', 'admin'], 'auth_provider': 'internal', 'auth_method': 'jwt'}, 'user_oauth_test_11223344556677889900': {'email': 'oauth@example.com', 'subscription': 'business', 'roles': ['user'], 'permissions': ['read', 'write'], 'auth_provider': 'google', 'auth_method': 'oauth2', 'oauth_sub': 'google-oauth2|1234567890123456789'}, 'user_free_test_99887766554433221100': {'email': 'free@example.com', 'subscription': 'free', 'roles': ['user'], 'permissions': ['read'], 'auth_provider': 'internal', 'auth_method': 'jwt'}}

    def _create_test_jwt(self, user_id: str, **claims) -> str:
        """Create a test JWT token for authentication testing."""
        now = datetime.now(timezone.utc)
        payload = {'sub': user_id, 'iat': int(now.timestamp()), 'exp': int((now + timedelta(hours=1)).timestamp()), 'iss': 'netra-test', 'aud': 'netra-api', **claims}
        return jwt.encode(payload, self.jwt_secret, algorithm=self.jwt_algorithm)

    def _validate_jwt(self, token: str) -> Dict[str, Any]:
        """Validate a JWT token and return claims."""
        try:
            return jwt.decode(token, self.jwt_secret, algorithms=[self.jwt_algorithm], audience='netra-api', issuer='netra-test')
    ...
    async def test_context_creation_with_authenticated_user(self, clean_context_registry, isolated_env):
        ...
        jwt_token = self._create_test_jwt(user_id=user_id, email=user_data['email'], subscription=user_data['subscription'], roles=user_data['roles'], permissions=user_data['permissions'])
        claims = self._validate_jwt(jwt_token)
        auth_context = UserExecutionContext(user_id=user_id, thread_id='thread_auth_test_98765432109876543210', run_id=f'run_auth_test_{int(time.time())}', agent_context={'agent_name': 'enterprise_optimizer', 'authenticated': True, 'auth_method': user_data['auth_method'], 'user_subscription': user_data['subscription'], 'user_roles': user_data['roles'], 'user_permissions': user_data['permissions']}, audit_metadata={'auth_token_issued': claims['iat'], 'auth_token_expires': claims['exp'], 'auth_provider': user_data['auth_provider'], 'auth_method': user_data['auth_method'], 'user_email': user_data['email'], 'subscription_tier': user_data['subscription']})
    ...
    async def test_context_oauth_authentication_integration(self, clean_context_registry, isolated_env):
        ...
        user_data = self.auth_users[user_id]
        oauth_token = self._create_test_jwt(user_id=user_id, email=user_data['email'], subscription=user_data['subscription'], oauth_provider='google', oauth_sub=user_data['oauth_sub'], scope='openid profile email')
        oauth_claims = self._validate_jwt(oauth_token)
        oauth_context = UserExecutionContext(user_id=user_id, thread_id='thread_oauth_test_88776655443322110099', run_id=f'run_oauth_test_{int(time.time())}', agent_context={'agent_name': 'business_optimizer', 'authenticated': True, 'auth_method': 'oauth2', 'oauth_provider': 'google', 'oauth_subject': user_data['oauth_sub'], 'user_subscription': user_data['subscription'], 'user_permissions': user_data['permissions']}, audit_metadata={'auth_token_issued': oauth_claims['iat'], 'auth_token_expires': oauth_claims['exp'], 'oauth_provider': 'google', 'oauth_subject': user_data['oauth_sub'], 'oauth_scope': 'openid profile email', 'user_email': user_data['email']})
    ...
    async def test_context_permission_enforcement_patterns(self, clean_context_registry, isolated_env):
        ...
        for user_id, user_data in self.auth_users.items():
            context = UserExecutionContext(user_id=user_id, thread_id=f'thread_perm_{user_id[-10:]}', run_id=f'run_perm_{int(time.time())}_{user_id[-4:]}', agent_context={'agent_name': 'permission_test_agent', 'user_permissions': user_data['permissions'], 'user_roles': user_data['roles'], 'subscription_tier': user_data['subscription']}, audit_metadata={'permission_level': len(user_data['permissions']), 'has_admin': 'admin' in user_data['roles'], 'subscription_tier': user_data['subscription']})
    ...
    async def test_context_session_validation_integration(self, clean_context_registry, isolated_env):
        ...
        session_context = UserExecutionContext(user_id=user_id, thread_id='thread_session_test_98765432109876543210', run_id=f'run_session_test_{int(time.time())}', agent_context={'agent_name': 'session_agent', 'session_id': session_id, 'session_valid': True, 'session_start': datetime.now(timezone.utc).isoformat(), 'session_timeout': (datetime.now(timezone.utc) + timedelta(hours=8)).isoformat(), 'authenticated': True}, audit_metadata={'session_id': session_id, 'session_created': datetime.now(timezone.utc).isoformat(), 'session_ip': '203.0.113.42', 'session_user_agent': 'NetraUI/2.1.0'})
...
class UserExecutionContextAuthorizationTests(BaseIntegrationTest):
    ...
    def setup_method(self):
        super().setup_method()
        self.role_permissions = {'admin': ['users:read', 'users:write', 'users:delete', 'agents:read', 'agents:write', 'agents:execute', 'agents:admin', 'system:read', 'system:write', 'system:admin', 'billing:read', 'billing:write', 'billing:admin'], 'user': ['agents:read', 'agents:execute', 'profile:read', 'profile:write', 'billing:read'], 'readonly': ['agents:read', 'profile:read']}
        self.subscription_features = {'free': ['basic_agents', 'basic_analytics', 'email_support'], 'business': ['basic_agents', 'advanced_agents', 'basic_analytics', 'advanced_analytics', 'email_support', 'chat_support', 'integrations:basic'], 'enterprise': ['basic_agents', 'advanced_agents', 'custom_agents', 'basic_analytics', 'advanced_analytics', 'custom_analytics', 'email_support', 'chat_support', 'phone_support', 'dedicated_support', 'integrations:basic', 'integrations:advanced', 'integrations:custom', 'sso', 'audit_logs', 'custom_policies']}
    ...
    async def test_context_role_based_authorization(self, clean_context_registry, isolated_env):
        ...
        test_scenarios = [{'user_id': 'user_admin_auth_12345678901234567890', 'roles': ['admin', 'user'], 'expected_permissions': self._get_user_permissions(['admin', 'user'])}, {'user_id': 'user_standard_auth_11223344556677889900', 'roles': ['user'], 'expected_permissions': self._get_user_permissions(['user'])}, {'user_id': 'user_readonly_auth_99887766554433221100', 'roles': ['readonly'], 'expected_permissions': self._get_user_permissions(['readonly'])}]
        for scenario in test_scenarios:
            context = UserExecutionContext(user_id=scenario['user_id'], thread_id=f"thread_role_auth_{scenario['user_id'][-10:]}", run_id=f"run_role_auth_{int(time.time())}_{scenario['user_id'][-4:]}", agent_context={'agent_name': 'authorization_test_agent', 'user_roles': scenario['roles'], 'computed_permissions': scenario['expected_permissions']}, audit_metadata={'authorization_test': True, 'roles': scenario['roles'], 'permission_count': len(scenario['expected_permissions'])})
    ...
    async def test_context_subscription_based_authorization(self, clean_context_registry, isolated_env):
        ...
        subscription_scenarios = [{'user_id': 'user_free_sub_12345678901234567890', 'subscription': 'free', 'expected_features': self._get_subscription_features('free')}, {'user_id': 'user_business_sub_11223344556677889900', 'subscription': 'business', 'expected_features': self._get_subscription_features('business')}, {'user_id': 'user_enterprise_sub_99887766554433221100', 'subscription': 'enterprise', 'expected_features': self._get_subscription_features('enterprise')}]
        for scenario in subscription_scenarios:
            context = UserExecutionContext(user_id=scenario['user_id'], thread_id=f"thread_sub_auth_{scenario['user_id'][-10:]}", run_id=f"run_sub_auth_{int(time.time())}_{scenario['user_id'][-4:]}", agent_context={'agent_name': 'subscription_test_agent', 'subscription_tier': scenario['subscription'], 'available_features': scenario['expected_features'], 'feature_access': {feature: True for feature in scenario['expected_features']}}, audit_metadata={'subscription_tier': scenario['subscription'], 'feature_count': len(scenario['expected_features']), 'authorization_test': 'subscription_based'})
    ...
    async def test_context_resource_access_authorization(self, clean_context_registry, isolated_env):
        ...
        resource_scenarios = [{'user_id': 'user_owner_12345678901234567890', 'owned_resources': ['thread_123', 'agent_config_456', 'report_789'], 'shared_resources': ['shared_dashboard_001'], 'admin_access': False}, {'user_id': 'user_collaborator_11223344556677889900', 'owned_resources': ['thread_456'], 'shared_resources': ['shared_dashboard_001', 'team_workspace_002'], 'admin_access': False}, {'user_id': 'user_admin_99887766554433221100', 'owned_resources': ['admin_panel_001'], 'shared_resources': [], 'admin_access': True}]
        for scenario in resource_scenarios:
            context = UserExecutionContext(user_id=scenario['user_id'], thread_id=f"thread_resource_auth_{scenario['user_id'][-10:]}", run_id=f"run_resource_auth_{int(time.time())}_{scenario['user_id'][-4:]}", agent_context={'agent_name': 'resource_access_agent', 'owned_resources': scenario['owned_resources'], 'shared_resources': scenario['shared_resources'], 'admin_access': scenario['admin_access'], 'resource_permissions': {'owned': ['read', 'write', 'delete'], 'shared': ['read', 'write'], 'admin': ['read', 'write', 'delete', 'admin'] if scenario['admin_access'] else []}}, audit_metadata={'resource_authorization_test': True, 'owned_resource_count': len(scenario['owned_resources']), 'shared_resource_count': len(scenario['shared_resources']), 'has_admin_access': scenario['admin_access']})
            ...
            if scenario['shared_resources']:
                ...
                assert can_access_resource(context, shared_resource, 'delete') is False
    ...
    async def test_context_child_authorization_inheritance(self, clean_context_registry, isolated_env):
        ...
        parent_context = UserExecutionContext(user_id='user_parent_auth_12345678901234567890', thread_id='thread_parent_auth_98765432109876543210', run_id=f'run_parent_auth_{int(time.time())}', agent_context={'agent_name': 'parent_auth_agent', 'user_roles': ['user', 'data_analyst'], 'user_permissions': ['agents:read', 'agents:execute', 'data:read', 'data:analyze'], 'subscription_tier': 'business', 'available_features': ['advanced_agents', 'advanced_analytics']}, audit_metadata={'authorization_level': 'business_user', 'inherited_permissions': True})
        child_context = parent_context.create_child_context('data_analysis_operation', additional_agent_context={'operation_type': 'data_analysis', 'requires_permissions': ['data:read', 'data:analyze'], 'operation_authorized': True}, additional_audit_metadata={'operation_authorization': 'inherited_from_parent', 'permission_check': 'passed'})
        ...
        with pytest.raises(Exception):
            unauthorized_child = parent_context.create_child_context('admin_operation', additional_agent_context={'operation_type': 'admin_operation', 'requires_permissions': ['system:admin'], 'operation_authorized': False})
...
pytestmark = [pytest.mark.integration, pytest.mark.auth_validation, pytest.mark.real_services]
