...
class SecurityComplianceRealValidationIntegrationTests(SSotBaseTestCase):
    ...
    async def test_data_encryption_compliance_validation(self):
        ...
        encryption_result = await self.compliance_validator.validate_data_encryption(data=sensitive_data, encryption_requirements=['AES-256', 'field_level'], test_prefix=self.test_prefix)
