...
from test_framework.performance_helpers import PerformanceProfiler, MemoryTracker, ConcurrencyBenchmark, StatisticalAnalyzer
...
class StateTransitionPerformanceBenchmark(BaseTestCase):
    ...
    def setUp(self):
        ...
        super().setUp()
        self.performance_slas = {'state_transition_p95_ms': 1.0, 'state_transition_p99_ms': 2.0, 'registry_operation_p95_ms': 5.0, 'registry_operation_p99_ms': 10.0, 'concurrent_operation_p95_ms': 10.0, 'memory_per_1000_connections_mb': 1.0, 'cpu_usage_under_load_percent': 50.0}
        self.sample_sizes = {'basic_performance': 1000, 'high_load': 10000, 'stress_test': 50000, 'concurrent_users': 100}
    ...
    def analyze_performance_distribution(self, test_name: str) -> Dict[str, float]:
        ...
        if not samples:
            return {}
        return {'count': len(samples), 'min_ms': min(samples), 'max_ms': max(samples), 'mean_ms': statistics.mean(samples), 'median_ms': statistics.median(samples), 'p95_ms': statistics.quantiles(samples, n=20)[18], 'p99_ms': statistics.quantiles(samples, n=100)[98], 'stdev_ms': statistics.stdev(samples) if len(samples) > 1 else 0}
    ...
    def check_sla_compliance(self, test_name: str, metrics: Dict[str, float]) -> bool:
        ...
        if 'state_transition' in test_name:
            if metrics.get('p95_ms', 0) > self.performance_slas['state_transition_p95_ms']:
                violations.append({'sla': 'state_transition_p95_ms', 'target': self.performance_slas['state_transition_p95_ms'], 'actual': metrics['p95_ms']})
            if metrics.get('p99_ms', 0) > self.performance_slas['state_transition_p99_ms']:
                violations.append({'sla': 'state_transition_p99_ms', 'target': self.performance_slas['state_transition_p99_ms'], 'actual': metrics['p99_ms']})
        if 'registry' in test_name:
            if metrics.get('p95_ms', 0) > self.performance_slas['registry_operation_p95_ms']:
                violations.append({'sla': 'registry_operation_p95_ms', 'target': self.performance_slas['registry_operation_p95_ms'], 'actual': metrics['p95_ms']})
        if violations:
            self.sla_violations.extend([{'test': test_name, 'timestamp': time.time(), **violation} for violation in violations])
    ...
    def test_basic_state_transition_performance(self):
        ...
        test_transitions = [(ApplicationConnectionState.ACCEPTED, 'transport_ready'), (ApplicationConnectionState.AUTHENTICATED, 'auth_complete'), (ApplicationConnectionState.SERVICES_READY, 'services_loaded'), (ApplicationConnectionState.PROCESSING_READY, 'fully_operational'), (ApplicationConnectionState.PROCESSING, 'message_received'), (ApplicationConnectionState.IDLE, 'message_processed')]
        ...
        self.assertLessEqual(metrics['p95_ms'], self.performance_slas['state_transition_p95_ms'], f"95th percentile state transition time {metrics['p95_ms']:.3f}ms exceeds SLA target of {self.performance_slas['state_transition_p95_ms']}ms")
        self.assertLessEqual(metrics['p99_ms'], self.performance_slas['state_transition_p99_ms'], f"99th percentile state transition time {metrics['p99_ms']:.3f}ms exceeds SLA target of {self.performance_slas['state_transition_p99_ms']}ms")
    ...
    def test_registry_operation_performance(self):
        ...
        operations_to_test = ['register_connection', 'get_connection_state_machine', 'get_all_operational_connections', 'get_registry_stats', 'unregister_connection']
        ...
        if all_samples:
            overall_metrics = {'count': len(all_samples), 'mean_ms': statistics.mean(all_samples), 'p95_ms': statistics.quantiles(all_samples, n=20)[18] if len(all_samples) >= 20 else max(all_samples), 'p99_ms': statistics.quantiles(all_samples, n=100)[98] if len(all_samples) >= 100 else max(all_samples)}
            sla_compliant = self.check_sla_compliance(test_name, overall_metrics)
            self.assertLessEqual(overall_metrics['p95_ms'], self.performance_slas['registry_operation_p95_ms'], f"95th percentile registry operation time {overall_metrics['p95_ms']:.3f}ms exceeds SLA target of {self.performance_slas['registry_operation_p95_ms']}ms")
    ...
    def test_concurrent_operation_performance(self):
        ...
        def user_operation_sequence(user_index: int) -> List[float]:
            ...
            try:
                ...
                transitions = [(ApplicationConnectionState.ACCEPTED, 'concurrent_test'), (ApplicationConnectionState.AUTHENTICATED, 'concurrent_test'), (ApplicationConnectionState.SERVICES_READY, 'concurrent_test'), (ApplicationConnectionState.PROCESSING_READY, 'concurrent_test')]
                ...
                operation_times.append(100.0)
            return operation_times
        start_time = time.perf_counter()
        with ThreadPoolExecutor(max_workers=min(concurrent_users, 50)) as executor:
            futures = [executor.submit(user_operation_sequence, i) for i in range(concurrent_users)]
            all_operation_times = []
            completed_users = 0
            for future in as_completed(futures):
                try:
                    ...
                    all_operation_times.extend([50.0] * operations_per_user)
        ...
        self.assertLessEqual(metrics['p95_ms'], self.performance_slas['concurrent_operation_p95_ms'], f"95th percentile concurrent operation time {metrics['p95_ms']:.3f}ms exceeds SLA target of {self.performance_slas['concurrent_operation_p95_ms']}ms")
        self.assertGreaterEqual(completed_users / concurrent_users, 0.95, f'Concurrent operation success rate {completed_users / concurrent_users:.1%} below 95%')
        min_required_ops_per_second = 1000
        self.assertGreaterEqual(operations_per_second, min_required_ops_per_second, f'Throughput {operations_per_second:.0f} ops/sec below requirement of {min_required_ops_per_second}')
    ...
    def test_memory_usage_performance(self):
        ...
        for i in range(0, connection_count, 100):
            batch_start = i
            ...
            self.memory_measurements.append({'connections': connections_created, 'memory_mb': memory_used_mb, 'memory_per_connection_kb': memory_used_mb * 1024 / connections_created if connections_created > 0 else 0})
        ...
        for connection_id, machine in state_machines[:100]:
            machine.get_metrics()
        ...
        self.assertLessEqual(memory_per_1000_connections_mb, self.performance_slas['memory_per_1000_connections_mb'], f"Memory usage {memory_per_1000_connections_mb:.2f}MB per 1000 connections exceeds SLA target of {self.performance_slas['memory_per_1000_connections_mb']}MB")
        self.assertLessEqual(operation_memory_growth, 10.0, f'Excessive memory growth {operation_memory_growth:.2f}MB during operations')
        memory_recovery_rate = memory_recovered_mb / final_memory_mb if final_memory_mb > 0 else 0
        self.assertGreaterEqual(memory_recovery_rate, 0.8, f'Memory recovery rate {memory_recovery_rate:.1%} below 80%, possible memory leak')
    ...
    def test_stress_test_performance_limits(self):
        ...
        for load_level in load_levels:
            ...
            if operation_times:
                ...
                stress_result = {'load_level': load_level, 'mean_operation_ms': load_mean, 'p95_operation_ms': load_p95, 'performance_degradation_ratio': performance_degradation, 'load_creation_time_s': load_creation_time, 'operations_time_s': operations_time, 'cleanup_time_s': cleanup_time, 'degraded_performance': performance_degradation > performance_degradation_threshold}
                ...
                if performance_degradation > performance_degradation_threshold:
                    break
        if stress_results:
            max_sustainable_load = max((result['load_level'] for result in stress_results if not result['degraded_performance']))
            peak_performance_result = next((result for result in stress_results if result['degraded_performance']), stress_results[-1])
            min_required_capacity = 10000
            self.assertGreaterEqual(max_sustainable_load, min_required_capacity, f'Maximum sustainable load {max_sustainable_load} below business requirement of {min_required_capacity}')
    ...
    def test_performance_regression_detection(self):
        ...
        historical_baselines = {'state_transition_mean_ms': 0.1, 'registry_operation_mean_ms': 0.5, 'concurrent_operation_mean_ms': 2.0}
        ...
        for metric_name, baseline_value in historical_baselines.items():
            ...
            if regression_ratio > regression_tolerance:
                regressions_detected.append({'metric': metric_name, 'baseline': baseline_value, 'current': current_value, 'regression_ratio': regression_ratio, 'tolerance': regression_tolerance})
        self.assertEqual(len(regressions_detected), 0, f'Performance regressions detected: {regressions_detected}')
    ...
    def tearDown(self):
        ...
        if self.performance_results:
            summary_report = {'test_timestamp': time.time(), 'performance_slas': self.performance_slas, 'sla_violations': self.sla_violations, 'test_results': {}}
            ...
            if self.sla_violations:
                ...
                for violation in self.sla_violations:
                    print(f"  {violation['test']}: {violation['sla']} (target: {violation['target']}, actual: {violation['actual']})")
            else:
