...
def mock_validation_context(temp_config_path: Path) -> ValidationContext:
    ...
    return ValidationContext(config_path=temp_config_path, is_interactive=True, is_ci_environment=False, cli_overrides={'REDIS_HOST': 'localhost'}, env_overrides={'POSTGRES_HOST': 'db.example.com'})
...
class TestConfigValidationResult:
    ...
    def test_validation_result_with_data(self) -> None:
        ...
        result = ConfigValidationResult(status=ConfigStatus.STALE, warnings=['Config is old'], errors=['Invalid endpoint'], config_age_days=45, reachable_endpoints=['http://api.example.com'], unreachable_endpoints=['http://old.example.com'])
...
class TestValidationContext:
    ...
    def test_validation_context_with_overrides(self, temp_config_path: Path) -> None:
        ...
        context = ValidationContext(config_path=temp_config_path, is_interactive=False, cli_overrides=cli_overrides, env_overrides=env_overrides)
