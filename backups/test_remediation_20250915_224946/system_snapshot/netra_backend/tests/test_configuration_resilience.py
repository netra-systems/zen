...
class ConfigurationResilienceTests:
    ...
    def test_missing_database_url_should_use_safe_default(self):
        ...
        with patch.dict(os.environ, {}, clear=True):
            with patch.dict(os.environ, {'ENVIRONMENT': 'development', 'SERVICE_NAME': 'netra_backend'}):
                try:
                    config = get_unified_config()
                    assert config is not None, 'Configuration should load even with missing DATABASE_URL'
                    database_url = getattr(config, 'database_url', None)
                    if database_url is None:
                        assert hasattr(config, 'configuration_errors'), 'Should track configuration errors when #removed-legacyis missing'
    ...
    def test_gcp_secret_manager_unavailable_should_fallback(self):
        ...
        """
        with patch.dict(os.environ, {'GCP_PROJECT_ID': 'invalid-project', 'GOOGLE_APPLICATION_CREDENTIALS': '/nonexistent/credentials.json', 'ENVIRONMENT': 'staging', 'GEMINI_API_KEY': 'fallback-api-key-from-env', 'JWT_SECRET_KEY': 'fallback-jwt-secret-from-env'}):
            try:
    ...
    def test_invalid_environment_values_should_be_sanitized(self):
        ...
        """
        with patch.dict(os.environ, {'ENVIRONMENT': 'development', 'DATABASE_URL': 'postgresql://user:pass@localhost:5432/db', 'REDIS_PORT': 'invalid_port_number', 'CORS_ORIGINS': 'not-a-valid-json-array', 'MAX_WORKERS': 'not-a-number', 'DEBUG': 'maybe', 'JWT_EXPIRE_MINUTES': 'forever'}):
            try:
                config = get_unified_config()
    ...
    def test_partial_configuration_loading_detected_and_handled(self):
        ...
        """
        with patch.dict(os.environ, {'ENVIRONMENT': 'production', 'DATABASE_URL': 'postgresql://user:pass@localhost:5432/db', 'SERVICE_NAME': 'netra_backend'}):
            try:
                config = get_unified_config()
                ...
                error_message = str(e)
                assert 'configuration' in error_message.lower(), f'Configuration errors should be clearly identified: {e}'
    ...
    def test_environment_isolation_prevents_cross_contamination(self):
        ...
        """
        dev_env = {'ENVIRONMENT': 'development', 'DATABASE_URL': 'postgresql://dev:pass@localhost:5432/dev_db', 'DEBUG': 'true', 'REDIS_URL': 'redis://localhost:6379/0'}
        staging_env = {'ENVIRONMENT': 'staging', 'DATABASE_URL': 'postgresql://staging:pass@staging:5432/staging_db', 'DEBUG': 'false', 'REDIS_URL': 'redis://staging-redis:6379/0'}
        try:
            ...
            with patch.dict(os.environ, staging_env, clear=True):
                ...
                assert staging_environment != dev_environment or staging_environment == 'staging', 'Staging config should not be contaminated by development config'
        except Exception as e:
    ...
    def test_secret_loading_timeout_handling(self):
        ...
        """
        with patch.dict(os.environ, {'ENVIRONMENT': 'staging', 'GCP_PROJECT_ID': 'test-project', 'GOOGLE_APPLICATION_CREDENTIALS': '/tmp/fake-credentials.json', 'DATABASE_URL': 'postgresql://user:pass@localhost:5432/db', 'GEMINI_API_KEY': 'fallback-key', 'JWT_SECRET_KEY': 'fallback-jwt-secret'}):
            try:
                import time
    ...
    async def test_async_configuration_loading_thread_safety(self):
        ...
        """
        test_env = {'ENVIRONMENT': 'development', 'DATABASE_URL': 'postgresql://user:pass@localhost:5432/test_db', 'SERVICE_NAME': 'netra_backend'}
        with patch.dict(os.environ, test_env, clear=True):
            try:
                ...
                if exceptions:
                    assert len(exceptions) == len(configs), 'Configuration loading should be consistent across async contexts'
                else:
                    first_config = configs[0]
                    for config in configs[1:]:
                        assert config.environment == first_config.environment, 'All async config loads should return same environment'
                        assert getattr(config, 'database_url', None) == getattr(first_config, 'database_url', None), 'All async config loads should return same database_url'
            except Exception as e:
