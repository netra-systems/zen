...
class RealJWTValidationTests:
    ...
    async def test_jwt_token_missing_required_claims(self, jwt_secret_key: str):
        ...
        incomplete_payload = {JWTConstants.EMAIL: 'incomplete@netrasystems.ai', JWTConstants.ISSUED_AT: int(datetime.utcnow().timestamp()), JWTConstants.EXPIRES_AT: int((datetime.utcnow() + timedelta(minutes=30)).timestamp())}
    ...
    async def test_jwt_token_user_context_isolation(self, jwt_secret_key: str):
        ...
        user1_payload = {JWTConstants.SUBJECT: 'user_1', JWTConstants.EMAIL: 'user1@netrasystems.ai', JWTConstants.ISSUED_AT: int(datetime.utcnow().timestamp()), JWTConstants.EXPIRES_AT: int((datetime.utcnow() + timedelta(minutes=30)).timestamp()), 'user_id': 1, 'permissions': ['read']}
        user2_payload = {JWTConstants.SUBJECT: 'user_2', JWTConstants.EMAIL: 'user2@netrasystems.ai', JWTConstants.ISSUED_AT: int(datetime.utcnow().timestamp()), JWTConstants.EXPIRES_AT: int((datetime.utcnow() + timedelta(minutes=30)).timestamp()), 'user_id': 2, 'permissions': ['read', 'write', 'admin']}
    ...
    async def test_jwt_token_permission_validation(self, jwt_secret_key: str):
        ...
        admin_payload = {JWTConstants.SUBJECT: 'admin_user', JWTConstants.EMAIL: 'admin@netrasystems.ai', JWTConstants.ISSUED_AT: int(datetime.utcnow().timestamp()), JWTConstants.EXPIRES_AT: int((datetime.utcnow() + timedelta(minutes=30)).timestamp()), 'user_id': 999, 'permissions': ['read', 'write', 'admin', 'delete']}
        readonly_payload = {JWTConstants.SUBJECT: 'readonly_user', JWTConstants.EMAIL: 'readonly@netrasystems.ai', JWTConstants.ISSUED_AT: int(datetime.utcnow().timestamp()), JWTConstants.EXPIRES_AT: int((datetime.utcnow() + timedelta(minutes=30)).timestamp()), 'user_id': 100, 'permissions': ['read']}
    ...
    async def test_jwt_token_refresh_mechanism(self, jwt_secret_key: str, real_db_session):
        ...
        access_payload = {JWTConstants.SUBJECT: 'refresh_test_user', JWTConstants.EMAIL: 'refresh@netrasystems.ai', JWTConstants.ISSUED_AT: int(datetime.utcnow().timestamp()), JWTConstants.EXPIRES_AT: int((datetime.utcnow() + timedelta(minutes=5)).timestamp()), 'token_type': JWTConstants.ACCESS_TOKEN_TYPE, 'user_id': 456}
        refresh_payload = {JWTConstants.SUBJECT: 'refresh_test_user', JWTConstants.EMAIL: 'refresh@netrasystems.ai', JWTConstants.ISSUED_AT: int(datetime.utcnow().timestamp()), JWTConstants.EXPIRES_AT: int((datetime.utcnow() + timedelta(days=7)).timestamp()), 'token_type': JWTConstants.REFRESH_TOKEN_TYPE, 'user_id': 456}
    ...
    async def test_jwt_token_api_endpoint_integration(self, async_client: AsyncClient, jwt_secret_key: str):
        ...
        api_payload = {JWTConstants.SUBJECT: 'api_test_user', JWTConstants.EMAIL: 'apitest@netrasystems.ai', JWTConstants.ISSUED_AT: int(datetime.utcnow().timestamp()), JWTConstants.EXPIRES_AT: int((datetime.utcnow() + timedelta(minutes=30)).timestamp()), JWTConstants.ISSUER: JWTConstants.NETRA_AUTH_SERVICE, 'user_id': 789, 'permissions': ['read', 'write']}
