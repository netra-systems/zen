...
def generate_markdown_report():
    ...
    with open(report_path, "w") as f:
        ...
        for priority, label in priority_labels.items():
            ...
            if tests:
                ...
                for test in tests:
                    ...
                    icon = " PASS: " if outcome == "passed" else " FAIL: " if outcome == "failed" else "[U+23ED][U+FE0F]"
        ...
        if failed_tests:
            ...
            for test in failed_tests:
                f.write(f"###  FAIL:  {test.get('nodeid', 'unknown')}\n\n")
        ...
        if pass_rate >= 95:
            f.write(" PASS:  **System is production ready** - Pass rate exceeds 95%\n\n")
        elif pass_rate >= 80:
            f.write(" WARNING: [U+FE0F] **System needs attention** - Pass rate is between 80-95%\n\n")
        else:
            f.write(" FAIL:  **System not ready** - Pass rate below 80%\n\n")
    ...
    print(f"\n CHART:  Report saved to: {report_path.absolute()}")
