...
def create_request_scoped_engine(user_id: str, thread_id: str, run_id: str, request_id: str = None, **kwargs):
    """Create a request-scoped execution engine for backward compatibility.
    
    This function provides backward compatibility with the legacy execution engine API
    while using the new UserExecutionEngine implementation.
    ...
    Returns:
        UserExecutionEngine instance
    """
    from netra_backend.app.services.user_execution_context import UserExecutionContext
    
    # Create context using the from_request_supervisor method for compatibility
    ...
    # Return a UserExecutionEngine instance
    return UserExecutionEngine(
        user_context=context,
        engine_id=f"request_scoped_{user_id}_{run_id}",
        factory_mode='request_scoped_compatibility'
    )


def create_execution_context_manager(user_id: str, thread_id: str, run_id: str, request_id: str = None, **kwargs):
    """Create an execution context manager for backward compatibility.
    
    This function provides backward compatibility with context manager patterns
    while using the new UserExecutionEngine implementation.
    ...
    Returns:
        UserExecutionEngine instance configured as context manager
    """
    from netra_backend.app.services.user_execution_context import UserExecutionContext
    
    # Create context using compatibility method
    context = UserExecutionContext.from_request_supervisor(
        user_id=user_id,
        thread_id=thread_id,
        run_id=run_id,
    ...
    # Return a UserExecutionEngine configured for context manager use
    engine = UserExecutionEngine(
        user_context=context,
        engine_id=f"context_mgr_{user_id}_{run_id}",
        factory_mode='context_manager_compatibility'
    )
