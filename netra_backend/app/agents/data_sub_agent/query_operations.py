...
class QueryOperations:
    """Static query building operations for ClickHouse."""
    ...
    def build_performance_metrics_query(
        ...
        time_function = QueryOperations._get_time_function(aggregation_level)
        workload_filter = QueryOperations._build_workload_filter(workload_id)
        select_clause = QueryOperations._build_performance_select_clause(time_function)
        subquery = QueryOperations._build_performance_subquery(user_id, start_time, end_time, workload_filter)
        return QueryOperations._assemble_performance_query(select_clause, subquery)
    ...
    def _build_performance_select_clause(time_function: str) -> str:
        ...
        latency_fields = QueryOperations._build_latency_fields()
        throughput_fields = QueryOperations._build_throughput_fields()
        cost_fields = QueryOperations._build_cost_fields()
    ...
    def _build_performance_subquery(user_id: int, start_time: datetime, end_time: datetime, workload_filter: str) -> str:
        ...
        select_part = QueryOperations._build_subquery_select_part()
        from_part = QueryOperations._build_subquery_from_part(user_id, start_time, end_time, workload_filter)
        return f"FROM ({select_part} {from_part})"
    
    @staticmethod
    def _build_subquery_select_part() -> str:
        """Build SELECT part of subquery."""
        index_fields = QueryOperations._build_index_fields()
        value_fields = QueryOperations._build_value_fields()
        flag_fields = QueryOperations._build_flag_fields()
    ...
    def _assemble_performance_query(select_clause: str, subquery: str) -> str:
        ...
        return f"""{QueryOperations.QUERY_SOURCE_MARKER}
    ...
    def build_anomaly_detection_query(
        ...
        baseline_clause = QueryOperations._build_anomaly_baseline_clause(user_id, metric_name, start_time)
        main_query = QueryOperations._build_anomaly_main_query(user_id, metric_name, start_time, end_time, z_score_threshold)
        return QueryOperations._assemble_anomaly_query(baseline_clause, main_query)
    ...
    def _build_anomaly_baseline_clause(user_id: int, metric_name: str, start_time: datetime) -> str:
        ...
        select_part = QueryOperations._build_baseline_select_part(metric_name)
        where_part = QueryOperations._build_baseline_where_part(user_id, baseline_start, start_time)
    ...
    def _build_anomaly_main_query(user_id: int, metric_name: str, start_time: datetime, end_time: datetime, z_score_threshold: float) -> str:
        ...
        select_part = QueryOperations._build_anomaly_select_part(metric_name, z_score_threshold)
        where_part = QueryOperations._build_anomaly_where_part(user_id, start_time, end_time)
    ...
    def _assemble_anomaly_query(baseline_clause: str, main_query: str) -> str:
        ...
        return f"{QueryOperations.QUERY_SOURCE_MARKER} {baseline_clause} {main_query}"
    ...
    def build_correlation_analysis_query(
        ...
        select_part = QueryOperations._build_correlation_select_part()
        subquery_part = QueryOperations._build_correlation_subquery(user_id, metric1, metric2, start_time, end_time)
        return f"{QueryOperations.QUERY_SOURCE_MARKER} {select_part} FROM ({subquery_part}) WHERE idx1 > 0 AND idx2 > 0"
    ...
    def build_usage_patterns_query(
        ...
        select_part = QueryOperations._build_usage_select_part()
        subquery_part = QueryOperations._build_usage_subquery(user_id, days_back)
        return f"{QueryOperations.QUERY_SOURCE_MARKER} {select_part} FROM ({subquery_part}) GROUP BY day_of_week, hour_of_day ORDER BY day_of_week, hour_of_day"
