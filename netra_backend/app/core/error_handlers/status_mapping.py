...
class StatusMapper:
    ...
    def __init__(self):
        """Initialize status mapper with all mappings."""
        self._status_map = self._build_complete_status_map()
    
    def get_status_code(self, error_code: ErrorCode) -> int:
    ...
    def _build_complete_status_map(self) -> Dict[ErrorCode, int]:
        """Build complete error code to status mapping."""
        mappings = self._collect_all_status_maps()
        complete_map = {}
        for mapping in mappings.values():
            complete_map.update(mapping)
        return complete_map
    
    def _collect_all_status_maps(self) -> Dict[str, Dict[ErrorCode, int]]:
        """Collect all error status maps by category."""
        return {
