...
def _import_auth_routers() -> dict:
    ...
    from netra_backend.app.routes.auth_resilience import router as auth_resilience_router
    # Auth router not needed - auth is handled by separate auth service
    return {"agent_router": agent_router, "auth_resilience_router": auth_resilience_router}
...
def _import_extended_router_modules() -> tuple:
    ...
    return health_extended_router, monitoring_router, gcp_monitoring_router, websocket_router


def _create_extended_router_dict(router_imports: tuple) -> dict:
    """Create extended router dictionary from imports."""
    health_extended_router, monitoring_router, gcp_monitoring_router, websocket_router = router_imports
