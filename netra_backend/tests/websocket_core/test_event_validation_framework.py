...
class EventValidatorTests:
    ...
    def setup_method(self):
        ...
        self.sample_context = {'thread_id': 'test-thread-123', 'run_id': 'run-456'}
    ...
    def test_validate_agent_started_event_valid(self):
        ...
        event = {'type': 'agent_started', 'payload': {'agent_name': 'test_agent', 'run_id': 'run-123', 'timestamp': time.time()}, 'thread_id': 'thread-123', 'timestamp': time.time()}
    ...
    def test_validate_agent_started_event_missing_fields(self):
        ...
        event = {'type': 'agent_started', 'payload': {'timestamp': time.time()}, 'thread_id': 'thread-123', 'timestamp': time.time()}
        validated = self.validator.validate_event(event, self.sample_context)
    ...
    def test_validate_agent_thinking_event_valid(self):
        ...
        event = {'type': 'agent_thinking', 'payload': {'thought': 'I am analyzing the user request', 'agent_name': 'test_agent', 'timestamp': time.time()}, 'thread_id': 'thread-123', 'timestamp': time.time()}
        validated = self.validator.validate_event(event, self.sample_context)
        assert validated.validation_result == ValidationResult.VALID
        assert len(validated.validation_errors) == 0
    ...
    def test_validate_agent_thinking_event_empty_thought(self):
        ...
        event = {'type': 'agent_thinking', 'payload': {'thought': '', 'agent_name': 'test_agent', 'timestamp': time.time()}, 'thread_id': 'thread-123', 'timestamp': time.time()}
        validated = self.validator.validate_event(event, self.sample_context)
    ...
    def test_validate_tool_executing_event_valid(self):
        ...
        event = {'type': 'tool_executing', 'payload': {'tool_name': 'web_search', 'agent_name': 'test_agent', 'timestamp': time.time()}, 'thread_id': 'thread-123', 'timestamp': time.time()}
        validated = self.validator.validate_event(event, self.sample_context)
        assert validated.validation_result == ValidationResult.VALID
        assert len(validated.validation_errors) == 0
    ...
    def test_validate_tool_completed_event_valid(self):
        ...
        event = {'type': 'tool_completed', 'payload': {'tool_name': 'web_search', 'agent_name': 'test_agent', 'result': {'status': 'success', 'data': 'search results'}, 'success': True, 'duration_ms': 1500, 'timestamp': time.time()}, 'thread_id': 'thread-123', 'timestamp': time.time()}
        validated = self.validator.validate_event(event, self.sample_context)
        assert validated.validation_result == ValidationResult.VALID
        assert len(validated.validation_errors) == 0
    ...
    def test_validate_agent_completed_event_valid(self):
        ...
        event = {'type': 'agent_completed', 'payload': {'agent_name': 'test_agent', 'run_id': 'run-123', 'result': {'answer': 'Task completed successfully'}, 'duration_ms': 5000, 'timestamp': time.time()}, 'thread_id': 'thread-123', 'timestamp': time.time()}
        validated = self.validator.validate_event(event, self.sample_context)
        assert validated.validation_result == ValidationResult.VALID
        assert len(validated.validation_errors) == 0
    ...
    def test_validate_invalid_timestamp(self):
        ...
        event = {'type': 'agent_started', 'payload': {'agent_name': 'test_agent', 'run_id': 'run-123', 'timestamp': time.time() - 7200}, 'thread_id': 'thread-123', 'timestamp': time.time() - 7200}
        validated = self.validator.validate_event(event, self.sample_context)
...
class EventSequenceValidatorTests:
    ...
    def test_valid_event_sequence(self):
        ...
        events = [{'type': 'agent_started', 'payload': {'agent_name': 'test_agent', 'run_id': self.run_id, 'timestamp': time.time()}, 'thread_id': self.thread_id, 'timestamp': time.time()}, {'type': 'agent_thinking', 'payload': {'thought': 'Processing user request', 'agent_name': 'test_agent', 'timestamp': time.time()}, 'thread_id': self.thread_id, 'timestamp': time.time()}, {'type': 'tool_executing', 'payload': {'tool_name': 'web_search', 'agent_name': 'test_agent', 'timestamp': time.time()}, 'thread_id': self.thread_id, 'timestamp': time.time()}, {'type': 'tool_completed', 'payload': {'tool_name': 'web_search', 'agent_name': 'test_agent', 'result': {'data': 'results'}, 'timestamp': time.time()}, 'thread_id': self.thread_id, 'timestamp': time.time()}, {'type': 'agent_completed', 'payload': {'agent_name': 'test_agent', 'run_id': self.run_id, 'result': {'answer': 'Done'}, 'timestamp': time.time()}, 'thread_id': self.thread_id, 'timestamp': time.time()}]
        validated_events = []
        for event in events:
    ...
    def test_unpaired_tool_events(self):
        ...
        events = [{'type': 'agent_started', 'payload': {'agent_name': 'test_agent', 'run_id': self.run_id, 'timestamp': time.time()}, 'thread_id': self.thread_id, 'timestamp': time.time()}, {'type': 'tool_executing', 'payload': {'tool_name': 'web_search', 'agent_name': 'test_agent', 'timestamp': time.time()}, 'thread_id': self.thread_id, 'timestamp': time.time()}, {'type': 'agent_completed', 'payload': {'agent_name': 'test_agent', 'run_id': self.run_id, 'result': {'answer': 'Done'}, 'timestamp': time.time()}, 'thread_id': self.thread_id, 'timestamp': time.time()}]
        for event in events:
            validated_event = self.sequence_validator.add_event_to_sequence(self.thread_id, event)
        sequence_status = self.sequence_validator.get_sequence_status(self.thread_id)
    ...
    def test_invalid_event_order(self):
        ...
        events = [{'type': 'tool_executing', 'payload': {'tool_name': 'web_search', 'agent_name': 'test_agent', 'timestamp': time.time()}, 'thread_id': self.thread_id, 'timestamp': time.time()}, {'type': 'agent_started', 'payload': {'agent_name': 'test_agent', 'run_id': self.run_id, 'timestamp': time.time()}, 'thread_id': self.thread_id, 'timestamp': time.time()}]
        for event in events:
            validated_event = self.sequence_validator.add_event_to_sequence(self.thread_id, event)
    ...
    def test_sequence_timing_validation(self):
        ...
        base_time = time.time()
        events = [{'type': 'agent_started', 'payload': {'agent_name': 'test_agent', 'run_id': self.run_id, 'timestamp': base_time}, 'thread_id': self.thread_id, 'timestamp': base_time}, {'type': 'agent_completed', 'payload': {'agent_name': 'test_agent', 'run_id': self.run_id, 'result': {'answer': 'Done'}, 'timestamp': base_time + 3600}, 'thread_id': self.thread_id, 'timestamp': base_time + 3600}]
        for event in events:
            validated_event = self.sequence_validator.add_event_to_sequence(self.thread_id, event)
        sequence_status = self.sequence_validator.get_sequence_status(self.thread_id)
...
class EventValidationFrameworkTests:
    ...
    async def test_validate_event_success(self):
        ...
        event = {'type': 'agent_started', 'payload': {'agent_name': 'test_agent', 'run_id': 'run-123', 'timestamp': time.time()}, 'thread_id': self.thread_id, 'timestamp': time.time()}
    ...
    async def test_validate_event_with_errors(self):
        ...
        event = {'type': 'agent_thinking', 'payload': {'thought': '', 'agent_name': 'test_agent', 'timestamp': time.time()}, 'thread_id': self.thread_id, 'timestamp': time.time()}
        validated_event = await self.framework.validate_event(event, self.context)
    ...
    async def test_circuit_breaker_functionality(self):
        ...
        invalid_event = {'type': 'agent_started', 'payload': {'timestamp': time.time()}, 'thread_id': self.thread_id, 'timestamp': time.time()}
    ...
    async def test_circuit_breaker_recovery(self):
        ...
        valid_event = {'type': 'agent_started', 'payload': {'agent_name': 'test_agent', 'run_id': 'run-123', 'timestamp': time.time()}, 'thread_id': self.thread_id, 'timestamp': time.time()}
    ...
    async def test_event_replay(self):
        ...
        events = [{'type': 'agent_started', 'payload': {'agent_name': 'test_agent', 'run_id': 'run-123', 'timestamp': time.time()}, 'thread_id': self.thread_id, 'timestamp': time.time()}, {'type': 'agent_completed', 'payload': {'agent_name': 'test_agent', 'run_id': 'run-123', 'result': {'answer': 'Done'}, 'timestamp': time.time()}, 'thread_id': self.thread_id, 'timestamp': time.time()}]
        for event in events:
...
class ConvenienceFunctionsTests:
    ...
    async def test_validate_websocket_event_function(self):
        ...
        event = {'type': 'agent_started', 'payload': {'agent_name': 'test_agent', 'run_id': 'run-123', 'timestamp': time.time()}, 'thread_id': 'test-thread', 'timestamp': time.time()}
...
async def test_concurrent_validation():
    ...
    events = []
    for i in range(10):
        event = {'type': 'agent_started', 'payload': {'agent_name': f'agent_{i}', 'run_id': f'run_{i}', 'timestamp': time.time()}, 'thread_id': f'thread_{i}', 'timestamp': time.time()}
        events.append(event)
...
async def test_validation_callback_system():
    ...
    valid_event = {'type': 'agent_started', 'payload': {'agent_name': 'test_agent', 'run_id': 'run-123', 'timestamp': time.time()}, 'thread_id': 'test-thread', 'timestamp': time.time()}
    await framework.validate_event(valid_event)
    assert len(callback_calls) == 1
    invalid_event = {'type': 'agent_started', 'payload': {'timestamp': time.time()}, 'thread_id': 'test-thread', 'timestamp': time.time()}
