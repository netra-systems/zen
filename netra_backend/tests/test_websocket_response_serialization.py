...
class TestWebSocketMessageSerialization:
    ...
    def test_websocket_message_serialization(self) -> None:
        """Test WebSocket message serialization."""
        message = WebSocketMessage(type=WebSocketMessageType.AGENT_STARTED, payload={'run_id': 'run123'})
    ...
    def test_websocket_agent_update_message(self) -> None:
        ...
        update_payload = {'content': 'Analysis in progress...', 'run_id': 'run123', 'metadata': {'progress': 45}}
        message = WebSocketMessage(type=WebSocketMessageType.AGENT_UPDATE, payload=update_payload)
    ...
    def test_websocket_tool_completed_message(self) -> None:
        ...
        tool_payload = {'tool_name': 'log_analyzer', 'tool_output': {'findings': 3, 'severity': 'high'}, 'run_id': 'run123', 'status': 'success'}
        message = WebSocketMessage(type=WebSocketMessageType.TOOL_COMPLETED, payload=tool_payload)
        json_str = message.model_dump_json()
        parsed = json.loads(json_str)
    ...
    def test_websocket_error_message(self) -> None:
        ...
        error_payload = {'run_id': 'run123', 'message': 'Connection lost', 'error_code': 'WEBSOCKET_ERROR'}
        message = WebSocketMessage(type=WebSocketMessageType.AGENT_ERROR, payload=error_payload)
        json_data = message.model_dump()
    ...
    def test_websocket_ping_pong_messages(self) -> None:
        ...
        ping_message = WebSocketMessage(type=WebSocketMessageType.PING, payload={'timestamp': '2025-01-01T12:00:00Z'})
        pong_message = WebSocketMessage(type=WebSocketMessageType.PONG, payload={'timestamp': '2025-01-01T12:00:01Z'})
...
class TestStreamingResponseSerialization:
    ...
    def test_stream_chunk_structure(self) -> None:
        ...
        chunk_data = {'chunk_id': 'test_chunk', 'content': 'Processing data...', 'is_final': False}
    ...
    def test_streaming_sequence(self) -> None:
        ...
        chunks = [StreamChunk(chunk_id='chunk_0', content='Processing', is_final=False), StreamChunk(chunk_id='chunk_1', content=' data', is_final=False), StreamChunk(chunk_id='chunk_2', content=' complete', is_final=True)]
    ...
    def test_stream_complete_structure(self) -> None:
        ...
        complete_data = {'stream_id': 'stream_123', 'total_chunks': 5, 'metadata': {'total_tokens': 150, 'duration': 5.2}}
    ...
    def test_stream_chunk_with_rich_metadata(self) -> None:
        ...
        rich_metadata = {'model': LLMModel.GEMINI_2_5_FLASH.value, 'tokens': {'input': 50, 'output': 25, 'total': 75}, 'timing': {'start': '2025-01-01T10:00:00Z', 'duration': 1.2}, 'quality': {'confidence': 0.95, 'relevance': 0.88}}
        chunk = StreamChunk(chunk_id='rich_chunk', content='Rich analysis chunk', is_final=False)
        json_data = chunk.model_dump()
    ...
    def test_stream_error_handling(self) -> None:
        ...
        error_chunk = StreamChunk(chunk_id='error_chunk', content='Error occurred during processing', is_final=True)
...
class TestRealtimeMessageFlow:
    ...
    def test_agent_lifecycle_websocket_flow(self) -> None:
        ...
        messages = [WebSocketMessage(type=WebSocketMessageType.AGENT_STARTED, payload={'run_id': 'run123'}), WebSocketMessage(type=WebSocketMessageType.AGENT_UPDATE, payload={'content': 'Starting analysis...', 'run_id': 'run123', 'metadata': {'step': 'initialization'}}), WebSocketMessage(type=WebSocketMessageType.TOOL_STARTED, payload={'tool_name': 'data_fetcher', 'tool_args': {'source': 'database'}, 'run_id': 'run123'}), WebSocketMessage(type=WebSocketMessageType.TOOL_COMPLETED, payload={'tool_name': 'data_fetcher', 'tool_output': {'rows': 1000}, 'run_id': 'run123', 'status': 'success'}), WebSocketMessage(type=WebSocketMessageType.AGENT_COMPLETED, payload={'run_id': 'run123', 'result': {'analysis': 'complete'}})]
        for msg in messages:
    ...
    def test_streaming_with_websocket_wrapper(self) -> None:
        ...
        stream_chunks = [StreamChunk(chunk_id='analyze_0', content='Analyzing', is_final=False), StreamChunk(chunk_id='analyze_1', content=' performance', is_final=False), StreamChunk(chunk_id='analyze_2', content=' metrics', is_final=True)]
        websocket_messages = [WebSocketMessage(type=WebSocketMessageType.STREAM_CHUNK, payload=chunk.model_dump()) for chunk in stream_chunks]
    ...
    def test_concurrent_agent_messages(self) -> None:
        ...
        concurrent_updates = [WebSocketMessage(type=WebSocketMessageType.AGENT_UPDATE, payload={'content': f'Agent {i} processing', 'run_id': f'run{i}', 'metadata': {'agent_id': i}}) for i in range(3)]
