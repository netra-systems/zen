...
class TestVariableScopingBug:
    ...
    def staging_environment_vars(self):
        ...
        return {'ENVIRONMENT': 'staging', 'GOOGLE_CLOUD_PROJECT': 'netra-staging-project', 'K_SERVICE': 'netra-staging-backend', 'E2E_TESTING': '0', 'STAGING_E2E_TEST': '0', 'PYTEST_RUNNING': '1'}

    def test_production_environment_detection(self, mock_websocket):
        """Test that production environment is correctly detected."""
        production_vars = {'ENVIRONMENT': 'production', 'GOOGLE_CLOUD_PROJECT': 'netra-production-project', 'K_SERVICE': 'netra-prod-backend'}
    ...
    def test_staging_environment_detection(self, mock_websocket):
        ...
        staging_vars = {'ENVIRONMENT': 'staging', 'GOOGLE_CLOUD_PROJECT': 'netra-staging-project', 'K_SERVICE': 'netra-staging-backend'}
    ...
    def test_local_environment_detection(self, mock_websocket):
        ...
        local_vars = {'ENVIRONMENT': 'local', 'GOOGLE_CLOUD_PROJECT': '', 'K_SERVICE': '', 'E2E_TESTING': '1'}
    ...
    def test_e2e_context_with_staging_triggers_bug(self, mock_websocket):
        ...
        """
        staging_vars = {'ENVIRONMENT': 'staging', 'GOOGLE_CLOUD_PROJECT': 'netra-staging-test-project', 'K_SERVICE': 'netra-backend-staging', 'E2E_TESTING': '0', 'STAGING_E2E_TEST': '0'}
        mock_websocket.headers = {'x-e2e-test': 'true', 'test-environment': 'staging'}
        with patch('shared.isolated_environment.get_env', return_value=staging_vars):
            try:
                e2e_context = extract_e2e_context_from_websocket(mock_websocket)
    ...
    def test_variable_declaration_order_validation(self, mock_websocket):
        ...
        problematic_vars = {'ENVIRONMENT': 'staging', 'GOOGLE_CLOUD_PROJECT': 'netra-staging-project', 'K_SERVICE': 'netra-staging-service', 'E2E_TESTING': '0'}
    ...
    def test_k_service_naming_variations(self, mock_websocket):
        ...
        k_service_variations = ['netra-staging-backend', 'staging-netra-api', 'netra-backend-staging', 'staging-service', 'netra-staging']
        for k_service in k_service_variations:
            staging_vars = {'ENVIRONMENT': 'staging', 'GOOGLE_CLOUD_PROJECT': 'netra-staging', 'K_SERVICE': k_service}
    ...
    def test_concurrent_environment_detection(self, mock_websocket):
        ...
        concurrent_vars = {'ENVIRONMENT': 'staging', 'GOOGLE_CLOUD_PROJECT': 'netra-staging', 'K_SERVICE': 'staging-backend', 'PYTEST_XDIST_WORKER': 'gw0', 'PYTEST_CURRENT_TEST': 'test_websocket_auth_scoping.py::test_staging', 'RACE_CONDITION_TEST_MODE': '1'}
...
class TestWebSocketAuthenticatorScoping:
    ...
    async def test_authenticate_with_scoping_bug_conditions(self, authenticator, mock_websocket):
        ...
        bug_trigger_vars = {'ENVIRONMENT': 'staging', 'GOOGLE_CLOUD_PROJECT': 'netra-staging-test', 'K_SERVICE': 'backend-staging', 'E2E_TESTING': '0'}
    ...
    async def test_error_handling_with_scoping_conditions(self, authenticator, mock_websocket):
        ...
        error_trigger_vars = {'ENVIRONMENT': 'staging', 'GOOGLE_CLOUD_PROJECT': 'netra-staging', 'K_SERVICE': 'staging-backend'}
...
class TestBackwardCompatibilityWithScoping:
    ...
    async def test_authenticate_websocket_connection_with_scoping_bug(self, mock_websocket):
        ...
        scoping_bug_vars = {'ENVIRONMENT': 'staging', 'GOOGLE_CLOUD_PROJECT': 'netra-staging-project', 'K_SERVICE': 'netra-staging-backend', 'E2E_TESTING': '0', 'STAGING_E2E_TEST': '0'}
        with patch('shared.isolated_environment.get_env', return_value=scoping_bug_vars):
            with patch('netra_backend.app.websocket_core.unified_websocket_auth.get_unified_auth_service') as mock_auth_service:
                ...
                mock_auth_instance.authenticate_websocket.return_value = (mock_auth_result, mock_user_context)
                try:
                    result = await authenticate_websocket_connection(websocket=mock_websocket, token='test_token_staging', e2e_context=None)
                    assert isinstance(result, WebSocketAuthResult)
                    if result.success:
