...
class TestWebSocketCloseCodes:
    ...
    async def test_normal_closure_clean_disconnect(self):
        ...
        welcome_message = json.dumps({'type': 'connection_established', 'connection_id': 'test-close-normal', 'server_time': datetime.now(timezone.utc).isoformat(), 'message': 'WebSocket connected - testing normal closure'})
        from websockets import ConnectionClosed
        mock_websocket.recv = AsyncMock(side_effect=[welcome_message, ConnectionClosed(None, None)])
        ...
        with patch('websockets.connect', return_value=mock_connection):
            async with mock_connection as websocket:
                ...
                close_message = {'type': 'disconnect', 'reason': 'user_initiated', 'code': 1000}
    ...
    async def test_protocol_error_close_code_1002(self):
        ...
        mock_websocket = AsyncMock()
        mock_websocket.application_state = WebSocketState.CONNECTED
        error_response = json.dumps({'type': 'error', 'error_code': 'PROTOCOL_ERROR', 'message': 'Invalid WebSocket protocol usage', 'close_code': 1002, 'timestamp': datetime.now(timezone.utc).isoformat()})
        from websockets import ConnectionClosedError
        mock_websocket.recv = AsyncMock(side_effect=[error_response, ConnectionClosedError(None, None)])
    ...
    async def test_invalid_data_close_code_1003(self):
        ...
        mock_websocket = AsyncMock()
        mock_websocket.application_state = WebSocketState.CONNECTED
        error_response = json.dumps({'type': 'error', 'error_code': 'INVALID_DATA', 'message': 'Received data type not supported', 'close_code': 1003, 'timestamp': datetime.now(timezone.utc).isoformat()})
        from websockets import ConnectionClosedError
        mock_websocket.recv = AsyncMock(side_effect=[error_response, ConnectionClosedError(None, None)])
        mock_websocket.send = AsyncMock()
        mock_connection = AsyncMock()
        ...
        with patch('websockets.connect', return_value=mock_connection):
            async with mock_connection as websocket:
                unsupported_data = b'\xff\xfe\xfd\xfc'
    ...
    async def test_policy_violation_close_code_1008(self):
        ...
        mock_websocket = AsyncMock()
        mock_websocket.application_state = WebSocketState.CONNECTED
        error_response = json.dumps({'type': 'error', 'error_code': 'POLICY_VIOLATION', 'message': 'Message rate limit exceeded', 'close_code': 1008, 'details': {'rate_limit': '30 messages per minute', 'current_rate': '45 messages per minute'}, 'timestamp': datetime.now(timezone.utc).isoformat()})
        from websockets import ConnectionClosedError
        mock_websocket.recv = AsyncMock(side_effect=[error_response, ConnectionClosedError(None, None)])
        mock_websocket.send_json = AsyncMock()
        mock_connection = AsyncMock()
        ...
        with patch('websockets.connect', return_value=mock_connection):
            async with mock_connection as websocket:
                for i in range(50):
                    spam_message = {'type': 'user_message', 'payload': {'content': f'Spam message {i}'}}
    ...
    async def test_message_too_big_close_code_1009(self):
        ...
        mock_websocket = AsyncMock()
        mock_websocket.application_state = WebSocketState.CONNECTED
        error_response = json.dumps({'type': 'error', 'error_code': 'MESSAGE_TOO_BIG', 'message': 'Message exceeds maximum size limit', 'close_code': 1009, 'details': {'max_size_bytes': 8192, 'received_size_bytes': 16384}, 'timestamp': datetime.now(timezone.utc).isoformat()})
        from websockets import ConnectionClosedError
        mock_websocket.recv = AsyncMock(side_effect=[error_response, ConnectionClosedError(None, None)])
        mock_websocket.send_json = AsyncMock()
        mock_connection = AsyncMock()
        ...
        with patch('websockets.connect', return_value=mock_connection):
            async with mock_connection as websocket:
                oversized_content = 'x' * 16384
                oversized_message = {'type': 'user_message', 'payload': {'content': oversized_content}}
    ...
    async def test_internal_error_close_code_1011(self):
        ...
        mock_websocket = AsyncMock()
        mock_websocket.application_state = WebSocketState.CONNECTED
        error_response = json.dumps({'type': 'error', 'error_code': 'INTERNAL_ERROR', 'message': 'An internal server error occurred', 'close_code': 1011, 'details': {'error_id': 'ERR_2025_001', 'suggestion': 'Please reconnect and try again'}, 'timestamp': datetime.now(timezone.utc).isoformat()})
        from websockets import ConnectionClosedError
        mock_websocket.recv = AsyncMock(side_effect=[error_response, ConnectionClosedError(None, None)])
        mock_websocket.send_json = AsyncMock()
        mock_connection = AsyncMock()
        ...
        with patch('websockets.connect', return_value=mock_connection):
            async with mock_connection as websocket:
                trigger_message = {'type': 'trigger_internal_error', 'payload': {'simulate': 'server_crash'}}
    ...
    async def test_clean_disconnect_sequence(self):
        ...
        mock_websocket = AsyncMock()
        mock_websocket.application_state = WebSocketState.CONNECTED
        welcome_message = json.dumps({'type': 'connection_established', 'connection_id': 'test-clean-disconnect'})
        ping_response = json.dumps({'type': 'pong', 'timestamp': datetime.now(timezone.utc).isoformat()})
        disconnect_confirmation = json.dumps({'type': 'disconnect_acknowledged', 'connection_id': 'test-clean-disconnect', 'cleanup_completed': True, 'resources_released': ['connection', 'heartbeat', 'message_queue'], 'timestamp': datetime.now(timezone.utc).isoformat()})
        from websockets import ConnectionClosed
        mock_websocket.recv = AsyncMock(side_effect=[welcome_message, ping_response, disconnect_confirmation, ConnectionClosed(None, None)])
        mock_websocket.send_json = AsyncMock()
        mock_websocket.close = AsyncMock()
        mock_connection = AsyncMock()
        mock_connection.__aenter__ = AsyncMock(return_value=mock_websocket)
        ...
        with patch('websockets.connect', return_value=mock_connection):
            async with mock_connection as websocket:
                ...
                disconnect_message = {'type': 'disconnect', 'reason': 'user_logout', 'cleanup_requested': True}
    ...
    async def test_unexpected_disconnection_handling(self):
        ...
        mock_websocket = AsyncMock()
        mock_websocket.application_state = WebSocketState.CONNECTED
        welcome_message = json.dumps({'type': 'connection_established', 'connection_id': 'test-unexpected-disconnect'})
        from websockets import ConnectionClosedError
        mock_websocket.recv = AsyncMock(side_effect=[welcome_message, ConnectionClosedError(None, None)])
        mock_websocket.send_json = AsyncMock()
        mock_connection = AsyncMock()
        mock_connection.__aenter__ = AsyncMock(return_value=mock_websocket)
        mock_connection.__aexit__ = AsyncMock(return_value=None)
        with patch('websockets.connect', return_value=mock_connection):
            async with mock_connection as websocket:
                ...
                welcome_data = json.loads(welcome)
                print(f" PASS:  Connected: {welcome_data['connection_id']}")
                test_message = {'type': 'user_message', 'payload': {'content': 'This message may not get a response'}}
