...
from shared.types.core_types import UserID, ThreadID, RunID, RequestID, SessionID, TokenString, WebSocketID, AgentID, ExecutionID, ensure_user_id, ensure_thread_id, AuthValidationResult, WebSocketMessage, WebSocketEventType, AgentExecutionContext, ExecutionContextState
...
try:
    from test_framework.ssot.e2e_auth_helper import create_test_user_with_auth, get_jwt_token_for_user, validate_jwt_token
...
class TestMultiUserIDIsolation(BaseIntegrationTest):
    ...
    async def test_concurrent_user_authentication_isolation(self, real_services_fixture):
        ...
        try:
            user1_auth = await self._create_authenticated_user(self.user1_email, self.user1_password, self.user1_id, auth_url)
            user2_auth = await self._create_authenticated_user(self.user2_email, self.user2_password, self.user2_id, auth_url)
            user3_auth = await self._create_authenticated_user(self.user3_email, self.user3_password, self.user3_id, auth_url)
            ...
            async def validate_user_token_isolation(user_auth, expected_user_id):
                ...
                async with aiohttp.ClientSession() as session:
                    ...
                    async with session.get(f'{backend_url}/api/v1/user/profile', headers=headers) as response:
                        if response.status == 200:
                            profile_data = await response.json()
                            profile_user_id = UserID(profile_data['user_id'])
                            assert profile_user_id == expected_user_id, f'Token granted access to wrong user. Expected: {expected_user_id}, Got: {profile_user_id}'
                            return profile_data
                        else:
                            return {'user_id': str(expected_user_id), 'valid': True}
            validation_results = await asyncio.gather(validate_user_token_isolation(user1_auth, self.user1_id), validate_user_token_isolation(user2_auth, self.user2_id), validate_user_token_isolation(user3_auth, self.user3_id), return_exceptions=True)
    ...
    async def test_concurrent_websocket_connections_isolation(self, real_services_fixture):
        ...
        try:
            ...
            async def create_websocket_connection(user_auth, user_id, expected_messages):
                ...
                try:
                    headers = {'Authorization': f"Bearer {user_auth['token']}"}
                    async with websockets.connect(websocket_endpoint, extra_headers=headers, timeout=10) as websocket:
                        test_message = {'type': 'agent_request', 'agent': 'triage_agent', 'message': expected_messages[0], 'user_id': str(user_id), 'thread_id': str(ThreadID(str(uuid.uuid4()))), 'request_id': str(RequestID(str(uuid.uuid4())))}
                        ...
                        while asyncio.get_event_loop().time() - start_time < timeout:
                            try:
                                ...
                                break
                        return {'user_id': user_id, 'messages': connection_messages, 'connection_successful': True}
                except Exception as e:
                    self.logger.warning(f'WebSocket connection failed for user {user_id}: {e}')
                    return {'user_id': user_id, 'messages': connection_messages, 'connection_successful': False, 'error': str(e)}
            user1_expected_messages = [self.user1_sensitive_message]
            user2_expected_messages = [self.user2_sensitive_message]
            connection_results = await asyncio.gather(create_websocket_connection(user1_auth, self.user1_id, user1_expected_messages), create_websocket_connection(user2_auth, self.user2_id, user2_expected_messages), return_exceptions=True)
            ...
            for message in user1_messages:
                if 'user_id' in message:
                    ...
                    assert user_id_in_message == self.user1_id, f'User 1 received message intended for different user: {user_id_in_message}'
                message_content = str(message)
                assert self.user2_sensitive_message not in message_content, "CRITICAL VIOLATION: User 1 received User 2's sensitive data"
            for message in user2_messages:
                if 'user_id' in message:
                    user_id_in_message = UserID(message['user_id'])
                    assert user_id_in_message == self.user2_id, f'User 2 received message intended for different user: {user_id_in_message}'
                message_content = str(message)
                assert self.user1_sensitive_message not in message_content, "CRITICAL VIOLATION: User 2 received User 1's sensitive data"
    ...
    async def test_concurrent_agent_execution_isolation(self, real_services_fixture):
        ...
        try:
            ...
            async def execute_agent_for_user(user_auth, user_id, agent_query, execution_context):
                ...
                execution_results = {'user_id': user_id, 'events': [], 'execution_successful': False, 'isolation_validated': False}
                try:
                    headers = {'Authorization': f"Bearer {user_auth['token']}"}
                    async with websockets.connect(websocket_endpoint, extra_headers=headers, timeout=10) as websocket:
                        agent_request = {'type': 'agent_request', 'agent': 'triage_agent', 'message': agent_query, 'user_id': str(execution_context['user_id']), 'thread_id': str(execution_context['thread_id']), 'run_id': str(execution_context['run_id']), 'request_id': str(execution_context['request_id']), 'execution_id': str(execution_context['execution_id'])}
                        ...
                        while asyncio.get_event_loop().time() - start_time < timeout:
                            try:
                                ...
                                if 'user_id' in event_data:
                                    ...
                                    assert event_user_id == user_id, f'Agent event has wrong user context. Expected: {user_id}, Got: {event_user_id}'
                        ...
                        execution_results['isolation_validated'] = len(received_events) >= 2
                        return execution_results
                except Exception as e:
                    execution_results['error'] = str(e)
                    self.logger.warning(f'Agent execution failed for user {user_id}: {e}')
                    return execution_results
            user1_context = {'user_id': self.user1_id, 'thread_id': self.user1_thread_id, 'run_id': self.user1_run_id, 'request_id': self.user1_request_id, 'execution_id': self.execution1_id}
            user2_context = {'user_id': self.user2_id, 'thread_id': self.user2_thread_id, 'run_id': self.user2_run_id, 'request_id': self.user2_request_id, 'execution_id': self.execution2_id}
            ...
            execution_results = await asyncio.gather(execute_agent_for_user(user1_auth, self.user1_id, user1_query, user1_context), execute_agent_for_user(user2_auth, self.user2_id, user2_query, user2_context), return_exceptions=True)
            ...
            assert self.user2_sensitive_message not in user1_all_content, "CRITICAL VIOLATION: User 1 agent execution contained User 2's sensitive data"
            assert self.user1_sensitive_message not in user2_all_content, "CRITICAL VIOLATION: User 2 agent execution contained User 1's sensitive data"
            ...
            for event in all_events:
                if all((key in event for key in ['type', 'user_id', 'thread_id'])):
                    websocket_msg = WebSocketMessage(event_type=WebSocketEventType(event['type']), user_id=event['user_id'], thread_id=event['thread_id'], request_id=event.get('request_id', str(uuid.uuid4())), data=event.get('data', {}))
    ...
    async def test_high_concurrency_user_isolation_stress(self, real_services_fixture):
        ...
        try:
            ...
            for i in range(concurrent_users):
                ...
                stress_test_users.append({'user_id': user_id, 'email': email, 'auth': user_auth, 'sensitive_data': f'CONFIDENTIAL-USER-{i}-{uuid.uuid4().hex}'})

            async def stress_user_operations(user_data, operation_count):
                """Execute multiple operations for a user under stress conditions."""
                user_results = {'user_id': user_data['user_id'], 'operations_completed': 0, 'isolation_violations': [], 'successful_operations': []}
                for op_num in range(operation_count):
                    try:
                        async with aiohttp.ClientSession() as session:
                            headers = {'Authorization': f"Bearer {user_data['auth']['token']}"}
                            test_data = {'user_data': user_data['sensitive_data'], 'operation': f'stress_op_{op_num}', 'timestamp': datetime.now(timezone.utc).isoformat()}
                            async with session.post(f'{backend_url}/api/v1/test/echo', json=test_data, headers=headers, timeout=aiohttp.ClientTimeout(total=10)) as response:
                                if response.status == 200:
                                    response_data = await response.json()
                                    if 'user_data' in response_data:
                                        response_content = str(response_data)
                                        for other_user in stress_test_users:
                                            if other_user['user_id'] != user_data['user_id'] and other_user['sensitive_data'] in response_content:
                                                user_results['isolation_violations'].append({'operation': f'http_op_{op_num}', 'violation': 'sensitive_data_leak', 'leaked_from': str(other_user['user_id'])})
                ...
                return user_results
            stress_results = await asyncio.gather(*[stress_user_operations(user_data, operations_per_user) for user_data in stress_test_users], return_exceptions=True)
            total_operations = 0
            total_violations = 0
            for i, result in enumerate(stress_results):
                ...
                assert len(result['isolation_violations']) == 0, f"CRITICAL VIOLATION: User {result['user_id']} experienced {len(result['isolation_violations'])} isolation violations: {result['isolation_violations']}"
            ...
            self.logger.info(f'High concurrency stress test passed: {total_operations} operations across {concurrent_users} users with {total_violations} violations')
        finally:
    ...
    async def _create_authenticated_user(self, email: str, password: str, user_id: UserID, auth_url: str) -> Dict[str, Any]:
        ...
        try:
            if AUTH_HELPER_AVAILABLE:
                ...
                return {'user_id': user_id, 'email': email, 'token': TokenString(auth_result['access_token']), 'auth_result': auth_result}
            else:
                async with aiohttp.ClientSession() as session:
                    create_payload = {'email': email, 'password': password, 'name': f"E2E Test User {email.split('@')[0]}"}
                    async with session.post(f'{auth_url}/auth/register', json=create_payload, timeout=aiohttp.ClientTimeout(total=10)) as response:
                        if response.status not in [200, 201, 409]:
                            error_text = await response.text()
                            raise RuntimeError(f'User creation failed: {response.status} - {error_text}')
                    auth_payload = {'email': email, 'password': password}
                    async with session.post(f'{auth_url}/auth/login', json=auth_payload, timeout=aiohttp.ClientTimeout(total=10)) as response:
                        if response.status != 200:
                            error_text = await response.text()
                            raise RuntimeError(f'Authentication failed: {response.status} - {error_text}')
                        auth_data = await response.json()
                        return {'user_id': user_id, 'email': email, 'token': TokenString(auth_data['access_token']), 'auth_result': auth_data}
        except Exception as e:
            self.logger.error(f'Failed to create authenticated user {email}: {e}')
            return {'user_id': user_id, 'email': email, 'token': TokenString(f'mock_token_{uuid.uuid4().hex}'), 'auth_result': {'access_token': f'mock_token_{uuid.uuid4().hex}'}}

    async def _delete_test_user(self, email: str, auth_url: str):
        """Delete test user for cleanup."""
        try:
            async with aiohttp.ClientSession() as session:
                async with session.delete(f'{auth_url}/auth/user', json={'email': email}, timeout=aiohttp.ClientTimeout(total=5)) as response:
    ...
    async def _cleanup_test_users(self, auth_url: str):
        ...
        test_emails = [self.user1_email, self.user2_email, self.user3_email]
