                ...
                while True:
                    try:
                        ...
                        if data.get('type') == 'memory_stress':
                            ...
                            await self.websocket_manager.send_to_user(user_id, {'type': 'memory_stress_response', 'payload': large_data, 'connection_id': connection_id})
                        elif data.get('type') == 'agent_state':
                            state = DeepAgentState(user_request=data.get('request', 'test'), chat_thread_id=thread_id, user_id=user_id, step_count=data.get('step', 1))
                            state.conversation_history = [{'role': 'user', 'content': 'x' * 5000} for _ in range(10)]
                            await self.websocket_manager.send_to_user(user_id, {'type': 'agent_state_response', 'state': state.model_dump(), 'connection_id': connection_id})
                        else:
                            await self.websocket_manager.send_to_user(user_id, {'type': 'echo', 'original': data, 'connection_id': connection_id})
...
class TestRealWebSocketMemoryManagement:
        ...
        for batch in range(5):
            ...
            for user_num in range(20):
                ...
                try:
                    client = RealWebSocketClient(url)
                    await client.connect()
                    await client.send_message({'type': 'memory_stress', 'iteration': user_num, 'batch': batch})
    ...
    async def test_rapid_connect_disconnect_cycles_real(self, real_websocket_server, memory_profiler):
        ...
        for cycle in range(cycles):
            user_id = f'cycle_user_{cycle % 10}'
            thread_id = f'cycle_thread_{cycle}'
            url = real_websocket_server.get_url(user_id, thread_id)
            client = RealWebSocketClient(url)
            try:
                await client.connect()
                await client.send_message({'type': 'rapid_cycle', 'cycle': cycle, 'data': 'x' * 500})
        ...
        memory_growth = memory_profiler.get_memory_growth()
        final_connections = real_websocket_server.connection_count
        logger.info(f'After {cycles} rapid cycles - Memory growth: {memory_growth:.2f} MB, Final connections: {final_connections}')
    ...
    async def test_concurrent_connection_operations_memory(self, real_websocket_server, memory_profiler):
        ...
        async def connect_disconnect_user(user_id: str, iterations: int):
            ...
            for i in range(iterations):
                ...
                try:
                    client = RealWebSocketClient(url)
                    await client.connect()
                    await client.send_message({'type': 'concurrent_test', 'user': user_id, 'iteration': i})
                    await asyncio.sleep(0.1)
                    await client.disconnect()
                except Exception as e:
        ...
        for i in range(10):
            ...
            task = connect_disconnect_user(user_id, 5)
            tasks.append(task)
        ...
        memory_growth = memory_profiler.get_memory_growth()
        final_connections = real_websocket_server.connection_count
        logger.info(f'Concurrent operations result - Connections: {final_connections}, Memory growth: {memory_growth:.2f} MB')
    ...
    async def test_websocket_state_consistency_real(self, real_websocket_server, memory_profiler):
        ...
        try:
            ...
            for i in range(1, len(clients), 2):
                try:
                    await clients[i].send_message({'type': 'state_consistency_test', 'index': i})
    ...
    async def test_agent_state_memory_management(self, real_websocket_server, memory_profiler):
        ...
        try:
            ...
            for step in range(20):
                message = {'type': 'agent_state', 'request': f'Processing step {step}', 'step': step}
...
class TestRealWebSocketResourceManagement:
    ...
    async def test_comprehensive_resource_cleanup_real(self, real_websocket_server, memory_profiler, real_database):
        ...
        try:
            for i in range(15):
                ...
                client = RealWebSocketClient(url)
                await client.connect()
                await client.send_message({'type': 'agent_state', 'request': f'Database operation for user {i}', 'step': i})
                clients_and_data.append({'client': client, 'user_id': user_id, 'thread_id': thread_id})
            initial_connections = real_websocket_server.connection_count
    ...
    async def test_extreme_load_memory_behavior_real(self, real_websocket_server, memory_profiler):
        ...
        try:
                ...
                for user_num in range(10):
                    ...
                    for conn_num in range(connections_per_user):
                        ...
                        try:
                            client = RealWebSocketClient(url)
                            await client.connect()
                            await client.send_message({'type': 'extreme_load', 'batch': batch, 'user': user_num, 'connection': conn_num})
            ...
            final_connections = real_websocket_server.connection_count
            logger.info(f'After extreme load cleanup - Connections: {final_connections}, Final memory growth: {final_memory_growth:.2f} MB')
            assert final_connections == 0, f'All connections should be cleaned up, {final_connections} remain'
            extreme_threshold = 100.0
            assert final_memory_growth < extreme_threshold, f'Memory growth {final_memory_growth:.2f} MB exceeds extreme load threshold {extreme_threshold} MB'
        except Exception as e:
