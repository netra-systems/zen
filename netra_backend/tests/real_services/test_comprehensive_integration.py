...
class ComprehensiveIntegrationTestServer:
    ...
    def __init__(self, host: str='127.0.0.1', port: int=0):
        ...
        self.performance_metrics = {'messages_processed': 0, 'agent_executions': 0, 'database_operations': 0, 'errors_handled': 0}

    async def initialize_services(self):
        """Initialize all real services."""
        env_manager = get_test_env_manager()
        env = env_manager.setup_test_environment(additional_vars={'USE_REAL_SERVICES': 'true', 'ENABLE_REAL_LLM_TESTING': 'true', 'DATABASE_URL': 'postgresql://netra:netra123@localhost:5432/netra_test'}, enable_real_llm=True)
    ...
    async def start_server(self) -> int:
        ...
        async def websocket_endpoint(websocket: WebSocket, user_id: str, thread_id: str):
            ...
            try:
                ...
                self.active_sessions[session_id] = {'user_id': user_id, 'thread_id': thread_id, 'connection_id': connection_id, 'connected_at': datetime.now(timezone.utc), 'messages_sent': 0, 'messages_received': 0}
                logger.info(f'User {user_id} connected to thread {thread_id}')
                while True:
                    try:
                        message = await websocket.receive_text()
                        data = json.loads(message)
                        self.message_history.append({'timestamp': datetime.now(timezone.utc).isoformat(), 'user_id': user_id, 'thread_id': thread_id, 'type': data.get('type', 'unknown'), 'message': data})
    ...
    async def _process_message(self, user_id: str, thread_id: str, message: Dict) -> Optional[Dict]:
        ...
        try:
            if message_type == 'chat_message':
                ...
                return {'type': 'integration_response', 'original': message, 'processed_by': 'comprehensive_integration_server', 'services_active': {'websocket': True, 'database': self.database_manager is not None, 'agents': self.agent_registry is not None}, 'timestamp': datetime.now(timezone.utc).isoformat()}
        except Exception as e:
            logger.error(f'Message processing error: {e}')
            self.performance_metrics['errors_handled'] += 1
            return {'type': 'error', 'error': str(e), 'message_type': message_type}
    ...
    async def _handle_chat_message(self, user_id: str, thread_id: str, message: Dict) -> Dict:
        ...
        agent_state = DeepAgentState(user_request=user_request, chat_thread_id=thread_id, user_id=user_id, step_count=0)
        ...
        if triage_agent:
            ...
            if result.success:
                return {'type': 'chat_response', 'content': result.agent_reasoning or 'Agent executed successfully', 'next_agent': result.next_agent, 'agent_state': result.updated_state.model_dump(), 'execution_time': time.time(), 'real_llm_used': True}
            else:
                return {'type': 'chat_error', 'error': result.error_message, 'content': 'I encountered an issue processing your request.'}
        else:
            return {'type': 'chat_response', 'content': f'Received: {user_request}', 'note': 'Agent system not available'}
    ...
    async def _handle_agent_request(self, user_id: str, thread_id: str, message: Dict) -> Dict:
        ...
        if not agent:
            return {'type': 'agent_error', 'error': f"Agent '{agent_name}' not found"}
        agent_state = DeepAgentState(user_request=user_request, chat_thread_id=thread_id, user_id=user_id, step_count=message.get('step', 0))
        self.performance_metrics['agent_executions'] += 1
        start_time = time.time()
        result = await agent.execute(agent_state)
        execution_time = time.time() - start_time
        return {'type': 'agent_result', 'agent': agent_name, 'success': result.success, 'reasoning': result.agent_reasoning, 'next_agent': result.next_agent, 'execution_time': execution_time, 'error': result.error_message if not result.success else None, 'state_updates': {'step_count': result.updated_state.step_count if result.updated_state else 0}}

    async def _handle_health_check(self) -> Dict:
        """Handle health check request."""
        health_status = {'type': 'health_check_response', 'timestamp': datetime.now(timezone.utc).isoformat(), 'services': {}}
        try:
            db_health = await self.database_manager.health_check()
            health_status['services']['database'] = {'status': db_health.get('status', 'unknown'), 'healthy': db_health.get('status') == 'healthy'}
            self.performance_metrics['database_operations'] += 1
        except Exception as e:
            health_status['services']['database'] = {'status': 'error', 'healthy': False, 'error': str(e)}
        health_status['services']['websocket'] = {'status': 'healthy', 'healthy': True, 'active_connections': len(self.websocket_manager.connections)}
        health_status['services']['agents'] = {'status': 'healthy', 'healthy': self.agent_registry is not None, 'available_agents': len(self.agent_registry._agents) if self.agent_registry else 0}
        all_healthy = all((service.get('healthy', False) for service in health_status['services'].values()))
    ...
    async def _handle_performance_test(self, message: Dict) -> Dict:
        ...
        if test_type == 'database_operations':
            ...
            try:
                ...
                return {'type': 'performance_result', 'test_type': test_type, 'success': False, 'error': str(e)}
        else:
            return {'type': 'performance_result', 'test_type': test_type, 'metrics': self.performance_metrics.copy(), 'active_sessions': len(self.active_sessions)}
    ...
    def get_performance_metrics(self) -> Dict[str, Any]:
        ...
        return {**self.performance_metrics, 'active_sessions': len(self.active_sessions), 'total_messages_in_history': len(self.message_history)}
...
class ComprehensiveIntegrationClient:
    ...
    def get_messages_by_type(self, message_type: str) -> List[Dict]:
        ...
        return [msg for msg in self.messages_received if msg['data'].get('type') == message_type]
...
class ComprehensiveIntegrationTests:
    ...
    async def test_full_system_health_check(self, integration_server):
        ...
        try:
            ...
            await client.send_message({'type': 'health_check', 'timestamp': datetime.now(timezone.utc).isoformat()})
            ...
            assert health_response['overall_status'] in ['healthy', 'degraded'], f"Should have valid overall status: {health_response['overall_status']}"
    ...
    async def test_end_to_end_chat_flow_with_real_llm(self, integration_server):
        ...
        try:
            await client.connect()
            await client.send_message({'type': 'chat_message', 'content': 'I need help analyzing some business data to find growth opportunities', 'user_id': user_id, 'thread_id': thread_id})
    ...
    async def test_multi_agent_orchestration_integration(self, integration_server):
        ...
        try:
            await client.connect()
            await client.send_message({'type': 'agent_request', 'agent': 'triage', 'request': 'I need a comprehensive analysis of customer retention data with actionable recommendations'})
            ...
            if next_agent:
                await client.send_message({'type': 'agent_request', 'agent': next_agent, 'request': 'Continue with the analysis as recommended by triage', 'step': 1})
    ...
    async def test_concurrent_user_sessions_integration(self, integration_server):
        ...
        try:
            ...
            for i, (client, user_id, thread_id) in enumerate(clients):
                message = {'type': 'chat_message', 'content': f'Concurrent test message from user {i}', 'user_id': user_id, 'thread_id': thread_id}
            ...
            assert successful_responses >= num_concurrent_users * 0.8, f'Should handle most concurrent users: {successful_responses}/{num_concurrent_users}'
            ...
            assert performance_metrics['messages_processed'] >= num_concurrent_users, 'Should have processed messages from concurrent users'
            assert performance_metrics['active_sessions'] >= 0, 'Should track active sessions'
    ...
    async def test_error_handling_and_recovery_integration(self, integration_server):
        ...
        try:
            ...
            await client.send_message({'type': 'agent_request', 'agent': 'nonexistent_agent', 'request': 'This should fail'})
            ...
            assert 'not found' in agent_error_response.get('error', '').lower(), 'Should identify missing agent'
            logger.info('Invalid agent request handled correctly')
            await client.send_message({'type': 'health_check', 'recovery_test': True})
            ...
            assert recovery_response.get('overall_status') in ['healthy', 'degraded'], 'System should be functional after error recovery'
        ...
        try:
            ...
            async def user_load_operations(client_info):
                ...
                for op in range(operations_per_user):
                    try:
                        if op % 3 == 0:
                            await client.send_message({'type': 'health_check', 'user': user_index, 'operation': op})
                        elif op % 3 == 1:
                            await client.send_message({'type': 'performance_test', 'test_type': 'database_operations', 'user': user_index, 'operation': op})
                        else:
                            await client.send_message({'type': 'chat_message', 'content': f'Load test message {op} from user {user_index}', 'user_id': user_id, 'thread_id': thread_id})
            ...
            assert successful_operations >= total_expected_operations * 0.7, f'Should complete most operations: {successful_operations}/{total_expected_operations}'
            assert successful_users >= num_users * 0.7, f'Most users should complete successfully: {successful_users}/{num_users}'
