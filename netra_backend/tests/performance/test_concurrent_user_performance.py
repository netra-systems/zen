...
class LoadTestResults:
    ...
    def meets_sla_requirements(self) -> bool:
        ...
        return self.success_rate >= 99.0 and self.avg_response_time <= 2.0 and (self.p95_response_time <= 5.0) and (self.error_rate <= 0.01)
...
class ConcurrentUserSimulator:
    ...
    async def simulate_user_session(self, user_id: str, session_duration: float, operations_per_minute: int) -> UserSession:
        """Simulate a single user session."""
        session = UserSession(user_id=user_id, session_id=str(uuid.uuid4()), connection_time=time.perf_counter())
    ...
    def _get_random_operation(self) -> str:
        ...
        import random
        operations = {'chat_message': 0.4, 'data_query': 0.25, 'file_upload': 0.1, 'system_status': 0.15, 'user_settings': 0.1}
    ...
    async def run_load_test(self, concurrent_users: int, test_duration: float, operations_per_minute: int=30) -> LoadTestResults:
    ...
    def _calculate_results(self, sessions: List[UserSession], test_duration: float, concurrent_users: int) -> LoadTestResults:
        """Calculate load test results from session data."""
        valid_sessions = [s for s in sessions if isinstance(s, UserSession)]
        if not valid_sessions:
            return LoadTestResults(total_users=concurrent_users, test_duration=test_duration, total_messages=0, successful_messages=0, failed_messages=0, avg_response_time=0.0, p95_response_time=0.0, p99_response_time=0.0, messages_per_second=0.0, error_rate=0.0, concurrent_peak=0, memory_peak_mb=0.0)
        ...
        return LoadTestResults(total_users=concurrent_users, test_duration=test_duration, total_messages=total_messages, successful_messages=successful_messages, failed_messages=failed_messages, avg_response_time=avg_response_time, p95_response_time=p95_response_time, p99_response_time=p99_response_time, messages_per_second=messages_per_second, error_rate=error_rate, concurrent_peak=getattr(self, 'peak_concurrent', concurrent_users), memory_peak_mb=0.0)
...
class ScalabilityTester:
    ...
    @pytest.mark.asyncio
    async def test_user_scaling(self, max_users: int=100, step_size: int=10, test_duration: float=30.0) -> Dict[int, LoadTestResults]:
        ...
        for user_count in range(step_size, max_users + 1, step_size):
            ...
            self.simulator = ConcurrentUserSimulator()
            result = await self.simulator.run_load_test(concurrent_users=user_count, test_duration=test_duration, operations_per_minute=20)
            results[user_count] = result
            await asyncio.sleep(1.0)
        return results

    @pytest.mark.asyncio
    async def test_burst_capacity(self, base_users: int=50, burst_users: int=200, burst_duration: float=10.0) -> Dict[str, Any]:
        """Test system response to sudden user burst."""
        base_task = asyncio.create_task(self.simulator.run_load_test(concurrent_users=base_users, test_duration=6.0, operations_per_minute=15))
        await asyncio.sleep(1.5)
        burst_simulator = ConcurrentUserSimulator()
        burst_task = asyncio.create_task(burst_simulator.run_load_test(concurrent_users=burst_users, test_duration=min(burst_duration, 3.0), operations_per_minute=30))
        burst_result = await burst_task
        base_result = await base_task
        return {'base_load': base_result, 'burst_load': burst_result, 'burst_impact': {'response_time_increase': burst_result.avg_response_time / max(base_result.avg_response_time, 0.001), 'error_rate_increase': burst_result.error_rate - base_result.error_rate, 'throughput_ratio': burst_result.messages_per_second / max(base_result.messages_per_second, 1)}}

    @pytest.mark.asyncio
    async def test_sustained_load(self, concurrent_users: int=75, duration_minutes: float=0.5) -> LoadTestResults:
        """Test system performance under sustained load."""
        duration_seconds = duration_minutes * 60.0
        result = await self.simulator.run_load_test(concurrent_users=concurrent_users, test_duration=duration_seconds, operations_per_minute=25)
        return result
...
class TestConcurrentUserPerformance:
    ...
    async def test_basic_concurrent_users(self):
        ...
        result = await self.simulator.run_load_test(concurrent_users=25, test_duration=20.0, operations_per_minute=20)
    ...
    async def test_medium_concurrent_load(self):
        ...
        result = await self.simulator.run_load_test(concurrent_users=50, test_duration=30.0, operations_per_minute=25)
    ...
    async def test_high_concurrent_load(self):
        ...
        result = await self.simulator.run_load_test(concurrent_users=100, test_duration=45.0, operations_per_minute=30)
    ...
    async def test_user_scaling_performance(self):
        ...
        scaling_results = await self.scalability_tester.test_user_scaling(max_users=80, step_size=20, test_duration=20.0)
        ...
        for i, user_count in enumerate(user_counts):
            ...
            if i > 0:
                ...
                assert response_time_ratio <= user_ratio * 2, f'Response time degraded too much: {response_time_ratio}x for {user_ratio}x users'
        print('\nScaling Results:')
    ...
    async def test_burst_capacity_handling(self):
        ...
        burst_results = await self.scalability_tester.test_burst_capacity(base_users=30, burst_users=120, burst_duration=15.0)
        ...
        assert impact['response_time_increase'] <= 5.0, f"Response time increased {impact['response_time_increase']}x during burst"
        assert impact['error_rate_increase'] <= 0.15, f"Error rate increased by {impact['error_rate_increase']} during burst"
    ...
    async def test_sustained_load_stability(self):
        ...
        result = await self.scalability_tester.test_sustained_load(concurrent_users=60, duration_minutes=3.0)
        ...
        expected_min_throughput = 60 * 25 / 60
        assert result.messages_per_second >= expected_min_throughput * 0.8, f'Throughput {result.messages_per_second} too low for sustained load'
    ...
    async def test_performance_degradation_thresholds(self):
        ...
        for users in load_levels:
            result = await self.simulator.run_load_test(concurrent_users=users, test_duration=15.0, operations_per_minute=20)
        ...
        for users, result in results.items():
            ...
            assert response_time_multiplier <= max_acceptable_degradation, f'{users} users: response time degraded {response_time_multiplier:.2f}x, acceptable max: {max_acceptable_degradation:.2f}x'
            if users <= 50:
            ...
            assert result.success_rate >= min_success_rate, f'{users} users: success rate {result.success_rate:.1f}% below {min_success_rate}%'
