...
from netra_backend.app.websocket_core import WebSocketManager, get_websocket_manager, MessageRouter, get_message_router
...
class StateMachineCoordinationTest(BaseIntegrationTest):
    ...
    def setUp(self):
        ...
        self.coordination_check_interval = 0.01
        self.race_condition_window = 0.5

    def _record_coordination_event(self, event_type: str, connection_id: str, user_id: str, component: str, coordination_success: bool=True, state_before: Optional[str]=None, state_after: Optional[str]=None, error: Optional[str]=None, metadata: Optional[Dict[str, Any]]=None):
        """Record coordination event for analysis."""
        event = CoordinationEvent(timestamp=time.time(), event_type=event_type, connection_id=connection_id, user_id=user_id, component=component, state_before=state_before, state_after=state_after, coordination_success=coordination_success, error=error, metadata=metadata or {})
    ...
    def test_state_machine_registry_coordination_race(self):
            ...
            while time.time() - start_time < monitoring_duration:
                try:
                    ...
                    if state_machine:
                        ...
                        expected_operational = individual_state in [ApplicationConnectionState.PROCESSING_READY, ApplicationConnectionState.PROCESSING, ApplicationConnectionState.IDLE, ApplicationConnectionState.DEGRADED]
                        if registry_operational != expected_operational:
                            mismatch = {'timestamp': time.time(), 'connection_id': connection_id, 'individual_state': individual_state.value, 'individual_ready': individual_ready, 'registry_operational': registry_operational, 'expected_operational': expected_operational, 'mismatch_type': 'registry_individual_state'}
                            state_machine_registry_mismatches.append(mismatch)
                            self.coordination_failures.append(f'Registry coordination mismatch: Registry={registry_operational}, Expected={expected_operational}, State={individual_state.value}')
                            self._record_coordination_event('coordination_mismatch', connection_id, user_data['user_id'], 'registry', coordination_success=False, state_before=individual_state.value, error='Registry-StateMachine coordination mismatch', metadata=mismatch)
                        coordination_checks.append({'timestamp': time.time(), 'individual_state': individual_state.value, 'registry_operational': registry_operational, 'coordination_ok': registry_operational == expected_operational})
                    else:
                        registry_coordination_issues.append({'timestamp': time.time(), 'connection_id': connection_id, 'issue': 'StateMachine not found in registry'})
                        self._record_coordination_event('registry_lookup_failure', connection_id, user_data['user_id'], 'registry', coordination_success=False, error='StateMachine not found in registry')
                    time.sleep(self.coordination_check_interval)
                except Exception as e:
                    registry_coordination_issues.append({'timestamp': time.time(), 'connection_id': connection_id, 'error': str(e)})
        ...
        def trigger_concurrent_state_transitions(connection_id: str, user_id: str):
            ...
            registry = get_connection_state_registry()
            try:
                state_machine = registry.register_connection(connection_id=connection_id, user_id=user_id)
                ...
                for i, target_state in enumerate(transitions):
                    success = state_machine.transition_to(target_state, reason=f'Coordination race test transition {i}', metadata={'transition_index': i, 'test': 'registry_coordination'})
                    self._record_coordination_event('state_transition', connection_id, user_id, 'state_machine', coordination_success=success, state_after=target_state.value, metadata={'transition_index': i, 'success': success})
                    time.sleep(0.02)
            except Exception as e:
                self.coordination_failures.append(f'State transition error: {e}')
        connection_id = f'coord_race_test_{int(time.time())}'
        monitoring_thread = threading.Thread(target=lambda: setattr(self, 'coordination_results', monitor_registry_coordination(connection_id, self.race_condition_window)), name='CoordinationMonitor')
        monitoring_thread.start()
        with self.client.websocket_connect(f'/ws?token={auth_token}', headers={'Authorization': f'Bearer {auth_token}'}) as websocket:
            transition_thread = threading.Thread(target=trigger_concurrent_state_transitions, args=(connection_id, user_data['user_id']), name='StateTransitionTrigger')
        ...
        if state_machine_registry_mismatches:
            ...
            for mismatch in state_machine_registry_mismatches[:3]:
                print(f"  - {mismatch['individual_state']} vs registry_op={mismatch['registry_operational']}")
        coordination_race_detected = len(state_machine_registry_mismatches) > 0 or len(registry_coordination_issues) > 2 or coordination_fail_count > coordination_ok_count or (len(self.coordination_failures) > 0)
        pytest.fail(f'Registry coordination race test completed. Race detected: {coordination_race_detected}, Mismatches: {len(state_machine_registry_mismatches)}, Issues: {len(registry_coordination_issues)}')
    ...
    def test_websocket_transport_state_machine_sync_race(self):
                    ...
                    if state_machine:
                        ...
                        if app_state not in [ApplicationConnectionState.CONNECTING, ApplicationConnectionState.FAILED, ApplicationConnectionState.CLOSED]:
                            if not transport_connected:
                                violation = {'timestamp': time.time(), 'connection_id': connection_id, 'app_state': app_state.value, 'app_ready': app_ready, 'transport_connected': transport_connected, 'violation_type': 'transport_behind_app_state'}
                                transport_sync_violations.append(violation)
                                self.synchronization_errors.append(f'Transport sync violation: App state {app_state.value} but transport not connected')
                                self._record_coordination_event('transport_sync_violation', connection_id, user_data['user_id'], 'transport', coordination_success=False, state_before=app_state.value, error='Transport behind application state', metadata=violation)
                        sync_checks.append({'timestamp': time.time(), 'app_state': app_state.value, 'app_ready': app_ready, 'transport_connected': transport_connected, 'sync_ok': True})
                    time.sleep(self.coordination_check_interval)
                except Exception as e:
                    accept_timing_issues.append({'timestamp': time.time(), 'connection_id': connection_id, 'error': str(e)})
            return sync_checks
        ...
        def simulate_accept_timing_race(connection_id: str, user_id: str, websocket_ref: Dict[str, Any]):
            ...
            try:
                ...
                async def slow_accept_simulation():
                    state_machine.transition_to(ApplicationConnectionState.ACCEPTED, reason='Simulated accept start')
                    await asyncio.sleep(0.1)
                    websocket_ref['transport_ready'] = True
                    state_machine.transition_to(ApplicationConnectionState.AUTHENTICATED, reason='Authentication completed')
                    state_machine.transition_to(ApplicationConnectionState.SERVICES_READY, reason='Services ready')
                    return True
                asyncio.run(slow_accept_simulation())
            except Exception as e:
                accept_timing_issues.append({'connection_id': connection_id, 'simulation_error': str(e), 'timestamp': time.time()})
        connection_id = f'transport_sync_test_{int(time.time())}'
        websocket_ref = {'websocket': None, 'transport_ready': False}
        sync_monitoring_thread = threading.Thread(target=lambda: setattr(self, 'sync_results', monitor_transport_state_sync(connection_id, websocket_ref)), name='TransportSyncMonitor')
        sync_monitoring_thread.start()
        with self.client.websocket_connect(f'/ws?token={auth_token}', headers={'Authorization': f'Bearer {auth_token}'}) as websocket:
            websocket_ref['websocket'] = websocket
            race_simulation_thread = threading.Thread(target=simulate_accept_timing_race, args=(connection_id, user_data['user_id'], websocket_ref), name='AcceptRaceSimulation')
        ...
        if transport_sync_violations:
            ...
            for violation in transport_sync_violations[:3]:
                print(f"  - App: {violation['app_state']}, Transport: {violation['transport_connected']}")
        sync_race_detected = sync_violations > 0 or timing_issues > 1 or len(self.synchronization_errors) > 0
        pytest.fail(f'Transport state sync race test completed. Race detected: {sync_race_detected}, Sync violations: {sync_violations}, Timing issues: {timing_issues}')
    ...
    def test_multi_component_coordination_race_condition(self):
        ...
        def monitor_all_components(connection_id: str, monitoring_duration: float):
            ...
            while time.time() - start_time < monitoring_duration:
                try:
                    ...
                    operational_connections = registry.get_all_operational_connections()
                    registry_operational = connection_id in operational_connections
                    components_ready = {'state_machine': sm_ready, 'ws_manager': ws_manager_ready, 'registry': registry_operational}
                    ready_states = list(components_ready.values())
                    if not all(ready_states) and any(ready_states):
                        violation = {'timestamp': timestamp, 'connection_id': connection_id, 'sm_state': sm_state, 'components_ready': components_ready, 'violation_type': 'partial_component_readiness'}
                        multi_component_violations.append(violation)
                        self.coordination_failures.append(f'Multi-component violation: {components_ready}')
                        self._record_coordination_event('multi_component_violation', connection_id, user_data['user_id'], 'multi_component', coordination_success=False, state_before=sm_state, error='Components have inconsistent readiness states', metadata=violation)
                    component_states.append({'timestamp': timestamp, 'sm_state': sm_state, 'sm_ready': sm_ready, 'ws_manager_ready': ws_manager_ready, 'registry_operational': registry_operational, 'coordination_ok': len(set(ready_states)) <= 1})
                    time.sleep(self.coordination_check_interval)
                except Exception as e:
                    coordination_chaos_events.append({'timestamp': time.time(), 'error': str(e), 'component': 'monitoring'})
        ...
        def create_coordination_chaos(connection_id: str, user_id: str):
            ...
            try:
                ...
                def rapid_state_transitions():
                    ...
                    for i in range(5):
                        target_states = [ApplicationConnectionState.ACCEPTED, ApplicationConnectionState.AUTHENTICATED, ApplicationConnectionState.SERVICES_READY, ApplicationConnectionState.PROCESSING_READY, ApplicationConnectionState.PROCESSING]
                        if i < len(target_states):
                            state_machine.transition_to(target_states[i], reason=f'Chaos transition {i}')
                        time.sleep(0.01)
                ...
                def registry_operations():
                    ...
                    for i in range(3):
                        try:
                            ...
                            coordination_chaos_events.append({'timestamp': time.time(), 'error': str(e), 'component': 'registry_ops'})
                with ThreadPoolExecutor(max_workers=3) as executor:
                    futures = [executor.submit(rapid_state_transitions), executor.submit(registry_operations)]
                    for future in as_completed(futures, timeout=5.0):
                        try:
                            future.result()
                        except Exception as e:
                            coordination_chaos_events.append({'timestamp': time.time(), 'error': str(e), 'component': 'chaos_execution'})
            except Exception as e:
                coordination_chaos_events.append({'timestamp': time.time(), 'error': str(e), 'component': 'chaos_setup'})
        connection_id = f'multi_coord_test_{int(time.time())}'
        monitoring_thread = threading.Thread(target=lambda: setattr(self, 'multi_coord_results', monitor_all_components(connection_id, self.race_condition_window)), name='MultiComponentMonitor')
        monitoring_thread.start()
        with self.client.websocket_connect(f'/ws?token={auth_token}', headers={'Authorization': f'Bearer {auth_token}'}) as websocket:
            chaos_thread = threading.Thread(target=create_coordination_chaos, args=(connection_id, user_data['user_id']), name='CoordinationChaos')
            chaos_thread.start()
            time.sleep(self.race_condition_window)
        ...
        if multi_component_violations:
            ...
            for violation in multi_component_violations[:3]:
                print(f"  - State: {violation['sm_state']}, Ready states: {violation['components_ready']}")
        multi_race_detected = coordination_violations > 0 or chaos_events > 3 or coordination_ok_checks < total_checks / 2 or (len(self.coordination_failures) > 0)
        pytest.fail(f'Multi-component coordination race test completed. Race detected: {multi_race_detected}, Violations: {coordination_violations}, Chaos events: {chaos_events}, Coordination success rate: {coordination_ok_checks}/{total_checks}')
