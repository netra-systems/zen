...
class HeartbeatMonitoringWebSocket:
        ...
        try:
            ...
            if message_type == 'ping' and self.simulate_heartbeat_loss:
                ...
                if random.random() < self.heartbeat_loss_rate:
                    ...
                    return
            self.messages_sent.append({'message': message, 'message_data': message_data, 'timestamp': datetime.now(UTC).isoformat(), 'type': message_type})
        except json.JSONDecodeError:
            self.messages_sent.append({'message': message, 'timestamp': datetime.now(UTC).isoformat(), 'type': 'raw'})
    ...
    def _record_health_event(self, event_type: str, description: str):
        ...
        event = {'event_type': event_type, 'description': description, 'timestamp': datetime.now(UTC).isoformat(), 'connection_id': self.connection_id, 'user_id': self.user_id}
    ...
    def get_heartbeat_metrics(self) -> HeartbeatMetrics:
        ...
        if self.timeout_count > 0:
            health_score -= min(self.timeout_count * 10, 50)
        if self.missed_heartbeat_count > 0:
            health_score -= min(self.missed_heartbeat_count * 5, 30)
        if avg_response_time > 1.0:
            health_score -= min((avg_response_time - 1.0) * 20, 20)
        health_score = max(health_score, 0.0)
        return HeartbeatMetrics(heartbeats_sent=heartbeats_sent, heartbeats_received=heartbeats_received, missed_heartbeats=self.missed_heartbeat_count, average_response_time=avg_response_time, max_response_time=max_response_time, timeout_events=self.timeout_count, reconnection_events=len([e for e in self.health_events if e['event_type'] == 'reconnection']), connection_uptime=connection_age, health_score=health_score)
...
class WebSocketHeartbeatMonitoringTests(SSotAsyncTestCase):
    ...
    def setup_method(self, method):
        ...
        self.test_user = TestUserData(user_id=f'heartbeat_user_{uuid.uuid4().hex[:8]}', email='heartbeat-test@netrasystems.ai', tier='enterprise', thread_id=f'heartbeat_thread_{uuid.uuid4().hex[:8]}')
    ...
    async def create_mock_user_context(self, user_data: TestUserData) -> Any:
        ...
        return type('MockUserContext', (), {'user_id': user_data.user_id, 'thread_id': user_data.thread_id, 'request_id': f'heartbeat_request_{uuid.uuid4().hex[:8]}', 'email': user_data.email, 'tier': user_data.tier, 'is_test': True})()
    ...
    async def test_regular_heartbeat_generation_and_monitoring(self):
        ...
        mock_ws.heartbeat_interval = 1.0
        ...
        with patch.object(manager, '_websocket_transport', mock_ws):
            ...
            while time.time() - start_time < test_duration:
                heartbeat_message = {'type': 'ping', 'timestamp': datetime.now(UTC).isoformat(), 'connection_id': connection_id}
    ...
    async def test_heartbeat_timeout_detection_and_recovery(self):
        ...
        mock_ws = HeartbeatMonitoringWebSocket(self.test_user.user_id, connection_id)
        mock_ws.heartbeat_timeout = 2.0
        mock_ws.simulate_heartbeat_loss = True
        mock_ws.heartbeat_loss_rate = 1.0
        self.mock_websockets.append(mock_ws)
        with patch.object(manager, '_websocket_transport', mock_ws):
            await manager.connect_user(user_id=ensure_user_id(self.test_user.user_id), websocket=mock_ws, connection_metadata={'tier': self.test_user.tier})
            timeout_detected = False
            for i in range(5):
                heartbeat_message = {'type': 'ping', 'timestamp': datetime.now(UTC).isoformat(), 'sequence': i}
                await mock_ws.send(json.dumps(heartbeat_message))
            ...
            recovery_message = {'type': 'ping', 'timestamp': datetime.now(UTC).isoformat(), 'recovery_attempt': True}
    ...
    async def test_connection_health_scoring_and_metrics(self):
        ...
        for scenario in test_scenarios:
            ...
            mock_ws = HeartbeatMonitoringWebSocket(self.test_user.user_id, connection_id)
            self.mock_websockets.append(mock_ws)
            mock_ws.simulate_network_issues(delay_ms=scenario['delay_ms'], loss_rate=scenario['loss_rate'])
            with patch.object(manager, '_websocket_transport', mock_ws):
                await manager.connect_user(user_id=ensure_user_id(self.test_user.user_id), websocket=mock_ws, connection_metadata={'tier': self.test_user.tier})
                for i in range(8):
                    heartbeat_message = {'type': 'ping', 'timestamp': datetime.now(UTC).isoformat(), 'scenario': scenario['name'], 'sequence': i}
                    try:
                        await mock_ws.send(json.dumps(heartbeat_message))
                    except Exception as e:
                ...
                metrics = mock_ws.get_heartbeat_metrics()
                assert metrics.health_score >= scenario['expected_min_health'], f"Health score too low for {scenario['name']}: {metrics.health_score:.1f}% (expected >={scenario['expected_min_health']}%)"
    ...
    async def test_circuit_breaker_activation_for_unhealthy_connections(self):
        ...
        mock_ws = HeartbeatMonitoringWebSocket(self.test_user.user_id, connection_id)
        mock_ws.max_missed_heartbeats = 2
        mock_ws.heartbeat_timeout = 1.0
        mock_ws.simulate_heartbeat_loss = True
        mock_ws.heartbeat_loss_rate = 1.0
        self.mock_websockets.append(mock_ws)
        with patch.object(manager, '_websocket_transport', mock_ws):
            ...
            for i in range(10):
                heartbeat_message = {'type': 'ping', 'timestamp': datetime.now(UTC).isoformat(), 'circuit_breaker_test': True, 'sequence': i}
                await mock_ws.send(json.dumps(heartbeat_message))
    ...
    async def test_performance_monitoring_under_various_network_conditions(self):
        ...
        for condition in network_conditions:
            ...
            mock_ws = HeartbeatMonitoringWebSocket(self.test_user.user_id, connection_id)
            self.mock_websockets.append(mock_ws)
            mock_ws.simulate_network_issues(delay_ms=condition['delay_ms'], loss_rate=condition['loss_rate'])
            with patch.object(manager, '_websocket_transport', mock_ws):
                await manager.connect_user(user_id=ensure_user_id(self.test_user.user_id), websocket=mock_ws, connection_metadata={'tier': self.test_user.tier})
                test_start = time.time()
                heartbeat_sent = 0
                while time.time() - test_start < 5:
                    heartbeat_message = {'type': 'ping', 'timestamp': datetime.now(UTC).isoformat(), 'network_condition': condition['name'], 'sequence': heartbeat_sent}
                    try:
                        await mock_ws.send(json.dumps(heartbeat_message))
                        heartbeat_sent += 1
                    except Exception:
                ...
                expected_avg_response = condition['delay_ms'] / 1000.0
                assert metrics.average_response_time <= expected_avg_response + 0.2, f"Response time higher than expected for {condition['name']}: {metrics.average_response_time:.3f}s"
