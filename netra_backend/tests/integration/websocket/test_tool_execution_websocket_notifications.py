...
class WebSocketEventCollector:
    ...
    def record_event(self, event_type: str, data: Dict[str, Any], metadata: Dict[str, Any]=None):
        ...
        event_record = {'event_type': event_type, 'data': data.copy(), 'metadata': metadata or {}, 'timestamp': time.time(), 'event_id': str(uuid.uuid4())}
    ...
    def validate_tool_execution_sequence(self, tool_name: str, user_id: str=None) -> Dict[str, Any]:
        ...
        validation_result = {'valid': True, 'issues': [], 'sequence_length': len(sequence), 'has_executing': False, 'has_completed': False, 'execution_time_ms': None}
        if len(sequence) == 0:
        ...
        if 'tool_completed' in event_types:
            ...
            if executing_event and completed_event:
                validation_result['execution_time_ms'] = (completed_event['timestamp'] - executing_event['timestamp']) * 1000
        else:
            validation_result['valid'] = False
...
class TestWebSocketToolExecutionNotifications(SSotAsyncTestCase):
    ...
    def setUp(self):
        ...
        self.user1_context = UserExecutionContext(user_id='websocket_user_001', run_id=f'ws_run_{int(time.time() * 1000)}', thread_id='ws_thread_001', session_id='ws_session_001')
        self.user2_context = UserExecutionContext(user_id='websocket_user_002', run_id=f'ws_run_{int(time.time() * 1000) + 1}', thread_id='ws_thread_002', session_id='ws_session_002')
        self.agent1_context = AgentExecutionContext(agent_name='WebSocketTestAgent1', run_id=self.user1_context.run_id, thread_id=self.user1_context.thread_id, user_id=self.user1_context.user_id)
        self.agent2_context = AgentExecutionContext(agent_name='WebSocketTestAgent2', run_id=self.user2_context.run_id, thread_id=self.user2_context.thread_id, user_id=self.user2_context.user_id)
    ...
    async def test_tool_execution_websocket_event_delivery(self):
        ...
        dispatcher = await UnifiedToolDispatcher.create_for_user(user_context=self.user1_context, websocket_bridge=self.websocket_manager, tools=[self.reporting_tool])
        self.event_collector.clear_events()
        result = await dispatcher.execute_tool('reporting_tool', {'report_type': 'quarterly_metrics'})
        self.assertTrue(result.success)
        validation = self.event_collector.validate_tool_execution_sequence('reporting_tool', self.user1_context.user_id)
    ...
    async def test_websocket_events_user_isolation(self):
        ...
        dispatcher1 = await UnifiedToolDispatcher.create_for_user(user_context=self.user1_context, websocket_bridge=self.websocket_manager, tools=[self.reporting_tool])
        dispatcher2 = await UnifiedToolDispatcher.create_for_user(user_context=self.user2_context, websocket_bridge=self.websocket_manager, tools=[self.reporting_tool])
        self.event_collector.clear_events()
    ...
    async def test_websocket_failure_during_tool_execution(self):
        ...
        result = await dispatcher.execute_tool('reporting_tool', {'report_type': 'failure_test_report'})
        self.assertTrue(result.success)
    ...
    async def test_websocket_reconnection_during_execution(self):
        ...
        execution_task = dispatcher.execute_tool('reporting_tool', {'report_type': 'reconnection_test'})
    ...
    async def test_websocket_intermittent_failures(self):
        ...
        tasks = []
        for i in range(5):
            task = dispatcher.execute_tool('reporting_tool', {'report_type': f'intermittent_test_{i}'})
            tasks.append(task)
    ...
    async def test_concurrent_websocket_event_delivery(self):
        ...
        for i in range(num_concurrent):
            context = UserExecutionContext(user_id=f'concurrent_ws_user_{i}', run_id=f'concurrent_ws_run_{i}', thread_id=f'concurrent_ws_thread_{i}', session_id=f'concurrent_ws_session_{i}')
            contexts.append(context)
            dispatcher = await UnifiedToolDispatcher.create_for_user(user_context=context, websocket_bridge=self.websocket_manager, tools=[self.reporting_tool])
            dispatchers.append(dispatcher)
        self.event_collector.clear_events()
        tasks = []
        for i, dispatcher in enumerate(dispatchers):
            task = dispatcher.execute_tool('reporting_tool', {'report_type': f'concurrent_report_{i}'})
            tasks.append(task)
        results = await asyncio.gather(*tasks, return_exceptions=True)
        success_count = sum((1 for r in results if not isinstance(r, Exception) and r.success))
    ...
    async def test_websocket_event_ordering(self):
        ...
        dispatcher = await UnifiedToolDispatcher.create_for_user(user_context=self.user1_context, websocket_bridge=self.websocket_manager, tools=[self.reporting_tool])
        self.event_collector.clear_events()
        result = await dispatcher.execute_tool('reporting_tool', {'report_type': 'ordering_test'})
        self.assertTrue(result.success)
        user_events = self.event_collector.get_events_for_user(self.user1_context.user_id)
        if len(user_events) >= 2:
            for i in range(1, len(user_events)):
                self.assertGreaterEqual(user_events[i]['timestamp'], user_events[i - 1]['timestamp'], 'Events should be in chronological order')
