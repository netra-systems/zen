...
class TestWebSocketPayloadValidation(SSotAsyncTestCase):
    ...
    def setup_method(self, method):
        ...
        self.test_user = TestUserData(user_id=f'payload_user_{uuid.uuid4().hex[:8]}', email='payload-test@netra.ai', tier='enterprise', thread_id=f'payload_thread_{uuid.uuid4().hex[:8]}')
