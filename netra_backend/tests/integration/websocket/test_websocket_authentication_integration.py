...
class TestWebSocketAuthenticationIntegration(SSotAsyncTestCase):
    ...
    async def test_authentication_context_extraction_validation(self):
        ...
        """
        test_user_data = TestUserData(user_id=f'context_user_{uuid.uuid4().hex[:8]}', email='context-test@netra.ai', tier='mid', thread_id=f'context_thread_{uuid.uuid4().hex[:8]}')
