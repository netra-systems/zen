...
class ErrorHandlingAgent(BaseAgent):
    ...
    async def execute(self, state: DeepAgentState, run_id: str, stream_updates: bool=True) -> Dict[str, Any]:
        ...
        try:
            await self.websocket_bridge.notify_agent_started(run_id, self.name, {'error_testing_mode': True, 'error_scenario': self.error_scenario, 'recovery_enabled': True})
            await self.websocket_bridge.notify_agent_thinking(run_id, self.name, 'Initializing error handling and recovery mechanisms...', step_number=1, progress_percentage=20)
            ...
            result = {'success': self.error_scenario in ['none', 'recovery_success'], 'agent_name': self.name, 'error_handling': {'scenario_tested': self.error_scenario, 'recovery_attempts': len(self.recovery_attempts), 'recovery_successful': len([r for r in self.recovery_attempts if r['success']]) > 0, 'error_transparency': True}, 'business_continuity': {'service_degradation': 'graceful' if self.error_scenario != 'critical_failure' else 'severe', 'user_experience': 'maintained' if self.error_scenario in ['none', 'recovery_success'] else 'impacted', 'data_integrity': 'preserved'}}
            await self.websocket_bridge.notify_agent_completed(run_id, self.name, result, execution_time_ms=150, error_handled=self.error_scenario != 'none')
            return result
    ...
    async def _handle_tool_failure_scenario(self, run_id: str):
        ...
        await self.websocket_bridge.notify_tool_executing(run_id, self.name, 'failing_tool', {'test_failure': True})
        await asyncio.sleep(0.05)
        failure_info = {'timestamp': datetime.now(timezone.utc), 'error_type': 'tool_execution_failure', 'recovery_strategy': 'retry_with_fallback', 'success': False}
        await self.websocket_bridge.notify_tool_completed(run_id, self.name, 'failing_tool', {'result': {'error': 'Tool execution failed', 'success': False}, 'recovery_attempted': True, 'fallback_used': True})
        await self.websocket_bridge.notify_agent_thinking(run_id, self.name, 'Attempting recovery with fallback tool...', step_number=2, progress_percentage=60)
        recovery_info = {**failure_info, 'success': True, 'recovery_method': 'fallback_tool'}
        self.recovery_attempts.append(recovery_info)
        await self.websocket_bridge.notify_tool_executing(run_id, self.name, 'fallback_tool', {'recovery': True})
        await self.websocket_bridge.notify_tool_completed(run_id, self.name, 'fallback_tool', {'result': {'success': True, 'data': 'recovery_successful'}, 'recovery_success': True})

    async def _handle_llm_timeout_scenario(self, run_id: str):
        """Simulate LLM timeout with graceful degradation."""
        await self.websocket_bridge.notify_agent_thinking(run_id, self.name, 'Executing complex LLM operation...', step_number=2, progress_percentage=40)
        await asyncio.sleep(0.1)
        recovery_attempt = {'timestamp': datetime.now(timezone.utc), 'error_type': 'llm_timeout', 'recovery_strategy': 'cached_response', 'success': True}
        self.recovery_attempts.append(recovery_attempt)
        await self.websocket_bridge.notify_agent_thinking(run_id, self.name, 'LLM timeout detected, using cached response for continuity...', step_number=3, progress_percentage=70)

    async def _handle_partial_failure_scenario(self, run_id: str):
        """Simulate partial failure with partial results."""
        await self.websocket_bridge.notify_tool_executing(run_id, self.name, 'success_tool', {'partial_test': True})
        await self.websocket_bridge.notify_tool_completed(run_id, self.name, 'success_tool', {'result': {'success': True, 'data': 'partial_result'}})
        await self.websocket_bridge.notify_tool_executing(run_id, self.name, 'partial_fail_tool', {'expected_failure': True})
        await self.websocket_bridge.notify_tool_completed(run_id, self.name, 'partial_fail_tool', {'result': {'error': 'Partial failure', 'success': False}, 'partial_results_available': True})

    async def _handle_recovery_success_scenario(self, run_id: str):
        """Simulate successful error recovery."""
        await self.websocket_bridge.notify_agent_thinking(run_id, self.name, 'Detected potential issue, implementing preventive recovery...', step_number=2, progress_percentage=50)
        recovery_info = {'timestamp': datetime.now(timezone.utc), 'error_type': 'preventive_recovery', 'recovery_strategy': 'proactive_handling', 'success': True}
        self.recovery_attempts.append(recovery_info)
    ...
    async def _execute_normal_path(self, run_id: str, recovery_mode: bool=False):
        ...
        await self.websocket_bridge.notify_tool_executing(run_id, self.name, tool_name, {'normal_execution': True})
        await self.websocket_bridge.notify_tool_completed(run_id, self.name, tool_name, {'result': {'success': True, 'data': 'normal_execution_complete'}, 'recovery_mode': recovery_mode})

    async def _handle_critical_error(self, run_id: str, error_msg: str):
        """Handle critical errors that cannot be recovered."""
        await self.websocket_bridge.notify_agent_error(run_id, self.name, error_msg, error_context={'critical': True, 'recovery_possible': False})
...
class ErrorHandlingEventCollector:
    ...
    async def create_error_handling_bridge(self, user_context: UserExecutionContext):
        ...
        async def track_error_event(event_type: str, run_id: str, agent_name: str, data: Any=None, **kwargs):
            ...
            return True
        bridge.notify_agent_started = AsyncMock(side_effect=lambda run_id, agent_name, context=None: track_error_event('agent_started', run_id, agent_name, context))
        bridge.notify_agent_thinking = AsyncMock(side_effect=lambda run_id, agent_name, thinking, **kwargs: track_error_event('agent_thinking', run_id, agent_name, {'reasoning': thinking}, **kwargs))
        bridge.notify_tool_executing = AsyncMock(side_effect=lambda run_id, agent_name, tool_name, params: track_error_event('tool_executing', run_id, agent_name, {'tool_name': tool_name, 'parameters': params}))
        bridge.notify_tool_completed = AsyncMock(side_effect=lambda run_id, agent_name, tool_name, result: track_error_event('tool_completed', run_id, agent_name, {'tool_name': tool_name, **result}))
        bridge.notify_agent_completed = AsyncMock(side_effect=lambda run_id, agent_name, result, **kwargs: track_error_event('agent_completed', run_id, agent_name, result, **kwargs))
        bridge.notify_agent_error = AsyncMock(side_effect=lambda run_id, agent_name, error, **kwargs: track_error_event('agent_error', run_id, agent_name, {'error': str(error)}, **kwargs))
        return bridge
...
class TestWebSocketAgentExecutionErrorHandlingRecoveryIntegration(BaseIntegrationTest):
    ...
    async def error_handling_user_context(self):
        return UserExecutionContext(user_id=f'error_user_{uuid.uuid4().hex[:8]}', thread_id=f'error_thread_{uuid.uuid4().hex[:8]}', run_id=f'error_run_{uuid.uuid4().hex[:8]}', request_id=f'error_req_{uuid.uuid4().hex[:8]}', metadata={'error_handling_test': True})

    @pytest.mark.integration
    @pytest.mark.real_services
    async def test_tool_failure_error_handling_with_websocket_recovery(self, error_handling_user_context, mock_llm_manager):
        ...
        execution_engine = ExecutionEngine._init_from_factory(registry=registry, websocket_bridge=websocket_bridge, user_context=error_handling_user_context)
        exec_context = AgentExecutionContext(user_id=error_handling_user_context.user_id, thread_id=error_handling_user_context.thread_id, run_id=error_handling_user_context.run_id, request_id=error_handling_user_context.request_id, agent_name='error_handler', step=PipelineStep.PROCESSING, execution_timestamp=datetime.now(timezone.utc), pipeline_step_num=1)
        agent_state = DeepAgentState(user_request={'test_error_handling': True}, user_id=error_handling_user_context.user_id, chat_thread_id=error_handling_user_context.thread_id, run_id=error_handling_user_context.run_id)
    ...
    @pytest.mark.integration
    @pytest.mark.real_services
    async def test_multiple_error_scenarios_comprehensive_recovery(self, error_handling_user_context, mock_llm_manager):
        ...
        for scenario in error_scenarios:
            ...
            websocket_bridge = await self.error_collector.create_error_handling_bridge(error_handling_user_context)
            execution_engine = ExecutionEngine._init_from_factory(registry=registry, websocket_bridge=websocket_bridge, user_context=error_handling_user_context)
            exec_context = AgentExecutionContext(user_id=error_handling_user_context.user_id, thread_id=error_handling_user_context.thread_id, run_id=f'{error_handling_user_context.run_id}_{scenario}', request_id=f'{error_handling_user_context.request_id}_{scenario}', agent_name=f'handler_{scenario}', step=PipelineStep.PROCESSING, execution_timestamp=datetime.now(timezone.utc), pipeline_step_num=1)
            agent_state = DeepAgentState(user_request={f'test_{scenario}': True}, user_id=error_handling_user_context.user_id, chat_thread_id=error_handling_user_context.thread_id, run_id=exec_context.run_id)
            result = await execution_engine.execute_agent(exec_context, error_handling_user_context)
        ...
        error_analysis = self.error_collector.analyze_error_handling()
        assert error_analysis['total_events'] >= len(error_scenarios) * 3
        self.logger.info(f" PASS:  Multiple error scenarios test PASSED - {len(error_scenarios)} scenarios, {error_analysis['total_events']} total events, {error_analysis['recovery_events']} recovery events")
