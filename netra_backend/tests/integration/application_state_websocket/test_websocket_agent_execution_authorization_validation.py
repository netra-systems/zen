...
class AuthorizedAgent(BaseAgent):
    ...
    async def execute(self, state: DeepAgentState, run_id: str, stream_updates: bool=True) -> Dict[str, Any]:
        auth_check = {'timestamp': datetime.now(timezone.utc), 'user_id': getattr(state, 'user_id', None), 'authorized': True, 'security_level': 'enterprise'}
        self.authorization_checks.append(auth_check)
        await self.websocket_bridge.notify_agent_started(run_id, self.name, {'authorization_verified': True, 'security_context': 'validated'})
        result = {'success': True, 'agent_name': self.name, 'authorization_validation': {'checks_performed': len(self.authorization_checks), 'all_authorized': all((check['authorized'] for check in self.authorization_checks)), 'security_compliance': 'verified'}}
...
class WebSocketAgentExecutionAuthorizationValidationTests(BaseIntegrationTest):
    ...
    async def authorized_user_context(self):
        return UserExecutionContext(user_id=f'auth_user_{uuid.uuid4().hex[:8]}', thread_id=f'auth_thread_{uuid.uuid4().hex[:8]}', run_id=f'auth_run_{uuid.uuid4().hex[:8]}', request_id=f'auth_req_{uuid.uuid4().hex[:8]}', metadata={'authorization_required': True, 'security_level': 'enterprise'})

    @pytest.mark.integration
    @pytest.mark.real_services
    async def test_agent_execution_authorization_validation_through_websocket(self, authorized_user_context, mock_llm_manager):
        ...
        async def track_auth_event(event_type, run_id, agent_name, data=None, **kwargs):
            ...
            return True
        bridge.notify_agent_started = AsyncMock(side_effect=lambda run_id, name, ctx: track_auth_event('agent_started', run_id, name, ctx))
        bridge.notify_agent_completed = AsyncMock(side_effect=lambda run_id, name, result, **kwargs: track_auth_event('agent_completed', run_id, name, result, **kwargs))
        execution_engine = ExecutionEngine._init_from_factory(registry=registry, websocket_bridge=bridge, user_context=authorized_user_context)
        result = await execution_engine.execute_agent(AgentExecutionContext(user_id=authorized_user_context.user_id, thread_id=authorized_user_context.thread_id, run_id=authorized_user_context.run_id, request_id=authorized_user_context.request_id, agent_name='secure_agent', step=PipelineStep.PROCESSING, execution_timestamp=datetime.now(timezone.utc), pipeline_step_num=1), authorized_user_context)
