...
class PydanticTestModel:
    ...
    def __init__(self, name: str, value: int, timestamp: datetime):
        ...
        self.nested_data = {'levels': [1, 2, 3], 'config': {'enabled': True, 'timeout': 30.5}}

    def model_dump(self, mode='python'):
        """Simulate Pydantic model_dump method."""
        data = {'name': self.name, 'value': self.value, 'timestamp': self.timestamp.isoformat() if mode == 'json' else self.timestamp, 'nested_data': self.nested_data}
        return data
...
class WebSocketTestStateSerializationIntegration(SSotBaseTestCase):
    ...
    async def asyncSetUp(self):
        ...
        for i in range(3):
            ...
            self.user_connections[user_id] = {'connection': connection, 'mock_websocket': mock_websocket}
    ...
    async def test_enum_serialization_websocket_state(self):
        ...
        """
        test_enums = [WebSocketTestState.CONNECTED, MockWebSocketState.CONNECTED, AgentStatus.ACTIVE, AgentStatus.COMPLETED]
        for enum_value in test_enums:
            with self.subTest(enum=enum_value):
                ...
                message = {'type': 'agent_status_update', 'status': enum_value, 'timestamp': datetime.now(timezone.utc)}
    ...
    async def test_complex_nested_object_serialization(self):
        ...
        """
        complex_data = ComplexNestedData(id=str(uuid.uuid4()), status=WebSocketTestState.CONNECTED, metadata={'agent_type': 'supervisor', 'capabilities': ['reasoning', 'tool_use'], 'config': {'timeout': 30.5, 'retries': 3, 'nested_status': WebSocketTestState.CONNECTING, 'timestamps': {'created': datetime.now(timezone.utc), 'modified': datetime.now(timezone.utc)}}}, timestamp=datetime.now(timezone.utc), nested_list=[{'type': 'event', 'status': WebSocketTestState.CONNECTED}, {'type': 'result', 'enum_field': AgentStatus.ACTIVE}])
        ...
        json_str = json.dumps(serialized)
        self.assertIsInstance(json_str, str)
        message = {'type': 'complex_agent_state', 'data': complex_data, 'meta': {'nested_enum': WebSocketTestState.CONNECTED, 'timestamp': datetime.now(timezone.utc)}}
    ...
    async def test_pydantic_model_serialization(self):
        ...
        """
        test_model = PydanticTestModel(name='test_agent', value=42, timestamp=datetime.now(timezone.utc))
        ...
        message = {'type': 'agent_result', 'agent_data': test_model, 'metadata': {'execution_time': 1.5, 'model_version': test_model}}
        safe_message = _serialize_message_safely(message)
    ...
    async def test_edge_case_serialization_scenarios(self):
        ...
        """
        message_with_nones = {'type': 'partial_data', 'result': None, 'optional_field': None, 'metadata': {'value': None, 'status': WebSocketTestState.CONNECTED}}
        serialized = _serialize_message_safely(message_with_nones)
        self.assertIsNone(serialized['result'])
        json.dumps(serialized)
        empty_collections = {'empty_list': [], 'empty_dict': {}, 'empty_set': set(), 'mixed': [[], {}, set()]}
        ...
        self.assertIsInstance(serialized['empty_set'], list)
        json.dumps(serialized)
        obj_with_method = ObjectWithToDict({'public_data': 'visible', 'nested': {'status': WebSocketTestState.CONNECTED}})
        ...
        json.dumps(serialized)
        large_nested = {'level_1': {'level_2': {'level_3': {'data': [{'item': i, 'status': WebSocketTestState.CONNECTED} for i in range(100)]}}}}
    ...
    async def test_websocket_state_enum_specific_handling(self):
        ...
        websocket_like_objects = [mock_starlette_state, mock_fastapi_state, MockWebSocketState.CONNECTED]
        for state_obj in websocket_like_objects:
            with self.subTest(state=state_obj):
                ...
                json.dumps(serialized)
                message = {'type': 'connection_status', 'websocket_state': state_obj, 'timestamp': datetime.now(timezone.utc)}
                safe_message = _serialize_message_safely(message)
                json.dumps(safe_message)
                await self.ws_manager.send_to_user('user_0', safe_message)
    ...
    async def test_concurrent_serialization_safety(self):
        ...
        messages = []
        for i in range(10):
            message = {'type': f'concurrent_test_{i}', 'data': ComplexNestedData(id=str(uuid.uuid4()), status=WebSocketTestState.CONNECTED, metadata={'index': i, 'enum': AgentStatus.ACTIVE}, timestamp=datetime.now(timezone.utc)), 'pydantic': PydanticTestModel(f'model_{i}', i, datetime.now(timezone.utc)), 'enum': WebSocketTestState.CONNECTING}
        ...
        tasks = [serialize_and_send(msg, f'user_{i % 3}') for i, msg in enumerate(messages)]
    ...
    async def test_serialization_fallback_mechanisms(self):
        ...
        json.dumps(serialized)
        message = {'type': 'fallback_test', 'unknown_object': unserializable, 'normal_data': {'status': WebSocketTestState.CONNECTED}}
        safe_message = _serialize_message_safely(message)
    ...
    async def test_memory_efficiency_large_objects(self):
        ...
        """
        large_data = {'type': 'memory_test', 'large_list': [{'index': i, 'status': WebSocketTestState.CONNECTED, 'data': f"data_{'x' * 100}_{i}", 'nested': {'timestamp': datetime.now(timezone.utc), 'metadata': {'item': j for j in range(10)}}} for i in range(1000)], 'enum_field': AgentStatus.ACTIVE}
        import psutil
        import os
