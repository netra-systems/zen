...
class TestAuthConfigOAuthDualNamingConvention:
    ...
    def setup_method(self):
        ...
        oauth_vars = ['GOOGLE_CLIENT_ID', 'GOOGLE_CLIENT_SECRET', 'GOOGLE_OAUTH_CLIENT_ID_STAGING', 'GOOGLE_OAUTH_CLIENT_SECRET_STAGING', 'GOOGLE_OAUTH_CLIENT_ID_PRODUCTION', 'GOOGLE_OAUTH_CLIENT_SECRET_PRODUCTION', 'GOOGLE_OAUTH_CLIENT_ID_DEVELOPMENT', 'GOOGLE_OAUTH_CLIENT_SECRET_DEVELOPMENT', 'OAUTH_GOOGLE_CLIENT_ID', 'OAUTH_GOOGLE_CLIENT_SECRET', 'GITHUB_CLIENT_ID', 'GITHUB_CLIENT_SECRET', 'OAUTH_GITHUB_CLIENT_ID', 'OAUTH_GITHUB_CLIENT_SECRET', 'ENVIRONMENT']
    ...
    def test_oauth_dual_naming_precedence_resolution(self):
        ...
        try:
            ...
            client_secret = auth_env.get_oauth_google_client_secret()
            if client_id:
                assert client_id in ['env_specific_staging_client_id', 'generic_oauth_client_id', 'simple_client_id', '']
            if client_secret:
                assert client_secret in ['env_specific_staging_secret', 'generic_oauth_secret', 'simple_secret', '']
        except Exception as e:
    ...
    def test_oauth_environment_isolation_security(self):
        ...
        try:
            ...
            for env_name in environments:
                ...
                credentials_by_env[env_name] = {'client_id': resolved_client_id, 'client_secret': resolved_secret}
...
class TestAuthServiceEnvironmentDetectionPortDiscovery:
    ...
    def setup_method(self):
        ...
        self.env = get_env()
        self.original_env_vars = {}
        env_vars = ['ENVIRONMENT', 'AUTH_SERVICE_PORT', 'AUTH_SERVICE_HOST', 'FRONTEND_URL', 'BACKEND_URL', 'AUTH_SERVICE_URL', 'DOCKER_ENVIRONMENT', 'CONTAINER_MODE']
        for var in env_vars:
            self.original_env_vars[var] = self.env.get(var)
    ...
    def test_environment_detection_accuracy(self):
        ...
        """
        try:
            test_environments = [('development', True, False, False, False), ('testing', False, False, True, False), ('staging', False, True, False, False), ('production', False, False, False, True)]
    ...
    def test_auth_service_port_discovery_integration(self):
        ...
        """
        try:
            port_configs = [('development', 8081, '0.0.0.0'), ('testing', 8082, '127.0.0.1'), ('staging', 8080, '0.0.0.0'), ('production', 8080, '0.0.0.0')]
    ...
    def test_docker_environment_detection(self):
        ...
        """
        try:
            docker_scenarios = [{'DOCKER_ENVIRONMENT': 'true', 'expected_docker': True}, {'CONTAINER_MODE': 'docker', 'expected_docker': True}, {'ENVIRONMENT': 'development', 'expected_docker': False}]
    ...
    def test_service_url_resolution_cross_environment(self):
        ...
        """
        try:
            services_config = [('frontend', 'get_frontend_url', ['http://localhost:3000', 'https://app.netrasystems.ai']), ('backend', 'get_backend_url', ['http://localhost:8000', 'https://api.netrasystems.ai']), ('auth', 'get_auth_service_url', ['http://localhost:8081', 'https://auth.netrasystems.ai'])]
...
class TestJWTSecretBackendIntegration:
    ...
    def setup_method(self):
        ...
        self.original_env_vars = {}
        jwt_vars = ['JWT_SECRET_KEY', 'JWT_SECRET', 'JWT_ALGORITHM', 'JWT_SECRET_DEVELOPMENT', 'JWT_SECRET_STAGING', 'JWT_SECRET_PRODUCTION', 'ENVIRONMENT']
        for var in jwt_vars:
            self.original_env_vars[var] = self.env.get(var)
    ...
    def test_jwt_websocket_authentication_config(self):
        ...
        try:
            ...
            self.env.set('JWT_ALGORITHM', 'HS256', 'test_setup')
            self.env.set('JWT_EXPIRATION_MINUTES', '30', 'test_setup')
            self.env.set('ENVIRONMENT', 'development', 'test_setup')
            self.jwt_manager.clear_cache()
            ...
            assert auth_secret == jwt_secret
            assert auth_algorithm in ['HS256', 'HS384', 'HS512']
            assert 5 <= auth_expiry <= 1440
            assert len(jwt_secret) >= 32
            refresh_expiry = auth_env.get_refresh_token_expiration_days()
            assert 1 <= refresh_expiry <= 365
        except Exception as e:
...
class TestDockerEnvironmentIntegration:
    ...
    def setup_method(self):
        ...
        self.env = get_env()
        self.original_env_vars = {}
        docker_vars = ['DOCKER_ENVIRONMENT', 'CONTAINER_MODE', 'K_SERVICE', 'GOOGLE_CLOUD_PROJECT', 'AUTH_SERVICE_HOST', 'AUTH_SERVICE_PORT', 'ENVIRONMENT']
    ...
    def test_docker_environment_detection_signals(self):
        ...
        """
        try:
            docker_scenarios = [{'name': 'explicit_docker_env', 'env_vars': {'DOCKER_ENVIRONMENT': 'true', 'ENVIRONMENT': 'development'}, 'expected_docker': True}, {'name': 'container_mode', 'env_vars': {'CONTAINER_MODE': 'docker', 'ENVIRONMENT': 'development'}, 'expected_docker': True}, {'name': 'cloud_run_k_service', 'env_vars': {'K_SERVICE': 'netra-auth-service', 'ENVIRONMENT': 'production'}, 'expected_docker': True}, {'name': 'local_development', 'env_vars': {'ENVIRONMENT': 'development'}, 'expected_docker': False}]
            for scenario in docker_scenarios:
    ...
    def test_cloud_run_deployment_configuration(self):
        ...
        try:
            ...
            self.env.set('ENVIRONMENT', 'staging', 'test_setup')
            self.env.set('PORT', '8080', 'test_setup')
            auth_env = get_auth_env()
            host = auth_env.get_auth_service_host()
            port = auth_env.get_auth_service_port()
            service_url = auth_env.get_auth_service_url()
    ...
    def test_docker_database_connectivity_config(self):
        ...
        """
        try:
            docker_db_scenarios = [{'name': 'docker_compose_postgres', 'env_vars': {'DOCKER_ENVIRONMENT': 'true', 'POSTGRES_HOST': 'postgres', 'POSTGRES_PORT': '5432', 'POSTGRES_DB': 'netra_auth', 'POSTGRES_USER': 'postgres', 'POSTGRES_PASSWORD': 'test_password', 'ENVIRONMENT': 'development'}}, {'name': 'cloud_sql_proxy', 'env_vars': {'K_SERVICE': 'netra-auth', 'POSTGRES_HOST': '127.0.0.1', 'POSTGRES_PORT': '5432', 'POSTGRES_DB': 'netra_staging', 'POSTGRES_USER': 'netra_user', 'POSTGRES_PASSWORD': 'secure_password', 'ENVIRONMENT': 'staging'}}]
            for scenario in docker_db_scenarios:
                for var in ['DOCKER_ENVIRONMENT', 'K_SERVICE', 'POSTGRES_HOST', 'POSTGRES_PORT', 'POSTGRES_DB', 'POSTGRES_USER', 'POSTGRES_PASSWORD']:
                    self.env.delete(var)
                for key, value in scenario['env_vars'].items():
                    self.env.set(key, value, 'test_setup')
                auth_env = get_auth_env()
                if scenario['env_vars']['ENVIRONMENT'] != 'testing':
    ...
    def test_docker_service_discovery_integration(self):
        ...
        try:
            ...
            for url in [frontend_url, backend_url, auth_url]:
                assert url.startswith(('http://', 'https://'))
            for url in [frontend_url, backend_url, auth_url]:
                assert 'localhost' in url or '127.0.0.1' in url or any((service in url for service in ['frontend', 'backend', 'auth']))
        except Exception as e:
...
class TestBackendAppConfigSchemaValidation:
    ...
    def setup_method(self):
        ...
        self.env = get_env()
        self.original_env_vars = {}
        backend_vars = ['ENVIRONMENT', 'DATABASE_URL', 'JWT_SECRET_KEY', 'SECRET_KEY', 'FRONTEND_URL', 'REDIS_URL', 'DEBUG', 'LOG_LEVEL']
    ...
    def test_app_config_environment_specific_loading(self):
        ...
        """
        try:
            environments = [('development', DevelopmentConfig), ('testing', NetraTestingConfig), ('staging', StagingConfig), ('production', ProductionConfig)]
    ...
    def test_app_config_database_integration(self):
        ...
        """
        try:
            db_configs = [{'name': 'postgresql_standard', 'url': 'postgresql://user:pass@localhost:5432/netra_dev', 'environment': 'development'}, {'name': 'postgresql_ssl', 'url': 'postgresql://user:pass@prod-db.example.com:5432/netra?sslmode=require', 'environment': 'production'}, {'name': 'sqlite_memory', 'url': 'sqlite:///:memory:', 'environment': 'testing'}]
    ...
    def test_app_config_websocket_support_validation(self):
        ...
        try:
            ...
            self.env.set('DEBUG', 'true', 'test_setup')
            config_manager = UnifiedConfigManager()
            config = config_manager.reload_config(force=True)
            assert isinstance(config, AppConfig)
            assert hasattr(config, 'environment')
            ...
            if hasattr(validation_result, 'errors'):
                critical_errors = [error for error in validation_result.errors if any((keyword in error.lower() for keyword in ['jwt', 'secret', 'auth', 'websocket']))]
    ...
    def test_app_config_agent_execution_support(self):
        ...
        try:
            ...
            validation_result = validator.validate_complete_config(config)
            if hasattr(validation_result, 'errors'):
                blocking_errors = [error for error in validation_result.errors if any((keyword in error.lower() for keyword in ['database', 'jwt', 'secret']))]
...
class TestConfigServiceAPIEndpoints:
    ...
    def setup_method(self):
        ...
        self.env = get_env()
        self.original_env_vars = {}
        api_vars = ['ENVIRONMENT', 'JWT_SECRET_KEY', 'SECRET_KEY', 'DATABASE_URL']
        for var in api_vars:
            self.original_env_vars[var] = self.env.get(var)
            ...
            try:
                ...
                for key in expected_keys:
                    if key in backup_result:
                        ...
                        if key == 'backup_id':
                            assert len(backup_result[key]) > 5
                        elif key == 'timestamp':
...
class TestConfigurationHotReloadCaching:
    ...
    def setup_method(self):
        ...
        self.jwt_manager = get_jwt_secret_manager()
        self.original_env_vars = {}
        cache_vars = ['ENVIRONMENT', 'DEBUG', 'LOG_LEVEL', 'JWT_SECRET_KEY', 'DATABASE_URL']
