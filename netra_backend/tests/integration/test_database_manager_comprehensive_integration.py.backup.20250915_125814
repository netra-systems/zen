...
class TestDatabaseManagerRealConnectionManagement(BaseIntegrationTest):
    ...
    async def test_real_database_connection_lifecycle(self, isolated_env):
        ...
        with patch('netra_backend.app.core.config.get_config') as mock_config, patch.object(DatabaseManager, '_get_database_url') as mock_get_url:
            ...
            async with db_manager.get_session() as session:
                await session.execute(text('INSERT INTO integration_users (email, username, subscription_tier, is_active) VALUES (:email, :username, :tier, :active)'), {'email': 'integration@netra.ai', 'username': 'integration_user', 'tier': 'premium', 'active': True})
    ...
    async def test_real_database_auto_initialization_safety(self, isolated_env):
        ...
        with patch('netra_backend.app.core.config.get_config') as mock_config, patch.object(DatabaseManager, '_get_database_url') as mock_get_url:
            ...
            await self._create_real_test_tables(engine)
            async with db_manager.get_session() as session:
                await session.execute(text('INSERT INTO integration_users (email, username) VALUES (:email, :username)'), {'email': 'auto_init@netra.ai', 'username': 'auto_init_user'})
                await session.commit()
                result = await session.execute(text('SELECT username FROM integration_users WHERE email = :email'), {'email': 'auto_init@netra.ai'})
...
class TestDatabaseManagerTransactionSafety(BaseIntegrationTest):
    ...
    async def test_real_transaction_commit_rollback_behavior(self, isolated_env):
        ...
        with patch('netra_backend.app.core.config.get_config') as mock_config, patch.object(DatabaseManager, '_get_database_url') as mock_get_url:
            ...
            await self._create_real_test_tables(engine)
            async with db_manager.get_session() as session:
                await session.execute(text('INSERT INTO integration_users (email, username, subscription_tier) VALUES (:email, :username, :tier)'), {'email': 'commit_test@netra.ai', 'username': 'commit_user', 'tier': 'enterprise'})
            async with db_manager.get_session() as session:
                result = await session.execute(text('SELECT subscription_tier FROM integration_users WHERE email = :email'), {'email': 'commit_test@netra.ai'})
                tier = result.scalar()
                assert tier == 'enterprise'
            rollback_occurred = False
            try:
                async with db_manager.get_session() as session:
                    await session.execute(text('INSERT INTO integration_users (email, username, subscription_tier) VALUES (:email, :username, :tier)'), {'email': 'rollback_test@netra.ai', 'username': 'rollback_user', 'tier': 'premium'})
                    raise ValueError('Simulated business logic error')
            except ValueError:
                rollback_occurred = True
            assert rollback_occurred
            async with db_manager.get_session() as session:
                result = await session.execute(text('SELECT COUNT(*) FROM integration_users WHERE email = :email'), {'email': 'rollback_test@netra.ai'})
                count = result.scalar()
                assert count == 0
            await db_manager.close_all()
    ...
    async def test_real_database_constraint_violation_handling(self, isolated_env):
        ...
        with patch('netra_backend.app.core.config.get_config') as mock_config, patch.object(DatabaseManager, '_get_database_url') as mock_get_url:
            ...
            await self._create_real_test_tables(engine)
            async with db_manager.get_session() as session:
                await session.execute(text('INSERT INTO integration_users (email, username, subscription_tier) VALUES (:email, :username, :tier)'), {'email': 'unique@netra.ai', 'username': 'unique_user', 'tier': 'premium'})
                await session.commit()
            constraint_violation_occurred = False
            try:
                async with db_manager.get_session() as session:
                    await session.execute(text('INSERT INTO integration_users (email, username, subscription_tier) VALUES (:email, :username, :tier)'), {'email': 'unique@netra.ai', 'username': 'duplicate_user', 'tier': 'free'})
                    await session.commit()
            ...
            async with db_manager.get_session() as session:
                result = await session.execute(text('SELECT username, subscription_tier FROM integration_users WHERE email = :email'), {'email': 'unique@netra.ai'})
                row = result.fetchone()
                assert row is not None
                assert row[0] == 'unique_user'
                assert row[1] == 'premium'
                result = await session.execute(text('SELECT COUNT(*) FROM integration_users WHERE email = :email'), {'email': 'unique@netra.ai'})
                count = result.scalar()
                assert count == 1
            await db_manager.close_all()
...
class TestDatabaseManagerMultiUserConcurrency(BaseIntegrationTest):
        ...
        with patch('netra_backend.app.core.config.get_config') as mock_config, patch.object(DatabaseManager, '_get_database_url') as mock_get_url:
            ...
            async with db_manager.get_session() as session:
                for i in range(num_users):
                    await session.execute(text('INSERT INTO integration_users (email, username, subscription_tier, is_active) VALUES (:email, :username, :tier, :active)'), {'email': f'bulk_user_{i}@netra.ai', 'username': f'bulk_user_{i}', 'tier': 'free', 'active': True})
                    for j in range(3):
    ...
    async def test_real_database_connection_resilience(self, isolated_env):
        ...
        with patch('netra_backend.app.core.config.get_config') as mock_config, patch.object(DatabaseManager, '_get_database_url') as mock_get_url:
            ...
            await self._create_real_test_tables(engine)
            async with db_manager.get_session() as session:
                await session.execute(text('INSERT INTO integration_users (email, username, subscription_tier) VALUES (:email, :username, :tier)'), {'email': 'resilience@netra.ai', 'username': 'resilience_user', 'tier': 'premium'})
                await session.commit()
            await db_manager.close_all()
            assert not db_manager._initialized
            async with db_manager.get_session() as session:
                result = await session.execute(text('SELECT username FROM integration_users WHERE email = :email'), {'email': 'resilience@netra.ai'})
                username = result.scalar()
