...
def mock_llm_manager():
    ...
    mock_llm.stress_metrics = {'total_requests': 0, 'concurrent_requests': 0, 'max_concurrent': 0, 'error_count': 0, 'avg_response_time': 0.0}
...
def stress_websocket_manager():
    ...
    ws_manager._stress_metrics = {'total_events': 0, 'events_per_second': 0, 'max_events_per_second': 0, 'failed_events': 0, 'backpressure_events': 0, 'last_second_events': deque(maxlen=1000)}
    ws_manager._event_buffer = deque(maxlen=10000)
    ...
    async def stress_send_event(*args, **kwargs):
        ...
        async with ws_manager._buffer_lock:
            ...
            if current_time - ws_manager._last_metrics_update >= 1.0:
                recent_events = [t for t in ws_manager._stress_metrics['last_second_events'] if current_time - t <= 1.0]
            ...
            ws_manager._event_buffer.append({'timestamp': current_time, 'event': event_data, 'thread_id': threading.get_ident()})
    ...
    ws_manager.send_to_thread = stress_send_event
    ws_manager.is_connected = Mock(return_value=True)
    ws_manager.get_connection_count = Mock(return_value=50)
    return ws_manager
...
def stress_test_user_contexts():
    ...
    for i in range(25):
        ...
        context = UserExecutionContext(user_id=f'stress_user_{i}_{uuid.uuid4().hex[:6]}', request_id=f'stress_req_{i}_{uuid.uuid4().hex[:6]}', thread_id=f'stress_thread_{i}_{uuid.uuid4().hex[:6]}', run_id=f'stress_run_{i}_{uuid.uuid4().hex[:6]}', agent_context={'user_tier': tier, 'stress_test': True, 'user_index': i, 'load_factor': random.uniform(0.5, 2.0), 'simulated_complexity': random.choice(['low', 'medium', 'high', 'extreme'])})
...
def mock_stress_test_agent():
    ...
    class MockStressTestAgent:
        ...
        def __init__(self, llm_manager=None, tool_dispatcher=None):
            ...
            self._stress_metrics = {'total_executions': 0, 'failed_executions': 0, 'avg_execution_time': 0.0, 'memory_usage': 0, 'cpu_usage': 0.0}
        ...
        async def execute(self, state, run_id):
            ...
            try:
                ...
                if self._websocket_bridge:
                    await self._websocket_bridge.notify_agent_started(run_id=self._run_id, agent_name=self._agent_name, context={'state': 'started', 'complexity': complexity, 'load_factor': load_factor, 'execution_count': self._stress_metrics['total_executions'], 'stress_test': True})
                ...
                for step in range(steps):
                    if self._websocket_bridge:
                        await self._websocket_bridge.notify_agent_thinking(run_id=self._run_id, agent_name=self._agent_name, reasoning=f'Processing step {step + 1}/{steps} - Complexity: {complexity}', step_number=step + 1, progress_percentage=(step + 1) / steps * 100)
                ...
                for i in range(tool_count):
                    ...
                    if self._websocket_bridge:
                        await self._websocket_bridge.notify_tool_executing(run_id=self._run_id, agent_name=self._agent_name, tool_name=tool_name, parameters={'input': str(state), 'complexity': complexity, 'tool_index': i, 'load_factor': load_factor})
                    tool_time = base_processing_time / tool_count
                    await asyncio.sleep(tool_time)
                    if self._websocket_bridge:
                        await self._websocket_bridge.notify_tool_completed(run_id=self._run_id, agent_name=self._agent_name, tool_name=tool_name, result={'output': f'processed_by_{tool_name}', 'complexity': complexity, 'processing_time': tool_time * 1000}, execution_time_ms=tool_time * 1000)
                ...
                result = {'status': 'success', 'data': f'Stress processed: {state}', 'complexity': complexity, 'load_factor': load_factor, 'execution_metrics': {'execution_time_ms': total_execution_time, 'steps_processed': steps, 'tools_used': tool_count, 'thread_id': threading.get_ident(), 'run_id': run_id}, 'stress_metrics': self._stress_metrics.copy()}
                if self._websocket_bridge:
                    await self._websocket_bridge.notify_agent_completed(run_id=self._run_id, agent_name=self._agent_name, result=result, execution_time_ms=total_execution_time)
                self.execution_history.append({'execution_id': f'{run_id}_{time.time()}', 'state': str(state), 'run_id': run_id, 'result': result, 'execution_time': total_execution_time, 'complexity': complexity, 'thread_id': threading.get_ident()})
                return result
            except Exception as e:
                self._stress_metrics['failed_executions'] += 1
                if self._websocket_bridge:
                    await self._websocket_bridge.notify_agent_error(run_id=self._run_id, agent_name=self._agent_name, error=str(e), context={'complexity': complexity, 'load_factor': load_factor, 'execution_time': (time.time() - execution_start) * 1000})
                raise
...
class TestHighLoadConcurrentStress(SSotBaseTestCase):
    ...
    @pytest.mark.asyncio
    async def test_25_concurrent_users_extreme_stress(self, mock_llm_manager, stress_websocket_manager, mock_stress_test_agent, stress_test_user_contexts):
        ...
        stress_metrics = {'total_users': 0, 'successful_executions': 0, 'failed_executions': 0, 'total_execution_time': 0.0, 'peak_memory_usage': 0, 'peak_websocket_events': 0, 'user_performance': {}}
        user_agents = []
        ...
        for i, context in enumerate(stress_test_user_contexts):
            ...
            created_agent = await registry.create_agent_for_user(user_id=context.user_id, agent_type=f'stress_agent_{i}', user_context=context, websocket_manager=stress_websocket_manager)
        ...
        for agent, context in user_agents:
            ...
            stress_input = type('StressInput', (), {'data': f'stress_data_{context.user_id}', 'complexity': complexity, 'load_factor': load_factor, 'user_index': context.agent_context['user_index']})()
        ...
        stress_metrics.update({'successful_executions': len(successful_results), 'failed_executions': len(failed_results), 'total_execution_time': total_stress_execution_time, 'peak_memory_usage': peak_memory, 'peak_websocket_events': peak_events, 'agent_creation_time': agent_creation_time})
        for i, (result, (agent, context)) in enumerate(zip(results, execution_tasks)):
            ...
            if not isinstance(result, Exception):
                ...
                stress_metrics['user_performance'][user_id] = {'success': True, 'execution_time_ms': execution_time, 'complexity': context.agent_context['simulated_complexity'], 'load_factor': context.agent_context['load_factor']}
            else:
                stress_metrics['user_performance'][user_id] = {'success': False, 'error': str(result), 'complexity': context.agent_context['simulated_complexity'], 'load_factor': context.agent_context['load_factor']}
    ...
    @pytest.mark.asyncio
    async def test_sustained_concurrent_load_memory_leak_detection(self, mock_llm_manager, stress_websocket_manager, mock_stress_test_agent, stress_test_user_contexts):
        ...
        for round_num in range(rounds):
            ...
            for i in range(users_per_round):
                ...
                agent = mock_stress_test_agent()
                registry.get_async = AsyncMock(return_value=agent)
                created_agent = await registry.create_agent_for_user(user_id=f'{context.user_id}_round_{round_num}', agent_type=f'leak_test_agent_{round_num}_{i}', user_context=context, websocket_manager=stress_websocket_manager)
    ...
    @pytest.mark.asyncio
    async def test_websocket_event_throughput_under_stress(self, mock_llm_manager, stress_websocket_manager, mock_stress_test_agent, stress_test_user_contexts):
        """Test WebSocket event throughput and delivery reliability under stress conditions."""
        registry = AgentRegistry(mock_llm_manager)
        registry.set_websocket_manager(stress_websocket_manager)
        throughput_metrics = {'events_generated': 0, 'events_processed': 0, 'max_throughput': 0, 'backpressure_events': 0, 'delivery_reliability': 0.0, 'event_latency_samples': []}
        stress_agents = []
        for i, context in enumerate(stress_test_user_contexts[:15]):
            agent = mock_stress_test_agent()
            registry.get_async = AsyncMock(return_value=agent)
            created_agent = await registry.create_agent_for_user(user_id=context.user_id, agent_type=f'websocket_stress_agent_{i}', user_context=context, websocket_manager=stress_websocket_manager)
        ...
        for agent, context in stress_agents:
            ...
            stress_input = type('WebSocketStressInput', (), {'data': f'websocket_stress_{context.user_id}', 'complexity': 'extreme', 'load_factor': 2.0})()
            task = agent.execute(stress_input, context.run_id)
            execution_tasks.append(task)
        ...
        throughput_metrics.update({'events_generated': total_events, 'events_processed': total_events - backpressure_events, 'backpressure_events': backpressure_events, 'delivery_reliability': (total_events - backpressure_events) / total_events if total_events > 0 else 0, 'avg_throughput': sum(throughput_samples) / len(throughput_samples) if throughput_samples else 0})
        successful_results = [r for r in results if not isinstance(r, Exception) and r.get('status') == 'success']
...
class TestStressErrorHandlingRecovery(SSotBaseTestCase):
    ...
    @pytest.mark.asyncio
    async def test_partial_failure_isolation_under_concurrent_stress(self, mock_llm_manager, stress_websocket_manager, mock_stress_test_agent, stress_test_user_contexts):
        """Test that partial failures are properly isolated during concurrent stress execution."""
        registry = AgentRegistry(mock_llm_manager)
        registry.set_websocket_manager(stress_websocket_manager)
        success_agents = []
        failure_agents = []
        for i, context in enumerate(stress_test_user_contexts[:20]):
            agent = mock_stress_test_agent()
            if i < 12:
                ...
                async def stress_failure_execute(state, run_id):
                    if agent._websocket_bridge:
                        await agent._websocket_bridge.notify_agent_started(run_id=agent._run_id, agent_name=agent._agent_name, context={'state': 'started', 'will_fail_under_stress': True})
                        await agent._websocket_bridge.notify_agent_thinking(run_id=agent._run_id, agent_name=agent._agent_name, reasoning='Processing under stress before failure', step_number=1, progress_percentage=25.0)
                    await asyncio.sleep(0.1)
        ...
        for agent, context in all_stress_agents:
            registry.get_async = AsyncMock(return_value=agent)
            created_agent = await registry.create_agent_for_user(user_id=context.user_id, agent_type='stress_failure_test_agent', user_context=context, websocket_manager=stress_websocket_manager)
    ...
    @pytest.mark.asyncio
    async def test_stress_recovery_after_transient_failures(self, mock_llm_manager, stress_websocket_manager, mock_stress_test_agent, stress_test_user_contexts):
        """Test system recovery after transient failures under stress conditions."""
        registry = AgentRegistry(mock_llm_manager)
        registry.set_websocket_manager(stress_websocket_manager)
        recovery_metrics = {'initial_failures': 0, 'recovery_attempts': 0, 'successful_recoveries': 0, 'persistent_failures': 0, 'recovery_time_samples': []}
        recovery_test_agents = []
        for i, context in enumerate(stress_test_user_contexts[:10]):
            agent = mock_stress_test_agent()
            ...
            async def transient_failure_execute(state, run_id):
                ...
                if agent._failure_count <= agent._max_failures:
                    ...
                    if agent._websocket_bridge:
                        await agent._websocket_bridge.notify_agent_started(run_id=agent._run_id, agent_name=agent._agent_name, context={'state': 'started', 'attempt': agent._failure_count, 'will_fail_transiently': True})
        ...
        for agent, context in recovery_test_agents:
            registry.get_async = AsyncMock(return_value=agent)
            created_agent = await registry.create_agent_for_user(user_id=context.user_id, agent_type='recovery_test_agent', user_context=context, websocket_manager=stress_websocket_manager)
