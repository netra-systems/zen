...
from netra_backend.app.agents.tool_dispatcher import UnifiedToolDispatcher, UnifiedToolDispatcherFactory, create_request_scoped_tool_dispatcher, ToolDispatchRequest, ToolDispatchResponse
...
class ToolDispatcherRealIntegrationTests(SSotBaseTestCase):
    ...
    def setup_method(self, method):
        ...
        self.user_context = create_isolated_user_context(user_id='integration_test_user', run_id='integration_test_run')
    ...
    async def test_factory_creates_real_isolated_dispatcher(self):
        ...
        """
        dispatcher = await UnifiedToolDispatcher.create_for_user(user_context=self.user_context, websocket_bridge=self.websocket_manager)
    ...
    async def test_scoped_context_manager_provides_real_cleanup(self):
        ...
        """
        dispatcher_id = None
        async with UnifiedToolDispatcher.create_scoped(user_context=self.user_context, websocket_bridge=self.websocket_manager) as dispatcher:
    ...
    async def test_real_tool_registration_and_execution(self):
        ...
        async with UnifiedToolDispatcher.create_scoped(user_context=self.user_context, websocket_bridge=self.websocket_manager, tools=[real_tool]) as dispatcher:
            assert dispatcher.has_tool('real_test_tool')
            result = await dispatcher.execute_tool('real_test_tool', {'test_param': 'integration_value'})
    ...
    async def test_real_error_handling_with_websocket_events(self):
        ...
        async with UnifiedToolDispatcher.create_scoped(user_context=self.user_context, websocket_bridge=self.websocket_manager, tools=[failing_tool]) as dispatcher:
    ...
    async def test_multiple_user_contexts_remain_isolated(self):
        ...
        dispatcher_1 = await UnifiedToolDispatcher.create_for_user(user_context=user_context_1, websocket_bridge=ws_manager_1, tools=[tool])
        dispatcher_2 = await UnifiedToolDispatcher.create_for_user(user_context=user_context_2, websocket_bridge=ws_manager_2, tools=[tool])
        try:
            result_1 = await dispatcher_1.execute_tool('isolation_tool', {'message': 'user_1_message'})
            result_2 = await dispatcher_2.execute_tool('isolation_tool', {'message': 'user_2_message'})
    ...
    async def test_concurrent_tool_execution_performance(self):
        ...
        async with UnifiedToolDispatcher.create_scoped(user_context=self.user_context, websocket_bridge=self.websocket_manager, tools=[perf_tool]) as dispatcher:
            ...
            for result in results:
                assert result.success
            assert len(self.emitted_events) >= 10
    ...
    async def test_real_state_based_execution(self):
        ...
        with patch.object(CoreDispatcher, '_init_from_factory') as mock_factory:
            ...
            mock_executor = Mock()
            mock_executor.execute_with_state = AsyncMock(return_value={'success': True, 'result': 'Stateful execution with context: state_data', 'metadata': {'state_used': True}})
            ...
            result = await core_dispatcher.dispatch_tool('stateful_tool', {'context_info': 'state_data'}, agent_state, 'test_run_id')
            mock_executor.execute_with_state.assert_called_once()
            call_args = mock_executor.execute_with_state.call_args[0]
            assert call_args[3] == agent_state
            assert call_args[4] == 'test_run_id'
            assert result.success
...
class ToolDispatcherFactoryIntegrationTests(SSotBaseTestCase):
    ...
    async def test_factory_manages_dispatcher_lifecycle(self):
        ...
        dispatcher = await factory.create_dispatcher(user_context=self.user_context)
    ...
    async def test_factory_with_tool_registry_integration(self):
        ...
        mock_tool_registry.list_tools.return_value = [Mock(id='registry_tool_1', name='Registry Tool 1', handler=AsyncMock()), Mock(id='registry_tool_2', name='Registry Tool 2', handler=AsyncMock())]
        factory = UnifiedToolDispatcherFactory()
...
class ToolDispatcherErrorScenarioIntegrationTests(SSotBaseTestCase):
    ...
    async def test_real_permission_validation_errors(self):
        ...
        """
        async with UnifiedToolDispatcher.create_scoped(user_context=self.user_context, enable_admin_tools=True) as dispatcher:
            try:
