...
class TestAgentThinkingEvents(SSotAsyncTestCase):
    ...
    async def create_authenticated_websocket_client(self, user_email: str=None) -> WebSocketTestClient:
        ...
        client = WebSocketTestClient(auth_token=auth_result.access_token, base_url='ws://localhost:8000')
    ...
    async def test_10_thinking_content_filtering(self):
        ...
        for event in thinking_events:
            ...
            assert not any((char in content for char in ['/var/', '/usr/', 'import ', 'class '])), 'Thinking events should not contain system code'
    ...
    async def test_12_thinking_interruption_scenarios(self):
        ...
        """
        client = await self.create_authenticated_websocket_client()
        await client.send_json({'type': 'agent_request', 'agent': 'triage_agent', 'message': 'Long analysis task', 'request_id': str(uuid.uuid4())})
        initial_events = []
        try:
            for _ in range(2):
    ...
    async def test_17_thinking_reliability_under_load(self):
        ...
        for i in range(3):
            ...
            task = asyncio.create_task(self.trigger_agent_thinking(client, f'Load test query {i}'))
            tasks.append(task)
    ...
    async def test_20_business_intelligence_from_thinking(self):
        ...
        thinking_events = [e for e in events if e.get('type') == 'agent_thinking']
        intelligence_metrics = {'total_thinking_events': len(thinking_events), 'avg_thinking_length': 0, 'reasoning_depth_score': 0, 'user_engagement_time': 0}
        if thinking_events:
            total_length = sum((len(e['data'].get('thought', e['data'].get('content', ''))) for e in thinking_events))
            ...
            depth_score = sum((sum((1 for word in reasoning_words if word in e['data'].get('thought', e['data'].get('content', '')).lower())) for e in thinking_events))
        ...
        has_actionable_data = intelligence_metrics['total_thinking_events'] > 0 or intelligence_metrics['avg_thinking_length'] > 10 or intelligence_metrics['reasoning_depth_score'] > 0
