...
class ToolCompletedEventsTests(BaseIntegrationTest):
    ...
    async def test_06_tool_error_completion_handling(self):
        ...
        for event in tool_events:
            ...
            if 'error' in data or 'failed' in str(data).lower():
                ...
                assert not any((tech_term in error_info.lower() for tech_term in ['traceback', 'stack', 'exception', 'null pointer'])), 'Errors should be user-friendly, not technical'
    ...
    async def test_11_tool_completion_performance_metrics(self):
        ...
        if tool_events:
            ...
            metrics = {'total_completions': len(tool_events), 'avg_completion_time': total_duration / len(tool_events), 'completion_rate': len(tool_events) / total_duration}
    ...
    async def test_12_tool_result_business_value_content(self):
        ...
        for event in tool_events:
            ...
            if result and isinstance(result, str) and (len(result) > 20):
                business_indicators = ['analysis', 'result', 'data', 'information', 'finding', 'recommendation', 'insight', 'conclusion', 'summary']
                ...
                is_technical_only = any((tech in result_lower for tech in technical_only)) and (not any((biz in result_lower for biz in business_indicators)))
    ...
    async def test_13_tool_completion_audit_trail(self):
        ...
        for event in tool_events:
            ...
            data = event['data']
            completion_id = data.get('tool_name') or data.get('tool') or data.get('operation') or data.get('task')
    ...
    async def test_15_concurrent_tool_completion_handling(self):
        ...
        tasks = []
        for i in range(3):
            task = asyncio.create_task(self.trigger_tool_completion(client, f'Concurrent completion task {i}'))
            tasks.append(task)
    ...
    async def test_18_tool_completion_business_intelligence(self):
        ...
        tool_events = [e for e in events if e.get('type') == 'tool_completed']
        bi_metrics = {'completion_count': len(tool_events), 'success_rate': 0, 'avg_result_quality': 0, 'tool_usage_patterns': {}}
        if tool_events:
            ...
            for event in tool_events:
                ...
                if isinstance(result, str):
                    quality = min(1.0, len(result.split()) / 10.0)
                    quality_scores.append(quality)
                tool_name = data.get('tool_name') or data.get('tool') or 'unknown'
                bi_metrics['tool_usage_patterns'][tool_name] = bi_metrics['tool_usage_patterns'].get(tool_name, 0) + 1
        ...
        has_actionable_data = bi_metrics['completion_count'] > 0 or bi_metrics['success_rate'] > 0 or len(bi_metrics['tool_usage_patterns']) > 0
    ...
    async def test_19_tool_completion_error_recovery(self):
        ...
        """
        client = await self.create_authenticated_websocket_client()
        test_scenarios = ['Process invalid data format', 'Access restricted resource', 'Perform complex calculation with edge cases']
        all_events = []
    ...
    async def test_20_tool_completion_end_to_end_value_delivery(self):
        ...
        end_time = time.time()
        tool_events = [e for e in events if e.get('type') == 'tool_completed']
        value_metrics = {'completion_time': end_time - start_time, 'tools_completed': len(tool_events), 'value_delivered': 0, 'user_satisfaction_indicators': 0, 'actionable_results': 0}
        ...
        if total_result_content:
            value_metrics['actionable_results'] = sum((1 for indicator in actionable_indicators if indicator in total_result_content))
            value_metrics['user_satisfaction_indicators'] = sum((1 for indicator in satisfaction_indicators if indicator in total_result_content))
            value_metrics['value_delivered'] = len(total_result_content) / 100.0
        ...
        delivers_value = value_metrics['actionable_results'] > 0 or value_metrics['user_satisfaction_indicators'] > 0 or value_metrics['value_delivered'] > 0.5
        assert delivers_value, 'Tool completions should deliver measurable business value'
        enables_satisfaction = value_metrics['tools_completed'] > 0 and value_metrics['completion_time'] > 0 and (len(total_result_content) > 10)
