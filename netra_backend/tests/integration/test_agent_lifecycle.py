...
class ComprehensiveLifecycleAgent(BaseAgent):
    ...
    async def _execute_with_user_context(self, context: UserExecutionContext, stream_updates: bool=False) -> Dict[str, Any]:
        ...
        try:
            ...
            result = {'success': True, 'agent_name': self.name, 'execution_id': execution_id, 'execution_count': self.execution_count, 'lifecycle_info': {'current_state': self.detailed_state.value, 'state_transitions': len(self.lifecycle_history), 'initialization_completed': self.initialization_time is not None, 'resources_managed': len(self.resource_allocations), 'total_processing_time': self.total_processing_time}, 'resource_management': {'allocated_resources': list(self.resource_allocations.keys()), 'cleanup_status': 'pending' if not self.cleanup_performed else 'completed', 'resource_efficiency': self._calculate_resource_efficiency()}, 'business_value': {'lifecycle_managed': True, 'resource_optimized': True, 'consistent_behavior': True, 'scalable_operations': True}}
    ...
    async def _perform_full_execution_lifecycle(self, execution_id: str, context: UserExecutionContext, stream_updates: bool=False):
        ...
        processing_tasks = [{'name': 'data_validation', 'duration': 0.015}, {'name': 'business_logic', 'duration': 0.025}, {'name': 'result_generation', 'duration': 0.02}]
        for task in processing_tasks:
            if stream_updates and self.has_websocket_context():
                await self.emit_thinking(f"Lifecycle-managed processing: {task['name']}")
                await self.emit_tool_executing(task['name'], {'lifecycle_stage': self.detailed_state.value, 'resource_managed': True})
            await self._perform_managed_task(task['name'], task['duration'])
            if stream_updates and self.has_websocket_context():
                await self.emit_tool_completed(task['name'], {'duration': task['duration'], 'lifecycle_managed': True, 'resources_utilized': list(self.resource_allocations.keys())})
    ...
    async def _perform_managed_task(self, task_name: str, duration: float):
        ...
        self.resource_allocations[resource_id] = {'task': task_name, 'allocated_at': datetime.now(timezone.utc).isoformat(), 'type': 'processing_resource'}
    ...
    async def _allocate_resources(self, context: UserExecutionContext):
        ...
        self.resource_allocations[memory_resource_id] = {'type': 'memory', 'size_mb': 64, 'allocated_at': datetime.now(timezone.utc).isoformat(), 'user_id': context.user_id}
        cpu_resource_id = f'cpu_{uuid.uuid4().hex[:8]}'
        self.resource_allocations[cpu_resource_id] = {'type': 'cpu', 'cores': 1, 'allocated_at': datetime.now(timezone.utc).isoformat(), 'user_id': context.user_id}
    ...
    async def _transition_to_stage(self, new_stage: LifecycleStage, description: str, stream_updates: bool=False):
        ...
        if self.lifecycle_manager:
            await self.lifecycle_manager.record_state_transition(self.name, previous_stage.value, new_stage.value, description)
        if stream_updates and self.has_websocket_context():
    ...
    def _record_lifecycle_event(self, stage: LifecycleStage, description: str):
        ...
        self.lifecycle_history.append({'stage': stage.value, 'description': description, 'timestamp': datetime.now(timezone.utc).isoformat(), 'execution_count': self.execution_count, 'resources_allocated': len(self.resource_allocations)})
    ...
    def get_lifecycle_summary(self) -> Dict[str, Any]:
        ...
        return {'agent_name': self.name, 'current_state': self.detailed_state.value, 'base_agent_state': self.state.value, 'total_executions': self.execution_count, 'lifecycle_events': len(self.lifecycle_history), 'initialization_time': self.initialization_time, 'total_processing_time': self.total_processing_time, 'resource_efficiency': self._calculate_resource_efficiency(), 'cleanup_performed': self.cleanup_performed, 'active_resources': len(self.resource_allocations), 'lifecycle_history': self.lifecycle_history[-5:] if self.lifecycle_history else []}
...
class MockLifecycleManager:
    ...
    async def initialize_agent_state(self, agent_name: str, user_id: str):
        ...
        state_data = {'agent_name': agent_name, 'user_id': user_id, 'initialized_at': datetime.now(timezone.utc).isoformat(), 'state': 'initialized'}
    ...
    async def record_state_transition(self, agent_name: str, from_state: str, to_state: str, description: str):
        ...
        transition_record = {'transition_id': transition_id, 'agent_name': agent_name, 'from_state': from_state, 'to_state': to_state, 'description': description, 'timestamp': datetime.now(timezone.utc).isoformat()}
        self.state_transitions[transition_id] = transition_record
        if self.redis_manager:
            try:
                cache_key = f'state_transition:{agent_name}:{transition_id}'
                await self.redis_manager.set_json(cache_key, transition_record, ex=1800)
            except Exception as e:
    ...
    async def record_cleanup(self, agent_name: str, cleanup_duration: float):
        ...
        self.operation_count += 1
        cleanup_record = {'agent_name': agent_name, 'cleanup_duration': cleanup_duration, 'completed_at': datetime.now(timezone.utc).isoformat()}
        self.cleanup_records[agent_name] = cleanup_record
        if self.redis_manager:
            try:
                cache_key = f'cleanup_record:{agent_name}'
                await self.redis_manager.set_json(cache_key, cleanup_record, ex=7200)
            except Exception as e:
...
class TestAgentLifecycle(BaseIntegrationTest):
    ...
    async def lifecycle_test_context(self):
        ...
        return UserExecutionContext(user_id=f'lifecycle_user_{uuid.uuid4().hex[:8]}', thread_id=f'lifecycle_thread_{uuid.uuid4().hex[:8]}', run_id=f'lifecycle_run_{uuid.uuid4().hex[:8]}', request_id=f'lifecycle_req_{uuid.uuid4().hex[:8]}', metadata={'user_request': 'Test comprehensive agent lifecycle management', 'lifecycle_test': True, 'requires_resource_management': True})
    ...
    async def test_complete_agent_lifecycle_with_persistence(self, real_services_fixture, lifecycle_agent, lifecycle_test_context, test_lifecycle_manager):
        ...
        assert result['lifecycle_info']['state_transitions'] >= 6
        resource_mgmt = result['resource_management']
        assert len(resource_mgmt['allocated_resources']) >= 2
        assert resource_mgmt['resource_efficiency'] > 0
        ...
        assert cleanup_summary['active_resources'] == 0
    ...
    async def test_lifecycle_state_transitions_validation(self, real_services_fixture, mock_llm_manager, test_lifecycle_manager, lifecycle_test_context):
        ...
        expected_stages = ['created', 'initializing', 'initialized', 'starting', 'running', 'processing', 'completing', 'completed']
    ...
    async def test_resource_management_and_cleanup(self, real_services_fixture, lifecycle_agent, lifecycle_test_context):
        ...
        assert len(resource_info['allocated_resources']) == 2
        ...
        assert post_cleanup_summary['active_resources'] == 0
        assert post_cleanup_summary['current_state'] == 'terminated'
        assert cleanup_time < 0.1
        await lifecycle_agent.cleanup_resources()
        final_summary = lifecycle_agent.get_lifecycle_summary()
        assert final_summary['active_resources'] == 0
    ...
    async def test_lifecycle_persistence_across_sessions(self, real_services_fixture, mock_llm_manager, test_lifecycle_manager):
        ...
        session1_context = UserExecutionContext(user_id='persistent_lifecycle_user', thread_id='persistent_lifecycle_thread', run_id='persistent_lifecycle_run_1', request_id='persistent_lifecycle_req_1', metadata={'session': 1, 'persistence_test': True})
        ...
        session2_context = UserExecutionContext(user_id='persistent_lifecycle_user', thread_id='persistent_lifecycle_thread', run_id='persistent_lifecycle_run_2', request_id='persistent_lifecycle_req_2', metadata={'session': 2, 'persistence_test': True})
    ...
    async def test_concurrent_agent_lifecycle_isolation(self, real_services_fixture, mock_llm_manager, test_lifecycle_manager):
        ...
        concurrent_agents = []
        for i in range(3):
            context = UserExecutionContext(user_id=f'concurrent_lifecycle_user_{i}', thread_id=f'concurrent_lifecycle_thread_{i}', run_id=f'concurrent_lifecycle_run_{i}', request_id=f'concurrent_lifecycle_req_{i}', metadata={'concurrent_test': True, 'agent_index': i})
    ...
    async def test_lifecycle_error_handling_and_recovery(self, real_services_fixture, mock_llm_manager, test_lifecycle_manager, lifecycle_test_context):
        ...
        manager_stats = test_lifecycle_manager.get_lifecycle_stats()
        assert manager_stats['state_transitions_recorded'] >= 1
    ...
    async def test_lifecycle_performance_optimization(self, real_services_fixture, mock_llm_manager, test_lifecycle_manager, lifecycle_test_context):
        ...
        for i in range(5):
            ...
            execution_time = time.time() - start_time
            performance_results.append({'iteration': i, 'execution_time': execution_time, 'state_transitions': result['lifecycle_info']['state_transitions'], 'resource_efficiency': result['resource_management']['resource_efficiency']})
        execution_times = [r['execution_time'] for r in performance_results]
        avg_execution_time = sum(execution_times) / len(execution_times)
        assert avg_execution_time < 0.2
        assert all((r['resource_efficiency'] > 0 for r in performance_results))
