...
async def authenticated_user(auth_helper):
    ...
    return await auth_helper.create_authenticated_user(email='datahelper_test@netra.ai', full_name='Data Helper Test User', permissions=['read', 'write', 'agent_execution'])
...
class TestDataHelperUserExecutionContext(SSotBaseTestCase):
    ...
    async def test_data_helper_user_context_isolation(self, real_services_fixture, data_helper_agent, mock_websocket_manager, auth_helper):
        ...
        """
        user1 = await auth_helper.create_authenticated_user(email='datahelper_user1@netra.ai', full_name='Data Helper User 1')
        user2 = await auth_helper.create_authenticated_user(email='datahelper_user2@netra.ai', full_name='Data Helper User 2')
    ...
    async def test_data_helper_concurrent_user_contexts(self, real_services_fixture, data_helper_agent, mock_websocket_manager, auth_helper):
        ...
        for i in range(num_concurrent_users):
            user = await auth_helper.create_authenticated_user(email=f'concurrent_user_{i}@netra.ai', full_name=f'Concurrent User {i}')
            users.append(user)
