...
class EnvironmentConfigurationComprehensiveTests(SSotBaseTestCase):
    ...
    def test_environment_variable_loading_and_isolation(self):
        ...
        try:
            ...
            assert 'password' not in env_dict['DATABASE_URL']
        finally:
    ...
    def test_database_url_configuration_validation(self):
        ...
        try:
            ...
            invalid_env.set('POSTGRES_HOST', '', source='test')
            invalid_env.set('POSTGRES_PORT', 'invalid_port', source='test')
    ...
    def test_jwt_secret_configuration_consistency(self):
        ...
        try:
            ...
            service_configs = {'backend': {'JWT_SECRET_KEY': secret1}, 'auth_service': {'JWT_SECRET_KEY': secret1}, 'frontend': {'JWT_SECRET_KEY': secret1}}
            consistency_result = consistency_validator.validate_consistency(service_configs)
            assert consistency_result.is_consistent, f'Consistent secrets marked as inconsistent: {consistency_result.inconsistencies}'
            inconsistent_configs = {'backend': {'JWT_SECRET_KEY': secret1}, 'auth_service': {'JWT_SECRET_KEY': 'different_secret_causing_auth_failures'}, 'frontend': {'JWT_SECRET_KEY': secret1}}
    ...
    def test_configuration_change_tracking_and_drift_detection(self):
        ...
        test_env = self.setup_test_isolation()
        try:
            initial_config = {'DATABASE_URL': 'postgresql://localhost:5432/initial_db', 'REDIS_URL': 'redis://localhost:6379/0', 'JWT_SECRET_KEY': 'initial_secret_key_32_chars_long', 'DEBUG_MODE': 'false', 'SERVICE_TIMEOUT': '30'}
            ...
            test_env.set('SERVICE_TIMEOUT', '60', source='update')
            test_env.set('DEBUG_MODE', 'true', source='update')
            ...
            assert not change_analysis.has_critical_changes
            ...
            critical_changes_after_rollback = [var for var in post_rollback_analysis.changed_variables if var in ['DATABASE_URL', 'JWT_SECRET_KEY']]
            assert len(critical_changes_after_rollback) == 0, f'Critical changes not rolled back: {critical_changes_after_rollback}'
            production_baseline = {'ENVIRONMENT': 'production', 'DATABASE_URL': 'postgresql://prod-db:5432/prod', 'DEBUG_MODE': 'false', 'LOG_LEVEL': 'INFO'}
            production_current = production_baseline.copy()
            production_current['DEBUG_MODE'] = 'true'
            production_current['LOG_LEVEL'] = 'DEBUG'
            prod_drift = change_tracker.analyze_environment_drift(production_baseline, production_current, environment='production')
            assert prod_drift.has_environment_violations
            assert 'DEBUG_MODE' in prod_drift.environment_violations
            assert prod_drift.severity == 'HIGH'
        finally:
            self.teardown_test_isolation()
    ...
    def test_multi_environment_configuration_scenarios(self):
        ...
        try:
            ...
            dev_config = {'ENVIRONMENT': 'development', 'DATABASE_URL': 'postgresql://dev:dev@localhost:5432/dev_db', 'REDIS_URL': 'redis://localhost:6379/0', 'DEBUG_MODE': 'true', 'LOG_LEVEL': 'DEBUG', 'CORS_ORIGINS': 'http://localhost:3000,http://localhost:8080', 'JWT_SECRET_KEY': 'dev_secret_key_for_local_testing_only', 'RATE_LIMITING': 'false'}
            ...
            for env_name, env_instance in environments.items():
                ...
                assert validation.is_valid, f'Environment {env_name} missing required variables: {validation.missing_variables}'
                if env_name in ['staging', 'production']:
                    assert env_instance.get('DEBUG_MODE') == 'false', f'Debug mode enabled in {env_name} - security risk'
                    assert env_instance.get('RATE_LIMITING') == 'true', f'Rate limiting disabled in {env_name} - security risk'
            ...
            for env_name, env_instance in environments.items():
                ...
                if env_name == 'development':
                    env_instance.set('MAX_CONNECTIONS', '10', source='dev_override')
                elif env_name == 'production':
                    env_instance.set('MAX_CONNECTIONS', '500', source='prod_override')
                    env_instance.set('DEFAULT_TIMEOUT', '60', source='prod_override')
    ...
    def test_configuration_security_and_sensitive_data_handling(self):
        ...
        try:
            ...
            for key in sensitive_config.keys():
                ...
                assert masked_dict[key] == '***MASKED***', f'Sensitive value not masked: {key}={masked_dict[key]}'
            for key, expected_value in non_sensitive_config.items():
                assert masked_dict[key] == expected_value, f'Non-sensitive value incorrectly masked: {key}'
            from shared.config_access_control import ConfigAccessControl
            access_control = ConfigAccessControl()
            authorized_result = access_control.get_sensitive_value(test_env, 'DATABASE_PASSWORD', requester='netra_backend.database_manager', authorization_token='valid_service_token')
            assert authorized_result.success
            assert authorized_result.value == 'super_secret_db_password_123'
            unauthorized_result = access_control.get_sensitive_value(test_env, 'DATABASE_PASSWORD', requester='unknown_service', authorization_token='invalid_token')
            ...
            for key, encrypted_value in encrypted_config.items():
                assert sensitive_config[key] not in encrypted_value, f'Original value visible in encrypted config: {key}'
                assert encrypted_value.startswith('ENC[') and encrypted_value.endswith(']'), f'Invalid encryption format: {key}={encrypted_value}'
            decrypted_config = encryptor.decrypt_sensitive_values(encrypted_config)
            for key, original_value in sensitive_config.items():
                assert decrypted_config[key] == original_value, f'Decryption failed for {key}: expected {original_value}, got {decrypted_config[key]}'
            from shared.config_audit_logger import ConfigAuditLogger
            audit_logger = ConfigAuditLogger()
            audit_logger.log_config_access(user_id='system', config_key='JWT_SECRET_KEY', access_type='READ', source_ip='127.0.0.1', result='SUCCESS')
            audit_logger.log_config_change(user_id='admin', config_key='DATABASE_PASSWORD', old_value='***MASKED***', new_value='***MASKED***', change_reason='Security rotation', source_ip='10.0.0.1')
            ...
            for log in audit_logs:
                ...
                for sensitive_value in sensitive_config.values():
                    assert sensitive_value not in log_text, f'Sensitive value leaked in audit log: {sensitive_value[:10]}...'
        finally:
            self.teardown_test_isolation()
