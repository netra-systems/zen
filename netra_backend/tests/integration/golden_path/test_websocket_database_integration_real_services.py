...
class WebSocketDatabaseIntegrationRealServicesTests(SSotBaseTestCase):
    """
    Integration tests for WebSocket + Database integration with real services.
    ...
    async def test_websocket_user_session_database_persistence(self):
        ...
        auth_result = await self.auth_helper.create_test_user_with_auth(email='ws_db_integration@example.com', user_id=user_id, permissions=['read', 'write', 'websocket'])
        jwt_token = auth_result['jwt_token']
        websocket_headers = self.auth_helper.get_websocket_headers(jwt_token)
        session_data = {'user_id': user_id, 'websocket_client_id': f'ws_client_{user_id}', 'connection_timestamp': datetime.now(timezone.utc), 'active_threads': ['thread_1', 'thread_2'], 'chat_history': [{'message': 'Hello, I need help with database optimization', 'timestamp': datetime.now(timezone.utc)}, {'message': "AI Agent: I'll help you optimize your database queries", 'timestamp': datetime.now(timezone.utc)}]}
    ...
    async def test_multi_user_websocket_database_isolation(self):
        ...
        """
        test_users = [{'user_id': 'multi_user_1', 'company': 'TechCorp', 'tier': 'enterprise'}, {'user_id': 'multi_user_2', 'company': 'StartupInc', 'tier': 'early'}, {'user_id': 'multi_user_3', 'company': 'FreeCorp', 'tier': 'free'}]
        stored_sessions = {}
        for user_info in test_users:
            user_id = user_info['user_id']
            auth_result = await self.auth_helper.create_test_user_with_auth(email=f"{user_id}@{user_info['company'].lower()}.com", user_id=user_id, permissions=['read', 'write'])
            session_data = {'user_id': user_id, 'websocket_client_id': f'ws_{user_id}', 'company_context': user_info['company'], 'user_tier': user_info['tier'], 'sensitive_data': f'confidential_{user_id}_data', 'active_threads': [f'thread_{user_id}_1', f'thread_{user_id}_2'], 'chat_messages': [f"Private message for {user_info['company']}"]}
            session_stored = await self._store_websocket_session_in_database(session_data)
    ...
    async def test_websocket_message_queue_database_integration(self):
        ...
        auth_result = await self.auth_helper.create_test_user_with_auth(email='message_queue@example.com', user_id=user_id, permissions=['read', 'write'])
        user_online = False
        queued_messages = [{'type': 'agent_started', 'payload': {'message': 'Your cost analysis agent is starting...'}, 'timestamp': datetime.now(timezone.utc), 'priority': 'high'}, {'type': 'agent_thinking', 'payload': {'message': 'Analyzing your billing data...', 'progress': 30}, 'timestamp': datetime.now(timezone.utc), 'priority': 'medium'}, {'type': 'agent_completed', 'payload': {'message': 'Analysis complete! Found $500/month in savings.', 'results': {'savings': 500}}, 'timestamp': datetime.now(timezone.utc), 'priority': 'high'}]
    ...
    async def test_websocket_session_recovery_database_consistency(self):
        ...
        auth_result = await self.auth_helper.create_test_user_with_auth(email='session_recovery@example.com', user_id=user_id, permissions=['read', 'write'])
        initial_session = {'user_id': user_id, 'websocket_client_id': f'ws_initial_{user_id}', 'active_operations': ['database_optimization'], 'operation_status': {'database_optimization': 'in_progress'}, 'partial_results': {'tables_analyzed': 5, 'recommendations': 2}, 'last_activity': datetime.now(timezone.utc)}
        session_stored = await self._store_websocket_session_in_database(initial_session)
        assert session_stored is True
        updated_session = {**initial_session, 'websocket_client_id': f'ws_recovered_{user_id}', 'operation_status': {'database_optimization': 'completed'}, 'partial_results': {'tables_analyzed': 15, 'recommendations': 8, 'total_savings': '$1,200'}, 'final_results': {'optimization_summary': 'Successfully optimized 15 tables with potential $1,200/month savings'}, 'last_activity': datetime.now(timezone.utc)}
    ...
    async def test_websocket_database_transaction_integrity(self):
        ...
        auth_result = await self.auth_helper.create_test_user_with_auth(email='transaction_integrity@example.com', user_id=user_id, permissions=['read', 'write'])
        operation_data = {'user_id': user_id, 'operation_type': 'cost_analysis', 'steps': [{'step': 'data_collection', 'status': 'completed', 'data': {'billing_records': 1500}}, {'step': 'analysis', 'status': 'in_progress', 'data': {'patterns_found': 12}}, {'step': 'optimization', 'status': 'pending', 'data': {}}, {'step': 'report_generation', 'status': 'pending', 'data': {}}], 'websocket_events': [{'event': 'operation_started', 'sent': True}, {'event': 'data_collected', 'sent': True}, {'event': 'analysis_progress', 'sent': False}, {'event': 'optimization_complete', 'sent': False}]}
    ...
    async def _store_websocket_session_in_database(self, session_data: Dict[str, Any]) -> bool:
        ...
        try:
            async with self.db_session() as session:
                ...
                await self.redis_client.setex(cache_key, 3600, session_json)
                return True
    ...
    async def _update_websocket_session_in_database(self, user_id: str, updated_session: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            await self.redis_client.setex(cache_key, 7200, session_json)
            return True
        except Exception as e:
    ...
    async def _perform_transactional_websocket_operation(self, operation_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            return {'success': True, 'all_steps_atomic': True, 'partial_failures': 0, 'rollback_required': False}
        except Exception as e:
            print(f'Error in transactional operation: {e}')
            return {'success': False, 'all_steps_atomic': False, 'partial_failures': 1, 'rollback_required': True}
