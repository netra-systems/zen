...
from test_framework.websocket_helpers import WebSocketTestHelpers, WebSocketTestClient, assert_websocket_events_sent, create_test_websocket_connection
from test_framework.ssot.e2e_auth_helper import create_authenticated_user_context, get_test_jwt_token, E2EAuthHelper
...
class WebSocketIntegrationTestSuite(WebSocketIntegrationTest):
    ...
    async def create_authenticated_websocket_connection(self, user_id: Optional[str]=None) -> tuple[Any, str, str]:
        """
        ...
        jwt_token = get_test_jwt_token(user_id=user_id, permissions=['read', 'write'], exp_minutes=30)
        headers = {'Authorization': f'Bearer {jwt_token}'}
        websocket_url = 'ws://localhost:8000/ws/chat'
        try:
            websocket = await WebSocketTestHelpers.create_test_websocket_connection(url=websocket_url, headers=headers, timeout=10.0, max_retries=3, user_id=user_id)
            return (websocket, user_id, jwt_token)
        except Exception as e:
            pytest.skip(f'Could not establish authenticated WebSocket connection: {e}')

    async def send_agent_execution_request(self, websocket: Any, user_id: str, agent_type: str='data_analyzer', request_data: Optional[Dict]=None) -> str:
        """
        ...
        if not request_data:
            request_data = {'prompt': 'Analyze the performance metrics for our Q3 infrastructure costs', 'context': 'quarterly_review'}
        agent_request = {'type': 'agent_request', 'user_id': user_id, 'thread_id': thread_id, 'agent_type': agent_type, 'request_data': request_data, 'timestamp': time.time()}
        await WebSocketTestHelpers.send_test_message(websocket, agent_request)
        return thread_id

    async def collect_websocket_events(self, websocket: Any, timeout: float=30.0, expected_event_count: int=5) -> List[Dict[str, Any]]:
        """
    ...
    async def test_websocket_event_delivery_all_critical_events(self, real_services: RealServicesManager):
        ...
        try:
            ...
            self.assert_business_value_delivered({'events': events, 'user_communication': True}, 'automation')
    ...
    async def test_multi_user_websocket_isolation_separate_channels(self, real_services: RealServicesManager):
        ...
        try:
            ...
            thread1_id = await self.send_agent_execution_request(ws1, user1_id, 'data_analyzer', {'prompt': 'User 1 analysis request', 'context': 'user1_context'})
            thread2_id = await self.send_agent_execution_request(ws2, user2_id, 'data_analyzer', {'prompt': 'User 2 analysis request', 'context': 'user2_context'})
            ...
            user1_events = []
            user2_events = []
            await asyncio.gather(collect_user_events(ws1, user1_id, user1_events), collect_user_events(ws2, user2_id, user2_events), return_exceptions=True)
    ...
    async def test_websocket_error_recovery_connection_resilience(self, real_services: RealServicesManager):
        ...
        try:
            ...
            error_scenarios = ['{"type": "invalid_json", "missing_quote: true}', {'type': 'non_existent_message_type', 'user_id': user_id}, {'type': 'agent_request'}, {'type': 'agent_request', 'user_id': 'invalid_user', 'thread_id': thread_id}]
            error_responses = []
            ...
            recovery_thread_id = await self.send_agent_execution_request(websocket, user_id, 'data_analyzer', {'prompt': 'Recovery test - analyze system health', 'context': 'recovery_test'})
    ...
    async def test_websocket_event_ordering_correct_sequence(self, real_services: RealServicesManager):
        ...
        try:
            ...
            ordering_rules = [('agent_started', 'agent_completed'), ('agent_started', 'agent_thinking'), ('tool_executing', 'tool_completed'), ('agent_thinking', 'tool_executing')]
            ...
            if len(event_timestamps) > 1:
                assert all((event_timestamps[i] <= event_timestamps[i + 1] for i in range(len(event_timestamps) - 1))), 'Event timestamps not in chronological order'
            ...
            assert all((receive_timestamps[i] <= receive_timestamps[i + 1] for i in range(len(receive_timestamps) - 1))), 'Events received out of chronological order'
    ...
    async def test_websocket_agent_bridge_integration_functionality(self, real_services: RealServicesManager):
        ...
        try:
            ...
            critical_events_found = [event_type for event_type in event_types if event_type in ['agent_started', 'agent_thinking', 'tool_executing', 'tool_completed', 'agent_completed']]
    ...
    async def test_websocket_authentication_integration_jwt_validation(self, real_services: RealServicesManager):
        ...
        try:
            ...
            invalid_scenarios = [get_test_jwt_token(user_id='expired_user', exp_minutes=-1), 'invalid.jwt.token', 'not_a_jwt_token_at_all']
            for i, invalid_token in enumerate(invalid_scenarios):
                try:
                    headers = {'Authorization': f'Bearer {invalid_token}'}
                    websocket_url = 'ws://localhost:8000/ws/chat'
                    try:
                        invalid_websocket = await WebSocketTestHelpers.create_test_websocket_connection(url=websocket_url, headers=headers, timeout=5.0, max_retries=1)
                        invalid_request = {'type': 'agent_request', 'user_id': 'unauthorized_user', 'request_data': {'prompt': 'This should be rejected'}}
            ...
            limited_token = get_test_jwt_token(user_id='limited_user', permissions=['read'])
    ...
    async def test_websocket_message_types_different_message_handling(self, real_services: RealServicesManager):
        ...
        websocket, user_id, jwt_token = await self.create_authenticated_websocket_connection()
        try:
            message_types_to_test = [{'type': 'agent_request', 'user_id': user_id, 'thread_id': UnifiedIdGenerator.generate_thread_id(user_id), 'agent_type': 'data_analyzer', 'request_data': {'prompt': 'Test standard agent request'}}, {'type': 'chat', 'user_id': user_id, 'message': 'Test chat message', 'timestamp': time.time()}, {'type': 'ping', 'user_id': user_id, 'timestamp': time.time()}, {'type': 'agent_status_request', 'user_id': user_id, 'thread_id': UnifiedIdGenerator.generate_thread_id(user_id)}]
            ...
            if 'ping' in responses_by_message_type:
                ...
                assert any((rt in ['pong', 'heartbeat_ack', 'ping'] for rt in ping_response_types)), 'Ping message should receive appropriate response'
            if 'agent_request' in responses_by_message_type:
                ...
                assert any((rt in ['agent_started', 'agent_thinking', 'agent_response', 'agent_progress'] for rt in agent_response_types)), 'Agent request should trigger agent events'
    ...
    async def test_websocket_connection_lifecycle_complete_flow(self, real_services: RealServicesManager):
        ...
        try:
            ...
            ping_message = {'type': 'ping', 'user_id': actual_user_id, 'timestamp': time.time()}
            ...
            operations = [{'name': 'agent_request_1', 'message': {'type': 'agent_request', 'user_id': actual_user_id, 'thread_id': UnifiedIdGenerator.generate_thread_id(actual_user_id), 'request_data': {'prompt': 'First request during lifecycle test'}}}, {'name': 'chat_message', 'message': {'type': 'chat', 'user_id': actual_user_id, 'message': 'Chat during lifecycle test', 'timestamp': time.time()}}, {'name': 'agent_request_2', 'message': {'type': 'agent_request', 'user_id': actual_user_id, 'thread_id': UnifiedIdGenerator.generate_thread_id(actual_user_id), 'request_data': {'prompt': 'Second request during lifecycle test'}}}]
            operation_results = {}
            ...
            health_ping = {'type': 'ping', 'user_id': actual_user_id, 'timestamp': time.time()}
    ...
    async def test_websocket_business_value_delivery_chat_functionality(self, real_services: RealServicesManager):
        ...
        websocket, user_id, jwt_token = await self.create_authenticated_websocket_connection()
        try:
            cost_analysis_request = {'type': 'agent_request', 'user_id': user_id, 'thread_id': UnifiedIdGenerator.generate_thread_id(user_id), 'agent_type': 'cost_optimizer', 'request_data': {'prompt': 'Analyze our Q3 infrastructure costs and identify optimization opportunities', 'context': 'quarterly_review', 'business_priority': 'cost_reduction'}}
            thread_id = cost_analysis_request['thread_id']
            await WebSocketTestHelpers.send_test_message(websocket, cost_analysis_request)
            chat_session_events = []
            business_value_indicators = {'agent_started': False, 'reasoning_shown': False, 'tools_used': False, 'results_delivered': False, 'session_completed': False}
                ...
                try:
                    ...
                    if event_type == 'agent_started':
                        ...
                        if isinstance(response_data, dict):
                            if any((keyword in str(response_data).lower() for keyword in ['cost', 'saving', 'optimization', 'reduction', 'efficiency'])):
                                business_value_indicators['session_completed'] = True
                        elif isinstance(response_data, str):
                            if any((keyword in response_data.lower() for keyword in ['cost', 'saving', 'optimization', 'reduction', 'efficiency'])):
                                business_value_indicators['session_completed'] = True
                except Exception:
                    continue
            if business_value_indicators['results_delivered']:
                followup_request = {'type': 'agent_request', 'user_id': user_id, 'thread_id': thread_id, 'request_data': {'prompt': 'Can you provide more details on the top 3 cost optimization recommendations?', 'context': 'followup_question'}}
            ...
            valuable_events = len([e for e in chat_session_events if e.get('type') in ['agent_thinking', 'tool_executing', 'agent_response', 'agent_completed']])
            ...
            business_value_data = {'chat_session_completed': True, 'interactions': total_interactions, 'valuable_events': valuable_events, 'session_duration': session_duration, 'business_context_maintained': True}
            ...
            await WebSocketTestHelpers.close_test_connection(websocket)
pytestmark = [pytest.mark.integration, pytest.mark.real_services, pytest.mark.timeout(120), pytest.mark.asyncio]
