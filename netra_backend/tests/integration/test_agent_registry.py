...
class SpecializedTestAgent(BaseAgent):
    ...
    def __init__(self, name: str, llm_manager: LLMManager, specialization: str, capabilities: List[str]):
        ...
        self.registry_metadata = {'agent_type': specialization, 'supported_capabilities': capabilities, 'performance_tier': 'standard', 'resource_requirements': {'memory_mb': 256, 'cpu_cores': 1}}
    ...
    def get_registry_metadata(self) -> Dict[str, Any]:
        ...
        return {**self.registry_metadata, 'name': self.name, 'specialization': self.specialization, 'capabilities': self.capabilities, 'state': self.state.value, 'initialization_count': self.initialization_count, 'execution_count': self.execution_count, 'last_updated': datetime.now(timezone.utc).isoformat()}
    ...
    async def _execute_with_user_context(self, context: UserExecutionContext, stream_updates: bool=False) -> Dict[str, Any]:
        ...
        return {'success': True, 'agent_name': self.name, 'specialization': self.specialization, 'capabilities_used': self.capabilities, 'execution_count': self.execution_count, 'specialized_result': specialized_result, 'registry_integration': {'initialized_from_registry': self.initialization_count > 0, 'registry_metadata': self.get_registry_metadata()}, 'business_value': {'specialization_applied': True, 'user_matched_to_expert': True, 'dynamic_dispatch_successful': True}}
    ...
    def _generate_specialized_result(self, context: UserExecutionContext) -> Dict[str, Any]:
        ...
        base_result = {'specialization': self.specialization, 'timestamp': datetime.now(timezone.utc).isoformat(), 'user_id': context.user_id, 'confidence_score': 0.88}
        if self.specialization == 'cost_optimization':
            return {**base_result, 'cost_analysis': {'potential_savings': '$12,400/month', 'optimization_opportunities': 8, 'implementation_timeline': '2-4 weeks'}, 'recommendations': ['Right-size overprovisioned instances', 'Implement auto-scaling policies', 'Optimize storage allocation']}
        elif self.specialization == 'performance_monitoring':
            return {**base_result, 'performance_analysis': {'latency_improvement': '35%', 'throughput_increase': '28%', 'error_rate_reduction': '67%'}, 'monitoring_recommendations': ['Deploy advanced APM tooling', 'Implement distributed tracing', 'Set up predictive alerting']}
        elif self.specialization == 'security_analysis':
            return {**base_result, 'security_assessment': {'vulnerabilities_identified': 5, 'risk_score': 'medium', 'compliance_status': '87%'}, 'security_recommendations': ['Update security policies', 'Implement zero-trust networking', 'Enhance access controls']}
        else:
            return {**base_result, 'generic_analysis': f'Applied {self.specialization} expertise', 'capabilities_demonstrated': self.capabilities}
...
class MockAgentRegistry(AgentRegistry):
    ...
    def _initialize_test_agents(self):
        ...
        self.registered_classes = {'cost_optimizer': (SpecializedTestAgent, {'specialization': 'cost_optimization', 'capabilities': ['cost_analysis', 'resource_optimization', 'spend_forecasting']}), 'performance_monitor': (SpecializedTestAgent, {'specialization': 'performance_monitoring', 'capabilities': ['latency_analysis', 'throughput_optimization', 'error_tracking']}), 'security_analyzer': (SpecializedTestAgent, {'specialization': 'security_analysis', 'capabilities': ['vulnerability_scanning', 'compliance_checking', 'threat_detection']}), 'data_processor': (SpecializedTestAgent, {'specialization': 'data_processing', 'capabilities': ['data_transformation', 'quality_validation', 'pipeline_optimization']})}
    ...
    async def _create_agent_instance(self, agent_name: str, agent_class: Type[BaseAgent], config: Dict[str, Any], context: Optional[UserExecutionContext]) -> Optional[BaseAgent]:
        """Create agent instance with dependency injection."""
        self.initialization_count += 1
        try:
            from unittest.mock import AsyncMock
            mock_llm = AsyncMock(spec=LLMManager)
            agent = agent_class(name=agent_name, llm_manager=mock_llm, specialization=config['specialization'], capabilities=config['capabilities'])
            registry_config = {'performance_tier': 'standard', 'resource_requirements': {'memory_mb': 512, 'cpu_cores': 2}}
...
class TestAgentRegistry(BaseIntegrationTest):
    ...
    async def registry_test_context(self):
        ...
        return UserExecutionContext(user_id=f'registry_user_{uuid.uuid4().hex[:8]}', thread_id=f'registry_thread_{uuid.uuid4().hex[:8]}', run_id=f'registry_run_{uuid.uuid4().hex[:8]}', request_id=f'registry_req_{uuid.uuid4().hex[:8]}', metadata={'user_request': 'I need expert AI assistance for cost optimization', 'registry_test': True, 'requires_specialization': 'cost_optimization'})
    ...
    async def test_agent_registration_with_real_services(self, real_services_fixture, test_agent_registry, mock_llm_manager):
        ...
        cost_agent = SpecializedTestAgent(name='test_cost_optimizer', llm_manager=mock_llm_manager, specialization='cost_optimization', capabilities=['cost_analysis', 'resource_optimization', 'budget_planning'])
    ...
    async def test_registry_caching_with_redis(self, real_services_fixture, test_agent_registry):
        ...
        assert final_stats['cache_hit_rate'] >= 0.5
    ...
    async def test_concurrent_agent_access(self, real_services_fixture, test_agent_registry, mock_llm_manager):
        ...
        contexts = []
        for i in range(5):
            context = UserExecutionContext(user_id=f'concurrent_user_{i}', thread_id=f'concurrent_thread_{i}', run_id=f'concurrent_run_{i}', request_id=f'concurrent_req_{i}', metadata={'concurrent_test': True, 'user_index': i})
        ...
        assert successful_retrievals >= 4
    ...
    async def test_agent_lifecycle_management(self, real_services_fixture, test_agent_registry, mock_llm_manager):
        ...
        lifecycle_agent = SpecializedTestAgent(name='lifecycle_test_agent', llm_manager=mock_llm_manager, specialization='data_processing', capabilities=['data_validation', 'transformation', 'quality_checks'])
        ...
        context = UserExecutionContext(user_id='lifecycle_user', thread_id='lifecycle_thread', run_id='lifecycle_run', request_id='lifecycle_req', metadata={'lifecycle_test': True})
        ...
        assert same_agent.execution_count >= 1
        final_stats = test_agent_registry.get_registry_stats()
    ...
    async def test_registry_error_handling_and_recovery(self, real_services_fixture, test_agent_registry):
        ...
        assert stats_after['active_agents'] >= stats_before['active_agents']
    ...
    async def test_multi_agent_workflow_coordination(self, real_services_fixture, test_agent_registry):
        ...
        workflow_context = UserExecutionContext(user_id='workflow_user', thread_id='workflow_thread', run_id='workflow_run', request_id='workflow_req', metadata={'workflow_request': 'Comprehensive infrastructure optimization', 'requires_multiple_agents': True, 'agent_sequence': ['cost_optimizer', 'performance_monitor', 'security_analyzer']})
        ...
        final_stats = test_agent_registry.get_registry_stats()
        assert final_stats['active_agents'] >= 3
        assert final_stats['initialization_count'] >= 3
