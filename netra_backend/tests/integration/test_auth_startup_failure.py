...
async def test_startup_with_missing_jwt_secret():
    ...
    try:
        ...
        if "JWT secret" in str(e) or "jwt_secret" in str(e):
            print("PASS: Correct error - JWT secret validation failure detected")
            return True
        else:
...
async def test_startup_with_invalid_auth_service_url():
    ...
    try:
        ...
        # Verify it's the right error (HTTPS or other auth failure)
        if "HTTPS" in str(e) or "auth" in str(e).lower():
            print("PASS: Correct error - Auth validation prevented startup")
            return True
        else:
            print(f"WARNING: Unexpected error type: {e}")
            return False
...
async def test_startup_orchestrator_integration():
    ...
    try:
        ...
        if "DeterministicStartupError" in str(type(e)):
            print("PASS: Correct error type - DeterministicStartupError prevents startup")
            return True
        elif "validation failed" in str(e).lower():
            print("PASS: Auth validation error properly raised")
            return True
        else:
...
async def main():
    ...
    if all_passed:
        ...
        print("\nWARNING: Some auth validation tests failed!")
