...
class DatabaseManagerRealConnectionManagementTests(BaseIntegrationTest):
    ...
    def setup_method(self):
        ...
        self.integration_env_vars = {'ENVIRONMENT': 'test', 'USE_MEMORY_DB': 'false', 'POSTGRES_HOST': 'localhost', 'POSTGRES_PORT': '5434', 'POSTGRES_USER': 'integration_user', 'POSTGRES_PASSWORD': 'integration_pass', 'POSTGRES_DB': 'integration_db', 'SQLITE_URL': f'sqlite+aiosqlite:///{self.primary_db_path}', 'TESTING': '1'}
    ...
    async def _create_real_test_tables(self, engine: AsyncEngine):
        ...
        metadata = MetaData()
        users_table = Table('integration_users', metadata, Column('id', Integer, primary_key=True), Column('email', String(255), unique=True, nullable=False), Column('username', String(100), nullable=False), Column('subscription_tier', String(50), default='free'), Column('created_at', DateTime), Column('is_active', Boolean, default=True))
        conversations_table = Table('chat_conversations', metadata, Column('id', Integer, primary_key=True), Column('user_id', Integer, nullable=False), Column('title', String(200)), Column('message_count', Integer, default=0), Column('created_at', DateTime), Column('last_activity', DateTime))
        performance_table = Table('performance_test', metadata, Column('id', Integer, primary_key=True), Column('thread_id', String(50)), Column('operation_id', String(100)), Column('data_payload', String(500)), Column('timestamp', DateTime))
    ...
    async def test_real_database_connection_lifecycle(self, isolated_env):
        ...
        with patch('netra_backend.app.core.config.get_config') as mock_config, patch.object(DatabaseManager, '_get_database_url') as mock_get_url:
            ...
            async with db_manager.get_session() as session:
                await session.execute(text('INSERT INTO integration_users (email, username, subscription_tier, is_active) VALUES (:email, :username, :tier, :active)'), {'email': 'integration@netrasystems.ai', 'username': 'integration_user', 'tier': 'premium', 'active': True})
                await session.commit()
                result = await session.execute(text('SELECT email, subscription_tier FROM integration_users WHERE username = :username'), {'username': 'integration_user'})
                row = result.fetchone()
                assert row is not None
                assert row[0] == 'integration@netrasystems.ai'
    ...
    async def test_real_connection_pool_performance(self, isolated_env):
        ...
        with patch('netra_backend.app.core.config.get_config') as mock_config, patch.object(DatabaseManager, '_get_database_url') as mock_get_url:
            ...
            for i in range(num_operations):
                async with db_manager.get_session() as session:
                    await session.execute(text('INSERT INTO performance_test (thread_id, operation_id, data_payload) VALUES (:thread_id, :op_id, :data)'), {'thread_id': 'main_thread', 'op_id': f'perf_op_{i}', 'data': f'performance_data_{i}'})
                    await session.commit()
                    result = await session.execute(text('SELECT COUNT(*) FROM performance_test WHERE thread_id = :thread_id'), {'thread_id': 'main_thread'})
                    count = result.scalar()
                    assert count >= i + 1
    ...
    async def test_real_database_auto_initialization_safety(self, isolated_env):
        ...
        with patch('netra_backend.app.core.config.get_config') as mock_config, patch.object(DatabaseManager, '_get_database_url') as mock_get_url:
            ...
            await self._create_real_test_tables(engine)
            async with db_manager.get_session() as session:
                await session.execute(text('INSERT INTO integration_users (email, username) VALUES (:email, :username)'), {'email': 'auto_init@netrasystems.ai', 'username': 'auto_init_user'})
                await session.commit()
                result = await session.execute(text('SELECT username FROM integration_users WHERE email = :email'), {'email': 'auto_init@netrasystems.ai'})
...
class DatabaseManagerTransactionSafetyTests(BaseIntegrationTest):
    ...
    def setup_method(self):
        ...
        self.transaction_env_vars = {'ENVIRONMENT': 'test', 'SQLITE_URL': f'sqlite+aiosqlite:///{self.transaction_db_path}', 'TESTING': '1'}
        import netra_backend.app.db.database_manager
        netra_backend.app.db.database_manager._database_manager = None
    ...
    async def test_real_transaction_commit_rollback_behavior(self, isolated_env):
        ...
        with patch('netra_backend.app.core.config.get_config') as mock_config, patch.object(DatabaseManager, '_get_database_url') as mock_get_url:
            ...
            await self._create_real_test_tables(engine)
            async with db_manager.get_session() as session:
                await session.execute(text('INSERT INTO integration_users (email, username, subscription_tier) VALUES (:email, :username, :tier)'), {'email': 'commit_test@netrasystems.ai', 'username': 'commit_user', 'tier': 'enterprise'})
            async with db_manager.get_session() as session:
                result = await session.execute(text('SELECT subscription_tier FROM integration_users WHERE email = :email'), {'email': 'commit_test@netrasystems.ai'})
                tier = result.scalar()
                assert tier == 'enterprise'
            rollback_occurred = False
            try:
                async with db_manager.get_session() as session:
                    await session.execute(text('INSERT INTO integration_users (email, username, subscription_tier) VALUES (:email, :username, :tier)'), {'email': 'rollback_test@netrasystems.ai', 'username': 'rollback_user', 'tier': 'premium'})
                    raise ValueError('Simulated business logic error')
            except ValueError:
                rollback_occurred = True
            assert rollback_occurred
            async with db_manager.get_session() as session:
                result = await session.execute(text('SELECT COUNT(*) FROM integration_users WHERE email = :email'), {'email': 'rollback_test@netrasystems.ai'})
                count = result.scalar()
                assert count == 0
            await db_manager.close_all()
        ...
        with patch('netra_backend.app.core.config.get_config') as mock_config, patch.object(DatabaseManager, '_get_database_url') as mock_get_url:
            ...
            async def user_conversation_session(user_id: int, conversation_count: int):
                ...
                for i in range(conversation_count):
                    async with db_manager.get_session() as session:
                        await session.execute(text('INSERT OR IGNORE INTO integration_users (email, username, subscription_tier) \n                                   VALUES (:email, :username, :tier)'), {'email': f'user_{user_id}@netrasystems.ai', 'username': f'user_{user_id}', 'tier': 'free'})
                        result = await session.execute(text('INSERT INTO chat_conversations (user_id, title, message_count) VALUES (:user_id, :title, :count) RETURNING id'), {'user_id': user_id, 'title': f'Chat_{user_id}_{i}', 'count': 5})
            ...
            async with db_manager.get_session() as session:
                ...
                for user_id in range(1, num_users + 1):
                    result = await session.execute(text('SELECT COUNT(*) FROM chat_conversations WHERE user_id = :user_id'), {'user_id': user_id})
    ...
    async def test_real_database_constraint_violation_handling(self, isolated_env):
        ...
        with patch('netra_backend.app.core.config.get_config') as mock_config, patch.object(DatabaseManager, '_get_database_url') as mock_get_url:
            ...
            await self._create_real_test_tables(engine)
            async with db_manager.get_session() as session:
                await session.execute(text('INSERT INTO integration_users (email, username, subscription_tier) VALUES (:email, :username, :tier)'), {'email': 'unique@netrasystems.ai', 'username': 'unique_user', 'tier': 'premium'})
                await session.commit()
            constraint_violation_occurred = False
            try:
                async with db_manager.get_session() as session:
                    await session.execute(text('INSERT INTO integration_users (email, username, subscription_tier) VALUES (:email, :username, :tier)'), {'email': 'unique@netrasystems.ai', 'username': 'duplicate_user', 'tier': 'free'})
                    await session.commit()
            ...
            async with db_manager.get_session() as session:
                result = await session.execute(text('SELECT username, subscription_tier FROM integration_users WHERE email = :email'), {'email': 'unique@netrasystems.ai'})
                row = result.fetchone()
                assert row is not None
                assert row[0] == 'unique_user'
                assert row[1] == 'premium'
                result = await session.execute(text('SELECT COUNT(*) FROM integration_users WHERE email = :email'), {'email': 'unique@netrasystems.ai'})
                count = result.scalar()
                assert count == 1
            await db_manager.close_all()
...
class DatabaseManagerMultiUserConcurrencyTests(BaseIntegrationTest):
    ...
    def setup_method(self):
        ...
        self.concurrent_env_vars = {'ENVIRONMENT': 'test', 'SQLITE_URL': f'sqlite+aiosqlite:///{self.concurrent_db_path}', 'TESTING': '1'}
        import netra_backend.app.db.database_manager
        netra_backend.app.db.database_manager._database_manager = None
    ...
    async def test_real_concurrent_user_operations(self, isolated_env):
        ...
        with patch('netra_backend.app.core.config.get_config') as mock_config, patch.object(DatabaseManager, '_get_database_url') as mock_get_url:
            ...
            async def simulate_user_workflow(user_id: int, operations: int):
                ...
                user_email = f'concurrent_user_{user_id}@netrasystems.ai'
                user_results = []
                for op_num in range(operations):
                    async with db_manager.get_session() as session:
                        await session.execute(text('INSERT OR REPLACE INTO integration_users \n                                   (email, username, subscription_tier, is_active) \n                                   VALUES (:email, :username, :tier, :active)'), {'email': user_email, 'username': f'user_{user_id}', 'tier': 'premium', 'active': True})
                        result = await session.execute(text('INSERT INTO chat_conversations (user_id, title, message_count) VALUES (:user_id, :title, :count) RETURNING id'), {'user_id': user_id, 'title': f'Concurrent Chat {op_num}', 'count': op_num + 1})
                        conversation_id = result.scalar()
                        await session.commit()
    ...
    async def test_real_session_isolation_between_users(self, isolated_env):
        ...
        with patch('netra_backend.app.core.config.get_config') as mock_config, patch.object(DatabaseManager, '_get_database_url') as mock_get_url:
            ...
            engine = db_manager.get_engine('primary')
            await self._create_real_test_tables(engine)
            user_a_email = 'sensitive_user_a@netrasystems.ai'
            async with db_manager.get_session() as session_a:
                await session_a.execute(text('INSERT INTO integration_users (email, username, subscription_tier) VALUES (:email, :username, :tier)'), {'email': user_a_email, 'username': 'sensitive_user_a', 'tier': 'enterprise'})
                result = await session_a.execute(text('INSERT INTO chat_conversations (user_id, title, message_count) VALUES (:user_id, :title, :count) RETURNING id'), {'user_id': 1, 'title': 'Confidential Business Plan', 'count': 10})
                sensitive_conversation_id = result.scalar()
                await session_a.commit()
            user_b_email = 'regular_user_b@netrasystems.ai'
            user_b_data = None
            sensitive_data_visible_to_b = False
            async with db_manager.get_session() as session_b:
                await session_b.execute(text('INSERT INTO integration_users (email, username, subscription_tier) VALUES (:email, :username, :tier)'), {'email': user_b_email, 'username': 'regular_user_b', 'tier': 'free'})
                await session_b.execute(text('INSERT INTO chat_conversations (user_id, title, message_count) VALUES (:user_id, :title, :count)'), {'user_id': 2, 'title': 'Regular Chat', 'count': 3})
                await session_b.commit()
                result = await session_b.execute(text('SELECT title FROM chat_conversations WHERE user_id = :user_id'), {'user_id': 1})
                ...
                result = await session_b.execute(text('SELECT username, subscription_tier FROM integration_users WHERE email = :email'), {'email': user_b_email})
            ...
            async with db_manager.get_session() as verification_session:
                result = await verification_session.execute(text('SELECT username, subscription_tier FROM integration_users WHERE email = :email'), {'email': user_a_email})
                ...
                result = await verification_session.execute(text('SELECT title FROM chat_conversations WHERE id = :id'), {'id': sensitive_conversation_id})
        ...
        with patch('netra_backend.app.core.config.get_config') as mock_config, patch.object(DatabaseManager, '_get_database_url') as mock_get_url:
            ...
            async with db_manager.get_session() as session:
                for i in range(num_users):
                    await session.execute(text('INSERT INTO integration_users (email, username, subscription_tier, is_active) VALUES (:email, :username, :tier, :active)'), {'email': f'bulk_user_{i}@netrasystems.ai', 'username': f'bulk_user_{i}', 'tier': 'free', 'active': True})
                    for j in range(3):
                        await session.execute(text('INSERT INTO chat_conversations (user_id, title, message_count) VALUES (:user_id, :title, :count)'), {'user_id': i + 1, 'title': f'Bulk Chat {j}', 'count': j + 1})
                await session.commit()
    ...
    async def test_real_database_connection_resilience(self, isolated_env):
        ...
        with patch('netra_backend.app.core.config.get_config') as mock_config, patch.object(DatabaseManager, '_get_database_url') as mock_get_url:
            ...
            await self._create_real_test_tables(engine)
            async with db_manager.get_session() as session:
                await session.execute(text('INSERT INTO integration_users (email, username, subscription_tier) VALUES (:email, :username, :tier)'), {'email': 'resilience@netrasystems.ai', 'username': 'resilience_user', 'tier': 'premium'})
                await session.commit()
            await db_manager.close_all()
            assert not db_manager._initialized
            async with db_manager.get_session() as session:
                result = await session.execute(text('SELECT username FROM integration_users WHERE email = :email'), {'email': 'resilience@netrasystems.ai'})
                username = result.scalar()
            ...
            assert health_result['status'] == 'healthy'
            async with db_manager.get_session() as session:
                await session.execute(text('INSERT INTO chat_conversations (user_id, title, message_count) VALUES (:user_id, :title, :count)'), {'user_id': 1, 'title': 'Post-Recovery Chat', 'count': 5})
                await session.commit()
                result = await session.execute(text('SELECT title FROM chat_conversations WHERE title = :title'), {'title': 'Post-Recovery Chat'})
                title = result.scalar()
