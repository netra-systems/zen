...
class TestWebSocketAuthIntegration(SSotBaseTestCase):
    ...
    async def test_websocket_connection_with_valid_jwt(self):
        ...
        """
        auth_user = await self.e2e_helper.create_authenticated_user(email='integration_test@example.com', permissions=['read', 'write'])
        websocket_headers = self.e2e_helper.get_websocket_headers(auth_user.jwt_token)
        try:
            async with websockets.connect(self.websocket_url, additional_headers=websocket_headers, timeout=10.0) as websocket:
                self.assertEqual(websocket.state.value, 1)
                auth_test_message = {'type': 'auth_test', 'timestamp': datetime.now(timezone.utc).isoformat(), 'user_id': auth_user.user_id}
    ...
    async def test_websocket_upgrade_preserves_auth_context(self):
        ...
        """
        auth_user = await self.e2e_helper.create_authenticated_user(email='context_test@example.com', permissions=['read', 'write', 'websocket'])
        websocket_headers = self.e2e_helper.get_websocket_headers(auth_user.jwt_token)
        try:
            async with websockets.connect(self.websocket_url, additional_headers=websocket_headers, timeout=10.0) as websocket:
                context_test_message = {'type': 'context_validation', 'action': 'get_user_context', 'timestamp': datetime.now(timezone.utc).isoformat(), 'expected_user_id': auth_user.user_id}
    ...
    async def test_websocket_multi_user_isolation(self):
        ...
        """
        user1 = await self.e2e_helper.create_authenticated_user(email='user1_isolation@example.com', permissions=['read', 'write'])
        user2 = await self.e2e_helper.create_authenticated_user(email='user2_isolation@example.com', permissions=['read', 'write'])
        ...
        try:
            connection_tasks = [self._test_isolated_user_connection(user1.user_id, headers1, 'user1'), self._test_isolated_user_connection(user2.user_id, headers2, 'user2')]
    ...
    async def _test_isolated_user_connection(self, user_id: str, headers: Dict[str, str], user_label: str) -> bool:
        ...
        try:
            async with websockets.connect(self.websocket_url, additional_headers=headers, timeout=5.0) as websocket:
                isolation_message = {'type': 'isolation_test', 'user_identifier': user_label, 'user_id': user_id, 'timestamp': datetime.now(timezone.utc).isoformat()}
...
class TestWebSocketAuthWithRealServices(SSotBaseTestCase):
    ...
    async def _check_service_availability(self) -> bool:
        ...
        try:
            async with websockets.connect(self.websocket_url, timeout=2.0) as websocket:
                return True
        except Exception:
            return False
    ...
    async def test_websocket_auth_with_unified_service(self):
        ...
        auth_user = await self.e2e_helper.create_authenticated_user(email='unified_service_test@example.com', permissions=['read', 'write', 'websocket'])
        websocket_headers = self.e2e_helper.get_websocket_headers(auth_user.jwt_token)
        try:
            async with websockets.connect(self.websocket_url, additional_headers=websocket_headers, timeout=10.0) as websocket:
                service_test_message = {'type': 'service_integration_test', 'auth_service': 'UnifiedAuthenticationService', 'authenticator': 'UnifiedWebSocketAuthenticator', 'user_id': auth_user.user_id, 'timestamp': datetime.now(timezone.utc).isoformat()}
    ...
    async def test_websocket_auth_performance_under_load(self):
        ...
        for i in range(num_concurrent):
            user = await self.e2e_helper.create_authenticated_user(email=f'perf_test_{i}@example.com', permissions=['read', 'write'])
            users.append(user)
        ...
        async def connect_user(user):
            ...
            try:
                async with websockets.connect(self.websocket_url, additional_headers=headers, timeout=5.0) as websocket:
                    perf_message = {'type': 'performance_test', 'user_id': user.user_id, 'timestamp': datetime.now(timezone.utc).isoformat()}
                    await websocket.send(json.dumps(perf_message))
                    await asyncio.sleep(0.1)
                    return True
            except Exception as e:
                return f'Connection failed: {e}'
        results = await asyncio.gather(*[connect_user(user) for user in users], return_exceptions=True)
        end_time = time.time()
        ...
        self.assertGreater(successful_connections, 0, f'At least some connections should succeed. Results: {results}')
