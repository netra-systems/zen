...
class TestDataPipelineRealProcessingIntegration(SSotBaseTestCase):
    ...
    async def test_end_to_end_data_processing_pipeline(self):
        ...
        test_events = [('user_login', 'user_001', {'session_id': 'sess_1', 'device': 'mobile'}), ('page_view', 'user_001', {'page': '/dashboard', 'duration': 30}), ('feature_usage', 'user_002', {'feature': 'cost_optimizer', 'duration': 120}), ('user_logout', 'user_001', {'session_duration': 300})]
        async with self.db_helper.get_connection() as conn:
            await conn.executemany(f'\n                INSERT INTO {self.test_prefix}_raw_events (event_type, user_id, event_data)\n                VALUES ($1, $2, $3)\n            ', test_events)
        try:
            pipeline_id = await self.data_pipeline.start_pipeline(source_table=f'{self.test_prefix}_raw_events', destination_table=f'{self.test_prefix}_processed_analytics', processing_rules=['aggregate_by_user', 'calculate_metrics'], test_prefix=self.test_prefix)
