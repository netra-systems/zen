...
class WorkflowMockAgent(BaseAgent):
    ...
    def __init__(self, name: str, llm_manager: LLMManager, dependencies: List[str]=None, produces_data: bool=False, consumes_data: bool=False):
    ...
    async def execute(self, state: DeepAgentState, run_id: str, stream_updates: bool=True) -> Dict[str, Any]:
        ...
        if stream_updates and self.websocket_bridge:
            await self.websocket_bridge.notify_agent_thinking(run_id, self.name, f'Starting {self.name} workflow step...', step_number=1)
        await self._simulate_agent_work(state, run_id, stream_updates)
        result = {'success': True, 'agent_name': self.name, 'execution_count': self.execution_count, 'dependencies_satisfied': len(missing_deps) == 0, 'workflow_phase': self._get_workflow_phase(), 'timestamp': datetime.now(timezone.utc).isoformat()}
        ...
        if stream_updates and self.websocket_bridge:
            await self.websocket_bridge.notify_agent_thinking(run_id, self.name, f'Completed {self.name} workflow step', step_number=2)
        return result
    ...
    async def _simulate_triage_work(self, state: DeepAgentState, run_id: str, stream_updates: bool):
        ...
        if stream_updates and self.websocket_bridge:
            await self.websocket_bridge.notify_tool_executing(run_id, self.name, 'triage_classifier', {'request': 'classify_request'})
        await asyncio.sleep(0.03)
        if stream_updates and self.websocket_bridge:
            await self.websocket_bridge.notify_tool_completed(run_id, self.name, 'triage_classifier', {'category': 'optimization_request'})
    ...
    async def _simulate_data_helper_work(self, state: DeepAgentState, run_id: str, stream_updates: bool):
        ...
        if stream_updates and self.websocket_bridge:
            await self.websocket_bridge.notify_tool_executing(run_id, self.name, 'data_analyzer', {'source': 'user_data'})
        await asyncio.sleep(0.08)
        if stream_updates and self.websocket_bridge:
            await self.websocket_bridge.notify_tool_completed(run_id, self.name, 'data_analyzer', {'analysis_complete': True, 'data_quality': 'high'})
    ...
    async def _simulate_optimization_work(self, state: DeepAgentState, run_id: str, stream_updates: bool):
        ...
        if stream_updates and self.websocket_bridge:
            await self.websocket_bridge.notify_tool_executing(run_id, self.name, 'optimization_engine', {'strategy': 'cost_performance'})
        await asyncio.sleep(0.12)
        if stream_updates and self.websocket_bridge:
            await self.websocket_bridge.notify_tool_completed(run_id, self.name, 'optimization_engine', {'recommendations': 3, 'estimated_savings': 15000})
    ...
    async def _simulate_actions_work(self, state: DeepAgentState, run_id: str, stream_updates: bool):
        ...
        if stream_updates and self.websocket_bridge:
            await self.websocket_bridge.notify_tool_executing(run_id, self.name, 'action_planner', {'recommendations': 'process'})
        await asyncio.sleep(0.06)
        if stream_updates and self.websocket_bridge:
            await self.websocket_bridge.notify_tool_completed(run_id, self.name, 'action_planner', {'action_plan': 'generated', 'timeline': '2_weeks'})

    def _get_workflow_phase(self) -> str:
        """Get workflow phase for this agent."""
        phase_map = {'triage': 'analysis', 'data_helper': 'data_gathering', 'optimization': 'recommendation', 'actions': 'implementation', 'reporting': 'synthesis'}
    ...
    def _generate_agent_output(self, state: DeepAgentState) -> Dict[str, Any]:
        ...
        if self.name == 'triage':
            return {'category': 'cost_optimization', 'priority': 'high', 'data_sufficiency': 'sufficient', 'next_agents': ['data_helper', 'optimization']}
        elif self.name == 'data_helper':
            return {'data_sources': 3, 'data_quality': 'high', 'analysis_complete': True, 'key_metrics': {'monthly_spend': 45000, 'model_usage': {'gpt4': 60, 'claude': 40}}}
        elif self.name == 'optimization':
            return {'recommendations': [{'type': 'model_switching', 'savings': 8000}, {'type': 'batch_processing', 'savings': 5000}, {'type': 'caching', 'savings': 2000}], 'total_savings': 15000, 'confidence': 0.85}
        elif self.name == 'actions':
            return {'action_plan': {'phase1': 'Implement caching (Week 1)', 'phase2': 'Model optimization (Week 2)', 'phase3': 'Batch processing (Week 3)'}, 'implementation_ready': True}
        else:
...
class MockWebSocketManager:
    ...
    async def create_bridge(self, user_context: UserExecutionContext):
        ...
        bridge.notify_agent_error = AsyncMock(side_effect=lambda run_id, agent_name, error, error_context=None: track_emit('agent_error', {'agent_name': agent_name, 'error': error, 'error_context': error_context}))
        return bridge
...
class TestMultiAgentWorkflowIntegration(BaseIntegrationTest):
    ...
    async def workflow_user_context(self):
        ...
        return UserExecutionContext(user_id=f'workflow_user_{uuid.uuid4().hex[:8]}', thread_id=f'workflow_thread_{uuid.uuid4().hex[:8]}', run_id=f'workflow_run_{uuid.uuid4().hex[:8]}', request_id=f'workflow_req_{uuid.uuid4().hex[:8]}', agent_context={'user_request': 'Help me optimize my AI costs and performance', 'workflow_type': 'cost_optimization'})

    @pytest.fixture
    async def workflow_agents(self, mock_llm_manager):
        """Create complete workflow agent set."""
        agents = {'triage': WorkflowMockAgent('triage', mock_llm_manager, dependencies=[], produces_data=True), 'data_helper': WorkflowMockAgent('data_helper', mock_llm_manager, dependencies=['triage'], produces_data=True, consumes_data=True), 'optimization': WorkflowMockAgent('optimization', mock_llm_manager, dependencies=['data_helper'], produces_data=True, consumes_data=True), 'actions': WorkflowMockAgent('actions', mock_llm_manager, dependencies=['optimization'], produces_data=True, consumes_data=True), 'reporting': WorkflowMockAgent('reporting', mock_llm_manager, dependencies=[], produces_data=True)}
        return agents
    ...
    async def test_sequential_workflow_execution_order(self, workflow_user_context, workflow_registry, workflow_agents, mock_llm_manager):
        ...
        engine = ExecutionEngine._init_from_factory(registry=workflow_registry, websocket_bridge=websocket_bridge, user_context=workflow_user_context)
        pipeline_steps = [PipelineStep(agent_name='triage', strategy=AgentExecutionStrategy.SEQUENTIAL, metadata={'phase': 'analysis'}), PipelineStep(agent_name='data_helper', strategy=AgentExecutionStrategy.SEQUENTIAL, metadata={'phase': 'data_gathering'}), PipelineStep(agent_name='optimization', strategy=AgentExecutionStrategy.SEQUENTIAL, metadata={'phase': 'recommendation'}), PipelineStep(agent_name='actions', strategy=AgentExecutionStrategy.SEQUENTIAL, metadata={'phase': 'implementation'})]
        base_context = AgentExecutionContext(user_id=workflow_user_context.user_id, thread_id=workflow_user_context.thread_id, run_id=workflow_user_context.run_id, request_id=workflow_user_context.request_id, agent_name='workflow', step=PipelineStep.INITIALIZATION, execution_timestamp=datetime.now(timezone.utc), pipeline_step_num=0)
        start_time = time.time()
        results = await engine.execute_pipeline(steps=pipeline_steps, context=base_context, user_context=workflow_user_context)
    ...
    @pytest.mark.integration
    @pytest.mark.real_services
    async def test_datahelper_to_optimization_handoff(self, workflow_user_context, workflow_registry, workflow_agents, mock_llm_manager):
        """Test critical DataHelper  ->  Optimization agent handoff with data integrity."""
        websocket_bridge = await self.websocket_manager.create_bridge(workflow_user_context)
        engine = ExecutionEngine._init_from_factory(registry=workflow_registry, websocket_bridge=websocket_bridge, user_context=workflow_user_context)
        shared_state = DeepAgentState(user_request='Optimize my $45k monthly AI spend', user_id=workflow_user_context.user_id, chat_thread_id=workflow_user_context.thread_id, run_id=workflow_user_context.run_id, agent_input={'optimization_target': 'cost_performance'})
        data_helper_context = AgentExecutionContext(user_id=workflow_user_context.user_id, thread_id=workflow_user_context.thread_id, run_id=workflow_user_context.run_id, request_id=workflow_user_context.request_id, agent_name='data_helper', step=PipelineStep.DATA_GATHERING, execution_timestamp=datetime.now(timezone.utc), pipeline_step_num=1)
        ...
        optimization_context = AgentExecutionContext(user_id=workflow_user_context.user_id, thread_id=workflow_user_context.thread_id, run_id=workflow_user_context.run_id, request_id=workflow_user_context.request_id, agent_name='optimization', step=PipelineStep.OPTIMIZATION, execution_timestamp=datetime.now(timezone.utc), pipeline_step_num=2)
    ...
    @pytest.mark.integration
    @pytest.mark.real_services
    async def test_parallel_agent_execution_with_dependencies(self, workflow_user_context, workflow_registry, workflow_agents, mock_llm_manager):
        """Test agents can execute in parallel when dependencies allow."""
        websocket_bridge = await self.websocket_manager.create_bridge(workflow_user_context)
        engine = ExecutionEngine._init_from_factory(registry=workflow_registry, websocket_bridge=websocket_bridge, user_context=workflow_user_context)
        shared_state = DeepAgentState(user_request='Parallel workflow test', user_id=workflow_user_context.user_id, chat_thread_id=workflow_user_context.thread_id, run_id=workflow_user_context.run_id, agent_input={'parallel_test': True})
        shared_state.metadata = {'triage_result': {'category': 'optimization', 'next_agents': ['data_helper', 'optimization']}, 'data_helper_result': {'data_quality': 'high', 'analysis_complete': True}}
        pipeline_steps = [PipelineStep(agent_name='reporting', strategy=AgentExecutionStrategy.PARALLEL, metadata={'can_run_parallel': True}), PipelineStep(agent_name='optimization', strategy=AgentExecutionStrategy.PARALLEL, metadata={'can_run_parallel': True})]
        base_context = AgentExecutionContext(user_id=workflow_user_context.user_id, thread_id=workflow_user_context.thread_id, run_id=workflow_user_context.run_id, request_id=workflow_user_context.request_id, agent_name='parallel_workflow', step=PipelineStep.INITIALIZATION, execution_timestamp=datetime.now(timezone.utc), pipeline_step_num=0)
        start_time = time.time()
        results = await engine.execute_pipeline(steps=pipeline_steps, context=base_context, user_context=workflow_user_context)
        execution_time = time.time() - start_time
    ...
    @pytest.mark.integration
    @pytest.mark.real_services
    async def test_workflow_error_propagation_and_recovery(self, workflow_user_context, workflow_registry, workflow_agents, mock_llm_manager):
        """Test workflow handles agent failures and continues with recovery."""
        failing_optimization = WorkflowMockAgent('optimization', mock_llm_manager, dependencies=['data_helper'], consumes_data=True)
        ...
        websocket_bridge = await self.websocket_manager.create_bridge(workflow_user_context)
        engine = ExecutionEngine._init_from_factory(registry=workflow_registry, websocket_bridge=websocket_bridge, user_context=workflow_user_context)
        shared_state = DeepAgentState(user_request='Test error recovery', user_id=workflow_user_context.user_id, chat_thread_id=workflow_user_context.thread_id, run_id=workflow_user_context.run_id, agent_input={'error_test': True})
        shared_state.metadata = {'data_helper_result': {'data_quality': 'high', 'analysis_complete': True}}
        pipeline_steps = [PipelineStep(agent_name='data_helper', strategy=AgentExecutionStrategy.SEQUENTIAL, metadata={'continue_on_error': True}), PipelineStep(agent_name='optimization', strategy=AgentExecutionStrategy.SEQUENTIAL, metadata={'continue_on_error': True}), PipelineStep(agent_name='reporting', strategy=AgentExecutionStrategy.SEQUENTIAL, metadata={'continue_on_error': True})]
        base_context = AgentExecutionContext(user_id=workflow_user_context.user_id, thread_id=workflow_user_context.thread_id, run_id=workflow_user_context.run_id, request_id=workflow_user_context.request_id, agent_name='error_recovery_workflow', step=PipelineStep.INITIALIZATION, execution_timestamp=datetime.now(timezone.utc), pipeline_step_num=0)
        results = await engine.execute_pipeline(steps=pipeline_steps, context=base_context, user_context=workflow_user_context)
    ...
    @pytest.mark.integration
    @pytest.mark.real_services
    async def test_concurrent_multi_user_workflows(self, workflow_registry, workflow_agents, mock_llm_manager):
        ...
        for i, (context, bridge) in enumerate(zip(user_contexts, websocket_bridges)):
            engine = ExecutionEngine._init_from_factory(registry=workflow_registry, websocket_bridge=bridge, user_context=context)
            engines.append(engine)

        async def run_user_workflow(user_index, user_context, engine):
            """Run workflow for a specific user."""
            base_context = AgentExecutionContext(user_id=user_context.user_id, thread_id=user_context.thread_id, run_id=user_context.run_id, request_id=user_context.request_id, agent_name='concurrent_workflow', step=PipelineStep.INITIALIZATION, execution_timestamp=datetime.now(timezone.utc), pipeline_step_num=0)
            pipeline_steps = [PipelineStep(agent_name='triage', strategy=AgentExecutionStrategy.SEQUENTIAL), PipelineStep(agent_name='reporting', strategy=AgentExecutionStrategy.SEQUENTIAL)]
            results = await engine.execute_pipeline(steps=pipeline_steps, context=base_context, user_context=user_context)
        ...
        total_triage_executions = sum((1 for agent_dict in [workflow_agents] * 3 if agent_dict['triage'].execution_count > 0))
