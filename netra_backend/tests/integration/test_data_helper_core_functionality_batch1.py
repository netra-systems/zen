...
async def authenticated_user(auth_helper):
    ...
    return await auth_helper.create_authenticated_user(email='datahelper_test@netrasystems.ai', full_name='Data Helper Test User', permissions=['read', 'write', 'agent_execution'])
...
async def user_execution_context(authenticated_user, real_services_fixture):
    ...
    return await create_authenticated_user_context(user_email=authenticated_user.email, user_id=authenticated_user.user_id, environment='test', permissions=authenticated_user.permissions)
...
class DataHelperCoreBasicGenerationTests(SSotBaseTestCase):
    ...
    async def test_data_helper_basic_request_generation(self, real_services_fixture, data_helper_agent, user_execution_context, mock_websocket_manager, authenticated_user):
        ...
        context.metadata['triage_result'] = {'category': 'Cost Optimization', 'priority': 'high', 'confidence_score': 0.85}
    ...
    @pytest.mark.integration
    @pytest.mark.real_services
    async def test_data_helper_with_triage_context_integration(self, real_services_fixture, data_helper_agent, user_execution_context, mock_websocket_manager):
        ...
        """
        detailed_triage = {'category': 'Performance Optimization', 'sub_category': 'API Latency Reduction', 'priority': 'high', 'complexity': 'medium', 'confidence_score': 0.92, 'data_sufficiency': 'partial', 'extracted_entities': {'models': ['gpt-3.5-turbo', 'claude-2'], 'metrics': ['latency', 'throughput', 'error_rate'], 'providers': ['openai', 'anthropic'], 'time_ranges': ['last_30_days'], 'thresholds': [500, 95]}, 'user_intent': {'primary_intent': 'optimize_performance', 'action_required': True, 'confidence': 0.89}, 'next_steps': ['Analyze current performance', 'Identify bottlenecks', 'Generate optimization plan']}
    ...
    @pytest.mark.integration
    @pytest.mark.real_services
    async def test_data_helper_category_extraction_accuracy(self, real_services_fixture, data_helper_agent, user_execution_context, mock_websocket_manager):
        ...
        triage_result = {'category': 'Multi-System Optimization', 'priority': 'critical', 'confidence_score': 0.95, 'extracted_entities': {'systems': ['recommendations', 'pricing', 'inventory', 'chatbot'], 'models': ['collaborative_filtering', 'gpt-4'], 'metrics': ['cost', 'latency', 'accuracy'], 'budget': [15000], 'timeline': ['peak_season']}}
        context = user_execution_context
    ...
    @pytest.mark.integration
    @pytest.mark.real_services
    async def test_data_helper_user_instructions_generation(self, real_services_fixture, data_helper_agent, user_execution_context, mock_websocket_manager):
        ...
        triage_result = {'category': 'Enterprise Infrastructure Optimization', 'priority': 'strategic', 'complexity': 'high', 'confidence_score': 0.88, 'extracted_entities': {'cloud_providers': ['aws', 'gcp', 'azure'], 'workloads': ['ml_training', 'inference', 'data_pipelines'], 'budget': [2500000], 'compliance': ['soc2', 'hipaa'], 'team_size': [50]}}
        context = user_execution_context
    ...
    @pytest.mark.integration
    @pytest.mark.real_services
    async def test_data_helper_response_structure_validation(self, real_services_fixture, data_helper_agent, user_execution_context, mock_websocket_manager):
        ...
        triage_result = {'category': 'Cost Optimization', 'sub_category': 'API Cost Reduction', 'priority': 'high', 'confidence_score': 0.87}
        context = user_execution_context
        context.metadata['user_request'] = user_request
        context.metadata['triage_result'] = triage_result
        mock_websocket_manager.clear_events()
...
class DataHelperUserExecutionContextTests(SSotBaseTestCase):
    ...
    @pytest.mark.integration
    @pytest.mark.real_services
    async def test_data_helper_user_context_isolation(self, real_services_fixture, data_helper_agent, mock_websocket_manager, auth_helper):
        ...
        """
        user1 = await auth_helper.create_authenticated_user(email='datahelper_user1@netrasystems.ai', full_name='Data Helper User 1')
        user2 = await auth_helper.create_authenticated_user(email='datahelper_user2@netrasystems.ai', full_name='Data Helper User 2')
        context1 = await create_authenticated_user_context(user_email=user1.email, user_id=user1.user_id, environment='test')
        context2 = await create_authenticated_user_context(user_email=user2.email, user_id=user2.user_id, environment='test')
        context1.metadata['user_request'] = f'User {user1.user_id}: Optimize my $50,000 AWS budget for financial services ML'
        context1.metadata['triage_result'] = {'category': 'Financial Services Optimization', 'budget': 50000, 'compliance': ['PCI-DSS', 'SOX'], 'sensitive_data': 'financial_records'}
        context2.metadata['user_request'] = f'User {user2.user_id}: Reduce healthcare AI costs for patient data processing'
        context2.metadata['triage_result'] = {'category': 'Healthcare Optimization', 'compliance': ['HIPAA', 'GDPR'], 'sensitive_data': 'patient_records'}
        mock_websocket_manager.clear_events()
    ...
    @pytest.mark.integration
    @pytest.mark.real_services
    async def test_data_helper_metadata_storage_retrieval(self, real_services_fixture, data_helper_agent, user_execution_context, mock_websocket_manager):
        ...
        """
        initial_metadata = {'session_id': f'session_{uuid.uuid4().hex[:8]}', 'workflow_stage': 'data_collection', 'optimization_history': [{'date': '2024-01-01', 'type': 'cost_analysis', 'savings': 15.2}, {'date': '2024-01-15', 'type': 'performance_tuning', 'improvement': 8.7}], 'user_preferences': {'priority': 'cost_over_performance', 'risk_tolerance': 'conservative', 'notification_frequency': 'daily'}, 'enterprise_config': {'department': 'ML Engineering', 'budget_code': 'AI-OPT-2024', 'approval_required': True}}
        context = user_execution_context
    ...
    @pytest.mark.integration
    @pytest.mark.real_services
    async def test_data_helper_context_metadata_preservation(self, real_services_fixture, data_helper_agent, user_execution_context, mock_websocket_manager):
        ...
        """
        previous_agent_results = {'triage_agent': {'timestamp': datetime.now(timezone.utc).isoformat(), 'category': 'Infrastructure Optimization', 'confidence': 0.91, 'recommendations': ['cost_analysis', 'performance_review']}, 'data_agent': {'timestamp': datetime.now(timezone.utc).isoformat(), 'data_sources': ['cloudwatch', 'billing_api', 'performance_logs'], 'quality_score': 0.87}}
        complex_metadata = {'request_chain_id': f'chain_{uuid.uuid4().hex[:8]}', 'agent_sequence': ['triage', 'data_helper', 'optimization', 'report'], 'current_agent': 'data_helper', 'previous_results': previous_agent_results, 'workflow_config': {'max_execution_time': 300, 'fallback_enabled': True, 'quality_threshold': 0.8}, 'business_context': {'customer_tier': 'enterprise', 'use_case': 'multi_cloud_optimization', 'regulatory_requirements': ['SOC2', 'ISO27001']}}
        context = user_execution_context
    ...
    @pytest.mark.integration
    @pytest.mark.real_services
    async def test_data_helper_concurrent_user_contexts(self, real_services_fixture, data_helper_agent, mock_websocket_manager, auth_helper):
        ...
        for i in range(num_concurrent_users):
            user = await auth_helper.create_authenticated_user(email=f'concurrent_user_{i}@netrasystems.ai', full_name=f'Concurrent User {i}')
            users.append(user)
            context = await create_authenticated_user_context(user_email=user.email, user_id=user.user_id, environment='test')
            scenarios = [{'request': 'Optimize chatbot costs for customer service', 'category': 'Customer Service AI'}, {'request': 'Reduce ML training costs for recommendation system', 'category': 'ML Training Optimization'}, {'request': 'Improve API performance for mobile app', 'category': 'API Performance'}, {'request': 'Optimize data pipeline costs for analytics', 'category': 'Data Pipeline'}, {'request': 'Reduce cloud storage costs for ML datasets', 'category': 'Storage Optimization'}]
            scenario = scenarios[i]
            context.metadata['user_request'] = f"User {i}: {scenario['request']}"
            context.metadata['triage_result'] = {'category': scenario['category'], 'priority': 'high', 'user_index': i}
    ...
    @pytest.mark.integration
    @pytest.mark.real_services
    async def test_data_helper_context_factory_pattern(self, real_services_fixture, data_helper_agent, user_execution_context, mock_websocket_manager):
        ...
        factory_metadata = {'context_version': '2.1.0', 'factory_pattern': 'UserExecutionContextFactory', 'creation_timestamp': datetime.now(timezone.utc).isoformat(), 'validation_checksum': f'checksum_{hash(str(original_context.user_id)) & 4294967295:08x}', 'factory_config': {'isolation_level': 'strict', 'security_validation': True, 'metadata_encryption': False, 'audit_logging': True}}
        ...
        enhanced_context.metadata['triage_result'] = {'category': 'Enterprise Infrastructure', 'complexity': 'high', 'factory_validated': True}
        mock_websocket_manager.clear_events()
...
class DataHelperLLMIntegrationTests(SSotBaseTestCase):
    ...
    @pytest.mark.integration
    @pytest.mark.real_services
    async def test_data_helper_llm_prompt_optimization(self, real_services_fixture, data_helper_agent, user_execution_context, mock_websocket_manager):
        ...
        """
        complex_scenario = {'user_request': 'Our fintech startup needs to optimize AI costs across multiple systems:\n            - Credit scoring model (XGBoost + neural networks)\n            - Fraud detection (real-time ML inference) \n            - Customer service chatbot (GPT-4)\n            - Document processing (OCR + NLP)\n            - Risk assessment automation\n            \n            Current challenges:\n            - $45k monthly AI costs (exceeding budget by 80%)\n            - Regulatory compliance (PCI-DSS, SOX, GDPR)\n            - Real-time latency requirements (<100ms for fraud detection)\n            - Audit trail requirements for all decisions\n            - Multi-language support (English, Spanish, French)', 'triage_result': {'category': 'Fintech Multi-System Optimization', 'complexity': 'very_high', 'priority': 'critical', 'confidence_score': 0.94, 'extracted_entities': {'industry': 'fintech', 'systems': ['credit_scoring', 'fraud_detection', 'chatbot', 'ocr', 'risk_assessment'], 'models': ['xgboost', 'neural_networks', 'gpt-4'], 'budget_current': 45000, 'budget_overage': 0.8, 'compliance': ['pci-dss', 'sox', 'gdpr'], 'latency_requirements': [100], 'languages': ['english', 'spanish', 'french']}, 'optimization_targets': ['cost', 'performance', 'compliance'], 'constraints': ['regulatory', 'latency', 'audit']}}
        context = user_execution_context
        ...
        assert len(thinking_data['message']) > 20
    ...
    @pytest.mark.integration
    @pytest.mark.real_services
    async def test_data_helper_llm_response_parsing_robustness(self, real_services_fixture, user_execution_context, mock_websocket_manager):
    ...
    @pytest.mark.integration
    @pytest.mark.real_services
    async def test_data_helper_temperature_consistency(self, real_services_fixture, user_execution_context, mock_websocket_manager):
        ...
        """
        test_scenario = {'user_request': 'Optimize costs for e-commerce recommendation system using collaborative filtering', 'triage_result': {'category': 'ML System Optimization', 'sub_category': 'Recommendation Engine', 'priority': 'high', 'confidence_score': 0.89, 'extracted_entities': {'system_type': 'recommendation_engine', 'algorithm': 'collaborative_filtering', 'domain': 'ecommerce'}}}
        mock_llm_manager = AsyncMock(spec=LLMManager)
    ...
    @pytest.mark.integration
    @pytest.mark.real_services
    async def test_data_helper_token_limit_handling(self, real_services_fixture, user_execution_context, mock_websocket_manager):
        ...
        complex_triage = {'category': 'Enterprise Global AI Infrastructure Optimization', 'complexity': 'extremely_high', 'priority': 'strategic', 'confidence_score': 0.97, 'scope': 'global_enterprise', 'estimated_savings': 840000, 'systems_count': 50, 'compliance_frameworks': 7, 'regions': ['US', 'EU', 'APAC']}
        mock_llm_manager = AsyncMock(spec=LLMManager)
    ...
    @pytest.mark.integration
    @pytest.mark.real_services
    async def test_data_helper_llm_retry_mechanism(self, real_services_fixture, user_execution_context, mock_websocket_manager):
...
class DataHelperWebSocketIntegrationTests(SSotBaseTestCase):
    ...
    async def test_data_helper_websocket_events_complete_flow(self, real_services_fixture, data_helper_agent, user_execution_context, mock_websocket_manager):
        ...
        context.metadata['triage_result'] = {'category': 'Multi-Modal AI Optimization', 'priority': 'high', 'complexity': 'high', 'confidence_score': 0.91}
        mock_websocket_manager.clear_events()
        ...
        assert len(thinking_event['data']['message']) > 10
    ...
    @pytest.mark.real_services
    @pytest.mark.websocket
    async def test_data_helper_websocket_event_timing(self, real_services_fixture, data_helper_agent, user_execution_context, mock_websocket_manager):
        ...
        context.metadata['triage_result'] = {'category': 'High-Frequency Trading Optimization', 'priority': 'critical', 'latency_requirements': ['<10ms'], 'compliance': ['SEC', 'FINRA']}
        mock_websocket_manager.clear_events()
        start_time = time.time()
        result_context = await data_helper_agent.execute(context)
    ...
    @pytest.mark.real_services
    @pytest.mark.websocket
    async def test_data_helper_websocket_event_authentication(self, real_services_fixture, data_helper_agent, mock_websocket_manager, auth_helper):
        ...
        """
        authenticated_user = await auth_helper.create_authenticated_user(email='secure_user@enterprise.com', full_name='Enterprise Security User', permissions=['read', 'write', 'agent_execution', 'websocket_events'])
        context = await create_authenticated_user_context(user_email=authenticated_user.email, user_id=authenticated_user.user_id, environment='test', permissions=authenticated_user.permissions)
        context.metadata['user_request'] = 'Secure optimization for financial trading algorithms'
        context.metadata['triage_result'] = {'category': 'Secure Financial Optimization', 'security_level': 'enterprise', 'compliance': ['SOX', 'PCI-DSS']}
        mock_websocket_manager.clear_events()
        result_context = await data_helper_agent.execute(context)
        events = mock_websocket_manager.events
        required_events = ['agent_started', 'agent_thinking', 'tool_executing', 'tool_completed', 'agent_completed']
    ...
    @pytest.mark.real_services
    @pytest.mark.websocket
    async def test_data_helper_websocket_event_error_handling(self, real_services_fixture, user_execution_context, mock_websocket_manager):
    ...
    @pytest.mark.real_services
    @pytest.mark.websocket
    async def test_data_helper_websocket_event_metadata_inclusion(self, real_services_fixture, data_helper_agent, user_execution_context, mock_websocket_manager):
        ...
        """
        rich_context = {'user_request': 'Optimize enterprise ML pipeline with comprehensive monitoring and cost analysis', 'triage_result': {'category': 'Enterprise ML Pipeline Optimization', 'sub_category': 'Cost and Performance Analysis', 'priority': 'strategic', 'complexity': 'high', 'confidence_score': 0.94, 'estimated_impact': 'high', 'timeline': 'Q2_2024'}, 'business_context': {'department': 'Data Science', 'budget_range': '500k_1m', 'team_size': 25, 'optimization_goals': ['cost_reduction', 'performance_improvement', 'reliability']}}
        context = user_execution_context
