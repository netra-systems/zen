...
from netra_backend.app.services.user_execution_context import UserExecutionContext, UserContextFactory, InvalidContextError, ContextIsolationError, create_isolated_execution_context, clear_shared_object_registry, managed_user_context
...
class TestUserContextFactoryIsolation(IntegrationTestBase):
    ...
    def setUp(self):
        ...
        self.test_users = [f'test_user_{uuid.uuid4().hex[:8]}' for _ in range(5)]
    ...
    def test_factory_creates_completely_isolated_contexts(self):
        ...
        for i, user_id in enumerate(self.test_users):
            context = UserContextFactory.create_context(user_id=user_id, thread_id=f'thread_isolation_test_{i}', run_id=f'run_isolation_test_{i}', websocket_client_id=f'ws_client_{i}')
    ...
    def test_factory_thread_safety_concurrent_creation(self):
        ...
        def create_context_worker(worker_id: int) -> IsolationTestResult:
            ...
            context = UserContextFactory.create_context(user_id=user_id, thread_id=f'thread_concurrent_{worker_id}', run_id=f'run_concurrent_{worker_id}')
            context.verify_isolation()
            result = IsolationTestResult(context_id=context.request_id, user_id=context.user_id, thread_id=context.thread_id, creation_time=context.created_at, isolation_verified=True, memory_address=id(context), data_integrity=all([context.user_id == user_id, context.thread_id.startswith('thread_concurrent'), context.run_id.startswith('run_concurrent')]))
            with creation_lock:
                creation_results.append(result)
            return result
        with ThreadPoolExecutor(max_workers=10) as executor:
            futures = [executor.submit(create_context_worker, i) for i in range(20)]
    ...
    def test_factory_prevents_context_data_pollution(self):
        ...
        """
        context1 = UserContextFactory.create_context(user_id='user_pollution_test_1', thread_id='thread_1', run_id='run_1')
        context2 = UserContextFactory.create_context(user_id='user_pollution_test_2', thread_id='thread_2', run_id='run_2')
        original_context1_data = context1.to_dict()
        original_context2_data = context2.to_dict()
        child_context1 = context1.create_child_context(operation_name='pollution_test', additional_agent_context={'test_data': 'modified_value_1'}, additional_audit_metadata={'test_audit': 'audit_value_1'})
        child_context2 = context2.create_child_context(operation_name='isolation_test', additional_agent_context={'test_data': 'modified_value_2'}, additional_audit_metadata={'test_audit': 'audit_value_2'})
    ...
    def test_factory_memory_isolation_validation(self):
        ...
        for i in range(10):
            context = UserContextFactory.create_context(user_id=f'memory_test_user_{i}', thread_id=f'memory_thread_{i}', run_id=f'memory_run_{i}')
            contexts.append(context)
            ...
            context.verify_isolation()
        expected_addresses = 10 * 3
        self.assertEqual(len(memory_addresses), expected_addresses, f'Expected {expected_addresses} unique memory addresses, got {len(memory_addresses)}')
        for i, context_a in enumerate(contexts):
            for j, context_b in enumerate(contexts):
                if i != j:
    ...
    async def test_factory_async_context_isolation(self):
        ...
        async def create_and_validate_context(user_index: int) -> bool:
            ...
            context = UserContextFactory.create_context(user_id=user_id, thread_id=f'async_thread_{user_index}', run_id=f'async_run_{user_index}', websocket_client_id=f'async_ws_{user_index}')
            ...
            async with managed_user_context(context) as managed_ctx:
                child_ctx = managed_ctx.create_child_context(operation_name=f'async_operation_{user_index}', additional_agent_context={'async_data': f'async_value_{user_index}'})
                child_isolated = child_ctx.verify_isolation()
                return is_isolated and child_isolated
        tasks = [create_and_validate_context(i) for i in range(15)]
    ...
    def test_factory_uses_ssot_id_generation_patterns(self):
        ...
        contexts = []
        for i in range(20):
            context = UserContextFactory.create_context(user_id=f'ssot_test_user_{i}', thread_id=f'ssot_thread_{i}', run_id=f'ssot_run_{i}')
            contexts.append(context)
    ...
    def test_factory_websocket_context_ssot_id_consistency(self):
        ...
        for i in range(10):
            ...
            context = UserExecutionContext.from_websocket_request(user_id=user_id, websocket_client_id=f'ws_ssot_client_{i}', operation='websocket_ssot_test')
    ...
    def test_factory_prevents_id_collision_across_sessions(self):
        ...
        for session in range(5):
            ...
            for user in range(10):
                context = UserContextFactory.create_context(user_id=f'collision_test_session_{session}_user_{user}', thread_id=f'collision_thread_{session}_{user}', run_id=f'collision_run_{session}_{user}')
        ...
        for batch in session_batches:
            for context in batch:
                ...
                all_context_combinations.append((context.user_id, context.thread_id, context.run_id, context.request_id))
        self.assertEqual(len(set(all_request_ids)), len(all_request_ids), 'Request IDs must be globally unique across all sessions')
        self.assertEqual(len(set(all_context_combinations)), len(all_context_combinations), 'Context ID combinations must be globally unique')
    ...
    def test_factory_integration_with_unified_id_manager(self):
        ...
        for i in range(10):
            ...
            context = asyncio.run(create_isolated_execution_context(user_id=user_id, request_id=request_id, thread_id=None, run_id=None, validate_user=False, isolation_level='standard'))
    ...
    def test_factory_id_extraction_and_consistency_validation(self):
        ...
        factory_methods = [lambda i: UserContextFactory.create_context(user_id=f'extract_user_{i}', thread_id=f'extract_thread_{i}', run_id=f'extract_run_{i}'), lambda i: UserExecutionContext.from_websocket_request(user_id=f'websocket_extract_user_{i}', operation='extraction_test')]
    ...
    def test_factory_context_validation_prevents_invalid_data(self):
        ...
        """
        valid_context = UserContextFactory.create_context(user_id='valid_user_12345', thread_id='valid_thread_67890', run_id='valid_run_abcdef')
        self.assertTrue(valid_context.verify_isolation())
        forbidden_patterns = [('placeholder', 'placeholder_thread', 'placeholder_run'), ('default', 'default_thread', 'default_run'), ('temp', 'temp_thread', 'temp_run'), ('test', 'test_thread', 'test_run') if not get_env().is_test() else None, ('null', 'null_thread', 'null_run'), ('', 'valid_thread', 'valid_run'), ('valid_user', '', 'valid_run'), ('valid_user', 'valid_thread', '')]
        for pattern in forbidden_patterns:
            if pattern is None:
                continue
            user_id, thread_id, run_id = pattern
            with self.assertRaises(InvalidContextError, msg=f'Should reject forbidden pattern: {pattern}'):
                UserContextFactory.create_context(user_id=user_id, thread_id=thread_id, run_id=run_id)
        valid_patterns = [('user_12345', 'thread_67890', 'run_abcdef'), ('production_user_uuid', 'production_thread_uuid', 'production_run_uuid'), (str(uuid.uuid4()), str(uuid.uuid4()), str(uuid.uuid4()))]
        for user_id, thread_id, run_id in valid_patterns:
            valid_context = UserContextFactory.create_context(user_id=user_id, thread_id=thread_id, run_id=run_id)
            self.assertTrue(valid_context.verify_isolation())
    ...
    def test_factory_creates_immutable_contexts_with_proper_boundaries(self):
        ...
        """
        context = UserContextFactory.create_context(user_id='immutability_test_user', thread_id='immutability_thread', run_id='immutability_run')
        ...
        self.assertEqual(context.agent_context, original_agent_context, 'Original agent_context should be unchanged')
        child_context = context.create_child_context(operation_name='immutability_test', additional_agent_context={'child_data': 'child_value'}, additional_audit_metadata={'child_audit': 'child_audit_value'})
    ...
    def test_factory_child_context_creation_maintains_hierarchy(self):
        ...
        """
        parent_context = UserContextFactory.create_context(user_id='hierarchy_test_user', thread_id='hierarchy_thread', run_id='hierarchy_run')
        child_contexts = []
        for i in range(3):
            child_context = parent_context.create_child_context(operation_name=f'child_operation_{i}', additional_agent_context={'child_index': i, 'child_operation_type': f'type_{i}'}, additional_audit_metadata={'child_created_at': datetime.now(timezone.utc).isoformat(), 'parent_context_id': parent_context.request_id})
        ...
        grandchild = child_contexts[0].create_child_context(operation_name='grandchild_operation')
    ...
    def test_factory_database_session_integration(self):
        ...
        context_with_session = UserContextFactory.create_with_session(user_id='db_session_test_user', thread_id='db_thread', run_id='db_run', db_session=mock_db_session, websocket_client_id='db_ws_client')
        ...
        child_with_session = context_with_session.create_child_context(operation_name='db_child_operation')
    ...
    def test_factory_websocket_context_creation_patterns(self):
        ...
        """
        websocket_contexts = []
        connection_scenarios = [('new_connection', 'websocket_session'), ('reconnection', 'websocket_reconnect'), ('agent_execution', 'agent_websocket'), ('background_task', 'background_websocket')]
        for i, (scenario, operation) in enumerate(connection_scenarios):
            user_id = f'websocket_user_{scenario}_{i}'
            ws_context = UserExecutionContext.from_websocket_request(user_id=user_id, websocket_client_id=f'ws_client_{scenario}_{i}', operation=operation)
        ...
        ws_child = base_ws_context.create_child_context(operation_name='websocket_agent_execution', additional_agent_context={'agent_type': 'chat_agent'})
    ...
    async def test_factory_managed_context_resource_cleanup(self):
        ...
        context = UserContextFactory.create_context(user_id='cleanup_test_user', thread_id='cleanup_thread', run_id='cleanup_run')
    ...
    def test_factory_resource_tracking_and_leak_prevention(self):
        ...
        def create_context_with_resources(user_index: int) -> Tuple[UserExecutionContext, List[Any]]:
            ...
            context = UserContextFactory.create_context(user_id=f'resource_user_{user_index}', thread_id=f'resource_thread_{user_index}', run_id=f'resource_run_{user_index}')
            mock_resources = [{'type': 'database_connection', 'id': f'db_{user_index}', 'cleanup_called': False}, {'type': 'websocket_connection', 'id': f'ws_{user_index}', 'cleanup_called': False}, {'type': 'temp_file', 'id': f'file_{user_index}', 'cleanup_called': False}]
        ...
        for context, resources in context_resource_pairs:
            ...
            for resource in resources:
                self.assertTrue(resource['cleanup_called'], f"Resource {resource['id']} should be cleaned up")
    ...
    async def test_factory_concurrent_resource_cleanup_safety(self):
        ...
        async def create_and_cleanup_context(context_id: int) -> Dict[str, Any]:
            ...
            context = UserContextFactory.create_context(user_id=f'concurrent_cleanup_user_{context_id}', thread_id=f'concurrent_cleanup_thread_{context_id}', run_id=f'concurrent_cleanup_run_{context_id}')
            ...
            def mock_sync_cleanup():
                cleanup_call_count['count'] += 1
            context.cleanup_callbacks.extend([mock_async_cleanup, mock_sync_cleanup, mock_async_cleanup])
            await context.cleanup()
            cleanup_end_time = time.time()
            result = {'context_id': context_id, 'cleanup_duration': cleanup_end_time - cleanup_start_time, 'cleanup_calls': cleanup_call_count['count'], 'success': cleanup_call_count['count'] == 3}
    ...
    async def test_factory_cleanup_error_resilience(self):
        ...
        """
        context = UserContextFactory.create_context(user_id='cleanup_error_test_user', thread_id='cleanup_error_thread', run_id='cleanup_error_run')
        ...
        context.cleanup_callbacks.extend([successful_async_cleanup, failing_async_cleanup, successful_sync_cleanup, failing_sync_cleanup, successful_async_cleanup])
        await context.cleanup()
        expected_log = ['async_success', 'sync_attempt', 'sync_success', 'async_attempt', 'async_success']
        self.assertEqual(cleanup_execution_log, expected_log, 'Cleanup should execute in LIFO order despite failures')
        self.assertEqual(len(context.cleanup_callbacks), 0, 'Cleanup callbacks should be cleared after execution')
    ...
    async def test_factory_context_lifecycle_resource_boundaries(self):
        ...
        async with managed_user_context(UserContextFactory.create_context(user_id='lifecycle_test_user', thread_id='lifecycle_thread', run_id='lifecycle_run')) as managed_ctx:
            tracker1 = ResourceLifecycleTracker('resource_1')
            tracker2 = ResourceLifecycleTracker('resource_2')
            managed_ctx.cleanup_callbacks.extend([tracker1.cleanup, tracker2.cleanup])
        ...
        expected_events = ['allocated_resource_1', 'allocated_resource_2', 'allocated_resource_3', 'deallocated_resource_3', 'deallocated_resource_2', 'deallocated_resource_1']
        self.assertEqual(lifecycle_events, expected_events, 'Resource lifecycle events should follow expected pattern')
    ...
    def test_factory_websocket_routing_isolation_per_user(self):
        ...
        for i in range(8):
            ...
            ws_context = UserExecutionContext.from_websocket_request(user_id=user_id, websocket_client_id=f'routing_client_{i}', operation='message_routing')
        ...
        child_context = base_context.create_child_context(operation_name='child_message_handling', additional_agent_context={'message_type': 'agent_response'})
    ...
    def test_factory_websocket_connection_context_isolation(self):
        ...
        connection_contexts = []
        connection_states = {}
        connection_scenarios = [('initial_connect', 'user_connect_1'), ('reconnect_after_drop', 'user_reconnect_1'), ('multiple_tabs', 'user_multitab_1'), ('mobile_connection', 'user_mobile_1'), ('background_connection', 'user_background_1')]
        for scenario, user_base in connection_scenarios:
            user_id = f'{user_base}_{scenario}'
            ws_context = UserExecutionContext.from_websocket_request(user_id=user_id, websocket_client_id=f'conn_{scenario}_{hash(user_id) % 1000}', operation=f'connection_{scenario}')
            connection_contexts.append((scenario, ws_context))
            connection_states[ws_context.websocket_client_id] = {'user_id': user_id, 'scenario': scenario, 'context_id': ws_context.request_id, 'connected': True, 'isolation_verified': ws_context.verify_isolation()}
        ...
        child_with_update = test_context.create_child_context(operation_name='connection_update', additional_agent_context={'connection_status': 'updated'})
    ...
    def test_factory_websocket_message_context_boundaries(self):
        ...
        for user in users:
            base_context = UserExecutionContext.from_websocket_request(user_id=f'message_user_{user}', websocket_client_id=f'message_client_{user}', operation='message_handling')
            user_contexts[user] = base_context
            message_contexts[user] = []
            message_types = ['user_message', 'agent_response', 'system_notification']
            for msg_type in message_types:
                msg_context = base_context.create_child_context(operation_name=f'{msg_type}_handling', additional_agent_context={'message_type': msg_type, 'user': user, 'timestamp': datetime.now(timezone.utc).isoformat()}, additional_audit_metadata={'message_category': msg_type, 'routing_user': user})
    ...
    async def test_factory_websocket_agent_execution_routing_isolation(self):
        ...
        for i in range(4):
            ...
            ws_context = UserExecutionContext.from_websocket_request(user_id=user_id, websocket_client_id=f'agent_exec_client_{i}', operation='agent_execution_routing')
            agent_execution_contexts.append(ws_context)
            execution_routing_map[user_id] = {'websocket_client_id': ws_context.websocket_client_id, 'base_context': ws_context, 'agent_executions': []}
        for context in agent_execution_contexts:
            user_routing = execution_routing_map[context.user_id]
            agent_context = context.create_child_context(operation_name='agent_execution', additional_agent_context={'agent_type': 'cost_optimizer', 'execution_id': f'exec_{uuid.uuid4().hex[:8]}', 'routing_target': context.websocket_client_id}, additional_audit_metadata={'execution_start': datetime.now(timezone.utc).isoformat(), 'target_user': context.user_id, 'websocket_routing': context.websocket_client_id})
            ...
            result_context = agent_context.create_child_context(operation_name='agent_result_delivery', additional_agent_context={'result_type': 'optimization_complete', 'delivery_target': context.websocket_client_id, 'result_data': {'savings': 1000, 'recommendations': 5}})
    ...
    async def test_factory_concurrent_websocket_context_isolation(self):
        ...
        async def simulate_concurrent_websocket_operations(operation_id: int) -> Dict[str, Any]:
            ...
            ws_context = UserExecutionContext.from_websocket_request(user_id=user_id, websocket_client_id=f'concurrent_ws_client_{operation_id}', operation='concurrent_websocket_test')
            operation_start = time.time()
            child_contexts = []
            for op_type in ['message', 'agent_execution', 'notification']:
                await asyncio.sleep(0.001)
                child_ctx = ws_context.create_child_context(operation_name=f'concurrent_{op_type}', additional_agent_context={'operation_type': op_type, 'operation_id': operation_id, 'concurrent_timestamp': time.time()})
            ...
            result = {'operation_id': operation_id, 'user_id': user_id, 'websocket_client_id': ws_context.websocket_client_id, 'base_context_id': ws_context.request_id, 'child_context_count': len(child_contexts), 'isolation_success': all(isolation_results), 'operation_duration': operation_end - operation_start, 'child_context_ids': [ctx.request_id for ctx in child_contexts]}
        ...
        for result in results:
            self.assertTrue(result['isolation_success'], f"Operation {result['operation_id']} should maintain isolation")
            self.assertEqual(result['child_context_count'], 3, 'Each operation should create 3 child contexts')
        ...
        self.assertEqual(len(set(all_child_context_ids)), 75, 'All child context IDs should be unique (25 operations  x  3 children)')
