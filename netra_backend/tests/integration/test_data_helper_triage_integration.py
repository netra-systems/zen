...
async def test_users():
    ...
    users = []
    for i in range(3):
        user = User(id=f'test_user_{i}_{uuid.uuid4().hex[:8]}', email=f'datahelper_test_{i}@netrasystems.ai', username=f'datahelper_user_{i}', is_active=True, created_at=datetime.now(timezone.utc))
        users.append(user)
    yield users
