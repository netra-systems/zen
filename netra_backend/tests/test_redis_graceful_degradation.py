...
class RedisGracefulDegradationTests:
    ...
    async def test_backend_startup_without_redis_should_succeed(self):
        ...
        """
        with patch.dict(os.environ, {'REDIS_REQUIRED': 'false', 'REDIS_FALLBACK_ENABLED': 'false', 'REDIS_URL': 'redis://nonexistent:6379/0', 'ENVIRONMENT': 'development', 'TEST_DISABLE_REDIS': 'true'}):
    ...
    async def test_health_endpoints_with_redis_down_should_be_degraded_not_failed(self):
        ...
        """
        with patch.dict(os.environ, {'REDIS_URL': 'redis://nonexistent:6379/0', 'REDIS_REQUIRED': 'false', 'ENVIRONMENT': 'development'}):
            try:
                readiness_response = await readiness_probe()
                assert readiness_response.status in ['healthy', 'degraded'], f'Health should be degraded when Redis down, got: {readiness_response.status}'
                if 'redis' in readiness_response.checks:
                    redis_check = readiness_response.checks['redis']
                    assert redis_check['status'] == 'unhealthy', 'Redis check should show unhealthy when Redis is down'
                assert readiness_response.checks.get('database', {}).get('status') == 'healthy', 'Database should remain healthy when only Redis is down'
            except Exception as e:
    ...
    async def test_core_business_logic_without_redis_should_function(self):
        ...
        """
        with patch.dict(os.environ, {'REDIS_REQUIRED': 'false', 'REDIS_FALLBACK_ENABLED': 'false', 'REDIS_URL': 'redis://invalid-host:6379/0', 'ENVIRONMENT': 'development'}):
            redis_manager = RedisManager()
            try:
    ...
    async def test_redis_connection_retry_logic_eventually_gives_up(self):
        ...
        with patch.dict(os.environ, {'REDIS_URL': 'redis://192.0.2.1:6379/0', 'REDIS_REQUIRED': 'false', 'ENVIRONMENT': 'development'}):
            redis_manager = RedisManager()
            try:
    ...
    async def test_redis_configuration_loading_handles_missing_secrets(self):
        ...
        """
        with patch.dict(os.environ, {'GCP_PROJECT_ID': '', 'GOOGLE_APPLICATION_CREDENTIALS': '', 'REDIS_URL': '', 'REDIS_HOST': '', 'ENVIRONMENT': 'staging'}, clear=False):
            try:
