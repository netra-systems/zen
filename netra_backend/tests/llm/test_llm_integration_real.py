...
import pytest
from netra_backend.app.llm.llm_response_processing import attempt_json_fallback_parse, create_llm_response, extract_response_content, parse_nested_json_recursive
...
def test_llm_config():
    ...
    return AppConfig(llm_configs={'primary': LLMConfig(provider='openai', model_name=LLMModel.GEMINI_2_5_FLASH.value, api_key='test-key', generation_config={'temperature': 0.7}), 'fallback': LLMConfig(provider='openai', model_name=LLMModel.GEMINI_2_5_FLASH.value, api_key='test-key', generation_config={'temperature': 0.5})})
...
class TestRealResponsePatterns:
    ...
    def test_string_vs_dict_field_conversion(self):
        ...
        mixed_response = {'category': 'analysis', 'confidence': 0.8, 'recommendations': [{'tool_config': '{"timeout": 30, "retries": 3}'}], 'metadata': '{"source": "llm", "timestamp": "2024-01-01T00:00:00Z"}'}
    ...
    def test_deeply_nested_json_strings(self):
        ...
        nested_data = {'analysis_id': 'test_001', 'results': '{"metrics": "{\\"latency\\": 50, \\"throughput\\": 1000}"}', 'tool_configs': [{'config': '{"param1": "value1"}'}], 'performance_metrics': {'cpu': 0.75, 'memory': 0.6}}
...
class TestCostOptimization:
    ...
    async def test_cheap_model_for_simple_tasks(self, llm_manager):
        ...
        mock_llm_response = LLMResponse(choices=[{'message': {'content': 'positive'}}], usage=TokenUsage(prompt_tokens=20, completion_tokens=5, total_tokens=25), provider=LLMProvider.OPENAI, model=LLMModel.GEMINI_2_5_FLASH.value, response_time_ms=100.0)
        with patch.object(llm_manager._core, 'ask_llm_full') as mock_ask_full:
    ...
    async def test_expensive_model_for_complex_reasoning(self, llm_manager):
        ...
        mock_llm_response = LLMResponse(choices=[{'message': {'content': 'Detailed analysis...'}}], usage=TokenUsage(prompt_tokens=100, completion_tokens=50, total_tokens=150), provider=LLMProvider.OPENAI, model=LLMModel.GEMINI_2_5_FLASH.value, response_time_ms=200.0)
        with patch.object(llm_manager._core, 'ask_llm_full') as mock_ask_full:
            mock_ask_full.return_value = mock_llm_response
...
class TestStructuredGenerationEdgeCases:
    ...
    async def test_structured_output_with_string_fallback(self, llm_manager):
        ...
        fallback_response = ResponseModel(category='test', confidence=0.7, recommendations=[])
        with patch.object(llm_manager._structured, 'get_structured_llm') as mock_get_structured:
            mock_structured_llm = AsyncNone
            ...
            with patch.object(llm_manager._structured, 'ask_structured_llm') as mock_ask_structured:
                ...
                result = await llm_manager.ask_structured_llm('test', 'primary', ResponseModel, use_cache=False)
    ...
    async def test_validation_error_handling(self, llm_manager):
        ...
        invalid_json = json.dumps({'category': 'test', 'confidence': 1.5, 'recommendations': 'not_a_list'})
...
class TestTokenUsageMonitoring:
    ...
    async def test_response_with_usage_metadata(self, llm_manager):
        ...
        expected_response = LLMResponse(choices=[{'message': {'content': 'Test response'}}], usage=TokenUsage(prompt_tokens=50, completion_tokens=25, total_tokens=75), provider=LLMProvider.OPENAI, model=LLMModel.GEMINI_2_5_FLASH.value, response_time_ms=150.0)
        with patch.object(llm_manager._core, 'ask_llm_full') as mock_ask_full:
...
class TestProviderSwitching:
    ...
    async def test_openai_to_anthropic_fallback(self, llm_manager):
        ...
        anthropic_response = LLMResponse(choices=[{'message': {'content': 'Anthropic response'}}], usage=TokenUsage(prompt_tokens=50, completion_tokens=30, total_tokens=80), provider=LLMProvider.OPENAI, model=LLMModel.GEMINI_2_5_FLASH.value, response_time_ms=100.0)
        with patch.object(llm_manager._core, 'ask_llm_full') as mock_ask_full:
