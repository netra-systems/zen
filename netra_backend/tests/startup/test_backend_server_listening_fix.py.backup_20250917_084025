...
class BackendServerController:
    ...
    def start_backend_server(self) -> bool:
        ...
        try:
            self.process = subprocess.Popen([sys.executable, str(main_path)], cwd=str(backend_path.parent), env=env_vars, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True)
...
class BackendServerListeningTests:
    ...
    def test_port_availability_check(self):
        ...
        assert port_available == controller_says_available, 'Port availability detection mismatch'
