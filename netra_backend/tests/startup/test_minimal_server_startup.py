    ...
    def start_server_thread(self) -> bool:
        ...
        def run_server():
            try:
                ...
                config = uvicorn.Config(self.app, host='0.0.0.0', port=self.port, log_level='error', access_log=False)
...
class FullAppServerTest:
    ...
    def create_netra_app_minimal(self) -> FastAPI:
        ...
        try:
            from fastapi import FastAPI
            app = FastAPI(title='Netra API Minimal Test', description='Minimal test version of Netra API', version='0.1.0-test')
...
class MinimalServerStartupTests:
    ...
    def test_minimal_fastapi_server_startup(self):
        ...
        if not listening:
            pytest.fail(f'Minimal server failed to start listening. Error: {self.minimal_server.startup_error}')
