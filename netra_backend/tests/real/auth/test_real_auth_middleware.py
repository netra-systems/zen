...
from netra_backend.app.core.auth_constants import JWTConstants, AuthErrorConstants, HeaderConstants, AuthConstants
...
class TestRealAuthMiddleware:
    ...
    @pytest.mark.asyncio
    async def test_middleware_valid_token_processing(self, async_client: AsyncClient, create_test_token: Callable[[Dict[str, Any]], str]):
        """Test middleware processes valid tokens correctly."""
        token = create_test_token({'user_id': 54321, 'permissions': ['read', 'write']})
    ...
    @pytest.mark.asyncio
    async def test_middleware_invalid_token_rejection(self, async_client: AsyncClient):
        """Test middleware rejects invalid tokens."""
        invalid_tokens = ['invalid.jwt.token', 'Bearer malformed_token', 'not_a_bearer_token', '']
        for invalid_token in invalid_tokens:
            if not invalid_token:
                continue
            headers = {HeaderConstants.AUTHORIZATION: invalid_token, HeaderConstants.CONTENT_TYPE: HeaderConstants.APPLICATION_JSON}
            try:
                response = await async_client.get('/health', headers=headers)
    ...
    async def test_middleware_malformed_authorization_header(self, async_client: AsyncClient):
        ...
        malformed_headers = [{'Authorization': 'Bearer'}, {'Authorization': 'Basic token'}, {'Authorization': 'Bearer token1 token2'}, {'Authorization': 'bearer lowercase_scheme'}, {'Authorization': 'Token jwt_token'}]
    ...
    @pytest.mark.asyncio
    async def test_middleware_performance_under_load(self, async_client: AsyncClient, create_test_token: Callable[[Dict[str, Any]], str]):
        """Test middleware performance under concurrent load."""
        token = create_test_token({'user_id': 88888, 'permissions': ['read']})
        headers = {HeaderConstants.AUTHORIZATION: f'{HeaderConstants.BEARER_PREFIX}{token}', HeaderConstants.CONTENT_TYPE: HeaderConstants.APPLICATION_JSON}
        ...
        async def make_request(request_id: int) -> Dict[str, Any]:
            ...
            try:
                ...
                return {'request_id': request_id, 'status_code': response.status_code, 'duration': end_time - start_time, 'success': response.status_code < 500}
            except Exception as e:
    ...
    async def test_middleware_error_handling_and_responses(self, async_client: AsyncClient):
        ...
        error_scenarios = [{'headers': {'Authorization': 'Bearer invalid_token'}, 'expected_behavior': 'Invalid token should be handled gracefully'}, {'headers': {'Authorization': 'InvalidScheme token'}, 'expected_behavior': 'Invalid scheme should be rejected'}, {'headers': {'Authorization': ''}, 'expected_behavior': 'Empty authorization should be handled'}]
    ...
    async def test_middleware_security_headers_and_cors(self, async_client: AsyncClient):
        ...
        cors_headers = {'Origin': 'http://localhost:3000', 'Access-Control-Request-Method': 'POST', 'Access-Control-Request-Headers': 'Authorization, Content-Type'}
        try:
