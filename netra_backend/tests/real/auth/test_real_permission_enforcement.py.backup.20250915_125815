...
from netra_backend.app.core.auth_constants import JWTConstants, AuthErrorConstants, HeaderConstants, ValidationConstants
...
class TestRealPermissionEnforcement:
    ...
    def role_permission_mapping(self) -> Dict[Role, List[Permission]]:
        ...
        return {Role.GUEST: [], Role.USER: [Permission.READ], Role.POWER_USER: [Permission.READ, Permission.WRITE], Role.ADMIN: [Permission.READ, Permission.WRITE, Permission.DELETE, Permission.VIEW_ANALYTICS], Role.SYSTEM_ADMIN: [Permission.READ, Permission.WRITE, Permission.DELETE, Permission.ADMIN, Permission.MANAGE_USERS, Permission.VIEW_ANALYTICS, Permission.EXPORT_DATA, Permission.SYSTEM_CONFIG]}
    ...
    async def test_role_based_permission_assignment(self, role_permission_mapping: Dict[Role, List[Permission]], jwt_secret_key: str):
        ...
        for role, expected_permissions in role_permission_mapping.items():
            ...
            assert actual_permissions == expected_permission_values, f'Role {role.value} permissions mismatch: expected {expected_permission_values}, got {actual_permissions}'
            test_users.append({'user_id': user_id, 'role': role, 'token': token, 'permissions': expected_permissions})
        ...
        for i in range(len(role_hierarchy) - 1):
            ...
            assert current_permissions.issubset(next_permissions) or len(current_permissions) == 0, f'Role hierarchy violation: {next_role.value} should have at least {current_role.value} permissions'
        print(f' PASS:  Role-based permission assignment validated for {len(test_users)} roles')

    @pytest.mark.asyncio
    async def test_permission_enforcement_on_resource_access(self, role_permission_mapping: Dict[Role, List[Permission]], jwt_secret_key: str):
        """Test permission enforcement when accessing protected resources."""
        protected_resources = [{'resource': self.create_protected_resource(20001, [Permission.READ]), 'required_permissions': [Permission.READ], 'description': 'Read-only resource'}, {'resource': self.create_protected_resource(20002, [Permission.READ, Permission.WRITE]), 'required_permissions': [Permission.READ, Permission.WRITE], 'description': 'Read-write resource'}, {'resource': self.create_protected_resource(20003, [Permission.ADMIN]), 'required_permissions': [Permission.ADMIN], 'description': 'Admin-only resource'}]
        for role, user_permissions in role_permission_mapping.items():
            ...
            for resource_data in protected_resources:
                ...
                if has_access:
                    ...
                    assert user_permission_values.intersection(required_permissions), f"{role.value} should have access to {resource_data['description']}"
                else:
                    print(f" FAIL:  {role.value} cannot access {resource_data['description']}")
                    assert not required_permissions.issubset(user_permission_values), f"{role.value} should NOT have access to {resource_data['description']}"
        print(' PASS:  Permission enforcement on resource access validated')

    @pytest.mark.asyncio
    async def test_unauthorized_access_prevention(self, jwt_secret_key: str, async_client: AsyncClient):
        """Test prevention of unauthorized access attempts."""
        limited_user_token = self.create_user_with_role(30001, Role.USER, [Permission.READ], jwt_secret_key)
        admin_user_token = self.create_user_with_role(30002, Role.ADMIN, [Permission.READ, Permission.WRITE, Permission.DELETE, Permission.VIEW_ANALYTICS], jwt_secret_key)
        admin_only_endpoints = ['/admin/users', '/admin/system-config', '/admin/analytics', '/admin/export-data']
        for endpoint in admin_only_endpoints:
            limited_headers = {HeaderConstants.AUTHORIZATION: f'{HeaderConstants.BEARER_PREFIX}{limited_user_token}', HeaderConstants.CONTENT_TYPE: HeaderConstants.APPLICATION_JSON}
            try:
                limited_response = await async_client.get(endpoint, headers=limited_headers)
                assert limited_response.status_code in [401, 403, 404], f'Limited user should be denied access to {endpoint}'
            ...
            admin_headers = {HeaderConstants.AUTHORIZATION: f'{HeaderConstants.BEARER_PREFIX}{admin_user_token}', HeaderConstants.CONTENT_TYPE: HeaderConstants.APPLICATION_JSON}
            try:
    ...
    async def test_permission_inheritance_and_delegation(self, jwt_secret_key: str):
        ...
        org_users = [{'user_id': 40001, 'role': Role.SYSTEM_ADMIN, 'level': 'organization'}, {'user_id': 40002, 'role': Role.ADMIN, 'level': 'workspace', 'parent': 40001}, {'user_id': 40003, 'role': Role.POWER_USER, 'level': 'team', 'parent': 40002}, {'user_id': 40004, 'role': Role.USER, 'level': 'individual', 'parent': 40003}]
        role_permission_mapping = {Role.SYSTEM_ADMIN: [Permission.READ, Permission.WRITE, Permission.DELETE, Permission.ADMIN, Permission.MANAGE_USERS], Role.ADMIN: [Permission.READ, Permission.WRITE, Permission.DELETE, Permission.VIEW_ANALYTICS], Role.POWER_USER: [Permission.READ, Permission.WRITE], Role.USER: [Permission.READ]}
        user_tokens = {}
        for user in org_users:
            permissions = role_permission_mapping[user['role']]
            token = self.create_user_with_role(user['user_id'], user['role'], permissions, jwt_secret_key, organizational_level=user['level'])
            user_tokens[user['user_id']] = {'token': token, 'role': user['role'], 'permissions': permissions, 'level': user['level']}
        for user in org_users:
            ...
            if 'parent' in user:
                ...
                parent_permissions = set(jwt.decode(user_tokens[parent_id]['token'], jwt_secret_key, algorithms=[JWTConstants.HS256_ALGORITHM])['permissions'])
                assert user_permissions.issubset(parent_permissions), f'User {user_id} permissions should inherit from parent {parent_id}'
    ...
    async def test_resource_level_permission_granularity(self, jwt_secret_key: str):
        ...
        resources = [{'id': 'doc_001', 'type': 'document', 'owner_id': 50001, 'permissions': {'read': [50001, 50002, 50003], 'write': [50001, 50002], 'delete': [50001]}}, {'id': 'report_002', 'type': 'report', 'owner_id': 50002, 'permissions': {'read': [50002, 50003], 'write': [50002], 'delete': [50002]}}]
        test_users = []
        for i in range(3):
            user_id = 50001 + i
            token = self.create_user_with_role(user_id, Role.POWER_USER, [Permission.READ, Permission.WRITE, Permission.DELETE], jwt_secret_key)
    ...
    async def test_permission_caching_and_invalidation(self, jwt_secret_key: str):
        ...
        assert expected_initial.issubset(expected_updated)
        ...
        assert not (expected_updated - expected_revoked).issubset(revoked_cached)
    ...
    async def test_cross_tenant_permission_isolation(self, jwt_secret_key: str):
        ...
        for i, tenant in enumerate(tenants):
            user_id = 70001 + i
            token = self.create_user_with_role(user_id, Role.ADMIN, [Permission.READ, Permission.WRITE, Permission.DELETE, Permission.ADMIN], jwt_secret_key, tenant_id=tenant, workspace_id=f'workspace_{tenant}')
            tenant_users[tenant] = {'user_id': user_id, 'token': token, 'tenant_id': tenant}
        tenant_resources = {}
        for tenant in tenants:
            tenant_resources[tenant] = {'resource_id': f'resource_{tenant}', 'tenant_id': tenant, 'content': f'Sensitive data for {tenant}', 'access_control': f'restricted_to_{tenant}'}
