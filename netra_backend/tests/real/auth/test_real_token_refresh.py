...
from netra_backend.app.core.auth_constants import JWTConstants, CacheConstants, AuthErrorConstants, HeaderConstants, AuthConstants
...
class TestRealTokenRefresh:
    ...
    def create_token_pair(self, user_id: int, jwt_secret_key: str) -> Tuple[str, str]:
        ...
        now = datetime.utcnow()
        access_payload = {JWTConstants.SUBJECT: f'user_{user_id}', JWTConstants.EMAIL: f'user{user_id}@netra.ai', JWTConstants.ISSUED_AT: int(now.timestamp()), JWTConstants.EXPIRES_AT: int((now + timedelta(minutes=15)).timestamp()), JWTConstants.ISSUER: JWTConstants.NETRA_AUTH_SERVICE, 'token_type': JWTConstants.ACCESS_TOKEN_TYPE, 'user_id': user_id, 'permissions': ['read', 'write']}
        refresh_payload = {JWTConstants.SUBJECT: f'user_{user_id}', JWTConstants.EMAIL: f'user{user_id}@netra.ai', JWTConstants.ISSUED_AT: int(now.timestamp()), JWTConstants.EXPIRES_AT: int((now + timedelta(days=7)).timestamp()), JWTConstants.ISSUER: JWTConstants.NETRA_AUTH_SERVICE, 'token_type': JWTConstants.REFRESH_TOKEN_TYPE, 'user_id': user_id, 'refresh_id': secrets.token_hex(16)}
    ...
    async def test_access_token_refresh_mechanism(self, redis_client, jwt_secret_key: str):
        ...
        refresh_id = refresh_decoded['refresh_id']
        refresh_key = f'{CacheConstants.SERVICE_TOKEN_PREFIX}refresh:{user_id}:{refresh_id}'
        refresh_data = {'user_id': user_id, 'refresh_token': refresh_token, 'refresh_id': refresh_id, 'is_active': True, 'use_count': 0}
        try:
            await redis_client.setex(refresh_key, CacheConstants.SERVICE_TOKEN_CACHE_TTL, json.dumps(refresh_data))
            ...
            now = datetime.utcnow()
            new_access_payload = {JWTConstants.SUBJECT: f'user_{user_id}', JWTConstants.EMAIL: f'user{user_id}@netra.ai', JWTConstants.ISSUED_AT: int(now.timestamp()), JWTConstants.EXPIRES_AT: int((now + timedelta(minutes=15)).timestamp()), JWTConstants.ISSUER: JWTConstants.NETRA_AUTH_SERVICE, 'token_type': JWTConstants.ACCESS_TOKEN_TYPE, 'user_id': user_id, 'permissions': ['read', 'write'], 'refreshed_at': now.isoformat()}
    ...
    async def test_token_rotation_on_refresh(self, redis_client, jwt_secret_key: str):
        ...
        try:
            ...
            new_refresh_payload = {JWTConstants.SUBJECT: f'user_{user_id}', JWTConstants.EMAIL: f'user{user_id}@netra.ai', JWTConstants.ISSUED_AT: int(now.timestamp()), JWTConstants.EXPIRES_AT: int((now + timedelta(days=7)).timestamp()), JWTConstants.ISSUER: JWTConstants.NETRA_AUTH_SERVICE, 'token_type': JWTConstants.REFRESH_TOKEN_TYPE, 'user_id': user_id, 'refresh_id': new_refresh_id, 'generation': 2}
            new_access_token = jwt.encode(new_access_payload, jwt_secret_key, algorithm=JWTConstants.HS256_ALGORITHM)
            ...
            new_refresh_data = {'user_id': user_id, 'refresh_token': new_refresh_token, 'refresh_id': new_refresh_id, 'is_active': True, 'use_count': 0, 'generation': 2, 'previous_refresh_id': old_refresh_id}
            ...
            await redis_client.setex(old_refresh_key, 3600, json.dumps(old_refresh_data))
            assert new_access_token != old_access_token
    ...
    async def test_expired_refresh_token_rejection(self, redis_client, jwt_secret_key: str):
        ...
        expired_refresh_payload = {JWTConstants.SUBJECT: f'user_{user_id}', JWTConstants.EMAIL: f'user{user_id}@netra.ai', JWTConstants.ISSUED_AT: int(expired_time.timestamp()), JWTConstants.EXPIRES_AT: int(expired_time.timestamp()), JWTConstants.ISSUER: JWTConstants.NETRA_AUTH_SERVICE, 'token_type': JWTConstants.REFRESH_TOKEN_TYPE, 'user_id': user_id, 'refresh_id': secrets.token_hex(16)}
    ...
    async def test_concurrent_token_refresh_prevention(self, redis_client, jwt_secret_key: str):
        ...
        try:
            ...
            async def attempt_refresh(attempt_id: int) -> Dict[str, Any]:
                ...
                try:
                    lock_acquired = await redis_client.set(lock_key, attempt_id, nx=True, ex=30)
    ...
    async def test_refresh_token_family_invalidation(self, redis_client, jwt_secret_key: str):
        ...
        try:
            for i in range(3):
                ...
                refresh_data = {'user_id': user_id, 'refresh_token': refresh_token, 'refresh_id': refresh_id, 'is_active': True, 'family_id': f'family_{user_id}', 'device_id': f'device_{i}'}
                await redis_client.setex(refresh_key, CacheConstants.SERVICE_TOKEN_CACHE_TTL, json.dumps(refresh_data))
            ...
            for refresh_key in refresh_keys:
                ...
                await redis_client.setex(refresh_key, 3600, json.dumps(data))
