...
class BusinessValueAdvancedTests(BusinessValueFixtures):
    ...
    async def _execute_with_mocked_state(self, supervisor, user_request: str, run_id: str):
        ...
        return await self._execute_with_patches(supervisor, user_request, run_id, save_mock, load_mock, context_mock)

    async def _execute_with_patches(self, supervisor, user_request, run_id, save_mock, load_mock, context_mock):
    ...
    def _create_cache_metrics(self):
        ...
        return {'hit_rate': 0.35, 'miss_rate': 0.65, 'avg_response_time_cached': 50, 'avg_response_time_uncached': 1500, 'cost_savings_percentage': 35}
    ...
    def _get_report_calls(self, llm_manager):
        ...
        return [call for call in llm_manager.ask_llm.call_args_list if 'report' in str(call).lower() or 'summary' in str(call).lower()]
    ...
    def _create_context_tracker(self, user_contexts):
        ...
        async def track_user_context(user_id, thread_id, request):
            user_contexts[user_id] = {'thread_id': thread_id, 'request': request}
    ...
    async def _execute_concurrent_requests(self, agent_service, user1_request, user2_request):
        ...
        await agent_service.start_agent_run(user1_request['user_id'], str(uuid.uuid4()), user1_request['request'])
        await agent_service.start_agent_run(user2_request['user_id'], str(uuid.uuid4()), user2_request['request'])
