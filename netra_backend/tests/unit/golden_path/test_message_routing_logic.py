...
class MockMessageRoutingEngine:
    ...
    def _initialize_agent_profiles(self) -> Dict[AgentID, AgentProfile]:
        ...
        return {AgentID('cost_optimizer'): AgentProfile(agent_id=AgentID('cost_optimizer'), capabilities={AgentCapability.COST_ANALYSIS}, specialties=['cloud cost optimization', 'savings analysis', 'resource rightsizing'], business_value_focus='Deliver quantifiable cost savings recommendations', response_quality_score=0.92, average_response_time=45.0, success_rate=0.89), AgentID('data_analyzer'): AgentProfile(agent_id=AgentID('data_analyzer'), capabilities={AgentCapability.DATA_PROCESSING, AgentCapability.COST_ANALYSIS}, specialties=['data analysis', 'pattern recognition', 'trend analysis'], business_value_focus='Extract actionable insights from complex data', response_quality_score=0.88, average_response_time=60.0, success_rate=0.85), AgentID('report_generator'): AgentProfile(agent_id=AgentID('report_generator'), capabilities={AgentCapability.REPORT_GENERATION}, specialties=['executive summaries', 'detailed reports', 'visualizations'], business_value_focus='Create comprehensive business reports', response_quality_score=0.9, average_response_time=90.0, success_rate=0.87), AgentID('triage_agent'): AgentProfile(agent_id=AgentID('triage_agent'), capabilities={AgentCapability.TRIAGE, AgentCapability.GENERAL_CONVERSATION}, specialties=['intent classification', 'requirement gathering', 'initial assessment'], business_value_focus='Route users to optimal AI expertise efficiently', response_quality_score=0.85, average_response_time=15.0, success_rate=0.93), AgentID('general_assistant'): AgentProfile(agent_id=AgentID('general_assistant'), capabilities={AgentCapability.GENERAL_CONVERSATION, AgentCapability.TECHNICAL_EXPERTISE}, specialties=['general assistance', 'clarifications', 'basic support'], business_value_focus='Provide helpful general assistance and guidance', response_quality_score=0.78, average_response_time=25.0, success_rate=0.82)}

    def _initialize_routing_rules(self) -> Dict[str, Any]:
        """Initialize business rules for message routing."""
        return {'prefer_specialist_over_generalist': True, 'consider_conversation_context': True, 'route_follow_ups_to_same_agent': True, 'fallback_to_triage_when_uncertain': True, 'minimum_confidence_for_direct_routing': 0.75, 'maximum_routing_chain_length': 3, 'business_value_prioritization': True, 'response_time_considerations': {'urgent_requests_max_time': 30.0, 'standard_requests_max_time': 120.0, 'complex_requests_max_time': 300.0}}

    def _initialize_message_patterns(self) -> Dict[MessageType, Dict[str, Any]]:
        """Initialize message patterns for classification."""
        return {MessageType.COST_OPTIMIZATION: {'keywords': ['optimize', 'reduce costs', 'save money', 'cost analysis', 'efficiency', 'budget'], 'patterns': ['how to reduce', 'optimize my', 'cost savings', 'lower expenses'], 'context_clues': ['monthly spend', 'cloud costs', 'AWS bill', 'resource usage'], 'business_priority': 'HIGH', 'typical_urgency': 'MEDIUM'}, MessageType.DATA_ANALYSIS: {'keywords': ['analyze', 'data', 'trends', 'patterns', 'insights', 'metrics'], 'patterns': ['analyze my', 'what does the data show', 'trends in', 'patterns'], 'context_clues': ['CSV', 'data file', 'metrics', 'dashboard'], 'business_priority': 'HIGH', 'typical_urgency': 'MEDIUM'}, MessageType.REPORT_REQUEST: {'keywords': ['report', 'summary', 'document', 'generate', 'create', 'export'], 'patterns': ['generate report', 'create summary', 'export data', 'detailed analysis'], 'context_clues': ['PDF', 'executive summary', 'stakeholders', 'presentation'], 'business_priority': 'MEDIUM', 'typical_urgency': 'LOW'}, MessageType.GENERAL_INQUIRY: {'keywords': ['help', 'how', 'what', 'explain', 'understand', 'question'], 'patterns': ['can you help', 'how do I', 'what is', 'explain to me'], 'context_clues': ['confused', "don't understand", 'clarify', 'explain'], 'business_priority': 'MEDIUM', 'typical_urgency': 'MEDIUM'}, MessageType.FOLLOW_UP: {'keywords': ['also', 'additionally', 'furthermore', 'and', 'more', 'continue'], 'patterns': ['also can you', 'and what about', 'continue with', 'more details'], 'context_clues': ['previous', 'earlier', 'before', 'last time'], 'business_priority': 'LOW', 'typical_urgency': 'LOW'}}
    ...
    def classify_message(self, message_content: str, conversation_context: Dict[str, Any]) -> MessageClassification:
        ...
        for message_type, pattern_info in self.message_patterns.items():
            ...
            pattern_matches = sum((1 for pattern in pattern_info['patterns'] if pattern in message_lower))
            score += pattern_matches * 0.4
            context_matches = sum((1 for clue in pattern_info['context_clues'] if clue.lower() in message_lower))
        ...
        business_value_potential = self._assess_business_value_potential(best_type, confidence, extracted_entities)
        return MessageClassification(message_type=best_type, confidence_level=confidence, key_intent=self._extract_primary_intent(message_content, best_type), extracted_entities=extracted_entities, context_indicators=context_indicators, urgency_level=pattern_info['typical_urgency'], business_value_potential=business_value_potential)

    def route_message(self, message_classification: MessageClassification, user_context: Dict[str, Any], conversation_context: Dict[str, Any]) -> RoutingDecision:
        ...
        routing_strategy = self._select_routing_strategy(message_classification, conversation_context, candidate_agents)
        selected_agent = self._select_best_agent(candidate_agents, routing_strategy, message_classification, conversation_context)
        if not selected_agent:
            selected_agent = AgentID('triage_agent')
            routing_strategy = RoutingStrategy.FALLBACK
        routing_confidence = self._calculate_routing_confidence(selected_agent, message_classification, routing_strategy)
        business_justification = self._generate_business_justification(selected_agent, message_classification, routing_strategy)
        ...
        alternative_agents = [agent_id for agent_id in candidate_agents if agent_id != selected_agent][:3]
        routing_decision = RoutingDecision(selected_agent=selected_agent, routing_strategy=routing_strategy, confidence_level=routing_confidence, business_justification=business_justification, expected_response_quality=expected_quality, estimated_completion_time=estimated_time, alternative_agents=alternative_agents)
        self.routing_history.append({'timestamp': time.time(), 'message_type': message_classification.message_type.value, 'selected_agent': str(selected_agent), 'confidence': routing_confidence.value, 'strategy': routing_strategy.value})
    ...
    def _determine_required_capabilities(self, classification: MessageClassification) -> Set[AgentCapability]:
        ...
        capability_mapping = {MessageType.COST_OPTIMIZATION: {AgentCapability.COST_ANALYSIS}, MessageType.DATA_ANALYSIS: {AgentCapability.DATA_PROCESSING}, MessageType.REPORT_REQUEST: {AgentCapability.REPORT_GENERATION}, MessageType.GENERAL_INQUIRY: {AgentCapability.GENERAL_CONVERSATION}, MessageType.TECHNICAL_SUPPORT: {AgentCapability.TECHNICAL_EXPERTISE}, MessageType.FOLLOW_UP: {AgentCapability.GENERAL_CONVERSATION}, MessageType.CLARIFICATION: {AgentCapability.GENERAL_CONVERSATION, AgentCapability.TRIAGE}}
    ...
    def _select_routing_strategy(self, classification: MessageClassification, conversation_context: Dict[str, Any], candidate_agents: List[AgentID]) -> RoutingStrategy:
        """Select appropriate routing strategy based on context."""
        if classification.message_type == MessageType.FOLLOW_UP and 'last_agent' in conversation_context:
            return RoutingStrategy.CONTEXT_AWARE
        if classification.confidence_level == ConfidenceLevel.HIGH and len(candidate_agents) > 0:
            return RoutingStrategy.DIRECT_MATCH
        if classification.confidence_level == ConfidenceLevel.MEDIUM and len(candidate_agents) > 1:
    ...
    def _select_best_agent(self, candidates: List[AgentID], strategy: RoutingStrategy, classification: MessageClassification, conversation_context: Dict[str, Any]) -> Optional[AgentID]:
        ...
        if strategy == RoutingStrategy.BEST_FIT:
            ...
            def business_value_score(agent_id: AgentID) -> float:
                ...
                speed_score = 1.0 - profile.average_response_time / 300.0
    ...
    def _calculate_routing_confidence(self, selected_agent: AgentID, classification: MessageClassification, strategy: RoutingStrategy) -> ConfidenceLevel:
        """Calculate confidence in routing decision."""
        agent_profile = self.agent_profiles.get(selected_agent)
        if not agent_profile:
            return ConfidenceLevel.LOW
        base_confidence = {ConfidenceLevel.HIGH: 0.9, ConfidenceLevel.MEDIUM: 0.7, ConfidenceLevel.LOW: 0.5}[classification.confidence_level]
        performance_factor = agent_profile.success_rate
        strategy_confidence = {RoutingStrategy.DIRECT_MATCH: 0.9, RoutingStrategy.CONTEXT_AWARE: 0.85, RoutingStrategy.BEST_FIT: 0.75, RoutingStrategy.CHAIN_ROUTING: 0.6, RoutingStrategy.FALLBACK: 0.4}[strategy]
        combined_confidence = base_confidence * 0.4 + performance_factor * 0.3 + strategy_confidence * 0.3
    ...
    def _generate_business_justification(self, selected_agent: AgentID, classification: MessageClassification, strategy: RoutingStrategy) -> str:
        """Generate business justification for routing decision."""
        agent_profile = self.agent_profiles.get(selected_agent)
        if not agent_profile:
            return 'Fallback routing due to no suitable agent match'
        justifications = {RoutingStrategy.DIRECT_MATCH: f'Direct route to {selected_agent} - optimal capability match for {classification.message_type.value} with {agent_profile.response_quality_score:.0%} success rate', RoutingStrategy.BEST_FIT: f'Best fit routing to {selected_agent} - {agent_profile.business_value_focus.lower()} aligns with user intent', RoutingStrategy.CONTEXT_AWARE: f'Context-aware routing to {selected_agent} - maintains conversation continuity for better user experience', RoutingStrategy.FALLBACK: f'Fallback routing to {selected_agent} - ensures user receives assistance while determining optimal routing', RoutingStrategy.CHAIN_ROUTING: f'Multi-agent routing through {selected_agent} - complex request requires specialized expertise chain'}
    ...
    def _extract_context_indicators(self, message_content: str, conversation_context: Dict[str, Any]) -> List[str]:
    ...
    def _extract_primary_intent(self, message_content: str, message_type: MessageType) -> str:
        ...
        message_lower = message_content.lower()
        intent_patterns = {MessageType.COST_OPTIMIZATION: {'reduce': 'reduce_costs', 'optimize': 'optimize_resources', 'save': 'save_money', 'analyze': 'analyze_costs'}, MessageType.DATA_ANALYSIS: {'analyze': 'analyze_data', 'show': 'show_insights', 'trends': 'identify_trends', 'patterns': 'find_patterns'}, MessageType.REPORT_REQUEST: {'generate': 'generate_report', 'create': 'create_document', 'export': 'export_data', 'summary': 'create_summary'}}
    ...
    def _assess_business_value_potential(self, message_type: MessageType, confidence: ConfidenceLevel, entities: List[str]) -> str:
        """Assess potential business value of the request."""
        base_values = {MessageType.COST_OPTIMIZATION: 'High - Direct cost savings impact', MessageType.DATA_ANALYSIS: 'High - Actionable insights generation', MessageType.REPORT_REQUEST: 'Medium - Executive communication value', MessageType.GENERAL_INQUIRY: 'Medium - User satisfaction and retention', MessageType.FOLLOW_UP: 'Low - Incremental value addition', MessageType.CLARIFICATION: 'Low - Support and clarification value'}
    ...
    def analyze_routing_performance(self) -> Dict[str, Any]:
        ...
        efficiency_score = (high_confidence_routings / total_routings * 0.4 + direct_match_routings / total_routings * 0.3 + (total_routings - fallback_routings) / total_routings * 0.3) * 100
        return {'routing_efficiency_score': efficiency_score, 'total_routings': total_routings, 'agent_utilization': agent_usage, 'strategy_distribution': strategy_usage, 'confidence_distribution': confidence_distribution, 'high_confidence_rate': high_confidence_routings / total_routings * 100, 'fallback_rate': fallback_routings / total_routings * 100, 'business_impact_assessment': self._assess_routing_business_impact(efficiency_score)}
...
class MessageRoutingLogicTests(SSotBaseTestCase):
    ...
    def test_agent_capability_matching_logic(self):
        ...
        cost_classification = MessageClassification(message_type=MessageType.COST_OPTIMIZATION, confidence_level=ConfidenceLevel.HIGH, key_intent='optimize_costs', extracted_entities=['aws', 'ec2', 'cloud'], context_indicators=['user_tier:enterprise'], urgency_level='MEDIUM', business_value_potential='High - Direct cost savings impact')
        user_context = {'user_tier': 'enterprise'}
        conversation_context = {}
        routing_decision = self.routing_engine.route_message(cost_classification, user_context, conversation_context)
        ...
        data_classification = MessageClassification(message_type=MessageType.DATA_ANALYSIS, confidence_level=ConfidenceLevel.HIGH, key_intent='analyze_data', extracted_entities=['csv', 'metrics'], context_indicators=[], urgency_level='MEDIUM', business_value_potential='High - Actionable insights generation')
        data_routing = self.routing_engine.route_message(data_classification, user_context, conversation_context)
    ...
    def test_context_aware_routing_logic(self):
        ...
        follow_up_classification = MessageClassification(message_type=MessageType.FOLLOW_UP, confidence_level=ConfidenceLevel.MEDIUM, key_intent='continue_analysis', extracted_entities=['also', 'additionally'], context_indicators=['previous:cost_analysis'], urgency_level='LOW', business_value_potential='Low - Incremental value addition')
        conversation_context = {'last_agent': 'cost_optimizer', 'previous_message_type': 'cost_optimization', 'conversation_length': 3}
        user_context = {'user_tier': 'enterprise'}
        follow_up_routing = self.routing_engine.route_message(follow_up_classification, user_context, conversation_context)
        ...
        no_context_routing = self.routing_engine.route_message(follow_up_classification, user_context, {})
    ...
    def test_fallback_routing_logic(self):
        ...
        uncertain_classification = MessageClassification(message_type=MessageType.GENERAL_INQUIRY, confidence_level=ConfidenceLevel.LOW, key_intent='unclear_request', extracted_entities=[], context_indicators=[], urgency_level='MEDIUM', business_value_potential='Medium - Standard assistance value')
        user_context = {}
        conversation_context = {}
        fallback_routing = self.routing_engine.route_message(uncertain_classification, user_context, conversation_context)
        ...
        complex_routing = self.routing_engine.route_message(complex_classification, user_context, conversation_context)
    ...
    def test_business_value_prioritization_logic(self):
        ...
        high_value_classification = MessageClassification(message_type=MessageType.COST_OPTIMIZATION, confidence_level=ConfidenceLevel.HIGH, key_intent='reduce_costs', extracted_entities=['aws', 'cloud', 'ec2', 's3'], context_indicators=['user_tier:enterprise'], urgency_level='HIGH', business_value_potential='High - Direct cost savings impact')
        low_value_classification = MessageClassification(message_type=MessageType.CLARIFICATION, confidence_level=ConfidenceLevel.MEDIUM, key_intent='clarify_process', extracted_entities=[], context_indicators=['user_tier:free'], urgency_level='LOW', business_value_potential='Low - Support and clarification value')
        user_context = {'user_tier': 'enterprise'}
        conversation_context = {}
        high_value_routing = self.routing_engine.route_message(high_value_classification, user_context, conversation_context)
        low_value_routing = self.routing_engine.route_message(low_value_classification, user_context, conversation_context)
        ...
        self.record_metric('high_value_specialist_routing', high_value_routing.selected_agent == AgentID('cost_optimizer'))

    @pytest.mark.unit
    def test_routing_strategy_selection_logic(self):
        """Test routing strategy selection based on different scenarios."""
        high_confidence_classification = MessageClassification(message_type=MessageType.COST_OPTIMIZATION, confidence_level=ConfidenceLevel.HIGH, key_intent='optimize_costs', extracted_entities=['aws'], context_indicators=[], urgency_level='MEDIUM', business_value_potential='High - Direct cost savings impact')
        ...
        strategy = self.routing_engine._select_routing_strategy(high_confidence_classification, {}, candidates)
        assert strategy == RoutingStrategy.DIRECT_MATCH
        medium_confidence_classification = MessageClassification(message_type=MessageType.DATA_ANALYSIS, confidence_level=ConfidenceLevel.MEDIUM, key_intent='analyze_data', extracted_entities=['data'], context_indicators=[], urgency_level='MEDIUM', business_value_potential='High - Actionable insights generation')
        ...
        medium_strategy = self.routing_engine._select_routing_strategy(medium_confidence_classification, {}, medium_candidates)
    ...
    def test_agent_performance_consideration_logic(self):
        ...
        cost_classification = MessageClassification(message_type=MessageType.COST_OPTIMIZATION, confidence_level=ConfidenceLevel.HIGH, key_intent='optimize_costs', extracted_entities=['cloud', 'costs'], context_indicators=[], urgency_level='MEDIUM', business_value_potential='High - Direct cost savings impact')
    ...
    def test_routing_confidence_calculation_logic(self):
        ...
        high_conf_classification = MessageClassification(message_type=MessageType.COST_OPTIMIZATION, confidence_level=ConfidenceLevel.HIGH, key_intent='optimize_costs', extracted_entities=['aws', 'ec2'], context_indicators=[], urgency_level='MEDIUM', business_value_potential='High - Direct cost savings impact')
        high_conf_routing_confidence = self.routing_engine._calculate_routing_confidence(AgentID('cost_optimizer'), high_conf_classification, RoutingStrategy.DIRECT_MATCH)
        assert high_conf_routing_confidence == ConfidenceLevel.HIGH
        low_conf_classification = MessageClassification(message_type=MessageType.GENERAL_INQUIRY, confidence_level=ConfidenceLevel.LOW, key_intent='unclear_request', extracted_entities=[], context_indicators=[], urgency_level='LOW', business_value_potential='Medium - Standard assistance value')
        low_conf_routing_confidence = self.routing_engine._calculate_routing_confidence(AgentID('general_assistant'), low_conf_classification, RoutingStrategy.FALLBACK)
        ...
        self.record_metric('high_confidence_routing_accuracy', high_conf_routing_confidence == ConfidenceLevel.HIGH)

    @pytest.mark.unit
    def test_alternative_agents_suggestion_logic(self):
        """Test alternative agents suggestion for routing flexibility."""
        cost_classification = MessageClassification(message_type=MessageType.COST_OPTIMIZATION, confidence_level=ConfidenceLevel.HIGH, key_intent='optimize_costs', extracted_entities=['aws'], context_indicators=[], urgency_level='MEDIUM', business_value_potential='High - Direct cost savings impact')
        routing_decision = self.routing_engine.route_message(cost_classification, {}, {})
    ...
    def test_routing_performance_analysis_logic(self):
        ...
        test_messages = [('Optimize my AWS costs', MessageType.COST_OPTIMIZATION), ('Analyze my data trends', MessageType.DATA_ANALYSIS), ('Generate a cost report', MessageType.REPORT_REQUEST), ('Help me understand the platform', MessageType.GENERAL_INQUIRY), ('Also can you show me more details', MessageType.FOLLOW_UP)]
        ...
        assert len(business_impact) > 20
        assert any((keyword in business_impact.lower() for keyword in ['excellent', 'good', 'fair', 'poor']))
    ...
    def test_enterprise_customer_routing_priority_logic(self):
        ...
        enterprise_classification = MessageClassification(message_type=MessageType.COST_OPTIMIZATION, confidence_level=ConfidenceLevel.HIGH, key_intent='optimize_costs', extracted_entities=['aws', 'enterprise'], context_indicators=['user_tier:enterprise', 'urgency:high'], urgency_level='HIGH', business_value_potential='High - Direct cost savings impact')
        enterprise_context = {'user_tier': 'enterprise', 'account_value': 'high'}
        enterprise_routing = self.routing_engine.route_message(enterprise_classification, enterprise_context, {})
        free_classification = MessageClassification(message_type=MessageType.COST_OPTIMIZATION, confidence_level=ConfidenceLevel.HIGH, key_intent='optimize_costs', extracted_entities=['aws'], context_indicators=['user_tier:free'], urgency_level='MEDIUM', business_value_potential='High - Direct cost savings impact')
        free_context = {'user_tier': 'free', 'account_value': 'low'}
        free_routing = self.routing_engine.route_message(free_classification, free_context, {})
