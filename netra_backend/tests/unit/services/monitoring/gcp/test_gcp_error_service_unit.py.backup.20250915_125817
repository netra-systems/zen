...
from netra_backend.app.schemas.monitoring_schemas import GCPErrorServiceConfig, GCPCredentialsConfig, ErrorQuery, ErrorResponse, ErrorSummary, ErrorDetailResponse, ErrorResolution, ErrorSeverity, ErrorStatus, GCPError, GCPErrorEvent, MonitoringErrorContext
...
class TestGCPErrorServiceUnit(SSotBaseTestCase):
    ...
    def setup_method(self):
        ...
        super().setup_method()
        self.test_config = GCPErrorServiceConfig(project_id='test-project', credentials=GCPCredentialsConfig(project_id='test-project'), rate_limit_per_minute=100, batch_size=50, timeout_seconds=30, retry_attempts=3, enable_pii_redaction=True)
    ...
    def test_build_list_request(self):
        ...
        print('=' * 32)
        service = GCPErrorService(self.test_config)
        query = ErrorQuery(status=ErrorStatus.OPEN, limit=25, service='test-service', severity=ErrorSeverity.ERROR, time_range='6h', page_token='test-token')
        ...
        assert request_minimal['page_size'] == 50
    ...
    def test_count_by_severity(self):
        ...
        print('=' * 31)
        service = GCPErrorService(self.test_config)
        errors = [GCPError(id='error1', message='Critical error', service='test-service', severity=ErrorSeverity.CRITICAL, first_seen=datetime.now(timezone.utc), last_seen=datetime.now(timezone.utc)), GCPError(id='error2', message='Regular error', service='test-service', severity=ErrorSeverity.ERROR, first_seen=datetime.now(timezone.utc), last_seen=datetime.now(timezone.utc)), GCPError(id='error3', message='Another critical', service='test-service', severity=ErrorSeverity.CRITICAL, first_seen=datetime.now(timezone.utc), last_seen=datetime.now(timezone.utc)), GCPError(id='error4', message='Warning message', service='test-service', severity=ErrorSeverity.WARNING, first_seen=datetime.now(timezone.utc), last_seen=datetime.now(timezone.utc))]
        counts = service._count_by_severity(errors)
        assert counts['critical_errors'] == 2
        assert counts['error_errors'] == 1
        assert counts['warning_errors'] == 0
    ...
    def test_count_by_status(self):
        ...
        print('=' * 28)
        service = GCPErrorService(self.test_config)
        errors = [GCPError(id='error1', message='Open error', service='test-service', severity=ErrorSeverity.ERROR, status=ErrorStatus.OPEN, first_seen=datetime.now(timezone.utc), last_seen=datetime.now(timezone.utc)), GCPError(id='error2', message='Resolved error', service='test-service', severity=ErrorSeverity.ERROR, status=ErrorStatus.RESOLVED, first_seen=datetime.now(timezone.utc), last_seen=datetime.now(timezone.utc)), GCPError(id='error3', message='Another open', service='test-service', severity=ErrorSeverity.ERROR, status=ErrorStatus.OPEN, first_seen=datetime.now(timezone.utc), last_seen=datetime.now(timezone.utc))]
    ...
    async def test_create_summary(self):
        ...
        print('=' * 35)
        service = GCPErrorService(self.test_config)
        errors = [GCPError(id='error1', message='Test error 1', service='service-a', severity=ErrorSeverity.CRITICAL, status=ErrorStatus.OPEN, first_seen=datetime.now(timezone.utc), last_seen=datetime.now(timezone.utc)), GCPError(id='error2', message='Test error 2', service='service-b', severity=ErrorSeverity.ERROR, status=ErrorStatus.RESOLVED, first_seen=datetime.now(timezone.utc), last_seen=datetime.now(timezone.utc))]
    ...
    async def test_fetch_errors_rate_limiting(self):
        ...
        print('=' * 40)
        service = GCPErrorService(self.test_config)
        service.rate_limiter.enforce_rate_limit = AsyncMock(side_effect=Exception('Rate limit exceeded'))
        query = ErrorQuery()
    ...
    def test_get_service_status(self):
        ...
        print('=' * 31)
        service = GCPErrorService(self.test_config)
        service.rate_limiter.get_current_usage = MagicMock(return_value={'requests': 10, 'window_start': 1000})
    ...
    async def test_build_error_context(self):
        ...
        print('=' * 33)
        service = GCPErrorService(self.test_config)
        service.rate_limiter.get_current_usage = MagicMock(return_value={'current': 5, 'limit': 100})
        test_error = GCPError(id=self.test_error_id, message='Test error', service='test-service', severity=ErrorSeverity.ERROR, first_seen=datetime.now(timezone.utc), last_seen=datetime.now(timezone.utc))
    ...
    async def test_mark_error_resolved(self):
        ...
        print('=' * 33)
        service = GCPErrorService(self.test_config)
        resolution = ErrorResolution(resolution_note='Fixed in deployment v1.2.3', resolved_by='engineer@company.com')
