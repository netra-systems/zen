...
class TestAuthResult(BaseTestCase):
    ...
    def test_auth_result_constructor_success(self):
        ...
        result = AuthResult(success=True, user_id='user123', email='test@example.com', permissions=['read', 'write'], metadata={'context': 'test'})
    ...
    def test_auth_result_constructor_failure(self):
        ...
        result = AuthResult(success=False, error='Invalid token', error_code='INVALID_TOKEN', metadata={'debug': 'token_length_0'})
    ...
    def test_auth_result_to_dict_success(self):
        ...
        result = AuthResult(success=True, user_id='user456', email='user@test.com', permissions=['admin'], metadata={'source': 'test'})
        dict_result = result.to_dict()
        expected_keys = {'valid', 'success', 'user_id', 'email', 'permissions', 'error', 'error_code', 'metadata', 'validated_at'}
    ...
    def test_auth_result_to_dict_failure(self):
        ...
        result = AuthResult(success=False, error='Service unavailable', error_code='SERVICE_ERROR')
        dict_result = result.to_dict()
...
class TestAuthenticationEnums(BaseTestCase):
    ...
    def test_authentication_method_enum_values(self):
        ...
        expected_methods = {'jwt_token', 'basic_auth', 'api_key', 'service_account'}
    ...
    def test_authentication_context_enum_values(self):
        ...
        expected_contexts = {'rest_api', 'websocket', 'graphql', 'grpc', 'internal_service'}
...
class TestAuthenticateToken(AsyncTestCase):
    ...
    async def test_authenticate_token_jwt_success(self, test_env):
        ...
        service = UnifiedAuthenticationService()
        service._auth_client.validate_token = AsyncMock(return_value={'valid': True, 'user_id': 'user123', 'email': 'test@example.com', 'permissions': ['read', 'write'], 'iat': 1234567890, 'exp': 1234567999})
        with patch('netra_backend.app.services.unified_authentication_service.validate_jwt_format', return_value=True):
            result = await service.authenticate_token('eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9.test.signature', context=AuthenticationContext.REST_API, method=AuthenticationMethod.JWT_TOKEN)
        assert result.success is True
        assert result.user_id == 'user123'
        assert result.email == 'test@example.com'
        assert result.permissions == ['read', 'write']
    ...
    async def test_authenticate_token_invalid_format(self, test_env):
        ...
        with patch('netra_backend.app.services.unified_authentication_service.validate_jwt_format', return_value=False):
            result = await service.authenticate_token('invalid-token', context=AuthenticationContext.WEBSOCKET, method=AuthenticationMethod.JWT_TOKEN)
        assert result.success is False
        assert result.user_id is None
    ...
    async def test_authenticate_token_validation_failed(self, test_env):
        ...
        service = UnifiedAuthenticationService()
        service._auth_client.validate_token = AsyncMock(return_value={'valid': False, 'error': 'Token expired', 'details': {'exp': 1234567890, 'current': 1234568000}})
        with patch('netra_backend.app.services.unified_authentication_service.validate_jwt_format', return_value=True):
            result = await service.authenticate_token('eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9.expired.signature', context=AuthenticationContext.GRAPHQL, method=AuthenticationMethod.JWT_TOKEN)
        assert result.success is False
        assert result.user_id is None
    ...
    async def test_authenticate_token_auth_service_error(self, test_env):
        ...
        service = UnifiedAuthenticationService()
        service._auth_client.validate_token = AsyncMock(side_effect=AuthServiceConnectionError('Connection refused'))
        service._validate_token_with_enhanced_resilience = AsyncMock(side_effect=AuthServiceConnectionError('Connection refused'))
        with patch('netra_backend.app.services.unified_authentication_service.validate_jwt_format', return_value=True):
            result = await service.authenticate_token('eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9.test.signature', context=AuthenticationContext.GRPC, method=AuthenticationMethod.API_KEY)
        assert result.success is False
    ...
    async def test_authenticate_token_unexpected_error(self, test_env):
        ...
        service = UnifiedAuthenticationService()
        service._auth_client.validate_token = AsyncMock(side_effect=ValueError('Unexpected validation error'))
        service._validate_token_with_enhanced_resilience = AsyncMock(side_effect=ValueError('Unexpected validation error'))
        with patch('netra_backend.app.services.unified_authentication_service.validate_jwt_format', return_value=True):
            result = await service.authenticate_token('eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9.test.signature', context=AuthenticationContext.INTERNAL_SERVICE, method=AuthenticationMethod.SERVICE_ACCOUNT)
        assert result.success is False
    ...
    async def test_authenticate_token_no_validation_result(self, test_env):
        ...
        with patch('netra_backend.app.services.unified_authentication_service.validate_jwt_format', return_value=True):
            result = await service.authenticate_token('eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9.test.signature')
        assert result.success is False
...
class TestWebSocketAuthentication(AsyncTestCase):
    ...
    async def test_authenticate_websocket_authorization_header_success(self, test_env):
        ...
        service = UnifiedAuthenticationService()
        websocket = self.create_mock_websocket({'authorization': 'Bearer eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9.test.signature', 'sec-websocket-protocol': 'chat'})
        service._auth_client.validate_token = AsyncMock(return_value={'valid': True, 'user_id': 'ws_user123', 'email': 'ws@example.com', 'permissions': ['websocket', 'chat']})
        with patch('netra_backend.app.services.unified_authentication_service.validate_jwt_format', return_value=True):
    ...
    async def test_authenticate_websocket_subprotocol_success(self, test_env):
        ...
        websocket = self.create_mock_websocket({'sec-websocket-protocol': f'jwt.{encoded_token}, chat'})
        service._auth_client.validate_token = AsyncMock(return_value={'valid': True, 'user_id': 'subprotocol_user', 'email': 'sub@example.com', 'permissions': ['read']})
        with patch('netra_backend.app.services.unified_authentication_service.validate_jwt_format', return_value=True):
            auth_result, user_context = await service.authenticate_websocket(websocket)
        assert auth_result.success is True
    ...
    async def test_authenticate_websocket_query_params_fallback(self, test_env):
        ...
        service = UnifiedAuthenticationService()
        websocket = self.create_mock_websocket(headers={}, query_params={'token': 'eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9.query.signature'})
        service._auth_client.validate_token = AsyncMock(return_value={'valid': True, 'user_id': 'query_user', 'email': 'query@example.com', 'permissions': []})
        with patch('netra_backend.app.services.unified_authentication_service.validate_jwt_format', return_value=True):
            auth_result, user_context = await service.authenticate_websocket(websocket)
        assert auth_result.success is True
    ...
    async def test_authenticate_websocket_no_token(self, test_env):
        ...
        service = UnifiedAuthenticationService()
        websocket = self.create_mock_websocket({'user-agent': 'WebSocket Client', 'sec-websocket-version': '13'})
        auth_result, user_context = await service.authenticate_websocket(websocket)
    ...
    async def test_authenticate_websocket_token_validation_failed(self, test_env):
        ...
        service = UnifiedAuthenticationService()
        websocket = self.create_mock_websocket({'authorization': 'Bearer invalid.token.signature'})
        service._auth_client.validate_token = AsyncMock(return_value={'valid': False, 'error': 'Invalid signature'})
        with patch('netra_backend.app.services.unified_authentication_service.validate_jwt_format', return_value=True):
            auth_result, user_context = await service.authenticate_websocket(websocket)
        assert auth_result.success is False
    ...
    async def test_authenticate_websocket_exception_handling(self, test_env):
        ...
        service = UnifiedAuthenticationService()
        websocket = self.create_mock_websocket({'authorization': 'Bearer eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9.test.signature'})
...
class TestExtractWebSocketToken(BaseTestCase):
    ...
    def test_extract_websocket_token_authorization_header_with_spaces(self, test_env):
        ...
        service = UnifiedAuthenticationService()
        websocket = self.create_mock_websocket({'authorization': 'Bearer   eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9.spaced.signature   '})
        token = service._extract_websocket_token(websocket)
    ...
    def test_extract_websocket_token_subprotocol(self, test_env):
        ...
        encoded_token = base64.urlsafe_b64encode(test_token.encode()).decode().rstrip('=')
        websocket = self.create_mock_websocket({'sec-websocket-protocol': f'jwt.{encoded_token}, chat, binary'})
        token = service._extract_websocket_token(websocket)
    ...
    def test_extract_websocket_token_subprotocol_with_padding(self, test_env):
        ...
        encoded_token = base64.urlsafe_b64encode(test_token.encode()).decode().rstrip('=')
        websocket = self.create_mock_websocket({'sec-websocket-protocol': f'jwt.{encoded_token}'})
        token = service._extract_websocket_token(websocket)
        assert token == test_token
    ...
    def test_extract_websocket_token_priority_order(self, test_env):
        ...
        websocket = self.create_mock_websocket(headers={'authorization': 'Bearer eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9.auth.signature', 'sec-websocket-protocol': f'jwt.{encoded_token}'}, query_params={'token': 'eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9.query.signature'})
        token = service._extract_websocket_token(websocket)
    ...
    def test_extract_websocket_token_no_bearer_prefix(self, test_env):
        ...
        service = UnifiedAuthenticationService()
        websocket = self.create_mock_websocket({'authorization': 'Basic dXNlcjpwYXNzd29yZA=='})
        token = service._extract_websocket_token(websocket)
    ...
    def test_extract_websocket_token_invalid_subprotocol_encoding(self, test_env):
        ...
        service = UnifiedAuthenticationService()
        websocket = self.create_mock_websocket({'sec-websocket-protocol': 'jwt.invalid@base64!, chat'})
        token = service._extract_websocket_token(websocket)
        assert token is None
...
class TestServiceTokenValidation(AsyncTestCase):
    ...
    async def test_validate_service_token_success(self, test_env):
        ...
        service = UnifiedAuthenticationService()
        service._auth_client.validate_token_for_service = AsyncMock(return_value={'valid': True, 'service_id': 'backend-service', 'permissions': ['read', 'write', 'admin']})
        result = await service.validate_service_token('service-token-12345', 'backend-service')
        assert result.success is True
        assert result.user_id == 'backend-service'
        ...
        service._auth_client.validate_token_for_service.assert_called_once_with('service-token-12345', 'backend-service')

    async def test_validate_service_token_failed_validation(self, test_env):
        """Test service token validation failure."""
        service = UnifiedAuthenticationService()
        service._auth_client.validate_token_for_service = AsyncMock(return_value={'valid': False, 'error': 'Service not authorized'})
        result = await service.validate_service_token('invalid-service-token', 'unauthorized-service')
        assert result.success is False
    ...
    async def test_validate_service_token_no_result(self, test_env):
        ...
        result = await service.validate_service_token('missing-token', 'test-service')
        assert result.success is False
    ...
    async def test_validate_service_token_exception(self, test_env):
        ...
        service = UnifiedAuthenticationService()
        service._auth_client.validate_token_for_service = AsyncMock(side_effect=Exception('Service validation error'))
        result = await service.validate_service_token('error-token', 'error-service')
        assert result.success is False
...
class TestAuthenticationStats(BaseTestCase):
    ...
    async def test_get_authentication_stats_after_operations(self, test_env):
        ...
        service = UnifiedAuthenticationService()
        service._auth_client.validate_token = AsyncMock(return_value={'valid': True, 'user_id': 'user1', 'email': 'user1@test.com'})
        with patch('netra_backend.app.services.unified_authentication_service.validate_jwt_format', return_value=True):
            await service.authenticate_token('valid-jwt-token', context=AuthenticationContext.REST_API, method=AuthenticationMethod.JWT_TOKEN)
            await service.authenticate_token('valid-api-key', context=AuthenticationContext.WEBSOCKET, method=AuthenticationMethod.API_KEY)
        with patch('netra_backend.app.services.unified_authentication_service.validate_jwt_format', return_value=False):
            await service.authenticate_token('invalid-token', context=AuthenticationContext.GRAPHQL, method=AuthenticationMethod.JWT_TOKEN)
        stats = service.get_authentication_stats()
...
class TestEnhancedResilienceAndRetryLogic(AsyncTestCase):
    ...
    async def test_validate_token_with_enhanced_resilience_success_first_try(self, test_env):
        ...
        service = UnifiedAuthenticationService()
        service._auth_client.validate_token = AsyncMock(return_value={'valid': True, 'user_id': 'resilient_user', 'email': 'resilient@test.com'})
        service._check_circuit_breaker_status = AsyncMock(return_value={'open': False, 'state': 'closed', 'failure_count': 0, 'reason': 'Circuit breaker is closed'})
        result = await service._validate_token_with_enhanced_resilience('test-token', AuthenticationContext.REST_API, AuthenticationMethod.JWT_TOKEN)
    ...
    async def test_validate_token_with_enhanced_resilience_retry_success(self, test_env):
        ...
        async def mock_validate_token(token):
            ...
            return {'valid': True, 'user_id': 'retry_user', 'email': 'retry@test.com'}
        service._auth_client.validate_token = mock_validate_token
        service._classify_auth_error = Mock(return_value={'category': 'network', 'retryable': True, 'reason': 'Network connectivity issue - likely transient'})
        result = await service._validate_token_with_enhanced_resilience('retry-token', AuthenticationContext.WEBSOCKET, AuthenticationMethod.JWT_TOKEN, max_retries=2)
        assert result is not None
        assert result['valid'] is True
    ...
    async def test_validate_token_with_enhanced_resilience_all_attempts_fail(self, test_env):
        ...
        service = UnifiedAuthenticationService()
        service._check_circuit_breaker_status = AsyncMock(return_value={'open': False, 'state': 'closed', 'failure_count': 0, 'reason': 'Circuit breaker is closed'})
        service._auth_client.validate_token = AsyncMock(side_effect=AuthServiceConnectionError('Persistent connection error'))
        service._classify_auth_error = Mock(return_value={'category': 'network', 'retryable': True, 'reason': 'Network connectivity issue - likely transient'})
        result = await service._validate_token_with_enhanced_resilience('fail-token', AuthenticationContext.GRPC, AuthenticationMethod.API_KEY, max_retries=2)
        assert result is not None
        assert result['valid'] is False
        assert result['error'] == 'All validation attempts failed'
        assert 'details' in result
        assert result['resilience_metadata']['attempts_made'] == 3
    ...
    async def test_validate_token_with_enhanced_resilience_non_retryable_error(self, test_env):
        ...
        service = UnifiedAuthenticationService()
        service._check_circuit_breaker_status = AsyncMock(return_value={'open': False, 'state': 'closed', 'failure_count': 0, 'reason': 'Circuit breaker is closed'})
        service._auth_client.validate_token = AsyncMock(side_effect=AuthServiceValidationError('Invalid token format'))
        service._classify_auth_error = Mock(return_value={'category': 'invalid_token', 'retryable': False, 'reason': "Token is invalid - retry won't help"})
        result = await service._validate_token_with_enhanced_resilience('invalid-token', AuthenticationContext.INTERNAL_SERVICE, AuthenticationMethod.SERVICE_ACCOUNT, max_retries=3)
        assert result is not None
        assert result['valid'] is False
        assert result['resilience_metadata']['attempts_made'] == 1
        assert result['resilience_metadata']['final_failure'] is True
    ...
    async def test_validate_token_with_enhanced_resilience_circuit_breaker_open(self, test_env):
        ...
        service = UnifiedAuthenticationService()
        service._check_circuit_breaker_status = AsyncMock(return_value={'open': True, 'state': 'open', 'failure_count': 5, 'reason': 'Too many failures - circuit breaker open'})
        service._auth_client.validate_token = AsyncMock(return_value={'valid': True, 'user_id': 'breaker_user', 'email': 'breaker@test.com'})
        result = await service._validate_token_with_enhanced_resilience('breaker-token', AuthenticationContext.REST_API, AuthenticationMethod.JWT_TOKEN)
        assert result is not None
        assert result['valid'] is True
        assert 'resilience_metadata' in result
    ...
    async def test_check_circuit_breaker_status_available(self, test_env):
        ...
        service = UnifiedAuthenticationService()
        mock_breaker = Mock()
        mock_breaker.get_status.return_value = {'state': 'closed', 'failure_count': 2}
...
class TestUserExecutionContextCreation(BaseTestCase):
    ...
    def test_create_user_execution_context_success(self, test_env):
        ...
        service = UnifiedAuthenticationService()
        auth_result = AuthResult(success=True, user_id='context_user_12345', email='context@test.com', permissions=['read', 'write'])
        websocket = Mock()
        ...
        assert len(context.thread_id.split('_')) == 3
        assert len(context.run_id.split('_')) == 3
        assert len(context.request_id.split('_')) >= 3
        assert len(context.websocket_client_id.split('_')) >= 3
