...
class MockWebSocketManager:
    ...
    async def send_to_thread(self, thread_id: str, message: Dict[str, Any]) -> bool:
        ...
        self.sent_to_threads[thread_id].append({**message, '_test_thread_id': thread_id, '_test_timestamp': time.time()})
        return True
    ...
    async def broadcast(self, message: Dict[str, Any]) -> None:
        ...
        self.broadcast_messages.append({**message, '_test_broadcast_timestamp': time.time()})
    ...
    async def notify_agent_started(self, context: Dict[str, Any]) -> None:
        ...
        self.agent_started_calls.append({**context, '_test_notification_type': 'agent_started', '_test_timestamp': time.time()})

    async def notify_agent_thinking(self, context: Dict[str, Any]) -> None:
        """Mock notify_agent_thinking for Golden Path compatibility."""
        self.agent_thinking_calls.append({**context, '_test_notification_type': 'agent_thinking', '_test_timestamp': time.time()})

    async def notify_tool_executing(self, context: Dict[str, Any]) -> None:
        """Mock notify_tool_executing for Golden Path compatibility."""
        self.tool_executing_calls.append({**context, '_test_notification_type': 'tool_executing', '_test_timestamp': time.time()})

    async def notify_tool_completed(self, context: Dict[str, Any]) -> None:
        """Mock notify_tool_completed for Golden Path compatibility."""
        self.tool_completed_calls.append({**context, '_test_notification_type': 'tool_completed', '_test_timestamp': time.time()})

    async def notify_agent_completed(self, context: Dict[str, Any]) -> None:
        """Mock notify_agent_completed for Golden Path compatibility."""
        self.agent_completed_calls.append({**context, '_test_notification_type': 'agent_completed', '_test_timestamp': time.time()})
...
class TestWebSocketNotifierUnit(SSotAsyncTestCase, unittest.TestCase):
    ...
    def setup_method(self, method):
        ...
        self.mock_exec_context = UserExecutionContext(user_id='test_user_websocket_notifier', thread_id='test_thread_websocket_notifier', run_id='test_run_websocket_notifier', websocket_client_id='test_ws_client_websocket_notifier')
    ...
    async def test_send_agent_thinking_enhanced(self):
        ...
        await self.notifier.send_agent_thinking(context, thought='Analyzing your request for optimization opportunities', step_number=2, progress_percentage=40.0, estimated_remaining_ms=5000, current_operation='data_analysis')
        ...
        self.assertEqual(payload['urgency'], 'medium_priority')
    ...
    async def test_send_tool_executing_critical(self):
        ...
        await self.notifier.send_tool_executing(context, tool_name='cost_optimizer', tool_purpose='Analyze spending patterns and identify savings opportunities', estimated_duration_ms=3000, parameters_summary='analyzing 6 months of data')
    ...
    async def test_send_tool_completed_critical(self):
        ...
        result = {'savings_identified': 1500.0, 'recommendations': ['Switch to annual billing', 'Optimize instance sizes'], 'confidence': 0.95}
    ...
    async def test_send_agent_completed_critical(self):
        ...
        result = {'summary': 'Identified $1,500 in monthly savings', 'actions_taken': 3, 'recommendations': ['Action 1', 'Action 2']}
    ...
    async def test_enhanced_thinking_payload_urgency_classification(self):
        ...
        await self.notifier.send_agent_thinking(context, 'Quick analysis', estimated_remaining_ms=2000)
        ...
        self.mock_manager.clear_messages()
        await self.notifier.send_agent_thinking(context, 'Medium analysis', estimated_remaining_ms=7000)
        messages = self.mock_manager.get_messages_for_thread('thread_urgency')
        self.assertEqual(messages[0]['payload']['urgency'], 'medium_priority')
        self.mock_manager.clear_messages()
        await self.notifier.send_agent_thinking(context, 'Deep analysis', estimated_remaining_ms=15000)
        messages = self.mock_manager.get_messages_for_thread('thread_urgency')
    ...
    async def test_tool_context_hints(self):
        ...
        test_cases = [('search_optimizer', 'information_retrieval', 'medium'), ('analyze_patterns', 'data_processing', 'long'), ('query_database', 'database_operation', 'short'), ('generate_report', 'content_creation', 'medium'), ('validate_input', 'verification', 'short'), ('optimize_performance', 'performance_tuning', 'long'), ('unknown_tool', 'general', 'medium')]
    ...
    async def test_enhanced_error_payload_generation(self):
        ...
        await self.notifier.send_agent_error(context, error_message='Rate limit exceeded', error_type='rate_limit', error_details={'limit': 100, 'current': 105}, recovery_suggestions=['Wait 60 seconds', 'Upgrade plan'], is_recoverable=True, estimated_retry_delay_ms=60000)
    ...
    async def test_error_severity_classification(self):
        ...
        test_cases = [('authentication', 'Auth failed', 'critical'), ('database', 'DB connection lost', 'critical'), ('timeout', 'Request timed out', 'high'), ('validation', 'Invalid input', 'high'), ('general', 'Unknown error', 'medium'), (None, 'No type', 'medium')]
    ...
    async def test_agent_registered_compatibility(self):
        ...
        self.mock_manager.clear_messages()
        await self.notifier.send_agent_registered(agent_id='param_agent', agent_type='optimizer', thread_id='thread_param')
    ...
    async def test_agent_failed_compatibility(self):
        ...
        await self.notifier.send_agent_failed(context, error_message='Context error', error_details={'code': 500})
        ...
        self.mock_manager.clear_messages()
        await self.notifier.send_agent_failed(agent_id='param_fail_agent', error='Parameter error', thread_id='thread_param_fail')
    ...
    async def test_agent_status_changed_compatibility(self):
        ...
        await self.notifier.send_agent_status_changed(context, old_status=AgentStatus.IDLE, new_status=AgentStatus.RUNNING)
    ...
    async def test_partial_result_streaming(self):
        ...
        await self.notifier.send_partial_result(context, content='Partial analysis results: Found 3 optimization opportunities...', is_complete=False)
    ...
    async def test_stream_chunk_delivery(self):
        ...
        await self.notifier.send_stream_chunk(context, chunk_id='chunk_001', content='This is chunk 1 of the analysis...', is_final=False)
    ...
    async def test_stream_completion(self):
        ...
        await self.notifier.send_stream_complete(context, stream_id='stream_analysis', total_chunks=5, metadata={'duration_ms': 2500})
    ...
    async def test_periodic_update_for_long_operations(self):
        ...
        await self.notifier.send_periodic_update(context, operation_name='deep_analysis', progress_percentage=65.0, status_message='Analyzing complex patterns...', estimated_remaining_ms=8000, current_step='pattern_matching')
    ...
    async def test_operation_started_notification(self):
        ...
        await self.notifier.send_operation_started(context, operation_name='comprehensive_audit', operation_type='analysis', expected_duration_ms=12000, operation_description='Performing comprehensive cost audit across all services')
    ...
    async def test_operation_completed_notification(self):
        ...
        metrics = {'items_processed': 1500, 'patterns_found': 12, 'accuracy': 0.94}
        await self.notifier.send_operation_completed(context, operation_name='pattern_analysis', duration_ms=9500.0, result_summary='Successfully identified 12 cost optimization patterns', metrics=metrics)
    ...
    async def test_subagent_lifecycle_notifications(self):
        ...
        await self.notifier.send_subagent_started(context, subagent_name='DataAnalyzer', subagent_id='subagent_001')
        ...
        await self.notifier.send_subagent_completed(context, subagent_name='DataAnalyzer', subagent_id='subagent_001', result=subagent_result, duration_ms=3500.0)
        messages = self.mock_manager.get_messages_for_thread('thread_parent')
        self.assertEqual(len(messages), 1)
    ...
    async def test_websocket_manager_none_handling(self):
        ...
        mock_context = UserExecutionContext(user_id='test_user_none', thread_id='test_thread_none', run_id='test_run_none')
        with warnings.catch_warnings():
            warnings.simplefilter('ignore', DeprecationWarning)
    ...
    async def test_websocket_send_failure_handling(self):
        ...
        mock_context = UserExecutionContext(user_id='test_user_failing', thread_id='test_thread_failing', run_id='test_run_failing')
        with warnings.catch_warnings():
            warnings.simplefilter('ignore', DeprecationWarning)
    ...
    def test_user_friendly_error_messages(self):
        ...
        test_cases = [('timeout', 'The TestAgent is taking longer than usual'), ('rate_limit', "You've made many requests recently"), ('validation', "There's an issue with your request format"), ('network', "There's a temporary connectivity issue"), ('database', "We're experiencing a temporary data access issue"), ('unknown', 'The TestAgent encountered an unexpected issue')]
        for error_type, expected_fragment in test_cases:
            message = self.notifier._generate_user_friendly_error_message(error_type, 'Test error', 'TestAgent')
