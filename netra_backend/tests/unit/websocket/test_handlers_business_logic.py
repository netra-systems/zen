...
from netra_backend.app.websocket_core.handlers import BaseMessageHandler, ConnectionHandler, TypingHandler, HeartbeatHandler, AgentRequestHandler, E2EAgentHandler, AgentHandler, UserMessageHandler, JsonRpcHandler, ErrorHandler, BatchMessageHandler, MessageRouter, get_message_router, get_router_handler_count, list_registered_handlers, send_error_to_websocket, send_system_message
...
class TestTypingHandler(SSotBaseTestCase):
    ...
    async def test_typing_message_creates_acknowledgment(self, mock_connected):
        ...
        message = create_standard_message(MessageType.USER_TYPING, {'thread_id': thread_id})
...
class TestAgentRequestHandler(SSotBaseTestCase):
    ...
    async def test_agent_request_generates_agent_response(self):
        ...
        message = create_standard_message(MessageType.AGENT_REQUEST, {'message': user_message, 'turn_id': turn_id, 'require_multi_agent': False, 'real_llm': False})
        result = await self.handler.handle_message(user_id, websocket, message)
    ...
    async def test_multi_agent_request_involves_multiple_agents(self):
        ...
        websocket = create_mock_websocket()
        message = create_standard_message(MessageType.AGENT_REQUEST, {'message': 'Complex optimization task', 'turn_id': 'multi-turn-789', 'require_multi_agent': True, 'real_llm': True})
        result = await self.handler.handle_message(user_id, websocket, message)
...
class TestAgentHandler(SSotBaseTestCase):
    ...
    async def test_agent_message_acknowledgment(self, mock_connected):
        ...
        websocket = create_mock_websocket()
        message = create_standard_message(MessageType.AGENT_RESPONSE_CHUNK, {'task_id': 'task-123', 'chunk': 'Processing optimization...', 'chunk_index': 0})
        result = await self.handler.handle_message(user_id, websocket, message)
...
class TestUserMessageHandler(SSotBaseTestCase):
    ...
    async def test_message_processing_updates_stats(self):
        ...
        websocket = create_mock_websocket()
        message = create_standard_message(MessageType.SYSTEM_MESSAGE, {'content': 'System notification'})
...
class TestErrorHandler(SSotBaseTestCase):
    ...
    async def test_error_message_tracking_and_acknowledgment(self, mock_connected):
        ...
        message = create_standard_message(MessageType.ERROR_MESSAGE, {'error_code': error_code, 'error_message': error_message})
...
class TestBatchMessageHandler(SSotBaseTestCase):
    ...
    async def test_message_batching_creates_pending_message(self):
        ...
        websocket = create_mock_websocket()
        message = create_standard_message(MessageType.BROADCAST, {'content': 'Broadcast message', 'priority': 1})
...
class TestMessageRouter(SSotBaseTestCase):
    ...
    async def test_route_message_finds_appropriate_handler(self):
        ...
        websocket = create_mock_websocket()
        raw_message = {'type': 'ping', 'payload': {'test': 'data'}, 'timestamp': time.time()}
    ...
    async def test_unknown_message_type_acknowledgment(self):
        ...
        websocket = create_mock_websocket()
        raw_message = {'type': 'unknown_custom_message', 'payload': {'data': 'test'}}
        with patch('netra_backend.app.websocket_core.handlers.is_websocket_connected', return_value=True):
            result = await self.router.route_message(user_id, websocket, raw_message)
