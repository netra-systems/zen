...
class MockPydanticModel:
    ...
    def model_dump(self, mode: str=None) -> Dict[str, Any]:
        ...
        if mode == 'json':
            return {'field1': self.field1, 'timestamp': self.timestamp.isoformat(), 'number': self.number}
        return {'field1': self.field1, 'timestamp': str(self.timestamp), 'number': self.number}
...
class TestMessageSerialization(SSotAsyncTestCase, unittest.TestCase):
    ...
    def test_serialize_already_serializable_dict(self):
        ...
        data = {'string': 'hello', 'number': 42, 'float': 3.14, 'boolean': True, 'null': None, 'list': [1, 2, 3], 'nested': {'key': 'value'}}
    ...
    def test_serialize_basic_types(self):
        ...
        test_cases = [('hello world', 'hello world'), (42, 42), (3.14159, 3.14159), (True, True), (False, False), (None, None), ([], []), ({}, {})]
    ...
    def test_serialize_generic_enum_types(self):
        ...
        test_cases = [(GenericTestEnum.OPTION_A, 'option_a'), (GenericTestEnum.OPTION_B, 'option_b'), (GenericTestEnum.OPTION_C, 42), (IntegerTestEnum.FIRST, 1), (IntegerTestEnum.SECOND, 2), (IntegerTestEnum.THIRD, 3)]
    ...
    def test_serialize_to_dict_object(self):
        ...
        data = {'agent_state': 'running', 'progress': 0.75, 'metadata': {'step': 3}}
    ...
    def test_serialize_simple_dataclass(self):
        ...
        dc = SerializationTestDataclass(name='test_dataclass', value=42, timestamp=timestamp)
    ...
    def test_serialize_nested_dataclass(self):
        ...
        timestamp = datetime.now()
        nested_data = {'config': {'debug': True}, 'stats': [1, 2, 3], 'enum_val': GenericTestEnum.OPTION_A}
        dc = SerializationTestDataclass(name='nested_test', value=99, timestamp=timestamp, nested=nested_data)
        result = _serialize_message_safely(dc)
    ...
    def test_serialize_datetime_objects(self):
        ...
        test_datetimes = [datetime.now(), datetime(2024, 1, 1, 12, 0, 0), datetime.now(timezone.utc), datetime(2023, 12, 25, 9, 30, 15, tzinfo=timezone.utc)]
    ...
    def test_serialize_lists_with_mixed_types(self):
        ...
        data = ['string', 42, 3.14, True, GenericTestEnum.OPTION_A, datetime.now(), {'nested': 'dict'}, [1, 2, 3]]
        result = _serialize_message_safely(data)
        self.assertEqual(len(result), len(data))
        self.assertEqual(result[0], 'string')
        self.assertEqual(result[1], 42)
        self.assertEqual(result[4], 'option_a')
    ...
    def test_serialize_sets_to_lists(self):
        ...
        data = {'item1', 'item2', 'item3', GenericTestEnum.OPTION_B}
        result = _serialize_message_safely(data)
    ...
    def test_serialize_dict_with_enum_keys(self):
        ...
        data = {WebSocketStateForTesting.OPEN: 'connection_active', WebSocketStateForTesting.CLOSED: 'connection_inactive', GenericTestEnum.OPTION_A: 'selected_option', 'normal_key': 'normal_value'}
        result = _serialize_message_safely(data)
    ...
    def test_serialize_complex_dict_with_mixed_enum_keys(self):
        ...
        data = {WebSocketStateForTesting.CONNECTING: {'status': GenericTestEnum.OPTION_A, 'timestamp': datetime.now(), 'nested': {IntegerTestEnum.FIRST: [1, 2, GenericTestEnum.OPTION_B]}}, 'metadata': {'enum_value': WebSocketStateForTesting.OPEN, 'count': 42}}
        result = _serialize_message_safely(data)
        ...
        self.assertEqual(nested_list[2], 'option_b')
    ...
    def test_serialize_deeply_nested_structure(self):
        ...
        timestamp = datetime.now()
        data = {'user_context': {'user_id': 'user_123', 'session_state': WebSocketStateForTesting.OPEN, 'permissions': ['read', 'write'], 'metadata': {'created_at': timestamp, 'preferences': {'theme': GenericTestEnum.OPTION_A, 'notifications': True, 'settings': {'level': IntegerTestEnum.SECOND, 'tags': {'urgent', 'priority', 'review'}}}}}, 'agent_states': [{'agent_id': 'agent_001', 'status': WebSocketStateForTesting.CONNECTING, 'progress': 0.75, 'last_update': timestamp}, {'agent_id': 'agent_002', 'status': WebSocketStateForTesting.CLOSED, 'error': None}]}
        result = _serialize_message_safely(data)
    ...
    def test_serialize_dict_with_problematic_values(self, mock_logger):
        ...
        data = {'normal': 'value', 'enum': GenericTestEnum.OPTION_A, 'datetime': datetime.now(), 'problematic': problematic_obj, 'nested': {'also_problematic': problematic_obj}}
        result = _serialize_message_safely(data)
    ...
    def test_serialize_large_nested_structure(self):
        ...
        large_data = {f'section_{i}': {'items': [{'id': f'item_{i}_{j}', 'status': GenericTestEnum.OPTION_A if j % 2 == 0 else GenericTestEnum.OPTION_B, 'timestamp': datetime.now(), 'metadata': {'tags': {f'tag_{k}' for k in range(3)}, 'priority': IntegerTestEnum.FIRST}} for j in range(10)]} for i in range(5)}
    ...
    def test_serialize_circular_reference_prevention(self):
        ...
        data = {'key': 'value'}
        nested_data = {'level1': {'level2': {'level3': {'data': data}}}, 'reference': data}
    ...
    def test_serialize_message_with_special_websocket_fields(self):
        ...
        message = {'type': 'agent_update', 'user_id': 'user_12345', 'thread_id': 'thread_67890', 'timestamp': datetime.now(timezone.utc), 'connection_state': WebSocketStateForTesting.OPEN, 'payload': {'agent_name': 'CostOptimizer', 'status': GenericTestEnum.OPTION_A, 'progress': 0.65, 'current_step': 'analyzing_patterns', 'metadata': {'tools_used': ['search', 'analyze', 'optimize'], 'start_time': datetime.now(), 'estimat_remaining': 5000, 'tags': {'cost', 'optimization', 'analysis'}, 'state_info': {IntegerTestEnum.FIRST: 'initialized', IntegerTestEnum.SECOND: 'processing'}}}, 'delivery_confirmation': True}
