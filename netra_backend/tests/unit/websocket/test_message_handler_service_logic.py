...
class StartAgentHandlerTests(SSotBaseTestCase):
    ...
    async def test_setup_thread_and_run_handles_thread_creation_failure(self, mock_create_manager, mock_uow):
        ...
        mock_manager.send_error.assert_called_once_with(user_id, 'Failed to create or retrieve thread')
    ...
    async def test_execute_agent_workflow_creates_websocket_bridge(self):
        ...
        with patch('netra_backend.app.services.websocket.message_handler.create_agent_websocket_bridge') as mock_create_bridge:
            with patch('netra_backend.app.services.websocket.message_handler.get_user_execution_context') as mock_get_context:
                ...
                result = await self.handler._execute_agent_workflow(user_request, thread_id, user_id, run_id)
                assert self.handler.supervisor.websocket_bridge == mock_bridge
                mock_get_context.assert_called_once_with(user_id=user_id, thread_id=thread_id, run_id=run_id)
                mock_create_bridge.assert_called_once_with(mock_context)
                self.handler.supervisor.run.assert_called_once_with(user_request, thread_id, user_id, run_id)
    ...
    async def test_save_assistant_response_stores_agent_output(self, mock_uow):
        ...
        mock_uow_instance.runs.update_status.assert_called_once_with(mock_uow_instance.session, run_id, 'completed')
    ...
    async def test_handle_agent_error_sends_error_to_user(self, mock_create_manager):
        ...
        mock_manager.send_error.assert_called_once_with(user_id, 'Failed to start agent: Agent execution failed')
    ...
    async def test_handle_processes_complete_workflow(self, mock_uow):
        ...
        payload = {'request': {'query': 'Optimize my Kubernetes deployment'}}
...
class UserMessageHandlerTests(SSotBaseTestCase):
    ...
    def test_extract_message_data_from_payload(self):
        ...
        payload = {'text': 'What are the optimization results?', 'references': [{'type': 'document', 'id': 'doc-123'}, {'type': 'previous_chat', 'id': 'chat-456'}]}
    ...
    async def test_create_user_message_and_run_with_references(self, mock_uow):
        ...
        thread_id, run_id = await self.handler._create_user_message_and_run(mock_uow_instance, mock_thread, text, references)
...
class ThreadHistoryHandlerTests(SSotBaseTestCase):
    ...
    async def test_process_thread_history_request_retrieves_messages(self, mock_create_manager, mock_uow):
        ...
        mock_uow_instance.messages.get_thread_messages.assert_called_once_with(mock_uow_instance.session, 'history-thread-123', limit=10)
        mock_manager.send_to_user.assert_called_once()
        call_args = mock_manager.send_to_user.call_args[0]
        assert call_args[0] == user_id
...
class MessageHandlerServiceTests(SSotBaseTestCase):
    ...
    def test_register_handler_adds_to_handlers_and_queue(self):
        ...
        with patch('netra_backend.app.services.websocket.message_handler.message_queue') as mock_queue:
            ...
            mock_queue.register_handler.assert_called_once_with('custom_message', mock_handler.handle)
    ...
    async def test_extract_message_type_validates_known_types(self, mock_create_manager):
        ...
        mock_manager.send_error.assert_called_with(user_id, 'Unknown message type: unknown_type')
    ...
    async def test_get_stats_returns_comprehensive_statistics(self, mock_queue):
        ...
        mock_queue.get_queue_stats = AsyncMock(return_value={'pending_messages': 5, 'processed_messages': 100})
