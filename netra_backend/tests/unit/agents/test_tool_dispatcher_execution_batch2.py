...
class TestToolExecutionEngineUnit(SSotBaseTestCase):
    ...
    async def test_execute_tool_with_input_delegates_to_core(self):
        ...
        with patch('netra_backend.app.agents.tool_dispatcher_execution.UnifiedToolExecutionEngine') as mock_unified:
            ...
            mock_core_engine.execute_tool_with_input.assert_called_once_with(tool_input, mock_tool, kwargs)
    ...
    async def test_execute_with_state_delegates_and_transforms_response(self):
        ...
        with patch('netra_backend.app.agents.tool_dispatcher_execution.UnifiedToolExecutionEngine') as mock_unified:
            mock_core_engine = Mock()
            mock_core_result = {'success': True, 'result': {'output': 'success'}, 'metadata': {'execution_time': 100}}
            ...
            run_id = 'test_run_123'
            result = await engine.execute_with_state(mock_tool, 'test_tool', parameters, self.mock_state, run_id)
            mock_core_engine.execute_with_state.assert_called_once_with(mock_tool, 'test_tool', parameters, self.mock_state, run_id)
    ...
    async def test_execute_with_state_handles_error_response(self):
        ...
        with patch('netra_backend.app.agents.tool_dispatcher_execution.UnifiedToolExecutionEngine') as mock_unified:
            mock_core_engine = Mock()
            mock_core_result = {'success': False, 'error': 'Tool execution failed', 'metadata': {'error_code': 500}}
            mock_core_engine.execute_with_state = AsyncMock(return_value=mock_core_result)
            mock_unified.return_value = mock_core_engine
            engine = ToolExecutionEngine()
            result = await engine.execute_with_state(Mock(), 'failing_tool', {}, self.mock_state, 'test_run')
            from netra_backend.app.agents.tool_dispatcher_core import ToolDispatchResponse
            assert isinstance(result, ToolDispatchResponse)
    ...
    async def test_execute_tool_delegates_to_core(self):
        ...
        with patch('netra_backend.app.agents.tool_dispatcher_execution.UnifiedToolExecutionEngine') as mock_unified:
            ...
            mock_core_engine.execute_tool.assert_called_once_with('test_tool', {'param': 'value'})
    ...
    async def test_execute_tool_with_input_propagates_exceptions(self):
        ...
        with patch('netra_backend.app.agents.tool_dispatcher_execution.UnifiedToolExecutionEngine') as mock_unified:
            mock_core_engine = Mock()
            mock_core_engine.execute_tool_with_input = AsyncMock(side_effect=ValueError('Core engine error'))
            mock_unified.return_value = mock_core_engine
            engine = ToolExecutionEngine()
    ...
    async def test_execute_with_state_handles_malformed_core_response(self):
        ...
        with patch('netra_backend.app.agents.tool_dispatcher_execution.UnifiedToolExecutionEngine') as mock_unified:
            ...
            mock_unified.return_value = mock_core_engine
            engine = ToolExecutionEngine()
            result = await engine.execute_with_state(Mock(), 'test_tool', {}, self.mock_state, 'test_run')
            from netra_backend.app.agents.tool_dispatcher_core import ToolDispatchResponse
            assert isinstance(result, ToolDispatchResponse)
            assert result.success is False
...
class TestToolExecutionEngineCompatibilityUnit(SSotBaseTestCase):
    ...
    async def test_production_tool_compatibility(self):
        ...
        with patch('netra_backend.app.agents.tool_dispatcher_execution.UnifiedToolExecutionEngine') as mock_unified:
            ...
            result = await engine.execute_tool_with_input(tool_input, self.mock_production_tool, {})
            assert result == mock_result
            mock_core_engine.execute_tool_with_input.assert_called_once_with(tool_input, self.mock_production_tool, {})
    ...
    async def test_state_based_execution_compatibility(self):
        ...
        with patch('netra_backend.app.agents.tool_dispatcher_execution.UnifiedToolExecutionEngine') as mock_unified:
            ...
            mock_unified.return_value = mock_core_engine
            engine = ToolExecutionEngine()
            result = await engine.execute_with_state(Mock(), 'stateful_tool', {'param': 'value'}, mock_state, 'run_123')
            from netra_backend.app.agents.tool_dispatcher_core import ToolDispatchResponse
            assert isinstance(result, ToolDispatchResponse)
            assert result.success is True
...
class TestToolExecutionEngineArchitectureUnit(SSotBaseTestCase):
    ...
    def test_follows_delegation_pattern(self):
        ...
        with patch('netra_backend.app.agents.tool_dispatcher_execution.UnifiedToolExecutionEngine') as mock_unified:
            ...
            assert engine._core_engine == mock_core_engine
            assert not hasattr(engine, 'execute_tool_logic')
            assert not hasattr(engine, 'websocket_events')
    ...
    def test_minimal_wrapper_implementation(self):
        ...
        delegation_patterns = ['self._core_engine', 'return await self._core_engine', 'UnifiedToolExecutionEngine']
        for pattern in delegation_patterns:
            assert pattern in source_text, f"Delegation pattern '{pattern}' not found"
        complex_patterns = ['if.*else.*if', 'for.*in.*:', 'try:.*except:.*except:']
        import re
