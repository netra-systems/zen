...
class MockAgentWebSocketBridge:
    ...
    async def notify_tool_executing(self, run_id: str, agent_name: str, tool_name: str, parameters: Dict[str, Any]=None) -> bool:
        ...
        call_record = {'run_id': run_id, 'agent_name': agent_name, 'tool_name': tool_name, 'parameters': parameters, 'timestamp': datetime.now(timezone.utc).isoformat()}
    ...
    async def notify_tool_completed(self, run_id: str, agent_name: str, tool_name: str, result: Dict[str, Any]=None, execution_time_ms: float=None) -> bool:
        ...
        call_record = {'run_id': run_id, 'agent_name': agent_name, 'tool_name': tool_name, 'result': result, 'execution_time_ms': execution_time_ms, 'timestamp': datetime.now(timezone.utc).isoformat()}
        self.tool_completed_calls.append(call_record)
        return self.should_return_success

    async def notify_progress_update(self, run_id: str, agent_name: str, progress: Dict[str, Any]) -> bool:
        """Mock progress update notification."""
        call_record = {'run_id': run_id, 'agent_name': agent_name, 'progress': progress, 'timestamp': datetime.now(timezone.utc).isoformat()}
...
class TestUnifiedToolExecutionEngineWebSocketNotifications(SSotAsyncTestCase):
    ...
    def setUp(self):
        ...
        super().setUp()
        self.user_context = UserExecutionContext(user_id='test_user_websocket', run_id=f'run_{int(time.time() * 1000)}', thread_id='thread_websocket_test', session_id='session_websocket_test')
        self.agent_context = AgentExecutionContext(agent_name='TestAgent', run_id=self.user_context.run_id, thread_id=self.user_context.thread_id, user_id=self.user_context.user_id, retry_count=0, max_retries=3)
    ...
    async def test_tool_execution_emits_websocket_events(self):
        ...
        tool_input = ToolInput(tool_name='data_processor', parameters={'dataset': 'customer_data.csv', 'operation': 'analyze'}, request_id=self.user_context.run_id)
        result = await self.execution_engine.execute_tool_with_input(tool_input=tool_input, tool=self.data_tool, kwargs={'context': self.agent_context, 'dataset': 'customer_data.csv', 'operation': 'analyze'})
    ...
    async def test_websocket_events_on_tool_failure(self):
        ...
        tool_input = ToolInput(tool_name='data_processor', parameters={'dataset': 'corrupted_data.csv'}, request_id=self.user_context.run_id)
        result = await self.execution_engine.execute_tool_with_input(tool_input=tool_input, tool=self.failing_tool, kwargs={'context': self.agent_context, 'dataset': 'corrupted_data.csv'})
    ...
    async def test_websocket_bridge_failure_handling(self):
        ...
        tool_input = ToolInput(tool_name='data_processor', parameters={'dataset': 'test_data.csv'}, request_id=self.user_context.run_id)
        result = await self.execution_engine.execute_tool_with_input(tool_input=tool_input, tool=self.data_tool, kwargs={'context': self.agent_context, 'dataset': 'test_data.csv'})
        self.assertEqual(result.status, ToolStatus.SUCCESS)
        self.assertEqual(len(self.websocket_bridge.tool_executing_calls), 1)
        self.assertEqual(len(self.websocket_bridge.tool_completed_calls), 1)
    ...
    async def test_websocket_events_without_bridge(self):
        ...
        tool_input = ToolInput(tool_name='data_processor', parameters={'dataset': 'offline_data.csv'}, request_id=self.user_context.run_id)
        result = await engine_no_websocket.execute_tool_with_input(tool_input=tool_input, tool=self.data_tool, kwargs={'context': self.agent_context, 'dataset': 'offline_data.csv'})
        self.assertEqual(result.status, ToolStatus.SUCCESS)
        self.assertIsNotNone(result.payload.result)

    async def test_context_propagation_to_tools(self):
        """Test that execution context is properly propagated to tools."""
        tool_input = ToolInput(tool_name='data_processor', parameters={'dataset': 'context_test.csv'}, request_id=self.user_context.run_id)
        await self.execution_engine.execute_tool_with_input(tool_input=tool_input, tool=self.data_tool, kwargs={'context': self.agent_context, 'dataset': 'context_test.csv'})
    ...
    async def test_missing_context_error_handling(self):
        ...
        with patch.object(self.execution_engine, '_send_tool_executing', side_effect=ValueError('Missing execution context')):
            tool_input = ToolInput(tool_name='data_processor', parameters={'dataset': 'no_context.csv'}, request_id=self.user_context.run_id)
            result = await self.execution_engine.execute_tool_with_input(tool_input=tool_input, tool=self.data_tool, kwargs={'dataset': 'no_context.csv'})
            self.assertEqual(result.status, ToolStatus.ERROR)
    ...
    async def test_execution_metrics_tracking(self):
        ...
        tool_input = ToolInput(tool_name='data_processor', parameters={'dataset': 'metrics_test.csv'}, request_id=self.user_context.run_id)
        await self.execution_engine.execute_tool_with_input(tool_input=tool_input, tool=self.data_tool, kwargs={'context': self.agent_context, 'dataset': 'metrics_test.csv'})
    ...
    async def test_failed_execution_metrics(self):
        ...
        initial_metrics = self.execution_engine.get_execution_metrics()
        tool_input = ToolInput(tool_name='data_processor', parameters={'dataset': 'bad_data.csv'}, request_id=self.user_context.run_id)
        await self.execution_engine.execute_tool_with_input(tool_input=tool_input, tool=self.failing_tool, kwargs={'context': self.agent_context, 'dataset': 'bad_data.csv'})
        updated_metrics = self.execution_engine.get_execution_metrics()
    ...
    async def test_progress_updates_for_long_running_tools(self):
        ...
        await self.execution_engine.send_progress_update(context=self.agent_context, tool_name='long_runner', progress_percentage=25.0, status_message='Processing step 1 of 4')
        await self.execution_engine.send_progress_update(context=self.agent_context, tool_name='long_runner', progress_percentage=75.0, status_message='Processing step 3 of 4')
    ...
    async def test_tool_dispatcher_enhancement(self):
        ...
        enhanced_dispatcher = await enhance_tool_dispatcher_with_notifications(tool_dispatcher=mock_dispatcher, websocket_manager=mock_websocket_manager, enable_notifications=True)
    ...
    async def test_backward_compatibility_alias(self):
        ...
        engine_via_alias = UserExecutionEngine(websocket_bridge=self.websocket_bridge)
        self.assertIsInstance(engine_via_alias, UnifiedToolExecutionEngine)
        tool_input = ToolInput(tool_name='data_processor', parameters={'dataset': 'alias_test.csv'}, request_id=self.user_context.run_id)
        result = await engine_via_alias.execute_tool_with_input(tool_input=tool_input, tool=self.data_tool, kwargs={'context': self.agent_context, 'dataset': 'alias_test.csv'})
        self.assertEqual(result.status, ToolStatus.SUCCESS)
