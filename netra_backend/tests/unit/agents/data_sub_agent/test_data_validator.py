...
class AnalysisRequestValidationTests:
    ...
    def test_validate_valid_analysis_request(self, validator):
        ...
        request = {'type': 'performance', 'timeframe': '24h', 'metrics': ['latency_ms', 'cost_cents']}
...
class RawDataValidationTests:
    ...
    def sample_valid_data(self):
        ...
        return [{'timestamp': (base_time - timedelta(hours=i)).isoformat(), 'user_id': f'user_{i}', 'workload_id': f'workload_{i}', 'latency_ms': 100.0 + i * 10, 'cost_cents': 2.0 + i * 0.5, 'throughput': 50.0 + i * 5} for i in range(15)]
...
class QualityScoringTests:
    ...
    def test_calculate_quality_score_perfect_data(self, validator):
        ...
        validation_result = {'errors': [], 'warnings': [], 'data_points': 50}
