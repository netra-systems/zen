...
class TestExecutionEngineFactoryInitialization(AsyncTestCase):
    ...
    def test_factory_initialization_success(self):
        ...
        redis_manager = Mock()
        factory = ExecutionEngineFactory(websocket_bridge=websocket_bridge, database_session_manager=database_session_manager, redis_manager=redis_manager)
        ...
        expected_metrics = {'total_engines_created': 0, 'total_engines_cleaned': 0, 'active_engines_count': 0, 'creation_errors': 0, 'cleanup_errors': 0, 'timeout_cleanups': 0, 'user_limit_rejections': 0}
...
class TestUserEngineCreation(AsyncTestCase):
    ...
    def factory(self, websocket_bridge):
        ...
        return ExecutionEngineFactory(websocket_bridge=websocket_bridge, database_session_manager=Mock(), redis_manager=Mock())

    @pytest.fixture
    def user_context(self):
        """Create valid user execution context."""
        return UserExecutionContext.from_request_supervisor(user_id='test-user-123', thread_id='thread-456', run_id='run-789', metadata={'source': 'test'})
...
class TestUserEngineLimitsAndIsolation(AsyncTestCase):
    ...
    def user_context_1(self):
        ...
        return UserExecutionContext.from_request_supervisor(user_id='user-1', thread_id='thread-1', run_id='run-1', metadata={'source': 'test'})

    @pytest.fixture
    def user_context_2(self):
        """Create user context for user 2."""
        return UserExecutionContext.from_request_supervisor(user_id='user-2', thread_id='thread-2', run_id='run-2', metadata={'source': 'test'})
    ...
    async def test_user_isolation_different_users_separate_limits(self, factory):
        ...
        await factory._enforce_user_engine_limits(user2_id)
        assert factory._factory_metrics['user_limit_rejections'] == 1
...
class TestWebSocketEmitterCreation(AsyncTestCase):
    ...
    def user_context(self):
        ...
        return UserExecutionContext.from_request_supervisor(user_id='test-user', thread_id='test-thread', run_id='test-run', metadata={'source': 'test'})
    ...
    async def test_create_user_websocket_emitter_success(self, factory, user_context):
        ...
        with patch('netra_backend.app.agents.supervisor.execution_engine_factory.UserWebSocketEmitter') as mock_emitter_class:
            ...
            mock_emitter_class.assert_called_once_with(user_id=user_context.user_id, thread_id=user_context.thread_id, run_id=user_context.run_id, websocket_bridge=factory._websocket_bridge)
...
class TestUserExecutionScope(AsyncTestCase):
    ...
    def user_context(self):
        """Create user context."""
        return UserExecutionContext.from_request_supervisor(user_id='scope-user', thread_id='scope-thread', run_id='scope-run', metadata={'source': 'test'})
...
class TestFactorySingleton(AsyncTestCase):
    ...
    async def test_configure_execution_engine_factory_success(self):
        ...
        db_manager = Mock()
        redis_manager = Mock()
        factory = await configure_execution_engine_factory(websocket_bridge=websocket_bridge, database_session_manager=db_manager, redis_manager=redis_manager)
        assert factory._websocket_bridge is websocket_bridge
    ...
    async def test_user_execution_engine_context_manager(self):
        ...
        user_context = UserExecutionContext.from_request_supervisor(user_id='ctx-user', thread_id='ctx-thread', run_id='ctx-run', metadata={'source': 'test'})
        mock_engine = Mock()
...
class TestFactoryAliasMethodsCompatibility(AsyncTestCase):
    ...
    def user_context(self):
        """Create user context."""
        return UserExecutionContext.from_request_supervisor(user_id='compat-user', thread_id='compat-thread', run_id='compat-run', metadata={'source': 'test'})
...
class TestFactoryPerformanceBenchmarks(AsyncTestCase):
    ...
    async def test_user_engine_creation_performance(self, factory):
        ...
        user_context = UserExecutionContext.from_request_supervisor(user_id='perf-user', thread_id='perf-thread', run_id='perf-run', metadata={'source': 'test'})
        mock_engine = Mock()
