...
from netra_backend.app.agents.supervisor.agent_execution_core import AgentExecutionCore, get_agent_state_tracker
...
from netra_backend.app.core.agent_execution_tracker import AgentExecutionTracker, AgentExecutionPhase, CircuitBreakerOpenError
...
class AgentExecutionCoreBusinessLogicTests(SSotAsyncTestCase):
    ...
    async def asyncSetUp(self):
        ...
        self.execution_context = AgentExecutionContext(agent_name=self.test_agent_name, run_id=str(self.test_run_id), thread_id=self.test_thread_id, user_id=self.test_user_id, correlation_id='corr-789')
        self.user_context = UserExecutionContext.from_request(user_id=self.test_user_id, thread_id=self.test_thread_id, run_id=str(self.test_run_id))
    ...
    async def test_agent_not_found_error_provides_graceful_degradation(self):
        ...
        self.execution_core.agent_tracker.update_execution_state.assert_called_with('state-exec-123', ExecutionState.FAILED)
                    ...
                    with patch.object(self.execution_core.agent_tracker, 'create_execution', return_value='state-exec-123'):
                        with patch.object(self.execution_core.agent_tracker, 'start_execution', return_value=True):
                            with patch.object(self.execution_core.agent_tracker, 'transition_state'):
                                ...
                                result = await self.execution_core.execute_agent(context=self.execution_context, state=self.user_context, timeout=0.1)
...
class AgentExecutionCoreUserIsolationTests(SSotAsyncTestCase):
    ...
    async def test_user_execution_context_isolation_validation(self):
        ...
        """
        user1_context = UserExecutionContext.create_for_user(user_id='enterprise-user-1', thread_id='thread-1', run_id=str(uuid4()))
        user2_context = UserExecutionContext.create_for_user(user_id='enterprise-user-2', thread_id='thread-2', run_id=str(uuid4()))
        result1_context = self.execution_core._ensure_user_execution_context(state=user1_context, context=AgentExecutionContext(agent_name='test-agent', run_id=uuid4(), thread_id='thread-1'))
        result2_context = self.execution_core._ensure_user_execution_context(state=user2_context, context=AgentExecutionContext(agent_name='test-agent', run_id=uuid4(), thread_id='thread-2'))
    ...
    async def test_secure_user_execution_context_migration_from_deepagentstate(self):
        ...
        """
        legacy_data = {'user_id': 'legacy-user', 'thread_id': 'legacy-thread', 'run_id': 'legacy-run', 'user_request': 'Legacy user request'}
        context = AgentExecutionContext(agent_name='migration-test-agent', run_id=str(uuid4()), thread_id='legacy-thread', user_id='legacy-user')
        secure_context = UserExecutionContext.from_request(user_id=legacy_data['user_id'], thread_id=legacy_data['thread_id'], run_id=legacy_data['run_id'])
    ...
    async def test_concurrent_user_execution_isolation(self):
        ...
        user_contexts = [UserExecutionContext.create_for_user(f'user-{i}', f'thread-{i}', str(uuid4())) for i in range(5)]
        execution_contexts = [AgentExecutionContext(agent_name='concurrent-agent', run_id=uuid4(), thread_id=f'thread-{i}') for i in range(5)]
            ...
            with patch.object(self.execution_core.execution_tracker, 'start_execution'):
                with patch.object(self.execution_core.execution_tracker, 'update_execution_state'):
                    with patch.object(self.execution_core.agent_tracker, 'create_execution', side_effect=[f'exec-{i}' for i in range(5)]):
                        with patch.object(self.execution_core.agent_tracker, 'start_execution', return_value=True):
                            with patch.object(self.execution_core.agent_tracker, 'transition_state'):
                                tasks = [self.execution_core.execute_agent(context=execution_contexts[i], state=user_contexts[i], timeout=1.0) for i in range(5)]
...
class AgentExecutionCoreErrorHandlingTests(SSotAsyncTestCase):
    ...
    def setup_method(self, method=None):
        ...
        self.execution_context = AgentExecutionContext(agent_name='error-agent', run_id=str(uuid4()), thread_id='error-thread', user_id='error-test-user')
...
class AgentExecutionCorePerformanceTests(SSotAsyncTestCase):
    ...
    async def test_execution_timeout_configuration_business_impact(self):
        ...
        """
        self.assertEqual(self.execution_core.DEFAULT_TIMEOUT, 25.0, 'Default timeout should be 25s for balance of AI processing vs user experience')
        self.assertEqual(self.execution_core.HEARTBEAT_INTERVAL, 5.0, 'Heartbeat every 5s provides regular user progress updates')
    ...
    async def test_metrics_collection_enables_business_insights(self):
        ...
        with patch.object(self.execution_core, '_collect_metrics') as mock_collect:
            with patch.object(self.execution_core, '_persist_metrics') as mock_persist:
                mock_metrics = {'execution_time': 2.5, 'memory_usage': 150.5, 'cpu_usage': 45.2, 'websocket_events_sent': 5, 'business_value_score': 95}
                mock_collect.return_value = mock_metrics
                result_metrics = self.execution_core._calculate_performance_metrics(start_time=time.time() - 2.5, end_time=time.time(), success=True, context=AgentExecutionContext(agent_name='metrics-agent', run_id=uuid4(), thread_id='metrics-thread'))
...
class AgentExecutionCoreIntegrationPointsTests(SSotAsyncTestCase):
    ...
    async def test_websocket_event_delivery_sequence_business_critical(self):
        ...
        execution_context = AgentExecutionContext(agent_name='websocket-agent', run_id=uuid4(), thread_id='ws-thread')
        mock_exec_id = uuid4()
        with patch.object(self.execution_core.execution_tracker, 'register_execution', return_value=mock_exec_id):
            with patch.object(self.execution_core.execution_tracker, 'start_execution'):
                with patch.object(self.execution_core.execution_tracker, 'update_execution_state'):
                    with patch.object(self.execution_core.agent_tracker, 'create_execution', return_value='ws-exec-123'):
                        with patch.object(self.execution_core.agent_tracker, 'start_execution', return_value=True):
                            with patch.object(self.execution_core.agent_tracker, 'transition_state'):
                                await self.execution_core.execute_agent(context=execution_context, state=user_context)
        self.mock_websocket_bridge.notify_agent_started.assert_called_once_with(run_id=execution_context.run_id, agent_name='websocket-agent', context={'status': 'starting', 'phase': 'initialization'})
    ...
    async def test_execution_tracking_integration_business_monitoring(self):
        ...
        execution_context = AgentExecutionContext(agent_name='tracking-agent', run_id=uuid4(), thread_id='tracking-thread')
        mock_exec_id = uuid4()
            ...
            with patch.object(self.execution_core.execution_tracker, 'start_execution') as mock_start:
                with patch.object(self.execution_core.agent_tracker, 'create_execution', return_value=mock_state_exec_id) as mock_create:
                    with patch.object(self.execution_core.agent_tracker, 'start_execution', return_value=True) as mock_state_start:
                        ...
                        with patch.object(self.execution_core.agent_tracker, 'transition_state'):
                            await self.execution_core.execute_agent(context=execution_context, state=user_context, timeout=30.0)
        mock_register.assert_called_once_with(agent_name='tracking-agent', correlation_id=pytest.unordered.Any(), thread_id='tracking-thread', user_id='tracking-user', timeout_seconds=30.0)
        mock_create.assert_called_once_with(agent_name='tracking-agent', thread_id='tracking-thread', user_id='tracking-user', timeout_seconds=30.0, metadata=pytest.unordered.Any())
...
class AgentExecutionCoreCompatibilityTests(SSotAsyncTestCase):
    ...
    async def test_unsupported_state_type_validation(self):
        ...
        execution_context = AgentExecutionContext(agent_name='validation-agent', run_id=uuid4(), thread_id='validation-thread')
        execution_core = AgentExecutionCore(registry=MagicMock(), websocket_bridge=AsyncMock())
