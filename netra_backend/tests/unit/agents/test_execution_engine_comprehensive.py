...
try:
    from netra_backend.app.agents.supervisor.user_execution_engine import UserExecutionEngine as ExecutionEngine, create_execution_context_manager, detect_global_state_usage
...
class MockUserExecutionContext:
    ...
    def __init__(self, user_id: str=None, run_id: str=None, thread_id: str=None):
        ...
        self.metadata = {'user_prompt': 'Test user prompt', 'final_answer': 'Test final answer', 'step_count': 3, 'created_at': datetime.now(timezone.utc).isoformat()}
...
class MockAgentWebSocketBridge:
    ...
    async def notify_agent_started(self, run_id: str, agent_name: str, data: Dict):
        ...
        self.events.append({'type': 'agent_started', 'run_id': run_id, 'agent_name': agent_name, 'data': data, 'timestamp': datetime.now(timezone.utc).isoformat()})
    ...
    async def notify_agent_thinking(self, run_id: str, agent_name: str, reasoning: str, step_number: int=None, progress_percentage: float=None):
        ...
        if self.should_fail:
            self.metrics['errors'] += 1
            raise ConnectionError('WebSocket connection failed')
        self.events.append({'type': 'agent_thinking', 'run_id': run_id, 'agent_name': agent_name, 'reasoning': reasoning, 'step_number': step_number, 'progress_percentage': progress_percentage, 'timestamp': datetime.now(timezone.utc).isoformat()})
        self.metrics['messages_sent'] += 1
    ...
    async def notify_tool_executing(self, run_id: str, agent_name: str, tool_name: str, parameters: Dict):
        ...
        if self.should_fail:
            self.metrics['errors'] += 1
            raise ConnectionError('WebSocket connection failed')
        self.events.append({'type': 'tool_executing', 'run_id': run_id, 'agent_name': agent_name, 'tool_name': tool_name, 'parameters': parameters, 'timestamp': datetime.now(timezone.utc).isoformat()})
        self.metrics['messages_sent'] += 1
    ...
    async def notify_agent_completed(self, run_id: str, agent_name: str, result: Dict, execution_time_ms: float):
        ...
        if self.should_fail:
            self.metrics['errors'] += 1
            raise ConnectionError('WebSocket connection failed')
        self.events.append({'type': 'agent_completed', 'run_id': run_id, 'agent_name': agent_name, 'result': result, 'execution_time_ms': execution_time_ms, 'timestamp': datetime.now(timezone.utc).isoformat()})
        self.metrics['messages_sent'] += 1
    ...
    async def notify_agent_death(self, run_id: str, agent_name: str, death_type: str, context: Dict):
        ...
        self.death_notifications.append({'type': 'agent_death', 'run_id': run_id, 'agent_name': agent_name, 'death_type': death_type, 'context': context, 'timestamp': datetime.now(timezone.utc).isoformat()})
        self.metrics['messages_sent'] += 1
    ...
    async def notify_agent_error(self, run_id: str, agent_name: str, error: str, error_context: Dict):
        ...
        self.error_notifications.append({'type': 'agent_error', 'run_id': run_id, 'agent_name': agent_name, 'error': error, 'error_context': error_context, 'timestamp': datetime.now(timezone.utc).isoformat()})
        self.metrics['messages_sent'] += 1
...
class MockAgentRegistry:
    ...
    def __init__(self):
        self.agents = {'test_agent': MockAgent('test_agent'), 'data_agent': MockAgent('data_agent'), 'optimization_agent': MockAgent('optimization_agent'), 'failing_agent': MockFailingAgent('failing_agent'), 'slow_agent': MockSlowAgent('slow_agent')}
...
class MockAgent:
    ...
    async def run(self, context: AgentExecutionContext, user_context: Optional[MockUserExecutionContext]=None) -> Dict[str, Any]:
        ...
        return {'success': True, 'result': f'Result from {self.name}', 'execution_count': self.execution_count, 'user_id': user_context.user_id if user_context else 'unknown', 'context_metadata': context.metadata or {}}
...
class MockAgentExecutionCore:
    ...
    async def execute_agent(self, context: AgentExecutionContext, user_context: Optional[MockUserExecutionContext]=None) -> AgentExecutionResult:
        ...
        if not agent:
            return AgentExecutionResult(success=False, agent_name=context.agent_name, execution_time=0.0, error=f'Agent {context.agent_name} not found', state=None)
        try:
            start_time = time.time()
            result_data = await agent.run(context, user_context)
            execution_time = time.time() - start_time
            return AgentExecutionResult(success=True, agent_name=context.agent_name, execution_time=execution_time, data=result_data, state=None, metadata={'execution_count': self.execution_count})
...
class TestExecutionEngineConstruction(SSotAsyncTestCase):
    ...
    async def test_direct_construction_succeeds(self):
        """Test that direct ExecutionEngine construction succeeds with proper arguments"""
        from netra_backend.app.services.user_execution_context import UserExecutionContext
        import uuid
        real_user_context = UserExecutionContext(user_id=f'user_{uuid.uuid4().hex[:12]}', run_id=f'run_{uuid.uuid4().hex[:12]}', thread_id=f'thread_{uuid.uuid4().hex[:12]}', websocket_client_id=f'ws_{uuid.uuid4().hex[:12]}')
        mock_agent_factory = MockAgentRegistry()
        mock_websocket_emitter = MockAgentWebSocketBridge()
        engine = ExecutionEngine(real_user_context, mock_agent_factory, mock_websocket_emitter)
    ...
    async def test_factory_initialization_success(self):
        ...
        mock_agent_factory = MockAgentRegistry()
        mock_websocket_emitter = MockAgentWebSocketBridge()
        engine = ExecutionEngine(context=real_user_context, agent_factory=mock_agent_factory, websocket_emitter=mock_websocket_emitter)
        assert engine.context == real_user_context
        assert hasattr(engine, 'active_runs')
        assert hasattr(engine, 'run_history')
    ...
    async def test_factory_initialization_without_websocket_bridge_fails(self):
        ...
        with pytest.raises((TypeError, ValueError)) as exc_info:
            ExecutionEngine(context=None, agent_factory=mock_agent_factory, websocket_emitter=mock_websocket_emitter)
    ...
    async def test_factory_initialization_with_invalid_context_fails(self):
        ...
        with pytest.raises((TypeError, AttributeError, ValueError)) as exc_info:
            ExecutionEngine(context=invalid_context, agent_factory=mock_agent_factory, websocket_emitter=mock_websocket_emitter)
        error_message = str(exc_info.value)
...
class TestUserExecutionContextIntegration(SSotAsyncTestCase):
    ...
    async def test_user_context_storage_and_isolation(self):
        ...
        mock_agent_factory = MockAgentRegistry()
        mock_websocket_emitter = MockAgentWebSocketBridge()
        engine_1 = ExecutionEngine(context=user_context_1, agent_factory=mock_agent_factory, websocket_emitter=mock_websocket_emitter)
        engine_2 = ExecutionEngine(context=user_context_2, agent_factory=mock_agent_factory, websocket_emitter=mock_websocket_emitter)
    ...
    async def test_user_state_lock_creation_and_isolation(self):
        ...
        mock_agent_factory = MockAgentRegistry()
        mock_websocket_emitter = MockAgentWebSocketBridge()
        engine = ExecutionEngine(context=mock_user_context, agent_factory=mock_agent_factory, websocket_emitter=mock_websocket_emitter)
...
class TestContextValidation(SSotAsyncTestCase):
    ...
    async def test_validation_rejects_empty_user_id(self):
        ...
        invalid_context = AgentExecutionContext(agent_name='test_agent', run_id='run_123', thread_id='thread_456', user_id='')
    ...
    async def test_validation_rejects_registry_placeholder_run_id(self):
        ...
        mock_bridge = MockAgentWebSocketBridge()
        engine = ExecutionEngine._init_from_factory(mock_registry, mock_bridge)
        invalid_context = AgentExecutionContext(agent_name='test_agent', run_id='registry', thread_id='thread_456', user_id='user_789')
        with pytest.raises(ValueError) as exc_info:
            engine._validate_execution_context(invalid_context)
...
class TestWebSocketEventEmission(SSotAsyncTestCase):
    ...
    async def test_websocket_bridge_direct_notification_calls(self):
        ...
        mock_user_context = MockUserExecutionContext()
        engine = ExecutionEngine._init_from_factory(mock_registry, mock_websocket_bridge, mock_user_context)
        await engine.websocket_bridge.notify_agent_started('test_run', 'test_agent', {'status': 'started'})
        await engine.websocket_bridge.notify_agent_thinking('test_run', 'test_agent', 'Processing request...', 1)
        await engine.websocket_bridge.notify_agent_completed('test_run', 'test_agent', {'result': 'success'}, 1000.0)
...
class TestConcurrencyAndIsolation(SSotAsyncTestCase):
    ...
    async def test_multi_user_isolation_state_separation(self):
        ...
        assert id(state_1['active_runs']) != id(state_2['active_runs'])
...
class TestFactoryPatternsAndMigration(SSotAsyncTestCase):
    ...
    async def test_isolation_status_reporting(self):
        ...
        mock_registry = MockAgentRegistry()
        mock_bridge = MockAgentWebSocketBridge()
        engine_without_context = ExecutionEngine._init_from_factory(mock_registry, mock_bridge)
        ...
        mock_user_context = MockUserExecutionContext()
        engine_with_context = ExecutionEngine._init_from_factory(mock_registry, mock_bridge, mock_user_context)
...
@pytest.mark.parametrize('user_id,run_id,should_fail', [('', 'run_123', True), (None, 'run_123', True), ('user_123', '', True), ('user_123', None, True), ('user_123', 'registry', True), ('user_123', 'run_123', False)])
async def test_context_validation_parameters(user_id, run_id, should_fail):
    ...
    mock_bridge = MockAgentWebSocketBridge()
    engine = ExecutionEngine._init_from_factory(mock_registry, mock_bridge)
    context = AgentExecutionContext(agent_name='test_agent', run_id=run_id, thread_id='thread_123', user_id=user_id)
    if should_fail:
...
@pytest.mark.parametrize('isolation_level', ['global_state', 'user_isolated'])
