...
class TestToolDispatcherFacadeUnit(SSotBaseTestCase):
    ...
    def test_create_request_scoped_tool_dispatcher_delegates_to_factory(self):
        ...
        with patch('netra_backend.app.agents.tool_dispatcher.UnifiedToolDispatcherFactory.create_for_request') as mock_factory:
            mock_dispatcher = Mock()
            mock_factory.return_value = mock_dispatcher
            result = create_request_scoped_tool_dispatcher(user_context=self.mock_user_context, websocket_manager=self.mock_websocket_manager, tools=self.mock_tools)
            mock_factory.assert_called_once_with(user_context=self.mock_user_context, websocket_manager=self.mock_websocket_manager, tools=self.mock_tools)
            assert result == mock_dispatcher
    ...
    def test_all_expected_exports_available(self):
        ...
        expected_exports = ['ToolDispatcher', 'create_tool_dispatcher', 'UnifiedToolDispatcher', 'UnifiedToolDispatcherFactory', 'create_request_scoped_tool_dispatcher', 'create_request_scoped_dispatcher', 'ToolDispatchRequest', 'ToolDispatchResponse', 'DispatchStrategy', 'ToolExecutionResult', 'UnifiedTool']
    ...
    def test_create_request_scoped_dispatcher_propagates_context_validation_errors(self):
        ...
        with patch('netra_backend.app.agents.tool_dispatcher.UnifiedToolDispatcherFactory.create_for_request') as mock_factory:
            ...
            with pytest.raises(ValueError, match='Invalid user context'):
                create_request_scoped_tool_dispatcher(user_context=None)
    ...
    def test_tool_dispatch_request_model_validation(self):
        ...
        """
        valid_request = ToolDispatchRequest(tool_name='test_tool', parameters={'param1': 'value1', 'param2': 42})
    ...
    def test_tool_dispatch_response_model_validation(self):
        ...
        """
        success_response = ToolDispatchResponse(success=True, result={'output': 'success'}, metadata={'execution_time': 100})
        assert success_response.success is True
        assert success_response.result == {'output': 'success'}
        assert success_response.error is None
        assert success_response.metadata == {'execution_time': 100}
        error_response = ToolDispatchResponse(success=False, error='Tool execution failed')
    ...
    def test_facade_only_imports_from_unified_implementation(self):
        ...
        forbidden_imports = ['from netra_backend.app.agents.tool_dispatcher_core', 'from netra_backend.app.agents.tool_dispatcher_execution', 'from netra_backend.app.agents.tool_dispatcher_legacy']
...
class TestToolDispatcherBackwardCompatibilityUnit(SSotBaseTestCase):
    ...
    def test_legacy_create_function_parameters_compatible(self):
        ...
        with patch('netra_backend.app.agents.tool_dispatcher.UnifiedToolDispatcherFactory.create_legacy_global') as mock_factory:
            ...
            with warnings.catch_warnings():
                ...
                result = create_tool_dispatcher(tools=[Mock(name='legacy_tool')], websocket_bridge=Mock(), permission_service=Mock())
            assert result == mock_dispatcher
    ...
    def test_new_factory_method_signature_correct(self):
        ...
        with patch('netra_backend.app.agents.tool_dispatcher.UnifiedToolDispatcherFactory.create_for_request') as mock_factory:
            mock_dispatcher = Mock()
            mock_factory.return_value = mock_dispatcher
            result = create_request_scoped_tool_dispatcher(user_context=self.mock_user_context, websocket_manager=Mock(), tools=[Mock(name='new_tool')])
            assert result == mock_dispatcher
