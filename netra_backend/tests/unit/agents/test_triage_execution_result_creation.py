...
class TestExecutionResultCreation:
    ...
    def sample_execution_context(self):
        return ExecutionContext(request_id='test_request_123', run_id='test_run_456', agent_name='TestTriageAgent', state={'user_request': 'Test triage request'}, correlation_id='test_correlation_789')

    def test_successful_result_creation(self, sample_execution_context):
        """Test creation of successful ExecutionResult."""
        result_data = {'category': 'data_analysis', 'confidence_score': 0.9, 'status': 'success'}
        result = ExecutionResult(status=ExecutionStatus.COMPLETED, request_id=sample_execution_context.request_id, data=result_data, error_message=None, execution_time_ms=100.0)
    ...
    def test_failure_result_creation(self, sample_execution_context):
        ...
        result = ExecutionResult(status=ExecutionStatus.FAILED, request_id=sample_execution_context.request_id, data=None, error_message=error_msg, execution_time_ms=50.0)
...
class TestResultDataStructures:
    ...
    def test_triage_result_structure(self):
        ...
        triage_data = {'category': 'optimization', 'confidence_score': 0.85, 'should_route': True, 'metadata': {'processing_time': 0.02, 'model_version': 'v1.0'}}
    ...
    def test_complex_result_data(self):
        ...
        complex_data = {'primary_category': 'data_analysis', 'secondary_categories': ['reporting', 'visualization'], 'confidence_breakdown': {'data_analysis': 0.9, 'reporting': 0.7, 'visualization': 0.6}, 'entities_extracted': [{'type': 'metric', 'value': 'CPU usage'}, {'type': 'threshold', 'value': '80%'}]}
...
class TestResultValidation:
    ...
    def test_result_consistency_validation(self):
        ...
        valid_result = {'category': 'optimization', 'confidence_score': 0.8, 'status': 'success'}
...
class TestErrorHandling:
    ...
    def sample_context(self):
        return ExecutionContext(request_id='error_test', run_id='error_run', agent_name='ErrorTestAgent', state={'error': 'test'}, correlation_id='error_correlation')

    def test_error_result_handling(self, sample_context):
        """Test handling of error results."""
        error_result = ExecutionResult(status=ExecutionStatus.FAILED, request_id=sample_context.request_id, data=None, error_message='Simulated triage error', execution_time_ms=10.0)
    ...
    def test_partial_failure_handling(self, sample_context):
        ...
        partial_data = {'category': 'unknown', 'confidence_score': 0.3, 'status': 'partial_success', 'warnings': ['Low confidence score', 'Ambiguous request']}
        result = ExecutionResult(status=ExecutionStatus.COMPLETED, request_id=sample_context.request_id, data=partial_data, error_message=None, execution_time_ms=50.0)
        assert result.is_success is True
...
class TestPerformanceOptimization:
    ...
    def test_rapid_result_creation(self):
        ...
        results = []
        for i in range(100):
            result_data = {'category': f'category_{i}', 'confidence_score': 0.8, 'result_id': i}
    ...
    def test_memory_efficient_results(self):
        ...
        results = []
        for i in range(50):
            result = {'id': i, 'category': 'test', 'confidence': 0.8}
...
class TestMetadataHandling:
    ...
    def context_with_metadata(self):
        return ExecutionContext(request_id='metadata_test', run_id='metadata_run', agent_name='MetadataAgent', state={'user_request': 'Test with metadata', 'timestamp': time.time(), 'user_id': 'test_user_123'}, correlation_id='metadata_correlation')

    def test_metadata_preservation(self, context_with_metadata):
        """Test preservation of metadata in results."""
        result_with_metadata = {'category': 'test', 'confidence_score': 0.8, 'metadata': {'original_request': context_with_metadata.state['user_request'], 'processing_timestamp': time.time(), 'user_context': context_with_metadata.state.get('user_id')}}
    ...
    def test_context_continuity(self, context_with_metadata):
        ...
        result = ExecutionResult(status=ExecutionStatus.COMPLETED, request_id=context_with_metadata.request_id, data={'test': 'data'}, error_message=None, execution_time_ms=100.0)
...
class TestEdgeCases:
    ...
    def test_large_result_data(self):
        ...
        large_data = {'category': 'large_analysis', 'confidence_score': 0.9, 'detailed_analysis': ['item'] * 1000}
