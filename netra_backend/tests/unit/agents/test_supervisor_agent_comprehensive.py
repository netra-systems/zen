...
class MockWebSocketManager:
    ...
    async def send_to_user(self, user_id: str, event_type: str, data: dict):
        ...
        self.events_sent.append({'user_id': user_id, 'event_type': event_type, 'data': data, 'timestamp': time.time()})
...
class MockAgentInstance(BaseAgent):
    ...
    def __init__(self, agent_name: str, should_succeed: bool=True, execution_result: Dict=None):
        ...
        super().__init__(llm_manager=mock_llm_manager, name=agent_name, description=f'Mock {agent_name} agent for testing', enable_reliability=False, enable_execution_engine=False, enable_caching=False)
...
class TestSupervisorAgentCore(BaseTestCase):
    ...
    def setUp(self):
        ...
        self.test_context = UserExecutionContext.from_request(user_id=f'test-user-{uuid.uuid4().hex[:8]}', thread_id=f'test-thread-{uuid.uuid4().hex[:8]}', run_id=f'test-run-{uuid.uuid4().hex[:8]}', websocket_client_id='test-connection-123', agent_context={'user_request': 'test request for SupervisorAgent'})
        self.mock_db_session = AsyncMock()
        self.test_context = self.test_context.with_db_session(self.mock_db_session)
        self.mock_class_registry = Mock()
        self.mock_class_registry.get_agent_classes.return_value = {'triage': Mock(), 'reporting': Mock(), 'data': Mock()}
    ...
    async def test_supervisor_execute_with_valid_context(self):
        ...
        mock_triage = MockAgentInstance('triage', should_succeed=True, execution_result={'data_sufficiency': 'sufficient', 'intent': {'primary_intent': 'test'}})
        mock_reporting = MockAgentInstance('reporting', should_succeed=True, execution_result={'report': 'test report', 'status': 'completed'})
    ...
    async def test_concurrent_user_execution_isolation(self):
        ...
        contexts = []
        for i in range(5):
            context = UserExecutionContext.from_request(user_id=f'concurrent-user-{i}', thread_id=f'concurrent-thread-{i}', run_id=f'concurrent-run-{i}', websocket_client_id=f'connection-{i}', agent_context={'user_request': f'concurrent test {i}'})
        ...
        async def execute_with_tracking(context, index):
            ...
            mock_reporting = MockAgentInstance('reporting', execution_result={'user_id': context.user_id, 'execution_index': index, 'timestamp': time.time()})
    ...
    async def test_websocket_event_delivery_all_5_events(self):
        ...
        with patch.object(self.supervisor, '_create_isolated_agent_instances', mock_create_agents), patch('netra_backend.app.agents.supervisor_consolidated.managed_session', return_value=mock_session_manager), patch.object(self.supervisor, '_get_tool_classes_from_context', return_value=[]), patch('netra_backend.app.agents.supervisor_consolidated.UserContextToolFactory') as mock_factory:
            ...
            thinking_events = [call for call in event_calls if call[1]['event_type'] == 'agent_thinking' or call[0][0] == 'agent_thinking']
    ...
    async def test_dynamic_workflow_determination(self):
        ...
        triage_result = {'data_sufficiency': 'sufficient', 'intent': {'primary_intent': 'analyze performance metrics'}}
    ...
    async def test_agent_retry_logic_exponential_backoff(self):
        ...
        result = await self.supervisor._execute_agent_with_retry(retry_agent, self.test_context, 'triage')
        execution_time = time.time() - start_time
        self.assertEqual(result['status'], 'completed')
        self.assertEqual(result['attempt'], 3)
        self.assertGreater(execution_time, 1.0)

    async def test_fallback_report_creation(self):
        """Test UVS fallback report creation when primary reporting fails."""
        partial_results = {'triage': {'status': 'completed', 'category': 'optimization', 'priority': 'high'}, 'data_helper': {'status': 'completed', 'guidance': 'Provide usage data for analysis'}, 'data': {'status': 'failed', 'error': 'Connection timeout'}}
    ...
    async def test_execution_context_isolation_validation(self):
        ...
        context1 = UserExecutionContext.from_request_supervisor(user_id='user-1', thread_id='thread-1', run_id='run-1', metadata={'test_data': 'context1_data'}).with_db_session(AsyncMock())
        context2 = UserExecutionContext.from_request_supervisor(user_id='user-2', thread_id='thread-2', run_id='run-2', metadata={'test_data': 'context2_data'}).with_db_session(AsyncMock())
        ...
        class ContextCapturingAgent(MockAgentInstance):
            ...
            async def execute(self, context, stream_updates=False):
                self.captured_contexts.append({'user_id': context.user_id, 'test_data': context.metadata.get('test_data'), 'metadata_id': id(context.metadata)})
    ...
    async def test_invalid_context_handling(self):
        ...
        invalid_context = UserExecutionContext.from_request_supervisor(user_id='', thread_id='test-thread', run_id='test-run')
        with self.assertRaises(InvalidContextError):
            await self.supervisor.execute(invalid_context)
        context_no_db = UserExecutionContext.from_request_supervisor(user_id='test-user', thread_id='test-thread', run_id='test-run')
    ...
    async def test_agent_metadata_propagation(self):
        ...
        triage_agent = MockAgentInstance('triage', execution_result={'data_sufficiency': 'partial', 'category': 'analysis', 'intent': {'primary_intent': 'data analysis'}})
        reporting_agent = MockAgentInstance('reporting', execution_result={'report': 'Analysis complete', 'summary': ['Data analyzed', 'Insights generated']})
    ...
    async def test_recoverable_error_detection(self):
        ...
        recoverable_errors = [ConnectionError('Network timeout'), TimeoutError('Request timeout'), RuntimeError('temporary unavailable'), Exception('rate limit exceeded')]
        for error in recoverable_errors:
            self.assertTrue(self.supervisor._is_recoverable_error(error))
        non_recoverable_errors = [ValueError('invalid request format'), RuntimeError('authentication failed'), Exception('permission denied'), RuntimeError('not found')]
    ...
    async def test_child_context_metadata_propagation(self):
        ...
        child_context = parent_context.create_child_context(operation_name='test_operation', additional_metadata={'child_data': 'test_value'})
        child_context.metadata['triage_result'] = {'data_sufficiency': 'sufficient', 'category': 'optimization'}
        child_context.metadata['custom_result'] = {'test': 'data'}
        self.supervisor._merge_child_metadata_to_parent(parent_context, child_context, 'triage')
        self.assertIn('triage_result', parent_context.metadata)
        self.assertEqual(parent_context.metadata['triage_result']['data_sufficiency'], 'sufficient')
...
class TestSupervisorAgentErrorScenarios(BaseTestCase):
    ...
    async def test_execution_lock_prevents_concurrent_same_instance(self):
        ...
        context = UserExecutionContext.from_request_supervisor(user_id='test-user', thread_id='test-thread', run_id='test-run').with_db_session(AsyncMock())
        execution_order = []
        ...
        async def mock_execution(ctx):
            ...
            return {'supervisor_result': 'completed', 'orchestration_successful': True, 'user_isolation_verified': True, 'results': {}, 'user_id': ctx.user_id, 'run_id': ctx.run_id}
        with patch.object(self.supervisor, '_orchestrate_agents', mock_execution), patch.object(self.supervisor, '_get_tool_classes_from_context', return_value=[]), patch('netra_backend.app.agents.supervisor_consolidated.UserContextToolFactory') as mock_factory:
            mock_tool_system = {'dispatcher': Mock(), 'registry': Mock(), 'tools': []}
            mock_tool_system['registry'].name = 'test_registry'
            mock_tool_system['dispatcher'].dispatcher_id = 'test_dispatcher'
            mock_factory.create_user_tool_system = AsyncMock(return_value=mock_tool_system)
    ...
    async def test_all_agents_fail_scenario(self):
        ...
        context = UserExecutionContext.from_request_supervisor(user_id='test-user', thread_id='test-thread', run_id='test-run').with_db_session(AsyncMock())
        failing_agents = {'triage': MockAgentInstance('triage', should_succeed=False), 'reporting': MockAgentInstance('reporting', should_succeed=False), 'data_helper': MockAgentInstance('data_helper', should_succeed=False)}
    ...
    async def test_websocket_bridge_failure_graceful_degradation(self):
        ...
        mock_session_manager.__aenter__ = AsyncMock(return_value=mock_session_manager)
        mock_session_manager.__aexit__ = AsyncMock(return_value=None)
        with patch.object(supervisor_with_failing_ws, '_create_isolated_agent_instances', mock_create_agents), patch('netra_backend.app.agents.supervisor_consolidated.managed_session', return_value=mock_session_manager), patch.object(supervisor_with_failing_ws, '_get_tool_classes_from_context', return_value=[]), patch('netra_backend.app.agents.supervisor_consolidated.UserContextToolFactory') as mock_factory:
            mock_tool_system = {'dispatcher': Mock(), 'registry': Mock(), 'tools': []}
            mock_tool_system['registry'].name = 'test_registry'
            mock_tool_system['dispatcher'].dispatcher_id = 'test_dispatcher'
            mock_factory.create_user_tool_system = AsyncMock(return_value=mock_tool_system)
...
class TestSupervisorAgentPerformance(BaseTestCase):
    ...
    async def test_high_concurrency_performance(self):
        ...
        contexts = []
        for i in range(10):
            context = UserExecutionContext.from_request_supervisor(user_id=f'perf-user-{i}', thread_id=f'perf-thread-{i}', run_id=f'perf-run-{i}', metadata={'test_index': i}).with_db_session(AsyncMock())
            contexts.append(context)
    ...
    async def test_memory_usage_under_load(self):
        ...
        for i in range(20):
            context = UserExecutionContext.from_request_supervisor(user_id=f'memory-user-{i}', thread_id=f'memory-thread-{i}', run_id=f'memory-run-{i}').with_db_session(AsyncMock())
