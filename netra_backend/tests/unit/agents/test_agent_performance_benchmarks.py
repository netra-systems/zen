...
class TestInitializationPerformance:
    ...
    def test_base_agent_initialization_time(self, mock_llm_manager):
        ...
        agents = []
        for i in range(10):
            agent = MockPerformanceAgent(llm_manager=mock_llm_manager, name=f'PerfAgent_{i}', enable_reliability=True)
            agents.append(agent)
...
class TestExecutionPerformance:
    ...
    def performance_agent(self, mock_llm_manager):
        return MockPerformanceAgent(llm_manager=mock_llm_manager, name='PerformanceTestAgent', enable_reliability=True, execution_delay=0.001)
...
class TestMemoryPerformance:
    ...
    def test_memory_usage_stability(self, mock_llm_manager):
        ...
        agent = MockPerformanceAgent(llm_manager=mock_llm_manager, name='MemoryTestAgent', enable_reliability=True, memory_pressure=True)
        assert agent.get_health_status() is not None
    ...
    def test_garbage_collection_efficiency(self, mock_llm_manager):
        ...
        agent_refs = []
        for i in range(10):
            agent = MockPerformanceAgent(llm_manager=mock_llm_manager, name=f'GCTestAgent_{i}', enable_reliability=True)
...
class TestCircuitBreakerPerformance:
    ...
    def test_circuit_breaker_overhead(self, mock_llm_manager):
        ...
        agent_with_cb = MockPerformanceAgent(llm_manager=mock_llm_manager, name='WithCBAgent', enable_reliability=True)
        agent_without_cb = MockPerformanceAgent(llm_manager=mock_llm_manager, name='WithoutCBAgent', enable_reliability=False)
...
class TestLongRunningOperationStability:
    ...
    def test_extended_operation_stability(self, mock_llm_manager):
        ...
        agent = MockPerformanceAgent(llm_manager=mock_llm_manager, name='ExtendedOpAgent', enable_reliability=True, execution_delay=0.001)
        assert agent.get_health_status() is not None
    ...
    def test_resource_cleanup_over_time(self, mock_llm_manager):
        ...
        agents = []
        for i in range(5):
            agent = MockPerformanceAgent(llm_manager=mock_llm_manager, name=f'CleanupAgent_{i}', enable_reliability=True)
            agents.append(agent)
        assert len(agents) == 5
        del agents
