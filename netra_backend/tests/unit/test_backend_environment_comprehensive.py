...
from netra_backend.app.core.backend_environment import BackendEnvironment, get_backend_env, get_jwt_secret_key, get_database_url, get_environment, is_production, is_development
...
class TestBackendEnvironmentComprehensive:
    ...
    def setup_method(self, method=None):
        ...
        self.database_builder_patcher = patch('shared.database_url_builder.DatabaseURLBuilder', MockDatabaseURLBuilder)
        self.database_builder_patcher.start()
        self.jwt_secret_patcher = patch('netra_backend.app.core.configuration.unified_secrets.get_jwt_secret', mock_get_jwt_secret)
    ...
    def test_init_validation_provides_test_defaults_in_test_context(self):
        ...
        with patch('netra_backend.app.core.backend_environment.logger') as mock_logger:
            ...
            assert backend_env.get_jwt_secret_key()
    ...
    def test_init_validation_builds_database_url(self):
        ...
        with patch('netra_backend.app.core.backend_environment.logger') as mock_logger:
            backend_env = BackendEnvironment()
            mock_logger.info.assert_called_with('Built database URL from POSTGRES_* environment variables')
    ...
    def test_get_postgres_port_returns_default_for_invalid(self):
        ...
        with patch('netra_backend.app.core.backend_environment.logger') as mock_logger:
            port = self.backend_env.get_postgres_port()
            assert port == 5432
            mock_logger.warning.assert_called_with('Invalid POSTGRES_PORT: not-a-number, using default 5432')
    ...
    def test_get_redis_port_returns_default_for_invalid(self):
        ...
        with patch('netra_backend.app.core.backend_environment.logger') as mock_logger:
            port = self.backend_env.get_redis_port()
            assert port == 6379
            mock_logger.warning.assert_called_with('Invalid REDIS_PORT: invalid-port, using default 6379')
    ...
    def test_validate_allows_dev_secrets_in_development(self):
        ...
        self.set_env_var('POSTGRES_DB', 'test_db')
        result = self.backend_env.validate()
        dev_secret_issues = [issue for issue in result['issues'] if 'development' in issue.lower() and 'secret' in issue.lower()]
    ...
    def test_boolean_conversion_edge_cases(self):
        ...
        test_cases = [('True', True), ('true', True), ('TRUE', True), ('1', True), ('yes', True), ('on', True), ('False', False), ('false', False), ('FALSE', False), ('0', False), ('no', False), ('off', False), ('', False), ('random', False)]
