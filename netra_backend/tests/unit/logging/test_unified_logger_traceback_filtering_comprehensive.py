...
from netra_backend.app.core.unified_logging import UnifiedLogger, central_logger, get_central_logger, get_logger, log_execution_time
from netra_backend.app.core.logging_formatters import LogFormatter, LogHandlerConfig, SensitiveDataFilter, LogEntry
from netra_backend.app.core.logging_context import LoggingContext, PerformanceTracker, request_id_context, trace_id_context, user_id_context
...
class UnifiedLoggerTracebackFilteringTests(SSotBaseTestCase):
    ...
    def test_gcp_json_formatter_excludes_multiline_tracebacks_in_staging(self):
        ...
        mock_record = {'level': type('Level', (), {'name': 'ERROR'})(), 'time': datetime.now(timezone.utc), 'message': 'Test error with traceback', 'name': 'test_module', 'function': 'test_function', 'line': 42, 'exception': type('Exception', (), {'type': ValueError, 'value': ValueError('Test exception'), 'traceback': 'Traceback (most recent call last):\n  File "test.py", line 1, in <module>\n    raise ValueError("Test")\nValueError: Test'})(), 'extra': {}}
    ...
    def test_gcp_json_formatter_escapes_newlines_in_exception_messages(self):
        ...
        mock_record = {'level': type('Level', (), {'name': 'CRITICAL'})(), 'time': datetime.now(timezone.utc), 'message': f'Critical error: {multiline_exception}', 'name': 'test_module', 'function': 'test_function', 'line': 100, 'exception': type('Exception', (), {'type': RuntimeError, 'value': RuntimeError(multiline_exception), 'traceback': f'Traceback:\n  File test.py\n    error\nRuntimeError: {multiline_exception}'})(), 'extra': {}}
    ...
    def test_gcp_json_formatter_handles_large_tracebacks_efficiently(self):
        ...
        mock_record = {'level': type('Level', (), {'name': 'ERROR'})(), 'time': datetime.now(timezone.utc), 'message': 'Error with large traceback', 'name': 'performance_test', 'function': 'large_stack_function', 'line': 500, 'exception': type('Exception', (), {'type': RuntimeError, 'value': RuntimeError('Large stack trace error'), 'traceback': large_traceback})(), 'extra': {}}
    ...
    def test_gcp_json_formatter_vs_regular_json_formatter_traceback_handling(self):
        ...
        mock_record = {'level': type('Level', (), {'name': 'ERROR'})(), 'time': datetime.now(timezone.utc), 'message': 'Comparison test error', 'name': 'comparison_test', 'function': 'compare_formatters', 'line': 25, 'exception': type('Exception', (), {'type': Exception, 'value': Exception('test error'), 'traceback': traceback_text})(), 'extra': {'context_key': 'context_value'}}
...
class UnifiedLoggerGCPIntegrationTests(SSotBaseTestCase):
    ...
    def test_gcp_reporter_lazy_initialization_success(self):
        ...
        with patch('netra_backend.app.services.monitoring.gcp_error_reporter.get_error_reporter', mock_get_error_reporter):
...
class UnifiedLoggerContextBuildingTests(SSotBaseTestCase):
    ...
    def test_context_building_includes_correlation_ids(self):
        ...
        logger.set_context(request_id=test_request_id, user_id=test_user_id, trace_id=test_trace_id)
        context = logger._build_log_context({'custom_key': 'custom_value', 'operation': 'test_operation'})
    ...
    def test_context_building_filters_sensitive_data(self):
        ...
        logger = UnifiedLogger()
        context = logger._build_log_context({'password': 'secret123', 'api_key': 'key_abc123', 'username': 'test_user', 'operation': 'login'})
    ...
    def test_context_building_with_nested_sensitive_data(self):
        ...
        logger = UnifiedLogger()
        context = logger._build_log_context({'user_info': {'username': 'testuser', 'password': 'secret123', 'profile': {'email': 'test@example.com', 'api_key': 'nested_key_123'}}, 'config': {'database_url': 'postgres://user:pass@host/db', 'debug': True}})
        assert context['user_info']['password'] == 'REDACTED'
        assert context['user_info']['profile']['api_key'] == 'REDACTED'
        assert context['user_info']['username'] == 'testuser'
...
class UnifiedLoggerPerformanceTests(SSotBaseTestCase):
    ...
    def test_context_variable_isolation_across_threads(self):
        ...
        def context_worker(thread_id):
            ...
            return {'thread_id': thread_id, 'expected_request_id': request_id, 'expected_user_id': user_id, 'context': context}
        num_threads = 5
        with ThreadPoolExecutor(max_workers=num_threads) as executor:
...
class UnifiedLoggerErrorHandlingTests(SSotBaseTestCase):
    ...
    def test_null_and_none_values_in_context(self):
        ...
        logger = UnifiedLogger()
        logger.info('Test with None values', null_value=None, empty_string='', zero_value=0, false_value=False)
        context = logger._build_log_context({'none_value': None, 'empty_list': [], 'empty_dict': {}, 'valid_value': 'test'})
        assert isinstance(context, dict)
...
class UnifiedLoggerIntegrationTests(SSotBaseTestCase):
    ...
    def test_integration_with_sensitive_data_filter(self):
        ...
        filtered_context = logger._build_log_context({'user_password': 'secret123', 'database_url': 'postgres://user:pass@host/db', 'credit_card': '4111-1111-1111-1111', 'normal_field': 'normal_value'})
    ...
    def test_performance_tracking_integration(self):
        ...
        logger = UnifiedLogger()
        logger.log_performance('database_query', 0.25, query_type='SELECT')
        logger.log_api_call('POST', '/api/users', 201, 0.35, user_count=100)
...
class UnifiedLoggerConfigurationTests(SSotBaseTestCase):
    ...
    def test_configuration_loading_with_unified_config(self):
        ...
        with patch('netra_backend.app.core.unified_logging.unified_config_manager', mock_unified_config_manager):
            config = logger._load_config()
    ...
    def test_configuration_fallback_on_import_error(self):
        ...
        with patch('netra_backend.app.core.unified_logging.unified_config_manager', side_effect=ImportError('Config module not available')):
            config = logger._load_config()
...
class UnifiedLoggerAsyncSupportTests(SSotAsyncTestCase):
    ...
    async def test_concurrent_async_logging(self):
        ...
        async def async_log_worker(worker_id):
            ...
            for i in range(10):
                logger.info(f'Async worker {worker_id} message {i}', worker_id=worker_id, message_num=i)
