...
class TestTimeoutConfigurationIssues(SSotBaseTestCase):
    ...
    def test_timeout_config_empty_error_message_bug(self):
        ...
        if dead_executions:
            ...
            # Check if error message lacks important context
            ...
            if missing_context:
                self.fail(
                    f"BUG REPRODUCED: Timeout error message lacks important context: {missing_context}. "
                    f"Error message: '{error_message}'. "
                    "Timeout errors should include execution ID, duration, and timeout threshold."
                )
        else:
    ...
    async def test_circuit_breaker_timeout_error_message_bug(self):
        ...
        try:
            ...
            if not error_message or error_message.strip() == "":
                self.fail(
                    "BUG REPRODUCED: Circuit breaker timeout produced empty error message"
                )
        ...
        try:
            ...
            if not error_message or error_message.strip() == "":
                self.fail(
                    "BUG REPRODUCED: Circuit breaker open error produced empty error message"
                )
