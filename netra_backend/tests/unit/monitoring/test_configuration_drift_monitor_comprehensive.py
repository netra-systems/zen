...
from netra_backend.app.monitoring.configuration_drift_monitor import ConfigurationDriftMonitor, E2EOAuthSimulationKeyValidator, JWTSecretAlignmentValidator, WebSocketConfigurationValidator, ConfigurationDrift, DriftSeverity, ConfigurationScope, extend_staging_health_monitor_with_drift_detection, get_configuration_drift_monitor
...
class TestConfigurationDriftDataClasses:
    ...
    def test_configuration_drift_initialization(self):
        ...
        drift = ConfigurationDrift(config_key='TEST_KEY', scope=ConfigurationScope.AUTHENTICATION, severity=DriftSeverity.CRITICAL, current_value='current', expected_value='expected', environment='test', detection_timestamp=time.time())
    ...
    def test_configuration_drift_to_dict(self):
        ...
        drift = ConfigurationDrift(config_key='SERIALIZATION_TEST', scope=ConfigurationScope.WEBSOCKET, severity=DriftSeverity.HIGH, current_value='current_value', expected_value='expected_value', environment='staging', detection_timestamp=1234567890.0, business_impact_mrr=50000.0, cascade_risk=['auth_failure', 'websocket_disconnect'], remediation_priority=2)
    ...
    def test_configuration_drift_value_truncation(self):
        ...
        long_value = 'x' * 100
        drift = ConfigurationDrift(config_key='TRUNCATION_TEST', scope=ConfigurationScope.DATABASE, severity=DriftSeverity.MODERATE, current_value=long_value, expected_value=long_value, environment='test', detection_timestamp=time.time())
        drift_dict = drift.to_dict()
        assert len(drift_dict['current_value']) == 53
        assert drift_dict['current_value'].endswith('...')
        assert len(drift_dict['expected_value']) == 53
...
class TestE2EOAuthSimulationKeyValidator:
    ...
    async def test_validate_key_consistency_staging_with_functional_test(self):
        ...
        with patch.object(self.validator, '_test_e2e_oauth_simulation') as mock_test:
            mock_test.return_value = {'success': True, 'user_id': 'test_user_123', 'e2e_bypass_used': True, 'test_timestamp': time.time()}
            result = await self.validator.validate_key_consistency()
    ...
    async def test_validate_key_consistency_staging_with_non_functional_key(self):
        ...
        self.env.set('ENVIRONMENT', 'staging', 'test')
        with patch.object(self.validator, '_test_e2e_oauth_simulation') as mock_test:
            mock_test.return_value = {'success': False, 'error': 'Authentication failed', 'test_timestamp': time.time()}
            result = await self.validator.validate_key_consistency()
    ...
    async def test_test_e2e_oauth_simulation_functionality(self):
        ...
        with patch('netra_backend.app.monitoring.configuration_drift_monitor.get_unified_auth_service') as mock_auth:
            ...
            mock_auth_service.authenticate_websocket.return_value = (MagicMock(success=True, user_id='test_user', metadata={'e2e_bypass': True}), MagicMock())
    ...
    async def test_test_e2e_oauth_simulation_failure(self):
        ...
        with patch('netra_backend.app.monitoring.configuration_drift_monitor.get_unified_auth_service') as mock_auth:
            mock_auth_service = AsyncMock()
            mock_auth_service.authenticate_websocket.return_value = (MagicMock(success=False, error='Authentication failed'), None)
            mock_auth.return_value = mock_auth_service
            result = await self.validator._test_e2e_oauth_simulation(oauth_key)
...
class TestJWTSecretAlignmentValidator:
    ...
    async def test_validate_jwt_secret_alignment_valid_secret(self):
        ...
        with patch.object(self.validator, '_test_jwt_functionality') as mock_test:
            mock_test.return_value = {'success': True, 'auth_service_status': 'healthy', 'test_timestamp': time.time()}
            result = await self.validator.validate_jwt_secret_alignment()
    ...
    async def test_validate_jwt_secret_alignment_non_functional_secret(self):
        ...
        self.env.set('ENVIRONMENT', 'staging', 'test')
        with patch.object(self.validator, '_test_jwt_functionality') as mock_test:
            mock_test.return_value = {'success': False, 'error': 'Auth service unhealthy: degraded', 'auth_service_status': 'degraded', 'test_timestamp': time.time()}
            result = await self.validator.validate_jwt_secret_alignment()
...
class TestWebSocketConfigurationValidator:
    ...
    async def test_validate_websocket_config_coherence_missing_url(self):
        ...
        assert websocket_drift['business_impact_mrr'] == 120000.0
    ...
    async def test_validate_websocket_config_coherence_with_url(self):
        ...
        with patch.object(self.validator, '_test_websocket_auth_integration') as mock_auth_test:
            mock_auth_test.return_value = {'success': True, 'auth_service_healthy': True, 'websocket_enabled': True, 'test_timestamp': time.time()}
            with patch.object(self.validator, '_validate_websocket_cors_config') as mock_cors:
                mock_cors.return_value = {'drift_detected': False, 'validation_timestamp': time.time()}
                result = await self.validator.validate_websocket_config_coherence()
    ...
    async def test_validate_websocket_config_coherence_auth_integration_failure(self):
        ...
        self.env.set('ENVIRONMENT', 'staging', 'test')
        with patch.object(self.validator, '_test_websocket_auth_integration') as mock_auth_test:
            mock_auth_test.return_value = {'success': False, 'auth_service_healthy': False, 'websocket_enabled': True, 'error': 'Auth service unhealthy: degraded', 'test_timestamp': time.time()}
            with patch.object(self.validator, '_validate_websocket_cors_config') as mock_cors:
                mock_cors.return_value = {'drift_detected': False, 'validation_timestamp': time.time()}
                result = await self.validator.validate_websocket_config_coherence()
                ...
                auth_drifts = [d for d in result['drift_details'] if d['config_key'] == 'WEBSOCKET_AUTHENTICATION_INTEGRATION']
...
class TestConfigurationDriftMonitor:
    ...
    async def test_check_health_critical_drift(self):
        ...
        self.env.set('ENVIRONMENT', 'staging', 'test')
        critical_drift_detail = {'config_key': 'JWT_SECRET_KEY', 'scope': 'authentication', 'severity': 'critical', 'current_value': '<MISSING>', 'expected_value': '<REQUIRED>', 'detection_timestamp': time.time(), 'business_impact_mrr': 120000.0, 'cascade_risk': ['complete_authentication_failure'], 'remediation_priority': 1}
        with patch.object(self.monitor.e2e_oauth_validator, 'validate_key_consistency') as mock_oauth:
            mock_oauth.return_value = {'drift_detected': False, 'drift_details': []}
            with patch.object(self.monitor.jwt_secret_validator, 'validate_jwt_secret_alignment') as mock_jwt:
                mock_jwt.return_value = {'drift_detected': True, 'drift_details': [critical_drift_detail]}
                with patch.object(self.monitor.websocket_config_validator, 'validate_websocket_config_coherence') as mock_ws:
                    mock_ws.return_value = {'drift_detected': False, 'drift_details': []}
                    result = await self.monitor.check_health()
    ...
    async def test_check_health_high_impact_drift(self):
        ...
        self.env.set('ENVIRONMENT', 'staging', 'test')
        high_impact_drift = {'config_key': 'E2E_OAUTH_SIMULATION_KEY', 'scope': 'authentication', 'severity': 'high', 'current_value': '<MISSING>', 'expected_value': '<REQUIRED>', 'detection_timestamp': time.time(), 'business_impact_mrr': 75000.0, 'cascade_risk': ['e2e_test_failures'], 'remediation_priority': 2}
        with patch.object(self.monitor.e2e_oauth_validator, 'validate_key_consistency') as mock_oauth:
            mock_oauth.return_value = {'drift_detected': True, 'drift_details': [high_impact_drift]}
            with patch.object(self.monitor.jwt_secret_validator, 'validate_jwt_secret_alignment') as mock_jwt:
                mock_jwt.return_value = {'drift_detected': False, 'drift_details': []}
                with patch.object(self.monitor.websocket_config_validator, 'validate_websocket_config_coherence') as mock_ws:
                    mock_ws.return_value = {'drift_detected': False, 'drift_details': []}
    ...
    def test_generate_remediation_recommendations(self):
        ...
        drift_details = [{'config_key': 'E2E_OAUTH_SIMULATION_KEY', 'severity': 'critical'}, {'config_key': 'JWT_SECRET_KEY', 'severity': 'critical'}, {'config_key': 'NEXT_PUBLIC_WS_URL', 'severity': 'high'}]
    ...
    async def test_get_drift_history(self):
        ...
        old_drift = ConfigurationDrift(config_key='OLD_DRIFT', scope=ConfigurationScope.ENVIRONMENT, severity=DriftSeverity.LOW, current_value='old', expected_value='new', environment='test', detection_timestamp=time.time() - 48 * 3600)
        recent_drift = ConfigurationDrift(config_key='RECENT_DRIFT', scope=ConfigurationScope.AUTHENTICATION, severity=DriftSeverity.HIGH, current_value='recent', expected_value='new', environment='test', detection_timestamp=time.time() - 1 * 3600)
    ...
    async def test_clear_resolved_drifts(self):
        ...
        drift1 = ConfigurationDrift(config_key='RESOLVED_DRIFT_1', scope=ConfigurationScope.AUTHENTICATION, severity=DriftSeverity.HIGH, current_value='old', expected_value='new', environment='test', detection_timestamp=time.time())
        drift2 = ConfigurationDrift(config_key='UNRESOLVED_DRIFT', scope=ConfigurationScope.DATABASE, severity=DriftSeverity.MODERATE, current_value='old', expected_value='new', environment='test', detection_timestamp=time.time())
