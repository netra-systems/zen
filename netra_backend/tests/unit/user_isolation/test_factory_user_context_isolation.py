...
from netra_backend.app.services.user_execution_context import UserExecutionContext, UserContextFactory, InvalidContextError, ContextIsolationError, validate_user_context, managed_user_context, register_shared_object, clear_shared_object_registry
from netra_backend.app.services.websocket_bridge_factory import WebSocketBridgeFactory, UserWebSocketContext, WebSocketFactoryConfig, ConnectionStatus
...
class TestFactoryUserContextIsolation(SSotBaseTestCase):
    ...
    def test_user_context_factory_creates_isolated_instances(self):
        ...
        for i in range(5):
            ...
            run_id = f'run_{i}'
            context = UserContextFactory.create_context(user_id=user_id, thread_id=thread_id, run_id=run_id)
        ...
        for i, ctx1 in enumerate(context_list):
            for j, ctx2 in enumerate(context_list):
                if i != j:
                    assert id(ctx1.agent_context) != id(ctx2.agent_context), f'Contexts {i} and {j} share agent_context object - ISOLATION VIOLATION'
                    assert id(ctx1.audit_metadata) != id(ctx2.audit_metadata), f'Contexts {i} and {j} share audit_metadata object - ISOLATION VIOLATION'
        ...
        for user_id, ctx in user_contexts.items():
            if user_id != 'user_0':
                assert 'test_key' not in ctx.agent_context, f'Context {user_id} contaminated with test_key - ISOLATION VIOLATION'
                assert 'audit_key' not in ctx.audit_metadata, f'Context {user_id} contaminated with audit_key - ISOLATION VIOLATION'
    ...
    def test_child_context_inheritance_maintains_isolation(self):
        ...
        parent_context = UserContextFactory.create_context(user_id='parent_user', thread_id='parent_thread', run_id='parent_run')
        parent_context.agent_context['parent_data'] = 'parent_value'
        parent_context.audit_metadata['parent_audit'] = 'parent_audit_value'
        child_contexts = []
        for i in range(3):
            child = parent_context.create_child_context(operation_name=f'child_operation_{i}', additional_agent_context={'child_specific': f'child_value_{i}'}, additional_audit_metadata={'child_audit': f'child_audit_{i}'})
            ...
            assert child.operation_depth == parent_context.operation_depth + 1
            ...
            assert id(child.agent_context) != id(parent_context.agent_context), 'Child shares agent_context object with parent - ISOLATION VIOLATION'
            assert id(child.audit_metadata) != id(parent_context.audit_metadata), 'Child shares audit_metadata object with parent - ISOLATION VIOLATION'
        ...
        for i, child1 in enumerate(child_contexts):
            for j, child2 in enumerate(child_contexts):
                if i != j:
                    ...
                    assert id(child1.agent_context) != id(child2.agent_context), f'Child contexts {i} and {j} share agent_context - ISOLATION VIOLATION'
                    assert id(child1.audit_metadata) != id(child2.audit_metadata), f'Child contexts {i} and {j} share audit_metadata - ISOLATION VIOLATION'
        ...
        assert 'isolation_test' not in parent_context.agent_context, 'Parent context contaminated by child modification - ISOLATION VIOLATION'
        for i in range(1, len(child_contexts)):
            assert 'isolation_test' not in child_contexts[i].agent_context, f'Child context {i} contaminated by sibling modification - ISOLATION VIOLATION'
    ...
    def test_concurrent_context_creation_isolation(self):
        ...
        def create_user_context(user_index: int) -> IsolationTestContext:
            ...
            test_ctx = IsolationTestContext(user_id=user_id, context_id=context.request_id, created_objects=[context, context.agent_context, context.audit_metadata], shared_state_violations=[])
            try:
        ...
        with ThreadPoolExecutor(max_workers=5) as executor:
            future_to_index = {executor.submit(create_user_context, i): i for i in range(num_users)}
            test_results = {}
        ...
        for i, ctx1 in enumerate(context_list):
            assert ctx1.isolation_verified, f'Context {i} failed isolation verification: {ctx1.shared_state_violations}'
            for j, ctx2 in enumerate(context_list):
                if i != j:
                    for obj1 in ctx1.created_objects:
                        for obj2 in ctx2.created_objects:
                            assert id(obj1) != id(obj2), f'Contexts {i} and {j} share object reference - ISOLATION VIOLATION'
                    assert ctx1.user_id != ctx2.user_id
        ...
        for user_index, test_ctx in test_results.items():
            ...
            for other_user_index in range(num_users):
                if other_user_index != user_index:
                    ...
                    assert other_key not in actual_context.agent_context, f'Context {user_index} contaminated with data from user {other_user_index} - ISOLATION VIOLATION'

    def test_websocket_bridge_factory_user_isolation(self):
        """Test WebSocketBridgeFactory creates isolated per-user emitters."""
        config = WebSocketFactoryConfig(max_events_per_user=100, event_timeout_seconds=10.0)
        ...
        for i in range(3):
            ...
            context = UserWebSocketContext(user_id=user_id, thread_id=thread_id, connection_id=connection_id)
            user_contexts[user_id] = context
            assert context.user_id == user_id
            assert context.thread_id == thread_id
            ...
            assert context.event_queue._maxsize == 1000
        context_list = list(user_contexts.values())
        for i, ctx1 in enumerate(context_list):
            for j, ctx2 in enumerate(context_list):
                if i != j:
                    assert id(ctx1.event_queue) != id(ctx2.event_queue), f'WebSocket contexts {i} and {j} share event queue - ISOLATION VIOLATION'
                    assert id(ctx1.sent_events) != id(ctx2.sent_events), f'WebSocket contexts {i} and {j} share sent_events list - ISOLATION VIOLATION'
                    assert id(ctx1.failed_events) != id(ctx2.failed_events), f'WebSocket contexts {i} and {j} share failed_events list - ISOLATION VIOLATION'
                    assert id(ctx1.cleanup_callbacks) != id(ctx2.cleanup_callbacks), f'WebSocket contexts {i} and {j} share cleanup_callbacks - ISOLATION VIOLATION'
                    assert ctx1.user_id != ctx2.user_id
    ...
    def test_context_validation_prevents_dangerous_values(self):
        ...
        forbidden_values = ['registry', 'placeholder', 'default', 'temp', 'none', 'null', 'undefined', '0', '1', 'xxx', 'yyy', 'example', 'demo']
        for forbidden_value in forbidden_values:
            with pytest.raises(InvalidContextError, match='forbidden placeholder value'):
                UserContextFactory.create_context(user_id=forbidden_value, thread_id='valid_thread', run_id='valid_run')
            with pytest.raises(InvalidContextError, match='forbidden placeholder value'):
                UserContextFactory.create_context(user_id='valid_user', thread_id=forbidden_value, run_id='valid_run')
            with pytest.raises(InvalidContextError, match='forbidden placeholder value'):
                UserContextFactory.create_context(user_id='valid_user', thread_id='valid_thread', run_id=forbidden_value)
        forbidden_patterns = ['placeholder_', 'registry_', 'default_', 'temp_', 'example_', 'demo_', 'sample_', 'template_', 'mock_', 'fake_']
        for pattern in forbidden_patterns:
            short_value = pattern + 'short'
            with pytest.raises(InvalidContextError, match='placeholder pattern'):
                UserContextFactory.create_context(user_id=short_value, thread_id='valid_thread', run_id='valid_run')

    def test_context_immutability_enforces_isolation(self):
        """Test that context immutability prevents accidental modification."""
        context = UserContextFactory.create_context(user_id='immutable_user', thread_id='immutable_thread', run_id='immutable_run')
    ...
    def test_managed_context_ensures_resource_cleanup(self):
        ...
        context = UserContextFactory.create_with_session(user_id='managed_user', thread_id='managed_thread', run_id='managed_run', db_session=mock_session)
        ...
        context_2 = UserContextFactory.create_with_session(user_id='managed_user_2', thread_id='managed_thread_2', run_id='managed_run_2', db_session=mock_session_2)
    ...
    def test_factory_prevents_shared_object_leakage(self):
        ...
        context = UserContextFactory.create_context(user_id='isolation_test_user', thread_id='isolation_test_thread', run_id='isolation_test_run')
    ...
    def test_deep_child_context_hierarchy_maintains_isolation(self):
        ...
        for depth in range(1, 4):
            parent = contexts[-1]
            child = parent.create_child_context(operation_name=f'operation_depth_{depth}', additional_agent_context={f'depth_{depth}_data': f'value_{depth}'}, additional_audit_metadata={f'depth_{depth}_audit': f'audit_{depth}'})
            ...
            assert child.thread_id == root_context.thread_id
            assert child.run_id == root_context.run_id
            assert child.verify_isolation() is True
        ...
        for i, ctx in enumerate(contexts):
            if i != 2:
                assert 'hierarchy_test' not in ctx.agent_context, f'Context at level {i} contaminated by level 2 modification'
        deepest_child = contexts[-1]
        for depth in range(1, 4):
    ...
    def test_context_with_websocket_connection_isolation(self):
        ...
        contexts_with_ws = []
        for i in range(3):
            context = UserContextFactory.create_context(user_id=f'ws_context_user_{i}', thread_id=f'ws_context_thread_{i}', run_id=f'ws_context_run_{i}')
