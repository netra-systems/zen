...
class TestWebSocketCORSHandler(SSotBaseTestCase):
    ...
    def test_cors_handler_origin_validation_allowed_exact_match(self):
        ...
        assert handler.is_origin_allowed("https://app.netra.ai") is True
    ...
    def test_cors_handler_origin_validation_wildcard_patterns(self):
        ...
        assert handler.is_origin_allowed("https://app.netra.ai") is True
        assert handler.is_origin_allowed("https://staging.netra.ai") is True
        
        # Different domain should be denied
        assert handler.is_origin_allowed("https://app.evil.com") is False
        assert handler.is_origin_allowed("https://netra.ai.evil.com") is False
    ...
    def test_cors_handler_production_security_enforcement(self):
        ...
        suspicious_origins = [
            "http://app.netra.ai",  # HTTP in production
...
class TestWebSocketCORSMiddleware(SSotBaseTestCase):
    ...
    async def test_cors_middleware_websocket_scope_handling(self):
        ...
        scope = {
            "type": "websocket",
            "headers": [(b"origin", b"https://app.netra.ai")]
        }
        receive = AsyncMock()
        send = AsyncMock()
        ...
        assert scope["websocket_cors_origin"] == "https://app.netra.ai"
...
class TestWebSocketCORSUtilityFunctions(SSotBaseTestCase):
    ...
    def test_extract_origin_from_websocket_single_header(self):
        ...
        assert origin == "https://app.netra.ai"
    ...
    def test_extract_origin_from_websocket_multiple_identical_headers(self):
        ...
        mock_websocket.headers.items.return_value = [
            ("origin", "https://app.netra.ai"),
            ("origin", "https://app.netra.ai"),  # Duplicate
            ("user-agent", "Mozilla/5.0")
        ]
    ...
    def test_extract_origin_from_websocket_case_insensitive(self):
        ...
        mock_websocket = Mock()
        mock_websocket.headers.items.return_value = [
            ("Origin", "https://app.netra.ai"),  # Capital O
            ("USER-AGENT", "Mozilla/5.0")
        ]
