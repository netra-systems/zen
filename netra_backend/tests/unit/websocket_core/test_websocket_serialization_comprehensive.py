...
class WebSocketStateSerializationTests:
    ...
    def test_websocket_state_in_dictionary(self):
        ...
        try:
            from starlette.websockets import WebSocketState
            data = {'connection_state': WebSocketState.CONNECTED, 'message': 'Connection established', 'timestamp': 1234567890}
    ...
    def test_websocket_state_in_nested_structure(self):
        ...
        try:
            from starlette.websockets import WebSocketState
            data = {'connection': {'state': WebSocketState.CONNECTED, 'diagnostics': {'client_state': WebSocketState.CONNECTED, 'application_state': WebSocketState.CONNECTED}}, 'metadata': {'states_list': [WebSocketState.CONNECTING, WebSocketState.CONNECTED, WebSocketState.DISCONNECTED]}}
            result = _serialize_message_safely(data)
...
class GeneralSerializationTests:
    ...
    def test_list_serialization(self):
        ...
        try:
            from starlette.websockets import WebSocketState
            data = ['string', 42, WebSocketState.CONNECTED, SerializationTestEnum.VALUE_ONE, datetime(2023, 1, 1, tzinfo=timezone.utc)]
            result = _serialize_message_safely(data)
            ...
            data = ['string', 42, SerializationTestEnum.VALUE_ONE, datetime(2023, 1, 1, tzinfo=timezone.utc)]
            result = _serialize_message_safely(data)
            assert result[0] == 'string'
            assert result[1] == 42
...
class SerializationPerformanceTests:
    ...
    def test_large_nested_structure(self):
        ...
        try:
            from starlette.websockets import WebSocketState
            data = {'connections': {f'conn_{i}': {'state': WebSocketState.CONNECTED if i % 2 == 0 else WebSocketState.DISCONNECTED, 'metadata': {'created_at': datetime.now(timezone.utc), 'message_count': i * 10}} for i in range(100)}}
            result = _serialize_message_safely(data)
            for i in range(100):
                conn_key = f'conn_{i}'
...
class OriginalBugReproductionTests:
    ...
    def test_websocket_error_message_serialization(self):
        ...
        try:
            from starlette.websockets import WebSocketState
            error_context = {'error_type': 'ConnectionError', 'environment': 'staging', 'connection_diagnostics': {'has_websocket': True, 'websocket_type': 'WebSocket', 'connection_age_seconds': 1.23, 'metadata_present': False, 'websocket_state': WebSocketState.CONNECTED}}
            ...
            assert json_str is not None
            assert '"websocket_state": "connected"' in json_str
            message = {'type': 'error', 'error': {'code': 'CONNECTION_ERROR', 'message': 'Connection diagnostics available', 'context': safe_error_context}, 'timestamp': 1234567890}
