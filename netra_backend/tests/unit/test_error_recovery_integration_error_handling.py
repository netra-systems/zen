...
class EnhancedErrorRecoverySystemErrorHandlingTests:
    ...
    def test_error_data_preparation_for_agents(self):
        ...
        user_id = 'user123'
        error_data = self.recovery_system._prepare_error_data('triage', 'process_request', error, context_data, user_id)
    ...
    def test_error_data_preparation_for_database(self):
        ...
        error_data = self.recovery_system._prepare_database_error_data('users', 'insert', error, 'tx123')
    ...
    def test_error_data_preparation_for_api(self):
        ...
        error_data = self.recovery_system._prepare_api_error_data('/api/users', 'GET', error, 503)
    ...
    def test_error_data_preparation_for_websocket(self):
        ...
        error_data = self.recovery_system._prepare_websocket_error_data('conn123', error)
    ...
    def test_recovery_context_building_for_agents(self):
        ...
        context = self.recovery_system._build_recovery_context('triage', 'process_request', error)
    ...
    def test_recovery_context_building_for_api(self):
        ...
        from netra_backend.app.core.error_recovery import OperationType
        error = TimeoutError('API timeout')
        context = self.recovery_system._build_api_recovery_context('/api/data', 'POST', error, 504)
    ...
    def test_database_recovery_logic_with_rollback_data(self):
        ...
        result = self.recovery_system._handle_database_recovery_logic(rollback_data, 'orders', error)
        assert result['status'] == 'rollback_queued'
    ...
    def test_database_recovery_logic_without_rollback_data(self):
        ...
        with pytest.raises(ConnectionError, match='DB connection lost'):
            self.recovery_system._handle_database_recovery_logic(None, 'orders', error)

    @pytest.mark.asyncio
    async def test_memory_exhaustion_handling(self):
        """Test memory exhaustion handling."""
        with patch.object(self.recovery_system.memory_monitor, 'take_snapshot') as mock_snapshot, patch.object(self.recovery_system.memory_monitor, 'check_and_recover') as mock_recover:
    ...
    def test_error_handling_edge_cases(self):
        ...
        error_data = self.recovery_system._prepare_error_data('triage', 'test', ValueError('test'), None, None)
        assert error_data['context'] == {}
        assert error_data['user_id'] is None
        error_data = self.recovery_system._prepare_error_data('triage', 'test', ValueError('test'), {}, 'user123')
        assert error_data['context'] == {}
        assert error_data['user_id'] == 'user123'
