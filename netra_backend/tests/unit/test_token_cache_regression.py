...
class TestTokenCacheRegression:
    ...
    async def test_cache_token_stores_validation_result(self, token_cache):
        ...
        validation_result = {'valid': True, 'user_id': 'user_123', 'email': 'test@example.com', 'permissions': ['read', 'write'], 'exp': int(time.time()) + 3600}
    ...
    async def test_integration_with_auth_client_core_flow(self, token_cache):
        ...
        validation_result = {'valid': True, 'user_id': 'integration_user', 'email': 'user@example.com', 'permissions': ['read'], 'exp': int(time.time()) + 3600}
        await token_cache.cache_token(test_token, validation_result)
        cached = await token_cache.get_cached_token(test_token)
...
class TestTokenCacheErrorScenarios:
    ...
    async def test_cache_token_with_complex_validation_result(self):
        ...
        complex_result = {'valid': True, 'user': {'id': 'user_789', 'email': 'complex@example.com', 'profile': {'name': 'Test User', 'roles': ['admin', 'user'], 'metadata': {'last_login': '2025-01-01T00:00:00Z', 'mfa_enabled': True}}}, 'session': {'id': 'session_123', 'expires_at': int(time.time()) + 7200}, 'permissions': ['read', 'write', 'delete'], 'token_metadata': {'issued_at': int(time.time()), 'issuer': 'auth_service', 'audience': ['api', 'web']}}
