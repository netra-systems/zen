...
class TestAgentExecutionCoreImportValidation(SSotBaseTestCase):
    ...
    def test_get_agent_state_tracker_import_should_fail(self):
        ...
        with self.expect_exception((ImportError, AttributeError), 'get_agent_state_tracker'):
            ...
            try:
                ...
                assert False, 'UNEXPECTED: get_agent_state_tracker was imported successfully. This suggests the import issue may have been fixed or the function exists but has other problems.'
    ...
    def test_available_imports_in_agent_execution_core(self):
        ...
        try:
            ...
            if missing_functions:
                assert False, f'BUG REPRODUCED: agent_execution_core module is missing expected functions: {missing_functions}. Available attributes: {available_attributes}. This causes ImportError when other modules try to import these functions.'
    ...
    def test_import_path_consistency_check(self):
        ...
        if import_results.get('get_agent_state_tracker') == 'NOT_FOUND_IN_MODULE':
            assert False, f'BUG REPRODUCED: get_agent_state_tracker is expected to be in netra_backend.app.agents.supervisor.agent_execution_core but is not found. Full import results: {import_results}'
        elif import_results.get('get_agent_state_tracker') == 'MODULE_IMPORT_ERROR':
            assert False, f'BUG REPRODUCED: Cannot import agent_execution_core module that should contain get_agent_state_tracker. Full import results: {import_results}'
