...
class AgentExecutionCoreImportValidationTests(SSotBaseTestCase):
    ...
    def test_get_agent_state_tracker_import_should_fail(self):
        ...
        with self.expect_exception((ImportError, AttributeError), 'get_agent_state_tracker'):
            ...
            try:
                ...
                self.fail('UNEXPECTED: get_agent_state_tracker was imported successfully. This suggests the import issue may have been fixed or the function exists but has other problems.')
    ...
    def test_available_imports_in_agent_execution_core(self):
        ...
        try:
            ...
            expected_functions = ['get_agent_state_tracker', 'AgentExecutionCore', 'get_execution_tracker', 'create_agent_execution_context']
            missing_functions = []
            for func_name in expected_functions:
                if not hasattr(core_module, func_name):
                    missing_functions.append(func_name)
            if missing_functions:
                self.fail(f'BUG REPRODUCED: agent_execution_core module is missing expected functions: {missing_functions}. Available attributes: {available_attributes}. This causes ImportError when other modules try to import these functions.')
    ...
    def test_agent_execution_core_class_instantiation(self):
        ...
        try:
            core = AgentExecutionCore(registry=mock_registry, websocket_bridge=mock_websocket_bridge)
            expected_methods = ['execute_agent', '_ensure_user_execution_context', '_execute_with_protection', '_setup_agent_websocket']
            missing_methods = []
            for method_name in expected_methods:
                if not hasattr(core, method_name):
                    missing_methods.append(method_name)
            if missing_methods:
                self.fail(f'BUG REPRODUCED: AgentExecutionCore is missing expected methods: {missing_methods}. This indicates incomplete implementation that could cause runtime errors.')
        except Exception as e:
    ...
    def test_execution_tracker_factory_function_missing(self):
        ...
        try:
            ...
            try:
                ...
                for method_name in expected_methods:
                    if not hasattr(state_tracker, method_name):
                        missing_methods.append(method_name)
                if missing_methods:
                    self.fail(f'BUG REPRODUCED: get_agent_state_tracker returns object missing methods: {missing_methods}')
            except Exception as e:
            ...
            alternatives_tried = []
            alternative_imports = [('netra_backend.app.core.agent_execution_tracker', 'get_execution_tracker'), ('netra_backend.app.core.agent_execution_tracker', 'AgentExecutionTracker'), ('netra_backend.app.agents.supervisor.agent_execution_core', 'AgentExecutionCore')]
            ...
            if not working_alternatives:
                self.fail(f'BUG REPRODUCED: get_agent_state_tracker is missing and no working alternatives found. Tried alternatives: {alternatives_tried}. This will cause ImportError in code that depends on agent state tracking.')
    ...
    def test_import_path_consistency_check(self):
        ...
        """
        import_expectations = {'get_execution_tracker': 'netra_backend.app.core.agent_execution_tracker', 'ExecutionState': 'netra_backend.app.core.agent_execution_tracker', 'AgentExecutionCore': 'netra_backend.app.agents.supervisor.agent_execution_core', 'get_agent_state_tracker': 'netra_backend.app.agents.supervisor.agent_execution_core'}
        import_results = {}
        ...
        if import_results.get('get_agent_state_tracker') == 'NOT_FOUND_IN_MODULE':
            self.fail(f'BUG REPRODUCED: get_agent_state_tracker is expected to be in netra_backend.app.agents.supervisor.agent_execution_core but is not found. Full import results: {import_results}')
        elif import_results.get('get_agent_state_tracker') == 'MODULE_IMPORT_ERROR':
            self.fail(f'BUG REPRODUCED: Cannot import agent_execution_core module that should contain get_agent_state_tracker. Full import results: {import_results}')
