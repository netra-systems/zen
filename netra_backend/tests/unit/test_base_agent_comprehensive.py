...
class MockBaseAgent(BaseAgent):
    ...
    async def _execute_with_user_context(self, context: UserExecutionContext, stream_updates: bool=False) -> Dict[str, Any]:
        ...
        self._test_execution_results.append({'user_id': context.user_id, 'thread_id': context.thread_id, 'run_id': context.run_id, 'request_id': context.request_id, 'stream_updates': stream_updates, 'timestamp': time.time()})
        result = {'status': 'success', 'message': 'Test agent execution completed', 'user_id': context.user_id, 'execution_id': len(self._test_execution_results)}
        return result
...
class LegacyTestAgent(BaseAgent):
    ...
    async def execute_core_logic(self, context) -> Dict[str, Any]:
        ...
        self._legacy_executions.append({'execution_time': time.time(), 'context_type': type(context).__name__})
