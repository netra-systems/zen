...
async def test_database_url_consistency_across_environments():
    ...
    NOTE: Currently the system uses a single DATABASE_URL for all environments.
    ...
    for env in environments:
        with patch.dict(os.environ, {"ENVIRONMENT": env}):
            ...
            assert db_url is not None, f"DATABASE_URL must be configured for {env}"
            assert db_url.startswith("postgresql"), f"DATABASE_URL must be postgresql URL, got: {db_url}"
