...
def mock_auth_client():
    ...
    with patch('netra_backend.app.clients.auth_client_core.auth_client') as mock:
        mock.get_oauth_config.return_value = {'client_id': 'test_client_id', 'redirect_uri': 'http://localhost:3000/auth/callback', 'scope': 'openid email profile'}
...
class TestOAuthCompleteFlow:
    ...
    async def test_complete_oauth_login_flow(self, test_client, mock_auth_client):
        ...
        callback_response = test_client.get('/auth/callback?code=test_auth_code&state=test_state', follow_redirects=False)
...
class TestTokenGenerationAndValidation:
    ...
    async def test_token_generation_and_validation(self, mock_security_service):
        ...
        access_token = await mock_security_service.create_access_token(user_id='test_user', email='test@example.com')
        refresh_token = await mock_security_service.create_refresh_token(user_id='test_user')
        assert access_token == 'jwt_access_token'
        assert refresh_token == 'jwt_refresh_token'
        mock_security_service.validate_token_jwt.return_value = {'valid': True, 'user_id': 'test_user', 'email': 'test@example.com', 'permissions': ['read', 'write']}
    ...
    async def test_token_expiration_handling(self, mock_security_service):
        ...
        mock_security_service.validate_token_jwt.return_value = {'valid': False, 'error': 'Token expired'}
...
class TestWebSocketAuthentication:
    ...
    async def test_websocket_authentication(self, mock_websocket_manager):
        ...
        """
        mock_websocket_manager.authenticate_connection.return_value = {'authenticated': True, 'user_id': 'test_user', 'permissions': ['read', 'write']}
        token = 'valid_jwt_token'
        auth_result = await mock_websocket_manager.authenticate_connection(token=token)
    ...
    async def test_websocket_invalid_token_rejection(self, mock_websocket_manager):
        ...
        mock_websocket_manager.authenticate_connection.return_value = {'authenticated': False, 'error': 'Invalid token'}
        invalid_token = 'invalid_jwt_token'
        auth_result = await mock_websocket_manager.authenticate_connection(token=invalid_token)
...
class TestTokenRefreshFlow:
    ...
    async def test_token_refresh_across_services(self, test_client, mock_security_service):
        ...
        """
        mock_security_service.validate_refresh_token = AsyncMock(return_value={'valid': True, 'user_id': 'test_user'})
        ...
        with patch('netra_backend.app.services.security_service.SecurityService', return_value=mock_security_service):
            refresh_result = {'access_token': await mock_security_service.create_access_token(user_id='test_user', email='test@example.com'), 'refresh_token': await mock_security_service.create_refresh_token(user_id='test_user'), 'token_type': 'Bearer', 'expires_in': 900}
...
class TestOAuthErrorScenarios:
    ...
    async def test_invalid_authorization_code(self, test_client):
        ...
        with patch('netra_backend.app.routes.auth_routes.callback_processor.handle_callback_request') as mock_callback:
            ...
            response = test_client.get('/auth/callback?code=invalid_code&state=test_state')
...
class TestCrossServiceTokenValidation:
    ...
    async def test_auth_service_token_validation(self, mock_auth_client):
        ...
        with patch('netra_backend.app.clients.auth_client_core.auth_client.validate_token') as mock_validate:
            mock_validate.return_value = {'valid': True, 'user_id': 'test_user', 'email': 'test@example.com', 'permissions': ['read', 'write']}
    ...
    async def test_backend_service_token_acceptance(self, test_client):
        ...
        with patch('netra_backend.app.auth_integration.auth.get_current_user') as mock_user:
            mock_user.return_value = {'id': 'test_user', 'email': 'test@example.com', 'authenticated': True}
            response = test_client.get('/auth/me', headers={'Authorization': 'Bearer valid_cross_service_token'})
...
class TestDevLoginFlow:
    ...
    async def test_dev_login_flow(self, test_client, mock_auth_client):
        ...
        dev_request = DevLoginRequest(email='dev@example.com', name='Development User')
        with patch('netra_backend.app.routes.auth_routes.dev_login.handle_dev_login') as mock_dev_login:
            mock_dev_login.return_value = {'access_token': 'dev_access_token', 'refresh_token': 'dev_refresh_token', 'user': {'id': 'dev_user_id', 'email': 'dev@example.com', 'name': 'Development User'}}
            response = test_client.post('/auth/dev_login', json=dev_request.model_dump())
            assert response.status_code == 200
            data = response.json()
...
class TestOAuthIntegrationFlow:
    ...
    async def test_end_to_end_oauth_integration(self, test_client):
        ...
        with patch('httpx.AsyncClient') as mock_client_class:
            ...
            mock_response.json.return_value = {'access_token': 'test_access_token', 'token_type': 'Bearer', 'user': {'id': 'test_user_123', 'email': 'test@example.com', 'name': 'Test User'}}
            mock_client.get.return_value = mock_response
            response = test_client.get('/auth/callback?code=test_code&state=test_state', follow_redirects=False)
            assert response.status_code == 302
            redirect_url = response.headers.get('location', '')
