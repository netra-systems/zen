...
class WebSocketStateManagementTests:
    ...
    async def test_session(self):
        ...
        return WebSocketTestSession(user_id=f'test_user_{uuid.uuid4()}', session_id=f'session_{uuid.uuid4()}', thread_id=f'thread_{uuid.uuid4()}', conversation_data={'messages': [{'id': 'msg_1', 'role': 'user', 'content': 'Start optimization task', 'timestamp': datetime.now(timezone.utc).isoformat()}, {'id': 'msg_2', 'role': 'assistant', 'content': 'Starting AI optimization...', 'timestamp': datetime.now(timezone.utc).isoformat()}], 'active_tools': ['data_analysis', 'model_optimization'], 'progress': 35}, agent_state={'active_agents': ['TriageSubAgent', 'DataSubAgent'], 'current_stage': 'data_analysis', 'progress_percentage': 35, 'execution_context': {'workflow_id': f'workflow_{uuid.uuid4()}', 'step_index': 2, 'pending_operations': ['validate_data', 'process_metrics']}, 'last_activity': datetime.now(timezone.utc).isoformat()})
    ...
    @pytest.mark.asyncio
    async def test_websocket_reconnection_flow(self, connection_manager, reconnection_handler, test_session, redis_cleanup):
        """
        ...
        conn_info = await self._establish_connection_with_state(connection_manager, test_session, mock_ws)
        await self._build_conversation_state(conn_info, test_session, mock_ws)
        reconnection_token = await self._simulate_disconnect_with_state_preservation(connection_manager, reconnection_handler, conn_info, test_session)
        new_mock_ws = MockWebSocket(test_session.user_id)
        restored_conn = await self._execute_reconnection_with_state_recovery(connection_manager, reconnection_handler, reconnection_token, new_mock_ws, test_session)
        await self._verify_state_continuity_and_message_resume(restored_conn, test_session, reconnection_handler, reconnection_token)
    ...
    async def _establish_connection_with_state(self, manager, session, mock_ws):
        ...
        conn_info = ConnectionInfo(connection_id=f'conn_{session.user_id}', user_id=session.user_id, connected_at=datetime.now(timezone.utc), last_activity=datetime.now(timezone.utc), message_count=0)
    ...
    async def _build_conversation_state(self, conn_info, session, mock_ws=None):
        ...
        for message in session.conversation_data['messages']:
            if websocket:
                await websocket.send(json.dumps({'type': 'conversation_message', 'thread_id': session.thread_id, 'message': message, 'timestamp': datetime.now(timezone.utc).isoformat()}))
        if redis_manager.enabled and redis_manager.redis_client:
            ...
            await redis_manager.set(conversation_key, json.dumps(session.conversation_data), ex=3600)
            await redis_manager.set(agent_state_key, json.dumps(session.agent_state), ex=3600)
        logger.info(f'Built conversation state: {conn_info.message_count} messages')

    async def _simulate_disconnect_with_state_preservation(self, manager, reconnection_handler, conn_info, session):
        """Simulate connection drop and prepare for reconnection."""
        reconnection_token = await reconnection_handler.prepare_for_reconnection(conn_info, session.agent_state)
    ...
    async def _execute_reconnection_with_state_recovery(self, manager, reconnection_handler, token, new_ws, session):
    ...
    async def _verify_state_continuity_and_message_resume(self, conn_info, session, reconnection_handler, token):
        ...
        continuation_message = {'type': 'workflow_continue', 'thread_id': session.thread_id, 'agent_state': preserved_state, 'timestamp': datetime.now(timezone.utc).isoformat()}
    ...
    @pytest.mark.asyncio
    async def test_state_preservation_on_disconnect(self, connection_manager, reconnection_handler, test_session, redis_cleanup):
        """
        ...
        reconnection_token = await reconnection_handler.prepare_for_reconnection(conn_info, test_session.agent_state)
        await mock_ws.close(code=1006, reason='Connection lost')
        await self._verify_state_persisted_to_redis(test_session)
        await self._verify_reconnection_context_preserved(reconnection_handler, reconnection_token, test_session)

    async def _simulate_active_agent_execution(self, conn_info, session):
        """Simulate active agent execution with state updates."""
        agent_updates = [{'type': 'agent_progress', 'agent_name': 'DataSubAgent', 'progress': 45, 'current_operation': 'analyzing_metrics', 'tools_active': ['database_analyzer', 'performance_monitor']}, {'type': 'agent_status', 'agent_name': 'TriageSubAgent', 'status': 'waiting', 'dependent_on': 'DataSubAgent'}]
    ...
    @pytest.mark.asyncio
    async def test_message_queue_persistence(self, connection_manager, reconnection_handler, test_session, redis_cleanup):
        """
        ...
        queued_messages = await self._simulate_disconnect_with_queued_messages(conn_info, test_session)
        reconnection_token = await reconnection_handler.prepare_for_reconnection(conn_info, test_session.agent_state)
        await self._persist_message_queue_to_redis(test_session, queued_messages)
        new_ws = MockWebSocket(test_session.user_id)
        restored_conn = await reconnection_handler.attempt_reconnection(reconnection_token, new_ws)
    ...
    async def _send_initial_message_batch(self, conn_info, session):
        ...
        initial_messages = [{'type': 'task_start', 'task_id': f'task_{uuid.uuid4()}', 'priority': 'high'}, {'type': 'data_request', 'tables': ['metrics', 'performance'], 'filters': {}}, {'type': 'progress_update', 'percentage': 25, 'status': 'processing'}]
    ...
    async def _simulate_disconnect_with_queued_messages(self, conn_info, session):
        ...
        queued_messages = [{'type': 'urgent_alert', 'message': 'Connection lost during critical operation', 'timestamp': datetime.now(timezone.utc).isoformat(), 'sequence': 1}, {'type': 'agent_state_backup', 'agent_states': session.agent_state, 'timestamp': datetime.now(timezone.utc).isoformat(), 'sequence': 2}, {'type': 'recovery_instruction', 'instruction': 'Resume from data_analysis stage', 'context': session.agent_state['execution_context'], 'timestamp': datetime.now(timezone.utc).isoformat(), 'sequence': 3}]
    ...
    async def _persist_message_queue_to_redis(self, session, messages):
        ...
        for message in messages:
            await redis_manager.add_to_list(queue_key, json.dumps(message), max_size=100)
        await redis_manager.redis_client.expire(queue_key, 3600)
    ...
    @pytest.mark.asyncio
    async def test_reconnection_with_auth(self, connection_manager, reconnection_handler, test_session, redis_cleanup):
        """
        ...
        mock_ws = MockWebSocket(test_session.user_id)
        conn_info = await self._establish_authenticated_connection(connection_manager, test_session, mock_ws)
    ...
    async def _establish_authenticated_connection(self, manager, session, mock_ws):
        ...
        conn_info = ConnectionInfo(connection_id=f'conn_auth_{session.user_id}', user_id=session.user_id, connected_at=datetime.now(timezone.utc), last_activity=datetime.now(timezone.utc), message_count=0)
        auth_message = {'type': 'auth', 'user_id': session.user_id, 'session_id': session.session_id, 'timestamp': datetime.now(timezone.utc).isoformat()}
    ...
    async def _store_auth_context(self, session, jwt_token):
        ...
        auth_context = {'jwt_token': jwt_token, 'user_id': session.user_id, 'session_id': session.session_id, 'created_at': datetime.now(timezone.utc).isoformat()}
    ...
    async def _simulate_auth_during_reconnection(self, mock_ws, jwt_token):
        ...
        auth_reconnect_message = {'type': 'auth_reconnect', 'jwt_token': jwt_token, 'timestamp': datetime.now(timezone.utc).isoformat()}
