...
class SupervisorOrchestrationTests(BaseTestCase):
    ...
    def setUp(self):
        ...
        self.test_context = UserExecutionContext(user_id='test-user-basic', thread_id='test-thread-basic', run_id='test-run-basic', agent_context={'user_request': 'basic test request'}).with_db_session(AsyncMock())
    ...
    async def test_basic_execution_flow(self):
        ...
        with patch('netra_backend.app.agents.supervisor_consolidated.managed_session', return_value=mock_session_manager), patch.object(self.supervisor, '_get_tool_classes_from_context', return_value=[]), patch('netra_backend.app.agents.supervisor_consolidated.UserContextToolFactory') as mock_factory, patch.object(self.supervisor, '_orchestrate_agents') as mock_orchestrate:
            ...
            mock_orchestrate.return_value = {'supervisor_result': 'completed', 'orchestration_successful': True, 'user_isolation_verified': True, 'results': {'reporting': {'status': 'completed'}}, 'user_id': self.test_context.user_id, 'run_id': self.test_context.run_id}
